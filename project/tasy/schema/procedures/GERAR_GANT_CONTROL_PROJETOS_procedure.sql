...
CREATE OR REPLACE PROCEDURE gerar_gant_control_projetos ( nr_seq_gerencia_p bigint, nr_seq_classificacao_p bigint, dt_analise_p timestamp, nm_usuario_p text) AS $body$
DECLARE
...
and	p.nr_seq_classif = coalesce(nr_seq_classificacao_p, p.nr_seq_classif)
and	p.nr_seq_estagio = nr_seq_estagio_w
and 	p.ie_posicao_externa = 'S'
and	((trunc(dt_alocacao_w,'dd') between p.dt_inicio_prev and p.dt_fim_prev) or (p.dt_inicio_prev between trunc(dt_alocacao_w,'dd') and p.dt_fim_prev)) /* casos onde a semana começa antes do início do projeto */
and	not exists (
		SELECT	1
...
BEGIN
if (nr_seq_gerencia_p IS NOT NULL AND nr_seq_gerencia_p::text <> '') and
	--(nr_seq_classificacao_p is not null) and
	(dt_analise_p IS NOT NULL AND dt_analise_p::text <> '') and (nm_usuario_p IS NOT NULL AND nm_usuario_p::text <> '') then
	begin
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_gant_control_projetos ( nr_seq_gerencia_p bigint, nr_seq_classificacao_p bigint, dt_analise_p timestamp, nm_usuario_p text) FROM PUBLIC;
