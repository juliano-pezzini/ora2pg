...
CREATE OR REPLACE PROCEDURE pfcs_calculate_box_time ( nr_seq_indicator_p bigint, cd_estabelecimento_p text, nm_usuario_p text) AS $body$
DECLARE


	c01 CURSOR FOR
	SELECT	x.nr_sequencia cd_group_pa, x.ds_grupo ds_group_pa, z.qt_time qt_time_rule
	from	pa_grupo_local x,
		pfcs_time_unit_pa z
	where	ie_situacao = 'A'
	and	x.nr_sequencia = z.cd_rule
	...
	where	y.nr_seq_grupo_pa = nr_seq_grupo_p
	and	y.nr_sequencia = a.nr_seq_local_pa
	and	y.ie_situacao = 'A'
			...
			begin
				...
				for c02_w in c02(c01_w.cd_group_pa) loop
					...
					begin
						...
						if (c01_w.qt_time_rule < c02_w.qt_time_box_pa) then
				...
				commit;
				 ...
				 := pfcs_pck.pfcs_generate_results(
					vl_indicator_p => qt_total_w, ds_reference_value_p => c01_w.ds_group_pa, cd_reference_value_p => c01_w.cd_group_pa, nr_seq_indicator_p => nr_seq_indicator_p, nr_seq_operational_level_p => nr_seq_operational_level_w, nm_usuario_p => nm_usuario_p, nr_seq_panel_p => nr_seq_panel_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pfcs_calculate_box_time ( nr_seq_indicator_p bigint, cd_estabelecimento_p text, nm_usuario_p text) FROM PUBLIC;
