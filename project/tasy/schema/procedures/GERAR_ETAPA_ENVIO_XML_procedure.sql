...
CREATE OR REPLACE PROCEDURE gerar_etapa_envio_xml ( nr_seq_protocolo_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR 
	...
	and (coalesce(ie_evento,'X') = coalesce(ie_evento_w,'X')) 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_etapa_envio_xml ( nr_seq_protocolo_p bigint, nm_usuario_p text) FROM PUBLIC;
