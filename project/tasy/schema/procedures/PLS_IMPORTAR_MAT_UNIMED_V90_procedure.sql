...
CREATE OR REPLACE PROCEDURE pls_importar_mat_unimed_v90 ( ds_conteudo_pai_p text, ie_tipo_p text, dt_vigencia_p timestamp, ds_versao_tiss_p text, nm_usuario_p text, cd_material_p INOUT pls_material_unimed.cd_material%type, nr_seq_mat_unimed_p INOUT pls_material_unimed.nr_sequencia%type, nm_material_p INOUT pls_material_unimed.nm_material%type, ie_tipo_pai_p INOUT text, qt_linha_filha_p INOUT bigint) AS $body$
DECLARE
...
id_alc_w			pls_mat_unimed_trib.id_alc%type;
...
-- Material
	...
	-- 7.0
	ds_unidade_medida_w	:= trim(both substr(ds_conteudo_pai_p, 406, 3));
...
-- Medicamento
	...
	cd_material_p			:= trim(both substr(ds_conteudo_pai_p, 12, 8));
	cd_unidade_medida_w		:= substr(ds_conteudo_pai_p, 20, 10);
	...
	ie_produto_med_w	:= trim(both substr(ds_conteudo_pai_p, 187, 1));
	ie_confaz_w		:= trim(both substr(ds_conteudo_pai_p, 188, 1));

	-- 7.0
	ds_unidade_medida_w	:= somente_numero(trim(both substr(ds_conteudo_pai_p,184,3)));
...
-- Medicamento - PERCENTUAL
	...
	id_alc_w	:= trim(both substr(ds_conteudo_pai_p, 34, 1));
...
if (ie_possui_mat_w = 0) and (ie_mat_med_w = 'S') then
	...
	if (ie_tipo_p in ('902','905')) then
		...
		set	nm_usuario		= nm_usuario_p,
			...
			nm_fabricante		= coalesce(nm_fabricante_w,nm_fabricante), --prioriza o nome vindo no arquivo, caso não tenha a informação, então mantém.
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_importar_mat_unimed_v90 ( ds_conteudo_pai_p text, ie_tipo_p text, dt_vigencia_p timestamp, ds_versao_tiss_p text, nm_usuario_p text, cd_material_p INOUT pls_material_unimed.cd_material%type, nr_seq_mat_unimed_p INOUT pls_material_unimed.nr_sequencia%type, nm_material_p INOUT pls_material_unimed.nm_material%type, ie_tipo_pai_p INOUT text, qt_linha_filha_p INOUT bigint) FROM PUBLIC;
