...
CREATE OR REPLACE PROCEDURE pls_fechar_discussao ( nr_seq_lote_discussao_p bigint, ie_estorno_p text, nm_usuario_p text, cd_estabelecimento_p text) AS $body$
DECLARE

				
-- OPS - Faturamento (Receber)
...
nr_titulo_pag_ndr_w		bigint;
nr_titulo_rec_w			bigint;
nr_titulo_rec_ndr_w		bigint;
ie_status_w			varchar(3);
ie_status_contest_w		varchar(3);
...
ie_canc_tit_vinc_disc_w		pls_parametros.ie_canc_tit_vinc_disc%type;
qt_titulo_disc_w		integer;
...
select	coalesce(ie_canc_tit_vinc_disc,'S')	-- Cancelar titulos vinc na discussao ao desfazer fechamento
into STRICT	ie_canc_tit_vinc_disc_w
from	pls_parametros
...
-- Atualizar valores da discussao
open C01;
loop
fetch C01 into	
	nr_seq_discussao_w;
...
if (coalesce(ie_estorno_p,'N') = 'N') then
	...
	and	b.nr_sequencia		= nr_seq_lote_discussao_p
	...
	elsif (nr_seq_lote_discussao_p IS NOT NULL AND nr_seq_lote_discussao_p::text <> '') then
		select	max(a.nr_seq_lote_contest),
			max(a.nr_seq_lote_conta),
...
union
		...
		from	pls_lote_contestacao a,
			pls_fatura f
		where	f.nr_sequencia	= a.nr_seq_pls_fatura
		and	a.nr_sequencia	= nr_seq_lote_contest_w;
		...
		select	coalesce(max(nr_sequencia), nr_seq_camara_contest_w),
			coalesce(max(ie_tipo_arquivo), ie_tipo_arquivo_w)
		into STRICT	nr_seq_camara_contest_w,
			ie_tipo_arquivo_w
		from	ptu_camara_contestacao
		where	nr_seq_lote_contest 	= nr_seq_lote_contest_w
		and	nr_seq_lote_discussao	= nr_seq_lote_discussao_p;
		
		if (nr_seq_pls_fatura_w IS NOT NULL AND nr_seq_pls_fatura_w::text <> '') then
		
			qt_integral_w := 0;
			if (ie_tipo_arquivo_w in (9)) then
				-- Se existir ao menos um questionamento com tipo de acordo 10, entao a discussao devera ser concluida
				...
				and	c.nr_seq_lote_contest	= nr_seq_lote_contest_w
				and	q.ie_tipo_acordo	in ('10', '15'); --  Encerrado pelo administrador
			end if;
			
			if (qt_integral_w > 0) then
				ie_status_contest_w := 'C';
			else
				ie_status_contest_w := 'D';
			end if;
			
			if (coalesce(ie_tipo_arquivo_w::text, '') = '') then
				ie_status_contest_w := 'C';
				
			elsif (ie_tipo_arquivo_w in (1)) then
				ie_status_contest_w := 'E';
				
			elsif (ie_tipo_arquivo_w in (3,4)) then
				ie_status_contest_w := 'D';
				
			elsif (ie_tipo_arquivo_w in (5,6,7,8)) then -- Fechamento /Fechamento complementar
				ie_status_contest_w := 'C';
				
			end if;
			
			CALL pls_altera_status_contestacao(nr_seq_lote_contest_w, ie_status_contest_w, nm_usuario_p);
		...
		-- Gerar LOG
		pls_gerar_contest_log(	nr_seq_lote_contest_w, nr_seq_lote_discussao_p, null, null, null, null, 'FLD', 'N', nm_usuario_p);		
		
		if (nr_seq_pls_fatura_w IS NOT NULL AND nr_seq_pls_fatura_w::text <> '') then
			nr_seq_acao_w := pls_obter_acao_intercambio(	'5',  -- Fechamento de discussao
							'20',  -- Validar baixa no titulo da fatura
							null, nr_seq_pls_fatura_w, nr_seq_lote_contest_w, null, clock_timestamp(), 'A550', ie_estorno_p, nr_seq_acao_w);
							
			if (nr_seq_acao_w IS NOT NULL AND nr_seq_acao_w::text <> '') and (nr_titulo_rec_w IS NOT NULL AND nr_titulo_rec_w::text <> '') then
				select	count(1)
			...
			if (nr_seq_acao_w IS NOT NULL AND nr_seq_acao_w::text <> '') then
				-- Baixa no titulo receber
				CALL pls_baixar_glosas_discuss_rec(nr_seq_lote_discussao_p,nr_seq_acao_w,'N',nm_usuario_p,cd_estabelecimento_p);
				
				-- Baixa no titulo pagar
				CALL pls_baixar_glosas_discuss_pag(nr_seq_lote_discussao_p,nr_seq_acao_w,'N',nm_usuario_p,cd_estabelecimento_p);
			end if;
			...
			if (nr_seq_acao_w IS NOT NULL AND nr_seq_acao_w::text <> '') then
				...
				from	titulo_pagar a
				where	a.nr_seq_pls_lote_disc = nr_seq_lote_discussao_p;
				
				CALL pls_alterar_status_fatura_pag(nr_titulo_w,nm_usuario_p);
			end if;
			
			select	count(1)
			into STRICT	qt_titulo_disc_w
			from (	SELECT	1
				from	pls_lote_discussao
				where	nr_sequencia = nr_seq_lote_discussao_p
				and	((nr_titulo_pagar IS NOT NULL AND nr_titulo_pagar::text <> '') or (nr_titulo_pagar_ndr IS NOT NULL AND nr_titulo_pagar_ndr::text <> ''))
...
union all
				...
				and	((a.nr_titulo_receber IS NOT NULL AND a.nr_titulo_receber::text <> '') or (a.nr_titulo_receber_ndr IS NOT NULL AND a.nr_titulo_receber_ndr::text <> ''))) alias7;
				
			-- Se nao ha titulos vinculados ao lote de discussao verifica as acoes e gera o titulo conforme parametrizacao
			if (qt_titulo_disc_w = 0) then
				nr_seq_acao_w := pls_obter_acao_intercambio(	'5',  -- Fechamento de discussao
				...
				if (nr_seq_acao_w IS NOT NULL AND nr_seq_acao_w::text <> '') and (ie_tipo_arquivo_disc_w not in (9)) then
					...
					CALL pls_gerar_tit_receber_discuss(nr_seq_lote_discussao_p,nr_seq_acao_w,clock_timestamp(),nm_usuario_p);
				end if;
		...
		if (ie_tipo_arquivo_disc_w not in (9)) then
			-- Gerar A560
			CALL pls_gerar_nota_debito_a560( nr_seq_lote_discussao_p, nr_seq_camara_contest_w, cd_estabelecimento_p, nm_usuario_p);
		end if;
		
		qt_vago_w := ctb_pls_atualizar_discussao(	nr_seq_lote_discussao_p, null, nm_usuario_p, cd_estabelecimento_p, qt_vago_w);
		...
		-- Recalcular a classificacao do titulo
		if (nr_titulo_receber_w IS NOT NULL AND nr_titulo_receber_w::text <> '') then
			...
			CALL pls_gerar_tit_rec_class_disc( nr_titulo_receber_w, nm_usuario_p, 'N');
		end if;
						
		-- Gerar eventos financeiros
		CALL pls_gerar_evento_contestacao( nr_seq_lote_discussao_p, 'N', cd_estabelecimento_p, nm_usuario_p);
	end if;

	if (ie_concil_contab_w = 'S') then
		CALL pls_ctb_onl_gravar_movto_pck.gravar_movto_fechar_disc(nr_seq_lote_discussao_p, cd_estabelecimento_p, nm_usuario_p);
	end if;
...
if (coalesce(ie_estorno_p,'N') = 'S') then
	...
	if (ie_concil_contab_w = 'S') then
		CALL pls_ctb_onl_gravar_movto_pck.gravar_movto_desf_fech_disc(nr_seq_lote_discussao_p, cd_estabelecimento_p, nm_usuario_p);
	end if;
	...
	select	CASE WHEN coalesce(nr_seq_lote_conta::text, '') = '' THEN 'A'  ELSE 'AN' END
	...
	-- Gerar LOG
	pls_gerar_contest_log(	nr_seq_lote_contest_w, nr_seq_lote_discussao_p, null, null, null, null, 'DFLD', 'N', nm_usuario_p);
	...
	from	pls_fatura a
	where	a.nr_sequencia	= nr_seq_pls_fatura_w;
	
	if (nr_titulo_pag_w IS NOT NULL AND nr_titulo_pag_w::text <> '') or (nr_titulo_rec_w IS NOT NULL AND nr_titulo_rec_w::text <> '') then
		select	max(a.nr_seq_pls_fatura)
		into STRICT	nr_seq_pls_fatura_w
		from	pls_lote_contestacao a
		where	a.nr_sequencia = nr_seq_lote_contest_w;
		...
		if (nr_titulo_rec_w IS NOT NULL AND nr_titulo_rec_w::text <> '') and (nr_seq_acao_w IS NOT NULL AND nr_seq_acao_w::text <> '') and (ie_canc_tit_vinc_disc_w = 'S') then
			select	count(1)
			into STRICT	qt_registro_w
			from	titulo_receber_liq
			where	nr_titulo = nr_titulo_rec_w
			and	nr_seq_pls_lote_disc = nr_seq_lote_discussao_p;
		
			if (qt_registro_w > 0) then
				CALL pls_baixar_glosas_discuss_rec(nr_seq_lote_discussao_p,nr_seq_acao_w,'S',nm_usuario_p,cd_estabelecimento_p);
			end if;
		end if;
		
		if (nr_titulo_pag_w IS NOT NULL AND nr_titulo_pag_w::text <> '') and (nr_seq_acao_w IS NOT NULL AND nr_seq_acao_w::text <> '') and (ie_canc_tit_vinc_disc_w = 'S') then
			CALL pls_baixar_glosas_discuss_pag(nr_seq_lote_discussao_p,nr_seq_acao_w,'S',nm_usuario_p,cd_estabelecimento_p);
		end if;
	end if;	
	
	-- Cancelar os titulos que foram gerados
	if (nr_titulo_pag_w IS NOT NULL AND nr_titulo_pag_w::text <> '') and (ie_canc_tit_vinc_disc_w = 'S') then
	...
	if (nr_titulo_pag_ndr_w IS NOT NULL AND nr_titulo_pag_ndr_w::text <> '') and (ie_canc_tit_vinc_disc_w = 'S') then
		CALL cancelar_titulo_pagar(nr_titulo_pag_ndr_w,nm_usuario_p,clock_timestamp());
	...
	select	max(a.nr_titulo_receber),
		max(a.nr_titulo_receber_ndr)
	into STRICT	nr_titulo_rec_w,
		nr_titulo_rec_ndr_w
	from	titulo_receber		b,
		pls_lote_discussao	a
	...
	if (nr_titulo_rec_w IS NOT NULL AND nr_titulo_rec_w::text <> '') and (ie_canc_tit_vinc_disc_w = 'S') then
	...
	if (nr_titulo_rec_ndr_w IS NOT NULL AND nr_titulo_rec_ndr_w::text <> '') and (ie_canc_tit_vinc_disc_w = 'S') then
		CALL cancelar_titulo_receber(nr_titulo_rec_ndr_w,nm_usuario_p,'N',clock_timestamp());
		...
		where	nr_sequencia	=	(SELECT	max(a.nr_seq_nota_debito)
						from	ptu_nota_deb_conclusao 	a,
							titulo_receber 		b
						where	a.nr_sequencia		= b.nr_seq_nota_deb_conclusao
						and	b.nr_titulo		= nr_titulo_rec_ndr_w);
	end if;
	...
	and	a.nr_seq_lote_disc	= nr_seq_lote_discussao_p
	...
	if (nr_seq_lote_conta_w IS NOT NULL AND nr_seq_lote_conta_w::text <> '') and (nr_seq_acao_w IS NOT NULL AND nr_seq_acao_w::text <> '') then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_fechar_discussao ( nr_seq_lote_discussao_p bigint, ie_estorno_p text, nm_usuario_p text, cd_estabelecimento_p text) FROM PUBLIC;
