...
CREATE OR REPLACE PROCEDURE pls_mens_gerar_mensagem ( nr_seq_mensalidade_p pls_mensalidade.nr_sequencia%type, nr_seq_pagador_p pls_mensalidade.nr_seq_pagador%type, dt_referencia_p pls_mensalidade.dt_referencia%type, ie_tipo_estipulante_p pls_mensalidade.ie_tipo_estipulante%type, nr_seq_conta_banco_p pls_mensalidade.nr_seq_conta_banco%type, nr_seq_classif_itens_p pls_contrato_pagador.nr_seq_classif_itens%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
C01 CURSOR FOR 
	...
	where (dt_referencia_p between trunc(coalesce(a.dt_inicial,dt_referencia_p),'dd') and fim_dia(coalesce(a.dt_final,dt_referencia_p))) 
	and	((a.ie_aniversario		= 'S' and ie_aniversariante_w = 'S') or (a.ie_aniversario = 'N')) 
	and	a.cd_estabelecimento		= cd_estabelecimento_p 
	and	((nr_seq_conta_banco_p = a.nr_seq_conta_banco and (nr_seq_conta_banco_p IS NOT NULL AND nr_seq_conta_banco_p::text <> '') and (a.nr_seq_conta_banco IS NOT NULL AND a.nr_seq_conta_banco::text <> '')) or (coalesce(a.nr_seq_conta_banco::text, '') = '')) 
	and	((nr_seq_classif_itens_p = a.nr_seq_classif_itens and (nr_seq_classif_itens_p IS NOT NULL AND nr_seq_classif_itens_p::text <> '') and (a.nr_seq_classif_itens IS NOT NULL AND a.nr_seq_classif_itens::text <> '')) or (coalesce(nr_seq_classif_itens::text, '') = '')) 
	and	coalesce(a.ie_regulamentacao,ie_regulamentacao_w)	= ie_regulamentacao_w 
	and	((coalesce(a.ie_tipo_estipulante,'A')	= coalesce(ie_tipo_estipulante_p,coalesce(a.ie_tipo_estipulante,'A'))) or 
	...
	and (exists (SELECT x.nr_ano 
			...
			and 	x.nr_seq_pagador	= nr_seq_pagador_p 
...
for r_c02_w in C02 loop 
	if (coalesce(r_c02_w.dt_nascimento,clock_timestamp()) <> clock_timestamp()) then 
		if	to_char(r_c02_w.dt_nascimento,'mm') = to_char(dt_referencia_p,'mm') then 
...
for r_c01_w in C01 loop 
	...
	if (r_c01_w.qt_meses_portabilidade IS NOT NULL AND r_c01_w.qt_meses_portabilidade::text <> '') then 
		if (qt_registro_w > 0) then 
			...
			SELECT * FROM pls_obter_se_seg_portabilidade(nr_seq_mensalidade_p, dt_referencia_p, cd_estabelecimento_p, r_c01_w.qt_meses_portabilidade, r_c01_w.nr_seq_tipo_portabilidade, ds_segurados_w, ds_valores_pre_w) INTO STRICT ds_segurados_w, ds_valores_pre_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_mens_gerar_mensagem ( nr_seq_mensalidade_p pls_mensalidade.nr_sequencia%type, nr_seq_pagador_p pls_mensalidade.nr_seq_pagador%type, dt_referencia_p pls_mensalidade.dt_referencia%type, ie_tipo_estipulante_p pls_mensalidade.ie_tipo_estipulante%type, nr_seq_conta_banco_p pls_mensalidade.nr_seq_conta_banco%type, nr_seq_classif_itens_p pls_contrato_pagador.nr_seq_classif_itens%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
