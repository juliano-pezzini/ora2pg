...
CREATE OR REPLACE PROCEDURE pat_importar_w_imp_html5 ( ds_conteudo_p text, qt_bem_p INOUT bigint, ie_opcao_p text, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (ie_opcao_p = 'G') then
	...
	while(qt_registro_w <= 37) loop
		...
		if (qt_registro_w = 0) then
			...
			if (ds_valor_w IS NOT NULL AND ds_valor_w::text <> '') then
				tx_deprec_w		:= substr(ds_valor_w,1,11) || '.' || substr(ds_valor_w,12,4);
			end if;
		elsif (qt_registro_w = 13) then
			nm_coluna_w		:= 'TX_FISCAL';

			if (ds_valor_w IS NOT NULL AND ds_valor_w::text <> '') then
				tx_fiscal_w		:= substr(ds_valor_w,1,11) || '.' || substr(ds_valor_w,12,4);
			end if;
		...
		/*elsif	(qt_registro_w = 16) then
			...
			if (ds_valor_w IS NOT NULL AND ds_valor_w::text <> '') then
				vl_original_w		:= substr(ds_valor_w,1,13) || '.' || substr(ds_valor_w,14,2);
			end if;
			...
			dt_garantia_w		:= to_date(substr(ds_valor_w,1,10));
			...
			if (ds_valor_w IS NOT NULL AND ds_valor_w::text <> '') then
				vl_residual_w		:= substr(ds_valor_w,1,13) || '.' || substr(ds_valor_w,14,2);
			end if;
			...
			if (ds_valor_w IS NOT NULL AND ds_valor_w::text <> '') then
				vl_mercado_w		:= substr(ds_valor_w,1,13) || '.' || substr(ds_valor_w,14,2);
			end if;
			...
			if (ds_valor_w IS NOT NULL AND ds_valor_w::text <> '') then
				vl_base_deprec_w	:= substr(ds_valor_w,1,18) || '.' || substr(ds_valor_w,19,4);
			end if;
		elsif (qt_registro_w = 35) then
			nm_coluna_w		:= 'VL_CONTABIL';

			if (ds_valor_w IS NOT NULL AND ds_valor_w::text <> '') then
				vl_contabil_w		:= substr(ds_valor_w,1,18) || '.' || substr(ds_valor_w,19,4);
			end if;
		elsif (qt_registro_w = 36) then
			nm_coluna_w		:= 'VL_DEPREC_ACUM';

			if (ds_valor_w IS NOT NULL AND ds_valor_w::text <> '') then
				vl_deprec_acum_w	:= substr(ds_valor_w,1,18) || '.' || substr(ds_valor_w,19,4);
			end if;
		elsif (qt_registro_w = 37) then
			nm_coluna_w		:= 'VL_DEPREC_MES';

			if (ds_valor_w IS NOT NULL AND ds_valor_w::text <> '') then
				vl_deprec_mes_w		:= substr(ds_valor_w,1,18) || '.' || substr(ds_valor_w,19,4);
			end if;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pat_importar_w_imp_html5 ( ds_conteudo_p text, qt_bem_p INOUT bigint, ie_opcao_p text, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
