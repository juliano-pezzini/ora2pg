...
CREATE OR REPLACE PROCEDURE gerar_movto_caixa_cartao (nm_usuario_p text, nr_seq_parcela_p text) AS $body$
DECLARE
...
and coalesce(dt_fechamento::text, '') = '';
	...
	WHERE d.nr_sequencia = nr_seq_parcela_p;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_movto_caixa_cartao (nm_usuario_p text, nr_seq_parcela_p text) FROM PUBLIC;
