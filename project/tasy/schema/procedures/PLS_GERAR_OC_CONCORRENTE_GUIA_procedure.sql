...
CREATE OR REPLACE PROCEDURE pls_gerar_oc_concorrente_guia ( nr_seq_regra_p bigint, cd_procedimento_p bigint, ie_origem_proced_p bigint, nr_seq_guia_p bigint, ie_concorrente_anterior_p text, cd_estabelecimento_p bigint, nm_usuario_p text, ie_gerar_ocorrencia_p INOUT text) AS $body$
DECLARE

				
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: Obter se gera ocorrencia conforme a regra de conconrrente
-------------------------------------------------------------------------------------------------------------------
...
C02 CURSOR FOR
	...
	and	a.ie_status		= 'S'
	and	ESTABLISHMENT_TIMEZONE_UTILS.startOfDay(b.dt_solicitacao)	= ESTABLISHMENT_TIMEZONE_UTILS.startOfDay(dt_solicitacao_w);
...
if (qt_procedimentos_principal_w > 0) then
	...
	/*Se haver ao menor um procedimento principal com um nao principal e gerado ocorrencia*/
		
	if (qt_procedimentos_w > 0) then
		ie_gera_ocorrencia_w := 'S';		
		
		/*Se for uma verificacao de itens e verificado se deve gerar ocorrencia naquele item*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_oc_concorrente_guia ( nr_seq_regra_p bigint, cd_procedimento_p bigint, ie_origem_proced_p bigint, nr_seq_guia_p bigint, ie_concorrente_anterior_p text, cd_estabelecimento_p bigint, nm_usuario_p text, ie_gerar_ocorrencia_p INOUT text) FROM PUBLIC;
