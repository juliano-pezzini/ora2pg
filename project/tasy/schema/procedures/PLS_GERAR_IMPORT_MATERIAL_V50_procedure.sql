...
[ ]  Objetos do dicionário [ ] Tasy (Delphi/Java) [  ] Portal [  ]  Relatórios [ ] Outros:
 ------------------------------------------------------------------------------------------------------------------
Pontos de atenção:		FEITO POR LOTE - 5.0 ou menos
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
...
-- Material
	...
	nm_importador_w			:= trim(both substr(ds_linha_p,94,50));
	ie_origem_w			:= trim(both substr(ds_linha_p,144,1));
	nr_registro_anvisa_w		:= trim(both substr(ds_linha_p,145,20));
	ds_motivo_ativo_inativo_w	:= trim(both substr(ds_linha_p,174,40));
	dt_validade_anvisa_w		:= to_date(trim(both substr(ds_linha_p,165,8)),'yyyy/mm/dd');
	...
	ie_tipo_ww			:= '902';
	ie_mat_med_w			:= 'S';
	cd_ref_material_fab_w		:= trim(both substr(ds_linha_p,252,30));
	...
	select	CASE WHEN substr(ds_linha_p,173,1)='1' THEN 'A'  ELSE 'I' END
...
-- Medicamento
	...
	cd_material_w			:= trim(both substr(ds_linha_p,12,8));
	cd_unidade_medida_w		:= trim(both substr(ds_linha_p,20,10));
	nr_registro_anvisa_w		:= trim(both substr(ds_linha_p,30,20));
	cd_cnpj_w			:= trim(both substr(ds_linha_p,58,14));
	ds_motivo_ativo_inativo_w	:= trim(both substr(ds_linha_p,73,40));
	vl_fator_conversao_w		:= trim(both substr(ds_linha_p,114,5));
	ie_generico_w			:= trim(both substr(ds_linha_p,119,1));

	begin
	dt_inicio_obrigatorio_w		:= to_date(trim(both substr(ds_linha_p,120,8)),'yyyy/mm/dd');
	exception
	when others then
		dt_inicio_obrigatorio_w := null;
	end;
	...
	begin
	dt_validade_anvisa_w		:= to_date(trim(both substr(ds_linha_p,50,8)),'yyyy/mm/dd');
	exception
	when others then
		dt_validade_anvisa_w := null;
	end;

	ie_origem_w			:= trim(both substr(ds_linha_p,113,1));
	ie_tipo_ww			:= '905';
	ie_mat_med_w			:= 'S';
	...
	select	CASE WHEN substr(ds_linha_p,72,1)='1' THEN 'A'  ELSE 'I' END
	into STRICT	ie_situacao_w
	;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_import_material_v50 ( ds_linha_p text, ie_tipo_p text, nr_seq_lote_imp_p bigint, nm_arquivo_p text, nm_usuario_p text, nr_seq_mat_unimed_p INOUT w_pls_material_unimed.nr_sequencia%type, nm_material_p INOUT w_pls_material_unimed.nm_material%type, ie_tipo_pai_p INOUT text, qt_linha_filha_p INOUT bigint, ds_erro_p INOUT text) FROM PUBLIC;
