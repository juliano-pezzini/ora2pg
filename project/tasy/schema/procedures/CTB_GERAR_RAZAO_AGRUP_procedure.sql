...
CREATE OR REPLACE PROCEDURE ctb_gerar_razao_agrup ( cd_empresa_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estab_p text, cd_conta_contabil_p text, cd_classificacao_p text, cd_centro_custo_p text, nm_usuario_p text, ie_valor_p text, cd_classif_inic_p text, cd_classif_final_p text, ie_centro_custo_p text, ie_ordenacao_p text, ie_contra_partida_p text) AS $body$
DECLARE
...
c001 CURSOR FOR
	SELECT	distinct s.cd_conta_contabil,
			c.cd_conta_contabil  || ' - ' || coalesce(s.cd_classificacao, c.cd_classificacao) || '  ' || c.ds_conta_contabil,
...
c002 CURSOR FOR
	SELECT	'M',   /*1*/
			a.dt_movimento, /*2*/
			substr(a.ds_historico,1,255) ds_historico, /*3*/
			a.ie_debito_credito, /*4*/
			substr(a.ds_compl_historico,1,255) ds_compl_historico, /*5*/
			a.cd_contrapartida,  /*6*/
			a.cd_historico,  /*7*/
			a.vl_credito,   /*8*/
			a.vl_debito,    /*9*/
			a.vl_movimento,  /*10*/
			a.nr_lote_contabil,   /*11*/
			a.nr_sequencia,  /*12*/
			a.cd_estabelecimento,  /*13*/
			a.nr_agrup_sequencial   /*14*/
	from	ctb_mes_ref b,
			ctb_movimento_v a
...
EXIT WHEN NOT FOUND; /* apply on c001 */
	...
	if (ie_gerar_w	= 'S')  then
		...
		fetch c002 into
			...
			ds_compl_historico_w,  /* 5*/
			cd_contrapartida_w,   /* 6*/
			cd_historico_w,       /* 7*/
			vl_credito_w, 		  /* 8*/
			vl_debito_w,		/* 9*/
			vl_movimento_w,		/* 10*/
			nr_lote_contabil_w,	/* 11*/
			nr_seq_movto_w,		/* 12*/
			cd_estabelecimento_w,   /* 13*/
			nr_lancamento_w;		/* 14*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ctb_gerar_razao_agrup ( cd_empresa_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estab_p text, cd_conta_contabil_p text, cd_classificacao_p text, cd_centro_custo_p text, nm_usuario_p text, ie_valor_p text, cd_classif_inic_p text, cd_classif_final_p text, ie_centro_custo_p text, ie_ordenacao_p text, ie_contra_partida_p text) FROM PUBLIC;
