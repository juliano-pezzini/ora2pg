...
CREATE OR REPLACE PROCEDURE gerar_comunic_solic_transf ( nr_ordem_compra_p bigint, nr_item_oci_p bigint, cd_evento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
/* 
30 - Controle transferência de estoques - Liberação aprovação 
31 - Controle transferência de estoques - Estorno da Liberação 
32 - Controle transferência de estoques - Cancelamento de um item (Baixa Incorreta ou cancelamento de Entrega) 
33 - Controle transferência de estoques - Atendimento da solicitação - geração NF Saída 
34 - Controle transferência de estoques - Atendimento da solicitação - geração NF Entrada 
*/
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_seq_regra_w > 0) then 
		...
		ds_destinatario_w := obter_usuarios_comunic_compras(nr_ordem_compra_p,null,cd_evento_w,nr_seq_regra_w,'');
 
		select	substr(max(obter_dados_regra_com_compra(nr_ordem_compra_p, nr_item_oci_p, 146, cd_evento_w, nr_seq_regra_w, 'M')),1,2000) ds_comunicacao, 
			substr(max(obter_dados_regra_com_compra(nr_ordem_compra_p, nr_item_oci_p, 146, cd_evento_w, nr_seq_regra_w,'T')),1,80) ds_titulo 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_comunic_solic_transf ( nr_ordem_compra_p bigint, nr_item_oci_p bigint, cd_evento_p bigint, nm_usuario_p text) FROM PUBLIC;
