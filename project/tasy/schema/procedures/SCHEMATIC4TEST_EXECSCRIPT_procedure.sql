...
CREATE OR REPLACE PROCEDURE schematic4test_execscript (NR_SEQUENCIA_P bigint, IE_START_P bigint, NM_USUARIO_P text) AS $body$
DECLARE
...
BEGIN
  --procedure that execute or create script
  INSERT INTO SCHEM_TEST_BEHOLDER(nr_sequencia, nm_usuario, dt_atualizacao, nm_usuario_nrec, dt_atualizacao_nrec, ds_interaction, ds_param_a, ds_param_b)
	VALUES (nextval('schem_test_beholder_seq'), NM_USUARIO_P, clock_timestamp(), 'Robot', clock_timestamp(), 'SCHEMATIC4TEST_EXECSCRIPT', 
  ...
  IF (IE_START_P = 1) THEN
    NM_SUITE_W := 'Creating - '||NM_SCRIPT_W;
    IE_JOBS_W := '4';
    IE_START_W := '1';
  ELSE
  IF (IE_START_P = 2) THEN  
      NM_SUITE_W := 'Test - '||NM_SCRIPT_W;
      IE_JOBS_W := '0';
      IE_START_W := '2';
    END IF;
  END IF;
...
;
  ...
  INSERT INTO SCHEM_TEST_SUITE(nr_sequencia, nm_suite, ds_version, ie_runtype, nm_owner, dt_last, dt_atualizacao, nm_usuario, dt_atualizacao_nrec, nr_seq_preset, ds_repo, ie_jobs, ie_test_local, nm_usuario_nrec, ie_situacao) 
      VALUES (NEWSEQUENCESUITE, NM_SUITE_W2, '0', 2, NM_USUARIO_P, clock_timestamp(), clock_timestamp(), NM_USUARIO_P, clock_timestamp(), NR_SEQ_PRESET_W, '0', IE_JOBS_W, '1', NM_USUARIO_P, 'I');
  COMMIT;
...
;
  ...
  INSERT INTO SCHEM_TEST_SCHEDULE(nr_sequencia, dt_schedule, ie_status, nm_owner, nr_seq_suite, dt_atualizacao, nm_usuario, dt_atualizacao_nrec, nm_usuario_nrec, nr_seq_url, nr_seq_service, nr_seq_browser, ie_jobs)
      VALUES (NEWSEQUENCESCHEDULE, clock_timestamp(), IE_STATUS_W, NM_USUARIO_P, NEWSEQUENCESUITE, clock_timestamp(), NM_USUARIO_P, clock_timestamp(), NM_USUARIO_P, NR_SEQ_URL_W, NR_SEQ_SERVICE_W, NR_SEQ_BROWSER_W, IE_JOBS_W);
  COMMIT;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE schematic4test_execscript (NR_SEQUENCIA_P bigint, IE_START_P bigint, NM_USUARIO_P text) FROM PUBLIC;
