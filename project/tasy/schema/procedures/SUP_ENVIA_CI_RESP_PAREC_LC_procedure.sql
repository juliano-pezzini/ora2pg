...
CREATE OR REPLACE PROCEDURE sup_envia_ci_resp_parec_lc ( nr_seq_parecer_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
and	substr(obter_se_envia_ci_regra_compra(b.nr_sequencia,nr_seq_parecer_p,'PC',obter_perfil_ativo,nm_usuario_p,null),1,1) = 'S';
...
from	segmento_compras_rel_it a, 
	segmento_compras_rel_parec b 
where	b.nr_seq_item_rel = a.nr_sequencia 
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	select	substr(obter_dados_regra_com_compra(nr_seq_parecer_p, nr_seq_item_rel_w, 981, 67, nr_seq_regra_w,'T'),1,80), 
		substr(obter_dados_regra_com_compra(nr_seq_parecer_p, nr_seq_item_rel_w, 981, 67, nr_seq_regra_w,'M'),1,2000) 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE sup_envia_ci_resp_parec_lc ( nr_seq_parecer_p bigint, nm_usuario_p text) FROM PUBLIC;
