...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_c114_icmsipi ( nr_seq_controle_p bigint, nr_seq_nota_p bigint, nr_linha_p INOUT bigint) AS $body$
DECLARE
...
type reg_c_reg_C114 is table of fis_efd_icmsipi_C114%RowType;
...
fetch c_reg_C114 bulk collect into vetRegC114 limit 1000;
	for i in 1 .. vetRegC114.Count loop
		...
		ds_linha_w := substr(	ds_sep_w || vetRegC114[i].cd_reg ||
					ds_sep_w || vetRegC114[i].cd_mod ||
					ds_sep_w || vetRegC114[i].nr_ecf_fab ||
					ds_sep_w || vetRegC114[i].nr_ecf_cx ||
					ds_sep_w || vetRegC114[i].nr_doc ||
					ds_sep_w || to_char(vetRegC114[i].dt_doc,'ddmmyyyy') || ds_sep_w, 1, 8000);
		...
		fis_registros_w[qt_cursor_w].cd_registro 		:=  vetRegC114[i].cd_reg;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_c114_icmsipi ( nr_seq_controle_p bigint, nr_seq_nota_p bigint, nr_linha_p INOUT bigint) FROM PUBLIC;
