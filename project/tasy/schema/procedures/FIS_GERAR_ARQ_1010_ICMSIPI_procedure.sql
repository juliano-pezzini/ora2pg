...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_1010_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
c_reg_1010 CURSOR FOR
SELECT	*
from	fis_efd_icmsipi_1010
where	nr_seq_controle = nr_seq_controle_p;

/*Criação do array com o tipo sendo do cursor eespecificado - c_reg_1010*/
...
fetch c_reg_1010 bulk collect into vetReg1010 limit 1000;
	...
	if (vetReg1010[i](.ie_ind_giaf1 IS NOT NULL AND .ie_ind_giaf1::text <> '')) then
	...
	if (vetReg1010[i](.ie_rest_ressarc_compl_icms IS NOT NULL AND .ie_rest_ressarc_compl_icms::text <> '')) then
		ds_linha_w := 	ds_linha_w ||
...
EXIT WHEN NOT FOUND; /* apply on c_reg_1010 */
end loop;
close c_reg_1010;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_1010_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) FROM PUBLIC;
