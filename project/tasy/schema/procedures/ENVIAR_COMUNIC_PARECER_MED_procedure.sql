...
CREATE OR REPLACE PROCEDURE enviar_comunic_parecer_med ( nr_parecer_p bigint, ie_tipo_p bigint, cd_estabelecimento_p bigint, nr_seq_parecer_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (nr_parecer_p IS NOT NULL AND nr_parecer_p::text <> '') then
	...
	if (ie_tipo_p = 1) then
		...
		if (ds_parecer_w IS NOT NULL AND ds_parecer_w::text <> '') then
			...
			if (ie_mostra_pedido_w = 'S') then
				ds_parecer_ww := ds_parecer_ww || wheb_mensagem_pck.get_texto(1013116) || ': \par \par '|| substr(ds_parecer_w, position(ds_pos_inicio_rtf_w in ds_pos_inicio_rtf_w)+1,length(ds_parecer_w));
			else
		...
		if (nm_usuario_cad_w IS NOT NULL AND nm_usuario_cad_w::text <> '') then
			if (ds_usuario_espec_w = 'S') then
				...
				if (qt_reg_w > 0) then
					nm_destino_w := substr(nm_usuario_cad_w|| ',',1,4000);	
				end if;			
		...
		EXIT WHEN NOT FOUND; /* apply on C01 */
			...
			if (obter_se_estabelecido_usuario(nm_usuario_w, cd_estabelecimento_p ) = 'S') then
				...
				if (ds_usuario_espec_w = 'S') and (obter_se_usuario_medico(nm_usuario_w) = 'S') then
					...
					if (qt_reg_w > 0) then
						nm_destino_w := substr(nm_destino_w||nm_usuario_w|| ',',1,4000);	
					end if;	
	...
	if (ie_tipo_p = 2) then
		...
		if (ds_parecer_w IS NOT NULL AND ds_parecer_w::text <> '') then
			...
			ds_parecer_ww := ds_parecer_ww || wheb_mensagem_pck.get_texto(1013115) || ' \par \par '|| substr(ds_parecer_w,position(ds_pos_inicio_rtf_w in ds_pos_inicio_rtf_w)+1,length(ds_parecer_w));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE enviar_comunic_parecer_med ( nr_parecer_p bigint, ie_tipo_p bigint, cd_estabelecimento_p bigint, nr_seq_parecer_p bigint, nm_usuario_p text) FROM PUBLIC;
