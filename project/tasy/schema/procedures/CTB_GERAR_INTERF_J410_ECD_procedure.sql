...
CREATE OR REPLACE PROCEDURE ctb_gerar_interf_j410_ecd ( nr_seq_controle_p bigint, nm_usuario_p text, dt_inicio_p timestamp, dt_fim_p timestamp, qt_linha_p INOUT bigint, nr_sequencia_p INOUT bigint) AS $body$
DECLARE
...
sep_w						varchar(1) := '|';
tp_registro_w				varchar(15) := 'J410';
nr_seq_demo_dva_w			bigint;
...
and		a.nr_seq_demo		= nr_seq_demo_dva_w
...
select	coalesce(max(nr_seq_demo_dva),0)
into STRICT	nr_seq_demo_dva_w
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	ds_linha_w	:= substr(	sep_w || tp_registro_w								||
							...
							sep_w || vet01.ie_saldo								||sep_w,1,8000);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ctb_gerar_interf_j410_ecd ( nr_seq_controle_p bigint, nm_usuario_p text, dt_inicio_p timestamp, dt_fim_p timestamp, qt_linha_p INOUT bigint, nr_sequencia_p INOUT bigint) FROM PUBLIC;
