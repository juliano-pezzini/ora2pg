...
CREATE OR REPLACE PROCEDURE ageint_gerar_agend_list_espera ( nr_seq_lista_p bigint, cd_profissional_p text, cd_estabelecimento_p bigint, nm_usuario_p text, ie_agrupa_agendamento_p text default null) AS $body$
DECLARE
...
if (coalesce(nr_seq_lista_p,0) > 0) then
	...
	if ie_agrupa_agendamento_p = 'S' then
		select count(*)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ageint_gerar_agend_list_espera ( nr_seq_lista_p bigint, cd_profissional_p text, cd_estabelecimento_p bigint, nm_usuario_p text, ie_agrupa_agendamento_p text default null) FROM PUBLIC;
