...
CREATE OR REPLACE PROCEDURE inserir_dados_nf_cc_presum_htm ( nr_sequencia_p bigint) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	and	trunc(n.dt_emissao) between trunc(dt_inicial_w) and fim_dia(dt_final_w)
...
C03 CURSOR FOR
	...
	and 	((ie_filter_by_estab_w = 'N') or (ie_filter_by_estab_w = 'S' and n.cd_estabelecimento = cd_estabelecimento_w))
	and	trunc(l.dt_recebimento) between trunc(dt_inicial_w) and fim_dia(dt_final_w)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE inserir_dados_nf_cc_presum_htm ( nr_sequencia_p bigint) FROM PUBLIC;
