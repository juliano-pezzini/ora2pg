...
CREATE OR REPLACE PROCEDURE ajustar_proj_rec_ordem_compra ( nr_ordem_compra_p bigint, nr_seq_projeto_anterior_p bigint, nr_seq_projeto_novo_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (coalesce(nr_seq_projeto_novo_p,0) <> coalesce(nr_seq_projeto_anterior_p,0)) then
	...
	ds_historico_novo_w := 	'Alteração do Projeto Recurso ' || chr(13) || chr(10) || chr(13) || chr(10) ||
				'Ordem de compras nrº: ' || nr_ordem_compra_p || chr(13) || chr(10) ||
				'Projeto recurso anterior: ' || ds_projeto_anterior_w || chr(13) || chr(10) ||
				'Projeto recurso atual: ' || ds_projeto_novo_w;
	*/
	ds_historico_novo_w := 	wheb_mensagem_pck.get_texto(297030,	'NR_ORDEM_COMPRA='||nr_ordem_compra_p||
	...
	/*ds_historico_anterior_w := 	'Alteração do Projeto Recurso ' || chr(13) || chr(10) || chr(13) || chr(10) ||
				...
				'Projeto recurso anterior: ' || ds_projeto_anterior_w || chr(13) || chr(10) ||
				'Projeto recurso atual: ' || ds_projeto_novo_w;*/
	ds_historico_anterior_w := 	wheb_mensagem_pck.get_texto(297030,	'NR_ORDEM_COMPRA='||nr_ordem_compra_p||
						';NR_PROJ_ANTERIOR='||ds_projeto_anterior_w||
						';NR_PROJ_ATUAL='||ds_projeto_novo_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ajustar_proj_rec_ordem_compra ( nr_ordem_compra_p bigint, nr_seq_projeto_anterior_p bigint, nr_seq_projeto_novo_p bigint, nm_usuario_p text) FROM PUBLIC;
