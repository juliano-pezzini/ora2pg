...
CREATE OR REPLACE PROCEDURE gerar_interf_mamo_sism_coord ( dt_inicial_p timestamp , dt_final_p timestamp , nm_usuario_p text ) AS $body$
DECLARE
...
NR_SEQ_SISMAMA_C        constant varchar(17) := 'nr_seq_sismama_w=';

c01 CURSOR FOR
	SELECT	b.nr_sequencia,
		substr(b.cd_cgc_unidade_saude,1,14),
		b.cd_estabelecimento,
		substr(b.cd_pf_resp_emissao_filme,1,10),
		substr(b.cd_profissional,1,10),
		to_char(b.dt_emissao,'ddmmyyyy'),
		to_char(b.dt_liberacao,'ddmmyyyy'),
		b.nr_seq_unidade_saude,
		b.nr_atendimento,
		a.nr_interno_conta
	from	procedimento_paciente c,
		atendimento_paciente e,
		sismama_atendimento b,
		conta_paciente a
	...
	and	b.dt_liberacao   = (	SELECT 	max(x.dt_liberacao)
...
union
	...
	and	exists (select 	1
			...
			where	y.nr_seq_sismama = b.nr_sequencia
			and	y.dt_liberacao between dt_inicial_p and dt_final_p)
...
union
	...
	and	exists (select 	1
			from	sismama_mam_conclusao y
			where	y.nr_seq_sismama = b.nr_sequencia
			and	y.dt_liberacao between dt_inicial_p and dt_final_p or
				y.dt_liberacao < dt_inicial_p)
...
ie_considera_data_diag_w		:= coalesce(obter_valor_param_usuario(1125,102,obter_perfil_ativo,nm_usuario_p,1 ),'N');
...
EXIT WHEN NOT FOUND; /* apply on c02 */
	...
	delete	from	w_mamo_interf_coord
	where	nr_interno_conta = nr_interno_conta_w;
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	and	a.nr_sequencia   = nr_seq_sismama_w;
	...
	where	d.nr_seq_sismama = nr_seq_sismama_w;
	...
	where	e.nr_seq_sismama = nr_seq_sismama_w;
	...
	where	f.nr_seq_sismama = nr_seq_sismama_w;
	...
	where	g.nr_seq_sismama = nr_seq_sismama_w
	and	g.ie_lado = 'D';
	...
	exception
		when others then
		CALL wheb_mensagem_pck.exibir_mensagem_abort(263370,'nr_seq_sismama_w='||nr_seq_sismama_w);
		end;
	...
	where	h.nr_seq_sismama = nr_seq_sismama_w;
	exception
                when no_data_found then
                CALL wheb_mensagem_pck.exibir_mensagem_abort(nr_seq_mensagem_p=>263373,vl_macros_p=>NR_SEQ_SISMAMA_C||nr_seq_sismama_w);
		end;
	...
	insert into w_mamo_interf_coord(
			nr_sequencia,
			nr_interno_conta,
			dt_atualizacao,
			nm_usuario,
			dt_atualizacao_nrec,
			nm_usuario_nrec,
			nr_seq_sismama_atend,
			dt_inicial,
			dt_final,
			dt_mesano_apres,
			dt_envio_coord,
			nm_usuario_envio)
	values ( 	nextval('w_mamo_interf_coord_seq'),
			nr_interno_conta_w,
			clock_timestamp(),
			nm_usuario_p,
			clock_timestamp(),
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_interf_mamo_sism_coord ( dt_inicial_p timestamp , dt_final_p timestamp , nm_usuario_p text ) FROM PUBLIC;
