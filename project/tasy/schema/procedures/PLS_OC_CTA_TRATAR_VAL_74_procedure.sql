...
CREATE OR REPLACE PROCEDURE pls_oc_cta_tratar_val_74 ( dados_regra_p pls_tipos_ocor_pck.dados_regra, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE

 /* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  Finalidade:	Validar se no atendimento, um determiando grupo de itens ultrapassa o valor permitido ou
	estipulado pela operadora.
	Funciona da seguinte forma, os itens desejados sao filtrados normalmente na tabela de selecao.
	Durante esta validacao estes sao agrupados de acordo com a necessidade e se o valor
	dos mesmos ultrapassar o estipulado entao a ocorrencia sera gerada.
  -------------------------------------------------------------------------------------------------------------------
  ...
  Tomar cuidado com a manutencao dos registros na tabela PLS_SELECAO_OCOR_CTA, utilizar sempre a 
  ...
  Alteracao: Criacao da rotina.
  ------------------------------------------------------------------------------------------------------------------

  jjung 15/09/2014 - OS 744694 - 
  
  Alteracao:	Modificada a forma de trabalho para que a soma seja feita no PL\SQL.
  
  Motivo:	Na tabela de selecao pode ter procedimentos e materiais que nao foram filtrados pelos filtros da
	ocorrencia e foram incluedos de acordo com a regra para atender a necessidade quando temos
	a excecao por um item e aplicacao na conta e um item atendeu a excecao a ocorrencia nao deve
	ser gerada, para esta validacao esta regra nao e respeitada por que a excecao define os itens que temos considerar
	na hora de filtrar os registros.
  ------------------------------------------------------------------------------------------------------------------
...
ie_sem_rel_aud_obrigatorio_w	varchar(1);

-- Informacoes da regra de validacao
cs_val CURSOR(nr_seq_oc_cta_comb_p	dados_regra_p.nr_sequencia%type) FOR
	SELECT	a.ie_incidencia,
		...
		coalesce(a.ie_rel_nao_preenchido,'N') ie_rel_nao_preenchido,
...
cs_agrup CURSOR(nr_id_transacao_pc 		pls_oc_cta_selecao_ocor_v.nr_id_transacao%type,
		ie_origem_proced_pc		pls_conta_proc_ocor_v.ie_origem_proced%type,
	...
	SELECT	a.nr_seq_segurado,
		...
		c.vl_procedimento_imp vl_taxa_imp,
		c.vl_total_sem_taxa_int_imp vl_sem_taxa_imp,
		c.vl_procedimento vl_taxa,
		c.vl_total_sem_taxa_int vl_sem_taxa,
		c.ie_origem_proced,
		c.cd_procedimento,
		null as nr_seq_material,
		(	SELECT 	count(1)
			from 	w_pls_analise_rel_aud_int 
			where 	nr_seq_analise = c.nr_seq_analise) qt_rel_auditoria,
		coalesce(c.ie_tipo_despesa, c.ie_tipo_despesa_imp)  ie_tipo_despesa_proc,
		null ie_tipo_despesa_mat,
		c.nr_seq_guia,
		c.cd_estabelecimento
	FROM pls_conta_proc_ocor_v c, pls_oc_cta_selecao_ocor_v a
LEFT OUTER JOIN pls_oc_cta_filtro b ON (a.nr_seq_filtro = b.nr_sequencia)
WHERE a.nr_id_transacao	= nr_id_transacao_pc and a.ie_valido		= 'S'  and c.nr_sequencia 		= a.nr_seq_conta_proc
...
union all
	...
	select	a.nr_seq_segurado,
		...
		coalesce(b.ie_filtro_proc, 'N') ie_filtro_proc,
		coalesce(b.ie_filtro_mat, 'N') ie_filtro_mat,
		c.vl_material_imp vl_taxa_imp,
		c.vl_total_sem_taxa_int_imp vl_sem_taxa_imp,
		c.vl_material vl_taxa,
		c.vl_total_sem_taxa_int vl_sem_taxa,
		...
		c.nr_seq_guia,
		c.cd_estabelecimento
	FROM pls_conta_mat_ocor_v c, pls_oc_cta_selecao_ocor_v a
LEFT OUTER JOIN pls_oc_cta_filtro b ON (a.nr_seq_filtro = b.nr_sequencia)
WHERE a.nr_id_transacao	= nr_id_transacao_pc and a.ie_valido		= 'S'  and c.nr_sequencia 		= a.nr_seq_conta_mat
...
union all
	...
	select	a.nr_seq_segurado,
		...
		coalesce(b.ie_filtro_proc, 'N') ie_filtro_proc,
		coalesce(b.ie_filtro_mat, 'N') ie_filtro_mat,
		c.vl_total_imp vl_taxa_imp,
		((coalesce((select sum(case 	when(ie_considera_referencia_pc = 'N') and (ie_origem_proced_pc = proc.ie_origem_proced) and (cd_procedimento_pc = proc.cd_procedimento) then 0
					else 	proc.vl_total_sem_taxa_int_imp 
				end)
		from	 pls_conta_proc_ocor_v proc
		where	 proc.nr_seq_conta = c.nr_sequencia),0)) +
		(coalesce((select sum(case	when(ie_considera_referencia_pc = 'N') and (nr_seq_material_pc = mat.nr_seq_material) then 0
					else mat.vl_total_sem_taxa_int_imp
				end)
		from	 pls_conta_mat_ocor_v mat
		where	 mat.nr_seq_conta = c.nr_sequencia),0))) vl_sem_taxa_imp,		
		((coalesce((select sum(case when(ie_considera_referencia_pc = 'N') and (ie_origem_proced_pc = proc.ie_origem_proced) and (cd_procedimento_pc = proc.cd_procedimento) then 0
					else 	proc.vl_procedimento
				end)
		from	 pls_conta_proc_ocor_v proc
		where	 proc.nr_seq_conta = c.nr_sequencia),0)) +
		(coalesce((select sum(case	when(ie_considera_referencia_pc = 'N') and (nr_seq_material_pc = mat.nr_seq_material) then 0
					else mat.vl_material
				end)
		from	 pls_conta_mat_ocor_v mat
		where	 mat.nr_seq_conta = c.nr_sequencia),0))) vl_taxa,		
		((coalesce((select sum(case when(ie_considera_referencia_pc = 'N') and (ie_origem_proced_pc = proc.ie_origem_proced) and (cd_procedimento_pc = proc.cd_procedimento) then 0
					else 	proc.vl_total_sem_taxa_int
				end)
		from	 pls_conta_proc_ocor_v proc
		where	 proc.nr_seq_conta = c.nr_sequencia),0)) +
		(coalesce((select sum(case	when(ie_considera_referencia_pc = 'N') and (nr_seq_material_pc = mat.nr_seq_material) then 0
					else mat.vl_total_sem_taxa_int
				end)
		from	 pls_conta_mat_ocor_v mat
		where	 mat.nr_seq_conta = c.nr_sequencia),0))) vl_sem_taxa,
		null ie_origem_proced,
		null cd_procedimento,
		...
		c.nr_seq_guia,
		c.cd_estabelecimento
	FROM pls_conta_ocor_v c, pls_oc_cta_selecao_ocor_v a
LEFT OUTER JOIN pls_oc_cta_filtro b ON (a.nr_seq_filtro = b.nr_sequencia)
WHERE a.nr_id_transacao	= nr_id_transacao_pc and a.ie_valido		= 'S' and coalesce(a.nr_seq_conta_proc::text, '') = '' and coalesce(a.nr_seq_conta_mat::text, '') = ''  and c.nr_sequencia 		= a.nr_seq_conta and ie_desconsidera_opme_pacote_pc = 'N' --necessario essa restricao porque esse union tras procedimentos e materiais concatenados na mesma consulta. Para validar o tipo de despesa foi preciso incluir unions a parte para cada tipo
...
union all
	...
	select	a.nr_seq_segurado,
		a.cd_guia_referencia,
		a.nr_seq_conta,
		d.nr_seq_conta_proc nr_seq_conta_proc,
		...
		from	 pls_conta_proc_ocor_v proc
		where	 proc.nr_seq_conta = c.nr_sequencia
		and	proc.nr_sequencia = d.nr_sequencia),0)) vl_sem_taxa_imp,
		(coalesce((select sum(case when(ie_considera_referencia_pc = 'N') and (ie_origem_proced_pc = proc.ie_origem_proced) and (cd_procedimento_pc = proc.cd_procedimento) then 0
					else 	proc.vl_procedimento
				end)
		from	 pls_conta_proc_ocor_v proc
		where	 proc.nr_seq_conta = c.nr_sequencia
		and	proc.nr_sequencia = d.nr_sequencia),0)) vl_taxa,
		(coalesce((select sum(case when(ie_considera_referencia_pc = 'N') and (ie_origem_proced_pc = proc.ie_origem_proced) and (cd_procedimento_pc = proc.cd_procedimento) then 0
					else 	proc.vl_total_sem_taxa_int
				end)
		from	 pls_conta_proc_ocor_v proc
		where	 proc.nr_seq_conta = c.nr_sequencia
		and	proc.nr_sequencia = d.nr_sequencia),0)) vl_sem_taxa,
		...
		c.nr_seq_guia,
		c.cd_estabelecimento
	FROM pls_conta_proc_ocor_v d, pls_conta_ocor_v c, pls_oc_cta_selecao_ocor_v a
LEFT OUTER JOIN pls_oc_cta_filtro b ON (a.nr_seq_filtro = b.nr_sequencia)
WHERE a.nr_id_transacao	= nr_id_transacao_pc and a.ie_valido		= 'S'  and c.nr_sequencia 		= a.nr_seq_conta and c.nr_sequencia		= d.nr_seq_conta and ie_desconsidera_opme_pacote_pc = 'S'
...
union all
	...
	select	a.nr_seq_segurado,
		...
		d.nr_sequencia nr_seq_conta_mat,
		coalesce(b.ie_filtro_proc, 'N') ie_filtro_proc,
		coalesce(b.ie_filtro_mat, 'N') ie_filtro_mat,
		c.vl_total_imp vl_taxa_imp,
		(coalesce((select sum(case	when(ie_considera_referencia_pc = 'N') and (nr_seq_material_pc = mat.nr_seq_material) then 0
		...
		from	 pls_conta_mat_ocor_v mat
		where	 mat.nr_seq_conta = c.nr_sequencia
		and	mat.nr_sequencia = d.nr_sequencia),0)) vl_sem_taxa_imp,
		(coalesce((select sum(case	when(ie_considera_referencia_pc = 'N') and (nr_seq_material_pc = mat.nr_seq_material) then 0
					else mat.vl_material
				end)
		from	 pls_conta_mat_ocor_v mat
		where	 mat.nr_seq_conta = c.nr_sequencia
		and	mat.nr_sequencia = d.nr_sequencia),0)) vl_taxa,
		(coalesce((select sum(case	when(ie_considera_referencia_pc = 'N') and (nr_seq_material_pc = mat.nr_seq_material) then 0
					else mat.vl_total_sem_taxa_int
				end)
		from	 pls_conta_mat_ocor_v mat
		where	 mat.nr_seq_conta = c.nr_sequencia
		and	mat.nr_sequencia = d.nr_sequencia),0)) vl_sem_taxa,
		null ie_origem_proced,
		null cd_procedimento,
		d.nr_seq_material,
		(	select 	count(1) 
		...
		c.nr_seq_guia,
		c.cd_estabelecimento
	FROM pls_conta_mat_ocor_v d, pls_conta_ocor_v c, pls_oc_cta_selecao_ocor_v a
LEFT OUTER JOIN pls_oc_cta_filtro b ON (a.nr_seq_filtro = b.nr_sequencia)
WHERE a.nr_id_transacao	= nr_id_transacao_pc and a.ie_valido		= 'S'  and c.nr_sequencia 		= a.nr_seq_conta and c.nr_sequencia 		= d.nr_seq_conta and ie_desconsidera_opme_pacote_pc = 'S' order by nr_seq_segurado, cd_guia_referencia, nr_seq_conta;
...
procedure gerencia_selecao(	ie_incidencia_p		pls_oc_cta_val_vl_atend.ie_incidencia%type,	
				ds_agrupamento_p	ds_agrupamento_w%type,
				ie_verifica_commit_p	boolean,
				ie_atualiza_nulo_p	boolean) is;
BEGIN
...
if (ds_agrupamento_p IS NOT NULL AND ds_agrupamento_p::text <> '') then
	-- se for incidencia no atendimento decompoe os dados.
	if (ie_incidencia_p = 'A') then
		
		nr_seq_segurado_w := substr(ds_agrupamento_p, 0, position('||' in ds_agrupamento_p) - 1);
		cd_guia_referencia_w := substr(ds_agrupamento_p, position('||' in ds_agrupamento_p) + 2, length(ds_agrupamento_p));
	-- se nao e so a sequencia da conta mesmo
	else
		nr_seq_conta_w := ds_agrupamento_p;
	end if;

	-- obter todos os registros da selecao para aquele atendimento para atualizar todos.
	dados_selecao_w.ds_seqs_selecao(i) := pls_tipos_ocor_pck.obter_sequencia_selecao(	
		null, null, nr_seq_conta_w, nr_id_transacao_p, null,
		null, dados_regra_p.ie_excecao, 'V', 'S',
		cd_guia_referencia_w, nr_seq_segurado_w);
		
	dados_selecao_w.ie_valido(i) := 'S';
	dados_selecao_w.ds_observacao(i) :=
		'O atendimento supera o valor maximo permitido pela operadora.' || pls_util_pck.enter_w ||
		'Valor total: ' || campo_mascara_virgula_casas(tb_valor_w(ds_agrupamento_p), 2);
	...
	if (i = pls_cta_consistir_pck.qt_registro_transacao_w or ie_verifica_commit_p = false) then
		...
		CALL pls_tipos_ocor_pck.gerencia_selecao_validacao(
			dados_selecao_w.nr_seq_selecao, dados_selecao_w.ds_seqs_selecao,
			'LISTA', dados_selecao_w.ds_observacao, dados_selecao_w.ie_valido,
			nm_usuario_p);
		-- Limpar as tabelas
		pls_tipos_ocor_pck.limpar_nested_tables(dados_selecao_w);
...
if (dados_regra_p.nr_sequencia IS NOT NULL AND dados_regra_p.nr_sequencia::text <> '') then
	...
	for rw_val_w in cs_val(dados_regra_p.nr_sequencia) loop
		...
		if (rw_val_w.cd_procedimento IS NOT NULL AND rw_val_w.cd_procedimento::text <> '') then
			...
			FROM pls_conta_proc c, pls_oc_cta_selecao_ocor_v a
LEFT OUTER JOIN pls_oc_cta_filtro b ON (a.nr_seq_filtro = b.nr_sequencia)
WHERE a.nr_id_transacao	= nr_id_transacao_p and a.ie_valido		= 'S'  and c.nr_seq_conta 		= a.nr_seq_conta and c.cd_procedimento	= rw_val_w.cd_procedimento and c.ie_origem_proced	= rw_val_w.ie_origem_proced;
		end if;
		
		if (rw_val_w.nr_seq_material IS NOT NULL AND rw_val_w.nr_seq_material::text <> '') then
			select	count(1)
			into STRICT	qt_material
			FROM pls_conta_mat c, pls_oc_cta_selecao_ocor_v a
LEFT OUTER JOIN pls_oc_cta_filtro b ON (a.nr_seq_filtro = b.nr_sequencia)
WHERE a.nr_id_transacao	= nr_id_transacao_p and a.ie_valido		= 'S'  and c.nr_seq_conta 		= a.nr_seq_conta and c.nr_seq_material	= rw_val_w.nr_seq_material;
		end if;
		...
		if (ie_realiza_validacao = 'S') then
			...
			-- Esta validacao em especefico funciona um pouco diferente, pois os filtros serao utilizados para definir
			...
			-- e feito um outer join com os filtros para definir se o procedimento ou material foi incluedo por conta da regra que ja descrevi acima
			...
			for rw_agrup_w	in cs_agrup(	nr_id_transacao_p,
					...
					ie_sem_rel_aud_obrigatorio_w := 'N';
					--Se regra tiver opcao para validar relatorio nao preenchido e nao existir nenhum registro de relatorio auditoria vinculado a analise
					if ( rw_val_w.ie_rel_nao_preenchido = 'S' and rw_agrup_w.qt_rel_auditoria = 0) then
						
						--Nesse caso a regra obria ter relatorio auditoria e o mesmo nao foi preenchido(Se tiver registro ja considera preenchido)
						ie_sem_rel_aud_obrigatorio_w := 'S';
					...
					vl_item_w := 0;
					-- Se for para considerar a taxa usa o campo que agrega o valor, se nao nao

					-- Adotei a estrategia de deixar a responsabilidade da soma no cursor por que e diferente para procedimento e material,

					-- portanto ficariam muitas comparacoes se fosse feito tudo aqui.
					if (rw_val_w.ie_considera_tx_intercambio = 'S' ) then
					
						-- Verifica qual valor ira considerar 

						-- Apresentado
						if (rw_val_w.ie_valor_base = 'A') then
						
							vl_item_w 	:= rw_agrup_w.vl_taxa_imp;
						-- calculado
						else
							vl_item_w 	:= rw_agrup_w.vl_taxa;
						end if;
					else	
						-- Verifica qual valor ira considerar 
						...
						if (rw_val_w.ie_valor_base = 'A') then
							...
							vl_item_w 	:= rw_agrup_w.vl_sem_taxa_imp;
						-- calculado
						else
							vl_item_w 	:= rw_agrup_w.vl_sem_taxa;
						end if;
					end if;
					...
					if (rw_val_w.ie_desconsidera_opme_pacote = 'S' AND rw_agrup_w.ie_tipo_despesa_proc = 4) then
						...
						for dados_conta_autor_proc_w in c_conta_autor_proc(rw_agrup_w.nr_seq_guia,
					...
					if (rw_val_w.ie_desconsidera_opme_pacote = 'S' AND rw_agrup_w.ie_tipo_despesa_mat= 7) then
						...
						for dados_conta_autor_mat_w in c_conta_autor_mat(rw_agrup_w.nr_seq_guia,
					...
					if (vl_item_w	> 0) then
						...
						if (rw_val_w.cd_procedimento IS NOT NULL AND rw_val_w.cd_procedimento::text <> '') then
							...
							from (SELECT c.nr_sequencia
								FROM pls_conta_proc c, pls_oc_cta_selecao_ocor_v a
LEFT OUTER JOIN pls_oc_cta_filtro b ON (a.nr_seq_filtro = b.nr_sequencia)
WHERE a.nr_id_transacao	= nr_id_transacao_p and a.ie_valido		= 'S'  and c.nr_seq_conta 		= a.nr_seq_conta and c.cd_procedimento	= rw_val_w.cd_procedimento and c.ie_origem_proced	= rw_val_w.ie_origem_proced and rw_val_w.ie_incidencia	= 'A' and a.cd_guia_referencia	= rw_agrup_w.cd_guia_referencia and a.nr_seq_segurado	= rw_agrup_w.nr_seq_segurado
...
union all
								...
								FROM pls_conta_proc c, pls_oc_cta_selecao_ocor_v a
LEFT OUTER JOIN pls_oc_cta_filtro b ON (a.nr_seq_filtro = b.nr_sequencia)
WHERE a.nr_id_transacao	= nr_id_transacao_p and a.ie_valido		= 'S'  and c.nr_seq_conta 		= a.nr_seq_conta and c.cd_procedimento	= rw_val_w.cd_procedimento and c.ie_origem_proced	= rw_val_w.ie_origem_proced and rw_val_w.ie_incidencia	= 'C' and a.nr_seq_conta		= rw_agrup_w.nr_seq_conta ) alias3;	
						end if;
						...
						if (rw_val_w.nr_seq_material IS NOT NULL AND rw_val_w.nr_seq_material::text <> '') then
							...
							from (SELECT c.nr_sequencia	
								FROM pls_conta_mat c, pls_oc_cta_selecao_ocor_v a
LEFT OUTER JOIN pls_oc_cta_filtro b ON (a.nr_seq_filtro = b.nr_sequencia)
WHERE a.nr_id_transacao	= nr_id_transacao_p and a.ie_valido		= 'S'  and c.nr_seq_conta 		= a.nr_seq_conta and c.nr_seq_material	= rw_val_w.nr_seq_material and rw_val_w.ie_incidencia	= 'A' and a.cd_guia_referencia	= rw_agrup_w.cd_guia_referencia and a.nr_seq_segurado	= rw_agrup_w.nr_seq_segurado
...
union all
								...
								FROM pls_conta_mat c, pls_oc_cta_selecao_ocor_v a
LEFT OUTER JOIN pls_oc_cta_filtro b ON (a.nr_seq_filtro = b.nr_sequencia)
WHERE a.nr_id_transacao	= nr_id_transacao_p and a.ie_valido		= 'S'  and c.nr_seq_conta 		= a.nr_seq_conta and c.nr_seq_material	= rw_val_w.nr_seq_material and rw_val_w.ie_incidencia	= 'C' and a.nr_seq_conta		= rw_agrup_w.nr_seq_conta ) alias2;
						end if;
					...
					-- Se o filtro for a nevel de conta, podemos ter procedimentos, materiais e ate mesmo contas como retorno, por isso
					...
					--Se ie_sem_rel_aud_obrigatorio_w = 'S', entao regra pede relatorio preenchido e o mesmo nao foi, caso  rw_val_w.ie_rel_nao_preenchido = 'N'. entao nao esta marcado na regra e so valida o valor
					if (ie_sem_rel_aud_obrigatorio_w = 'S' or rw_val_w.ie_rel_nao_preenchido = 'N') then
						...
						if (coalesce(ds_agrup_old_w::text, '') = '' or ds_agrup_old_w <> ds_agrupamento_w) then
							...
							if (tb_valor_w.exists(ds_agrup_old_w) and tb_valor_w(ds_agrup_old_w) > rw_val_w.vl_base) then														
								...
								CALL gerencia_selecao(rw_val_w.ie_incidencia, ds_agrup_old_w, true, false);
							end if;
						end if;
			...
			if (ds_agrup_old_w IS NOT NULL AND ds_agrup_old_w::text <> '') then
				...
				if (tb_valor_w.exists(ds_agrup_old_w) and tb_valor_w(ds_agrup_old_w) > rw_val_w.vl_base) then
					...
					CALL gerencia_selecao(rw_val_w.ie_incidencia, ds_agrup_old_w, false, false);
				else
					-- Se a guia nao for valida tem que gravar as outras que ja estavam antes na tabela de selecao;
					CALL gerencia_selecao(rw_val_w.ie_incidencia, null, false, true);
				end if;
			end if;
		...
		-- Tem que limpar esta tabela por que na hora de gravar esta ocorrencia, se um item tiver caedo na excecao nao tem problema, pode gerar para a conta.
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_oc_cta_tratar_val_74 ( dados_regra_p pls_tipos_ocor_pck.dados_regra, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
