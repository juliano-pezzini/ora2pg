...
CREATE OR REPLACE PROCEDURE importar_extrato_cartao_master (nr_seq_extrato_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
SELECT	nr_sequencia,
	...
	--to_date(decode(somente_numero(substr(ds_conteudo,45,6)),0,null,substr(ds_conteudo,45,6)),'yymmdd') dt_compra,
...
select	somente_numero(substr(ds_conteudo,39,7)) nr_extrato,
	to_date(trim(both substr(ds_conteudo,12,8)),'yyyymmdd') dt_processamento,
	to_date(trim(both substr(ds_conteudo,20,8)),'yyyymmdd') dt_inicial,
	to_date(trim(both substr(ds_conteudo,28,8)),'yyyymmdd') dt_final
	...
	if (ie_tipo_registro_w = '1') then
		...
		if (ie_status_w <> '12') then
			...
			if (coalesce(nr_seq_conta_banco_w::text, '') = '') then
				...
				CALL wheb_mensagem_pck.exibir_mensagem_abort(267080);
			end if;
...
exception
	when others then
	-- #@DS_ERRO#@
	CALL wheb_mensagem_pck.exibir_mensagem_abort(267081, 'DS_ERRO=' || nr_sequencia_w || ' ' || sqlerrm);
end;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE importar_extrato_cartao_master (nr_seq_extrato_p bigint, nm_usuario_p text) FROM PUBLIC;
