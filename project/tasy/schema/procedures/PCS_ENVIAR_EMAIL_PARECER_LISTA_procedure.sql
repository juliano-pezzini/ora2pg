...
CREATE OR REPLACE PROCEDURE pcs_enviar_email_parecer_lista ( nr_seq_registro_p bigint, cd_material_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
nm_usuario_destinos_w		varchar(4000);

C01 CURSOR FOR
	SELECT  distinct c.cd_pessoa_dest,
			c.nr_seq_grupo_dest
	...
	and 	(c.cd_pessoa_dest IS NOT NULL AND c.cd_pessoa_dest::text <> '')
	and		coalesce(c.dt_envio::text, '') = ''
	and		c.ie_selecionado = 'S'
	and		((ie_comunic_interna = 'S') or (ie_email = 'S'))
	group by c.cd_pessoa_dest,nr_seq_grupo_dest;
...
C02 CURSOR FOR
	...
	and		a.nr_sequencia = nr_seq_registro_p
	and	c.cd_material	 = cd_material_p
	and		c.cd_pessoa_dest = cd_pessoa_dest_w
	and		c.ie_selecionado = 'S'
	and		((ie_comunic_interna = 'S') or (ie_email = 'S'))
	and		coalesce(c.dt_envio::text, '') = ''
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	EXIT WHEN NOT FOUND; /* apply on C03 */
		...
		nm_usuario_destinos_w := nm_usuario_destinos_w || nm_usuario_dest_w || ';';
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pcs_enviar_email_parecer_lista ( nr_seq_registro_p bigint, cd_material_p bigint, nm_usuario_p text) FROM PUBLIC;
