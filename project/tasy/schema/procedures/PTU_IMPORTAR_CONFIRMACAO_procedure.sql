...
CREATE OR REPLACE PROCEDURE ptu_importar_confirmacao ( ds_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
	...
	if (substr(ds_conteudo_w,1,4)	<> 'FIM$') then 
		if (nr_seq_import_w	= 1) then 
			...
			and	not exists (	SELECT 1 
						...
						where	x.nr_seq_execucao	= a.nr_seq_execucao 
			...
			if (qt_reg_canelamento_w	<> 0) then 
			...
			elsif (qt_reg_insistencia_w	<> 0) then 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_importar_confirmacao ( ds_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
