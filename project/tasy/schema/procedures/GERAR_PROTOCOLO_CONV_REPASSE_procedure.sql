...
CREATE OR REPLACE PROCEDURE gerar_protocolo_conv_repasse ( nr_seq_protocolo_p bigint, nm_usuario_p text, ie_gerar_repasse_p text) AS $body$
DECLARE
...
/*Descontabilizacao online repasse */

ie_ctb_online_w := ctb_online_pck.get_modo_lote(14, cd_estabelecimento_w);
ie_geracao_w := ctb_online_pck.get_geracao_lote_receita(cd_convenio_protocolo_w,
                                                        cd_estabelecimento_w,
                                                        nm_usuario_p,
                                                        ie_tipo_convenio_w);
if (ie_ctb_online_w = 'S' and ie_geracao_w = 'FPR') then
CALL ctb_contab_onl_repasse( nr_seq_protocolo_p,null,null,nm_usuario_p,2,'N','N');
end if;
...
for	r_c_contas in c_contas(nr_seq_protocolo_p) loop
    ...
    if (ie_pagto_w = 'T') then
      begin
        ...
        and b.nr_seq_procedimento = r_c_procedimentos.nr_sequencia;
        exception
        when no_data_found or too_many_rows then
          ie_pagto_w	:= null;
      end;
	...
	for	r_c_materiais in c_materiais(r_c_contas.nr_interno_conta) loop
		...
		if	((ie_repasse_mat_w	= 'P') and (coalesce(ie_gerar_repasse_p,'P') <> 'T')) or
			...
			CALL gerar_material_repasse(		r_c_materiais.nr_sequencia,
							cd_estabelecimento_w,
							r_c_contas.cd_medico_resp,
							nm_usuario_p,
							cd_edicao_amb_w,
    ...
    if (ie_pagto_w = 'T') then
      begin
        ...
        from	material_repasse b,
          terceiro a
        where	a.nr_sequencia	= b.nr_seq_terceiro
        and b.nr_seq_material = r_c_materiais.nr_sequencia;
        exception
        when no_data_found or too_many_rows then
          ie_pagto_w	:= null;
      end;
		...
		if (ie_gravar_log_w = 'S') and (ie_gravar_log_mat_w = 'S') then
			begin
			ds_log_repasse_w := substr(ds_log_repasse_w||';cd_regra_w='|| cd_regra_w ||';ie_pagto_w='|| ie_pagto_w ,1,4000);								
			CALL grava_log_geracao_repasse(null,r_c_contas.nr_interno_conta,null,null,null,null,r_c_materiais.nr_sequencia,null,ds_log_repasse_w,nm_usuario_p,nr_seq_protocolo_p);
			end;
		end if;
	--fim do cursor dos materiais
	END LOOP;
...
if (ie_ctb_online_w = 'S' and ie_geracao_w = 'FPR') then
    CALL ctb_contab_onl_repasse( nr_seq_protocolo_p,null,null,nm_usuario_p);
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_protocolo_conv_repasse ( nr_seq_protocolo_p bigint, nm_usuario_p text, ie_gerar_repasse_p text) FROM PUBLIC;
