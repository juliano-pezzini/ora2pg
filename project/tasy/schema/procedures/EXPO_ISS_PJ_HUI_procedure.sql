...
CREATE OR REPLACE PROCEDURE expo_iss_pj_hui (dt_inicio_p timestamp, dt_fim_p timestamp, cd_estabelecimento_p text, nm_usuario_p text) AS $body$
DECLARE
...
SELECT	substr(n.nr_sequencia,1,10), 
	...
	2 nr_tomador, 
	CASE WHEN upper(elimina_caracteres_especiais(obter_dados_pf_pj(null,n.cd_cgc,'CI')))=upper(elimina_caracteres_especiais(obter_dados_pf_pj(null,n.cd_cgc_emitente,'CI'))) THEN 'S'  ELSE 'N' END , 
	rpad(coalesce(substr(obter_dados_pf_pj(null,n.cd_cgc, 'N'),1,100),' '),100,' '), 
	lpad(coalesce(obter_dados_pf_pj(null,n.cd_cgc,'IM'),'0'),10,'0'), 
	lpad(' ',2,' '), 
	lpad(coalesce(n.cd_cgc,' '),14,' '), 
	CASE WHEN obter_dados_pf_pj(null,n.cd_cgc,'IE')='' THEN 'S'  ELSE 'N' END , 
	lpad(coalesce(obter_dados_pf_pj(null,n.cd_cgc,'IE'),' '),15,' '), 
	lpad(coalesce(substr(elimina_caracteres_especiais(obter_dados_pf_pj(null,n.cd_cgc,'CEP')),1,8),' '),8,' '), 
	rpad(coalesce(substr(obter_dados_pf_pj(null,n.cd_cgc,'LO'),1,5),' '),5,' '), 
	lpad(' ',5,' '), 
	rpad(coalesce(substr(obter_dados_pf_pj(null,n.cd_cgc,'R'),1,50),' '),50,' '), 
	rpad(coalesce(substr(obter_dados_pf_pj(null,n.cd_cgc,'CO'),1,40),' '),40,' '), 
	lpad(coalesce(substr(obter_dados_pf_pj(null,n.cd_cgc,'NR'),1,10),' '),10,'0'), 
	rpad(coalesce(substr(obter_dados_pf_pj(null,n.cd_cgc,'B'),1,50),' '),50,' '), 
	rpad(coalesce(substr(obter_dados_pf_pj(null,n.cd_cgc,'UF'),1,2),' '),2,' '), 
	rpad(coalesce(substr(obter_dados_pf_pj(null,n.cd_cgc,'CI'),1,50),' '),50,' '), 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE expo_iss_pj_hui (dt_inicio_p timestamp, dt_fim_p timestamp, cd_estabelecimento_p text, nm_usuario_p text) FROM PUBLIC;
