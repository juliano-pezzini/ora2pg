...
CREATE OR REPLACE PROCEDURE gerar_w_regra_qtde_mat (nr_interno_conta_p bigint, nm_usuario_p text) AS $body$
DECLARE

 
/* 
 
1 - Materiais com quantidade inferior à regra 
2 - Materiais com quantidade superior à regra 
...
c03 CURSOR FOR 
SELECT	distinct trunc(dt_atendimento, 'dd') 
from	material_atend_paciente 
	...
	EXIT WHEN NOT FOUND; /* apply on c02 */
		if (ie_tipo_qtde_w = 'D') then 
			...
			EXIT WHEN NOT FOUND; /* apply on c03 */
				...
				and	trunc(dt_atendimento, 'dd')	= dt_atendimento_w 
				...
				if (qt_executada_dia_w > qt_permitida_w) then 
					...
					insert	into W_REGRA_QTDE_MAT(NR_INTERNO_CONTA, 
						CD_MATERIAL, 
						QT_REGRA, 
						IE_ACAO_EXCESSO, 
						DS_ERRO, 
					...
					values (nr_interno_conta_p, 
						...
						qt_permitida_w, 
						ie_acao_excesso_w, 
						ds_erro_w|| ' ' || PKG_DATE_FORMATERS.to_varchar(dt_atendimento_w, 'shortDate', WHEB_USUARIO_PCK.GET_CD_ESTABELECIMENTO, nm_usuario_p), 
						clock_timestamp(), 
						nm_usuario_p, 
						nr_seq_regra_w, 
						qt_executada_dia_w, 
		...
		if (ie_tipo_qtde_w = 'H') then 
			...
			EXIT WHEN NOT FOUND; /* apply on c04 */
				...
				and	coalesce(cd_motivo_exc_conta::text, '') = '' 
				and	nr_interno_conta	= nr_interno_conta_p 
				and	dt_atendimento		between	dt_atendimento_w and 
									dt_atendimento_w + ((1 / 24) * qt_horas_intervalo_w);
				...
				if (qt_executada_periodo_w > qt_permitida_w) then 
					...
					values (nr_interno_conta_p, 
						...
						ie_acao_excesso_w, 
						ds_erro_w|| ' ' || 
						PKG_DATE_FORMATERS.to_varchar(dt_atendimento_w, 'timestamp', WHEB_USUARIO_PCK.GET_CD_ESTABELECIMENTO, nm_usuario_p) || ' '||OBTER_DESC_EXPRESSAO(347642)||' ' || 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_w_regra_qtde_mat (nr_interno_conta_p bigint, nm_usuario_p text) FROM PUBLIC;
