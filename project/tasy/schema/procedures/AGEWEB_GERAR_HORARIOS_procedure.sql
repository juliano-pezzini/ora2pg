...
CREATE OR REPLACE PROCEDURE ageweb_gerar_horarios (cd_agenda_p bigint, cd_pessoa_fisica_p text, cd_medico_p text, ie_pesquisa_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	and		a.dt_agenda between trunc(dt_agenda_w) and trunc(dt_agenda_w) + 86399/86400
...
dt_agenda_w	:= trunc(clock_timestamp(),'dd') + 1;
...
if (cd_tipo_agenda_w = 3) then
	while(qt_dias_geracao_w < qt_dias_marc_w) loop
		begin
		
		if (((trunc(clock_timestamp()+qt_dias_marc_w) = trunc(dt_agenda_w)) and
			(hr_perm_age_w IS NOT NULL AND hr_perm_age_w::text <> '') and (clock_timestamp() >= to_date(to_char(clock_timestamp(), 'dd/mm/yyyy') || to_char(hr_perm_age_w, 'hh24:mi:ss'), 'dd/mm/yyyy hh24:mi:ss'))) or (trunc(clock_timestamp()+qt_dias_marc_w) <> trunc(dt_agenda_w)) or (coalesce(hr_perm_age_w::text, '') = '')) then
			...
			qt_dias_geracao_w	:= qt_dias_geracao_w + 1;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ageweb_gerar_horarios (cd_agenda_p bigint, cd_pessoa_fisica_p text, cd_medico_p text, ie_pesquisa_p text) FROM PUBLIC;
