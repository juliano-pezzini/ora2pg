...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_c141_icmsipi ( nr_seq_controle_p bigint, nr_seq_nota_p bigint, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
type reg_c_reg_C141 is table of fis_efd_icmsipi_C141%RowType;
...
fetch c_reg_C141 bulk collect into vetRegC141 limit 1000;
	...
	ds_linha_w := substr(	ds_sep_w || vetRegC141[i].cd_reg ||
				ds_sep_w || vetRegC141[i].nr_parc ||
				ds_sep_w || to_char(vetRegC141[i].dt_vcto,'ddmmyyyy')  ||
				ds_sep_w || substr(sped_obter_campo_valor(vetRegC141[i].vl_parc), 1, 30) || ds_sep_w, 1, 8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro 		:=  vetRegC141[i].cd_reg;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_c141_icmsipi ( nr_seq_controle_p bigint, nr_seq_nota_p bigint, nr_linha_p INOUT bigint ) FROM PUBLIC;
