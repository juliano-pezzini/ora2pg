...
CREATE OR REPLACE PROCEDURE gerar_evento_pac_prev_alta (nr_seq_evento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C00 CURSOR FOR 
	...
	and	substr(obter_se_prev_alta(a.nr_atendimento),1,10) = to_char(clock_timestamp(),'dd/mm/yyyy') 
...
EXIT WHEN NOT FOUND; /* apply on C00 */
	...
	select	max(substr(obter_cid_atendimento(nr_atendimento_w, 'P'),1,10)), 
		...
		max(substr(obter_pessoa_atendimento(nr_atendimento_w,'C'),1,10)), 
		max(substr(obter_se_prev_alta(nr_atendimento_w),1,20)) 
	...
	ds_unidade_w		:= substr(obter_unidade_atendimento(nr_atendimento_w,'A','U'),1,60);
	nr_ramal_w		:= substr(obter_unidade_atendimento(nr_atendimento_w,'A','RA'),1,60);
	nr_telefone_w		:= substr(obter_unidade_atendimento(nr_atendimento_w,'A','TL'),1,60);
	ds_setor_atendimento_w	:= substr(obter_unidade_atendimento(nr_atendimento_w,'A','S'),1,60);
	cd_setor_atend_pac_w	:= substr(obter_unidade_atendimento(nr_atendimento_w,'A','CS'),1,60);
	...
	ds_mensagem_w		:= substr(replace_macro(ds_mensagem_w,'@dt_alta_prevista',dt_alta_prevista_w),1,4000);
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		elsif (ie_pessoa_destino_w = '10') then /* Pessoa parecer */
 
		 
			cd_pessoa_destino_w	:= cd_medico_resp_w;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_evento_pac_prev_alta (nr_seq_evento_p bigint, nm_usuario_p text) FROM PUBLIC;
