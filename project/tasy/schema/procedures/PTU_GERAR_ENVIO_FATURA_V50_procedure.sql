...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_fatura_v50 ( nr_seq_fatura_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C06 CURSOR FOR 	-- 507 
	SELECT	coalesce(ie_tipo,' '), 
		...
		max(rpad(coalesce(ds_endereco,' '),40,' ')), 
		max(rpad(coalesce(ds_complemento,' '),20,' ')), 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_fatura_v50 ( nr_seq_fatura_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
