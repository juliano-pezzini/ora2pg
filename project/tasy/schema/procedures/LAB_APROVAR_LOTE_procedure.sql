...
CREATE OR REPLACE PROCEDURE lab_aprovar_lote ( nr_seq_lote_p bigint, cd_medico_resp_p text, ie_exames_sem_result_p text, nm_usuario_p text, ds_mensagem_p INOUT text) AS $body$
DECLARE
...
BEGIN
    ...
    EXIT WHEN NOT FOUND; /* apply on c01 */
        if (ie_exames_sem_result_w = 'N') or (ie_status_atend_w >= 30) then
            ...
            qt_nao_aprov_w  := qt_nao_aprov_w + 1;
        end if;
    end loop;
    close c01;

    if (qt_nao_aprov_w > 0) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE lab_aprovar_lote ( nr_seq_lote_p bigint, cd_medico_resp_p text, ie_exames_sem_result_p text, nm_usuario_p text, ds_mensagem_p INOUT text) FROM PUBLIC;
