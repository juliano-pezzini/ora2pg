...
CREATE OR REPLACE PROCEDURE gerar_log_prescricao ( nr_prescricao_p bigint, nr_seq_item_p bigint, ie_agrupador_p bigint, nr_seq_horario_p bigint, ie_tipo_item_p text, ds_log_p text, nm_usuario_p text, nr_seq_objeto_p bigint, ie_commit_p text) AS $body$
DECLARE
...
if (nr_prescricao_p IS NOT NULL AND nr_prescricao_p::text <> '') and (ds_log_p IS NOT NULL AND ds_log_p::text <> '') then
	begin
	select	nextval('log_prescricao_seq')
	into STRICT	nr_sequencia_w
	;

	insert	into
		log_prescricao(nr_sequencia,
				dt_atualizacao,
				nm_usuario,
				dt_atualizacao_nrec,
				nm_usuario_nrec,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_log_prescricao ( nr_prescricao_p bigint, nr_seq_item_p bigint, ie_agrupador_p bigint, nr_seq_horario_p bigint, ie_tipo_item_p text, ds_log_p text, nm_usuario_p text, nr_seq_objeto_p bigint, ie_commit_p text) FROM PUBLIC;
