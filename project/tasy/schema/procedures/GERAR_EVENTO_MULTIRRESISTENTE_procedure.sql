...
CREATE OR REPLACE PROCEDURE gerar_evento_multirresistente ( nr_atendimento_p bigint, cd_pessoa_fisica_p text, nm_usuario_p text, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
C01 CURSOR FOR 
	...
	and	upper(b.ds_evento) like upper('%Multirresistente%') 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_evento_multirresistente ( nr_atendimento_p bigint, cd_pessoa_fisica_p text, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
