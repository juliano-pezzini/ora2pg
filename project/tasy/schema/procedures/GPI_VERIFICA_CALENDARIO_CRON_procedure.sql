...
CREATE OR REPLACE PROCEDURE gpi_verifica_calendario_cron (dt_inicio_ativ_p timestamp, qt_hora_prev_p bigint, dt_inicio_definitivo_p INOUT timestamp, dt_fim_definitivo_p INOUT timestamp, nr_seq_cronograma_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
while(ie_fim_definido_w = 'N') loop
	...
	and	((ie_dia_semana = ie_dia_semana_w) or (pkg_date_utils.IS_BUSINESS_DAY(dt_inicio_ativ_w) = 1 and ie_dia_semana = '9')); -- ie_dia_semana_w in ('2','3','4','5','6')
	select	count(*)
	into STRICT	qt_excessao_w
	from	gpi_cron_excessao
	where	nr_seq_cronograma = nr_seq_cronograma_p
	and	PKG_DATE_UTILS.start_of(dt_inicio_ativ_w,'dd',0) between PKG_DATE_UTILS.start_of(dt_inicial,'dd',0) and PKG_DATE_UTILS.start_of(dt_final,'dd',0);
	...
	if (qt_excessao_w > 0) then
		...
		if (pkg_date_utils.IS_BUSINESS_DAY(dt_inicio_ativ_w) = 1) then -- ie_dia_semana_w in ('2','3','4','5','6')
			select	max(hr_final),
				max(hr_inicio),
				max(qt_min_intervalo)
			into STRICT	hr_final_w,
		...
		elsif (pkg_date_utils.IS_BUSINESS_DAY(dt_inicio_ativ_w) = 0) then -- ie_dia_semana_w in ('1','7')
			select	max(hr_final),
				max(hr_inicio),
				max(qt_min_intervalo)
			into STRICT	hr_final_w,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gpi_verifica_calendario_cron (dt_inicio_ativ_p timestamp, qt_hora_prev_p bigint, dt_inicio_definitivo_p INOUT timestamp, dt_fim_definitivo_p INOUT timestamp, nr_seq_cronograma_p bigint, nm_usuario_p text) FROM PUBLIC;
