...
CREATE OR REPLACE PROCEDURE gerar_w_fluxo_caixa_lote ( cd_estabelecimento_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, ie_nivel_p bigint, ie_origem_p text, ie_classif_fluxo_p text, ie_situacao_p text, ie_operacao_p text, ds_lista_conta_financ_p text, ie_conta_sem_valor_p text, nm_usuario_p text) AS $body$
DECLARE
...
/*OS 1661460 Se o nome do usuario tiver ponto virgula ou espaço, vai substituir por _*/
...
ds_values_w	:= '(select c.cd_conta_financ, c.cd_conta_apres, c.ds_conta_estrut, obter_ds_moeda_estrangeira(b.cd_moeda) ds_moeda ';
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	begin
	ds_alias_w	:= 'F' || to_char(dt_referencia_w,'ddmmyyyy');
	ds_create_w	:= ds_create_w || ' , ' || ds_alias_w || ' number(15,2) ';
	ds_insert_w	:= ds_insert_w || ' , ' || ds_alias_w;
	ds_values_w	:= substr(ds_values_w || ' , sum(decode(a.dt_referencia, to_date(' || chr(39) || to_char(dt_referencia_w,'dd/mm/yyyy') ||  chr(39) || ',' || chr(39) || 'dd/mm/yyyy' || chr(39) || '), a.vl_fluxo, 0)) ' || ds_alias_w,1,10000);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_w_fluxo_caixa_lote ( cd_estabelecimento_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, ie_nivel_p bigint, ie_origem_p text, ie_classif_fluxo_p text, ie_situacao_p text, ie_operacao_p text, ds_lista_conta_financ_p text, ie_conta_sem_valor_p text, nm_usuario_p text) FROM PUBLIC;
