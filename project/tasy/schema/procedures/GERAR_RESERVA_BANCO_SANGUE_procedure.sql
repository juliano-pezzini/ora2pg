...
CREATE OR REPLACE PROCEDURE gerar_reserva_banco_sangue ( nm_usuario_p text, nr_seq_agenda_p bigint, cd_estabelecimento_p bigint ) AS $body$
DECLARE
...
ie_gera_prescr_w := Obter_Param_Usuario(871, 741, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_p, ie_gera_prescr_w);
...
if (nr_seq_agenda_p IS NOT NULL AND nr_seq_agenda_p::text <> '') then
	if (ie_gera_prescr_w = 'S') then
		nr_prescricao_w := gerar_prescricao_banco_sangue(nr_seq_agenda_p, nm_usuario_p, cd_estabelecimento_p, nr_prescricao_w);
	end if;
...
if (nr_seq_agenda_p > 0 ) then
	CALL executar_evento_agenda('GRH','CI',nr_seq_agenda_p,cd_estabelecimento_p,nm_usuario_p);
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_reserva_banco_sangue ( nm_usuario_p text, nr_seq_agenda_p bigint, cd_estabelecimento_p bigint ) FROM PUBLIC;
