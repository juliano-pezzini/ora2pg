...
CREATE OR REPLACE PROCEDURE enviar_comunic_oc_pend_lib ( cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE

 
/* Gerar comunicação interna para ordem de compra liberada 
...
nm_usuario_aprov_ww		varchar(255);
...
c02 CURSOR FOR 
	...
	and	dt_liberacao < (clock_timestamp() - qt_dia_oc_pend_aprov_w) 
...
select	coalesce(max(qt_dia_oc_pend_aprov),0) 
into STRICT	qt_dia_oc_pend_aprov_w 
		...
		if (cd_perfil_w IS NOT NULL AND cd_perfil_w::text <> '') then 
			...
			EXIT WHEN NOT FOUND; /* apply on C02 */
				...
				select substr(obter_select_concatenado_bv('select	obter_desc_material(a.cd_material) 
									  ...
									  where	a.nr_ordem_compra = :nr_ordem_compra', 
									  'nr_ordem_compra= ' || nr_ordem_compra_w,ds_enter_w),1,4000) 
				into STRICT ds_material_w							 
				;
				 
							 
				SELECT	obter_select_concatenado_bv('SELECT	SUBSTR(obter_usuarios_proc_aprov(a.nr_sequencia, a.nr_seq_proc_aprov),1,255) nm_usuarios_aprov 
								   ...
								   AND 	a.nr_sequencia IN 
											(SELECT DISTINCT nr_seq_aprovacao 
											 ...
											 WHERE nr_ordem_compra = :nr_ordem_compra_w)','nr_ordem_compra_w='||nr_ordem_compra_w,',') 
				into STRICT nm_usuario_aprov_ww									 
				;				
 
				 
				 
				ds_destinatario_w := obter_usuarios_comunic_compras(nr_ordem_compra_w, null, 58, nr_seq_regra_w,'');
				 
				 
				 
				select	max(substr(b.ds_titulo,1,80)) ds_titulo, 
					max(substr(b.ds_comunicacao,1,4000)) ds_comunicacao 
				...
				ds_titulo_w	:= substr(replace_macro(ds_titulo_w, '@nr_ordem', nr_ordem_compra_w),1,80);
 
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@nr_ordem', nr_ordem_compra_w),1,4000);
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@ds_material', ds_material_w),1,4000);
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@nm_usuario_aprov', nm_usuario_aprov_ww),1,4000);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE enviar_comunic_oc_pend_lib ( cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
