...
CREATE OR REPLACE PROCEDURE gerar_pagto_brasil_cnab_240 ( nr_seq_envio_p bigint, nm_usuario_p text) AS $body$
DECLARE


/* PADRAO CNAB 240 VERSAO 8.7 DE 21/09/2012 */
...
ie_tipo_servi_w		varchar(2);

c01 CURSOR FOR
SELECT	distinct
	CASE WHEN b.ie_tipo_pagamento='CC' THEN '01' WHEN b.ie_tipo_pagamento='DOC' THEN CASE WHEN b.cd_banco=cd_banco_hosp_w THEN '01'  ELSE '03' END  WHEN b.ie_tipo_pagamento='TED' THEN '41' END ,
	b.ie_tipo_pagamento,
	CASE WHEN b.IE_TIPO_SERVICO='00020' THEN '20' WHEN b.IE_TIPO_SERVICO='00030' THEN '30' WHEN b.IE_TIPO_SERVICO='00098' THEN '98'  ELSE '98' END ,
	CASE WHEN b.ie_tipo_pagamento='CC' THEN '01'  ELSE '99' END
from	titulo_pagar_escrit b,
	banco_escritural a
where	b.ie_tipo_pagamento	in ('DOC','CC','TED')
...
SELECT	coalesce(coalesce((SELECT	x.cd_banco_externo
	...
	lpad(substr(somente_numero(coalesce(b.cd_agencia_bancaria,' ')),1,5) || substr(coalesce(b.ie_digito_agencia,' '),1,1),6,'0'),
...
ds_espaco_trailer_w varchar(24);
...
c03 CURSOR FOR
SELECT	distinct
	CASE WHEN(SELECT	count(*)		from	gps_titulo x		where	x.nr_titulo	= b.nr_titulo)=0 THEN		CASE WHEN(select	count(*)			from	darf_titulo_pagar x			where	x.nr_titulo	= b.nr_titulo)=0 THEN 			CASE WHEN 	b.ie_tipo_pagamento='PC' THEN '11'  ELSE CASE WHEN 	CASE WHEN coalesce(d.cd_banco_externo::text, '') = '' THEN b.cd_banco  ELSE somente_numero(d.cd_banco_externo) END =cd_banco_hosp_w THEN '30'  ELSE CASE WHEN(select	count(*)						from	banco x						where	CASE WHEN coalesce(x.cd_banco_externo::text, '') = '' THEN x.cd_banco  ELSE somente_numero(x.cd_banco_externo) END 	= somente_numero(substr(c.nr_bloqueto,1,3)))=0 THEN '11'  ELSE '31' END  END  END   ELSE CASE WHEN 	coalesce(c.nr_bloqueto::text, '') = '' THEN '16'  ELSE '11' END  END   ELSE CASE WHEN coalesce(c.nr_bloqueto::text, '') = '' THEN '17'  ELSE '11' END  END  ie_forma_lanc,
	b.ie_tipo_pagamento
...
WHERE b.nr_titulo		= c.nr_titulo and CASE WHEN(SELECT	count(*)		from	gps_titulo x		where	x.nr_titulo	= b.nr_titulo)=0 THEN		CASE WHEN(select	count(*)			from	darf_titulo_pagar x			where	x.nr_titulo	= b.nr_titulo)=0 THEN 			CASE WHEN 	b.ie_tipo_pagamento='PC' THEN '11'  ELSE CASE WHEN 	CASE WHEN coalesce(e.cd_banco_externo::text, '') = '' THEN b.cd_banco  ELSE somente_numero(e.cd_banco_externo) END =cd_banco_hosp_w THEN '30'  ELSE CASE WHEN(select	count(*)						from	banco x						where	CASE WHEN coalesce(x.cd_banco_externo::text, '') = '' THEN x.cd_banco  ELSE somente_numero(x.cd_banco_externo) END 	= somente_numero(substr(c.nr_bloqueto,1,3)))=0 THEN '11'  ELSE '31' END  END  END   ELSE CASE WHEN 	coalesce(c.nr_bloqueto::text, '') = '' THEN '16'  ELSE '11' END  END   ELSE CASE WHEN coalesce(c.nr_bloqueto::text, '') = '' THEN '17'  ELSE '11' END  END 	= ie_forma_lanc_w  and (b.ie_tipo_pagamento in ('BLQ','PC','DDA') or
	exists (select	1
	from	darf_titulo_pagar x
	where	x.nr_titulo	= b.nr_titulo) or
	exists (select	1
...
select	coalesce((select	somente_numero(coalesce(max(x.cd_banco_externo),to_char(c.cd_banco)))
	...
	rpad(upper(elimina_acentuacao(substr(obter_nome_estabelecimento(b.cd_estabelecimento),1,30))),30,' '),
...
ds_conteudo_w	:=	lpad(cd_banco_hosp_w,3,'0') || --1 3
			...
			'0' ||
			nm_empresa_w ||
			rpad('BANCO DO BRASIL',30,' ') ||
			rpad(' ',10,' ') ||
			'1' ||
			dt_geracao_w ||
			lpad(nr_remessa_w,6,'0') ||
			'087' ||
			'06250' || --OS 1504285, densidade do arquivo.
			rpad(' ',54,' ') ||
      			rpad('0',15,'0');
	...
	EXIT WHEN NOT FOUND; /* apply on c02 */
		...
		if (coalesce(qt_favorecido_w,0)	> 0) then
			...
			select	somente_numero(substr(obter_conta_pessoa(coalesce(a.cd_cgc,a.cd_pessoa_fisica),CASE WHEN coalesce(a.cd_cgc::text, '') = '' THEN 'F'  ELSE 'J' END ,'B'),1,3)) cd_banco,
				upper(lpad(substr(obter_conta_pessoa(coalesce(a.cd_cgc,a.cd_pessoa_fisica),CASE WHEN coalesce(a.cd_cgc::text, '') = '' THEN 'F'  ELSE 'J' END ,'A'),1,5) ||
				substr(coalesce(obter_conta_pessoa(coalesce(a.cd_cgc,a.cd_pessoa_fisica),CASE WHEN coalesce(a.cd_cgc::text, '') = '' THEN 'F'  ELSE 'J' END ,'DA'),'0'),1,1),6,'0')) cd_agencia_bancaria,
				...
				lpad(coalesce(b.nr_cpf,a.cd_cgc),14,'0') nr_inscricao
...
WHERE a.nr_titulo	= nr_titulo_w;
		...
		if (ie_forma_lanc_w not in ('03', '41', '43')) then
			cd_camara_w := '000';
		end if;

		ds_conteudo_w	:=	lpad(cd_banco_hosp_w,3,'0') ||
					...
					cd_camara_w || -- 18 20
					lpad(cd_banco_w,3,'0') ||
					lpad(cd_agencia_bancaria_w,6,'0') ||
					lpad(coalesce(substr(nr_conta_w,1,12), '0'),12,'0') ||
					ie_digito_conta_w ||
					' ' ||
					rpad(nm_pessoa_w,30,' ') ||
					rpad(nr_titulo_w,20,' ') ||
...
EXIT WHEN NOT FOUND; /* apply on c03 */
	...
	if (ie_forma_lanc_w	= '16') or (ie_forma_lanc_w	= '17') or (ie_forma_lanc_w	= '11') then
		...
		ds_espaco_trailer_w		:= rpad(' ',24,' ');

	else

		ds_espaco_trailer_w		:= rpad('0',24,'0');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_pagto_brasil_cnab_240 ( nr_seq_envio_p bigint, nm_usuario_p text) FROM PUBLIC;
