...
c_titulo CURSOR FOR
	...
	from	(
		SELECT t.nr_titulo nr_titulo,
			...
			coalesce(pls_obter_dados_pagador(s.nr_seq_pagador,'CPF'),pls_obter_dados_pagador(s.nr_seq_pagador,'CGC')) cd_cpf_cnpj,	
			substr(pls_obter_nomes_contrato(s.nr_seq_pagador,'P'),1,255) nm_usuario_principal,	
...
WHERE l.nr_sequencia		= m.nr_seq_lote and t.nr_titulo 		= q.nr_titulo and m.nr_sequencia 		= a.nr_seq_mensalidade and a.nr_sequencia 		= i.nr_seq_mensalidade_seg and m.nr_sequencia 		= t.nr_seq_mensalidade    and i.ie_tipo_item	not in ('6','3','7') and ((q.dt_recebimento >= l.dt_mesano_referencia and q.dt_recebimento between dt_inicial_w and dt_final_w)
		...
		or (coalesce(ie_tipo_segurado_w,'X') = 'X')) group by t.nr_titulo,
			t.dt_emissao,
			t.dt_vencimento,
			c.nr_contrato,
			c.dt_contrato,
...
union all
		...
		select 	t.nr_titulo nr_titulo,
			...
			substr(pls_obter_nomes_contrato(v.nr_seq_pagador,'P'),1,255) nm_usuario_principal,	
			v.cd_beneficiario,
		...
		group by v.dt_emissao,
			...
			substr(pls_obter_nomes_contrato(v.nr_seq_pagador,'P'),1,255),	
			v.cd_beneficiario,
			...
			v.nr_seq_congenere,
			v.dt_mesano_referencia) alias369
	where	1 = 1
	group by nr_titulo,
		dt_emissao,
		dt_vencimento,
...
if (ie_gerar_arquivo_p = 'S') then
	...
	utl_file.put_line(arq_texto_w, substr(obter_desc_expressao(962596), 1, 4000));
