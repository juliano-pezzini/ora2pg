...
CREATE OR REPLACE PROCEDURE integra_evento_ura ( nr_seq_interno_p bigint, cd_usuario_p text, cd_status_p text, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
cd_perfil_inicial_w		bigint;
ie_user_default_w		varchar(10);
...
select	max(a.nm_usuario),
	...
	max(a.cd_perfil_inicial)
into STRICT	nm_usuario_w,
	cd_pessoa_fisica_w,
	cd_perfil_inicial_w
from	usuario a
where 	a.cd_barras = cd_usuario_p;

ie_verificar_pac_leito_w := obter_param_usuario(3111, 266, cd_perfil_inicial_w, nm_usuario_w, cd_estabelecimento_p, ie_verificar_pac_leito_w);
ie_aguarda_hig_w := obter_param_usuario(44, 145, cd_perfil_inicial_w, nm_usuario_w, cd_estabelecimento_p, ie_aguarda_hig_w);
ie_user_default_w := obter_param_usuario(75, 117, cd_perfil_inicial_w, nm_usuario_w, cd_estabelecimento_p, ie_user_default_w);

if (coalesce(ie_user_default_w,'T') = 'D') then
	nm_usuario_w := cd_usuario_p;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE integra_evento_ura ( nr_seq_interno_p bigint, cd_usuario_p text, cd_status_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
