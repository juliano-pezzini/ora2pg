...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_fatura_v40 ( nr_seq_fatura_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR 
	SELECT	lpad(coalesce(elimina_caractere_especial(to_char(nr_lote)),'0'),8,'0'), 
		lpad(coalesce(substr(elimina_caractere_especial(to_char(pls_obter_dados_conta(nr_nota,'G'))),1,11),'0'),11,'0'), 
		lpad(coalesce(cd_unimed,'0'),4,'0'), 
		lpad(coalesce(cd_usuario_plano,'0'),13,'0'), 
		rpad(coalesce(elimina_acentuacao(nm_beneficiario),' '),25,' '), 
		rpad(coalesce(to_char(dt_atendimento,'yyyy/mm/ddhh:mi:ss') || '-30',' '),21,' '), 
		...
		lpad(coalesce(to_char(nr_guia_principal),'0'),11,'0'), 
		nr_sequencia 
...
C02 CURSOR FOR 
	SELECT	lpad(elimina_caractere_especial(a.nr_lote),8,'0') nr_lote_503, 
		lpad(substr(elimina_caractere_especial(to_char(pls_obter_dados_conta(a.nr_nota,'G'))),1,11),11,'0')	nr_nota_503, 
		lpad(a.cd_unimed_hospital,4,'0'), 
		lpad(a.cd_hospital,8,'0'), 
		rpad(a.nm_hospital,25,' '), 
		rpad(a.ie_tipo_acomodacao,2,' '), 
		to_char(a.dt_internacao,'yyyy/mm/ddhh:mi:ss') || '-30', 
		to_char(a.dt_alta,'yyyy/mm/ddhh:mi:ss') || '-30', 
		lpad(replace(replace(campo_mascara(a.tx_mult_amb,2),',',''),'.',''),4,'0'), 
		lpad(a.ie_ind_acidente,1,' '), 
		rpad(a.cd_motivo_saida,2,' '), 
		lpad(a.cd_cgc_hospital,14,' '), 
		lpad(a.ie_tipo_internacao,1,'0'), 
...
C03 CURSOR FOR 
	SELECT	lpad(coalesce(elimina_caractere_especial(a.nr_lote),'0'),8,'0'), 
		lpad(coalesce(substr(elimina_caractere_especial(to_char(pls_obter_dados_conta(a.nr_nota,'G'))),1,11),'0'),11,'0'), 
	...
	C05 CURSOR FOR 
		SELECT	rpad(coalesce(a.nr_nota_fiscal,'0'),20,' '), 
			rpad(coalesce(a.ds_link_nfe,' '),120,' ') 
	...
	C08 CURSOR FOR 
		SELECT	rpad(coalesce(cd_banc,' '),6,' '), 
			...
			rpad(coalesce(cd_especie_doc,' '),8,' '), 
...
EXIT WHEN NOT FOUND; /* apply on C00 */
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		EXIT WHEN NOT FOUND; /* apply on C02 */
			...
			ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '503' || nr_lote_503_w || nr_nota_503_w || 
					...
					nr_declara_vivo_1_w || nr_declara_vivo_2_w || nr_declara_vivo_3_w || nr_declara_vivo_4_w || 
					nr_declara_vivo_5_w || ie_obito_mulher_w || nr_declara_obito_w || cd_cid_obito_1_w || cd_cid_obito_2_w || 
					cd_cid_obito_3_w || cd_cid_obito_4_w || cd_cid_obito_5_w || nr_declara_obito_1_w || nr_declara_obito_2_w || 
					nr_declara_obito_3_w || nr_declara_obito_4_w || nr_declara_obito_5_w;
		...
		EXIT WHEN NOT FOUND; /* apply on C03 */
			...
			ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '504' || nr_lote_504_w || nr_nota_504_w || 
					...
					cd_pacote_w || lpad(nr_nota_504_w,20,'0') || tp_autoriz_w ||ds_servico_w;
	...
	EXIT WHEN NOT FOUND; /* apply on C07 */
		...
		ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '511' || nr_linha_w || ds_linha_w || vl_item_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_fatura_v40 ( nr_seq_fatura_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
