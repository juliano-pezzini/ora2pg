...
CREATE OR REPLACE PROCEDURE gerar_retorno_caixa_400 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
dt_liquidacao2_w		timestamp;

c01 CURSOR FOR
	SELECT	trim(both substr(ds_string,59,15)),
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_titulo_w IS NOT NULL AND nr_titulo_w::text <> '') then
		...
		/*OS 1667846 - Quando for arquivo de Confirmação de Entrada, não tem data de liquidação, é 000000 no arquivo.*/

		if (dt_liquidacao_w = '000000') then
			dt_liquidacao2_w := null;
		else
			dt_liquidacao2_w := coalesce(to_Date(dt_liquidacao_w,'dd/mm/yy'),clock_timestamp());
		end if;
		...
		CALL fin_gerar_log_controle_banco(3,substr(obter_desc_expressao(344512)/*'Não foi importado o título ' || ds_titulo_w || ', pois não foi encontrado no Tasy'*/
,1,4000),nm_usuario_p,'N');
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_retorno_caixa_400 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
