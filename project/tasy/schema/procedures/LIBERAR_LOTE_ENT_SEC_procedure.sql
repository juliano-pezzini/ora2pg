...
CREATE OR REPLACE PROCEDURE liberar_lote_ent_sec (nr_seq_lote_sec_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
cd_setor_atend_ww		varchar(5);
cd_setor_coleta_w		varchar(5);
cd_setor_entrega_w		varchar(5);
...
ds_ficha_sem_medicacao_w	varchar(255);
vl_parametro_p			varchar(1);

--variavel que verifica ficha sem exame
ds_ficha_sem_exame_w		varchar(255);
...
C01 CURSOR FOR
	SELECT	nr_sequencia,
		cd_pessoa_fisica,
		cd_medico_resp,
		coalesce(to_date(to_char(coalesce(DT_COLETA_FICHA_F,clock_timestamp()), 'dd/mm/yyyy')   ||    to_char(HR_COLETA_F, 'hh24:mi'),'dd/mm/yyyy hh24:mi'),clock_timestamp()),
		ie_susp_am_inad
...
C03 CURSOR FOR
	...
	and 	not exists (SELECT	1
			   ...
			   and		trunc(coalesce(dt_inicio,clock_timestamp())) between trunc(clock_timestamp() - interval '15 days') and trunc(clock_timestamp()));
...
vl_parametro_p := Obter_Param_Usuario(10060, 68, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_p, vl_parametro_p);
...
if (nr_seq_lote_sec_p IS NOT NULL AND nr_seq_lote_sec_p::text <> '') then
	...
	--trata parametro 68
		...
		if ((ds_ficha_sem_medicacao_w IS NOT NULL AND ds_ficha_sem_medicacao_w::text <> '') or ds_ficha_sem_medicacao_w <> '') then
			ds_ficha_sem_medicacao_w := substr(ds_ficha_sem_medicacao_w,3,255);
			CALL wheb_mensagem_pck.exibir_mensagem_abort(285082, 'DS_FICHAS='||ds_ficha_sem_medicacao_w);
		end if;
	end if;
	...
	if ((ds_ficha_sem_exame_w IS NOT NULL AND ds_ficha_sem_exame_w::text <> '') or ds_ficha_sem_exame_w <> '') then
		ds_ficha_sem_exame_w := substr(ds_ficha_sem_exame_w,3,255);
		CALL wheb_mensagem_pck.exibir_mensagem_abort(285132, 'DS_FICHAS='||ds_ficha_sem_exame_w);
	end if;
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		and ((a.nr_seq_pacote = c.nr_seq_pacote and a.cd_pacote_ext = c.cd_pacote_ext) OR (coalesce(a.cd_pacote_ext::text, '') = '' AND coalesce(a.nr_seq_pacote::text, '') = ''))
		...
		if (nr_atend_ficha_w = 0 AND nr_prescr_ficha_w = 0) then
			...
			insert into atendimento_paciente(
				...
				'S',
				dt_entrada_w,
				coalesce(ie_tipo_atendimento_w, 7),
				cd_procedencia_w,
				cd_pessoa_fisica_resp_w,
				cd_pessoa_fisica_w,
			...
			EXIT WHEN NOT FOUND; /* apply on C02 */
				...
				*/
				...
				SELECT * FROM obter_setor_exame_lab(	nr_prescricao_w, nr_seq_exame_w, cd_setor_atendimento_w, cd_material_exame_w, coalesce(dt_coleta_w,clock_timestamp()), 'S', cd_setor_atend_ww, cd_setor_coleta_w, cd_setor_entrega_w, qt_dia_entrega_w, ie_emite_mapa_w, ds_hora_fixa_w, ie_data_resultado_w, qt_min_entrega_w, ie_recoleta_w, ie_ficha_urg_w, ie_dia_semana_final_w, ie_forma_atual_dt_result_w, qt_min_atraso_w) INTO STRICT cd_setor_atend_ww, cd_setor_coleta_w, cd_setor_entrega_w, qt_dia_entrega_w, ie_emite_mapa_w, ds_hora_fixa_w, ie_data_resultado_w, qt_min_entrega_w, ie_recoleta_w, ie_dia_semana_final_w, ie_forma_atual_dt_result_w, qt_min_atraso_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE liberar_lote_ent_sec (nr_seq_lote_sec_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
