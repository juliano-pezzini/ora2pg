...
cd_unidade_medida_compra_w	varchar(5);
cd_unidade_medida_estoque_w	varchar(5);
...
c02 CURSOR FOR
	SELECT	a.cd_cgc,
		a.cd_pessoa_fisica,
		(obter_dados_pf_pj_estab(cd_estabelecimento_p ,null,a.cd_cgc,'ECP'))::numeric  cd_condicao_pagamento,
		a.cd_material,
...
;
	...
	EXIT WHEN NOT FOUND; /* apply on C02 */
		...
		if (coalesce(cd_condicao_pagamento_w::text, '') = '') then
			CALL wheb_mensagem_pck.exibir_mensagem_abort(266004);
			--'Nao encontrada nenhuma condicao de pagamento para geracao da Nota Fiscal.' || chr(13) || chr(10) ||
		...
		if (qt_existe_w = 0) then
			...
			CALL consiste_nota_mesmo_numero(cd_estabelecimento_p, cd_cgc_emitente_w,cd_pessoa_fisica_w,cd_serie_nf_p,nr_nota_fiscal_p,nm_usuario_p);
			...
			values (	cd_estabelecimento_p,			cd_cgc_emitente_w,
				...
				wheb_mensagem_pck.get_Texto(310507, 'NR_SEQ_INSPECAO_P='|| NR_SEQ_INSPECAO_P),	nr_danfe_p);
				/*Nota gerada pela inspecao recebimento: #@NR_SEQ_INSPECAO_P#@*/
		...
		nr_seq_conta_financeira_w := obter_conta_financeira(	'S', cd_estabelecimento_p, cd_material_w, null, null, null, null, cd_cgc_emitente_w, cd_centro_custo_w, nr_seq_conta_financeira_w, null, cd_operacao_nota_p, null, null, null, null, null, null, null, null, null, null, cd_local_estoque_w, null, null, null, null, null, null);
		...
		SELECT * FROM define_conta_material(
				cd_estabelecimento_p, cd_material_w, ie_tipo_conta_w, 0, 0, 0, 0, 0, 0, 0, cd_local_estoque_w, cd_operacao_nota_p, trunc(clock_timestamp()), cd_conta_contabil_ww, cd_centro_conta_w, null) INTO STRICT cd_conta_contabil_ww, cd_centro_conta_w;
		...
		values (	cd_estabelecimento_p,			coalesce(cd_cgc_emitente_w,cd_cgc_estabelecimento_w),
			cd_serie_nf_p,				nr_nota_fiscal_p,
			nr_sequencia_nf_w,			nr_item_nf_w,
			cd_natureza_operacao_w,			qt_prevista_entrega_w,
			vl_unitario_item_nf_w,			(qt_prevista_entrega_w * vl_unitario_item_nf_w),
			clock_timestamp(),					nm_usuario_p,
			0,					0,
			0,					cd_material_w,
			cd_local_estoque_w,			wheb_mensagem_pck.get_Texto(310507, 'NR_SEQ_INSPECAO_P='|| NR_SEQ_INSPECAO_P),/*Nota gerada pela inspecao recebimento: #@NR_SEQ_INSPECAO_P#@*/
...
CALL gerar_historico_nota_fiscal(nr_sequencia_p, nm_usuario_p, '19', wheb_mensagem_pck.get_Texto(310507, 'NR_SEQ_INSPECAO_P='|| NR_SEQ_INSPECAO_P));
								/*Nota gerada pela inspecao recebimento: #@NR_SEQ_INSPECAO_P#@*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_nf_contrato_inspecao ( nr_seq_inspecao_p bigint, nr_nota_fiscal_p bigint, cd_serie_nf_P text, cd_operacao_nota_p bigint, cd_natureza_operacao_p bigint, cd_condicao_pagto_p bigint, dt_emissao_p timestamp, cd_setor_digitacao_p bigint, cd_local_estoque_p bigint, nr_sequencia_p INOUT bigint, nm_usuario_p text, nr_danfe_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
