...
CREATE OR REPLACE PROCEDURE pls_lag_consistir_idade_sexo ( nr_seq_lote_guia_imp_p bigint, nr_seq_lote_proc_imp_p bigint, nr_seq_guia_plano_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (coalesce(nr_seq_guia_plano_p,0) <> 0) then
	...
	where	nr_sequencia = nr_seq_guia_plano_p;
		...
		if (qt_sexo_permitido_w = 0) and (ie_tipo_segurado_w not in ('I','H')) then
			CALL pls_inserir_anexo_glosa_aut('1802', nr_seq_lote_guia_imp_p, nr_seq_lote_proc_imp_p, null, '', nm_usuario_p);
		end if;
		...
		if (qt_idade_permitida_w = 0) and (ie_tipo_segurado_w not in ('I','H')) then
			CALL pls_inserir_anexo_glosa_aut('1803', nr_seq_lote_guia_imp_p, nr_seq_lote_proc_imp_p, null, '', nm_usuario_p);
		end if;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_lag_consistir_idade_sexo ( nr_seq_lote_guia_imp_p bigint, nr_seq_lote_proc_imp_p bigint, nr_seq_guia_plano_p bigint, nm_usuario_p text) FROM PUBLIC;
