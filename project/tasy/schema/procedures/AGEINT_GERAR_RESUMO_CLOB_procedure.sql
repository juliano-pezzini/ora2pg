...
CREATE OR REPLACE PROCEDURE ageint_gerar_resumo_clob ( nr_seq_ageint_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) is ie_tipo_resumo_w varchar(15) RETURNS bigint AS $body$
DECLARE
...
UNION ALL
	...
	IF (obter_se_contido(i.nr_row, lista_orient_w) = 'N') THEN
        return_w := return_w || obter_texto_macro_prep_ageint2(i.nr_row, i.nr_seq_item, i.ds_orientacao_preparo, 'N') || ds_separador_p;
        nr_row := i.nr_row;
...
UNION ALL
    ...
    IF (obter_se_contido(i.nr_row, lista_orient_w) = 'N') THEN
        ...
        END IF;
        return_w := return_w || obter_texto_macro_prep_ageint2(i.nr_row, i.nr_seq_item, i.ds_orientacao_preparo) || ds_separador_p;
        nr_row := i.nr_row;
...
UNION ALL
    ...
    IF nr_row  <> i.nr_row THEN
        return_w := return_w || obter_texto_macro_prep_ageint2(i.nr_row, nr_seq_item_p, i.ds_orientacao_preparo) || ds_separador_p;
        nr_row := i.nr_row;
    END IF;
...
begin
  ...
  if ie_local_chamada_p in ('C', 'T') then
    ...
    ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@paciente', nm_paciente_w);
    ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@inicioAgendamento',  dt_inicio_agendamento_w);
    ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@convenio', ds_convenio_w);
    ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@nascimento', pkg_date_formaters.to_varchar(date_p => dt_nascimento_w, ds_mask => 'shortDate', cd_estabelcimento_p => cd_estabelecimento_p, nm_usuario_p => nm_usuario_p));
    ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@categoria', ds_categoria_w);
    ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@plano', ds_plano_w);
    ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@telefone', nr_telefone_w);
    ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@quebra', chr(10));
    ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@observacao', ds_observacao_w);
    ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@checklist', ds_item_checklist_w);
    ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@usuario', cd_pessoa_fisica_w);
    ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@senha', ds_senha_w);
    ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@protocoloagenda', to_char(nr_seq_ageint_p));
    ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@menordtchegada', pkg_date_formaters.to_varchar(date_p => dt_menor_agendamento_w, ds_mask => 'short', cd_estabelcimento_p => cd_estabelecimento_p, nm_usuario_p => nm_usuario_p));
	ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@valorexamelab', '
'||vl_total_exames_w);

    if (dt_entrega_item_w IS NOT NULL AND dt_entrega_item_w::text <> '') then
      ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@dtentrega', Wheb_mensagem_pck.get_texto(307471)||': ' /*'Data de entrega do laudo: '*/||pkg_date_formaters.to_varchar(date_p => dt_entrega_item_w, ds_mask => 'short', cd_estabelcimento_p => cd_estabelecimento_p, nm_usuario_p => nm_usuario_p));
    else
      ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@dtentrega', null);
    end if;

    if (ds_forma_entrega_laudo_w IS NOT NULL AND ds_forma_entrega_laudo_w::text <> '') then
      ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@formaentrega' , Wheb_mensagem_pck.get_texto(307472)||': ' /*'Forma de entrega do laudo: '*/||ds_forma_entrega_laudo_w);
    else
      ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@formaentrega' , null);
    end if;

    ds_texto_replace_w    := replace_macro_clob2(ds_texto_replace_w, '@maiorjejum', ds_tempo_max_jejum_w);
    ds_texto_replace_w    := replace_macro_clob2(ds_texto_replace_w, '@ds_end_estab_logado', ds_end_estab_logado_w);
    ds_texto_replace_w    := replace_macro_clob2(ds_texto_replace_w, '@ds_estab_logado', ds_estab_logado_w);

    if (ie_orientacao_ageint_item_w = 'S') THEN
      ds_texto_replace_w := replace_macro_clob2(ds_texto_replace_w, '@OrientacaoPreparoAgeintResumido', obter_orient_preparo_ageint(nr_seq_ageint_p, cd_estabelecimento_p, ds_separador_w, 'P'));
      ds_texto_replace_w := replace_macro_clob2(ds_texto_replace_w, '@OrientConvResumido', obter_orient_preparo_ageint(nr_seq_ageint_p, cd_estabelecimento_p, ds_separador_w, 'C'));
    else
      ds_texto_replace_w := replace_macro_clob2(ds_texto_replace_w, '@OrientacaoPreparoAgeintResumido', null);
      ds_texto_replace_w := replace_macro_clob2(ds_texto_replace_w, '@OrientConvResumido', null);
    end if;

    ds_texto_replace_w := replace_macro_clob2(ds_texto_replace_w, '@OrientAgeIntResumoDia', obter_ori_pre_ageint_dia(nr_seq_ageint_p, cd_estabelecimento_p, ds_separador_w, 'P'));
    ds_texto_replace_w := replace_macro_clob2(ds_texto_replace_w, '@OrientConvResumido', obter_orient_preparo_ageint(nr_seq_ageint_p, cd_estabelecimento_p, ds_separador_w, 'C'));
    ds_texto_replace_w := replace_macro_clob2(ds_texto_replace_w, '@prontuario', to_char(nr_prontuario_w));
    ds_texto_replace_w := (replace_macro_clob2(ds_texto_replace_w, '@orientlab', substr(ds_orient_lab_macro_w,1, 32000 - length(ds_texto_replace_w))));
...
'||vl_total_exames_w);
    ...
    if (dt_entrega_item_w IS NOT NULL AND dt_entrega_item_w::text <> '') then
      ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@dtentrega', Wheb_mensagem_pck.get_texto(307471)||': ' /*'Data de entrega do laudo: '*/||to_char(dt_entrega_item_w, pkg_date_formaters.localize_mask('short', pkg_date_formaters.getUserLanguageTag(cd_estabelecimento_p, nm_usuario_p))));
    else
      ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@dtentrega', null);
    end if;
    ...
    if (ie_rodape_setor_w  = 'S') and (ds_texto_replace_w like '%@textopadraosetor%') then
      ds_texto_replace_w  := ds_Texto_setor_w;
      ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@usuario', cd_pessoa_fisica_w);
      ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@senha', ds_senha_w);
      ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@telefone', nr_telefone_w);
      ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@ds_end_estab_logado', ds_end_estab_logado_w);
    ...
    ds_texto_replace_w  := replace_macro_clob2(ds_texto_replace_w, '@textopadraosetor', null);
  end if;
...
if (nr_seq_ageint_p IS NOT NULL AND nr_seq_ageint_p::text <> '') then
  ...
  select  a.nm_paciente,
    ...
    Ageint_obter_checklist(a.nr_sequencia, ds_separador_w),
    a.cd_categoria,
    a.cd_plano,
    b.cd_pessoa_fisica,
    b.ds_senha,
          ...
          if (coalesce(nr_seq_pend_quimio_w::text, '') = '') then
            ...
            if (ie_tipo_agendamento_w = 'Q') then
              ...
              close C05;
                ...
                ds_item_w  := replace_macro_clob2(ds_item_w, '@agenda', ds_agenda_w);
                if (ie_tipo_agendamento_w = 'C') then
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@telefone', nr_telefone_w);
                else
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@telefone', '');
                end if;
                ds_item_w  := replace_macro_clob2(ds_item_w, '@horario', to_char(dt_agendamento_w, pkg_date_formaters.localize_mask('short', pkg_date_formaters.getUserLanguageTag(cd_estabelecimento_p, nm_usuario_p))));
                ds_item_w  := replace_macro_clob2(ds_item_w, '@respautor', ds_autorizacao_w);
                ds_item_w  := replace_macro_clob2(ds_item_w, '@dtagenda', to_char(dt_agenda_w, pkg_date_formaters.localize_mask('shortDate', pkg_date_formaters.getUserLanguageTag(cd_estabelecimento_p, nm_usuario_p))));
                ds_item_w  := replace_macro_clob2(ds_item_w, '@horagenda', to_char(dt_agenda_w, pkg_date_formaters.localize_mask('shortTime', pkg_date_formaters.getUserLanguageTag(cd_estabelecimento_p, nm_usuario_p))));
                ds_item_w  := replace_macro_clob2(ds_item_w, '@diasemana', ds_dia_semana_w);
                ds_item_w  := replace_macro_clob2(ds_item_w, '@medreq', nm_medico_req_w);

                ds_item_w  := replace_macro_clob2(ds_item_w, '@medico', nm_medico_exec_w);
                ds_item_w  := replace_macro_clob2(ds_item_w, '@procinterno', ''||nr_seq_proc_interno_w);
            /* NAO ALTERAR  OS ESPACAMENTOS */

            ds_item_w  := replace_macro_clob2(ds_item_w, '@orientpac', replace(ds_orient_pac_w,'
',ds_separador_w));--NAO ALTERAR ESSE ESPAcAMENTO
            ds_item_w  := replace_macro_clob2(ds_item_w, '@orientexame', replace(coalesce(ds_orient_proc_w,''),'
',ds_separador_w)); --NAO ALTERAR ESSE ESPAcAMENTO
            ds_item_w  := replace_macro_clob2(ds_item_w, '@orientanestesia', replace(ds_orient_anest_w,'
',ds_separador_w)); --NAO ALTERAR ESSE ESPAcAMENTO
                ds_item_w  := replace_macro_clob2(ds_item_w, '@item', ds_item_Ageint_w);
                ds_item_w  := replace_macro_clob2(ds_item_w, '@valor', ''||vl_item_w);
                ds_item_w  := replace_macro_clob2(ds_item_w, '@regra', ds_Regra_w);
                ds_item_w  := replace_macro_clob2(ds_item_w, '@glosa', ds_glosa_w);
                ds_item_w  := replace_macro_clob2(ds_item_w, '@dtprevista', ''||dt_entrega_prevista_w);
                ds_item_w  := replace_macro_clob2(ds_item_w, '@obsagenda', ds_obs_agenda_w);
                ds_item_w  := replace_macro_clob2(ds_item_w, '@sala', ds_sala_consulta_w);
                ds_item_w  := replace_macro_clob2(ds_item_w, '@tipoagenda', ds_tipo_agenda_w);
                ds_item_w  := replace_macro_clob2(ds_item_w, '@textopadraosetor', replace(ds_texto_padrao_setor_w,'
',ds_separador_w));--NAO ALTERAR ESSE ESPAcAMENTO)
                ds_item_w  := replace_macro_clob2(ds_item_w, '@textopadraoexame', replace(ds_texto_padrao_exame_w,'
',ds_separador_w));--NAO ALTERAR ESSE ESPAcAMENTO)
                ds_item_w  := replace_macro_clob2(ds_item_w, '@dtchegada', to_char(dt_agendamento_w - nr_min_chegada_prev_w/1440, pkg_date_formaters.localize_mask('timestamp', pkg_date_formaters.getUserLanguageTag(cd_estabelecimento_p, nm_usuario_p))));
                ds_item_w  := replace_macro_clob2(ds_item_w, '@reserva', nr_reserva_w);
                ds_item_w  := replace_macro_clob2(ds_item_w, '@protocoloagenda', ''||nr_seq_ageint_p);
                ds_item_w  := replace_macro_clob2(ds_item_w, '@lado', ie_lado_w);
                ds_item_w  := replace_macro_clob2(ds_item_w, '@inicioAgendamento',  dt_inicio_agendamento_w);

                ds_item_w := replace_macro_clob2(ds_item_w, '@OrientAgenda',    ds_orientagenda_w);
                ds_item_w := replace_macro_clob2(ds_item_w, '@EndAgenda',       ds_endagenda_w);

            ds_item_w  := replace_macro_clob2(ds_item_w, '@orientpreparo', replace(ds_orient_prep_proc_w,'
',ds_separador_w)); --NAO ALTERAR ESSE ESPAcAMENTO
                if (ds_quimio_item_ww IS NOT NULL AND ds_quimio_item_ww::text <> '') then
                  ds_item_w    := replace_macro_clob2(ds_item_w, '@quimio', ds_quimio_item_ww);
                  ds_quimio_item_ww   := '';
                else
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@quimio', '');
                end if;

                if (ds_quimio_sem_pro_ww IS NOT NULL AND ds_quimio_sem_pro_ww::text <> '') then
                  ds_item_w    := replace_macro_clob2(ds_item_w, '@onc_item', ds_quimio_sem_pro_ww);
                  ds_quimio_sem_pro_ww   := '';
                else
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@onc_item', '');
                end if;

                if (ie_encaixe_w = 'S') then
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@msgencaixe', ds_msg_encaixe_w);
                else
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@msgencaixe', '');
                end if;
                ...
                if (nr_seq_item_princ_w IS NOT NULL AND nr_seq_item_princ_w::text <> '') and (trunc(dt_recorrencia_w) = trunc(dt_agenda_w)) then
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@textopriexame', replace(ds_texto_pri_exame_w,'
',ds_separador_w));--NAO ALTERAR ESSE ESPAcAMENTO
                elsif (coalesce(nr_seq_item_princ_w::text, '') = '') and (coalesce(dt_prevista_item_w::text, '') = '') then
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@textopriexame', replace(ds_texto_pri_exame_w,'
',ds_separador_w));--NAO ALTERAR ESSE ESPAcAMENTO
                else
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@textopriexame', '');
                end if;

                ---Richart---
                if (Format_obs_prescr_w IS NOT NULL AND Format_obs_prescr_w::text <> '') then
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@FormatacaoObsPrescr', Format_obs_prescr_w);
                  Format_obs_prescr_w   := '';
                else
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@FormatacaoObsPrescr', '');
                end if;

                if (Format_obs_prescr_estab_age_w IS NOT NULL AND Format_obs_prescr_estab_age_w::text <> '') then
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@FormatObsPrescrEstabAge', Format_obs_prescr_estab_age_w);
                  Format_obs_prescr_estab_age_w   := '';
                else
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@FormatObsPrescrEstabAge', '');
                end if;

                if (Format_obs_pac_w IS NOT NULL AND Format_obs_pac_w::text <> '') then
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@FormatacaoOrientPac', Format_obs_pac_w);
                  Format_obs_pac_w   := '';
                else
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@FormatacaoOrientPac', '');
                end if;

                if (Format_obs_pac_estab_age_w IS NOT NULL AND Format_obs_pac_estab_age_w::text <> '') then
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@FormatOrientPacEstabAge', Format_obs_pac_estab_age_w);
                  Format_obs_pac_estab_age_w   := '';
                else
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@FormatOrientPacEstabAge', '');
                end if;

                if (Format_orient_pre_w IS NOT NULL AND Format_orient_pre_w::text <> '') then
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@FormatacaoOrientPre', Format_orient_pre_w);
                  Format_orient_pre_w   := '';
                else
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@FormatacaoOrientPre', '');
                end if;

                if (Format_orient_pre_estab_age_w IS NOT NULL AND Format_orient_pre_estab_age_w::text <> '') then
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@FormatOrientPreEstabAge', Format_orient_pre_estab_age_w);
                  Format_orient_pre_estab_age_w   := '';
                else
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@FormatOrientPreEstabAge', '');
                end if;

                if (Format_orient_pac_Ext_w IS NOT NULL AND Format_orient_pac_Ext_w::text <> '') then
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@FormatacaoOrientExtPre', Format_orient_pac_Ext_w);
                  Format_orient_pac_Ext_w  := '';
                else
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@FormatacaoOrientExtPre', '');
                end if;

                if (Format_orient_pac_Ext_est_ag_w IS NOT NULL AND Format_orient_pac_Ext_est_ag_w::text <> '') then
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@FormatOrientExtPreEstabAg', Format_orient_pac_Ext_est_ag_w);
                  Format_orient_pac_Ext_est_ag_w  := '';
                else
                  ds_item_w  := replace_macro_clob2(ds_item_w, '@FormatOrientExtPreEstabAg', '');
                end if;

                ds_item_w  := replace_macro_clob2(ds_item_w, '@ds_estab_agenda', ds_estab_agenda_w);
                ds_item_w  := replace_macro_clob2(ds_item_w, '@ds_end_estab_agenda', ds_end_estab_agenda_w);
                ds_item_w := replace_macro_clob2(ds_item_w, '@OrientacaoPreparoAgeintItem',    obter_orient_preparo_item(nr_seq_proc_interno_w, cd_estabelecimento_w, ds_separador_w, nr_seq_topografia_w, ie_tipo_atendimento_w, nr_seq_item_w));
                ds_item_w := replace_macro_clob2(ds_item_w, '@TempoJejumProc',   ''||obter_tempo_jejum_item(nr_seq_item_w));
		ds_item_w := replace_macro_clob2(ds_item_w, '@MaiorTempoJejumProc',    ''||obter_tempo_jejum_ageint(nr_seq_ageint_p, trunc(dt_agenda_w)));
                ...
                ds_item_w := replace_macro_clob2(ds_item_w, '@setor_executor',   ds_setor_executor_w);
                ...
                from   agenda_integrada a
                where   a.nr_sequencia = nr_seq_ageint_p;
                ds_item_w := replace_macro_clob2(ds_item_w, '@prontuario',   ''||nr_prontuario_w);
            ...
            ds_item_w  := replace_macro_clob2(ds_item_w, '@agenda', ds_agenda_w);
            ds_item_w  := replace_macro_clob2(ds_item_w, '@horario', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@dtagenda', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@horagenda', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@telefone', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@medreq', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@medico', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@orientpac', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@orientexame', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@orientanestesia', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@item', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@valor', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@regra', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@glosa', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@dtprevista', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@obsagenda', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@textopadraosetor', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@textopadraoexame', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@dtchegada', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@reserva', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@protocoloagenda', ''||nr_seq_ageint_p);
            ds_item_w  := replace_macro_clob2(ds_item_w, '@orientpreparo', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@msgencaixe', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@lado', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@textopriexame', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@sala', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@tipoagenda', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@procinterno', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@ds_estab_agenda', '');
            ds_item_w  := replace_macro_clob2(ds_item_w, '@ds_end_estab_agenda', '');
            ds_item_w   := replace_macro_clob2(ds_item_w, '@setor_executor',   '');
            ds_item_w   := replace_macro_clob2(ds_item_w, '@prontuario',   '');
            ds_item_w   := replace_macro_clob2(ds_item_w, '@TempoJejumProc', '' );
            ...
            ds_item_w  := replace_macro_clob2(ds_item_w, '@quimio', ds_quimio_ww);
            ds_item_w  := replace_macro_clob2(ds_item_w, '@onc_item', ds_quimio_sem_profis_ww);
          ...
          ds_item_w  := replace_macro_clob2(ds_item_w, '@orientacaoexameadic', Replace_Clob2(ds_orient_exame_adic_ww,'
',ds_separador_w));--NAO ALTERAR ESSE ESPAcAMENTO
          ds_item_w  := replace_macro_clob2(ds_item_w, '@orientacaocomexameadic', Replace_Clob2(ds_orient_com_exame_ww,'
',ds_separador_w));--NAO ALTERAR ESSE ESPAcAMENTO
          ds_item_w  := replace_macro_clob2(ds_item_w, '@exameadicional', ds_exame_adic_ww);
	        ds_item_w  := replace_macro_clob2(ds_item_w, '@URL_TELEMEDICINA', ds_url_telemedicina_w);
          ds_item_w  := replace_macro_clob2(ds_item_w, '@urlTelemedicinaExterno', ds_url_tele_terc_w);
          ds_item_w  := replace_macro_clob2(ds_item_w, '@usuario', cd_pessoa_fisica_w);
          ds_item_w  := replace_macro_clob2(ds_item_w, '@senha', ds_senha_w);
          ds_item_w  := replace_macro_clob2(ds_item_w, '@ds_estab_agenda', ds_estab_agenda_w);
          ds_item_w  := replace_macro_clob2(ds_item_w, '@ds_end_estab_agenda', ds_end_estab_agenda_w);
          ...
          --while indice_w <= qt_posicoes_w loop
            --ds_texto_pos_w := DBMS_LOB.SUBSTR(ds_item_w,qt_tamanho_cont_w,pos_ini_w);
            inserir_resumo_item(nr_seq_ageint_p, ds_item_w, nm_usuario_p);
  ...
  if (ie_utiliza_regra_texto_w = 'S') then
    ...
    EXIT WHEN NOT FOUND; /* apply on C02 */
      ...
      if (nr_seq_regra_texto_proc_w IS NOT NULL AND nr_seq_regra_texto_proc_w::text <> '') then
        ...
        ds_regra_texto_proc_w  := replace_macro_clob2(ds_regra_texto_proc_w, '@regratextoproced', ds_orient_regra_texto_proc_w);
        if (position('@titulo' in ds_regra_texto_proc_w) > 0) then
          ds_regra_texto_proc_w  := replace_macro_clob2(ds_regra_texto_proc_w, '@titulo', ds_titulo_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ageint_gerar_resumo_clob ( nr_seq_ageint_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) is ie_tipo_resumo_w varchar(15) FROM PUBLIC;
