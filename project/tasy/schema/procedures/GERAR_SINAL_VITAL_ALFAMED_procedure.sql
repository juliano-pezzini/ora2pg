...
CREATE OR REPLACE PROCEDURE gerar_sinal_vital_alfamed (cd_pessoa_fisica_p text, dt_sinal_vital_p text, ds_lista_parametros_p text, nm_usuario_p text) AS $body$
DECLARE
...
if (nr_atendimento_w IS NOT NULL AND nr_atendimento_w::text <> '') and (qt_reg_w	= 0) and (coalesce(dt_alta_w::text, '') = '')	and (ie_monitorado_w = 'S')then
	for j in 1..Vetor_w.count loop
		...
		ds_lista_aux_w	:= substr(ds_lista_aux_w,position('#@#@' in ds_lista_aux_w)+8,40000);
    ...
    elsif (Vetor_w[i].nm_sinal_vital	= 'PVCs') then
    ...
    elsif (Vetor_w[i].nm_sinal_vital	= 'CO2_AwRR') then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_sinal_vital_alfamed (cd_pessoa_fisica_p text, dt_sinal_vital_p text, ds_lista_parametros_p text, nm_usuario_p text) FROM PUBLIC;
