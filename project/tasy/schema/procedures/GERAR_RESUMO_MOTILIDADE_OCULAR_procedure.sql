...
CREATE OR REPLACE PROCEDURE gerar_resumo_motilidade_ocular (cd_estabelecimento_p bigint, nr_seq_consulta_p bigint, cd_pessoa_fisica_p text, ie_mostra_todos_p text, nr_seq_item_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	ds_motilidade_ocupar_w	:= 	upper(expressao1_w) || ' ' || dt_registro_w || ds_espaco_20_w || 
						upper(expressao2_w) || ' ' ||nm_profissional_w || ds_enter_w||ds_enter_w;
	ds_motilidade_ocupar_w	:=	ds_motilidade_ocupar_w || expressao3_w || ds_enter_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_resumo_motilidade_ocular (cd_estabelecimento_p bigint, nr_seq_consulta_p bigint, cd_pessoa_fisica_p text, ie_mostra_todos_p text, nr_seq_item_p bigint, nm_usuario_p text) FROM PUBLIC;
