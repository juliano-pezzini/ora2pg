...
CREATE OR REPLACE PROCEDURE pls_inserir_conta_resumo_item (nr_seq_proc_p bigint, nr_seq_partic_p bigint, nr_seq_mat_p bigint, ie_tipo_resumo_p text, nr_prest_pgto_coleta_p bigint, nm_usuario_p text, ds_log_p text) AS $body$
DECLARE

					
/* IE_TIPO_RESUMO_P
	P - Prestador
	C - Medico executor da conta
	R - Participante
	M - Material 
	I - Intercambio
*/
...
vl_material_w				pls_conta_mat.vl_material%type		:=0;
vl_material_imp_w			pls_conta_mat.vl_material_imp%type	:=0;
...
qt_contas_taxa_coleta_w			bigint	:= 0;
...
if (nr_seq_proc_p IS NOT NULL AND nr_seq_proc_p::text <> '') then
	...
	--aaschlote 17/04/2015 OS 873317 - Quando tiver valor liberado zerado e valor de glosa, coloca o ie_glosa para "S"
	if (ie_glosa_w = 'N') and (coalesce(vl_liberado_w,0) = 0) and (vl_glosa_w > 0) then
		ie_glosa_w := 'S';
	end if;
	
	select	sum(a.vl_pag_medico_conta)
	into STRICT	vl_pag_medico_w
	...
	select	sum(p.vl_participante),
		sum(p.vl_calculado)
	into STRICT	vl_total_partic_w,
		vl_total_partic_calc_w
	from	pls_conta_proc		a,
	...
	if (coalesce(nr_seq_partic_p::text, '') = '') then
		...
		select	b.nr_seq_prestador_pgto,
			...
			b.nr_seq_prestador,
			a.ie_status,
			coalesce(a.vl_glosa,vl_glosa_w)
	...
	select	nr_seq_conta,
		...
		vl_glosa,
		vl_liberado,
		coalesce(dt_atendimento, clock_timestamp()) dt_material
	into STRICT	nr_seq_conta_w,
		nr_seq_material_w,
		qt_item_w,
...
if (coalesce(vl_prestador_w,0)	<> 0) then
	vl_liberado_ww	:= vl_prestador_w;
end if;
...
if (nr_seq_proc_p IS NOT NULL AND nr_seq_proc_p::text <> '') then
	...
	-- Obtem a regra de data para o calculo do preco do material
	begin
	select	coalesce(ie_regra_data_preco_mat,'E')
	into STRICT	ie_regra_data_preco_mat_w
	from	pls_prestador
	where	nr_sequencia = nr_seq_prestador_exec_w;
	exception
	when others then
		ie_regra_data_preco_mat_w := 'E';
	end;
...
-- Verifica se o cooperado esta ativo pela data do procedimento e nao pela regra do prestador
...
if (ie_tipo_resumo_p	= 'P') then
	...
	cd_cooperativa_w	:= '';
	--DiogoAqui verificar o tratamento para dividir corretamente os valores a serem pagos
	...
	vl_materiais_pag_w	:= 0;
	vl_hm_w			:= vl_liberado_w;
elsif (ie_tipo_resumo_p	= 'M') then
	
	if (coalesce(nr_seq_prest_fornec_w,0) > 0) then
		nr_seq_prestador_pgto_w	:= nr_seq_prest_fornec_w;
	else
	...
	if (ie_status_ww = 'P') or (ie_status_ww = 'A') then
		vl_liberado_w := 0;
		vl_provisorio_w := coalesce(vl_material_w,vl_material_imp_w);
	else
	...
	vl_apresentado_w		:= vl_material_imp_w;
	vl_liberado_w			:= vl_liberado_w;
	dt_item_w			:= dt_atendimento_w;
	cd_cooperativa_w		:= '';
	nr_seq_honorario_crit_w	:= null;
	nr_seq_exame_coleta_w	:= null;
	vl_exame_coleta_w	:= 0;
	vl_co_pag_w		:= 0;
	vl_materiais_pag_w	:= vl_liberado_w;
	vl_hm_w			:= 0;
elsif (ie_tipo_resumo_p	 = 'I') then
	...
	if (nr_seq_proc_p IS NOT NULL AND nr_seq_proc_p::text <> '') then
		vl_custo_operacional_w	:= vl_co_ptu_w;
		vl_materiais_w		:= vl_material_ptu_w;
		vl_calculado_w		:= vl_procedimento_ptu_w;
		vl_liberado_w		:= vl_co_ptu_w + vl_material_ptu_w + vl_procedimento_ptu_w;
		dt_item_w		:= dt_procedimento_w;
	elsif (nr_seq_mat_p IS NOT NULL AND nr_seq_mat_p::text <> '') then
		vl_calculado_w		:= 0;
		vl_liberado_w		:= vl_material_ptu_w;
		dt_item_w		:= dt_atendimento_w;
	end if;
end if;
...
vl_exame_coleta_w	:= coalesce(vl_exame_coleta_w,0);
...
if (coalesce(nr_prest_pgto_coleta_p,0) > 0) then
	...
	/* Francisco - 17/05/2012 - OS 433083 - Se for para inserir item de coleta, deve obter o evento denovo */
	...
	if (coalesce(nr_seq_regra_w,0) > 0) and (ie_define_pagamento_w = 'A') and (ie_proc_mat_w = 'P') then
		...
		--Se cair aqui e que devera buscar o prestador diretamente vinculado ao medico executor da conta. 
...
/*Se nao existir taca de coleta ja lancada para o procedimento ou este registro deve lancar taxa de coleta entao e calculado as taxas administrativas*/

if	((coalesce(nr_seq_exame_coleta_w,0) = 0) and (qt_contas_taxa_coleta_w = 0)) then
...
if (coalesce(vl_provisorio_w,0) > 0) or (coalesce(vl_liberado_w,0) > 0) or (ie_glosa_w = 'S') or
	...
	--Valor que sera utilizado pelo contabil como valor apresentado. Esse valor sera proporcional ao valor do item, ja

	--que o vl_apresentado se repete de maneira integral em todas as linhas que representam cada item no resumo.
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_inserir_conta_resumo_item (nr_seq_proc_p bigint, nr_seq_partic_p bigint, nr_seq_mat_p bigint, ie_tipo_resumo_p text, nr_prest_pgto_coleta_p bigint, nm_usuario_p text, ds_log_p text) FROM PUBLIC;
