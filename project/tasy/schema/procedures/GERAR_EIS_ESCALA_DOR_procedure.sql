...
CREATE OR REPLACE PROCEDURE gerar_eis_escala_dor ( dt_sinal_vital_p timestamp, cd_estabelecimento_p bigint, nm_usuario_p text ) AS $body$
DECLARE
...
if (qt_intensidade_w IS NOT NULL AND qt_intensidade_w::text <> '') then
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		EXIT WHEN NOT FOUND; /* apply on c02 */
			...
			if (cw02.cd_escala_dor IS NOT NULL AND cw02.cd_escala_dor::text <> '') and (cw02.qt_escala_dor IS NOT NULL AND cw02.qt_escala_dor::text <> '') and (cw02.nr_seq_result_dor > nr_seq_min_intensidade_w)then
			...
			and	cw02.dt_sinal_vital < (dt_sinal_vital + ((1/24) *2));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_eis_escala_dor ( dt_sinal_vital_p timestamp, cd_estabelecimento_p bigint, nm_usuario_p text ) FROM PUBLIC;
