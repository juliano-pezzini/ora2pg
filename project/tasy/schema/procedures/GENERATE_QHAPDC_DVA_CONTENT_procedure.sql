...
CREATE OR REPLACE PROCEDURE generate_qhapdc_dva_content ( nr_seq_dataset_p bigint , nm_usuario_p text, returned_value_p INOUT bigint, other_exception_p INOUT text) AS $body$
DECLARE
...
BEGIN
  ...
  BEGIN
    ...
    SELECT 'N'
      ...
      || CASE WHEN coalesce(cd_usuario_convenio::text, '') = '' THEN  '          '   ELSE Rpad(cd_usuario_convenio, 10, ' ') END

      || CASE WHEN coalesce(cd_plano_convenio::text, '') = '' THEN  ' '   ELSE Lpad(trim(both cd_plano_convenio),1,' ') END 
      || chr(10)

    INTO STRICT ds_segment_w

    FROM qhapdc_segment_dva
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE generate_qhapdc_dva_content ( nr_seq_dataset_p bigint , nm_usuario_p text, returned_value_p INOUT bigint, other_exception_p INOUT text) FROM PUBLIC;
