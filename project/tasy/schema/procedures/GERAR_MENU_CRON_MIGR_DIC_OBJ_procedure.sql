...
CREATE OR REPLACE PROCEDURE gerar_menu_cron_migr_dic_obj ( cd_funcao_p bigint, nr_seq_obj_menu_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
and	a.nr_seq_obj_sup = nr_seq_obj_menu_p
...
BEGIN
if (cd_funcao_p IS NOT NULL AND cd_funcao_p::text <> '') and (nr_seq_obj_menu_p IS NOT NULL AND nr_seq_obj_menu_p::text <> '') and (nm_usuario_p IS NOT NULL AND nm_usuario_p::text <> '') then
	begin
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_menu_cron_migr_dic_obj ( cd_funcao_p bigint, nr_seq_obj_menu_p bigint, nm_usuario_p text) FROM PUBLIC;
