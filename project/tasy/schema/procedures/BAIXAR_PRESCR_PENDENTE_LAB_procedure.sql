...
CREATE OR REPLACE PROCEDURE baixar_prescr_pendente_lab (qt_hora_adicional_p bigint, cd_motivo_baixa_p bigint, dt_parametro_p timestamp) AS $body$
DECLARE
		...
		EXIT WHEN NOT FOUND; /* apply on C04 */
			...
			if (nm_usuario_w IS NOT NULL AND nm_usuario_w::text <> '') or (ds_perfil_dest_w IS NOT NULL AND ds_perfil_dest_w::text <> '') then
				...
				if (ds_lista_mat_w IS NOT NULL AND ds_lista_mat_w::text <> '') then
				 ds_lista_mat_w := substr(obter_desc_expressao(341565) ||' '||chr(13)||ds_lista_mat_w||chr(13),1,4000);
				end if;

				if (ds_lista_proc_w IS NOT NULL AND ds_lista_proc_w::text <> '') then
				 ds_lista_proc_w := substr(obter_desc_expressao(341568) ||' '||chr(13)||ds_lista_proc_w,1,4000);
				end if;

				ds_comunicado_w := substr(ds_mensagem_w||chr(13)||' '|| obter_desc_expressao(293500) ||'='||cd_motivo_baixa_p||' '||obter_desc_expressao(286579) ||'='||to_char(clock_timestamp(),'dd/mm hh24:mi')|| chr(13)||ds_lista_mat_w||ds_lista_proc_w,1,4000);
   				...
   				nr_sequencia,
    				...
    				values (
      				...
      				clock_timestamp(),
      				ds_titulo_w||' (' || obter_desc_expressao(309308) || ' '||nr_prescricao_w||')',
      				ds_comunicado_w,
      				nm_usuario_regra_w,
      				nm_usuario_dest_w,
      				clock_timestamp(),
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE baixar_prescr_pendente_lab (qt_hora_adicional_p bigint, cd_motivo_baixa_p bigint, dt_parametro_p timestamp) FROM PUBLIC;
