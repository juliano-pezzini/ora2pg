...
and	((coalesce(REPLACE(ds_filtro_ng_w, ' ', ''),'0') = '0') or (obter_se_contido_char(a.nr_seq_ng,REPLACE(ds_filtro_ng_w, ' ', '')) = ie_result_possib_ng_w))
and	((ie_result_possib_gng_w = 'N' and coalesce(a.nr_seq_ng::text, '') = '') or exists(	SELECT	1
										...
										and	((coalesce(REPLACE(ds_filtro_gng_w, ' ', ''),'0') = '0') or (obter_se_contido_char(z.nr_seq_gng,REPLACE(ds_filtro_gng_w, ' ', '')) = ie_result_possib_gng_w))
										 LIMIT 1))
group by
...
from	(
	...
	and	d.dt_mes_referencia between dt_inicial_w and dt_final_w
	and 	coalesce(d.ie_situacao,'A') 	= 'A'
	and	((coalesce(REPLACE(ds_filtro_ng_w, ' ', ''), '0') = '0') or (obter_se_contido_char(e.nr_sequencia,REPLACE(ds_filtro_ng_w, ' ', '')) = ie_result_possib_ng_w))
	and	((coalesce(REPLACE(ds_filtro_gng_w, ' ', ''), '0') = '0') or (obter_se_contido_char(f.nr_sequencia,REPLACE(ds_filtro_gng_w, ' ', '')) = ie_result_possib_gng_w))	
    ) y
where	1 = 1
...
SELECT	CASE WHEN cd_estab_w=0 THEN 0  ELSE cd_estabelecimento END  cd_estabelecimento,
	...
	substr(WHEB_MENSAGEM_PCK.get_texto(299243)|| ' ' || CASE WHEN cd_estab_p=0 THEN obter_nome_empresa(cd_empresa_p)  ELSE Obter_nome_fantasia_estab(cd_estab_w) END ,1,255) ds_centro_controle,
	ie_classif_conta,
	ds_classificacao,
	ds_natureza_gasto,
	ds_tipo_indicador,
...
if (coalesce(ds_filtro_gng_w,'0') = '0') then
    ...
    if (coalesce(ds_filtro_gng_w::text, '') = '') then
    begin
        ds_filtro_gng_w := nr_seq_gng_p;
    end;
    end if;
...
if (coalesce(ds_filtro_ng_w,'0') = '0') then
    ...
    if (coalesce(ds_filtro_ng_w::text, '') = '') then
    begin
        ds_filtro_ng_w := nr_seq_ng_p;
    end;
    end if;
	...
	for i in 1..centro_w.count loop
		...
		EXIT WHEN NOT FOUND; /* apply on C02 */
			...
			if (ie_gerar_w = 'S') then
				...
				/* Acumular a Classificacao se ela for a receita Liquida, para o calculo do percentual */
				...
				if (vet02.ie_emite_perc = 'S') then
					...
					valores_w[nr_seq_w].ie_classif_conta		:= vet02.cd_classificacao;
					valores_w[nr_seq_w].ie_emite			:= vet02.ie_emite;
					valores_w[nr_seq_w].ds_natureza_gasto		:= WHEB_MENSAGEM_PCK.get_texto(299231); /* % Sobre Receita Liquida */
		...
		close C02; /* Fim Classificac?es DRE*/
...
if (ie_totalizar_centro_p = 'S') then
	...
	EXIT WHEN NOT FOUND; /* apply on C09 */
		...
		if (coalesce(vet09.ie_classif_conta,0) <> 0) then
			...
			if (ie_receita_liquida_w = 'S') and (coalesce(vet09.ds_natureza_gasto::text, '') = '') and (coalesce(vet09.ds_tipo_indicador::text, '') = '') and (vet09.ds_classificacao IS NOT NULL AND vet09.ds_classificacao::text <> '') then
				begin
