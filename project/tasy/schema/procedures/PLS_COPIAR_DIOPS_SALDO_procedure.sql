...
CREATE OR REPLACE PROCEDURE pls_copiar_diops_saldo ( nr_seq_diops_p bigint, nr_seq_diops_ant_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	where	nr_seq_idade_saldo	= nr_seq_diops_ant_p;
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (qt_registro_w	= 0) then
		insert into  diops_conta_idade_saldo(nr_sequencia, nr_seq_idade_saldo, cd_conta_contabil,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_copiar_diops_saldo ( nr_seq_diops_p bigint, nr_seq_diops_ant_p bigint, nm_usuario_p text) FROM PUBLIC;
