...
CREATE OR REPLACE PROCEDURE pls_gerar_exclusao_mov_ted ( nr_seq_lote_mov_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	and	trunc(a.dt_rescisao,'dd') between dt_referencia_inicial_w and dt_referencia_final_w
	and (trunc(a.dt_contratacao,'month') <> trunc(a.dt_rescisao,'month') and (a.dt_rescisao IS NOT NULL AND a.dt_rescisao::text <> ''))
...
LEFT OUTER JOIN pls_segurado_carteira d ON (a.nr_sequencia = d.nr_seq_segurado)
WHERE a.cd_pessoa_fisica	= b.cd_pessoa_fisica  and a.nr_seq_plano		= c.nr_sequencia and a.nr_sequencia		= nr_seq_segurado_w;
		...
		if (qt_endereco_residencial_w > 0) then
			...
			select	substr(b.ds_endereco,1,30),
				...
				coalesce(nr_telefone_celular_w, b.nr_telefone_celular)
		...
		values (	nextval('pls_movimentacao_benef_seq'),clock_timestamp(),nm_usuario_p,clock_timestamp(),nm_usuario_p,
				nr_seq_lote_mov_p,nr_seq_segurado_w,'E',ds_logradouro_w,cep_w,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_exclusao_mov_ted ( nr_seq_lote_mov_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
