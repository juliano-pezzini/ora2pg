...
CREATE OR REPLACE PROCEDURE ctb_pls_contab_prov_ppcng ( nr_lote_contabil_p bigint, nm_usuario_p text, ie_exclusao_p text, ds_retorno_p INOUT text) AS $body$
DECLARE
...
C02 CURSOR FOR
	...
	and	a.ie_tipo_item not in ('3','6','20','43','5') /* Nao contabilizar parcela da negociacao nas receitas */
...
union all
	...
	select	'CO' ie_tipo_valor,
		...
		f.nr_sequencia nr_seq_tab_orig,
		d.nr_sequencia nr_seq_tab_compl,
		j.nr_sequencia nr_doc_analitico,
		c.nr_seq_contrato nr_seq_contrato
...
union all
	...
	select	'P',
		...
		pls_obter_data_contab_ppcng(a.ie_cancelamento,a.dt_cancelamento,b.dt_inicio_cobertura,i.dt_mesano_referencia,c.nr_seq_item_cancel) dt_inicio_cobertura,
		'PLS_MENSALIDADE_ITEM_CONTA' nm_tabela,
		'VL_POS_ESTABELECIDO' nm_atributo, --VALIDAR MARCIO
		f.cd_conta_deb,
		f.cd_conta_cred,
		58 nr_seq_info_ctb,
		b.dt_mesano_referencia,
...
union all
	...
	and	coalesce(f.nr_lote_contab_antecip,0)	= 0
	and	coalesce(d.nr_seq_cancel_rec_mens::text, '') = ''
	and	a.ie_tipo_item not in ('3','6','20','43') /* Nao contabilizar parcela da negociacao nas receitas */
...
c04 CURSOR(nr_seq_mensalidade_p	pls_mensalidade.nr_sequencia%type)FOR
	SELECT	nr_titulo
	from	titulo_receber
...
c05 CURSOR(nr_seq_item_cancel_p	pls_mensalidade_seg_item.nr_sequencia%type)FOR
	SELECT	nr_titulo
...
if (ie_exclusao_p <> 'S') then
        ...
        if (cd_tipo_lote_contabil_w <> 49) then
		...
		for z in 1..s_array.count loop
			...
			EXIT WHEN NOT FOUND; /* apply on c04 */
				...
				if (coalesce(nr_titulo_w,0) = 0) then
					begin
					nr_titulo_w:=	c04_w.nr_titulo;
					end;
				end if;
				
				ds_titulo_w:=	substr(ds_titulo_w || c04_w.nr_titulo, 0,255) || ', ';
			...
			if (coalesce(nr_titulo_w, 0) = 0) then
				...
				EXIT WHEN NOT FOUND; /* apply on c05 */
					...
					if (coalesce(nr_titulo_w,0) = 0) then
						begin
						nr_titulo_w:=	c05_w.nr_titulo;
						end;
					end if;
					
					ds_titulo_w:=	substr(ds_titulo_w || c05_w.nr_titulo, 0,255) || ', ';
			...
			ds_titulo_w	:= substr(ds_titulo_w, 0, length(ds_titulo_w) - 2);
			...
			if (trunc(dt_inicio_cobertura_w,'month') > trunc(dt_referencia_w,'month')) then
				dt_inicio_cobertura_w := trunc(dt_referencia_w,'month');
			end if;
...
union all
					...
					ds_conteudo_deb_w	:= substr(	nr_nota_fiscal_w		||'#@'||
										cd_cgc_pagador_w		||'#@'||
										cd_pf_pagador_w			||'#@'||
										ds_titulo_w			||'#@'|| /* Numero do titulo, se tiver mais de um traz todos separado por virgula*/
										nm_pagador_w			||'#@'||
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ctb_pls_contab_prov_ppcng ( nr_lote_contabil_p bigint, nm_usuario_p text, ie_exclusao_p text, ds_retorno_p INOUT text) FROM PUBLIC;
