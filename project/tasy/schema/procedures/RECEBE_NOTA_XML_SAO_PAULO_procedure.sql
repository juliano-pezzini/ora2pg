...
if (ie_opcao_p = 2) then
	...
	select	cd_material_estoque,
		      ...
		      coalesce(obter_dados_material_fornec(cd_estabelecimento_p, cd_cgc_emitente_w, cd_material_w, 1), substr(qt_conv_compra_estoque,1,255)) qt_conv_compra_estoque
...
if (ie_opcao_p = 3) then
	...
	if (tx_tributo_icmsst_p > 0) then	
		...
		if (cd_tributo_w > 0) then
			...
			values (	nr_sequencia_w,
				...
				nr_item_nf_w,
				cd_tributo_w,
				replace(vl_tributo_icmsst_p,'.',','),
				clock_timestamp(),
				nm_usuario_p,
				replace(vl_base_calculo_icmsst_p,'.',','),
				replace(tx_tributo_icmsst_p,'.',','),
				0,
				'N',
				0,
				0,
	...
	/*Importa ICMS Desoneracao*/

	if ((vl_icms_deson_p)::numeric  > 0) then	
...
/*Importa os lote fornecedor*/

if (ie_opcao_p = 4) and (cd_lote_fabricacao_p IS NOT NULL AND cd_lote_fabricacao_p::text <> '') then
	
	nr_item_nf_w		:= nr_item_nf_p;
		
	select	coalesce(obter_quantidade_convertida(max(cd_material),replace(qt_material_lote_p,'.',','), max(cd_unidade_medida_compra), 'UME'),replace(qt_material_lote_p,'.',','))
	...
	values (	nextval('nota_fiscal_item_lote_seq'),
		...
		nr_sequencia_w,
		nr_item_nf_w,
		CASE WHEN coalesce(dt_validade_p::text, '') = '' THEN trunc(clock_timestamp(),'dd')  ELSE to_date(substr(dt_validade_p,9,2) || '/' || substr(dt_validade_p,6,2) || '/' || substr(dt_validade_p,1,4),'dd/mm/yyyy') END ,
		--replace(qt_material_lote_p,'.',','),
		qt_material_lote_w,
		cd_lote_fabricacao_p,
		clock_timestamp(),
		nm_usuario_p,
		'N',
		CASE WHEN coalesce(dt_fabricacao_p::text, '') = '' THEN trunc(clock_timestamp(),'dd')  ELSE to_date(substr(dt_fabricacao_p,9,2) || '/' || substr(dt_fabricacao_p,6,2) || '/' || substr(dt_fabricacao_p,1,4),'dd/mm/yyyy') END );
end if;
...
if (ie_opcao_p = 6) then
	...
	/*Busca conversao da Condicao de Pagamento*/
	
	select	obter_conversao_interna(cd_cgc_emitente_w,'CONDICAO_PAGAMENTO','CD_CONDICAO_PAGAMENTO',cd_condicao_pagamento_p)
	into STRICT	cd_condicao_pagamento_w
	;	
	
	if (coalesce(cd_condicao_pagamento_w::text, '') = '') and (coalesce(cd_cond_pagamento_padrao_p, 0) = 0) then
		ie_erro_w := 'S';
		CALL Wheb_mensagem_pck.exibir_mensagem_abort(184847,'CD_CONDICAO_PAGAMENTO='||cd_condicao_pagamento_p||';CD_CGC_EMITENTE='||cd_cgc_emitente_p );
	end if;	
	
	if (coalesce(cd_condicao_pagamento_w, 0) = 0) and (coalesce(cd_cond_pagamento_padrao_p, 0) <> 0) then
		cd_condicao_pagamento_w:= cd_cond_pagamento_padrao_p;
	end if;
...
if (ie_opcao_p = 7) and (nr_danfe_p IS NOT NULL AND nr_danfe_p::text <> '')then
	begin
...
if (ie_opcao_p = 8) and (ds_ad_item_p IS NOT NULL AND ds_ad_item_p::text <> '') then
	begin
	
	update	nota_fiscal_item
	set		ds_complemento	= ds_ad_item_p
	where	nr_sequencia	= nr_sequencia_p;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE recebe_nota_xml_sao_paulo ( ie_opcao_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text, ds_natureza_operacao_p text, cd_condicao_pagamento_p text, cd_serie_nf_p text, nr_nota_fiscal_p text, nr_ordem_compra_p text, dt_emissao_p text, dt_entrada_saida_p text, ie_entrada_saida_p text, cd_cgc_emitente_p text, ds_razao_emit_p text, ds_fantasia_emit text, ds_logradouro_emit_p text, nr_endereco_emit_p text, ds_bairro_emit_p text, ds_municipio_emit_p text, ds_uf_emit_p text, cd_cep_emit_p text, ds_pais_emit_p text, nr_telefone_emit_p text, nr_inscr_estadual_emit_p text, cd_local_estoque_p bigint, cd_centro_custo_p bigint, nr_seq_lote_p bigint, cd_material_p text, qt_item_nf_p text, vl_unitario_item_nf_p text, cd_lote_fabricacao_p text, qt_material_lote_p text, dt_fabricacao_p text, dt_validade_p text, vl_base_calculo_icms_p text, vl_tributo_icms_p text, tx_tributo_icms_p text, vl_base_calculo_pis_p text, vl_tributo_pis_p text, tx_tributo_pis_p text, tx_tributo_cofins_p text, vl_base_calculo_cofins_p text, vl_tributo_cofins_p text, vl_base_calculo_ipi_p text, vl_tributo_ipi_p text, tx_tributo_ipi_p text, cd_barra_material_p text, nr_danfe_p text, nr_seq_registro_insp_p bigint, nr_ordem_compra_insp_p bigint, nr_sequencia_p INOUT bigint, nr_item_nf_p INOUT bigint, ie_erro_p INOUT text, cd_serie_nf_padrao_p text, cd_cond_pagamento_padrao_p bigint, cd_nat_operacao_p bigint, cd_operacao_nf_p bigint default 0, nr_registro_anvisa_p text default '', vl_base_calculo_icmsst_p text default null, vl_tributo_icmsst_p text default null, tx_tributo_icmsst_p text default null, vl_icms_deson_p text default '0', cst_icms40_p text default '0', ds_ad_item_p text default null) FROM PUBLIC;
