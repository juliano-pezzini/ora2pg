...
;
	...
	CALL gravar_log_lab(1234, 'atualizar_lab_result_item ('||nr_prescricao_p||', '||nr_seq_prescr_w||', nvl('||cd_analito_w||','||cd_exame_p||'), '||ds_resultado_p||', 0, '||ds_resultado_p||', , null, n, eclipse, to_date('||ds_date_time_obs_p||', yyyymmddhh24miss), '||ds_referencia_p||', '||ds_units_p||', null, null, ds_erro_w, null);','ddmarco');
	...
	ds_erro_w := atualizar_lab_result_item(nr_prescricao_p, nr_seq_prescr_w, coalesce(cd_analito_w,cd_exame_princ_w), qt_resultado_w, pr_resultado_w, ds_resultado_w, ds_observacao_p, NULL, 'N', 'ECLIPSE', TO_DATE(ds_date_time_obs_p, 'yyyymmddhh24miss'), ds_referencia_p, ds_units_p, NULL, NULL, ds_erro_w, nr_seq_analito_w);
