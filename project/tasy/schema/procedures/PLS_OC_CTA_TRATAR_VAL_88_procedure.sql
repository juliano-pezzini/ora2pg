...
CREATE OR REPLACE PROCEDURE pls_oc_cta_tratar_val_88 ( dados_regra_p pls_tipos_ocor_pck.dados_regra, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
c_conta_verificar CURSOR(	nr_id_transacao_pc	pls_oc_cta_selecao_ocor_v.nr_sequencia%type,
				nr_seq_item_ref_pc	pls_oc_val_itens.nr_seq_item_ref%type) FOR
	SELECT	x.nr_seq_conta,
		b.ie_origem_proced,
		b.cd_procedimento,
		sum(b.qt_procedimento_imp) qt_item
...
c_proc_conta CURSOR(	nr_seq_conta_pc		pls_conta.nr_sequencia%type,
			nr_seq_item_ref_pc	pls_oc_val_itens.nr_seq_item_ref%type) FOR
	...
	group by b.nr_seq_conta,
		...
		b.cd_procedimento,
		a.ie_situacao_gerar
	order by b.ie_origem_proced,
		b.cd_procedimento;


c_mat_conta CURSOR(	nr_seq_conta_pc		pls_conta.nr_sequencia%type,
			nr_seq_item_ref_pc	pls_oc_val_itens.nr_seq_item_ref%type) FOR
	SELECT	b.nr_seq_conta,
		b.nr_seq_material,
		a.ie_situacao_gerar,
...
procedure processa_procedimento(	ie_situacao_gerar_p	in pls_oc_item_dep.ie_situacao_gerar%type,
					...
					ie_origem_proced_p	in pls_conta_proc.ie_origem_proced%type,
					cd_procedimento_p	in pls_conta_proc.cd_procedimento%type,
					nr_seq_item_ref_p	in pls_oc_item_ref.nr_sequencia%type,
					dados_tb_selecao_p	in out pls_tipos_ocor_pck.dados_table_selecao_ocor,
					nr_indice_p		in out integer) is

ie_gera_ocorrrencia_w	varchar(1);

c_reg_sel_proc CURSOR(	nr_id_transacao_pc	pls_oc_cta_selecao_ocor_v.nr_sequencia%type,
...
if (ie_gera_ocorrrencia_w = 'S') then
	...
	for r_c_reg_sel_proc_w in c_reg_sel_proc(nr_id_transacao_p, ie_origem_proced_p, cd_procedimento_p, nr_seq_item_ref_p) loop
		...
		dados_tb_selecao_p.nr_seq_selecao(nr_indice_p) := r_c_reg_sel_proc_w.nr_seq_selecao;
		dados_tb_selecao_p.ie_valido(nr_indice_p) := 'S';
		dados_tb_selecao_p.ds_observacao(nr_indice_p) := substr('Ocorrência gerada com base na validação da regra ' || nr_seq_item_ref_p, 1, 2000);

		-- se já atingiu a quantidade manda para o banco
		if (nr_indice_p >= pls_util_pck.qt_registro_transacao_w) then
			CALL pls_tipos_ocor_pck.gerencia_selecao_validacao(	dados_tb_selecao_p.nr_seq_selecao,
									dados_tb_selecao_p.ds_seqs_selecao,
									'SEQ',
									dados_tb_selecao_p.ds_observacao,
									dados_tb_selecao_p.ie_valido,
									nm_usuario_p);
			dados_tb_selecao_p.nr_seq_selecao.delete;
			dados_tb_selecao_p.ds_seqs_selecao.delete;
			dados_tb_selecao_p.ie_valido.delete;
			dados_tb_selecao_p.ds_observacao.delete;
			nr_indice_p := 0;
		else
...
c_reg_sel_mat CURSOR(	nr_id_transacao_pc	pls_oc_cta_selecao_ocor_v.nr_sequencia%type,
...
if (ie_gera_ocorrrencia_w = 'S') then
	...
	for r_c_reg_sel_mat_w in c_reg_sel_mat(nr_id_transacao_p, nr_seq_material_p, nr_seq_item_ref_p) loop
		...
		dados_tb_selecao_p.nr_seq_selecao(nr_indice_p) := r_c_reg_sel_mat_w.nr_seq_selecao;
		dados_tb_selecao_p.ie_valido(nr_indice_p) := 'S';
		dados_tb_selecao_p.ds_observacao(nr_indice_p) := substr('Ocorrência gerada com base na validação da regra ' || nr_seq_item_ref_p, 1, 2000);
			...
			for r_c_proc_conta_w in c_proc_conta(r_c_conta_verificar_w.nr_seq_conta, r_C01_w.nr_seq_item_ref) loop
				...
				if (ds_proc_w = coalesce(ds_ultimo_proc_w, ds_proc_w)) then
					...
					processa_procedimento(	ie_situacao_gerar_proc, r_c_conta_verificar_w.qt_item,
								...
								r_C01_w.nr_seq_item_ref, dados_tb_selecao_w,
								nr_indice_w);
				end if;
			...
			for r_c_mat_conta_w in c_mat_conta(r_c_conta_verificar_w.nr_seq_conta, r_C01_w.nr_seq_item_ref) loop
				...
				if (ds_mat_w = coalesce(ds_ultimo_mat_w, ds_mat_w)) then
					...
					processa_material(	ie_situacao_gerar_mat, r_c_conta_verificar_w.qt_item,
								...
								dados_tb_selecao_w, nr_indice_w);
				end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_oc_cta_tratar_val_88 ( dados_regra_p pls_tipos_ocor_pck.dados_regra, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
