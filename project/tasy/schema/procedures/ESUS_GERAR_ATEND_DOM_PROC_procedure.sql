...
CREATE OR REPLACE PROCEDURE esus_gerar_atend_dom_proc ( cd_profissional_p text, cd_cbo_p text, nr_atendimento_p bigint, dt_atendimento_p timestamp, nr_seq_ficha_p bigint, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (qt_proc_gerado_w = 0) then 
		...
		values (	nm_usuario_p, 
			...
			ie_origem_proced_w, 
			nr_seq_ficha_p, 
			nextval('esus_atend_dom_proced_seq'));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE esus_gerar_atend_dom_proc ( cd_profissional_p text, cd_cbo_p text, nr_atendimento_p bigint, dt_atendimento_p timestamp, nr_seq_ficha_p bigint, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
