...
CREATE OR REPLACE PROCEDURE baixar_devolucao_palmweb ( nr_devolucao_p bigint, nr_atendimento_p bigint, cd_local_estoque_p bigint, cd_setor_atendimento_p bigint, cd_material_p bigint, cd_motivo_devolucao_p text, cd_unidade_medida_p text, nr_seq_lote_fornec_p bigint, qt_material_p bigint, ie_gerar_p text, nm_usuario_p text) AS $body$
DECLARE
...
ie_local_user_logado_w 		varchar(1);
...
ie_local_user_logado_w := obter_param_usuario(88, 321, obter_perfil_ativo, nm_usuario_p, obter_estabelecimento_ativo, ie_local_user_logado_w);
 
cd_local_estoque_w := cd_local_estoque_p;
 
if (coalesce(ie_local_user_logado_w,'N') = 'S') then 
	...
	select 	coalesce(cd_local_estoque,cd_local_estoque_p) 
...
if (ie_gerar_p = 'S') and (nr_atendimento_p IS NOT NULL AND nr_atendimento_p::text <> '') then 
...
elsif (nr_atendimento_p IS NOT NULL AND nr_atendimento_p::text <> '') and (nr_devolucao_p IS NOT NULL AND nr_devolucao_p::text <> '') then 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE baixar_devolucao_palmweb ( nr_devolucao_p bigint, nr_atendimento_p bigint, cd_local_estoque_p bigint, cd_setor_atendimento_p bigint, cd_material_p bigint, cd_motivo_devolucao_p text, cd_unidade_medida_p text, nr_seq_lote_fornec_p bigint, qt_material_p bigint, ie_gerar_p text, nm_usuario_p text) FROM PUBLIC;
