...
CREATE OR REPLACE PROCEDURE gerar_eis_atend_pac_alta ( dt_parametro_p timestamp, ie_opcao_p text, nm_usuario_p text) AS $body$
DECLARE
...
if (dt_parametro_p IS NOT NULL AND dt_parametro_p::text <> '') then 
	...
	nr_sequencia_w := Gravar_Log_Indicador(1200, wheb_mensagem_pck.get_texto(307508), clock_timestamp(), trunc(dt_parametro_p), nm_usuario_p, nr_sequencia_w); -- Atendimentos Paciente - Alta (Novo) 
	delete	FROM eis_atend_pac_alta where trunc(dt_alta, ie_opcao_w) = trunc(dt_parametro_p, ie_opcao_w);
	 
	/* Atualiza o indicador de alta */
 
	insert into eis_atend_pac_alta( 
			dt_entrada, 
			dt_alta, 
			nr_atendimento, 
			nr_atend_original, 
		...
		SELECT	a.dt_entrada, 
			...
			from	atend_paciente_unidade 
			where	nr_seq_interno = obter_atepacu_paciente(a.nr_atendimento, 'P'))),1,255),wheb_mensagem_pck.get_texto(796842)) ds_setor_entrada, 
...
WHERE a.cd_pessoa_fisica	= b.cd_pessoa_fisica and a.nr_atendimento	= c.nr_atendimento and d.nr_atendimento	= a.nr_atendimento and a.cd_estabelecimento	= f.cd_estabelecimento  and trunc(a.dt_alta, ie_opcao_w) = trunc(dt_parametro_p, ie_opcao_w) and coalesce(h.ie_cancelado,'N')	= 'N' and c.nr_seq_interno 	= (select obter_atepacu_paciente(c.nr_atendimento, 'A') ) and d.nr_seq_interno	= Obter_Atecaco_atendimento(a.nr_atendimento);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_eis_atend_pac_alta ( dt_parametro_p timestamp, ie_opcao_p text, nm_usuario_p text) FROM PUBLIC;
