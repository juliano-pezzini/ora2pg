...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_arq_a700_113 (nr_seq_servico_p bigint, cd_estabelecimento_p text, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	SELECT	lpad(coalesce(elimina_caractere_especial(to_char(nr_lote)),'0'),8,'0'),
		...
		rpad(coalesce(elimina_caractere_especial(elimina_acentuacao(nm_prest_exec)),' '),60,' '),
		lpad(coalesce(tp_prest_exec,'0'),2,'0'),
		rpad(coalesce(to_char(id_rec_proprio),' '),1,' '),
		rpad(coalesce(cd_cid_obito,' '),6,' '),
		rpad(coalesce(ie_cobertura_especial,' '),2 ,' '),
		rpad(coalesce(ie_regime_atendimento,' '),2 ,' '),
		rpad(coalesce(ie_saude_ocupacional,' '),2 ,' ')
...
EXIT WHEN NOT FOUND; /* apply on C00 */
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		--Regras: Para TP_NOTA=1, 3 ou 4, preencher com brancos 
		if tp_nota_w in (1,3,4) then
			ie_tipo_atendimento_w := '  ';
		end if;
		
		--Para atendimento de Atencao Domiciliar, TP_PREST_EXEC = 08 e TP_NOTA = 2, o campo devera ser preenchido com 06 (Atendimento Domiciliar) para a verso da TISS igual ou inferior a 3.05.00		
		if somente_numero(nr_ver_tiss_w) <= 30500 then
			if tp_prest_exec_w = '08' and tp_nota_w = 2 then
				ie_tipo_atendimento_w := '06';
			end if;
		end if;

		--Obrigatorio se TP_NOTA = 4. Para os demais casos, preencher com branco
		if tp_nota_w <> 4 then
			nm_prest_exec_w := rpad(' ',60,' ');
		end if;
		...
		ds_conteudo_w	:=	lpad(to_char(nr_seq_registro_w),8,'0') || '702' || nr_lote_w || lpad(' ',12,' ') || cd_unimed_destino_w || '   ' ||
					...
					id_rec_proprio_w || cd_cid_obito_cobr_w || ie_cobertura_especial_w || ie_regime_atendimento_w || ie_saude_ocupacional_w;
		...
		EXIT WHEN NOT FOUND; /* apply on C03 */
			...
			-- O sistema deve seguir o padrao de 4 casas para de inteiros e 4 para casas decimais, a representacao da quantidade 1 e a seguinte --> '00010000'
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_arq_a700_113 (nr_seq_servico_p bigint, cd_estabelecimento_p text, nm_usuario_p text) FROM PUBLIC;
