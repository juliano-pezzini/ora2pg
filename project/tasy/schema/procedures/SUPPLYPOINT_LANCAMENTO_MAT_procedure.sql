...
ie_cobra_paciente_w			material.ie_cobra_paciente%type;
...
nr_sequencia_w				dis_regra_setor.nr_sequencia%type;
...
--parametros converte_codigo_barras
nr_seq_tipo_baixa_w			tipo_baixa_prescricao.nr_sequencia%type;
qt_total_dispensar_item_w	prescr_material.qt_total_dispensar%type;
qt_total_item_conta_w		material_atend_paciente.qt_material%type;
qt_existe_w					bigint;
cd_motivo_baixa_int_w		ap_lote.cd_tipo_baixa%type;
...
if (qt_material_w = 0) then
	-- Quantidade de contagem igual ao saldo em estoque
	ds_erro_w := substr(WHEB_MENSAGEM_PCK.get_texto(278822),1,255);
end if;

if	(((coalesce(cd_estabelecimento_w::text, '') = '') or
	cd_estabelecimento_w = 0) and (cd_setor_atendimento_w > 0)) then
...
if (cd_barras_p IS NOT NULL AND cd_barras_p::text <> '') then
	SELECT * FROM converte_codigo_barras(	cd_barras_p, cd_estabelecimento_w, null, cd_local_estoque_w, cd_material_w, qt_material_ww, nr_seq_lote_w, nr_seq_lote_agrup_w, cd_kit_mat_w, ds_validade_w, ds_material_w, cd_unid_med_w, nr_etiqueta_lp_w, ds_erro_w) INTO STRICT cd_material_w, qt_material_ww, nr_seq_lote_w, nr_seq_lote_agrup_w, cd_kit_mat_w, ds_validade_w, ds_material_w, cd_unid_med_w, nr_etiqueta_lp_w, ds_erro_w;
end if;
...
if (ie_consignado_w <> '0') and (cd_fornecedor_p IS NOT NULL AND cd_fornecedor_p::text <> '') then
	...
	exception
	when others then
		-- Nao encontrado cadastro do fornecedor no Tasy
...
if (coalesce(ds_erro_w::text, '') = '') then
	...
	if (coalesce(nr_atendimento_w,0) > 0) then
		...
		if (ie_cobra_paciente_w = 'S') and (ie_conta_paciente_w = 'S') then
			...
			ie_atualizou_p := 'S';
			CALL atualiza_preco_material(nr_sequencia_w,'SupplyPoint');
...
ds_erro_p := substr(ds_erro_w,2000);
...
exception
when others then
	ds_erro_p := substr('LENGTH=' || LENGTH(ds_erro_w) || ds_erro_w||' | '||SQLERRM(SQLSTATE),1,2000);
end;
end;
$body$
LANGUAGE PLPGSQL
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE supplypoint_lancamento_mat ( cd_acao_p text, nr_atendimento_p bigint, cd_local_estoque_p bigint, cd_material_p bigint, qt_atual_p bigint, dt_movimento_p timestamp, cd_fornecedor_p text, nr_prescricao_p bigint, nr_seq_material_p bigint, nr_seq_lote_p bigint, ie_atualizou_p INOUT text, ds_erro_p INOUT text, cd_barras_p text default null, ie_commit_p text default 'S') FROM PUBLIC;
