...
CREATE OR REPLACE PROCEDURE ptu_imp_a100_v18 ( ds_conteudo_p text, nm_arquivo_p text, ie_tipo_contrato_p text, nm_usuario_p text) AS $body$
DECLARE
...
dt_comp_risco_ww		varchar(8);
dt_comp_risco_w			timestamp;
ie_exclusao_rn412_w		varchar(2);
...
BEGIN
nr_linha_w		:= substr(ds_conteudo_p,1,8);
...
if (cd_tipo_registro_w	= '101') then
	...
	nr_telefone_w := trim(both substr(ds_conteudo_p,296,9));
	...
	nm_empr_abrev_w		:= trim(both substr(ds_conteudo_p,320,30));
	dt_inclusao_ww		:= substr(ds_conteudo_p,269,2)||substr(ds_conteudo_p,267,2)||substr(ds_conteudo_p,263,4);
	ie_contrato_local_w	:= trim(both substr(ds_conteudo_p,350,1));
	cd_caepf_w		:= trim(both substr(ds_conteudo_p,351,14));
	...
	nm_cidade_w		:= substr(obter_desc_municipio_ibge(substr(cd_municipio_ibge_w,1,6)),1,100);
	sg_uf_w			:= substr(obter_uf_ibge(substr(cd_municipio_ibge_w,1,6)),1,2);
	...
	insert into ptu_intercambio_empresa(nr_sequencia, cd_filial, ds_razao_social,
		...
		ie_contrato_local, cd_caepf)
	values (nr_seq_empresa_w, cd_filial_w, ds_razao_social_w,
		nm_empr_abrev_w, ie_tipo_pessoa_w, coalesce(cd_cgc_w,'00000000000000'),
		...
		ie_contrato_local_w, cd_caepf_w);
	...
	values (nextval('ptu_intercambio_plano_seq') , nr_seq_empresa_w, cd_plano_origem_w,
	...
	if (dt_exclusao_benef_ww <> '        ') then
		dt_exclusao_benef_w	:= to_Date(dt_exclusao_benef_ww,'dd/mm/yyyy');
	end if;
	...
	dt_comp_risco_ww		:= substr(ds_conteudo_p,537,2) || substr(ds_conteudo_p,535,2) || substr(ds_conteudo_p,531,4);

	if (dt_comp_risco_ww <> '        ') then
		dt_comp_risco_w		:= to_date(dt_comp_risco_ww, 'dd/mm/yyyy');
	end if;
	...
	ie_exclusao_rn412_w	:= trim(both substr(ds_conteudo_p,539,1));
	nm_social_w		:= trim(both substr(ds_conteudo_p,555,70));
	nm_social_abreviado_w	:= trim(both substr(ds_conteudo_p,625,25));
	tp_genero_social_w	:= trim(both substr(ds_conteudo_p,650,1));
	cd_motivo_exclusao_w	:= trim(both substr(ds_conteudo_p,651,2));
	...
	insert into ptu_intercambio_benef(nr_sequencia, nr_seq_empresa, cd_unimed,
		...
		dt_comp_risco, ie_exclusao_rn412, nm_social, nm_social_abreviado,
		cd_motivo_exclusao, ie_tipo_genero_social)
	values (nr_seq_benef_w, nr_seq_empresa_w,cd_unimed_w,
		...
		cd_dependencia_w, cd_local_atendimento_w, dt_comp_risco_w,
		dt_inclusao_plano_dest_w, lpad(cd_cgc_cpf_benef_w,11,'0'), nr_rg_w,
		...
		dt_comp_risco_w, ie_exclusao_rn412_w, nm_social_w, nm_social_abreviado_w,
		cd_motivo_exclusao_w, tp_genero_social_w);
	
	/*Nao alterar a data que vem do arquivo, essa data vai ser alterada depois da importacao, com opcoes de botao direito
	...
	*/
	...
	nr_fone_w := trim(both substr(ds_conteudo_p,138,9));
	...
	insert into ptu_benef_preexistencia(nr_sequencia, nr_seq_beneficiario, cd_cid,
		dt_fim_carencia, dt_atualizacao, nm_usuario,
		dt_atualizacao_nrec, nm_usuario_nrec)
	values (nextval('ptu_benef_preexistencia_seq'), nr_seq_benef_w, cd_cid_w,
	...
	if (ie_tipo_contrato_w = 'F') then --Tratamento para Fundacoes
		CALL ptu_ajustar_cod_empresa_a100(nr_seq_intercambio_w,nm_usuario_p);
	end if;
end if;
exception
	when others then
	CALL wheb_mensagem_pck.exibir_mensagem_abort(1158495,'NR_LINHA='||nr_linha_w||';DS_MENSAGEM='||sqlerrm);
end;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_imp_a100_v18 ( ds_conteudo_p text, nm_arquivo_p text, ie_tipo_contrato_p text, nm_usuario_p text) FROM PUBLIC;
