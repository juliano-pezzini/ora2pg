...
CREATE OR REPLACE PROCEDURE pls_aplicar_deflator_item_tab ( cd_tabela_servico_p bigint, vl_reajuste_p bigint, dt_inicio_vigencia_p timestamp, dt_inicio_servico_p timestamp, cd_procedimento_p bigint, ie_origem_proced_p bigint, vl_servico_p bigint, ds_observacao_p text, nm_usuario_p text, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
values		(	nr_seq_log_w, cd_tabela_servico_p, cd_estabelecimento_p, clock_timestamp(),
			...
			clock_timestamp(),'Aplicação de ajuste deflator de preço'||(vl_reajuste_p)||' %  Observação : '||ds_observacao_p);
commit;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_aplicar_deflator_item_tab ( cd_tabela_servico_p bigint, vl_reajuste_p bigint, dt_inicio_vigencia_p timestamp, dt_inicio_servico_p timestamp, cd_procedimento_p bigint, ie_origem_proced_p bigint, vl_servico_p bigint, ds_observacao_p text, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
