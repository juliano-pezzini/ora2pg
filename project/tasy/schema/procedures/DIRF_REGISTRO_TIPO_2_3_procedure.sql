...
CREATE OR REPLACE PROCEDURE dirf_registro_tipo_2_3 (nr_seq_lote_dirf_p bigint, nr_seq_arquivo_p bigint, cd_cgc_estab_p text, nm_usuario_p text) AS $body$
DECLARE
...
vl_piscofins_w		double precision;
vl_valor_pis_w		double precision;
...
WHERE trunc(p.dt_emissao,'YYYY') = trunc(dt_lote_w,'YYYY') --and 	trunc(decode(ie_data_w,1,p.dt_contabil,p.dt_liquidacao),'YYYY') = trunc(dt_lote_w,'YYYY') 
  and p.cd_cgc = cd_cgc_w and coalesce(i.cd_darf,'1708') = CASE WHEN cd_darf_w='5952' THEN '9999'  ELSE cd_darf_w END and p.ie_situacao <> 'C' and p.ie_origem_titulo in (1,3,5) and coalesce(to_char(p.dt_emissao,'mm'),'00') = lpad(ie_mes_w,2,'0') and exists (select 	1 
		from 	titulo_pagar x, 
...
union all
	...
	and	p.ie_origem_titulo in (1,3,5) 
	and	cd_darf_w = '5952' 
	and	coalesce(to_char(p.dt_emissao,'mm'),'00') = lpad(ie_mes_w,2,'0') 
	and	exists (select 	1 
		from 	titulo_pagar x, 
			titulo_pagar_imposto z, 
			tributo t 
		...
		and	x.cd_cgc = P.cd_cgc 
		and	z.vl_imposto > 0 
		and 	t.ie_tipo_tributo = 'COFINS')) alias25;
	
 
		 
 
	/*select 	nvl(to_char(p.dt_contabil,'mm'),'00'), 
		lpad(replace(nvl(sum(p.vl_titulo),0),',',''),15,'0' ), 
		lpad(replace(nvl(sum(i.vl_imposto),0),',',''),15,'0') 
	from 	titulo_pagar p, 
		titulo_pagar_imposto i 
	where	p.nr_titulo = i.nr_titulo(+) 
	and 	trunc(decode(ie_data_w,1,p.dt_contabil,p.dt_liquidacao),'YYYY') = trunc(dt_lote_w,'YYYY') 
	...
	and	p.ie_situacao <> 'C' 
	group by nvl(to_char(p.dt_contabil,'mm'),'00') 
	order by to_number(nvl(to_char(p.dt_contabil,'mm'),'00')); */
...
WHERE trunc(p.dt_liquidacao,'YYYY') = trunc(dt_lote_w,'YYYY') and (p.cd_pessoa_fisica IS NOT NULL AND p.cd_pessoa_fisica::text <> '') and coalesce(j.cd_darf,'0588') in ('0588','3208') and p.vl_titulo >= vl_min_ano_pf_w and p.ie_tipo_titulo in (SELECT z.ie_tipo_titulo from dirf_regra_tipo_tit z) --and 	p.ie_tipo_titulo in (11,13,9) 
...
union
	...
	--and 	a.ie_tipo_titulo in (11,13,9) 
...
C04 CURSOR FOR  -- titulos das pessoas fisicas. 
	...
	FROM titulo_pagar p
LEFT OUTER JOIN titulo_pagar_imposto i ON (p.nr_titulo = i.nr_titulo)
WHERE trunc(p.dt_liquidacao,'YYYY') = trunc(dt_lote_w,'YYYY') and p.cd_pessoa_fisica = cd_pessoa_fisica_w and coalesce(i.cd_darf,'0588') = cd_darf_w and coalesce(to_char(p.dt_liquidacao,'mm'),'00') = lpad(ie_mes_w,2,'0') and p.ie_tipo_titulo in (SELECT z.ie_tipo_titulo from dirf_regra_tipo_tit z) --and 	p.ie_tipo_titulo in (11,13,9) 
  and p.ie_situacao <> 'C';

	 
	/*select 	lpad(replace(campo_mascara(nvl(sum(nvl(i.vl_base_calculo,p.vl_titulo)),0),2),'.',''),15,'0' ), 
		lpad(replace(campo_mascara(nvl(sum(nvl(d.vl_titulo,0)),0),2),'.',''),15,'0') 
	from 	titulo_pagar d --imposto 
		titulo_pagar_imposto i, 
		titulo_pagar p, --original 
	where	p.nr_titulo = i.nr_titulo(+) 
	and	i.nr_sequencia = d.nr_seq_tributo(+) 
	and 	trunc(p.dt_liquidacao,'YYYY') = trunc(dt_lote_w,'YYYY') 
	and	p.cd_pessoa_fisica = cd_pessoa_fisica_w 
	and	nvl(d.cd_darf,'0588') = cd_darf_w --imposto 
	--and	nvl(i.cd_darf,'0588') = cd_darf_w 
	and	nvl(to_char(p.dt_liquidacao,'mm'),'00') = lpad(ie_mes_w,2,'0') 
	and 	p.ie_tipo_titulo in (11,13,9) 
	and	p.ie_situacao <> 'C';*/
...
union
	...
	FROM titulo_pagar p
LEFT OUTER JOIN titulo_pagar_imposto j ON (p.nr_titulo = j.nr_titulo)
WHERE trunc(p.dt_liquidacao,'YYYY') = trunc(dt_lote_w,'YYYY') and coalesce(j.cd_darf,'0588') in ('0588','3208') and (p.cd_pessoa_fisica IS NOT NULL AND p.cd_pessoa_fisica::text <> '') 
...
union
	...
	and	trunc(a.dt_liquidacao,'YYYY') = trunc(dt_lote_w,'YYYY') 
	and	coalesce(i.cd_darf,'0588') in ('0588','3208') 
	and	(a.cd_pessoa_fisica IS NOT NULL AND a.cd_pessoa_fisica::text <> '')) alias24 
where	cd_darf in ('5952','5960','5979','5987','0588','1708','8045','3280','3208');

c07 CURSOR FOR  -- tipo de registro 3 - Totaliza os valores por codigo da darf 
SELECT lpad(replace(campo_mascara(coalesce(sum(vl_rendimento),'0'),2),'.',''),15,'0'), 
	lpad(replace(campo_mascara(coalesce(sum(vl_imposto),'0'),2),'.',''),15,'0') 
...
union all
	...
	and	a.nr_sequencia = nr_seq_lote_dirf_p 
	and	cd_darf_w = '0588' 
	and	to_char(f.dt_liquidacao,'mm') = lpad(ie_mes_w,2,'0')) alias46;

C08 CURSOR FOR 
	SELECT	lpad(replace(campo_mascara(coalesce(sum((obter_dados_tit_pagar(f.nr_titulo,'V'))::numeric ),0),2),'.',''),15,'0') 
	from	dirf_lote a, 
		gps d, 
		gps_titulo e, 
		titulo_pagar p,  
	...
	and	a.nr_sequencia = nr_seq_lote_dirf_p 
	and	p.cd_pessoa_fisica = cd_pessoa_fisica_w 
	and	to_char(f.dt_liquidacao,'mm') = lpad(ie_mes_w,2,'0');
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	begin 
		...
		-- insere valores de rendimento e IR 
	...
	if (cd_pessoa_fisica_w IS NOT NULL AND cd_pessoa_fisica_w::text <> '') then 
		...
		while (ie_mes_w)::numeric  <= 13 loop 
			...
			EXIT WHEN NOT FOUND; /* apply on c04 */
				vl_rendimento_w := vl_rendimento_w + coalesce(obter_valor_rendimento_dirf_pf(nr_titulo_w),0);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dirf_registro_tipo_2_3 (nr_seq_lote_dirf_p bigint, nr_seq_arquivo_p bigint, cd_cgc_estab_p text, nm_usuario_p text) FROM PUBLIC;
