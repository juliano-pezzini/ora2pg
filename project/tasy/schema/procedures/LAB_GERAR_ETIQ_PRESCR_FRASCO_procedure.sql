...
CREATE TYPE campos AS (nr_seq_grupo    bigint,
            ...
            nr_seq_frasco        bigint);


CREATE OR REPLACE PROCEDURE lab_gerar_etiq_prescr_frasco (nr_prescricao_p bigint, nr_seq_prescr_p text, qt_exames_p bigint, ie_opcao_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
nr_amostra_w	prescr_proc_material.nr_amostra%type;
...
union
    ...
    WHERE    a.nr_seq_exame = b.nr_seq_exame
      ...
      and   a.nr_sequencia  = e.nr_seq_prescr
      AND    c.nr_sequencia = d.nr_seq_material
      and    e.nr_seq_prescr_proc_mat = f.nr_sequencia
      and    g.nr_seq_exame = a.nr_seq_exame
      and    ((g.nr_seq_amostra IS NOT NULL AND g.nr_seq_amostra::text <> '' AND g.nr_seq_amostra = f.nr_amostra)
      or (coalesce(g.nr_seq_amostra::text, '') = ''))
      AND    a.ie_suspenso = 'N'
      AND    a.nr_prescricao = nr_prescricao_p
      AND    a.nr_sequencia = CASE WHEN nr_sequencia_w=0 THEN a.nr_sequencia  ELSE nr_sequencia_w END 
      and     ie_opcao_p = 15
...
--                        ((Vetor_Exame_w(i).cd_cgc_externo = cd_cgc_externo_w) or (Vetor_Exame_w(i).cd_cgc_externo is null)) AND
                        ...
                        ((Vetor_Exame_w[i].nr_seq_frasco = nr_seq_frasco_w) or ((Vetor_Exame_w[i]coalesce(.nr_seq_frasco::text, '') = '') and (coalesce(nr_seq_frasco_w::text, '') = ''))) and
                        ...
                        ((ie_opcao_p IN (1,2,6,10)) OR (Vetor_Exame_w[i].qt_exame < qt_exames_p) and (ie_opcao_p not in (5,15))) or (Vetor_Exame_w[i].nr_seq_prescr_proc_mat = nr_seq_prescr_proc_mat_w and ie_opcao_p = 5) or
                        ((Vetor_Exame_w[i].nr_seq_prescr_proc_mat = nr_seq_prescr_proc_mat_w and ie_opcao_p = 15) or (nr_amostra_w > 1)) and
                        ((Vetor_Exame_w[i].nr_seq_frasco = nr_seq_frasco_w) or ((Vetor_Exame_w[i]coalesce(.nr_seq_frasco::text, '') = '') and (coalesce(nr_seq_frasco_w::text, '') = ''))))  THEN
...
--            end if;
            ...
            Vetor_Exame_w[y].nr_seq_frasco        := nr_seq_frasco_w;
...
nr_etiqueta, nr_amostra, nr_seq_prescr_proc_mat, cd_cgc_externo, ie_externo, nr_seq_grupo_imp, ie_urgencia, nr_seq_frasco, ds_material_especial)
    VALUES (nextval('w_lab_etiqueta_seq'), nm_usuario_p, nr_prescricao_p, Vetor_Exame_w[i].nr_seq_grupo, Vetor_Exame_w[i].nr_seq_material,
            ...
            Vetor_Exame_w[i].ie_urgencia, Vetor_Exame_w[i].nr_seq_frasco, Vetor_Exame_w[i].ds_material_especial);
END LOOP;
COMMIT;
END;
$body$
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE lab_gerar_etiq_prescr_frasco (nr_prescricao_p bigint, nr_seq_prescr_p text, qt_exames_p bigint, ie_opcao_p bigint, nm_usuario_p text) FROM PUBLIC;
