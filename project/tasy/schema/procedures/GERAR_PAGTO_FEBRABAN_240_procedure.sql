...
CREATE OR REPLACE PROCEDURE gerar_pagto_febraban_240 ( nr_seq_envio_p bigint, nm_usuario_p text) AS $body$
DECLARE

 
/* PADRÃO CNAB 240 VERSÃO 084 DE 07/2010 */
...
c02 CURSOR FOR 
SELECT	b.cd_banco, 
	lpad(substr(coalesce(b.cd_agencia_bancaria,'0'),1,5) || substr(coalesce(b.ie_digito_agencia,'0'),1,1),6,'0'), 
	...
	CASE WHEN b.ie_tipo_pagamento='DOC' THEN CASE WHEN cd_banco_pagto_w=b.cd_banco THEN '000'  ELSE '700' END  WHEN b.ie_tipo_pagamento='TED' THEN '018'  ELSE '000' END  cd_camara 
...
from	titulo_pagar c, 
	...
	banco_escritural a 
where	ie_forma_lanc_w		= CASE WHEN b.cd_banco=cd_banco_pagto_w THEN '30'  ELSE '31' END  
...
select	lpad(b.cd_cgc,14,'0'), 
	...
	lpad(substr(coalesce(c.cd_agencia_bancaria,'0'),1,5) || substr(coalesce(d.ie_digito,'0'),1,1),6,'0'), 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_pagto_febraban_240 ( nr_seq_envio_p bigint, nm_usuario_p text) FROM PUBLIC;
