...
CREATE OR REPLACE PROCEDURE gerar_fanep ( cd_pessoa_fisica_p text, cd_medico_anestesista_p text, nr_atendimento_p bigint, nr_seq_agenda_p bigint, nr_sequencia_p INOUT bigint, nm_usuario_p text, dt_processo_p timestamp default null) AS $body$
DECLARE
...
nr_seq_agenda_ww := to_char(nr_seq_agenda_p);
nm_procedure_w := substr('GERAR_FANEP' || nr_seq_agenda_ww,1,32);
...
if (qt_processo_w = 0) then
  begin
  CALL gravar_processo_longo('Execucao da geracao da FANEP', nm_procedure_w, 1);
  ...
  if not(coalesce(nr_seq_pepo_w,0) > 0) then
...
;
  ...
  begin
  CALL wheb_mensagem_pck.Exibir_Mensagem_Abort(1130944);
  end;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_fanep ( cd_pessoa_fisica_p text, cd_medico_anestesista_p text, nr_atendimento_p bigint, nr_seq_agenda_p bigint, nr_sequencia_p INOUT bigint, nm_usuario_p text, dt_processo_p timestamp default null) FROM PUBLIC;
