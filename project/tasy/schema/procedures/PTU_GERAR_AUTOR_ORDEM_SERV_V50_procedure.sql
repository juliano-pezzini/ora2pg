...
CREATE OR REPLACE PROCEDURE ptu_gerar_autor_ordem_serv_v50 ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, nm_usuario_p text, nr_seq_aut_ordem_serv_p INOUT ptu_autorizacao_ordem_serv.nr_sequencia%type) AS $body$
DECLARE
...
if	((nr_seq_guia_p IS NOT NULL AND nr_seq_guia_p::text <> '') or (nr_seq_requisicao_p IS NOT NULL AND nr_seq_requisicao_p::text <> '')) then
	...
	insert into ptu_autorizacao_ordem_serv(nr_sequencia, dt_atualizacao, nm_usuario,
		 ...
		 ds_mensagem)
	values (nr_seq_aut_ordem_serv_p, clock_timestamp(), nm_usuario_p,
		 ...
		 nr_transacao_solicitante_w, cd_unimed_solicitante_w, '050',
		 ds_observacao_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_autor_ordem_serv_v50 ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, nm_usuario_p text, nr_seq_aut_ordem_serv_p INOUT ptu_autorizacao_ordem_serv.nr_sequencia%type) FROM PUBLIC;
