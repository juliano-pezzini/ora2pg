...
CREATE OR REPLACE PROCEDURE gerar_menuitem_cron_proj_migr ( nr_seq_cronograma_p bigint, cd_funcao_p bigint, nr_seq_atividade_p bigint, nm_usuario_p text) AS $body$
DECLARE


nr_seq_atividade_w		bigint;
nr_seq_apresent_w		bigint := 3000;
...
from (
	...
	and	somente_nao_numero(a.nm_objeto) <> 'N'
	group by
		a.nm_objeto
	order by
		a.nm_objeto
...
if (nr_seq_cronograma_p IS NOT NULL AND nr_seq_cronograma_p::text <> '') and (cd_funcao_p IS NOT NULL AND cd_funcao_p::text <> '') and (nr_seq_atividade_p IS NOT NULL AND nr_seq_atividade_p::text <> '') and (nm_usuario_p IS NOT NULL AND nm_usuario_p::text <> '') then
	...
	CALL gerar_menu_cron_migr_dic_obj2(cd_funcao_p, nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_menuitem_cron_proj_migr ( nr_seq_cronograma_p bigint, cd_funcao_p bigint, nr_seq_atividade_p bigint, nm_usuario_p text) FROM PUBLIC;
