...
CREATE OR REPLACE PROCEDURE gerar_parecer_medicamentos ( nr_seq_encaminhamento_p bigint, ds_conteudo_p text, nr_seq_regulacao_p bigint, nr_seq_parecer_p INOUT bigint) AS $body$
DECLARE
...
BEGIN
	...
	if (dt_avaliacao_w IS NOT NULL AND dt_avaliacao_w::text <> '') then
		...
		Select 	obter_desc_expressao(314903) -- Solicitações de material / medicamento
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_parecer_medicamentos ( nr_seq_encaminhamento_p bigint, ds_conteudo_p text, nr_seq_regulacao_p bigint, nr_seq_parecer_p INOUT bigint) FROM PUBLIC;
