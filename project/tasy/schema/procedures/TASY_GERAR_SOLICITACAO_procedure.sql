...
--
-- dblink wrapper to call function tasy_gerar_solicitacao as an autonomous transaction
--
CREATE EXTENSION IF NOT EXISTS dblink;

CREATE OR REPLACE PROCEDURE tasy_gerar_solicitacao ( cd_pessoa_fisica_p bigint) AS $body$
DECLARE
...
BEGIN
	v_query := 'CALL tasy_gerar_solicitacao_atx ( ' || quote_nullable(cd_pessoa_fisica_p) || ' )';
...
CREATE OR REPLACE PROCEDURE tasy_gerar_solicitacao_atx ( cd_pessoa_fisica_p bigint) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (i_w = 0) then
		...
		insert into tasy_solic_alteracao(	 nr_sequencia,dt_atualizacao,nm_usuario,dt_atualizacao_nrec,nm_usuario_nrec,ie_processo,
				cd_funcao, cd_estabelecimento, ie_status, cd_pessoa_fisica)
		values (	nr_seq_tasy_solic_alt_w,clock_timestamp(),nm_usuario_w,clock_timestamp(),nm_usuario_w,'M',
				wheb_usuario_pck.get_cd_funcao, wheb_usuario_pck.get_cd_estabelecimento, 'A', cd_pessoa_fisica_p);
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE tasy_gerar_solicitacao ( cd_pessoa_fisica_p bigint) FROM PUBLIC; -- REVOKE ALL ON PROCEDURE tasy_gerar_solicitacao_atx ( cd_pessoa_fisica_p bigint) FROM PUBLIC;
