...
CREATE OR REPLACE PROCEDURE alterar_dt_entrega_prescr (nr_prescricao_p bigint, cd_perfil_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
IF (ie_ajustar_entrega_w	= 'R') then

	CALL ajustar_data_prescr_maior_proc(nr_prescricao_p,				
                                 nm_usuario_p,				    
                                 'N'
                                );
END IF;
...
--Bruno
	...
	where	a.nr_atendimento = (SELECT x.nr_atendimento from prescr_medica x where x.nr_prescricao = nr_prescricao_p)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE alterar_dt_entrega_prescr (nr_prescricao_p bigint, cd_perfil_p bigint, nm_usuario_p text) FROM PUBLIC;
