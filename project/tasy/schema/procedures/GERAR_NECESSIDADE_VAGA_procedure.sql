...
CREATE OR REPLACE PROCEDURE gerar_necessidade_vaga ( nr_atendimento_p bigint, ie_solicitacao_p text, ie_tipo_vaga_p text, cd_motivo_transf_p bigint, cd_setor_desejado_p bigint, cd_acomodacao_desejada_p bigint, cd_estabelecimento_p bigint, cd_convenio_p bigint, cd_categoria_p text, cd_plano_p text, ds_hd_p text, cd_procedimento_p bigint, ie_origem_proced_p bigint, nm_usuario_p text, nr_seq_proc_interno_p bigint, cd_medico_resp_p text, ie_clinica_p bigint, cd_cid_principal_p text, ie_necessita_isolamento_p text, ie_necessita_dialise_p text, cd_unid_basica_p text default null, cd_unid_compl_p text default null, cd_funcionario_p text default null, nr_seq_classif_medico_p bigint default null, qt_dia_p bigint default null, ie_opme_p text default null, ie_quimioterapia_p text default null, nr_seq_p bigint default null, qt_acompanhante_p bigint default null, ds_observacao_p text default null, cd_departamento_desejado_p bigint default null, ie_tipo_isolamento_p text default null, ie_agendamento_cirurgico_p text default null, ie_solic_uti_eci_p text default null, nr_seq_linha_cuidado_p bigint default null, cd_especialidade_p bigint default null, nr_seq_prioridade_p bigint default null, DT_ADMISSION_P timestamp default null, DS_RISKS_P text default null, CD_SPECIALTY_P bigint default null, IE_PRIVATE_ROOM_P text default null, NR_SEQUENCIA_P INOUT bigint DEFAULT NULL, DT_PREVISTA_P timestamp default null, DS_LISTA_PROFISSIONAIS_P text default null) AS $body$
DECLARE
...
cd_departamento_atual_w atend_paciente_unidade.cd_departamento%type;
...
ie_gerar_evolucao_bed_req_w     varchar(1);
...
ie_gerar_evolucao_bed_req_w := obter_param_usuario(281, 1642, obter_perfil_ativo, wheb_usuario_pck.get_nm_usuario, coalesce(wheb_usuario_pck.get_cd_estabelecimento, 1), ie_gerar_evolucao_bed_req_w);
...
elsif (coalesce(nr_seq_proc_interno_p,0) = 0) and (cd_procedimento_p IS NOT NULL AND cd_procedimento_p::text <> '') and (ie_origem_proced_p IS NOT NULL AND ie_origem_proced_p::text <> '') and (cd_procedimento_p	> 0) then
	cd_procedimento_w	:= cd_procedimento_p;
	ie_origem_proced_w	:= ie_origem_proced_p;
elsif (coalesce(nr_seq_proc_interno_p,0) > 0) and (coalesce(cd_procedimento_p::text, '') = '') and (coalesce(ie_origem_proced_p::text, '') = '') then
	begin
		select	cd_procedimento,
				ie_origem_proced
...
IF (ie_existe_w = 'N') then
	...
	select	obter_tipo_acomod_atend(nr_atendimento_w,'C'),
		...
		obter_ultimo_depto_medico(nr_atendimento_w),
	...
	*/
	...
	ds_observacao_w := substr((ds_observacao_w || ' ' || ds_observacao_p),1,4000);
	...
	*/
	...
	select  max(coalesce(cd_especialidade, null))
	...
	;
		...
		values (
			...
			CASE WHEN coalesce(dt_prevista_p::text, '') = '' THEN CASE WHEN VarGerarDataPrevista_w='S' THEN clock_timestamp()  ELSE null END   ELSE dt_prevista_p END ,
			ds_observacao_w,
			cd_unid_basica_p,
			cd_unid_compl_p,
			coalesce(cd_especialidade_w, cd_especialidade_p),
			cd_funcionario_p,
			...
			ie_agendamento_cirurgico_p,
			ie_solic_uti_eci_p,
			nr_seq_linha_cuidado_p,
			nr_seq_prioridade_p,
			DT_ADMISSION_P,
			DS_RISKS_P,
			CD_SPECIALTY_P,
		...
		CALL Gerar_Autor_Regra(nr_atendimento_w, null, null, null, null, null, 'GVE', nm_usuario_p, null,null, nr_sequencia_w,null,null,null,'','','');

		SELECT * FROM obter_reg_regulacao_gest_vagas(ie_solicitacao_p, ie_tipo_vaga_p, ie_espec_valid_w, ie_cid_valid_w) INTO STRICT ie_espec_valid_w, ie_cid_valid_w;

		if ((coalesce(ie_espec_valid_w, 'N') = 'N') or ((coalesce(ie_espec_valid_w, 'N') = 'S') and (coalesce(cd_especialidade_w, coalesce(cd_especialidade_p, 0)) > 0)))
			and ((coalesce(ie_cid_valid_w, 'N') = 'N') or ((coalesce(ie_cid_valid_w, 'N') = 'S') and (cd_cid_principal_p IS NOT NULL AND cd_cid_principal_p::text <> ''))) then
			ie_consistencia_w := CONSISTIR_REGULACAO(nr_atendimento_w, nr_sequencia_w, 'GESTAO_VAGA', ie_consistencia_w);
		end if;

		if (ds_lista_profissionais_p IS NOT NULL AND ds_lista_profissionais_p::text <> '') then
			CALL gravar_prof_atendimento(nr_atendimento_w, nr_sequencia_w, ds_lista_profissionais_p, nm_usuario_p);
		end if;
...
IF (nr_sequencia_w IS NOT NULL AND nr_sequencia_w::text <> '') THEN
  ...
  CALL Gerar_Evolucao_Neces_Vaga(nr_sequencia_p, nm_usuario_p, cd_estabelecimento_p);
END IF;

if ( ie_gerar_evolucao_bed_req_w = 'S' ) then
	if (nr_atendimento_w IS NOT NULL AND nr_atendimento_w::text <> '') then
		cd_evolucao_w := clinical_notes_pck.gerar_soap(nr_atendimento_w, nr_sequencia_p, 'BED_RQST', NULL, 'P', 1, cd_evolucao_w, null, cd_pessoa_fisica_w, null);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_necessidade_vaga ( nr_atendimento_p bigint, ie_solicitacao_p text, ie_tipo_vaga_p text, cd_motivo_transf_p bigint, cd_setor_desejado_p bigint, cd_acomodacao_desejada_p bigint, cd_estabelecimento_p bigint, cd_convenio_p bigint, cd_categoria_p text, cd_plano_p text, ds_hd_p text, cd_procedimento_p bigint, ie_origem_proced_p bigint, nm_usuario_p text, nr_seq_proc_interno_p bigint, cd_medico_resp_p text, ie_clinica_p bigint, cd_cid_principal_p text, ie_necessita_isolamento_p text, ie_necessita_dialise_p text, cd_unid_basica_p text default null, cd_unid_compl_p text default null, cd_funcionario_p text default null, nr_seq_classif_medico_p bigint default null, qt_dia_p bigint default null, ie_opme_p text default null, ie_quimioterapia_p text default null, nr_seq_p bigint default null, qt_acompanhante_p bigint default null, ds_observacao_p text default null, cd_departamento_desejado_p bigint default null, ie_tipo_isolamento_p text default null, ie_agendamento_cirurgico_p text default null, ie_solic_uti_eci_p text default null, nr_seq_linha_cuidado_p bigint default null, cd_especialidade_p bigint default null, nr_seq_prioridade_p bigint default null, DT_ADMISSION_P timestamp default null, DS_RISKS_P text default null, CD_SPECIALTY_P bigint default null, IE_PRIVATE_ROOM_P text default null, NR_SEQUENCIA_P INOUT bigint DEFAULT NULL, DT_PREVISTA_P timestamp default null, DS_LISTA_PROFISSIONAIS_P text default null) FROM PUBLIC;
