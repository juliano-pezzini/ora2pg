...
CREATE OR REPLACE PROCEDURE gerar_kit_mat_auditoria (CD_KIT_MATERIAL_P bigint, DT_ATENDIMENTO_P timestamp, NR_PRESCRICAO_P bigint, IE_AUDITORIA_P text, CD_LOCAL_ESTOQUE_P text, NR_SEQ_PROC_PRINC_P text, NR_SEQ_ATEPACU_P bigint, IE_BAIXA_SEM_ESTOQUE_P text, NM_USUARIO_P text, QT_KIT_P bigint, DS_ERRO_P INOUT text, nr_doc_convenio_p bigint, nr_seq_auditoria_p bigint) AS $body$
DECLARE
...
msg_erro_padrao_w		varchar(4000):= WHEB_MENSAGEM_PCK.get_texto(278990) || CHR(13);
...
LOOP 
	...
	if (IE_BAIXA_SEM_ESTOQUE_P = 'S') or (ie_baixa_estoque_w = 'N') OR (coalesce(cd_local_estoque_w::text, '') = '') or (IE_ESTOQUE_DISP_w = 'S') then 
		...
		if (ie_entra_conta_w	= 'S') then 
			...
			CALL ATUALIZA_PRECO_MATERIAL(NR_SEQ_W, NM_USUARIO_P);/*AQUI RECALCULA O MATERIAL*/
			...
			CALL atualizar_itens_audit_conta(nr_seq_auditoria_p, nr_seq_w, 1, (QT_MATERIAL_W * QT_KIT_P), nm_usuario_p, null);
		else 
			if (ie_baixa_estoque_w	= 'S') and (ie_baixa_estoq_pac_w  = 'S') and (cd_local_estoque_w IS NOT NULL AND cd_local_estoque_w::text <> '') then 
			CALL Gerar_Prescricao_Estoque(cd_estabelecimento_W, 
					NR_ATENDIMENTO_W, 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_kit_mat_auditoria (CD_KIT_MATERIAL_P bigint, DT_ATENDIMENTO_P timestamp, NR_PRESCRICAO_P bigint, IE_AUDITORIA_P text, CD_LOCAL_ESTOQUE_P text, NR_SEQ_PROC_PRINC_P text, NR_SEQ_ATEPACU_P bigint, IE_BAIXA_SEM_ESTOQUE_P text, NM_USUARIO_P text, QT_KIT_P bigint, DS_ERRO_P INOUT text, nr_doc_convenio_p bigint, nr_seq_auditoria_p bigint) FROM PUBLIC;
