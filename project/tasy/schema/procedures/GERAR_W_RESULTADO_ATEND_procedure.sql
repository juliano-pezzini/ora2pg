...
CREATE OR REPLACE PROCEDURE gerar_w_resultado_atend ( dt_inicial_p timestamp, dt_final_p timestamp, cd_convenio_p bigint, nr_seq_protocolo_p bigint, nr_atendimento_p bigint, nr_interno_conta_p bigint, cd_medico_p bigint, ie_quebra_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
and	((nr_seq_protocolo	= coalesce(nr_seq_protocolo_ww, nr_seq_protocolo)) or (coalesce(nr_seq_protocolo_ww::text, '') = '')) 
...
select	CASE WHEN nr_atendimento_p=0 THEN null  ELSE nr_atendimento_p END  nr_atendimento, 
	...
	CASE WHEN nr_seq_protocolo_p=0 THEN null  ELSE nr_seq_protocolo_p END  nr_seq_protocolo, 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_w_resultado_atend ( dt_inicial_p timestamp, dt_final_p timestamp, cd_convenio_p bigint, nr_seq_protocolo_p bigint, nr_atendimento_p bigint, nr_interno_conta_p bigint, cd_medico_p bigint, ie_quebra_p bigint, nm_usuario_p text) FROM PUBLIC;
