...
CREATE OR REPLACE PROCEDURE insert_dpr_request ( nr_interno_conta_p bigint, --account number patient billing
 cd_estabelecimento_p bigint, nm_usuario_p text, cd_error_string_p INOUT text) AS $body$
DECLARE


nr_seq_dvr_w					dbs_claim.nr_sequencia%type;
cd_service_provider_w				dbs_claim.cd_provider%type;
cd_claim_w					dbs_claim.cd_claim%type;
dt_claim_w					dbs_claim.dt_atualizacao%type;
...
select	nextval('dpr_request_seq')
...
into STRICT	cd_service_provider_w,
	cd_claim_w,
...
if (coalesce(cd_service_provider_w::text, '') = '' ) then
...
if (coalesce(cd_error_string_p::text, '') = '') then
	...
	insert	into dpr_request(nr_sequencia ,
				dt_atualizacao ,
				nm_usuario ,
				dt_atualizacao_nrec,
				nm_usuario_nrec,
	...
	CALL execute_bifrost_integration(201, request_sts);
	commit;
	end;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE insert_dpr_request ( nr_interno_conta_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text, cd_error_string_p INOUT text) FROM PUBLIC;
