...
CREATE OR REPLACE PROCEDURE san_gerar_ex_lab_transf (nr_seq_transfusao_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	where	exists (
		SELECT 1 from san_producao_v b
		where b.nr_seq_derivado = a.nr_seq_derivado and b.nr_seq_transfusao = nr_seq_transfusao_p
	)
	and	a.ie_situacao = 'A';
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE san_gerar_ex_lab_transf (nr_seq_transfusao_p bigint, nm_usuario_p text) FROM PUBLIC;
