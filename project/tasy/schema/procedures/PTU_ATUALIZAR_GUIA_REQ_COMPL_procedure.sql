...
CREATE OR REPLACE PROCEDURE ptu_atualizar_guia_req_compl ( nr_seq_pedido_aut_compl_p ptu_pedido_compl_aut.nr_sequencia%type, ie_possui_pedido_p text, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) AS $body$
DECLARE
...
for c01_w in C01( nr_seq_pedido_aut_compl_p ) loop 
	...
	if ( ie_possui_pedido_p = 'N' ) then 
		...
		--Select para verificar se o prestador é de alto custo, este campo é informado no Pedido de Autorização 00600 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_atualizar_guia_req_compl ( nr_seq_pedido_aut_compl_p ptu_pedido_compl_aut.nr_sequencia%type, ie_possui_pedido_p text, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) FROM PUBLIC;
