...
CREATE OR REPLACE PROCEDURE vivalle_gerar_w_fluxo_saldo_v2 ( cd_empresa_p bigint, cd_estabelecimento_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, nm_usuario_p text) AS $body$
DECLARE
...
while(dt_referencia_w >= dt_inicial_p) and (dt_referencia_w <= dt_final_p) loop
    ...
    select    coalesce(sum(obter_saldo_banco_diario(b.nr_sequencia, fim_dia(dt_referencia_w))),0)
    ...
    and    b.dt_referencia =    (SELECT    max(dt_referencia)
                    ...
                    and    trunc(x.dt_referencia,'dd') <= dt_referencia_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE vivalle_gerar_w_fluxo_saldo_v2 ( cd_empresa_p bigint, cd_estabelecimento_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, nm_usuario_p text) FROM PUBLIC;
