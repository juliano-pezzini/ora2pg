...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_b510_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint) AS $body$
DECLARE
...
c_reg_B510 CURSOR FOR
SELECT 	*
from 	fis_efd_icmsipi_B510
where 	nr_seq_controle = nr_seq_controle_p;

/*Criação do array com o tipo sendo do cursor eespecificado - c_reg_B510*/

type reg_c_reg_B510 is table of fis_efd_icmsipi_B510%RowType;
vetRegB510 reg_c_reg_B510;
...
open c_reg_B510;
loop
fetch c_reg_B510 bulk collect into vetRegB510 limit 1000;
  for i in 1 .. vetRegB510.Count loop
  begin
  ...
  ds_linha_w := substr(	ds_sep_w || vetRegB510[i].cd_reg ||
			ds_sep_w || vetRegB510[i].ie_ind_prof ||
			ds_sep_w || vetRegB510[i].ie_ind_esc ||
			ds_sep_w || vetRegB510[i].ie_ind_soc ||
			ds_sep_w || vetRegB510[i].nr_cpf ||
			ds_sep_w || vetRegB510[i].ds_nome ||
			ds_sep_w,
			1,
			8000);
...
;
  ...
  fis_registros_w[qt_cursor_w].cd_registro     		:=  vetRegB510[i].cd_reg;
...
EXIT WHEN NOT FOUND; /* apply on c_reg_B510 */
end loop;
close c_reg_B510;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_b510_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint) FROM PUBLIC;
