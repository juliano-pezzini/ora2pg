...
CREATE OR REPLACE PROCEDURE plt_estender_liberacao ( nr_prescricao_p bigint, nm_usuario_p text, cd_perfil_p bigint, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
ie_acm_w			varchar(1);
hr_prim_hor_leite_w		varchar(5);
hr_prim_hor_mat_w		varchar(5);
...
if (nr_prescricao_p IS NOT NULL AND nr_prescricao_p::text <> '')	then 
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
	 	begin  
		nr_ocorrencia_w := 0;
		SELECT * FROM Calcular_Horario_Prescricao(nr_prescricao_w, cd_intervalo_w, coalesce(dt_primeiro_horario_w,dt_prescricao_w), to_date(hr_prim_hor_mat_w,'hh24:mi'), nr_horas_validade_w, cd_material_w, qt_hora_intervalo_w, qt_min_intervalo_w, nr_ocorrencia_w, ds_horarios_w, ds_horarios2_w, 'N', null) INTO STRICT nr_ocorrencia_w, ds_horarios_w, ds_horarios2_w;
	...
	EXIT WHEN NOT FOUND; /* apply on c03 */
		...
		set	qt_tempo_aplicacao	=	qt_tempo_aplicacao + 24 
		...
		if (ie_tipo_dosagem_w IS NOT NULL AND ie_tipo_dosagem_w::text <> '')	and (upper(ie_tipo_dosagem_w) <> 'ACM') and 
	...
	CALL plt_ajustar_extensao_prescr(nr_prescricao_p,nm_usuario_p);
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE plt_estender_liberacao ( nr_prescricao_p bigint, nm_usuario_p text, cd_perfil_p bigint, cd_estabelecimento_p bigint) FROM PUBLIC;
