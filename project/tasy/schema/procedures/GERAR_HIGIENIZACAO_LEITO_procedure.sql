...
CREATE OR REPLACE PROCEDURE gerar_higienizacao_leito (dt_servico_p timestamp, nm_usuario_p text, cd_estabelecimento_p bigint, ie_evento_p text, cd_motivo_alta_p bigint, nr_atendimento_p bigint, cd_tipo_acomod_transf_p bigint default null) AS $body$
DECLARE
...
is_atend_rn_w				varchar(1);

c01 CURSOR FOR
	SELECT	a.nr_sequencia,
			a.nr_seq_servico,
			ESTABLISHMENT_TIMEZONE_UTILS.dateAtTime(dt_servico_p, coalesce(dt_prevista, clock_timestamp())),
...
select obter_se_Atend_rn_prag(nr_atendimento_p)
into STRICT is_atend_rn_w
;

if (is_atend_rn_w = 'S') then
	nr_seq_atepacu_w := obter_atepacu_paciente(nr_atendimento_p, 'IAR');
else
...
and		a.cd_estabelecimento	= cd_estabelecimento_w
and (e.nr_atendimento 		= nr_atendimento_p or coalesce(e.nr_atendimento,0) = 0)
and		c.nr_seq_interno		= nr_seq_atepacu_w
and (coalesce(ie_somente_ativo_w,'N') = 'N' or (a.ie_situacao = 'A' and e.ie_situacao = 'A'));
exception
	when others then
...
values (nm_usuario_p,
		...
		substr('1 - nr_atendimento: ' || nr_atendimento_p || chr(13) ||
				...
				'dt_servico_p: ' || PKG_DATE_FORMATERS_TZ.TO_VARCHAR(dt_servico_p, 'timestamp', ESTABLISHMENT_TIMEZONE_UTILS.getTimeZone)|| chr(13) ||
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	if (qt_regra_w = 0) and (ie_gerar_servico_w = 'S') then
		...
		if (ie_hig_leito_ant_w = 'N') then
			...
			if (coalesce(cd_tipo_acomod_transf_p, 0) > 0) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_higienizacao_leito (dt_servico_p timestamp, nm_usuario_p text, cd_estabelecimento_p bigint, ie_evento_p text, cd_motivo_alta_p bigint, nr_atendimento_p bigint, cd_tipo_acomod_transf_p bigint default null) FROM PUBLIC;
