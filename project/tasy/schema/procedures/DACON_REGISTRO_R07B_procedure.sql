...
CREATE OR REPLACE PROCEDURE dacon_registro_r07b ( nr_seq_dacon_p bigint, nm_usuario_P text, vl_receita_p INOUT bigint) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	and	cd_registro  = 'R07B';

C02 CURSOR FOR
	SELECT	rpad('R07B',4,' ')						--01
		||lpad((to_char(dt_referencia, 'MM'))::numeric ,2,0)		--02
		||lpad(1,2,0) 							--03
		||lpad(0,14,0)							--04
		||lpad(somente_numero(coalesce(vl_receita_bens_w,0)),14,0) 					--05
		||lpad(somente_numero(coalesce(vl_receita_bem_165_w,0)),14,0)				--06
		||lpad(0,14,0)							--07
		||lpad(0,14,0)					--08
		||lpad(somente_numero(coalesce(vl_receita_bens_w,0)),14,0)							--09
		||lpad(somente_numero(coalesce(vl_receita_bem_065_w,0)),14,0)				--10
		||lpad(0,14,0)							--11
		||lpad(0,14,0)							--12
		||lpad(0,14,0)							--13
		||lpad(0,14,0)							--14
...
vl_receita_bem_165_w	:=	(vl_receita_bens_w * '1,65') / 100;
vl_receita_bem_065_w	:=	(vl_receita_bens_w * '0,65') / 100;
...
vl_receita_p	:=	vl_receita_bem_165_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dacon_registro_r07b ( nr_seq_dacon_p bigint, nm_usuario_P text, vl_receita_p INOUT bigint) FROM PUBLIC;
