...
CREATE OR REPLACE PROCEDURE ptu_gerar_a1200_13_0 ( nr_seq_pacote_p ptu_pacote.nr_sequencia%type, cd_interface_p interface.cd_interface%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
c01 CURSOR(nr_seq_pacote_pc  ptu_pacote.nr_sequencia%type) FOR
	SELECT	nr_sequencia,
		...
		coalesce(nr_versao_transacao,'08') nr_versao_transacao
	from	ptu_pacote
	where	nr_sequencia	= nr_seq_pacote_pc;
	
-- R202 PACOTE (OBRIGATORIO)
c02 CURSOR(nr_seq_pacote_pc  ptu_pacote.nr_sequencia%type) FOR
	SELECT	nr_sequencia,		
		...
		lpad(coalesce(replace(replace(campo_mascara(get_tot_serv_item(ptu_pacote_reg.nr_sequencia, 1),2),',',null),'.',null),'0'),14,'0') vl_tot_taxas,
		lpad(coalesce(replace(replace(campo_mascara(get_tot_serv_item(ptu_pacote_reg.nr_sequencia, 2),2),',',null),'.',null),'0'),14,'0') vl_tot_diarias,
		lpad(coalesce(null,'0'),14,'0') vl_tot_gases,        
		lpad(coalesce(replace(replace(campo_mascara(get_tot_serv_item(ptu_pacote_reg.nr_sequencia, 4),2),',',null),'.',null),'0'),14,'0') vl_tot_mat,
		lpad(coalesce(replace(replace(campo_mascara(get_tot_serv_item(ptu_pacote_reg.nr_sequencia, 5),2),',',null),'.',null),'0'),14,'0') vl_tot_med,
		lpad(coalesce(replace(replace(campo_mascara(get_tot_serv_item(ptu_pacote_reg.nr_sequencia, 6),2),',',null),'.',null),'0'),14,'0') vl_tot_proc,
		lpad(coalesce(replace(replace(campo_mascara(get_tot_serv_item(ptu_pacote_reg.nr_sequencia, 7),2),',',null),'.',null),'0'),14,'0') vl_tot_opme,
		lpad(coalesce(replace(replace(campo_mascara(get_tot_serv_item(ptu_pacote_reg.nr_sequencia),2),',',null),'.',null),'0'),14,'0') vl_tot_pacote,				
...
-- R204 SERVICO - PACOTE (OBRIGATORIO)
c03 CURSOR(nr_seq_pacote_reg_pc  ptu_pacote_reg.nr_sequencia%type) FOR
	SELECT	nr_sequencia,
		...
		lpad(coalesce(qt_servico*10000,'0'),9,'0') qt_servico,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_a1200_13_0 ( nr_seq_pacote_p ptu_pacote.nr_sequencia%type, cd_interface_p interface.cd_interface%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
