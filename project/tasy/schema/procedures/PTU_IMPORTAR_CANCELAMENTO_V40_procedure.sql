...
CREATE OR REPLACE PROCEDURE ptu_importar_cancelamento_v40 ( ds_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_origem_p INOUT bigint) AS $body$
DECLARE
		...
		if (nr_seq_import_w	= 1) then 
			...
			when others then 
				...
				if (coalesce(nr_seq_origem_p::text, '') = '') then 
					nr_seq_origem_p	:= nr_seq_execucao_w;
				end if;
			end;
				 
			if (cd_transacao_w	= '00311') then 
				if (nr_seq_pedido_compl_w	<> 0) then 
					...
					select coalesce(max(nr_sequencia),2) 
					into STRICT 	nr_seq_cancel_wsd_w 
					from 	pls_guia_motivo_cancel 
					where 	ie_motivo_cancel_wsd = 'S';
...
CALL ptu_gestao_envio_confirmacao(nr_seq_origem_p, cd_estabelecimento_p, 'C', nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_importar_cancelamento_v40 ( ds_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_origem_p INOUT bigint) FROM PUBLIC;
