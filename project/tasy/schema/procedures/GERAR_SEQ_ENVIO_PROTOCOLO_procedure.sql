...
CREATE OR REPLACE PROCEDURE gerar_seq_envio_protocolo ( nr_seq_protocolo_p bigint, cd_convenio_p bigint, cd_interface_envio_p bigint, ie_tipo_protocolo_p text, dt_mesano_referencia_p timestamp, nm_usuario_p text, nr_seq_envio_conv_p INOUT bigint, nr_seq_env_calc_p INOUT bigint, nm_arquivo_p INOUT text) AS $body$
DECLARE
...
union all
	...
	if (ie_regra_arquivo_w IS NOT NULL AND ie_regra_arquivo_w::text <> '') then
		...
		cd_interno_w := obter_valor_conv_estab(cd_convenio_p,cd_estabelecimento_w, 'CD_INTERNO');
		...
		if (coalesce(cd_interface_envio_p,0) = 0) then
		...
		select	coalesce(max(CASE WHEN CASE WHEN coalesce(cd_interface_envio_p,0)=0 THEN cd_interface_envio_ww  ELSE coalesce(cd_interface_envio_p,0) END =1010 THEN '.035' WHEN CASE WHEN coalesce(cd_interface_envio_p,0)=0 THEN cd_interface_envio_ww  ELSE coalesce(cd_interface_envio_p,0) END =1009 THEN '.055' WHEN CASE WHEN coalesce(cd_interface_envio_p,0)=0 THEN cd_interface_envio_ww  ELSE coalesce(cd_interface_envio_p,0) END =1006 THEN CASE WHEN ie_tipo_protocolo_p=1 THEN '.075'  ELSE '.085' END  WHEN CASE WHEN coalesce(cd_interface_envio_p,0)=0 THEN cd_interface_envio_ww  ELSE coalesce(cd_interface_envio_p,0) END =2676 THEN CASE WHEN ie_tipo_protocolo_p=1 THEN '.075'  ELSE '.085' END  END ),'.000')
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_seq_envio_protocolo ( nr_seq_protocolo_p bigint, cd_convenio_p bigint, cd_interface_envio_p bigint, ie_tipo_protocolo_p text, dt_mesano_referencia_p timestamp, nm_usuario_p text, nr_seq_envio_conv_p INOUT bigint, nr_seq_env_calc_p INOUT bigint, nm_arquivo_p INOUT text) FROM PUBLIC;
