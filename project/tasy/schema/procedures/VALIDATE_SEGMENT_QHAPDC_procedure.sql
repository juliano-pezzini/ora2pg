...
CREATE OR REPLACE PROCEDURE validate_segment_qhapdc (nr_seq_segment_p bigint, nr_seq_dataset_p bigint, ie_segmento_p text, nm_usuario_p text, nr_atendimento_p bigint) AS $body$
DECLARE
...
BEGIN
    ...
    LOOP 
        ...
        IF ( coalesce(trim(both vl_retorno_w)::text, '') = '' ) THEN 
          ...
          CALL Generate_dataset_inconsistency(nr_seq_dataset_p, 'E', 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE validate_segment_qhapdc (nr_seq_segment_p bigint, nr_seq_dataset_p bigint, ie_segmento_p text, nm_usuario_p text, nr_atendimento_p bigint) FROM PUBLIC;
