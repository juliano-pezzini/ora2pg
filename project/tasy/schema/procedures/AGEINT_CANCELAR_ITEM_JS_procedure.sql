...
CREATE OR REPLACE PROCEDURE ageint_cancelar_item_js (nr_seq_agenda_int_p bigint, nr_sequencia_p bigint, ds_motivo_p text default null, ie_cancel_agenda_p INOUT text DEFAULT NULL, nm_usuario_p text DEFAULT NULL, cd_estabelecimento_p bigint DEFAULT NULL) AS $body$
DECLARE
...
if (nr_seq_agenda_cons_w <> 0) then --no caso de agenda de consulta mantido tratamento anterior pois nao vincula procedimento na agenda e sim atraves de regra de autorizacao
	...
	if (ie_status_agenda_w	<> 'C') then
		CALL alterar_status_agecons(cd_agenda_w,nr_seq_agenda_cons_w,'C',null,ds_motivo_p,'N',nm_usuario_p,null);
	...
	if (ie_status_agenda_w	<> 'C') then
		CALL alterar_status_agenda(cd_agenda_w, nr_seq_agenda_exame_w,'C', null, ds_motivo_p, 'N', nm_usuario_p);
			...
			if (coalesce(nr_seq_proc_autor_w,0) > 0) then
				...
				insert	into	autorizacao_convenio_hist(nr_sequencia,
							...
							values (nextval('autorizacao_convenio_hist_seq'),
								...
								clock_timestamp(),
								nr_seq_autor_age_w,
								substr(WHEB_MENSAGEM_PCK.get_texto(790635,'CD_PROCEDIMENTO_W='||cd_procedimento_w||';DS_PROCEDIMENTO_W='||ds_procedimento_w),1,4000));
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		if (nr_seq_Agenda_quimio_w IS NOT NULL AND nr_seq_Agenda_quimio_w::text <> '') then
			CALL Qt_Alterar_Status_Agenda(nr_seq_Agenda_quimio_w, 'C', nm_usuario_p, 'N', null, null, cd_estabelecimento_p,'');
		end if;
		end;
	end loop;
	close C01;
...
ie_cancel_agenda_p := ageint_obter_se_cancel_agenda(nr_seq_agenda_int_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ageint_cancelar_item_js (nr_seq_agenda_int_p bigint, nr_sequencia_p bigint, ds_motivo_p text default null, ie_cancel_agenda_p INOUT text DEFAULT NULL, nm_usuario_p text DEFAULT NULL, cd_estabelecimento_p bigint DEFAULT NULL) FROM PUBLIC;
