...
CREATE OR REPLACE PROCEDURE extrato_santander_200_v2011 ( nr_seq_extrato_p bigint) AS $body$
DECLARE

/*
Procedure de acordo com o Layout Santander 200  Versão: Dezembro/2011
*/
...
c01 CURSOR FOR
SELECT	substr(ds_conteudo,75,6) nr_documento,
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	if (nr_conta_ant_w	= '-1') or (nr_conta_ant_w <> nr_conta_w) then
		...
		select	max(to_date(substr(a.ds_conteudo,81,6),'dd/mm/yy')) dt_lancto,
			max(somente_numero(substr(a.ds_conteudo,87,16))) / 100 vl_lancto,
			max(substr(a.ds_conteudo,105,1)) ie_deb_cred_saldo_ini,
			max(substr(a.ds_conteudo,105,1)) ie_deb_cred
		...
		select	max(to_date(substr(a.ds_conteudo,81,6),'dd/mm/yy')) dt_lancto,
			max(somente_numero(substr(a.ds_conteudo,87,16))) / 100 vl_lancto,
			max(substr(a.ds_conteudo,105,1)) ie_cred_deb
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE extrato_santander_200_v2011 ( nr_seq_extrato_p bigint) FROM PUBLIC;
