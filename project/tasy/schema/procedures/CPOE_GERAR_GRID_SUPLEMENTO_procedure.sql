...
CREATE OR REPLACE PROCEDURE cpoe_gerar_grid_suplemento (( nr_atendimento_p cpoe_dieta.nr_atendimento%type, nr_seq_suplemento_p cpoe_dieta.nr_sequencia%type, nm_usuario_p cpoe_dieta.nm_usuario%type) is type cursor01 REFCURSOR) RETURNS timestamp AS $body$
DECLARE
...
if (nr_seq_suplemento_p IS NOT NULL AND nr_seq_suplemento_p::text <> '') and (nr_seq_suplemento_p	> 0) then 
...
fetch c01 
bulk collect 
into suplemento_record_w;
close c01;
 
for i in 1..suplemento_record_w.count loop 
	begin 
	if (nr_seq_supl_cpoe_ant_w	<> suplemento_record_w[i].nr_seq_oral_cpoe) then 
		begin 
		nr_seq_supl_cpoe_ant_w	:= suplemento_record_w[i].nr_seq_oral_cpoe;
		ind_w	:= ind_w +1;
		nr_seq_supl_cpoe_w(ind_w)	:= nr_seq_supl_cpoe_ant_w;		
	...
	nr_hora_w	:= to_char(suplemento_record_w[i].dt_horario,'hh24');	
	ds_hora_w	:= 'P';
	if (suplemento_record_w[i](.dt_fim_horario IS NOT NULL AND .dt_fim_horario::text <> '')) then 
		ds_hora_w	:= 'S';
	end if;	
	if (suplemento_record_w[i](.dt_suspensao IS NOT NULL AND .dt_suspensao::text <> '')) then 
		ds_hora_w	:= 'N';
	end if;
...
for i in 1..nr_seq_supl_cpoe_w.count loop 
	begin 
	nr_seq_supl_cpoe_ant_w	:= nr_seq_supl_cpoe_w(i);
	...
	into STRICT	dt_horario_fim_w, 
			dt_horario_inicio_w 
	from	cpoe_dieta 
	where	nr_sequencia = nr_seq_supl_cpoe_ant_w;
...
forall i in nr_seq_supl_cpoe_w.first .. nr_seq_supl_cpoe_w.last 
	update	cpoe_dieta 
	...
	where	nr_sequencia = 	nr_seq_supl_cpoe_w(i);
commit;
end;
$body$
LANGUAGE PLPGSQL
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE cpoe_gerar_grid_suplemento (( nr_atendimento_p cpoe_dieta.nr_atendimento%type, nr_seq_suplemento_p cpoe_dieta.nr_sequencia%type, nm_usuario_p cpoe_dieta.nm_usuario%type) is type cursor01 REFCURSOR) FROM PUBLIC;
