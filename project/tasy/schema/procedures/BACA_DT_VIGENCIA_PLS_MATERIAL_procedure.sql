...
CREATE OR REPLACE PROCEDURE baca_dt_vigencia_pls_material () AS $body$
DECLARE
...
C01 CURSOR FOR 
 SELECT nr_sequencia, 
  pls_util_pck.obter_dt_vigencia_null(dt_inclusao, 'I') dt_inicio_vigencia_ref, 
  pls_util_pck.obter_dt_vigencia_null(coalesce(dt_limite_utilizacao, dt_exclusao), 'F') dt_fim_vigencia_ref 
 from pls_material;


BEGIN 
 
CALL exec_sql_dinamico('Tasy',' alter trigger pls_material_atual_tot disable');
...
loop 
 ...
 forall i in tb_nr_sequencia_w.first..tb_nr_sequencia_w.last 
  update pls_material 
  set dt_inclusao_ref = tb_dt_inicio_vig_ref_w(i), 
...
CALL exec_sql_dinamico('Tasy',' alter trigger pls_material_atual_tot enable');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE baca_dt_vigencia_pls_material () FROM PUBLIC;
