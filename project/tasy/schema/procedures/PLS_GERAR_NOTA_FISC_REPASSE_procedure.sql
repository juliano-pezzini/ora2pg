...
CREATE OR REPLACE PROCEDURE pls_gerar_nota_fisc_repasse ( nr_seq_repasse_venc_p pls_repasse_vend_venc.nr_sequencia%type, dt_vencimento_p pls_comissao_titulo.dt_vencimento%type, nm_usuario_p text, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
cd_pf_vendedor_w		pessoa_fisica.cd_pessoa_fisica%type;
cd_cgc_vendedor_w		pessoa_juridica.cd_cgc%type;
...
C02 CURSOR FOR 
	SELECT	b.ie_tipo_item, 
		sum(b.vl_repasse) vl_comissao, 
		count(1) qt_item 
	from	pls_repasse_mens	a, 
		pls_repasse_mens_item	b 
	where	a.nr_sequencia 		= b.nr_seq_repasse 
	and	a.nr_seq_repasse	= nr_seq_repasse_w 
	group	by b.ie_tipo_item 
	
union all
 
	SELECT	'X' ie_tipo_item, 
		vl_comissao_canal_w vl_comissao, 
		count(1) qt_item 
	from	pls_repasse_mens a 
...
C03 CURSOR FOR 
	...
	and  	e.nr_sequencia = d.nr_seq_repasse 
	and  	e.nr_seq_repasse = nr_seq_repasse_w 
...
where	nr_sequencia	= nr_seq_repasse_venc_p;
...
and	a.nr_sequencia	= nr_seq_repasse_w;
...
insert into nota_fiscal(	nr_sequencia, cd_estabelecimento, cd_cgc_emitente, cd_serie_nf, 
	...
	values (	 
			...
			0, 0, 0, 0, cd_pf_vendedor_w, cd_cgc_vendedor_w, 
			null, '1', 0, 'N');
...
EXIT WHEN NOT FOUND; /* apply on C02 */
	...
	if (ie_tipo_item_w = 'X') then 
		...
		where	coalesce(ie_tipo_item::text, '') = '' 
	...
	if	((coalesce(cd_procedimento_w::text, '') = '') or (coalesce(ie_origem_proced_w::text, '') = '')) and (coalesce(cd_material_w::text, '') = '') and (coalesce(nr_seq_grupo_item_w::text, '') = '') then 
		if (ie_tipo_item_w = 'X') then 
			CALL wheb_mensagem_pck.exibir_mensagem_abort(384694);		
			-- Mensagem: 	Favor criar uma regra com o procedimento ou material a ser lançado quando não houver item de mensalidade ou lançamento adicional informados.	 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_nota_fisc_repasse ( nr_seq_repasse_venc_p pls_repasse_vend_venc.nr_sequencia%type, dt_vencimento_p pls_comissao_titulo.dt_vencimento%type, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
