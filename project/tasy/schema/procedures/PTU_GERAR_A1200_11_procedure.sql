...
CREATE OR REPLACE PROCEDURE ptu_gerar_a1200_11 ( nr_seq_pacote_p ptu_pacote.nr_sequencia%type, cd_interface_p interface.cd_interface%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
c01 CURSOR(nr_seq_pacote_pc	ptu_pacote.nr_sequencia%type) FOR
	SELECT	nr_sequencia,
		...
		coalesce(nr_versao_transacao,'05') nr_versao_transacao
	from	ptu_pacote
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_a1200_11 ( nr_seq_pacote_p ptu_pacote.nr_sequencia%type, cd_interface_p interface.cd_interface%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
