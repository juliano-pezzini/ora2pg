...
CREATE OR REPLACE PROCEDURE gerar_cobr_bradesco_400_be_v06 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR 
	SELECT	lpad(' ',5,' ') cd_agencia_debito, 
		...
		lpad(' ',7,' ') cd_conta, 
		' ' ie_digito_conta, 
		'0' || lpad(substr(coalesce(x.cd_carteira,'0'),1,3),3,'0') || lpad(coalesce(x.cd_agencia_bancaria,'0'),5,'0') || lpad(coalesce(x.cd_conta,'0'),7,'0') || coalesce(x.ie_digito_conta,'0') id_empresa, 
		lpad(coalesce(f.cd_cgc,'0'),25,'0') nr_controle_partic, 
		lpad(substr(coalesce(a.cd_banco,'0'),1,3),3,'0') cd_banco, 
		'0' ie_multa, 
		lpad('0',4,'0') pr_multa, 
		lpad(to_char(coalesce(b.nr_titulo,'0')),11,'0') nr_nosso_numero, 
		...
		lpad(substr(coalesce(c.cd_instrucao,'0'),1,2),2,'0') ie_instrucao1, 
		'00' ie_instrucao2, 
		lpad(elimina_caracteres_especiais(coalesce(c.vl_acrescimo,'0')),13,'0') vl_acrescimo, 
		to_char(clock_timestamp(),'ddmmyy') dt_desconto, 
		lpad(elimina_caracteres_especiais(coalesce(c.vl_desconto,'0')),13,'0') vl_desconto, 
		lpad('0',13,'0') vl_iof, 
		...
		rpad(substr(upper(elimina_acentuacao(coalesce(b.nm_pessoa,''))),1,40),40,' ') nm_sacado, 
		rpad(substr(CASE WHEN coalesce(d.nr_sequencia::text, '') = '' THEN  			obter_dados_pf_pj(b.cd_pessoa_fisica, b.cd_cgc, 'ECM')  ELSE pls_obter_compl_pagador(d.nr_seq_pagador,'EC') END ,1,40),40,' ') ds_endereco_sacado,			 
...
select	lpad(substr(coalesce(c.cd_convenio_banco,'0'),1,20),20,'0'), 
	...
	to_char(coalesce(a.dt_remessa_retorno,clock_timestamp()),'ddmmyy'), 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_cobr_bradesco_400_be_v06 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
