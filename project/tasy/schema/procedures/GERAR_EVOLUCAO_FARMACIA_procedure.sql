...
CREATE OR REPLACE PROCEDURE gerar_evolucao_farmacia ( nr_prescricao_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ds_erro_p INOUT text) AS $body$
DECLARE
...
c01 CURSOR FOR
	SELECT	a.nr_prescricao,
		...
		substr(b.ds_observacao_far,1,4000),
		a.cd_pessoa_fisica
...
if (obter_funcao_ativa = 252) then
	VarIeEvolucaoClinica_w := Obter_Param_Usuario(252, 50, cd_perfil_w, nm_usuario_p, cd_estabelecimento_p, VarIeEvolucaoClinica_w);
else
	VarIeEvolucaoClinica_w := Obter_Param_Usuario(7010, 164, cd_perfil_w, nm_usuario_p, cd_estabelecimento_p, VarIeEvolucaoClinica_w);
end if;
...
cabecalho_w := '{\rtf1\ansi\ansicpg1252\deff0\deflang1046{\fonttbl{\f0\fswiss\fcharset0 '||ds_fonte_w||';}}'|| '{\*\generator Msftedit 5.41.15.1507;}\viewkind4\uc1\pard\f0\fs'||nr_tam_fonte_w||' ' || obter_desc_expressao(729070) || wheb_rtf_pck.get_quebra_linha|| wheb_rtf_pck.get_quebra_linha|| wheb_rtf_pck.get_quebra_linha;
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (coalesce(cabecalho_w2::text, '') = '') then
		cabecalho_w2 := obter_desc_expressao(325814) || nr_prescricao_w || wheb_rtf_pck.get_quebra_linha ||
						...
						obter_desc_expressao(729062) || nm_farmaceutico_w || wheb_rtf_pck.get_quebra_linha || wheb_rtf_pck.get_quebra_linha;
	end if;
	if (coalesce(ds_medicamentos_w::text, '') = '') then
		ds_medicamentos_w := obter_desc_expressao(330076) || ' ' || ds_material_w || ' - ' || obter_desc_expressao(288285) || ': ' ||ds_dose_um_inter_w || wheb_rtf_pck.get_quebra_linha ||
							 obter_desc_expressao(341947) || ds_observacao_w;
	else
		ds_medicamentos_w := ds_medicamentos_w || wheb_rtf_pck.get_quebra_linha || wheb_rtf_pck.get_quebra_linha ||
							 obter_desc_expressao(330076) || ' ' || ds_material_w || ' - ' || obter_desc_expressao(288285) || ': ' ||ds_dose_um_inter_w || wheb_rtf_pck.get_quebra_linha ||
							 obter_desc_expressao(341947) || ds_observacao_w;
	end if;
	end;
end loop;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_evolucao_farmacia ( nr_prescricao_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ds_erro_p INOUT text) FROM PUBLIC;
