...
CREATE OR REPLACE PROCEDURE pls_consistir_cpt_guia ( nr_seq_guia_p bigint, nm_usuario_p text) AS $body$
DECLARE


/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: Realizar a consistência de CPT do beneficiário(Cobertura Parcial Temporária). Esta cobertura é vincuilada ao contrato do mesmo.
-------------------------------------------------------------------------------------------------------------------
...
c01 CURSOR FOR
	...
	and	((trim(both a.CD_CATEGORIA_CID) in (	select	y.CD_CATEGORIA_CID
						...
						and	x.nr_seq_guia	= nr_seq_guia_p)) or (coalesce(trim(both a.CD_CATEGORIA_CID)::text, '') = '')) /*aaschlote - Coloquei o trim pois colocava um espaço vazio no final*/
...
if (ie_liberado_w	= 'S') then
	...
	if (coalesce(dt_solicitacao_w,clock_timestamp()) < dt_carencia_w) then
		CALL pls_gravar_motivo_glosa('1421', nr_seq_guia_p, null, null,'CPT - Liberado a partir de '||to_char(dt_carencia_w,'dd/mm/yyyy'),nm_usuario_p,'','CG',nr_seq_prestador_w, null,null);
	end if;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_consistir_cpt_guia ( nr_seq_guia_p bigint, nm_usuario_p text) FROM PUBLIC;
