...
CREATE OR REPLACE PROCEDURE ptu_imp_a1200_80 (ds_conteudo_p text, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, ds_arquivo_p ptu_pacote.ds_arquivo%type) AS $body$
DECLARE
...
if (ds_retorno_w = '201')	then
	...
	cd_unimed_prestador_reg_w	:= trim(both substr(ds_conteudo_p,80,4));
	cd_prestador_reg_w		:= trim(both substr(ds_conteudo_p,84,8));
	nm_prestador_reg_w		:= trim(both substr(ds_conteudo_p,92,40));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_imp_a1200_80 (ds_conteudo_p text, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, ds_arquivo_p ptu_pacote.ds_arquivo%type) FROM PUBLIC;
