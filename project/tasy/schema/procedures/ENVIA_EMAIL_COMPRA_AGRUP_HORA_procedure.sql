...
CREATE OR REPLACE PROCEDURE envia_email_compra_agrup_hora ( ie_tipo_mensagem_p bigint, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C03 */
	...
	if (ds_email_origem_w IS NOT NULL AND ds_email_origem_w::text <> '') and (ie_enviar_mensagem_w = 'S') then 
		begin 
		...
		begin 
		CALL enviar_email(ds_assunto_w, ds_mensagem_envio_w, ds_email_origem_w, ds_destino_w, null, 'M');
		exception 
		when others then 
			/*rollback;*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE envia_email_compra_agrup_hora ( ie_tipo_mensagem_p bigint, cd_estabelecimento_p bigint) FROM PUBLIC;
