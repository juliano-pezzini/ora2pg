...
CREATE OR REPLACE PROCEDURE gerar_interf_cab_reenvio_apac (nr_seq_protocolo_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
coalesce(CASE WHEN ie_exporta_resp_w='C' THEN substr(obter_compl_pf(c.cd_pessoa_fisica, 3,'N'),1,60)  ELSE substr(obter_nome_pf(c.cd_pessoa_responsavel),1,60) END , coalesce(substr(obter_compl_pf(c.cd_pessoa_fisica, 5,'N'),1,60),substr(obter_compl_pf(c.cd_pessoa_fisica, 4,'N'),1,60))), 
	...
	and	e.ie_status_ajuste	= 'A' 
	and	e.ie_situacao_retorno	= 'G' 
	and	e.dt_envio_conta	= dt_envio_conta_w 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_interf_cab_reenvio_apac (nr_seq_protocolo_p bigint, nm_usuario_p text) FROM PUBLIC;
