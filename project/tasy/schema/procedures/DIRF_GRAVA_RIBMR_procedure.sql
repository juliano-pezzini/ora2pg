...
CREATE OR REPLACE PROCEDURE dirf_grava_ribmr ( nr_seq_lote_dirf_p bigint, nm_usuario_p text, nr_seq_apres_p INOUT bigint, cd_darf_p text, cd_estabelecimento_p bigint, cd_pessoa_fisica_p text) is vl_rendimento_jan_w varchar(20) AS $body$
BEGIN
return;
end;
...
if	((abs(vl_rendimento_jan_w) +
	...
	ds_arquivo_w := 'RIBMR'	|| separador_w || campo_dirf(vl_rendimento_jan_w)	|| separador_w || campo_dirf(vl_rendimento_fev_w) || separador_w || campo_dirf(vl_rendimento_mar_w)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dirf_grava_ribmr ( nr_seq_lote_dirf_p bigint, nm_usuario_p text, nr_seq_apres_p INOUT bigint, cd_darf_p text, cd_estabelecimento_p bigint, cd_pessoa_fisica_p text) is vl_rendimento_jan_w varchar(20) FROM PUBLIC;
