...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_contest_v110 (nr_seq_camara_p ptu_camara_contestacao.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
C14 CURSOR FOR
	SELECT	'558' ||
		...
		rpad(coalesce(substr(nm_profissional,1,60),' '),60,' ')
...
-- REGISTRO 557
	...
	select	sum(coalesce(vl_reconh_serv,0)),
		sum(coalesce(vl_dif_vl_inter,0))
	into STRICT	vl_reconhecido_w,
		vl_cobrado_w
	from	ptu_quest_serv_rrs s,
		ptu_questionamento_rrs x
	where	x.nr_sequencia	= s.nr_seq_quest_rrs
	and	x.nr_seq_contestacao = nr_seq_camara_p
	and	s.tp_acordo not in ('11');
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_contest_v110 (nr_seq_camara_p ptu_camara_contestacao.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
