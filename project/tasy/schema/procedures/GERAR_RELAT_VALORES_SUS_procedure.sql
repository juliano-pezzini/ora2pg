...
CREATE OR REPLACE PROCEDURE gerar_relat_valores_sus ( nr_seq_protocolo_p bigint, ie_tipo_p text, ie_conta_cancel_p text) AS $body$
DECLARE

/* ie_tipo_p 
 S - sintético 
 A - analítico 
ie_conta_cancel_p	- Se N - não traz as contas canceladas se S traz inclusive as contas canceladas 
*/
 
ie_especialidade_aih_w		numeric(22);
cd_grupo_sus_w			varchar(8);
ie_tipo_servico_sus_w		numeric(22);
qt_ato_w			numeric(22);
...
nr_aih_w			bigint;
dt_alta_w			timestamp;
qt_dia_acompanhante_w		bigint;
nr_seq_protocolo_w		bigint;
nm_pessoa_w			varchar(60);
qt_dia_uti_w			numeric(22);
...
vl_cateter_w			double precision	:= 0;
vl_renal_cro_w			double precision	:= 0;
vl_total_w			double precision	:= 0;
vl_analgesia_w			double precision	:= 0;
vl_sadt_ww			double precision	:= 0;
nr_seq_w			bigint 	:= 0;
ie_especialidade_aih_aux	numeric(22);
cd_procedimento_solic_aux	bigint;
nr_aih_aux			bigint;
dt_alta_aux			timestamp;
qt_dia_acompanhante_aux 	bigint;
qt_dia_uti_aux			bigint;
qt_ato_aux			bigint;
...
qt_aih_aux			bigint;
qt_pto_sp_w			bigint	:= 0;
qt_pto_sadt_w			bigint	:= 0;
...
c01 CURSOR FOR
	...
	FROM sus_valor_proc_paciente h, procedimento g, procedimento_paciente c, conta_paciente d
LEFT OUTER JOIN sus_aih a ON (d.nr_interno_conta = a.nr_interno_conta)
WHERE c.nr_interno_conta	= d.nr_interno_conta and c.nr_sequencia    	= h.nr_sequencia  and c.cd_procedimento   	= g.cd_procedimento and c.ie_origem_proced  	= g.ie_origem_proced and coalesce(c.cd_motivo_exc_conta::text, '') = '' and d.nr_seq_protocolo   = nr_seq_protocolo_p /*	and 	d.ie_cancelamento    is null*/
  and c.ie_origem_proced		= 2 and ((ie_conta_cancel_p = 'N') or (coalesce(d.ie_cancelamento::text, '') = '')) group by	a.ie_especialidade_aih, 
			a.nr_seq_faec, 
...
LEFT OUTER JOIN sus_aih a ON (d.nr_interno_conta = a.nr_interno_conta)
LEFT OUTER JOIN valor_dominio f ON (a.ie_especialidade_aih = f.vl_dominio AND 805 = f.cd_dominio)
WHERE c.nr_interno_conta   = d.nr_interno_conta and c.nr_sequencia     = h.nr_sequencia  and c.nr_atendimento    = x.nr_atendimento and c.cd_procedimento    = g.cd_procedimento and c.ie_origem_proced   = g.ie_origem_proced   /*	and	d.ie_cancelamento 	is null*/
  and coalesce(c.cd_motivo_exc_conta::text, '') = '' and d.nr_seq_protocolo   = nr_seq_protocolo_w and a.nr_aih        = nr_aih_w and c.ie_origem_proced		= 2 and ((ie_conta_cancel_p = 'N') or (coalesce(d.ie_cancelamento::text, '') = '')) group by	a.ie_especialidade_aih, 
			a.nr_seq_faec, 
	 	 	f.ds_valor_dominio, 
		 	coalesce(g.cd_grupo_sus,0), 
		 	a.cd_procedimento_solic, 
...
if (ie_tipo_p = 'S') then 
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		begin 
		if (cd_grupo_sus_w not in (4,8,9,10,13,19)) then 
			vl_sp_w	:= vl_medico_w;
		end if;
		...
		if (cd_grupo_sus_w in (3,6)) then 
			...
			vl_uti_esp_w	:= vl_matmed_w + vl_medico_w;
			...
			vl_transpl_w	:= vl_matmed_w + vl_medico_w;
		...
		elsif (cd_grupo_sus_w = 19) then 
			if (cd_procedimento_solic_w = 47810017) then 
				vl_cateter_w	:= vl_medico_w + vl_matmed_w;
			else 
				vl_cateter_w	:= vl_medico_w;
				vl_ssm_w	:= vl_matmed_w;
			end if;
		elsif (cd_grupo_sus_w = 20) then 
			vl_renal_cro_w	:= vl_matmed_w;
		elsif (cd_grupo_sus_w = 999) then 
			qt_aih_w	:= qt_aih_w;
		end if;
		vl_total_w	:= (vl_uti_esp_w + vl_ssm_w + vl_sp_w + vl_sadt_w + vl_acomp_w + 
				vl_sangue_w + vl_transpl_w + vl_neuro_w + vl_rn_w + vl_opm_w + 
				vl_uti_w + vl_anest_w + vl_nut_enteral_w + vl_componente_w + 
				vl_regcivil_w + vl_notif_w + vl_hiv_w + vl_cateter_w + vl_renal_cro_w);
		vl_pto_sp_W	:= vl_sp_w;
	...
	EXIT WHEN NOT FOUND; /* apply on c02 */
		...
		EXIT WHEN NOT FOUND; /* apply on c03 */
			...
			if (cd_grupo_sus_w in (3,6)) then 
				...
				vl_uti_esp_w	:= vl_uti_esp_w + vl_matmed_aux + vl_medico_aux;
			elsif (cd_grupo_sus_w	= 5) then 
				vl_acomp_w	:= vl_acomp_w + vl_matmed_aux;
			elsif (cd_grupo_sus_w	= 7) then 
				vl_sangue_w	:= vl_sangue_w + vl_matmed_aux;
			elsif (cd_grupo_sus_w = 8) then 
				vl_transpl_w	:= vl_transpl_w + vl_matmed_aux + vl_medico_aux;
			elsif (cd_grupo_sus_w = 9) then 
				...
				if (cd_procedimento_solic_w = 47810017) then 
					vl_cateter_w	:= vl_cateter_w + vl_medico_aux + vl_matmed_aux;
				else 
					vl_cateter_w	:= vl_cateter_w + vl_medico_aux;
					vl_ssm_w	:= vl_ssm_w	+ vl_matmed_aux;
				end if;
			elsif (cd_grupo_sus_w = 20) then 
				vl_renal_cro_w	:= vl_renal_cro_w + vl_matmed_aux;
			end if;
			qt_aih_w	:= 1;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_relat_valores_sus ( nr_seq_protocolo_p bigint, ie_tipo_p text, ie_conta_cancel_p text) FROM PUBLIC;
