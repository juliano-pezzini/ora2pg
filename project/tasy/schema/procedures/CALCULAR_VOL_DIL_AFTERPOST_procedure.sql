...
CREATE OR REPLACE PROCEDURE calcular_vol_dil_afterpost ( nr_prescricao_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_sequencia_princ_w IS NOT NULL AND nr_sequencia_princ_w::text <> '') then
		...
		where	1 = 1
		and		ie_agrupador		=	3
		and		nr_sequencia_diluicao	=	nr_sequencia_princ_w
		and		nr_prescricao		=	nr_prescricao_p;
		...
		if (ie_subtrair_volume_medic_w = 'S') and
			...
			and		cd_material		= cd_material_dil_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE calcular_vol_dil_afterpost ( nr_prescricao_p bigint, nm_usuario_p text) FROM PUBLIC;
