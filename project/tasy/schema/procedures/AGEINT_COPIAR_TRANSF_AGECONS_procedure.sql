...
CREATE OR REPLACE PROCEDURE ageint_copiar_transf_agecons (cd_estabelecimento_p bigint, nr_seq_origem_p bigint, nr_seq_destino_p bigint, ie_acao_p text, nr_seq_motivo_p bigint, ds_motivo_p text, nm_usuario_p text, ds_retorno_p INOUT text) AS $body$
DECLARE
...
if (cd_estabelecimento_p IS NOT NULL AND cd_estabelecimento_p::text <> '') and (nr_seq_origem_p IS NOT NULL AND nr_seq_origem_p::text <> '') and (nr_seq_destino_p IS NOT NULL AND nr_seq_destino_p::text <> '') and (ie_acao_p IS NOT NULL AND ie_acao_p::text <> '') and (nm_usuario_p IS NOT NULL AND nm_usuario_p::text <> '') then
	...
	select 	coalesce(max(obter_valor_param_usuario(869, 77, obter_perfil_ativo, nm_usuario_p, wheb_usuario_pck.get_cd_estabelecimento)), '0')
	...
	and	    nr_seq_agenda					= nr_seq_destino_p
    ...
    if (qt_dias_confirm_aut_w > 0) and (Ageint_Obter_Qt_Dia_Util_Per(clock_timestamp(), hr_agenda_w, wheb_usuario_pck.get_cd_estabelecimento) <= qt_dias_confirm_aut_w)
	...
	if (ie_acao_p = 'T') then
		...
		/*envia anexos para novo agendamento*/
		...
		ds_texto_w := wheb_mensagem_pck.get_texto(790939,
							...
							';HR_DESTINO_W='||to_char(hr_destino_w, pkg_date_formaters.localize_mask('timestamp', pkg_date_formaters.getUserLanguageTag(cd_estabelecimento_p, nm_usuario_p))));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ageint_copiar_transf_agecons (cd_estabelecimento_p bigint, nr_seq_origem_p bigint, nr_seq_destino_p bigint, ie_acao_p text, nr_seq_motivo_p bigint, ds_motivo_p text, nm_usuario_p text, ds_retorno_p INOUT text) FROM PUBLIC;
