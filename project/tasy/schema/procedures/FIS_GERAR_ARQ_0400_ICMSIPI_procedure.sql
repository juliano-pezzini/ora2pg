...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_0400_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
fetch c_reg_0400 bulk collect into vetReg0400 limit 1000;
	...
	ds_linha_w := substr(ds_sep_w || vetReg0400[i].cd_reg ||
			     ds_sep_w || vetReg0400[i].cd_nat ||
			     ds_sep_w || vetReg0400[i].ds_nat || ds_sep_w,
			     1,
			     8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro 		:=  vetReg0400[i].cd_reg;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_0400_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) FROM PUBLIC;
