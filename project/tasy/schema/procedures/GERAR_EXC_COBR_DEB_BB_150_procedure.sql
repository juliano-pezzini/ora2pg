...
CREATE OR REPLACE PROCEDURE gerar_exc_cobr_deb_bb_150 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
Vigência: 05/05/2011 a 24/04/2013 
 
Utilizado pelo Vera Cruz - Contém campos fixos*/
...
C01 CURSOR FOR 
	SELECT	lpad(coalesce(substr(x.cd_agencia_bancaria,1,4),'0'),4,'0') cd_agencia_bancaria, 
		...
		lpad(' ',26,' ') ds_mensagen, 
		rpad(coalesce(substr(pls_obter_carteira_segurado(pls_obter_segurado_pagador(f.nr_sequencia)),1,25),' '),25,' ') ds_ident_cliente_emp, 
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	ds_conteudo_w	:= 	'D' || ds_ident_cliente_emp_w || cd_agencia_bancaria_w || ds_ident_cliente_banco_w || ds_ident_cliente_emp_atual_w || 
				ds_ocorrencia_w || ds_brancos_20_w || '1' ; 
	...
	ds_conteudo_w	:= 	'E' || ds_ident_cliente_emp_w || cd_agencia_bancaria_w || ds_ident_cliente_banco_w || 
				...
				ds_uso_empresa_w || ds_brancos_20_w ||'1';
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_exc_cobr_deb_bb_150 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
