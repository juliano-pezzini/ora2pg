...
if (dados_filtro_p.nr_sequencia IS NOT NULL AND dados_filtro_p.nr_sequencia::text <> '') then 
	...
	for r_c_filtro in c_filtro(dados_filtro_p.nr_sequencia) loop		 
		...
		dados_restricao_w.ds_restricao_proc := 	dados_restricao_w.ds_restricao_proc || 
							pls_oc_cta_obter_restr_proc('RESTRICAO',dados_regra_p,var_cur_w,dados_filtro_proc_w);
		 
		-- Montar o select a ser executado. 
		...
		begin 
			...
			-- Dos Filtros 
			ret_null_w := pls_oc_cta_obter_restr_proc('BIND',dados_regra_p,var_cur_w,dados_filtro_proc_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_oc_cta_aplicar_fil_proc ( dados_regra_p pls_tipos_ocor_pck.dados_regra, dados_filtro_p pls_tipos_ocor_pck.dados_filtro, dados_consistencia_p pls_tipos_ocor_pck.dados_consistencia, dados_forma_geracao_ocor_p pls_tipos_ocor_pck.dados_forma_geracao_ocor, ie_incidencia_regra_p text, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, qt_registro_selecao_p integer default null) FROM PUBLIC;
