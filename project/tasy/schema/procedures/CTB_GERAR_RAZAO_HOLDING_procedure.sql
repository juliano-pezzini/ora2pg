...
CREATE OR REPLACE PROCEDURE ctb_gerar_razao_holding ( cd_empresa_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estab_p text, cd_conta_referencia_p text, cd_classificacao_p text, cd_centro_custo_p text, nm_usuario_p text, ie_valor_p text, cd_classif_inic_p text, cd_classif_final_p text, ie_centro_custo_p text, ie_ordenacao_p text, ie_contra_partida_p text) AS $body$
DECLARE
...
cd_conta_referencia_w	varchar(080);
...
c001 CURSOR FOR
SELECT  distinct coalesce(c.cd_conta_referencia, c.cd_conta_contabil),
		substr(substr(coalesce(c.cd_conta_referencia, c.cd_conta_contabil),1,20)  || ' - ' || substr(coalesce(s.cd_classificacao, c.cd_classificacao),1,40) || '  ' || substr(c.ds_conta_contabil,1,190),1,255),
...
and (s.cd_conta_contabil   = c.cd_conta_contabil OR s.cd_conta_contabil = c.cd_conta_referencia)
AND  	c.cd_grupo    = d.cd_grupo
and  	c.cd_empresa    = r.cd_empresa
and  	r.cd_empresa    = g.cd_empresa
...
c002 CURSOR FOR
	...
	and (c.cd_conta_referencia = cd_conta_referencia_w or c.cd_conta_contabil = cd_conta_referencia_w)
...
nr_seq_grupo_w	:= holding_pck.get_grupo_emp_usuario(cd_empresa_p);
...
EXIT WHEN NOT FOUND; /* apply on c001 */
	...
	if (ie_valor_p = 'N') then
		...
		where	exists ( SELECT 1
				...
				and     y.cd_empresa	= g.cd_empresa
				and	    g.nr_seq_grupo	= nr_seq_grupo_w
				and		x.cd_conta_debito	= c.cd_conta_contabil
				and (c.cd_conta_referencia	= cd_conta_referencia_w or c.cd_conta_contabil = cd_conta_referencia_w)
		...
		if (ie_gerar_w = 'N') then
			...
			where	exists ( SELECT 1
					...
					and     y.cd_empresa   	= g.cd_empresa
					and	    g.nr_seq_grupo	= nr_seq_grupo_w
					and	x.cd_conta_credito	= c.cd_conta_contabil
					and (c.cd_conta_referencia	= cd_conta_referencia_w or c.cd_conta_contabil = cd_conta_referencia_w)
	...
	if (ie_gerar_w	= 'S')  and (coalesce(cd_conta_referencia_p,'0')	<> '0') then
		select	ctb_obter_se_conta_contida(cd_conta_referencia_w, cd_conta_referencia_p)
		into STRICT	ie_gerar_w
		;
	end if;
	...
	if (ie_gerar_w	= 'S')  and (coalesce(cd_classificacao_p,'0') <> '0')  then
		...
		while(ie_gerar_w = 'N') and (length(cd_classif_w) > 0)  loop
			...
			select	ctb_obter_se_conta_classif_sup(cd_conta_referencia_w, cd_classif_ww)
			into STRICT	ie_gerar_w
			;
			--ou

			/*select	ctb_obter_se_conta_classif_sup(c.cd_conta_contabil, cd_classif_ww)
			into	ie_gerar_w
			from	conta_contabil c
			where	c.cd_conta_referencia	= cd_conta_referencia_w;*/
			end;
		end loop;
		end;
	end if;
	...
	if (ie_gerar_w	= 'S')  then
		...
		if (coalesce(cd_estab_p,'0') = '0') then
			...
			and		c.cd_conta_referencia	= cd_conta_referencia_w
			and     r.cd_empresa        			= g.cd_empresa
			and	    g.nr_seq_grupo					= nr_seq_grupo_w;
			...
			if (ie_contrapartida_w = 'N') then
				...
				if (coalesce(cd_contrapartida_ww, 'X') <> 'X') then
					...
					if (ie_contrapartida_w = 'TCR') then
						begin
						cd_contrapartida_ww := cd_contrapartida_w;
						end;
					end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ctb_gerar_razao_holding ( cd_empresa_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estab_p text, cd_conta_referencia_p text, cd_classificacao_p text, cd_centro_custo_p text, nm_usuario_p text, ie_valor_p text, cd_classif_inic_p text, cd_classif_final_p text, ie_centro_custo_p text, ie_ordenacao_p text, ie_contra_partida_p text) FROM PUBLIC;
