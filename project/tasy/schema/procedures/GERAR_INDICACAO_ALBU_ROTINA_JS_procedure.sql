...
CREATE OR REPLACE PROCEDURE gerar_indicacao_albu_rotina_js (nr_sequencia_p bigint, ds_lista_p text, nm_usuario_p text ) AS $body$
DECLARE
...
if (ds_lista_p IS NOT NULL AND ds_lista_p::text <> '') then
	...
	while(ds_lista_w IS NOT NULL AND ds_lista_w::text <> '') and (nr_max_loop_w > 0) loop
		...
		if (posicao_virgula_w > 1) and ((substr(ds_lista_w, 1, posicao_virgula_w -1) IS NOT NULL AND (substr(ds_lista_w, 1, posicao_virgula_w -1))::text <> '')) then
			begin
			nr_seq_indicacao_w := (substr(ds_lista_w, 1, posicao_virgula_w -1))::numeric;
			ds_lista_w := substr(ds_lista_w, posicao_virgula_w +1, tamanho_lista_w);
			end;
		elsif (ds_lista_w IS NOT NULL AND ds_lista_w::text <> '') then
			begin
			nr_seq_indicacao_w := replace(ds_lista_w, ',', '');
			ds_lista_w := '';
			end;
		end if;

		if (nr_seq_indicacao_w IS NOT NULL AND nr_seq_indicacao_w::text <> '') and (nr_seq_indicacao_w > 0) then
			begin
			CALL gerar_indicacao_albu_rotina(nr_seq_indicacao_w,nm_usuario_p,nr_sequencia_p, null);
			end;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_indicacao_albu_rotina_js (nr_sequencia_p bigint, ds_lista_p text, nm_usuario_p text ) FROM PUBLIC;
