...
CREATE OR REPLACE PROCEDURE conciliar_ext_cartao_cielo ( nr_seq_extrato_arq_p bigint, ds_lista_extrato_movto_fin_p text, ds_lista_extrato_movto_cred_p text, ie_opcao_p text, nm_usuario_p text) AS $body$
DECLARE
...
and	coalesce(c.cd_bandeira,0)		= coalesce(coalesce(cd_bandeira_w,c.cd_bandeira),0)
...
and	a.nr_autorizacao	= coalesce(nr_autorizacao_w,a.nr_autorizacao)
and	((coalesce(ds_comprovante_w::text, '') = '' and coalesce(nr_documento_w::text, '') = '') or (coalesce(lpad(a.ds_comprovante,100,'0'),'X') = lpad(ds_comprovante_w,100,'0')) or (coalesce(lpad(a.ds_comprovante,100,'0'),'X') = lpad(nr_documento_w,100,'0')));
...
if (coalesce(ds_lista_extrato_movto_cred_p::text, '') = '') then
	...
	if (ie_opcao_p	= 'C') then
		...
		and	nr_seq_extrato_parcela in (SELECT	distinct
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE conciliar_ext_cartao_cielo ( nr_seq_extrato_arq_p bigint, ds_lista_extrato_movto_fin_p text, ds_lista_extrato_movto_cred_p text, ie_opcao_p text, nm_usuario_p text) FROM PUBLIC;
