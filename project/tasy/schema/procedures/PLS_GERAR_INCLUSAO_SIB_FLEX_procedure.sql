...
CREATE OR REPLACE PROCEDURE pls_gerar_inclusao_sib_flex ( nr_seq_lote_sib_p bigint, ie_gerar_todos_p text, nm_usuario_p text, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
WHERE a.cd_pessoa_fisica	= b.cd_pessoa_fisica  and e.nr_sequencia		= a.nr_seq_contrato and c.nr_sequencia		= a.nr_seq_plano and a.nr_sequencia		= nr_seq_segurado_w;
	...
	if (ie_gerar_todos_p = 'S') then
		...
		if (coalesce(nr_seq_portabilidade_w,0) > 0) then /* se for de portabilidade o motivo e o 41 */
			...
			begin
			select	lpad(somente_numero(max(nr_prot_ans_origem)),9,0)
	...
	if (ie_regulamentacao_w = 'R') then
		...
		ie_itens_excluid_cobertura_w	:= 2;
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_inclusao_sib_flex ( nr_seq_lote_sib_p bigint, ie_gerar_todos_p text, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
