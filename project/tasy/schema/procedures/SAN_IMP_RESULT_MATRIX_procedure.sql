...
CREATE OR REPLACE PROCEDURE san_imp_result_matrix ( cd_pessoa_fisica_p text, cd_barras_p text, dt_resultado_p timestamp, dt_liberacao_p timestamp, cd_exame_p text, ds_resultado_p text, ds_justificativa_p text, ie_tipo_integracao_p text, nm_operador_p text, nm_usuario_p text, ds_invalidos_p INOUT text, ds_lote_kit_p text, ds_abs_p text, ds_metsbs_p text, nr_cutoff_p bigint) AS $body$
DECLARE
...
vl_resultado_w		san_exame_realizado.vl_resultado%type;
ie_pf_realizou_w	char(1);
cd_pf_realizou_w	bigint;
...
BEGIN
	if (cd_barras_p IS NOT NULL AND cd_barras_p::text <> '') then
		...
		if (coalesce(nr_seq_exame_lote_w::text, '') = '') then
			...
			WHERE	a.cd_cadastro = cd_pessoa_fisica_p;
		...
		if (coalesce(nr_seq_exame_lote_w::text, '') = '') then
			...
			if (coalesce(nr_seq_exame_w::text, '') = '') then						
				...
				CALL gravar_log_tasy(9245, wheb_mensagem_pck.get_texto(793520,'CD_EXAME='||cd_exame_p) || ' ' || cd_barras_w, nm_usuario_p);				
				...
				if (coalesce(nr_seq_exame_lote_w::text, '') = '') then						
					...
					begin
					if (ie_tipo_resultado_w = 'N') then
						select 	to_number(ds_resultado_p,'9999999999D9999',' NLS_NUMERIC_CHARACTERS = ''.,''')
						into STRICT	vl_resultado_w
						;
					end if;
					
					begin
					select 	to_number(nm_operador_p, '9999999999')
					into STRICT	cd_pf_realizou_w
					;
					
					select	CASE WHEN count(*)=0 THEN  'N'  ELSE 'S' END
					into STRICT	ie_pf_realizou_w
					from	pessoa_fisica
					where	cd_pessoa_fisica = nm_operador_p;
					...
					select	coalesce(max(a.ds_valor_tasy), ds_resultado_p)
					...
					and	a.ds_valor_integracao = ds_resultado_p
					and	b.ie_tipo_integ_tasyger = '5'
					and	b.ie_tabela = 'RED';
					
					update	san_exame_realizado set
						ds_resultado		= CASE WHEN ie_tipo_resultado_w='D' THEN  ds_res_qualitativo_w  ELSE ds_resultado END ,
						vl_resultado		= CASE WHEN ie_tipo_resultado_w='N' THEN  vl_resultado_w  ELSE vl_resultado END ,
						ds_abs			= substr(ds_abs_p, 1, 255),
						...
						cd_pf_realizou		= CASE WHEN ie_pf_realizou_w='S' THEN  nm_operador_p  ELSE cd_pf_realizou END ,
						ds_justificativa	= ds_justificativa || ds_justificativa_p
					where 	nr_seq_exame_lote = nr_seq_exame_lote_w
					and	nr_seq_exame	= nr_seq_exame_w
					...
					exception
						when others then
							ds_invalidos_p := ds_invalidos_p || '14=' || cd_exame_p || ',' || cd_barras_w || ',' || ds_resultado_p || '//';
							CALL gravar_log_tasy(9245, wheb_mensagem_pck.get_texto(793575,
												'VL_EXAME_ATUAL='||cd_exame_p||
												';CD_BARRAS='||cd_barras_w||
												';DS_CT='||ds_resultado_p), nm_usuario_p);
					end;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE san_imp_result_matrix ( cd_pessoa_fisica_p text, cd_barras_p text, dt_resultado_p timestamp, dt_liberacao_p timestamp, cd_exame_p text, ds_resultado_p text, ds_justificativa_p text, ie_tipo_integracao_p text, nm_operador_p text, nm_usuario_p text, ds_invalidos_p INOUT text, ds_lote_kit_p text, ds_abs_p text, ds_metsbs_p text, nr_cutoff_p bigint) FROM PUBLIC;
