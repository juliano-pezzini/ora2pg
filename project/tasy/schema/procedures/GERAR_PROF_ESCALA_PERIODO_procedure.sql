...
CREATE OR REPLACE PROCEDURE gerar_prof_escala_periodo ( cd_profissional_p bigint, dt_inicio_p timestamp, dt_fim_p timestamp, nr_seq_escala_p bigint, nm_usuario_p text, ds_consistencia_p INOUT text, ie_dias_intercalado_p text) AS $body$
DECLARE
		...
		if (ie_gerar_dia_escala_w	= 'S') then 
			...
			if (qt_anestesista_w = 0) then 
				...
				if (ds_consistencia_w IS NOT NULL AND ds_consistencia_w::text <> '') then 
					...
					ds_consistencia_p := substr(wheb_mensagem_pck.get_texto(799406, 'DS_CONSISTENCIA_W='||ds_consistencia_w),1,255);
				end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_prof_escala_periodo ( cd_profissional_p bigint, dt_inicio_p timestamp, dt_fim_p timestamp, nr_seq_escala_p bigint, nm_usuario_p text, ds_consistencia_p INOUT text, ie_dias_intercalado_p text) FROM PUBLIC;
