...
CREATE OR REPLACE PROCEDURE ptu_imp_fatura_v100a ( ds_conteudo_p text, ie_tipo_arquivo_p bigint, cd_estabelecimento_p bigint, nr_linha_p bigint, nm_usuario_p text, nr_seq_fatura_p INOUT bigint, nr_seq_fat_imp_p INOUT ptu_fatura.nr_sequencia%type) AS $body$
DECLARE

				
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade:	ESTA ROTINA FAZ A IMPORTAÇÃO DOS DADOS DO A500 PTU 10.0a
-------------------------------------------------------------------------------------------------------------------
...
if (coalesce(nr_seq_fat_imp_p::text, '') = '') or (ds_registro_w	= '501') then
	select	max(sid),
		max(serial#)
...
if (ds_registro_w	= '501') then
	...
	-- jtonon - OS 824747 - No A550 deve ir exatamente os dados que estão no arquivo, mesmo que os valores sejam 0 (zeros)
	...
	/* Verifica se a fatura está em duplicidade */
	...
	-- parâmetro [32] - Permite importar faturas devolvidas - OS 473542
...
if (ds_registro_w	= '502') then
	...
	nm_prest_req_w			:= trim(both substr(ds_conteudo_p,519,70));
	...
	nm_prest_exec_w			:= trim(both substr(ds_conteudo_p,612,70));
	...
	if (dt_conhecimento_w IS NOT NULL AND dt_conhecimento_w::text <> '') then
		...
		from (	-- busca por pessoa juridica
			...
			and	ac.cd_cpf_executante	= nr_cnpj_cpf_w
			and	tp_pessoa_w		= 'J'
			and	aa.nr_lote		= nr_lote_prest_w
			and	ac.nr_guia_prestador	= nr_guia_tiss_prestador_w
			and	ac.nr_carteira_benef	= cd_usuario_plano_w
...
union all
			...
			and	aa.nr_lote		= nr_lote_prest_w
			and	ac.nr_guia_prestador	= nr_guia_tiss_prestador_w
			and	ac.nr_carteira_benef	= cd_usuario_plano_w) x;
	end if;
...
if (ds_registro_w = '503') then
	...
	nm_hospital_w		:= trim(both substr(ds_conteudo_p,510,70));
	...
	if (somente_numero(nr_ver_tiss_w) >= 30000) then
		...
		qt_nasc_vivos_w			:= trim(both substr(ds_conteudo_p,136,2));  --QT_NASC_VIVOS
		qt_nasc_mortos_w		:= trim(both substr(ds_conteudo_p,138,2));  --QT_NASC_MORTOS 
		qt_nasc_vivos_pre_w		:= trim(both substr(ds_conteudo_p,140,2));  --QT_NASC_VIVOS_PRE 
		qt_obito_precoce_w		:= trim(both substr(ds_conteudo_p,142,1));  --QT_OBITO_PRECOCE 
		qt_obito_tardio_w		:= trim(both substr(ds_conteudo_p,143,1));  --QT_OBITO_TARDIO 
		ie_int_gestacao_w		:= trim(both substr(ds_conteudo_p,144,1));  -- ID_INT_OBSTETRICIA_1
		ie_int_aborto_w			:= trim(both substr(ds_conteudo_p,145,1));  -- ID_INT_OBSTETRICIA_2
		ie_int_transtorno_w		:= trim(both substr(ds_conteudo_p,146,1));  -- ID_INT_OBSTETRICIA_3
		ie_int_comp_puerperio_w		:= trim(both substr(ds_conteudo_p,147,1));  -- ID_INT_OBSTETRICIA_4
		ie_int_atend_parto_w		:= trim(both substr(ds_conteudo_p,148,1));  -- ID_INT_OBSTETRICIA_5
		ie_int_compl_neonatal_w		:= trim(both substr(ds_conteudo_p,149,1));  -- ID_INT_OBSTETRICIA_6
		ie_int_baixo_peso_w		:= trim(both substr(ds_conteudo_p,150,1));  -- ID_INT_OBSTETRICIA_7
		ie_int_parto_cesaria_w		:= trim(both substr(ds_conteudo_p,151,1));  -- ID_INT_OBSTETRICIA_8
		ie_int_parto_normal_w		:= trim(both substr(ds_conteudo_p,152,1));  -- ID_INT_OBSTETRICIA_9
	end if;
...
if (ds_registro_w = '504') then
	...
	if ((substr(ds_conteudo_p,221,2))::numeric  > 0) then
		cd_especialidade_w := (substr(ds_conteudo_p,221,2))::numeric;
	end if;
	...
	/*Para arquivo importados de versão 3.7*/
	...
	-- Se houver prestador requisitante, é obrigatorio a informação de Unimed requisitante
	...
	if (ie_tipo_tabela_w in (2,3)) then				
		...
		if (qt_regra_w > 0) then
			...
			SELECT * FROM pls_obter_proced_conversao(	null, null, null, cd_estabelecimento_p, 1, null, 3, 'R', null, null, null, cd_mat_number_inter_w, ie_tipo_intercambio_w, cd_proc_conversao_w, ie_origem_proced_conv_w, nr_seq_regra_conv_w, ie_somente_codigo_w, clock_timestamp(), null, null, null) INTO STRICT cd_proc_conversao_w, ie_origem_proced_conv_w, nr_seq_regra_conv_w, ie_somente_codigo_w;
		end if;
		...
		/*A origem do procedimento é buscado por regra existente no cadastro de regras / Procedimentos/ regra origem.*/

		SELECT * FROM pls_obter_proced_conversao(	cd_servico_w, null, null, cd_estabelecimento_p, 1, null, 3, 'R', null, null, null, null, ie_tipo_intercambio_w, cd_proc_conversao_w, ie_origem_proced_conv_w, nr_seq_regra_conv_w, ie_somente_codigo_w, clock_timestamp(), null, null, null) INTO STRICT cd_proc_conversao_w, ie_origem_proced_conv_w, nr_seq_regra_conv_w, ie_somente_codigo_w;
		...
		--Criado tratamento para atender as OS 575917 que solicita que seja buscado a última origem valida e a OS 644869 onde indica que deve ser respeitada a origem padrão do sistema	
		if (ie_origem_proc_valido_w	= 'S') then
			...
			/*Se este procedimento não existir na origem padrão é selecionado o max origem proced*/
			...
			if (qt_proced_origem_w = 0) then	
				...
				-- Se não encontrar a origem em procedimentos ativos, busca em procedimentos que não estiverem ativos.
				if (coalesce(ie_origem_proced_conv_w::text, '') = '') then
					select	max(ie_origem_proced)
					into STRICT	ie_origem_proced_conv_w
					from	procedimento
			...
			/*Se este procedimento não existir na origem padrão é selecionado o max origem proced
			OS 644869 Conforme solicitado foi realizado tratamento para caso não */
			if (qt_proced_origem_w = 0) then	
				-- Buscar a origem do procedimento ativo
		...
		-- Tem que deixar o item como NÃO ENCONTRADO
	...
	nm_profissional_prestador_w 	:= trim(both substr(ds_conteudo_p,942,70));
	nm_fornecedor_w			:= trim(both substr(ds_conteudo_p,1012,70));
...
if (ds_registro_w = '507') then
	...
	nm_prestador_507_w	:= trim(both substr(ds_conteudo_p,157,70));
	...
	SELECT * FROM pls_gerar_prest_intercambio(	nr_cpf_prestador_w, cd_cgc_prestador_w, nm_prestador_507_w, null, null, nm_usuario_p, null, null, nr_seq_prest_inter_w, nr_seq_prestador_w) INTO STRICT nr_seq_prest_inter_w, nr_seq_prestador_w;
...
if (ds_registro_w = '508') then
	...
	nm_profissional_508_w	:= trim(both substr(ds_conteudo_p,253,70));
	cd_servico_508_w	:= (substr(ds_conteudo_p,323,10))::numeric;
	
	if (tp_tabela_508_w not in (2,3)) then
	...
	elsif (tp_tabela_508_w in (2,3)) then
...
if (ds_registro_w = '999') then
	-- parâmetro [21] - Ao importar o arquivo, gravar na data de emissão, a mesma data do recebimento
	ie_parametro_21_w := obter_param_usuario(1293, 21, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_p, ie_parametro_21_w);
	
	-- parâmetro [33] - Gerar data de recebimento do A500 conforme a data de postagem do mesmo
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_imp_fatura_v100a ( ds_conteudo_p text, ie_tipo_arquivo_p bigint, cd_estabelecimento_p bigint, nr_linha_p bigint, nm_usuario_p text, nr_seq_fatura_p INOUT bigint, nr_seq_fat_imp_p INOUT ptu_fatura.nr_sequencia%type) FROM PUBLIC;
