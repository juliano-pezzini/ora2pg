...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_fatura_v70_utl ( nr_seq_fatura_p ptu_fatura.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, ds_local_windows_utl_p INOUT text ) AS $body$
DECLARE
...
C03 CURSOR FOR
	SELECT	lpad(coalesce(elimina_caractere_especial(a.nr_lote),'0'),8,'0'),
		...
		rpad(coalesce(elimina_caractere_especial(a.nm_prestador),' '),40,' '),
		...
		rpad(coalesce(elimina_caractere_especial(a.nm_profissional_prestador),' '),40,' '),
		...
		rpad(coalesce(a.nm_fornecedor,' '),40,' '),
...
/*Cursor C11	( nr_seq_ptu_fatura_pc	ptu_fatura.nr_sequencia%type ) is
	select	nr_sequencia,
		lpad(nvl(elimina_caractere_especial(to_char(nr_lote)),'0'),8,'0') nr_lote,
		rpad(nvl(substr(ptu_somente_caracter_permitido(nr_nota,'ANS'),1,20),' '),20,' ') nr_nota,
		lpad(nvl(cd_unimed,'0'),4,'0') cd_unimed,
		lpad(nvl(id_benef,'0'),13,'0') id_benef,
		rpad(substr(nvl(ptu_somente_caracter_permitido(nm_beneficiario,'ANS'),' '),1,25),25,' ') nm_beneficiario,
		rpad(nvl(to_char(dt_nasc,'yyyymmdd'),' '),8,' ') dt_nasc,
		nvl(tp_sexo,' ') tp_sexo,
		nvl(id_rn,' ') id_rn,
		rpad(nvl(to_char(dt_servico,'yyyymmdd'),' '),8,' ') dt_servico,
		rpad(nvl(to_char(dt_reembolso,'yyyymmdd'),' '),8,' ') dt_reembolso,
		nvl(tp_carater_atend,' ') tp_carater_atend,
		nvl(tp_pessoa,' ') tp_pessoa,
		lpad(nvl(nr_cnpj_cpf,'0'),14,'0') nr_cnpj_cpf,
		rpad(nvl(ptu_somente_caracter_permitido(nm_prestador,'ANS'),' '),40,' ') nm_prestador,
		rpad(nvl(ptu_somente_caracter_permitido(nm_profissional,'ANS'),' '),40,' ') nm_profissional,
		rpad(nvl(sg_cons_prof,' '),12,' ') sg_cons_prof,
		rpad(nvl(nr_cons_prof,' '),15,' ') nr_cons_prof,
		rpad(nvl(sg_uf_cons_prof,' '),2,' ') sg_uf_cons_prof,
		nvl(tp_particip,' ') tp_particip,
		nvl(tp_tabela,'0') tp_tabela,
		lpad(nvl(cd_servico,'0'),8,'0') cd_servico,
		lpad(nvl(qt_cobrada*10000,'0'),8,'0') qt_cobrada,
		lpad(nvl(replace(replace(campo_mascara(vl_dif_vl_inter,2),',',''),'.',''),'0'),14,'0') vl_dif_vl_inter,
		lpad(nvl(replace(replace(campo_mascara(vl_serv_cob,2),',',''),'.',''),'0'),14,'0') vl_serv_cob,
		lpad(nvl(nr_autoriz,'0'),10,'0') nr_autoriz
	from	ptu_nota_cobranca_reem b
...
if (nr_seq_fatura_p IS NOT NULL AND nr_seq_fatura_p::text <> '') then
	...
	-- NOME DO ARQUIVO NO PADRÃO PTU 7.0
	select	substr(ptu_obter_nome_exportacao(nr_seq_fatura_p,'FA',2767),1,255)
	into STRICT	nm_arquivo_w
	;

	-- Gerar arquivo UTL
	ds_local_windows_utl_p := pls_utl_file_pck.novo_arquivo( 20, nm_arquivo_w, 'S', null, ds_local_windows_utl_p);
	...
	EXIT WHEN NOT FOUND; /* apply on C00 */
		...
		ie_tipo_documento_1_w	:= '1';
		ie_tipo_documento_2_w	:= '1';
		...
		/*for r_C11_w in C11(nr_seq_ptu_fatura_w) loop
			...
			ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') ||
					...
					r_C11_w.nm_profissional ||
					r_C11_w.sg_cons_prof ||
					r_C11_w.nr_cons_prof ||
					r_C11_w.sg_uf_cons_prof ||
					r_C11_w.tp_particip ||
					r_C11_w.tp_tabela ||
					r_C11_w.cd_servico ||
					r_C11_w.qt_cobrada ||
					r_C11_w.vl_dif_vl_inter ||
					r_C11_w.vl_serv_cob ||
					r_C11_w.nr_autoriz;

			-- Gerar dados no arquivo
			pls_utl_file_pck.escrever(ds_conteudo_w);
			ds_conteudo_w := null;

			update	ptu_nota_cobranca_reem b
			set	nr_seq_a500	= nr_seq_registro_w
			where	nr_sequencia	= r_C11_w.nr_sequencia;
		end loop;*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_fatura_v70_utl ( nr_seq_fatura_p ptu_fatura.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, ds_local_windows_utl_p INOUT text ) FROM PUBLIC;
