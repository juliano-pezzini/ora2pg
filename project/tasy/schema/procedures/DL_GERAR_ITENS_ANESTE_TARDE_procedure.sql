...
CREATE OR REPLACE PROCEDURE dl_gerar_itens_aneste_tarde ( nr_seq_distribuicao_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, vl_distribuicao_p bigint) AS $body$
DECLARE
...
qt_rotinas_tarde_w		bigint;	
qt_min_cirurgia_w		double precision;				
qt_min_cirurgia_socio_w		double precision;				
vl_unitario_anestesia_w		double precision;
vl_caixa_anestesias_tarde_w	double precision;
...
dt_inicio_ves_w			timestamp;
dt_termino_ves_w		timestamp;
...
if (dl_obter_dados_socio(nr_seq_socio_w,'T') = 'A') then 
	...
	qt_max_minutos_w		:= 0;
	qt_rotinas_tarde_w		:= 0;
	qt_min_cirurgia_w		:= 0;
	qt_min_cirurgia_socio_w		:= 0;
	vl_unitario_anestesia_w		:= 0;
	vl_caixa_anestesias_tarde_w	:= 0;
	vl_distribuicao_w		:= 0;
	vl_hora_w			:= 0;
	vl_pagar_socio_w		:= 0;
	tx_criterio_w			:= 0;
	...
	/* dominio DL - Tipo de item para calculo da distribuicao / Anestesias da tarde */
	...
	if (coalesce(nr_seq_tipo_distrib_w::text, '') = '') then -- afstringari 236244 28/07/2010 
		...
		ds_msg_tp_distrib_w	:= substr(dl_obter_msg_tp_distrib(1,7),1,255);
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		begin 
			...
			dt_inicio_ves_w 	:= to_date((to_char(trunc(dt_entrada_w),'dd/mm/yyyy') || ' ' || to_char(dt_hora_inicial_w, 'hh24:mi:ss')), 'dd/mm/yyyy hh24:mi:ss');
			dt_termino_ves_w 	:= to_date((to_char(trunc(dt_saida_w),'dd/mm/yyyy') || ' ' || to_char(dt_hora_final_w, 'hh24:mi:ss')), 'dd/mm/yyyy hh24:mi:ss');	
			 
			if (dt_saida_w > dt_inicio_ves_w) then	 
			 
				dt_calc_inicio_w	:= dt_entrada_w;
				dt_calc_termino_w	:= dt_saida_w;
			 
				if (dt_entrada_w < dt_inicio_ves_w) then 
					dt_calc_inicio_w := dt_inicio_ves_w;
				end if;
				 
				if (dt_saida_w > dt_termino_ves_w) then 
					dt_calc_termino_w := dt_termino_ves_w;
				end if;
 
				qt_min_cirurgia_w	:= ((dt_calc_termino_w - dt_calc_inicio_w) * 1440);
				 
				-- qtd de minutos trabalhados pelo socio 
				if (cd_pessoa_fisica_w = cd_pessoa_fisica_socio_w) then 
					qt_min_cirurgia_socio_w	:=	qt_min_cirurgia_socio_w + qt_min_cirurgia_w;
				end if;
			 
				qt_rotinas_tarde_w	:= (qt_rotinas_tarde_w + ceil(qt_min_cirurgia_w / qt_max_minutos_w));			
			end if;	
		end;
	end loop;
	close C01;
 
	vl_caixa_anestesias_tarde_w	:= vl_unitario_anestesia_w * qt_rotinas_tarde_w;
	vl_hora_w		:= dividir_sem_round(vl_caixa_anestesias_tarde_w,(qt_min_cirurgia_w / 60));
	vl_pagar_socio_w		:= vl_hora_w * (qt_min_cirurgia_socio_w /60);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dl_gerar_itens_aneste_tarde ( nr_seq_distribuicao_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, vl_distribuicao_p bigint) FROM PUBLIC;
