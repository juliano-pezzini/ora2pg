...
CREATE OR REPLACE PROCEDURE copiar_transferir_agenda_cons (cd_estabelecimento_p bigint, nr_seq_origem_p bigint, nr_seq_destino_p bigint, ie_acao_p text, nr_seq_motivo_p bigint, ds_motivo_p text, nm_usuario_p text, nr_seq_atendimento_p bigint, nr_seq_evento_atend_p bigint, ds_retorno_p INOUT text) AS $body$
DECLARE
...
ie_utiliza_prof_horario_w		varchar(1);
...
ie_transacao_sus_copia_w		varchar(1);
ie_transacao_sus_transf_w		varchar(1);
...
nr_seq_agendamento_w			agenda_consulta.nr_seq_agendamento%type;
ie_consiste_regra_conv_turno_w		varchar(1);
ie_convenio_cancelamento_w		varchar(1);
ie_consiste_mesmo_paciente_w		varchar(1);
...
qt_solic_cip_w			bigint;
cd_estab_solic_cpi_w		smallint;
cd_setor_solic_cpi_w		bigint;
...
if (cd_estabelecimento_p IS NOT NULL AND cd_estabelecimento_p::text <> '') and (nr_seq_origem_p IS NOT NULL AND nr_seq_origem_p::text <> '') and (nr_seq_destino_p IS NOT NULL AND nr_seq_destino_p::text <> '') and (ie_acao_p IS NOT NULL AND ie_acao_p::text <> '') and (nm_usuario_p IS NOT NULL AND nm_usuario_p::text <> '') then
	...
	ie_reservado_w := reservar_horario_agecons(nr_seq_destino_p, nm_usuario_p, ie_reservado_w);	
				
	if (ie_reservado_w = 'N') then
		CALL Wheb_mensagem_pck.exibir_mensagem_abort(262368);
	end if;
	...
	ie_convenio_cancelamento_w := Obter_Param_Usuario(821, 372, obter_perfil_ativo, nm_usuario_p, wheb_usuario_pck.get_cd_estabelecimento, ie_convenio_cancelamento_w);
	ie_consiste_tempo_minimo_w := Obter_Param_Usuario(821, 488, obter_perfil_ativo, nm_usuario_p, wheb_usuario_pck.get_cd_estabelecimento, ie_consiste_tempo_minimo_w);
	ie_consiste_mesmo_paciente_w := Obter_Param_Usuario(821, 432, obter_perfil_ativo, nm_usuario_p, wheb_usuario_pck.get_cd_estabelecimento, ie_consiste_mesmo_paciente_w);
	...
	select	b.cd_agenda,
			...
			b.ie_classif_agenda,
			b.cd_medico_req,
			coalesce(a.ie_utiliza_prof_horario, 'N'),
			a.cd_setor_agenda,
			a.cd_pessoa_fisica
	...
	if (nr_seq_segurado_w IS NOT NULL AND nr_seq_segurado_w::text <> '') then
		...
		if (ie_situacao_atend_w <> 'A') then
			CALL WHEB_MENSAGEM_PCK.Exibir_Mensagem_Abort(239972);
		end if;
	end if;
	...
	if	((ie_consiste_mesmo_paciente_w	= 'S') or (ie_consiste_mesmo_paciente_w = 'A')) then
		ie_permite_agendamento_w := Consistir_agend_espec_dif(cd_pessoa_fisica_w, cd_agenda_destino_w, hr_destino_w, nr_minuto_duracao_w, ie_permite_agendamento_w);
		...
		if (ie_permite_agendamento_w = 'N') then
			if (ie_consiste_mesmo_paciente_w	= 'S') then
				...
				CALL Wheb_mensagem_pck.exibir_mensagem_abort(213811);
	...
	if (ie_consiste_tempo_minimo_w = 'S') then
		begin
			CALL Consistir_quimio_dur_agecons(hr_destino_w, cd_pessoa_fisica_w, nm_usuario_p, cd_estabelecimento_p);
		Exception when others then
			liberar_horario_agecons(nr_seq_destino_p, nm_usuario_p, nr_seq_turno_w);
			CALL Wheb_mensagem_pck.exibir_mensagem_abort( 262328 , 'DS_MENSAGEM='||substr(sqlerrm,1,255));
		end;
	end if;
	
	CALL consiste_convenio_agecons(nr_seq_destino_p, cd_agenda_destino_w, cd_convenio_w, hr_destino_w, nm_usuario_p);
	
	IF (ie_acao_p = 'C') THEN
    		BEGIN
    		SELECT * FROM consistir_qtd_conv_regra_cons(	nr_seq_destino_p, cd_convenio_w, hr_destino_w, cd_agenda_destino_w, cd_pessoa_fisica_w, cd_categoria_w, cd_plano_w, cd_estabelecimento_p, nm_usuario_p, cd_medico_agenda_destino_w, nr_seq_proc_interno_w, ie_tipo_atendimento_w, ds_erro_ww, ds_erro_ww, ds_erro_ww) INTO STRICT ds_erro_ww, ds_erro_ww, ds_erro_ww;
	...
	if (ie_perm_agendar_classif_w IS NOT NULL AND ie_perm_agendar_classif_w::text <> '') then
		CALL liberar_horario_agenda_cons(nr_seq_destino_p, nm_usuario_p);
		CALL Wheb_mensagem_pck.exibir_mensagem_abort( 262370 , 'DS_MENSAGEM='||ie_perm_agendar_classif_w);
	end if;
	...
	if (ie_acao_p = 'C') then
		...
		if (ie_med_req_copia_w = 'N') and (ie_utiliza_prof_horario_w = 'N')then
			cd_medico_req_w := null;
		else
			select	CASE WHEN ie_utiliza_prof_horario_w='S' THEN  cd_medico_req_ww  ELSE cd_medico_req_w END
		...
		if (ie_med_req_transf_w = 'N') and (ie_utiliza_prof_horario_w = 'N')then
			cd_medico_req_w := null;
		else
			select	CASE WHEN ie_utiliza_prof_horario_w='S' THEN  cd_medico_req_ww  ELSE cd_medico_req_w END
			into STRICT	cd_medico_req_w
	...
	ds_erro_w := consistir_prazo_convenio_agend(cd_pessoa_fisica_w, hr_destino_w, cd_convenio_w, nm_usuario_p, cd_estabelecimento_p, ds_erro_w);
	if (ds_erro_w IS NOT NULL AND ds_erro_w::text <> '') then
		CALL liberar_horario_agenda_cons(nr_seq_destino_p, nm_usuario_p);
		CALL Wheb_mensagem_pck.exibir_mensagem_abort( 262328 , 'DS_MENSAGEM='||ds_erro_w);
	end if;
	...
	SELECT * FROM consistir_classif_agecon(cd_estabelecimento_p, cd_pessoa_fisica_w, hr_destino_w, cd_agenda_w, cd_convenio_w, null, null, null, ie_classif_proc_w, nr_seq_destino_p, ie_consistencia_w, ie_agenda_w) INTO STRICT ie_consistencia_w, ie_agenda_w;
	if (ie_agenda_w = 'N') then
		CALL liberar_horario_agenda_cons(nr_seq_destino_p, nm_usuario_p);
		CALL Wheb_mensagem_pck.exibir_mensagem_abort( 262328 , 'DS_MENSAGEM='||ie_consistencia_w);
	end if;
	
	if (ie_convenio_cancelamento_w = 'S') and (cd_convenio_w IS NOT NULL AND cd_convenio_w::text <> '') then
		CALL Agecons_cons_se_conv_canc(cd_convenio_w,hr_destino_w);
	end if;					
	...
	ie_consiste_regra_conv_turno_w := Obter_Param_Usuario(821, 381, obter_perfil_ativo, nm_usuario_p, wheb_usuario_pck.get_cd_estabelecimento, ie_consiste_regra_conv_turno_w);
	if (ie_acao_p 	= 'C') then	
		ds_consistencia_w := consistir_turno_convenio(nr_seq_destino_p, cd_agenda_destino_w, hr_destino_w, nr_seq_turno_w, cd_convenio_w, cd_categoria_w, ds_consistencia_w, nm_usuario_p, cd_estabelecimento_p, ie_acao_p, nr_seq_origem_p);	
	elsif (ie_acao_p = 'T') and (ie_consiste_regra_conv_turno_w = 'S') then		
		ds_consistencia_w := consistir_turno_convenio(nr_seq_destino_p, cd_agenda_destino_w, hr_destino_w, nr_seq_turno_w, cd_convenio_w, cd_categoria_w, ds_consistencia_w, nm_usuario_p, cd_estabelecimento_p, ie_acao_p, nr_seq_origem_p);
	...
	ds_consistencia_w := consistir_turno_esp_convenio(cd_agenda_destino_w, hr_destino_w, nr_seq_turno_esp_w, cd_convenio_w, nr_seq_destino_p, ds_consistencia_w, cd_categoria_w, ie_acao_p);
...
;
  ...
  if (ie_conv_nao_lib_agenda_w = 'N') then
	...
	if (ie_acao_p = 'T') then
		...
		select	count(*)
		into STRICT	qt_solic_cip_w
		from	same_cpi_solic
		where	nr_seq_agenda = nr_seq_origem_p;
		
		select	max(cd_setor_exclusivo),
			max(cd_estabelecimento)
		into STRICT	cd_setor_solic_cpi_w,
		...
		if (qt_solic_cip_w > 0) then
			...
			set	nr_seq_agenda = nr_seq_destino_p,
				...
				cd_setor_atendimento = coalesce(cd_setor_solic_cpi_w,cd_setor_atendimento),
				cd_estabelecimento  = cd_estab_solic_cpi_w
			where	nr_seq_agenda = nr_seq_origem_p;
		end if;
	end if;
	
	CALL agecons_consiste_cmce(cd_pessoa_fisica_w, cd_convenio_w, cd_agenda_destino_w, hr_destino_w);
	...
	set	cd_pessoa_fisica		= cd_pessoa_fisica_w,
		...
		dt_validade_senha		= CASE WHEN ie_acao_p='C' THEN dt_validade_senha_w  ELSE null END ,
    ...
    begin
		select gerenciador_mipres_agenda_pck.get_valid_mipres_agenda(aca.nr_seq_controle_mipres, hr_inicio_w)
		  into STRICT nr_seq_controle_mipres_w
		  from agenda_consulta_adic aca
		 where aca.nr_seq_agenda = nr_seq_origem_p;
	...
	if (coalesce(nr_seq_controle_mipres_w, 0) > 0) then
		...
		values (nr_seq_controle_mipres_w, nr_seq_destino_p, clock_timestamp(), nm_usuario_p);
	end if;
	...
	if (ie_manut_proced_w = 'S') or
		...
		EXIT WHEN NOT FOUND; /* apply on c01 */
			...
			nr_sequencia_mipres_w := gerenciador_mipres_agenda_pck.get_valid_mipres_agenda(nr_seq_controle_mipres_w, hr_destino_w);
	...
	if (ie_acao_p = 'T') then
		...
		CALL gerar_agenda_consulta_hist(cd_agenda_destino_w,nr_seq_origem_p,'T',nm_usuario_p,wheb_mensagem_pck.get_texto(793141,
		...
		CALL gerar_agenda_consulta_hist(cd_agenda_destino_w,nr_seq_origem_p,'CP',nm_usuario_p,wheb_mensagem_pck.get_texto(793142,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE copiar_transferir_agenda_cons (cd_estabelecimento_p bigint, nr_seq_origem_p bigint, nr_seq_destino_p bigint, ie_acao_p text, nr_seq_motivo_p bigint, ds_motivo_p text, nm_usuario_p text, nr_seq_atendimento_p bigint, nr_seq_evento_atend_p bigint, ds_retorno_p INOUT text) FROM PUBLIC;
