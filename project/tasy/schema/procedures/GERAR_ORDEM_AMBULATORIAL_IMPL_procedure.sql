...
CREATE OR REPLACE PROCEDURE gerar_ordem_ambulatorial_impl ( dt_referencia_p timestamp, cd_estabelecimento_p bigint, ie_gerar_ordem_pac_chegada_p text, nm_usuario_p text, ie_gera_inconsistencia_p text default 'N') AS $body$
DECLARE
...
ie_prescr_anterior_w  varchar(1);

C01 CURSOR FOR
  SELECT  a.nr_seq_atendimento,
    a.nr_seq_paciente,
    a.nr_prescricao,
    coalesce(a.dt_real,dt_prevista),
...
ie_estabe_tratamento_w := Obter_Param_Usuario(3130, 331, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_p, ie_estabe_tratamento_w);
...
;
            ...
            CALL gravar_log_tasy(0207,   'nr_sequencia_medic_w: '||nr_sequencia_medic_w||chr(13)||
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_ordem_ambulatorial_impl ( dt_referencia_p timestamp, cd_estabelecimento_p bigint, ie_gerar_ordem_pac_chegada_p text, nm_usuario_p text, ie_gera_inconsistencia_p text default 'N') FROM PUBLIC;
