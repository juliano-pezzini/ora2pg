...
CREATE OR REPLACE PROCEDURE ctb_pls_contab_fat_proj ( nr_lote_contabil_p bigint, nm_usuario_p text, ie_exclusao_p text, ds_retorno_p INOUT text) AS $body$
DECLARE
...
C01 CURSOR FOR
	SELECT	'P' ie_proc_mat,
		...
		CASE WHEN s.ie_tipo_segurado='I' THEN (pls_obter_vl_pag_fat_pos(e.nr_sequencia,'T'))::numeric  WHEN s.ie_tipo_segurado='T' THEN (pls_obter_vl_pag_fat_pos(e.nr_sequencia,'T'))::numeric   ELSE 0 END  vl_contab_dif,
...
WHERE t.nr_sequencia 		= a.nr_seq_conta and a.nr_sequencia 		= e.nr_seq_conta_pos and s.nr_sequencia 		= t.nr_seq_segurado and a.nr_sequencia 		= p.nr_seq_conta_pos_estab and c.nr_sequencia 		= p.nr_seq_fatura_conta and d.nr_sequencia 		= c.nr_seq_fatura_evento and f.nr_sequencia 		= d.nr_seq_fatura and l.nr_sequencia 		= f.nr_seq_lote and ((ie_gerar_fat_contab_w	= 'N')
	or (e.nr_sequencia		= p.nr_seq_conta_pos_contab))   and coalesce(f.nr_seq_cancel_fat::text, '') = '' and coalesce(f.nr_lote_contabil,0)	= 0 and (((ie_data_rec_faturamento_w	= 'TI' -- Emissao do titulo
...
union
				...
				where	x.nr_lote_contabil 	= nr_lote_contabil_p
				and	f.nr_seq_lote		= x.vl_parametro)))
...
WHERE t.nr_sequencia 		= a.nr_seq_conta and a.nr_sequencia 		= e.nr_seq_conta_pos and s.nr_sequencia 		= t.nr_seq_segurado and a.nr_sequencia 		= m.nr_seq_conta_pos_estab and c.nr_sequencia 		= m.nr_seq_fatura_conta and d.nr_sequencia 		= c.nr_seq_fatura_evento and f.nr_sequencia 		= d.nr_seq_fatura and l.nr_sequencia 		= f.nr_seq_lote and ((ie_gerar_fat_contab_w	= 'N')
	or (e.nr_sequencia		= m.nr_seq_conta_pos_contab))   and coalesce(f.nr_seq_cancel_fat::text, '') = '' and coalesce(f.nr_lote_contabil,0)	= 0 and (((ie_data_rec_faturamento_w	= 'TI' -- Emissao do titulo
	and 	coalesce(f.ie_cancelamento,'C')	<> 'E'
	and (r.dt_emissao between dt_inicial_w and dt_final_w
	or (coalesce(f.nr_titulo,0)	= 0
	and	f.ie_cancelamento 	= 'C'
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ctb_pls_contab_fat_proj ( nr_lote_contabil_p bigint, nm_usuario_p text, ie_exclusao_p text, ds_retorno_p INOUT text) FROM PUBLIC;
