...
CREATE OR REPLACE PROCEDURE hnsg_gerar_w_iss_pmc (cd_estabelecimento_p bigint, nm_usuario_p text, dt_mesano_ini_p timestamp, dt_mesano_fim_p timestamp) AS $body$
DECLARE
...
SELECT	1					ie_tipo_registro,
	CASE WHEN a.cd_cgc_emitente='76562198000169' THEN 1  ELSE 1 END 		ie_tipo_documento,
	''					ie_tipo_recolhimento,
	'C'					ds_registro,
	a.dt_entrada_saida			dt_emissao,
...
and	trunc(a.dt_entrada_saida,'month') between trunc(dt_mesano_ini_p,'month') and trunc(dt_mesano_fim_p,'month')
...
and	((a.cd_cgc_emitente = '76562198000169' and a.cd_operacao_nf = 25) or (a.cd_operacao_nf = 3))

union

SELECT	2					ie_tipo_registro,
	CASE WHEN a.cd_cgc_emitente='76562198000169' THEN 1  ELSE 3 END 		ie_tipo_documento,
...
select	3 	ie_tipo_registro,
	...
	'' cd_item_lista_serv,
	b.vl_vencimento,
	(select coalesce(max(x.vl_imposto),0)
	from	tributo t,
...
and	trunc(dt_emissao,'month') between trunc(dt_mesano_ini_p,'month') and trunc(dt_mesano_fim_p,'month')
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE hnsg_gerar_w_iss_pmc (cd_estabelecimento_p bigint, nm_usuario_p text, dt_mesano_ini_p timestamp, dt_mesano_fim_p timestamp) FROM PUBLIC;
