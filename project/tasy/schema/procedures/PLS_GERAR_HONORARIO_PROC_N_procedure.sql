...
CREATE OR REPLACE PROCEDURE pls_gerar_honorario_proc_n ( nr_seq_conta_proc_p bigint, cd_procedimento_p bigint, ie_origem_proced_p bigint, nr_seq_grau_partic_p bigint, ie_tipo_guia_p text, nr_seq_tipo_atendimento_p bigint, ie_tipo_conta_p text, nr_seq_prestador_p bigint, ie_tipo_protocolo_p text, nr_auxiliares_p bigint, ie_status_p text, ie_repassa_medico_p text, vl_anestesista_p bigint, vl_medico_p bigint, vl_auxiliares_p bigint, cd_estabelecimento_p bigint, dt_procedimento_p timestamp, nm_usuario_p text, dt_preco_p timestamp, nr_seq_plano_p bigint, ie_origem_calc_p text, vl_total_partic_p INOUT bigint, ie_tipo_partic_prof_p INOUT text, nr_seq_prestador_pgto_p bigint, ie_repassa_medico_pgto_p text) AS $body$
DECLARE


--Rotina utilizada na nova regra de valorização, não é compatível com a antiga deve haver atenção na utilização
...
WHERE a.nr_sequencia		= b.nr_seq_conta_proc  and a.nr_sequencia		= nr_seq_conta_proc_p and ((coalesce(b.ie_status::text, '') = '') or (b.ie_status != 'C')) and ((coalesce(b.ie_gerada_cta_honorario::text, '') = '') or (b.ie_gerada_cta_honorario <> 'S')) and coalesce(nr_seq_cp_comb_entao::text, '') = ''
...
union all
	...
	SELECT	'P' ie_tipo_tabela_partic,
		...
		a.ie_status,
		b.vl_medico_base,
		b.vl_anestesista_base,
		b.vl_auxiliares_base
	FROM pls_conta_proc a, pls_proc_participante b
LEFT OUTER JOIN pls_grau_participacao c ON (b.nr_seq_grau_partic = c.nr_sequencia)
WHERE a.nr_sequencia		= b.nr_seq_conta_proc  and a.nr_sequencia		= nr_seq_conta_proc_p and ((coalesce(b.ie_status::text, '') = '') or (b.ie_status != 'C')) and ((coalesce(b.ie_gerada_cta_honorario::text, '') = '') or (b.ie_gerada_cta_honorario <> 'S')) and (nr_seq_cp_comb_entao IS NOT NULL AND nr_seq_cp_comb_entao::text <> '')
...
fetch C01 into
	...
	ie_status_w,
	vl_medico_part_w,
	vl_anestesista_part_w,
	vl_auxiliares_part_w;
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (ie_origem_proced_p = 4) or (ie_anestesista_w = 'S') or (ie_medico_w = 'S') or (ie_auxiliar_w = 'S') or (ie_tipo_protocolo_p = 'R') or (nr_aux_w <= nr_auxiliares_p) then
		...
		if (vl_medico_part_w	> 0) or (vl_anestesista_part_w	> 0) or (vl_auxiliares_part_w 	> 0) then

			vl_auxiliares_w := vl_auxiliares_part_w;
			vl_medico_w	:= vl_medico_part_w;
			vl_anestesista_w:= vl_anestesista_part_w;
	...
	vl_participante_w	:= round((vl_participante_w)::numeric,2);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_honorario_proc_n ( nr_seq_conta_proc_p bigint, cd_procedimento_p bigint, ie_origem_proced_p bigint, nr_seq_grau_partic_p bigint, ie_tipo_guia_p text, nr_seq_tipo_atendimento_p bigint, ie_tipo_conta_p text, nr_seq_prestador_p bigint, ie_tipo_protocolo_p text, nr_auxiliares_p bigint, ie_status_p text, ie_repassa_medico_p text, vl_anestesista_p bigint, vl_medico_p bigint, vl_auxiliares_p bigint, cd_estabelecimento_p bigint, dt_procedimento_p timestamp, nm_usuario_p text, dt_preco_p timestamp, nr_seq_plano_p bigint, ie_origem_calc_p text, vl_total_partic_p INOUT bigint, ie_tipo_partic_prof_p INOUT text, nr_seq_prestador_pgto_p bigint, ie_repassa_medico_pgto_p text) FROM PUBLIC;
