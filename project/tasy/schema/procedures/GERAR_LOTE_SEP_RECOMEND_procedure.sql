...
CREATE OR REPLACE PROCEDURE gerar_lote_sep_recomend ( nr_prescricao_p bigint, nr_sequencia_p bigint, nr_seq_horario_p bigint, ie_so_aprazado_p text, nm_usuario_p text, ie_origem_lote_p text, ie_send_integration_p text default 'N') AS $body$
DECLARE
...
nr_seq_recomendacao_ant_w	prescr_material.nr_seq_recomendacao%type := 0;
nr_atendimento_w		prescr_medica.nr_atendimento%type;
ie_somente_gerar_acm_sn_w	varchar(1);
ie_gerar_lote_acm_sn_uti_w 	varchar(1);

C01 CURSOR FOR  --Especifico para recomendacao
	...
	and	coalesce(c.nr_seq_recomendacao,0) <> 0
...
C04 CURSOR FOR
  ...
  --PKG_DATE_UTILS.start_of(nvl(dt_fim_vigencia,sysdate), 'dd', 0) 
...
if (coalesce(nr_seq_regra_sn_w,0) > 0) then
...
values (nextval('ap_lote_log_seq'),
	...
	' cd_estabelecimento_w='           || cd_estabelecimento_w           || ' cd_setor_atendimento_w='     || cd_setor_atendimento_w     ||
	...
	' ie_gerar_solucao_separado_w='    || ie_gerar_solucao_separado_w    || ' ie_classif_nao_padrao_w='    || ie_classif_nao_padrao_w    ||
	' nr_seq_regra_sn_w='              || nr_seq_regra_sn_w              || ' ie_gera_lote_orig_w='        || ie_gera_lote_orig_w        ||
	' ie_gera_lote_medic_pac_w='       || ie_gera_lote_medic_pac_w       || ' ie_lote_acm_sn_w='           || ie_lote_acm_sn_w,
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	values (nextval('ap_lote_log_seq'),
		...
		nr_seq_horario_p,
		null,
		'gerar_lote_sep_recomend1',
		' ie_origem_lote_p = '||ie_origem_lote_p || ' nr_seq_turno_w = ' || nr_seq_turno_w || ' nr_seq_turno_ant_w = ' || nr_seq_turno_ant_w ||
		' nr_seq_classif_w = ' || nr_seq_classif_w || ' nr_seq_classif_ant_w = ' || nr_seq_classif_ant_w || ' dt_horario_w = ' || PKG_DATE_FORMATERS_TZ.TO_VARCHAR(dt_horario_w,'timestamp',ESTABLISHMENT_TIMEZONE_UTILS.gettimezone) ||
		' dt_horario_ant_w = ' || PKG_DATE_FORMATERS_TZ.TO_VARCHAR(dt_horario_ant_w,'timestamp',ESTABLISHMENT_TIMEZONE_UTILS.gettimezone) || ' ie_gerar_novo_lote_turno_dia_w = ' || ie_gerar_novo_lote_turno_dia_w ||
		...
		' nr_seq_lote = ' || nr_sequencia_w || ' nr_seq_prescr_mat_w = ' || nr_seq_prescr_mat_w || ' ie_mesmo_turno_w = ' || ie_mesmo_turno_w || 
		' nr_seq_recomendacao_w=' || nr_seq_recomendacao_w || ' nr_seq_recomendacao_ant_w=' || nr_seq_recomendacao_ant_w,
		substr(dbms_utility.format_call_stack,1,2000));

	if (nr_seq_recomendacao_w <> nr_seq_recomendacao_ant_w) or (nr_seq_turno_w <> nr_seq_turno_ant_w) or (nr_seq_classif_w <> nr_seq_classif_ant_w) or
		((PKG_DATE_UTILS.start_of(dt_horario_w, 'dd', 0) <> PKG_DATE_UTILS.start_of(dt_horario_ant_w, 'dd', 0)) and (ie_gerar_novo_lote_turno_dia_w = 'S')) or (cd_local_estoque_w <> cd_local_estoque_ant_w) or
	...
	nr_seq_recomendacao_ant_w	:= nr_seq_recomendacao_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_lote_sep_recomend ( nr_prescricao_p bigint, nr_sequencia_p bigint, nr_seq_horario_p bigint, ie_so_aprazado_p text, nm_usuario_p text, ie_origem_lote_p text, ie_send_integration_p text default 'N') FROM PUBLIC;
