...
CREATE OR REPLACE PROCEDURE gerar_comunic_liberacao_compra ( nr_ordem_compra_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
/* Comunica liberação de uma determinda ordem de compra */
...
EXIT WHEN NOT FOUND; /* apply on C05 */
	...
	if (nr_seq_regra_w > 0) then 
		...
		select	substr(max(obter_dados_regra_com_compra(nr_ordem_compra, null, 917, cd_evento_w, nr_seq_regra_w, 'M')),1,2000) ds_comunicacao, 
			substr(max(obter_dados_regra_com_compra(nr_ordem_compra, null, 917, cd_evento_w, nr_seq_regra_w,'T')),1,80) ds_titulo 
		...
		if (ds_destinatario_w IS NOT NULL AND ds_destinatario_w::text <> '') or (cd_perfil_w IS NOT NULL AND cd_perfil_w::text <> '') then 
			...
			values (		cd_estabelecimento_w, 
					...
					ds_titulo_w, 
					ds_comunicacao_w, 
					WHEB_MENSAGEM_PCK.get_texto(301407), 
					clock_timestamp(), 
					'N', 
					ds_destinatario_w, 
					nr_sequencia_w, 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_comunic_liberacao_compra ( nr_ordem_compra_p bigint, nm_usuario_p text) FROM PUBLIC;
