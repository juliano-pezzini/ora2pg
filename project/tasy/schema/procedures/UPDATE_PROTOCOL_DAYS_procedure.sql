...
CREATE OR REPLACE PROCEDURE update_protocol_days ( NR_SEQUENCIA_P bigint , NR_SEQ_PRT_INT_PAC_ETAPA_P bigint , NR_DIA_PROTOCOLO_P bigint , NR_DIA_P bigint , IE_DIA_AHEAD_P text , IE_DIA_BACK_P text , NR_SEQ_ORDEM_P bigint ) AS $body$
DECLARE
...
NEW_ESTIMATED_DATE_W timestamp;
...
C01 CURSOR FOR
    ...
    WHERE  A.NR_SEQUENCIA = B.NR_SEQ_PROTOCOLO_INT_PAC 
           ...
           AND A.NR_SEQUENCIA = (SELECT NR_SEQ_PROTOCOLO_INT_PAC 
                                  FROM   PROTOCOLO_INT_PAC_ETAPA 
                                  WHERE  NR_SEQUENCIA = NR_SEQ_PRT_INT_PAC_ETAPA_P  LIMIT 1)
...
C02 CURSOR FOR
    SELECT *
    FROM PROTOCOLO_INT_PAC_ETAPA 
    WHERE NR_SEQ_PROTOCOLO_INT_PAC = (SELECT NR_SEQ_PROTOCOLO_INT_PAC 
                                      FROM   PROTOCOLO_INT_PAC_ETAPA 
                                      WHERE  NR_SEQUENCIA = NR_SEQ_PRT_INT_PAC_ETAPA_P  LIMIT 1);
...
OPEN C01;
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
        ...
        IF IE_DIA_AHEAD_P = 'S' THEN
                ...
                NEW_ESTIMATED_DATE_W := CURSOR_W.DT_PREVISTA+NR_DIA_P;
        ...
        IF (NEW_STAGE_W = 0) THEN
            ...
            DT_PREVISTA = NEW_ESTIMATED_DATE_W,
            NR_SEQ_PRT_INT_PAC_ETAPA = NEW_STAGE_W
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE update_protocol_days ( NR_SEQUENCIA_P bigint , NR_SEQ_PRT_INT_PAC_ETAPA_P bigint , NR_DIA_PROTOCOLO_P bigint , NR_DIA_P bigint , IE_DIA_AHEAD_P text , IE_DIA_BACK_P text , NR_SEQ_ORDEM_P bigint ) FROM PUBLIC;
