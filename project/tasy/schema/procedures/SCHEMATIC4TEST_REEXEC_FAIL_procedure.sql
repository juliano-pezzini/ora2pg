...
CREATE OR REPLACE PROCEDURE schematic4test_reexec_fail (NR_SEQUENCIA_P bigint, NM_USUARIO_P text) AS $body$
DECLARE
...
--loop scripts
C01 CURSOR FOR
  SELECT DISTINCT LOG1.NR_SEQ_SCRIPT NR_SEQ_SCRIPT
      INTO STRICT NR_SEQ_SCRIPT_W
  FROM SCHEM_TEST_LOG LOG1 
  WHERE LOG1.NR_SEQ_SCHEDULE = NR_SEQUENCIA_W 
  AND LOG1.IE_STATUS in ('2','5');


BEGIN
	--procedure that reexec test failed
	...
	VALUES (nextval('schem_test_beholder_seq'), NM_USUARIO_P, clock_timestamp(), 'Robot', clock_timestamp(), 'SCHEMATIC4TEST_REEXEC_FAIL', 
  ...
  IF (NR_SEQUENCIA_W IS NOT NULL AND NR_SEQUENCIA_W::text <> '') THEN  
      ...
      NM_SUITE_W2 := 'Failure from '||NR_SEQUENCIA_W||' - '||NM_SUITE_W;
...
;
                ...
                INSERT INTO SCHEM_TEST_TEST(nr_sequencia, nr_seq_execution, nr_seq_script, nr_seq_suite, nr_seq_robot, nr_seq_grid, dt_atualizacao, nm_usuario, dt_atualizacao_nrec, nm_usuario_nrec, ie_jobs, ie_switch) 
                    VALUES (NEWSEQUENCETEST, NR_SEQ_EXECUTION_W+5, NR_SEQ_SCRIPT_W, NR_SEQ_SUITE_W, NR_SEQ_ROBOT_W2, NR_SEQ_GRID_W2, clock_timestamp(), NM_USUARIO_P, clock_timestamp(), NM_USUARIO_P, '0', '1');
                COMMIT;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE schematic4test_reexec_fail (NR_SEQUENCIA_P bigint, NM_USUARIO_P text) FROM PUBLIC;
