...
CREATE OR REPLACE PROCEDURE registrar_nivel_glicemia_prot ( nr_atendimento_p bigint, nr_prescricao_p bigint, nr_seq_proced_p bigint, nr_seq_horario_p bigint, nr_seq_glic_atend_p bigint, nr_seq_protocolo_p bigint, dt_glicemia_p timestamp, qt_glicemia_p bigint, nm_usuario_p text, ie_aprazar_p text, ie_extrapol_p text, nr_seq_pergunta_p bigint, nr_seq_glicemia_p INOUT bigint, ie_medic_suplementar_p INOUT text, ds_orientacao_item_p INOUT text, ie_resposta_p text default null, ds_parametros_p text default null) AS $body$
DECLARE
...
ie_chamada_glicemia_w	varchar(1);
ie_existe_mat_prot_w	varchar(1);
qt_item_assoc_w			prescr_mat_hor.qt_dispensar_hor%type;
...
C01 CURSOR FOR
	...
	and		coalesce(ie_glicemia, 'X') in (ie_glicose_w)
...
C02 CURSOR FOR /* Perguntas e Respostas */
...
if (nr_atendimento_p IS NOT NULL AND nr_atendimento_p::text <> '') and (nr_seq_protocolo_p IS NOT NULL AND nr_seq_protocolo_p::text <> '') and (qt_glicemia_p IS NOT NULL AND qt_glicemia_p::text <> '') and (nm_usuario_p IS NOT NULL AND nm_usuario_p::text <> '') then
	...
	/* consistir data medic?o glicemia */
	...
	if (nr_seq_anterior_w = 0) then
		...
		if (coalesce(nr_prescricao_p,0) = 0) and (coalesce(nr_seq_proced_p,0) = 0) and (coalesce(nr_seq_glic_atend_p,0) = 0) then
			...
			--N?o foi possivel realizar a busca dos niveis de glicemia!
		...
		if (nr_seq_nivel_w > 0) then
			...
			if (coalesce(nr_prescricao_p,0) = 0) and (coalesce(nr_seq_proced_p,0) = 0) and (coalesce(nr_seq_glic_atend_p,0) = 0) then
				...
				if (ie_manutencao_w = 'S') then
					select	max(coalesce(qt_insulina_bolus, 0)),
							max(coalesce(qt_glicose, 0)),
				...
				if (ie_manutencao_w = 'S') then
					select	max(qt_insulina_bolus),
							max(qt_glicose),
							coalesce(ds_orientacao_pergunta_w, max(ds_sugestao))
					into STRICT	qt_ui_insulina_calc_w,
						qt_glicose_w,
						ds_sugestao_w
					from	prot_glic_manutencao
			...
			if (ie_gera_hor_assoc_w	= 'N') and (coalesce(nr_seq_pergunta_p,0)	> 0) then
				...
				and		coalesce(a.ie_glicemia, 'N') in (ie_glicose_w)
				and		x.nr_seq_registro = nr_seq_pergunta_p
			...
			if (coalesce(nr_seq_pergunta_p, 0) > 0) then
				ie_chamada_glicemia_w := 'S';
			else
				ie_chamada_glicemia_w := 'N';
			end if;
			...
			if (coalesce(ie_aprazar_p, 'N') = 'S') and (coalesce(nr_seq_pergunta_p,0) = 0) and
				...
				EXIT WHEN NOT FOUND; /* apply on C01 */
					...
					if (nr_seq_prot_glic_w > 0) then
						...
						into STRICT	ie_existe_mat_prot_w
						from	protocolo_glic_mat_med a
						where	a.nr_seq_protocolo_glic = nr_seq_prot_glic_w
						and		a.cd_material = cd_material_w;

						if (ie_existe_mat_prot_w = 'N') then
							select	coalesce(max(a.qt_dispensar_hor), 0)
							into STRICT	qt_item_assoc_w
							from	prescr_mat_hor a
							where	a.nr_prescricao = nr_prescricao_p
							and		a.nr_seq_material = nr_sequencia_w;
						end if;
					end if;

					if (coalesce(qt_item_assoc_w, 0) = 0) then
						qt_item_assoc_w := qt_glicemia_w;
					end if;

					SELECT * FROM aprazar_item_prescr('N', obter_estab_atend(nr_atendimento_p), nr_atendimento_p, 'IAG', 0, cd_material_w, cd_intervalo_w, qt_item_assoc_w, dt_horario_ccg_w, nr_prescricao_p, nr_prescricao_p, nr_sequencia_w, 'N', null, null, nm_usuario_p, ie_inconsistencia_w, ds_inconsistentes_w, null, null, null, null, null, 'S', nr_seq_horario_p, nr_seq_prot_glic_w, null, null, '', null, null) INTO STRICT ie_inconsistencia_w, ds_inconsistentes_w;
			...
			/* Verifica se permite alterar o intervalo na prescric?o. Se sim, busca o intervalo na prescric?o com nvl para o protocolo.. */
	...
	if (Ie_registrar_sv_w = 'S') then
		...
		CALL ADEP_gerar_SV_Glicemia(nr_atendimento_p, qt_glicemia_mg_w, nm_usuario_p, qt_ui_insulina_calc_w, dt_glicemia_w, nr_seq_assinatura_w, null, ie_extrapol_p, qt_glicose_adm_w, qt_glicemia_mmol_w);
	end if;
...
ie_medic_suplementar_p := coalesce(qt_ui_insulina_calc_w, qt_glicose_w);

ds_orientacao_item_p := ds_orientacao_item_w;

if (coalesce(qt_ui_insulina_calc_w,0) > 0) or (coalesce(qt_ui_insulina_int_calc_w,0) > 0) or (coalesce(qt_glicose_w,0) > 0) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE registrar_nivel_glicemia_prot ( nr_atendimento_p bigint, nr_prescricao_p bigint, nr_seq_proced_p bigint, nr_seq_horario_p bigint, nr_seq_glic_atend_p bigint, nr_seq_protocolo_p bigint, dt_glicemia_p timestamp, qt_glicemia_p bigint, nm_usuario_p text, ie_aprazar_p text, ie_extrapol_p text, nr_seq_pergunta_p bigint, nr_seq_glicemia_p INOUT bigint, ie_medic_suplementar_p INOUT text, ds_orientacao_item_p INOUT text, ie_resposta_p text default null, ds_parametros_p text default null) FROM PUBLIC;
