...
CREATE OR REPLACE PROCEDURE hslgv_gerar_w_conta_pmmg ( nr_interno_conta_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
ds_procedimento_convenio_ww	varchar(254);
...
SELECT	a.cd_material_convenio,
	a.cd_setor_atendimento,
	b.cd_grupo_material,
	b.cd_subgrupo_material,
	b.cd_classe_material,
	substr(a.ds_material_convenio,1,254),
	a.dt_atendimento,
...
SELECT	c.cd_procedimento,
	...
	Obter_Dados_Preco_Proc(cd_estabelecimento_w,c.cd_convenio,c.cd_categoria,c.cd_edicao_amb,c.cd_procedimento,c.ie_origem_proced,c.dt_conta,'P') qt_porte_w
...
SELECT	a.cd_procedimento,
	...
	a.nr_cpf_medico,
	obter_conversao_externa(cd_cgc_convenio_w, 'PROCEDIMENTO_PARTICIPANTE','IE_FUNCAO_MEDICO', a.ie_funcao_medico),
	Obter_Nome_Item_Convenio(a.cd_convenio,1,a.cd_procedimento,a.ie_origem_proced,'S',0,0,0,a.cd_setor_atendimento),
	a.cd_procedimento_convenio,
	a.vl_procedimento
from	estrutura_procedimento_v b,
...
--			dbms_output.put_line('cd_material_w = ' || cd_material_w);
		...
		EXIT WHEN NOT FOUND; /* apply on cregraproc */
			...
			if (nr_seq_inf_w IS NOT NULL AND nr_seq_inf_w::text <> '') then
				...
				if (nr_seq_inf_w = 1) then
					...
					cd_item_w		:= cd_procedimento_convenio_w;
					qt_item_w		:= qt_porte_w;
	...
	EXIT WHEN NOT FOUND; /* apply on chonorario */
		...
		EXIT WHEN NOT FOUND; /* apply on cregrahonorario */
			...
			ds_item_w	:= coalesce(ds_procedimento_convenio_ww,ds_procedimento_w);
			dt_item_w	:= null;
			dt_item_w	:= null;
			vl_item_w	:= null;
			ie_tipo_item_w	:= null;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE hslgv_gerar_w_conta_pmmg ( nr_interno_conta_p bigint, nm_usuario_p text) FROM PUBLIC;
