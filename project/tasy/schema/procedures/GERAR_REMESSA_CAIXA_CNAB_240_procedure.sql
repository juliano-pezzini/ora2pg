...
CREATE OR REPLACE PROCEDURE gerar_remessa_caixa_cnab_240 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
select	lpad(b.cd_cgc,14,'0'), 
	...
	rpad(substr(coalesce(elimina_caractere_especial(obter_nome_banco(c.cd_banco)),' '),1,30),30,' ') nm_banco, 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_remessa_caixa_cnab_240 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
