...
CREATE OR REPLACE PROCEDURE ctb_consistir_base_ecd ( nr_seq_controle_sped_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (coalesce(cd_pf_auditor_w,'X') = 'X') and (coalesce(cd_cnpj_auditor_w, 'X') = 'X') and (ie_porte_empresa_w =  'G') then
	i					:= i + 1;
	consistencia_w[i].cd_consistencia	:= 5;
	consistencia_w[i].ds_consistencia	:= 'Falta informar o auditor da Empresa/Estabelecimento';
	consistencia_w[i].ds_acao		:= 'Para empresas de grande porte é preciso cadastrar o auditor na função Empresa/Estabelecimento/Conta/CC';
	consistencia_w[i].qt_tempo		:= 1;
end if;
...
if (qt_registro_w > 0)then
	...
	consistencia_w[i].ds_acao		:= 'A falta do agrupador nos movimentos contábeis pode gerar problemas na identificação dos lançamentos contábeis.'
										...
										|| 'conforme orientações disponíveis no Manual da Escrituração Contábil Digital disponível na extranet';
...
if (qt_mes_aberto_w > 0)then
	...
	consistencia_w[i].ds_consistencia	:= 'Quantidade de meses abertos na Contabilidade: ' || qt_mes_aberto_w;
...
select	count(*)
into STRICT 	qt_registro_w
from	ctb_demo_rubrica_v a,
	ctb_sped_controle b
where	a.nr_seq_demo	= b.nr_seq_demo_bp
and	b.nr_sequencia	= nr_seq_controle_sped_p
and 	trim(both a.ds_rubrica) = '';

if (qt_registro_w > 0)  then
	i					:= i + 1;
	consistencia_w[i].cd_consistencia	:= 28;
	consistencia_w[i].ds_consistencia	:= 'Existem linhas em branco nos demonstrativos de Balanço';
	consistencia_w[i].ds_acao		:= 'Inativar a rúbrica do demonstrativo que gera a linha em branco e recalcular o demonstrativo';
	consistencia_w[i].qt_tempo		:= 1;
end if;
...
from	ctb_demo_rubrica_v a,
	ctb_sped_controle b
where	a.nr_seq_demo	= b.nr_seq_demo_dre
and	b.nr_sequencia	= nr_seq_controle_sped_p
and 	trim(both a.ds_rubrica) = '';

if (qt_registro_w > 0)  then
	i					:= i + 1;
	consistencia_w[i].cd_consistencia	:= 29;
	consistencia_w[i].ds_consistencia	:= 'Existem linhas em branco nos demonstrativos de DRE';
	consistencia_w[i].ds_acao		:= 'Inativar a rúbrica do demonstrativo que gera a linha em branco e recalcular o demonstrativo';
	consistencia_w[i].qt_tempo		:= 1;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ctb_consistir_base_ecd ( nr_seq_controle_sped_p bigint, nm_usuario_p text) FROM PUBLIC;
