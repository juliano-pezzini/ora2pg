...
CREATE OR REPLACE PROCEDURE pls_copia_parametro_prestador ( nr_seq_pres_origem_p bigint, nr_seq_pres_destino_p bigint, nm_tabela_p text, ie_excluir_p text, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	SELECT	nr_sequencia nr_seq_prest_medico,
		nextval('pls_prestador_medico_seq') nr_seq_regra_med
...
if (nm_tabela_p = 'PLS_PRESTADOR_PROC') then
	if (ie_excluir_p = 'S') then
		delete	FROM pls_prestador_proc
...
if (nm_tabela_p = 'PLS_PRESTADOR_AREA') then
	if (ie_excluir_p = 'S') then
		delete	FROM pls_prestador_area
		where 	nr_seq_prestador = nr_seq_pres_destino_p;
	end if;
...
if (nm_tabela_p = 'PLS_PRESTADOR_PLANO') then
	if (ie_excluir_p = 'S') then
		delete	FROM pls_prestador_plano
		where 	nr_seq_prestador = nr_seq_pres_destino_p;
	end if;
...
if (nm_tabela_p = 'PLS_PRESTADOR_MEDICO') then
	if (ie_excluir_p = 'S') then
		delete	FROM pls_prestador_tributo
		where 	nr_seq_prestador_med in (SELECT	nr_sequencia
						from	pls_prestador_medico
						where	nr_seq_prestador = nr_seq_pres_destino_p)
		and	(nr_seq_prestador_med IS NOT NULL AND nr_seq_prestador_med::text <> '');
		
		delete	FROM pls_prestador_medico
		where 	nr_seq_prestador = nr_seq_pres_destino_p;
	end if;
	...
	for r_c01_w in c01 loop
		...
		SELECT	r_c01_w.nr_seq_regra_med,		clock_timestamp(),				nm_usuario_p,
			clock_timestamp(),				nm_usuario_p,				nr_seq_pres_destino_p,
			cd_medico,				ie_situacao,				dt_inclusao,
			dt_exclusao,				ie_tipo_endereco,			ie_guia_medico,
			ie_tipo_complemento,			ie_tipo_vinculo,			nr_seq_tipo_prestador,
...
if (nm_tabela_p = 'PLS_PRESTADOR_PAGTO') then
	if (ie_excluir_p = 'S') then
		...
		if (qt_registro_w > 0) then
			CALL Wheb_mensagem_pck.exibir_mensagem_abort(264257);
		end if;
	end if;
...
if (nm_tabela_p = 'PLS_PRESTADOR_GRUPO_SERV') then
	if (ie_excluir_p = 'S') then
		delete	FROM pls_prestador_grupo_serv
		where 	nr_seq_prestador = nr_seq_pres_destino_p;
	end if;
	
	insert into pls_prestador_grupo_serv(nr_sequencia,				dt_atualizacao,				nm_usuario,
		dt_atualizacao_nrec,			nm_usuario_nrec,			nr_seq_prestador,
		cd_grupo_servico,			nr_seq_prest_end)
	SELECT	nextval('pls_prestador_grupo_serv_seq'),	clock_timestamp(),				nm_usuario_p,
		clock_timestamp(),				nm_usuario_p,				nr_seq_pres_destino_p,
...
if (nm_tabela_p = 'PLS_PRESTADOR_REDE_REF') then
	if (ie_excluir_p = 'S') then
		delete	FROM pls_prestador_rede_ref
		where 	nr_seq_prestador = nr_seq_pres_destino_p;
	end if;
	
	insert into pls_prestador_rede_ref(nr_sequencia,				dt_atualizacao,				nm_usuario,
		dt_atualizacao_nrec,			nm_usuario_nrec,			nr_seq_prestador,
		cd_rede)
	SELECT	nextval('pls_prestador_rede_ref_seq'),	clock_timestamp(),				nm_usuario_p,
		clock_timestamp(),				nm_usuario_p,				nr_seq_pres_destino_p,
		cd_rede
...
if (nm_tabela_p = 'PLS_PRESTADOR_TIPO') then
	if (ie_excluir_p = 'S') then
		delete	FROM pls_prestador_tipo
		where 	nr_seq_prestador = nr_seq_pres_destino_p;
	end if;
	
	insert into pls_prestador_tipo(nr_sequencia,				dt_atualizacao,				nm_usuario,
		dt_atualizacao_nrec,			nm_usuario_nrec,			nr_seq_prestador,
		dt_inicio_vigencia,			dt_fim_vigencia,			nr_seq_tipo,
		ie_ptu)
	SELECT	nextval('pls_prestador_tipo_seq'),		clock_timestamp(),				nm_usuario_p,
		clock_timestamp(),				nm_usuario_p,				nr_seq_pres_destino_p,
		dt_inicio_vigencia,			dt_fim_vigencia,			nr_seq_tipo,
		ie_ptu
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_copia_parametro_prestador ( nr_seq_pres_origem_p bigint, nr_seq_pres_destino_p bigint, nm_tabela_p text, ie_excluir_p text, nm_usuario_p text) FROM PUBLIC;
