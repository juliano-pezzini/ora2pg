...
CREATE OR REPLACE PROCEDURE gerar_etapa_titulo ( nr_titulo_p bigint, ie_evento_p text, nm_usuario_p text, ie_commit_p text) AS $body$
DECLARE
...
C02 CURSOR FOR
	...
	from	conta_paciente
	where	((nr_interno_conta_w <> 0 AND nr_interno_conta = nr_interno_conta_w) or
		 (nr_seq_protocolo_w <> 0 AND nr_seq_protocolo = nr_seq_protocolo_w))
...
if (coalesce(qt_existe_w,0) > 0) then
	...
	ie_evento_w := coalesce(ie_evento_p,'A');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_etapa_titulo ( nr_titulo_p bigint, ie_evento_p text, nm_usuario_p text, ie_commit_p text) FROM PUBLIC;
