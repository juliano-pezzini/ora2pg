...
CREATE OR REPLACE PROCEDURE gerar_dmed_reembolso_depend ( nr_seq_dependente_p bigint, cd_pessoa_fisica_p text, nr_seq_titular_p bigint, nr_seq_dmed_p text, nm_usuario_p text, cd_estabelecimento_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	and	 b.vl_total > 0
	and	 obter_se_reembolso_pago(d.nr_sequencia, dt_ano_calendario_w) = 'S'
	...
	group by cd_pessoa_fisica_p, b.cd_pessoa_fisica, a.nr_seq_titular, obter_cpf_pessoa_fisica(b.cd_pessoa_fisica), b.cd_cgc
	order by obter_cpf_pessoa_fisica(b.cd_pessoa_fisica), b.cd_cgc;
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	Insert into dmed_ops_reembolso(
					...
					nr_seq_dependente)
				values (
					nextval('dmed_ops_reembolso_seq'),
					clock_timestamp(),
					nm_usuario_p,
					clock_timestamp(),
					nm_usuario_p,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_dmed_reembolso_depend ( nr_seq_dependente_p bigint, cd_pessoa_fisica_p text, nr_seq_titular_p bigint, nr_seq_dmed_p text, nm_usuario_p text, cd_estabelecimento_p text) FROM PUBLIC;
