...
CREATE OR REPLACE PROCEDURE pls_cta_val_aplicar_fil_prod ( dados_regra_p pls_tipos_cta_val_pck.dados_regra, dados_filtro_p pls_tipos_cta_val_pck.dados_filtro, dados_consistencia_p pls_tipos_cta_val_pck.dados_consistencia, nr_id_transacao_p pls_rp_cta_selecao.nr_id_transacao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE


dados_filtro_prod_w	pls_tipos_cta_val_pck.dados_filtro_prod;
...
if (dados_regra_p.nr_seq_regra IS NOT NULL AND dados_regra_p.nr_seq_regra::text <> '') then
	...
	for	r_C_filtro_w in C_filtro(dados_filtro_p.nr_sequencia) loop
		...
		dados_restricao_w.ds_restricao_conta := dados_restricao_w.ds_restricao_conta ||
							pls_cta_val_obter_restr_prod(	'RESTRICAO',
											var_cur_w,
											dados_filtro_prod_w);
		...
		begin
			...
			ret_null_w := pls_cta_val_obter_restr_prod(	'BIND',
									var_cur_w,
									dados_filtro_prod_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_cta_val_aplicar_fil_prod ( dados_regra_p pls_tipos_cta_val_pck.dados_regra, dados_filtro_p pls_tipos_cta_val_pck.dados_filtro, dados_consistencia_p pls_tipos_cta_val_pck.dados_consistencia, nr_id_transacao_p pls_rp_cta_selecao.nr_id_transacao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
