...
CREATE OR REPLACE PROCEDURE ptu_gerar_a560_80 ( nr_seq_nota_deb_p bigint, nm_usuario_p text) AS $body$
DECLARE


/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: Gerar o arquivo A560 V8.0
-------------------------------------------------------------------------------------------------------------------
...
Pontos de atenção:
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ds_arquivo_w			text;
ds_hash_w			ptu_nota_debito.ds_hash%type;
...
if (nr_seq_nota_deb_p IS NOT NULL AND nr_seq_nota_deb_p::text <> '') then
	...
	select	lpad(a.cd_unimed_origem,4,'0'),
		...
		rpad(coalesce(a.nr_fatura,' '),20,' '), -- OS 785572 wcbernardino -- rpad(a.nr_fatura,20,' '), --  jtonon - OS 720552 - 24/06/2014
		...
		rpad(coalesce(a.nr_nota_credito_debito_a500,' '),20,' '), -- OS 785572 wcbernardino -- rpad(nvl(a.nr_nota_credito_debito_a500,'0'),20,' '), --  jtonon - OS 720552 - 24/06/2014
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		if (qt_registro_w > 0) then
			...
			ds_conteudo_w 	:=	lpad(qt_linha_w,8,'0') ||
						...
						rpad(coalesce(ds_uso_banco_w,' '),15,' ') ||
						rpad(coalesce(cd_carteira_w,' '),10,' ') ||
			...
			ds_conteudo_w 	:= 	lpad(qt_linha_w,8,'0') ||
						'564' ||
						rpad(coalesce(ds_instrucao_w,' '),60,' ');
			...
			ds_conteudo_w 	:=	lpad(qt_linha_w,8,'0') ||
						'565' ||
						rpad(coalesce(ds_observacao_w,' '),60,' ');
	...
	ds_conteudo_w 	:= 	lpad(qt_linha_w,8,'0') ||
				...
				rpad(coalesce(nr_fatura_orig_w,' '),20,' ') ||
				rpad(coalesce(nr_ndr_w,' '),20,' ');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_a560_80 ( nr_seq_nota_deb_p bigint, nm_usuario_p text) FROM PUBLIC;
