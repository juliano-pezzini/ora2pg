...
CREATE OR REPLACE PROCEDURE baca_acertar_acento_pf () AS $body$
DECLARE
...
into STRICT	cont_w
from	pessoa_fisica
where	coalesce(nm_pessoa_fisica_sem_acento::text, '') = '';

while	cont_w > 0 loop

	update	pessoa_fisica a
	set	nm_pessoa_fisica_sem_acento	= elimina_acentuacao(nm_pessoa_fisica)
	where	coalesce(nm_pessoa_fisica_sem_acento::text, '') = ''  LIMIT 2000;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE baca_acertar_acento_pf () FROM PUBLIC;
