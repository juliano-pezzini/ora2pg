...
CREATE OR REPLACE PROCEDURE gerar_dic_obj_idioma_wcp (cd_funcao_p bigint, nr_seq_idioma_p bigint, nm_usuario_p text) AS $body$
DECLARE


    nr_seq_objeto_wcpanel_w bigint;
    ...
    c02 CURSOR FOR
        SELECT a.nr_seq_obj_sup,
         ...
         where a.cd_funcao = cd_funcao_p
           ...
           and (a.nm_campo_tela IS NOT NULL AND a.nm_campo_tela::text <> '')
           and not exists (SELECT 1
                  from dic_objeto_idioma x
                 where x.nr_seq_objeto = a.nr_sequencia
                   and x.nm_atributo = 'NM_CAMPO_TELA'
                   and x.nr_seq_idioma = nr_seq_idioma_p)
         order by a.nr_seq_obj_sup,
                  a.nr_sequencia;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_dic_obj_idioma_wcp (cd_funcao_p bigint, nr_seq_idioma_p bigint, nm_usuario_p text) FROM PUBLIC;
