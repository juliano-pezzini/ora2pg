...
CREATE OR REPLACE PROCEDURE hem_gerar_calculos_area_sganz ( nr_seq_hem_calc_p bigint, nm_usuario_p text, ds_mensagem_p INOUT text) AS $body$
DECLARE


C01 CURSOR FOR
	SELECT	qt_pa_diastolica_fim,
		qt_pam,
		ie_tipo
...
qt_pam_AE_w		hem_manometria_completa.qt_pam%type;
...
qt_pam_VD_w		hem_manometria_completa.qt_pam%type;
qt_pam_VE_w		hem_manometria_completa.qt_pam%type;
...
if (coalesce(nr_seq_hem_calc_p, 0) > 0) then
	...
	begin
		select	max(qt_fluxo_sist)
		into STRICT	qt_fluxo_sist_w		--Fluxo Sist (l/min)
	...
	if (coalesce(qt_fluxo_sist_w::text, '') = '') then
		select	max(qt_fluxo_pulmonar)
		into STRICT	qt_fluxo_sist_w
		from	hem_calc_hem_result
		where	nr_seq_calc_hemo = nr_seq_hem_calc_p;
	end if;
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		if (ie_tipo_w = 'AE') then
			qt_pam_AE_w		:= qt_pam_w;
		end if;
		if (ie_tipo_w = 'AD') then
			qt_pam_AD_w		:= qt_pam_w;
		end if;
		if (ie_tipo_w = 'VD') then
			qt_pam_VD_w		:= qt_pam_w;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE hem_gerar_calculos_area_sganz ( nr_seq_hem_calc_p bigint, nm_usuario_p text, ds_mensagem_p INOUT text) FROM PUBLIC;
