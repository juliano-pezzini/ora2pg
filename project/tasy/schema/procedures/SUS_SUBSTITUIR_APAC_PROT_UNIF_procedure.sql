...
CREATE OR REPLACE PROCEDURE sus_substituir_apac_prot_unif (nr_interno_conta_p bigint , nr_apac_nova_p bigint , ie_consiste_apac_p text , nm_usuario_p text ) AS $body$
DECLARE
...
select 	consistir_digito('AIH',to_char(nr_apac_nova_w))
into STRICT	ie_digito_apac_w
;
if (ie_consiste_apac_p	= 'S') and (ie_digito_apac_w	= 'N') then
...
if (coalesce(nr_seq_apac_nova_w::text, '') = '') then
	...
	insert into sus_apac_unif(
		...
		qt_diametro_arteria,ie_fremito_traj_fav,ie_pulso_fremito,  cd_medic_antineo_1,cd_medic_antineo_2,cd_medic_antineo_3,cd_medic_antineo_4,cd_medic_antineo_5,
...
exception
	when others then
		...
		/*r.aise_application_error(-20011,'Ocorreu um problema ao excluir APAC antiga ' ||
			chr(10) || ds_erro_w);*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE sus_substituir_apac_prot_unif (nr_interno_conta_p bigint , nr_apac_nova_p bigint , ie_consiste_apac_p text , nm_usuario_p text ) FROM PUBLIC;
