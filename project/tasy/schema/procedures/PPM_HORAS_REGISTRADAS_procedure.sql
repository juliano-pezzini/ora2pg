...
CREATE OR REPLACE PROCEDURE ppm_horas_registradas (nr_seq_metrica_p bigint, nr_seq_objetivo_metrica_p bigint, cd_pessoa_fisica_p text, nm_usuario_p text, dt_referencia_p timestamp) AS $body$
DECLARE
...
if (nr_seq_grupo_w IS NOT NULL AND nr_seq_grupo_w::text <> '') then
	...
	select	KPI_OBTER_HORAS_REGIST(null, nr_seq_grupo_w, null, dt_referencia_p)
	into STRICT	resultado_w
	;

elsif (nr_seq_gerencia_w IS NOT NULL AND nr_seq_gerencia_w::text <> '') then

	select	KPI_OBTER_HORAS_REGIST(nr_seq_gerencia_w, null, null, dt_referencia_p)
	into STRICT	resultado_w
	;

elsif (nr_seq_diretoria_w IS NOT NULL AND nr_seq_diretoria_w::text <> '') then

	select	avg(KPI_OBTER_HORAS_REGIST(nr_sequencia, null, null, dt_referencia_p))
	into STRICT	resultado_w
	...
	select	KPI_OBTER_HORAS_REGIST(null, null, cd_pessoa_fisica_p, dt_referencia_p)
	into STRICT	resultado_w
	;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ppm_horas_registradas (nr_seq_metrica_p bigint, nr_seq_objetivo_metrica_p bigint, cd_pessoa_fisica_p text, nm_usuario_p text, dt_referencia_p timestamp) FROM PUBLIC;
