...
CREATE OR REPLACE PROCEDURE pls_gerar_lote_tx_atend ( nr_seq_lote_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) AS $body$
DECLARE


ie_forma_pagamento_w		pls_parametro_pagamento.ie_forma_pagamento%type;
nr_seq_regra_w			pls_regra_lib_tx_atend.nr_sequencia%type;
dt_competencia_w		pls_lote_taxa_atend.dt_competencia%type;
ie_periodo_w			pls_regra_tx_atend_item.ie_periodo%type;
...
qt_item_w			bigint;
vl_taxa_atendimento_w		pls_conta_val_atend.vl_liberado%type;
...
C01 CURSOR FOR
	SELECT	'P' ie_proc_mat,
		a.nr_sequencia nr_seq_tx_atend,
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	from	pls_lib_taxa_atend
	where	nr_seq_tx_atend	= nr_seq_tx_atend_w;
	...
	if (qt_registro_w = 0) then
		...
		insert	into	pls_lib_taxa_atend(	nr_sequencia, nr_seq_lote, nr_seq_conta,
				nr_seq_segurado, nr_seq_tx_atend, dt_atualizacao,
				nm_usuario, dt_atualizacao_nrec, nm_usuario_nrec,
				cd_procedimento, ie_origem_proced, nr_seq_material,
				nr_seq_analise, nr_seq_prestador_exec, nr_seq_prestador_atend,
				ie_proc_mat,
				qt_item, vl_taxa_atendimento, vl_liberado,
				ds_item, dt_mes_competencia, dt_atendimento_referencia,
				nr_contrato, nr_seq_contrato, nr_seq_intercambio,
				nr_seq_pagador)
			values (	nextval('pls_lib_taxa_atend_seq'), nr_seq_lote_p, nr_seq_conta_w,
				nr_seq_segurado_w, nr_seq_tx_atend_w, clock_timestamp(),
				...
				ie_proc_mat_w,
				qt_item_w, vl_taxa_atendimento_w, vl_liberado_w,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_lote_tx_atend ( nr_seq_lote_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
