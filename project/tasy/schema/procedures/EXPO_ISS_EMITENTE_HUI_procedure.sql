...
CREATE OR REPLACE PROCEDURE expo_iss_emitente_hui (dt_inicio_p timestamp, dt_fim_p timestamp, cd_estabelecimento_p text, nm_usuario_p text) AS $body$
DECLARE
...
SELECT	substr(n.nr_sequencia,1,10), 
	...
	CASE WHEN coalesce(cd_cgc::text, '') = '' THEN 1  ELSE 2 END  nr_tomador, 
	CASE WHEN coalesce(cd_cgc::text, '') = '' THEN (CASE WHEN upper(elimina_caracteres_especiais(obter_dados_pf_pj(n.cd_pessoa_fisica,null,'CI')))=upper(elimina_caracteres_especiais(obter_dados_pf_pj(null,n.cd_cgc_emitente,'CI'))) THEN 'S'  ELSE 'N' END )  ELSE (CASE WHEN upper(elimina_caracteres_especiais(obter_dados_pf_pj(null,n.cd_cgc,'CI')))=upper(elimina_caracteres_especiais(obter_dados_pf_pj(null,n.cd_cgc_emitente,'CI'))) THEN 'S'  ELSE 'N' END ) END , 
	rpad(coalesce(substr(obter_dados_pf_pj(null,n.cd_cgc_emitente, 'N'),1,100),' '),100,' '), 
	lpad(coalesce(obter_dados_pf_pj(null,n.cd_cgc_emitente,'IM'),'0'),10,'0'), 
	lpad(' ',2,' '), 
	lpad(coalesce(n.cd_cgc_emitente,' '),14,' '), 
	CASE WHEN obter_dados_pf_pj(null,n.cd_cgc_emitente,'IE')='' THEN 'S'  ELSE 'N' END , 
	lpad(coalesce(obter_dados_pf_pj(null,n.cd_cgc_emitente,'IE'),' '),15,' '), 
	lpad(coalesce(substr(elimina_caracteres_especiais(obter_dados_pf_pj(null,n.cd_cgc_emitente,'CEP')),1,8),' '),8,' '), 
	rpad(coalesce(substr(obter_dados_pf_pj(null,n.cd_cgc_emitente,'LO'),1,5),' '),5,' '), 
	lpad(' ',5,' '), 
	rpad(coalesce(substr(obter_dados_pf_pj(null,n.cd_cgc_emitente,'R'),1,50),' '),50,' '), 
	rpad(coalesce(substr(obter_dados_pf_pj(null,n.cd_cgc_emitente,'CO'),1,40),' '),40,' '), 
	lpad(coalesce(substr(obter_dados_pf_pj(null,n.cd_cgc_emitente,'NR'),1,10),' '),10,'0'), 
	rpad(coalesce(substr(obter_dados_pf_pj(null,n.cd_cgc_emitente,'B'),1,50),' '),50,' '), 
	rpad(coalesce(substr(obter_dados_pf_pj(null,n.cd_cgc_emitente,'UF'),1,2),' '),2,' '), 
	rpad(coalesce(substr(obter_dados_pf_pj(null,n.cd_cgc_emitente,'CI'),1,50),' '),50,' '), 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE expo_iss_emitente_hui (dt_inicio_p timestamp, dt_fim_p timestamp, cd_estabelecimento_p text, nm_usuario_p text) FROM PUBLIC;
