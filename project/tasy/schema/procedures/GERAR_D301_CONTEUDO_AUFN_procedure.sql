...
CREATE OR REPLACE PROCEDURE gerar_d301_conteudo_aufn (nr_seq_dataset_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
select 'EAD+'||
	...
	obter_descricao_padrao('C301_16_LOCALIZACAO','IE_LOCALIZACAO',NR_SEQ_301_LOCAL_REF_1)||'+'||
	CD_CID_REFERENCIA_2||CASE WHEN coalesce(NR_SEQ_301_LOCAL_REF_2::text, '') = '' THEN null  ELSE ':' END ||
	obter_descricao_padrao('C301_16_LOCALIZACAO','IE_LOCALIZACAO',NR_SEQ_301_LOCAL_REF_2)||'+'||chr(39)
into STRICT	ds_segmento_w
FROM 	D301_SEGMENTO_EAD
where	nr_seq_dataset	= nr_seq_dataset_p;
exception
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_d301_conteudo_aufn (nr_seq_dataset_p bigint, nm_usuario_p text) FROM PUBLIC;
