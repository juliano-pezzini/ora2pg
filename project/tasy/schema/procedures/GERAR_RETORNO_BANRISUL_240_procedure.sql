...
CREATE OR REPLACE PROCEDURE gerar_retorno_banrisul_240 ( nr_seq_banco_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
c03 CURSOR FOR 
SELECT	somente_numero(substr(ds_string,110,20)) nr_titulo, 
	substr(ds_string,66,8)	ds_dt_liquidacao, 
	somente_numero(substr(ds_string,74,15)) ds_vl_liquidacao, 
	substr(ds_string,231,2)	cd_ocorrencia 
from	w_retorno_banco 
where	substr(ds_string,8,1)	= '3' 
and	substr(ds_string,14,1)	= 'D' 
and	nr_seq_banco_escrit	= nr_seq_banco_escrit_p;
...
if (cd_arquivo_w	= '2')	then 
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		if (cd_retorno_liq_w = cd_ocorrencia_w) and (qt_reg_w	> 0)	then 
			...
			where	a.cd_banco		= 41 
			and	a.cd_erro			= cd_ocorrencia_w;
 
			if (coalesce(ie_rejeitado_w,'O') <> 'R') then 
	...
	--r.aise_application_error(-20011,'O arquivo que está sendo importado não é de retorno, favor verifique!'); 
	CALL wheb_mensagem_pck.exibir_mensagem_abort(267379);
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_retorno_banrisul_240 ( nr_seq_banco_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
