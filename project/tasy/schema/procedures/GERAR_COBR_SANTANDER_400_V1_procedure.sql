...
CREATE OR REPLACE PROCEDURE gerar_cobr_santander_400_v1 (nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p text, nm_usuario_p text) AS $body$
DECLARE
...
cd_agencia_cobrad_w     banco_estabelecimento.cd_agencia_bancaria%type;
...
SELECT	lpad(coalesce(to_char(c.cd_agencia_bancaria),'0'),4,'0') cd_agencia,
	...
	'0000' pr_multa,
	'00' vl_moeda,
	replace(to_char(0, 'fm00000000000.00'),'.',null) vl_tit_unidade,
	...
	lpad(coalesce(to_char(x.cd_agencia_bancaria),'0'),5,'0') cd_agencia_cobrad,
	lpad(coalesce(substr(obter_especie_titulo_banco(x.cd_banco,b.ie_tipo_titulo),1,2),'0'),2,'0')ie_documento,
	'N' ie_aceite,
	lpad(coalesce(to_char(b.dt_emissao,'ddmmyy'),' '),6,' ') dt_emissao,
	'00' ie_primeira_instr,
	'00' ie_segunda_intr,
	lpad(replace(to_char(coalesce(obter_vl_juros_diario_tit(null,b.nr_titulo),0),'fm00000000000.00'),'.',null),13,'0') vl_juros_diario,
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	ds_vl_titulo_w	:= lpad(replace(to_char(vl_titulo_w,'fm00000000000.00'),'.',null),13,'0');
...
ds_conteudo_w	:=	'9' 				||
			...
			lpad(replace(to_char(vl_total_w,'fm000000000.99'),'.',null),13,'0')		||
			ds_zeros_374_w			||
			lpad(nr_seq_reg_arq_w,6,'0');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_cobr_santander_400_v1 (nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p text, nm_usuario_p text) FROM PUBLIC;
