...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_fatura_v91_utl ( nr_seq_fatura_p ptu_fatura.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, ds_local_windows_utl_p INOUT text ) AS $body$
DECLARE
...
C02 CURSOR FOR
	SELECT	lpad(elimina_caractere_especial(a.nr_lote),8,'0') nr_lote_503,
		...
		rpad(coalesce(a.nm_hospital,' '),70,' '),
...
C03 CURSOR FOR
	SELECT	lpad(coalesce(elimina_caractere_especial(a.nr_lote),'0'),8,'0'),
		...
		rpad(coalesce(elimina_caractere_especial(a.nm_prestador),' '),70,' '),
		...
		rpad(coalesce(elimina_caractere_especial(a.nm_profissional_prestador),' '),70,' '),
		...
		rpad(coalesce(a.nm_fornecedor,' '),70,' '),
...
union  all
	...
	PERFORM	rpad(coalesce(a.nr_nota_fiscal,'0'),20,' '),
		a.ds_link_nfe
	from	ptu_nota_fiscal		a,
		ptu_nota_cobranca	b
	where	a.nr_seq_nota_cobr	= b.nr_sequencia
...
C11 CURSOR( nr_seq_ptu_fatura_pc	ptu_fatura.nr_sequencia%type ) FOR
	SELECT	nr_sequencia,
		...
		rpad(coalesce(ptu_somente_caracter_permitido(nm_prestador,'ANS'),' '),70,' ') nm_prestador,
...
c12 CURSOR( nr_seq_nota_cobr_rrs_pc	ptu_nota_cobranca_rrs.nr_sequencia%type ) FOR
	SELECT	nr_sequencia,
		...
		rpad(coalesce(ptu_somente_caracter_permitido(nm_profissional,'ANS'),' '),70,' ') nm_profissional,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_fatura_v91_utl ( nr_seq_fatura_p ptu_fatura.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, ds_local_windows_utl_p INOUT text ) FROM PUBLIC;
