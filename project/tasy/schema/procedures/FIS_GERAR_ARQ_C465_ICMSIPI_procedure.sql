...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_c465_icmsipi ( nr_seq_controle_p bigint, nr_seq_nota_p bigint, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
c_reg_C465 CURSOR FOR
SELECT	*
from	fis_efd_icmsipi_C465
where	nr_seq_controle = nr_seq_controle_p
and 	nr_seq_nota = nr_seq_nota_p;

/*Criação do array com o tipo sendo do cursor eespecificado - c_reg_C465*/

type reg_c_reg_C465 is table of fis_efd_icmsipi_C465%RowType;
vetRegC465 reg_c_reg_C465;
...
open c_reg_C465;
loop
fetch c_reg_C465 bulk collect into vetRegC465 limit 1000;
	for i in 1 .. vetRegC465.Count loop
	begin
	...
	ds_linha_w := substr(ds_sep_w || vetRegC465[i].cd_reg ||
			     ds_sep_w || vetRegC465[i].ds_chv_cfe ||
			     ds_sep_w || vetRegC465[i].nr_ccf || ds_sep_w,
			     1,
			     8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro		:=  vetRegC465[i].cd_reg;

	if (nr_vetor_w >= 1000) then
	begin
	/*Inserindo registros definitivamente na tabela especifica - FIS_EFD_ICMSIPI_C465 */
...
EXIT WHEN NOT FOUND; /* apply on c_reg_C465 */
end loop;
close c_reg_C465;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_c465_icmsipi ( nr_seq_controle_p bigint, nr_seq_nota_p bigint, nr_linha_p INOUT bigint ) FROM PUBLIC;
