...
CREATE OR REPLACE PROCEDURE reordenar_sne (nr_prescricao_p bigint) AS $body$
DECLARE
...
WHERE a.nr_prescricao 		= nr_prescricao_p and a.cd_material			= b.cd_material  --  and	ie_tipo_material		> 1
  and ie_agrupador			= 8 and coalesce(nr_sequencia_dieta::text, '') = '' and coalesce(nr_sequencia_proc::text, '') = '' and coalesce(nr_sequencia_solucao::text, '') = '' and coalesce(nr_sequencia_diluicao::text, '') = '' Order by c.nr_impressao,
	a.ie_via_aplicacao,
	a.nr_agrupamento,
	a.nr_sequencia;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE reordenar_sne (nr_prescricao_p bigint) FROM PUBLIC;
