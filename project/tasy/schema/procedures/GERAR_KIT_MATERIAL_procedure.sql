...
CREATE OR REPLACE PROCEDURE gerar_kit_material (CD_KIT_MATERIAL_P bigint, DT_ATENDIMENTO_P timestamp, NR_PRESCRICAO_P bigint, IE_AUDITORIA_P text, CD_LOCAL_ESTOQUE_P text, NR_SEQ_PROC_PRINC_P text, NR_SEQ_ATEPACU_P bigint, IE_BAIXA_SEM_ESTOQUE_P text, NM_USUARIO_P text, DS_ERRO_P INOUT text) AS $body$
DECLARE
...
VinculaGuiaPrescricao_w		varchar(2);
NR_DOC_CONVENIO_WW		varchar(25);
...
if (obter_funcao_ativa	= 24) and (NR_PRESCRICAO_P IS NOT NULL AND NR_PRESCRICAO_P::text <> '')	then 
	VinculaGuiaPrescricao_w := Obter_Param_Usuario(obter_funcao_ativa, 143, obter_perfil_ativo, NM_USUARIO_P, cd_estabelecimento_W, VinculaGuiaPrescricao_w);
...
LOOP 
	...
	ELSE 
		DS_ERRO_P := WHEB_MENSAGEM_PCK.get_texto(279026) || CD_MATERIAL_W || WHEB_MENSAGEM_PCK.get_texto(279027) || CHR(13);
	END IF;
END LOOP;
CLOSE C001;
END;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_kit_material (CD_KIT_MATERIAL_P bigint, DT_ATENDIMENTO_P timestamp, NR_PRESCRICAO_P bigint, IE_AUDITORIA_P text, CD_LOCAL_ESTOQUE_P text, NR_SEQ_PROC_PRINC_P text, NR_SEQ_ATEPACU_P bigint, IE_BAIXA_SEM_ESTOQUE_P text, NM_USUARIO_P text, DS_ERRO_P INOUT text) FROM PUBLIC;
