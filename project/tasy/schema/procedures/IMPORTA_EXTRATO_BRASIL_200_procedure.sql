...
CREATE OR REPLACE PROCEDURE importa_extrato_brasil_200 ( nr_seq_extrato_p bigint) AS $body$
DECLARE
...
cd_categoria_lanc_w	varchar(3);

/* Saldos inicial e final sem lançamentos */

qt_lancamento_w		bigint;
nr_sequencia_w		bigint;
nr_seq_final_w		bigint;

--Detalhe - 03 Registro de lançamento
c01 CURSOR FOR
	...
	if (nr_conta_ant_w	= '-1') or (nr_conta_ant_w <> nr_conta_w) or (ie_digito_ant_w <> digito_conta_w) then
		...
		if (coalesce(nr_seq_conta_nova_w::text, '') = '') and (digito_conta_w = ' ') then
			...
			where (a.nr_sequencia = nr_seq_conta_w or
				...
				and	x.cd_conta		= a.cd_conta
				and	x.nr_sequencia		= nr_seq_conta_w))
			and	somente_numero(a.ie_digito_conta)	= somente_numero(coalesce(digito_conta_w,a.ie_digito_conta))
			and	somente_numero(a.cd_conta)		= somente_numero(nr_conta_w)
			and	cd_banco				= cd_banco_w;
...
/* Importar os saldos inicial e final das contas que não possuem lançamento */
...
EXIT WHEN NOT FOUND; /* apply on c02 */
	...
	if (qt_lancamento_w	= 0) then
		...
		elsif (nr_seq_conta_nova_w IS NOT NULL AND nr_seq_conta_nova_w::text <> '') and
			((coalesce(vl_saldo_inicial_w,0) <> 0) or (coalesce(vl_saldo_final_w,0) <> 0)) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE importa_extrato_brasil_200 ( nr_seq_extrato_p bigint) FROM PUBLIC;
