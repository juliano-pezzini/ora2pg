...
CREATE OR REPLACE PROCEDURE fis_nfse_envio_automatico (cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C02 CURSOR FOR
	...
	and	(((ie_competencia_w = 'S') and (to_date(n.dt_emissao, 'dd/mm/rrrr') between to_date(mes_atual_w, 'dd/mm/rrrr') and to_date(mes_atual_fin_w,'dd/mm/rrrr'))) or (ie_competencia_w = 'N'))
	...
	order by n.nr_sequencia asc LIMIT 100;
...
BEGIN
	...
	if (qt_regra_w = 0) then
		begin
			/* UTILIZA O ENVIO AUTOMATICO SEM RESTRICAO */

			CALL fis_envio_rps_sequencial(cd_estabelecimento_p, nm_usuario_p);
		end;
	else
		begin
			/* UTILIZA O ENVIO AUTOMATICO COM RESTRICAO */
			...
			begin
				/* VERIFICA SE A RESTRICAO E SO POR HORA ATUAL OU DATA E HORA ATUAIS */

				if 	(((dt_inicio_w IS NOT NULL AND dt_inicio_w::text <> '')
					and (dt_final_w IS NOT NULL AND dt_final_w::text <> '') 
					and (dia_atual_w >= dt_inicio_w)
					and (dia_atual_w <= dt_final_w)
					and ((horario_atual_w)::numeric  >= (hr_inicio_w)::numeric )
					and ((horario_atual_w)::numeric  <= (hr_final_w)::numeric ))
					or  ((coalesce(dt_inicio_w::text, '') = '') 
					and (coalesce(dt_final_w::text, '') = '') 
					and ((horario_atual_w)::numeric  >= (hr_inicio_w)::numeric )
					and ((horario_atual_w)::numeric  <= (hr_final_w)::numeric ))) then
					begin
						if (qt_restr_regra_w = 0) then
							begin
								...
								begin
									/* GERAR O NUMERO DE SEQUENCIA DA TRANSMISSAO */

									nr_seq_transmissao_w := gerar_transmissao_nf(nm_usuario_p, 1, 'NFSE', nr_seq_transmissao_w);
														
									/* INSERI O NUMERO DE SEQUENCIA DA TRANSMISSAO NO LOTE DE TRANSMISSAO */
							...
							begin
								...
								/* RESTRIGE TAMBEM POR SERIE E OPERACAO DA NOTA FISCAL - CURSOR C03*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_nfse_envio_automatico (cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
