...
CREATE OR REPLACE PROCEDURE vipe_gerar_ordem_apres_inf ( nr_seq_grupo_p bigint, ie_acm_sn_final_p text, ie_informacao_p text) AS $body$
DECLARE
...
if (nr_seq_grupo_p IS NOT NULL AND nr_seq_grupo_p::text <> '') then
	begin
	if (ie_informacao_p = 'M') or (ie_informacao_p = 'SOL') then
		begin
		open c01;
		loop
		fetch c01 into
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE vipe_gerar_ordem_apres_inf ( nr_seq_grupo_p bigint, ie_acm_sn_final_p text, ie_informacao_p text) FROM PUBLIC;
