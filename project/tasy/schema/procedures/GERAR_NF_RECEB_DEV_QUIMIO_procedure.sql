...
CREATE OR REPLACE PROCEDURE gerar_nf_receb_dev_quimio ( nr_ordem_compra_p bigint, nr_seq_nf_p bigint, cd_operacao_nf_p bigint, cd_nat_oper_nf_p bigint, cd_local_estoque_p bigint, cd_serie_nf_p text, nr_nota_fiscal_p text, ds_observacao_p text, nm_usuario_p text, cd_estabelecimento_p bigint, cd_setor_atendimento_p bigint, nr_sequencia_p INOUT bigint, ds_erro_p INOUT text, ds_erro_item_p INOUT text, ds_erro_nota_p INOUT text) AS $body$
DECLARE
...
select	substr(obter_cgc_estabelecimento(cd_estabelecimento_w),1,14),
...
values ( nr_sequencia_w,		cd_estab_destino_w,
...
CALL gerar_historico_nota_fiscal(nr_sequencia_w, nm_usuario_p, '17', WHEB_MENSAGEM_PCK.get_texto(281672));
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	select	substr(obter_dados_material_estab(cd_material,cd_cgc_destino_w,'UME'),1,30) cd_unidade_medida_estoque,
	...
	SELECT * FROM define_conta_material(
		cd_cgc_destino_w, cd_material_w, ie_tipo_conta_w, 0, 0, 0, 0, 0, 0, 0, cd_local_estoque_p, cd_operacao_nf_p, trunc(clock_timestamp()), cd_conta_contabil_w, cd_centro_custo_w, null) INTO STRICT cd_conta_contabil_w, cd_centro_custo_w;
...
if (qt_itens_nota_w = 0) then
	...
	nr_sequencia_w	:= 0;
	ds_erro_w	:= WHEB_MENSAGEM_PCK.get_texto(281673);
	end;
end if;
...
if (coalesce(ds_erro_p::text, '') = '') and (coalesce(ds_erro_item_p::text, '') = '') and (coalesce(ds_erro_nota_p::text, '') = '') then
	CALL gravar_historico_transf_quimio(nm_usuario_p,nr_ordem_compra_p,'N',WHEB_MENSAGEM_PCK.get_texto(281674) || nr_sequencia_p);
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_nf_receb_dev_quimio ( nr_ordem_compra_p bigint, nr_seq_nf_p bigint, cd_operacao_nf_p bigint, cd_nat_oper_nf_p bigint, cd_local_estoque_p bigint, cd_serie_nf_p text, nr_nota_fiscal_p text, ds_observacao_p text, nm_usuario_p text, cd_estabelecimento_p bigint, cd_setor_atendimento_p bigint, nr_sequencia_p INOUT bigint, ds_erro_p INOUT text, ds_erro_item_p INOUT text, ds_erro_nota_p INOUT text) FROM PUBLIC;
