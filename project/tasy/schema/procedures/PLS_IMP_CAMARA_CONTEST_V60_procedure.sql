...
CREATE OR REPLACE PROCEDURE pls_imp_camara_contest_v60 ( ds_conteudo_p text, nm_usuario_p text, nr_seq_camara_contest_p bigint, nr_seq_referencia_p INOUT bigint, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
if (substr(ds_conteudo_p,9,3) = '551') then 
	...
	if (coalesce(nr_seq_contestacao_w::text, '') = '') then	 
		...
		insert into ptu_camara_contestacao(nr_sequencia, dt_atualizacao, nm_usuario, 
			...
			ds_serial_processo,ie_tipo_arquivo_cob) 
	...
	if (coalesce(nr_seq_camara_contest_p::text, '') = '') or (nr_seq_camara_contest_p = 0) then 
		...
		values (nextval('ptu_questionamento_codigo_seq'), clock_timestamp(), nm_usuario_p, 
			...
			nr_seq_questionamento_w, cd_motivo_quest_w, ds_motivo_quest_w, 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_imp_camara_contest_v60 ( ds_conteudo_p text, nm_usuario_p text, nr_seq_camara_contest_p bigint, nr_seq_referencia_p INOUT bigint, cd_estabelecimento_p bigint) FROM PUBLIC;
