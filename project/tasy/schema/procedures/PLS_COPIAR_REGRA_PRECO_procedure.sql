...
CREATE OR REPLACE PROCEDURE pls_copiar_regra_preco ( ds_regras_p text, ie_tipo_regra_p text, ie_destino_p text, dt_inicio_p timestamp, nm_usuario_p text, cd_estabelecimento_p text, ie_checado_p text) AS $body$
DECLARE
...
BEGIN
ds_regra_w := ds_regras_p;
dt_inicio_w:= dt_inicio_p;
while(position(',' in ds_regra_w) <> 0) loop
	begin			
	--obtem-se a primeira regra
	nr_seq_regra_w	:= substr(ds_regra_w,1,position(',' in ds_regra_w) - 1);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_copiar_regra_preco ( ds_regras_p text, ie_tipo_regra_p text, ie_destino_p text, dt_inicio_p timestamp, nm_usuario_p text, cd_estabelecimento_p text, ie_checado_p text) FROM PUBLIC;
