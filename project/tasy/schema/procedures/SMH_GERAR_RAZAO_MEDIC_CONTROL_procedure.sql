...
CREATE OR REPLACE PROCEDURE smh_gerar_razao_medic_control ( dt_inicial_p timestamp, dt_final_p timestamp, dt_movto_inic_p timestamp, dt_movto_fim_p timestamp, cd_estabelecimento_p bigint, cd_setor_atendimento_p bigint, ie_gerar_saldo_p text, ie_imprime_local_p text, ie_imprime_endereco_p text, ie_imprime_setor_p text, ie_imprime_lote_p text, ie_imprime_movimento_p text, ie_analitico_p text, ie_desconsidera_transfer_p text, nm_usuario_p text, cd_local_estoque_p text, cd_medicamento_p bigint, ie_imprime_centro_p text, cd_centro_custo_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	group by	PKG_DATE_UTILS.start_of(a.dt_movimento_estoque, 'ddd', 0), a.cd_operacao_estoque, b.ds_operacao, PKG_DATE_UTILS.start_of(a.dt_validade, 'ddd', 0), a.cd_lote_fabricacao
...
union all
	...
	select distinct	
		...
		'Saldo anterior', -- 'Saldo anterior' não precisa ser substituída por uma expressão pois é uma procedure específica de um cliente.
		'',
		'',
		'0',
		0,
...
EXIT WHEN NOT FOUND; /* apply on C00 */
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		if (ie_tipo_w <> 'A') then
			-- 799987 - (#@NR_DOCUMENTO_W#@) #@DS_HISTORICO_W#@
			ds_historico_w	:= substr(wheb_mensagem_pck.get_texto(799987,'NR_DOCUMENTO_W=' || nr_documento_w || ';' ||
								'DS_HISTORICO_W=' || ds_historico_w),1,250);
								
			if (ie_imprime_local_p = 'S') then
				-- 302167 - #@DS_HISTORICO_W#@ Local: #@DS_LOCAL_ESTOQUE_W#@
				...
				-- 302168 - #@DS_HISTORICO_W#@ Lote: #@CD_LOTE_W#@ - Validade: #@DT_VALIDADE_W#@
			...
			if (ie_imprime_centro_p = 'S') and (cd_centro_custo_w IS NOT NULL AND cd_centro_custo_w::text <> '') then
				...
				;
				-- 302170 - #@DS_HISTORICO_W#@ Centro custo: #@DS_CENTRO_CUSTO_W#@
