...
CREATE OR REPLACE PROCEDURE phi_vincular_os_etapa ( nr_seq_proj_cron_etapa_p bigint, nr_seq_ordem_p bigint, nm_usuario_p text, nr_product_requirement_p reg_product_requirement.nr_sequencia%type) AS $body$
DECLARE
...
if (ie_main_os_proj_w = 'S') then
	...
	CALL wheb_mensagem_pck.exibir_mensagem_abort(1150201);
end if;
...
select	coalesce(max(nr_seq_severidade_wheb), 0),
	coalesce(max(nr_seq_severidade), 0)
...
CALL proj_do_impact_analysis(nr_seq_ordem_p, nr_product_requirement_p, nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE phi_vincular_os_etapa ( nr_seq_proj_cron_etapa_p bigint, nr_seq_ordem_p bigint, nm_usuario_p text, nr_product_requirement_p reg_product_requirement.nr_sequencia%type) FROM PUBLIC;
