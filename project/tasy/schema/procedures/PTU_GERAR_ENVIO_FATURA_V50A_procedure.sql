...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_fatura_v50a ( nr_seq_fatura_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C00 CURSOR FOR 	-- 501 
	SELECT	lpad(coalesce(cd_unimed_destino,'0'),4,'0'), 
		...
		lpad(coalesce(nr_nota_credito_debito,'0'),11,'0'), 
...
C03 CURSOR FOR 	-- 504 
	SELECT	lpad(coalesce(elimina_caractere_especial(a.nr_lote),'0'),8,'0'), 
		...
		rpad(to_char(coalesce(ptu_somente_caracter_permitido(elimina_caractere_especial(elimina_acentuacao(ds_servico)),'ANS'),' ')),80,' '),		 
...
C04 CURSOR FOR 	-- 505 
	SELECT	lpad(coalesce(elimina_caractere_especial(a.nr_lote),'0'),8,'0'), 
		...
		rpad(coalesce(replace(replace(elimina_caractere_especial(elimina_acentuacao(a.ds_complemento)),chr(10),' '),chr(13),' '),' '),100,' '), 
		rpad(coalesce(replace(replace(elimina_caractere_especial(elimina_acentuacao(a.especif_material)),chr(10),' '),chr(13),' '),' '),500,' ') 
...
C05 CURSOR FOR 	-- 506 
	SELECT	rpad(coalesce(a.nr_nota_fiscal,'0'),20,' '), 
		a.ds_link_nfe 
...
C06 CURSOR FOR 	-- 507 
	SELECT	coalesce(ie_tipo,' '), 
		max(rpad(coalesce(cd_ans,'0'),10,' ')), 
		max(rpad(coalesce(elimina_acentuacao(coalesce(nm_cedente_sacado,' ')),' '),60,' ')), 
		max(rpad(coalesce(ptu_somente_caracter_permitido(ds_endereco,'ANS'),' '),40,' ')), 
		max(rpad(coalesce(ptu_somente_caracter_permitido(ds_complemento,'ANS'),' '),20,' ')), 
		max(rpad(coalesce(ptu_somente_caracter_permitido(ds_bairro,'ANS'),' '),30,' ')), 
		max(rpad(coalesce(cd_cep,' '),8,' ')), 
		max(rpad(coalesce(nm_cidade,' '),30,' ')), 
		max(rpad(coalesce(sg_uf,' '),2,' ')), 
		max(lpad(coalesce(cd_cgc_cpf,'0'),14,'0')), 
		max(lpad(coalesce(nr_inscricao_estadual,'0'),20,'0')), 
		max(lpad(coalesce(nr_ddd,'0'),4,'0')), 
		max(lpad(coalesce(nr_telefone,'0'),9,'0')), 
		max(lpad(coalesce(nr_fax,'0'),9,'0')), 
		max(rpad(coalesce(nr_end,'S/N'),6,' ')) 
...
C07 CURSOR FOR 	-- 508 
	SELECT	lpad(nr_linha,2,'0'), 
		...
		lpad(coalesce(replace(replace(campo_mascara(vl_item,2),',',''),'.',''),'0'),14,'0'), 
		coalesce(ie_documento,'2') 
	from	ptu_fatura_corpo 
	where	nr_seq_fatura		= nr_seq_ptu_fatura_w 
	order by nr_linha;
	
C08 CURSOR FOR 	-- 509 
	SELECT	rpad(coalesce(cd_banc,' '),5,' '), 
		rpad(coalesce(cd_agencia_cedente,' '),25,' '), 
		rpad(coalesce(cd_nosso_numero,' '),20,' '), 
		rpad(coalesce(ds_obs_banco,' '),15,' '), 
		rpad(coalesce(ds_carteira,' '),10,' '), 
		rpad(coalesce(ds_especie,' '),4,' '), 
		lpad(coalesce(replace(replace(campo_mascara(vl_documento,2),',',''),'.',''),'0'),14,'0'), 
		rpad(coalesce(to_char(dt_documento,'yyyymmdd'),' '),8,' '), 
		rpad(coalesce(cd_especie_doc,' '),5,' '), 
		rpad(coalesce(ie_aceite,' '),2,' '), 
		rpad(coalesce(to_char(dt_processamento,'yyyymmdd'),' '),8,' '), 
		rpad(coalesce(ds_local_pagamento,' '),60,' '), 
		rpad(coalesce(ds_obs_local_pagto,' '),60,' '), 
		coalesce(tp_boleto,'0') 
	from	ptu_fatura_boleto 
	where	nr_seq_fatura		= nr_seq_ptu_fatura_w;

C09 CURSOR FOR 	-- 510 
	SELECT	rpad(coalesce(ds_instrucao,' '),60,' '), 
		rpad(coalesce(ds_observacao,' '),60,' '), 
		rpad(coalesce(ds_linha_digitavel,' '),60,' '), 
		rpad(coalesce(cd_barras,' '),44,' ') 
	from	ptu_fatura_boleto 
	where	nr_seq_fatura		= nr_seq_ptu_fatura_w;
...
if (nr_seq_fatura_p IS NOT NULL AND nr_seq_fatura_p::text <> '') then 
	...
	EXIT WHEN NOT FOUND; /* apply on C00 */
		...
		EXIT WHEN NOT FOUND; /* apply on C01 */
			...
			ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '502' || nr_lote_w || lpad(' ',12,' ') || cd_unimed_destino_w || '  ' || 
					cd_usuario_plano_w || nm_beneficiario_w || dt_atendimento_w || cd_excecao_w || ie_carater_atendimento_w || ' ' || 
					cd_cid_w || lpad(' ',14,' ') || ie_paciente_w || ie_tipo_saida_spdat_w || ie_tipo_atendimento_w || lpad(' ',11,' ') || 
					nr_nota_w || nr_guia_principal_w || dt_internacao_502_w || dt_alta_502_w || dt_ultima_autoriz_w || tp_nota_w || 
					id_nota_principal_w || nr_ver_tiss_w || nr_guia_tiss_prestador_w || nr_guia_tiss_principal_w || nr_guia_tiss_operadora_w || 
					tp_ind_acidente_w || motivo_encerram_w || nr_cnpj_cpf_req_w || nm_prest_req_w || sg_cons_prof_req_502_w || 
					nr_cons_prof_req_502_w || sg_uf_cons_req_502_w || nr_cbo_req_w || nr_fatura_glosada_w || nr_ndr_glosada_w || 
					nr_lote_glosado_w || nr_nota_glosada_w || dt_protocolo_w || id_rn_w;
			...
			EXIT WHEN NOT FOUND; /* apply on C02 */
				...
				ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '503' || nr_lote_503_w || lpad(' ',11,' ') || 
						...
						ie_int_parto_cesarea_w || ie_int_parto_normal_w || ie_faturamento_w || cd_cid_obito_0_w || '    ' || 
						rpad(coalesce(nr_declara_vivo_1_w,' '),15,' ') || rpad(coalesce(nr_declara_vivo_2_w,' '),15,' ') || 
						rpad(coalesce(nr_declara_vivo_3_w,' '),15,' ') || rpad(coalesce(nr_declara_vivo_4_w,' '),15,' ') || 
						rpad(coalesce(nr_declara_vivo_5_w,' '),15,' ') || ie_obito_mulher_w || nr_declara_obito_w || 
						rpad(coalesce(cd_cid_obito_1_w,' '),6,' ') || rpad(coalesce(cd_cid_obito_2_w,' '),6,' ') || rpad(coalesce(cd_cid_obito_3_w,' '),6,' ') || 
						rpad(coalesce(cd_cid_obito_4_w,' '),6,' ') || rpad(coalesce(cd_cid_obito_5_w,' '),6,' ') || 
						rpad(coalesce(nr_declara_obito_1_w,' '),17,' ') || rpad(coalesce(nr_declara_obito_2_w,' '),17,' ') || 
						rpad(coalesce(nr_declara_obito_3_w,' '),17,' ') || rpad(coalesce(nr_declara_obito_4_w,' '),17,' ') || 
						rpad(coalesce(nr_declara_obito_5_w,' '),17,' ') || nr_nota_503_w || reg_internacao_w;
			...
			EXIT WHEN NOT FOUND; /* apply on C03 */
				...
				ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '504' || nr_lote_504_w || lpad(' ',11,' ') || 
						...
						sg_uf_cons_prest_w || lpad(' ',85,' ') || ie_reembolso_w || nr_autorizacao_w || vl_pago_prest_w || 
						cd_pacote_w || lpad(' ',20,' ') || tp_autoriz_w || ds_servico_w || ie_prestador_alto_custo_w || nr_nota_504_w ||					 
						hr_final_w || id_acres_urg_emer_w || nr_cbo_exec_w || tec_utilizada_w || dt_autoriz_w || dt_solicitacao_w || 
						unidade_medida_w || nr_reg_anvisa_w || cd_munic_w || cd_ref_material_fab_w || dt_pgto_prestador_w;
			...
			EXIT WHEN NOT FOUND; /* apply on C04 */
				...
				ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '505' || nr_lote_505_w || lpad(' ',11,' ') || 
						ie_tipo_complemento_w || ds_complemento_w || nr_nota_505_w;-- || especif_material_w; 
				insert into w_ptu_envio_arq(nr_sequencia, 
					dt_atualizacao, 
					nm_usuario, 
					ds_conteudo, 
		...
		EXIT WHEN NOT FOUND; /* apply on C06 */
			...
			ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '510' || ie_tipo_w || cd_ans_w || nm_cedente_sacado_w || 
					...
					nr_inscricao_estadual_w || nr_ddd_w || lpad(' ',16,' ') || nr_telefone_w || nr_fax_w || nr_end_w;
		...
		EXIT WHEN NOT FOUND; /* apply on C08 */
			...
			ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '512' || cd_banc_w || cd_agencia_cedente_w || cd_nosso_numero_w || 
					...
					ie_aceite_w || dt_processamento_w || ds_local_pagamento_w || ds_obs_local_pagto_w || tp_boleto_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_fatura_v50a ( nr_seq_fatura_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
