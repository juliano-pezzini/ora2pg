...
CREATE OR REPLACE PROCEDURE pls_atualiza_valor_mat ( nr_seq_material_p pls_conta_mat.nr_sequencia%type, ie_gravar_log_p text, nm_usuario_p usuario.nm_usuario%type, ie_conta_inteira_p text, nr_seq_conta_p pls_conta.nr_sequencia%type, ie_recalcular_conta_p text default 'N') AS $body$
DECLARE
...
nr_seq_regra_cobr_prev_w	pls_conta_mat.nr_seq_regra_cobr_prev%type;
vl_mat_cobranca_prev_w		pls_conta_mat.vl_mat_cobranca_prev%type;
...
for k in 1.. i loop
 ...
 vl_mat_copartic_w := _ora2pg_r.vl_material_p; ds_retorno_ww := _ora2pg_r.dt_ult_vigencia_p; ds_retorno_ww := _ora2pg_r.nr_seq_material_preco_p; ds_retorno_ww := _ora2pg_r.vl_material_simpro_p; ds_retorno_ww := _ora2pg_r.vl_material_brasindice_p; ds_retorno_ww := _ora2pg_r.vl_material_tabela_p; dados_regra_preco_material_w := _ora2pg_r.dados_regra_preco_material_p;
		...
		if (ie_cobranca_prevista_w = 'S' or ie_cobranca_prevista_ww = 'S' or ie_cobranca_prevista_inf_w = 'S') and (qt_regra_cobr_prev_w > 0) then
			
			SELECT * FROM pls_define_preco_material(	cd_estabelecimento_w, null, dt_preco_w, nr_seq_material_w, 4, ie_tipo_despesa_w, ie_origem_preco_w, 'CP', nr_seq_outorgante_w, nr_seq_segurado_w, nr_seq_intercambio_w, nr_seq_fornecedor_w, nr_seq_categoria_w, nr_seq_tipo_acomodacao_w, nr_seq_tipo_atendimento_w, nr_seq_clinica_w, ie_tipo_guia_w, ie_autorizacao_espec_w, nr_seq_plano_espec_w, ie_gravar_log_w, nm_usuario_p, nr_seq_conta_mat_w, '', ie_tipo_segurado_w, ie_tipo_vinculo_w, nr_seq_tipo_prestador_w, ie_preco_plano_w, nr_seq_estrut_mat_w, nr_seq_tipo_uso_w, nr_seq_conta_w, nr_seq_plano_w, nr_seq_contrato_w, nr_seq_congenere_w, ie_internado_w, nr_seq_classificacao_prest_w, sg_estado_outorg_w, sg_estado_int_w, ie_tipo_contratacao_w, vl_mat_cobranca_prev_w, ds_retorno_ww, ds_retorno_ww, ds_retorno_ww, ds_retorno_ww, ds_retorno_ww, dados_regra_preco_material_w, null, ie_regime_atendimento_w, ie_saude_ocupacional_w) INTO STRICT _ora2pg_r;
 vl_mat_cobranca_prev_w := _ora2pg_r.vl_material_p; ds_retorno_ww := _ora2pg_r.dt_ult_vigencia_p; ds_retorno_ww := _ora2pg_r.nr_seq_material_preco_p; ds_retorno_ww := _ora2pg_r.vl_material_simpro_p; ds_retorno_ww := _ora2pg_r.vl_material_brasindice_p; ds_retorno_ww := _ora2pg_r.vl_material_tabela_p; dados_regra_preco_material_w := _ora2pg_r.dados_regra_preco_material_p;
														
							nr_seq_regra_cobr_prev_w := dados_regra_preco_material_w.nr_sequencia;
		end if;
		...
		if ( qt_regra_tx_w > 0 ) then
			SELECT * FROM pls_obter_taxa_adm_item( null, null, nr_seq_material_w, nr_seq_prestador_exec_w, nm_usuario_p, ie_preco_plano_w, ie_tipo_segurado_w, nr_seq_contrato_w, dt_material_w, tx_adm_w, vl_fixo_tx_adm_w, nr_seq_regra_tx_adm_w) INTO STRICT tx_adm_w, vl_fixo_tx_adm_w, nr_seq_regra_tx_adm_w;
			...
			if ( tx_adm_w > 0 ) then
				...
				if (ie_gravar_log_w = 'S') then
					...
					ds_observacao_log_w :=	substr((ds_observacao_log_w ||
								'tx_adm_w=' || to_char(tx_adm_w) ||
								';nr_seq_regra_tx_adm=' || to_char(nr_seq_regra_tx_adm_w)),1,4000);
				end if;
				...
				if (ie_gravar_log_w = 'S') then
					...
					ds_observacao_log_w :=	substr((ds_observacao_log_w ||
								'vl_fixo_tx_adm_w=' || to_char(vl_fixo_tx_adm_w) ||
								';nr_seq_regra_tx_adm=' || to_char(nr_seq_regra_tx_adm_w)),1,4000);
				end if;
		...
		if (coalesce(ie_nao_gera_tx_inter_w,'N') = 'N') then
			if (ie_tipo_conta_w = 'I') then
				...
				if (ie_gravar_log_w = 'S') then				
					CALL pls_gravar_log_calculo_mat(	ie_gravar_log_p, nr_seq_conta_mat_w, cd_estabelecimento_w,
									'Tx. intercambio', ds_observacao_log_w, 'pls_atualiza_valor_mat',
									vl_calculado_w, 4, nr_seq_regra_w, 
									ie_tipo_conta_w, nr_seq_material_w, nm_usuario_p);
				end if;
			end if;
		end  if;
		...
		dados_conta_mat_w.nr_seq_regra(nr_update_index_w)		:= nr_seq_regra_w;
		dados_conta_mat_w.ie_status(nr_update_index_w)			:= ie_status_w;
		dados_conta_mat_w.nr_seq_regra_pos_estab(nr_update_index_w)	:= nr_seq_regra_pos_estab_w;
		...
		dados_conta_mat_w.vl_mat_cobranca_prev(nr_update_index_w)	:= vl_mat_cobranca_prev_w;
		dados_conta_mat_w.nr_seq_regra_cobr_prev(nr_update_index_w)	:= nr_seq_regra_cobr_prev_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_atualiza_valor_mat ( nr_seq_material_p pls_conta_mat.nr_sequencia%type, ie_gravar_log_p text, nm_usuario_p usuario.nm_usuario%type, ie_conta_inteira_p text, nr_seq_conta_p pls_conta.nr_sequencia%type, ie_recalcular_conta_p text default 'N') FROM PUBLIC;
