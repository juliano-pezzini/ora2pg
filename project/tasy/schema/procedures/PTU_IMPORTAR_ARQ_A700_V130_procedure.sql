...
CREATE OR REPLACE PROCEDURE ptu_importar_arq_a700_v130 ( ds_conteudo_p text, nm_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text, ie_versao_p INOUT text) AS $body$
DECLARE
...
if (ds_registro_w	= '702') then
	...
	ie_regime_atendimento_w		:= trim(both substr(ds_conteudo_p,569,2));
	ie_saude_ocupacional_w		:= trim(both substr(ds_conteudo_p,571,2));
	ie_cobertura_especial_w		:= trim(both substr(ds_conteudo_p,573,2));
...
if (ds_registro_w = '704') then
	...
	cd_servico_w := null;
	if ((substr(ds_conteudo_p,765,10))::numeric  > 0) then
		cd_servico_w := (substr(ds_conteudo_p,765,10))::numeric;
	end if;
	
	qt_procedimento_w := 0;
	if ((substr(ds_conteudo_p,101,8))::numeric  > 0) then
		qt_procedimento_w := (substr(ds_conteudo_p,775,5)||','|| substr(ds_conteudo_p,780,4))::numeric;
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_importar_arq_a700_v130 ( ds_conteudo_p text, nm_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text, ie_versao_p INOUT text) FROM PUBLIC;
