...
CREATE OR REPLACE PROCEDURE ctb_pls_contab_provisao_prod ( nr_lote_contabil_p bigint, nm_usuario_p text, ie_exclusao_p text, ds_retorno_p INOUT text) AS $body$
DECLARE
...
c_itens CURSOR FOR
	SELECT	'P', /*Procedimento*/
		...
		d.nr_seq_prestador_pgto nr_seq_prestador_pag,
...
union all
	...
	select	'P', /*Procedimento*/
		...
		CASE WHEN a.ie_tipo_protocolo='F' THEN  coalesce(c.vl_liberado,0)  ELSE coalesce(c.vl_provisao,0) END ,
		a.dt_mes_competencia,
		c.cd_historico_prov,
		b.nr_sequencia,
		b.nr_seq_segurado,
		...
		b.nr_seq_prestador nr_seq_prestador_solic,
		b.nr_seq_prestador_exec nr_seq_prestador_exec,
		null nr_seq_prestador_pag,
		a.nr_seq_prestador nr_seq_prestador_atend
	FROM pls_conta_proc c, pls_protocolo_conta a, pls_conta b
LEFT OUTER JOIN pls_plano e ON (b.nr_seq_plano = e.nr_sequencia)
LEFT OUTER JOIN pls_prot_conta_titulo f ON (b.nr_seq_prot_conta = f.nr_sequencia)
WHERE b.nr_sequencia		= c.nr_seq_conta and a.nr_sequencia		= b.nr_seq_protocolo   and a.dt_mes_competencia 	between dt_referencia_month_w and dt_referencia_w and a.cd_estabelecimento	= cd_estabelecimento_w and a.ie_tipo_protocolo	in ('C', 'F') and (ie_status_prov_pagto_w	= 'NC' or a.ie_tipo_protocolo = 'F') and b.ie_status		not in ('C', 'U') and a.ie_situacao not in ('RE','I') and c.ie_status not in ('D','M') and ((coalesce(c.vl_provisao, 0) <> 0) or ((coalesce(c.vl_liberado,0) <> 0) and (coalesce(c.vl_provisao, 0) = 0))) and coalesce(c.nr_lote_contabil_prov,0) 	= 0 and ((ie_consiste_recalculo_w <> 'S')
	or ('P' not in (	select	coalesce(max(x.ie_tipo_relacao),'X')
				from	pls_prestador	x
				where	x.nr_sequencia	= b.nr_seq_prestador_exec))
	or (dt_referencia_trunc_w	= dt_ref_last_day_w))
...
LEFT OUTER JOIN pls_plano e ON (b.nr_seq_plano = e.nr_sequencia)
LEFT OUTER JOIN pls_prot_conta_titulo f ON (b.nr_seq_prot_conta = f.nr_sequencia)
WHERE b.nr_sequencia		= c.nr_seq_conta and a.nr_sequencia		= b.nr_seq_protocolo   and a.dt_mes_competencia 	between dt_referencia_month_w and dt_referencia_w and a.cd_estabelecimento	= cd_estabelecimento_w and a.ie_tipo_protocolo	in ('C', 'F') and (ie_status_prov_pagto_w	= 'NC' or a.ie_tipo_protocolo = 'F') and b.ie_status		not in ('C', 'U') and a.ie_situacao not in ('RE','I') and c.ie_status not in ('D','M') and coalesce(c.nr_lote_contabil_prov,0) 	= 0 and ((coalesce(c.vl_provisao, 0) <> 0) or ((coalesce(c.vl_liberado,0) <> 0) and (coalesce(c.vl_provisao, 0) = 0))) and ((ie_consiste_recalculo_w <> 'S')
	or ('P' not in (	select	coalesce(max(x.ie_tipo_relacao),'X')
				from	pls_prestador	x
				where	x.nr_sequencia	= b.nr_seq_prestador_exec))
	or (dt_referencia_trunc_w	= dt_ref_last_day_w));
...
c_lote_contabil CURSOR FOR
	SELECT 	oid
	from 	lote_contabil
	where	nr_lote_contabil	= nr_lote_contabil_p;

c_pls_conta_proc CURSOR FOR
	SELECT 	oid
	from	pls_conta_proc
	where 	nr_lote_contabil_prov	= nr_lote_contabil_p;

c_pls_conta_mat CURSOR FOR
	SELECT 	oid
	from 	pls_conta_mat
	where 	nr_lote_contabil_prov	= nr_lote_contabil_p;

c_pls_conta_medica_resumo CURSOR FOR
	SELECT 	oid
	from 	pls_conta_medica_resumo
	where	nr_lote_contabil_prov	= nr_lote_contabil_p;

c_pls_protocolo_conta CURSOR FOR
	SELECT 	oid
...
type t_lote_contabil is table of oid;
desfazer_lote_contab_w t_lote_contabil;

type t_pls_conta_proc_update is table of oid;
desfazer_pls_conta_proc_w t_pls_conta_proc_update;

type t_pls_conta_mat_update is table of oid;
desfazer_pls_conta_mat_w t_pls_conta_mat_update;

type t_pls_conta_medica_resumo is table of oid;
desfazer_pls_conta_med_res_w t_pls_conta_medica_resumo;

type t_pls_protocolo_conta is table of oid;
desfazer_pls_protocolo_conta_w t_pls_protocolo_conta;
...
-- Variavei do C_delete | cursor para limpar a tabela
...
if (ie_exclusao_p <> 'S') then
        ...
        if (cd_tipo_lote_contabil_w <> 40) then
...
if (ie_exclusao_p = 'S') then
	...
	open c_lote_contabil;
	loop
	fetch c_lote_contabil bulk collect into desfazer_lote_contab_w limit 1000;
		forall i in desfazer_lote_contab_w.first .. desfazer_lote_contab_w.last
			-- Atualizar os valores dos registros na tabela
			update	lote_contabil
			set	vl_credito		= 0,
				vl_debito		= 0,
				dt_geracao_lote		 = NULL
			where	rowid	= desfazer_lote_contab_w(i);
		commit;
	EXIT WHEN NOT FOUND; /* apply on c_lote_contabil */
	end loop;
	close c_lote_contabil;
	
	open c_pls_conta_proc;
	loop
	fetch c_pls_conta_proc bulk collect into desfazer_pls_conta_proc_w limit 1000;
		forall i in desfazer_pls_conta_proc_w.first .. desfazer_pls_conta_proc_w.last
			-- Atualizar os valores dos registros na tabela
			update	pls_conta_proc a
			set	a.nr_lote_contabil_prov	= 0
			where	rowid	= desfazer_pls_conta_proc_w(i);
		commit;
	EXIT WHEN NOT FOUND; /* apply on c_pls_conta_proc */
	end loop;
	close c_pls_conta_proc;

	open c_pls_conta_mat;
	loop
	fetch c_pls_conta_mat bulk collect into desfazer_pls_conta_mat_w limit 1000;
		forall i in desfazer_pls_conta_mat_w.first .. desfazer_pls_conta_mat_w.last
			-- Atualizar os valores dos registros na tabela
			update	pls_conta_mat a
			set	a.nr_lote_contabil_prov	= 0
			where	rowid	= desfazer_pls_conta_mat_w(i);
	  	commit;
	EXIT WHEN NOT FOUND; /* apply on c_pls_conta_mat */
	end loop;
	close c_pls_conta_mat;

	open c_pls_conta_medica_resumo;
	loop
	fetch c_pls_conta_medica_resumo bulk collect into desfazer_pls_conta_med_res_w limit 1000;
		forall i in desfazer_pls_conta_med_res_w.first .. desfazer_pls_conta_med_res_w.last
			-- Atualizar os valores dos registros na tabela
			update	pls_conta_medica_resumo	a
			set	a.nr_lote_contabil_prov	= 0
			where	rowid	= desfazer_pls_conta_med_res_w(i);
		commit;
	EXIT WHEN NOT FOUND; /* apply on c_pls_conta_medica_resumo */
	end loop;
	close c_pls_conta_medica_resumo;

	open c_pls_protocolo_conta;
	loop
	fetch c_pls_protocolo_conta bulk collect into desfazer_pls_protocolo_conta_w limit 1000;
		forall i in desfazer_pls_protocolo_conta_w.first .. desfazer_pls_protocolo_conta_w.last
			-- Atualizar os valores dos registros na tabela
			update	pls_protocolo_conta
			set	nr_lote_contabil_prov	= 0
			where	rowid	= desfazer_pls_protocolo_conta_w(i);
		commit;
	EXIT WHEN NOT FOUND; /* apply on c_pls_protocolo_conta */
	end loop;
	close c_pls_protocolo_conta;
	...
	if (ie_gerar_recalculo_w = 'S' and dt_referencia_trunc_w = dt_ref_last_day_w) then
		-- Necessario gerar o recalculo do recurso proprio para esta competencia antes da geracao do lote contabil.
	...
	/*Ultimo dia do mes, devera consistir contas medicas em aberto*/
	...
	if (ie_status_prov_pagto_w = 'F' and dt_referencia_trunc_w	=  dt_ref_last_day_w) then
		...
		if (coalesce(ds_contas_w,'X') <> 'X') then
			/* Existem contas medicas em aberto: #@DS_CONTAS#@*/
	...
	EXIT WHEN NOT FOUND; /* apply on c_itens */
		...
		EXIT WHEN NOT FOUND; /* apply on c_tipo_movimento */
			...
			if (ie_movimento_w = 'C') then /* Conta */
				...
				if (ie_status_prov_pagto_w = 'F' and ie_tipo_protocolo_w <> 'F') then
				...
				if (cd_conta_deb_w IS NOT NULL AND cd_conta_deb_w::text <> '') then
					...
					if (ie_centro_custo_deb_w = 'S') then
						if (cd_centro_custo_deb_fixo_w IS NOT NULL AND cd_centro_custo_deb_fixo_w::text <> '') then
							...
							SELECT * FROM pls_obter_centro_custo(	'D', nr_seq_plano_w, cd_estabelecimento_w, '', '', ie_regulamentacao_w, nr_seq_segurado_w, null, cd_centro_custo_deb_w, nr_seq_regra_cc_w, null, 				--nr_seq_tipo_lanc_p
					...
					/* Como neste caso nao se trata de um estorno, a substring referente a parte de estorno e removida do ds_compl_historico_w antes do mesmo ser atribuido ao item_mov_contabil_w */

					item_mov_contabil_w.ds_compl_historico	:= regexp_replace(ds_compl_historico_w, ds_estorno_w, '');
					...
					item_mov_contabil_w.nr_seq_tab_compl	:= nr_seq_doc_compl_w;
					...
					if (ie_estorno_corresp_pre_w = 'S' and ie_tipo_protocolo_w = 'F') then
						item_mov_contabil_w.ds_compl_historico			:= ds_compl_historico_w;
						item_mov_contabil_w.vl_movimento                        := vl_contab_w * -1;
						movimento_contabil_w(movimento_contabil_w.count)        := item_mov_contabil_w;
					end if;
					...
					if (not pls_conta_proc_w.nr_id.exists(nr_doc_analitico_w)) then
						pls_conta_proc_w.nr_id(nr_doc_analitico_w)		:= nr_id_proc_w;
					end if;
					
					if (not pls_conta_mat_w.nr_id.exists(nr_doc_analitico_w)) then
						pls_conta_mat_w.nr_id(nr_doc_analitico_w)		:= nr_id_mat_w;
					end if;
					
					if (ie_status_prov_pagto_w = 'F') then
						if (not pls_conta_res_w.nr_id.exists(nr_doc_analitico_w)) then
							pls_conta_res_w.nr_id(nr_doc_analitico_w)	:= nr_id_res_w;
						end if;
					end if;
				end if;
				...
				if (cd_conta_cred_w IS NOT NULL AND cd_conta_cred_w::text <> '') then
					...
					if (ie_centro_custo_cred_w = 'S') then
						if (cd_centro_custo_cred_fixo_w IS NOT NULL AND cd_centro_custo_cred_fixo_w::text <> '') then
							...
							SELECT * FROM pls_obter_centro_custo(	'D', nr_seq_plano_w, cd_estabelecimento_w, '', '', ie_regulamentacao_w, nr_seq_segurado_w, null, cd_centro_custo_cred_w, nr_seq_regra_cc_w, null, 				--nr_seq_tipo_lanc_p
										null, 				--nr_seq_bonificacao_p
										null, 				--nr_seq_sca_p
										nr_seq_prestador_pag_w, nr_seq_prestador_exec_w, nr_seq_prestador_atend_w, nr_seq_prestador_solic_w, null				-- nr_seq_evento_p
		...
		if (pls_conta_proc_w.nr_id.count >= 1000) then
			begin
			forall i in indices of pls_conta_proc_w.nr_id
				update	pls_conta_proc	a
		...
		if (pls_conta_mat_w.nr_id.count >= 1000) then
			begin
			forall i in indices of pls_conta_mat_w.nr_id
				update	pls_conta_mat	a
				set	a.nr_lote_contabil_prov		= nr_lote_contabil_p
		...
		if (ie_status_prov_pagto_w = 'F') then			
			if (pls_conta_res_w.nr_id.count >= 1000) then
				begin
				forall i in indices of pls_conta_res_w.nr_id
					update	pls_conta_medica_resumo	a
					set	a.nr_lote_contabil_prov		= nr_lote_contabil_p
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ctb_pls_contab_provisao_prod ( nr_lote_contabil_p bigint, nm_usuario_p text, ie_exclusao_p text, ds_retorno_p INOUT text) FROM PUBLIC;
