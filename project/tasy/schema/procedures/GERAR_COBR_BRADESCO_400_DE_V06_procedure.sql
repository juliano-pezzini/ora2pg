...
CREATE OR REPLACE PROCEDURE gerar_cobr_bradesco_400_de_v06 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR 
	SELECT	lpad(' ',5,' ') cd_agencia_debito, 
		...
		lpad(c.cd_instrucao,2,'0') ie_instrucao1, 
		'00' ie_instrucao2, 
		lpad(CASE WHEN coalesce(elimina_caracteres_especiais(c.vl_acrescimo),0)=0 THEN ' '  ELSE elimina_caracteres_especiais(c.vl_acrescimo) END ,13,'0') vl_acrescimo, 
		to_char(clock_timestamp(),'ddmmyy') dt_desconto, 
		lpad(CASE WHEN coalesce(elimina_caracteres_especiais(c.vl_desconto),0)=0 THEN ' '  ELSE elimina_caracteres_especiais(c.vl_desconto) END ,13,'0') vl_desconto, 
		lpad('0',13,'0') vl_iof, 
...
select	substr(c.cd_convenio_banco,1,20), 
	upper(elimina_acentuacao(substr(obter_nome_pf_pj(null, b.cd_cgc),1,30))), 
...
ds_conteudo_w	:= '01' || 'REMESSA' || '01' || 'COBRANCA    ' ||cd_empresa_w || nm_empresa_w || '237' || 'Bradesco    ' || dt_geracao_w || 
			ds_branco_8_w|| 'MX' || nr_seq_arquivo_w || ds_brancos_277_w || nr_seq_registro_w;	
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	ds_conteudo_w	:= '1' || cd_agencia_debito_w || ie_digito_agencia_w || cd_agencia_bancaria_w || cd_conta_w || ie_digito_conta_w || 
				id_empresa_w || nr_controle_partic_w || cd_banco_w || ie_multa_w || pr_multa_w || nr_dig_nosso_numero_w || 
				vl_desconto_dia_w || cd_condicao_w || ie_emite_papeleta_w || ie_rateio_w || ie_endereco_w || ds_brancos_2_w || 
				ie_ocorrencia_w || nr_documento_w || dt_vencimento_w || vl_titulo_w || cd_banco_cobranca_w || 
				cd_agencia_deposito_w || ie_especie_w || ie_aceite_w || dt_emissao_w || ie_instrucao_1_w || ie_instrucao_2_w || 
				vl_acrescimo_w || dt_desconto_w || vl_desconto_w || vl_iof_w || vl_abatimento_w || ie_tipo_inscricao_w || 
...
ds_conteudo_w	:= '9' || ds_brancos_393_w || nr_seq_registro_w;	
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_cobr_bradesco_400_de_v06 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
