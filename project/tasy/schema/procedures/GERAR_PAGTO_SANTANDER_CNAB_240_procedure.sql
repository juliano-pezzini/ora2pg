...
CREATE OR REPLACE PROCEDURE gerar_pagto_santander_cnab_240 ( nr_seq_envio_p bigint, nm_usuario_p text) AS $body$
DECLARE


/* PADRAO CNAB 240 VERSAO 060 DE 08/2009 */
...
03 = Pagamento de Duplicatas e Titulos
04 = Pagamento de Dividendos
05 = Pagamento de Mensalidades Escolares
07 = Pagamento a Fornecedor / Honorarios
08 = Pagamento de Cambio / Fundos / Bolsas
09 = Repasse de Arrecadacao / Pagamento de Tributos
11 = DOC / TED para Poupanca
12 = DOC / TED para Deposito Judicial
13 = Pensao Alimenticia
14 = Restituicao de Imposto de Renda
99 = Outros 
*/
c01 CURSOR FOR
SELECT	distinct
	CASE WHEN b.ie_tipo_pagamento='CC' THEN '01' WHEN b.ie_tipo_pagamento='DOC' THEN '03' WHEN b.ie_tipo_pagamento='TED' THEN '03' WHEN b.ie_tipo_pagamento='CCP' THEN '05' WHEN b.ie_tipo_pagamento='OP' THEN '10' END ,
...
/* Se todos os titulos forem de folha de pagamento, entao sera gerado o arquivo para folha de pagamento */
	...
	EXIT WHEN NOT FOUND; /* apply on c02 */
		...
		if (qt_reg_w > 0) then
			...
			select	somente_numero(substr(obter_conta_pessoa(coalesce(a.cd_cgc,a.cd_pessoa_fisica),CASE WHEN coalesce(a.cd_cgc::text, '') = '' THEN 'F'  ELSE 'J' END ,'B'),1,3)) cd_banco,
					...
					rpad(coalesce(elimina_caractere_especial(substr(obter_dados_pf_pj(a.cd_pessoa_fisica,a.cd_cgc,'B'),1,255)),' '),15,' ') ds_bairro,
		...
		ds_conteudo_w	:=	'033' ||
					...
					lpad(somente_numero(to_char(coalesce(vl_escritural_w,0) + coalesce(vl_acrescimo_w,0) + coalesce(vl_despesa_w,0),'9999999999990.00')),15,'0') ||
					'000000000000000' ||
...
EXIT WHEN NOT FOUND; /* apply on c03 */
	...
	EXIT WHEN NOT FOUND; /* apply on c04 */
		...
		if (coalesce(nr_bloqueto_w,'X') <> 'X')  then
			...
			if (ie_forma_lanc_w in ('11', '16', '17')) then
				nr_bloqueto_cov_w := Substr(Substr(trim(both nr_tot_bloqueto_w),0,12),0,11) ||
					Substr(Substr(trim(both nr_tot_bloqueto_w),13,12),0,11) ||
					Substr(Substr(trim(both nr_tot_bloqueto_w),25,12),0,11) ||
					Substr(Substr(trim(both nr_tot_bloqueto_w),37,12),0,11);	
			else    		
		...
		if (qt_reg_w > 0) then
			...
			ds_conteudo_w	:=	'033' ||															--  1 3
						...
						'0' ||																-- 15 15
						'00' ||																-- 16 17
						lpad(substr(coalesce(nr_bloqueto_w,' '),1,44),44,' ') ||										-- 18 61
						lpad(substr(coalesce(nm_pessoa_w,' '),1,30),30,' ') ||										-- 62 91
						...
						lpad(somente_numero(to_char(coalesce(vl_escritural_w,0) - coalesce(vl_desconto_w,0) + coalesce(vl_acrescimo_w,0) + coalesce(vl_despesa_w,0) + coalesce(vl_juros_w,0) + coalesce(vl_multa_w,0),'9999999999990.00')),15,'0') || --153 167
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_pagto_santander_cnab_240 ( nr_seq_envio_p bigint, nm_usuario_p text) FROM PUBLIC;
