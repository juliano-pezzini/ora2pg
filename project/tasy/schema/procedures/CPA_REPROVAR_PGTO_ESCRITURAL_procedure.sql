...
CREATE OR REPLACE PROCEDURE cpa_reprovar_pgto_escritural ( nm_usuario_p text, nr_seq_banco_escrit_p bigint, nr_titulo_p bigint) AS $body$
DECLARE
...
if (nr_titulo_p <> 0) then
	...
	if (ie_reprovacao_w = 'U') then
		...
		where	
			nr_seq_banco_escrit	= nr_seq_banco_escrit_p
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE cpa_reprovar_pgto_escritural ( nm_usuario_p text, nr_seq_banco_escrit_p bigint, nr_titulo_p bigint) FROM PUBLIC;
