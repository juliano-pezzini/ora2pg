...
ie_remito_w			operacao_nota.ie_remito%type;
...
cd_federal_voucher_w ordem_compra_item.cd_federal_voucher%type;
/*	tributos		*/
...
cd_moeda_estrangeira_w		nota_fiscal.cd_moeda_estrangeira%type := null;
vl_conv_estrangeiro_w		nota_fiscal.vl_conv_estrangeiro%type := null;
cd_nat_ope_regra_w		nota_fiscal.cd_natureza_operacao%type;
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	select wheb_moeda_pck.get_moeda_padrao(cd_estabelecimento_p)
	into STRICT cd_moeda_padrao_w
	;
	
	if (vl_cotacao_w <> 1 AND cd_moeda_padrao_w <> cd_moeda_w) then
		cd_moeda_estrangeira_w := cd_moeda_w;
		vl_conv_estrangeiro_w := vl_cotacao_w;
	end if;
...
/*		select	min(dt_prevista_entrega)
		...
		if	((coalesce(dt_entrega_ordem_w::text, '') = '') or
			...
			select
			CASE WHEN cd_natureza_operacao_p=0 THEN cd_natureza_operacao_w  ELSE cd_natureza_operacao_p END
			into STRICT cd_nat_ope_regra_w
			;

			SELECT * FROM define_conta_material(
					cd_estabelecimento_p => cd_estabelecimento_p, cd_material_p => cd_material_w, ie_tipo_conta_p => ie_tipo_conta_w, ie_clinica_p => 0, cd_setor_atendimento_p => 0, ie_classif_convenio_p => 0, ie_tipo_atendimento_p => 0, ie_tipo_convenio_p => 0, cd_convenio_p => 0, cd_categoria_convenio_p => 0, cd_local_estoque_p => cd_local_estoque_w, cd_operacao_estoque_p => cd_operacao_estoque_w, dt_vigencia_p => trunc(dt_atualizacao_w), cd_conta_contabil_p => cd_conta_contabil_ww, cd_centro_custo_p => cd_centro_custo_w, cd_plano_p => null, ie_regra_pacote_p => null, nr_seq_motivo_solic_p => nr_seq_motivo_solic_w, ie_complexidade_sus_p => null, ie_tipo_financ_sus_p => null, cd_operacao_nf_p => cd_operacao_nota_p, ie_cancelamento_p => null, cd_natureza_operacao_p => cd_nat_ope_regra_w, vl_item_regra_p => null, nr_seq_proj_rec_p => nr_seq_proj_rec_w
					) INTO STRICT cd_conta_contabil_p => cd_conta_contabil_ww, cd_centro_custo_p => cd_centro_custo_w;
...
/*			if	(nvl(cd_conta_contabil_w, 'X') = 'X') or
	...
	if (ie_valor_frete_inf_adic_ww = 'S') and (coalesce(vl_frete_w,0) > 0) and
		((coalesce(ie_remito_w, '') <> 'RE') or (coalesce(ie_remito_w::text, '') = '')) then
	...
	if (ie_valor_seguro_inf_adic_ww = 'S') and (coalesce(vl_seguro_ww,0) > 0) and
		((coalesce(ie_remito_w, '') <> 'RE') or (coalesce(ie_remito_w::text, '') = '')) then
...
if (coalesce(ie_gerar_adiant_oc_w,'N') = 'S') and
        ((coalesce(ie_remito_w, '') <> 'RE') or (coalesce(ie_remito_w::text, '') = '')) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_nota_fiscal ( cd_estabelecimento_p bigint, nr_nota_fiscal_p nota_fiscal.nr_nota_fiscal%type, cd_serie_nf_p text, nr_ordem_compra_p bigint, nm_usuario_p text, cd_operacao_nota_p bigint, cd_natureza_operacao_p bigint, vl_cotacao_p bigint, dt_emissao_p timestamp, dt_entrega_ordem_p timestamp, ie_data_prevista_menor_p text, ie_restricao_p text, ie_grava_observacao_p text, ie_retorna_frete_p text, nr_sequencia_p INOUT bigint, ie_venc_ordem_p text, ie_inspecionado_p text, cd_setor_digitacao_p bigint, ie_gerar_itens_oc_p text, nr_danfe_p text, dt_entrada_saida_p timestamp, ds_lista_itens_p text, ds_lista_entrega_p text, cd_cgc_emitente_p text default '', nr_autorizacao_p text default null, cd_control_code_p text default null) FROM PUBLIC;
