...
CREATE OR REPLACE PROCEDURE gerar_outros_cron_proj_migr ( nr_seq_cronograma_p bigint, cd_funcao_p bigint, nr_seq_atividade_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (nr_seq_cronograma_p IS NOT NULL AND nr_seq_cronograma_p::text <> '') and (cd_funcao_p IS NOT NULL AND cd_funcao_p::text <> '') and (nr_seq_atividade_p IS NOT NULL AND nr_seq_atividade_p::text <> '') and (nm_usuario_p IS NOT NULL AND nm_usuario_p::text <> '') then
	...
	from (
		SELECT	coalesce(sum(a.qt_tempo_desenv),0) + 15 qt_tempo_desenv
		from	w_analise_migr_proj a
		where	a.cd_funcao = cd_funcao_p
		and	a.ie_objeto in ('TPageControl','TPopUpMenu')
		) a;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_outros_cron_proj_migr ( nr_seq_cronograma_p bigint, cd_funcao_p bigint, nr_seq_atividade_p bigint, nm_usuario_p text) FROM PUBLIC;
