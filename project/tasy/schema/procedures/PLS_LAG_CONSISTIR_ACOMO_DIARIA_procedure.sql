...
CREATE OR REPLACE PROCEDURE pls_lag_consistir_acomo_diaria ( nr_seq_lote_guia_imp_p bigint, nr_seq_lote_proc_imp_p bigint, nr_seq_guia_plano_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (nr_seq_tipo_acomodacao_w IS NOT NULL AND nr_seq_tipo_acomodacao_w::text <> '')	then
	...
	if (ie_classificacao_w = '3') then
		...
		if (ie_gerar_glosa_w = 'S') then
			...
			CALL pls_inserir_anexo_glosa_aut('1402', nr_seq_lote_guia_imp_p, nr_seq_lote_proc_imp_p, null, '', nm_usuario_p);
		end if;
	end if;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_lag_consistir_acomo_diaria ( nr_seq_lote_guia_imp_p bigint, nr_seq_lote_proc_imp_p bigint, nr_seq_guia_plano_p bigint, nm_usuario_p text) FROM PUBLIC;
