...
CREATE OR REPLACE PROCEDURE gerar_erro_prescr_gas (nr_prescricao_p bigint, nr_seq_gasoterapia_p bigint, nr_regra_p bigint, ie_permite_liberar_p text, ds_compl_erro_p text, cd_perfil_p bigint, nm_usuario_p text, nr_seq_erro_p INOUT bigint) AS $body$
DECLARE
...
BEGIN
if (nr_prescricao_p IS NOT NULL AND nr_prescricao_p::text <> '') and (nr_seq_gasoterapia_p IS NOT NULL AND nr_seq_gasoterapia_p::text <> '') and (ie_permite_liberar_p IS NOT NULL AND ie_permite_liberar_p::text <> '') and (nm_usuario_p IS NOT NULL AND nm_usuario_p::text <> '') then
	...
	if (obter_funcao_origem_prescr(nr_prescricao_p) = 2314) and --CPOE
	   (cpoe_obter_se_consiste(nr_regra_p) = 'S') then
		...
		if (coalesce(nr_seq_gas_cpoe_w,0) > 0) then
			nr_seq_erro_p := gerar_erro_cpoe(nr_seq_gas_cpoe_w, 'O', nr_regra_p, ie_permite_liberar_p, ds_compl_erro_p, cd_perfil_p, nm_usuario_p, obter_atendimento_prescr(nr_prescricao_p), nr_prescricao_p, nr_seq_erro_p);
		end if;
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_erro_prescr_gas (nr_prescricao_p bigint, nr_seq_gasoterapia_p bigint, nr_regra_p bigint, ie_permite_liberar_p text, ds_compl_erro_p text, cd_perfil_p bigint, nm_usuario_p text, nr_seq_erro_p INOUT bigint) FROM PUBLIC;
