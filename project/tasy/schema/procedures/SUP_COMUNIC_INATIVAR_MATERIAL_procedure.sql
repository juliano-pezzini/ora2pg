...
CREATE OR REPLACE PROCEDURE sup_comunic_inativar_material ( cd_material_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
c01 CURSOR FOR
	...
	and	coalesce(ie_inativa_cadastro,'N')				= 'S'
...
/*select	substr('O material código: ' || cd_material_p || ' - '  || substr(obter_desc_material(cd_material_p),1,120) || ' foi inativado pelo usuário ' || nm_usuario_p || chr(13) || chr(10) ||
	'Grupo: ' || ds_grupo_material_w || chr(13) || chr(10) ||
	'Subgrupo: ' || ds_subgrupo_material_w || chr(13) || chr(10) ||
	'Classe: ' || ds_classe_material_w,1,255),
	'Aviso material inativo.'
into	ds_comunic_w,
	ds_titulo_w
from 	dual;*/
select	substr(wheb_mensagem_pck.get_texto(314888, 'CD_MATERIAL=' || cd_material_p || ';' ||
	...
	wheb_mensagem_pck.get_texto(314887)
...
from 	regra_aviso_exec_mat
where	coalesce(ie_inativa_cadastro,'N') = 'S';
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE sup_comunic_inativar_material ( cd_material_p bigint, nm_usuario_p text) FROM PUBLIC;
