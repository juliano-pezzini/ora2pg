...
CREATE OR REPLACE PROCEDURE rop_virada_saldo_roupa ( dt_mesano_referencia_p timestamp, nm_usuario_p text) AS $body$
DECLARE
...
if (PKG_DATE_UTILS.start_of(clock_timestamp(),'dd',0) = PKG_DATE_UTILS.start_of(clock_timestamp(),'mm',0)) then
	begin
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE rop_virada_saldo_roupa ( dt_mesano_referencia_p timestamp, nm_usuario_p text) FROM PUBLIC;
