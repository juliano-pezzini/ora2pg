...
CREATE OR REPLACE PROCEDURE gerar_prescricao_ag_job ( nm_usuario_p text, qt_horas_p bigint, cd_perfil_p bigint default null, ie_libera_prescr_p text default 'N') AS $body$
DECLARE
...
and		to_date(to_char(a.dt_agenda,'dd/mm/yyyy')||' '||to_char(a.hr_inicio,'hh24:mi:ss'),'dd/mm/yyyy hh24:mi:ss') between clock_timestamp() and clock_timestamp() + qt_horas_p/24;
...
EXIT WHEN NOT FOUND; /* apply on c000 */
	...
	SELECT * FROM consiste_medico_executor( 
			cd_estabelecimento_w, cd_convenio_agend_w, cd_setor_atend_agend_w, cd_procedimento_agend_w, ie_origem_proced_agend_w, ie_tipo_atend_agend_w, null, nr_seq_proc_interno_agend_w, ie_medico_executor_w, cd_cgc_w, cd_medico_agendamento_w, cd_pf_fisica_w, null, clock_timestamp(), null, 'N', null, null) INTO STRICT ie_medico_executor_w, cd_cgc_w, cd_medico_agendamento_w, cd_pf_fisica_w;
			 
	if (coalesce(cd_medico_agendamento_w::text, '') = '') or (obter_se_medico(cd_medico_agendamento_w,'M') = 'N') then 
		cd_medico_agendamento_w	:= '821699';
	end if;
	if (coalesce(cd_medico_agendamento_w::text, '') = '') then 
		--É necessário informar o médico para gerar a prescrição! Verifique a regra Consiste Setor Proced. 
	...
	/* Rafael em 30/18/ OS81037 incluí este IF */
	...
	/* Obter as informações da agenda */
	...
	if ie_atualizar_setor_agenda_w = 'S' then 
		select 	obter_dados_agendamento(nr_seq_agenda_w,'CSA') 
	...
	select	Obter_Setor_exec_proc_interno(nr_seq_proc_interno,cd_setor_atendimento,null,null,null) 
	into STRICT	cd_setor_atendimento_proc_w 
	from	agenda_paciente 
	where	nr_sequencia = nr_seq_agenda_w;
	 
	if (ie_regra_prescr_w = 'U') and (cd_paciente_w IS NOT NULL AND cd_paciente_w::text <> '') then 
		begin 
		...
		values	(nr_prescricao_w, 
			cd_paciente_w, 
			cd_medico_agendamento_w, 
			trunc(dt_prescricao_w,'mi'), 
			clock_timestamp(), 
			nm_usuario_p, 
			nm_usuario_p, 
			24, 
			null, 
			coalesce(cd_setor_atendimento_proc_w,coalesce(cd_setor_atendimento_w,cd_setor_padrao_w)), 
			coalesce(cd_setor_entrega_270_w,(coalesce(cd_setor_padrao_w,cd_setor_atendimento_w))), 
			clock_timestamp(), 
			'N', 
			ie_origem_inf_w, 
			cd_estabelecimento_w, 
			CASE WHEN coalesce(nr_seq_agenda_w,0)=0 THEN  null  ELSE nr_seq_agenda_w END , 
		...
		if (ie_chama_fanep_w = 'FANEP') then 
			...
			nr_seq_fanep := gerar_fanep_gestao_exames(cd_pessoa_fisica_ww, cd_medico_anestesista_w, nr_atendimento_w, nr_seq_agenda_w, nr_seq_fanep, nm_usuario_p, nr_seq_proc_interno_ww, cd_procedimento_ww, ie_origem_proced_ww);
		...
		if (cd_setor_atendimento_w <> 0) and (nr_seq_agenda_w IS NOT NULL AND nr_seq_agenda_w::text <> '') and (nr_prescricao_w > 0) then 
			begin 
			/* Gera os procedimentos da agenda na prescrição criada */
 
			CALL Gerar_Proc_Agenda_Prescricao(nr_seq_agenda_w, nr_prescricao_w, coalesce(cd_setor_atendimento_proc_w,cd_setor_atendimento_w), nm_usuario_p);
		...
		if (coalesce(nr_seq_agenda_w,0) > 0) and (coalesce(nr_prescricao_w,0) > 0) then 
	...
	commit;
	if (ie_libera_prescr_p = 'S') then 
		begin 
		if (coalesce(nr_prescricao_w,0) > 0) then 
			ds_erro_ww := Liberar_prescricao(nr_prescricao_w, null, 'S', cd_perfil_p, nm_usuario_p, 'S', ds_erro_ww);
		end if;
		exception 
			when others then 
				null;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_prescricao_ag_job ( nm_usuario_p text, qt_horas_p bigint, cd_perfil_p bigint default null, ie_libera_prescr_p text default 'N') FROM PUBLIC;
