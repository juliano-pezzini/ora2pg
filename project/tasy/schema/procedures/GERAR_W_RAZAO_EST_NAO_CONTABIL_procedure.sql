...
CREATE OR REPLACE PROCEDURE gerar_w_razao_est_nao_contabil ( cd_estabelecimento_p bigint, dt_mesano_referencia_p timestamp, dt_mesano_referencia_fim_p timestamp, cd_local_estoque_p bigint, cd_material_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
/*
Procedue para gerar o relatório Razão do estoque NÃO CONTÁBIL
Este relatório apresentará tambem os empréstimos nos movimentos, o que ocasiona a não conferencia
dos saldos finais com os saldos inicias do proximo mês
*/
...
nm_usuario_movto_w		varchar(15);
dt_atualizacao_movto_w		timestamp;
ie_nao_contabil_w		varchar(1);
ie_tipo_informacao_w		integer;
...
select	6,
	PKG_DATE_UTILS.start_of(a.dt_emprestimo, 'dd', 0) dt_movimento_estoque,
	a.dt_emprestimo,
	sum(CASE WHEN a.ie_tipo='E' THEN  qt_emprestimo  ELSE qt_emprestimo * -1 END ),
	a.dt_emprestimo dt_processo,
	a.cd_local_estoque,
	0,
	a.ie_tipo,
	...
	a.nr_emprestimo,
	'1',
	PKG_DATE_UTILS.start_of(a.dt_emprestimo, 'month', 0),
	a.nm_usuario,
	a.dt_atualizacao
from	material e,
	emprestimo a,
...
and	e.cd_material_estoque	= cd_material_p
and	PKG_DATE_UTILS.start_of(a.dt_emprestimo, 'month', 0) between dt_mesano_referencia_p and dt_mesano_referencia_fim_p
group by
	a.dt_emprestimo,
	a.dt_emprestimo,
	a.cd_local_estoque,
...
select	7,
	PKG_DATE_UTILS.start_of(a.dt_emprestimo, 'dd') dt_movimento_estoque,
	a.dt_emprestimo,
	sum(CASE WHEN a.ie_tipo='E' THEN  c.qt_material * -1  ELSE c.qt_material * -1 END ),
	a.dt_atualizacao dt_processo,
	a.cd_local_estoque,
	0,
	a.ie_tipo,
	e.cd_material_estoque,
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	vl_custo_medio_w	:= 0;
	if (ie_tipo_informacao_w = 5) then
	...
	if (dt_movimento_w < dt_mesano_referencia_p) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_w_razao_est_nao_contabil ( cd_estabelecimento_p bigint, dt_mesano_referencia_p timestamp, dt_mesano_referencia_fim_p timestamp, cd_local_estoque_p bigint, cd_material_p bigint, nm_usuario_p text) FROM PUBLIC;
