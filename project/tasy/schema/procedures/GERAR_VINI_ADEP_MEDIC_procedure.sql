...
CREATE OR REPLACE PROCEDURE gerar_vini_adep_medic ( nr_atendimento_p bigint, cd_material_p bigint, ie_tipo_item_p text, ie_atend_pac_p text, nr_prescricao_p bigint, ie_dias_medicamento_p text, nm_usuario_p text, dt_inicio_p timestamp, dt_fim_p timestamp ) AS $body$
DECLARE
...
elsif (ie_atend_pac_p	= 'PERIOD') or (ie_atend_pac_p	= 'PERIOD_PAC') or (ie_atend_pac_p	= 'PERIOD_ATE') then
...
for i in 1..qt_dias_w loop
	...
	if (ie_dias_medicamento_p = 'S') then
		...
		if (ie_tipo_item_p = 'PA') then
			...
			and		d.nr_seq_ficha_tecnica = cd_material_p
			and		b.dt_prescricao between dt_entrada_w and dt_alta_w
			and		trunc(a.dt_horario) = trunc(dt_medicacao_w)
			and		b.cd_pessoa_fisica = cd_pessoa_fisica_w
			and		((coalesce(ie_atend_pac_p,'PAC') <> 'ATEND') or (b.nr_atendimento = nr_atendimento_p))
...
if (coalesce(ie_atend_pac_p,'PAC') = 'ATEND') and (ie_tipo_item_p = 'PA') then
...
elsif (coalesce(ie_atend_pac_p,'PAC') = 'PAC') and (ie_tipo_item_p = 'PA') then
...
elsif (coalesce(ie_atend_pac_p,'PAC') = 'PERIOD') and (ie_tipo_item_p = 'PA') then
	...
	and		a.nr_seq_material = c.nr_sequencia
	and		b.cd_pessoa_fisica = cd_pessoa_fisica_w
	and		coalesce(nr_atendimento_p,b.nr_atendimento) = b.nr_atendimento
...
elsif (coalesce(ie_atend_pac_p,'PAC') = 'PERIOD_ATE') and (ie_tipo_item_p = 'PA') then
...
elsif (coalesce(ie_atend_pac_p,'PAC') = 'PERIOD_PAC') and (ie_tipo_item_p = 'PA') then
...
elsif (coalesce(ie_atend_pac_p,'PAC') = 'ATEND') and (ie_tipo_item_p <> 'PA') then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_vini_adep_medic ( nr_atendimento_p bigint, cd_material_p bigint, ie_tipo_item_p text, ie_atend_pac_p text, nr_prescricao_p bigint, ie_dias_medicamento_p text, nm_usuario_p text, dt_inicio_p timestamp, dt_fim_p timestamp ) FROM PUBLIC;
