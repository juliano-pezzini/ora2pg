...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_fatura_v41 ( nr_seq_fatura_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
nr_guia_principal_int_w		varchar(20);
nr_guia_principal_w		varchar(11);
nr_lote_w			varchar(8);
cd_cid_w			varchar(6);
...
C01 CURSOR FOR 
	SELECT	lpad(coalesce(elimina_caractere_especial(to_char(nr_lote)),'0'),8,'0'), 
		lpad(coalesce(substr(elimina_caractere_especial(to_char(nr_nota)),1,11),'0'),11,'0'), 
...
C02 CURSOR FOR 
	SELECT	lpad(elimina_caractere_especial(a.nr_lote),8,'0') nr_lote_503, 
		...
		lpad(coalesce(a.ie_tipo_internacao,0),1,'0'), 
		...
		rpad(coalesce(a.cd_cid_obito,' '),6,' '), 
...
C03 CURSOR FOR 
	SELECT	lpad(coalesce(elimina_caractere_especial(a.nr_lote),'0'),8,'0'), 
		...
		lpad(coalesce(a.qt_procedimento,'0'),8,'0'), 
		...
		rpad(coalesce(CASE WHEN a.cd_porte_anestesico='0' THEN null  ELSE a.cd_porte_anestesico END ,' '),1,' '), 
...
C06 CURSOR FOR 
	SELECT	coalesce(ie_tipo,' '), 
		rpad(coalesce(cd_ans,'0'),10,' '), 
		rpad(coalesce(elimina_acentuacao(coalesce(nm_cedente_sacado,' ')),' '),60,' '), 
		rpad(coalesce(ds_endereco,' '),40,' '), 
		rpad(coalesce(ds_complemento,' '),20,' '), 
		rpad(coalesce(ds_bairro,' '),30,' '), 
		rpad(coalesce(cd_cep,' '),8,' '), 
		rpad(coalesce(nm_cidade,' '),30,' '), 
		rpad(coalesce(sg_uf,' '),2,' '), 
		rpad(coalesce(cd_cgc_cpf,' '),14,' '), 
		lpad(coalesce(nr_inscricao_estadual,'0'),20,'0'), 
		lpad(coalesce(nr_ddd,'0'),4,'0'), 
		lpad(coalesce(nr_telefone,'0'),8,'0'), 
		lpad(coalesce(nr_fax,'0'),8,'0') 
...
if (nr_seq_fatura_p IS NOT NULL AND nr_seq_fatura_p::text <> '') then 
	...
	EXIT WHEN NOT FOUND; /* apply on C00 */
		...
		EXIT WHEN NOT FOUND; /* apply on C01 */
			...
			if (length(nr_guia_principal_int_w) > 11) and (substr(lpad(nr_guia_principal_int_w,20,'0'),10,11) <> lpad('0',11,'0')) then			 
				nr_guia_principal_w := substr(nr_guia_principal_int_w,length(nr_guia_principal_int_w)-10,length(nr_guia_principal_int_w));
			else	 
				nr_guia_principal_w := lpad(coalesce(nr_guia_principal_int_w,'0'),11,'0');
			end if;
			 
			if (length(nr_nota_geral_w) > 11) and (substr(lpad(nr_nota_geral_w,20,'0'),10,11) <> lpad('0',11,'0')) then			 
				nr_nota_w := substr(nr_nota_geral_w,length(nr_nota_geral_w)-10,length(nr_nota_geral_w));
			else	 
				nr_nota_w := lpad(coalesce(nr_nota_geral_w,'0'),11,'0');
			end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_fatura_v41 ( nr_seq_fatura_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
