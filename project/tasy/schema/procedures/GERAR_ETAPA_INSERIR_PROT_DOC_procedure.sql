...
CREATE OR REPLACE PROCEDURE gerar_etapa_inserir_prot_doc ( nr_interno_conta_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR 
	...
	and (coalesce(ie_evento,'A') = coalesce(ie_evento_w,'N')) 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_etapa_inserir_prot_doc ( nr_interno_conta_p bigint, nm_usuario_p text) FROM PUBLIC;
