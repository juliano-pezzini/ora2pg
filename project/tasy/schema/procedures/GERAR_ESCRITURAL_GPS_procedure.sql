...
CREATE OR REPLACE PROCEDURE gerar_escritural_gps ( nr_seq_gps_p bigint, cd_banco_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, nr_seq_conta_p bigint default null) AS $body$
DECLARE
...
--OS 1750569 início
if (nr_seq_gps_p IS NOT NULL AND nr_seq_gps_p::text <> '') then

	select	max(a.vl_juros),
			max(a.vl_multa)
	into STRICT	vl_juros_gps_w,
			vl_multa_gps_w
	from	gps a
	...
	if (nr_titulo_w IS NOT NULL AND nr_titulo_w::text <> '') then
		...
		set		vl_juros = coalesce(vl_juros,0) + coalesce(vl_juros_gps_w,0),
				vl_multa = coalesce(vl_multa,0) + coalesce(vl_multa_gps_w,0)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_escritural_gps ( nr_seq_gps_p bigint, cd_banco_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, nr_seq_conta_p bigint default null) FROM PUBLIC;
