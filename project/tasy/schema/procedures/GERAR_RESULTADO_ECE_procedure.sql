...
IF (ie_status_atend_w <= 30) THEN --nvl(ie_status_receb_w,30)) then 
	...
	IF (coalesce(nr_seq_resultado_w::text, '') = '') THEN 
		...
		AND		a.nr_prescricao = nr_prescricao_p 
		AND		b.nr_seq_prescr	= nr_seq_prescr_w 
		AND 	(b.nr_seq_material IS NOT NULL AND b.nr_seq_material::text <> '');WITH RECURSIVE cte AS (
...
;
	...
	CALL gravar_log_lab(1234, '01 - atualizar_lab_result_item ('||nr_prescricao_p||', '||nr_seq_prescr_w||', nvl('||cd_analito_w||','||cd_exame_p||'), '||ds_resultado_p||', 0, '||ds_resultado_p||', , null, n, ece, to_date('||ds_date_time_obs_p||', yyyymmddhh24miss), '||ds_referencia_p||', '||ds_units_p||', null, null, ds_erro_w, null);','ddmarco');	
	...
	CALL gravar_log_lab(1234, substr('atualizar_lab_result_item ('''||nr_prescricao_p||''', '''||nr_seq_prescr_w||''', '''||coalesce(cd_analito_w,cd_exame_princ_w)||''', '''||qt_resultado_w||''', '''||pr_resultado_w||''', '''||ds_resultado_w||''', '''||ds_observacao_p||''', NULL, ''N'', ''ECE'', '''||TO_DATE(ds_date_time_obs_p, 'yyyymmddhh24miss')||''', '''||ds_referencia_p||''', '''||ds_units_p||''', NULL, NULL, '''||ds_erro_w||''', '''||nr_seq_analito_w||''');',1,2000),nm_usuario_p, nr_prescricao_p);
	 
 
	ds_erro_w := atualizar_lab_result_item(nr_prescricao_p, nr_seq_prescr_w, coalesce(cd_analito_w,cd_exame_princ_w), qt_resultado_w, pr_resultado_w, ds_resultado_w, ds_observacao_p, NULL, 'N', 'ECE', TO_DATE(ds_date_time_obs_p, 'yyyymmddhh24miss'), ds_referencia_p, ds_units_p, NULL, NULL, ds_erro_w, nr_seq_analito_w);
