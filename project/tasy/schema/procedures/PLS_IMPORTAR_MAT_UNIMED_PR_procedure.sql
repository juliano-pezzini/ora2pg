...
CREATE OR REPLACE PROCEDURE pls_importar_mat_unimed_pr ( cd_estabelecimento_p bigint, nm_usuario_p text, ds_versao_tiss_p text) AS $body$
DECLARE
...
if (ie_produto_w	= 'S') then
	...
	for r_C01_w in C01 loop
		...
		if (substr(r_C01_w.ds_conteudo,7,1) = 'D') then
			...
			begin
			vl_tnumm_w		:= (substr(r_C01_w.ds_conteudo,846,10) || ',' || substr(r_C01_w.ds_conteudo,856,2))::numeric;
			exception
			when others then
				vl_tnumm_w	:= 0;
			end;
			...
			ie_uso_restrito_w		:= trim(both substr(r_C01_w.ds_conteudo,886,1));
			ds_nome_comercial_compl_w	:= trim(both substr(r_C01_w.ds_conteudo,887,200));
			cd_material_tuss_w		:= (trim(both substr(r_C01_w.ds_conteudo,1087,10)))::numeric;
			...
			if (qt_material_cad_w = 0) then
				...
				exception
				when others then
					CALL wheb_mensagem_pck.exibir_mensagem_abort(328181,	'CD_MATERIAL=' || cd_material_w);
				end;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_importar_mat_unimed_pr ( cd_estabelecimento_p bigint, nm_usuario_p text, ds_versao_tiss_p text) FROM PUBLIC;
