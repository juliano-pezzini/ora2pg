...
CREATE OR REPLACE PROCEDURE dacon_registro_r17b ( nr_seq_dacon_p bigint, nm_usuario_P text, vl_receita_p INOUT bigint) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	and	cd_registro  = 'R17B';

C02 CURSOR FOR
	SELECT	rpad('R17B',4,' ')						--01
...
BEGIN
select	substr(replace(campo_mascara_virgula_casas(obter_pr_imposto(cd_tributo_cofins),2),'.',''),1,15) pr_aliquota_cofins
into STRICT	pr_aliquota_w
from	DACON
where	nr_sequencia  = nr_seq_dacon_p;
...
if (vl_receita_total_w > 0) then
		vl_receita_p	 :=	(vl_receita_total_w * pr_aliquota_w) / 100;
elsif (vl_receita_nao_cum_w > 0)then
		vl_receita_p 	 := (vl_receita_nao_cum_w * pr_aliquota_w) / 100;
elsif (vl_desp_energ_w > 0)	then
		vl_receita_p	 := (vl_desp_energ_w * pr_aliquota_w) / 100;
elsif (vl_receita_bens_w > 0)	then
		vl_receita_p	 := (vl_receita_bens_w * pr_aliquota_w) / 100;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dacon_registro_r17b ( nr_seq_dacon_p bigint, nm_usuario_P text, vl_receita_p INOUT bigint) FROM PUBLIC;
