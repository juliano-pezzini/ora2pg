...
CREATE OR REPLACE PROCEDURE sac_gerar_envio_comunicacao ( nr_sequencia_p bigint, ie_evento_p text, ds_valor_ant_p text, ds_valor_atual_p text, nm_usuario_p text, ie_gerou_p INOUT text) AS $body$
DECLARE
...
nm_usuario_hist_w		usuario.ds_usuario%type;
dt_historico_w			varchar(30);
...
nr_seq_processo_regra_w		sac_regra_envio_comunic.nr_seq_processo%type;
nr_seq_processo_sac_bo_w	sac_boletim_ocorrencia.nr_seq_processo%type;
...
nr_atendimento_w		sac_boletim_ocorrencia.nr_atendimento%type;
...
c02 CURSOR FOR 
	...
	where	nr_seq_regra = nr_seq_regra_w; -- Ok 
...
c03 CURSOR FOR
	...
	and	ie_evento_p	not in ('1','3','8','9','10') 
...
union all
	...
	select	a.nr_seq_responsavel, 
		b.cd_setor_atendimento, 
		c.cd_estabelecimento 
	from	sac_hist_bol_ocor	d, 
		sac_resp_bol_ocor	a, 
		sac_responsavel		b, 
		sac_boletim_ocorrencia	c 
	where	b.nr_sequencia	= a.nr_seq_responsavel 
	and	c.nr_sequencia	= a.nr_seq_bo 
	and	a.nr_sequencia	= d.nr_seq_resp_bo 
	and	d.nr_sequencia	= nr_sequencia_p 
	and	ie_evento_p	in ('9','10');
...
if (ie_evento_p in ('1','3','8')) then 
...
elsif (ie_evento_p in ('5','7')) then 
	select	nr_seq_boletim_ocorrencia 
	into STRICT	nr_seq_bo_w 
	from	sac_boletim_ocor_analise 
	where	nr_sequencia = nr_sequencia_p;
elsif (ie_evento_p in ('9','10')) then 
	select	b.nr_seq_bo, 
	...
	from	sac_resp_bol_ocor b, 
		sac_hist_bol_ocor a 
	where	b.nr_sequencia = a.nr_seq_resp_bo 
	and	a.nr_sequencia = nr_sequencia_p;
elsif (ie_evento_p = '11') then 
	select	coalesce(max(nr_seq_bol_ocor),0) 
	into STRICT	nr_seq_bo_w 
	from	qua_nao_conformidade 
	where	nr_sequencia = nr_sequencia_p;
else 
	nr_seq_bo_w := nr_sequencia_p;
end if;
exception 
when others then 
	nr_seq_bo_w := 0;
...
if (coalesce(nr_seq_bo_w,0) > 0) and (qt_existe_w > 0) then 
	begin 
	open C01;
	loop 
	fetch C01 into 
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		into STRICT	ds_ocorrencia_w, 
			...
			nr_seq_processo_sac_bo_w, 
			nr_atendimento_w 
		from	sac_boletim_ocorrencia 
		where	nr_sequencia = nr_seq_bo_w; -- ok 
		
		if (coalesce(ds_valor_ant_w,'X') <> 'X') and (coalesce(ds_valor_ant_p,'X') <> 'X') and (coalesce(ds_valor_ant_w,'X') <> coalesce(ds_valor_ant_p,'X')) then 
			ie_gerar_w := 'N';
		end if;
 
		if (coalesce(ds_valor_atual_w,'X') <> 'X') and (coalesce(ds_valor_atual_p,'X') <> 'X') and (coalesce(ds_valor_atual_w,'X') <> coalesce(ds_valor_atual_p,'X')) then 
			ie_gerar_w := 'N';
		end if;
		 
		if	((coalesce(nr_seq_processo_regra_w,'0') <> '0') and	/*No caso da regra ter um processo específico mas o BO não tiver processo, não gera*/
 
			(coalesce(nr_seq_processo_regra_w,'0') <> coalesce(nr_seq_processo_sac_bo_w,'0'))) OR 
			((coalesce(nr_seq_processo_regra_w,'0') <> '0') and 	/*No caso da regra ter um processo específico e o BO tiver um processo diferente, também não gera*/
 
			(coalesce(nr_seq_processo_sac_bo_w,'0') <> '0') and (coalesce(nr_seq_processo_regra_w,'0') <> coalesce(nr_seq_processo_sac_bo_w,'0'))) then 
			ie_gerar_w := 'N';
		end if;	
		 
		if (coalesce(ie_gerar_w,'S') = 'S') then 
			begin 
			if (ie_evento_p in ('3')) then /* Resposta do B.O */
 
				select	substr(sac_obter_ds_status_bo(ie_status),1,60) 
			...
			if (ds_motivo_w IS NOT NULL AND ds_motivo_w::text <> '') then 
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@ds_motivo', ds_motivo_w),1,255);
				ds_titulo_w		:= substr(replace_macro(ds_titulo_w, '@ds_motivo', ds_motivo_w),1,255);	
			end if;
			...
			if (coalesce(nm_pessoa_abertura_w,'X') <> 'X') then 
				...
				--ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@ds_pessoa_abertura','Inf. não registrada'),1,32000); 
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@ds_pessoa_abertura',wheb_mensagem_pck.get_texto(305396)),1,32000);
				ds_titulo_w	:= substr(replace_macro(ds_titulo_w, '@ds_pessoa_abertura', obter_desc_expressao(345776)/*'Inf. não registrada'*/),1,255);
			end if;
			...
			if (coalesce(nm_pessoa_referencia_w,'X') <> 'X') then 
				...
				--ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@ds_pessoa_ref','Inf. não registrada'),1,32000); 
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@ds_pessoa_ref',wheb_mensagem_pck.get_texto(305396)),1,32000);
				ds_titulo_w	:= substr(replace_macro(ds_titulo_w, '@ds_pessoa_ref', wheb_mensagem_pck.get_texto(305396)),1,255);
			end if;
			...
			if (coalesce(ds_setor_referencia_w,'X') <> 'X') then 
				...
				--ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@ds_setor_ref','Inf. não registrada'),1,32000); 
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@ds_setor_ref',wheb_mensagem_pck.get_texto(305396)),1,32000);
				ds_titulo_w	:= substr(replace_macro(ds_titulo_w, '@ds_setor_ref', wheb_mensagem_pck.get_texto(305396)),1,255);
			end if;
			...
			if (coalesce(nr_seq_resp_w,0) > 0) then 
				...
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@ds_responsavel',wheb_mensagem_pck.get_texto(305396)),1,32000);
				ds_titulo_w		:= substr(replace_macro(ds_titulo_w, '@ds_responsavel', wheb_mensagem_pck.get_texto(305396)),1,255);
			end if;
			...
			if (ie_evento_p = '1') then	/*Alteração status*/
 
				begin 
				 
				select	substr(sac_obter_ds_status_bo(substr(ds_valor_ant_p,1,1)),1,255), 
					substr(sac_obter_ds_status_bo(substr(ds_valor_atual_p,1,1)),1,255) 
				into STRICT	ds_status_ant_w, 
					ds_status_atual_w 
				;
 
				if (ds_status_ant_w IS NOT NULL AND ds_status_ant_w::text <> '') then 
					ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@ds_status_ant', ds_status_ant_w),1,32000);
					ds_titulo_w	:= substr(replace_macro(ds_titulo_w, '@ds_status_ant', ds_status_ant_w),1,255);
				end if;
 
				if (ds_status_atual_w IS NOT NULL AND ds_status_atual_w::text <> '') then 
					ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@ds_status_atual', ds_status_atual_w),1,32000);
					ds_titulo_w	:= substr(replace_macro(ds_titulo_w, '@ds_status_atual', ds_status_atual_w),1,255);
				end if;
			...
			if (ie_evento_p in ('1','3','8')) then 
				...
				from	sac_resp_bol_ocor b, 
					sac_hist_bol_ocor a 
				where	a.nr_seq_resp_bo = b.nr_sequencia 
				and	nr_seq_resp_Bo = nr_sequencia_p 
				and	b.nr_seq_bo = nr_seq_bo_w;
			...
			/* Pega o último histórico da ordem de serviço*/
			...
			if (qt_existe_w > 0) then 
				if (ie_evento_p in ('1','3','8')) then 
					...
					from	sac_resp_bol_ocor b, 
						sac_hist_bol_ocor a 
					where	a.nr_seq_resp_bo = b.nr_sequencia 
					and	a.nr_Seq_resp_bo = nr_sequencia_p 
					and	b.nr_seq_bo = nr_seq_bo_w 
			...
			if (coalesce(ds_historico_w,'X') <> 'X') then 
				...
				--ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@ds_ultimo_historico','B.O sem histórico registrado'),1,32000); 
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@ds_ultimo_historico',wheb_mensagem_pck.get_texto(305397)),1,32000);
				ds_titulo_w	:= substr(replace_macro(ds_titulo_w, '@ds_ultimo_historico', wheb_mensagem_pck.get_texto(305397)),1,255);
			end if;
			...
			if (coalesce(dt_resposta_w,'X') <> 'X') then 
				...
				--ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@dt_resposta','Resposta sem data registrada'),1,32000); 
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@dt_resposta',wheb_mensagem_pck.get_texto(305398)),1,32000);
			end if;
			...
			if (coalesce(nm_pessoa_registro_w,'X') <> 'X') then 
				...
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@nm_pessoa_reg',wheb_mensagem_pck.get_texto(305399)),1,32000);
			end if;
			...
			if (coalesce(dt_encerramento_w,'X') <> 'X') then 
				...
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@dt_encerramento',wheb_mensagem_pck.get_texto(305400)),1,32000);
			end if;
			...
			if (coalesce(nm_usuario_envio_w,'X') <> 'X') then 
				...
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@nm_usuario_envio',wheb_mensagem_pck.get_texto(305401)),1,32000);
			end if;
			 
			ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@dt_ocorrencia',dt_ocorrencia_w),1,32000);
			 
			if (ie_evento_p in ('9','10')) then 
				begin 
				select	substr(obter_desc_usuario(a.nm_usuario),1,100), 
					to_char(CASE WHEN ie_evento_p='9' THEN a.dt_historico WHEN ie_evento_p='10' THEN a.dt_liberacao END ,'dd/mm/yyyy hh24:mi:ss') 
				into STRICT	nm_usuario_hist_w, 
					dt_historico_w 
				from	sac_hist_bol_ocor a 
				where	nr_sequencia = nr_sequencia_p;
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@usuario_historico',nm_usuario_hist_w),1,32000);
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@data_historico',dt_historico_w),1,32000);
				end;
			end if;
 
			if (ie_evento_p = '11') then 
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@nr_seq_nao_conf', nr_sequencia_p),1,32000);
				ds_titulo_w		:= substr(replace_macro(ds_titulo_w,'@nr_seq_nao_conf', nr_sequencia_p),1,255);
			end if;
			...
			if (coalesce(nr_atendimento_w,0) <> 0) then 
				ds_comunicacao_w	:= substr(replace_macro(ds_comunicacao_w, '@nm_paciente_atend', substr(obter_nome_paciente(nr_atendimento_w),1,255)),1,32000);
				ds_titulo_w		:= substr(replace_macro(ds_titulo_w,'@nm_paciente_atend ', substr(obter_nome_paciente(nr_atendimento_w),1,255)),1,255);
			end if;
			...
			if (ie_email_w = 'S') then 
				...
				insert into sac_bol_ocor_envio( 
					...
					values (	nextval('sac_bol_ocor_envio_seq'), 
						...
						--'Envio de boletim pela regra de envio automático', 
						wheb_mensagem_pck.get_texto(305402), 
						clock_timestamp(), 
						clock_timestamp(), 
						clock_timestamp(), 
						nm_usuario_p, 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE sac_gerar_envio_comunicacao ( nr_sequencia_p bigint, ie_evento_p text, ds_valor_ant_p text, ds_valor_atual_p text, nm_usuario_p text, ie_gerou_p INOUT text) FROM PUBLIC;
