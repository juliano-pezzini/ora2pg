...
CREATE OR REPLACE PROCEDURE pls_atualiza_valor_proc_n ( nr_sequencia_p pls_conta_proc.nr_sequencia%type, ie_gravar_log_p text, nm_usuario_p usuario.nm_usuario%type, ie_atualiza_p text, ie_origem_rec_p text, nr_param_dois_p bigint, ie_recalcular_conta_p text default 'N') AS $body$
DECLARE


/*	IE_ORIGEM_REC_P
	C - Consistência da conta : Mantém o processo da mesma forma como ocorre hoje, utilizado na pls_consistir_conta
	G - Glosar item : Por hora irá realizar comente o recálculo quando dos itens estiverem com o campo ie_via_obrigatória = 'S', deve recálcular estes itens sem impactar nos status dos mesmo que deve ficar inalterado após a consistência
*/
--Rotina utilizada na nova estrutura de valorização esta não pode ser utilizada em conjunto com a regra antiga devido aos tratamentos embutidos na mesma
...
ie_conta_repasse_w		varchar(1)	:= 'N';--DiogoAqui
...
nr_seq_prest_solic_w		bigint;--Diogoaqui
...
vl_taxa_material_w		double precision	:= 0;--Apesar do campo estar como 12,4 na tabela deve ser realizado apenas duas casas decimais devido a geração de problemas ligados ao arredondamento
...
nr_seq_prestador_solic_ref_w	pls_conta_proc_v.nr_seq_prestador_solic%type; --Diogoaqui
...
nr_seq_conta_princ_w		pls_conta_proc_v.nr_seq_conta_princ%type;
vl_medico_part_w		pls_proc_participante.vl_medico%type;
vl_anestesista_part_w		pls_proc_participante.vl_anestesista%type;
vl_auxiliares_part_w		pls_proc_participante.vl_auxiliares%type;
nr_seq_cp_comb_proc_entao_w	pls_conta_proc_regra.nr_seq_cp_comb_proc_entao%type;
...
C01 CURSOR(nr_seq_conta_proc_pc	pls_proc_participante.nr_seq_conta_proc%type)FOR
	SELECT	vl_medico_base,
		vl_anestesista_base,
		vl_auxiliares_base,
		nr_sequencia
	...
	and	(nr_seq_cp_comb_entao IS NOT NULL AND nr_seq_cp_comb_entao::text <> '');
BEGIN
...
--Tratamento necessário para zerar as variáveis da pls_proc_participante
update	pls_proc_participante
set	vl_medico_base		= vl_medico,
	vl_anestesista_base 	= vl_anestesista,
	vl_auxiliares_base	= vl_auxiliares
...
if (cd_guia_w IS NOT NULL AND cd_guia_w::text <> '') then
	...
	/* Edgar 11/09/2013, OS 643699, conforme autorizado pelo Adriano, podemos tratar fixo para obter o prestador da requisição como solicitante quando existir*/
	...
	if (ie_conta_internacao_w		= 'S')or (ie_tipo_guia_w 		= '6')then
...
if (nr_seq_segurado_w IS NOT NULL AND nr_seq_segurado_w::text <> '') then
	...
	CALL pls_gravar_conta_glosa('1013',nr_seq_conta_w,null,
				null,'N','Beneficirio não informado',nm_usuario_p,
...
-- A variável IE_CONTA_REPASSE é utilizada para a PLS_OBTER_REGRA_HONORARIO
if (ie_tipo_protocolo_w	= 'R') then
	...
	if	((ie_tipo_segurado_w = 'I') or (ie_tipo_segurado_w = 'T') or (ie_tipo_segurado_w = 'C') or (ie_tipo_segurado_w = 'H')) and (ie_tipo_protocolo_w = 'C') then
		/*precisa verificar sempre o do beneficiário OS - 318635, tipo segurado = C  Intercâmbio entre OPS congêneres (Resp. Assumida), de outra operadora porém estadual.
		nr_seq_congenere_w	:= nr_seq_congenere_seg_w;*/
		ie_tipo_tabela_int_w	:= 'IC';
	end if;

	/* Francisco - 14/05/2012 - OS 442042 - Só mudar para a congenere do segurado, se a congenere do segurado
	não for a própria operadora */
...
if (ie_calcula_valor_w	= 'S') then
	...
	/* Francisco - 12/05/2012 - Antes chamava só no final, tem que chamar antes de definir o preço,
	pois tem regras associadas a isso */
	/* Felipe - 31/10/2010 - OS - 247474 - Gravar os valores em relação a coleta de exame */
	...
	select 	max(vl_procedimento_base),
		...
		max(vl_auxiliares),
		max(cd_edicao_amb),
		max(nr_seq_cp_comb_proc_entao),
		max(vl_procedimento_cop),
		max(vl_custo_operacional_cop),
		max(vl_anestesista_cop),
		max(vl_medico_cop),
		max(vl_filme_cop),
		max(vl_auxiliares_cop),
		coalesce(max(tx_item),0)
	into STRICT	vl_procedimento_w,
		...
		vl_auxiliares_w,
		cd_edicao_amb_w,
		nr_seq_cp_comb_proc_entao_w,
		vl_proc_copartic_w,
		vl_co_copartic_w,
		vl_anestesista_copartic_w,
		vl_medico_copartic_w,
		vl_filme_copartic_w,
		vl_auxiliares_copartic_w,
		tx_item_regra_w
	...
	if	(
		...
		for r_c01_w in C01(nr_sequencia_p) loop
			begin
			vl_medico_part_w	:= ((tx_item_w	* r_c01_w.vl_medico_base)/100);
			vl_auxiliares_part_w	:= ((tx_item_w	* r_c01_w.vl_auxiliares_base)/100);
			vl_anestesista_part_w	:= ((tx_item_w	* r_c01_w.vl_anestesista_base)/100);

			update	pls_proc_participante
			set	vl_medico_base		= vl_medico_part_w,
				vl_auxiliares_base	= vl_auxiliares_part_w,
				vl_anestesista_base	= vl_anestesista_part_w
	...
	if (ie_gravar_log_w = 'S') then
		...
		CALL pls_entao_regra_preco_cta_pck.gerencia_log_proc(	ds_observacao_log_w,nr_sequencia_p, vl_anestesista_w,
									vl_auxiliares_w, vl_custo_operacional_w, vl_filme_w,
									vl_medico_w, vl_procedimento_w,	null,
									nm_usuario_p);
	end if;
	...
	if (tx_item_regra_w > 0) then
		...
		if (pls_cta_valorizacao_pck.obter_se_taxa_filme(nr_seq_prestador_w, cd_estabelecimento_w) = 'S') then
			vl_filme_w		:= tx_item_regra_w * vl_filme_w;
		end if;
		for r_c01_w in C01(nr_sequencia_p) loop
			begin
			vl_medico_part_w	:= tx_item_regra_w * r_c01_w.vl_medico_base;
			vl_auxiliares_part_w	:= tx_item_regra_w * r_c01_w.vl_auxiliares_base;
			vl_anestesista_part_w	:= tx_item_regra_w * r_c01_w.vl_anestesista_base;
	...
	for r_c01_w in C01(nr_sequencia_p) loop
		begin
		vl_medico_part_w	:= r_c01_w.vl_medico_base * qt_procedimento_w;
		vl_auxiliares_part_w	:= r_c01_w.vl_auxiliares_base * qt_procedimento_w;
		vl_anestesista_part_w	:= r_c01_w.vl_anestesista_base * qt_procedimento_w;
	...
	CALL pls_entao_regra_preco_cta_pck.gerencia_log_proc(	'Preço atual x Qt. procedimento',nr_sequencia_p, vl_anestesista_w,
								vl_auxiliares_w, vl_custo_operacional_w, vl_filme_w,
								vl_medico_w, vl_procedimento_w,	null,
								nm_usuario_p);
	...
	/* Tratar os adicionais por horário */
	...
	for r_c01_w in C01(nr_sequencia_p) loop
		begin
		vl_medico_part_w	:= r_c01_w.vl_medico_base * dados_adic_horario_w.tx_medico;
		vl_auxiliares_part_w	:= r_c01_w.vl_auxiliares_base * dados_adic_horario_w.tx_auxiliares;
		vl_anestesista_part_w	:= r_c01_w.vl_anestesista_base * dados_adic_horario_w.tx_anestesista;

		vl_medico_part_w	:= r_c01_w.vl_medico_base * dados_adic_horario_w.tx_procedimento;
		vl_auxiliares_part_w	:= r_c01_w.vl_auxiliares_base * dados_adic_horario_w.tx_procedimento;
		vl_anestesista_part_w	:= r_c01_w.vl_anestesista_base * dados_adic_horario_w.tx_procedimento;
	...
	/* Francisco - 09/05/2012 - Coloquei a chamada aqui para que o repasse de honorário funcione corretamente */
	...
	/*aplica regra de quantidade de execução se o procedimento tiver se encaixado em alguma*/

	if (coalesce(nr_seq_regra_qtde_exec_w,0) > 0 ) and (coalesce(ie_atualiza_p,'S') = 'S' )then
		/*Caso a regra tenha sido alterada de forma manual não é aplicada a regra de quantidade de execução sobre o item*/
		...
		if (coalesce(ie_tx_manual_w::text, '') = '') or (ie_tx_manual_w = 'N' ) then
			...
			/* aplicação da taxa do procedimento sobre todos os valores para que seja ajustada corretamente*/
			...
			/*Tratado na OS 650687 caso seja aplicada a regra de quantidade de execução a taxa deverá subscrever a taxa informada do item*/

			/* Francisco - 30/10/2013 - OS 662881 */

			for r_c01_w in C01(nr_sequencia_p) loop
				begin
				vl_medico_part_w	:= dividir((r_c01_w.vl_medico_base* coalesce(tx_medico_w,100)),100);
				vl_auxiliares_part_w	:= dividir((r_c01_w.vl_auxiliares_base* coalesce(tx_auxiliares_w,100)),100);
				vl_anestesista_part_w	:= dividir((r_c01_w.vl_anestesista_base* coalesce(tx_anestesista_w,100)),100);

				vl_medico_part_w	:= dividir((r_c01_w.vl_medico_base* coalesce(tx_procedimento_w,100)),100);
				vl_auxiliares_part_w	:= dividir((r_c01_w.vl_auxiliares_base* coalesce(tx_procedimento_w,100)),100);
				vl_anestesista_part_w	:= dividir((r_c01_w.vl_anestesista_base* coalesce(tx_procedimento_w,100)),100);
	...
	if	((ie_tx_manual_w = 'S') and (pls_cta_valorizacao_pck.obter_se_taxa_filme(nr_seq_prestador_w, cd_estabelecimento_w) = 'S')) then
		--Utiliza a variavel "tx_custo_operacional_temp_w" por que se chegou aqui, a variavel tx_custo_operacional_w não tem valor...
		...
		for r_c01_w in C01(nr_sequencia_p) loop
			begin
			vl_medico_part_w	:= dividir((r_c01_w.vl_medico_base* coalesce(tx_medico_temp_w,100)),100);
	...
	/*obtenção da taxa administrativa do item*/
	...
	if (coalesce(tx_adm_w,0) > 0) then
		...
		for r_c01_w in C01(nr_sequencia_p) loop
			begin
			vl_medico_part_w	:= r_c01_w.vl_medico_base + dividir((r_c01_w.vl_medico_base * tx_adm_w),100);
			vl_auxiliares_part_w	:= r_c01_w.vl_auxiliares_base + dividir((r_c01_w.vl_auxiliares_base * tx_adm_w),100);
			vl_anestesista_part_w	:= r_c01_w.vl_anestesista_base + dividir((r_c01_w.vl_anestesista_base * tx_adm_w),100);
		...
		CALL pls_gravar_log_calculo_proc(	ie_gravar_log_w, nr_sequencia_p, cd_estabelecimento_w,
						'Preço atual x Taxa adm do item', ds_observacao_log_w, 'pls_atualiza_valor_proc',
						vl_procedimento_w, vl_filme_w, vl_custo_operacional_w,
						vl_medico_w, vl_auxiliares_w, vl_anestesista_w,
						8, nr_seq_regra_tx_adm_w, null,
		...
		for r_c01_w in C01(nr_sequencia_p) loop
			...
			vl_base_para_taxa_adm_w := vl_medico_part_w + vl_auxiliares_part_w + vl_anestesista_part_w;

			vl_fixo_tx_adm_aplicar_w := (vl_fixo_tx_adm_w * (dividir(vl_medico_part_w, vl_base_para_taxa_adm_w)));
			vl_medico_part_w	:= r_c01_w.vl_medico_base + vl_fixo_tx_adm_aplicar_w;

			vl_fixo_tx_adm_aplicar_w := (vl_fixo_tx_adm_w * (dividir(vl_auxiliares_part_w, vl_base_para_taxa_adm_w)));
			vl_auxiliares_part_w	:= r_c01_w.vl_auxiliares_base + vl_fixo_tx_adm_aplicar_w;

			vl_fixo_tx_adm_aplicar_w := (vl_fixo_tx_adm_w * (dividir(vl_anestesista_part_w, vl_base_para_taxa_adm_w)));
			vl_anestesista_part_w	:= r_c01_w.vl_anestesista_base + vl_fixo_tx_adm_aplicar_w;

			if (vl_medico_part_w > vl_auxiliares_part_w and vl_medico_part_w > vl_anestesista_part_w) then
				vl_medico_part_w := vl_medico_part_w + (vl_base_para_taxa_adm_w + vl_fixo_tx_adm_w) - (vl_medico_part_w + vl_auxiliares_part_w + vl_anestesista_part_w);
			elsif (vl_auxiliares_part_w > vl_anestesista_part_w) then
				vl_auxiliares_part_w := vl_auxiliares_part_w + (vl_base_para_taxa_adm_w + vl_fixo_tx_adm_w) - (vl_medico_part_w + vl_auxiliares_part_w + vl_anestesista_part_w);
			else
				vl_anestesista_part_w := vl_anestesista_part_w + (vl_base_para_taxa_adm_w + vl_fixo_tx_adm_w) - (vl_medico_part_w + vl_auxiliares_part_w + vl_anestesista_part_w);
			end if;
		...
		CALL pls_gravar_log_calculo_proc(	ie_gravar_log_w, nr_sequencia_p, cd_estabelecimento_w,
						'Preço atual + Valor da taxa adm do item', ds_observacao_log_w, 'pls_atualiza_valor_proc',
						vl_procedimento_w, vl_filme_w, vl_custo_operacional_w,
						vl_medico_w, vl_auxiliares_w, vl_anestesista_w,
						8, nr_seq_regra_tx_adm_w, null,
						nr_auxiliares_w, null, null, nm_usuario_p);
	...
	/* Gerava aqui os honorários e valores de participantes através do C01 */

	SELECT * FROM pls_gerar_honorario_proc_n(	nr_sequencia_p, cd_procedimento_w, ie_origem_proced_w, nr_seq_grau_partic_w, ie_tipo_guia_w, nr_seq_tipo_atendimento_w, ie_tipo_conta_w, nr_seq_prestador_w, ie_tipo_protocolo_w, nr_auxiliares_w, ie_status_w, ie_repassa_medico_w, vl_anestesista_w, vl_medico_w, vl_auxiliares_w, cd_estabelecimento_w, dt_procedimento_w, nm_usuario_p, dt_preco_w, nr_seq_plano_w, 'C', vl_total_partic_w, ie_tipo_partic_prof_w, nr_seq_prestador_pgto_w, ie_repassa_medico_pgto_w) INTO STRICT vl_total_partic_w, ie_tipo_partic_prof_w;

	if (ie_copartic_grau_partic_w = 'S')  and (coalesce(ie_preco_plano_w,'1') = '1') then
		SELECT * FROM pls_gerar_honorario_proc_n(	nr_sequencia_p, cd_procedimento_w, ie_origem_proced_w, nr_seq_grau_partic_w, ie_tipo_guia_w, nr_seq_tipo_atendimento_w, ie_tipo_conta_w, nr_seq_prestador_w, ie_tipo_protocolo_w, nr_auxiliares_w, ie_status_w, ie_repassa_medico_w, vl_anestesista_copartic_w, vl_medico_copartic_w, vl_auxiliares_copartic_w, cd_estabelecimento_w, dt_procedimento_w, nm_usuario_p, dt_preco_w, nr_seq_plano_w, 'CO', vl_total_partic_copartic_w, ie_tipo_partic_prof_w, nr_seq_prestador_pgto_w, null) INTO STRICT vl_total_partic_copartic_w, ie_tipo_partic_prof_w;
	end if;
	...
	vl_proc_copartic_w := (vl_proc_copartic_w * tx_adicional_proc_copartic_w);

	CALL pls_entao_regra_preco_cta_pck.gerencia_log_proc(	'Tipo repasse médico',nr_sequencia_p, vl_anestesista_w,
								vl_auxiliares_w, vl_custo_operacional_w, vl_filme_w,
								vl_medico_w, vl_procedimento_w,	null,
								nm_usuario_p);
	...
	vl_prestador_w		:= round((vl_procedimento_w)::numeric,2);
	...
	/*Necessário ser após a atribuição dos valores para que seja atualizado corretamente o vl_procedimento com a tx de intercâmbio OS376887*/
	...
	where	nr_sequencia 	= nr_seq_cp_comb_proc_entao_w;
	--no caso do 'ie_nao_gera_tx_inter_w' estar como 'S' não poderá ser aplicada a taxa de intercambio
	--no caso do 'ie_franquia_w' estar como 'S' deverá procurar o valor franquia cadastrado no produto e descontar do valor calculado
	...
	if (coalesce( ie_nao_gera_tx_inter_w,'N') in ('N','F')) then
		...
		if (ie_tipo_conta_w = 'I') then
			...
			--no caso do parametro estar como 'F' não poderá ser aplicada a taxa de intercambio sobre o filme
	...
	--Vl_medico_original é zerado no processo de desfazer consistência e obtido novamente na pls_gerar_honorario_proc, então
	...
	if (vl_franquia_w > 0) then
		...
		--Para garantir que vl_medico_original não seja negativo.
	...
	/* Solicitação de Adriano Geyer para deixar a data do item como a data informada.
	   Conforme tratado com Adriano Geyer e Paulo Rosa, foi criado o campo ie_tx_manual para quando a tx for informada manualmente não pode sofrer alteração*/
	...
	--Se não tiver regra, dizendo que não aplica,  então verifica se taxa foi alterada manualmente e nesse caso atualiza valores com base nessas taxas
	...
	if (coalesce(ie_origem_rec_p,'C') = 'C') then
		...
		set	tx_medico			= CASE WHEN ie_tx_manual_atualiza_w='N' THEN  coalesce(tx_medico_w,100)  ELSE tx_medico END ,
			...
			ie_cobranca_prevista		= CASE WHEN ie_cobranca_prevista_inf = NULL THEN  ie_cobranca_prevista_w  ELSE ie_cobranca_prevista END , --Se foi alterado manualmente na análise, então mantém o valor.,
		...
		-- data valorizacao é atualizada toda vez que é executada a valorização do item
...
-- se sobrou alguma coisa de log manda pro banco, senão tiver ainda sim é necessário chamar para reiniciar as variáveis
CALL pls_entao_regra_preco_cta_pck.atualizar_log_proc(nm_usuario_p, null);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_atualiza_valor_proc_n ( nr_sequencia_p pls_conta_proc.nr_sequencia%type, ie_gravar_log_p text, nm_usuario_p usuario.nm_usuario%type, ie_atualiza_p text, ie_origem_rec_p text, nr_param_dois_p bigint, ie_recalcular_conta_p text default 'N') FROM PUBLIC;
