-- Generated by Ora2Pg, the Oracle database Schema converter, version 23.1
-- Copyright 2000-2022 Gilles DAROLD. All rights reserved.
-- DATASOURCE: dbi:Oracle:host=srv-dbora-03.whebdc.com.br;service_name=DEV_1815;port=1521

SET client_encoding TO 'UTF8';



CREATE TYPE colunas AS (nm_coluna_w varchar(100));


CREATE OR REPLACE PROCEDURE hd_gerar_contr_dial_rel_6 ( nr_seq_protocolo_p bigint, ds_itens_p text, ds_itens_hb_p text, ds_itens_fosforo_p text, ds_itens_ktv_p text, ds_itens_albumina_p text, ds_itens_pth_p text, nm_usuario_p text, nr_seq_unidade_p bigint, nr_seq_turno_p bigint, nr_seq_tratamento_p text, ie_dia_semana_p text, ie_pac_ativos_p text, ie_hemodialise_fim_p text, ie_pac_hemodialise_p text, dt_referencia_p timestamp, ie_diabetico_p text, ie_hipertenso_p text, qt_faixa_inicial_p bigint, qt_faixa_final_p bigint, nr_seq_exame_p bigint, qt_result_exame_1_p bigint, qt_result_exame_2_p bigint) AS $body$
DECLARE



/*Ordem:
1 -> Titulo
5 -> Data
*/
/* vetor */

type vetor is table of colunas index by integer;


dt_referencia_w		timestamp;
/* globais */

vetor_w				vetor;
vetor1_w			vetor;
ds_item_w			varchar(2000);
ds_item_ant_w		varchar(2000);
ivet				integer;
ind					integer;
nr_seq_nova_w		bigint;
ds_formula_w		varchar(2000);
ds_cores_w			varchar(2000);
ds_cor_item_w		varchar(20);
ds_cor_w			varchar(20);
nr_seq_exame_w		bigint;
nr_seq_apresent_w	bigint;
nr_sequencia_w		bigint;
qt_casas_decimais_w	bigint;
qt_valor_minimo_w	double precision;
qt_valor_maximo_w	double precision;
ds_cor_fora_padrao_w	varchar(15);
nr_seq_exame_grid_w		bigint;
cd_pessoa_fisica_w		varchar(10);
ie_exame_grid_controle_w varchar(1);
nr_seq_item_cont_w		bigint;
ie_exclui_reg_w			varchar(1) := 'N';


c01 CURSOR FOR
	SELECT	ds_item
	from	hd_item_controle_exame where	((coalesce(nr_seq_protocolo_p,0) = 0) or (Hd_obter_se_item_protocolo(nr_seq_protocolo_p,nr_sequencia) = 'S'))
	and	((coalesce(ds_itens_p::text, '') = '') or (OBTER_SE_CONTIDO(nr_sequencia,ds_itens_p) = 'S'))
	and 	nr_sequencia = ds_itens_p
	order by nr_seq_apresent, nr_sequencia LIMIT 49;

c1Teste1 CURSOR FOR
	SELECT	ds_item
	from	hd_item_controle_exame where	((coalesce(nr_seq_protocolo_p,0) = 0) or (Hd_obter_se_item_protocolo(nr_seq_protocolo_p,nr_sequencia) = 'S'))
	and	((coalesce(ds_itens_hb_p::text, '') = '') or (OBTER_SE_CONTIDO(nr_sequencia,ds_itens_hb_p) = 'S'))
	and 	nr_sequencia = ds_itens_hb_p
	order by nr_seq_apresent, nr_sequencia LIMIT 49;

c1Teste2 CURSOR FOR
	SELECT	ds_item
	from	hd_item_controle_exame where	((coalesce(nr_seq_protocolo_p,0) = 0) or (Hd_obter_se_item_protocolo(nr_seq_protocolo_p,nr_sequencia) = 'S'))
	and	((coalesce(ds_itens_fosforo_p::text, '') = '') or (OBTER_SE_CONTIDO(nr_sequencia,ds_itens_fosforo_p) = 'S'))
	and 	nr_sequencia = ds_itens_fosforo_p
	order by nr_seq_apresent, nr_sequencia LIMIT 49;

c1Teste3 CURSOR FOR
	SELECT	ds_item
	from	hd_item_controle_exame where	((coalesce(nr_seq_protocolo_p,0) = 0) or (Hd_obter_se_item_protocolo(nr_seq_protocolo_p,nr_sequencia) = 'S'))
	and	((coalesce(ds_itens_ktv_p::text, '') = '') or (OBTER_SE_CONTIDO(nr_sequencia,ds_itens_ktv_p) = 'S'))
	and 	nr_sequencia = ds_itens_ktv_p
	order by nr_seq_apresent, nr_sequencia LIMIT 49;
	
c1Teste4 CURSOR FOR
	SELECT	ds_item
	from	hd_item_controle_exame where	((coalesce(nr_seq_protocolo_p,0) = 0) or (Hd_obter_se_item_protocolo(nr_seq_protocolo_p,nr_sequencia) = 'S'))
	and	((coalesce(ds_itens_albumina_p::text, '') = '') or (OBTER_SE_CONTIDO(nr_sequencia,ds_itens_albumina_p) = 'S'))
	and 	nr_sequencia = ds_itens_albumina_p
	order by nr_seq_apresent, nr_sequencia LIMIT 49;
	
c1Teste5 CURSOR FOR
	SELECT	ds_item
	from	hd_item_controle_exame where	((coalesce(nr_seq_protocolo_p,0) = 0) or (Hd_obter_se_item_protocolo(nr_seq_protocolo_p,nr_sequencia) = 'S'))
	and	((coalesce(ds_itens_pth_p::text, '') = '') or (OBTER_SE_CONTIDO(nr_sequencia,ds_itens_pth_p) = 'S'))
	and 	nr_sequencia = ds_itens_pth_p
	order by nr_seq_apresent, nr_sequencia LIMIT 49;

------------------------------------------------------------------------------------------------
c02 CURSOR FOR
	SELECT	ds_formula,
		nr_seq_exame,
		nr_seq_apresent,
		ds_cor,
		coalesce(qt_casas_decimais,2) qt_casas_decimais,
		qt_valor_minimo,
		qt_valor_maximo	,
		ds_cor_fora_padrao,
		nr_sequencia
	from	hd_item_controle_exame where	((coalesce(nr_seq_protocolo_p,0) = 0) or (Hd_obter_se_item_protocolo(nr_seq_protocolo_p,nr_sequencia) = 'S'))
	and	((coalesce(ds_itens_p::text, '') = '') or (OBTER_SE_CONTIDO(nr_sequencia,ds_itens_p) = 'S'))
	and 	nr_sequencia = ds_itens_p
	order by nr_seq_apresent, nr_sequencia LIMIT 49;

c2Teste1 CURSOR FOR
	SELECT	ds_formula,
		nr_seq_exame,
		nr_seq_apresent,
		ds_cor,
		coalesce(qt_casas_decimais,2) qt_casas_decimais,
		qt_valor_minimo,
		qt_valor_maximo	,
		ds_cor_fora_padrao,
		nr_sequencia
	from	hd_item_controle_exame where	((coalesce(nr_seq_protocolo_p,0) = 0) or (Hd_obter_se_item_protocolo(nr_seq_protocolo_p,nr_sequencia) = 'S'))
	and	((coalesce(ds_itens_hb_p::text, '') = '') or (OBTER_SE_CONTIDO(nr_sequencia,ds_itens_hb_p) = 'S'))
	and 	nr_sequencia = ds_itens_hb_p
	order by nr_seq_apresent, nr_sequencia LIMIT 49;

c2Teste2 CURSOR FOR
	SELECT	ds_formula,
		nr_seq_exame,
		nr_seq_apresent,
		ds_cor,
		coalesce(qt_casas_decimais,2) qt_casas_decimais,
		qt_valor_minimo,
		qt_valor_maximo	,
		ds_cor_fora_padrao,
		nr_sequencia
	from	hd_item_controle_exame where	((coalesce(nr_seq_protocolo_p,0) = 0) or (Hd_obter_se_item_protocolo(nr_seq_protocolo_p,nr_sequencia) = 'S'))
	and	((coalesce(ds_itens_fosforo_p::text, '') = '') or (OBTER_SE_CONTIDO(nr_sequencia,ds_itens_fosforo_p) = 'S'))
	and 	nr_sequencia = ds_itens_fosforo_p
	order by nr_seq_apresent, nr_sequencia LIMIT 49;

c2Teste3 CURSOR FOR
	SELECT	ds_formula,
		nr_seq_exame,
		nr_seq_apresent,
		ds_cor,
		coalesce(qt_casas_decimais,2) qt_casas_decimais,
		qt_valor_minimo,
		qt_valor_maximo	,
		ds_cor_fora_padrao,
		nr_sequencia
	from	hd_item_controle_exame where	((coalesce(nr_seq_protocolo_p,0) = 0) or (Hd_obter_se_item_protocolo(nr_seq_protocolo_p,nr_sequencia) = 'S'))
	and	((coalesce(ds_itens_ktv_p::text, '') = '') or (OBTER_SE_CONTIDO(nr_sequencia,ds_itens_ktv_p) = 'S'))
	and 	nr_sequencia = ds_itens_ktv_p
	order by nr_seq_apresent, nr_sequencia LIMIT 49;

c2Teste4 CURSOR FOR
	SELECT	ds_formula,
		nr_seq_exame,
		nr_seq_apresent,
		ds_cor,
		coalesce(qt_casas_decimais,2) qt_casas_decimais,
		qt_valor_minimo,
		qt_valor_maximo	,
		ds_cor_fora_padrao,
		nr_sequencia
	from	hd_item_controle_exame where	((coalesce(nr_seq_protocolo_p,0) = 0) or (Hd_obter_se_item_protocolo(nr_seq_protocolo_p,nr_sequencia) = 'S'))
	and	((coalesce(ds_itens_albumina_p::text, '') = '') or (OBTER_SE_CONTIDO(nr_sequencia,ds_itens_albumina_p) = 'S'))
	and 	nr_sequencia = ds_itens_albumina_p
	order by nr_seq_apresent, nr_sequencia LIMIT 49;
	
c2Teste5 CURSOR FOR
	SELECT	ds_formula,
		nr_seq_exame,
		nr_seq_apresent,
		ds_cor,
		coalesce(qt_casas_decimais,2) qt_casas_decimais,
		qt_valor_minimo,
		qt_valor_maximo	,
		ds_cor_fora_padrao,
		nr_sequencia
	from	hd_item_controle_exame where	((coalesce(nr_seq_protocolo_p,0) = 0) or (Hd_obter_se_item_protocolo(nr_seq_protocolo_p,nr_sequencia) = 'S'))
	and	((coalesce(ds_itens_pth_p::text, '') = '') or (OBTER_SE_CONTIDO(nr_sequencia,ds_itens_pth_p) = 'S'))
	and 	nr_sequencia = ds_itens_pth_p
	order by nr_seq_apresent, nr_sequencia LIMIT 49;
	
---------------------------------------------------------------------------------------------------------------
c03 CURSOR FOR
	SELECT	distinct
		dt_referencia
	from	HD_EXAME_GRID_CONTROLE
	where	nm_usuario = nm_usuario_p
	and	ie_ordem = 5
	and	(dt_referencia IS NOT NULL AND dt_referencia::text <> '')
	and 	dt_referencia = dt_referencia_w
	order by dt_referencia desc;

c04 CURSOR FOR
	SELECT	cd_pessoa_fisica
	from	hd_pac_renal_cronico
	where (coalesce(hd_obter_unidade_prc(CD_PESSOA_FISICA,'C'),NR_SEQ_UNID_DIALISE) = NR_SEQ_UNIDADE_p or coalesce(NR_SEQ_UNIDADE_p,0) = 0)
	and (coalesce(nr_seq_turno_p,0) = 0
	or	HD_Obter_Turno_PRC_data(cd_pessoa_fisica,clock_timestamp(), 'C') = nr_seq_turno_p
	or 	hd_obter_se_pac_dia_turno(cd_pessoa_fisica,nr_seq_turno_p) = 'S')
	and (coalesce(nr_seq_tratamento_p::text, '') = ''
	or	obter_se_paciente_tratamento(cd_pessoa_fisica, nr_seq_tratamento_p) = 'S')
	and (coalesce(ie_dia_semana_p::text, '') = ''
	or	hd_obter_se_pac_dia_dialise(cd_pessoa_fisica, ie_dia_semana_p, 'C') = 'S'
	or   	hd_obter_se_pac_dia_semana(cd_pessoa_fisica, ie_dia_semana_p) = 'S' )
	and (ie_pac_ativos_p = 'N' or substr(hd_obter_se_paciente_ativo(cd_pessoa_fisica),1,1) = 'S')
	and (ie_hemodialise_fim_p = 'S' or hd_obter_se_em_dialise(cd_pessoa_fisica, 'F') = 'N')
	and (ie_pac_hemodialise_p = 'N' or hd_obter_se_em_dialise(cd_pessoa_fisica, 'E') = 'S')
	and (ie_diabetico_p = 'N' or ie_diabetico = ie_diabetico_p)
	and (ie_hipertenso_p = 'N' or ie_hipertenso = ie_hipertenso_p)
	and	Obter_Idade_PF(cd_pessoa_fisica, clock_timestamp(), 'A') >= coalesce(qt_faixa_inicial_p,0)
	and	Obter_Idade_PF(cd_pessoa_fisica, clock_timestamp(), 'A') <= coalesce(qt_faixa_final_p,999)
	order by cd_pessoa_fisica;
BEGIN

dt_referencia_w	:= trunc(dt_referencia_p,'month');

commit;

ivet	:= 0;
begin

for c01_w in c01 loop
	begin
	ds_item_w := c01_w.ds_item;
	vetor_w[1].nm_coluna_w := ds_item_w;
	end;
end loop;

---------------------------
for c1Teste1_w in c1Teste1 loop
	begin
	ds_item_w := c1Teste1_w.ds_item;
	vetor_w[2].nm_coluna_w := ds_item_w;
	end;
end loop;

------------------------------
for c1Teste2_w in c1Teste2 loop
	begin
	ds_item_w := c1Teste2_w.ds_item;
	vetor_w[3].nm_coluna_w := ds_item_w;
	end;
end loop;

---------------------------
for c1Teste3_w in c1Teste3 loop
	begin
	ds_item_w := c1Teste3_w.ds_item;
	vetor_w[4].nm_coluna_w := ds_item_w;
	end;
end loop;

------------------------------
for c1Teste4_w in c1Teste4 loop
	begin
	ds_item_w := c1Teste4_w.ds_item;
	vetor_w[5].nm_coluna_w := ds_item_w;
	end;
end loop;

------------------------------
for c1Teste5_w in c1Teste5 loop
	begin
	ds_item_w := c1Teste5_w.ds_item;
	vetor_w[6].nm_coluna_w := ds_item_w;
	end;
end loop;
end;
------------------------------------------------------------
ind := ivet;
while(ind < 7) loop
	begin
	ind := ind + 1;
	vetor_w[ind].nm_coluna_w := null;
	end;
end loop;

select	nextval('hd_grid_controle_dialise_seq')
into STRICT	nr_sequencia_w
;
------------------------------------------------------------
insert into HD_GRID_CONTROLE_DIALISE(
	nr_sequencia,
	nm_usuario,
	ie_ordem,
	dt_referencia,
	ds_resultado1,
	ds_resultado2,
	ds_resultado3,
	ds_resultado4,
	ds_resultado5,
	ds_resultado6,
	dt_atualizacao)
Values (
	nr_sequencia_w,
	nm_usuario_p,
	1,
	dt_referencia_w,
	vetor_w[1].nm_coluna_w,
	vetor_w[2].nm_coluna_w,
	vetor_w[3].nm_coluna_w,
	vetor_w[4].nm_coluna_w,
	vetor_w[5].nm_coluna_w,
	vetor_w[6].nm_coluna_w,
	clock_timestamp());
--------------------------
for c04_w in c04 loop
	begin
	cd_pessoa_fisica_w := c04_w.cd_pessoa_fisica;

	select	CASE WHEN count(*)=0 THEN 'N'  ELSE 'S' END
	into STRICT	ie_exame_grid_controle_w
	from	HD_EXAME_GRID_CONTROLE
	where	nm_usuario = nm_usuario_p
	and	ie_ordem = 5
	and	(dt_referencia IS NOT NULL AND dt_referencia::text <> '')
	and	dt_referencia = dt_referencia_w;

	if (ie_exame_grid_controle_w = 'N') then
		insert into HD_EXAME_GRID_CONTROLE(
			nr_sequencia,
			nm_usuario,
			ie_ordem,
			dt_referencia,
			cd_pessoa_fisica,
			dt_atualizacao)
		Values (
			nextval('hd_exame_grid_controle_seq'),
			nm_usuario_p,
			5,
			dt_referencia_w,
			cd_pessoa_fisica_w,
			clock_timestamp());
	end if;

	select 	nextval('hd_grid_controle_dialise_seq')
	into STRICT	nr_seq_nova_w
	;

	insert into HD_GRID_CONTROLE_DIALISE(
		nr_sequencia,
		nm_usuario,
		ie_ordem,
		dt_referencia,
		cd_pessoa_fisica,
		dt_atualizacao)
	Values ( nr_seq_nova_w,
		nm_usuario_p,
		5,
		dt_referencia_w,
		cd_pessoa_fisica_w,
		clock_timestamp());

	ind := 0;
	ds_cores_w	:= null;

	for c02_w in c02 loop
		begin
		ds_formula_w           := c02_w.ds_formula;
		nr_seq_exame_w         := c02_w.nr_seq_exame;
		nr_seq_apresent_w      := c02_w.nr_seq_apresent;
		ds_cor_item_w          := c02_w.ds_cor;
		qt_casas_decimais_w    := c02_w.qt_casas_decimais;
		qt_valor_minimo_w      := c02_w.qt_valor_minimo;
		qt_valor_maximo_w      := c02_w.qt_valor_maximo;
		ds_cor_fora_padrao_w   := c02_w.ds_cor_fora_padrao;
		nr_seq_item_cont_w     := c02_w.nr_sequencia;

		ds_item_w	:= 0;

		for c03_w in c03 loop
			begin
			dt_referencia_w := c03_w.dt_referencia;

			select	max(nr_sequencia)
			into STRICT	nr_seq_exame_grid_w
			from	HD_EXAME_GRID_CONTROLE
			where	nm_usuario = nm_usuario_p
			and	ie_ordem = 5
			and	dt_referencia = dt_referencia_w;

			if (nr_seq_exame_w IS NOT NULL AND nr_seq_exame_w::text <> '') then

				ds_item_w	:= 	obter_ult_resultado_data_lab(cd_pessoa_fisica_w, nr_seq_exame_grid_w, nr_seq_exame_w);
				ds_item_ant_w	:= 	ds_item_w;

				begin
				select	campo_mascara_virgula_casas((ds_item_w)::numeric , qt_casas_decimais_w)
				into STRICT	ds_item_w
				;
				exception
					when others then
					ds_item_w := ds_item_ant_w;
				end;

			elsif (ds_formula_w IS NOT NULL AND ds_formula_w::text <> '') then

				ds_item_w	:= 	hd_obter_formula(cd_pessoa_fisica_w, nr_seq_exame_grid_w, ds_formula_w,qt_casas_decimais_w,'N');

			end if;

			begin
			if (trim(both ds_item_w) <> '-') and (coalesce(hd_somente_numero(ds_item_w),-1) <> 0) then
				exit;
			end if;
			exception
			when others then
				if (coalesce(obter_somente_numero(ds_item_w),-1) <> 0) then
					exit;
				end if;
			end;

			end;
		end loop;

		ind := ind + 1;
		vetor1_w[ind].nm_coluna_w := ds_item_w;

		begin
		select	CASE WHEN length(to_char(ind))=1 THEN '0'||to_char(ind)  ELSE to_char(ind) END
		into STRICT	ds_cor_w
		;



		if ((ds_item_w)::numeric  < qt_valor_minimo_w) or ((ds_item_w)::numeric  > qt_valor_maximo_w) and (ds_cor_fora_padrao_w IS NOT NULL AND ds_cor_fora_padrao_w::text <> '') then
			ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_fora_padrao_w ||',';
		else
			ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_item_w ||',';
		end if;
		exception
			WHEN OTHERS THEN
				ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_item_w ||',';
		end;


		if (nr_seq_exame_p IS NOT NULL AND nr_seq_exame_p::text <> '') and (qt_result_exame_1_p IS NOT NULL AND qt_result_exame_1_p::text <> '') and (qt_result_exame_2_p IS NOT NULL AND qt_result_exame_2_p::text <> '') and (nr_seq_exame_p = nr_seq_item_cont_w) and ((obter_somente_numero(ds_item_w) IS NOT NULL AND (obter_somente_numero(ds_item_w))::text <> '')) and (hd_somente_numero(ds_item_w) >= qt_result_exame_1_p) and (hd_somente_numero(ds_item_w) <= qt_result_exame_2_p)then
			ie_exclui_reg_w := 'N';
		elsif (nr_seq_exame_p = nr_seq_item_cont_w) and (nr_seq_exame_p IS NOT NULL AND nr_seq_exame_p::text <> '') and (qt_result_exame_1_p IS NOT NULL AND qt_result_exame_1_p::text <> '') and (qt_result_exame_2_p IS NOT NULL AND qt_result_exame_2_p::text <> '') then
			ie_exclui_reg_w := 'S';
		end if;

		end;
	end loop;

------------------------------------
	for c2Teste1_w in c2Teste1 loop
		begin
		ds_formula_w          := c2Teste1_w.ds_formula;
		nr_seq_exame_w        := c2Teste1_w.nr_seq_exame;
		nr_seq_apresent_w     := c2Teste1_w.nr_seq_apresent;
		ds_cor_item_w         := c2Teste1_w.ds_cor;
		qt_casas_decimais_w   := c2Teste1_w.qt_casas_decimais;
		qt_valor_minimo_w     := c2Teste1_w.qt_valor_minimo;
		qt_valor_maximo_w     := c2Teste1_w.qt_valor_maximo;
		ds_cor_fora_padrao_w  := c2Teste1_w.ds_cor_fora_padrao;
		nr_seq_item_cont_w    := c2Teste1_w.nr_sequencia;

		ds_item_w	:= 0;

		for c03_w in c03 loop
			begin
			dt_referencia_w := c03_w.dt_referencia;

			select	max(nr_sequencia)
			into STRICT	nr_seq_exame_grid_w
			from	HD_EXAME_GRID_CONTROLE
			where	nm_usuario = nm_usuario_p
			and	ie_ordem = 5
			and	dt_referencia = dt_referencia_w;

			if (nr_seq_exame_w IS NOT NULL AND nr_seq_exame_w::text <> '') then

				ds_item_w	:= 	obter_ult_resultado_data_lab(cd_pessoa_fisica_w, nr_seq_exame_grid_w, nr_seq_exame_w);
				ds_item_ant_w	:= 	ds_item_w;

				begin
				select	campo_mascara_virgula_casas((ds_item_w)::numeric , qt_casas_decimais_w)
				into STRICT	ds_item_w
				;
				exception
					when others then
					ds_item_w := ds_item_ant_w;
				end;

			elsif (ds_formula_w IS NOT NULL AND ds_formula_w::text <> '') then

				ds_item_w	:= 	hd_obter_formula(cd_pessoa_fisica_w, nr_seq_exame_grid_w, ds_formula_w,qt_casas_decimais_w,'N');

			end if;

			begin
			if (trim(both ds_item_w) <> '-') and (coalesce(hd_somente_numero(ds_item_w),-1) <> 0) then
				exit;
			end if;
			exception
			when others then
				if (coalesce(obter_somente_numero(ds_item_w),-1) <> 0) then
					exit;
				end if;
			end;


			end;
		end loop;

		ind := ind + 1;
		vetor1_w[ind].nm_coluna_w := ds_item_w;

		begin
		select	CASE WHEN length(to_char(ind))=1 THEN '0'||to_char(ind)  ELSE to_char(ind) END
		into STRICT	ds_cor_w
		;



		if ((ds_item_w)::numeric  < qt_valor_minimo_w) or ((ds_item_w)::numeric  > qt_valor_maximo_w) and (ds_cor_fora_padrao_w IS NOT NULL AND ds_cor_fora_padrao_w::text <> '') then
			ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_fora_padrao_w ||',';
		else
			ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_item_w ||',';
		end if;
		exception
			WHEN OTHERS THEN
				ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_item_w ||',';
		end;


		if (nr_seq_exame_p IS NOT NULL AND nr_seq_exame_p::text <> '') and (qt_result_exame_1_p IS NOT NULL AND qt_result_exame_1_p::text <> '') and (qt_result_exame_2_p IS NOT NULL AND qt_result_exame_2_p::text <> '') and (nr_seq_exame_p = nr_seq_item_cont_w) and ((obter_somente_numero(ds_item_w) IS NOT NULL AND (obter_somente_numero(ds_item_w))::text <> '')) and (hd_somente_numero(ds_item_w) >= qt_result_exame_1_p) and (hd_somente_numero(ds_item_w) <= qt_result_exame_2_p)then
			ie_exclui_reg_w := 'N';
		elsif (nr_seq_exame_p = nr_seq_item_cont_w) and (nr_seq_exame_p IS NOT NULL AND nr_seq_exame_p::text <> '') and (qt_result_exame_1_p IS NOT NULL AND qt_result_exame_1_p::text <> '') and (qt_result_exame_2_p IS NOT NULL AND qt_result_exame_2_p::text <> '') then
			ie_exclui_reg_w := 'S';
		end if;



		end;
	end loop;

------------------------------------
	for c2Teste2_w in c2Teste2 loop
		begin
		ds_formula_w          := c2Teste2_w.ds_formula;
		nr_seq_exame_w        := c2Teste2_w.nr_seq_exame;
		nr_seq_apresent_w     := c2Teste2_w.nr_seq_apresent;
		ds_cor_item_w         := c2Teste2_w.ds_cor;
		qt_casas_decimais_w   := c2Teste2_w.qt_casas_decimais;
		qt_valor_minimo_w     := c2Teste2_w.qt_valor_minimo;
		qt_valor_maximo_w     := c2Teste2_w.qt_valor_maximo;
		ds_cor_fora_padrao_w  := c2Teste2_w.ds_cor_fora_padrao;
		nr_seq_item_cont_w    := c2Teste2_w.nr_sequencia;

		ds_item_w	:= 0;

		for c03_w in c03 loop
			begin
			dt_referencia_w := c03_w.dt_referencia;

			select	max(nr_sequencia)
			into STRICT	nr_seq_exame_grid_w
			from	HD_EXAME_GRID_CONTROLE
			where	nm_usuario = nm_usuario_p
			and	ie_ordem = 5
			and	dt_referencia = dt_referencia_w;

			if (nr_seq_exame_w IS NOT NULL AND nr_seq_exame_w::text <> '') then

				ds_item_w	:= 	obter_ult_resultado_data_lab(cd_pessoa_fisica_w, nr_seq_exame_grid_w, nr_seq_exame_w);
				ds_item_ant_w	:= 	ds_item_w;

				begin
				select	campo_mascara_virgula_casas((ds_item_w)::numeric , qt_casas_decimais_w)
				into STRICT	ds_item_w
				;
				exception
					when others then
					ds_item_w := ds_item_ant_w;
				end;

			elsif (ds_formula_w IS NOT NULL AND ds_formula_w::text <> '') then

				ds_item_w	:= 	hd_obter_formula(cd_pessoa_fisica_w, nr_seq_exame_grid_w, ds_formula_w,qt_casas_decimais_w,'N');

			end if;

			begin
			if (trim(both ds_item_w) <> '-') and (coalesce(hd_somente_numero(ds_item_w),-1) <> 0) then
				exit;
			end if;
			exception
			when others then
				if (coalesce(obter_somente_numero(ds_item_w),-1) <> 0) then
					exit;
				end if;
			end;


			end;
		end loop;

		ind := ind + 1;
		vetor1_w[ind].nm_coluna_w := ds_item_w;

		begin
		select	CASE WHEN length(to_char(ind))=1 THEN '0'||to_char(ind)  ELSE to_char(ind) END
		into STRICT	ds_cor_w
		;
		

		if ((ds_item_w)::numeric  < qt_valor_minimo_w) or ((ds_item_w)::numeric  > qt_valor_maximo_w) and (ds_cor_fora_padrao_w IS NOT NULL AND ds_cor_fora_padrao_w::text <> '') then
			ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_fora_padrao_w ||',';
		else
			ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_item_w ||',';
		end if;
		exception
			WHEN OTHERS THEN
				ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_item_w ||',';
		end;


		if (nr_seq_exame_p IS NOT NULL AND nr_seq_exame_p::text <> '') and (qt_result_exame_1_p IS NOT NULL AND qt_result_exame_1_p::text <> '') and (qt_result_exame_2_p IS NOT NULL AND qt_result_exame_2_p::text <> '') and (nr_seq_exame_p = nr_seq_item_cont_w) and ((obter_somente_numero(ds_item_w) IS NOT NULL AND (obter_somente_numero(ds_item_w))::text <> '')) and (hd_somente_numero(ds_item_w) >= qt_result_exame_1_p) and (hd_somente_numero(ds_item_w) <= qt_result_exame_2_p)then
			ie_exclui_reg_w := 'N';
		elsif (nr_seq_exame_p = nr_seq_item_cont_w) and (nr_seq_exame_p IS NOT NULL AND nr_seq_exame_p::text <> '') and (qt_result_exame_1_p IS NOT NULL AND qt_result_exame_1_p::text <> '') and (qt_result_exame_2_p IS NOT NULL AND qt_result_exame_2_p::text <> '') then
			ie_exclui_reg_w := 'S';
		end if;

		end;
	end loop;

------------------------------------
	for c2Teste3_w in c2Teste3 loop
		begin
		ds_formula_w           := c2Teste3_w.ds_formula;
		nr_seq_exame_w         := c2Teste3_w.nr_seq_exame;
		nr_seq_apresent_w      := c2Teste3_w.nr_seq_apresent;
		ds_cor_item_w          := c2Teste3_w.ds_cor;
		qt_casas_decimais_w    := c2Teste3_w.qt_casas_decimais;
		qt_valor_minimo_w      := c2Teste3_w.qt_valor_minimo;
		qt_valor_maximo_w      := c2Teste3_w.qt_valor_maximo;
		ds_cor_fora_padrao_w   := c2Teste3_w.ds_cor_fora_padrao;
		nr_seq_item_cont_w     := c2Teste3_w.nr_sequencia;

		ds_item_w	:= 0;

		for c03_w in c03 loop
			begin
			dt_referencia_w := c03_w.dt_referencia;

			select	max(nr_sequencia)
			into STRICT	nr_seq_exame_grid_w
			from	HD_EXAME_GRID_CONTROLE
			where	nm_usuario = nm_usuario_p
			and	ie_ordem = 5
			and	dt_referencia = dt_referencia_w;

			if (nr_seq_exame_w IS NOT NULL AND nr_seq_exame_w::text <> '') then

				ds_item_w	:= 	obter_ult_resultado_data_lab(cd_pessoa_fisica_w, nr_seq_exame_grid_w, nr_seq_exame_w);
				ds_item_ant_w	:= 	ds_item_w;

				begin
				select	campo_mascara_virgula_casas((ds_item_w)::numeric , qt_casas_decimais_w)
				into STRICT	ds_item_w
				;
				exception
					when others then
					ds_item_w := ds_item_ant_w;
				end;

			elsif (ds_formula_w IS NOT NULL AND ds_formula_w::text <> '') then

				ds_item_w	:= 	hd_obter_formula(cd_pessoa_fisica_w, nr_seq_exame_grid_w, ds_formula_w,qt_casas_decimais_w,'N');

			end if;

			begin
			if (trim(both ds_item_w) <> '-') and (coalesce(hd_somente_numero(ds_item_w),-1) <> 0) then
				exit;
			end if;
			exception
			when others then
				if (coalesce(obter_somente_numero(ds_item_w),-1) <> 0) then
					exit;
				end if;
			end;


			end;
		end loop;

		ind := ind + 1;
		vetor1_w[ind].nm_coluna_w := ds_item_w;

		begin
		select	CASE WHEN length(to_char(ind))=1 THEN '0'||to_char(ind)  ELSE to_char(ind) END
		into STRICT	ds_cor_w
		;


		if ((ds_item_w)::numeric  < qt_valor_minimo_w) or ((ds_item_w)::numeric  > qt_valor_maximo_w) and (ds_cor_fora_padrao_w IS NOT NULL AND ds_cor_fora_padrao_w::text <> '') then
			ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_fora_padrao_w ||',';
		else
			ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_item_w ||',';
		end if;
		exception
			WHEN OTHERS THEN
				ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_item_w ||',';
		end;


		if (nr_seq_exame_p IS NOT NULL AND nr_seq_exame_p::text <> '') and (qt_result_exame_1_p IS NOT NULL AND qt_result_exame_1_p::text <> '') and (qt_result_exame_2_p IS NOT NULL AND qt_result_exame_2_p::text <> '') and (nr_seq_exame_p = nr_seq_item_cont_w) and ((obter_somente_numero(ds_item_w) IS NOT NULL AND (obter_somente_numero(ds_item_w))::text <> '')) and (hd_somente_numero(ds_item_w) >= qt_result_exame_1_p) and (hd_somente_numero(ds_item_w) <= qt_result_exame_2_p)then
			ie_exclui_reg_w := 'N';
		elsif (nr_seq_exame_p = nr_seq_item_cont_w) and (nr_seq_exame_p IS NOT NULL AND nr_seq_exame_p::text <> '') and (qt_result_exame_1_p IS NOT NULL AND qt_result_exame_1_p::text <> '') and (qt_result_exame_2_p IS NOT NULL AND qt_result_exame_2_p::text <> '') then
			ie_exclui_reg_w := 'S';
		end if;

		end;
	end loop;

------------------------------------
	for c2Teste4_w in c2Teste4 loop
		begin
		ds_formula_w           := c2Teste4_w.ds_formula;
		nr_seq_exame_w         := c2Teste4_w.nr_seq_exame;
		nr_seq_apresent_w      := c2Teste4_w.nr_seq_apresent;
		ds_cor_item_w          := c2Teste4_w.ds_cor;
		qt_casas_decimais_w    := c2Teste4_w.qt_casas_decimais;
		qt_valor_minimo_w      := c2Teste4_w.qt_valor_minimo;
		qt_valor_maximo_w      := c2Teste4_w.qt_valor_maximo;
		ds_cor_fora_padrao_w   := c2Teste4_w.ds_cor_fora_padrao;
		nr_seq_item_cont_w     := c2Teste4_w.nr_sequencia;

		ds_item_w	:= 0;

		for c03_w in c03 loop
			begin
			dt_referencia_w := c03_w.dt_referencia;

			select	max(nr_sequencia)
			into STRICT	nr_seq_exame_grid_w
			from	HD_EXAME_GRID_CONTROLE
			where	nm_usuario = nm_usuario_p
			and	ie_ordem = 5
			and	dt_referencia = dt_referencia_w;

			if (nr_seq_exame_w IS NOT NULL AND nr_seq_exame_w::text <> '') then

				ds_item_w	:= 	obter_ult_resultado_data_lab(cd_pessoa_fisica_w, nr_seq_exame_grid_w, nr_seq_exame_w);
				ds_item_ant_w	:= 	ds_item_w;

				begin
				select	campo_mascara_virgula_casas((ds_item_w)::numeric , qt_casas_decimais_w)
				into STRICT	ds_item_w
				;
				exception
					when others then
					ds_item_w := ds_item_ant_w;
				end;

			elsif (ds_formula_w IS NOT NULL AND ds_formula_w::text <> '') then

				ds_item_w	:= 	hd_obter_formula(cd_pessoa_fisica_w, nr_seq_exame_grid_w, ds_formula_w,qt_casas_decimais_w,'N');

			end if;

			begin
			if (trim(both ds_item_w) <> '-') and (coalesce(hd_somente_numero(ds_item_w),-1) <> 0) then
				exit;
			end if;
			exception
			when others then
				if (coalesce(obter_somente_numero(ds_item_w),-1) <> 0) then
					exit;
				end if;
			end;


			end;
		end loop;

		ind := ind + 1;
		vetor1_w[ind].nm_coluna_w := ds_item_w;

		begin
		select	CASE WHEN length(to_char(ind))=1 THEN '0'||to_char(ind)  ELSE to_char(ind) END
		into STRICT	ds_cor_w
		;


		if ((ds_item_w)::numeric  < qt_valor_minimo_w) or ((ds_item_w)::numeric  > qt_valor_maximo_w) and (ds_cor_fora_padrao_w IS NOT NULL AND ds_cor_fora_padrao_w::text <> '') then
			ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_fora_padrao_w ||',';
		else
			ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_item_w ||',';
		end if;
		exception
			WHEN OTHERS THEN
				ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_item_w ||',';
		end;


		if (nr_seq_exame_p IS NOT NULL AND nr_seq_exame_p::text <> '') and (qt_result_exame_1_p IS NOT NULL AND qt_result_exame_1_p::text <> '') and (qt_result_exame_2_p IS NOT NULL AND qt_result_exame_2_p::text <> '') and (nr_seq_exame_p = nr_seq_item_cont_w) and ((obter_somente_numero(ds_item_w) IS NOT NULL AND (obter_somente_numero(ds_item_w))::text <> '')) and (hd_somente_numero(ds_item_w) >= qt_result_exame_1_p) and (hd_somente_numero(ds_item_w) <= qt_result_exame_2_p)then
			ie_exclui_reg_w := 'N';
		elsif (nr_seq_exame_p = nr_seq_item_cont_w) and (nr_seq_exame_p IS NOT NULL AND nr_seq_exame_p::text <> '') and (qt_result_exame_1_p IS NOT NULL AND qt_result_exame_1_p::text <> '') and (qt_result_exame_2_p IS NOT NULL AND qt_result_exame_2_p::text <> '') then
			ie_exclui_reg_w := 'S';
		end if;

		end;
	end loop;
	
------------------------------------
	for c2Teste5_w in c2Teste5 loop
		begin
		ds_formula_w           := c2Teste5_w.ds_formula;
		nr_seq_exame_w         := c2Teste5_w.nr_seq_exame;
		nr_seq_apresent_w      := c2Teste5_w.nr_seq_apresent;
		ds_cor_item_w          := c2Teste5_w.ds_cor;
		qt_casas_decimais_w    := c2Teste5_w.qt_casas_decimais;
		qt_valor_minimo_w      := c2Teste5_w.qt_valor_minimo;
		qt_valor_maximo_w      := c2Teste5_w.qt_valor_maximo;
		ds_cor_fora_padrao_w   := c2Teste5_w.ds_cor_fora_padrao;
		nr_seq_item_cont_w     := c2Teste5_w.nr_sequencia;

		ds_item_w	:= 0;

		for c03_w in c03 loop
			begin
			dt_referencia_w := c03_w.dt_referencia;

			select	max(nr_sequencia)
			into STRICT	nr_seq_exame_grid_w
			from	HD_EXAME_GRID_CONTROLE
			where	nm_usuario = nm_usuario_p
			and	ie_ordem = 5
			and	dt_referencia = dt_referencia_w;

			if (nr_seq_exame_w IS NOT NULL AND nr_seq_exame_w::text <> '') then

				ds_item_w	:= 	obter_ult_resultado_data_lab(cd_pessoa_fisica_w, nr_seq_exame_grid_w, nr_seq_exame_w);
				ds_item_ant_w	:= 	ds_item_w;

				begin
				select	campo_mascara_virgula_casas((ds_item_w)::numeric , qt_casas_decimais_w)
				into STRICT	ds_item_w
				;
				exception
					when others then
					ds_item_w := ds_item_ant_w;
				end;

			elsif (ds_formula_w IS NOT NULL AND ds_formula_w::text <> '') then

				ds_item_w	:= 	hd_obter_formula(cd_pessoa_fisica_w, nr_seq_exame_grid_w, ds_formula_w,qt_casas_decimais_w,'N');

			end if;

			begin
			if (trim(both ds_item_w) <> '-') and (coalesce(hd_somente_numero(ds_item_w),-1) <> 0) then
				exit;
			end if;
			exception
			when others then
				if (coalesce(obter_somente_numero(ds_item_w),-1) <> 0) then
					exit;
				end if;
			end;

			end;
		end loop;

		ind := ind + 1;
		vetor1_w[ind].nm_coluna_w := ds_item_w;

		begin
		select	CASE WHEN length(to_char(ind))=1 THEN '0'||to_char(ind)  ELSE to_char(ind) END
		into STRICT	ds_cor_w
		;


		if ((ds_item_w)::numeric  < qt_valor_minimo_w) or ((ds_item_w)::numeric  > qt_valor_maximo_w) and (ds_cor_fora_padrao_w IS NOT NULL AND ds_cor_fora_padrao_w::text <> '') then
			ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_fora_padrao_w ||',';
		else
			ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_item_w ||',';
		end if;
		exception
			WHEN OTHERS THEN
				ds_cores_w	:= ds_cores_w || ds_cor_w || ds_cor_item_w ||',';
		end;


		if (nr_seq_exame_p IS NOT NULL AND nr_seq_exame_p::text <> '') and (qt_result_exame_1_p IS NOT NULL AND qt_result_exame_1_p::text <> '') and (qt_result_exame_2_p IS NOT NULL AND qt_result_exame_2_p::text <> '') and (nr_seq_exame_p = nr_seq_item_cont_w) and ((obter_somente_numero(ds_item_w) IS NOT NULL AND (obter_somente_numero(ds_item_w))::text <> '')) and (hd_somente_numero(ds_item_w) >= qt_result_exame_1_p) and (hd_somente_numero(ds_item_w) <= qt_result_exame_2_p)then
			ie_exclui_reg_w := 'N';
		elsif (nr_seq_exame_p = nr_seq_item_cont_w) and (nr_seq_exame_p IS NOT NULL AND nr_seq_exame_p::text <> '') and (qt_result_exame_1_p IS NOT NULL AND qt_result_exame_1_p::text <> '') and (qt_result_exame_2_p IS NOT NULL AND qt_result_exame_2_p::text <> '') then
			ie_exclui_reg_w := 'S';
		end if;

		end;
	end loop;


	ind := ind;
	while(ind < 50) loop
		begin
		ind := ind + 1;
		vetor1_w[ind].nm_coluna_w := null;
		end;
	end loop;

	UPDATE 	HD_GRID_CONTROLE_DIALISE
	SET	ds_cor		= ds_cores_w,
		ds_resultado1	= vetor1_w[1].nm_coluna_w,
		ds_resultado2	= vetor1_w[2].nm_coluna_w,
		ds_resultado3	= vetor1_w[3].nm_coluna_w,
		ds_resultado4	= vetor1_w[4].nm_coluna_w,
		ds_resultado5	= vetor1_w[5].nm_coluna_w,
		ds_resultado6	= vetor1_w[6].nm_coluna_w,
		ds_resultado7	= vetor1_w[7].nm_coluna_w,
		ds_resultado8	= vetor1_w[8].nm_coluna_w,
		ds_resultado9	= vetor1_w[9].nm_coluna_w,
		ds_resultado10	= vetor1_w[10].nm_coluna_w,
		ds_resultado11	= vetor1_w[11].nm_coluna_w,
		ds_resultado12	= vetor1_w[12].nm_coluna_w,
		ds_resultado13	= vetor1_w[13].nm_coluna_w,
		ds_resultado14	= vetor1_w[14].nm_coluna_w,
		ds_resultado15	= vetor1_w[15].nm_coluna_w,
		ds_resultado16	= vetor1_w[16].nm_coluna_w,
		ds_resultado17	= vetor1_w[17].nm_coluna_w,
		ds_resultado18	= vetor1_w[18].nm_coluna_w,
		ds_resultado19	= vetor1_w[19].nm_coluna_w,
		ds_resultado20	= vetor1_w[20].nm_coluna_w,
		ds_resultado21	= vetor1_w[21].nm_coluna_w,
		ds_resultado22	= vetor1_w[22].nm_coluna_w,
		ds_resultado23	= vetor1_w[23].nm_coluna_w,
		ds_resultado24	= vetor1_w[24].nm_coluna_w,
		ds_resultado25	= vetor1_w[25].nm_coluna_w,
		ds_resultado26	= vetor1_w[26].nm_coluna_w,
		ds_resultado27	= vetor1_w[27].nm_coluna_w,
		ds_resultado28	= vetor1_w[28].nm_coluna_w,
		ds_resultado29	= vetor1_w[29].nm_coluna_w,
		ds_resultado30	= vetor1_w[30].nm_coluna_w,
		ds_resultado31	= vetor1_w[31].nm_coluna_w,
		ds_resultado32	= vetor1_w[32].nm_coluna_w,
		ds_resultado33	= vetor1_w[33].nm_coluna_w,
		ds_resultado34	= vetor1_w[34].nm_coluna_w,
		ds_resultado35	= vetor1_w[35].nm_coluna_w,
		ds_resultado36	= vetor1_w[36].nm_coluna_w,
		ds_resultado37	= vetor1_w[37].nm_coluna_w,
		ds_resultado38	= vetor1_w[38].nm_coluna_w,
		ds_resultado39	= vetor1_w[39].nm_coluna_w,
		ds_resultado40	= vetor1_w[40].nm_coluna_w,
		ds_resultado41	= vetor1_w[41].nm_coluna_w,
		ds_resultado42	= vetor1_w[42].nm_coluna_w,
		ds_resultado43	= vetor1_w[43].nm_coluna_w,
		ds_resultado44	= vetor1_w[44].nm_coluna_w,
		ds_resultado45	= vetor1_w[45].nm_coluna_w,
		ds_resultado46	= vetor1_w[46].nm_coluna_w,
		ds_resultado47	= vetor1_w[47].nm_coluna_w,
		ds_resultado48	= vetor1_w[48].nm_coluna_w,
		ds_resultado49	= vetor1_w[49].nm_coluna_w,
		ds_resultado50	= vetor1_w[50].nm_coluna_w
	WHERE	NR_SEQUENCIA = nr_seq_nova_w;

	end;
end loop;

commit;
end;
$body$
LANGUAGE PLPGSQL
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE hd_gerar_contr_dial_rel_6 ( nr_seq_protocolo_p bigint, ds_itens_p text, ds_itens_hb_p text, ds_itens_fosforo_p text, ds_itens_ktv_p text, ds_itens_albumina_p text, ds_itens_pth_p text, nm_usuario_p text, nr_seq_unidade_p bigint, nr_seq_turno_p bigint, nr_seq_tratamento_p text, ie_dia_semana_p text, ie_pac_ativos_p text, ie_hemodialise_fim_p text, ie_pac_hemodialise_p text, dt_referencia_p timestamp, ie_diabetico_p text, ie_hipertenso_p text, qt_faixa_inicial_p bigint, qt_faixa_final_p bigint, nr_seq_exame_p bigint, qt_result_exame_1_p bigint, qt_result_exame_2_p bigint) FROM PUBLIC;

