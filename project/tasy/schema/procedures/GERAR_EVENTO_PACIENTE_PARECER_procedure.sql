...
CREATE OR REPLACE PROCEDURE gerar_evento_paciente_parecer (nr_seq_evento_p bigint, nr_atendimento_p bigint, cd_pessoa_fisica_p text, nr_seq_laudo_p bigint, cd_evolucao_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
union
	...
	and	(a.CD_MEDICO_PARECER IS NOT NULL AND a.CD_MEDICO_PARECER::text <> '') 
...
union
	...
	select	'2' ie_forma_ev, 
		...
		null, 
		null 
	from	PESSOA_DESTINO_EVOLUCAO a 
	where	cd_evolucao	= cd_evolucao_p 
...
select 	max(a.CD_MEDICO_PARECER) 
into STRICT	cd_medico_parecer_w 
from	evolucao_paciente a 
where	cd_evolucao	= cd_evolucao_p 
and		(a.CD_MEDICO_PARECER IS NOT NULL AND a.CD_MEDICO_PARECER::text <> '');
...
cd_setor_atend_pac_w	:= substr(obter_unidade_atendimento(nr_atendimento_p,'A','CS'),1,60);
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (ie_pessoa_destino_w = '1') then /* Médico do atendimento */
		...
		cd_pessoa_destino_w	:= cd_medico_parecer_w;
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_evento_paciente_parecer (nr_seq_evento_p bigint, nr_atendimento_p bigint, cd_pessoa_fisica_p text, nr_seq_laudo_p bigint, cd_evolucao_p bigint, nm_usuario_p text) FROM PUBLIC;
