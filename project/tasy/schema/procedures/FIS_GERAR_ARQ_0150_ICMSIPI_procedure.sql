...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_0150_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
fetch c_reg_0150 bulk collect into vetReg0150 limit 1000;
	...
	ds_linha_w := substr(ds_sep_w || vetReg0150[i].cd_reg ||
			     ds_sep_w || vetReg0150[i].cd_part ||
			     ds_sep_w || vetReg0150[i].ds_nome ||
			     ds_sep_w || vetReg0150[i].cd_pais ||
			     ds_sep_w || vetReg0150[i].cd_cnpj ||
			     ds_sep_w || vetReg0150[i].cd_cpf ||
			     ds_sep_w || vetReg0150[i].nr_ie ||
			     ds_sep_w || vetReg0150[i].cd_mun ||
			     ds_sep_w || vetReg0150[i].ds_suframa ||
			     ds_sep_w || vetReg0150[i].ds_end ||
			     ds_sep_w || vetReg0150[i].nr_num ||
			     ds_sep_w || vetReg0150[i].ds_compl ||
			     ds_sep_w || vetReg0150[i].ds_bairro || ds_sep_w,
			     1,
			     8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro		:=  vetReg0150[i].cd_reg;

	-------> 0175
	/* Gera registros do 0175*/

        nr_linha_w := fis_gerar_arq_0175_icmsipi(nr_seq_controle_p, vetReg0150[i].nr_sequencia, nr_linha_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_0150_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) FROM PUBLIC;
