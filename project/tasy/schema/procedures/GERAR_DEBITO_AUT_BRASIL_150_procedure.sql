...
CREATE OR REPLACE PROCEDURE gerar_debito_aut_brasil_150 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
c01 CURSOR FOR 
	SELECT	ie_tipo_registro, 
		...
		ie_tipo_pessoa, 
		nr_seq_cobranca 
	from	(SELECT	'A' ie_tipo_registro, 
			...
			lpad(a.nr_remessa,6,0) nr_seq_envio, 
			0 nr_titulo, 
			0 vl_escritural, 
			0 vl_tot_registros, 
			...
			'' cd_cep, 
			'' ie_tipo_pessoa, 
			a.nr_sequencia nr_seq_cobranca 
...
union
		...
		select	'E' ie_tipo_registro, 
			...
			rpad(substr(pls_obter_dados_segurado(pls_obter_segurado_pagador(e.nr_seq_pagador),'C'),1,25),25,' ') nm_pagador, 
			coalesce(e.cd_agencia_bancaria,c.cd_agencia_bancaria) cd_agencia_bancaria, 
			0 cd_banco, 
			0 cd_movimento, 
			lpad(to_char(e.cd_conta), 14, '0') nr_conta, 
			null dt_geracao, 
			b.dt_vencimento, 
			'' ds_banco, 
			'                              X' ds_mensagem_2, 
...
union
		...
		select	'Z' ie_tipo_registro, 
			...
			--to_number(elimina_caracteres_especiais(sum(b.vl_cobranca))) vl_tot_registros, 
			a.nr_remessa nr_registro, 
			'' ds_cidade, 
			'' ds_bairro, 
			'' sg_estado, 
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	qt_commits_w	:= qt_commits_w + 1;
	 
	if (qt_commits_w = 500) then 
		qt_commits_w	:= 0;
		commit;
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_debito_aut_brasil_150 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
