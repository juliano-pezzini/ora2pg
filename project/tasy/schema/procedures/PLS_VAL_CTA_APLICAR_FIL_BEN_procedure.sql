...
CREATE OR REPLACE PROCEDURE pls_val_cta_aplicar_fil_ben ( dados_regra_p pls_tipos_cta_val_pck.dados_regra, dados_filtro_p pls_tipos_cta_val_pck.dados_filtro, dados_consistencia_p pls_tipos_cta_val_pck.dados_consistencia, nr_id_transacao_p pls_rp_cta_selecao.nr_id_transacao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE


dados_filtro_benef_w	pls_tipos_cta_val_pck.dados_filtro_benef;
...
C_filtro CURSOR(nr_seq_filtro_pc	pls_rp_cta_filtro.nr_sequencia%type) FOR
	SELECT	a.nr_sequencia,
		a.ie_tipo_segurado
	from	pls_rp_cta_filtro_benef a
...
if (dados_regra_p.nr_seq_regra IS NOT NULL AND dados_regra_p.nr_seq_regra::text <> '') then
	...
	for	r_C_filtro_w in C_filtro(dados_filtro_p.nr_sequencia) loop
		...
		dados_restricao_w.ds_restricao_conta := dados_restricao_w.ds_restricao_conta ||
							pls_cta_val_obter_restr_benef(	'RESTRICAO', var_cur_w,
											dados_filtro_benef_w);
		...
		begin
			...
			-- Trocar BINDS
			-- Do select original
			dados_restricao_w := pls_val_cta_obter_restr_padrao(
										'BINDS', dados_consistencia_p, nr_id_transacao_p,
										var_cur_w, dados_filtro_p, cd_estabelecimento_p,
										nm_usuario_p);
			-- Trocar binds do select Benef
			ret_null_w := pls_cta_val_obter_restr_benef(	'BIND', var_cur_w,
									dados_filtro_benef_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_val_cta_aplicar_fil_ben ( dados_regra_p pls_tipos_cta_val_pck.dados_regra, dados_filtro_p pls_tipos_cta_val_pck.dados_filtro, dados_consistencia_p pls_tipos_cta_val_pck.dados_consistencia, nr_id_transacao_p pls_rp_cta_selecao.nr_id_transacao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
