...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_arq_a700_130 (nr_seq_servico_p bigint, cd_estabelecimento_p text, nm_usuario_p text) AS $body$
DECLARE


-- Arquivo
ds_conteudo_w			varchar(5000);
ds_arquivo_w			text;
...
C01 CURSOR FOR
	SELECT	lpad(coalesce(elimina_caractere_especial(to_char(nr_lote)),'0'),8,'0'),
		...
		rpad(coalesce(ptu_somente_caracter_permitido(nm_prest_req,'ANS'),' '),60,' '),
		...
		rpad(coalesce(CASE WHEN tp_nota=4 THEN elimina_caractere_especial(elimina_acentuacao(nm_prest_exec))  ELSE lpad(' ',60,' ') END ,' '),60,' '), -- Regra Obrigatorio se TP_NOTA = 4. Para os demais casos, preencher com branco.
		lpad(coalesce(tp_prest_exec,'0'),2,'0'),
		rpad(coalesce(to_char(id_rec_proprio),' '),1,' '),
		rpad(coalesce(cd_cid_obito,' '),6,' '),
		lpad(coalesce(ie_cobertura_especial,'0'),2,'0'),
		lpad(coalesce(CASE WHEN tp_nota=2 THEN ie_regime_atendimento  ELSE lpad('0',2,'0') END ,'0'),2,'0'),
		lpad(coalesce(ie_saude_ocupacional,'0'),2,'0')
...
C02 CURSOR FOR
	SELECT	lpad(elimina_caractere_especial(a.nr_lote),8,'0') nr_lote_703,
		...
		rpad(coalesce(ptu_somente_caracter_permitido(a.nm_hospital,'ANS'),' '),60,' '),		
...
C03 CURSOR FOR
	SELECT	lpad(coalesce(elimina_caractere_especial(a.nr_lote),'0'),8,'0'),
		...
		lpad(coalesce(a.cd_servico,'0'),10,'0'),
		...
		rpad(coalesce(elimina_caractere_especial(elimina_acentuacao(a.nm_profissional_prestador)),' '),60,' '),
...
EXIT WHEN NOT FOUND; /* apply on C00 */
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		/* OS 649133 - Em atendimentos ao TISS o campo tipo de saida ( IE_TIPO_SAIDA_SPDAT_W ) deve ser informado em atendimento com data superior a 31/08/2007 quando nao houver internacao. */
		...
		-- Regra: Para atendimento de Atencao Domiciliar ou TP_NOTA = 3 (Internacao) o campo TP_PESSOA devera ser preenchido com J	-- OS 1999273 JTRINDADE
		if (tp_nota_w = 3) or (ie_tipo_atendimento_w = '6') then
			tp_pessoa_w	:= 'J';
		end if;
		...
		ds_conteudo_w	:=	lpad(to_char(nr_seq_registro_w),8,'0') || '702' || nr_lote_w || lpad(' ',12,' ') || cd_unimed_destino_w || '   ' ||
					...
					cd_cnes_cont_exec_w || cd_munic_cont_exec_w || nm_prest_req_w || lpad(' ',10,' ') || nm_prest_exec_w || lpad(' ',10,' ') || tp_prest_exec_w ||
					id_rec_proprio_w || cd_cid_obito_cobr_w || ie_regime_atendimento_w || ie_saude_ocupacional_w || ie_cobertura_especial_w;
		ds_arquivo_w	:=	ds_arquivo_w || ds_conteudo_w;
		...
		EXIT WHEN NOT FOUND; /* apply on C02 */
			...
			-- OBTER VERSAO DO PTU
			cd_interface_w	:= pls_obter_interf_ptu(cd_estabelecimento_p, null, clock_timestamp(), 'A700');
			ie_versao_w	:= somente_numero(ptu_obter_versao_dominio('A700', cd_interface_w));

			-- se for anterior a versao PTU 11.3
			if (ie_versao_w < 113) then
				-- se for pediatrica
				if (ie_tipo_internacao_w = 4) then
					ie_tipo_internacao_w	:= 6;
				end if;
				
				-- se for psiquiatrica
				if (ie_tipo_internacao_w = 5) then
					ie_tipo_internacao_w	:= 7;
				end if;
			end if;
		...
		EXIT WHEN NOT FOUND; /* apply on C03 */
			...
			-- Mesma informacao do registro 702
			nr_nota_704_w := nr_nota_w;

			-- O sistema deve seguir o padrao de 5 casas para de inteiros e 4 para casas decimais, a representacao da quantidade 1 e a seguinte --> '000010000'
			qt_procedimento_w	:= lpad(somente_numero(trunc(qt_procedimento_w)),5,'0') ||','|| rpad(somente_numero(qt_procedimento_w - trunc(qt_procedimento_w)),4,'0');
			qt_paga_w		:= somente_numero(qt_procedimento_w);
			
			if (ie_carater_atendimento_w = '1') and (coalesce(trim(both hr_final_w)::text, '') = '') then
				hr_final_w	:= ds_hora_procedimento_w;
			end if;

			-- Verifique a nota 00000000000235678160, lote 15421731. Sempre que houver um registro R703, no registro R704 as notas com participacao H (hospital) devem ter os campos R704.CD_UNI_PRE+R704.CD_PREST = R703.CD_UNI_HOSP+R703.CD_HOSP
			...
			ds_conteudo_w	:=	lpad(to_char(nr_seq_registro_w),8,'0') || '704' || nr_lote_704_w || lpad(' ',11,' ') ||
						...
						ie_tipo_tabela_w || lpad(' ',8,' ') || lpad(' ',8,' ') || vl_procedimento_w || vl_custo_operacional_w ||
						vl_filme_w || cd_porte_anestesico_w || lpad(' ',25,' ') || ie_via_acesso_w ||
						lpad(' ',42,' ') || '  ' || dt_pgto_prestador_w || lpad(' ',3,' ') || 
						...
						lpad(' ',37,' ') || lpad(' ',8,' ') || nr_cnpj_fornecedor_w || cd_rec_prestador_w || cd_ref_material_fab_w || lpad(' ',70,' ') ||
						nm_profissional_prestador_w || cd_procedimento_w || qt_paga_w;
			ds_arquivo_w	:=	ds_arquivo_w || ds_conteudo_w;

			qt_tot_r704_ww	:= qt_tot_r704_ww + somente_numero(qt_paga_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_arq_a700_130 (nr_seq_servico_p bigint, cd_estabelecimento_p text, nm_usuario_p text) FROM PUBLIC;
