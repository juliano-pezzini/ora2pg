...
CREATE OR REPLACE PROCEDURE pls_oc_cta_aplic_val_limi_mat ( dados_regra_p pls_tipos_ocor_pck.dados_regra, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE

/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade:	Aplicar as validações da limitação para os materiais cadastrados para as limitações
	dos beneficiários das contas filtradas para a ocorrência.
-------------------------------------------------------------------------------------------------------------------
...
AO ALTERARA VERIFICAR A NECESSIDADE DE ALTERAÇÃO NA PROCEDURE PLS_OC_CTA_APLIC_VAL_LIMI_PROC
...
dados_mat_limitacao_w		pls_grupos_pck.t_pls_limitacao_mat_row;
...
qt_mat_util_total_w		pls_conta_mat_v.qt_material%type;
qt_mat_gravar_w			pls_conta_mat_v.qt_material%type;

qt_permitida_w		pls_limitacao.qt_permitida%type;
nr_seq_titular_w	pls_segurado.nr_seq_titular%type;
qt_pessoas_w		integer := 0;

qt_mat_selecao_w		integer;
i				integer;
...
C05 CURSOR(	nr_id_transacao_pc		pls_oc_cta_val_limit_aux.nr_id_transacao%type,
	...
	SELECT	a.nr_seq_conta,
		...
		a.qt_item,
		a.ie_selecao,
		pls_tipos_ocor_pck.obter_sequencia_selecao(null, a.nr_seq_conta_mat, a.nr_seq_conta,
...
for	r_C02_w in C02(nr_id_transacao_p) loop
	...
	for	r_C03_w in C03(r_C02_w.nr_seq_contrato, r_C02_w.nr_seq_plano, r_C02_w.nr_seq_intercambio) loop
		...
		if (dados_limitacao_w.ie_tipo_periodo in ('D','S','M')) then
			...
			-- materiais utilizados pelo benenficiário para verificar se ultrapassa a quantidade permitida pelo contrato.
			...
			-- Se não tiver encontrado as datas verifica materiais apenas na mesma data do material.
			if (coalesce(dados_limitacao_w.dt_inicio_periodo::text, '') = '') then
				
				dados_limitacao_w.dt_inicio_periodo := trunc(r_C02_w.dt_atendimento);
			end if;
			
			if (coalesce(dados_limitacao_w.dt_fim_periodo::text, '') = '') then
			
				dados_limitacao_w.dt_fim_periodo := fim_dia(r_C02_w.dt_atendimento);
			end if;
		end if;
		
		-- Após encontrar as limitações contratuais verificar os materiais e\ou tipo de guia e\ou CID que cada uma delas irá bloquear o uso. 
		...
		-- deve ser gerada a ocorrência ou não. Para isso será populada a tabela auxiliar com os materiais que foram utilizacos e 

		-- se encaixam nos materiais para a limitação, após serão percorridos os registros passando para os materiais que não estão 
		...
		-- consistência atual. Será passado inicialmente pelos materiais que estão liberados e posteriormente para os que não estão liberados, 

		-- para que sejam removidos. A cada loop dos materiais selecionados, para os materiais que já haviam sido selecionados anteriormente será

		-- atualizado o campo IE_LIBERADO da tabela auxiliar, para que sejam removidos os materiais não liberados, e ao final do processo serão 

		-- considerados apenas os materiais que estão com o IE_LIBERADO como 'S' na tabela de seleção.
		for	r_C04_w in C04(dados_limitacao_w.nr_seq_tipo_limitacao) loop
		
			-- Alimentar os dados da regra da limitação. Serão usados para montar a restrição do select que será montado para buscar os materiais 

			-- a que a regra está bloqueando.
			dados_mat_limitacao_w.nr_seq_material	:= r_C04_w.nr_seq_material;
			dados_mat_limitacao_w.ie_tipo_guia	:= r_C04_w.ie_tipo_guia;
			dados_mat_limitacao_w.cd_doenca_cid	:= r_C04_w.cd_doenca_cid;
			dados_mat_limitacao_w.ie_liberado	:= r_C04_w.ie_liberado;
			dado_bind_w.delete;
			
			-- Montar a restrição da regra. 
			dado_bind_w := pls_oc_cta_obter_restr_val_lim(
								dados_segurado_w, nr_id_transacao_p, dados_regra_p, dados_limitacao_w, 'M', null, dados_mat_limitacao_w, dado_bind_w);
			...
			if	(not(	(coalesce(dados_restricao_w.ds_restricao_lote::text, '') = '') and (coalesce(dados_restricao_w.ds_restricao_protocolo::text, '') = '') and (coalesce(dados_restricao_w.ds_restricao_conta::text, '') = '') and (coalesce(dados_restricao_w.ds_restricao_benef::text, '') = '') and (coalesce(dados_restricao_w.ds_restricao_proc::text, '') = '') and (coalesce(dados_restricao_w.ds_restricao_mat::text, '') = ''))) then
				...
				ds_sql_w :=	'select	conta.nr_sequencia, ' || pls_tipos_ocor_pck.enter_w ||
						'	mat.nr_sequencia, ' || pls_tipos_ocor_pck.enter_w || 
						'	mat.qt_material, ' || pls_tipos_ocor_pck.enter_w || 
						'	mat.qt_material_imp, ' || pls_tipos_ocor_pck.enter_w || 
						'	mat.ie_status ' || pls_tipos_ocor_pck.enter_w ||
						...
						dados_restricao_w.ds_restricao_mat || pls_tipos_ocor_pck.enter_w ||
						'order by mat.dt_atendimento ';
				begin
					...
					loop
						fetch cursor_w into
							dados_conta_mat_w.nr_seq_conta, dados_conta_mat_w.nr_seq_conta_mat,
							dados_conta_mat_w.qt_material, dados_conta_mat_w.qt_material_imp,
							dados_conta_mat_w.ie_status;
						EXIT WHEN NOT FOUND; /* apply on cursor_w */
								
						-- Quando o material estiver em Análise, Pendente ou Consistido então considera a quantidade 

						-- apresentada, caso contrário considera a liberada. Não se considera materiais cancelados, já 

						-- está sendo restringido na view PLS_CONTA_MAT_OCOR_V. 
						if (dados_conta_mat_w.ie_status in ('A','P','C')) then
							
							qt_mat_gravar_w := dados_conta_mat_w.qt_material_imp;
						else
							qt_mat_gravar_w := dados_conta_mat_w.qt_material;
						end if;
						
						-- Aqui está sendo verificado se o material está na tabela de seleção. Caso esteja então flega o campo

						-- IE_SELECAO da tabela auxiliar para S para que após este processo seja ordenado e verificado se 

						-- pode ser lançada a ocorrência para este material ou não.
						select	count(1)
						into STRICT	qt_mat_selecao_w
						from	pls_oc_cta_selecao_ocor_v
						where	nr_id_transacao		= nr_id_transacao_p
						and	ie_valido		= 'S'
						and	ie_tipo_registro	= 'M'
						and	nr_seq_conta		= dados_conta_mat_w.nr_seq_conta
						and	nr_seq_conta_mat	= dados_conta_mat_w.nr_seq_conta_mat;
						...
						dados_tabela_aux_w.qt_item			:= qt_mat_gravar_w;
						...
						pls_tipos_ocor_pck.gerencia_tabela_aux_val_limit( 'I', 'M', nr_id_transacao_p, dados_tabela_aux_w, nm_usuario_p);
		...
		-- Zerar o totalizador para executar a soma dos materiais que foram utilizados pelo beneficiário.
		...
		for	r_C05_w	in C05(	nr_id_transacao_p, r_C02_w.nr_seq_segurado,
			...
			qt_mat_util_total_w := qt_mat_util_total_w + r_C05_w.qt_item;
			
			ds_itens_util_w	:= substr(ds_itens_util_w || pls_tipos_ocor_pck.enter_w  || 'Conta: ' || r_C05_w.nr_seq_conta || ' | ' || 'Material: ' || r_C05_w.nr_seq_conta_mat || ' | ' || 'Quantidade: ' || r_C05_w.qt_item, 1, 4000);
			
			
			
			-- Irá gerar a ocorrência apenas quando a quantidade utilizada ultrapassar a permitida e quando o material estiver na tabela de 

			-- seleção.
			if (qt_mat_util_total_w > qt_permitida_w) and (r_C05_w.ie_selecao = 'S') then
				
				ds_observacao_w	:= substr(	'O material ultrapassa a quantidade permitida pelas limitações contratuais do beneficiário. ' || pls_tipos_ocor_pck.enter_w ||
								'Este beneficiário possui liberação para utilizar apenas '
								|| qt_permitida_w || ' material(is) neste mesmo contexto em um período de ' || dados_limitacao_w.qt_meses_intervalo || ' ' || 
								obter_valor_dominio(4171, dados_limitacao_w.ie_tipo_periodo)|| pls_tipos_ocor_pck.enter_w || pls_tipos_ocor_pck.enter_w ||	
								'Materiais utilizados no período: ' || pls_tipos_ocor_pck.enter_w ||
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_oc_cta_aplic_val_limi_mat ( dados_regra_p pls_tipos_ocor_pck.dados_regra, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
