...
CREATE OR REPLACE PROCEDURE lab_gerar_analito_exame_ext (nr_seq_resultado_p bigint, nm_usuario_p text, cd_estabelecimento_p text) AS $body$
DECLARE
...
qt_analitos_exame_w		bigint;
qt_analitos_prescr_w		bigint;
...
if (ie_gerar_result_prescr_w = 'S') then
	...
	begin
		select (
			...
			and	b.nr_seq_superior = nr_seq_exame_w
  ...
  UNION ALL
		...
		into STRICT 	qt_analitos_exame_w,
			qt_analitos_prescr_w
		;

		if (qt_analitos_exame_w <> qt_analitos_prescr_w) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE lab_gerar_analito_exame_ext (nr_seq_resultado_p bigint, nm_usuario_p text, cd_estabelecimento_p text) FROM PUBLIC;
