...
CREATE OR REPLACE PROCEDURE fsfx_gerar_ret_femco_inadim ( ds_linha_p text, nr_seq_cobranca_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
ds_info_valor_w			varchar(255);
ds_rubrica_w        varchar(255);
...
cd_matricula_estip_w	:= substr(ds_linha_w,1,9)||'0'||substr(ds_linha_w,10,1);
ds_info_valor_w		:= substr(ds_linha_w,38,13);
ds_rubrica_w		:= substr(ds_linha_w,11,4);
vl_registro_w		:= (substr((ds_info_valor_w)::numeric ,1,length((ds_info_valor_w)::numeric )-2) ||','|| substr((ds_info_valor_w)::numeric ,length((ds_info_valor_w)::numeric )-1,2))::numeric;
...
and CASE WHEN ds_rubrica_w=4693 THEN 2  ELSE 3 END  = f.NR_SEQ_CLASSIF_ITENS;

if (coalesce(nr_titulo_w::text, '') = '') then
	insert	into	pls_desc_inconsistencia(	nr_sequencia,dt_atualizacao, nm_usuario,
dt_atualizacao_nrec,nm_usuario_nrec,
		nr_seq_cobranca, cd_estabelecimento,nr_matricula, vl_titulo,
		ds_inconsistencia)
	values (	nextval('pls_desc_inconsistencia_seq'), clock_timestamp(), nm_usuario_p,
clock_timestamp(),nm_usuario_p,
		nr_seq_cobranca_p, cd_estabelecimento_p,cd_matricula_estip_w, vl_registro_w,
		'Não foi possível encontrar o título do beneficiário ' ||nr_seq_segurado_w);
	goto final;
else
...
vl_liquidacao_w	:= titulo_receber_w.vl_titulo - titulo_receber_w.vl_titulo;

insert	into	titulo_receber_cobr(	nr_sequencia, dt_atualizacao, nm_usuario, dt_atualizacao_nrec,
nm_usuario_nrec,
...
values (	nextval('titulo_receber_cobr_seq'), clock_timestamp(), nm_usuario_p, clock_timestamp(),
nm_usuario_p,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fsfx_gerar_ret_femco_inadim ( ds_linha_p text, nr_seq_cobranca_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
