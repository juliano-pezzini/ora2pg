...
CREATE OR REPLACE PROCEDURE gerar_sinal_vital_hl7 ( cd_pessoa_fisica_p text, dt_sinal_vital_p text, ds_lista_parametros_p text, ds_lista_parametros2_p text, nm_usuario_p text, ie_trend_upload_p text default 'N') AS $body$
DECLARE
...
IF (nm_usuario_p = 'PROLIFE ATEND') THEN
...
ELSE
	...
	AND	   (((UPPER(nm_usuario_p) = 'MINDRAY') AND (coalesce(DT_ALTA::text, '') = '')) OR (UPPER(nm_usuario_p) <> 'MINDRAY'));
...
if (UPPER(nm_usuario_p) in ('SURESIGNS','PROLIFE','MINDRAY','PROLIFE ATEND')) then
	...
	select 	coalesce(max(ie_tipo_horario_w), 'A')                       ie_tipo_horario_w,
			coalesce(max(ie_controlar_data_apap_w), ie_horario_real_w)  ie_controlar_data_apap_w,
      coalesce(max(ie_tipo_periodico_w), 'T')
	into STRICT 	ie_tipo_horario_w,
...
union all
	...
	if (ie_controlar_data_apap_w = ie_horario_apap_w) then
		select  extract(hour from time)    hora
		into STRICT    hora_sv_w
		from (
            SELECT cast(to_char(round(dt_sinal_vital_w, 'HH'), 'DD-MON-YYYY HH24:MI:SS') as timestamp) time
		...
		into STRICT    hora_sv_w
		from (
			SELECT cast(to_char(trunc(dt_sinal_vital_w, 'HH'), 'DD-MON-YYYY HH24:MI:SS') as timestamp) time
...
-- Criado consistencia para verificar se o paciente se encontra em um setor/leito monitorado.
  ...
  if (ie_tipo_periodico_w = 'T') or (ie_tipo_periodico_w = 'P' and ie_integracao_extra_w = 'N') or (ie_tipo_periodico_w = 'A' and ie_integracao_extra_w = 'S') then
...
--- SHIFT+F11 - Regra intervalo sinal vital
	if (nr_atendimento_w IS NOT NULL AND nr_atendimento_w::text <> '') and (qt_setor_integrado_w > 0) and (ie_periodico_aceito_w = 'S') and (ie_tem_sv_periodo_w = 'S') then
...
if 	(
			(nr_atendimento_w IS NOT NULL AND nr_atendimento_w::text <> '') and
				( 
					(
						qt_setor_integrado_w > 0 and
						ie_horario_aceito_w = 'S' and (ie_tem_sv_periodo_w <> 'S' OR ie_integrar_regra_min_w  = 'S')
					)
					OR ie_trend_upload_p = 'S' 
				)
		) then
...
if (coalesce(nr_seq_sinal_vital_w,0) > 0) then
	...
	if (ie_trend_upload_p = 'S') then
		...
		ie_exige_aprovacao_w := obter_param_usuario(281, 1493, 0, wheb_usuario_pck.get_nm_usuario, cd_estabelecimento_w, ie_exige_aprovacao_w);
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_sinal_vital_hl7 ( cd_pessoa_fisica_p text, dt_sinal_vital_p text, ds_lista_parametros_p text, ds_lista_parametros2_p text, nm_usuario_p text, ie_trend_upload_p text default 'N') FROM PUBLIC;
