...
CREATE OR REPLACE PROCEDURE gerar_proj_cron_consultoria_cl (dt_referencia_p timestamp, cd_cliente_p bigint) AS $body$
DECLARE
...
and	a.ie_situacao 		= 'A' 
and	trunc(b.dt_agenda,'mm')	= trunc(dt_referencia_p,'mm') 
and	((b.nm_paciente IS NOT NULL AND b.nm_paciente::text <> '') or (b.cd_pessoa_fisica IS NOT NULL AND b.cd_pessoa_fisica::text <> '')) 
and	((coalesce(cd_cliente_p::text, '') = '') or (b.cd_convenio = cd_cliente_p)) 
group by 
	a.cd_agenda, 
	a.ds_agenda 
order by 
...
if (dt_referencia_p IS NOT NULL AND dt_referencia_p::text <> '') then 
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		while(dt_dia_atual_w <= last_day(dt_referencia_p)) loop 
			...
			and	((coalesce(cd_cliente_p::text, '') = '') or (a.cd_convenio = cd_cliente_p));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_proj_cron_consultoria_cl (dt_referencia_p timestamp, cd_cliente_p bigint) FROM PUBLIC;
