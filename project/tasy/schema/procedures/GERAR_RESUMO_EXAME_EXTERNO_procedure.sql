...
CREATE OR REPLACE PROCEDURE gerar_resumo_exame_externo (cd_estabelecimento_p bigint, nr_seq_consulta_p bigint, cd_pessoa_fisica_p text, ie_mostra_todos_p text, nr_seq_item_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
expressao3_w	varchar(255) := obter_desc_expressao_idioma(289699, null, wheb_usuario_pck.get_nr_seq_idioma);--EXAME EXTERNO OD 
expressao4_w	varchar(255) := obter_desc_expressao_idioma(289700, null, wheb_usuario_pck.get_nr_seq_idioma);--EXAME EXTERNO OE 
expressao5_w	varchar(255) := obter_desc_expressao_idioma(294639, null, wheb_usuario_pck.get_nr_seq_idioma);--Observação 
C01 CURSOR FOR 
...
loop 
	...
	CALL gravar_registro_resumo_oft(ds_exame_externo_w,nr_seq_item_p,nm_usuario_p,to_date(dt_registro_w,'dd/mm/yyyy hh24:mi:ss'));
	end;
end loop;
close C01;	
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_resumo_exame_externo (cd_estabelecimento_p bigint, nr_seq_consulta_p bigint, cd_pessoa_fisica_p text, ie_mostra_todos_p text, nr_seq_item_p bigint, nm_usuario_p text) FROM PUBLIC;
