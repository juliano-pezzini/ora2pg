...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_0002_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
c_reg_0002 CURSOR FOR
SELECT	*
from	fis_efd_icmsipi_0002
where	nr_seq_controle = nr_seq_controle_p;

/*Criação do array com o tipo sendo do cursor eespecificado - c_reg_0002*/

type reg_c_reg_0002 is table of fis_efd_icmsipi_0002%RowType;
vetReg0002 reg_c_reg_0002;
...
open c_reg_0002;
loop
fetch c_reg_0002 bulk collect into vetReg0002 limit 1000;
	for i in 1 .. vetReg0002.Count loop
	begin
	...
	ds_linha_w := substr(	ds_sep_w || vetReg0002[i].cd_reg ||
							ds_sep_w || vetReg0002[i].cd_clas_estab_ind|| ds_sep_w,
			     1,
			     8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro		:=  vetReg0002[i].cd_reg;

	if (nr_vetor_w >= 1000) then
	begin
	/*Inserindo registros definitivamente na tabela especifica - FIS_EFD_ICMSIPI_0002 */
...
EXIT WHEN NOT FOUND; /* apply on c_reg_0002 */
end loop;
close c_reg_0002;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_0002_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) FROM PUBLIC;
