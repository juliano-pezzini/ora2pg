...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_0015_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
c_reg_0015 CURSOR FOR
SELECT	*
from	fis_efd_icmsipi_0015
where	nr_seq_controle = nr_seq_controle_p;

/*Criação do array com o tipo sendo do cursor eespecificado - c_reg_0015*/

type reg_c_reg_0015 is table of fis_efd_icmsipi_0015%RowType;
...
fetch c_reg_0015 bulk collect into vetReg0015 limit 1000;
	...
	ds_linha_w := substr(ds_sep_w || vetReg0015[i].cd_reg ||
			     ds_sep_w || vetReg0015[i].sg_uf_st ||
			     ds_sep_w || vetReg0015[i].nr_ie_st || ds_sep_w,
			     1,
			     8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro		:=  vetReg0015[i].cd_reg;
...
EXIT WHEN NOT FOUND; /* apply on c_reg_0015 */
end loop;
close c_reg_0015;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_0015_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) FROM PUBLIC;
