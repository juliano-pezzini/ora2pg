...
CREATE OR REPLACE PROCEDURE gerar_grafico_proj_progresso (nr_seq_proj_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C02 CURSOR FOR
	SELECT  obter_pr_prev_etapa_proj(nr_seq_proj_p ,dt_ref_w) pr_prev,
		obter_pr_real_etapa_proj(nr_seq_proj_p, dt_ref_w) pr_real
	;


BEGIN

delete from W_GRAFICO_PROJ where nr_seq_proj = nr_seq_proj_p;
commit;
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	begin
		...
		if (trunc(dt_ref_w) = trunc(clock_timestamp())) or (trunc(dt_controle_w)	= trunc(clock_timestamp())) or (trunc(dt_ref_w)	= trunc(dt_controle_w)) then
			select   max(a.pr_real),
				max(a.pr_prev)
			into STRICT	pr_agregado_w,
				pr_prev_w
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_grafico_proj_progresso (nr_seq_proj_p bigint, nm_usuario_p text) FROM PUBLIC;
