...
CREATE OR REPLACE PROCEDURE gerar_w_sup_transf_estoque ( nr_ordem_compra_p bigint, nr_item_oci_p bigint, cd_material_p bigint, cd_unidade_medida_p text, nr_seq_lote_p bigint, qt_material_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (coalesce(qt_material_p,0) > 0) then
	...
	if (coalesce(nr_item_oci_p,0) > 0) then
		...
		and		(qt_material - (obter_qt_oci_trans_nota(nr_ordem_compra,nr_item_oci,'S'))) >= qt_material_p;
	...
	if (coalesce(nr_item_oci_w,0) > 0) then
		...
		select	coalesce(max(cd_unidade_medida_compra),cd_unidade_medida_p)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_w_sup_transf_estoque ( nr_ordem_compra_p bigint, nr_item_oci_p bigint, cd_material_p bigint, cd_unidade_medida_p text, nr_seq_lote_p bigint, qt_material_p bigint, nm_usuario_p text) FROM PUBLIC;
