...
CREATE OR REPLACE PROCEDURE ctb_gerar_razao_ans ( cd_empresa_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estab_p text, cd_conta_contabil_p text, cd_classificacao_p text, cd_centro_custo_p text, nm_usuario_p text, ie_valor_p text, cd_classif_inic_p text, cd_classif_final_p text, ie_centro_custo_p text, ie_ordenacao_p text, ie_contra_partida_p text) AS $body$
DECLARE
...
c001 CURSOR FOR
	SELECT	distinct s.cd_conta_contabil,
			c.cd_conta_contabil  || ' - ' || coalesce(s.cd_classificacao, p.cd_classificacao) || ' - ' || p.ds_conta,
	...
	from	ctb_grupo_conta d,
			...
			ctb_mes_ref r,
			ctb_plano_ans p,
			conta_contabil_ans a,
			ctb_versao_plano_ans v
	...
	and p.nr_sequencia 			= a.nr_seq_conta_ans
	and p.nr_seq_versao_plano = v.nr_sequencia
	and (r.dt_referencia  between v.dt_inicio_vigencia and coalesce(v.dt_fim_vigencia,clock_timestamp()))
	and v.nr_sequencia = (SELECT	max(nr_sequencia)
							from 	ctb_versao_plano_ans v2
							where (r.dt_referencia between v2.dt_inicio_vigencia and coalesce(v2.dt_fim_vigencia, clock_timestamp())))
