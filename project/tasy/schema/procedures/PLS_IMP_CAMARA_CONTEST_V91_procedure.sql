...
CREATE OR REPLACE PROCEDURE pls_imp_camara_contest_v91 ( ds_conteudo_p text, nm_usuario_p text, nr_seq_camara_contest_p bigint, nr_seq_referencia_p INOUT bigint, cd_estabelecimento_p bigint, nr_seq_camara_cont_p INOUT bigint) AS $body$
DECLARE
...
if (substr(ds_conteudo_p,9,3) = '551') then
	...
	-- Verificar se possui arquivo de inclusão importado
	if (nr_seq_pls_fatura_w IS NOT NULL AND nr_seq_pls_fatura_w::text <> '') and (ie_tp_arquivo_w != 1) then
		...
		if (nr_seq_lote_contest_w IS NOT NULL AND nr_seq_lote_contest_w::text <> '') then
			...
			from	pls_lote_discussao
			where	nr_seq_lote_contest = nr_seq_lote_contest_w
			and	ie_tipo_arquivo in (1) -- inclusão
			and	coalesce(ie_status,'A') <> 'C';
		end if;
		
		if (qt_arquivo_w = 0) then
			CALL wheb_mensagem_pck.exibir_mensagem_abort(1090815); -- Favor importar primeiro o arquivo de inclusão (tipo 1).
		end if;
	end if;
	...
	nm_prestador_557_w	:= trim(both substr(ds_conteudo_p,104,70));
	...
	nm_profissional_558_w	:= trim(both substr(ds_conteudo_p,235,70));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_imp_camara_contest_v91 ( ds_conteudo_p text, nm_usuario_p text, nr_seq_camara_contest_p bigint, nr_seq_referencia_p INOUT bigint, cd_estabelecimento_p bigint, nr_seq_camara_cont_p INOUT bigint) FROM PUBLIC;
