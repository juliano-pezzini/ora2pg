...
CREATE OR REPLACE PROCEDURE pls_tiss_gerar_w_guia_radio (nr_sequencia_autor_p bigint, nr_seq_lote_guia_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
--Procedimentos Complementares 1
...
nr_seq_apres_controle_w		bigint;
nr_cid_w			bigint;
nr_proc_w			bigint;
i				bigint;
...
c01 CURSOR FOR 	--Procedimentos Complementares 1
	SELECT	a.dt_prev_realizacao,
		...
		substr(pls_obter_dados_proc_guia_req(a.nr_seq_plano_proc,a.nr_seq_req_proc,'CD'),1,255),
		substr(pls_obter_dados_proc_guia_req(a.nr_seq_plano_proc,a.nr_seq_req_proc,'DS'),1,255),
		c.qt_solicitada
	...
	and	c.nr_sequencia = a.nr_seq_plano_proc
...
if (nr_sequencia_autor_p IS NOT NULL AND nr_sequencia_autor_p::text <> '')  then
	...
	begin	--Dados da guia	--Dados do beneficiário
		select	b.nr_seq_segurado,
			coalesce(b.cd_senha, b.cd_senha_externa),
			b.dt_autorizacao,
			b.nr_seq_prestador,
			coalesce(b.cd_guia_prestador, coalesce(b.cd_guia_manual,b.cd_guia)),
			a.cd_guia_referencia,
			b.cd_guia,
			pls_obter_dados_segurado(b.nr_seq_segurado,'CR') cd_carteira,
			pls_obter_dados_segurado(b.nr_seq_segurado,'NS') nm_beneficiario ,
			...
			b.cd_estabelecimento --OS - 12895002
	...
	begin	--Dados do profissional solicitante
		...
		where	c.nr_seq_guia = nr_sequencia_autor_p
		and (c.nr_sequencia = nr_seq_lote_guia_p
		or	0 = coalesce(nr_seq_lote_guia_p, 0))
		and	c.ie_tipo_anexo = 'RA'
		and 	coalesce(c.nr_seq_motivo_exc::text, '') = '';
	...
	begin
		tb_proc_w[nr_seq_apresentacao_w].dt_prev_w		:= dt_prev_realizacao_w;
		tb_proc_w[nr_seq_apresentacao_w].cd_tabela_w		:= cd_tipo_tabela_w;
		tb_proc_w[nr_seq_apresentacao_w].cd_proc_w		:= cd_procedimento_w;
		tb_proc_w[nr_seq_apresentacao_w].ds_proc_w		:= ds_procedimento_w;
		tb_proc_w[nr_seq_apresentacao_w].qt_solic_w		:= qt_solicitado_w;
		tb_proc_w[nr_seq_apresentacao_w].nr_seq_apre_w		:= nr_seq_apres_controle_w;
		tb_proc_w[nr_seq_apresentacao_w].nr_seq_autor_w		:= nr_seq_guia_w;
		nr_proc_w := nr_proc_w + 1;
		nr_seq_apresentacao_w := nr_seq_apresentacao_w + 1;
		nr_seq_apres_controle_w := nr_seq_apres_controle_w + 1;

		if (nr_seq_apres_controle_w = 13) then
			nr_proc_w := 1;
			nr_seq_apres_controle_w := 1;
		end if;
	...
	if (nr_seq_apres_controle_w < 12) then
		for i in 1.. 12 loop
			begin
				if ( i = nr_proc_w ) then
					tb_proc_w[nr_seq_apresentacao_w].dt_prev_w		:= null;
					tb_proc_w[nr_seq_apresentacao_w].cd_tabela_w		:= ' ';
					tb_proc_w[nr_seq_apresentacao_w].cd_proc_w		:= ' ';
					tb_proc_w[nr_seq_apresentacao_w].ds_proc_w		:= ' ';
					tb_proc_w[nr_seq_apresentacao_w].qt_solic_w		:= null;
					tb_proc_w[nr_seq_apresentacao_w].nr_seq_apre_w		:= nr_seq_apres_controle_w;
					tb_proc_w[nr_seq_apresentacao_w].nr_seq_autor_w		:= nr_seq_guia_w;
					nr_proc_w := nr_proc_w + 1;
					nr_seq_apresentacao_w := nr_seq_apresentacao_w + 1;
	...
	--Somente completo a quantidade de registros, caso alguma guia tenha sido gerada (ou que o cursor 1 tenho retornado algum registro)
	if (nr_seq_guia_w IS NOT NULL AND nr_seq_guia_w::text <> '') then
		for i in 1.. nr_seq_apresentacao_w loop
			begin
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_tiss_gerar_w_guia_radio (nr_sequencia_autor_p bigint, nr_seq_lote_guia_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
