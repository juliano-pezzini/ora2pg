...
CREATE OR REPLACE PROCEDURE pls_gerar_lote_ir_mens ( nr_seq_lote_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
ds_ano_base_w			pls_lote_mens_ir.ds_ano_base%type;
ie_tipo_lote_w			pls_lote_mens_ir.ie_tipo_lote%type;
...
ds_email_pagador_w		pls_contrato_pagador.ds_email%type;
...
C01 CURSOR FOR
	...
	where	exists (	SELECT	1
				...
				and	y.ie_situacao		= '2' /* ie_situacao = 2 = Liquidado */
				and	coalesce(x.ie_cancelamento::text, '') = '')
	and	((ie_tipo_lote_w = 'A') or (ie_tipo_lote_w = 'PF' and (a.cd_pessoa_fisica IS NOT NULL AND a.cd_pessoa_fisica::text <> '')) or (ie_tipo_lote_w = 'PJ' and (a.cd_cgc IS NOT NULL AND a.cd_cgc::text <> '')));

C02 CURSOR FOR
	SELECT	x.nr_sequencia nr_seq_segurado
	from	pls_segurado	x
	where	exists (	SELECT	1
				...
				and	c.ie_situacao			= '2' /* ie_situacao = 2 = Liquidado */
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (ds_email_pagador_w IS NOT NULL AND ds_email_pagador_w::text <> '') then
		ds_email_w	:= ds_email_pagador_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_lote_ir_mens ( nr_seq_lote_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
