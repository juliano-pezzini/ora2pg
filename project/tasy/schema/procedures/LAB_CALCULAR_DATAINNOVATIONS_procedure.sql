...
CREATE OR REPLACE PROCEDURE lab_calcular_datainnovations ( cd_barras_p text, cd_exame_p text, nm_usuario_aprov_p text, nm_usuario_p text) AS $body$
DECLARE
	...
	if (ie_curva_w IS NOT NULL AND ie_curva_w::text <> '') then
		...
		select	CASE WHEN count(*)=0 THEN 'N'  ELSE 'S' END
      		 into STRICT	ie_existe_w
        		...
        		where	nr_prescricao 	= nr_prescricao_w
        		and	nr_seq_prescr 	= nr_seq_prescr_w
        		and	ie_status < coalesce(ie_status_receb_w,30)
        		...
        		if (ie_existe_w = 'S') then
            			...
            			update	prescr_proc_mat_item
            			set	    ie_status 	= obter_valor_menor(ie_status_receb_w,30)
            			where	nr_prescricao 	= nr_prescricao_w
            			and	    nr_seq_prescr 	= nr_seq_prescr_w
            			and	    cd_barras = to_char((cd_barras_p)::numeric );
        		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE lab_calcular_datainnovations ( cd_barras_p text, cd_exame_p text, nm_usuario_aprov_p text, nm_usuario_p text) FROM PUBLIC;
