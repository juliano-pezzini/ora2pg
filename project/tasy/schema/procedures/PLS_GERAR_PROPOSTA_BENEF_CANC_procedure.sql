...
CREATE OR REPLACE PROCEDURE pls_gerar_proposta_benef_canc ( nr_seq_proposta_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
qt_benef_canc_w			bigint;
qt_declaracao_w			bigint;
qt_analise_adesao_w		bigint;
nr_seq_analise_adesao_w		bigint;
cd_beneficiario_prop_w		varchar(10);
nr_seq_proposta_pagador_novo_w	pls_proposta_pagador.nr_sequencia%type;
nr_seq_proposta_pagador_ant_w	pls_proposta_pagador.nr_sequencia%type;
...
cd_cgc_pagador_ant_w		pls_proposta_pagador.cd_cgc_pagador%type;
cd_pessoa_fisica_pag_ant_w	pls_proposta_pagador.cd_pagador%type;
...
select	count(*)
into STRICT	qt_benef_canc_w
from	pls_proposta_beneficiario
where	nr_seq_proposta = nr_seq_proposta_p
and	(dt_cancelamento IS NOT NULL AND dt_cancelamento::text <> '');

if (qt_benef_canc_w = 0) then
	CALL Wheb_mensagem_pck.exibir_mensagem_abort(238966);
end if;
...
EXIT WHEN NOT FOUND; /* apply on C02 */
	...
	insert into pls_proposta_plano(	nr_sequencia, nr_seq_proposta, nr_seq_plano, nr_seq_tabela, nr_seq_contrato_plano,
		      ...
		      (	SELECT	nextval('pls_proposta_plano_seq'),	nr_seq_proposta_nova_w, nr_seq_plano, nr_seq_tabela, nr_seq_contrato_plano,  
				ie_principal, nr_seq_contrato, dt_atualizacao, nm_usuario_p, dt_atualizacao_nrec, 
				nm_usuario_p
			from	pls_proposta_plano
...
fetch C03 into
	...
	nr_seq_proposta_pagador_ant_w;
EXIT WHEN NOT FOUND; /* apply on C03 */
	
	if (nr_seq_proposta_pagador_ant_w IS NOT NULL AND nr_seq_proposta_pagador_ant_w::text <> '') then
		select	max(cd_cgc_pagador),
			max(cd_pagador)
		into STRICT	cd_cgc_pagador_ant_w,
			cd_pessoa_fisica_pag_ant_w
		from	pls_proposta_pagador
		where	nr_sequencia = nr_seq_proposta_pagador_ant_w;
		
		if (cd_cgc_pagador_ant_w IS NOT NULL AND cd_cgc_pagador_ant_w::text <> '') then
			select	max(nr_sequencia)
			into STRICT	nr_seq_proposta_pagador_novo_w
			from	pls_proposta_pagador
			where	cd_cgc_pagador = cd_cgc_pagador_ant_w
			and	nr_seq_proposta = nr_seq_proposta_nova_w;
		elsif (cd_pessoa_fisica_pag_ant_w IS NOT NULL AND cd_pessoa_fisica_pag_ant_w::text <> '') then
			select	max(nr_sequencia)
			into STRICT	nr_seq_proposta_pagador_novo_w
			from	pls_proposta_pagador
			where	cd_pagador = cd_pessoa_fisica_pag_ant_w
			and	nr_seq_proposta = nr_seq_proposta_nova_w;
		else
			nr_seq_proposta_pagador_novo_w	:= null;
		end if;
	else
		nr_seq_proposta_pagador_novo_w	:= null;
	end if;
	...
	insert into pls_proposta_beneficiario(	nr_sequencia, nr_seq_proposta, cd_beneficiario, nr_seq_pagador,
		      ...
		      ( SELECT	nr_seq_prop_benef_novo_w, nr_seq_proposta_nova_w, cd_beneficiario, nr_seq_proposta_pagador_novo_w,  
				nr_seq_plano, nr_seq_titular, nr_seq_parentesco, nr_seq_tabela, vl_mensalidade,                
				nr_seq_regra_desconto, tx_desconto, nr_seq_titular_contrato, ie_nascido_plano, nr_seq_portabilidade,          
				dt_admissao, cd_cbo, cd_matricula_est, nr_seq_vinculo_estip, nr_seq_subestipulante,       
				nr_seq_beneficiario, nr_seq_inclusao_benef, nr_seq_benef_gerado, null,    
				null, vl_bonificacao, dt_contratacao, vl_sca, nr_seq_motivo_inclusao,        
				ie_taxa_inscricao, nr_seq_prop_beneficiario_w, nr_seq_simul_individ, cd_matricula_familia, nr_seq_tipo_comercial,
	...
	insert into pls_proposta_benef_hist(	nr_sequencia, nr_seq_benef_proposta, dt_historico, ds_historico,
			...
			values ( nextval('pls_proposta_benef_hist_seq'), nr_seq_prop_benef_novo_w, clock_timestamp(), wheb_mensagem_pck.get_texto(1122600, 'NR_SEQ_PROPOSTA='||nr_seq_proposta_p),
	...
	/*Atualizar Declaracao de Saude quando o beneficiario cancelado possuir alguma pendente vinculada,*/

	select	count(*)
	into STRICT	qt_declaracao_w
	from	pls_declaracao_segurado
	where	nr_seq_proposta_adesao	= nr_seq_proposta_p
	and	cd_pessoa_fisica	= cd_beneficiario_prop_w
	and	ie_status in ('P','U');
	
	if (qt_declaracao_w > 0) then
		update	pls_declaracao_segurado
		set	nr_seq_proposta_adesao	= nr_seq_proposta_nova_w
		where	nr_seq_proposta_adesao	= nr_seq_proposta_p
		and	cd_pessoa_fisica	= cd_beneficiario_prop_w
		and	ie_status in ('P','U');
	
		insert into	pls_proposta_benef_hist(	nr_sequencia, nr_seq_benef_proposta, dt_atualizacao,
				nm_usuario, dt_atualizacao_nrec, nm_usuario_nrec, 
				dt_historico, ds_historico, ie_tipo_historico)
		values (	nextval('pls_proposta_benef_hist_seq'), nr_seq_prop_beneficiario_w, clock_timestamp(), 
				nm_usuario_p, clock_timestamp(), nm_usuario_p, 
				clock_timestamp(), wheb_mensagem_pck.get_texto(1122598, 'NR_SEQ_PROPOSTA_NOVA='||nr_seq_proposta_nova_w), '15');
		
		insert into	pls_proposta_benef_hist(	nr_sequencia, nr_seq_benef_proposta, dt_atualizacao,
				nm_usuario, dt_atualizacao_nrec, nm_usuario_nrec, 
				dt_historico, ds_historico, ie_tipo_historico)
		values (	nextval('pls_proposta_benef_hist_seq'), nr_seq_prop_benef_novo_w, clock_timestamp(), 
				nm_usuario_p, clock_timestamp(), nm_usuario_p, 
				clock_timestamp(), wheb_mensagem_pck.get_texto(1122599, 'NR_SEQ_PROPOSTA='||nr_seq_proposta_p), '15');
			
	end if;
	
	/*Atualizar Analise de Adesao quando o beneficiario cancelado possuir alguma pendente vinculada,*/
	...
	if (qt_analise_adesao_w > 0) then
		select	nr_sequencia
		into STRICT	nr_seq_analise_adesao_w
		from	pls_analise_adesao
		where	nr_seq_pessoa_proposta	= nr_seq_prop_beneficiario_w
		and	ie_status <> 'E';
	
		update	pls_analise_adesao
		set	nr_seq_pessoa_proposta	= nr_seq_prop_benef_novo_w,
			nr_seq_proposta		= nr_seq_proposta_nova_w
		where	nr_seq_pessoa_proposta	= nr_seq_prop_beneficiario_w
		and	ie_status <> 'E';

		insert into	pls_analise_historico(	nr_sequencia, nr_seq_analise, cd_estabelecimento, dt_historico, nm_usuario_historico,
				dt_atualizacao, nm_usuario, dt_atualizacao_nrec, nm_usuario_nrec, ds_titulo, 
				ds_historico)
		values (	nextval('pls_analise_historico_seq'), nr_seq_analise_adesao_w, cd_estabelecimento_p, clock_timestamp(), nm_usuario_p, 
				clock_timestamp(), nm_usuario_p, clock_timestamp(), nm_usuario_p, wheb_mensagem_pck.get_texto(1122592), 
				wheb_mensagem_pck.get_texto(1122594, 'NR_SEQ_PROPOSTA='||nr_seq_proposta_p||';NR_SEQ_PROPOSTA_NOVA='||nr_seq_proposta_nova_w) || chr(13) ||
				wheb_mensagem_pck.get_texto(1122595, 'NR_SEQ_BENEF_PROP='||nr_seq_prop_beneficiario_w||';NR_SEQ_BENEF_PROP_NOVO='||nr_seq_prop_benef_novo_w) || chr(13) || chr(13) ||
				wheb_mensagem_pck.get_texto(1122597));
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_proposta_benef_canc ( nr_seq_proposta_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
