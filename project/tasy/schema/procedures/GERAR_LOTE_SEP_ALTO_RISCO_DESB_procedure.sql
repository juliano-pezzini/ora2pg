...
CREATE OR REPLACE PROCEDURE gerar_lote_sep_alto_risco_desb ( nr_prescricao_p bigint, nr_sequencia_p bigint, nr_seq_horario_p bigint, ie_so_aprazado_p text, nm_usuario_p text, ie_origem_lote_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	and	((coalesce(ie_alto_risco_w,'N') = 'S') and (obter_se_material_risco(a.cd_setor_atendimento,b.cd_material) = 'S'))
...
C20 CURSOR FOR
	...
	order by CASE WHEN ie_quimioterapicas='S' THEN 'A'  ELSE CASE WHEN ie_quimioterapicas='N' THEN 'C'  ELSE 'B' END  END ,coalesce(cd_setor_atendimento,0), coalesce(ie_motivo_prescricao,0);	
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_lote_sep_alto_risco_desb ( nr_prescricao_p bigint, nr_sequencia_p bigint, nr_seq_horario_p bigint, ie_so_aprazado_p text, nm_usuario_p text, ie_origem_lote_p text) FROM PUBLIC;
