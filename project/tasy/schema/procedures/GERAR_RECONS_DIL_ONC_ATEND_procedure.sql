...
CREATE OR REPLACE PROCEDURE gerar_recons_dil_onc_atend ( nr_seq_atendimento_p bigint, nr_seq_material_p bigint, ie_opcao_p text, nm_usuario_p text) AS $body$
DECLARE
...
if (nr_seq_atendimento_p > 0) and (nr_seq_material_p > 0) and
	...
	Select	coalesce(cd_material,0),
		...
		coalesce(cd_unid_med_dose, obter_unid_med_usua('ml')),
		1,
		...
		if (Controle_w = 0) then
			...
			INSERT INTO PACIENTE_ATEND_MEDIC( NR_SEQ_DILUICAO,
			 ...
			 VALUES ( 
								...
								cd_diluente_w, 
								qt_diluicao_w,
								coalesce(cd_unid_med_diluente_w,cd_unidade_medida_w), 
								ie_via_aplicacao_w, 
								clock_timestamp(), 
								nm_usuario_p, 
								null, 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_recons_dil_onc_atend ( nr_seq_atendimento_p bigint, nr_seq_material_p bigint, ie_opcao_p text, nm_usuario_p text) FROM PUBLIC;
