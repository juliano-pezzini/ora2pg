...
CREATE OR REPLACE PROCEDURE consiste_regra_inf_rep ( cd_estabelecimento_p bigint, nr_atendimento_p bigint, cd_setor_atendimento_p bigint, cd_perfil_p bigint, ie_momento_p text, ie_prescricao_alta_p text, dt_prescricao_p timestamp, nm_usuario_p text, ds_mensagem_p INOUT text, ds_mensagem_adic_p INOUT text, ie_acao_p INOUT text, ie_anamnese_p INOUT text, ie_evolucao_p INOUT text, ie_receita_p INOUT text, ie_diagnostico_p INOUT text, ie_escala_indice_p INOUT text, nr_prescricao_p bigint default null, cd_pessoa_fisica_p text DEFAULT NULL, qt_peso_p bigint default null, ie_recomendacao_p INOUT text DEFAULT NULL, cd_protocolo_p INOUT bigint DEFAULT NULL, ie_parecer_p INOUT text DEFAULT NULL, ie_peso_p INOUT text DEFAULT NULL, ie_autorizacao_p INOUT text DEFAULT NULL, ie_laudo_sus_p INOUT text DEFAULT NULL, ie_avaliacao_p INOUT text DEFAULT NULL, ie_item_prontuario_p INOUT text DEFAULT NULL, ie_posicionar_pep_p INOUT text DEFAULT NULL) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	and		((coalesce(cd_material::text, '') = '') or (exists (	select	1
						from	prescr_material x
						where	x.nr_prescricao = nr_prescricao_p
						and		x.cd_material = a.cd_material)))
	and		((coalesce(CD_CLASSE_MATERIAL::text, '') = '') or (exists (	select	1
						from	prescr_material x
						where	x.nr_prescricao = nr_prescricao_p
						and		Obter_estrutura_material(x.cd_material,'C') = a.CD_CLASSE_MATERIAL)))
	and		((coalesce(CD_SUBGRUPO_MATERIAL::text, '') = '') or (exists (	select	1
						from	prescr_material x
						where	x.nr_prescricao = nr_prescricao_p
						and		Obter_estrutura_material(x.cd_material,'S') = a.CD_SUBGRUPO_MATERIAL)))
	and		((coalesce(CD_GRUPO_MATERIAL::text, '') = '') or (exists (	select	1
						from	prescr_material x
						where	x.nr_prescricao = nr_prescricao_p
						and		Obter_estrutura_material(x.cd_material,'G') = a.CD_GRUPO_MATERIAL)))
	and     ((coalesce(CD_AREA_PROCEDIMENTO::text, '') = '') or (exists (	select	1
						from	prescr_procedimento x
						where	x.nr_prescricao = nr_prescricao_p
						and		OBTER_AREA_PROCEDIMENTO(x.cd_procedimento, x.ie_origem_proced) = a.CD_AREA_PROCEDIMENTO)))
	and     ((coalesce(CD_ESPECIALIDADE::text, '') = '') or (exists (	select	1
						from	prescr_procedimento x
						where	x.nr_prescricao = nr_prescricao_p
						and		Obter_Especialidade_Proced(x.cd_procedimento, x.ie_origem_proced) = a.CD_ESPECIALIDADE)))
	and     ((coalesce(CD_GRUPO_PROC::text, '') = '') or (exists (	select	1
						from	prescr_procedimento x
						where	x.nr_prescricao = nr_prescricao_p
						and		obter_grupo_procedimento(x.cd_procedimento, x.ie_origem_proced, 'C') = a.CD_GRUPO_PROC)))
	and     ((coalesce(NR_SEQ_PROC_INTERNO::text, '') = '') or (exists (	select	1
						from	prescr_procedimento x
						where	x.nr_prescricao = nr_prescricao_p
						and		(x.nr_seq_proc_interno IS NOT NULL AND x.nr_seq_proc_interno::text <> '')
						and		x.nr_seq_proc_interno = a.NR_SEQ_PROC_INTERNO)))
	and     ((coalesce(CD_PROCEDIMENTO_REGRA::text, '') = '') or (exists (	select	1
						from	prescr_procedimento x
						where	x.nr_prescricao = nr_prescricao_p
						and		X.CD_PROCEDIMENTO = a.CD_PROCEDIMENTO_REGRA)))			
...
if (cd_pessoa_fisica_w IS NOT NULL AND cd_pessoa_fisica_w::text <> '') then
	...
	select	coalesce(max(nm_usuario_original),nm_usuario_p),
			max(cd_medico)
		...
		if (qt_prescricoes_w >= coalesce(qt_prescricoes_reg_w,0)) then
			...
			if (ie_informacao_w 	= 'JS') and (ie_momento_p		= 'L') 	and (nr_atendimento_p IS NOT NULL AND nr_atendimento_p::text <> '') then
				...
				if (ie_informacao_w	= 'AP') then
					...
					select 	count(*)
					into STRICT   	qt_mov_uti_w -- Quantidade movimenta;ao na UTI
					from	atend_paciente_unidade a,
							setor_atendimento b					
					where 	a.nr_atendimento 	= nr_atendimento_p
					and		a.cd_setor_atendimento = b.cd_setor_atendimento
					and		b.cd_classif_setor  = 4
					and 	coalesce(a.ie_passagem_setor,'N') <> 'L'
				...
				elsif (ie_informacao_w	= 'SAPS2') then
					begin
						SELECT * FROM consiste_regra_inf_rep_saps2(
							nr_atendimento_p, cd_pessoa_fisica_w, nr_seq_regra_w, ie_inconsistente_w, ie_informacao_ww, ie_escala_indice_p, ie_informacao_w, quebra_w, ds_mensag_w, ds_mensagem_w) INTO STRICT ie_inconsistente_w, ie_informacao_ww, ie_escala_indice_p, ds_mensagem_w;
					end;
				elsif (ie_informacao_w	= 'NIC') then
					begin
						SELECT * FROM consiste_regra_inf_rep_nic(
							nr_atendimento_p, nr_prescricao_p, cd_pessoa_fisica_w, nr_seq_regra_w, ie_inconsistente_w, ie_informacao_ww, ie_escala_indice_p, ie_informacao_w, quebra_w, ds_mensag_w, ds_mensagem_w) INTO STRICT ie_inconsistente_w, ie_informacao_ww, ie_escala_indice_p, ds_mensagem_w;
					end;
					...
					select	count(*)
					into STRICT	qt_registros_w
					from	MED_RECEITA where		coalesce(nr_atendimento, nr_atendimento_hosp) = coalesce(nr_atendimento_p,coalesce(nr_atendimento, nr_atendimento_hosp))
					...
					select	count(*)
					into STRICT	qt_registros_w
					from	pep_pac_ci where		nr_atendimento		= coalesce(nr_atendimento_p,nr_atendimento)
					and	cd_pessoa_fisica	= cd_pessoa_fisica_w
					and	(dt_liberacao IS NOT NULL AND dt_liberacao::text <> '') LIMIT 1;
					...
					from	prescr_medica a,
							...
							exame_laboratorio c,
							lab_exame_equip d,
							equipamento_lab e where		a.nr_prescricao = b.nr_prescricao
					...
					if (ie_existe_lab_w = 'S') then
						...
						/*if (ie_revisar_w is null) then
							ds_retorno_w	:= substr(ds_retorno_w||obter_texto_tasy(106156, wheb_usuario_pck.get_nr_seq_idioma)||quebra_w,1,4000); --Campo de revisao
						end if; OS454355 */
					...
					EXIT WHEN NOT FOUND; /* apply on c02 */
						...
						select	count(*)
						into STRICT	ie_verif_aval_w	
						from	med_avaliacao_paciente  a,
								procedimento_prescricao b where		a.nr_seq_tipo_avaliacao 	= b.nr_seq_avaliacao
						...
						if (ie_verif_aval_w = 0) and (nr_seq_proc_interno_w IS NOT NULL AND nr_seq_proc_interno_w::text <> '') then
							select  count(*)
							into STRICT 	ie_verif_aval_w
							from	med_avaliacao_paciente	a,
									proc_interno		b where		a.nr_seq_tipo_avaliacao = b.nr_seq_avaliacao
							and 	a.nr_prescricao		= nr_prescricao_p
				...
				elsif (ie_informacao_w 	= 'RE') and (ie_momento_p		= 'L') then
					begin
					...
					if (qt_registros_ww = 0)	then
						...
						if (coalesce(cd_funcao_origem_w,924) <> 950)  then
							...
							from		prescr_recomendacao where		nr_prescricao = nr_prescricao_p LIMIT 1;
														
						else
							select	count(*)
							into STRICT	qt_recomendacao_w
							from	prescr_recomendacao a,
								prescr_medica b where		b.nr_atendimento = nr_atendimento_p
							and	a.nr_prescricao	 = b.nr_prescricao
							--and	b.nm_usuario_original	 = nm_usuario_p
							and	((Obter_se_prescr_plano(nr_prescricao_p,dt_rep_pt_w)	= 'S') or (Obter_se_prescr_plano(nr_prescricao_p,dt_rep_pt_ww)	= 'S'))
							and	coalesce(a.dt_suspensao::text, '') = ''
						...
						if (qt_recomendacao_w = 0) then
							if (ds_mensag_w IS NOT NULL AND ds_mensag_w::text <> '') then
								...
								ds_mensagem_w := ds_mensagem_w || wheb_mensagem_pck.get_texto(306637, null) || quebra_w; -- Essa prescricao nao possui nenhuma recomendacao prescrita.
							end if;	
						ie_inconsistente_w 	:= true;
						ie_informacao_ww	:= ie_informacao_w;
						ie_recomendacao_p	:= 'S';
						...
						EXIT WHEN NOT FOUND; /* apply on C03 */
							...
							if (coalesce(cd_funcao_origem_w,924) <> 950)  then
								...
								select	count(*)
								into STRICT	qt_recomendacao_w
								from	prescr_recomendacao where		nr_prescricao	= nr_prescricao_p
							...
							if (qt_recomendacao_w = 0) then
								...
								if (coalesce(ds_recomendacoes_w::text, '') = '') then
									ds_recomendacoes_w	:= ds_recomendacao_w;
								else
									ds_recomendacoes_w	:= ds_recomendacoes_w||quebra_w||ds_recomendacao_w;
								end if;
								ie_inconsistente_w 	:= true;
								ie_informacao_ww	:= ie_informacao_w;
								ie_recomendacao_p	:= 'S';
						...
						if (ie_inconsistente_w) then
							if (ds_mensag_w IS NOT NULL AND ds_mensag_w::text <> '') then
								ds_mensagem_w := ds_mensag_w||quebra_w;
							else
								ds_mensagem_w := wheb_mensagem_pck.get_texto(306635, null) -- Recomendacao do paciente nao foi prescrita. Tipo recomendacao: 
								||quebra_w|| ds_recomendacoes_w ||quebra_w;
							end if;										
						end if;
						end;
					end if;
				...
				elsif (ie_informacao_w = 'CS') then
					begin
					select	count(*)
					into STRICT	qt_registros_w
					from	escala_sepse a,
							atendimento_paciente b where		a.nr_atendimento 	= b.nr_atendimento
					and		a.nr_atendimento 	= coalesce(nr_atendimento_p, a.nr_atendimento)
					and		b.nr_atendimento 	= coalesce(nr_atendimento_p, b.nr_atendimento)
					and		b.cd_pessoa_fisica 	= cd_pessoa_fisica_w
					and		coalesce(a.ie_status_sepsis,'NONE') 	= 'SA'
					and		coalesce(a.ie_situacao,'A') 		= 'A'
					and		(a.dt_liberacao IS NOT NULL AND a.dt_liberacao::text <> '')
					and		coalesce(dt_inativacao::text, '') = '' LIMIT 1;
					...
					if (qt_registros_w	> 0) then
						if (ds_mensag_w IS NOT NULL AND ds_mensag_w::text <> '') then
							...
							ds_mensagem_w := ds_mensagem_w || wheb_mensagem_pck.get_texto(1073625, null) || quebra_w; -- O paciente possui uma suspeita de Sepse pendente de confirmacao ou descarte
						end if;
						ie_inconsistente_w		:= true;
						ie_informacao_ww		:= ie_informacao_w;
						ie_escala_indice_p		:= 'CS';
					end if;
					end;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE consiste_regra_inf_rep ( cd_estabelecimento_p bigint, nr_atendimento_p bigint, cd_setor_atendimento_p bigint, cd_perfil_p bigint, ie_momento_p text, ie_prescricao_alta_p text, dt_prescricao_p timestamp, nm_usuario_p text, ds_mensagem_p INOUT text, ds_mensagem_adic_p INOUT text, ie_acao_p INOUT text, ie_anamnese_p INOUT text, ie_evolucao_p INOUT text, ie_receita_p INOUT text, ie_diagnostico_p INOUT text, ie_escala_indice_p INOUT text, nr_prescricao_p bigint default null, cd_pessoa_fisica_p text DEFAULT NULL, qt_peso_p bigint default null, ie_recomendacao_p INOUT text DEFAULT NULL, cd_protocolo_p INOUT bigint DEFAULT NULL, ie_parecer_p INOUT text DEFAULT NULL, ie_peso_p INOUT text DEFAULT NULL, ie_autorizacao_p INOUT text DEFAULT NULL, ie_laudo_sus_p INOUT text DEFAULT NULL, ie_avaliacao_p INOUT text DEFAULT NULL, ie_item_prontuario_p INOUT text DEFAULT NULL, ie_posicionar_pep_p INOUT text DEFAULT NULL) FROM PUBLIC;
