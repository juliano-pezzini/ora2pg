...
CREATE OR REPLACE PROCEDURE tiss_desfazer_lib_lote_anexo (nm_usuario_p text, nr_seq_lote_p bigint) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_sequencia_autor_w > 0) then
		...
		EXIT WHEN NOT FOUND; /* apply on C02 */
			...
			and (qt_autorizada 			<>  0 or
				vl_autorizado 			<> 0)
			and (cd_procedimento		= (cd_item_w)::numeric  or
				cd_procedimento_tuss		= cd_item_w or
				cd_procedimento_convenio	= cd_item_w);

			if (nr_sequencia_proc_w IS NOT NULL AND nr_sequencia_proc_w::text <> '') then
				update	procedimento_autorizado
				set	qt_autorizada	= qt_autorizada - qt_autorizada_w,
					vl_autorizado	= vl_autorizado - vl_autorizado_w,
					nm_usuario	= nm_usuario_p,
					dt_atualizacao	= clock_timestamp()
				...
				and (qt_autorizada		<> 0 or
					vl_unitario		<> 0)
				and (cd_material		= (cd_item_w)::numeric  or
					cd_material_convenio	= cd_item_w);

				if (nr_sequencia_mat_w IS NOT NULL AND nr_sequencia_mat_w::text <> '') then
					update	material_autorizado
					set	qt_autorizada		= qt_autorizada - qt_autorizada_w,
						vl_unitario		= vl_unitario - vl_autorizado_w,
						vl_total_autorizado	= vl_total_autorizado - vl_autorizado_w,
						nm_usuario		= nm_usuario_p,
						dt_atualizacao		= clock_timestamp()
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE tiss_desfazer_lib_lote_anexo (nm_usuario_p text, nr_seq_lote_p bigint) FROM PUBLIC;
