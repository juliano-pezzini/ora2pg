...
CREATE OR REPLACE PROCEDURE gerar_comunic_imp_lote_fornec ( nm_usuario_p text, cd_estabelecimento_p bigint, nr_seq_lote_fornec_p bigint, qt_impressao_p bigint) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	begin
		...
		ds_titulo_w := substr(replace_macro(ds_titulo_w, '@Qt_impressao', qt_impressao_p),1,80);
		ds_titulo_w := substr(replace_macro(ds_titulo_w, '@Qt_impressas', qt_impressas_w),1,80);
		...
		ds_comunicacao_w := substr(replace_macro(ds_comunicacao_w, '@Qt_impressao', qt_impressao_p),1,2000);
		ds_comunicacao_w := substr(replace_macro(ds_comunicacao_w, '@Qt_impressas', qt_impressas_w),1,2000);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_comunic_imp_lote_fornec ( nm_usuario_p text, cd_estabelecimento_p bigint, nr_seq_lote_fornec_p bigint, qt_impressao_p bigint) FROM PUBLIC;
