...
CREATE OR REPLACE PROCEDURE pls_gerar_valores_faturar ( nr_seq_reg_auxiliar_p ctb_livro_auxiliar.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) AS $body$
DECLARE
...
union all
		...
		r.nr_contrato,
		(select nr_sequencia from pls_contrato where nr_contrato = r.nr_contrato) nr_seq_contrato,
...
EXIT WHEN NOT FOUND; /* apply on c_conta */
	...
	insert into pls_aux_valores_faturar(
		nr_sequencia,
		dt_atualizacao,                 
		nm_usuario,                
		dt_atualizacao_nrec,            
	...
	values (	
		nextval('pls_aux_valores_faturar_seq'),
		clock_timestamp(),
		nm_usuario_p,
		clock_timestamp(),
		nm_usuario_p,
		...
		vet_conta.dt_vencimento,
		vet_conta.dt_recebimento,
		vet_conta.dt_cancelamento,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_valores_faturar ( nr_seq_reg_auxiliar_p ctb_livro_auxiliar.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) FROM PUBLIC;
