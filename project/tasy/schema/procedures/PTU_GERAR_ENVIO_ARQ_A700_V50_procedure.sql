...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_arq_a700_v50 ( nr_seq_servico_p bigint, cd_estabelecimento_p text, nm_usuario_p text) AS $body$
DECLARE
...
C00 CURSOR FOR 
	SELECT	lpad(coalesce(cd_unimed_destino,'0'),4,'0'), 
		...
		lpad(coalesce(nr_versao_transacao,'16'),2,'0') 
...
C01 CURSOR FOR 
	SELECT	lpad(coalesce(elimina_caractere_especial(to_char(nr_lote)),'0'),8,'0'), 
		lpad(coalesce(substr(elimina_caractere_especial(to_char(pls_obter_dados_conta(nr_nota,'G'))),1,20),'0'),20,'0'), 
...
C04 CURSOR FOR 
	SELECT	lpad(coalesce(elimina_caractere_especial(a.nr_lote),'0'),8,'0'), 
		...
		rpad(coalesce(elimina_caractere_especial(elimina_acentuacao(a.ds_complemento)),' '),100,' '), 
...
EXIT WHEN NOT FOUND; /* apply on C00 */
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		/* Em atendimentos ao TISS o campo tipo de saída ( IE_TIPO_SAIDA_SPDAT_W ) deve ser informado em atendimento com data superior a 31/08/2007 quando não houver internação. */
		...
		ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '702' || nr_lote_w || lpad(' ',12,' ') || cd_unimed_destino_w || '  ' || 
				cd_usuario_plano_w || nm_beneficiario_w || dt_atendimento_w || ' ' || ie_carater_atendimento_w || ' ' || 
				cd_cid_w || lpad(' ',14,' ') || ie_paciente_w || ie_tipo_saida_spdat_w || ie_tipo_atendimento_w || lpad(' ',11,' ') || nr_nota_w || 
				...
				motivo_encerram_w || nr_cnpj_cpf_req_w || nm_prest_req_w || sg_cons_prof_req_702_w || 
				nr_cons_prof_req_702_w || sg_uf_cons_req_702_w || nr_cbo_req_w || dt_protocolo_w || id_rn_w;
		...
		EXIT WHEN NOT FOUND; /* apply on C02 */
			...
			ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '703' || nr_lote_703_w || lpad(' ',11,' ') || cd_unimed_hospital_w || cd_hospital_w || 
					nm_hospital_w || ie_tipo_acomodacao_w || lpad(' ',42,' ') || tx_mult_amb_w || '  ' || cd_cgc_hospital_w || ie_tipo_internacao_w || 
					' ' || qt_nasc_vivos_w || qt_nasc_mortos_w || qt_nasc_vivos_pre_w || qt_obito_precoce_w || qt_obito_tardio_w || '     ' || 
					ie_faturamento_w || cd_cid_obito_w || lpad(' ',7,' ') || nr_declara_vivo_1_w || nr_declara_vivo_2_w || nr_declara_vivo_3_w || 
					nr_declara_vivo_4_w || nr_declara_vivo_5_w || ' ' || nr_declara_obito_w || cd_cid_obito_1_w || cd_cid_obito_2_w || cd_cid_obito_3_w || 
					cd_cid_obito_4_w || cd_cid_obito_5_w || nr_declara_obito_1_w || nr_declara_obito_2_w || nr_declara_obito_3_w || nr_declara_obito_4_w || 
					nr_declara_obito_5_w || nr_nota_703_w || reg_internacao_w;
		...
		EXIT WHEN NOT FOUND; /* apply on C03 */
			...
			-- OS 649133 - Para o TP_TABELA 0 ou 4, deve-se informar somente valores inteiros para o campo 
			if	((ie_tipo_tabela_w = '0') or (ie_tipo_tabela_w = '4')) then 
				qt_procedimento_w	:= lpad(coalesce(qt_procedimento_w,'0'),8,'0'); -- 1 PROCEDIMENTO FORMATO INTEIRO.: 00000001 
			else
				qt_procedimento_w	:= rpad(coalesce(qt_procedimento_w,'0'),8,'0'); -- 1 PROCEDIMENTO FORMATO NÃO INTEIRO.: 00010000 
			end if;
			...
			ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '704' || nr_lote_704_w || lpad(' ',11,' ') || 
					...
					cd_ref_material_fab_w || dt_pgto_prestador_w;
...
select	--replace(nvl(ptu_obter_qt_registro_serv(nr_seq_servico_p,'VL_SERV'),'0'),',',''), 
	lpad(coalesce(replace(replace(campo_mascara(coalesce(ptu_obter_qt_registro_serv(nr_seq_servico_p,'VL_SERV'),'0'),2),',',''),'.',''),'0'),14,'0'), 
...
ds_conteudo_w	:=	lpad(to_char(nr_seq_registro_w),8,'0') || '709' || lpad(to_char(qt_tot_r702_w),5,'0') || lpad(to_char(qt_tot_r703_w),5,'0') || 
			lpad(coalesce(qt_tot_r704_ww*10000,'0'),11,'0') || lpad(to_char(qt_tot_r705_w),5,'0') || lpad(' ',10,' ') || lpad(to_char(qt_tot_r704_ww),11,'0') || 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_arq_a700_v50 ( nr_seq_servico_p bigint, cd_estabelecimento_p text, nm_usuario_p text) FROM PUBLIC;
