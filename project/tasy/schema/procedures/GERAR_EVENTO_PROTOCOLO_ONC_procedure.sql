...
CREATE OR REPLACE PROCEDURE gerar_evento_protocolo_onc ( nr_seq_evento_p bigint, cd_pessoa_fisica_p text, cd_protocolo_p bigint, nr_seq_medicamento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
BEGIN 
select	coalesce(MAX(obter_ultimo_atendimento(cd_pessoa_fisica_p)),0) 
...
ds_mensagem_w	:= substr(replace_macro(ds_mensagem_w,'@ds_protocolo',substr(obter_desc_protocolo(cd_protocolo_p),1,255)),1,4000);
ds_mensagem_w	:= substr(replace_macro(ds_mensagem_w,'@ds_medicacao',substr(obter_desc_protocolo_medic(nr_seq_medicamento_p,cd_protocolo_p),1,50)),1,4000);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_evento_protocolo_onc ( nr_seq_evento_p bigint, cd_pessoa_fisica_p text, cd_protocolo_p bigint, nr_seq_medicamento_p bigint, nm_usuario_p text) FROM PUBLIC;
