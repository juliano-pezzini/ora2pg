...
WHERE a.nr_seq_conta			= c.nr_sequencia and a.nr_seq_conta			= nr_seq_conta_p and c.ie_status <> 'C' and a.ie_status != 'M' and (a.ie_lanc_manual_pos = 'N' or coalesce(a.ie_lanc_manual_pos::text, '') = '') and ie_somente_ocor_w	= 'N'
...
union all
	...
	FROM pls_conta c, pls_conta_proc a
LEFT OUTER JOIN procedimento b ON (a.cd_procedimento = b.cd_procedimento AND a.ie_origem_proced = b.ie_origem_proced)
WHERE a.nr_seq_conta			= c.nr_sequencia and a.nr_seq_conta			= nr_seq_conta_p and c.ie_status <> 'C' and a.ie_status != 'M' and (a.ie_lanc_manual_pos = 'N' or coalesce(a.ie_lanc_manual_pos::text, '') = '') and ie_somente_ocor_w	= 'S' and exists (	select	1
		...
		if	(((ie_minha_analise_w = 'S') and ((tb_ie_pend_grupo_w(index_w) IS NOT NULL AND (tb_ie_pend_grupo_w(index_w))::text <> ''))) or (ie_minha_analise_w = 'N')) and
			...
			if (ie_ocultar_canc_w = 'N') or
				...
				if (coalesce(nr_seq_grau_partic_w::text, '') = '') then
					...
					into STRICT 	nr_total_partic_w
					from 	pls_proc_participante
					where 	nr_seq_conta_proc = tb_nr_seq_conta_proc_w(index_w)
					and 	ie_status != 'C';

					--Se possuir apenas um participante, mostrar o grau de participação no registro principal do procedimento
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_w_analise_proc_html ( nr_seq_analise_p bigint, nr_seq_conta_p bigint, nr_seq_grupo_p bigint, ie_minha_analise_p text, ie_pendentes_p text, nm_usuario_p text, ie_somente_ocor_p text default 'N', ie_ocultar_canc_p text default 'N', nr_id_transacao_p w_pls_analise_item.nr_id_transacao%type DEFAULT NULL) FROM PUBLIC;
