...
CREATE OR REPLACE PROCEDURE gerar_interacao_medic_assoc (nr_sequencia_p bigint, nm_usuario_p text, ie_opcao_p bigint) AS $body$
DECLARE
...
if	((ie_opcao_p  = 1) or (ie_opcao_p = 2)) then --- Inserindo ou alterando a interacao medicamentosa
	if (ie_opcao_p = 2) then
		...
		from	material_interacao_medic
		where 	cd_material = cd_material_interacao_w
		and 	cd_material_interacao = cd_material_w;

		if (ie_verifica_w = 1) then
			CALL apagar_int_medic_alterada(cd_material_w,cd_material_interacao_w);
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_interacao_medic_assoc (nr_sequencia_p bigint, nm_usuario_p text, ie_opcao_p bigint) FROM PUBLIC;
