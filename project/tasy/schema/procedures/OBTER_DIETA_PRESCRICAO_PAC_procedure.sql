...
CREATE OR REPLACE PROCEDURE obter_dieta_prescricao_pac (cd_setor_p bigint, qt_hora_p bigint, dt_dieta_p timestamp, cd_refeicao_p text, nm_usuario_p text, cd_pessoa_fisica_p text, nr_seq_mapa_p bigint, cd_estabelecimento_p bigint default null) AS $body$
DECLARE


/* AO ALTERAR ESTA PROCEDURE, DEVE SER VERIFICADO SE A PROCEDURE Obter_Dieta_Prescricao PRECISA SER ALTERADA TAMBÉM */
...
varPrescrRN_w := obter_param_usuario(1000, 19, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_w, varPrescrRN_w);
varManterObsTecnica_w := obter_param_usuario(1000, 34, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_w, varManterObsTecnica_w);
varDadosJejum_w := obter_param_usuario(1000, 42, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_w, varDadosJejum_w);
ie_excluir_dietas_w := obter_param_usuario(1000, 93, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_w, ie_excluir_dietas_w);
ie_prescricao_dia_w := obter_param_usuario(1000, 102, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_w, ie_prescricao_dia_w);
ie_excluir_dieta_ant_jejum_w := obter_param_usuario(1000, 73, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_w, ie_excluir_dieta_ant_jejum_w);
ie_ultima_prescricao_pac_w := obter_param_usuario(1000, 105, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_w, ie_ultima_prescricao_pac_w);
ie_considera_alter_dieta_w := obter_param_usuario(1000, 106, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_w, ie_considera_alter_dieta_w);
ie_prescr_liberadas_enferm_w := obter_param_usuario(1000, 114, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_w, ie_prescr_liberadas_enferm_w);
ie_manter_observacao_w := obter_param_usuario(1000, 116, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_w, ie_manter_observacao_w);
ie_todas_prescr_dietas_w := obter_param_usuario(1000, 118, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_w, ie_todas_prescr_dietas_w);
...
ie_util_dt_inicio_presc_w := obter_param_usuario(1000, 136, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_w, ie_util_dt_inicio_presc_w);
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	if (ie_liberado_w = 'N') then
		...
		if (ie_prescricao_dia_w = 'N') then
		...
		elsif (ie_prescricao_dia_w = 'S') then
			select	coalesce(max(a.nr_prescricao),0)
			into STRICT	nr_prescricao_w
			...
			and	(((coalesce(a.dt_liberacao,a.dt_liberacao_medico) IS NOT NULL AND (coalesce(a.dt_liberacao,a.dt_liberacao_medico))::text <> '') and ie_prescr_liberadas_enferm_w = 'N')
			or	(a.dt_liberacao IS NOT NULL AND a.dt_liberacao::text <> ''))
			and 	CASE WHEN ie_util_dt_inicio_presc_w='S' THEN  CASE WHEN coalesce(b.hr_dose_especial,'X')='X' THEN  a.dt_inicio_prescr  ELSE to_date(to_char(a.dt_inicio_prescr,'dd/mm/yyyy')||' '||b.hr_dose_especial,'dd/mm/yyyy hh24:mi:ss') END   ELSE a.dt_prescricao END  between inicio_dia(clock_timestamp()) and fim_dia(clock_timestamp())
			and 	coalesce(b.ie_suspenso,'N')	= 'N'
			and	((coalesce(a.ie_recem_nato,'N') = 'N') or (a.ie_recem_nato = varPrescrRN_w));
		end if;
		...
		if (nr_prescricao_w > 0) and (ie_gerar_dieta_prescr_w = 'S') then
			...
			EXIT WHEN NOT FOUND; /* apply on c02 */
				...
				if (nr_seq_controle_w > 1) or (ie_excluir_dietas_w = 'S') then
					insert into mapa_dieta(
						...
						nm_usuario_nrec, dt_atualizacao_nrec, nr_seq_superior,nr_prescricao,
						ds_observacao_tec, ie_recem_nato,dt_dieta_prescr, nm_usuario_prescr)
					values (nr_sequencia_w, cd_pessoa_fisica_w, dt_dieta_p, cd_refeicao_P,
						ie_destino_dieta_w, 'A', clock_timestamp(), nm_usuario_p,
						cd_setor_p, cd_unidade_basica_w, cd_unidade_compl_w,
						cd_dieta_w, ds_observacao_w, nr_atendimento_w,qt_parametro_w,
						nm_usuario_p, clock_timestamp(), CASE WHEN nr_sequencia_w=nr_seq_superior_w THEN  null  ELSE nr_seq_superior_w END ,nr_prescricao_w,
						ds_observacao_tecnica_w, ie_recem_nato_w,clock_timestamp(), nm_usuario_p);
				else
					update 	mapa_dieta
					...
					nr_seq_superior_w	:= nr_seq_mapa_p;
				end if;
				end;
			...
			if (ie_regra_proced_w = 'S') then
				CALL md_atualizar_obs_proced(nr_prescricao_w, nr_seq_superior_w, nr_atendimento_w, qt_hora_p, dt_final_w, dt_dieta_p, cd_refeicao_p, '', nm_usuario_p, 'P');
			end if;
				...
				if (ie_recem_nato_w = 'S') or (ie_todas_prescr_dietas_w = 'S') then
					...
					if (coalesce(cd_dieta_sup_c04_w::text, '') = '') and (ie_verifica_se_dieta_c04_w = 'S') then
						...
						values (nr_sequencia_w, cd_pessoa_fisica_w, dt_dieta_p, cd_refeicao_P,
							...
							cd_setor_p, cd_unidade_basica_w, cd_unidade_compl_w,
							cd_dieta_w, ds_observacao_w, nr_atendimento_w,qt_parametro_w,
							nm_usuario_p, clock_timestamp(), CASE WHEN nr_sequencia_w=nr_seq_superior_c04_W THEN  null  ELSE nr_seq_superior_c04_W END ,nr_prescr_w,
							ds_observacao_tecnica_w, ie_recem_nato_w,clock_timestamp(), nm_usuario_p);
					end if;
				end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE obter_dieta_prescricao_pac (cd_setor_p bigint, qt_hora_p bigint, dt_dieta_p timestamp, cd_refeicao_p text, nm_usuario_p text, cd_pessoa_fisica_p text, nr_seq_mapa_p bigint, cd_estabelecimento_p bigint default null) FROM PUBLIC;
