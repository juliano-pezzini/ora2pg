...
CREATE OR REPLACE PROCEDURE gerar_cobr_santander_240 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
tx_desc_antecipacao_w			varchar(15);
vl_desc_previsto_w				varchar(15);
dt_limite_desconto_w			varchar(8);
cd_codigo_desc_w				varchar(1);
...
C01 CURSOR FOR
	SELECT	substr(coalesce(c.cd_ocorrencia,'01'),1,2) cd_mov_remessa,
		...
		lpad(somente_numero(to_char(b.tx_desc_antecipacao,'9999999999990.00')),15,'0') tx_desc_antecipacao,
		lpad(somente_numero(to_char(b.vl_desc_previsto,'9999999999990.00')),15,'0') vl_desc_previsto, 
        CASE WHEN coalesce(b.dt_limite_desconto::text, '') = '' THEN  '00000000'  ELSE to_char(b.dt_limite_desconto,'ddmmyyyy') END  dt_limite_desconto 		
...
EXIT WHEN NOT FOUND; /* apply on C01 */
 ...
 || nr_seq_cobr_escrit_p,1,4000)
	...
	/*OS 1847786 - Para valor de desconto ou % de desconto, é obrigatorio a data conforme layout*/

	if (coalesce(dt_limite_desconto_w,'00000000') <> '00000000') then
		if (coalesce(tx_desc_antecipacao_w,'000000000000000') <> '000000000000000') then
			cd_codigo_desc_w   := '2'; --Percentual ate a data informada – Informar o percentual no campo “percentual de desconto a ser concedido”	
			vl_desc_previsto_w := null;
		elsif (coalesce(vl_desc_previsto_w,'000000000000000') <> '000000000000000') then
			cd_codigo_desc_w   := '1'; --Valor fixo ate a data informada – Informar o valor no campo “valor de desconto a ser  concedido”. 	
			tx_desc_antecipacao_w := null;
		else
			cd_codigo_desc_w := '0'; --ISENTO
			dt_limite_desconto_w := '00000000';
		end if;
	else
		cd_codigo_desc_w := '0';
	end if;
	...
	ds_conteudo_w	:=	'033' || --001 - 003
				...
				coalesce(cd_codigo_desc_w,'0') || --142 - 142
				coalesce(dt_limite_desconto_w,'00000000') || --143 - 150
				lpad(coalesce(coalesce(tx_desc_antecipacao_w,vl_desc_previsto_w),'0'),15,'0') || --151 - 165
				lpad('0', 15, '0') || --166 - 180
				lpad('0', 15, '0') || --181 - 195
				rpad(' ', 25, ' ') || --196 - 220
				'0' || --221 - 221
	...
	if (vetor_mensagem_w.count	= 1) then
		...
		ds_conteudo_w	:=	substr('033' ||
					...
					lpad(coalesce(cd_mov_remessa_w,'0'),2,'0') ||
					'2' ||
					coalesce(vetor_mensagem_w[1].ds_mensagem,' ') ||
					rpad(' ',22,' '),1,240);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_cobr_santander_240 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
