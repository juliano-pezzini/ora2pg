...
CREATE OR REPLACE PROCEDURE gerar_trans_agendamentos_exame (ds_lista_p text, cd_agenda_p bigint, dt_agenda_p timestamp, ie_feriado_p text, ie_sobra_horario_p text, nm_usuario_p text, cd_estabelecimento_p bigint, nr_seq_atendimento_p bigint, nr_seq_evento_atend_p bigint, ds_erro_p INOUT text) AS $body$
DECLARE
...
while(ds_lista_w IS NOT NULL AND ds_lista_w::text <> '')  loop
	...
	where	nr_seq_agenda_exame	= nr_seq_agendamento_w;
	...
	if (nr_seq_livre_w > 0) then
		ds_erro_w := copiar_transferir_agenda_exame(
					cd_estabelecimento_p, 'S', nr_seq_agendamento_w, nr_seq_livre_w, 'T', null, null, nm_usuario_p, ds_erro_w);

			if (coalesce(ds_erro_w::text, '') = '') and (nr_seq_ageint_w IS NOT NULL AND nr_seq_ageint_w::text <> '') then

			update 	agenda_integrada_item
			set		nr_seq_agenda_exame	= nr_seq_livre_w,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_trans_agendamentos_exame (ds_lista_p text, cd_agenda_p bigint, dt_agenda_p timestamp, ie_feriado_p text, ie_sobra_horario_p text, nm_usuario_p text, cd_estabelecimento_p bigint, nr_seq_atendimento_p bigint, nr_seq_evento_atend_p bigint, ds_erro_p INOUT text) FROM PUBLIC;
