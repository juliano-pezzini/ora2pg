...
CREATE OR REPLACE PROCEDURE gap_incluir_etapas ( nr_sequencia_p bigint, nm_usuario_p text, ds_erro_p INOUT text) AS $body$
DECLARE
...
if (coalesce(ds_erro_p::text, '') = '') then
	...
	EXIT WHEN NOT FOUND; /* apply on c03 */
		...
		if (ie_regra_pessoa_w = 'D')	and (nr_seq_depto_w IS NOT NULL AND nr_seq_depto_w::text <> '')	then
			begin
			cd_pessoa_fisica_w	:= obter_resp_depto(nr_seq_depto_w,clock_timestamp());
			end;
		...
		insert into gap_processo_etapa(
			...
			dt_prevista,
			nr_seq_depto)
		values (	nextval('gap_processo_etapa_seq'),
			nr_sequencia_p,
			nr_sequencia_w,
			clock_timestamp(),
			nm_usuario_p,
	...
	EXIT WHEN NOT FOUND; /* apply on c02 */
		...
		insert into gap_processo_doc(
			...
			dt_atualizacao,
			nm_usuario)
		values (	nextval('gap_processo_doc_seq'),
			nr_sequencia_p,
			nr_sequencia_doc_w,
			clock_timestamp(),
			nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gap_incluir_etapas ( nr_sequencia_p bigint, nm_usuario_p text, ds_erro_p INOUT text) FROM PUBLIC;
