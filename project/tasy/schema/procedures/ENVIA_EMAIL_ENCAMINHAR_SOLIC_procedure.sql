...
CREATE OR REPLACE PROCEDURE envia_email_encaminhar_solic ( nr_seq_aprovacao_p bigint, nr_seq_proc_aprov_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (coalesce(nr_documento_w,0) > 0) and (nr_seq_regra_w > 0) and 
	...
	select	coalesce(cd_comprador_resp,0), 
		...
		substr(coalesce(obter_desc_centro_custo(cd_centro_custo),wheb_mensagem_pck.get_texto(299998)),1,255), 
		coalesce(cd_local_estoque,0), 
		substr(coalesce(obter_desc_local_estoque(cd_local_estoque),wheb_mensagem_pck.get_texto(299999)),1,255), 
		obter_nome_pf_pj(cd_pessoa_fisica,cd_fornec_sugerido), 
		substr(coalesce(obter_valor_dominio(4200,ie_tipo_servico),wheb_mensagem_pck.get_texto(300000)),1,255), 
	...
	ds_assunto_w	:=	substr( 
				...
				replace_macro(ds_assunto_w, 
					...
					'@nm_pessoa_enc',nm_pessoa_orig_w), 
					'@nm_usuario_enc',nm_usuario_p),1,4000);
	...
	if (ie_momento_envio_w = 'A') then 
		...
		exception 
		when others then 
			/*gravar__log__tasy(91301,'Falha ao enviar e-mail compras - Evento: 67 - Seq. Regra: ' || nr_seq_regra_w,nm_usuario_p);*/
 
			CALL gerar_hist_solic_sem_commit( 
					nr_documento_w, 
					wheb_mensagem_pck.get_texto(299745), 
					wheb_mensagem_pck.get_texto(300001, 'NR_SEQ_REGRA=' || nr_seq_regra_w), 
					'FGE', 
					nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE envia_email_encaminhar_solic ( nr_seq_aprovacao_p bigint, nr_seq_proc_aprov_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
