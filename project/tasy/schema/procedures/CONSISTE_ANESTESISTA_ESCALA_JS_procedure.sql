...
CREATE OR REPLACE PROCEDURE consiste_anestesista_escala_js ( lista_informacao_p text, nr_seq_escala_dir_p bigint, nm_usuario_p text, ds_consistencia_p INOUT text) AS $body$
DECLARE

					 
qt_anestesista_w		integer;
ds_consistencia_w		varchar(255);
ds_consistencia_ww		varchar(255);
...
while	(lista_informacao_w IS NOT NULL AND lista_informacao_w::text <> '') or 
	...
	if (nr_seq_escala_dir_p > 0) then 
		...
		ds_consistencia_ww := gerar_anestesista_escala(cd_profissional_w, nr_seq_escala_dir_p, nm_usuario_p, ds_consistencia_ww);
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE consiste_anestesista_escala_js ( lista_informacao_p text, nr_seq_escala_dir_p bigint, nm_usuario_p text, ds_consistencia_p INOUT text) FROM PUBLIC;
