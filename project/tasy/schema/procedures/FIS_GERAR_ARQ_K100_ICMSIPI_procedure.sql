...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_k100_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
fetch c_reg_K100 bulk collect into vetRegK100 limit 1000;
	...
	ds_linha_w := substr(ds_sep_w || vetRegK100[i].cd_reg ||
			     ds_sep_w || to_char(vetRegK100[i].dt_ini ,'ddmmyyyy') ||
			     ds_sep_w || to_char(vetRegK100[i].dt_fin ,'ddmmyyyy') || ds_sep_w,
			     1,
			     8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro		:=  vetRegK100[i].cd_reg;

	-------> K200
	/* Gera registros do K200*/

        nr_linha_w := fis_gerar_arq_K200_icmsipi(nr_seq_controle_p, vetRegK100[i].nr_sequencia, nr_linha_w);

	-------> K280
	/* Gera registros do K280*/

        nr_linha_w := fis_gerar_arq_K280_icmsipi(nr_seq_controle_p, vetRegK100[i].nr_sequencia, nr_linha_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_k100_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) FROM PUBLIC;
