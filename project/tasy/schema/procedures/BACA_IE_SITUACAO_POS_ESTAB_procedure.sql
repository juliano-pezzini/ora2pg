...
CREATE OR REPLACE PROCEDURE baca_ie_situacao_pos_estab () AS $body$
DECLARE

 
c01 CURSOR(	nr_seq_reg_ini_pc	pls_conta_pos_estabelecido.nr_sequencia%type, 
		nr_seq_reg_fim_pc	pls_conta_pos_estabelecido.nr_sequencia%type) FOR 
	SELECT	nr_sequencia 
	from	pls_conta_pos_estabelecido 
	where	nr_sequencia between nr_seq_reg_ini_pc and nr_seq_reg_fim_pc;

nr_seq_reg_min_w	pls_conta_pos_estabelecido.nr_sequencia%type;
nr_seq_reg_max_w	pls_conta_pos_estabelecido.nr_sequencia%type;

nr_seq_reg_ini_w	pls_conta_pos_estabelecido.nr_sequencia%type;
nr_seq_reg_fim_w	pls_conta_pos_estabelecido.nr_sequencia%type;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE baca_ie_situacao_pos_estab () FROM PUBLIC;
