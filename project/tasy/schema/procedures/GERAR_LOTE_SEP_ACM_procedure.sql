...
CREATE OR REPLACE PROCEDURE gerar_lote_sep_acm ( nr_prescricao_p bigint, nr_sequencia_p bigint, nr_seq_horario_p bigint, ie_so_aprazado_p text, nm_usuario_p text, ie_origem_lote_p text, ie_send_integration_p text default 'N') AS $body$
DECLARE
...
C01 CURSOR FOR  --especifico para itens ACM
	...
	and	coalesce(c.ie_acm,'N') = 'S'
	and	b.qt_dispensar_hor > 0
	and	coalesce(b.nr_seq_lote::text, '') = ''
	and	((ie_origem_lote_p <> 'AIS') or (c.ie_agrupador in (4,13)))
...
C04 CURSOR FOR
	...
	and		clock_timestamp() between ESTABLISHMENT_TIMEZONE_UTILS.todayAtTime(coalesce(dt_hora_inicio,clock_timestamp())) and ESTABLISHMENT_TIMEZONE_UTILS.todayAtTime(coalesce(dt_hora_fim,clock_timestamp()))
...
if (coalesce(nr_seq_regra_acm_w,0) > 0) then
...
values (nextval('ap_lote_log_seq'),
	...
	' cd_estabelecimento_w='           || cd_estabelecimento_w           || ' ie_classif_nao_padrao_w='    || ie_classif_nao_padrao_w    ||
	...
	' ie_gerar_solucao_separado_w='    || ie_gerar_solucao_separado_w    || ' nr_seq_regra_acm_w='         || nr_seq_regra_acm_w         ||
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_seq_turno_w <> nr_seq_turno_ant_w) or (nr_seq_classif_w <> nr_seq_classif_ant_w) or
      ...
      dt_inicio_turno_w	:= trunc(ESTABLISHMENT_TIMEZONE_UTILS.dateAtTime(dt_horario_w - 1, replace(hr_inicio_turno_w,'24:','00:')),'mi');
		else
			dt_inicio_turno_w	:= trunc(ESTABLISHMENT_TIMEZONE_UTILS.dateAtTime(dt_horario_w, replace(hr_inicio_turno_w,'24:','00:')),'mi');
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_lote_sep_acm ( nr_prescricao_p bigint, nr_sequencia_p bigint, nr_seq_horario_p bigint, ie_so_aprazado_p text, nm_usuario_p text, ie_origem_lote_p text, ie_send_integration_p text default 'N') FROM PUBLIC;
