...
CREATE OR REPLACE PROCEDURE hsm_gerar_movto_sint_oper ( nm_usuario_p text, cd_estab_p bigint, dt_mesano_referencia_p timestamp, ie_consignado_p text, cd_operacao_estoque_p text, cd_material_p bigint, cd_grupo_material_p bigint, cd_subgrupo_material_p bigint, cd_classe_material_p bigint, cd_local_estoque_p bigint, ie_tipo_local_p text, ie_dif_saldo_p text, ie_dif_valor_p text, ie_geral_p text, ie_Grupo_p text, ie_subgrupo_p text, ie_Classe_p text, ie_material_p text) AS $body$
DECLARE
...
and	obter_se_contido(a.cd_operacao_estoque, cd_operacao_estoque_p) = 'S'
...
from	local_estoque c,
	estrutura_material_v e,
	saldo_estoque a
where	a.dt_mesano_referencia	= add_months(dt_mesano_referencia_p,-1)
and	((cd_estabelecimento_w = 0) or
	(cd_estabelecimento_w > 0 AND a.cd_estabelecimento	= cd_estabelecimento_w))
and	a.cd_local_estoque	= c.cd_local_estoque
and	c.ie_proprio		= CASE WHEN ie_tipo_local_p='P' THEN 'S' WHEN ie_tipo_local_p='T' THEN 'N'  ELSE c.ie_proprio END 
		...
		FOR	ie_nivel_w in 1 .. 5 LOOP
			...
			if	(ie_nivel_w = 5 AND ie_material_p = 'S') or
				...
				if (nr_sequencia_w = 0) then
					...
					select	CASE WHEN 	ie_nivel_w=1 THEN substr(wheb_mensagem_pck.get_texto(310771),1,255) WHEN 	ie_nivel_w=2 THEN substr(obter_desc_estrut_mat(cd_grupo_w,'','',''),1,100) WHEN 	ie_nivel_w=3 THEN substr(obter_desc_estrut_mat('',cd_subgrupo_w,'',''),1,100) WHEN 	ie_nivel_w=4 THEN substr(obter_desc_estrut_mat('','',cd_classe_w,''),1,100) WHEN 	ie_nivel_w=5 THEN substr(obter_desc_estrut_mat('','','',cd_material_w),1,100) END
