...
CREATE OR REPLACE PROCEDURE germany_gerar_laudo_int_hl7 ( nr_seg_reg_p bigint, nm_usuario_p text, nr_seq_laudo_p INOUT bigint, nr_controle_ext_p text default null) AS $body$
DECLARE
...
if (coalesce(nr_prescricao_w,0) > 0) and (coalesce(nr_seq_prescricao_w,0) > 0) then
	...
	if (qt_existe_medico_w = 0) then
		...
		/*temporariamente utilizado o campo nr_rqe pois o terceiro enviado o nome do usuário ao invés do código*/
		...
		if (nr_seq_laudo_ant_w > 0) then
			...
			if (ie_cancelar_laudo_w = 'S') then
				...
				if (coalesce(nr_sequencia_ww,0)<>0) then
					begin
					CALL cancelar_laudo_paciente(nr_sequencia_ww,
								'C',
								nm_usuario_p,
								'');
					end;
			...
			values (	nr_seq_laudo_w,
					...
					nr_seq_propaci_w,
					nr_seq_prescricao_w,
					clock_timestamp(),--dt_liberacao_laudo_w,
					0,
					nm_medico_solicitante_w,
					'LL',
					cd_laudo_externo_w,
			...
			if (ie_alterar_medico_conta_w = 'S') then
				CALL Atualizar_Propaci_Medico_Laudo(	nr_seq_laudo_w,
								'EX',
								nm_usuario_p);
			end if;
			...
			if (nr_seq_laudo_atual_w > 0) then
				CALL gerar_copia_laudo_padrao(	nr_seq_laudo_atual_w,
								nr_seq_laudo_w,
								null,
								nm_usuario_p,
								'Excluido na integração');
	...
	CALL gravar_log_cdi(	88877,
			'Não encontrou prescr_procedimento',
			nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE germany_gerar_laudo_int_hl7 ( nr_seg_reg_p bigint, nm_usuario_p text, nr_seq_laudo_p INOUT bigint, nr_controle_ext_p text default null) FROM PUBLIC;
