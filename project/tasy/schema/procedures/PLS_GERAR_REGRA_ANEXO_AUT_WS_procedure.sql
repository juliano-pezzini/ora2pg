...
CREATE OR REPLACE PROCEDURE pls_gerar_regra_anexo_aut_ws ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nm_usuario_p text) AS $body$
DECLARE
...
ie_restringe_estab_w	boolean := false;
cd_estab_atual_w	smallint;

BEGIN

ie_restringe_estab_w 	:= pls_obter_se_controle_estab('RE') = 'S';
--cd_estab_atual_w 	:= wheb_usuario_pck.get_cd_estabelecimento; OS 1257534
...
for c_01 in c01 loop
	begin
		if (ie_restringe_estab_w)then
			...
			and 	coalesce(a.cd_estabelecimento, cd_estab_atual_w) = cd_estab_atual_w;
		else
			select  max(a.ie_tipo_anexo)
			into STRICT	ie_tipo_anexo_w
			from    pls_regra_anexo_guia a,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_regra_anexo_aut_ws ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nm_usuario_p text) FROM PUBLIC;
