...
CREATE OR REPLACE PROCEDURE copiar_cron_migr_adic_wscb ( cd_funcao_origem_p bigint, cd_funcao_destino_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (cd_funcao_origem_p IS NOT NULL AND cd_funcao_origem_p::text <> '') and (cd_funcao_destino_p IS NOT NULL AND cd_funcao_destino_p::text <> '') and (nm_usuario_p IS NOT NULL AND nm_usuario_p::text <> '') then
	...
	if (nr_seq_cronograma_w > 0) then
		...
		and	a.ie_tipo_obj_proj_migr = 'TWSelecaoCB';
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE copiar_cron_migr_adic_wscb ( cd_funcao_origem_p bigint, cd_funcao_destino_p bigint, nm_usuario_p text) FROM PUBLIC;
