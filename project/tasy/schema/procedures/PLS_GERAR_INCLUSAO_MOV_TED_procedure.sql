...
CREATE OR REPLACE PROCEDURE pls_gerar_inclusao_mov_ted ( nr_seq_lote_mov_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
ie_data_inclusao_w		pls_ted_regra_mov.ie_data_inclusao%type;
...
c01 CURSOR FOR 
	...
	and	(((ie_data_inclusao_w = 'C') and (trunc(a.dt_contratacao,'dd') between dt_referencia_inicial_w and dt_referencia_final_w)) or 
		 ((ie_data_inclusao_w = 'L') and (trunc(a.dt_liberacao,'dd') between dt_referencia_inicial_w and dt_referencia_final_w))) 
...
select	max(ie_data_inclusao) 
into STRICT	ie_data_inclusao_w 
from	pls_ted_regra_mov 
where	nr_seq_regra = nr_seq_regra_ted_w;
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	if (ie_titularidade_w = 'A')		or 
		...
		and	dt_referencia_inicial_w between dt_inicio_vigencia and coalesce(dt_fim_vigencia,dt_referencia_inicial_w);
		...
		select 	count(1) 
		into STRICT 	qtd_plano_sca_w 
		 
		where	exists (SELECT 	1 
				from	pls_ted_regra_mov_sca a 
				where 	a.nr_seq_regra_ted	= nr_seq_regra_ted_w);
		 
		if (qtd_plano_sca_w > 0) then 
			select 	count(1) 
			into STRICT 	qtd_plano_sca_w 
			...
			where	exists (SELECT	1 
					from	pls_ted_regra_mov_sca a, 
						pls_sca_vinculo b 
					where 	a.nr_seq_plano_sca 	= b.nr_seq_plano 
					and 	a.nr_seq_regra_ted	= nr_seq_regra_ted_w 
		...
		select	lpad(max(a.nr_ddd_telefone),4,' ') || substr(elimina_caracteres_especiais(max(a.nr_telefone)),1,11) 
		...
		values (	nextval('pls_movimentacao_benef_seq'),clock_timestamp(),nm_usuario_p,clock_timestamp(),nm_usuario_p, 
				...
				nr_seq_pagador_w, cd_matricula_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_inclusao_mov_ted ( nr_seq_lote_mov_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
