...
CREATE OR REPLACE PROCEDURE pls_tiss_gerar_w_req_spsadt (nr_sequencia_autor_p bigint, ds_dir_padrao_p text, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
c01 CURSOR FOR
	SELECT	nr_sequencia,
		...
		ie_origem_proced,
		qt_solicitado,
		coalesce(qt_procedimento,0) qt_autorizada,
		'P' ie_tipo_guia
...
union all
	...
	SELECT	nr_sequencia,
		pls_obter_seq_codigo_material(nr_seq_material,'') cd_procedimento,
		substr(pls_obter_desc_material(nr_seq_material),1,255),
		null ie_origem_proced,
		qt_solicitado,
		coalesce(qt_material,0) qt_autorizada,
		'M' ie_tipo_guia
...
if (coalesce(nr_sequencia_autor_p,0) > 0) then
	...
	select  a.nr_sequencia, coalesce(a.cd_senha,a.cd_senha_externa), a.dt_requisicao,
		coalesce(a.dt_validade_senha,a.dt_valid_senha_ext), a.dt_requisicao,
		a.nr_seq_plano, a.nr_seq_segurado, a.cd_medico_solicitante,
		a.nr_seq_prestador, substr(pls_obter_dados_segurado(a.nr_seq_segurado,'CR'),1,255), to_date(pls_obter_dados_cart_segurado(a.nr_seq_segurado,'V'),'dd/mm/yyyy'),
		b.cd_pessoa_fisica, (select max(c.ds_plano) from pls_plano c where c.nr_sequencia = a.nr_seq_plano) ds_plano, a.ie_carater_atendimento,
		a.cd_guia_principal, a.ds_observacao, substr(a.ds_indicacao_clinica,1,400),
		a.ie_tipo_atendimento, a.dt_requisicao, substr(a.nr_sequencia,1,20),
		a.ie_tipo_saida, (select cd_prestador from pls_prestador where nr_sequencia = a.nr_seq_prestador) cd_prestador, a.ie_tipo_consulta,
		a.ie_tipo_guia, a.nr_seq_prestador_exec, a.ie_cobertura_especial,
	...
	into STRICT	cd_guia_w,cd_senha_w,dt_autorizacao_w,
		...
		ie_tipo_saida_w,cd_prestador_solic_w,ie_tipo_consulta_w,
		ie_tipo_guia_w, nr_seq_prestador_exec_w, ie_cobertura_especial_w,
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		if (qt_proc_guia_w = 1) then
			...
			values (nextval('w_tiss_contratado_solic_seq'), clock_timestamp(), nm_usuario_p,
				nr_seq_guia_w, pls_obter_dados_prestador(nr_seq_prestador_w,'CGC'),	coalesce(cd_prestador_solic_w, pls_obter_dados_prestador(nr_seq_prestador_w,'CGC')),
		...
		if (qt_proc_guia_w = 5) then
			...
			if	((cpf_exec_w IS NOT NULL AND cpf_exec_w::text <> '') or (cgc_exec_w IS NOT NULL AND cgc_exec_w::text <> '')) then
				...
				cd_cnes_w		:= substr(pls_obter_cnes_prestador(nr_seq_prestador_exec_w),1,20);	
			...
			values (nextval('w_tiss_contratado_exec_seq'),clock_timestamp(),nm_usuario_p,
				 nr_seq_guia_w, cgc_exec_w, coalesce(cd_prestador_exec_w, cgc_exec_w),
	...
	CALL pls_tiss_completar_requisicao(nr_seq_guia_w, nm_usuario_p);
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_tiss_gerar_w_req_spsadt (nr_sequencia_autor_p bigint, ds_dir_padrao_p text, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
