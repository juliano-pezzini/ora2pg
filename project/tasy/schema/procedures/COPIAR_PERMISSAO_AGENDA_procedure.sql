...
CREATE OR REPLACE PROCEDURE copiar_permissao_agenda ( nm_usuario_p text, ds_agendas_p text, nr_seq_permissao_p bigint) AS $body$
DECLARE
...
SELECT	cd_agenda, 
	cd_pessoa_fisica 
from	agenda 
where	obter_Se_Contido(cd_agenda, '(' || ds_agendas_p || ')') = 'S';			
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE copiar_permissao_agenda ( nm_usuario_p text, ds_agendas_p text, nr_seq_permissao_p bigint) FROM PUBLIC;
