...
CREATE OR REPLACE PROCEDURE hem_gerar_calculos_av ( nr_seq_hem_calc_p bigint, nm_usuario_p text, ie_valvula_p text, ds_mensagem_p INOUT text) AS $body$
DECLARE
...
qt_pa_sistolica_w	hem_manometria_completa.qt_pa_sistolica%type;
qt_pa_sistolica_VE_w	hem_manometria_completa.qt_pa_sistolica%type;
qt_pa_sistolica_AO_w	hem_manometria_completa.qt_pa_sistolica%type;
...
if (coalesce(nr_seq_hem_calc_p, 0) > 0) then
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		if (ie_tipo_w = 'AO') then
			...
			qt_pa_sistolica_AO_w	:= qt_pa_sistolica_w;
		...
		if (ie_tipo_w = 'VE') then
			qt_pa_sistolica_VE_w	:= qt_pa_sistolica_w;
	...
	begin
		qt_fluxo_pulmonar_w	:= qt_fluxo_sist_w; 	--Fluxo Pulm (l/min)
	exception when others then
	...
	begin
		if (ie_valvula_p = 'A') then
			qt_area_valvula_w		:= (qt_deb_sist_w/qt_ench_sist_w)/(44.3*sqrt(coalesce(qt_gradiente_w,0)));
		else
			qt_area_valvula_w		:= (qt_deb_sist_w/qt_ench_sist_w)/(0.85*44.3*sqrt(coalesce(qt_gradiente_w,0)));
		end if;
	exception
	when others then
	...
	if (coalesce(qt_deb_sist_w::text, '') = '') then
		ds_erro_w	:= substr(obter_desc_expressao(643063)||';'||ds_erro_w,1,255);
	end if;
	if (coalesce(qt_fluxo_sist_w::text, '') = '') then
		ds_erro_w	:= substr(obter_desc_expressao(643064)||';'||	ds_erro_w,1,255);
	end if;
	if (coalesce(qt_fluxo_pulmonar_w::text, '') = '') then
		ds_erro_w	:= substr(obter_desc_expressao(643066)||';'||	ds_erro_w,1,255);
	end if;
	if (coalesce(qt_ind_card_w::text, '') = '') then
		ds_erro_w	:= substr(obter_desc_expressao(291869)||';'||	ds_erro_w,1,255);
	end if;
	if (coalesce(qt_rv_sistemica_w::text, '') = '') then
		ds_erro_w	:= substr(obter_desc_expressao(297957)||';'||	ds_erro_w,1,255);
	end if;
	if (coalesce(qt_resis_arteriola_w::text, '') = '') then
		ds_erro_w	:= substr(obter_desc_expressao(643763)||';'||	ds_erro_w,1,255);
	end if;
	if (coalesce(qt_rvp_rvs_w::text, '') = '') then
		ds_erro_w	:= substr(obter_desc_expressao(643068)||';'||	ds_erro_w,1,255);
	end if;
	if (coalesce(qt_rv_tot_w::text, '') = '') then
		ds_erro_w	:= substr(obter_desc_expressao(643070)||';'||	ds_erro_w,1,255);
	end if;
	if (coalesce(qt_gradiente_w::text, '') = '') then
		ds_erro_w	:= substr(obter_desc_expressao(643640)||';'||	ds_erro_w,1,255);
	end if;
	if (coalesce(qt_area_valvula_w::text, '') = '') then
		ds_erro_w	:= substr(obter_desc_expressao(643645)||';'||	ds_erro_w,1,255);
	end if;
	if (coalesce(qt_rv_sistemica_wood_w::text, '') = '') then
		ds_erro_w	:= substr(obter_desc_expressao(731502)||';'||	ds_erro_w,1,255);
	end if;
	if (coalesce(qt_rv_tot_wood_w::text, '') = '') then
		ds_erro_w	:= substr(obter_desc_expressao(731506)||';'||	ds_erro_w,1,255);
	end if;
	...
	if (qt_registro_w = 0) then
		...
		set	dt_atualizacao		= clock_timestamp(),
			...
			qt_gradiente		= qt_gradiente_w,
			qt_area_valvula		= qt_area_valvula_w,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE hem_gerar_calculos_av ( nr_seq_hem_calc_p bigint, nm_usuario_p text, ie_valvula_p text, ds_mensagem_p INOUT text) FROM PUBLIC;
