...
CREATE OR REPLACE PROCEDURE gerar_pagto_itau_cnab_240_v85 ( nr_seq_envio_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
WHERE b.nr_titulo		= c.nr_titulo and b.ie_tipo_pagamento	= ie_tipo_pagamento_w and CASE WHEN b.ie_tipo_pagamento='CC' THEN   CASE WHEN coalesce(c.cd_cgc::text, '') = '' THEN '01'  ELSE CASE WHEN substr(c.cd_cgc,1,8)=substr(cd_cgc_estab_w,1,8) THEN '06'  ELSE '01' END  END  WHEN b.ie_tipo_pagamento='DOC' THEN '03' WHEN b.ie_tipo_pagamento='TED' THEN     CASE WHEN coalesce(c.cd_cgc::text, '') = '' THEN '41'  ELSE CASE WHEN substr(c.cd_cgc,1,8)=substr(cd_cgc_estab_w,1,8) THEN '43'  ELSE '41' END  END  WHEN b.ie_tipo_pagamento='OP' THEN '30' WHEN b.ie_tipo_pagamento='BLQ' THEN       CASE WHEN b.cd_banco=341 THEN '30'  ELSE '31' END  WHEN b.ie_tipo_pagamento='CCP' THEN '01'  ELSE '03' END  = ie_forma_lanc_w and a.nr_sequencia		= b.nr_seq_escrit and a.nr_sequencia		= nr_seq_envio_p;
...
c03 CURSOR FOR
SELECT	distinct
    CASE WHEN(select    count(*)        from    gps_titulo x        where    x.nr_titulo    = b.nr_titulo)=0 THEN         CASE WHEN(select    count(*)            from    darf_titulo_pagar x            where    x.nr_titulo    = b.nr_titulo)=0 THEN             CASE WHEN     b.ie_tipo_pagamento='PC' THEN CASE WHEN c.ie_tipo_titulo=4 THEN '91'  ELSE '13' END  WHEN     b.ie_tipo_pagamento='BLQ' THEN CASE WHEN c.ie_tipo_titulo=16 THEN '35'  ELSE '13' END   ELSE CASE WHEN     CASE WHEN coalesce(d.cd_banco_externo::text, '') = '' THEN b.cd_banco  ELSE somente_numero(d.cd_banco_externo) END =cd_banco_cobr_w THEN '30'  ELSE CASE WHEN(select	count(*)			from	banco x                        where    CASE WHEN coalesce(x.cd_banco_externo::text, '') = '' THEN x.cd_banco  ELSE somente_numero(x.cd_banco_externo) END     = somente_numero(substr(c.nr_bloqueto,1,3)))=0 THEN '13'  ELSE '31' END  END  END   ELSE CASE WHEN     coalesce(c.nr_bloqueto::text, '') = '' THEN '16'  ELSE CASE WHEN c.ie_tipo_titulo=4 THEN '91'  ELSE '13' END  END  END   ELSE CASE WHEN coalesce(c.nr_bloqueto::text, '') = '' THEN '17'  ELSE CASE WHEN c.ie_tipo_titulo=4 THEN '91'  ELSE '13' END  END  END ,
	b.ie_tipo_pagamento,
...
SELECT	rpad(substr(obter_nome_pf_pj(c.cd_pessoa_fisica,c.cd_cgc),1,30),30,' '),
	...
	rpad(substr(coalesce(obter_nome_pf_pj(null,(select max(x.cd_cgc) from estabelecimento x where x.cd_estabelecimento = c.cd_estabelecimento)),' '),1,30),30,' ') -- Nome do contribuinte, no caso quem ta efetuando, OS 2878223
FROM banco_escritural a, titulo_pagar c
LEFT OUTER JOIN pessoa_fisica d ON (c.cd_pessoa_fisica = d.cd_pessoa_fisica)
, titulo_pagar_escrit b
LEFT OUTER JOIN banco e ON (b.cd_banco = e.cd_banco)
WHERE b.nr_titulo        = c.nr_titulo and CASE WHEN(select    count(*)        from    gps_titulo x        where    x.nr_titulo    = b.nr_titulo)=0 THEN 	CASE WHEN(select	count(*)            from    darf_titulo_pagar x            where    x.nr_titulo    = b.nr_titulo)=0 THEN             CASE WHEN     b.ie_tipo_pagamento='PC' THEN CASE WHEN c.ie_tipo_titulo=4 THEN '91'  ELSE '13' END  WHEN     b.ie_tipo_pagamento='BLQ' THEN CASE WHEN c.ie_tipo_titulo=16 THEN '35'  ELSE '13' END   ELSE CASE WHEN     CASE WHEN coalesce(e.cd_banco_externo::text, '') = '' THEN b.cd_banco  ELSE somente_numero(e.cd_banco_externo) END =cd_banco_cobr_w THEN '30'  ELSE CASE WHEN(select	count(*)			from	banco x                        where    CASE WHEN coalesce(x.cd_banco_externo::text, '') = '' THEN x.cd_banco  ELSE somente_numero(x.cd_banco_externo) END     = somente_numero(substr(c.nr_bloqueto,1,3)))=0 THEN '13'  ELSE '31' END  END  END   ELSE CASE WHEN     coalesce(c.nr_bloqueto::text, '') = '' THEN '16'  ELSE CASE WHEN c.ie_tipo_titulo=4 THEN '91'  ELSE '13' END  END  END   ELSE CASE WHEN coalesce(c.nr_bloqueto::text, '') = '' THEN '17'  ELSE CASE WHEN c.ie_tipo_titulo=4 THEN '91'  ELSE '13' END  END  END     = ie_forma_lanc_w  and b.ie_tipo_pagamento	= ie_tipo_pagamento_w and CASE WHEN(select	count(*) from	banco x where	x.cd_banco	= somente_numero(substr(c.nr_bloqueto,1,3)))=0 THEN  'I'  ELSE 'B' END  = ie_tipo_bloqueto_w and a.nr_sequencia		= b.nr_seq_escrit and a.nr_sequencia		= nr_seq_envio_p;
...
EXIT WHEN NOT FOUND; /* apply on c03 */
	...
	EXIT WHEN NOT FOUND; /* apply on c04 */
        ...
        /* segmento N - FGTS */
        ...
        elsif (ie_forma_lanc_w = '35' and ie_tipo_bloqueto_w = 'I') then
            ...
            ds_conteudo_w    :=    '341' || -- 001/003
                        ...
                        '11' || -- 018/019
                        rpad(' ',4,' ') ||  -- 020/023
                        ie_tipo_identificacao_w || -- 024/024 1 cnpj ou 2 CEI
                        nr_inscricao_w || -- 25/38 CPF OU CNPJ DO CONTRIBUINTE
                        rpad(coalesce(nr_bloqueto_w,' '),48,' ') || -- 39/86 CODIGO DE BARRAS
                        rpad(' ',16,' ')|| -- 87/102 IDENTIFICADOR DO FGTS
                        rpad(' ',9,' ')|| --103 111 LACRE DE CONECTIVIDADE SOCIAL
                        rpad(' ',2,' ')|| -- 112 113 DIGITO DO LACRE DE CONECTIVIDADE SOC
                        nm_pessoa_w || -- 114/143 NOME DO CONTRIBUINTE
                        to_char(dt_vencimento_w,'ddmmyyyy') || -- 144/151 DATA DO PAGAMENTO
                        lpad(somente_numero(to_char(coalesce(vl_escritural_w,0) - coalesce(vl_desconto_w,0) + coalesce(vl_acrescimo_w,0) + coalesce(vl_despesa_w,0) + coalesce(vl_juros_w,0) + coalesce(vl_multa_w,0),'999999999990.00')),14,'0')|| --152-165 VALOR DO PAGAMENTO
                        rpad(' ',30,' ') || -- 166/195 COMPLEMENTO DE REGISTRO
                        rpad(nr_titulo_w,20,' ') || --196/215 N DOCTO ATRIBUIDO PELA EMPRESA
                        rpad(' ',15,' ') || --216/230 NUMERO ATRIBUIDO PELO BANCO
                        rpad(' ',10,' ');		--231/240 	CODIGO DE OCORRENCIAS P/ RETORNO
		...
		/* segmento O */
			...
			ds_conteudo_w	:=	'341' ||
						...
						to_char(dt_vencimento_w,'ddmmyyyy') ||
						lpad(somente_numero(to_char(coalesce(vl_escritural_w,0) + coalesce(vl_multa_w,0) + coalesce(vl_juros_w,0) + coalesce(vl_acrescimo_w,0) + coalesce(vl_despesa_w,0),'9999999999990.00')),15,'0') ||
						rpad(' ',3,' ') ||
						rpad(' ',9,' ') ||
						rpad(' ',3,' ') ||
						rpad(nr_titulo_w,20,' ') ||
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_pagto_itau_cnab_240_v85 ( nr_seq_envio_p bigint, nm_usuario_p text) FROM PUBLIC;
