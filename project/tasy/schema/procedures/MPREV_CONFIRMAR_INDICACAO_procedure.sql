...
CREATE OR REPLACE PROCEDURE mprev_confirmar_indicacao (nr_seq_indicacao_p bigint, nm_usuario_p text) AS $body$
DECLARE


/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: Confirmar a indicação da Medicina Preventiva.
-------------------------------------------------------------------------------------------------------------------
...
if (nr_seq_indicacao_p IS NOT NULL AND nr_seq_indicacao_p::text <> '') then
	...
	set	dt_inclusao = clock_timestamp(),
		ie_status = 'P',
		nr_seq_forma_ingresso = (	SELECT	max(x.nr_seq_forma_ingresso)
						from	mprev_indicacao_paciente x
						where	x.nr_sequencia = nr_seq_indicacao_p)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE mprev_confirmar_indicacao (nr_seq_indicacao_p bigint, nm_usuario_p text) FROM PUBLIC;
