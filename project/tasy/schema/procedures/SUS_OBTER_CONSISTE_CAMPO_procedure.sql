...
/* Identificador de qual campo sera feita a consistencia */
...
if (ie_origem_proced_p = 7) then
	...
	if (nm_campo_p = 'PROFISSIONAL') then
		...
		ds_erro_proc_w := sus_consiste_proc(nr_interno_conta_p, cd_procedimento_p, ie_origem_proced_p, cd_profissional_exec_p, qt_procedimento_p, nr_atendimento_p, nr_seq_proc_p, nm_usuario_p, 2, dt_procedimento_p);
...
/* Teste se ha algum erro */

if (length(ds_erro_w) > 0) then
	ds_inco_w	:= ds_erro_w;
	ds_retorno_w	:= wheb_mensagem_pck.get_texto(311881)||chr(10)||chr(13);
	while (ds_inco_w IS NOT NULL AND ds_inco_w::text <> '') loop
		begin
		/* Busca a posicao do separados das inconsistencias, no caso a virgula */
		...
		/* Se existir apenas um espaco em branco entao limpa a variavel para sair do loop */
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE sus_obter_consiste_campo ( nm_campo_p text, cd_procedimento_p bigint, ie_origem_proced_p bigint, cd_profissional_exec_p text, nm_usuario_p text, cd_procedimento_solic_p bigint, ie_origem_proc_solic_p bigint, ie_mudanca_proc_p text, qt_procedimento_p bigint, cd_pessoa_fisica_p text, nr_atendimento_p bigint, nr_seq_proc_p bigint, nr_seq_proc_princ_p bigint, ie_funcao_p bigint, nr_interno_conta_p bigint, cd_estabelecimento_p bigint, dt_procedimento_p timestamp, ds_retorno_p INOUT text) FROM PUBLIC;
