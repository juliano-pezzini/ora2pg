...
CREATE OR REPLACE PROCEDURE gerar_evolucao_avaliacao_pac ( nr_sequencia_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
ds_enter_w		varchar(15)	:=  wheb_rtf_pck.get_quebra_linha || wheb_rtf_pck.get_quebra_linha;
...
if (IE_EVOLUCAO_CLINICA_w IS NOT NULL AND IE_EVOLUCAO_CLINICA_w::text <> '') and (ie_gerar_evolucao_w <> 'N') and (ie_existe_evol_w = 'N') then
	...
	ds_evolucao_w	:= ds_cabecalho_w|| ds_titulo_rft_w ||ds_conteudo_w||ds_rodape_w;	
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_evolucao_avaliacao_pac ( nr_sequencia_p bigint, nm_usuario_p text) FROM PUBLIC;
