...
CREATE OR REPLACE PROCEDURE gerar_retorno_banrisul_150 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_titulo_w IS NOT NULL AND nr_titulo_w::text <> '') then
		...
		where	a.cd_banco = 041
		and	a.cd_ocorrencia = cd_ocorrencia_w;

		/* Tratar acrescimos/descontos */

		if (vl_titulo_w <> vl_cobranca_w) then
			vl_alterar_w	:= vl_cobranca_w - vl_titulo_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_retorno_banrisul_150 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
