...
CREATE OR REPLACE PROCEDURE dime_reg_entradas_uf ( nr_seq_controle_p bigint, cd_estabelecimento_p text, nm_usuario_p text, dt_referencia_p timestamp, ds_separador_p text, qt_linha_p INOUT bigint, nr_sequencia_p INOUT bigint) AS $body$
DECLARE
...
c_valores CURSOR FOR 
	...
	and	exists (SELECT	1 
			...
			and	(substr(o.cd_cfop,1,1))::numeric  in (1,2,3)) 
...
ds_linha_w	:= 	tp_registro_w								|| separador_w || -- campo 01: tipo de registro [tamanho 02] preencher com "22" 
			...
			lpad(replace(campo_mascara(vl_contabil_total_w,2),'.',''),17,'0')	|| separador_w || 
			lpad(replace(campo_mascara(vl_base_calculo_total_w,2),'.',''),17,'0')	|| separador_w || 
			lpad(replace(campo_mascara(vl_outras_total_w,2),'.',''),17,'0')		|| separador_w || 
			lpad(replace(campo_mascara(0,2),'.',''),17,'0')				|| separador_w || 
			lpad(replace(campo_mascara(0,2),'.',''),17,'0')				|| separador_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dime_reg_entradas_uf ( nr_seq_controle_p bigint, cd_estabelecimento_p text, nm_usuario_p text, dt_referencia_p timestamp, ds_separador_p text, qt_linha_p INOUT bigint, nr_sequencia_p INOUT bigint) FROM PUBLIC;
