...
CREATE OR REPLACE PROCEDURE gerar_cobranca_safra_240_reg ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
select	'1' ie_tipo_registro, 
	...
	rpad(obter_dados_pf_pj(null, b.cd_cgc,'CM'),15,' '), 
...
ds_conteudo_w	:= 	cd_banco_w || --1 3 
					lpad(coalesce(nr_seq_lote_w,0),4,'0') || --4 7 
					'1' || --8 8 
					'C' || --9 9 
					'01' || --10 11 
					rpad(' ',2,' ') || --12 13 
					...
					nr_inscricao_w || --19 32	 
					rpad(' ',20,' ') || --33 52 
					cd_agencia_bancaria_w || --cd_agencia_bancaria_w || --53 57 
					nr_digito_agencia_w || --58 58 
					nr_conta_corrente_w || --59 71 
					' ' || --72 72 
					nm_empresa_w || --73 102 
					rpad(' ',40,' ') || --103 142 
					substr(ds_endereco_w,1,30) || -- 143 172 
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	ds_conteudo_w	:=	cd_banco_w || --1 3 
				...
				'1' || --118 118 
				'00000000' || --119 126 
				lpad(somente_numero(to_char(coalesce(vl_juros_acobrar_w,0),'0000000000000.00')),15,'0') || --127 141 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_cobranca_safra_240_reg ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
