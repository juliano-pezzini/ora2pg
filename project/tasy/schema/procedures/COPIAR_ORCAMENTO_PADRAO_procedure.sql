...
CREATE OR REPLACE PROCEDURE copiar_orcamento_padrao ( cd_estabelecimento_orig_p bigint, cd_estabelecimento_dest_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	where	cd_estabelecimento	= cd_estabelecimento_orig_p
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	values (
		...
		c01_w.ds_orcamento_padrao,
		clock_timestamp(),
		clock_timestamp(),
		c01_w.ie_situacao,
		nm_usuario_p,
		nm_usuario_p,
        c01_w.ie_autor_padrao,
		nr_seq_orcamento_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE copiar_orcamento_padrao ( cd_estabelecimento_orig_p bigint, cd_estabelecimento_dest_p bigint, nm_usuario_p text) FROM PUBLIC;
