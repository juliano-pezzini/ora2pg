...
CREATE OR REPLACE PROCEDURE gerar_lote_sep_alto_custo ( nr_prescricao_p bigint, nr_sequencia_p bigint, nr_seq_horario_p bigint, ie_so_aprazado_p text, nm_usuario_p text, ie_origem_lote_p text, ie_send_integration_p text default 'N') AS $body$
DECLARE
...
C01 CURSOR FOR  --especifico para medicamentos de alto custo
	...
	and	obter_se_mat_alto_custo(b.cd_material,a.cd_estabelecimento) = 'S'
	...
	(coalesce(ie_itens_associados_w,'S') = 'S' and ((coalesce(b.nr_seq_superior::text, '') = '') or (exists (	SELECT	1
																						...
																						and 	obter_se_mat_alto_custo(d.cd_material,a.cd_estabelecimento) = 'N')))))
...
if (coalesce(nr_seq_regra_alto_custo_w,0) > 0) then
...
/*Fabio e Jonas - Para que busque a parametrizacao do ADEP e caso for D(Depende da regra) ai entao e 
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_seq_turno_w <> nr_seq_turno_ant_w) or (nr_seq_classif_w <> nr_seq_classif_ant_w) or
		...
		values ( nr_sequencia_w,
			...
			qt_prioridade_w,
			ie_origem_lote_p,
      		'AC', -- 'Alto custo' se refere a regra de geracao,  devendo ser fixa, portanto nao deve ser substituido por uma expressao
			'AC',
			nr_atendimento_w);
	 if (ie_send_integration_p ='S') then              		                    CALL acceptance_submission_cancel(nr_sequencia_w,clock_timestamp(),null,'A',nr_prescricao_p,nm_usuario_p);
	end if;
		end;
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_lote_sep_alto_custo ( nr_prescricao_p bigint, nr_sequencia_p bigint, nr_seq_horario_p bigint, ie_so_aprazado_p text, nm_usuario_p text, ie_origem_lote_p text, ie_send_integration_p text default 'N') FROM PUBLIC;
