...
CREATE OR REPLACE PROCEDURE rep_copia_suporte_nut ( nr_prescricao_orig_p bigint, nr_prescricao_p bigint, dt_prescricao_p timestamp, nr_seq_regra_p bigint, nr_seq_material_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
ie_prescricao_dieta_w	varchar(15);
...
c02 CURSOR FOR
	...
	and (coalesce(ie_agora,'S') = 'S' 		or coalesce(c01_w.ie_urgencia,'N') <> 'S')
	and		ie_tipo_item	= 'SNE'
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	EXIT WHEN NOT FOUND; /* apply on C02 */
		...
		nr_seq_regra_copia_w	:= nr_seq_regra_copia_w;
	...
	if (ie_copia_w	= 'S') then
		...
		while(ie_loop_w = 'S') loop
			...
			and		nr_agrupamento	= nr_agrupamento_w LIMIT 1;
		...
		c01_w.nr_agrupamento		:= nr_agrupamento_w;
		...
		if (ie_regra_geral_w = 'H') then
			if (ie_manter_intervalo_w = 'S') and (c01_w.cd_intervalo IS NOT NULL AND c01_w.cd_intervalo::text <> '') then
				...
				if (coalesce(qt_operacao_w,0) > 0) and (dt_inicio_item_w IS NOT NULL AND dt_inicio_item_w::text <> '') then
					...
					if (ie_operacao_w in ('H')) and (dt_inicio_item_w not between dt_inicio_prescr_w and (dt_inicio_item_w + qt_operacao_w/24)) then

						while(dt_inicio_item_w not between dt_inicio_prescr_w and (dt_inicio_item_w + qt_operacao_w/24)) loop
							dt_inicio_item_w := dt_inicio_item_w + qt_operacao_w/24;
						end loop;
					end if;
				else
				...
				if (c01_w.hr_prim_horario <> '') and (c01_w.hr_prim_horario <> ' : ') and (c01_w.hr_prim_horario < to_char(dt_inicio_prescr_w,'hh24:mi')) then
					c01_w.qt_dia_prim_hor := 1;
				end if;

			elsif (ie_manter_intervalo_w = 'N') then

				dt_ultimo_horario_w	:= PLT_obter_ultimo_horario(nr_prescricao_orig_p, c01_w.nr_sequencia, 'SNE', nm_usuario_p);
...
Obter_ocorrencia_intervalo(c01_w.cd_intervalo, coalesce(nr_horas_validade_w,24),'H')/ 24,'hh24:mi');
		...
		values (
			...
			c01_w.nr_prescricao,
			c01_w.nr_sequencia,
			c01_w.nr_seq_produto);

		Select	max(ie_prescricao_dieta),
		...
		c01_w.nr_ocorrencia		:= coalesce(c01_w.nr_ocorrencia,0);

		if (ie_prescricao_dieta_w <> 'S') and (ie_prescricao_dieta_w <> 'I') then
			c01_w.nr_ocorrencia		:= obter_ocorrencias_horarios_rep(c01_w.ds_horarios);
		elsif (ie_prescricao_dieta_w <> 'I') then
			c01_w.nr_ocorrencia		:= qt_operacao_w;
		end if;

		if (c01_w.nr_ocorrencia = 0) then
			c01_w.nr_ocorrencia		:= obter_ocorrencia_intervalo(c01_w.cd_intervalo, nr_horas_validade_w, 'O');
		end if;

		c01_w.nr_ocorrencia	:= round(c01_w.nr_ocorrencia);
		c01_w.qt_material	:= coalesce(c01_w.qt_material,1);

		SELECT * FROM Obter_Quant_Dispensar(	cd_estabelecimento_w, c01_w.cd_material, nr_prescricao_p, nr_sequencia_w, c01_w.cd_intervalo, c01_w.ie_via_aplicacao, c01_w.qt_unitaria, 0, c01_w.nr_ocorrencia, c01_w.ds_dose_diferenciada, c01_w.ie_origem_inf, c01_w.cd_unidade_medida_dose, 1, c01_w.qt_material, c01_w.qt_total_dispensar, c01_w.ie_regra_disp, ds_erro_w, c01_w.ie_se_necessario, c01_w.ie_acm) INTO STRICT c01_w.qt_material, c01_w.qt_total_dispensar, c01_w.ie_regra_disp, ds_erro_w;

		update	prescr_material
		set 	nr_ocorrencia		= c01_w.nr_ocorrencia,
				qt_total_dispensar	= c01_w.qt_total_dispensar,
				qt_material			= coalesce(c01_w.qt_material,qt_material),
				ie_regra_disp		= CASE WHEN coalesce(ie_regra_disp,'X')='D' THEN  ie_regra_disp  ELSE c01_w.ie_regra_disp END
		where	nr_sequencia		= nr_sequencia_w
		and		nr_prescricao 		= nr_prescricao_p;

		CALL gerar_elemento_mat_sne( nr_prescricao_p, nr_sequencia_w, 'G', nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE rep_copia_suporte_nut ( nr_prescricao_orig_p bigint, nr_prescricao_p bigint, dt_prescricao_p timestamp, nr_seq_regra_p bigint, nr_seq_material_p bigint, nm_usuario_p text) FROM PUBLIC;
