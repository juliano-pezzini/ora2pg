...
CREATE OR REPLACE PROCEDURE gerar_dic_obj_idioma_wdp (cd_funcao_p bigint, nr_seq_idioma_p bigint, nm_usuario_p text) AS $body$
DECLARE
    ...
    c02 CURSOR FOR
        SELECT a.nr_sequencia,
          ...
          from dic_objeto a
         where a.cd_funcao = cd_funcao_p
           and a.ie_tipo_objeto = 'AWDP'
           and (a.nm_campo_tela IS NOT NULL AND a.nm_campo_tela::text <> '')
           and coalesce(a.ie_tipo_atrib_wdp, 'SQL') in ('T', 'TXT')
           and not exists (SELECT 1
                  from dic_objeto_idioma x
                 where x.nr_seq_objeto = a.nr_sequencia
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_dic_obj_idioma_wdp (cd_funcao_p bigint, nr_seq_idioma_p bigint, nm_usuario_p text) FROM PUBLIC;
