...
CREATE OR REPLACE PROCEDURE pls_oc_cta_tratar_val_78 ( dados_regra_p pls_tipos_ocor_pck.dados_regra, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) AS $body$
DECLARE



/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade:	Aplicar a validação das classificação do porte do item
-------------------------------------------------------------------------------------------------------------------
...
aaschlote OS 616661 - Criação da procedure.
------------------------------------------------------------------------------------------------------------------
...
dt_procedimento_ant_w		pls_conta_proc.dt_procedimento_referencia%type := to_date('01/01/1899');
ie_ocorrencia_w 		pls_controle_estab.ie_ocorrencia%type := pls_obter_se_controle_estab('GO');
ds_comando_w			varchar(1000);

C00 CURSOR(	nr_id_transacao_pc	pls_oc_cta_selecao_ocor_v.nr_sequencia%type) FOR
...
C02 CURSOR(	nr_id_transacao_pc	pls_oc_cta_selecao_ocor_v.nr_sequencia%type,
	...
	SELECT	sel.nr_sequencia nr_seq_selecao,
		...
		proc.dt_atendimento_conta,
		sel.nr_seq_segurado,
		sel.cd_guia_referencia,
		proc.ie_origem_conta,
		pls_obter_classific_porte(proc.cd_procedimento, proc.ie_origem_proced,
					  proc.ie_origem_conta, proc.dt_atendimento_conta, cd_estabelecimento_pc) cd_classificacao,
	...
	and proc.nr_seq_analise = nr_seq_analise_pc
...
/*Esse cursor foi duplicado no cursor C07, a diferença que no C07 é realizado tratamento por estabelecimento. CASO ALTERAR ESSE CURSOR ALTERAR O C07 TAMBÉM*/
...
c04 CURSOR(	nr_id_transacao_pc		pls_oc_cta_selecao_ocor_v.nr_sequencia%type,
		nr_seq_conta_proc_pc		pls_oc_cta_selecao_ocor_v.nr_seq_conta_proc%type,
		nr_seq_segurado_pc		pls_oc_cta_selecao_ocor_v.nr_seq_segurado%type,
		cd_guia_referencia_pc		pls_oc_cta_selecao_ocor_v.cd_guia_referencia%type,
		...
		dt_procedimento_pc		pls_conta_proc.dt_procedimento_referencia%type,
		ie_considera_data_pc		pls_oc_cta_val_porte.ie_considera_data%type,
		nr_seq_analise_pc		pls_analise_conta.nr_sequencia%type) FOR
	SELECT	sel.nr_sequencia nr_seq_selecao
	from	pls_oc_cta_selecao_ocor_v	sel,
		pls_conta_proc_ocor_v		proc
...
C05 CURSOR(	nr_id_transacao_pc	pls_oc_cta_selecao_ocor_v.nr_sequencia%type,
		cd_estabelecimento_pc	estabelecimento.cd_estabelecimento%type,
		nr_seq_analise_pc		pls_analise_conta.nr_sequencia%type)	FOR
	SELECT	sel.nr_sequencia nr_seq_selecao,
		sel.nr_seq_conta_proc,
...
/*Cursor dos procedimentos do Porte conforme a validação retorna apenas aqueles vinculados a um porte com tipo conta = intercambio ou todos*/


/*Esse cursor foi duplicado no cursor C08, a diferença que no C08 é realizado tratamento por estabelecimento. CASO ALTERAR ESSE CURSOR ALTERAR O C08 TAMBÉM*/
...
C06 CURSOR( 	ie_tipo_validacao_pc	pls_oc_cta_val_porte.ie_tipo_validacao%type,	
	...
	and 	b.cd_classificacao 	< cd_classificacao_pc
	and	b.nr_sequencia	 	= c.nr_seq_classif_porte 
	and	coalesce(c.ie_tipo_conta,'T') in ('T','I')
	and	dt_atendimento_conta_pc between b.dt_inicio_vigencia and coalesce(b.dt_fim_vigencia,dt_atendimento_conta_pc)	
	and	dt_atendimento_conta_pc between c.dt_inicio_vigencia and coalesce(c.dt_fim_vigencia,dt_atendimento_conta_pc)	
...
C07 CURSOR( 	ie_tipo_validacao_pc	pls_oc_cta_val_porte.ie_tipo_validacao%type,	
		dt_atendimento_conta_pc	timestamp,
...
/*Esse cursor foi duplicado do cursor C06, a diferença que no C06 não é realizado tratamento por estabelecimento. CASO ALTERAR ESSE CURSOR ALTERAR O C06 TAMBÉM*/

C08 CURSOR( 	ie_tipo_validacao_pc	pls_oc_cta_val_porte.ie_tipo_validacao%type,	
		dt_atendimento_conta_pc	timestamp,
		cd_classificacao_pc	pls_grau_classif_item.cd_classificacao%type,
		cd_estabelecimento_pc	estabelecimento.cd_estabelecimento%type) FOR
	SELECT	b.cd_procedimento,
	...
	for r_C01_w in C01(dados_regra_p.nr_sequencia) loop
		...
		if (r_C01_w.ie_validar_porte = 'S')	then
			...
			if (r_C01_w.ie_considera_data = 'N') then
				...
				for r_c00_w in c00(nr_id_transacao_p) loop
			
					ie_executa_w := 'S';
			
					for r_C02_w in C02(nr_id_transacao_p, cd_estabelecimento_p, r_c00_w.nr_seq_analise) loop
						...
						if (r_C02_w.cd_classificacao IS NOT NULL AND r_C02_w.cd_classificacao::text <> '')
						and (ie_executa_w = 'S') then
							...
							if (r_C02_w.ie_origem_conta <> 'A') then
								...
								if (ie_ocorrencia_w = 'S') then
									...
									for r_C07_w in C07(r_c01_w.ie_tipo_validacao, r_c02_w.dt_atendimento_conta, r_C02_w.cd_classificacao, cd_estabelecimento_p) loop						
											cd_procedimento_porte_w(C07%rowCount)	:= r_C07_w.cd_procedimento;
											ie_origem_proced_porte_w(C07%rowCount)	:= r_C07_w.ie_origem_proced;							
									end loop;
								else
									/*Coloca num table o procedimento e sua origem das classificações, conforme a regra de ordenação da ocorrência*/
								...
								if (ie_ocorrencia_w = 'S') then
									...
									for r_C08_w in C08(r_c01_w.ie_tipo_validacao, r_c02_w.dt_atendimento_conta, r_C02_w.cd_classificacao, cd_estabelecimento_p) loop						
											cd_procedimento_porte_w(C08%rowCount)	:= r_C08_w.cd_procedimento;
											ie_origem_proced_porte_w(C08%rowCount)	:= r_C08_w.ie_origem_proced;							
									end loop;
								else
									/*Coloca num table o procedimento e sua origem das classificações, conforme a regra de ordenação da ocorrência*/

									for r_C06_w in C06(r_c01_w.ie_tipo_validacao, r_c02_w.dt_atendimento_conta, r_C02_w.cd_classificacao) loop						
							...
							if (cd_procedimento_porte_w.count > 0) then						
								for i in cd_procedimento_porte_w.first .. cd_procedimento_porte_w.count loop
									...
									for r_C04_w in C04(nr_id_transacao_p,r_C02_w.nr_seq_conta_proc,r_C02_w.nr_seq_segurado,
											...
											r_C02_w.dt_procedimento_trunc, r_C01_w.ie_considera_data, r_c00_w.nr_seq_analise) loop
				...
				for r_c00_w in c00(nr_id_transacao_p) loop
					for r_C05_w in C05(nr_id_transacao_p, cd_estabelecimento_p, r_c00_w.nr_seq_analise) loop
						...
						if (r_C05_w.cd_classificacao IS NOT NULL AND r_C05_w.cd_classificacao::text <> '') and (r_C05_w.dt_procedimento_trunc <> dt_procedimento_ant_w) then
							...
							if (ie_ocorrencia_w = 'S') then
								...
								for r_C07_w in C07(r_c01_w.ie_tipo_validacao, r_C05_w.dt_atendimento_conta, r_C05_w.cd_classificacao, cd_estabelecimento_p) loop
									cd_procedimento_porte_w(C07%rowCount)	:= r_C07_w.cd_procedimento;
									ie_origem_proced_porte_w(C07%rowCount)	:= r_C07_w.ie_origem_proced;
								end loop;
							else
							...
							if (r_C05_w.ie_origem_conta <> 'A') then
								...
								if (ie_ocorrencia_w = 'S') then
									...
									for r_C08_w in C08(r_c01_w.ie_tipo_validacao, r_c05_w.dt_atendimento_conta, r_C05_w.cd_classificacao, cd_estabelecimento_p) loop						
											cd_procedimento_porte_w(C08%rowCount)	:= r_C08_w.cd_procedimento;
											ie_origem_proced_porte_w(C08%rowCount)	:= r_C08_w.ie_origem_proced;							
									end loop;
								else
									...
									for r_C06_w in C06(r_c01_w.ie_tipo_validacao, r_c05_w.dt_atendimento_conta, r_C05_w.cd_classificacao) loop						
											cd_procedimento_porte_w(C06%rowCount)	:= r_C06_w.cd_procedimento;
											ie_origem_proced_porte_w(C06%rowCount)	:= r_C06_w.ie_origem_proced;							
									end loop;
								end if;						
							...
							if (cd_procedimento_porte_w.count > 0) then
								for i in cd_procedimento_porte_w.first .. cd_procedimento_porte_w.count loop
									...
									for r_C04_w in C04(nr_id_transacao_p,r_C05_w.nr_seq_conta_proc,r_C05_w.nr_seq_segurado,
											...
											r_C05_w.dt_procedimento_trunc, r_C01_w.ie_considera_data, r_c00_w.nr_seq_analise) loop
										dados_tb_selecao_w.ie_valido(nr_idx_w)		:= 'S';
										dados_tb_selecao_w.nr_seq_selecao(nr_idx_w)	:= r_C04_w.nr_seq_selecao;
										dados_tb_selecao_w.ds_observacao(nr_idx_w)	:= '';
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_oc_cta_tratar_val_78 ( dados_regra_p pls_tipos_ocor_pck.dados_regra, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) FROM PUBLIC;
