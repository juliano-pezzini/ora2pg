...
CREATE OR REPLACE PROCEDURE gerar_w_relat_compras_4_meses (dt_referencia_p timestamp, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
qt_mes_w		bigint 	:= 0;
qt_notas_w		bigint 	:= 0;
vl_unitario_item_w	double precision	:= 0;
vl_media_unitario_w	double precision	:= 0;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_w_relat_compras_4_meses (dt_referencia_p timestamp, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
