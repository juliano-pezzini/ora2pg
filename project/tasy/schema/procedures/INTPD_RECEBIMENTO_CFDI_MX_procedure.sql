...
CREATE OR REPLACE PROCEDURE intpd_recebimento_cfdi_mx ( ds_xml_p text, ds_nome_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text, ds_retorno_p INOUT text) AS $body$
DECLARE
...
exception when xml_parsing_failed then
	...
	ds_log_w := substr(obter_desc_expressao(580270) || ' XML inválido. '|| obter_desc_expressao(327167) || SQLSTATE ||' - '|| sqlerrm || ' ' || dbms_utility.format_error_backtrace,1, 2000);
...
begin
	...
	CALL intpd_recebe_interno_cfdi_mx(nm_usuario_p, cd_estabelecimento_p, nr_sequencia_w, ds_aux_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE intpd_recebimento_cfdi_mx ( ds_xml_p text, ds_nome_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text, ds_retorno_p INOUT text) FROM PUBLIC;
