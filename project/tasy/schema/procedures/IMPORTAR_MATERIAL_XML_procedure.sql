...
nr_ncm_w 						material_ncm.nr_ncm%type;
ds_ncm_w 						material_ncm.ds_ncm%type;
ie_atualizar_classe_w			parametros_opme.ie_atualizar_classe%type;
ie_atualiza_tuss_w				parametros_opme.ie_atualiza_tuss%type;
ie_atualiza_situacao_w			parametros_opme.ie_atualiza_situacao%type;
...
ie_permite_atualizacao_mat_w	parametros_opme.ie_permite_atualizacao_mat%type;
ie_consiste_fornecedor_w 		parametros_opme.ie_consiste_fornecedor%type;
nr_seq_fabric_ant_w				material.nr_seq_fabric%type;
ie_situacao_ant_w				material.ie_situacao%type;
cd_fabricante_ant_w				material.cd_fabricante%type;
nm_usuario_ant_w				material.nm_usuario%type;
dt_atualizacao_ant_w			material.dt_atualizacao%type;
ds_material_ant_w				material.ds_material%type;
ds_reduzida_ant_w				material.ds_reduzida%type;
cd_classe_material_ant_w		material.cd_classe_material%type;
cd_classif_fiscal_ant_w			material.cd_classif_fiscal%type;
dt_atualizacao_nova_w			material.dt_atualizacao%type;
ie_situacao_nova_w				material.ie_situacao%type;
ds_material_nova_w 				material.ds_material%type;
ds_reduzida_nova_w				material.ds_reduzida%type;
dt_vigencia_final_nova_w 		material_tuss.dt_vigencia_final%type;
nr_registro_anvisa_ant_w 		material_estab.nr_registro_anvisa%type;
dt_validade_reg_anvisa_ant_w	material_estab.dt_validade_reg_anvisa%type;
nm_usuario_mat_estab_ant_w		material_estab.nm_usuario%type;
dt_at_material_estab_ant_w		material_estab.dt_atualizacao%type;
dt_at_material_estab_nova_w		material_estab.dt_atualizacao%type;
ie_vigente_anvisa_ant_w			material_estab.ie_vigente_anvisa%type;
ie_material_estoque_ant_w		material_estab.ie_material_estoque%type;
ie_baixa_estoq_pac_ant_w		material_estab.ie_baixa_estoq_pac%type;
ie_ressuprimento_ant_w			material_estab.ie_ressuprimento%type;
ie_requisicao_ant_w				material_estab.ie_requisicao%type;
ie_prescricao_ant_w				material_estab.ie_prescricao%type;
ie_padronizado_ant_w			material_estab.ie_padronizado%type;
cd_codigo_ant_w					material_sistema_externo.cd_codigo%type;
cd_referencia_ant_w				material_sistema_externo.cd_referencia%type;
ds_marca_ant_w 					marca.ds_marca%type;

cd_funcao_w				funcao.cd_funcao%type;
cd_perfil_w				perfil.cd_perfil%type;

/*ie_sistema_externo_p	= É o código específico para relacionar o sistema externo de integração (Dom=3088)
I	= Inpart
B	= Bionexo
...
/* Funcao 1750 = Gerenciamento para Integracao - OPME */

cd_funcao_w := 1750;

begin
	/* Obtem perfil da integracao atraves de cadastro de compras, pois nao ha perfil padrao quando processa pela integracao */

	/* Origem: funcao Gerenciamento dos Parametros de Suprimentos - aba Integracoes - Perfil solic compra */
...
select	coalesce(ie_permite_atualizacao_mat, 'S'),
		coalesce(ie_consiste_fornecedor, 'N')
into STRICT 	ie_permite_atualizacao_mat_w,
		ie_consiste_fornecedor_w
...
elsif (ie_situacao_p = 'I') and (somente_numero(cd_material_tasy_p) > 0)then
	begin

	CALL consistir_inativar_material(cd_estabelecimento_p, cd_material_tasy_p, nm_usuario_p);
...
if ((ie_consiste_fornecedor_w = 'N') or (coalesce(cd_cnpj_p, '0') <> '0' and qt_existe_cnpj_w > 0)) then

	if (somente_numero(cd_material_externo_p) = 0) then
		ds_erro_w := ds_erro_w || WHEB_MENSAGEM_PCK.get_texto(280896);
	end if;

	if (somente_numero(cd_material_externo_p) > 0) and (somente_numero(cd_material_tasy_p) > 0) then
		select 	count(*)
		into STRICT	qt_existe_cd_mat_ext_w
		from	material_sistema_externo
		where	cd_codigo = cd_material_externo_p
		and	cd_material <> somente_numero(cd_material_tasy_p);
		if (qt_existe_cd_mat_ext_w > 0) then
			ds_erro_w := ds_erro_w || WHEB_MENSAGEM_PCK.get_texto(280897) || cd_material_externo_p || '; ';
		end if;
	end if;
	...
	select	cd_estab_regra,
		...
		ie_consist_fornec,
		ie_atualizar_classe,
		coalesce(ie_atualiza_tuss,'S'),
		coalesce(ie_atualiza_situacao,'S'),
	...
	into STRICT	cd_estab_regra_w,
		...
		ie_consist_fornec_w,
		ie_atualizar_classe_w,
		ie_atualiza_tuss_w,
		ie_atualiza_situacao_w,
	...
	if (coalesce(ie_preco_importacao_w,'N') = 'S') and (coalesce(vl_preco_p, 0) = 0) then
	...
	if (coalesce(nr_ncm_p, '0') <> '0') then

		select	coalesce(max(nr_ncm), '0'),
				max(ds_ncm)
		into STRICT    nr_ncm_w,
				ds_ncm_w
		from    material_ncm
		where   nr_ncm = nr_ncm_p;

		if (nr_ncm_w = '0') then
			insert into material_ncm(
				nr_sequencia,
				dt_atualizacao,
				nm_usuario,
				dt_atualizacao_nrec,
			...
			values (
				...
				ds_ncm_p,
				'A');
		elsif (ds_ncm_p <> ds_ncm_w) then
			update  material_ncm
			set     ds_ncm = ds_ncm_p
			where   nr_ncm = nr_ncm_p;

			if (ds_ncm_w <> ds_ncm_p) then
				CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL_NCM', 'DS_NCM', ds_ncm_w, ds_ncm_p, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
			end if;
		end if;
	end if;

	if (somente_numero(cd_material_tasy_p) > 0) then
		begin
		select	count(*)
		...
		if (qt_existe_w = 0) then
			ds_erro_w := 	ds_erro_w || WHEB_MENSAGEM_PCK.get_texto(280901)|| cd_material_tasy_p || '; ';
		elsif (ie_permite_atualizacao_mat_w = 'S') then
			begin
			cd_material_p :=  Somente_Numero(cd_material_tasy_p);
			select	coalesce(max(nr_sequencia), 0)
			into STRICT	nr_seq_fabric_w
			from	mat_fabricante
			where	cd_sistema_ant	= nr_seq_fabric_p;
			...
			if (nr_seq_fabric_w = 0) then
				...
				select
					...
					max(cd_classif_fiscal)
				into STRICT
					nr_seq_fabric_ant_w,
					ie_situacao_ant_w,
					cd_fabricante_ant_w,
					nm_usuario_ant_w,
					dt_atualizacao_ant_w,
					ds_material_ant_w,
					ds_reduzida_ant_w,
					cd_classe_material_ant_w,
					cd_classif_fiscal_ant_w
				from    material
				where	cd_material		= cd_material_tasy_p;

				dt_atualizacao_nova_w := clock_timestamp();

				if (coalesce(ie_atualizar_classe_w,'N') = 'S') then
					update	material
					set	nr_seq_fabric		= nr_seq_fabric_w,
						ie_situacao			= CASE WHEN ie_atualiza_situacao_w='S' THEN ie_situacao_p  ELSE ie_situacao END ,
						cd_fabricante		= cd_referencia_p,
						nm_usuario			= nm_usuario_p,
						dt_atualizacao		= dt_atualizacao_nova_w,
				...
				if (nr_seq_fabric_ant_w <> nr_seq_fabric_w) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL', 'NR_SEQ_FABRIC', nr_seq_fabric_ant_w, nr_seq_fabric_w, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (ie_situacao_ant_w <> ie_situacao_nova_w) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL', 'IE_SITUACAO', ie_situacao_ant_w, ie_situacao_nova_w, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (cd_fabricante_ant_w <> cd_referencia_p) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL', 'CD_FABRICANTE', cd_fabricante_ant_w, cd_referencia_p, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (nm_usuario_ant_w <> nm_usuario_p) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL', 'NM_USUARIO', nm_usuario_ant_w, nm_usuario_p, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (dt_atualizacao_ant_w <> dt_atualizacao_nova_w) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL', 'DT_ATUALIZACAO', dt_atualizacao_ant_w, dt_atualizacao_nova_w, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (ds_material_ant_w <> ds_material_nova_w) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL', 'DS_MATERIAL', ds_material_ant_w, ds_material_nova_w, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (ds_reduzida_ant_w <> ds_reduzida_nova_w) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL', 'DS_REDUZIDA', ds_reduzida_ant_w,ds_reduzida_nova_w, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (cd_classe_material_ant_w <> cd_classe_material_w) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL', 'CD_CLASSE_MATERIAL', cd_classe_material_ant_w, cd_classe_material_w, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (cd_classif_fiscal_ant_w <> nr_ncm_p) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL', 'CD_CLASSIF_FISCAL', cd_classif_fiscal_ant_w, nr_ncm_p, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				...
				if (cd_material_tuss_w IS NOT NULL AND cd_material_tuss_w::text <> '') and (ie_atualiza_tuss_w = 'S') then
					...
					if (qt_existe_w > 0) then
						-- Esse cód. TUSS já existe para esse material, não necessita fazer a importação novamente.
						goto Continuar;
					end if;
					...
					if (qt_existe_w = 0) then
						if (qt_existe_mat_tuss_w > 0) then
							...
							dt_vigencia_final_nova_w := clock_timestamp();

							update	material_tuss
							set	dt_vigencia_final = dt_vigencia_final_nova_w
							where	nr_sequencia = 	nr_seq_tuss_mat_w;

							CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL_TUSS', 'DT_VIGENCIA_FINAL', null, dt_vigencia_final_nova_w, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
						end if;
				...
				select
					...
					max(ie_material_estoque),
					max(ie_baixa_estoq_pac),
					max(ie_ressuprimento),
					max(ie_requisicao),
					max(ie_prescricao),
					max(ie_padronizado)
				into STRICT
					nr_registro_anvisa_ant_w,
					dt_validade_reg_anvisa_ant_w,
					nm_usuario_mat_estab_ant_w,
					dt_at_material_estab_ant_w,
					ie_vigente_anvisa_ant_w,
					ie_material_estoque_ant_w,
					ie_baixa_estoq_pac_ant_w,
					ie_ressuprimento_ant_w,
					ie_requisicao_ant_w,
					ie_prescricao_ant_w,
					ie_padronizado_ant_w
				from 	material_estab
				where	cd_material	= cd_material_tasy_p;

				dt_at_material_estab_nova_w := clock_timestamp();

				update	material_estab
				set	nr_registro_anvisa		= nr_registro_anvisa_p,
					dt_validade_reg_anvisa	= dt_validade_reg_anvisa_p,
					nm_usuario				= nm_usuario_p,
					dt_atualizacao			= dt_at_material_estab_nova_w,
					ie_vigente_anvisa 		= ie_vigente_anvisa_w,
					ie_material_estoque		= ie_material_estoque_w,
					ie_baixa_estoq_pac		= ie_baixa_estoq_pac_w,
					ie_ressuprimento		= ie_ressuprimento_p,
					ie_requisicao			= ie_requisicao_p,
					ie_prescricao			= ie_prescricao_p,
					ie_padronizado			= ie_padronizado_w
				where	cd_material			= cd_material_tasy_p;

				if (nr_registro_anvisa_ant_w <> nr_registro_anvisa_p) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL_ESTAB', 'NR_REGISTRO_ANVISA', nr_registro_anvisa_ant_w, nr_registro_anvisa_p, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (dt_validade_reg_anvisa_ant_w <> dt_validade_reg_anvisa_p) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL_ESTAB', 'DT_VALIDADE_REG_ANVISA', dt_validade_reg_anvisa_ant_w, dt_validade_reg_anvisa_p, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (nm_usuario_mat_estab_ant_w <> nm_usuario_p) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL_ESTAB', 'NM_USUARIO', nm_usuario_mat_estab_ant_w, nm_usuario_p, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (dt_at_material_estab_ant_w <> dt_at_material_estab_nova_w) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL_ESTAB', 'DT_ATUALIZACAO', dt_at_material_estab_ant_w, dt_at_material_estab_nova_w, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (ie_vigente_anvisa_ant_w <> ie_vigente_anvisa_w) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL_ESTAB', 'IE_VIGENTE_ANVISA', ie_vigente_anvisa_ant_w, ie_vigente_anvisa_w, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (ie_material_estoque_ant_w <> ie_material_estoque_w) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL_ESTAB', 'IE_MATERIAL_ESTOQUE', ie_material_estoque_ant_w, ie_material_estoque_w, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (ie_baixa_estoq_pac_ant_w <> ie_baixa_estoq_pac_w) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL_ESTAB', 'IE_BAIXA_ESTOQ_PAC', ie_baixa_estoq_pac_ant_w, ie_baixa_estoq_pac_w, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (ie_ressuprimento_ant_w <> ie_ressuprimento_p) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL_ESTAB', 'IE_RESSUPRIMENTO', ie_ressuprimento_ant_w, ie_ressuprimento_p, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (ie_requisicao_ant_w <> ie_requisicao_p) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL_ESTAB', 'IE_REQUISICAO', ie_requisicao_ant_w, ie_requisicao_p, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (ie_prescricao_ant_w <> ie_prescricao_p) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL_ESTAB', 'IE_PRESCRICAO', ie_prescricao_ant_w, ie_prescricao_p, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				if (ie_padronizado_ant_w <> ie_padronizado_w) then
					CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL_ESTAB', 'IE_PADRONIZADO', ie_padronizado_ant_w, ie_padronizado_w, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
				end if;
				...
				while(cd_estab_regra_w IS NOT NULL AND cd_estab_regra_w::text <> '') loop
					...
					from	material_estab
					where	cd_estabelecimento =  coalesce(cd_estabelecimento_w, cd_estabelecimento_p)
					and		cd_material = cd_material_tasy_p;
					...
					if (ie_atualiza_tabela_w = 'S') and (coalesce(vl_preco_p,0) <> 0) and (cd_moeda_w > 0)then

						cd_estab_number_w	:= (coalesce(cd_estabelecimento_w,cd_estabelecimento_p))::numeric;

						CALL inpart_atualiza_tab_preco(cd_material_p,clock_timestamp(),cd_tabela_preco_w,cd_estab_number_w,vl_preco_p, cd_cnpj_p, nm_usuario_p);
				...
				if (qt_existe_w = 0) then
					...
					if (cd_codigo_ant_w <> cd_material_externo_p) then
						CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL_SISTEMA_EXTERNO', 'CD_CODIGO', cd_codigo_ant_w, cd_material_externo_p, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
					end if;
					if (cd_referencia_ant_w <> cd_referencia_p) then
						CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MATERIAL_SISTEMA_EXTERNO', 'CD_REFERENCIA', cd_referencia_ant_w, cd_referencia_p, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
					end if;
				end if;
				...
				if (nr_seq_marca_w = 0) and (coalesce(nr_seq_tipo_marca_w,0) > 0) then
					...
					if (ds_marca_ant_w <> ds_fabricante_p) then
						CALL grava_material_log_alteracao(cd_estabelecimento_p, somente_numero(cd_material_tasy_p), 'MARCA', 'DS_MARCA', ds_marca_ant_w, ds_fabricante_p, nm_usuario_p, null, null, cd_funcao_w, cd_perfil_w);
					end if;
				end if;
			end if;
			end;
		...
		if (qt_existe_cd_mat_ext_w > 0) then
			...
			if (qt_existe_fornec_cad_w > 0)  then
				ds_erro_w	:= ds_erro_w || WHEB_MENSAGEM_PCK.get_texto(280897) || cd_material_externo_p || '; '; --Já existe material cadastrado com código Inpart
			end if;
			...
			if (coalesce(cd_cnpj_p, '0') <> '0')  and (coalesce(qt_existe_fornec_cad_w,0) = 0) then
				...
				if (qt_existe_fornec_w = 0 and qt_existe_cnpj_w > 0) then
				...
				insert into material_fornec(
					...
					values (nextval('material_fornec_seq'),
						...
						nm_usuario_p,
						cd_cnpj_p,
						coalesce(ie_consignado_w,ie_consignado_p),
						cd_unidade_medida_compra_p,
						1,
						clock_timestamp(),
						nm_usuario_p,
		...
		if (coalesce(ds_erro_w::text, '') = '') or (qt_existe_cd_mat_ext_w = 0) then
			...
			insert into material(
				...
				nr_seq_grupo_rec,		cd_classif_fiscal)
			values (	cd_material_p,			substr((ds_material_p),1,255),
				...
				ie_obrig_via_aplicacao_p,	coalesce(ie_consignado_w,ie_consignado_p),
				'S',				'N',
				'N',				'N',
				'N',				'S',
				'N',				nr_seq_fabric_w,
				cd_referencia_p,		cd_unidade_medida_solic_p,
				ie_receita_p,			nr_ncm_p);
			...
			while(cd_estab_regra_w IS NOT NULL AND cd_estab_regra_w::text <> '') loop
				...
				if (ie_atualiza_tabela_w = 'S') and (cd_moeda_w > 0) and (coalesce(vl_preco_p,0) <> 0) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE importar_material_xml ( cd_estabelecimento_p bigint, ie_atualizacao_p text, cd_material_externo_p text, cd_material_tasy_p text, ie_sistema_externo_p text, ds_material_p text, ds_reduzida_p text, ie_tipo_material_p text, cd_unidade_medida_compra_p text, cd_unidade_medida_estoque_p text, cd_unidade_medida_consumo_p text, cd_unidade_medida_solic_p text, ie_consignado_p text, ie_disponivel_mercado_p text, ie_obrig_via_aplicacao_p text, ie_receita_p text, ie_cobra_paciente_p text, ie_inf_ultima_compra_p text, ie_preco_compra_p text, ie_curva_abc_p text, qt_conv_compra_estoque_p bigint, qt_conv_estoque_consumo_p bigint, qt_minimo_multiplo_solic_p bigint, cd_classe_material_p bigint, ie_material_estoque_p text, ie_baixa_estoq_pac_p text, ie_ressuprimento_p text, ie_requisicao_p text, ie_prescricao_p text, ie_padronizado_p text, ie_classif_custo_p text, ie_controla_serie_p text, cd_tabela_preco_p bigint, vl_preco_p bigint, nm_usuario_p text, cd_referencia_p text, nr_registro_anvisa_p text, dt_validade_reg_anvisa_p timestamp, nr_seq_fabric_p text, ds_fabricante_p text, cd_cnpj_p text, ie_situacao_p text, cd_material_tuss_p text, nr_ncm_p text, ds_ncm_p text, cd_material_p INOUT bigint, ds_erro_p INOUT text) FROM PUBLIC;
