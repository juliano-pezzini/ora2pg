...
CREATE OR REPLACE PROCEDURE sus_gerar_apac_laudo_sem_autor ( nr_seq_interno_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
ie_atualiza_medico_resp_w	varchar(10);
ie_gerar_proced_w		varchar(1);
ie_verif_conta_proc_w	varchar(15);
ie_verif_conta_apac_w	varchar(15);
ie_medico_laudo_w		varchar(15);
cd_medico_requisitante_w	varchar(10);
ie_verf_conta_compet_w	varchar(15) := 'N';
i			smallint;
nr_atendimento_w		bigint;
ie_tipo_laudo_apac_w	smallint;
ie_tipo_apac_w		smallint := 0;
...
ie_atualiza_medico_resp_w	:= obter_valor_param_usuario(1124,27,obter_perfil_ativo,nm_usuario_p,0);
ie_gerar_proced_w		:= obter_valor_param_usuario(1124,42,obter_perfil_ativo,nm_usuario_p,0);
ie_verif_conta_proc_w	:= obter_valor_param_usuario(1124,53,obter_perfil_ativo,nm_usuario_p,0);
ie_verif_conta_apac_w	:= obter_valor_param_usuario(1124,54,obter_perfil_ativo,nm_usuario_p,0);
ie_medico_laudo_w		:= obter_valor_param_usuario(1124,66,obter_perfil_ativo,nm_usuario_p,0);
ie_verf_conta_compet_w	:= obter_valor_param_usuario(1124,69,obter_perfil_ativo,nm_usuario_p,0);
...
exception
	when others then
	CALL wheb_mensagem_pck.exibir_mensagem_abort(209355);
...
if (coalesce(ie_carater_inter_sus_w::text, '') = '') then
	CALL wheb_mensagem_pck.exibir_mensagem_abort(209356);
...
exception
	when others then
	CALL wheb_mensagem_pck.exibir_mensagem_abort(209357);
...
if (ie_atualiza_medico_resp_w	= 'S') then
	cd_medico_resp_w	:= sus_obter_diretor_ups(cd_procedimento_w,ie_origem_proced_w);
end if;
if (ie_medico_laudo_w	= 'S') then
	cd_medico_resp_w	:= cd_medico_requisitante_w;
end if;

CALL sus_recalc_contas_atend_apac(nr_atendimento_w,nm_usuario_p);
...
for i in 1..3 loop
	...
	if (ie_tipo_apac_w < 2) then
		ie_tipo_apac_w := ie_tipo_apac_w + 1;
	end if;
	...
	if (sus_obter_se_detalhe_proc(cd_procedimento_w,ie_origem_proced_w,'014',dt_emissao_w) = 0) then
		...
		dt_fim_val_apac_w       := establishment_timezone_utils.startofday(last_day(dt_competencia_apac_w));
		
	end if;
	
	--OS 739069 - lhalves em 12/08/2014
	dt_autorizacao_w	:= clock_timestamp();
	if (dt_emissao_w > dt_autorizacao_w) then
		dt_autorizacao_w	:= coalesce(dt_emissao_w,clock_timestamp());
	end if;
	...
	if (coalesce(ie_verif_conta_apac_w,'N') = 'S') and (coalesce(ie_verf_conta_compet_w,'N') = 'N') then
		begin
		...
		and	establishment_timezone_utils.startOfMonth(a.dt_periodo_inicial) <= establishment_timezone_utils.startOfMonth(dt_fim_val_apac_w)
...
union
	...
	if (coalesce(ie_verf_conta_compet_w,'N') = 'S') then
		...
		and	a.ie_status_acerto	= 1
		and	a.cd_convenio_parametro = cd_convenio_sus_w
		and (establishment_timezone_utils.startOfMonth(a.dt_mesano_referencia) = establishment_timezone_utils.startOfMonth(dt_competencia_apac_w))
		and (establishment_timezone_utils.startOfMonth(a.dt_periodo_inicial) <= establishment_timezone_utils.startOfMonth(dt_fim_val_apac_w))
		and	not exists (SELECT	1
				from	sus_apac_unif w
				where	w.nr_interno_conta = a.nr_interno_conta);
	...
	if (coalesce(ie_verif_conta_proc_w,'N') = 'S') 	then
		...
		if	(((coalesce(ie_verif_conta_apac_w,'N') = 'S') and (coalesce(nr_interno_conta_w,0) = 0))  or (coalesce(ie_verif_conta_apac_w,'N') = 'N'))
			...
			and	establishment_timezone_utils.startOfMonth(b.dt_periodo_inicial) <= establishment_timezone_utils.startOfMonth(dt_fim_val_apac_w)
	...
	dt_competencia_apac_w := add_months(dt_competencia_apac_w,1);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE sus_gerar_apac_laudo_sem_autor ( nr_seq_interno_p bigint, nm_usuario_p text) FROM PUBLIC;
