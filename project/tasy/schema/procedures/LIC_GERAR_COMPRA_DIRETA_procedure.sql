...
CREATE OR REPLACE PROCEDURE lic_gerar_compra_direta ( cd_estabelecimento_p bigint, nr_cot_compra_p bigint, nm_usuario_p text, ds_observacao_solic_p INOUT text) AS $body$
DECLARE
...
if (nr_seq_forma_compra_w = 0) then 
	CALL wheb_mensagem_pck.exibir_mensagem_abort(266191);
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_seq_mod_compra_w <> nr_seq_mod_compra_ww) then 
		--(nr_solic_compra_w <> nr_solic_compra_ww) then 
		begin 
		...
		insert into solic_compra( 
			...
			nr_solic_compra_origem_lic,		nr_cot_compra_origem_lic, 
		...
		values (	nr_solic_compra_nova_w,		cd_estabelecimento_p,		clock_timestamp(), 
			...
			cd_conta_contabil_w,		cd_setor_atendimento_w,		ds_observacao_solic_w, 
			...
			nr_solic_compra_w,			nr_cot_compra_p, 
	...
	insert into solic_compra_item( 
		...
		qt_conv_compra_est_orig,	ie_amostra,			ie_bula, 
		ie_catalogo,			qt_saldo_disp_estoque) 
	values (	nr_solic_compra_nova_w,		nr_item_solic_compra_ww,		cd_material_w, 
		...
		obter_dados_material(cd_material_w,'QCE'),ie_amostra_w,			ie_bula_w, 
		ie_catalogo_w,			obter_saldo_disp_estoque(cd_estabelecimento_p, cd_material_w, cd_local_estoque_w, trunc(clock_timestamp(),'mm')));
	...
	values (	nr_cot_compra_nova_w,		nr_item_cot_compra_w,		cd_material_w, 
	...
	CALL gerar_cot_compra_item_entrega(nr_cot_compra_nova_w,nr_item_cot_compra_w,nm_usuario_p);
	...
	EXIT WHEN NOT FOUND; /* apply on C03 */
		...
		EXIT WHEN NOT FOUND; /* apply on C04 */
			...
			SELECT	nr_seq_fornec_item_w,		nr_cot_compra_nova_w,		nr_item_cot_compra_w, 
...
EXIT WHEN NOT FOUND; /* apply on C05 */
	...
	CALL Gerar_Ordem_Compra(nr_cot_compra_w,'',nm_usuario_p,'N');
...
ds_observacao_solic_p := ds_observacao_solic_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE lic_gerar_compra_direta ( cd_estabelecimento_p bigint, nr_cot_compra_p bigint, nm_usuario_p text, ds_observacao_solic_p INOUT text) FROM PUBLIC;
