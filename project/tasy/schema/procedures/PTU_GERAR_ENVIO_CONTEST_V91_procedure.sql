...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_contest_v91 ( nr_seq_camara_p ptu_camara_contestacao.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
C13 CURSOR FOR
	SELECT	'557' ||
		...
		rpad(coalesce(substr(nm_prestador,1,70),' '),70,' '),
		nr_sequencia
...
C14 CURSOR FOR
	SELECT	'558' ||
		...
		rpad(coalesce(substr(nm_profissional,1,70),' '),70,' ')
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_contest_v91 ( nr_seq_camara_p ptu_camara_contestacao.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
