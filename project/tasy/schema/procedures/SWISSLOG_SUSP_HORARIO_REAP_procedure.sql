...
CREATE OR REPLACE PROCEDURE swisslog_susp_horario_reap ( nr_prescricao_p bigint, nr_seq_material_p bigint, nr_seq_horario_p bigint, dt_horario_p timestamp, nm_usuario_p text) AS $body$
DECLARE
...
if (ie_setor_swisslog_w = 'S') and (qt_existe_w > 0) then
	...
	if (ie_estrutura_w = 'S') then
		ds_param_integ_hl7_w := 'nr_prescricao=' || nr_prescricao_p || obter_separador_bv ||
								...
								'dt_horario=' || to_char(dt_horario_p,'HH24mi') || obter_separador_bv;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE swisslog_susp_horario_reap ( nr_prescricao_p bigint, nr_seq_material_p bigint, nr_seq_horario_p bigint, dt_horario_p timestamp, nm_usuario_p text) FROM PUBLIC;
