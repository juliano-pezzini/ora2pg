...
CREATE OR REPLACE PROCEDURE pls_tratar_proc_partic_analise ( nr_seq_analise_p bigint, ie_tipo_analise_p text, nr_seq_participante_p bigint, nr_seq_conta_proc_p bigint, ie_acao_executada_p bigint, ie_evento_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_conta_proc_novo_p INOUT bigint) AS $body$
DECLARE
...
if (ie_tipo_analise_p = 'IN') then
	if (ie_acao_executada_p in (1,2)) then
		if (ie_evento_p = 'B') then
			if (ie_acao_executada_p = 1) then
				select	count(nr_sequencia)
				into STRICT	qt_partic_w
				from	pls_proc_participante
				where	nr_seq_conta_proc = nr_seq_conta_proc_p  LIMIT 1;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_tratar_proc_partic_analise ( nr_seq_analise_p bigint, ie_tipo_analise_p text, nr_seq_participante_p bigint, nr_seq_conta_proc_p bigint, ie_acao_executada_p bigint, ie_evento_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_conta_proc_novo_p INOUT bigint) FROM PUBLIC;
