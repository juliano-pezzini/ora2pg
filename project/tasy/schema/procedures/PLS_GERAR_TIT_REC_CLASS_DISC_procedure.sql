...
CREATE OR REPLACE PROCEDURE pls_gerar_tit_rec_class_disc ( nr_titulo_p titulo_receber.nr_titulo%type, nm_usuario_p usuario.nm_usuario%type, ie_commit_p text ) AS $body$
DECLARE
...
c01 CURSOR(	nr_seq_lote_disc_pc	pls_lote_discussao.nr_sequencia%type ) FOR
	...
	and	(dp.cd_conta_negado_deb IS NOT NULL AND dp.cd_conta_negado_deb::text <> '')
...
union all
	...
	and	ds.nr_seq_lote	= nr_seq_lote_disc_pc
	and	(dm.cd_conta_negado_deb IS NOT NULL AND dm.cd_conta_negado_deb::text <> '')
...
if (nr_titulo_p IS NOT NULL AND nr_titulo_p::text <> '') then
	...
	cd_conta_financ_w := pls_obter_conta_financ_regra(	'ICR', null, cd_estabelecimento_w, null, null, nr_seq_camara_w, null, null, null, null, null, null, null, null, ie_tipo_intercambio_w, null, null, cd_conta_financ_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_tit_rec_class_disc ( nr_titulo_p titulo_receber.nr_titulo%type, nm_usuario_p usuario.nm_usuario%type, ie_commit_p text ) FROM PUBLIC;
