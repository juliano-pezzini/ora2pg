...
CREATE OR REPLACE PROCEDURE pls_oc_cta_val_27_mat_imp ( nr_seq_combinada_p pls_oc_cta_combinada.nr_sequencia%type, nr_id_transacao_p pls_oc_cta_selecao_imp.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
for	r_C01_w in C01(nr_id_transacao_p) loop
	...
	for	r_C02_w in C02(r_C01_w.nr_seq_contrato, r_C01_w.nr_seq_plano, r_C01_w.nr_seq_intercambio) loop
		...
		for r_C03_w in C03(dados_limitacao_w.nr_seq_tipo_limitacao) loop
			...
			-- Montar a restrição da regra utilizando os campos IMP
			dado_bind_w := pls_obter_restr_val_27_imp(	dados_segurado_w, nr_id_transacao_p, dados_limitacao_w, 'M', null, dados_mat_limitacao_w, 'IMP', dado_bind_w);
			
			-- Montar a restrição da regra. 
			dado_bind_w := pls_obter_restr_val_27_imp(	dados_segurado_w, nr_id_transacao_p, dados_limitacao_w, 'M', null, dados_mat_limitacao_w, 'N', dado_bind_w);
 	...
 	dados_tb_selecao_w.nr_seq_selecao := _ora2pg_r.tb_nr_seq_selecao_p; dados_tb_selecao_w.ie_valido := _ora2pg_r.tb_ie_valido_p; dados_tb_selecao_w.ds_observacao := _ora2pg_r.tb_ds_observacao_p;
		...
		for	r_C04_w	in C04(	nr_id_transacao_p, r_C01_w.nr_seq_segurado_conv,
			...
			ds_itens_util_w	:= substr(ds_itens_util_w || pls_util_pck.enter_w  ||  wheb_mensagem_pck.get_texto(791834/*Conta:*/
) || ' ' || r_C04_w.nr_seq_conta || ' | ' ||
						wheb_mensagem_pck.get_texto(791832/*Material:*/
) || ' ' || r_C04_w.nr_seq_conta_mat || ' | ' || 
						wheb_mensagem_pck.get_texto(791833/*Quantidade:*/
			...
			-- seleção.			
			if (qt_mat_util_total_w > qt_permitida_w) and (r_C04_w.ie_selecao = 'S') then
				...
				/*
				791835 - O material ultrapassa a quantidade permitida pelas limitações contratuais do beneficiário.
				791826 - Este beneficiário possui liberação para realizar apenas
				791836 -  material(is) neste mesmo contexto em um período de
				791837 - Materiais utilizados no período:
				*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_oc_cta_val_27_mat_imp ( nr_seq_combinada_p pls_oc_cta_combinada.nr_sequencia%type, nr_id_transacao_p pls_oc_cta_selecao_imp.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
