...
CREATE OR REPLACE PROCEDURE glosa_proc_valor_exc_filme (nr_sequencia_p bigint, vl_glosa_p bigint) AS $body$
DECLARE
...
if (nr_sequencia_w IS NOT NULL AND nr_sequencia_w::text <> '') then
        pr_glosa_w		:= dividir((vl_glosa_p * 100), (vl_procedimento_w - vl_materiais_w));
        ...
        vl_materiais_ww		:= vl_materiais_w;
        vl_adic_plant_ww	:= dividir(vl_adic_plant_w		* pr_glosa_w, 100);
        vl_procedimento_ww	:= dividir((vl_procedimento_w - vl_materiais_w)	* pr_glosa_w, 100);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE glosa_proc_valor_exc_filme (nr_sequencia_p bigint, vl_glosa_p bigint) FROM PUBLIC;
