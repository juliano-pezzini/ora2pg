...
CREATE OR REPLACE PROCEDURE limpar_dados_marc_ageint_itens ( ds_lista_itens_ageint_p text, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	and	obter_se_contido(nr_seq_ageint_item, ds_lista_itens_ageint_p) = 'S'
...
where	nm_usuario	= nm_usuario_p
and	obter_se_contido(nr_seq_item, ds_lista_itens_ageint_p) = 'S';

delete 	FROM ageint_pri_hor_livre
where	nm_usuario		= nm_usuario_p
and	obter_se_contido(nr_seq_ageint_item, ds_lista_itens_ageint_p) = 'S';

/*delete	agenda_integrada_prof_item
where	nm_usuario = nm_usuario_p
and		nvl(ie_confirmado,'S') = 'N'
and		ie_deletar_prof_p	= 'S';*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE limpar_dados_marc_ageint_itens ( ds_lista_itens_ageint_p text, nm_usuario_p text) FROM PUBLIC;
