...
CREATE OR REPLACE PROCEDURE tiss_gerar_w_guia_solic_opme (nr_sequencia_autor_p bigint, nr_seq_lote_anexo_p bigint, nr_prescr_tiss_p bigint, nm_usuario_p text, ds_dir_padrao_p text) AS $body$
DECLARE
...
where	c.nr_prescricao		= nr_prescr_tiss_p
...
if (coalesce(nr_sequencia_autor_p,-1) > 0) or (coalesce(nr_prescr_tiss_p,-1) > 0) or (coalesce(nr_seq_lote_anexo_p,-1) > 0) then
	...
	elsif (coalesce(nr_prescr_tiss_p,-1) > 0) then
	...
	if (coalesce(ie_gerar_tiss_w,'S') = 'S') then
		...
		EXIT WHEN NOT FOUND; /* apply on C02 */
			...
			EXIT WHEN NOT FOUND; /* apply on C01 */
				...
				if (qt_item_w = 0) then
					nr_seq_guia_w := tiss_gerar_cabec_solic_opme( nr_sequencia_autor_w, nr_seq_anexo_w, nm_usuario_p, nr_seq_guia_w);
				end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE tiss_gerar_w_guia_solic_opme (nr_sequencia_autor_p bigint, nr_seq_lote_anexo_p bigint, nr_prescr_tiss_p bigint, nm_usuario_p text, ds_dir_padrao_p text) FROM PUBLIC;
