...
CREATE OR REPLACE PROCEDURE gerar_etapa_inserir_etapa ( nr_interno_conta_p bigint, nr_seq_etapa_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR 
	...
	and	coalesce(nr_seq_estagio, coalesce(nr_seq_estagio_conta_w,0)) = coalesce(nr_seq_estagio_conta_w,0) 
	and	coalesce(nr_seq_etapa_filtro, coalesce(nr_seq_etapa_p,0)) = coalesce(nr_seq_etapa_p,0) 
...
and	a.nr_sequencia = (	SELECT	max(x.nr_sequencia) 
			...
			and	coalesce(x.dt_fechamento::text, '') = ''	);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_etapa_inserir_etapa ( nr_interno_conta_p bigint, nr_seq_etapa_p bigint, nm_usuario_p text) FROM PUBLIC;
