...
CREATE OR REPLACE PROCEDURE ageint_gerar_pri_hor_livre (nr_seq_ageint_p bigint, dt_agenda_p timestamp, nm_usuario_p text, cd_estabelecimento_p bigint) AS $body$
DECLARE


/*
	ATENCAO, AO ALTERAR ESTA PROCEDURE, ALTERAR TAMBEM A ageint_gerar_pri_hor_livre_VET
*/
  ...
  qt_dias_combo_w PARAMETRO_AGENDA_INTEGRADA.QT_DIAS_COMBO%type;
  qt_dias_sugestao_w PARAMETRO_AGENDA_INTEGRADA.QT_DIAS_SUGESTAO%type;
  ...
  ie_regra_marc_indiv_w PARAMETRO_AGENDA_INTEGRADA.IE_REGRA_PRIM_HOR_MARC_INDIV%type;

BEGIN
	
	qt_tmp_inicial_w := clock_timestamp();
	
	select coalesce(max(QT_DIAS_COMBO),0),
		   coalesce(max(QT_DIAS_SUGESTAO),0),
		   coalesce(max(IE_REGRA_PRIM_HOR_MARC_INDIV),'N')
	  into STRICT qt_dias_combo_w,
		   qt_dias_sugestao_w,
		   ie_regra_marc_indiv_w
	  from PARAMETRO_AGENDA_INTEGRADA
	 where cd_estabelecimento = cd_estabelecimento_p;
	
	 
  if (qt_dias_combo_w > 0 and qt_dias_sugestao_w > 0 and ie_regra_marc_indiv_w = 'N') then

	CALL ageint_gerar_pri_hor_livre_vet(nr_seq_ageint_p,
								   dt_agenda_p,
								   nm_usuario_p,
	  ...
	  --  dbms_output.put_line(' Itens Total ' || qt_total_itens_w);
	  ...
	  for i in 0 .. qt_dias_w loop
		--  dbms_output.put_line(' Data ' || (dt_inicio_w + i));
	
		SELECT * FROM gerar_horarios_ageint(dt_inicio_w + i, nm_usuario_p, nr_seq_ageint_p, cd_estabelecimento_p, null, ds_aux_w, ds_aux_w, ds_aux_w, ds_aux_w) INTO STRICT ds_aux_w, ds_aux_w, ds_aux_w, ds_aux_w;
		...
		select count(distinct a.nr_seq_ageint_item)
		 ...
		 where a.nr_sequencia = b.nr_seq_ageint_lib
		   and a.nr_seq_ageint = nr_seq_ageint_p
		   and b.hr_agenda >= trunc(dt_inicio_w)
		   and b.ie_status_agenda = 'L';
	
		--dbms_output.put_line(' Itens Gerados ' || qt_itens_gerados_w);
	  ...
	  loop
		...
		--dbms_output.put_line(' Qtd.' || ageint_horario_livre_w.count);
		for i in 1 .. ageint_horario_livre_w.count loop
		    ...
		    cd_estab_agenda_w := null;
			 ...
			 SELECT min(cd_estabelecimento) cd_estab			
			   ...
			   from agenda a,
				    ageint_horarios_usuario ahu,
                    ageint_lib_usuario lib
			 where a.cd_agenda = ahu.cd_agenda
			   AND lib.nr_sequencia = ahu.nr_seq_ageint_lib
			   and ahu.hr_agenda = ageint_horario_livre_w[i].dt_agenda
			   and a.cd_estabelecimento = cd_estabelecimento_p
			   and lib.nr_seq_ageint = nr_seq_ageint_p
			   and ahu.nm_usuario = nm_usuario_p
			   and lib.nr_seq_ageint_item = ageint_horario_livre_w[i].nr_seq_ageint_item
			   and ahu.ie_status_agenda = 'L';
	  ...
	  ds_log_w := 'Integrated Schedule Reg: '||to_char(nr_seq_ageint_p)||' included by ageint_gerar_pri_hor_livre in '||qt_calc_tempo_w||' seconds';
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ageint_gerar_pri_hor_livre (nr_seq_ageint_p bigint, dt_agenda_p timestamp, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
