...
CREATE OR REPLACE PROCEDURE gerar_alocacao_recursos_proj ( nr_seq_gerencia_p bigint, nr_seq_classificacao_p bigint, dt_analise_p timestamp, nm_usuario_p text) AS $body$
DECLARE
...
cd_pessoa_recurso_w	varchar(10);
...
SELECT	f.cd_pessoa_fisica,
	p.nr_sequencia
from	proj_equipe_papel f,
	proj_equipe e,
	proj_projeto p
where	f.nr_seq_equipe = e.nr_sequencia
and	e.nr_seq_proj = p.nr_sequencia
and	coalesce(f.ie_situacao,'A') = 'A'
and	f.nr_seq_funcao = 44
and	e.nr_seq_equipe_funcao = 11
and	p.nr_seq_gerencia = nr_seq_gerencia_p
and	p.nr_seq_classif = nr_seq_classificacao_p
and	p.nr_seq_estagio = nr_seq_estagio_w
and	trunc(dt_alocacao_w,'dd') between p.dt_inicio_prev and p.dt_fim_prev
and	not exists (
		SELECT	1
		from	w_alocacao_recurso w
		where	w.cd_pessoa_recurso = f.cd_pessoa_fisica
		and	w.nr_seq_projeto = p.nr_sequencia)
group by
	f.cd_pessoa_fisica,
	p.nr_sequencia
order by
...
BEGIN
if (nr_seq_gerencia_p IS NOT NULL AND nr_seq_gerencia_p::text <> '') and (nr_seq_classificacao_p IS NOT NULL AND nr_seq_classificacao_p::text <> '') and (dt_analise_p IS NOT NULL AND dt_analise_p::text <> '') and (nm_usuario_p IS NOT NULL AND nm_usuario_p::text <> '') then
	begin
	...
	while(nr_semana_w < 11) loop
		...
		open c01;
		loop
		fetch c01 into	cd_pessoa_recurso_w,
				nr_seq_projeto_w;
	...
	while(nr_semana_w < 11) loop
		...
		open c02;
		loop
		fetch c02 into	cd_pessoa_recurso_w,
				nr_seq_projeto_w;
		EXIT WHEN NOT FOUND; /* apply on c02 */
			...
			where	cd_pessoa_recurso = cd_pessoa_recurso_w
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_alocacao_recursos_proj ( nr_seq_gerencia_p bigint, nr_seq_classificacao_p bigint, dt_analise_p timestamp, nm_usuario_p text) FROM PUBLIC;
