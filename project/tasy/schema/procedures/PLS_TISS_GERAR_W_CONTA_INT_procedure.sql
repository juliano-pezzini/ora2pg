...
CREATE TYPE rc_cid AS (cd_cid_w			pls_diagnostico_conta.cd_doenca%type);


CREATE OR REPLACE PROCEDURE pls_tiss_gerar_w_conta_int ( nr_seq_conta_p bigint, nm_usuario_p text) AS $body$
DECLARE

 
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
Finalidade:  Gerar o relatório de Conta Guia de Resumo de Internação. 
---------------------------------------------------------------------------------------------------------------------------------------------------- 
...
c01 CURSOR FOR 	--Procedimentos 
	SELECT	a.dt_procedimento, 
		...
		a.vl_unitario_imp as vl_proc_unitario, --a.vl_proc_unitario, 
		(a.qt_procedimento_imp * a.vl_unitario_imp) as vl_total_procedimento, --a.vl_total_procedimento, 
		a.nr_sequencia 
...
c03 CURSOR FOR 	--Profissional Executante 
...
if (nr_seq_conta_p IS NOT NULL AND nr_seq_conta_p::text <> '') then 
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */		
		...
		tb_proc_w[nr_proc_w].nr_seq_apre_con_w	:= ct_proc_w;
		...
		open c03;
		loop 
		fetch c03 into	--Profissional Executante 
				nr_grau_part_w, 
				nm_contratado_w, 
				nr_seq_conselho_w, 
				sg_conselho_w, 
		...
		EXIT WHEN NOT FOUND; /* apply on C03 */	
			...
			--Insere as informações do cursor(C03) de profissionais no record. 
			tb_prof_w[nr_prof_w].nr_seq_aprese_w	:= tb_proc_w[nr_proc_w].nr_seq_apre_w;
			tb_prof_w[nr_prof_w].nr_seq_cont_proc_w	:= ct_prof_w;
	...
	close c01;
	--Verifica se a ultima guia possui 24 procedimentos, caso não possua será gerado os procedimentos faltantes em branco até completar a página. 
	if ( ct_proc_w < 24 ) then	 
		ct_proc_w := ct_proc_w + 1;
		for x in 1..24 loop 
			begin 
	...
	--Verifica se a ultima guia possui 26 profissionais, caso não possua será gerado os profissionais faltantes em branco até completar a página. 
	if ( ct_prof_w < 26 ) then	 
		ct_prof_w := ct_prof_w + 1;
		for y in 1..26 loop 
			begin 
	...
	if ( qt_pag_proc_w > 0 ) then 
		ct_proc_w := 0;
		for x in 1..( qt_pag_proc_w * 24 ) loop 
			begin 
				ct_proc_w := ct_proc_w + 1;
				nr_proc_w := nr_proc_w + 1;
				nr_seq_apresentacao_w := nr_seq_apresentacao_w + 1;
	...
	if ( qt_pag_prof_w > 0 ) then 
		ct_prof_w := 0;
		for y in 1..( qt_pag_prof_w * 26 ) loop 
			begin 
				ct_prof_w := ct_prof_w + 1;
				nr_prof_w := nr_prof_w + 1;
				tb_prof_w[nr_prof_w].nr_seq_aprese_w	:= '';
	...
	for i in 1.. tb_prof_w.count loop 
	begin 
		--Gera uma nova guia a cada 24 procedimentos e 26 profissionais inseridos nas tabelas temporárias. 
		...
		insert	into	w_tiss_proc_partic(nr_sequencia , nr_seq_guia, nm_usuario, 
			...
			values (nextval('w_tiss_proc_partic_seq'), nr_seq_tiss_guia_w, nm_usuario_p, 
				clock_timestamp(), tb_prof_w[i].sg_conselho_w, tb_prof_w[i].nr_seq_conselho_w, 
				tb_prof_w[i].uf_crm_w, tb_prof_w[i].cd_cbo_saude_w, tb_prof_w[i].nm_contratado_w, 
				tb_prof_w[i].nr_cpf_w, tb_prof_w[i].nr_seq_aprese_w, tb_prof_w[i].nr_grau_part_w, 
				tb_prof_w[i].nr_seq_cont_proc_w);
 
		--Insere os registros de procedimentos na tabela temporária. 
		if ( qt_pag_guia_w = 26 ) then 
			begin 
				qt_pagina_guia_w := qt_pagina_guia_w + 24;
				while( ct_insere_partic_w < qt_pagina_guia_w ) loop 
					begin 
						...
						values (nextval('w_tiss_proc_paciente_seq'), clock_timestamp(), nm_usuario_p, 
							tb_proc_w[ct_insere_partic_w].nr_seq_apre_w, tb_proc_w[ct_insere_partic_w].dt_proce_w, tb_proc_w[ct_insere_partic_w].dt_entrada_w, 
							tb_proc_w[ct_insere_partic_w].dt_alta_w, tb_proc_w[ct_insere_partic_w].cd_edicao_amb_w, tb_proc_w[ct_insere_partic_w].cd_proc_w, 
							tb_proc_w[ct_insere_partic_w].ds_proc_w, tb_proc_w[ct_insere_partic_w].qt_proc_w, tb_proc_w[ct_insere_partic_w].ie_via_aces_w, 
							tb_proc_w[ct_insere_partic_w].ie_tecn_util_w, tb_proc_w[ct_insere_partic_w].vl_red_acresc_w, tb_proc_w[ct_insere_partic_w].vl_unit_w, 
							tb_proc_w[ct_insere_partic_w].vl_proc_w, nr_seq_conta_p, nr_seq_tiss_guia_w, 
							tb_proc_w[ct_insere_partic_w].nr_seq_apre_con_w);	
					end;
	...
	if (nr_cid_w = 1 )then 
		...
		values (nextval('w_tiss_solicitacao_seq'), clock_timestamp(), nm_usuario_p, 
			tb_cid_w[0].cd_cid_w, nr_seq_conta_p);
	elsif (nr_cid_w = 2 )then 
		insert into w_tiss_solicitacao(nr_sequencia, dt_atualizacao, nm_usuario, 
			cd_cid,	cd_cid2, nr_seq_guia) 
		values (nextval('w_tiss_solicitacao_seq'), clock_timestamp(), nm_usuario_p, 
			tb_cid_w[0].cd_cid_w, tb_cid_w[1].cd_cid_w, nr_seq_conta_p);
	elsif (nr_cid_w = 3 )then 
		insert into w_tiss_solicitacao(nr_sequencia, dt_atualizacao, nm_usuario, 
			cd_cid, cd_cid2, cd_cid3, 
			nr_seq_guia) 
		values (nextval('w_tiss_solicitacao_seq'), clock_timestamp(), nm_usuario_p, 
			tb_cid_w[0].cd_cid_w, tb_cid_w[1].cd_cid_w, tb_cid_w[2].cd_cid_w, 
			nr_seq_conta_p);
		...
		values (nextval('w_tiss_solicitacao_seq'), clock_timestamp(),nm_usuario_p, 
			tb_cid_w[0].cd_cid_w, tb_cid_w[1].cd_cid_w, tb_cid_w[2].cd_cid_w, 
			tb_cid_w[3].cd_cid_w, nr_seq_conta_p);
	end if;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_tiss_gerar_w_conta_int ( nr_seq_conta_p bigint, nm_usuario_p text) FROM PUBLIC;
