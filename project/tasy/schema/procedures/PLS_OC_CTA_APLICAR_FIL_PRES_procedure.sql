...
CREATE OR REPLACE PROCEDURE pls_oc_cta_aplicar_fil_pres ( dados_regra_p pls_tipos_ocor_pck.dados_regra, dados_filtro_p pls_tipos_ocor_pck.dados_filtro, dados_consistencia_p pls_tipos_ocor_pck.dados_consistencia, dados_forma_geracao_ocor_p pls_tipos_ocor_pck.dados_forma_geracao_ocor, ie_incidencia_regra_p pls_oc_cta_combinada.ie_aplicacao_ocorrencia%type, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, qt_registro_selecao_p integer default null) AS $body$
DECLARE
			
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
Finalidade:	Responsável por aplicar os filtro de prestador nas regras das ocorrências conforme 
	cadastro dos filtros. 
------------------------------------------------------------------------------------------------------------------- 
...
Alteração:	Linhas 82 e 99: adicionado parametro ie_incidencia_regra_p para a function 
	pls_oc_cta_obter_restr_pres. 
	 
Motivo:	Se fez necessário devido a mudança na estrutura da function. 
------------------------------------------------------------------------------------------------------------------ 
	...
	for	rw_tipo_filtro_w in cs_tipo_filtro(dados_filtro_p.nr_sequencia) loop 
		...
		for	r_C_filtro_w in C_filtro(dados_filtro_p.nr_sequencia, rw_tipo_filtro_w.ie_tipo_filtro) loop 
			...
			dados_restricao_w.ds_restricao_conta := dados_restricao_w.ds_restricao_conta || 
								pls_oc_cta_obter_restr_pres('RESTRICAO',dados_regra_p, 
											var_cur_w,dados_filtro_prest_w, 
			...
			begin 
				...
				ret_null_w := pls_oc_cta_obter_restr_pres('BIND',dados_regra_p,var_cur_w,dados_filtro_prest_w, 
									ie_incidencia_regra_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_oc_cta_aplicar_fil_pres ( dados_regra_p pls_tipos_ocor_pck.dados_regra, dados_filtro_p pls_tipos_ocor_pck.dados_filtro, dados_consistencia_p pls_tipos_ocor_pck.dados_consistencia, dados_forma_geracao_ocor_p pls_tipos_ocor_pck.dados_forma_geracao_ocor, ie_incidencia_regra_p pls_oc_cta_combinada.ie_aplicacao_ocorrencia%type, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, qt_registro_selecao_p integer default null) FROM PUBLIC;
