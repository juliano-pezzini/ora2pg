...
CREATE OR REPLACE PROCEDURE pfcs_avg_bed_cleaning_time ( nr_seq_indicator_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
-- Variables
qt_total_w				        bigint := 0;
qt_sum_cleaning_time_w			bigint := 0;
...
if (pfcs_flag_settings_w = 0 or pfcs_flag_settings_w = 2) then
    for c01_w in c01_tasy loop
        ...
        qt_sum_cleaning_time_w := qt_sum_cleaning_time_w + c01_w.qt_time_sanitization;
...
;
 ...
 := pfcs_pck.pfcs_generate_results(
		vl_indicator_p => qt_sum_cleaning_time_w, vl_indicator_aux_p => qt_total_w, nr_seq_indicator_p => nr_seq_indicator_p, nr_seq_operational_level_p => nr_seq_operational_level_w, nm_usuario_p => nm_usuario_p, nr_seq_panel_p => pfcs_panel_seq_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pfcs_avg_bed_cleaning_time ( nr_seq_indicator_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
