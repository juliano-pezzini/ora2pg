...
CREATE OR REPLACE PROCEDURE reverter_medic_quimio (nr_seq_atendimento_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (ie_administrar_seq_w = 'S') then
		CALL reverter_status_medic_quimio(nr_seq_atendimento_p,nr_agrupamento_w,nm_usuario_p,'A');
	else
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE reverter_medic_quimio (nr_seq_atendimento_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
