...
CREATE OR REPLACE PROCEDURE alvaro_cad_exame_lab_antib ( nr_origem_p bigint, nr_seq_resultado_p bigint, nr_seq_result_item_p bigint, cd_medicamento_p text, ie_resultado_p text, nm_usuario_p text, qt_microorganismo_p text, ds_obs_microorganismo_p text, qt_mic_p bigint, ie_micro_sem_antib_p text, ds_resultado_antib_p text, ie_primeira_escolha_p text, ds_microorganismo_integr_p text, nr_seq_prescr_p bigint, cd_erro_p INOUT bigint, ds_erro_p INOUT text ) AS $body$
DECLARE
...
BEGIN
  ...
  if (coalesce(ds_microorganismo_integr_p::text, '') = '') then
    cd_erro_p	:= 28;
     ...
     inner join equipamento_lab b on b.cd_equipamento = a.cd_equipamento and b.ds_sigla = 'ALVARO'
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE alvaro_cad_exame_lab_antib ( nr_origem_p bigint, nr_seq_resultado_p bigint, nr_seq_result_item_p bigint, cd_medicamento_p text, ie_resultado_p text, nm_usuario_p text, qt_microorganismo_p text, ds_obs_microorganismo_p text, qt_mic_p bigint, ie_micro_sem_antib_p text, ds_resultado_antib_p text, ie_primeira_escolha_p text, ds_microorganismo_integr_p text, nr_seq_prescr_p bigint, cd_erro_p INOUT bigint, ds_erro_p INOUT text ) FROM PUBLIC;
