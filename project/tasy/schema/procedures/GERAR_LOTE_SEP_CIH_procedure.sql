...
CREATE OR REPLACE PROCEDURE gerar_lote_sep_cih ( nr_prescricao_p bigint, nr_sequencia_p bigint, nr_seq_horario_p bigint, ie_so_aprazado_p text, nm_usuario_p text, ie_origem_lote_p text, ie_send_integration_p text default 'N') AS $body$
DECLARE
...
C01 CURSOR FOR  --especifico para medicamentos cih
	...
	and	(x.cd_medicamento IS NOT NULL AND x.cd_medicamento::text <> '')
	and	coalesce(x.ie_controle_medico,0) = 2
	...
	(coalesce(ie_itens_associados_w,'S') = 'S' and ((coalesce(b.nr_seq_superior::text, '') = '') or (exists (	SELECT	1
																						...
																						and 	obter_se_medic_cih(d.cd_material) = 'N')))))
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_lote_sep_cih ( nr_prescricao_p bigint, nr_sequencia_p bigint, nr_seq_horario_p bigint, ie_so_aprazado_p text, nm_usuario_p text, ie_origem_lote_p text, ie_send_integration_p text default 'N') FROM PUBLIC;
