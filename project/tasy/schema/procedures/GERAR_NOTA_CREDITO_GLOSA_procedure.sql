...
CREATE OR REPLACE PROCEDURE gerar_nota_credito_glosa (nr_seq_lote_guia_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
select	d.cd_cgc, 
	...
	d.cd_convenio, 
	(obter_valores_guia_grc(a.nr_seq_lote_hist,a.nr_interno_conta,a.cd_autorizacao,'VG'))::numeric , 
	a.vl_saldo_guia 
...
if (vl_saldo_w < vl_glosa_w) and (vl_saldo_w > 0) then 
	...
	values (cd_cgc_w, 
		...
		cd_tipo_taxa_juro_w, 
		cd_tipo_taxa_multa_w, 
		Wheb_mensagem_pck.get_Texto(307161, 'NR_SEQ_LOTE_GUIA_P='|| NR_SEQ_LOTE_GUIA_P), /*'Nota de crédito gerada pela diferença entre a glosa e o sado da guia ' || nr_seq_lote_guia_p,*/ 
		clock_timestamp(), 
		clock_timestamp(), 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_nota_credito_glosa (nr_seq_lote_guia_p bigint, nm_usuario_p text) FROM PUBLIC;
