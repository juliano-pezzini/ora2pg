...
CREATE OR REPLACE PROCEDURE gerar_tributo_titulo_prev ( nr_titulo_p bigint, nm_usuario_p text, nr_bordero_p bigint, nr_seq_escrit_p bigint, dt_baixa_escrit_p timestamp, dt_baixa_p timestamp, cd_estabelecimento_p bigint, vl_pago_tit_p bigint) AS $body$
DECLARE
...
vl_base_retido_outro_w		double precision	:= 0;
...
ie_baixa_titulo_bordero_w	varchar(1);
dt_baixa_bordero_w		timestamp;
...
c01 CURSOR FOR
	...
	and (not exists (select	1
			from	titulo_pagar_imposto_prev x
			where	x.nr_titulo	= nr_titulo_p
			and	x.cd_tributo	= a.cd_tributo) or
...
ie_baixa_titulo_bordero_w := obter_param_usuario(855, 13, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_p, ie_baixa_titulo_bordero_w);

if (coalesce(nr_bordero_p, 0) > 0) then
	select	coalesce(dt_real_pagamento, dt_prev_pagamento)
	into STRICT	dt_baixa_bordero_w
	from 	bordero_pagamento
	where	nr_bordero = nr_bordero_p;
end if;

if (coalesce(nr_bordero_p, 0) > 0) and (coalesce(ie_baixa_titulo_bordero_w,'N') = 'S') then
	begin
...
elsif (coalesce(nr_seq_escrit_p,0) > 0) and (coalesce(ie_baixa_titulo_bordero_w,'N') = 'S') then
...
delete	FROM titulo_pagar_imposto_prev
where 	nr_titulo = nr_titulo_p;
...
EXIT WHEN NOT FOUND; /* apply on C02 */
	...
	from	titulo_pagar_imposto_prev	a,
		titulo_pagar			b
	where	a.nr_titulo			= b.nr_titulo
	and	a.cd_tributo			= cd_tributo_pf_w
	and	b.cd_pessoa_fisica		= cd_pessoa_fisica_pf_w
	and	PKG_DATE_UTILS.start_of(a.dt_atualizacao,'month',0) = PKG_DATE_UTILS.start_of(dt_emissao_w,'month',0);

	if (qt_registro_w = 0) then
		select	nextval('titulo_pagar_imposto_prev_seq')
		into STRICT	nr_seq_imposto_w
		;

		insert into titulo_pagar_imposto_prev(nr_sequencia,
			nr_titulo,
			cd_tributo,
			dt_atualizacao,
			nm_usuario,
		...
		vl_base_retido_outro_w	:= vl_base_retido_outro_w + vl_base_tributo_pf_w;
	end if;
	end;
end loop;
close C02;
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (coalesce(pr_aliquota_w,0) > 0) then
		...
		if (ie_baixa_titulo_w = 'S') then
			...
			/*Coloquei a atratativa abaixo referente ao ie_valor_base_titulo_nf_w conforme existe na GERAR_TRIBUTO_TITULO, apenas foi replicado de la para ca.*/


			/*lhalves OS594282 em 21/06/2013 - Se parametrizado busca o valo base cálculo da nota fiscal de origem */
			...
			and (a.nr_seq_escrit = nr_seq_escrit_p or
				PKG_DATE_UTILS.start_of(CASE WHEN ie_vencimento_w='V' THEN  b.dt_vencimento_atual WHEN ie_vencimento_w='C' THEN  coalesce(b.dt_contabil, b.dt_emissao) WHEN ie_vencimento_w='B' THEN  b.dt_vencimento_atual  ELSE /* ahoffelder - 02/01/2013 - alterei para não acumular tributo no mesmo mês caso algum título de meses anteriores fiquem em aberto */
								b.dt_emissao END , 'month', 0) 	= PKG_DATE_UTILS.start_of(dt_venc_tributo_w, 'month', 0))
		...
		from	titulo_pagar_imposto_prev b,
			titulo_pagar a
		where	a.nr_titulo			= b.nr_titulo
		and	a.nr_titulo			<> nr_titulo_p
		and	((a.cd_cgc			= cd_cgc_titulo_w) or (a.cd_pessoa_fisica 		= cd_pf_titulo_w))
		and	b.cd_tributo			= cd_tributo_w
		and	PKG_DATE_UTILS.start_of(b.dt_atualizacao, 'month', 0) = PKG_DATE_UTILS.start_of(dt_venc_tributo_w, 'month', 0)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_tributo_titulo_prev ( nr_titulo_p bigint, nm_usuario_p text, nr_bordero_p bigint, nr_seq_escrit_p bigint, dt_baixa_escrit_p timestamp, dt_baixa_p timestamp, cd_estabelecimento_p bigint, vl_pago_tit_p bigint) FROM PUBLIC;
