...
CREATE OR REPLACE PROCEDURE cpoe_shoppingcart_suplemento (nr_sequencia_p bigint, ie_retrogrado_p text, ds_retorno_p INOUT text) AS $body$
DECLARE
...
if (nr_sequencia_p IS NOT NULL AND nr_sequencia_p::text <> '') then
	...
	loop
		...
		if (coalesce(r_c01_w.cd_material::text, '') = '' or coalesce(r_c01_w.dt_inicio::text, '') = '' or coalesce(r_c01_w.ie_duracao::text, '') = '' or coalesce(r_c01_w.cd_intervalo::text, '') = ''
		...
		if (ie_return_value_w = 'S' and coalesce(r_c01_w.qt_tempo_aplic::text, '') = '') then
			ds_retorno_p := 'N';
			goto return_value;
		end if;		
		...
		if (((r_c01_w.cd_material IS NOT NULL AND r_c01_w.cd_material::text <> '') and coalesce(r_c01_w.qt_dose,0) = 0)
			or ((r_c01_w.cd_mat_prod1 IS NOT NULL AND r_c01_w.cd_mat_prod1::text <> '') and coalesce(r_c01_w.qt_dose_prod1,0) = 0)) then
			ds_retorno_p := 'N';
			goto return_value;
		end if;
		...
		and		obter_se_cpoe_regra_duplic('S', wheb_usuario_pck.get_cd_perfil, r_c01_w.cd_setor_atend) = 'S'
...
exception when others then
	CALL gravar_log_cpoe(substr('CPOE_SHOPPINGCART_SUPLEMENTO EXCEPTION:'|| substr(to_char(sqlerrm),1,2000) || '//nr_sequencia_p: '|| nr_sequencia_p,1,40000));
	ds_retorno_p := 'N';
end;
$body$
LANGUAGE PLPGSQL
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE cpoe_shoppingcart_suplemento (nr_sequencia_p bigint, ie_retrogrado_p text, ds_retorno_p INOUT text) FROM PUBLIC;
