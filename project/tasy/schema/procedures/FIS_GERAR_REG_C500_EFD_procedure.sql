...
CREATE OR REPLACE PROCEDURE fis_gerar_reg_c500_efd ( nr_seq_controle_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, dt_inicio_p timestamp, dt_fim_p timestamp, cd_empresa_p bigint, ds_separador_p text, qt_linha_p INOUT bigint, nr_sequencia_p INOUT bigint) AS $body$
DECLARE
...
WHERE a.cd_estabelecimento	= cd_estabelecimento_p  -- nota fiscal de serviço de transporte (código 07) e conhecimentos de transporte rodoviário de cargas (código 08), conhecimentos de transporte de cargas avulso (código 8b), aquaviário de cargas (código 09), aéreo (código 10), ferroviário de cargas (código 11) e multimodal de cargas (código 26),  
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (vet01.cd_situacao = '00') then 
		SELECT * FROM fis_gerar_reg_C590_efd(	nr_seq_controle_p, nm_usuario_p, cd_estabelecimento_p, dt_inicio_p, dt_fim_p, cd_empresa_p, ds_separador_p, nr_sequencia_w, nr_linha_w, nr_seq_registro_w) INTO STRICT nr_linha_w, nr_seq_registro_w;
	end if;			
	end;
end loop;
close C01;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_reg_c500_efd ( nr_seq_controle_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, dt_inicio_p timestamp, dt_fim_p timestamp, cd_empresa_p bigint, ds_separador_p text, qt_linha_p INOUT bigint, nr_sequencia_p INOUT bigint) FROM PUBLIC;
