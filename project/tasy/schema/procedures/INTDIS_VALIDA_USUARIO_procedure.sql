...
CREATE OR REPLACE PROCEDURE intdis_valida_usuario ( cd_barras_p text, cd_local_estoque_p text, nm_usuario_lib_p INOUT text, ie_tipo_lib_p INOUT text, ds_erro_p INOUT text) AS $body$
DECLARE
...
exception
when others then
	ds_erro_p := WHEB_MENSAGEM_PCK.get_texto(280526);
end;

if (ie_situacao_w <> 'A') then
	ds_erro_p := WHEB_MENSAGEM_PCK.get_texto(280527);
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE intdis_valida_usuario ( cd_barras_p text, cd_local_estoque_p text, nm_usuario_lib_p INOUT text, ie_tipo_lib_p INOUT text, ds_erro_p INOUT text) FROM PUBLIC;
