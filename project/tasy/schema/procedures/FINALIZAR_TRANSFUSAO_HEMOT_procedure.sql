...
CREATE OR REPLACE PROCEDURE finalizar_transfusao_hemot ( ie_opcao_p text, nr_seq_exame_lote_p bigint, nr_seq_transfusao_p bigint, cd_estabelecimento_p bigint, cd_perfil_p bigint, nm_usuario_p text, ds_erro_p INOUT text) AS $body$
DECLARE
...
UNION
	...
	if (coalesce(ds_erro_p::text, '') = '') then
		...
		if (ie_dt_fim_ADEP_w = 'S') and (nr_prescricao_w IS NOT NULL AND nr_prescricao_w::text <> '') then
			...
			if (nr_seq_prescr_w IS NOT NULL AND nr_seq_prescr_w::text <> '') then
				...
				ds_msg_w := Gerar_alteracao_solucao_prescr(	cd_estabelecimento_p, nr_atendimento_w, 3, nr_prescricao_w, nr_seq_prescr_w, 4, clock_timestamp(), null, null, null, null, null, null, null, null, null, wheb_mensagem_pck.get_texto(279024), nm_usuario_p, null, clock_timestamp(), null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, ds_msg_w);
			else
				open C01;
				loop
				fetch C01 into
	...
	if (ie_dt_fim_ADEP_w = 'S') and (nr_prescricao_w IS NOT NULL AND nr_prescricao_w::text <> '') then
		...
		if (nr_seq_prescr_w IS NOT NULL AND nr_seq_prescr_w::text <> '') then
			...
			CALL reverter_alteracao_solucao(cd_estabelecimento_p, nr_atendimento_w, 3, nr_prescricao_w, nr_seq_prescr_w, null, null, nm_usuario_p, nr_seq_reserva_w, null, null);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE finalizar_transfusao_hemot ( ie_opcao_p text, nr_seq_exame_lote_p bigint, nr_seq_transfusao_p bigint, cd_estabelecimento_p bigint, cd_perfil_p bigint, nm_usuario_p text, ds_erro_p INOUT text) FROM PUBLIC;
