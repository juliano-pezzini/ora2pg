...
CREATE OR REPLACE PROCEDURE ub_gerar_ret_santander_150 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_titulo_w IS NOT NULL AND nr_titulo_w::text <> '') then
		...
		select coalesce(max(nr_seq_tipo),5)
		...
		CALL fin_gerar_log_controle_banco(3,substr(wheb_mensagem_pck.get_texto(304455,'DS_TITULO_W='||ds_titulo_w),1,4000),nm_usuario_p,'N');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ub_gerar_ret_santander_150 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
