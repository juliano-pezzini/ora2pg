...
CREATE OR REPLACE PROCEDURE pls_gerar_emissao_intercambio ( nr_seq_carterinha_p bigint, cd_estabelecimento_p bigint, ie_commit_p text, nm_usuario_p text) AS $body$
DECLARE
...
/*aaschlote 02/05/2011 OS - 314162*/
...
C01 CURSOR FOR
	...
	and	((ie_tipo_contrato <> 'C')
	or (coalesce(ie_tipo_contrato::text, '') = ''))
	and	((ie_agrupar_lote_w = 'N' and nr_seq_tipo_lote = nr_seq_tipo_lote_w and (nr_seq_tipo_lote IS NOT NULL AND nr_seq_tipo_lote::text <> '')
		and (nr_seq_contrato_inter = nr_seq_intercambio_w and (nr_seq_intercambio_w IS NOT NULL AND nr_seq_intercambio_w::text <> '')))
...
if (nr_seq_lote_w > 0) then
	CALL wheb_mensagem_pck.exibir_mensagem_abort(201724,'NR_SEQ_CARTEIRA='||nr_seq_carterinha_p||';'||'NR_SEQ_LOTE='||nr_seq_lote_w);
end if;

if (qt_cart_provisorias_w > 0) then
	CALL wheb_mensagem_pck.exibir_mensagem_abort(201723);
end if;
...
if (coalesce(nr_seq_lote_w::text, '') = '') then
	...
	insert into pls_lote_carteira(nr_sequencia, cd_estabelecimento, dt_atualizacao,
		...
		nr_seq_tipo_lote,nr_seq_contrato_inter,
		ie_tipo_data,ie_situacao_atend,ie_tipo_pessoa)
	values (	nr_seq_lote_w, cd_estabelecimento_p, clock_timestamp(),
		nm_usuario_p, clock_timestamp(), nm_usuario_p,
		null, 'G', 'E',
		null,'I',ie_tipo_segurado_w,
		nr_seq_tipo_lote_w,nr_seq_intercambio_w,
		'N','T','A');
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_emissao_intercambio ( nr_seq_carterinha_p bigint, cd_estabelecimento_p bigint, ie_commit_p text, nm_usuario_p text) FROM PUBLIC;
