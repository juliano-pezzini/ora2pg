...
CREATE OR REPLACE PROCEDURE pls_importar_arquivo_a560_80 ( ds_conteudo_p text, nr_seq_camara_contest_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
union
	...
	-- Caso esteja utilizando a novo contestação, permitir importar arquivo A560 com lote de discussão em aberto.
	...
	if (nr_seq_ptu_fatura_w IS NOT NULL AND nr_seq_ptu_fatura_w::text <> '') or (nr_seq_pls_fatura_w IS NOT NULL AND nr_seq_pls_fatura_w::text <> '') then
		...
		if	((nr_fatura_w <> nr_fatura_contest_w) and ((coalesce(nr_fatura_contest_tit_w::text, '') = '') or (nr_fatura_contest_tit_w IS NOT NULL AND nr_fatura_contest_tit_w::text <> '' AND nr_fatura_w <> nr_fatura_contest_tit_w))) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_importar_arquivo_a560_80 ( ds_conteudo_p text, nr_seq_camara_contest_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
