...
CREATE OR REPLACE PROCEDURE obter_pj_dirf_nota_venc ( nr_sequencia_p bigint, cd_estabelecimento_p bigint, cd_tributo_p bigint, cd_darf_p text, ie_tipo_data_p bigint, ie_consistir_retencao_p text, dt_mes_referencia_p timestamp, cd_empresa_p bigint) AS $body$
DECLARE
...
ds_comando_w	:=	'select	p.cd_cgc, '											||
			...
			'	p.ie_tipo_titulo, '										||
			'	p.nr_seq_classe '										||
			' from	titulo_pagar p, nota_fiscal n, operacao_nota o, nota_fiscal_venc_trib v, tributo t '		||
...
if (ie_tipo_data_p = 1) then -- Pega pela data de emissão
	...
	if (ie_consistir_retencao_p = 'S') then
		ds_comando_w 	:= ds_comando_w || ' and to_char(nvl(v.dt_vencimento,p.dt_vencimento_atual),''YYYY'') = to_char(:dt_mes_referencia, ''YYYY'') and nvl(v.dt_vencimento,p.dt_vencimento_atual) <= fim_mes(:dt_mes_referencia) ';
		dt_escolhida_w	:= ' and to_char(z.dt_imposto,''YYYY'') = to_char(:dt_mes_referencia, ''YYYY'') and z.dt_imposto <= fim_mes(:dt_mes_referencia) and z.dt_imposto >= p.dt_liquidacao';
	else
		ds_comando_w 	:= ds_comando_w	|| ' and to_char(nvl(v.dt_vencimento,p.dt_vencimento_atual),''MM/YYYY'') = to_char(:dt_mes_referencia, ''MM/YYYY'') ';
		dt_escolhida_w	:= ' and to_char(z.dt_imposto,''MM/YYYY'') = to_char(:dt_mes_referencia, ''MM/YYYY'') ';
	end if;
end if;
...
-- Regra CD_DARF
/*
if	(cd_darf_p is not null) then
	ds_comando_w := ds_comando_w || ' and	t.cd_retencao = :cd_darf ';
end if;
*/
-- Regra CD_DARF
if (cd_darf_p IS NOT NULL AND cd_darf_p::text <> '') then
	ds_comando_w := ds_comando_w || ' and	((v.cd_darf = :cd_darf) or ' 										||
					'	substr(obter_codigo_darf(v.cd_tributo, ' 								||
					'		to_number(obter_descricao_padrao(''NOTA_FISCAL'',''CD_ESTABELECIMENTO'',n.nr_sequencia)),'	||
					'		obter_descricao_padrao(''NOTA_FISCAL'',''CD_CGC'',n.nr_sequencia),'				||
					'		obter_descricao_padrao(''NOTA_FISCAL'',''CD_PESSOA_FISICA'',n.nr_sequencia)),1,10) = :cd_darf)';
end if;
...
if	((ie_tipo_data_p = 1) or (ie_tipo_data_p = 2) or (ie_tipo_data_p = 3) or (ie_tipo_data_p = 4) or (ie_tipo_data_p = 5) or (ie_tipo_data_p = 6)) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE obter_pj_dirf_nota_venc ( nr_sequencia_p bigint, cd_estabelecimento_p bigint, cd_tributo_p bigint, cd_darf_p text, ie_tipo_data_p bigint, ie_consistir_retencao_p text, dt_mes_referencia_p timestamp, cd_empresa_p bigint) FROM PUBLIC;
