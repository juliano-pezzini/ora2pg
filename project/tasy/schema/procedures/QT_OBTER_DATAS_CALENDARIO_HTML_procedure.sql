...
CREATE OR REPLACE PROCEDURE qt_obter_datas_calendario_html ( nr_seq_paciente_p bigint, nr_seq_pend_agenda_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text, ie_somente_futuras_p text, dt_min_p INOUT timestamp, dt_max_p INOUT timestamp, qt_duracao_p INOUT bigint) AS $body$
DECLARE
...
if (ie_somente_futuras_p	= 'S') then
	if (nr_seq_paciente_p IS NOT NULL AND nr_seq_paciente_p::text <> '') then
		...
		and		trunc(coalesce(dt_real,dt_prevista),'month') >= trunc(clock_timestamp(),'month');
...
if (ie_somente_futuras_p	= 'N') or (coalesce(dt_min_w::text, '') = '') then
	if (nr_seq_paciente_p IS NOT NULL AND nr_seq_paciente_p::text <> '') then
		select 	min(coalesce(dt_real,dt_prevista)),
				max(coalesce(dt_real,dt_prevista)),
				months_between(trunc(max(coalesce(dt_real,dt_prevista)),'month'),trunc(min(coalesce(dt_real,dt_prevista)),'month'))+1
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE qt_obter_datas_calendario_html ( nr_seq_paciente_p bigint, nr_seq_pend_agenda_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text, ie_somente_futuras_p text, dt_min_p INOUT timestamp, dt_max_p INOUT timestamp, qt_duracao_p INOUT bigint) FROM PUBLIC;
