...
CREATE OR REPLACE PROCEDURE pcs_reenviar_email_parecer ( nr_seq_reclassif_item_p bigint, cd_destinatario_p text, cd_grupo_destino_p text, nm_usuario_p text) AS $body$
DECLARE
...
C02 CURSOR FOR
	...
	and		(b.dt_envio IS NOT NULL AND b.dt_envio::text <> '')   --Para reenviar, precisa que já esteja enviado.
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pcs_reenviar_email_parecer ( nr_seq_reclassif_item_p bigint, cd_destinatario_p text, cd_grupo_destino_p text, nm_usuario_p text) FROM PUBLIC;
