...
CREATE OR REPLACE PROCEDURE jas_gerar_pagto_sicoob_240 ( nr_seq_envio_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
nr_tipo_pagamento_w     varchar(2);
ie_tipo_titulo_w        varchar(2);

c01 CURSOR FOR
SELECT	distinct
		CASE WHEN b.ie_tipo_pagamento='CC' THEN '01' WHEN b.ie_tipo_pagamento='DOC' THEN '03' WHEN b.ie_tipo_pagamento='CCP' THEN '05' WHEN b.ie_tipo_pagamento='TED' THEN '41' WHEN b.ie_tipo_pagamento='OP' THEN '30'  ELSE '03' END ,
		b.ie_tipo_pagamento,
		CASE WHEN b.ie_tipo_pagamento='CC' THEN '01'  ELSE '99' END ,
		CASE WHEN c.ie_tipo_titulo=6 THEN '30'  ELSE '20' END
from	titulo_pagar c,
...
id_judicial_w			varchar(40);
informacoes2_w			varchar(40);
ie_tipo_servico_w	varchar(2);

c02 CURSOR FOR
SELECT	b.cd_banco,
	lpad(substr(b.cd_agencia_bancaria,1,5),5,'0'),
	coalesce(substr(b.ie_digito_agencia,1,1),' '),
	...
	c.ie_tipo_titulo,
	rpad(coalesce(substr(c.nr_titulo_externo,1,40),' '),40,' ')
...
WHERE b.nr_titulo		= c.nr_titulo and b.ie_tipo_pagamento	= ie_tipo_pagamento_w and ((ie_tipo_servico_w = '30' and c.ie_tipo_titulo = 6) or (ie_tipo_servico_w = '20' and c.ie_tipo_titulo <> 6)) and a.nr_sequencia		= b.nr_seq_escrit and a.nr_sequencia		= nr_seq_envio_p;
...
select	lpad(b.cd_cgc,14,'0'),
	...
	lpad(somente_numero(substr(d.cd_agencia_bancaria,1,5)) || somente_numero(replace(substr(coalesce(d.ie_digito,'0'),1,1),'X','0')),6,'0'),
...
if (nr_seq_conta_banco_w IS NOT NULL AND nr_seq_conta_banco_w::text <> '') then
	...
	into STRICT	cd_conv_banco_interf_w
	...
	if (cd_conv_banco_interf_w IS NOT NULL AND cd_conv_banco_interf_w::text <> '') then
		cd_convenio_banco_w	:= rpad(substr(coalesce(cd_conv_banco_interf_w,' '),1,20),20,' ');
	end if;
end if;
	...
	EXIT WHEN NOT FOUND; /* apply on c02 */
		...
		if (coalesce(qt_favorecido_w,0)	> 0) then
			...
			select	somente_numero(substr(obter_conta_pessoa(coalesce(a.cd_cgc,a.cd_pessoa_fisica),CASE WHEN coalesce(a.cd_cgc::text, '') = '' THEN 'F'  ELSE 'J' END ,'B'),1,3)) cd_banco,
				...
				coalesce(substr(obter_conta_pessoa(coalesce(a.cd_cgc,a.cd_pessoa_fisica),CASE WHEN coalesce(a.cd_cgc::text, '') = '' THEN 'F'  ELSE 'J' END ,'DA'),1,1),' ') cd_digito_agencia,
		...
		if (ie_tipo_titulo_w =    '7') or (ie_tipo_titulo_w =    '8') then
		        nr_tipo_pagamento_w := '11';
		else
		        nr_tipo_pagamento_w := '07';
		end if;

		if (ie_tipo_titulo_w =    '29') then
		        informacoes2_w := id_judicial_w;
		else
		        informacoes2_w := rpad(' ',40,' ');
		end if;
		...
		ds_conteudo_w	:=	lpad(cd_banco_cobr_w,3,'0') ||
					...
					lpad(somente_numero(to_char(coalesce(vl_escritural_w,0) + coalesce(vl_juros_w,0) + coalesce(vl_multa_w,0) - coalesce(vl_desconto_w,0),'9999999999990.00')),15,'0') ||
					'000000000000000' ||
	...
	EXIT WHEN NOT FOUND; /* apply on c04 */
		...
		/* segmento J */
			...
			ds_conteudo_w	:=	lpad(cd_banco_cobr_w,3,'0') ||
						...
						to_char(dt_remessa_retorno_w,'ddmmyyyy') ||
						lpad(somente_numero(to_char((coalesce(vl_escritural_w,0) - coalesce(vl_desconto_w,0) + coalesce(vl_acrescimo_w,0) + coalesce(vl_despesa_w,0) + coalesce(vl_juros_w,0) + coalesce(vl_multa_w,0)),'9999999999999990.00')),15,'0') ||
						'000000000000000' ||
						rpad(nr_titulo_w,20,' ') ||
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE jas_gerar_pagto_sicoob_240 ( nr_seq_envio_p bigint, nm_usuario_p text) FROM PUBLIC;
