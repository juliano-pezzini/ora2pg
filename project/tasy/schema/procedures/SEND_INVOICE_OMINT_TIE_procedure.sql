...
CREATE OR REPLACE PROCEDURE send_invoice_omint_tie (nr_sequencia_p bigint, nm_usuario_p text, ie_operacao_p text, nr_destino_p bigint) AS $body$
DECLARE
...
BEGIN
    ...
    json_response_w.put('data', json_data_w.to_json_value);
    json_response_w.put('destination', nr_destino_p);

    dbms_lob.createtemporary(response_w, true);
    json_response_w.(response_w);
    response_w := bifrost.send_integration_content('api.send.invoice.conditional', response_w, nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE send_invoice_omint_tie (nr_sequencia_p bigint, nm_usuario_p text, ie_operacao_p text, nr_destino_p bigint) FROM PUBLIC;
