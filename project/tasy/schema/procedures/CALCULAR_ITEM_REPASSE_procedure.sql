...
CREATE OR REPLACE PROCEDURE calcular_item_repasse ( nr_repasse_terceiro_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
and	coalesce(ie_gerar_acao_usuario,'N')			= 'N'
...
WHERE coalesce(b.ie_rep_auto,'S')	= 'S' and a.nr_repasse_terceiro	= nr_repasse_terceiro_p and ie_repasse_neg_w	= 'S' and coalesce(nr_seq_regra,0) <> nr_seq_regra_w
...
union	all
							...
							FROM repasse_terceiro_item a
LEFT OUTER JOIN regra_esp_repasse b ON (a.nr_seq_regra_esp = b.nr_sequencia)
WHERE coalesce(b.ie_rep_auto,'S')	= 'S' and a.nr_repasse_terceiro	= nr_repasse_terceiro_p and ie_repasse_neg_w	= 'N' and a.vl_repasse		> 0 and coalesce(nr_seq_regra,0) <> nr_seq_regra_w ) alias26;
					end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE calcular_item_repasse ( nr_repasse_terceiro_p bigint, nm_usuario_p text) FROM PUBLIC;
