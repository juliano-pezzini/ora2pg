...
CREATE OR REPLACE PROCEDURE pls_oc_cta_tratar_val_113_imp ( nr_seq_combinada_p pls_oc_cta_combinada.nr_sequencia%type, ie_regra_excecao_p pls_oc_cta_combinada.ie_excecao%type, nr_id_transacao_p pls_oc_cta_selecao_imp.nr_id_transacao%type) AS $body$
DECLARE
...
c02 CURSOR(	nr_id_transacao_pc		pls_oc_cta_selecao_ocor_v.nr_id_transacao%type) FOR
	...
	and	c.nr_sequencia		= b.nr_seq_guia_conv
	and	d.nr_sequencia		= b.nr_seq_tipo_atend_conv
		...
		if (r_c01_w.ie_validar_tp_atend = 'S') then
			...
			for r_c02_w in c02(nr_id_transacao_p) loop
				...
				if (r_c02_w.cd_tiss_pad IS NOT NULL AND r_c02_w.cd_tiss_pad::text <> '') and (r_c02_w.ie_tipo_atend_tiss_pad IS NOT NULL AND r_c02_w.ie_tipo_atend_tiss_pad::text <> '') and (r_c02_w.cd_tiss_pad != r_c02_w.ie_tipo_atend_tiss_pad) then
					...
					tb_observacao_w(i)	:= 'A autorização está com o tipo de atendimento TISS ('||r_c02_w.ie_tipo_atend_tiss_pad||') diferente da conta ('||r_c02_w.cd_tiss_pad||')';
					tb_valido_w(i)		:= 'S';
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_oc_cta_tratar_val_113_imp ( nr_seq_combinada_p pls_oc_cta_combinada.nr_sequencia%type, ie_regra_excecao_p pls_oc_cta_combinada.ie_excecao%type, nr_id_transacao_p pls_oc_cta_selecao_imp.nr_id_transacao%type) FROM PUBLIC;
