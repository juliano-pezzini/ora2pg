...
CREATE OR REPLACE PROCEDURE pls_obter_regra_tit_rec_evento ( nr_titulo_p bigint, dt_competencia_p timestamp, ie_inclui_evento_p INOUT text, nr_seq_evento_p INOUT bigint, nr_seq_prestador_p INOUT bigint, cd_centro_custo_p INOUT centro_custo.cd_centro_custo%type) AS $body$
DECLARE
...
C01 CURSOR FOR
	SELECT	a.ie_inclui_evento,
		...
		a.cd_centro_custo
	from	pls_evento_regra_tit_rec a
	...
	and	((a.nr_seq_classe_tit_rec 	= nr_seq_classe_w) 		or (coalesce(a.nr_seq_classe_tit_rec::text, '') = ''))
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_obter_regra_tit_rec_evento ( nr_titulo_p bigint, dt_competencia_p timestamp, ie_inclui_evento_p INOUT text, nr_seq_evento_p INOUT bigint, nr_seq_prestador_p INOUT bigint, cd_centro_custo_p INOUT centro_custo.cd_centro_custo%type) FROM PUBLIC;
