...
CREATE OR REPLACE PROCEDURE ptu_envio_ped_compl_aut_v60 ( nr_seq_guia_p pls_guia_plano_proc.nr_seq_guia%type, nr_seq_requisicao_p pls_requisicao_proc.nr_seq_requisicao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, nr_seq_pedido_novo_p INOUT ptu_pedido_compl_aut.nr_sequencia%type) AS $body$
DECLARE
...
values (nr_seq_pedido_novo_p, ie_tipo_cliente_w, cd_cooperativa_w, 
	...
	clock_timestamp(), '060', nm_medico_solicitante_w, 
...
for c01_w in C01( nr_seq_guia_p, nr_seq_requisicao_p ) loop 
	...
	if (c01_w.nr_seq_pacote IS NOT NULL AND c01_w.nr_seq_pacote::text <> '') and (coalesce(ie_permite_pacote_w,'N')	= 'S') then 
		...
		if (ie_tipo_pacote_w	= 'A') 	and (coalesce(ie_baixo_risco_w,'N') = 'N') then 
			...
			for c03_w in C03( nr_seq_composicao_w ) loop 
				...
				if (cd_tipo_procedimento_w = 37) then 
					...
					if (ie_existe_procedimento_w = 'N') then 
						ds_procedimento_observ_w	:= ' Procedimento: '||c03_w.cd_procedimento||' - '||ds_procedimento_w||' Valor: '||vl_procedimento_pacote_w||', ';
					else 
						ds_procedimento_observ_w	:= ds_procedimento_observ_w||' '||c03_w.cd_procedimento||' - '||ds_procedimento_w||' Valor: '||vl_procedimento_pacote_w||', ';
					end if;
			...
			if (vl_total_pacote_w IS NOT NULL AND vl_total_pacote_w::text <> '') then 
	...
	if (cd_material_w IS NOT NULL AND cd_material_w::text <> '') then 
		...
		if (nr_seq_requisicao_p IS NOT NULL AND nr_seq_requisicao_p::text <> '') then 
			...
			values (nr_seq_pedido_compl_serv_w, nr_seq_pedido_novo_p, ie_tipo_tabela_w, 
				 ...
				 clock_timestamp(), trim(both somente_numero(coalesce(coalesce(nr_registro_anvisa_w, c02_w.nr_registro_anvisa), nr_registro_anvisa_gen_w))), coalesce(coalesce(cd_ref_fabricante_imp_w, c02_w.cd_ref_fabricante_imp), cd_ref_fabricante_imp_gen_w), 
		...
		if (coalesce(c02_w.cd_material_ptu,0)	<> 0) then 
			...
			if (qt_reg_generico_w	> 0) then				 
				if (coalesce(c02_w.vl_material,0) = 0) 	and (coalesce(ds_material_ptu_w,'X')	<> 'X') then 
					CALL wheb_mensagem_pck.exibir_mensagem_abort(196741); -- É obrigatório informar valor para procedimentos e materiais genéricos ou pacotes! 
				end if;
				 
				if (coalesce(ds_material_ptu_w,'X')	= 'X') then 
					CALL wheb_mensagem_pck.exibir_mensagem_abort(216706); -- É obrigatório informar descrição para procedimentos e materiais genéricos. 
				end if;
			end if;
		end if;
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_envio_ped_compl_aut_v60 ( nr_seq_guia_p pls_guia_plano_proc.nr_seq_guia%type, nr_seq_requisicao_p pls_requisicao_proc.nr_seq_requisicao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, nr_seq_pedido_novo_p INOUT ptu_pedido_compl_aut.nr_sequencia%type) FROM PUBLIC;
