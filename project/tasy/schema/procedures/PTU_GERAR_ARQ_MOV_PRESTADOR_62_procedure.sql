...
CREATE OR REPLACE PROCEDURE ptu_gerar_arq_mov_prestador_62 (nr_seq_lote_p bigint, cd_interface_p text, nm_usuario_p text, ds_local_p text, nm_arquivo_p text) AS $body$
DECLARE

 
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
Finalidade: Gerar arquivo A400 de acordo com a versão PTU 6.2 
----------------------------------------------------------------------------------------------------------------------- 
...
C02 CURSOR FOR 
	SELECT	z.nr_sequencia, 
		...
		rpad(coalesce(substr(z.ds_email,1,40),' '),40,' '), 
		rpad(coalesce(substr(z.ds_endereco_web,1,40),' '),50,' '), 
		...
		CASE WHEN x.ie_tipo_prestador=1 THEN 'N' WHEN x.ie_tipo_prestador=5 THEN 'N' WHEN x.ie_tipo_prestador=7 THEN 'N'  ELSE CASE WHEN coalesce(z.ie_instituicao_acred_ptu::text, '') = '' THEN 'N'  ELSE 'S' END  END  ie_prest_acred, 
...
ds_conteudo_w :=	lpad(nr_seq_registro_w,8,'0') || '401' || cd_unimed_destino_w || cd_unimed_origem_w || dt_geracao_w || '22' || id_ope_prest_w || 
			nr_registro_ans_w;
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	ds_conteudo_w :=	lpad(nr_seq_registro_w,8,'0') || '402' || tp_prest_w || cd_prest_w || cd_cgc_cpf_w || cd_insc_est_w || nr_conselho_w || 		 
				...
				dt_atualizacao_cad_w || cod_titulacao_1_w || cod_titulacao_2_w;
	...
	EXIT WHEN NOT FOUND; /* apply on C02 */
		...
		ds_conteudo_w :=	lpad(nr_seq_registro_w,8,'0') || '403' || tp_end_w || ds_end_w || nr_end_w || lpad(' ',15,' ') || ds_bairro_w || cd_munic_w || 
					...
					nr_latitude_w || nr_longitude_w || nr_leitos_intermed_w || prest_acred_w || nr_referencia_end_w;
	...
	EXIT WHEN NOT FOUND; /* apply on C05 */
		...
		ds_conteudo_w :=	lpad(nr_seq_registro_w,8,'0') || '406' || ie_nivel_exclusao_ptu_w || cd_exclusao_rede_w || cd_exclusao_pla_prod_w || nr_exclusao_end_w || 
					dt_termino_prest_w || ie_substituicao_w || cd_cnpj_cpf_substit_w || dt_inicio_prest_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_arq_mov_prestador_62 (nr_seq_lote_p bigint, cd_interface_p text, nm_usuario_p text, ds_local_p text, nm_arquivo_p text) FROM PUBLIC;
