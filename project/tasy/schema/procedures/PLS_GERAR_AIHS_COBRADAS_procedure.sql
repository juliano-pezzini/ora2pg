...
CREATE OR REPLACE PROCEDURE pls_gerar_aihs_cobradas ( nr_seq_competencia_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
c_aih CURSOR FOR
	...
	and	not exists (	SELECT	1
				...
				and (c.ie_tipo_movimentacao('C')))
	and	a.ie_tipo_movimentacao = 'I'
	and 	(b.nr_seq_proc_gru IS NOT NULL AND b.nr_seq_proc_gru::text <> '')
...
if (coalesce(ie_cursor_informacao_w, 'N') = 'N') then
	CALL wheb_mensagem_pck.exibir_mensagem_abort(1099429);
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_aihs_cobradas ( nr_seq_competencia_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
