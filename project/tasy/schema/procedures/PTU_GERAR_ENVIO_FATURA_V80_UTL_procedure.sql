...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_fatura_v80_utl ( nr_seq_fatura_p ptu_fatura.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, ds_local_windows_utl_p INOUT text ) AS $body$
DECLARE
...
C11 CURSOR( nr_seq_ptu_fatura_pc	ptu_fatura.nr_sequencia%type ) FOR
	SELECT	nr_sequencia,
		...
		rpad(coalesce(ptu_somente_caracter_permitido(nm_prestador,'ANS'),' '),40,' ') nm_prestador,
...
c12 CURSOR( nr_seq_nota_cobr_rrs_pc	ptu_nota_cobranca_rrs.nr_sequencia%type ) FOR
	SELECT	nr_sequencia,
		...
		rpad(coalesce(ptu_somente_caracter_permitido(nm_profissional,'ANS'),' '),40,' ') nm_profissional,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_fatura_v80_utl ( nr_seq_fatura_p ptu_fatura.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, ds_local_windows_utl_p INOUT text ) FROM PUBLIC;
