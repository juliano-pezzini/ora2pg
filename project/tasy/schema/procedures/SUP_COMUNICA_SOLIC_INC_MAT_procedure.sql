...
CREATE OR REPLACE PROCEDURE sup_comunica_solic_inc_mat ( nr_sequencia_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
ds_marca_sugerida_w		varchar(255);
ds_indicacao_w			varchar(255);
ds_media_consumo_w		varchar(255);
ds_justificativa_padrao_w		varchar(2000);
...
if (qt_existe_w > 0) then
	...
	select	ds_material,
		/*nvl(ds_marca_sugerida,'[Sem marca sugerida]'),
		nvl(ds_indicacao,'[Sem indicação]'),
		nvl(ds_media_consumo,'[Sem média de consumo]'),
		nvl(ds_justicativa_padrao,'[Sem justificativa]')*/
		coalesce(ds_marca_sugerida, wheb_mensagem_pck.get_texto(314197)),
		coalesce(ds_indicacao, wheb_mensagem_pck.get_texto(314198)),
		coalesce(ds_media_consumo, wheb_mensagem_pck.get_texto(314199)),
		coalesce(ds_justicativa_padrao, wheb_mensagem_pck.get_texto(314200))
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		select	max(b.ds_titulo),
			max(substr(replace_macro(
				...
				replace_macro(
				replace_macro(
				replace_macro(b.ds_comunicacao,
					'@nr_solicitacao',nr_sequencia_p),
					'@material', ds_material_w),
					'@marca_sugerida', ds_marca_sugerida_w),
					'@indicacao', ds_indicacao_w),
					'@media_consumo', ds_media_consumo_w),
					'@justificativa', ds_justificativa_padrao_w),1,4000)) ds_comunicacao
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE sup_comunica_solic_inc_mat ( nr_sequencia_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
