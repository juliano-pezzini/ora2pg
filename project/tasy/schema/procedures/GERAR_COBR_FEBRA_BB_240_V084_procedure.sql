...
CREATE OR REPLACE PROCEDURE gerar_cobr_febra_bb_240_v084 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR 
	SELECT	lpad(x.cd_banco,3,'0') cd_banco, 
		rpad(coalesce(substr(c.cd_ocorrencia,1,2),'0'),2,'0') cd_mov_remessa, 
		...
		lpad(coalesce(substr(obter_nosso_numero_interf(x.cd_banco,b.nr_titulo),1,20),'0'),20,'0') nr_nosso_numero,		 
		'5' ie_tipo_cobranca, 
		'1' ie_forma_cad, 
		'2' ie_documento, 
...
select	lpad(b.cd_cgc,15,'0') cd_cgc, 
	...
	rpad(coalesce(calcula_digito('Modulo11',c.cd_agencia_bancaria),'0'),1,'0') nr_digito_agencia, 
	lpad(coalesce(lpad(substr(c.cd_conta,1,11),11,'0') || rpad(c.ie_digito_conta,2,'0'),'0'),13,'0') nr_conta_corrente, 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_cobr_febra_bb_240_v084 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
