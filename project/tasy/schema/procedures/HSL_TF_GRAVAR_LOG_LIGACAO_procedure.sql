...
CREATE OR REPLACE PROCEDURE hsl_tf_gravar_log_ligacao ( cd_estabelecimento_p bigint, nr_ramal_p text, cd_tipo_central_p text, nr_fone_discado_p text, dt_inicio_p timestamp, dt_fim_p timestamp, qt_tempo_p bigint, vl_ligacao_p bigint, nm_usuario_p text, ie_entrada_saida_p text, ds_erro_p INOUT text) AS $body$
DECLARE
...
if (coalesce(nr_seq_tipo_lig_w::text, '') = '') then
	ds_erro_w	:= WHEB_MENSAGEM_PCK.get_texto(281529) || cd_tipo_central_p ||'"!' || chr(10);
end if;
...
if (coalesce(nr_seq_ramal_w::text, '') = '') then
	ds_erro_w	:= ds_erro_w || WHEB_MENSAGEM_PCK.get_texto(281530) || nr_ramal_p ||'"!';
end if;
...
if (coalesce(ds_erro_w::text, '') = '') and (ie_ignora_tipo_ligacao_w = 'N') then
	...
	if (ie_tipo_ramal_w in (2,9)) then
		...
		exception
			when others then
				ds_erro_w := WHEB_MENSAGEM_PCK.get_texto(281531);
		end;
		...
		if (cd_setor_atendimento_w IS NOT NULL AND cd_setor_atendimento_w::text <> '') then
			...
			exception
				when others then
					ds_erro_w := WHEB_MENSAGEM_PCK.get_texto(281532);
			end;
		end if;
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE hsl_tf_gravar_log_ligacao ( cd_estabelecimento_p bigint, nr_ramal_p text, cd_tipo_central_p text, nr_fone_discado_p text, dt_inicio_p timestamp, dt_fim_p timestamp, qt_tempo_p bigint, vl_ligacao_p bigint, nm_usuario_p text, ie_entrada_saida_p text, ds_erro_p INOUT text) FROM PUBLIC;
