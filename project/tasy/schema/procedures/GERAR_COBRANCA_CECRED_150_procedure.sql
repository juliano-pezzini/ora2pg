...
CREATE OR REPLACE PROCEDURE gerar_cobranca_cecred_150 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
    ...
    rpad(coalesce(b.cd_moeda,0),2,' ') cd_moeda,
		...
		lpad(' ',26,' ') ds_mensagen,
    (case 
        when (f.cd_cgc IS NOT NULL AND f.cd_cgc::text <> '') then  lpad(substr(pls_obter_dados_segurado(h.nr_seq_segurado,'C'),1,8),14,'0')                                                                                                        
        else substr(pls_obter_dados_segurado(h.nr_seq_segurado,'C'),1,14) 
        end) ds_ident_cliente_emp,		
		lpad(' ',25,' ') ds_ident_cliente_emp_atual,
     lpad(coalesce(coalesce(substr(pls_obter_dados_pagador_fin(d.nr_seq_pagador,'C'),1,11),'0') ||
		substr(pls_obter_dados_pagador_fin(d.nr_seq_pagador,'DC'),1,1),'0'),12,'0') ds_ident_cliente_banco
	FROM banco_estabelecimento x, pls_mensalidade_segurado h, titulo_receber_cobr c, cobranca_escritural a, titulo_receber_v b
...
WHERE a.nr_sequencia		= c.nr_seq_cobranca and c.nr_titulo			= b.nr_titulo and a.nr_seq_conta_banco	= x.nr_sequencia     and d.nr_sequencia = h.nr_seq_mensalidade and coalesce(g.dt_fim_vigencia::text, '') = '' and h.nr_sequencia = (SELECT min(x.nr_sequencia) from pls_mensalidade_segurado x
                       where x.nr_seq_mensalidade = d.nr_sequencia) and a.nr_sequencia	= nr_seq_cobr_escrit_p;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_cobranca_cecred_150 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
