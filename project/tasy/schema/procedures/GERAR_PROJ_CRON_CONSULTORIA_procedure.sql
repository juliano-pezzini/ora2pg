...
CREATE OR REPLACE PROCEDURE gerar_proj_cron_consultoria (dt_referencia_p timestamp) AS $body$
DECLARE
...
if (dt_referencia_p IS NOT NULL AND dt_referencia_p::text <> '') then 
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		while(dt_dia_atual_w <= last_day(dt_referencia_p)) loop 
			...
			and	a.ie_status_agenda <> 'C'  			--Almir em 13/09/2008 OS106033 Estava considerando agendamentos cancelados. 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_proj_cron_consultoria (dt_referencia_p timestamp) FROM PUBLIC;
