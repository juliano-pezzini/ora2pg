...
CREATE OR REPLACE PROCEDURE fis_gerar_reg_d590_icmsipi ( nr_seq_controle_p bigint, nr_seq_nota_p bigint) AS $body$
DECLARE


/*REGISTRO D590: REGISTRO ANALTICO DO DOCUMENTO (CDIGO 21 E 22).*/
...
nr_seq_icmsipi_D590_w fis_efd_icmsipi_D590.nr_sequencia%type;
...
/*Cursor que retorna as informaes para o registro d590*/
...
/*Criao do array com o tipo sendo da tabela especificada - FIS_EFD_ICMSIPI_D590 */

type registro is table of fis_efd_icmsipi_D590%rowtype index by integer;
...
fetch c_reg_a_doc bulk collect into vetRegDoc limit 1000;
	for i in 1 .. vetRegDoc.Count loop
		...
		/*Busca da sequencia da tabela especificada - fis_efd_icmsipi_D590*/

		select	nextval('fis_efd_icmsipi_d590_seq')
		into STRICT	nr_seq_icmsipi_D590_w
		;

		/*Inserindo valores no array para realizao do forall posteriormente*/

		fis_registros_w[qt_cursor_w].nr_sequencia        := nr_seq_icmsipi_D590_w;
		fis_registros_w[qt_cursor_w].dt_atualizacao      := clock_timestamp();
		fis_registros_w[qt_cursor_w].nm_usuario          := nm_usuario_w;
		fis_registros_w[qt_cursor_w].dt_atualizacao_nrec := clock_timestamp();
		fis_registros_w[qt_cursor_w].nm_usuario_nrec     := nm_usuario_w;
		fis_registros_w[qt_cursor_w].cd_reg              := 'D590';
		...
		fis_registros_w[qt_cursor_w].cd_cfop             := vetRegDoc[i].cd_cfop;
		fis_registros_w[qt_cursor_w].vl_bc_icms_uf       := vetRegDoc[i].vl_bc_icms_uf;
		fis_registros_w[qt_cursor_w].vl_icms_uf		 := vetRegDoc[i].vl_icms_uf;
		...
		if (nr_vetor_w >= 1000) then
			begin
				/*Inserindo registros definitivamente na tabela especifica - FIS_EFD_ICMSIPI_D590 */

				forall i in fis_registros_w.first .. fis_registros_w.last
					insert into fis_efd_icmsipi_d590 values fis_registros_w(i);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_reg_d590_icmsipi ( nr_seq_controle_p bigint, nr_seq_nota_p bigint) FROM PUBLIC;
