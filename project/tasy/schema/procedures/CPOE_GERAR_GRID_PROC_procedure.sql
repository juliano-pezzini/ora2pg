...
CREATE OR REPLACE PROCEDURE cpoe_gerar_grid_proc (( nr_atendimento_p bigint, nr_seq_procedimento_p bigint, nm_usuario_p text) is type cursor01 REFCURSOR) RETURNS timestamp AS $body$
DECLARE
...
if (nr_seq_procedimento_p IS NOT NULL AND nr_seq_procedimento_p::text <> '') and (nr_seq_procedimento_p	> 0) then 
	sql_base_w:= sql_base_w||' and c.nr_seq_proc_cpoe = :nr_seq_proc_cpoe '||enter_w;
end if;
 
sql_base_w:= sql_base_w||' order by c.nr_seq_proc_cpoe, b.dt_horario, a.nr_prescricao asc';
...
fetch c01 
bulk collect 
into procedimento_record_w;
close c01;
 
for i in 1..procedimento_record_w.count loop 
	begin 
	if (nr_seq_proc_cpoe_ant_w	<> procedimento_record_w[i].nr_seq_proc_cpoe) then 
		begin 
		nr_seq_proc_cpoe_ant_w	:= procedimento_record_w[i].nr_seq_proc_cpoe;
		ind_w	:= ind_w +1;
		nr_seq_proc_cpoe_w(ind_w)	:= nr_seq_proc_cpoe_ant_w;
	...
	nr_hora_w	:= to_char(procedimento_record_w[i].dt_horario,'hh24');
	ds_hora_w	:= 'P';
	if (procedimento_record_w[i](.dt_fim_horario IS NOT NULL AND .dt_fim_horario::text <> '')) then 
		ds_hora_w	:= 'S';
	end if;
 
	if (procedimento_record_w[i](.dt_suspensao IS NOT NULL AND .dt_suspensao::text <> '')) then 
		ds_hora_w	:= 'N';
	end if;
...
for i in 1..nr_seq_proc_cpoe_w.count loop 
	begin 
	nr_seq_proc_cpoe_ant_w	:= nr_seq_proc_cpoe_w(i);
	...
	where	nr_sequencia = nr_seq_proc_cpoe_ant_w;
...
forall i in nr_seq_proc_cpoe_w.first .. nr_seq_proc_cpoe_w.last 
	...
	where	nr_sequencia = 	nr_seq_proc_cpoe_w(i);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE cpoe_gerar_grid_proc (( nr_atendimento_p bigint, nr_seq_procedimento_p bigint, nm_usuario_p text) is type cursor01 REFCURSOR) FROM PUBLIC;
