...
CREATE OR REPLACE PROCEDURE dl_gerar_itens_aneste_plantao ( nr_seq_distribuicao_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, vl_distribuicao_p bigint) AS $body$
DECLARE
...
if (dl_obter_dados_socio(nr_seq_socio_w,'T') = 'A') then 
	...
	/* dominio DL - Tipo de item para calculo da distribuicao / Anestesias em plantao */
	...
	if (coalesce(nr_seq_tipo_distrib_w::text, '') = '') then -- afstringari 236244 28/07/2010 
		...
		ds_msg_tp_distrib_w	:= substr(dl_obter_msg_tp_distrib(1,8),1,255);
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		EXIT WHEN NOT FOUND; /* apply on C02 */
			...
			if	((to_char(dt_entrada_w, 'hh24:mi:ss') >= to_char(dt_inicio_escala_w, 'hh24:mi:ss')) and (to_char(dt_saida_w, 'hh24:mi:ss') <= to_char(dt_fim_escala_w, 'hh24:mi:ss'))) then 
			 
				qt_min_cirurgia_w	:= ((dt_saida_w - dt_entrada_w) * 1440);
				qt_rotinas_plantao_w	:= (qt_rotinas_plantao_w + ceil(qt_min_cirurgia_w / qt_max_minutos_w));		
				...
				qt_min_cirurgia_w	:= ((dt_fim_escala_w - dt_inicio_escala_w) * 1440);
				qt_rotinas_plantao_w	:= (qt_rotinas_plantao_w + ceil(qt_min_cirurgia_w / qt_max_minutos_w));					
				 
			elsif	((to_char(dt_entrada_w, 'hh24:mi:ss') >= to_char(dt_inicio_escala_w, 'hh24:mi:ss')) and (to_char(dt_saida_w, 'hh24:mi:ss') >= to_char(dt_fim_escala_w, 'hh24:mi:ss'))) then 
				 
				qt_min_cirurgia_w	:= ((dt_fim_escala_w - dt_entrada_w) * 1440);
				qt_rotinas_plantao_w	:= (qt_rotinas_plantao_w + ceil(qt_min_cirurgia_w / qt_max_minutos_w));		
				 
			elsif	((to_char(dt_entrada_w, 'hh24:mi:ss') <= to_char(dt_inicio_escala_w, 'hh24:mi:ss')) and (to_char(dt_saida_w, 'hh24:mi:ss') <= to_char(dt_fim_escala_w, 'hh24:mi:ss'))) then 
				 
				qt_min_cirurgia_w	:= ((dt_saida_W - dt_inicio_escala_w) * 1440);			
				qt_rotinas_plantao_w	:= (qt_rotinas_plantao_w + ceil(qt_min_cirurgia_w / qt_max_minutos_w));		
	...
	vl_pagar_socio_w	:= vl_unitario_anestesia_w * qt_rotinas_plantao_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dl_gerar_itens_aneste_plantao ( nr_seq_distribuicao_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, vl_distribuicao_p bigint) FROM PUBLIC;
