...
CREATE OR REPLACE PROCEDURE ptu_imp_pedido_autorizacao ( ds_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_controle_exec_p INOUT bigint) AS $body$
DECLARE
...
nr_seq_material_w		varchar(8);
nr_seq_guia_w			bigint;
nr_crm_w			varchar(20);
nr_seq_prestador_w		bigint;
nr_seq_pedido_mat_w		bigint;
nr_seq_pedido_proc_w		bigint;
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	if (substr(ds_conteudo_w,1,4)	<> 'FIM$') then 
		if (nr_seq_import_w	= 1) then 
			...
			cd_unimed_w			:= (substr(ds_conteudo_w,39,4))::numeric;
			cd_usuario_plano_w		:= trim(both substr(ds_conteudo_w,43,13));
			nr_via_cartao_w			:= (substr(ds_conteudo_w,56,2))::numeric;
			...
			cd_unimed_prest_req_w		:= (substr(ds_conteudo_w,88,4))::numeric;
			cd_prestador_requisitante_w	:= (substr(ds_conteudo_w,92,8))::numeric;
			cd_unimed_prestador_w		:= (substr(ds_conteudo_w,100,4))::numeric;
			cd_prestador_w			:= (substr(ds_conteudo_w,104,8))::numeric;
			cd_especialidade_medica_w	:= (substr(ds_conteudo_w,112,2))::numeric;
			qt_dias_doenca_w		:= (substr(ds_conteudo_w,114,5))::numeric;
			ie_urgencia_emergencia_w	:= trim(both substr(ds_conteudo_w,119,1));
			dt_atendimento_ww		:= substr(ds_conteudo_w,126,2)||substr(ds_conteudo_w,124,2)||substr(ds_conteudo_w,120,4);
			dt_atendimento_w		:= to_date(dt_atendimento_ww, 'dd/mm/yyyy hh24:mi:ss');
			...
			if (nr_seq_pedido_ant_w	<> 0) then 
				...
				exception 
				when others then 
					nr_seq_controle_exec_p	:= null;
				end;
				 
				if (coalesce(nr_seq_controle_exec_p,0)	<> 0) then 
					goto final;
				end if;
			end if;
			...
			if (nr_seq_segurado_w IS NOT NULL AND nr_seq_segurado_w::text <> '') then 
				if (ie_funcao_scs_w	= 1) then 
					...
					values (nr_seq_guia_w, nr_seq_guia_w, coalesce(dt_atendimento_w,clock_timestamp()), 
			...
			/*select	nvl(trim(substr(ds_conteudo_w,1,255)),' ') 
			...
			ds_indicacao_clinica_w	:= trim(both substr(ds_conteudo_w,1,255));
			...
			select	substr(ds_conteudo_w,1,1), 
				...
				(substr(ds_conteudo_w,98,12)||','||substr(ds_conteudo_w,110,2))::numeric  
			...
			ptu_insere_servicos_scs(	ie_tipo_tabela_w, cd_servico_w, qt_servico_w, 
							ds_opme_w, vl_procedimento_w, nr_seq_prestador_w, 
...
CALL ptu_gerar_resp_pedido_auto(nr_seq_controle_exec_p, cd_estabelecimento_p, nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_imp_pedido_autorizacao ( ds_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_controle_exec_p INOUT bigint) FROM PUBLIC;
