...
CREATE OR REPLACE PROCEDURE gerar_cobr_citibank_400 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	SELECT	'02' ie_tipo_pessoa,
			...
			substr(somente_numero(to_char(coalesce(obter_juros_multa_prev_tit_rec(a.nr_titulo,'M'),0),'99999999990.00')),1,13),
			substr(somente_numero(to_char(coalesce(obter_juros_multa_prev_tit_rec(a.nr_titulo,'J'),0),'99999999990.00')),1,13),
			substr(coalesce(b.cd_ocorrencia,'01'),1,2) cd_ocorrencia,
			substr(coalesce(obter_dados_pf_pj_estab(c.cd_estabelecimento,a.cd_pessoa_fisica, a.cd_cgc, 'M'),' '),1,40) ds_mail
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	begin
		...
		if (coalesce(vl_desconto_previsto_w,0) = 0) then
			dt_limite_desconto_w := '000000';
		end if;
		...
		ds_conteudo_w :=	'1'										|| /*001/001 9 (01) Campo obrigatoriamente preenchido com:  código: 1 (código de identificação do arquivo detalhe) */
							...
							'07'									|| /*148/149  9(02) Banco imprime o boleto?*/
							...
							rpad(coalesce(ds_mail_w,' '),40,' ')			|| /*352/391  X(40)  Este campo tem 3 finalidades*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_cobr_citibank_400 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
