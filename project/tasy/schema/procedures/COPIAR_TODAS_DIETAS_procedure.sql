...
CREATE OR REPLACE PROCEDURE copiar_todas_dietas ( ds_lista_dieta_p text, nm_usuario_p text, qt_horas_p bigint) AS $body$
DECLARE
	...
	begin
	 ...
	 while(ds_lista_dieta_w IS NOT NULL AND ds_lista_dieta_w::text <> '')	loop
		...
		if (nr_seq_sequencia_w > 0)then
			begin
			CALL copiar_dieta_anterior(nr_seq_sequencia_w, nm_usuario_p,qt_horas_p);
			end;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE copiar_todas_dietas ( ds_lista_dieta_p text, nm_usuario_p text, qt_horas_p bigint) FROM PUBLIC;
