...
CREATE OR REPLACE PROCEDURE pep_gerar_medic_dose_onc_js ( ds_lista_p text, nm_usuario_p text ) AS $body$
DECLARE
...
if (ds_lista_p IS NOT NULL AND ds_lista_p::text <> '') then
	...
	while(ds_lista_w IS NOT NULL AND ds_lista_w::text <> '') and (nr_max_loop_w > 0) loop
		...
		if (nr_seq_interno_w IS NOT NULL AND nr_seq_interno_w::text <> '') and (nr_seq_interno_w > 0) then
			begin
			CALL gerar_medic_dose_onc(nr_seq_interno_w,nm_usuario_p);
			end;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pep_gerar_medic_dose_onc_js ( ds_lista_p text, nm_usuario_p text ) FROM PUBLIC;
