...
CREATE OR REPLACE PROCEDURE ageint_finalizar_itens_lab_js (nr_seq_ageint_p bigint, dt_finalizacao_p timestamp, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
ie_exame_ja_finalizado_w    	varchar(1);
...
select  max('S')
into STRICT    ie_exame_ja_finalizado_w
from    ageint_exame_lab
where   nr_seq_ageint = nr_seq_ageint_p
and	coalesce(ie_regra,0) not in (1,2,5,nr_glosa_part_w)
and	coalesce(ie_glosa,'X') <> 'T'
...
if (ie_exame_ja_finalizado_w = 'S') then
...
CALL ageint_gerar_resumo(nr_seq_ageint_p,nm_usuario_p,cd_estabelecimento_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ageint_finalizar_itens_lab_js (nr_seq_ageint_p bigint, dt_finalizacao_p timestamp, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
