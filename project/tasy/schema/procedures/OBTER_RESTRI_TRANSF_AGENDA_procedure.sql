...
CREATE OR REPLACE PROCEDURE obter_restri_transf_agenda ( nr_seq_origem_p bigint, hr_inicio_origem_p timestamp, nr_seq_destino_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ds_erro_p INOUT text) AS $body$
DECLARE
...
ie_transf_finalizada_w	varchar(1);
...
ie_transf_finalizada_w := Obter_Param_Usuario(871, 689, obter_perfil_ativo, nm_usuario_p, wheb_usuario_pck.get_cd_estabelecimento, ie_transf_finalizada_w);
...
exception
	when others then
		ds_erro_p	:= wheb_mensagem_pck.get_texto(277694);
	end;
...
exception
	when others then
		ds_erro_p	:= wheb_mensagem_pck.get_texto(277698);
	end;
...
if (coalesce(ds_erro_p::text, '') = '') then
	if (ie_transf_cirur_w = 'N') then
		...
		if (nr_cirurgia_w > 0) then
			ds_erro_p	:= wheb_mensagem_pck.get_texto(277699);
		end if;
	end if;
end if;
...
if (coalesce(ds_erro_p::text, '') = '') then
	...
	if (ie_transf_prescr_w = 'N') then
		...
		if (nr_prescricao_w > 0) then
			ds_erro_p	:= wheb_mensagem_pck.get_texto(277701);
		end if;
		...
		if	((dt_prescricao_w IS NOT NULL AND dt_prescricao_w::text <> '')  and (dt_agenda_w IS NOT NULL AND dt_agenda_w::text <> '') and (trunc(dt_prescricao_w,'dd') <> trunc(dt_agenda_w,'dd'))) then
			ds_erro_p	:= wheb_mensagem_pck.get_texto(277705);
		end if;
	end if;
	...
	if (nr_min_dur_orig_w > nr_min_dur_dest_w) then
		ds_erro_p	:= wheb_mensagem_pck.get_texto(277713);
	end if;
	...
	if (nr_seq_proc_interno_w IS NOT NULL AND nr_seq_proc_interno_w::text <> '') then
		...
		if (ie_permite_agendar_w = 'N') then
			ds_erro_p	:= wheb_mensagem_pck.get_texto(277715);
		else
			select 	coalesce(max('S'),'N')
	...
	if (ie_transf_cancel_w = 'N') then
		...
		if (ie_status_agenda_w = 'C') then
			ds_erro_p	:= wheb_mensagem_pck.get_texto(277716);
		end if;
	end if;
	...
	if (ie_transf_difer_data_w = 'N') then
		...
		if (dt_agenda_destino_w <> dt_agenda_origem_w) then
			ds_erro_p	:= wheb_mensagem_pck.get_texto(277717);
		end if;
	end if;
	...
	if (ie_transf_finalizada_w = 'N') then
		select	coalesce(max(nr_cirurgia),0)
		into STRICT	nr_cirurgia_w
		from	agenda_paciente
		where	nr_sequencia	=	nr_seq_origem_p;
		...
		if (nr_cirurgia_w > 0) then
			...
			if (dt_termino_w IS NOT NULL AND dt_termino_w::text <> '') then
				ds_erro_p	:= wheb_mensagem_pck.get_texto(277722);
			end if;
		end if;
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE obter_restri_transf_agenda ( nr_seq_origem_p bigint, hr_inicio_origem_p timestamp, nr_seq_destino_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ds_erro_p INOUT text) FROM PUBLIC;
