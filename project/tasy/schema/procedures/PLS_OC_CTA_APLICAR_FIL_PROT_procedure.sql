...
CREATE OR REPLACE PROCEDURE pls_oc_cta_aplicar_fil_prot ( dados_regra_p pls_tipos_ocor_pck.dados_regra, dados_filtro_p pls_tipos_ocor_pck.dados_filtro, dados_consistencia_p pls_tipos_ocor_pck.dados_consistencia, dados_forma_geracao_ocor_p pls_tipos_ocor_pck.dados_forma_geracao_ocor, ie_incidencia_regra_p text, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, qt_registro_selecao_p integer default null) AS $body$
DECLARE

/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
Finalidade: Verificar se a conta se encaixa nos filtros específicos de protocolo para 
...
Alterações: 
------------------------------------------------------------------------------------------------------------------ 
jjung OS 588263 02/07/2013 - Criação da rotina. 
------------------------------------------------------------------------------------------------------------------ 
jjung 10/07/2013 
...
if (dados_filtro_p.nr_sequencia IS NOT NULL AND dados_filtro_p.nr_sequencia::text <> '') then 
	...
	for r_c_filtro in c_filtro(dados_filtro_p.nr_sequencia) loop 
		...
		-- Como a restrição por protocolo é aplicável para o nível de protocolo então a restrição montada pelo filtro será aplicada juntamente com a restrição padrão por protocolo, pois conforme 
		-- a regra de granularidade se o protocolo não encaixar na regra não verifica as contas, portanto cada tipo de filtro deve ser aplicado ao seu nível. 
		dados_restricao_w.ds_restricao_protocolo:=	dados_restricao_w.ds_restricao_protocolo || 
								pls_oc_cta_obter_restr_prot('RESTRICAO',dados_regra_p,var_cur_w,dados_filtro_prot_w);
		...
		begin 
			...
			-- Dos Filtros 
			ret_null_w	:= pls_oc_cta_obter_restr_prot('BIND',dados_regra_p,var_cur_w,dados_filtro_prot_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_oc_cta_aplicar_fil_prot ( dados_regra_p pls_tipos_ocor_pck.dados_regra, dados_filtro_p pls_tipos_ocor_pck.dados_filtro, dados_consistencia_p pls_tipos_ocor_pck.dados_consistencia, dados_forma_geracao_ocor_p pls_tipos_ocor_pck.dados_forma_geracao_ocor, ie_incidencia_regra_p text, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, qt_registro_selecao_p integer default null) FROM PUBLIC;
