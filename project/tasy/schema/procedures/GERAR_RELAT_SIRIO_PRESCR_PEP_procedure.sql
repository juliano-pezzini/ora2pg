...
CREATE OR REPLACE PROCEDURE gerar_relat_sirio_prescr_pep (nr_atendimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
union all
	...
	and 	nr_seq_exame not in (select coalesce(nr_seq_exame,0) from prescr_procedimento where nr_prescricao in (nr_prescricao_w))
...
union all
	...
	and 	nr_seq_exame not in (select coalesce(nr_seq_exame,0) from prescr_procedimento where nr_prescricao in (nr_prescricao_w,nr_prescricao_ant1_w))
...
union all
	...
	and 	nr_seq_exame not in (select coalesce(nr_seq_exame,0) from prescr_procedimento where nr_prescricao in (nr_prescricao_w,nr_prescricao_ant1_w,nr_prescricao_ant2_w))
...
union all
	...
	and 	nr_seq_exame not in (select coalesce(nr_seq_exame,0) from prescr_procedimento where nr_prescricao in (nr_prescricao_w,nr_prescricao_ant1_w,nr_prescricao_ant2_w,nr_prescricao_ant3_w))
...
union all
	...
	and 	nr_seq_exame not in (select coalesce(nr_seq_exame,0) from prescr_procedimento where nr_prescricao in (nr_prescricao_w,nr_prescricao_ant1_w,nr_prescricao_ant2_w,nr_prescricao_ant3_w,nr_prescricao_ant4_w))
	order by 2;
...
C04 CURSOR FOR /* Buscando os exames da 6 últimas prescrições*/
	SELECT 	row_number() OVER () AS rownum, x.nr_prescricao
	FROM (
	SELECT	DISTINCT
		a.nr_prescricao, (SELECT MAX(y.dt_prev_execucao) FROM prescr_procedimento y WHERE y.nr_prescricao = a.nr_prescricao)dt_prev_execucao
	FROM	exame_laboratorio d,
		exame_lab_result_item c,
		exame_lab_resultado b,
		prescr_medica a
	...
	AND	a.nr_prescricao 	IN (	SELECT	x.nr_prescricao
						FROM (SELECT	DISTINCT a.nr_prescricao
							...
							ORDER BY nr_prescricao DESC) x LIMIT 5)
							ORDER BY dt_prev_execucao DESC) x;

BEGIN

delete from w_gerar_relat_sirio_pep where lower(nm_usuario) = lower(nm_usuario_p);
delete from w_gerar_sirio_sup where lower(nm_usuario) = lower(nm_usuario_p);
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	begin
		...
		select 	nextval('w_gerar_sirio_sup_seq')
		into STRICT	nr_seq_pai_w
		;

		insert into w_gerar_sirio_sup(nr_sequencia,
					nr_seq_exame,
					nr_seq_prescr,
					ds_exame,
					nr_prescricao,
		...
		EXIT WHEN NOT FOUND; /* apply on C03 */
			...
			select	CASE WHEN coalesce(max(obter_data_aprov_lab(nr_prescricao_w,c.nr_seq_prescr)),clock_timestamp())=clock_timestamp() THEN CASE WHEN lab_obter_exame_prescrito(nr_prescricao_w,nr_seq_exame_w)='' THEN '[*]'  ELSE 'Pendente' END   ELSE coalesce(max(coalesce(substr(coalesce(coalesce(CASE WHEN c.ds_resultado='0' THEN ''  ELSE CASE WHEN d.ie_formato_resultado='V' THEN ''  ELSE c.ds_resultado END  END ,	coalesce(to_char(c.qt_resultado),to_char(CASE WHEN c.pr_resultado=0 THEN ''  ELSE c.pr_resultado END ))),c.ds_resultado),1,100),'Pendente')),'[*]') END ,
			...
			insert into w_gerar_relat_sirio_pep(nr_prescricao,
							nr_seq_exame,
							nm_exame,
							ds_referencia,
							ds_result_atual,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_relat_sirio_prescr_pep (nr_atendimento_p bigint, nm_usuario_p text) FROM PUBLIC;
