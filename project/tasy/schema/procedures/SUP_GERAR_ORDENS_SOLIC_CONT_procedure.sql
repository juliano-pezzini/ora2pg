...
CREATE OR REPLACE PROCEDURE sup_gerar_ordens_solic_cont ( nr_solic_compra_p bigint, cd_comprador_p text, dt_ordem_p timestamp, dt_inclusao_p timestamp, dt_entrega_p timestamp, cd_estabelecimento_p bigint, cd_setor_atendimento_p bigint, ie_frete_p text, cd_moeda_p bigint, ie_aviso_chegada_p text, ie_emite_obs_p text, ie_urgente_p text, nm_usuario_p text, nr_seq_tipo_compra_p bigint, nr_seq_mod_compra_p bigint, ie_entrega_solic_p text, vl_frete_p bigint default 0, ds_lista_oc_p INOUT text DEFAULT NULL) AS $body$
DECLARE
...
ie_param_147_w			varchar(15);
ie_param_302_w			varchar(15);
...
ie_centro_custo_oc_contr_w	varchar(15) := 'SC';
...
from   	(SELECT	b.nr_sequencia,
	...
	and ((ie_param_302_w = 'S') or (ie_param_302_w = 'N' and ((coalesce(a.cd_estab_regra::text, '') = '') or (a.cd_estab_regra = cd_estabelecimento_p)
								or (exists (	select	1
										from	contrato_regra_nf_estab x
										where	x.cd_estab_regra = cd_estabelecimento_p
										and	x.nr_seq_regra_nf = a.nr_sequencia)))))
	and ((b.cd_pessoa_contratada IS NOT NULL AND b.cd_pessoa_contratada::text <> '') or substr(obter_dados_pf_pj(null,b.cd_cgc_contratado,'S'),1,1) = 'A')
...
BEGIN
if (coalesce(vl_frete_p::text, '') = '') or (ie_frete_p <> 'C') then
	vl_frete_w := 0;
else
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	if (nr_item_w <> nr_item_solic_compra_w) then
		...
		if (qt_dias_entrega_w > 0) then
			...
			dt_entrega_atual_w	:= trunc(clock_timestamp(),'dd') + qt_dias_entrega_w;
		...
		if (nr_ordem_compra_w > 0) then
			...
			if (ie_acao_disp_mercado_w in ('M', 'N')) then
				...
				if (nr_seq_contrato_w > 0) and (coalesce(nr_seq_regra_contrato_w,0) = 0) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE sup_gerar_ordens_solic_cont ( nr_solic_compra_p bigint, cd_comprador_p text, dt_ordem_p timestamp, dt_inclusao_p timestamp, dt_entrega_p timestamp, cd_estabelecimento_p bigint, cd_setor_atendimento_p bigint, ie_frete_p text, cd_moeda_p bigint, ie_aviso_chegada_p text, ie_emite_obs_p text, ie_urgente_p text, nm_usuario_p text, nr_seq_tipo_compra_p bigint, nr_seq_mod_compra_p bigint, ie_entrega_solic_p text, vl_frete_p bigint default 0, ds_lista_oc_p INOUT text DEFAULT NULL) FROM PUBLIC;
