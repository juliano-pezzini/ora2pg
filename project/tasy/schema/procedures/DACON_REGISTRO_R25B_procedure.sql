...
CREATE OR REPLACE PROCEDURE dacon_registro_r25b ( nr_seq_dacon_p bigint, nm_usuario_P text, vl_receita_p bigint) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	and	cd_registro  = 'R25B';

C02 CURSOR FOR
	SELECT	rpad('R25B',4,' ')						--01
		||lpad((to_char(dt_referencia, 'MM'))::numeric ,2,0)		--02
		||lpad(1,2,0) 							--03
		||lpad(0,14,0) 					--04
		||lpad(0,14,0)							--05
		||lpad(somente_numero(vl_receita_p),14,0)				--06
		||lpad(0,14,0)							--07
		||lpad(0,14,0)					--08
		||lpad(0,14,0)							--09
		||lpad(0,14,0)				--10
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dacon_registro_r25b ( nr_seq_dacon_p bigint, nm_usuario_P text, vl_receita_p bigint) FROM PUBLIC;
