...
CREATE OR REPLACE PROCEDURE html_gerar_marc_encaixe_ageint (cd_estabelecimento_p bigint, cd_agenda_p bigint, dt_agenda_p timestamp, hr_encaixe_p timestamp, qt_duracao_p bigint, cd_pessoa_fisica_p text, cd_convenio_p bigint, cd_categoria_p text, nr_seq_proc_interno_p bigint, nm_usuario_p text, cd_setor_atendimento_p bigint, nr_seq_ageint_item_p bigint, nr_Seq_ageint_p bigint, nr_seq_ageint_lib_p bigint, cd_medico_p text, nm_usuario_confirm_encaixe_p text default null, ie_classif_agenda_p text DEFAULT NULL, ds_observacao_p text DEFAULT NULL) AS $body$
DECLARE
...
qt_duracao_w := coalesce(qt_duracao_p,0);

if (to_char(hr_encaixe_p,'hh24:mi') = '  :  ') then
	CALL wheb_mensagem_pck.exibir_mensagem_abort(184601);
end if;

select	pkg_date_utils.get_DateTime(dt_agenda_p,hr_encaixe_p)
into STRICT	dt_encaixe_w
;
...
select  max(cd_tipo_agenda),
    ...
    coalesce(max(nr_dias_fut_agendamento),0)
...
if (ie_sobreposicao_w = 'S') then
	if (cd_tipo_agenda_w = 3) then
		ds_consistencia_w := consistir_horario_agecons(cd_agenda_p, dt_encaixe_w, qt_duracao_w, 'E', ds_consistencia_w);
		...
		ds_consistencia_w := consistir_horario_agenda_exame(cd_agenda_p, dt_encaixe_w, qt_duracao_w, 'E', ds_consistencia_w);
		if (ds_consistencia_w IS NOT NULL AND ds_consistencia_w::text <> '') then
			CALL Wheb_mensagem_pck.exibir_mensagem_abort( 262328 , 'DS_MENSAGEM='||ds_consistencia_w);
		end if;
	end if;
...
SELECT * FROM Atualiza_Dados_Marcacao(cd_agenda_p, dt_encaixe_w, nr_Seq_ageint_p, 'I', qt_duracao_w, nm_usuario_p, nr_seq_ageint_item_p, nr_seq_ageint_lib_p, 'S', cd_medico_p, ie_reservado_w, nm_usuario_confirm_encaixe_p, ie_principal_w) INTO STRICT ie_reservado_w, ie_principal_w;

if (ie_agendamento_coletivo_w = 'S') then
	CALL valida_itens_agend_coletivo(nr_Seq_ageint_p);

	update agendamento_coletivo
	set   hr_inicio = dt_encaixe_w,
	dt_agenda = dt_encaixe_w,
	nr_minuto_duracao = qt_duracao_w ,
  Nm_Usuario = Nm_Usuario_P,
  dt_atualizacao = clock_timestamp()
	where  nr_seq_agenda_int = nr_seq_ageint_item_p	
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE html_gerar_marc_encaixe_ageint (cd_estabelecimento_p bigint, cd_agenda_p bigint, dt_agenda_p timestamp, hr_encaixe_p timestamp, qt_duracao_p bigint, cd_pessoa_fisica_p text, cd_convenio_p bigint, cd_categoria_p text, nr_seq_proc_interno_p bigint, nm_usuario_p text, cd_setor_atendimento_p bigint, nr_seq_ageint_item_p bigint, nr_Seq_ageint_p bigint, nr_seq_ageint_lib_p bigint, cd_medico_p text, nm_usuario_confirm_encaixe_p text default null, ie_classif_agenda_p text DEFAULT NULL, ds_observacao_p text DEFAULT NULL) FROM PUBLIC;
