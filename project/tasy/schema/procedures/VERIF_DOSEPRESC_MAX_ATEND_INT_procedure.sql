...
CREATE OR REPLACE PROCEDURE verif_dosepresc_max_atend_int (nr_seq_atendimento_p bigint, nr_sequencia_p bigint, ie_utiliza_horarios_p text, ds_erro_p INOUT text, ds_erro2_p INOUT text, ie_forma_consistencia_p INOUT text, ds_mensagem_p INOUT text, cd_material_p bigint default null, cd_unid_med_prescr_p text default null, qt_dose_prescricao_p bigint default null, ie_via_aplicacao_p text default null, ds_observacao_p text default null, is_paciente_medic_update text default 'N', nr_seq_material_p bigint default null, qt_dose_p bigint default null, cd_unid_med_dose_p text default null, cd_intervalo_p text default null) AS $body$
DECLARE
  ...
  ie_dose_cumulativa_w   varchar(1);
  valida_comulativa_w    varchar(1);
  ds_mensagem_w          varchar(32000);
  cd_perfil_w		         bigint;
  nr_seq_erro_w			     bigint;
  cd_intervalo_adm_w         varchar(50);
  qt_adminstracao_w          double precision;
  cd_unidade_cumulativa_w    varchar(10);
  dose_cumulativa_w          double precision;
  ...
  c01 CURSOR FOR
    SELECT coalesce(qt_limite_pessoa, 0),
     ...
     where cd_material = cd_material_w
       ...
       and ((qt_limite_pessoa IS NOT NULL AND qt_limite_pessoa::text <> '') or (dose_cumulativa IS NOT NULL AND dose_cumulativa::text <> ''))
       ...
       and coalesce(cd_protocolo, coalesce(cd_protocolo_w, 0)) =
...
BEGIN
  ...
  if (is_paciente_medic_update = 'S') then
    ...
    qt_dose_ww               := qt_dose_prescricao_p;
    ie_via_aplicacao_w       := ie_via_aplicacao_p;
    ds_justificativa_w       := ds_observacao_p;
    cd_intervalo_adm_w       := cd_intervalo_p;

  end if;

  if (cd_intervalo_adm_w IS NOT NULL AND cd_intervalo_adm_w::text <> '')then
    select qt_operacao
      into STRICT qt_adminstracao_w
  ...
  select nr_seq_paciente,
         ...
         QT_SUPERF_CORPORAL
    into STRICT nr_seq_paciente_w, qt_peso_w, qt_altura_w, qt_sc_w
  ...
  if (coalesce(cd_material_w,0) > 0) and (cd_unidade_medida_dose_w IS NOT NULL AND cd_unidade_medida_dose_w::text <> '') and (coalesce(qt_dose_ww,0) > 0) then
    ...
    if (qt_regra_w > 0) then
      ...
      qt_dose_w := (trunc((qt_dose_ww * 1000 / qt_conversao_dose_w), 6)/ 1000);

       if (qt_max_prescricao_w > 0) and (qt_max_prescricao_w < coalesce(qt_dose_w,0) or (qt_max_prescricao_w <= coalesce(qt_dose_w,0) and ie_dose_limite_w = 'CA')) then
                   ds_erro_p  := wheb_mensagem_pck.get_texto(279159, 'QT_LIMITE_PESSOA_P=' || qt_limite_pessoa_w ||
                      ';CD_UNID_MED_LIMITE_P=' || cd_unid_med_limite_w); -- 'Dose limite:'
      end if;
      ...
      qt_dose_w         := (trunc(dividir(qt_dose_ww * 1000, qt_conversao_dose_w), 6)/ 1000);
      qt_dose_limite_w  := (trunc(dividir(qt_limite_pessoa_w * 1000, qt_conversao_dose_limite_w), 6)/ 1000);
     ...
     elsif (qt_peso_w > 0) and (ie_dose_limite_w = 'CA') then
        begin    /*por Ciclo agrupado*/
 -- Total do paciente  em todos os protocolos
          qt_dose_w := (qt_dose_ww + (coalesce(obter_dose_total_medic_pf_onc(cd_pessoa_fisica_w, cd_material_w),0)));
        end;
      ...
      if (qt_dose_limite_w > 0) and (qt_dose_limite_w < coalesce(qt_dose_w,0) or (qt_dose_limite_w <= coalesce(qt_dose_w,0) and ie_dose_limite_w = 'CA')) then
        ds_erro2_p  := ' ';
      end if;
      end;
    else
      begin
        ...
        if (qt_regra_w > 0) then
          ...
          EXIT WHEN NOT FOUND; /* apply on c01 */
            ...
            qt_dose_w  := (trunc((qt_dose_ww * 1000 / qt_conversao_dose_w), 6)/ 1000);
            qt_dose_limite_w  := (trunc((qt_limite_pessoa_w * 1000 / qt_conversao_dose_limite_w), 6)/ 1000);

            if (ie_dose_cumulativa_w = 'S') then
              if ('S' = ds_mensagem_w := valida_dose_comulativa(cd_material_w, qt_dose_ww, cd_unidade_medida_dose_w, cd_pessoa_fisica_w, coalesce(qt_adminstracao_w,1), dose_cumulativa_w, cd_unidade_cumulativa_w)) then
                nr_seq_erro_w := gerar_erro_pac_atend(nr_seq_atendimento_p, nr_seq_material_p, 1, null, cd_perfil_w, wheb_usuario_pck.get_nm_usuario, nr_seq_erro_w, IE_FORMA_CONSISTENCIA_W, ds_mensagem_w);
              end if;
            end if;
 ...
 -- Total do paciente  em todos os protocolos
            ...
            if (qt_dose_limite_w > 0) and (qt_dose_limite_w < coalesce(qt_dose_w,0) or (qt_dose_limite_w <= coalesce(qt_dose_w,0) and ie_dose_limite_w = 'CA')) and
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE verif_dosepresc_max_atend_int (nr_seq_atendimento_p bigint, nr_sequencia_p bigint, ie_utiliza_horarios_p text, ds_erro_p INOUT text, ds_erro2_p INOUT text, ie_forma_consistencia_p INOUT text, ds_mensagem_p INOUT text, cd_material_p bigint default null, cd_unid_med_prescr_p text default null, qt_dose_prescricao_p bigint default null, ie_via_aplicacao_p text default null, ds_observacao_p text default null, is_paciente_medic_update text default 'N', nr_seq_material_p bigint default null, qt_dose_p bigint default null, cd_unid_med_dose_p text default null, cd_intervalo_p text default null) FROM PUBLIC;
