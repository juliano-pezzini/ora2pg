...
CREATE OR REPLACE PROCEDURE duplicar_atendimento_radio (cd_pessoa_fisica_p text, nm_usuario_p text, cd_pessoa_biometria_p text, nr_atendimento_p INOUT bigint) AS $body$
DECLARE
...
if (coalesce(cd_pessoa_fisica_p::text, '') = '') then
	...
	where cd_pessoa_fisica = (
		...
		where NR_SEQUENCIA = cd_pessoa_biometria_p
	 LIMIT 1);
else
	select max(nr_atendimento)
	into STRICT nr_atend_origem_w
...
insert  into atendimento_paciente(NR_ATENDIMENTO, CD_PESSOA_FISICA, CD_ESTABELECIMENTO, CD_PROCEDENCIA,
  ...
  NR_SEQ_CLASSIFICACAO,IE_TIPO_ATEND_TISS,NR_SEQ_INDICACAO,NR_SEQ_FORMA_CHEGADA,NR_SEQ_QUEIXA)
SELECT  nr_atend_novo_w, cd_pessoa_fisica, cd_estabelecimento, cd_procedencia,
  clock_timestamp(), ie_tipo_atendimento, clock_timestamp(), nm_usuario,
  cd_medico_resp, null, DS_SINTOMA_PACIENTE, DS_OBSERVACAO,
  null, ie_clinica, nm_usuario, IE_RESPONSAVEL,
...
nr_atend_retorno_w := nr_atend_novo_w;
...
when others then
  ...
  (SELECT  nr_atend_novo_w || ' - ' || cd_pessoa_fisica || ' - ' || cd_estabelecimento || ' - ' || clock_timestamp() || ' - ' || ie_tipo_atendimento || ' - ' || cd_medico_resp || ' - ' || 
    ie_clinica || ' - ' || IE_TIPO_CONVENIO || ' - ' || nr_atend_origem_w
  from  atendimento_paciente
  where  nr_atendimento  = nr_atend_origem_w));
end;
...
SELECT  nr_atend_novo_w, cd_convenio, cd_categoria, clock_timestamp(),
  null, clock_timestamp(), nm_usuario, cd_usuario_convenio,
  cd_empresa, nr_doc_convenio, cd_tipo_acomodacao, ie_tipo_guia,
...
nr_atendimento_p := nr_atend_retorno_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE duplicar_atendimento_radio (cd_pessoa_fisica_p text, nm_usuario_p text, cd_pessoa_biometria_p text, nr_atendimento_p INOUT bigint) FROM PUBLIC;
