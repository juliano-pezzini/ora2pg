...
CREATE OR REPLACE PROCEDURE pls_cancelar_item_analise ( nr_sequencia_p text, cd_estabelecimento_p bigint, nr_seq_analise_p bigint, nm_usuario_p text, nr_seq_conta_p bigint, nr_seq_grupo_atual_p bigint) AS $body$
DECLARE
...
/* 
if	(nvl(ie_origem_analise_w,1) in (1,3)) then 
	/*Gravar a glosa na análise*/
/* 
	pls_grava_glosa_analise_conta(null, nr_seq_ocorrencia_p, nr_sequencia_p, 
				   0, 0, 'Item substituido pelo auditor '||Obter_Nome_Usuario(nm_usuario_p), 
				   nr_seq_grupo_atual_p, nr_seq_analise_p, nm_usuario_p, 
				   cd_estabelecimento_p, null, nr_retorno_w, 
				   'N', 'S'  	); 
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	CALL pls_desfazer_lib_glosa_oc(nr_seq_item_glosa_oc_w, nr_seq_glosa_oc_w, nm_usuario_p, 
				 nr_seq_conta_p, nr_seq_analise_p, nr_seq_item_glosa_oc_w, ie_tipo_item_w, 
	...
	values (nextval('pls_analise_parecer_item_seq'), nr_seq_item_glosa_oc_w, nr_seq_mot_liberacao_w, 
		 ...
		 nm_usuario_p, 'Parecer criado automáticamente pelo sistema por motivo de cancelamento do item.', ie_tipo_motivo_w); -- ie_tipo_motivo_w = 'N' 
	...
	ds_observacao_w := 	'Tipo de liberação: '||chr(13)||chr(10)|| 
				...
				chr(9)||'Parecer criado automáticamente pelo sistema por motivo de cancelamento do item.';
...
/*Colocando o procedimento como cancelado */
...
CALL pls_inserir_hist_analise(nr_seq_conta_p, nr_seq_analise_p, 33, 
			 nr_seq_item_w, ie_tipo_item_w, null, 
			 null, 'O item foi cacelado',nr_seq_grupo_atual_p, 
			 nm_usuario_p, cd_estabelecimento_p);
...
if (coalesce(nr_seq_item_pos_w,0) > 0) and (coalesce(nr_seq_analise_pos_w,0) > 0) then			 
	CALL pls_cancelar_item_analise(	nr_seq_item_pos_w, cd_estabelecimento_p, nr_seq_analise_pos_w, 
					nm_usuario_p, nr_seq_conta_p, nr_seq_grupo_atual_p );
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_cancelar_item_analise ( nr_sequencia_p text, cd_estabelecimento_p bigint, nr_seq_analise_p bigint, nm_usuario_p text, nr_seq_conta_p bigint, nr_seq_grupo_atual_p bigint) FROM PUBLIC;
