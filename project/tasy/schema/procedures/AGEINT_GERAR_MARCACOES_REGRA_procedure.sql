...
CREATE OR REPLACE PROCEDURE ageint_gerar_marcacoes_regra (nr_seq_ageint_p bigint, nr_seq_item_p bigint, ie_acao_p text, nm_usuario_p text, cd_estabelecimento_p bigint, ie_transferencia_p text, ie_pendente_p INOUT text) AS $body$
DECLARE
...
C01 CURSOR FOR                                    
	...
	and	(((ie_transferencia_p = 'N') and not exists (	SELECT	1                             
														from	ageint_marcacao_usuario b                         
														where	b.nr_seq_ageint_item	= a.nr_sequencia)) 
	or	((ie_transferencia_p = 'S') and coalesce(c.ie_exame_principal,'N') = 'N')) 
	/*and	exists (	select	1 
				from	ageint_lib_usuario c, 
					ageint_horarios_usuario d 
				where	c.nr_seq_ageint_item	= a.nr_Sequencia 
				and	c.nm_usuario		= a.nm_usuario 
				and	d.nr_seq_ageint_lib	= c.nr_Sequencia)*/
...
if (ie_acao_p = 'I') then 
	...
	/* Buscando a data da marcação do proximento principal */
 
	select	max(hr_agenda), 
			max(nr_minuto_duracao) 
	into STRICT	hr_agenda_princ_w, 
	...
	select	max(nr_seq_apres), 
			coalesce(max(c.nr_minuto_duracao),nr_min_duracao_princ_w) 
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		if (hr_marcacao_w >= clock_timestamp()) then 
			...
			and		trunc(hr_agenda)	= trunc(hr_marcacao_w) 
			and		ie_status_Agenda	= 'L';
			 
			if (trunc(hr_marcacao_w) <> trunc(clock_timestamp())) and (qt_hor_livre_w	= 0) then 
			...
			if (nr_seq_horario_w > 0) then 
				...
				/* Marcando o item na Agenda Integrada */
 
				SELECT * FROM Atualiza_Dados_Marcacao(cd_agenda_w, hr_marcacao_w, nr_seq_ageint_p, 'I', nr_minuto_duracao_w, nm_usuario_p, nr_seq_item_w, nr_seq_ageint_lib_w, 'N', cd_medico_w, ie_Reservado_w, null, ie_principal_w) INTO STRICT ie_Reservado_w, ie_principal_w;
	...
	if (qt_pendente_w > 0) then 
		...
		loop 
		fetch C03 into	 
			nr_minuto_dur_marcado_w, 
			hr_agenda_marcada_w, 
			cd_agenda_marcada_w, 
			nr_seq_ageint_item_marcado_w, 
			ie_encaixe_marcado_w, 
			cd_medico_marcado_w;
		EXIT WHEN NOT FOUND; /* apply on C03 */
			begin 
			/* Desmarcando o item na Agenda Integrada */
 
			SELECT * FROM Atualiza_Dados_Marcacao(cd_agenda_marcada_w, hr_agenda_marcada_w, nr_seq_ageint_p, 'D', nr_minuto_dur_marcado_w, nm_usuario_p, nr_seq_ageint_item_marcado_w, null, ie_encaixe_marcado_w, cd_medico_marcado_w, ie_Reservado_w, null, ie_principal_w) INTO STRICT ie_Reservado_w, ie_principal_w;
			end;
		end loop;
		close C03;
		end;
...
--		insert into donhini (ds) values (nr_minuto_dur_marcado_w||' - '||hr_agenda_marcada_w||' - '||cd_agenda_marcada_w||' - '||nr_seq_ageint_item_marcado_w||' - '||ie_encaixe_marcado_w||' - '||cd_medico_marcado_w); 
		SELECT * FROM Atualiza_Dados_Marcacao(cd_agenda_marcada_w, hr_agenda_marcada_w, nr_seq_ageint_p, 'D', nr_minuto_dur_marcado_w, nm_usuario_p, nr_seq_ageint_item_marcado_w, null, ie_encaixe_marcado_w, cd_medico_marcado_w, ie_Reservado_w, null, ie_principal_w) INTO STRICT ie_Reservado_w, ie_principal_w;
		end;
	end loop;
	close C02;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ageint_gerar_marcacoes_regra (nr_seq_ageint_p bigint, nr_seq_item_p bigint, ie_acao_p text, nm_usuario_p text, cd_estabelecimento_p bigint, ie_transferencia_p text, ie_pendente_p INOUT text) FROM PUBLIC;
