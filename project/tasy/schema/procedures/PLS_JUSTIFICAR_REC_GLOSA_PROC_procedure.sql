...
CREATE OR REPLACE PROCEDURE pls_justificar_rec_glosa_proc (nr_seq_rec_glosa_proc_p pls_rec_glosa_proc.nr_sequencia%type, ds_justificativa_oper_p pls_rec_glosa_proc.ds_justificativa_oper%type, vl_acatado_p pls_rec_glosa_proc.vl_acatado%type, nm_usuario_p usuario.nm_usuario%type, nr_seq_mot_lib_glosa_p pls_mot_rec_glosa.nr_sequencia%type) AS $body$
DECLARE


nr_seq_proc_imp_w	pls_rec_glosa_proc.nr_seq_conta_proc_imp%type;
vl_recursado_w		pls_rec_glosa_proc.vl_recursado%type;
ie_status_w		pls_rec_glosa_proc.ie_status%type := '1';
nr_seq_conta_rec_w	pls_rec_glosa_conta.nr_sequencia%type;
nr_seq_conta_proc_w	pls_rec_glosa_proc.nr_seq_conta_proc%type;
...
vl_saldo_w		:= pls_obter_saldo_rec_glosa_proc(nr_seq_conta_proc_w, nr_seq_rec_glosa_proc_p);
...
if (vl_acatado_p > vl_recursado_w) then
	...
	CALL pls_gerar_log_rec_glosa( 'NIP', vl_recursado_w, nr_seq_rec_glosa_proc_p, nm_usuario_p, 'P', null, nr_seq_mot_lib_glosa_p);
elsif (vl_acatado_p < vl_saldo_w) then
	ie_status_w		:= '2'; -- Parcialmente acatado
	CALL pls_gerar_log_rec_glosa( 'APIP', vl_acatado_p, nr_seq_rec_glosa_proc_p, nm_usuario_p, 'P', null, nr_seq_mot_lib_glosa_p);
elsif (vl_acatado_p = vl_saldo_w) then
	ie_status_w		:= '3'; -- Acatado
	CALL pls_gerar_log_rec_glosa( 'AIP', vl_acatado_p, nr_seq_rec_glosa_proc_p, nm_usuario_p, 'P', null, nr_seq_mot_lib_glosa_p);
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_justificar_rec_glosa_proc (nr_seq_rec_glosa_proc_p pls_rec_glosa_proc.nr_sequencia%type, ds_justificativa_oper_p pls_rec_glosa_proc.ds_justificativa_oper%type, vl_acatado_p pls_rec_glosa_proc.vl_acatado%type, nm_usuario_p usuario.nm_usuario%type, nr_seq_mot_lib_glosa_p pls_mot_rec_glosa.nr_sequencia%type) FROM PUBLIC;
