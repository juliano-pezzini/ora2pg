...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_0100_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
fetch c_reg_0100 bulk collect into vetReg0100 limit 1000;
	...
	ds_linha_w := substr(ds_sep_w || vetReg0100[i].cd_reg ||
			     ds_sep_w || vetReg0100[i].ds_nome ||
			     ds_sep_w || vetReg0100[i].cd_cpf ||
			     ds_sep_w || vetReg0100[i].cd_crc ||
			     ds_sep_w || vetReg0100[i].cd_cnpj ||
			     ds_sep_w || vetReg0100[i].cd_cep ||
			     ds_sep_w || vetReg0100[i].ds_end ||
			     ds_sep_w || vetReg0100[i].nr_num ||
			     ds_sep_w || vetReg0100[i].ds_compl ||
			     ds_sep_w || vetReg0100[i].ds_bairro ||
			     ds_sep_w || vetReg0100[i].nr_fone ||
			     ds_sep_w || vetReg0100[i].nr_fax ||
			     ds_sep_w || vetReg0100[i].ds_email ||
			     ds_sep_w || vetReg0100[i].cd_mun || ds_sep_w,
			     1,
			     8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro		:=  vetReg0100[i].cd_reg;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_0100_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) FROM PUBLIC;
