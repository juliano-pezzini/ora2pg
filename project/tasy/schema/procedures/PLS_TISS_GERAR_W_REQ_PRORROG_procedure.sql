...
CREATE OR REPLACE PROCEDURE pls_tiss_gerar_w_req_prorrog ( nr_sequencia_autor_p bigint, ds_dir_padrao_p text, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
cd_guia_oper_w			pls_requisicao.cd_guia_principal%type;
cd_guia_refer_w			pls_requisicao.nr_sequencia%type;
ds_indicacao_clinica_w		pls_requisicao.ds_indicacao_clinica%type;
qt_dia_solicitado_w		pls_requisicao.qt_dia_solicitado%type;
qt_dia_autorizado_w		pls_requisicao.qt_dia_autorizado%type;
...
union all
		...
		select (substr(rpad(pls_obter_desc_material(a.nr_seq_material),100, ' '),1,255)), 
			(pls_obter_seq_codigo_material(a.nr_seq_material, null))::numeric  cd_procedimento, 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_tiss_gerar_w_req_prorrog ( nr_sequencia_autor_p bigint, ds_dir_padrao_p text, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
