...
CREATE OR REPLACE PROCEDURE med_atualiza_resultado_exame (nr_seq_cliente_p bigint, dt_exame_p timestamp, nr_seq_exame_p bigint, ie_manual_p text) AS $body$
DECLARE
...
expressao1_w	varchar(255) := obter_desc_expressao_idioma(774228, null, wheb_usuario_pck.get_nr_seq_idioma);--Calcula Resultado Exame
c01 CURSOR FOR
	SELECT	nr_sequencia,
		ds_calculo,
		ds_rest_calculo
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	if (nr_sequencia_ref_w <> 0) and (ie_calcula_result_w = 'N') or
		...
		CALL Exec_sql_Dinamico(expressao1_w,
	...
	if (ds_calculo_ref_w <> '0') and (ds_calculo_ref_w IS NOT NULL AND ds_calculo_ref_w::text <> '') and
		...
		if (nr_sequencia_ref_w <> 0) then
			if (coalesce(ie_manual_p,'N') = 'N') then
				CALL Exec_sql_Dinamico(expressao1_w,
					' update med_result_exame ' ||
			...
			ds_comando_w :=
				...
				' ie_tipo_resultado) (select  ' || nr_sequencia_ref_w || 				',' ||
				...
				nr_seq_cliente_p || ',null,' || chr(39) || 'N' || chr(39) ||
				' from dual) ';

			CALL Exec_sql_Dinamico(expressao1_w, ds_comando_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE med_atualiza_resultado_exame (nr_seq_cliente_p bigint, dt_exame_p timestamp, nr_seq_exame_p bigint, ie_manual_p text) FROM PUBLIC;
