...
CREATE OR REPLACE PROCEDURE ptu_gerar_resp_auditoria_v40 ( nr_seq_guia_p bigint, nr_seq_requisicao_p bigint, nr_seq_complemento_p bigint, ds_mensagem_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_origem_p INOUT bigint) AS $body$
DECLARE
...
Cursor C01 is 
	...
	and	ie_status		not in ('A','U') 
	union 
	select	nvl(cd_procedimento,cd_procedimento_ptu), 
		ie_origem_proced, 
		ie_status, 
		qt_procedimento, 
		nr_seq_pacote 
...
Cursor C03 is 
	...
	where	nr_seq_pedido		= nr_seq_complemento_p 
	and	ie_utilizado		= 'N'; 
*/
C01 CURSOR FOR 
	SELECT	cd_servico, 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_resp_auditoria_v40 ( nr_seq_guia_p bigint, nr_seq_requisicao_p bigint, nr_seq_complemento_p bigint, ds_mensagem_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_origem_p INOUT bigint) FROM PUBLIC;
