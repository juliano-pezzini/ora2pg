...
CREATE OR REPLACE PROCEDURE pfcs_tele_discontinue_orders ( nr_seq_indicator_p bigint, cd_establishment_p bigint, nm_user_p text) AS $body$
DECLARE
...
c01_discontinue_orders CURSOR FOR
	SELECT  enc.id_encounter nr_encounter,
			...
			pfcs_get_tele_time(dev.nr_sequencia, 'S') dt_recommendation_time,
...
BEGIN
	...
	for c01 in c01_discontinue_orders loop
		...
		CALL pfcs_pck_v2.pfcs_insert_details(
			...
			dt_monitor_entrance_p => c01.dt_monitor_entrance,
			qt_time_telemetry_p => c01.dt_recommendation_time,
			...
			qt_red_alarms_p => c01.nr_red_alarm_count,
			qt_yellow_alarms_p => c01.nr_yellow_alarm_count,
			cd_equipamento_p => c01.cd_equipamento,
			ds_device_p => c01.ds_equipamento,
			ds_battery_status_p => c01.battery_status,
			ds_age_range_p => c01.qt_idade_paciente,
			ds_readmission_risk_p => c01.ds_laceplus,
			...
			ds_alarms_trends_p => c01.ds_alarms_trends,
	 ...
	 := pfcs_pck_v2.pfcs_generate_results(
		vl_indicator_p => qt_total_w, ds_reference_value_p => null, nr_seq_indicator_p => nr_seq_indicator_p, nr_seq_operational_level_p => cd_establishment_p, nm_usuario_p => nm_user_p, nr_seq_panel_p => nr_seq_panel_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pfcs_tele_discontinue_orders ( nr_seq_indicator_p bigint, cd_establishment_p bigint, nm_user_p text) FROM PUBLIC;
