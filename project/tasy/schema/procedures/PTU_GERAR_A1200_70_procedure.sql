...
CREATE OR REPLACE PROCEDURE ptu_gerar_a1200_70 ( nr_seq_pacote_p ptu_pacote.nr_sequencia%type, cd_interface_p interface.cd_interface%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
c01 CURSOR(nr_seq_pacote_pc	ptu_pacote.nr_sequencia%type) FOR
	SELECT	nr_sequencia,
		...
		coalesce(nr_versao_transacao,'02') nr_versao_transacao
	from	ptu_pacote
...
c02 CURSOR(nr_seq_pacote_pc	ptu_pacote.nr_sequencia%type) FOR
	SELECT	nr_sequencia,
		...
		rpad(coalesce(to_char(ie_tipo_pacote),' '),2,' ') ie_tipo_pacote,
		rpad(coalesce(to_char(cd_especialidade),' '),2,' ') cd_especialidade,
	...
	for r_C02_w in C02(r_C01_w.nr_sequencia) loop
		...
		for r_C03_w in C03(r_C02_w.nr_sequencia) loop
			...
			ds_conteudo_w 	:=	lpad(nr_seq_linha_w,8,'0') ||
						...
						r_C03_w.ie_honorario ||
						r_C03_w.ie_tipo_participacao ||
						r_C03_w.qt_servico ||
						r_C03_w.vl_servico;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_a1200_70 ( nr_seq_pacote_p ptu_pacote.nr_sequencia%type, cd_interface_p interface.cd_interface%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
