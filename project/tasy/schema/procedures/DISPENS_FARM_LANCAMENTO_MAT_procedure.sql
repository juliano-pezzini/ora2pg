...
CREATE OR REPLACE PROCEDURE dispens_farm_lancamento_mat (cd_acao_p text, nr_atendimento_p bigint, cd_material_p bigint, qt_atual_p bigint, dt_movimento_p timestamp, cd_fornecedor_p text, nr_prescricao_p bigint, nr_seq_material_p bigint, nr_seq_lote_p bigint, cd_barras_p text default null, nm_usuario_p text DEFAULT NULL, ds_erro_p INOUT text DEFAULT NULL) AS $body$
DECLARE
...
if (coalesce(cd_setor_atendimento_w,0) = 0) and (coalesce(nr_atendimento_w,0) > 0) then
	...
	ds_erro_w := WHEB_MENSAGEM_PCK.get_texto(278817); -- 'Não encontrado setor de atendimento para o local de estoque!';
end if;
...
exception
when others then
	ds_erro_w := WHEB_MENSAGEM_PCK.get_texto(278819); -- 'Não encontrado cadastro do material no Tasy!';
end;
...
if (ie_consignado_w <> '0') and (cd_fornecedor_p IS NOT NULL AND cd_fornecedor_p::text <> '') then
	...
	exception
	when others then
		ds_erro_w := substr(WHEB_MENSAGEM_PCK.get_texto(278821) || cd_fornecedor_p,1,255); -- 'Não encontrado cadastro do fornecedor no Tasy! CGC: ' || cd_fornecedor_p,1,255);
	end;
	end;
else
	begin
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dispens_farm_lancamento_mat (cd_acao_p text, nr_atendimento_p bigint, cd_material_p bigint, qt_atual_p bigint, dt_movimento_p timestamp, cd_fornecedor_p text, nr_prescricao_p bigint, nr_seq_material_p bigint, nr_seq_lote_p bigint, cd_barras_p text default null, nm_usuario_p text DEFAULT NULL, ds_erro_p INOUT text DEFAULT NULL) FROM PUBLIC;
