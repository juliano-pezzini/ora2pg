...
CREATE OR REPLACE PROCEDURE rxt_gerar_trat_prot ((nr_seq_tumor_p bigint, ds_seq_protocolo_p text, dt_inicio_trat_p timestamp, nm_usuario_p text, nr_seq_tratamento_p INOUT text) IS  -----------------------------------------------------------------------------------------------------------------------------------------------------------
 -- Ao alterar esta procedure, verificar se a alteração não é necessária também na procedure RXT_GERAR_PROT_PEP
...
BEGIN ie_braquiterapia_prot_w := obter_valor_param_usuario(3030, 31, obter_perfil_ativo, nm_usuario_p, wheb_usuario_pck.get_cd_estabelecimento);
...
IF (ie_tipo_trat_w <> 'R')
AND ((ie_tipo_trat_w <> 'B')
     OR (ie_braquiterapia_prot_w = 'N')) THEN /* obter valores protocolo */
...
elsif (ie_tipo_trat_w = 'B')
OR (ie_braquiterapia_prot_w = 'S') THEN /* obter valores protocolo */
SELECT MAX(qt_dose_total),
       MAX(qt_check_film),
       MAX(ie_necessita_bloco),
       MAX(ie_necessita_qa) INTO STRICT qt_dose_total_w,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE rxt_gerar_trat_prot ((nr_seq_tumor_p bigint, ds_seq_protocolo_p text, dt_inicio_trat_p timestamp, nm_usuario_p text, nr_seq_tratamento_p INOUT text) IS  qt_dose_total_w bigint) FROM PUBLIC;
