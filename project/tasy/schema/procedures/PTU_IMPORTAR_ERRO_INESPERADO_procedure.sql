...
CREATE OR REPLACE PROCEDURE ptu_importar_erro_inesperado ( ds_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
	...
	if (substr(ds_conteudo_w,1,4)	<> 'FIM$') then
		if (nr_seq_importacao_w	= 1) then
			...
			CALL ptu_inserir_inconsistencia(	null, null, cd_mens_excecao_w,'',cd_estabelecimento_p, nr_seq_erro_w, 'EI',
							'00210', null, null, null, nm_usuario_p);

			CALL ptu_gravar_log_erro_inesperado(nr_seq_guia_w, nr_seq_requisicao_w, cd_mens_excecao_w, '00210', nm_usuario_p);
		end if;
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_importar_erro_inesperado ( ds_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
