...
CREATE OR REPLACE PROCEDURE gerar_serasa_boleto ( nm_usuario_p text, cd_estabelecimento_P bigint) AS $body$
DECLARE
...
nr_dig_banco_w			varchar(1);
nm_banco_w			varchar(15);
ds_linha_digitavel_w		varchar(255);
ds_local_pagto_w		varchar(70);
cd_dig_agencia_cedente_w	varchar(25);
dt_docto_w			varchar(8);
nr_docto_w			varchar(25);
nr_nosso_num_reg_dois_w		varchar(25);
...
SELECT	a.nr_titulo, 
	...
	substr(CASE WHEN coalesce(obter_dados_pf_pj(t.cd_pessoa_fisica,t.cd_cgc,'LO'),'X')='X' THEN ''  ELSE obter_dados_pf_pj(t.cd_pessoa_fisica,t.cd_cgc,'LO')||':' END || 
	...
	CASE WHEN coalesce(obter_dados_pf_pj(t.cd_pessoa_fisica,t.cd_cgc,'CO'),'X')='X' THEN ''  ELSE ', '||obter_dados_pf_pj(t.cd_pessoa_fisica,t.cd_cgc,'CO') END ,1,45), 
	...
	lpad(coalesce(to_char(t.dt_vencimento,'yyyymmdd'),'0'),8,'0'), 
	lpad(somente_numero(to_char(coalesce(t.vl_titulo,0),'9999999999990.00')),15,'0'), 
	coalesce(substr(Converte_Codigo_Bloqueto('Barra_Lido', t.nr_bloqueto),1,255),'0') ds_linha_digitavel, 
	lpad(coalesce(v.cd_banco,0),3,'0'), 
	lpad(coalesce(calcula_digito('MODULO11',v.cd_banco),0),1,'0'), 
	rpad(coalesce(obter_nome_banco(v.cd_banco),' '),15,' '), 
	rpad(coalesce(substr((SELECT max(ds_instrucao) from banco_instrucao_boleto x where v.cd_banco = x.cd_banco and nr_linha = 1),1,70),' '),70,' '), 
	rpad(coalesce(v.cd_agencia_bancaria,' '),25,' '), 
	lpad(coalesce(to_char(t.dt_atualizacao,'yyyymmdd'),'0'),8,'0'), 
	rpad(coalesce(to_char(t.nr_titulo),' '),25,' '), 
	rpad(coalesce(t.nr_nosso_numero,' '),25,' '), 
	t.nr_bloqueto 
FROM w_titulo_serasa a, titulo_receber t
LEFT OUTER JOIN banco_estabelecimento v ON (t.nr_seq_conta_banco = v.nr_sequencia)
...
select	lpad(coalesce(p.cd_cnpj_raiz,'0'),9,'0'), 
	...
	rpad(coalesce(p.nm_pessoa_contato,' '),70,' ') 
...
fetch	c01 into 
	...
	nm_banco_w, 
	ds_local_pagto_w, 
	cd_dig_agencia_cedente_w, 
	dt_docto_w, 
	nr_docto_w, 
	nr_nosso_num_reg_dois_w, 
	nr_bloqueto_w;
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	nr_seq_reg_w		:= nr_seq_reg_w + 1;
	ds_seq_reg_w		:= lpad(coalesce(nr_seq_reg_w,0),7,'0');
	ds_endereco_reg_w	:= rpad(coalesce(substr(ds_endereco_w,1,45),' '),45,' ');
	ds_complemento_w	:= rpad(coalesce(substr(ds_endereco_w,46,25),' '),25,' ');
	...
	ds_conteudo_w	:=	'2' || 
				...
				nr_dig_banco_w || 
				nm_banco_w || 
				lpad(coalesce(substr(ds_linha_digitavel_w,1,5),'0'),5,'0') || 
				lpad(coalesce(substr(ds_linha_digitavel_w,6,5),'0'),5,'0') || 
				lpad(coalesce(substr(ds_linha_digitavel_w,11,5),'0'),5,'0') || 
				lpad(coalesce(substr(ds_linha_digitavel_w,16,6),'0'),6,'0') || 
				lpad(coalesce(substr(ds_linha_digitavel_w,22,5),'0'),5,'0') || 
				lpad(coalesce(substr(ds_linha_digitavel_w,27,6),'0'),6,'0') || 
				lpad(coalesce(substr(ds_linha_digitavel_w,33,1),'0'),1,'0') || 
				rpad(coalesce(substr(ds_linha_digitavel_w,34,14),' '),14,' ') || 
				ds_local_pagto_w || 
				rpad(' ',70,' ') || 
				rpad(' ',8,' ') || 
				' ' || 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_serasa_boleto ( nm_usuario_p text, cd_estabelecimento_P bigint) FROM PUBLIC;
