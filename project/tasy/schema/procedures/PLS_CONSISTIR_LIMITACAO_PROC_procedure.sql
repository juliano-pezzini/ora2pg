...
CREATE OR REPLACE PROCEDURE pls_consistir_limitacao_proc ( nr_seq_segurado_p bigint, nr_seq_guia_proc_p bigint, nr_seq_conta_proc_p bigint, nr_seq_reembolso_proc_p bigint, nr_seq_requisicao_proc_p bigint, cd_procedimento_p bigint, ie_origem_proced_p bigint, qt_solicitada_p bigint, nr_seq_guia_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
union all
	...
	and	coalesce(a.ie_tipo_guia::text, '') = ''
	and	b.ie_situacao	= 'A'
	and	c.nr_seq_intercambio	= nr_seq_intercambio_w
...
if (ie_liberado_w = 'S') then
	...
	if (ie_tipo_consistencia_w = 'G') then
		...
		if (ie_limita_w = 'S') then
			CALL pls_gravar_motivo_glosa('1423',null,nr_seq_guia_proc_p,null,'Procedimento com limitação',nm_usuario_p,ie_origem_w,'CG',nr_seq_prestador_w, '(Regra ' || nr_seq_tipo_limitacao_w || ' -> ' || nr_seq_limitacao_proc_w || ')',null);
		...
		ie_limita_w := pls_se_limitacao_conta(nr_seq_segurado_p, nr_seq_tipo_limitacao_w, qt_permitida_w, qt_meses_intervalo_w, ie_periodo_w, qt_solicitada_p, nr_seq_conta_proc_p, ie_limita_w, ie_tipo_periodo_w, nm_usuario_p, cd_estabelecimento_p);

		if (ie_limita_w = 'S') then
			CALL pls_gravar_conta_glosa('1423',null, nr_seq_conta_proc_p,
						null,'N','Procedimento com limitação',
						nm_usuario_p,'A','CC',
		...
		if (ie_limita_w = 'S') then
			CALL pls_gravar_requisicao_glosa('1423', null, nr_seq_requisicao_proc_p,
				null, 'Procedimento com limitação', nm_usuario_p,
				nr_seq_prestador_w, cd_estabelecimento_p, null,
				'');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_consistir_limitacao_proc ( nr_seq_segurado_p bigint, nr_seq_guia_proc_p bigint, nr_seq_conta_proc_p bigint, nr_seq_reembolso_proc_p bigint, nr_seq_requisicao_proc_p bigint, cd_procedimento_p bigint, ie_origem_proced_p bigint, qt_solicitada_p bigint, nr_seq_guia_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
