...
CREATE OR REPLACE PROCEDURE ptu_gerar_resp_status_tran_v40 ( nr_seq_controle_p bigint, nr_seq_pedido_status_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_origem_p INOUT bigint) AS $body$
DECLARE
...
union
	...
	where	u.nr_seq_inconsis_scs	= v.nr_sequencia
	and	u.nr_seq_ocorrencia	= w.nr_seq_ocorrencia
	and	((w.nr_seq_guia_plano	= nr_seq_guia_plano)
...
begin
	...
	or	nr_seq_pedido_compl	= nr_seq_controle_p);
exception
when others then
	nr_seq_origem_w	:= 0;
end;
...
if (qt_registros_w	> 0) then
	delete from ptu_resp_servico_status where nr_seq_resp_ped_status in (SELECT nr_sequencia from ptu_resp_pedido_status where nr_seq_origem = nr_seq_origem_w);
	delete from ptu_resp_pedido_status where nr_seq_origem = nr_seq_origem_w;
end if;
...
values (nr_seq_resp_ped_status_w, '00361', cd_unimed_benef_w,
	 ...
	 nr_seq_guia_w, nr_seq_requisicao_w);
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (qt_itens_aud_w	> 0) then
		ie_autorizado_w	:= 4;
		qt_autorizado_w	:= 0;
	elsif (ie_status_w	in ('G','N','K','M')) then
		ie_autorizado_w	:= 1;
		qt_autorizado_w	:= 0;
	...
	values (nr_seq_res_serv_staus_w, cd_servico_w, ds_servico_w,
		 ...
		 nr_seq_resp_ped_status_w, qt_autorizado_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_resp_status_tran_v40 ( nr_seq_controle_p bigint, nr_seq_pedido_status_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_origem_p INOUT bigint) FROM PUBLIC;
