...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_arq_a700_v60 ( nr_seq_servico_p bigint, cd_estabelecimento_p text, nm_usuario_p text) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C00 */
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '702' || nr_lote_w || lpad(' ',12,' ') || cd_unimed_destino_w || '  ' || 
				cd_usuario_plano_w || nm_beneficiario_w || dt_atendimento_w || ' ' || ie_carater_atendimento_w || ' ' || cd_cid_w || 
				...
				lpad(coalesce(nr_cbo_req_w,'0'),6,'0') || dt_protocolo_w || id_rn_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_arq_a700_v60 ( nr_seq_servico_p bigint, cd_estabelecimento_p text, nm_usuario_p text) FROM PUBLIC;
