...
CREATE OR REPLACE PROCEDURE pls_gerar_lote_comis_mov_grupo ( nr_seq_lote_p bigint, nr_seq_canal_princ_p bigint, nr_seq_canal_grupo_p bigint, nr_seq_comissao_p bigint, nr_seq_regra_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
C01 CURSOR FOR	/* Filtrando beneficiários por comissão */
	...
	and	b.nr_seq_vendedor_canal = nr_seq_canal_grupo_p
...
union all
	...
	and	coalesce(b.nr_seq_intercambio::text, '') = ''
	and	d.nr_seq_vendedor_canal = nr_seq_canal_grupo_p
	and (b.nr_seq_tipo_comercial = nr_seq_tipo_benef_comercial_w or coalesce(nr_seq_tipo_benef_comercial_w,0) = 0)
	and	((trunc(b.dt_contratacao,'month') = trunc(dt_referencia_w,'month')) or (trunc(b.dt_liberacao,'month') = trunc(dt_referencia_w,'month')));
...
WHERE a.nr_sequencia = nr_seq_regra_p and (b.nr_seq_tipo_benef_comercial = nr_seq_tipo_comercial_w or coalesce(b.nr_seq_tipo_benef_comercial,0) = 0) and ie_situacao = 'A' and dt_referencia_w	between dt_inicio_vigencia and fim_dia(dt_fim_vigencia) and position(ie_acao_contrato_w in coalesce(ie_acao_contrato,ie_acao_contrato_w)) > 0 and (coalesce(ie_tipo_pessoa,coalesce(ie_tipo_pessoa_w,'0')) = coalesce(ie_tipo_pessoa_w,'0') or (ie_tipo_pessoa = 'A')) and coalesce(ie_preco,coalesce(ie_preco_w,'0')) = coalesce(ie_preco_w,'0') and coalesce(ie_movimentacao_benef,'N') = 'S' and ((a.nr_seq_proposta = nr_seq_proposta_w) or (coalesce(a.nr_seq_proposta::text, '') = '')) and ((a.nr_seq_plano = nr_seq_plano_w) or (coalesce(a.nr_seq_plano::text, '') = '')) and (((a.nr_seq_grupo_produto IS NOT NULL AND a.nr_seq_grupo_produto::text <> '') and (nr_seq_plano_w IS NOT NULL AND nr_seq_plano_w::text <> '') and pls_se_grupo_preco_produto(a.nr_seq_grupo_produto,nr_seq_plano_w) = 'S')
...
and	nr_seq_vendedor	= nr_seq_canal_princ_p;
...
union all
					...
					and	position(b.ie_acao_contrato in coalesce(ie_acao_contrato_regra_w, b.ie_acao_contrato)) > 0
					and	((CASE WHEN coalesce(a.cd_pf_estipulante::text, '') = '' THEN 'PJ'  ELSE 'PF' END  = ie_tipo_pessoa_regra_w) or (ie_tipo_pessoa_regra_w = 'A'))
					and (coalesce(nr_seq_tipo_benef_comercial_w,0) = 0 or (b.nr_seq_tipo_comercial IS NOT NULL AND b.nr_seq_tipo_comercial::text <> ''))) alias52;
				...
				if (qt_vidas_w >= coalesce(qt_vidas_min_w,qt_vidas_w)) and (qt_vidas_w <= coalesce(qt_vidas_max_w,qt_vidas_w)) and (pr_meta_w >= coalesce(pr_meta_minimo_w,pr_meta_w)) and (pr_meta_w <= coalesce(pr_meta_maximo_w,pr_meta_w)) then
				...
				if	(ie_gerar_comissao_w	= 'S' AND qt_benef_comissao_w = 0) then
					...
					insert	into pls_comissao_beneficiario( nr_sequencia, nr_seq_comissao, nr_seq_segurado, vl_incremento, nr_seq_vendedor,
							dt_atualizacao, nm_usuario, dt_atualizacao_nrec, nm_usuario_nrec, vl_comissao_benef,
							vl_origem, nr_seq_regra_mensal, nr_seq_vendedor_pf, nr_seq_proposta, cd_pessoa_fisica )
						values (	nr_seq_comissao_benef_w, nr_seq_comissao_p, nr_seq_segurado_w, vl_incremento_w, nr_seq_canal_grupo_p,
							clock_timestamp(), nm_usuario_p, clock_timestamp(), nm_usuario_p, vl_comissao_w,
							vl_comissao_w, nr_seq_regra_w, nr_seq_vendedor_pf_w, nr_proposta_adesao_w, cd_pessoa_fisica_w);
...
where	nr_seq_canal_venda = nr_seq_canal_grupo_p
and	(dt_liberacao IS NOT NULL AND dt_liberacao::text <> '')
and	coalesce(nr_seq_comissao_benef::text, '') = '';

if (qt_estornos > 0) then
	CALL pls_gerar_comissao_estornada(nr_seq_canal_grupo_p, nr_seq_comissao_p, coalesce(vl_comissao_total_w,0), nm_usuario_p);
else
	update	pls_comissao
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_lote_comis_mov_grupo ( nr_seq_lote_p bigint, nr_seq_canal_princ_p bigint, nr_seq_canal_grupo_p bigint, nr_seq_comissao_p bigint, nr_seq_regra_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
