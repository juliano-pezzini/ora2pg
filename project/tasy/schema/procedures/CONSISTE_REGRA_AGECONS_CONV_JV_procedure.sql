...
CREATE OR REPLACE PROCEDURE consiste_regra_agecons_conv_jv ( cd_convenio_p bigint, cd_categoria_p text, cd_agenda_p bigint, cd_setor_atendimento_p bigint, cd_plano_convenio_p text, cd_pessoa_fisica_p text, dt_agenda_p timestamp, cd_estabelecimento_p bigint, cd_empresa_ref_p bigint, nr_sequencia_p bigint default null, ds_mensagem_p INOUT text  DEFAULT NULL) AS $body$
DECLARE
...
ds_mensagem_ww					varchar(255) := null;
		...
		if (ie_agenda_w = 'N') then
			...
			if (qt_agen_mesmo_dia_w > 0) then
				if (coalesce(ds_mensagem_w::text, '') = '') then
					ds_mensagem_ww := substr(obter_texto_tasy(262537,wheb_usuario_pck.get_nr_seq_idioma),1,255);
				else
					ds_mensagem_ww := substr(obter_texto_dic_objeto(262328,wheb_usuario_pck.get_nr_seq_idioma, 'DS_MENSAGEM='||ds_mensagem_w),1,255);
				end if;
			end if;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE consiste_regra_agecons_conv_jv ( cd_convenio_p bigint, cd_categoria_p text, cd_agenda_p bigint, cd_setor_atendimento_p bigint, cd_plano_convenio_p text, cd_pessoa_fisica_p text, dt_agenda_p timestamp, cd_estabelecimento_p bigint, cd_empresa_ref_p bigint, nr_sequencia_p bigint default null, ds_mensagem_p INOUT text  DEFAULT NULL) FROM PUBLIC;
