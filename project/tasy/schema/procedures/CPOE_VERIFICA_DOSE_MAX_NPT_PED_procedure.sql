...
CREATE OR REPLACE PROCEDURE cpoe_verifica_dose_max_npt_ped ( nr_sequencia_p bigint, cd_estabelecimento_p bigint, ds_erro_p INOUT text, ds_erro2_p INOUT text) AS $body$
DECLARE
...
c02 CURSOR FOR 
	...
	and	c.nr_seq_npt_cpoe	= d.nr_sequencia 
...
select	max(obter_pf_usuario(a.nm_usuario_nrec, 'C')), 
...
EXIT WHEN NOT FOUND; /* apply on c02 */
	if (coalesce(cd_material_w,0) > 0) and (cd_unidade_medida_dose_w IS NOT NULL AND cd_unidade_medida_dose_w::text <> '') and (coalesce(qt_dose_ww,0) > 0) then 
		...
		if (qt_regra_w > 0) then 
			...
			select	obter_setor_atendimento(nr_atendimento), 
			...
			if (qt_max_prescricao_w > 0) and (qt_max_prescricao_w < coalesce(qt_dose_w,0)) then 
				...
				if (ie_alterado_dose_w = 'N') then 
					...
					set		qt_volume = (trunc(qt_max_prescricao_w * 1000 / qt_conversao_dose_w)/ 1000) 
					where	nr_seq_elem_mat = nr_seq_elem_mat_w 
					nut_elem_material é ela 
					nut_pac_elem_mat = cpoe_npt_produto 
					nut_pac_elemento = cpoe_npt_elemento 
					nut_pac = cpoe_dieta 
			...
			if (qt_dose_limite_w > 0) and (qt_dose_limite_w < coalesce(qt_dose_w,0)) then 
				...
				/*	update	nut_pac_elem_mat 
					set		qt_volume = (trunc(qt_dose_limite_w * 1000 / qt_conversao_dose_w)/ 1000) 
					where	nr_seq_elem_mat = nr_seq_elem_mat_w 
					and		nr_sequencia = nr_sequencia_w;*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE cpoe_verifica_dose_max_npt_ped ( nr_sequencia_p bigint, cd_estabelecimento_p bigint, ds_erro_p INOUT text, ds_erro2_p INOUT text) FROM PUBLIC;
