...
CREATE OR REPLACE PROCEDURE duplicar_novo_dados_apae ( nr_sequencia_p bigint, nr_atendimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
ie_modo_template_w := Obter_Param_Usuario(874, 28, wheb_usuario_pck.get_cd_perfil, wheb_usuario_pck.get_nm_usuario, wheb_usuario_pck.get_cd_estabelecimento, ie_modo_template_w);
...
if (ie_modo_template_w = 'S') then
	select	max(nr_sequencia)
	into STRICT	nr_seq_ehr_registro_w
	from	ehr_registro
	where	nr_seq_aval_pre = nr_sequencia_p
	and	coalesce(dt_inativacao::text, '') = '';
	if (nr_seq_ehr_registro_w IS NOT NULL AND nr_seq_ehr_registro_w::text <> '') then
		nr_seq_novo_w := Duplicar_Template(nr_seq_ehr_registro_w, Obter_Pf_Usuario(nm_usuario_p,'C'), nr_seq_novo_w, nm_usuario_p, nr_atendimento_p, nr_seq_aval_pre_w);
	end if;	
end if;	
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE duplicar_novo_dados_apae ( nr_sequencia_p bigint, nr_atendimento_p bigint, nm_usuario_p text) FROM PUBLIC;
