...
CREATE OR REPLACE PROCEDURE diops_fin_gerar_mov_despesa ( nr_seq_operadora_p bigint, nr_seq_transacao_p bigint, nr_seq_periodo_p bigint, ie_parametro_p text, nm_usuario_p text) AS $body$
DECLARE

 
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
Finalidade: Gerar os valores referentes a despesa conforme as regras e o período 
------------------------------------------------------------------------------------------------------------------- 
...
C02 CURSOR FOR 
	...
	from	w_diops_fin_mov_despesa	a 
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (ie_parametro_p = 'N') and (vl_saldo_ant_w = 0) and (vl_debito_w = 0) and (vl_credito_w = 0) and (vl_saldo_w = 0) then 
		...
		insert into w_diops_fin_mov_despesa(nr_sequencia, 
			nr_seq_operadora, 
			nr_seq_transacao, 
			ds_conta, 
			vl_saldo_anterior, 
			...
			nr_seq_periodo, 
			ds_desc_conta) 
		values (nextval('w_diops_fin_mov_despesa_seq'), 
			nr_seq_operadora_p, 
			nr_seq_transacao_p, 
			ds_conta_w, 
			vl_saldo_ant_w, 
...
EXIT WHEN NOT FOUND; /* apply on C02 */
	...
	if (ie_parametro_p = 'N') and (vl_saldo_ant_w = 0) and (vl_debito_w = 0) and (vl_credito_w = 0) and (vl_saldo_w = 0) then 
		...
		insert into diops_fin_mov_despesa(nr_sequencia, 
			cd_estabelecimento, 
			nr_seq_periodo, 
			dt_atualizacao, 
			nm_usuario, 
			...
			ds_desc_conta, 
			nr_seq_operadora) 
		values (nextval('diops_fin_mov_despesa_seq'), 
			cd_estabelecimento_w, 
			nr_seq_periodo_p, 
			clock_timestamp(), 
			nm_usuario_p, 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE diops_fin_gerar_mov_despesa ( nr_seq_operadora_p bigint, nr_seq_transacao_p bigint, nr_seq_periodo_p bigint, ie_parametro_p text, nm_usuario_p text) FROM PUBLIC;
