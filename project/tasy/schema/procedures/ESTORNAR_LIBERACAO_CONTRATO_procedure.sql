...
CREATE OR REPLACE PROCEDURE estornar_liberacao_contrato ( nr_sequencia_p contrato.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE

					
nr_seq_contrato_w	contrato.nr_sequencia%type;
dt_liberacao_w		contrato.dt_liberacao%type;
dt_aprovacao_w		contrato.dt_aprovacao%type;
dt_desdobr_aprov_w	contrato.dt_desdobr_aprov%type;
dt_reprovacao_w		contrato.dt_reprovacao%type;
...
if (nr_seq_contrato_w IS NOT NULL AND nr_seq_contrato_w::text <> '') then
	...
	nr_aprov_w := 1;
	for r_C01_w in C01( nr_seq_contrato_w ) loop
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE estornar_liberacao_contrato ( nr_sequencia_p contrato.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
