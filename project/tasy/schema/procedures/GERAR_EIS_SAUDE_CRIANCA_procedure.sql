...
CREATE OR REPLACE PROCEDURE gerar_eis_saude_crianca ( dt_parametro_p timestamp, nm_usuario_p text) AS $body$
DECLARE

 
cd_pessoa_fisica_w			varchar(10);
ie_nasc_vivo_w				varchar(1);
...
qt_peso_nasc_w				double precision;
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	select	coalesce(max(ie_unico_nasc_vivo),'S'), 
			max(qt_peso_sala_parto) 
	into STRICT	ie_nasc_vivo_w, 
			qt_peso_nasc_w 
	from	nascimento 
	where	nr_atendimento = nr_atendimento_w;
	 
	if (coalesce(qt_peso_nasc_w,0) = 0) then 
		select	max(qt_peso_nasc) 
		into STRICT	qt_peso_nasc_w 
		from	pessoa_fisica 
	...
	insert into eis_saude_crianca( 
							...
							dt_nascimento, 
							qt_peso_nasc) 
			values (			nextval('eis_saude_crianca_seq'), 
							clock_timestamp(), 
							clock_timestamp(), 
							nm_usuario_p, 
							nm_usuario_p, 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_eis_saude_crianca ( dt_parametro_p timestamp, nm_usuario_p text) FROM PUBLIC;
