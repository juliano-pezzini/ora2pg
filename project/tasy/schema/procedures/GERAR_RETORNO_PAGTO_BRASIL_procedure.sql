...
CREATE OR REPLACE PROCEDURE gerar_retorno_pagto_brasil ( nr_seq_banco_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (cd_arquivo_w	= '2')	then
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		if (cd_retorno_liq_w = cd_ocorrencia1_w) and (qt_reg_w	> 0) and (coalesce(ie_movto_bco_pag_escrit_w,'T') = 'T')	then
			...
			if (vl_escritural_w  > 0 and vl_liquidacao_w	> vl_escritural_w) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_retorno_pagto_brasil ( nr_seq_banco_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
