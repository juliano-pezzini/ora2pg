...
CREATE OR REPLACE PROCEDURE pls_baixar_titulo_rec_cobr ( nr_seq_tit_rec_cobr_p bigint, ie_acao_p text, nm_usuario_p text, dt_recebimento_p timestamp) AS $body$
DECLARE
...
CALL pls_gerar_amortizacao_regra(nr_titulo_w, 
			nr_seq_baixa_w, 
			nm_usuario_p, 
			'N');
...
when others then 
	...
	if (coalesce(ie_desfazer_baixa_w, 'N') = 'S') then 
		...
		CALL wheb_mensagem_pck.exibir_mensagem_abort(213679, 'DS_ERRO=' || ds_erro_w);
	end if;
end;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_baixar_titulo_rec_cobr ( nr_seq_tit_rec_cobr_p bigint, ie_acao_p text, nm_usuario_p text, dt_recebimento_p timestamp) FROM PUBLIC;
