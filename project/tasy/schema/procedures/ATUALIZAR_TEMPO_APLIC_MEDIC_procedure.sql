...
CREATE OR REPLACE PROCEDURE atualizar_tempo_aplic_medic ( nr_prescricao_p bigint, nr_sequencia_p bigint) AS $body$
DECLARE
...
if (coalesce(ie_via_aplicacao_w::text, '') = '') then
	select	coalesce(max(qt_minuto_aplicacao),0)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE atualizar_tempo_aplic_medic ( nr_prescricao_p bigint, nr_sequencia_p bigint) FROM PUBLIC;
