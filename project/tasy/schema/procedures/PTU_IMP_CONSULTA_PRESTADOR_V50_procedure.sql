...
CREATE OR REPLACE PROCEDURE ptu_imp_consulta_prestador_v50 ( ds_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_retorno_p INOUT bigint) AS $body$
DECLARE

/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
Finalidade: Realizar a importação de uma consulta de dados de prestador enviada pela operador aexecutora, via SCS 
------------------------------------------------------------------------------------------------------------------- 
	...
	if (substr(ds_conteudo_w,1,4)	<> 'FIM$') then 
		if (nr_seq_importacao_w	= 1) then 
			select 	substr(ds_conteudo_w,1,5), 
				...
				trim(both substr(ds_conteudo_w,123,3)) 
			...
			values (nr_seq_consulta_w, cd_transacao_w, ie_tipo_cliente_w, 
				 ...
				 ds_arquivo_p, nm_usuario_p, clock_timestamp(), 
				 nr_versao_w);
		end if;
	end if;
...
nr_seq_retorno_p := ptu_gestao_env_resp_cons_prest(nr_seq_consulta_w, cd_estabelecimento_p, nm_usuario_p, nr_seq_retorno_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_imp_consulta_prestador_v50 ( ds_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_retorno_p INOUT bigint) FROM PUBLIC;
