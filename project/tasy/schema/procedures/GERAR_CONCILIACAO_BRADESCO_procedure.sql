...
CREATE OR REPLACE PROCEDURE gerar_conciliacao_bradesco (nr_seq_extrato_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
SELECT	substr(ds_conteudo,1,1),
	...
	somente_numero_char(substr(ds_conteudo,87,16)) || ',' || LPAD(somente_numero_char(substr(ds_conteudo,103,2)),2,0),
...
select	somente_numero_char(substr(ds_conteudo,87,16)) || ',' || somente_numero_char(substr(ds_conteudo,103,2)),
	substr(ds_conteudo,81,6)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_conciliacao_bradesco (nr_seq_extrato_p bigint, nm_usuario_p text) FROM PUBLIC;
