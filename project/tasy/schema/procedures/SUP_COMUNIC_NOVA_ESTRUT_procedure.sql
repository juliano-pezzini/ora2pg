...
CREATE OR REPLACE PROCEDURE sup_comunic_nova_estrut ( cd_grupo_material_p bigint, cd_subgrupo_material_p bigint, cd_classe_material_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
c01 CURSOR FOR
	...
	and	coalesce(ie_nova_estrut,'N')				= 'S'
...
from 	regra_aviso_exec_mat
where	coalesce(ie_nova_estrut,'N') = 'S';
...
if (qt_regras_w > 0) then
	...
	if (ie_regra_w = 'S') then
		...
		if (coalesce(cd_grupo_material_p,0) <> 0) then
			...
			--ds_titulo_w	:= 'Inclusão de uma nova estrutura de material';
			ds_titulo_w	:= wheb_mensagem_pck.get_texto(314889);
			/*ds_comunic_w	:= substr('Incluso um novo grupo na estrutura material: ' || chr(13) || chr(10) ||
						'Código: ' || cd_grupo_material_p || chr(13) || chr(10) ||
						'Grupo: ' || ds_grupo_material_w,1,255);*/
			ds_comunic_w	:= substr(wheb_mensagem_pck.get_texto(314891) || chr(13) || chr(10) ||
						wheb_mensagem_pck.get_texto(314216, 'CODIGO=' || cd_grupo_material_p) || chr(13) || chr(10) ||
			...
			--ds_titulo_w	:= 'Inclusão de uma nova estrutura de material';
			ds_titulo_w	:= wheb_mensagem_pck.get_texto(314889);
			/*ds_comunic_w	:= substr('Incluso um novo subgrupo na estrutura material: ' || chr(13) || chr(10) ||
						'Código: ' || cd_subgrupo_material_p || chr(13) || chr(10) ||
						'Subgrupo: ' || ds_subgrupo_material_w,1,255);*/
			ds_comunic_w	:= substr(wheb_mensagem_pck.get_texto(314892) || chr(13) || chr(10) ||
						wheb_mensagem_pck.get_texto(314216, 'CODIGO=' || cd_subgrupo_material_p) || chr(13) || chr(10) ||
			...
			--ds_titulo_w	:= 'Inclusão de uma nova estrutura de material';
			ds_titulo_w	:= wheb_mensagem_pck.get_texto(314889);
			/*ds_comunic_w	:= substr('Incluso uma nova classe na estrutura material: ' || chr(13) || chr(10) ||
						'Código: ' || cd_classe_material_p || chr(13) || chr(10) ||
						'Classe: ' || ds_classe_material_w,1,255);*/
			ds_comunic_w	:= substr(wheb_mensagem_pck.get_texto(314893) || chr(13) || chr(10) ||
						wheb_mensagem_pck.get_texto(314216, 'CODIGO=' || cd_classe_material_p) || chr(13) || chr(10) ||
		...
		if (coalesce(cd_estabelecimento_w, 0) > 0) then
			...
			--ds_titulo_w	:= substr('Inclusão de uma nova estrutura de material no estabelecimento - ' || ds_estab_w,1,255);
			ds_titulo_w	:= substr(wheb_mensagem_pck.get_texto(314890, 'DS_ESTAB=' || ds_estab_w),1,255);
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE sup_comunic_nova_estrut ( cd_grupo_material_p bigint, cd_subgrupo_material_p bigint, cd_classe_material_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
