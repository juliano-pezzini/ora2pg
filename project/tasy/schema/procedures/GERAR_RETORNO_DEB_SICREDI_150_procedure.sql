...
CREATE OR REPLACE PROCEDURE gerar_retorno_deb_sicredi_150 (nr_seq_cobr_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_titulo_w IS NOT NULL AND nr_titulo_w::text <> '') then
		...
		values (nextval('cobranca_escrit_log_seq'),
			...
			clock_timestamp(),
			nr_seq_cobr_escrit_p,
			somente_numero(ds_titulo_w),
			clock_timestamp(),
			vl_liquido_w,
			--'Não foi importado o título ' || somente_numero(ds_titulo_w) || ', pois o mesmo não foi encontrado no Tasy.'
			wheb_mensagem_pck.get_texto(303997,'DS_TITULO_W='||somente_numero(ds_titulo_w)));
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_retorno_deb_sicredi_150 (nr_seq_cobr_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
