...
CREATE OR REPLACE PROCEDURE gerar_retorno_pagto_febraban (nr_seq_banco_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
SELECT	nr_sequencia,
	...
	substr(ds_conteudo,231,2) ds_ocorrencia,
	null ds_vl_real,
	somente_numero(substr(ds_conteudo,115,15)) ds_vl_desconto_w	
from	w_interf_retorno_itau
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	EXIT WHEN NOT FOUND; /* apply on c02 */
		...
		if (cd_retorno_liq_w = ds_ocorrencia_w and coalesce(ie_movto_bco_pag_escrit_w,'T') = 'T') then
				...
				CALL baixa_titulo_pagar(cd_estabelecimento_w,
						cd_tipo_baixa_w,
						nr_titulo_w,
						vl_pagamento_w + coalesce(vl_desconto_w,0),
						nm_usuario_p,
						nr_seq_trans_escrit_w,
						null,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_retorno_pagto_febraban (nr_seq_banco_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
