...
CREATE OR REPLACE PROCEDURE obter_valor_dinamico_bv ( ds_comando_p text, ds_parametros_p text, ds_Retorno_P INOUT bigint, ie_retorna_nulo_p text default null) AS $body$
DECLARE
...
BEGIN
	...
	while(nr_pos_separador_w > 0 ) loop
		...
		nr_pos_separador_w := position(ds_sep_bv_w in ds_parametros_w);
		if (qt_parametros_w > 1000) then /* Matheus alterei de 20 para 1000 OS 435821 Coelho ciente*/
...
if (coalesce(vl_retorno_w::text, '') = '') and (coalesce(ie_retorna_nulo_p,'N') = 'N') then
	ds_retorno_p	:= 0;
else
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE obter_valor_dinamico_bv ( ds_comando_p text, ds_parametros_p text, ds_Retorno_P INOUT bigint, ie_retorna_nulo_p text default null) FROM PUBLIC;
