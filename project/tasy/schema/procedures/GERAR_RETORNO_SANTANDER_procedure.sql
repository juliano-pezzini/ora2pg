...
CREATE OR REPLACE PROCEDURE gerar_retorno_santander (nr_seq_banco_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
c02 CURSOR FOR 
SELECT	nr_sequencia, 
	substr(ds_string,74,20) ds_nr_titulo, 
	substr(ds_string,94,8) ds_dt_pagamento, 
	substr(ds_string,120,15) ds_vl_pagamento, 
	substr(ds_string,135,20) ds_nr_documento, 
	substr(ds_string,231,10) ds_ocorrencia 
from	w_retorno_banco 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_retorno_santander (nr_seq_banco_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
