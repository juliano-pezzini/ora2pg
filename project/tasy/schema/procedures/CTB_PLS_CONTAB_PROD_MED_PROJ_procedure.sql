...
CREATE OR REPLACE PROCEDURE ctb_pls_contab_prod_med_proj ( nr_lote_contabil_p bigint, nm_usuario_p text, ie_exclusao_p text, ds_retorno_p INOUT text) AS $body$
DECLARE


/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: Contabilizar as informações provenientes da produção médica
-------------------------------------------------------------------------------------------------------------------
...
c_reversao_provisao CURSOR FOR
	SELECT	'D' ie_debito_credito, /* débito procedimento*/
...
union all
	...
	SELECT	'C' ie_debito_credito, /* crédito procedimento */
		'P' ie_proc_mat, /*Procedimento*/
		c.nr_sequencia nr_seq_item,
...
union all
	...
	select	'D' ie_debito_credito, /* débito material */
...
union all
	...
	select	'C' ie_debito_credito, /* crédito material */
		'M' ie_proc_mat, /*Material*/
		c.nr_sequencia nr_seq_item,
		c.cd_conta_provisao_deb cd_conta_contabil,
		coalesce(c.vl_provisao,0) vl_provisao,
...
union all
	...
	select	'M' ie_proc_mat, /*Material*/
		...
		'N' ie_estorno,
		a.nr_lote_contabil
	FROM pls_periodo_pagamento x, lote_contabil_parametro_w h, pls_conta_mat c, pls_conta b, pls_lote_pagamento a, pls_conta_medica_resumo d
LEFT OUTER JOIN pls_prestador p ON (d.nr_seq_prestador_pgto = p.nr_sequencia)
WHERE c.nr_sequencia		= d.nr_seq_conta_mat and c.nr_seq_conta		= d.nr_seq_conta and b.nr_sequencia		= c.nr_seq_conta and a.nr_sequencia		= d.nr_seq_lote_pgto and a.nr_seq_periodo	= x.nr_sequencia  and coalesce(a.nr_lote_contabil,0) = 0 and coalesce(d.ie_situacao,'A') 	= 'A' and a.ie_status		= 'D' and ((ie_data_lote_prod_med_w = 'T' AND ie_recurso_proprio = 'N' AND a.dt_geracao_titulos between dt_ref_inicial_w and dt_ref_final_w)
	or	(((ie_data_lote_prod_med_w = 'C') or (ie_recurso_proprio = 'S'))
	and (a.dt_mes_competencia between dt_ref_inicial_w and dt_ref_final_w))) and a.nr_sequencia		= coalesce(h.nr_documento,a.nr_sequencia)
...
union all
	...
	select	'P' ie_proc_mat, /*Procedimento*/
		...
		(coalesce(c.vl_glosa_taxa_co,0) + coalesce(c.vl_glosa_taxa_material,0) + coalesce(c.vl_glosa_taxa_servico,0)) vl_tx_intercambio_glosa,
		CASE WHEN coalesce(d.nr_seq_esquema,0)=0 THEN  c.cd_historico_ajuste  ELSE d.cd_historico_vl_ajuste END  cd_historico_vl_ajuste,
		(d.vl_liberado - CASE WHEN coalesce(d.vl_apresentado,0)=0 THEN coalesce(d.vl_calculado,0)  ELSE coalesce(d.vl_apresentado,0) END ) * (-1) vl_ajuste,
		null nr_nota_fiscal,
		d.nr_sequencia nr_seq_conta_resumo,
		b.nr_seq_plano,
		d.nr_seq_prestador_pgto,
...
WHERE y.nr_lote_contabil	= nr_lote_contabil_p and y.nr_seq_parametro	= 1 )
	SELECT	a.nr_sequencia,
		...
		a.cd_conta_cred,
		a.vl_imposto,
		trunc(CASE WHEN ie_data_trib_prod_med_w='VE' THEN 			fim_mes(a.dt_imposto)  ELSE CASE WHEN ie_data_lote_prod_med_w='T' THEN  e.dt_geracao_titulos  ELSE fim_mes(e.dt_mes_competencia) END  END ),
		a.cd_historico,
		a.cd_tributo,
...
union all
	...
	select	c.nr_sequencia,
		...
		b.cd_conta_cred,
		b.vl_evento,
		trunc(CASE WHEN ie_data_trib_prod_med_w='VE' THEN 			fim_mes(c.dt_imposto)  ELSE CASE WHEN ie_data_lote_prod_med_w='T' THEN  f.dt_geracao_titulos  ELSE fim_mes(f.dt_mes_competencia) END  END ),
		b.cd_historico,
		c.cd_tributo,
...
c_pag_prest CURSOR FOR
	SELECT	oid
	from	pls_pagamento_prestador a
	where	coalesce(a.nr_lote_contabil_cancel,0) = 0
	and	a.dt_cancelamento between dt_ref_inicial_w and dt_ref_final_w
	and	exists (SELECT	1
			from	pls_lote_pagamento x
			where	x.nr_sequencia	= a.nr_seq_lote
			and	coalesce(x.nr_lote_contabil,0) <> 0 );

type c_pag_prest_type is table of oid;
c_pag_prest_regs_w c_pag_prest_type;
BEGIN
...
if (ie_exclusao_p = 'S') then
	...
	/* Atualizar eventos e ocorrências financeiras */
	...
	open c_pag_prest;
	loop
	fetch c_pag_prest bulk collect into c_pag_prest_regs_w limit 1000;

		forall i in c_pag_prest_regs_w.first .. c_pag_prest_regs_w.last
			update	pls_pagamento_prestador a
			set	a.nr_lote_contabil_cancel	= nr_lote_contabil_p
			where	rowid = c_pag_prest_regs_w(i);

		commit;
	EXIT WHEN NOT FOUND; /* apply on c_pag_prest */
	end loop;
	close c_pag_prest;
				...
				if (coalesce(cd_conta_contabil_w::text, '') = '') then
					...
					if (ie_esquema_contabil_w = 'N') then
						...
						values (nr_seq_mvto_sem_conta_w,
							...
							ie_proc_mat_w,
							ie_debito_credito_w,
							CASE WHEN substr(ie_proc_mat_w,2,1)='G' THEN 'Glosa - Reversão da provisão'  ELSE 'Reversão da provisão' END ,
							cd_tipo_lote_contabil_w);
						end;
					end if;
				else
					...
					if (ie_compl_hist_w = 'S') then
						...
						if (qt_compl_hist_w > 0) then
							...
							/*Caso seja necessário adicionar/retirar um campo no complemento histório, realizar procedimento nas procedures auxiliares (exemplo CTB_PLS_CONTAB_REC_GLOSA)*/
		...
		fetch c_pagamento bulk collect into s_array2 limit 1000;
			for z in 1..s_array2.count loop
				...
				EXIT WHEN NOT FOUND; /* apply on c_tipo_movimento */
					...
					elsif (ie_movimento_w = 'TC') then /* Taxa de intercâmbio */
					...
					elsif (ie_movimento_w = 'TG') then /* Taxa de intercâmbio - Glosa */
		...
		if (coalesce(cd_conta_contabil_w::text, '') = '') and (ie_esquema_contabil_w = 'N') then
			...
			if (ie_centro_custo_cred_w = 'S') then
				if (cd_centro_custo_fixo_cred_w IS NOT NULL AND cd_centro_custo_fixo_cred_w::text <> '') then
					...
					SELECT * FROM pls_obter_centro_custo(	'D', '', cd_estabelecimento_w, '', '', '', '', '', cd_centro_custo_cred_w, nr_seq_regra_cc_w) INTO STRICT cd_centro_custo_cred_w, nr_seq_regra_cc_w;
				end if;
			...
			if (ie_centro_custo_deb_w = 'S') then
				if (cd_centro_custo_fixo_deb_w IS NOT NULL AND cd_centro_custo_fixo_deb_w::text <> '') then
					...
					SELECT * FROM pls_obter_centro_custo(	'D', '', cd_estabelecimento_w, '', '', '', '', '', cd_centro_custo_deb_w, nr_seq_regra_cc_w) INTO STRICT cd_centro_custo_deb_w, nr_seq_regra_cc_w;
				end if;
			...
			item_mov_contabil_w.ds_compl_historico	:= ds_compl_historico_w;
			item_mov_contabil_w.nr_seq_agrupamento	:= nr_seq_agrupamento_w;
			item_mov_contabil_w.nr_seq_tab_orig	:= nr_seq_evento_mvto_w;
			item_mov_contabil_w.nr_seq_info		:= nr_seq_info_ctb_w;
			item_mov_contabil_w.nm_tabela		:= nm_tabela_w;
			item_mov_contabil_w.nm_atributo		:= nm_atributo_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ctb_pls_contab_prod_med_proj ( nr_lote_contabil_p bigint, nm_usuario_p text, ie_exclusao_p text, ds_retorno_p INOUT text) FROM PUBLIC;
