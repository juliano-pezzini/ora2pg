...
CREATE OR REPLACE PROCEDURE dar_start_mapping_tables_relat (table_p text, CD_ESTABELECIMENTO_P dar_tables_relat.CD_ESTABELECIMENTO%TYPE DEFAULT NULL, NM_USUARIO_P dar_tables_relat.NM_USUARIO%TYPE DEFAULT NULL) AS $body$
DECLARE
...
BEGIN
  ...
  BEGIN 
    ...
    IF l_reverse_relations.count > 0 THEN
      ...
      LOOP 
		...
		AND coalesce(nr_seq_reference::text, '') = '';
        ...
        IF already_exists_num_f_w < 1 THEN
          ...
          CALL DAR_RECUR_MAPPING_TABLES_RELAT(table_p, l_reverse_relations[x].table_name, nr_seq_reference_f_ret_w, 0, CD_ESTABELECIMENTO_P, NM_USUARIO_P);
        END IF;
      END LOOP;
      COMMIT;
    END IF;
  ...
  DECLARE
    relationship_cur CURSOR FOR
      ...
      AND        REF_COL.constraint_name = C.r_constraint_name 
      AND        REF_COL.position = CO.position 
      WHERE      C.table_name = table_p 
      AND        CO.column_name = REF_COL.column_name
      AND        C.constraint_type = 'R'
      AND        REF_COL.table_name IN (SELECT DS_TABLE FROM DAR_TABLES_FK);

    TYPE RelationshipType IS TABLE OF relationship_cur%ROWTYPE;
    l_relations RelationshipType;
  ...
  THEN
    ...
    LOOP
      ...
      IF already_exists_num_s_w < 1 THEN
        ...
        CALL DAR_RECUR_MAPPING_TABLES_RELAT(table_p, l_relations[i].ref_table_name, nr_seq_reference_s_ret_w, 0, CD_ESTABELECIMENTO_P, NM_USUARIO_P);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dar_start_mapping_tables_relat (table_p text, CD_ESTABELECIMENTO_P dar_tables_relat.CD_ESTABELECIMENTO%TYPE DEFAULT NULL, NM_USUARIO_P dar_tables_relat.NM_USUARIO%TYPE DEFAULT NULL) FROM PUBLIC;
