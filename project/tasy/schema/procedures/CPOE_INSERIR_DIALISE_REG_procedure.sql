...
qt_ultrafiltracao_total_w	cpoe_dialise.qt_ultrafiltracao_total%type;
...
qt_volume_sol_ml_w          prescr_material.qt_solucao%type;
...
procedure gerar_componente_material(	cd_material_p 			number,
	...
	begin
		...
		begin
            ...
            qt_unitaria_w := obter_conversao_unid_med_cons(cd_material_p, cd_unid_med_dose_sol_p, qt_dose_p);
            qt_volume_sol_ml_w := obter_conversao_ml(cd_material_p, qt_dose_p, cd_unid_med_dose_sol_p);
			SELECT * FROM obter_quant_dispensar(	cd_estabelecimento_p, cd_material_p, nr_prescricao_p, nr_sequencia_mat_w, null, upper(obter_via_usuario('IV')), qt_unitaria_w, 0, 1, null, null, cd_unid_med_dose_sol_p, 0, qt_material_w, qt_total_dispensar_w, ie_regra_disp_w, ds_erro_w, null, null) INTO STRICT qt_material_w, qt_total_dispensar_w, ie_regra_disp_w, ds_erro_w;
		exception
		when others then
			CALL gravar_log_cpoe(substr('CPOE_INSERIR_DIALISE_REG EXCEPTION (obter_quant_dispensar):'||substr(to_char(sqlerrm),1,2000)
				||'//cd_estabelecimento_p : '	|| cd_estabelecimento_p
				||' cd_material_p : '|| cd_material_p
				||' nr_prescricao_p : ' || nr_prescricao_p
				||' nr_sequencia_mat_w : ' || nr_sequencia_mat_w
				||' upper(obter_via_usuario(''IV'')) : '|| upper(obter_via_usuario('IV'))
				||' qt_dose_p : '|| qt_dose_p
				...
				nr_atendimento_p, 'D', nr_sequencia_p);			
		end;
		...
		begin
			insert into prescr_material(   cd_material,
								 ...
								 values (
										   ...
										   clock_timestamp(), 
										   nr_seq_solucao_w, 
										   coalesce(qt_material_w,qt_dose_p),
										   qt_unitaria_w, 
										   13,
										   'N',
		...
		exception when others then		
			CALL gravar_log_cpoe(substr('CPOE_INSERIR_DIALISE_REG EXCEPTION INSERT PRESCR_MATERIAL:'||substr(to_char(sqlerrm),1,2000)
				||'//cd_material_p : '	|| cd_material_p
				||' cd_unid_med_dose_sol_p : '|| cd_unid_med_dose_sol_p
				||' qt_dose_p : ' || qt_dose_p
				||' nr_sequencia_mat_w : ' || nr_sequencia_mat_w
	...
	begin
		...
		begin
			if	((qt_tempo_infusao_w IS NOT NULL AND qt_tempo_infusao_w::text <> '') and (trim(both qt_tempo_infusao_w) <> ':')) then
				qt_tempo_infusao_w :=  (((substr(qt_tempo_infusao_w,1,2))::numeric  * 60) + (substr(qt_tempo_infusao_w,4,5))::numeric );
			end if;
			
			if	((qt_tempo_permanencia_w IS NOT NULL AND qt_tempo_permanencia_w::text <> '') and (trim(both qt_tempo_permanencia_w) <> ':')) then
			...
			if	((qt_tempo_drenagem_w IS NOT NULL AND qt_tempo_drenagem_w::text <> '') and (trim(both qt_tempo_drenagem_w) <> ':')) then
				qt_tempo_drenagem_w := (((substr(qt_tempo_drenagem_w,1,2))::numeric  * 60) + (substr(qt_tempo_drenagem_w,4,5))::numeric );
			end if;				
		exception when others then			
			CALL gravar_log_cpoe(substr('CPOE_INSERIR_DIALISE_REG EXCEPTION CALC TEMPO DRENAGEM:'||substr(to_char(sqlerrm),1,2000)
		...
		begin		
			insert into prescr_solucao( nr_seq_solucao,
							 ...
							 values (
										...
										'N');	
		exception when others then	
			CALL gravar_log_cpoe(substr('CPOE_INSERIR_DIALISE_REG EXCEPTION INSERT PRESCR_SOLUCAO:'||substr(to_char(sqlerrm),1,2000)
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	exception when others then
		CALL gravar_log_cpoe(substr('CPOE_INSERIR_DIALISE_REG EXCEPTION LOCALIZAR_ULTIMO_ITEM:'||substr(to_char(sqlerrm),1,2000)
	...
	if ( dt_proxima_dose_w > dt_inicio_base_w + 1 - 1/86400) then
		...
		if ( coalesce(nr_prescricao_orig_w::text, '') = '') then				
			begin
				...
				commit;
			exception when others then
				CALL gravar_log_cpoe(substr('CPOE_INSERIR_DIALISE_REG EXCEPTION UPDATE CPOE_DIALISE PROX_GERACAO IF:'||substr(to_char(sqlerrm),1,2000)
			...
			begin
				...
				commit;
			exception when others then
				CALL gravar_log_cpoe(substr('CPOE_INSERIR_DIALISE_REG EXCEPTION UPDATE CPOE_DIALISE PROX_GERACAO ELSE:'||substr(to_char(sqlerrm),1,2000)
					||' dt_proxima_dose_w : ' || to_char(dt_proxima_dose_w,'dd/mm/yyyy hh24:mi:ss')
	...
	if (dt_fim_dialise_w IS NOT NULL AND dt_fim_dialise_w::text <> '') and (dt_fim_dialise_w <= dt_inicio_w) then		
		...
		CALL gravar_log_cpoe(substr('CPOE_INSERIR_DIALISE_REG LOG DT_FIM_DIALISE_W <= DT_INICIO_W:'||substr(to_char(sqlerrm),1,2000)
			||'//dt_inicio_base_w : ' || to_char(dt_inicio_base_w,'dd/mm/yyyy hh24:mi:ss')
			||' dt_inicio_w : ' || to_char(dt_inicio_w,'dd/mm/yyyy hh24:mi:ss')
	...
	begin
		insert into hd_prescricao(	nr_sequencia,
							...
							values (
									...
									nr_ciclos_w,
									qt_volume_ciclo_w,
									CASE WHEN trim(both qt_hora_min_duracao_w)=':' THEN  null  ELSE qt_hora_min_duracao_w END ,
									qt_min_duracao_w,
									nr_seq_maq_cicladora_w,
									nr_serie_w,
									qt_volume_total_di_w,
									...
									qt_cinco_seis_bic_w);
	exception when others then
		CALL gravar_log_cpoe(substr('CPOE_INSERIR_DIALISE_REG EXCEPTION INSERT HD_PRESCRICAO:'||substr(to_char(sqlerrm),1,2000)
			||'//nr_atendimento_p : ' 	|| nr_atendimento_p
			||' nr_prescricao_p : '	 	|| nr_prescricao_p
			||' nr_sequencia_p : ' 		|| nr_sequencia_p
    ...
    if (qt_min_sessao_w > 0) then
        qt_hora_sessao_w := qt_hora_min_sessao_w + (qt_min_sessao_w/60);
    else
        qt_hora_sessao_w := qt_hora_min_sessao_w;
    end if;
	...
	if (nr_seq_protocolo_w IS NOT NULL AND nr_seq_protocolo_w::text <> '') then
		...
		if (coalesce(cd_mat_soluc2_w,0) > 0) then
			gerar_componente_material(cd_mat_soluc2_w, cd_unid_med_dose_sol2_w, qt_dose_soluc2_w);
		end if;
        ...
        if (ie_tipo_solucao_w = 'C' or ie_categoria_w = 'C') then
            if (qt_solucao_total_w IS NOT NULL AND qt_solucao_total_w::text <> '') then
                qt_vol_total_w 	:= null;
                nr_etapas_w		:= null;
                if (qt_dose_soluc1_w IS NOT NULL AND qt_dose_soluc1_w::text <> '') then
                    qt_vol_total_w := obter_conversao_ml(cd_mat_soluc1_w, qt_dose_soluc1_w, cd_unid_med_dose_sol1_w);
                end if;
                if (qt_dose_soluc2_w IS NOT NULL AND qt_dose_soluc2_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc2_w, qt_dose_soluc2_w, cd_unid_med_dose_sol2_w);
                end if;
                if (qt_dose_soluc3_w IS NOT NULL AND qt_dose_soluc3_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc3_w, qt_dose_soluc3_w, cd_unid_med_dose_sol3_w);
                end if;
                if (qt_dose_soluc4_w IS NOT NULL AND qt_dose_soluc4_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc4_w, qt_dose_soluc4_w, cd_unid_med_dose_sol4_w);
                end if;
                if (qt_dose_soluc5_w IS NOT NULL AND qt_dose_soluc5_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc5_w, qt_dose_soluc5_w, cd_unid_med_dose_sol5_w);
                end if;
                if (qt_dose_soluc6_w IS NOT NULL AND qt_dose_soluc6_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc6_w, qt_dose_soluc6_w, cd_unid_med_dose_sol6_w);
                end if;
                if (qt_dose_soluc7_w IS NOT NULL AND qt_dose_soluc7_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc7_w, qt_dose_soluc7_w, cd_unid_med_dose_sol7_w);
                end if;
                nr_etapas_w := qt_solucao_total_w / qt_vol_total_w;
                CALL ajustar_inf_dialise_js(nr_prescricao_p, nr_seq_solucao_w, ceil(nr_etapas_w), dt_inicio_dialise_w, qt_hora_sessao_w, nm_usuario_p);
            end if;
        end if;
	end if;
	...
	if (nr_seq_protocolo2_w IS NOT NULL AND nr_seq_protocolo2_w::text <> '') then
        ...
        if (ie_tipo_solucao2_w = 'C' or ie_categoria_w = 'C') then
            if (qt_solucao_total2_w IS NOT NULL AND qt_solucao_total2_w::text <> '') then
                qt_vol_total_w 	:= null;
                nr_etapas_w		:= null;
                if (qt_dose_soluc1_2_w IS NOT NULL AND qt_dose_soluc1_2_w::text <> '') then
                    qt_vol_total_w := obter_conversao_ml(cd_mat_soluc1_2_w, qt_dose_soluc1_2_w, cd_unid_med_dose_sol1_2_w);
                end if;
                if (qt_dose_soluc2_2_w IS NOT NULL AND qt_dose_soluc2_2_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc2_2_w, qt_dose_soluc2_2_w, cd_unid_med_dose_sol2_2_w);
                end if;
                if (qt_dose_soluc3_2_w IS NOT NULL AND qt_dose_soluc3_2_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc3_2_w, qt_dose_soluc3_2_w, cd_unid_med_dose_sol3_2_w);
                end if;
                if (qt_dose_soluc4_2_w IS NOT NULL AND qt_dose_soluc4_2_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc4_2_w, qt_dose_soluc4_2_w, cd_unid_med_dose_sol4_2_w);
                end if;
                if (qt_dose_soluc5_2_w IS NOT NULL AND qt_dose_soluc5_2_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc5_2_w, qt_dose_soluc5_2_w, cd_unid_med_dose_sol5_2_w);
                end if;
                if (qt_dose_soluc6_2_w IS NOT NULL AND qt_dose_soluc6_2_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc6_2_w, qt_dose_soluc6_2_w, cd_unid_med_dose_sol6_2_w);
                end if;
                if (qt_dose_soluc7_2_w IS NOT NULL AND qt_dose_soluc7_2_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc7_2_w, qt_dose_soluc7_2_w, cd_unid_med_dose_sol7_2_w);
                end if;
                nr_etapas_w := qt_solucao_total2_w / qt_vol_total_w;
                CALL ajustar_inf_dialise_js(nr_prescricao_p, nr_seq_solucao_w, ceil(nr_etapas_w), dt_inicio_dialise_w, qt_hora_sessao_w, nm_usuario_p);
            end if;
        end if;
	end if;
	...
	if (nr_seq_protocolo3_w IS NOT NULL AND nr_seq_protocolo3_w::text <> '') then
        ...
        if (ie_tipo_solucao3_w = 'C' or ie_categoria_w = 'C') then
            if (qt_solucao_total3_w IS NOT NULL AND qt_solucao_total3_w::text <> '') then
                qt_vol_total_w 	:= null;
                nr_etapas_w		:= null;
                if (qt_dose_soluc1_3_w IS NOT NULL AND qt_dose_soluc1_3_w::text <> '') then
                    qt_vol_total_w := obter_conversao_ml(cd_mat_soluc1_3_w, qt_dose_soluc1_3_w, cd_unid_med_dose_sol1_3_w);
                end if;
                if (qt_dose_soluc2_3_w IS NOT NULL AND qt_dose_soluc2_3_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc2_3_w, qt_dose_soluc2_3_w, cd_unid_med_dose_sol2_3_w);
                end if;
                if (qt_dose_soluc3_3_w IS NOT NULL AND qt_dose_soluc3_3_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc3_3_w, qt_dose_soluc3_3_w, cd_unid_med_dose_sol3_3_w);
                end if;
                if (qt_dose_soluc4_3_w IS NOT NULL AND qt_dose_soluc4_3_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc4_3_w, qt_dose_soluc4_3_w, cd_unid_med_dose_sol4_3_w);
                end if;
                if (qt_dose_soluc5_3_w IS NOT NULL AND qt_dose_soluc5_3_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc5_3_w, qt_dose_soluc5_3_w, cd_unid_med_dose_sol5_3_w);
                end if;
                if (qt_dose_soluc6_3_w IS NOT NULL AND qt_dose_soluc6_3_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc6_3_w, qt_dose_soluc6_3_w, cd_unid_med_dose_sol6_3_w);
                end if;
                if (qt_dose_soluc7_3_w IS NOT NULL AND qt_dose_soluc7_3_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc7_3_w, qt_dose_soluc7_3_w, cd_unid_med_dose_sol7_3_w);
                end if;
                nr_etapas_w := qt_solucao_total3_w / qt_vol_total_w;
                CALL ajustar_inf_dialise_js(nr_prescricao_p, nr_seq_solucao_w, ceil(nr_etapas_w), dt_inicio_dialise_w, qt_hora_sessao_w, nm_usuario_p);
            end if;
        end if;
	end if;
	...
	if (nr_seq_protocolo4_w IS NOT NULL AND nr_seq_protocolo4_w::text <> '') then
        ...
        if (ie_tipo_solucao4_w = 'C' or ie_categoria_w = 'C') then
            if (qt_solucao_total4_w IS NOT NULL AND qt_solucao_total4_w::text <> '') then
                qt_vol_total_w 	:= null;
                nr_etapas_w		:= null;
                if (qt_dose_soluc1_4_w IS NOT NULL AND qt_dose_soluc1_4_w::text <> '') then
                    qt_vol_total_w := obter_conversao_ml(cd_mat_soluc1_4_w, qt_dose_soluc1_4_w, cd_unid_med_dose_sol1_4_w);
                end if;
                if (qt_dose_soluc2_4_w IS NOT NULL AND qt_dose_soluc2_4_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc2_4_w, qt_dose_soluc2_4_w, cd_unid_med_dose_sol2_4_w);
                end if;
                if (qt_dose_soluc3_4_w IS NOT NULL AND qt_dose_soluc3_4_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc3_4_w, qt_dose_soluc3_4_w, cd_unid_med_dose_sol3_4_w);
                end if;
                if (qt_dose_soluc4_4_w IS NOT NULL AND qt_dose_soluc4_4_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc4_4_w, qt_dose_soluc4_4_w, cd_unid_med_dose_sol4_4_w);
                end if;
                if (qt_dose_soluc5_4_w IS NOT NULL AND qt_dose_soluc5_4_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc5_4_w, qt_dose_soluc5_4_w, cd_unid_med_dose_sol5_4_w);
                end if;
                if (qt_dose_soluc6_4_w IS NOT NULL AND qt_dose_soluc6_4_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc6_4_w, qt_dose_soluc6_4_w, cd_unid_med_dose_sol6_4_w);
                end if;
                if (qt_dose_soluc7_4_w IS NOT NULL AND qt_dose_soluc7_4_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc7_4_w, qt_dose_soluc7_4_w, cd_unid_med_dose_sol7_4_w);
                end if;
                nr_etapas_w := qt_solucao_total4_w / qt_vol_total_w;
                CALL ajustar_inf_dialise_js(nr_prescricao_p, nr_seq_solucao_w, ceil(nr_etapas_w), dt_inicio_dialise_w, qt_hora_sessao_w, nm_usuario_p);
            end if;
        end if;
	end if;	
	...
	if (nr_seq_protocolo5_w IS NOT NULL AND nr_seq_protocolo5_w::text <> '') then
        ...
        if (ie_tipo_solucao5_w = 'C' or ie_categoria_w = 'C') then
            if (qt_solucao_total5_w IS NOT NULL AND qt_solucao_total5_w::text <> '') then
                qt_vol_total_w 	:= null;
                nr_etapas_w		:= null;
                if (qt_dose_soluc1_5_w IS NOT NULL AND qt_dose_soluc1_5_w::text <> '') then
                    qt_vol_total_w := obter_conversao_ml(cd_mat_soluc1_5_w, qt_dose_soluc1_5_w, cd_unid_med_dose_sol1_5_w);
                end if;
                if (qt_dose_soluc2_5_w IS NOT NULL AND qt_dose_soluc2_5_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc2_5_w, qt_dose_soluc2_5_w, cd_unid_med_dose_sol2_5_w);
                end if;
                if (qt_dose_soluc3_5_w IS NOT NULL AND qt_dose_soluc3_5_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc3_5_w, qt_dose_soluc3_5_w, cd_unid_med_dose_sol3_5_w);
                end if;
                if (qt_dose_soluc4_5_w IS NOT NULL AND qt_dose_soluc4_5_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc4_5_w, qt_dose_soluc4_5_w, cd_unid_med_dose_sol4_5_w);
                end if;
                if (qt_dose_soluc5_5_w IS NOT NULL AND qt_dose_soluc5_5_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc5_5_w, qt_dose_soluc5_5_w, cd_unid_med_dose_sol5_5_w);
                end if;
                if (qt_dose_soluc6_5_w IS NOT NULL AND qt_dose_soluc6_5_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc6_5_w, qt_dose_soluc6_5_w, cd_unid_med_dose_sol6_5_w);
                end if;
                if (qt_dose_soluc7_5_w IS NOT NULL AND qt_dose_soluc7_5_w::text <> '') then
                    qt_vol_total_w := qt_vol_total_w + obter_conversao_ml(cd_mat_soluc7_5_w, qt_dose_soluc7_5_w, cd_unid_med_dose_sol7_5_w);
                end if;
                nr_etapas_w := qt_solucao_total5_w / qt_vol_total_w;
                CALL ajustar_inf_dialise_js(nr_prescricao_p, nr_seq_solucao_w, ceil(nr_etapas_w), dt_inicio_dialise_w, qt_hora_sessao_w, nm_usuario_p);
            end if;
        end if;
	end if;
...
exception when others then
	CALL gravar_log_cpoe(substr('CPOE_INSERIR_DIALISE_REG EXCEPTION:'||substr(to_char(sqlerrm),1,2000)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE cpoe_inserir_dialise_reg (nr_atendimento_p atendimento_paciente.nr_atendimento%type, cd_pessoa_fisica_p pessoa_fisica.cd_pessoa_fisica%type, nr_prescricao_p prescr_medica.nr_prescricao%type, nr_sequencia_p cpoe_dialise.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, cd_perfil_p perfil.cd_perfil%type, nm_usuario_p usuario.nm_usuario%type, dt_inicio_prescr_p timestamp, dt_inicio_ret_p INOUT timestamp) FROM PUBLIC;
