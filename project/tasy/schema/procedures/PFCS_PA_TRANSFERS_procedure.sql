...
CREATE OR REPLACE PROCEDURE pfcs_pa_transfers ( cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE


nr_seq_indicator_w              pfcs_panel.nr_seq_indicator%type := 120;
pfcs_panel_detail_seq_w      	pfcs_panel_detail.nr_sequencia%type;
pfcs_panel_seq_w          		pfcs_panel.nr_sequencia%type;
qt_transfers_w			numeric(20) :=0;
...
where pat.nr_sequencia = enc.nr_seq_patient
    ...
    and upper(enc.cd_admit_source) like upper('hosp-trans')
...
for c01_w in c01 loop
  ...
  qt_transfers_w := qt_transfers_w + 1;
    ...
    values (
	 ...
	 := pfcs_pck.pfcs_generate_results(
    	vl_indicator_p => qt_transfers_w, nr_seq_indicator_p => nr_seq_indicator_w, nr_seq_operational_level_p => cd_estabelecimento_p, nm_usuario_p => nm_usuario_p, nr_seq_panel_p => pfcs_panel_seq_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pfcs_pa_transfers ( cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
