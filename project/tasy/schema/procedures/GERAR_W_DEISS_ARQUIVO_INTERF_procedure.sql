...
CREATE OR REPLACE PROCEDURE gerar_w_deiss_arquivo_interf (dt_inicio_p timestamp, dt_fim_p timestamp, cd_estabelecimento_p text, cd_interface_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
SELECT	n.nr_sequencia, 
	...
	CASE WHEN obter_se_nota_entrada_saida(n.nr_sequencia)='S' THEN  n.vl_mercadoria  ELSE n.vl_mercadoria END , 
...
and	((obter_se_nota_entrada_saida(n.nr_sequencia) = 'S') or (obter_se_nota_entrada_saida(n.nr_sequencia) = 'E')) 
...
select	'3', 
	'h', 
	substr(somente_numero(e.cd_cgc),1,14), -- cnpj do declarante 
	dt_anor_ref_w, 
	dt_mes_ref_w, 
	0, 
	(select	coalesce(substr(max(f.cd_acesso_deiss),1,14),'') 
	from	nfe_senha_conexao f 
	where	f.cd_estabelecimento = cd_estabelecimento_p), 
	obter_dados_pf(s.cd_contabilista,'CPF'), 
	coalesce(substr(Elimina_Caracteres_Especiais(e.cd_inscricao_municipal),1,15),'') 
...
ds_texto_w := 	('''3'''||','||'''H'''||','||''''||cd_cnpj_declarante_w||''''||','||nr_ano_referencia_w||','|| 
		nr_mes_referencia_w||','||0||',' || cd_cnpj_declarante_w ||','||''''||cd_cnpj_cpf_w||''''||','|| cd_contabilidade_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_w_deiss_arquivo_interf (dt_inicio_p timestamp, dt_fim_p timestamp, cd_estabelecimento_p text, cd_interface_p bigint, nm_usuario_p text) FROM PUBLIC;
