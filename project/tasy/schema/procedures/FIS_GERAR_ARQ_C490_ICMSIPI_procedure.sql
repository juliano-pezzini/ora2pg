...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_c490_icmsipi ( nr_seq_controle_p bigint, nr_ecf_cx_p bigint, dt_doc_p timestamp, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
/*Criação do array com o tipo sendo do cursor eespecificado - c_reg_C490*/

type reg_c_reg_C490 is table of fis_efd_icmsipi_C490%RowType;
...
fetch c_reg_C490 bulk collect into vetRegC490 limit 1000;
	...
	ds_linha_w := substr(ds_sep_w || vetRegC490[i].cd_reg ||
			     ds_sep_w || vetRegC490[i].cd_cst_icms ||
			     ds_sep_w || vetRegC490[i].cd_cfop ||
			     ds_sep_w || substr(sped_obter_campo_valor(vetRegC490[i].tx_aliq_icms ), 1, 30) ||
			     ds_sep_w || substr(sped_obter_campo_valor(vetRegC490[i].vl_opr ), 1, 30) ||
			     ds_sep_w || substr(sped_obter_campo_valor(vetRegC490[i].vl_bc_icms ), 1, 30) ||
			     ds_sep_w || substr(sped_obter_campo_valor(vetRegC490[i].vl_icms ), 1, 30) ||
			     ds_sep_w || vetRegC490[i].cod_obs || ds_sep_w,
			     1,
			     8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro		:=  vetRegC490[i].cd_reg;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_c490_icmsipi ( nr_seq_controle_p bigint, nr_ecf_cx_p bigint, dt_doc_p timestamp, nr_linha_p INOUT bigint ) FROM PUBLIC;
