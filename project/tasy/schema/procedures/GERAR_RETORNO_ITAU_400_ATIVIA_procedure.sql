...
CREATE OR REPLACE PROCEDURE gerar_retorno_itau_400_ativia ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
dt_credito_liquid_w		varchar(6);
ie_baixa_titulo_w 		varchar(1) := 'S';
cd_carteira_banco_w	banco_carteira.cd_carteira%type;
nr_seq_carteira_w		banco_carteira.nr_sequencia%type;
nosso_num_w		varchar(8);
qt_carteira_w		smallint;

c01 CURSOR FOR 
SELECT 	trim(both substr(ds_string,38,25)), --Identificação do título na empresa, posição 038 até 062 
	...
	coalesce(somente_numero(substr(ds_string,111,6)),0), --  nvl(somente_numero(substr(ds_string,296,6)),0), 
	substr(ds_string,109,2), 
	substr(ds_string,296,6), -- Data de crédito desta liquidação 
	substr(ds_string,83,3),--Número da carteira 
	substr(ds_string,86,8) --Nosso número 
...
if (coalesce(trim(both dt_credito_liquid_w ),0) = 0) then 
	ie_baixa_titulo_w := 'N';
else 
	ie_baixa_titulo_w := 'S';
end if;
 ...
 if (nr_titulo_w IS NOT NULL AND nr_titulo_w::text <> '') then 
	  ...
	  if (vl_titulo_w = coalesce(vl_liquido_w,0)) then 
 
	   vl_liquido_w := coalesce(vl_liquido_w,0) - coalesce(vl_juros_w,0) + coalesce(vl_tarifa_cobranca_w,0);
	...
	;
	 ...
	 insert into titulo_receber_cobr(nr_sequencia, 
				...
				values (nr_seq_tit_rec_cobr_w, 
					...
					CASE WHEN  ie_baixa_titulo_w='S' THEN coalesce(dt_liquidacao_w,clock_timestamp())  ELSE null END , 
					---- No caso de ie_baixa_titulo_w 'N', a data de liquidação deve ser nula, pois os títulos enviados sem a Data de crédito desta liquidação (a partir da posição 296), devem apenas ser atualizados (Nosso Numero e Carteira) para impressão da 2ª via do boleto. 
					clock_timestamp(), 
					nm_usuario_p, 
	...
	if ( ie_baixa_titulo_w = 'N') then --Os títulos que não são liquidados devem ter seus dados atualizados no Tasy 
 
		select 	count(*) 
		into STRICT	qt_carteira_w 
		from 	banco_carteira 
		where 	cd_carteira = cd_carteira_banco_w;
		...
		if ( qt_carteira_w = 1) then 
			...
			update 	titulo_receber 
			set	nr_nosso_numero = nosso_num_w, 
				nr_seq_carteira_cobr = nr_seq_carteira_w 
			where 	nr_titulo = nr_titulo_w;
			 
			CALL Gerar_Bloqueto_Tit_Rec(nr_titulo_w,'MTR');
		else 
			CALL fin_gerar_log_controle_banco(3,substr('Não foi atualizado o título ' || ds_titulo_w || 
								', pois a carteira de código '|| cd_carteira_w || 
								' não está cadastrada no Tasy (tabela BANCO_CARTEIRA)',1,4000),nm_usuario_p,'N');
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_retorno_itau_400_ativia ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
