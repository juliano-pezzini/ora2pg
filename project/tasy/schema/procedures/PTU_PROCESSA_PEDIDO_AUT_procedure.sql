...
if ( ie_possui_pedido_p = 'N' ) then
	...
	CALL ptu_gerar_guia_requisicao( nr_seq_pedido_aut_p, nm_usuario_p, cd_estabelecimento_w, null);
...
CALL ptu_atualizar_guia_req( nr_seq_pedido_aut_p, ie_possui_pedido_p, nm_usuario_p, cd_estabelecimento_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_processa_pedido_aut ( nr_seq_pedido_aut_p ptu_pedido_autorizacao.nr_sequencia%type, ie_possui_pedido_p text, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nr_seq_resp_pedido_aut_p INOUT ptu_resposta_autorizacao.nr_sequencia%type) FROM PUBLIC;
