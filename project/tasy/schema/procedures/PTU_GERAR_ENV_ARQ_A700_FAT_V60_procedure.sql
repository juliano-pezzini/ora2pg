...
CREATE OR REPLACE PROCEDURE ptu_gerar_env_arq_a700_fat_v60 ( nr_seq_fatura_p bigint, cd_estabelecimento_p text, nm_usuario_p text) AS $body$
DECLARE
...
C04 CURSOR FOR 
	SELECT	lpad(coalesce(elimina_caractere_especial(a.nr_lote),'0'),8,'0'), 
		...
		rpad(coalesce(substr(a.cd_ref_material_fab,1,30),' '),30,' '), 
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		EXIT WHEN NOT FOUND; /* apply on C02 */
			...
			ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '702' || nr_lote_w || lpad(' ',12,' ') || cd_unimed_destino_w || '  ' || 
					cd_usuario_plano_w || nm_beneficiario_w || dt_atendimento_w || ie_tipo_saida_spdat_w || ie_carater_atendimento_w || ' ' || 
					cd_cid_w || lpad(' ',14,' ') || ie_paciente_w || ' ' || ie_tipo_atendimento_w || lpad(' ',11,' ') || nr_nota_w || 
			...
			EXIT WHEN NOT FOUND; /* apply on C04 */
				...
				ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '704' || nr_lote_704_w || lpad(' ',11,' ') || 
						...
						ie_tipo_tabela_w || cd_procedimento_w || qt_procedimento_ww || vl_procedimento_w || vl_custo_operacional_w || 
						...
						cd_ref_material_fab_w || dt_pgto_prestador_w || nr_cnpj_fornecedor_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_env_arq_a700_fat_v60 ( nr_seq_fatura_p bigint, cd_estabelecimento_p text, nm_usuario_p text) FROM PUBLIC;
