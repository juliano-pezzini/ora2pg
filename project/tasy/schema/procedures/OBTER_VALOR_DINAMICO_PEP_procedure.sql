...
CREATE OR REPLACE PROCEDURE obter_valor_dinamico_pep ( ds_comando_p text, ds_parametros_p text, ds_Retorno_P INOUT bigint) AS $body$
DECLARE
...
BEGIN
	...
	if (upper(substr(ds_comando_p,1,6)) = 'SELECT') then
		...
		for contador_w in 1..ar_parametros_w.count loop
			if ( upper(substr(ar_parametros_w[contador_w].nm,1,2)) = 'DT') then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE obter_valor_dinamico_pep ( ds_comando_p text, ds_parametros_p text, ds_Retorno_P INOUT bigint) FROM PUBLIC;
