...
CREATE OR REPLACE PROCEDURE dmed_anual_arquivo_top ( nr_sequencia_p bigint, nm_usuario_p text, nr_linha_p INOUT bigint) AS $body$
DECLARE
...
c_titular CURSOR FOR
	...
	from (	SELECT	p.cd_pessoa_fisica,
		...
		where	exists (select	1
					...
					and	l.nr_seq_dmed_anual	= nr_sequencia_p)) alias3
	group by
		nr_cpf_titular,
...
fetch c_titular bulk collect into s_array limit 1000;
	vetor_titular_w(i)	:= s_array;
	i			:= i + 1;
EXIT WHEN NOT FOUND; /* apply on c_titular */
end loop;
close c_titular;

for i in 1..vetor_titular_w.count loop
	begin
	s_array := vetor_titular_w(i);
	for z in 1..s_array.count loop
		...
		if (nr_cpf_lido_w != coalesce(nr_cpf_titular_w,'0')) or (coalesce(nr_cpf_lido_w::text, '') = '') then
			...
			if (vl_pago_w > 0) then
				...
				ds_arquivo_w 	:= 'TOP' || '|' || nr_cpf_titular_w || '|' || nm_titular_w || '|' || vl_pago_w || '|';
			else
				--quando s?dependente tem mensalidade o valor do titular fica vazio no arquivo
				ds_arquivo_w 	:= 'TOP' || '|' || nr_cpf_titular_w || '|' || nm_titular_w || '|' || '' || '|';
			end if;
			...
			from	pls_segurado	a
			where	a.cd_pessoa_fisica	= cd_pessoa_titular_w;
			...
			nr_linha_w := dmed_anual_arquivo_rtop(nr_sequencia_p, cd_pessoa_titular_w, nm_usuario_p, nr_cpf_titular_w, nr_linha_w);
			...
			and	l.nr_seq_dmed_anual	= nr_sequencia_p
			and	t.ie_prestadora_ops	= 'O'  LIMIT 1;

			if (qt_registros_w > 0) then
				nr_linha_w := dmed_anual_arquivo_dtop(nr_sequencia_p, cd_pessoa_titular_w, nm_usuario_p, nr_cpf_titular_w, nr_linha_w);
			end if;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dmed_anual_arquivo_top ( nr_sequencia_p bigint, nm_usuario_p text, nr_linha_p INOUT bigint) FROM PUBLIC;
