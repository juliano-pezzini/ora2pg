...
CREATE OR REPLACE PROCEDURE gerar_w_nota_fiscal_dps ( dt_inicio_p timestamp, dt_final_p timestamp, nm_usuario_p text, ie_setor_usuario_p text, nr_nota_fiscal_p bigint, cd_estabelecimento_p bigint, ds_filtro_p text, ie_entrada_saida_P text, ie_usuario_filtro_p text default 'N') AS $body$
DECLARE
...
ds_comando_w 	:= ' select distinct a.nr_sequencia, a.nm_usuario from nota_fiscal a, titulo_pagar t ' || 
		   ' where 1 = 1 ' ||		
		   ' and a.nr_sequencia = t.nr_seq_nota_fiscal' ||
		   ' and t.dt_liquidacao between :dt_inicio and fim_dia(:dt_final)' || 	
		   ' and t.ie_tipo_titulo <> 4' || 	
		   --' and nvl(a.ie_status_envio, :ie_status_envio) = :ie_status_envio ' ||
		   ' and a.cd_estabelecimento = :cd_estabelecimento ' ||
		   ' and rownum <= :qt_notas_rps ' || ' ' || ds_filtro_p;

if (coalesce(nr_nota_fiscal_p,0) > 0) then
	ds_comando_w	:= ds_comando_w || ' and a.nr_nota_fiscal = :nr_nota_fiscal ';
end if;

if (ie_nota_cancelada_w = 'N') then
	ds_comando_w	:= ds_comando_w || ' and a.ie_situacao <> :ie_situacao ';
end if;

if (ie_nota_estornada_w = 'N') then
	ds_comando_w	:= ds_comando_w || ' and a.ie_situacao <> :ie_situacao_um ';	
	ds_comando_w	:= ds_comando_w || ' and a.ie_situacao <> :ie_situacao_dois ';		
elsif (ie_tratamento_estorno_w = 'S') then
	ds_comando_w	:= ds_comando_w || ' and obter_nf_estorno_periodo(a.nr_sequencia, :dt_inicio_est, fim_dia(:dt_final_est)) = :ie_opcao ';	
elsif (ie_nota_estornada_w = 'S') and (ie_tratamento_estorno_w = 'N') then
	ds_comando_w	:= ds_comando_w || ' and a.ie_situacao <> :ie_situacao_um ';	
end if;

if (ie_somente_nota_calculada_w = 'S') then
	ds_comando_w	:= ds_comando_w || ' and a.dt_atualizacao_estoque is not null ';
end if;		

if (ie_entrada_saida_w <> 'A') then
	ds_comando_w	:= ds_comando_w || ' and (substr(obter_se_nota_entrada_saida(a.nr_sequencia),1,1) = :ie_entrada_saida) ';
end if;		
...
while(dbms_sql.fetch_rows(c001) > 0 ) loop
	...
	if (ie_setor_usuario_p = 'S') then
		...
		;
		/*Se o usuario da nota, ou usuario da exportao nao tiver o setor, nao exporta. Nesse caso ie_setor_usuario_p deve ser N*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_w_nota_fiscal_dps ( dt_inicio_p timestamp, dt_final_p timestamp, nm_usuario_p text, ie_setor_usuario_p text, nr_nota_fiscal_p bigint, cd_estabelecimento_p bigint, ds_filtro_p text, ie_entrada_saida_P text, ie_usuario_filtro_p text default 'N') FROM PUBLIC;
