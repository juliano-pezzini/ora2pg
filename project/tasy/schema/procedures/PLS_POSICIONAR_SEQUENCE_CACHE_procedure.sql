...
CREATE OR REPLACE PROCEDURE pls_posicionar_sequence_cache ( nm_tabela_p text, nm_atributo_p text, ie_limite_p text, vl_cache_p text) AS $body$
DECLARE
...
if (ie_limite_p = 'S') and
	...
	CALL wheb_mensagem_pck.exibir_mensagem_abort(267143,
...
if (ie_limite_p in ('R','N')) then
	...
	ds_comando_w	:= ds_comando_w || ' Cache ' || vl_cache_p;
	CALL gravar_processo_longo(ds_comando_w,'TASY_POSICIONAR_SEQUENCE',null);
	CALL Exec_Sql_Dinamico_bv(nm_sequence_w, ds_comando_w,'');
/*else
	WHILE 	(qt_seq_w <= qt_atual_w) LOOP
		ds_comando_w	:= 'Select '  || nm_sequence_w || '_seq.nextval from dual';
		Obter_valor_Dinamico(ds_comando_w, qt_seq_w);
	END LOOP;*/
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_posicionar_sequence_cache ( nm_tabela_p text, nm_atributo_p text, ie_limite_p text, vl_cache_p text) FROM PUBLIC;
