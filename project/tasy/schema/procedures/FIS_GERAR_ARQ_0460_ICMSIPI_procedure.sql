...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_0460_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
c_reg_0460 CURSOR FOR
SELECT 	*
from 	fis_efd_icmsipi_0460
where 	nr_seq_controle = nr_seq_controle_p;

/*Criação do array com o tipo sendo do cursor eespecificado - c_reg_0460*/

type reg_c_reg_0460 is table of fis_efd_icmsipi_0460%RowType;
vetReg0460 reg_c_reg_0460;
...
open c_reg_0460;
loop
fetch c_reg_0460 bulk collect into vetReg0460 limit 1000;
	for i in 1 .. vetReg0460.Count loop
	begin
        ...
        ds_linha_w := substr(ds_sep_w || vetReg0460[i].cd_reg ||
                             ds_sep_w || vetReg0460[i].cd_obs ||
                             ds_sep_w || vetReg0460[i].ds_txt || ds_sep_w,
                             1,
                             8000);
	...
	fis_registros_w[qt_cursor_w].nr_sequencia		:= nr_sequencia_w;
        ...
        fis_registros_w[qt_cursor_w].cd_registro 		:=  vetReg0460[i].cd_reg;

        if (nr_vetor_w >= 1000) then
	begin
	/*Inserindo registros definitivamente na tabela especifica - FIS_EFD_ICMSIPI_0460 */
...
EXIT WHEN NOT FOUND; /* apply on c_reg_0460 */
end loop;
close c_reg_0460;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_0460_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) FROM PUBLIC;
