...
CREATE OR REPLACE PROCEDURE ptu_imp_intercamb_fundacao_sc ( ds_conteudo_p text, nm_arquivo_p text, ie_tipo_contrato_p text, nm_usuario_p text) AS $body$
DECLARE
...
if (cd_tipo_registro_w	= '101') then
	...
	select	CASE WHEN (substr(ds_conteudo_p,76,1))::numeric =0 THEN ''  ELSE (substr(ds_conteudo_p,76,1))::numeric  END ,
		...
		CASE WHEN (substr(ds_conteudo_p,113,1))::numeric =0 THEN ''  ELSE (substr(ds_conteudo_p,113,1))::numeric  END
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_imp_intercamb_fundacao_sc ( ds_conteudo_p text, nm_arquivo_p text, ie_tipo_contrato_p text, nm_usuario_p text) FROM PUBLIC;
