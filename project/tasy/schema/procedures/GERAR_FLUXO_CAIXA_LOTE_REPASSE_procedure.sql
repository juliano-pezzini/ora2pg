...
and	PKG_DATE_UTILS.start_of(a.dt_item, 'dd',0) between dt_inicial_w and dt_final_p 
...
dt_inicial_w	:= PKG_DATE_UTILS.ADD_MONTH(dt_inicial_p, qt_mes_anterior_p * -1,0);
dt_final_w	:= PKG_DATE_UTILS.ADD_MONTH(dt_final_p, qt_mes_anterior_p * -1,0);
...
if (ie_regra_data_p = 'F') then 
	...
	while(dt_mes_w <= PKG_DATE_UTILS.start_of(dt_final_p, 'month',0)) loop 
		...
		if (nr_dia_fixo_p > (PKG_DATE_UTILS.extract_field('DAY',pkg_date_utils.end_of(dt_mes_w, 'MONTH', 0)))::numeric ) then 
		...
		if (dt_referencia_w between dt_inicial_p and dt_final_p) then 
			...
			where	b.cd_estabelecimento	= cd_estabelecimento_p 
			and	a.nr_seq_terceiro	= b.nr_sequencia 
			and	PKG_DATE_UTILS.start_of(a.dt_item, 'dd',0) between dt_inicial_w and dt_final_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_fluxo_caixa_lote_repasse ( nr_seq_lote_fluxo_p bigint, nr_seq_regra_p bigint, cd_estabelecimento_p bigint, cd_conta_financ_p bigint, ie_regra_data_p text, nr_dia_fixo_p bigint, qt_mes_anterior_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, nm_usuario_p text, cd_empresa_p bigint) FROM PUBLIC;
