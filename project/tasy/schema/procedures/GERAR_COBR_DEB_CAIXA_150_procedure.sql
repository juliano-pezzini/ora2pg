...
CREATE OR REPLACE PROCEDURE gerar_cobr_deb_caixa_150 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	SELECT	lpad(coalesce(coalesce(pls_obter_dados_pagador_fin(e.nr_seq_pagador, 'A'), b.cd_agencia_bancaria), '0'), 4, '0') cd_agencia_bancaria,
		lpad(coalesce(coalesce(pls_obter_dados_pagador_fin(e.nr_seq_pagador, 'C'), b.nr_conta),'0'), 8, '0') nr_conta,
		...
		e.nr_seq_pagador,
		coalesce(CASE WHEN coalesce(d.cd_pessoa_fisica::text, '') = '' THEN rpad(' ',3,' ')  ELSE '001' END , '000') cd_operacao,
		d.cd_pessoa_fisica,
		d.nr_seq_mensalidade,
		d.nr_seq_mens_segurado
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	vl_soma_w	:= coalesce(vl_soma_w,0) + coalesce(vl_cobranca_w,0);
...
vl_total_w		:= lpad(somente_numero(to_char(vl_soma_w,'999999999999990.00')),17,'0');

qt_reg_arquivo_w	:= qt_reg_arquivo_w + 1;

ds_conteudo_w	:=	'Z' || lpad(qt_reg_arquivo_w, 6, '0') || vl_total_w || lpad('0', 17, '0') || ds_brancos_109_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_cobr_deb_caixa_150 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
