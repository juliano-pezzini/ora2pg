...
CREATE OR REPLACE PROCEDURE pls_aplicar_recalculo_autoger ( nr_seq_lote_p pls_lote_recalculo_autoger.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
vl_lib_anterior_w		pls_conta_proc.vl_liberado%type;
...
ie_situacao_ocorr_w		pls_ocorrencia_benef.ie_situacao%type;
...
c_contas CURSOR(nr_seq_lote_pc	pls_lote_recalculo_autoger.nr_sequencia%type) FOR
	SELECT	distinct nr_seq_conta,
		...
		from 	pls_conta 
		where 	nr_sequencia  = a.nr_seq_conta) nr_seq_protocolo
...
for r_itens_w in c_itens_recalculo(nr_seq_lote_p) loop
	...
	--Obtém informações do protocolo na qual o procedimento esta contido
	select	nr_sequencia,
		nr_lote_contabil,
		nr_lote_contab_pag,
	...
	if (nr_lote_contabil_w IS NOT NULL AND nr_lote_contabil_w::text <> '') then
		CALL wheb_mensagem_pck.exibir_mensagem_abort(1092375, 'NR_LOTE_CONTABIL_W=' || nr_lote_contabil_w);
	end if;
...
union all
	...
	--Verifica se glosa ativa, com glosa vinculada e que interfiram no pagamento. Retorna a NR_SEQ_MOTIVO_GLOSA pois

	--utilizará caso tiver que inserir a ocorrência
	select	max(nr_seq_motivo_glosa)
	into STRICT	nr_seq_motivo_glosa_w
	from	pls_ocorrencia a
	where	a.nr_sequencia = r_itens_w.nr_seq_ocorrencia
	and	a.ie_situacao = 'A'
	and	(a.nr_seq_motivo_glosa IS NOT NULL AND a.nr_seq_motivo_glosa::text <> '')
	and	a.ie_glosar_pagamento = 'S';
	
	--nr_seq_motivo_glosa_w será nulo caso não existir mais a ocorrência vinculada ao item da regra, ou a mesma estiver 

	--inativa ou ainda não for uma ocorrência que interfere pagamento( também tem que estar com glosa vinculada).

	--Somente irá aplicar o deflator se tiver uma ocorrência nessas condições.
	if (nr_seq_motivo_glosa_w IS NOT NULL AND nr_seq_motivo_glosa_w::text <> '') then

		--Processa os ajustes de valor nos itens.
		for r_procedimentos_w in c_procedimentos( r_itens_w.nr_seq_conta_proc) loop

			select	max(a.ie_situacao)
			into STRICT	ie_situacao_ocorr_w
			from	pls_ocorrencia_benef a
			where	nr_seq_proc = r_itens_w.nr_seq_conta_proc
			and	nr_seq_ocorrencia = r_itens_w.nr_seq_ocorrencia;

			--Se ie_situacao_ocorr_w for nulo é por que não tem na ocorrência vinculada ao procedimento ainda
			if (coalesce(ie_situacao_ocorr_w::text, '') = '') then

				nr_seq_ocorrencia_benef_w := pls_inserir_ocorrencia(	null, r_itens_w.nr_seq_ocorrencia, null, null, null, r_itens_w.nr_seq_conta_proc, null, null, nm_usuario_p, 'Ocorrência gerada pelo recálculo de autogerado', nr_seq_motivo_glosa_w, 3, cd_estabelecimento_w, 'S', null, nr_seq_ocorrencia_benef_w, null, null, null, null);

			--Caso tiver a ocorrência já vinculada ao procedimento e estiver inativa, entõa apenas ativa a mesma.
			elsif (ie_situacao_ocorr_w = 'I') then
			
				update 	pls_ocorrencia_benef
				set 	ie_situacao = 'A' 
				where 	nr_seq_ocorrencia = r_itens_w.nr_seq_ocorrencia
				and 	nr_seq_proc = r_itens_w.nr_seq_conta_proc;
			end if;
			
			vl_lib_anterior_w	:= r_procedimentos_w.vl_liberado; --Guarda o valor liberado anterior para utilizar posteriormente no log de alteração
			--Atualiza valores proporcionalmente ao valor de recalculo(Vl unitário será alterado automaticamente).
			vl_liberado_hi_w	:= r_itens_w.vl_item * dividir_sem_round(r_procedimentos_w.vl_calc_hi_util, r_procedimentos_w.vl_procedimento);
			vl_liberado_co_w	:= r_itens_w.vl_item * dividir_sem_round(r_procedimentos_w.vl_calc_co_util, r_procedimentos_w.vl_procedimento);
			vl_liberado_material_w	:= r_itens_w.vl_item * dividir_sem_round(r_procedimentos_w.vl_calc_mat_util,r_procedimentos_w.vl_procedimento);
			
			update	pls_conta_proc
			set	vl_liberado		= r_itens_w.vl_item,
				vl_unitario		= dividir(r_itens_w.vl_item, r_itens_w.qt_item),
				vl_pag_medico_conta	= 0,
				vl_total_partic		= 0,
				vl_prestador		= r_itens_w.vl_item,
				vl_liberado_hi		= vl_liberado_hi_w,
				vl_liberado_co		= vl_liberado_co_w,
				vl_liberado_material	= vl_liberado_material_w,
				vl_glosa		= pls_util_cta_pck.pls_obter_vl_glosa(r_itens_w.vl_item, vl_procedimento_imp)
			where	nr_sequencia		= r_itens_w.nr_seq_conta_proc;

			update	pls_proc_participante
			set	vl_participante		= dividir((r_itens_w.vl_item * vl_calculado),r_procedimentos_w.vl_procedimento),
				qt_liberada		= r_procedimentos_w.qt_procedimento_imp,
				vl_glosa		= 0,
			...
			if (vl_tot_partic_w > r_itens_w.vl_item) then
				
				 vl_dif_partic_w	:= vl_tot_partic_w - r_itens_w.vl_item;
...
for r_contas_w in c_contas(nr_seq_lote_p) loop
	...
	from	pls_conta
	where	nr_seq_protocolo = r_contas_w.nr_seq_protocolo
	and	ie_status	!= 'F';
	...
	if (qt_conta_w = 0) then
		...
		CALL pls_desfazer_resumo_conta_prot(r_contas_w.nr_seq_protocolo, nm_usuario_p);
		CALL pls_gerar_resumo_conta_prot(r_contas_w.nr_seq_protocolo, nm_usuario_p, cd_estabelecimento_w );
	end if;
end loop;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_aplicar_recalculo_autoger ( nr_seq_lote_p pls_lote_recalculo_autoger.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
