...
if (nr_atendimento_p IS NOT NULL AND nr_atendimento_p::text <> '') and (nm_usuario_p IS NOT NULL AND nm_usuario_p::text <> '') then
	begin
		...
		if (ie_consiste_evasao_w = 'N') then
			...
			SELECT * FROM consistir_desfecho_pa(nr_atendimento_p, nm_usuario_p, ds_erro_w, ie_item_posicionar_w, ds_erro2_w, ie_desfecho_p) INTO STRICT ds_erro_w, ie_item_posicionar_w, ds_erro2_w;
			...
			if (coalesce(ds_erro_diagnostico_w::text, '') = '') then
				begin
				...
				CALL gerar_lancamento_automatico(nr_atendimento_p, null, 251, nm_usuario_p, null, null, ds_adicional_2_w, null, null, null);
				end;
			end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE realizar_desfecho_pa_teste ( nr_atendimento_p bigint, ie_desfecho_p text, ie_exige_diagnostico_p text, ie_tipo_atend_internacao_p bigint, cd_motivo_alta_p bigint, cd_setor_transf_novo_atend_p bigint, cd_setor_transf_p bigint, ie_desdobrar_conta_p text, ie_atualizar_atendimento_pac_p text, cd_medico_destino_p text, ds_erro_diagnostico_p INOUT text, ds_erro_alta_p INOUT text, ds_compl_erro_p INOUT text, ie_lib_diag_medico_p INOUT text, nr_seq_motivo_p bigint, cd_cgc_p text, nm_usuario_p text, cd_estabelecimento_p bigint, ds_erro_consist_desfecho_p INOUT text, ds_erro2_p INOUT text, ie_item_posicionar_p INOUT text, nr_atend_gerado_p INOUT bigint) FROM PUBLIC;
