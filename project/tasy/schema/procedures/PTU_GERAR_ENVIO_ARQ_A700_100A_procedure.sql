...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_arq_a700_100a (nr_seq_servico_p bigint, cd_estabelecimento_p text, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	SELECT	lpad(coalesce(elimina_caractere_especial(to_char(nr_lote)),'0'),8,'0'),
		...
		rpad(coalesce(elimina_caractere_especial(elimina_acentuacao(nm_prest_exec)),' '),70,' '),
...
EXIT WHEN NOT FOUND; /* apply on C00 */
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		ds_conteudo_w	:=	lpad(to_char(nr_seq_registro_w),8,'0') || '702' || nr_lote_w || lpad(' ',12,' ') || cd_unimed_destino_w || '   ' ||
					...
					cd_cnes_cont_exec_w || cd_munic_cont_exec_w || nm_prest_req_w || nm_prest_exec_w || tp_prest_exec_w || id_rec_proprio_w || cd_cid_obito_cobr_w;
		ds_arquivo_w	:=	ds_arquivo_w || ds_conteudo_w;
		...
		EXIT WHEN NOT FOUND; /* apply on C03 */
			...
			ds_conteudo_w	:=	lpad(to_char(nr_seq_registro_w),8,'0') || '704' || nr_lote_704_w || lpad(' ',11,' ') ||
						...
						lpad(' ',42,' ') || cd_especialidade_w || dt_pgto_prestador_w || lpad(' ',3,' ') ||
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_arq_a700_100a (nr_seq_servico_p bigint, cd_estabelecimento_p text, nm_usuario_p text) FROM PUBLIC;
