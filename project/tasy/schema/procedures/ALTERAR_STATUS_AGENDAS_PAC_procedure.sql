...
CREATE OR REPLACE PROCEDURE alterar_status_agendas_pac ( ie_status_agenda_p text, ds_sequencia_p text, nm_usuario_p text, cd_motivo_p text default null, ds_motivo_p text default null) AS $body$
DECLARE
...
union
	...
	if (cd_tipo_agenda_w in (1,2))then
		CALL Alterar_Status_Agenda(cd_agenda_w, nr_sequencia_w, ie_status_agenda_p, cd_motivo_p, ds_motivo_p, 'N', nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE alterar_status_agendas_pac ( ie_status_agenda_p text, ds_sequencia_p text, nm_usuario_p text, cd_motivo_p text default null, ds_motivo_p text default null) FROM PUBLIC;
