...
CREATE OR REPLACE PROCEDURE ptu_gerar_arq_mov_prestad_111 (nr_seq_lote_p bigint, cd_interface_p text, nm_usuario_p text, ds_local_p text, nm_arquivo_p text) AS $body$
DECLARE


/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: Gerar arquivo A400 de acordo com a versao PTU 10.0
-----------------------------------------------------------------------------------------------------------------------
...
nr_ddd_fone_whatsapp_w		varchar(4);
nr_fone_whatsapp_w		varchar(12);
...
c01 CURSOR FOR
	SELECT	nr_sequencia,
		...
		rpad(coalesce(to_char(nr_crm),' '),15,' '),
		...
		rpad(coalesce(substr(nm_diretor_tecnico,1,60),' '),60,' '),
		...
		rpad(coalesce(substr(obter_conselho_profissional(nr_seq_conselho,'P'),1,255),' '),12,' '),
		...
		rpad(coalesce(substr(nm_prestador,1,60),' '),60,' '),
		...
		coalesce(ie_titulo_especialista,' '),
		coalesce(indic_residencia,' '),
		rpad(coalesce(to_char(dt_nascimento,'YYYYMMDD'),' '),8,' ')
	from	ptu_prestador
...
C02 CURSOR FOR
	SELECT	z.nr_sequencia,
		...
		lpad(coalesce(z.nr_ddd_fone_whatsapp,'0'),4,'0'),
		lpad(coalesce(z.nr_fone_whatsapp,'0'),12,'0')
...
C05 CURSOR FOR
	SELECT	lpad(coalesce(ie_nivel_exclusao_ptu,'0'),2,'0'),
		...
		lpad(coalesce(cd_motivo_substituicao,'0'),2,'0'),
		rpad(coalesce(nr_oficio,' '),45,' ')		
...
ds_conteudo_w	:=	lpad(nr_seq_registro_w,8,'0') || '401' || cd_unimed_destino_w || cd_unimed_origem_w || dt_geracao_w || '29' || id_ope_prest_w ||
			nr_registro_ans_w;
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	ds_conteudo_w	:=	lpad(nr_seq_registro_w,8,'0') || '402' || tp_prest_w || cd_prest_w || cd_cgc_cpf_w || cd_insc_est_w || lpad(' ',8,' ') || 		
				...
				indic_pos_grad_w || '  ' || nm_prest_w || lpad(' ',10,' ') || id_publica_ans_w || nr_cbo_w || ' ' || cd_uni_prestadora_w || 
				...
				indic_resid_area_1_w || indic_resid_area_2_w || nm_fantasia_w || ie_doutorado_pos_w || nr_conselho_w || id_iso9001_w || nm_diretor_tecnico_w || 
				lpad(' ',10,' ') || indic_mestrado_w ||	ie_titulo_especialista_w || indic_residencia_w || dt_nascimento_w;
	...
	EXIT WHEN NOT FOUND; /* apply on C02 */
		...
		ds_conteudo_w	:=	lpad(nr_seq_registro_w,8,'0') || '403' || tp_end_w || ds_end_w || nr_end_w || lpad(' ',15,' ') || ds_bairro_w || cd_munic_w ||
					...
					ds_endereco_web_w || nr_ddd_fone_whatsapp_w || nr_fone_whatsapp_w;
		pls_hash_ptu_pck.elimina_carac_linha_clob(ds_conteudo_w);
		ds_arquivo_w	:=	ds_arquivo_w || ds_conteudo_w;
		...
		-- 404 GRUPO DE SERVICO (OBRIGATORIO)
		open C03;
		loop
		fetch C03 into	
			cd_gr_serv_w;
	...
	-- 405 REDE REFERENCIADA (OBRIGATORIO)
	open C04;
	loop
	fetch C04 into	
		cd_rede_w,
	...
	-- 406 EXCLUSAO / SUBSTITUICAO (OPCIONAL)
	open C05;
	loop
	fetch C05 into	
	...
	EXIT WHEN NOT FOUND; /* apply on C05 */
		...
		ds_conteudo_w	:=	lpad(nr_seq_registro_w,8,'0') || '406' || ie_nivel_exclusao_ptu_w || cd_exclusao_rede_w || cd_exclusao_pla_prod_w || nr_exclusao_end_w ||
					dt_termino_prest_w || ie_substituicao_w || cd_cnpj_cpf_substit_w || dt_inicio_prest_w || cd_motivo_exclusao_w || cd_motivo_substituicao_w || nr_oficio_w;
		pls_hash_ptu_pck.elimina_carac_linha_clob(ds_conteudo_w);
		ds_arquivo_w	:=	ds_arquivo_w || ds_conteudo_w;
	...
	-- 408 QUALIFICACAO DOS PRESTADORES DE SERVICO (OPCIONAL) (Continuacao)
	open C07;
	loop
	fetch C07 into	
		cd_cgc_cpf_w,
...
-- 998 - HASH (OBRIGATORIO)
nr_seq_registro_w :=	nr_seq_registro_w + 1;
ds_arquivo_w := pls_hash_ptu_pck.obter_hash_txt(ds_arquivo_w, 'N'); -- Gerar HASH sem alterar a acentuacao
...
-- A Unimed Origem do arquivo nao deve gerar o Registro 999. Ele sera gerado exclusivamente pela Central de Movimentacoes Batch (CMB).

-- 999 REGISTRO GERADO PELA CMB (OBRIGATORIO)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_arq_mov_prestad_111 (nr_seq_lote_p bigint, cd_interface_p text, nm_usuario_p text, ds_local_p text, nm_arquivo_p text) FROM PUBLIC;
