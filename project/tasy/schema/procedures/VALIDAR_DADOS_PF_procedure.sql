...
CREATE OR REPLACE PROCEDURE validar_dados_pf (cd_estabelecimento_p bigint, cd_convenio_p bigint, cd_pessoa_fisica_p text, ie_evento_p text, ds_mensagem_p INOUT text, ie_acao_mensagem_p INOUT text, nr_seq_regra_p INOUT bigint) AS $body$
DECLARE
...
if (cd_pessoa_fisica_p IS NOT NULL AND cd_pessoa_fisica_p::text <> '') then
	...
	begin
		select	a.nr_cpf,
			...
			a.nr_ric,
			a.cd_nacionalidade,
			(obter_idade(a.dt_nascimento,clock_timestamp(),'A'))::numeric ,
			nr_pis_pasep,
			nr_cartao_nac_sus,
		...
		where	a.cd_pessoa_fisica	= cd_pessoa_fisica_p  LIMIT 1; --Residencial.
	exception
	when others then
		nr_cpf_w		:= null;
		nr_rg_w			:= null;
	...
	EXIT WHEN NOT FOUND or (ds_mensagem_ww IS NOT NULL AND ds_mensagem_ww::text <> '');  /* apply on C01 */
		...
		if (ie_passaporte_w = 'S') then
			qt_consistir_w	:= qt_consistir_w + 1;
			if (coalesce(nr_passaporte_w::text, '') = '') then
				qt_consistido_w	:= qt_consistido_w + 1;
			end if;
		end if;
		if (ie_rne_w = 'S') then
			qt_consistir_w	:= qt_consistir_w + 1;
			if (coalesce(nr_rne_w::text, '') = '') then
				qt_consistido_w	:= qt_consistido_w + 1;
			end if;
		end if;
		if (ie_ric_w = 'S') then
			qt_consistir_w	:= qt_consistir_w + 1;
			if (coalesce(nr_ric_w::text, '') = '') then
				qt_consistido_w	:= qt_consistido_w + 1;
			end if;
		end if;
		...
		if (ie_pis_w = 'S') then
			...
			if (coalesce(nr_pis_pasep_w::text, '') = '') then
				qt_consistido_w	:= qt_consistido_w + 1;
			end if;
		end if;
		...
		if (ie_dnv_w = 'S') then
			...
			if (coalesce(cd_declaracao_nasc_vivo_w::text, '') = '') then
				qt_consistido_w	:= qt_consistido_w + 1;
			end if;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE validar_dados_pf (cd_estabelecimento_p bigint, cd_convenio_p bigint, cd_pessoa_fisica_p text, ie_evento_p text, ds_mensagem_p INOUT text, ie_acao_mensagem_p INOUT text, nr_seq_regra_p INOUT bigint) FROM PUBLIC;
