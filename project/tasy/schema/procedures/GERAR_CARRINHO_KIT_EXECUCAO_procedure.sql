...
CREATE OR REPLACE PROCEDURE gerar_carrinho_kit_execucao ( cd_estabelecimento_p bigint, cd_pessoa_fisica_p bigint, nm_usuario_p text, cd_local_estoque_p bigint, nr_seq_kit_p bigint, nr_seq_reg_kit_p bigint, ds_retorno_p INOUT text, -- para registro de (kit kit_estoque_reg) 
...
from 	kit_estoque a 
where	a.nr_sequencia = nr_seq_kit_p 
and	coalesce(nr_seq_reg_kit_p::text, '') = '' 
and	obter_se_kit_gera_remontagem(a.cd_kit_material) = 'S' 

union
 
SELECT 	distinct a.nr_sequencia 
from 	kit_estoque a 
where	a.nr_seq_reg_kit = nr_seq_reg_kit_p 
and	obter_se_kit_gera_remontagem(a.cd_kit_material) = 'S' 
order by 1;
...
BEGIN 
ie_estrutura_original_w := 'N';--substr(nvl(obter_valor_param_usuario(900, 423, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_p),'N'),1,1); 
open c01;
loop 
fetch c01 into	 
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	EXIT WHEN NOT FOUND; /* apply on c02 */
		begin 
		qt_material_atendido_w := qt_material_w;
		cd_material_atendido_w := cd_material_w;
		nr_seq_lote_fornec_atendido_w := nr_seq_lote_fornec_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_carrinho_kit_execucao ( cd_estabelecimento_p bigint, cd_pessoa_fisica_p bigint, nm_usuario_p text, cd_local_estoque_p bigint, nr_seq_kit_p bigint, nr_seq_reg_kit_p bigint, ds_retorno_p INOUT text, ds_retorno_2_p INOUT text) FROM PUBLIC;
