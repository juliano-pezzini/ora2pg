...
CREATE OR REPLACE PROCEDURE atualiza_preco_mat_opme_conv ( nr_seq_material_p bigint, nm_usuario_p text) AS $body$
DECLARE
	...
	EXIT WHEN NOT FOUND; /* apply on c02 */
		...
		if (ie_usa_tabela_mat_especial_w = 'S') then
			...
			SELECT * FROM define_preco_material(	cd_estabelecimento_w, 	-- cd_estabelecimento_p		number,
						...
						nr_seq_mat_ajuste_w) INTO STRICT 
						vl_preco_w, 
						dt_ult_vigencia_w, 
						cd_tabela_preco_w, 
						ie_origem_preco_w, 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE atualiza_preco_mat_opme_conv ( nr_seq_material_p bigint, nm_usuario_p text) FROM PUBLIC;
