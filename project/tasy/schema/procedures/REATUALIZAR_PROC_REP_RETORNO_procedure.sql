...
CREATE OR REPLACE PROCEDURE reatualizar_proc_rep_retorno ( nr_seq_procedimento_p bigint, nr_seq_retorno_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
CALL gravar_log_tasy(55889, WHEB_MENSAGEM_PCK.get_texto(298863, 'NR_SEQ_PROCEDIMENTO_P=' || nr_seq_procedimento_p || ';NR_SEQ_RETORNO_P=' || nr_seq_retorno_p) || to_char(clock_timestamp(), 'dd/mm/yyyy hh24:mi:ss'), nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE reatualizar_proc_rep_retorno ( nr_seq_procedimento_p bigint, nr_seq_retorno_p bigint, nm_usuario_p text) FROM PUBLIC;
