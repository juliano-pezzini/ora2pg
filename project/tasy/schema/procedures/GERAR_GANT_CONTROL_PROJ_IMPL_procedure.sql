...
CREATE OR REPLACE PROCEDURE gerar_gant_control_proj_impl ( dt_analise_p timestamp, ie_analise_p text, nm_usuario_p text) AS $body$
DECLARE
...
from	proj_projeto p
where	coalesce(p.ie_origem,'P') = 'P'
and 	p.ie_posicao_externa = 'S'
and	(((ie_analise_p = 'S') and (pkg_date_utils.start_of(dt_alocacao_w,'DD',0) between p.dt_inicio_prev and p.dt_fim_prev)) or
	 ((ie_analise_p = 'M') and (pkg_date_utils.start_of(dt_alocacao_w,'MONTH',0) between pkg_date_utils.start_of(p.dt_inicio_prev,'MONTH',0) and pkg_date_utils.get_datetime(pkg_date_utils.end_of(p.dt_fim_prev, 'MONTH'),coalesce(p.dt_fim_prev, PKG_DATE_UTILS.get_Time('00:00:00'))))))
and	not exists (
		SELECT	1
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_gant_control_proj_impl ( dt_analise_p timestamp, ie_analise_p text, nm_usuario_p text) FROM PUBLIC;
