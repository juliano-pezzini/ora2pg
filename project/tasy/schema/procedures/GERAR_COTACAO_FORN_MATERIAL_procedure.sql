...
CREATE OR REPLACE PROCEDURE gerar_cotacao_forn_material ( nr_cot_compra_p bigint, dt_documento_p timestamp, ie_frete_p text, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
SELECT	a.cd_cgc,
	coalesce(obter_preco_vigente_mat_forn(a.cd_cgc, cd_material_w, cd_estabelecimento_p),0)
from	material_fornec a
where	substr(obter_dados_pf_pj(null,a.cd_cgc,'S'),1,1) = 'A'
and	coalesce(a.ie_cot_compra,'N') = 'S'
and	a.cd_estabelecimento = cd_estabelecimento_p
and	a.cd_material	= cd_material_w
and not exists (
	SELECT	1
	  from	cot_compra_forn x, cot_compra_forn_item y
	 where	x.nr_cot_compra = nr_cot_compra_p
       and x.nr_sequencia = y.nr_seq_cot_forn
	   and	x.cd_cgc_fornecedor = a.cd_cgc
	   and y.nr_item_cot_compra = nr_item_cot_compra_w);
...
if (coalesce(cd_moeda_w,0) = 0) then
	--(-20011,'e necessario informar a moeda, para que se possa Gerar itens pela regra do fornecedor');
...
EXIT WHEN NOT FOUND; /* apply on c00 */
	...
	if (ie_gerar_marca_w = 'S') then		
		select	substr(obter_desc_marca(obter_marca_material(cd_material_w, 'C')),1,30)
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		--	atualizar_item_cot_fornec(nr_cot_compra_p, nr_seq_cot_fornecedor_w, 1, nm_usuario_p);
		...
		--end if;
			...
			EXIT WHEN NOT FOUND; /* apply on c02 */
				...
				/*Anderson 21/05/2009 - OS 143982 - Coloquei este decode pois quando e 10/200 o round fica 0, mas se for 100/28 tem q usar o round*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_cotacao_forn_material ( nr_cot_compra_p bigint, dt_documento_p timestamp, ie_frete_p text, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
