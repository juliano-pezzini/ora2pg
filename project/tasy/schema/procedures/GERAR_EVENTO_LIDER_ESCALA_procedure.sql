...
CREATE OR REPLACE PROCEDURE gerar_evento_lider_escala (nr_seq_evento_p bigint, nr_atendimento_p bigint, cd_pessoa_fisica_p text, nr_seq_laudo_p bigint, nm_usuario_p text, dt_servico_p timestamp, cd_setor_atendimento_p bigint, nr_seq_unidade_p text) AS $body$
DECLARE
...
WHERE c.nr_sequencia = g.nr_seq_classif and g.nr_sequencia = e.nr_seq_grupo and e.nr_sequencia = d.nr_seq_escala and f.nr_seq_escala_diaria = d.nr_sequencia  and obter_tipo_classif_escala(e.nr_sequencia) = 'L' and dt_servico_p between dt_inicio and dt_fim and ((exists ( SELECT 	1 
		   from	escala_setor h 
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	elsif (ie_pessoa_destino_w = '7') then /*Profissional da escala de líder de setor */
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_evento_lider_escala (nr_seq_evento_p bigint, nr_atendimento_p bigint, cd_pessoa_fisica_p text, nr_seq_laudo_p bigint, nm_usuario_p text, dt_servico_p timestamp, cd_setor_atendimento_p bigint, nr_seq_unidade_p text) FROM PUBLIC;
