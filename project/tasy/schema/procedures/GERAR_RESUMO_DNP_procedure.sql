...
CREATE OR REPLACE PROCEDURE gerar_resumo_dnp (cd_estabelecimento_p bigint, nr_seq_consulta_p bigint, cd_pessoa_fisica_p text, ie_mostra_todos_p text, nr_seq_item_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
vl_oe_ard_dnp_w		real;
vl_od_ard_dnp_w		real;
vl_ard_dp_w		real;
...
loop 
	...
	ds_dnp_w 	:= ds_dnp_w || ds_enter_w || obter_desc_expressao(330933)/*'OBSERVAÇÃO: '*/
 || ds_observacao_w ||ds_enter_w;
	 
	CALL gravar_registro_resumo_oft(ds_dnp_w,nr_seq_item_p,nm_usuario_p,to_date(dt_registro_w,'dd/mm/yyyy hh24:mi:ss'));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_resumo_dnp (cd_estabelecimento_p bigint, nr_seq_consulta_p bigint, cd_pessoa_fisica_p text, ie_mostra_todos_p text, nr_seq_item_p bigint, nm_usuario_p text) FROM PUBLIC;
