...
CREATE OR REPLACE PROCEDURE ageint_env_email_ag_canc_exame () AS $body$
DECLARE
...
nr_seq_proc_interno_w		regra_email_exame_canc.nr_seq_proc_interno%type;
...
C02 CURSOR FOR 
	...
	SELECT	distinct 
			...
			to_char(a.hr_inicio, 'dd/mm/yyyy'),				 
...
WHERE a.cd_agenda			= b.cd_agenda   and b.cd_tipo_agenda		= 2 and b.ie_situacao			= 'A' and a.dt_cancelamento		between trunc(clock_timestamp()) and trunc(clock_timestamp()) + 86399/86400 and trunc(a.dt_cancelamento)	= trunc(a.hr_inicio) and (a.cd_pessoa_fisica IS NOT NULL AND a.cd_pessoa_fisica::text <> '') and (a.dt_cancelamento IS NOT NULL AND a.dt_cancelamento::text <> '') and a.ie_status_agenda		= 'C' and ((b.cd_estabelecimento		= cd_estabelecimento_w) or (coalesce(cd_estabelecimento_w::text, '') = '')) and a.nr_seq_proc_interno		= nr_seq_proc_interno_w and not exists (	SELECT	1 
				from	ageint_log_envio x							 
				where	x.nr_seq_agenda_int	= d.nr_seq_agenda_int 
...
from 	user_jobs 
where 	lower(what) like '%ageint_env_email_ag_canc_exame%';
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_seq_regra_w IS NOT NULL AND nr_seq_regra_w::text <> '') and (ds_assunto_email_w IS NOT NULL AND ds_assunto_email_w::text <> '') and (ds_mensagem_email_w IS NOT NULL AND ds_mensagem_email_w::text <> '')then 
		...
		EXIT WHEN NOT FOUND; /* apply on C02 */
			...
			/*insert into log_agenda (ds_dados) 
				values (nr_sequencia_w || ' - ' || nr_seq_agenda_int_w || ' - ' || nm_paciente_w);*/
			...
			if (coalesce(nr_seq_agenda_int_w::text, '') = '')then 
				...
				and		c.hr_inicio				> dt_agenda_ww 
				and		c.nr_seq_proc_interno	= nr_seq_proc_interno_w 
				and		b.dt_transferencia >= dt_ultima_exec_job_w;
			end if;	
			...
			if (qt_emails_enviados_w = 0)then 
				...
				if (nr_sequencia_w IS NOT NULL AND nr_sequencia_w::text <> '') and (ds_email_destino_w IS NOT NULL AND ds_email_destino_w::text <> '')then 
					...
					--Enviar e-mail 
					CALL enviar_email(ds_assunto_email_w, ds_mensagem_email_w, ds_email_origem_w, ds_email_destino_w, 'donhini', 'M');			
					exception 
					when others then						 
						ds_erro_w := substr( Wheb_mensagem_pck.get_texto(307437) /*'Ocorreu algum erro no envio do email!'*/
 ||chr(10)|| Wheb_mensagem_pck.get_texto(307438) || ': ' /*'Detalhes: '*/ ||sqlerrm,1,255);
						/*insert into log_agenda (ds_dados) 
						values (ds_erro_w);*/
...
, ds_email_destino_w, ds_assunto_email_w, 'Tasy', ie_utiliza_format_w, 'C', '');							
			...
			if (coalesce(ds_erro_w::text, '') = '') or (ds_erro_w = '')then 
				...
				exception 
				when others then					 
					ds_erro_w	:= substr( Wheb_mensagem_pck.get_texto(307442) /*'Não foi possível gravar o log de envio de e-mail.'*/
 ||chr(10)|| Wheb_mensagem_pck.get_texto(307438) || ': ' /*'Detalhes: '*/ ||sqlerrm,1,255);
				end;
			end if;
			qt_emails_enviados_w	:= 0;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ageint_env_email_ag_canc_exame () FROM PUBLIC;
