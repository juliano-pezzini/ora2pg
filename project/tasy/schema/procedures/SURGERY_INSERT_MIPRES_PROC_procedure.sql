...
CREATE OR REPLACE PROCEDURE surgery_insert_mipres_proc ( NR_PRESCRICAO_P PRESCR_MIPRES.NR_PRESCRICAO%TYPE, NR_CIRURGIA_P PRESCR_MIPRES.NR_CIRURGIA%TYPE ) AS $body$
DECLARE

    TYPE PRESCR_MIPRES_V_TYPE   IS TABLE OF SURGERY_ITEM_MIPRES_PROCED_V%ROWTYPE  INDEX BY integer;
...
BEGIN
  ...
  FROM    SURGERY_ITEM_MIPRES_PROCED_V V
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE surgery_insert_mipres_proc ( NR_PRESCRICAO_P PRESCR_MIPRES.NR_PRESCRICAO%TYPE, NR_CIRURGIA_P PRESCR_MIPRES.NR_CIRURGIA%TYPE ) FROM PUBLIC;
