...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_fat_v113 ( nr_seq_fatura_p ptu_fatura.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
if (nr_seq_fatura_p IS NOT NULL AND nr_seq_fatura_p::text <> '') then
	...
	EXIT WHEN NOT FOUND; /* apply on C00 */
		...
		-- Obter se a fatura e de beneficiario remido
		select	max(ie_remido)
		into STRICT	ie_benef_remido_w
		from	pls_fatura
		where	nr_sequencia	= nr_seq_pls_fatura_w;

		ie_benef_remido_w	:= coalesce(ie_benef_remido_w,'N');
		...
		EXIT WHEN NOT FOUND; /* apply on C01 */
			...
			-- Quando antecede registro 503, deve ser deixado o espaco em branco - OS 430452
			...
			-- Regra:	Obrigatorio para internacao e honorario individual.  

			--	Quando TP_NOTA = 1 ou 2 os campos deverao ser preenchidos com branco. 

			--	Para versoes anteriores a TISS 3.00.00, considerar a data da internacao e alta. 
			...
			if (ie_benef_remido_w = 'N') then
				cd_unimed_w	:= cd_unimed_destino_w;
			end if;
			...
			EXIT WHEN NOT FOUND; /* apply on C02 */
				...
				-- CAMPOS QUE FORAM "REMOVIDOS" DA VERSAO 5.0
				...
				if ie_benef_remido_w = 'S' then
					begin
						...
						and 	pnc.nr_sequencia = nr_seq_nota_cobranca_w;
						
						if (coalesce(cd_unimed_hospital_w::text, '') = '' and coalesce(nr_cgc_cpf_hosp_w::text, '') = '') then
							select	max(lpad(coalesce(pns.cd_unimed_prestador,'0'),4,'0')),
								max(lpad(coalesce(pns.nr_cgc_cpf,'0'),14,'0'))
							into STRICT	cd_unimed_hospital_w,
								nr_cgc_cpf_hosp_w
							from	ptu_nota_cobranca pnc,
								...
								procedimento  proc
							where   pns.nr_seq_nota_cobr = pnc.nr_sequencia
							and	proc.cd_procedimento = pns.cd_servico
							and	pnc.nr_sequencia = nr_seq_nota_cobranca_w
							and	proc.ie_classificacao  = '2';
						...
						if coalesce(cd_unimed_hospital_w,'X') <> coalesce(cd_unimed_origem_w,'X') then
							begin
			...
			EXIT WHEN NOT FOUND; /* apply on C03 */
				...
				-- Mesma informacao do registro 502
				...
				if (ie_benef_remido_w = 'S' ) then
					cd_unimed_autorizadora_w  := cd_unimed_destino_w;

					-- se for prestador de intercambio
					if coalesce(cd_unimed_prestador_w,'X') <> coalesce(cd_unimed_origem_w,'X') then
						begin
							select 	lpad(d.nr_sequencia,8,'0'),
								rpad(coalesce(elimina_caractere_especial(pls_obter_dados_prest_inter(d.nr_sequencia,'N')),' '),60,' '),
							...
							into STRICT 	cd_prestador_w,
								...
								nr_cbo_exec_w
							from 	pls_prestador_intercambio d
							where 	lpad(d.nr_cpf, 14, '0') = lpad(nr_cgc_cpf_w, 14, '0');
						exception
							when others then
								null;
						end;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_fat_v113 ( nr_seq_fatura_p ptu_fatura.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
