...
CREATE OR REPLACE PROCEDURE dankia_lancamento_material ( cd_acao_p text, nr_atendimento_p bigint, cd_local_estoque_p bigint, cd_material_p bigint, qt_atual_p bigint, dt_movimento_p timestamp, cd_fornecedor_p text, nr_prescricao_p bigint, nr_seq_material_p bigint, nr_seq_lote_p bigint, cd_operacao_estoque_p text, ds_erro_p INOUT text, cd_barras_p text default null, nm_usuario_p text DEFAULT null) AS $body$
DECLARE
...
ie_bloquear_lote_atendido_w varchar(1);
...
select	coalesce(max(ie_bloquear_lote_atendido), 'N')
into STRICT	ie_bloquear_lote_atendido_w
from	parametros_farmacia
where	cd_estabelecimento = cd_estabelecimento_w;

if (ie_bloquear_lote_atendido_w = 'S') and (nr_seq_lote_p IS NOT NULL AND nr_seq_lote_p::text <> '') then
	...
	if (coalesce(nr_atendimento_w,0) > 0) then
		...
		if (ie_cobra_paciente_w = 'S') and (ie_conta_paciente_w = 'S') then
			...
			values (	nr_sequencia_w,
				...
				qt_material_w,
				clock_timestamp(),
				substr(coalesce(nm_usuario_p, 'DANKIA'),1,15),
				cd_acao_w,
				cd_setor_atendimento_w,
				nr_seq_atepacu_w,
				cd_material_w,
			...
			CALL atualiza_preco_material(nr_sequencia_w,substr(coalesce(nm_usuario_p, 'DANKIA'),1,15));
			...
			if (nr_prescricao_p > 0) and (nr_seq_material_p > 0) then
				...
				if (qt_existe_w = 0) and (qt_total_dispensar_item_w <= qt_total_item_conta_w) then
					...
					set 	cd_motivo_baixa = cd_motivo_baixa_int_w,
						...
						nm_usuario	= substr(coalesce(nm_usuario_p, 'DANKIA'),1,15)
					where 	nr_prescricao 	= nr_prescricao_p
					and 	nr_sequencia	= nr_seq_material_p;
				end if;
			end if;
			...
			if (cd_acao_w = 1) and (coalesce(nr_seq_lote_p,0) > 0) then
				...
				if (qt_existe_w = 0) then
					...
					set	dt_atend_farmacia = clock_timestamp(),
						nm_usuario_atend = substr(coalesce(nm_usuario_p, 'DANKIA'),1,15),
						ie_status_lote   = 'A'
					where 	nr_sequencia = nr_seq_lote_p;
				end if;
		...
		if (coalesce(cd_operacao_estoque_p,0) > 0) then
			cd_operacao_estoque_w := (substr(cd_operacao_estoque_p,1,3))::numeric;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dankia_lancamento_material ( cd_acao_p text, nr_atendimento_p bigint, cd_local_estoque_p bigint, cd_material_p bigint, qt_atual_p bigint, dt_movimento_p timestamp, cd_fornecedor_p text, nr_prescricao_p bigint, nr_seq_material_p bigint, nr_seq_lote_p bigint, cd_operacao_estoque_p text, ds_erro_p INOUT text, cd_barras_p text default null, nm_usuario_p text DEFAULT null) FROM PUBLIC;
