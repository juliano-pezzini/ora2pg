...
CREATE OR REPLACE PROCEDURE bl_iniciar_finalizar_preparo ( nr_seq_item_p bigint, ie_opcao_p text, nm_usuario_p text) AS $body$
DECLARE
...
if (nr_seq_item_p IS NOT NULL AND nr_seq_item_p::text <> '') then
	...
	if (ie_opcao_p = 'I') then
		...
		EXIT WHEN NOT FOUND; /* apply on C01 */
			...
			CALL Nut_alterar_qtd_producao(nr_seq_item_alterar_w, qt_dose_w, nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE bl_iniciar_finalizar_preparo ( nr_seq_item_p bigint, ie_opcao_p text, nm_usuario_p text) FROM PUBLIC;
