...
ie_situacao_w			varchar(1);WITH RECURSIVE cte AS (
...
EXIT WHEN NOT FOUND; /* apply on C10 */
	...
	when unique_violation then
		CALL wheb_mensagem_pck.exibir_mensagem_abort(181090,'CD_EXAME_W='||cd_exame_w);
	end;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE lab_duplicar_exame (nr_seq_exame_p bigint, ie_metodo_p text, ie_material_p text, ie_referencia_p text, ie_opcional_p text, ie_convenio_p text, ie_pseudonimo_p text, ie_dependente_p text, ie_formato_p text, ie_liberacao_p text, ie_equipamento_p text, ie_filhos_p text, ie_frasco_p text, nm_usuario_p text) FROM PUBLIC;
