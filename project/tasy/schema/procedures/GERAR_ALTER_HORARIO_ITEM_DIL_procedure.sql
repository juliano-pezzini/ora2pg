...
CREATE OR REPLACE PROCEDURE gerar_alter_horario_item_dil (nr_atendimento_p bigint, ie_tipo_item_p text, nr_prescricao_p bigint, nr_seq_item_p bigint, nr_agrupamento_p bigint, dt_horario_p timestamp, ie_alteracao_p bigint, dt_alteracao_p timestamp, ie_conta_paciente_p text, nm_usuario_p text) AS $body$
DECLARE
...
BEGIN
if (nr_atendimento_p IS NOT NULL AND nr_atendimento_p::text <> '') and (ie_tipo_item_p IS NOT NULL AND ie_tipo_item_p::text <> '') and (nr_prescricao_p IS NOT NULL AND nr_prescricao_p::text <> '') and (nr_seq_item_p IS NOT NULL AND nr_seq_item_p::text <> '') and
	--(nr_agrupamento_p is not null) and
	(dt_horario_p IS NOT NULL AND dt_horario_p::text <> '') and (ie_alteracao_p IS NOT NULL AND ie_alteracao_p::text <> '') and (dt_alteracao_p IS NOT NULL AND dt_alteracao_p::text <> '') and (ie_conta_paciente_p IS NOT NULL AND ie_conta_paciente_p::text <> '') and (nm_usuario_p IS NOT NULL AND nm_usuario_p::text <> '') then

	if (ie_tipo_item_p = 'DIL') then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_alter_horario_item_dil (nr_atendimento_p bigint, ie_tipo_item_p text, nr_prescricao_p bigint, nr_seq_item_p bigint, nr_agrupamento_p bigint, dt_horario_p timestamp, ie_alteracao_p bigint, dt_alteracao_p timestamp, ie_conta_paciente_p text, nm_usuario_p text) FROM PUBLIC;
