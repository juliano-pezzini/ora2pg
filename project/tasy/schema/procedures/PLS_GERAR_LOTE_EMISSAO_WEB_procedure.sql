...
CREATE OR REPLACE PROCEDURE pls_gerar_lote_emissao_web ( nr_seq_carterinha_p bigint, cd_estabelecimento_p bigint, ie_commit_p text, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	and	((ie_agrupar_lote_w = 'N' and nr_seq_tipo_lote = nr_seq_tipo_lote_w and (nr_seq_tipo_lote IS NOT NULL AND nr_seq_tipo_lote::text <> '')
		and (coalesce(nr_contrato::text, '') = '' and ie_tipo_solicitacao = 'W'))
...
if (ie_gerar_emissao_w = 'N') then
	CALL wheb_mensagem_pck.exibir_mensagem_abort(232208,'');
end if;
...
if (ie_gerar_emissao_w = 'S') then
	...
	if (coalesce(nr_seq_lote_w::text, '') = '') then
		...
		insert	into	pls_lote_carteira(nr_sequencia, cd_estabelecimento, dt_atualizacao,
			...
			nr_seq_tipo_lote,nr_contrato, ie_tipo_solicitacao,
		...
		values (	nr_seq_lote_w, cd_estabelecimento_p, clock_timestamp(),
			...
			null, 'G', 'E',
			null,null,CASE WHEN ie_restringir_tipo_benef_w='S' THEN ie_tipo_segurado_w  ELSE '' END ,
			nr_seq_tipo_lote_w,null, 'W',
	...
	from (	SELECT	coalesce(a.ie_controle_carteira,'A') ie_controle_carteira
		from	pls_contrato 		a,
			pls_segurado 		b,
...
union
		...
		SELECT	coalesce(a.ie_controle_carteira,'A') ie_controle_carteira
		from	pls_intercambio		a,
			pls_segurado 		b,
			pls_segurado_carteira	c
		where	a.nr_sequencia	= b.nr_seq_intercambio
		and	b.nr_sequencia	= c.nr_seq_segurado
		and	c.nr_sequencia	= nr_seq_carterinha_p) alias2;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_lote_emissao_web ( nr_seq_carterinha_p bigint, cd_estabelecimento_p bigint, ie_commit_p text, nm_usuario_p text) FROM PUBLIC;
