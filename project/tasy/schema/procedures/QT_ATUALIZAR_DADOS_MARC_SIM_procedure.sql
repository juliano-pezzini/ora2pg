...
CREATE OR REPLACE PROCEDURE qt_atualizar_dados_marc_sim ( nr_seq_pac_sim_p bigint, dt_agenda_p timestamp, nm_usuario_p text, nr_seq_local_p bigint, cd_estabelecimento_p bigint, ie_gerar_p INOUT text) AS $body$
DECLARE
...
if (ie_dia_quimio_w	= 'S') then
	...
	if (qt_marcacao_w	> 0) then
		...
		if	/*(((dt_agenda_p + qt_tempo_medic_w / 1440) <= (dt_ultimo_horario_w + nr_min_apres_w / 1440)) and
			...
			(qt_pendencia_marcada_w	= 0) and (ie_prof_marcado_w	= 'N') then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE qt_atualizar_dados_marc_sim ( nr_seq_pac_sim_p bigint, dt_agenda_p timestamp, nm_usuario_p text, nr_seq_local_p bigint, cd_estabelecimento_p bigint, ie_gerar_p INOUT text) FROM PUBLIC;
