...
CREATE OR REPLACE PROCEDURE pls_atu_cd_material_ops_data () AS $body$
DECLARE
...
while(nr_seq_final_w <= nr_seq_ultimo_reg_w or nr_seq_inicial_w <= nr_seq_ultimo_reg_w) loop 
	...
	forall	i in tb_sequencia_w.first..tb_sequencia_w.last 
		...
		update 	pls_material set 
			...
			dt_referencia = coalesce(dt_limite_utilizacao, dt_exclusao) 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_atu_cd_material_ops_data () FROM PUBLIC;
