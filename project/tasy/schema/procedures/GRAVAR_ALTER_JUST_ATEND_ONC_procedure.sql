...
CREATE OR REPLACE PROCEDURE gravar_alter_just_atend_onc ( nr_seq_atendimento_p bigint, nr_seq_material_p bigint, nr_seq_regra_p bigint, ds_justificativa_p text, qt_dose_prescr_p bigint default 0) AS $body$
DECLARE
...
if (nr_seq_atendimento_p IS NOT NULL AND nr_seq_atendimento_p::text <> '') and (nr_seq_material_p IS NOT NULL AND nr_seq_material_p::text <> '') and (nr_seq_regra_p IS NOT NULL AND nr_seq_regra_p::text <> '') and ((trim(both ds_justificativa_p) IS NOT NULL AND (trim(both ds_justificativa_p))::text <> ''))then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gravar_alter_just_atend_onc ( nr_seq_atendimento_p bigint, nr_seq_material_p bigint, nr_seq_regra_p bigint, ds_justificativa_p text, qt_dose_prescr_p bigint default 0) FROM PUBLIC;
