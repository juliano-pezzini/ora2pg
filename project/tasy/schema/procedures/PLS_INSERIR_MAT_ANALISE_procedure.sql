...
CREATE OR REPLACE PROCEDURE pls_inserir_mat_analise ( nr_seq_material_p bigint, qt_material_p bigint, vl_uni_apres_p bigint, vl_total_apres_p bigint, dt_material_p timestamp, ie_tipo_guia_p text, nr_seq_fornecedor_p bigint, nr_seq_analise_p bigint, nr_nota_fiscal_p text, nr_seq_conta_p bigint, nr_seq_conta_princ_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ie_comitar_p text, nr_seq_grupo_atual_p bigint, nr_seq_item_criado_p INOUT bigint) AS $body$
DECLARE
...
CALL pls_atualiza_valor_mat(nr_seq_mat_w, 'N', nm_usuario_p,null,null);

CALL pls_consistir_conta_mat(nr_seq_mat_w, cd_estabelecimento_p, 'N', nm_usuario_p,null);

/*Rotina para vincular regra de liberacao de valores para conta no procedimento, quando inserido manualmente pela anaálise. */

CALL pls_obter_regra_valor_conta(nr_seq_conta_w,null,nr_seq_mat_w,null,nm_usuario_p);

CALL pls_atualizar_glosa_oc_analise(	null, nr_seq_mat_w, nr_seq_analise_p,
...
if (ie_origem_analise_w = 3) then		
	nr_seq_item_criado_w := pls_gerar_w_resumo_conta_ptu(	nr_seq_conta_w, null, nr_seq_mat_w, nr_seq_conta_princ_p, nr_seq_analise_p, nm_usuario_p, nr_seq_item_criado_w);
else
	nr_seq_item_criado_w := pls_gerar_w_resumo_conta(	nr_seq_conta_w, null, nr_seq_mat_w, nr_seq_conta_princ_p, nr_seq_analise_p, nm_usuario_p, nr_seq_item_criado_w);
end if;				
...
CALL pls_atualizar_conta_pos_estab(	nr_seq_conta_w, null, nr_seq_mat_w,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_inserir_mat_analise ( nr_seq_material_p bigint, qt_material_p bigint, vl_uni_apres_p bigint, vl_total_apres_p bigint, dt_material_p timestamp, ie_tipo_guia_p text, nr_seq_fornecedor_p bigint, nr_seq_analise_p bigint, nr_nota_fiscal_p text, nr_seq_conta_p bigint, nr_seq_conta_princ_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ie_comitar_p text, nr_seq_grupo_atual_p bigint, nr_seq_item_criado_p INOUT bigint) FROM PUBLIC;
