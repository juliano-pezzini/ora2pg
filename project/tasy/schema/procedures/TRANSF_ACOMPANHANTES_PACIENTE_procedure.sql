...
CREATE OR REPLACE PROCEDURE transf_acompanhantes_paciente ( nr_atendimento_p bigint, cd_setor_paciente_p bigint, cd_unid_bas_pac_p text, cd_unid_comp_pac_p text, nm_usuario_p text, ie_consiste_sexo_p text default 'N', ie_considera_ctrl_visita_p text default 'N', nr_seq_interno_para_acomp_p bigint default null) AS $body$
DECLARE
...
ie_PermiteInativos_w		varchar(1);
...
c01 CURSOR FOR
	SELECT	cd_unidade_basica,
		...
		substr(nm_pac_reserva,1,40) nm_pac_reserva,
...
union
	...
	SELECT	null,
		null,
		null,
		cd_pessoa_fisica cd_pessoa_acompanhante,
		substr(obter_nome_pf(cd_pessoa_fisica), 1,40) nm_pac_reserva,
		dt_acompanhante
...
if (nr_atendimento_p IS NOT NULL AND nr_atendimento_p::text <> '') and (cd_setor_paciente_p IS NOT NULL AND cd_setor_paciente_p::text <> '') and (cd_unid_bas_pac_p IS NOT NULL AND cd_unid_bas_pac_p::text <> '') and
	...
	ie_PermiteInativos_w := obter_param_usuario(3111, 54, wheb_usuario_pck.get_cd_perfil, nm_usuario_p, wheb_usuario_pck.get_cd_estabelecimento, ie_PermiteInativos_w);
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		if (coalesce(cd_unid_comp_acom_ww::text, '') = '') then
			...
			if (cd_unidade_basica_acom_ww IS NOT NULL AND cd_unidade_basica_acom_ww::text <> '') and (nr_agrupamento_acom_w IS NOT NULL AND nr_agrupamento_acom_w::text <> '') then
                ...
                if (ie_consiste_sexo_p = 'S') and (obter_se_leito_sexo_acomp(cd_setor_paciente_p, cd_unid_bas_pac_p, cd_unidade_basica_acom_ww, nr_atendimento_p, dt_acompanhante_w, cd_paciente_reserva_w) = 'N') and (obter_se_quarto_familiar(cd_setor_paciente_p,cd_unid_bas_pac_p,cd_unid_comp_pac_p,cd_paciente_reserva_w) = 'N')
					then
		...
		nr_seq_interno_ultimo_w := obter_atepacu_paciente(nr_atendimento_p, 'UAA');
		
		if ((nr_seq_interno_ultimo_w IS NOT NULL AND nr_seq_interno_ultimo_w::text <> '') and nr_seq_interno_ultimo_w <> 0) then
		  select coalesce(cd_tipo_acomodacao, 9999)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE transf_acompanhantes_paciente ( nr_atendimento_p bigint, cd_setor_paciente_p bigint, cd_unid_bas_pac_p text, cd_unid_comp_pac_p text, nm_usuario_p text, ie_consiste_sexo_p text default 'N', ie_considera_ctrl_visita_p text default 'N', nr_seq_interno_para_acomp_p bigint default null) FROM PUBLIC;
