...
CREATE OR REPLACE PROCEDURE gerar_ret_deb_cre_caixa_febr ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_titulo_w IS NOT NULL AND nr_titulo_w::text <> '') then
		...
		CALL fin_gerar_log_controle_banco(3,substr(wheb_mensagem_pck.get_texto(303945,'DS_TITULO=' || ds_titulo_w),1,4000),nm_usuario_p,'N');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_ret_deb_cre_caixa_febr ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
