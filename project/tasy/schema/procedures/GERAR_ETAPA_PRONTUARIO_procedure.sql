...
--
-- dblink wrapper to call function gerar_etapa_prontuario as an autonomous transaction
--
CREATE EXTENSION IF NOT EXISTS dblink;

CREATE OR REPLACE PROCEDURE gerar_etapa_prontuario ( nr_atendimento_p bigint, ie_status_p text, ie_evento_p text, nm_usuario_p text) AS $body$
DECLARE
...
BEGIN
	v_query := 'CALL gerar_etapa_prontuario_atx ( ' || quote_nullable(nr_atendimento_p) || ',' || quote_nullable(ie_status_p) || ',' || quote_nullable(ie_evento_p) || ',' || quote_nullable(nm_usuario_p) || ' )';
...
CREATE OR REPLACE PROCEDURE gerar_etapa_prontuario_atx ( nr_atendimento_p bigint, ie_status_p text, ie_evento_p text, nm_usuario_p text) AS $body$
DECLARE
...
cd_setor_atend_etapa_w	fatur_etapa_alta.cd_setor_atend_etapa%type;C01 CURSOR FOR
	...
	and (coalesce(ie_evento,'A') = coalesce(ie_evento_p,'A')) 
	and	coalesce(cd_perfil, coalesce(obter_perfil_ativo,0)) = coalesce(obter_perfil_ativo,0) 
	and	coalesce(ie_status, coalesce(ie_status_p, '1')) = coalesce(ie_status_p, '1') 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_etapa_prontuario ( nr_atendimento_p bigint, ie_status_p text, ie_evento_p text, nm_usuario_p text) FROM PUBLIC; -- REVOKE ALL ON PROCEDURE gerar_etapa_prontuario_atx ( nr_atendimento_p bigint, ie_status_p text, ie_evento_p text, nm_usuario_p text) FROM PUBLIC;
