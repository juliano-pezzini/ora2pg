...
CREATE OR REPLACE PROCEDURE update_last_dose_medic_html5 ( dt_ultima_dose_p timestamp, nr_prescricao_p bigint, ie_status_atual_p bigint, ie_status_p bigint, ds_sql_item_p text, ie_status_ult_dose_p bigint, ie_utilizacao_p text, ie_ultima_dose_p INOUT text ) AS $body$
DECLARE
...
if (ie_utilizacao_p = 'M') then
	...
	if (ie_status_ult_dose_p > 0) and (ie_status_ult_dose_p = ie_status_p) then
		...
		if (qt_registro_w > 0) then
			...
			CALL exec_sql_dinamico_bv('', ds_comando_update_w, 	'dt_ultima_dose=' || dt_ultima_dose_p || ds_sep_bv_w ||
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE update_last_dose_medic_html5 ( dt_ultima_dose_p timestamp, nr_prescricao_p bigint, ie_status_atual_p bigint, ie_status_p bigint, ds_sql_item_p text, ie_status_ult_dose_p bigint, ie_utilizacao_p text, ie_ultima_dose_p INOUT text ) FROM PUBLIC;
