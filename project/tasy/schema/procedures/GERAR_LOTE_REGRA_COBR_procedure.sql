...
CREATE OR REPLACE PROCEDURE gerar_lote_regra_cobr ( nr_seq_regra_p text, ie_opcao_p text, nm_usuario_p text) AS $body$
DECLARE
...
qtd_liberacao_exclusao_w	bigint;
qtd_seq_lote_w				bigint;
...
cheque CURSOR FOR
	...
	and	(a.dt_liberacao_envio IS NOT NULL AND a.dt_liberacao_envio::text <> '');
...
if (qtd_liberacao_exclusao_w > 0) then
	CALL wheb_mensagem_pck.exibir_mensagem_abort(673896);
end if;

if (qtd_seq_lote_w > 0) then
	CALL wheb_mensagem_pck.exibir_mensagem_abort(673897);
end if;
...
ds_historico_w := wheb_mensagem_pck.get_texto(337788, 'NM_USUARIO=' || nm_usuario_p || ';NR_SEQ_LOTE=' || nr_seq_lote_w || ';DT_ATUALIZACAO=' || to_char(clock_timestamp(),'dd/MM/yyyy hh24:mi:ss'));
...
if (ie_opcao_p = 'C')then
	...
	EXIT WHEN NOT FOUND; /* apply on cheque */
		...
		set	a.nr_seq_lote		= nr_seq_lote_w,
			a.ie_selecionado	= 'N',
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_lote_regra_cobr ( nr_seq_regra_p text, ie_opcao_p text, nm_usuario_p text) FROM PUBLIC;
