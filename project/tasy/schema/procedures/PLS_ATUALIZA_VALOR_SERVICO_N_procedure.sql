...
CREATE OR REPLACE PROCEDURE pls_atualiza_valor_servico_n ( nr_sequencia_p pls_conta_proc.nr_sequencia%type, ie_gravar_log_p text, nm_usuario_p usuario.nm_usuario%type, ie_atualiza_p text, ie_recalcular_conta_p text default 'N') AS $body$
DECLARE
...
nr_seq_cp_comb_serv_entao_w	pls_conta_proc_regra.nr_seq_cp_comb_serv_entao%type;
...
--Cursor de procedimentos presentes na conta diferentes do procedimento que está sendo valorizado
C01 CURSOR FOR
...
BEGIN
--Caso o procedimento que esta sendo valorizado seja um serviço
if (coalesce(ie_tipo_despesa_p,'X') = '2') then
	...
	for r_C02_w in C02 loop
		...
		for r_C01_w in C01 loop
			...
			--Caso sim busca a taxa de ajuste sobre o serviço e retorna como parâmetro
...
select 	max(vl_procedimento_base),
	max(nr_seq_cp_comb_serv_entao),
	max(vl_procedimento_cop),
	coalesce(max(tx_item),0)
into STRICT	vl_procedimento_w,
	nr_seq_cp_comb_serv_entao_w,
	vl_proc_copartic_w,
	tx_item_regra_w
from	pls_conta_proc_regra
where	nr_sequencia	= nr_sequencia_p;

vl_unitario_calc_w	:= vl_procedimento_w;
...
if	not(nr_seq_regra_qtde_exec_w > 0) and
	not(nr_seq_regra_tipo_exec_w > 0) and (coalesce(ie_atualiza_p,'S') = 'S') and
	((coalesce(ie_tx_manual_w::text, '') = '') or (ie_tx_manual_w = 'N' ))then
	vl_calculado_w		:= ((tx_item_w * vl_procedimento_w)/100);
end if;
...
if (ie_gravar_log_w = 'S') then
	CALL pls_entao_regra_preco_cta_pck.gerencia_log_serv(	'Preço definido X Taxa item' || ds_observacao_log_w || '.',
				nr_sequencia_p,
				vl_calculado_w,
				null,
				nm_usuario_p);
...
if (tx_item_regra_w > 0) then
	...
	ds_observacao_log_w	:= 'Tx item A500: ' || tx_item_w;

	if (ie_gravar_log_w = 'S') then
		CALL pls_entao_regra_preco_cta_pck.gerencia_log_serv(	'Preço definido X Taxa item A500' || ds_observacao_log_w || '.',
									nr_sequencia_p,
									vl_calculado_w,
									null,
									nm_usuario_p);
...
if (ie_gravar_log_w = 'S') then
	CALL pls_entao_regra_preco_cta_pck.gerencia_log_serv(	'Preço atual X Qt. procedimento ' || ds_observacao_log_w || '.',
				nr_sequencia_p,
				vl_calculado_w,
				null,
				nm_usuario_p);
...
--Usa o campo _temp, pois esse é  nesse caso, o campo tx_custo_operacional_w não obteve valor, pois nao executou a qtde_exec
...
if (coalesce(tx_adm_w,0) > 0) then
	...
	if (ie_gravar_log_w = 'S') then
		CALL pls_entao_regra_preco_cta_pck.gerencia_log_serv(	'Preço definido X Taxa adm prestador' || ds_observacao_log_w || '.',
									nr_sequencia_p,
									vl_calculado_w,
									null,
									nm_usuario_p);
	...
	if (ie_gravar_log_w = 'S') then
		CALL pls_entao_regra_preco_cta_pck.gerencia_log_serv(	'Preço definido + Valor da taxa adm prestador' || ds_observacao_log_w || '.',
									nr_sequencia_p,
									vl_calculado_w,
									null,
									nm_usuario_p);
...
/* Se obtem valor pelo vinculo direto o valor calculado recebe ele também. */
...
where	nr_sequencia 	= nr_seq_cp_comb_serv_entao_w;
...
--Verifica se há regra de valorização para taxas no cadastro de regras
...
CALL pls_entao_regra_preco_cta_pck.atualizar_log_serv(nm_usuario_p, null);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_atualiza_valor_servico_n ( nr_sequencia_p pls_conta_proc.nr_sequencia%type, ie_gravar_log_p text, nm_usuario_p usuario.nm_usuario%type, ie_atualiza_p text, ie_recalcular_conta_p text default 'N') FROM PUBLIC;
