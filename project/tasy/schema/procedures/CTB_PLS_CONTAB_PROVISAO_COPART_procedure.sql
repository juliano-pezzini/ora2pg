...
CREATE TYPE t_pls_conta_copartic_log AS (	nr_id	t_rowid);
...
CREATE OR REPLACE PROCEDURE ctb_pls_contab_provisao_copart ( nr_lote_contabil_p bigint, nm_usuario_p text, ie_exclusao_p text, ds_retorno_p INOUT text) AS $body$
DECLARE
...
nr_seq_copartic_log_w		pls_conta_copartic_log.nr_sequencia%type;
...
pls_conta_copartic_log_w		t_pls_conta_copartic_log;
...
c_prov_copart CURSOR FOR
	SELECT	c.nr_sequencia,
		...
		null nr_id_copartic_log,
		a.nr_seq_prestador nr_seq_prestador_solic,
		a.nr_seq_prestador_exec nr_seq_prestador_exec,
		(select	nr_seq_prestador_pgto
		 from	pls_conta_medica_resumo r
		 where	r.nr_sequencia = c.nr_seq_conta_resumo
		 and	r.nr_seq_conta = a.nr_sequencia) nr_seq_prestador_pgto,
		x.nr_seq_prestador nr_seq_prestador_atend
	from	pls_conta 			a,
	...
	/*and	not exists (	select	1
				from	pls_conta_copartic_log	l
				where	l.nr_seq_copartic_contab	= c.nr_sequencia
				and	l.ie_tipo_registro		= 'S')*/
...
union all
	...
	select	c.nr_sequencia,
		...
		c.cd_conta_deb_provisao,
		c.cd_conta_cred_provisao,
		coalesce(l.vl_coparticipacao - l.vl_coparticipacao_old,0),
		c.dt_mes_competencia,
		c.cd_historico_provisao,
		a.nr_sequencia,
		a.nr_seq_segurado,
		...
		b.ie_estorno_custo,
		x.dt_recebimento,
		'PLS_CONTA_COPARTIC_LOG' nm_tabela,
		'VL_COPARTICIPACAO' nm_atributo,
		null nr_sequencia,
		(	select	nm_prestador_pgto
			from	pls_conta_medica_resumo		r
	...
	from	pls_conta 			a,
		pls_conta_coparticipacao	b,
		pls_conta_copartic_contab	c,
		pls_conta_copartic_log		l,
		pls_plano			d,
		pls_protocolo_conta 		x
	where	a.nr_sequencia			= b.nr_seq_conta
	and	b.nr_sequencia			= c.nr_seq_conta_copartic
	and	c.nr_sequencia			= l.nr_seq_copartic_contab
	and	a.nr_sequencia			= b.nr_seq_conta
	and	x.nr_sequencia			= a.nr_seq_protocolo
	and	d.nr_sequencia			= a.nr_seq_plano
	and	coalesce(c.vl_coparticipacao,0)	> 0
	...
	and	b.ie_status_coparticipacao	<> 'N'))
	and	c.dt_mes_competencia between dt_referencia_month_w and dt_referencia_fm_w
	and	l.dt_atualizacao between dt_referencia_month_w and dt_referencia_fm_w
	and	x.cd_estabelecimento		= cd_estabelecimento_w
	and	coalesce(c.nr_lote_contabil_prov,0)	= 0
...
union all
	...
	and	coalesce(c.nr_lote_contabil_estorno,0) = 0
	and	b.dt_fechamento_discussao between dt_referencia_month_w and dt_referencia_fm_w;
...
-- Variaveis do c_movimento_w | cursor para limpar a tabela
...
if (ie_exclusao_p <> 'S') then
        ...
        if (cd_tipo_lote_contabil_w <> 42) then
...
if (ie_exclusao_p = 'S') then
	...
	EXIT WHEN NOT FOUND; /* apply on c_prov_copart */
		...
		if (ie_estorno_w = 'N') then
			...
			pls_conta_copartic_log_w.nr_id(pls_conta_copartic_log_w.nr_id.count)	:= nr_id_copartic_log_w;
			
			if (pls_conta_copartic_log_w.nr_id.count >= 1000) then
				begin
				forall i in pls_conta_copartic_log_w.nr_id.first .. pls_conta_copartic_log_w.nr_id.last
					update	pls_conta_copartic_log a
					set	a.nr_lote_contabil	= nr_lote_contabil_p
					where	rowid			= pls_conta_copartic_log_w.nr_id(i);
				commit;
				pls_conta_copartic_log_w.nr_id.delete;
				end;
			end if;
			
			if (not pls_copartic_contab_prov_w.nr_id.exists(nr_seq_copartic_contab_w)) then
				pls_copartic_contab_prov_w.nr_id(nr_seq_copartic_contab_w)	:= nr_id_copartic_contab_w;
			end if;
			
			if (pls_copartic_contab_prov_w.nr_id.count >= 1000) then
				begin
				forall i in indices of pls_copartic_contab_prov_w.nr_id
	...
	if (pls_conta_copartic_log_w.nr_id.count > 0) then
		forall i in pls_conta_copartic_log_w.nr_id.first .. pls_conta_copartic_log_w.nr_id.last
			update	pls_conta_copartic_log a
			set	a.nr_lote_contabil	= nr_lote_contabil_p
			where	rowid			= pls_conta_copartic_log_w.nr_id(i);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ctb_pls_contab_provisao_copart ( nr_lote_contabil_p bigint, nm_usuario_p text, ie_exclusao_p text, ds_retorno_p INOUT text) FROM PUBLIC;
