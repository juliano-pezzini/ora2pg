...
CREATE OR REPLACE PROCEDURE hd_gerar_controle_exame (cd_pessoa_fisica_p text, nr_seq_protocolo_p bigint, dt_referencia_p timestamp, ds_itens_p text, ie_atualizar_todos_p text, -- Ao chamar pela Função, passar 'F'
 qt_reg_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
c01 CURSOR FOR
	SELECT	ds_item
	from	hd_item_controle_exame where	((coalesce(nr_seq_protocolo_p::text, '') = '') or (Hd_obter_se_item_protocolo(nr_seq_protocolo_p,nr_sequencia) = 'S'))
...
while(qt_reg_w	> 0) loop
	...
	and	ie_ordem  = 1
	and	PKG_DATE_UTILS.start_of(dt_referencia, 'dd', 0) = PKG_DATE_UTILS.start_of(dt_referencia_w, 'dd', 0);

	if (qt_itens_w = 0) then -- criar titulos
		ivet	:= 0;
		begin
		open	c01;
		loop
	...
	EXIT WHEN NOT FOUND; /* apply on c03 */
		...
		and	ie_atualizar_todos_p in ('S','F');
	...
	into STRICT	qt_itens_w
	from	HD_EXAME_GRID_CONTROLE
	where	PKG_DATE_UTILS.start_of(dt_referencia, 'dd', 0) = PKG_DATE_UTILS.start_of(dt_referencia_w, 'dd', 0)
	and	cd_pessoa_fisica = cd_pessoa_fisica_p
	and	nm_usuario = nm_usuario_p
	and	ie_ordem = 5;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE hd_gerar_controle_exame (cd_pessoa_fisica_p text, nr_seq_protocolo_p bigint, dt_referencia_p timestamp, ds_itens_p text, ie_atualizar_todos_p text, qt_reg_p bigint, nm_usuario_p text) FROM PUBLIC;
