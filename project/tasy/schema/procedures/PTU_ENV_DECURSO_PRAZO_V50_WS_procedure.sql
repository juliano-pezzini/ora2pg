...
CREATE OR REPLACE PROCEDURE ptu_env_decurso_prazo_v50_ws ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type, nr_seq_decurso_p INOUT ptu_decurso_prazo.nr_sequencia%type) AS $body$
DECLARE
...
if	((nr_seq_guia_p IS NOT NULL AND nr_seq_guia_p::text <> '') or (nr_seq_requisicao_p IS NOT NULL AND nr_seq_requisicao_p::text <> '')) then
	insert	into ptu_decurso_prazo(nr_sequencia, cd_transacao, ie_tipo_cliente,
		 ...
		 nr_seq_guia)
	values (nextval('ptu_decurso_prazo_seq'), '00700',ie_tipo_cliente_w,
		 cd_unimed_executora_w, cd_unimed_beneficiario_w, nr_seq_execucao_w,
		 clock_timestamp(), nm_usuario_p, clock_timestamp(),
		 nm_usuario_p, '050', nr_seq_requisicao_p,
		 nr_seq_guia_p) returning nr_sequencia into nr_seq_decurso_p;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_env_decurso_prazo_v50_ws ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type, nr_seq_decurso_p INOUT ptu_decurso_prazo.nr_sequencia%type) FROM PUBLIC;
