...
CREATE OR REPLACE PROCEDURE w_ctb_gerar_razao ( cd_empresa_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estab_p text, cd_conta_contabil_p text, cd_classificacao_p text, cd_centro_custo_p text, nm_usuario_p text, ie_valor_p text, cd_classif_inic_p text, cd_classif_final_p text, ie_centro_custo_p text, ie_ordenacao_p text, ie_contra_partida_p text) AS $body$
DECLARE
...
union all
	...
	select	'S',
		dt_inicial_p,
		Wheb_mensagem_pck.get_texto(795959),
		'',
		'',
		'',
		0,
	...
	if (ie_gerar_w	= 'S')  then
		...
		EXIT WHEN NOT FOUND; /* apply on c002 */
			...
			if (ie_contrapartida_w = 'N') then
				...
				cd_contrapartida_ww	:= '';
				cd_contrapartida_ww	:= substr(w_ctb_obter_contrapartida_m(nr_seq_movto_w, nr_lote_contabil_w, ie_contrapartida_w,ie_debito_credito_w),1,4000);
