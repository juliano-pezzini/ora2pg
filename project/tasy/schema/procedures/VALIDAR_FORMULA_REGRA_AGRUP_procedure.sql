...
CREATE OR REPLACE PROCEDURE validar_formula_regra_agrup ( ds_formula_p text, ie_erro_p INOUT text) AS $body$
DECLARE
...
if (ds_formula_p IS NOT NULL AND ds_formula_p::text <> '') then
	...
	while(ds_formula_orig_w IS NOT NULL AND ds_formula_orig_w::text <> '') and (ie_erro_w = 'N') loop
		...
		when others then
			begin
			if (sqlerrm like '%ORA-01476%') then
				ie_erro_w	:= 'N';
			else
				ie_erro_w	:= 'S';
			end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE validar_formula_regra_agrup ( ds_formula_p text, ie_erro_p INOUT text) FROM PUBLIC;
