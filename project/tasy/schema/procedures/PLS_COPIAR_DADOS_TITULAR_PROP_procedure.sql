...
CREATE OR REPLACE PROCEDURE pls_copiar_dados_titular_prop ( nr_seq_beneficiario_p bigint, nr_seq_titular_p bigint, ie_bonificacao_p text, ie_sca_p text, nm_usuario_p text) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	ds_erro_w := pls_consistir_sca_proposta(nr_seq_beneficiario_p, nr_seq_plano_sca_w, nr_seq_tabela_w, wheb_usuario_pck.get_cd_estabelecimento, ds_erro_w, nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_copiar_dados_titular_prop ( nr_seq_beneficiario_p bigint, nr_seq_titular_p bigint, ie_bonificacao_p text, ie_sca_p text, nm_usuario_p text) FROM PUBLIC;
