...
CREATE OR REPLACE PROCEDURE consistir_aprovacao_exame (nr_prescricao_p bigint, nr_seq_prescr_p bigint, ds_consistencia_p INOUT text) AS $body$
DECLARE
...
loop
	...
	loop
		...
		ds_referencia_ww := substr(ds_referencia_w,1,2000);
		if (ie_formato_resultado_w = 'SM') or (ie_formato_resultado_w = 'SDM') or (ie_formato_resultado_w = 'S') or (ie_formato_resultado_w = 'MS') or (ie_formato_resultado_w = 'MSL') or (ie_formato_resultado_w = 'DL') or (ie_formato_resultado_w = 'DV') or (ie_formato_resultado_w = 'D') then
			if (length(ds_resultado_w) < 255) then
				if not(upper(ELIMINA_ACENTOS(ds_resultado_w)) like upper(ELIMINA_ACENTOS(ds_referencia_ww))) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE consistir_aprovacao_exame (nr_prescricao_p bigint, nr_seq_prescr_p bigint, ds_consistencia_p INOUT text) FROM PUBLIC;
