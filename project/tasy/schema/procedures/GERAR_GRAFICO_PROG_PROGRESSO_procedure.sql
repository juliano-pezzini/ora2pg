...
CREATE OR REPLACE PROCEDURE gerar_grafico_prog_progresso (nr_seq_prog_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	SELECT  distinct trunc(dt_referencia,'w')
...
C02 CURSOR FOR
	SELECT  obter_pr_prev_etapa_prog(nr_seq_prog_p ,dt_ref_w) pr_prev,
		obter_pr_agregado_etapa_prog(nr_seq_prog_p, dt_ref_w) pr_agregado
	;
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	begin
		...
		if (trunc(dt_ref_w,'w')	= trunc(clock_timestamp(),'w')) then

			select  max(a.pr_real),
				max(a.pr_prev)
			into STRICT	pr_real_w,
				pr_prev_w
			from     dashboard_pmo_prog_resumo_v a
			where    a.nr_sequencia = nr_seq_prog_p;
		elsif (dt_ref_w	>= clock_timestamp()) then
			pr_prev_w	:= obter_pr_prev_etapa_prog(nr_seq_prog_p ,dt_ref_w);
		else
			select	max(pr_real),
				max(pr_prev)
			into STRICT	pr_real_w,
		...
		if (coalesce(pr_prev_w,0) > 0) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_grafico_prog_progresso (nr_seq_prog_p bigint, nm_usuario_p text) FROM PUBLIC;
