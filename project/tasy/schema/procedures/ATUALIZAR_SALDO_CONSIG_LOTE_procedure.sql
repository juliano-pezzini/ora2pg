...
CREATE OR REPLACE PROCEDURE atualizar_saldo_consig_lote ( cd_estabelecimento_p bigint, cd_local_estoque_p bigint, cd_fornecedor_p text, cd_material_p bigint, dt_mesano_referencia_p timestamp, nr_seq_lote_p bigint, cd_operacao_estoque_p bigint, qt_estoque_p bigint, cd_acao_p text, nm_usuario_p text, ie_erro_p INOUT text) AS $body$
DECLARE
...
if (cd_operacao_estoque_p = 0) then
	...
	select	ie_entrada_saida,
		CASE WHEN ie_consignado=0 THEN 'N' WHEN ie_consignado=1 THEN 'N' WHEN coalesce(ie_consignado::text, '') = '' THEN 'N'  ELSE 'S' END
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE atualizar_saldo_consig_lote ( cd_estabelecimento_p bigint, cd_local_estoque_p bigint, cd_fornecedor_p text, cd_material_p bigint, dt_mesano_referencia_p timestamp, nr_seq_lote_p bigint, cd_operacao_estoque_p bigint, qt_estoque_p bigint, cd_acao_p text, nm_usuario_p text, ie_erro_p INOUT text) FROM PUBLIC;
