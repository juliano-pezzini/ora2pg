...
CREATE OR REPLACE PROCEDURE verifica_res_nao_aceit_alert (nr_prescricao_p bigint, nr_seq_prescr_p bigint, nr_seq_exame_p bigint, cd_material_exame_p text, possui_result_nao_aceit_p INOUT text) AS $body$
DECLARE
...
loop
	...
	loop
		...
		if (ie_formato_resultado_w = 'P') or (pr_resultado_w > 0 AND ie_formato_resultado_w = 'VP') then
			begin
		...
		elsif (ie_formato_resultado_w = 'V') or (qt_resultado_w > 0 AND ie_formato_resultado_w = 'VP') then
			begin
		...
		elsif	((ie_formato_resultado_w = 'D') or (ie_formato_resultado_w = 'SM') or (ie_formato_resultado_w = 'MS') or (ie_formato_resultado_w = 'S')  or (ie_formato_resultado_w = 'SDM')) then
			begin
			select ie_tipo_valor
			into STRICT	 ie_tipo_valor_w
			from exame_lab_padrao
			...
			if  ((ds_resultado_w <> somente_numero_virg_char(ds_resultado_w)) or (coalesce(somente_numero_virg_char(ds_resultado_w)::text, '') = '')) then
				...
				where ((ie_sexo = coalesce(ie_sexo_w,'0')) or (ie_sexo = '0'))
				  ...
				  and (((coalesce(trunc((qt_dias_w / 365.25),qt_casas_decimais_dias_w),0) between qt_idade_min and qt_idade_max) and (ie_periodo = 'A')) or
					 ...
					 (((coalesce(qt_dias_w,0)) between qt_idade_min and qt_idade_max) and (ie_periodo = 'D')) or
					 (((coalesce(qt_horas_w,0)) between qt_idade_min and qt_idade_max) and (ie_periodo = 'H')))
				  and upper(ELIMINA_ACENTOS(trim(both ds_resultado_w))) like upper(ELIMINA_ACENTOS(trim(both DS_OBSERVACAO)))
				  and coalesce(ie_situacao,'A') = 'A'
				order by nr_seq_material, ie_sexo;	
				...
				where ((ie_sexo = coalesce(ie_sexo_w,'0')) or (ie_sexo = '0'))
				  ...
				  and (((coalesce(trunc((qt_dias_w / 365.25),qt_casas_decimais_dias_w),0) between qt_idade_min and qt_idade_max) and (ie_periodo = 'A')) or
					 ...
					 (((coalesce(qt_dias_w,0)) between qt_idade_min and qt_idade_max) and (ie_periodo = 'D')) or
					 (((coalesce(qt_horas_w,0)) between qt_idade_min and qt_idade_max) and (ie_periodo = 'H')))
				  and ds_resultado_w between qt_minima and qt_maxima
				  and coalesce(ie_situacao,'A') = 'A'
				order by nr_seq_material, ie_sexo, ie_tipo_valor;	
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE verifica_res_nao_aceit_alert (nr_prescricao_p bigint, nr_seq_prescr_p bigint, nr_seq_exame_p bigint, cd_material_exame_p text, possui_result_nao_aceit_p INOUT text) FROM PUBLIC;
