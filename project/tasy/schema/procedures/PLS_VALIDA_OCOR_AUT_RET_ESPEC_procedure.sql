...
CREATE OR REPLACE PROCEDURE pls_valida_ocor_aut_ret_espec ( nr_seq_ocor_combinada_p pls_ocor_aut_combinada.nr_sequencia%type, nr_seq_ocorrencia_p pls_ocorrencia.nr_sequencia%type, nr_seq_motivo_glosa_p tiss_motivo_glosa.nr_sequencia%type, nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, nr_seq_execucao_p pls_execucao_requisicao.nr_sequencia%type, ie_utiliza_filtro_p pls_ocor_aut_combinada.ie_utiliza_filtro%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type ) AS $body$
DECLARE
...
ie_gerar_ocorrencia_ret_esp_w	varchar(2);
...
C01 CURSOR FOR 
	SELECT	cd_especialidade_medica, 
		qt_dias_regra_prest, 
		coalesce(ie_valida_execucao, 'N') ie_valida_execucao 
	from	pls_validacao_aut_ret_espc 
...
if (nr_seq_guia_p IS NOT NULL AND nr_seq_guia_p::text <> '') then 
	...
	for r_C01_w in C01 loop 
		begin 
		ie_gerar_ocorrencia_ret_esp_w	:= 'S';
		 
		/*Se for uma regra de quantidades de dias da regra de prestador */
	 
		if (coalesce(r_C01_w.qt_dias_regra_prest,0) = 0) then				 
			/*Se for uma regra com Especialidade médica informada*/
	 
			if (r_C01_w.cd_especialidade_medica IS NOT NULL AND r_C01_w.cd_especialidade_medica::text <> '') and (r_C01_w.cd_especialidade_medica <> cd_especialidade_w) then	 
				ie_gerar_ocorrencia_ret_esp_w	:= 'N';
			end if;
		else 
			ie_gerar_ocorrencia_ret_esp_w	:= pls_obter_se_regra_dias_aut(	nr_seq_guia_p, null, null, 
											r_C01_w.cd_especialidade_medica, r_C01_w.qt_dias_regra_prest, r_C01_w.ie_valida_execucao);
		end if;
		...
		for r_C02_w in C02 loop 
			begin 
				...
				if (ie_gerar_ocorrencia_w	= 'S') then					 
					ie_gerar_ocorrencia_w	:= ie_gerar_ocorrencia_ret_esp_w;
	...
	for r_C01_w in C01 loop 
		...
		if (coalesce(r_C01_w.qt_dias_regra_prest,0) = 0) then				 
			...
			ie_gerar_ocorrencia_ret_esp_w	:= pls_obter_se_regra_dias_aut(	null, nr_seq_requisicao_p, null, 
											r_C01_w.cd_especialidade_medica, r_C01_w.qt_dias_regra_prest, r_C01_w.ie_valida_execucao);
		end if;
	...
	for r_C01_w in C01 loop 
		...
		if (coalesce(r_C01_w.qt_dias_regra_prest,0) = 0) then				 
			...
			ie_gerar_ocorrencia_ret_esp_w	:= pls_obter_se_regra_dias_aut(	null, null, nr_seq_execucao_p, 
											r_C01_w.cd_especialidade_medica, r_C01_w.qt_dias_regra_prest, r_C01_w.ie_valida_execucao);
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_valida_ocor_aut_ret_espec ( nr_seq_ocor_combinada_p pls_ocor_aut_combinada.nr_sequencia%type, nr_seq_ocorrencia_p pls_ocorrencia.nr_sequencia%type, nr_seq_motivo_glosa_p tiss_motivo_glosa.nr_sequencia%type, nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, nr_seq_execucao_p pls_execucao_requisicao.nr_sequencia%type, ie_utiliza_filtro_p pls_ocor_aut_combinada.ie_utiliza_filtro%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type ) FROM PUBLIC;
