...
CREATE OR REPLACE PROCEDURE gerar_etapa_internacao_atend (nr_atendimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR 
	...
	and (coalesce(ie_evento,'H') = coalesce(ie_evento_w,'H')) 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_etapa_internacao_atend (nr_atendimento_p bigint, nm_usuario_p text) FROM PUBLIC;
