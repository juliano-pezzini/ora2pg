...
CREATE OR REPLACE PROCEDURE gqa_liberacao_diagnostico ( nr_sequencia_p bigint, nm_usuario_p text, nr_seq_mentor_p INOUT bigint, nr_regras_atendidas_p INOUT text, nr_atendimento_p bigint default null, nr_restrigir_regras_p text default null, qt_horas_retroativa_p bigint default 0) AS $body$
DECLARE
...
C01 CURSOR FOR
    ...
    and	    b.ie_tipo_pendencia	= 1
    and	    b.ie_situacao 		= 'A'
    and	    a.ie_situacao		= 'A'
    and	    obter_se_gqa_regra_liberada(a.nr_sequencia) = 'S'
    and	    a.cd_doenca_cid		= cd_doenca_cid_w
...
if ie_protocolo_assistencial_w then
    open CursorDiagnostico for SELECT max(nr_atendimento), max(obter_pessoa_atendimento(nr_atendimento,'C')), cd_doenca
							   from   diagnostico_doenca
							   where  nr_atendimento = nr_atendimento_p
							   and    ie_situacao = 'A'
							   and   (dt_liberacao IS NOT NULL AND dt_liberacao::text <> '')
							   and ( qt_horas_retroativa_p = 0 or dt_liberacao >= clock_timestamp() - qt_horas_retroativa_p )
							   group by cd_doenca;
else
    open CursorDiagnostico for SELECT max(nr_atendimento), max(obter_pessoa_atendimento(nr_atendimento,'C')), max(cd_doenca)
							   from   diagnostico_doenca
...
;
			...
			ds_doenca_cid_w := substr(obter_desc_cid(cd_doenca_cid_w),1,200);
			
			CALL gerar_consulta_reg_mentor(nm_usuario_p, 'DIAGNOSTICO_DOENCA', nr_seq_pend_regra_w, nr_sequencia_p, nr_sequencia_w, cd_doenca_cid_w,ds_doenca_cid_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gqa_liberacao_diagnostico ( nr_sequencia_p bigint, nm_usuario_p text, nr_seq_mentor_p INOUT bigint, nr_regras_atendidas_p INOUT text, nr_atendimento_p bigint default null, nr_restrigir_regras_p text default null, qt_horas_retroativa_p bigint default 0) FROM PUBLIC;
