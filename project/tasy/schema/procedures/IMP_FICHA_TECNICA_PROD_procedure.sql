...
CREATE OR REPLACE PROCEDURE imp_ficha_tecnica_prod ( productcode_p bigint, nm_usuario_p text, nr_seq_ficha_tecnica_p INOUT bigint, nr_mims_ver_p bigint) AS $body$
DECLARE
...
c04 CURSOR FOR
	...
	and a.nr_seq_mims_version = nr_mims_ver_p
	and	exists (	SELECT	1
				...
				and	coalesce(y.approved,y.GENCODE) = b.CD_DCB)
...
union
	...
	if (coalesce(nr_seq_superior_w::text, '') = '') then
		...
		if (produc_code_w IS NOT NULL AND produc_code_w::text <> '')  then
			...
			--	dbms_output.put_line('ds_superior_w = '||ds_superior_w);
				nr_seq_superior_w := mims_imp_ficha_tec(	null, ds_superior_w, null, nr_seq_superior_w, null, null, nm_usuario_p, nr_mims_ver_p);
			...
			for r_c02 in c02 loop
				...
				nr_seq_ficha_tecnica_w := mims_imp_ficha_tec(	r_c02.generic_code, r_c02.GENERIC, nr_seq_superior_w, nr_seq_ficha_tecnica_w, r_c02.generic_code, null, nm_usuario_p, nr_mims_ver_p);
    ...
    mims_ficha_prod_w.NR_SEQ_MIMS_VERSION := nr_mims_ver_p;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE imp_ficha_tecnica_prod ( productcode_p bigint, nm_usuario_p text, nr_seq_ficha_tecnica_p INOUT bigint, nr_mims_ver_p bigint) FROM PUBLIC;
