...
CREATE OR REPLACE PROCEDURE gerar_remessa_itau_cnab_240 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
when others then 
	if (SQLSTATE = -29289) then 
		...
		ds_erro_w := obter_desc_expressao(624984);--'Não foi possível apagar o arquivo (delete_failed).'; 
	elsif (SQLSTATE = -29286) then
		ds_erro_w := 'Erro interno desconhecido no package UTL_FILE (internal_error).';
	elsif (SQLSTATE = -29282) then 
		ds_erro_w := obter_desc_expressao(625550);--'O handle do arquivo não existe (invalid_filehandle).'; 
	elsif (SQLSTATE = -29288) then
		ds_erro_w := obter_desc_expressao(623584);--'O arquivo com o nome especificado não foi encontrado neste local (invalid_filename).'; 
	elsif (SQLSTATE = -29287) then
		ds_erro_w := obter_desc_expressao(624168);--'O valor de MAX_LINESIZE para FOPEN() é inválido; deveria estar na faixa de 1 a 32767 (invalid_maxlinesize).'; 
	elsif (SQLSTATE = -29281) then
		ds_erro_w := obter_desc_expressao(624449);--'O parâmetro open_mode na chamda FOPEN é inválido (invalid_mode).'; 
	elsif (SQLSTATE = -29290) then
		ds_erro_w := obter_desc_expressao(624294);--'O parâmetro ABSOLUTE_OFFSET para a chamada FSEEK() é inválido; deveria ser maior do que 0 e menor do que o número total de bytes do arquivo (invalid_offset).'; 
	elsif (SQLSTATE = -29283) then
		ds_erro_w := obter_desc_expressao(624128);--'O arquivo não pôde ser aberto ou operado da forma desejada - ou o caminho não foi encontrado (invalid_operation).'; 
	elsif (SQLSTATE = -29280) then
		ds_erro_w := obter_desc_expressao(623690);--'O caminho especificado não existe ou não está visível ao Oracle (invalid_path).'; 
	elsif (SQLSTATE = -29284) then
		ds_erro_w := obter_desc_expressao(624024);--'Não é possível efetuar a leitura do arquivo (read_error).'; 
	elsif (SQLSTATE = -29292) then
		ds_erro_w := obter_desc_expressao(625050);--'Não é possível renomear o arquivo.'; 
	elsif (SQLSTATE = -29285) then
		ds_erro_w := obter_desc_expressao(623622);--'Não foi possível gravar no arquivo (write_error).'; 
	else
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_remessa_itau_cnab_240 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
