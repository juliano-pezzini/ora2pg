...
CREATE OR REPLACE PROCEDURE ptu_gerar_resp_ordem_serv ( nr_seq_ordem_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (nr_seq_prestador_w	<> 0) then 
	select	coalesce(ie_prestador_alto_custo,'N') 
	into STRICT	ie_prest_alto_custo_w 
	from	pls_prestador 
...
CALL ptu_consistir_transacao('00507',nr_seq_guia_w, null, null,cd_estabelecimento_p,nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_resp_ordem_serv ( nr_seq_ordem_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
