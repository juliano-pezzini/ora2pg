...
CREATE OR REPLACE PROCEDURE dacon_registro_r06a ( nr_seq_dacon_p bigint, nm_usuario_P text) AS $body$
DECLARE
...
C01 CURSOR FOR
	SELECT  coalesce(cd_conta_contabil,'X'), --Regra por conta (pasta Dacon > Regra conta)
		coalesce(ie_tipo_conta,'X'),
		coalesce(ie_tipo_valor,'X')
	from    dacon_regra_conta_ctb
	where   nr_seq_dacon = nr_seq_dacon_p
	and	cd_registro  = 'R06A';

C02 CURSOR FOR
	SELECT	rpad('R06A',4,' ')			--01
		||lpad((to_char(dt_referencia, 'MM'))::numeric ,2,0)--02
		||lpad(1,2,0) --coluna,				--03
		||lpad(0,14,0) --coluna				--04
		||lpad(0,14,0)			--05
		||lpad(0,14,0)					--06
		||lpad(somente_numero(vl_desp_energ_w),14,0)			--07 -- Despesa de energia IE_TIPO_CONTA = 3
		||lpad(0,14,0)					--08
		||lpad(0,14,0)					--09
		||lpad(0,14,0)					--10
		||lpad(0,14,0)					--11
...
union all
	...
	SELECT	rpad('R06A',4,' ')				--01
		...
		||lpad(0,14,0)			--05
		||lpad(0,14,0)					--06
		||lpad(somente_numero(vl_desp_energ_w),14,0)			--07
		||lpad(0,14,0)					--08
		||lpad(0,14,0)					--09
		||lpad(0,14,0)					--10
		||lpad(0,14,0)					--11
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dacon_registro_r06a ( nr_seq_dacon_p bigint, nm_usuario_P text) FROM PUBLIC;
