...
CREATE OR REPLACE PROCEDURE imp_interacao_medic_ficha ( nm_usuario_p text, nr_mims_version_p bigint) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	order by cd_dcb, cd_dcb_interacao, druginteractclassid2 asc, druginteractclassid1 asc;
...
for r_c01 in c01 loop
	...
	nr_seq_ficha_w := mims_imp_ficha_tec(	r_c01.cd_dcb, r_c01.ds_dcb, null, nr_seq_ficha_w, r_c01.cd_dcb, null, nm_usuario_p, nr_mims_version_p);

	nr_seq_ficha_interacao_w := mims_imp_ficha_tec(	r_c01.cd_dcb_interacao, r_c01.ds_dcb_interacao, null, nr_seq_ficha_interacao_w, r_c01.cd_dcb_interacao, null, nm_usuario_p, nr_mims_version_p);
	...
	if (nr_seq_ficha_w IS NOT NULL AND nr_seq_ficha_w::text <> '') and (nr_seq_ficha_interacao_w IS NOT NULL AND nr_seq_ficha_interacao_w::text <> '') then
		...
		CALL mims_imp_interacao_ficha(	nr_seq_ficha_w,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE imp_interacao_medic_ficha ( nm_usuario_p text, nr_mims_version_p bigint) FROM PUBLIC;
