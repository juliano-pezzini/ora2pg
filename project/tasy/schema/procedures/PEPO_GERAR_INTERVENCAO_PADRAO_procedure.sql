...
CREATE OR REPLACE PROCEDURE pepo_gerar_intervencao_padrao ( nr_sequencia_p bigint, nm_usuario_p text, ds_lista_nr_seq_proc_p text) AS $body$
DECLARE
...
BEGIN 
if (ds_lista_nr_seq_proc_p IS NOT NULL AND ds_lista_nr_seq_proc_p::text <> '') then 
	begin 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pepo_gerar_intervencao_padrao ( nr_sequencia_p bigint, nm_usuario_p text, ds_lista_nr_seq_proc_p text) FROM PUBLIC;
