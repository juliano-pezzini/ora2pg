...
ds_horarios_aux_w		prescr_material.ds_horarios%type;
...
hr_referencia_dieta_w	intervalo_prescricao.hr_referencia_dieta%type;
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	into STRICT	hr_referencia_dieta_w
	from	intervalo_prescricao
	where	cd_intervalo = cd_intervalo_w;
	
	if (hr_referencia_dieta_w in ('__:__','  :  ')) then
		hr_referencia_dieta_w	:= '';
	end if;
	
	ie_continuo_w	:= 'C';
	if	((qt_hora_aplicacao_w*nr_ocorrencia_w) <> 24) then
		ie_continuo_w	:= 'I';
	end if;
	...
	insert into cpoe_dieta(
			...
			values (
				...
				clock_timestamp(),
				cd_perfil_p,
				trunc(qt_hora_aplicacao_w) || ':' || lpad(to_char(mod(qt_hora_aplicacao_w * 60,60)), 2, '0'),
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE cpoe_inserir_enteral_ant ( nr_atendimento_p prescr_medica.nr_atendimento%type, nr_atendimento_ant_p prescr_medica.nr_atendimento%type, nr_prescricao_p prescr_medica.nr_prescricao%type, nr_sequencia_p prescr_material.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type, cd_perfil_p perfil.cd_perfil%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nr_seq_item_gerado_p INOUT bigint, cd_pessoa_fisica_p pessoa_fisica.cd_pessoa_fisica%type, nr_seq_transcricao_p bigint default null, ie_item_alta_p text default 'N', ie_prescritor_aux_p text default 'N', cd_medico_p bigint default null, ie_retrogrado_p text default 'N', dt_inicio_p timestamp default null, nr_seq_pepo_p bigint default null, nr_cirurgia_p bigint default null, nr_cirurgia_patologia_p bigint default null, nr_seq_agenda_p bigint default null, nr_seq_conclusao_apae_p bigint default null, ie_futuro_p text default 'N', nr_seq_cpoe_order_unit_p cpoe_order_unit.nr_sequencia%type default null) FROM PUBLIC;
