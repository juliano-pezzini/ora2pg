...
CREATE OR REPLACE PROCEDURE cpoe_gerar_material_assoc ( nr_atendimento_p bigint, nr_seq_procedimento_p bigint, cd_estabelecimento_p bigint, cd_perfil_p bigint, nm_usuario_p text, cd_setor_atendimento_p bigint, ie_mat_insert_p INOUT text) AS $body$
DECLARE
...
ie_adm_proc_w				cpoe_procedimento.ie_administracao%type;
...
SELECT	a.cd_material,
	...
	coalesce(a.cd_intervalo,cd_interv_proc_w),
	a.ds_observacao,
	a.ds_horarios,
	a.ie_via_aplicacao,
...
select	max(cd_intervalo),
	...
	obter_sinal_vital(max(nr_atendimento),'PESO'),
	coalesce(max(ie_administracao),'P')
...
EXIT WHEN NOT FOUND; /* apply on C01 */	
	...
	if (ie_duplicar_w = 'S') then		
		...
		if (ie_separado_adep_w = 'N' and
			ie_adm_proc_w <> 'P') then
			ie_administracao_w := ie_adm_proc_w;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE cpoe_gerar_material_assoc ( nr_atendimento_p bigint, nr_seq_procedimento_p bigint, cd_estabelecimento_p bigint, cd_perfil_p bigint, nm_usuario_p text, cd_setor_atendimento_p bigint, ie_mat_insert_p INOUT text) FROM PUBLIC;
