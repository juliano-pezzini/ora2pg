...
SELECT	a.nr_item_oci,
	...
	obter_custo_medio_material(cd_estabelecimento_p, clock_timestamp(), a.cd_material) vl_unitario_material,
...
and    	ie_tipo_nota in ('ST', 'SE', 'SF','SD');

if (nr_sequencia_w > 0) then
	-- 956284 - Já existe nota fiscal de saída gerada para esta transferência!
	ds_erro_w	:= WHEB_MENSAGEM_PCK.get_texto(956284);
else
	...
	CALL gerar_historico_nota_fiscal(nr_sequencia_w, nm_usuario_p, '17', WHEB_MENSAGEM_PCK.get_texto(279058));
	...
	if (ie_item_nf_p = 0) then
		...
		EXIT WHEN NOT FOUND; /* apply on c01 */
			...
			if (nr_seq_lote_fornec_w IS NOT NULL AND nr_seq_lote_fornec_w::text <> '') then			
				select	dt_validade,
					coalesce(ds_lote_fornec,''),
					lpad(nr_sequencia || nr_digito_verif,11,0) ds_barra
		...
		insert into nota_fiscal_item(
			...
			nr_item_oci,			dt_entrega_ordem,		
			nr_seq_conta_financ,		pr_desc_financ,			
			nr_seq_lote_fornec,			dt_validade,	
			cd_lote_fabricacao,			ie_indeterminado,
		...
		values (	nr_sequencia_w,			cd_estabelecimento_p,
			...
			vl_liquido_w,			0,	
			nr_item_oci_w,			dt_prevista_entrega_w,		
			nr_seq_conta_financeira_w,		0,			
			nr_seq_lote_fornec_w,		dt_validade_w,		
			ds_lote_fornec_w,			ie_indeterminado_w,
	...
	if (qt_itens_nota_w = 0) then
		...
		nr_sequencia_w	:= 0;
		ds_erro_w	:= WHEB_MENSAGEM_PCK.get_texto(279059);
		end;
	end if;
...
if (coalesce(ds_erro_p::text, '') = '') and (coalesce(ds_erro_item_p::text, '') = '') and (coalesce(ds_erro_nota_p::text, '') = '') then
	CALL gravar_historico_transf_quimio(nm_usuario_p,nr_ordem_compra_p,'N',WHEB_MENSAGEM_PCK.get_texto(279060) || nr_sequencia_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_nf_saida_transf_quimio ( nr_ordem_compra_p bigint, cd_estabelecimento_p bigint, cd_operacao_nf_p bigint, cd_nat_oper_nf_p bigint, cd_serie_nf_p text, nr_nota_fiscal_p text, cd_local_estoque_p bigint, ds_observacao_p text, nm_usuario_p text, ie_item_nf_p text, cd_setor_atendimento_p bigint, nr_sequencia_p INOUT bigint, ds_erro_p INOUT text, ds_erro_item_p INOUT text, ds_erro_nota_p INOUT text) FROM PUBLIC;
