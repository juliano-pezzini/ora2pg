...
CREATE TYPE t_pls_conta_pos_estab_prev AS (	nr_id	t_rowid);
CREATE TYPE t_pls_conta_pos_estab_prev_fat AS (	nr_id	t_nr_sequencia);


CREATE OR REPLACE PROCEDURE ctb_pls_contab_provisao_fat ( nr_lote_contabil_p bigint, nm_usuario_p text, ie_exclusao_p text, ds_retorno_p INOUT text) AS $body$
DECLARE
...
ie_tipo_contab_w		varchar(1);
ie_estorno_anterior_w		varchar(1);
...
cd_conta_taxa_adm_deb_w		pls_conta_pos_taxa_contab.cd_conta_deb%type;
...
cd_conta_taxa_adm_cred_w	pls_conta_pos_taxa_contab.cd_conta_cred%type;
...
vl_taxa_adm_prov_w		pls_conta_pos_taxa_contab.vl_provisao%type;
...
dt_mes_competencia_w		pls_conta_pos_estab_contab.dt_mes_competencia%type;
ie_gerar_previa_pos_w		pls_parametros.ie_gerar_previa_pos%type;
vl_prev_provisao_w		pls_conta_pos_estab_prev.vl_provisao%type;
cd_conta_cred_prev_w		pls_conta_pos_estab_prev.cd_conta_cred_provisao%type;
cd_conta_deb_prev_w		pls_conta_pos_estab_prev.cd_conta_deb_provisao%type;
cd_historico_prev_w		pls_conta_pos_estab_prev.cd_historico_provisao%type;
nr_seq_prev_w			pls_conta_pos_estab_prev.nr_sequencia%type;
nr_lote_contab_prev_prov_w	pls_conta_pos_estab_prev.nr_lote_contabil_prov%type;
nr_lote_contab_fat_prev_w	pls_conta_pos_estab_prev.nr_lote_contab_faturamento%type;
ie_status_faturamento_new_w	pls_log_pos_estabelecido.ie_status_faturamento_new%type;
nr_id_pos_estab_contab_w	oid;
nr_id_log_pos_estab_w		oid;
nr_id_pos_taxa_adm_w		oid;
nr_id_pos_estab_prev_w		oid;
...
type t_nr_sequencia is table of bigint index by integer;
...
pls_conta_pos_estab_prev_w		t_pls_conta_pos_estab_prev;
pls_conta_pos_estab_prev_fat_w		t_pls_conta_pos_estab_prev;
...
c_faturamento CURSOR FOR
	-- Contabilizao quando nao ha previa
	SELECT	'L' ie_tipo_contab,
		CASE WHEN coalesce(d.nr_seq_material,0)=0 THEN 'P'  ELSE 'M' END ,
		e.nr_sequencia,
		e.cd_conta_deb_provisao,
		e.cd_conta_deb_taxa_prov,
		e.cd_conta_deb_dif,
		e.cd_conta_deb_ndc_prov,
		null cd_conta_deb_taxa_adm,
		e.cd_conta_cred_provisao,
		e.cd_conta_cred_taxa_prov,
		e.cd_conta_cred_dif,
		e.cd_conta_cred_ndc_prov,
		null cd_conta_cred_taxa_adm,
		CASE WHEN f.ie_status_faturamento_new='C' THEN -1 WHEN f.ie_status_faturamento_new='N' THEN -1 WHEN f.ie_status_faturamento_new='U' THEN -1  ELSE 1 END  * coalesce(e.vl_provisao,0) vl_provisao,
		...
		CASE WHEN f.ie_status_faturamento_new='C' THEN -1 WHEN f.ie_status_faturamento_new='N' THEN -1 WHEN f.ie_status_faturamento_new='U' THEN -1  ELSE 1 END  * coalesce((pls_obter_vl_pag_fat_pos(e.nr_sequencia,'T'))::numeric , 0) vl_diferenca,
		...
		0 vl_taxa_adm_prov,
		case when f.dt_atualizacao_nrec between dt_referencia_month_w and dt_referencia_fm_w then trunc(f.dt_atualizacao_nrec)
		else trunc(e.dt_mes_competencia) end dt_referencia_mens,
		e.cd_historico_provisao,
		e.cd_historico_taxa_prov,
		e.cd_historico_dif,
		e.cd_historico_ajuste,
		e.cd_historico_ndc_prov,
		null cd_historico_taxa_adm,
		b.nr_sequencia,
		b.nr_seq_segurado,
		b.nr_seq_protocolo,
		...
		trunc(e.dt_mes_competencia),
		e.oid nr_id_pos_estab_contab,
		f.oid nr_id_log_pos_estab,
		null nr_id_pos_taxa_contab,
		b.nr_seq_prestador nr_seq_prestador_solic,
		b.nr_seq_prestador_exec nr_seq_prestador_exec,
		(select	r.nr_seq_prestador_pgto
		 from	pls_conta_medica_resumo r
		 where	r.nr_sequencia = e.nr_seq_conta_resumo
		 and	r.nr_seq_conta = b.nr_sequencia) nr_seq_prestador_pgto,
		a.nr_seq_prestador nr_seq_prestador_atend,
		null nr_id_pos_estab_prev,
		null nr_seq_prev,
		0 vl_provisao_prev,
		null cd_conta_cred_prev,
		null cd_conta_deb_prev,
		null cd_historico_prev,
		null nr_lote_contab_fat_prev,
		f.ie_status_faturamento_new,
		CASE WHEN f.ie_status_faturamento_new='C' THEN 'S' WHEN f.ie_status_faturamento_new='N' THEN 'S' WHEN f.ie_status_faturamento_new='U' THEN 'S'  ELSE 'N' END  ie_estorno,
		null nr_lote_contab_previa_prov
	...
	and	(
			(
				f.dt_atualizacao_nrec between dt_referencia_month_w and dt_referencia_fm_w
			)
			or (
				f.dt_atualizacao_nrec > e.dt_mes_competencia
				and
				e.dt_mes_competencia between dt_referencia_month_w and dt_referencia_fm_w
			)	
		)
	...
	and	coalesce(d.ie_situacao,'A')	= 'A'
        and     coalesce(d.ie_status_faturamento, 'A') <> 'A'
	and	coalesce((	select	max(r.ie_situacao)
			from	pls_conta_medica_resumo r
			where	b.nr_sequencia	= r.nr_seq_conta
			and	r.nr_sequencia	= e.nr_seq_conta_resumo),'X')	<> 'I'
	and	coalesce(e.nr_seq_previa_pos, 0) = 0
	
union all

	-- Contabilizacao quando ha previa gerada
	select	'L' ie_tipo_contab,
		CASE WHEN coalesce(d.nr_seq_material,0)=0 THEN 'P'  ELSE 'M' END ,
		e.nr_sequencia,
		e.cd_conta_deb_provisao,
		...
		a.nr_seq_prestador nr_seq_prestador_atend,
		null nr_id_pos_estab_prev,
		g.nr_sequencia nr_seq_prev,
		coalesce(g.vl_provisao, 0) vl_provisao_prev,
		g.cd_conta_cred_provisao cd_conta_cred_prev,
		g.cd_conta_deb_provisao cd_conta_deb_prev,
		g.cd_historico_provisao cd_historico_prev,
		g.nr_lote_contab_faturamento nr_lote_contab_fat_prev,
		f.ie_status_faturamento_new,
		CASE WHEN f.ie_status_faturamento_new='C' THEN 'S' WHEN f.ie_status_faturamento_new='N' THEN 'S' WHEN f.ie_status_faturamento_new='U' THEN 'S'  ELSE 'N' END  ie_estorno,
		g.nr_lote_contabil_prov nr_lote_contab_previa_prov
	from	pls_conta_pos_estab_prev	g,
	...
	and	a.nr_sequencia		= b.nr_seq_protocolo
	and	s.nr_sequencia 		= b.nr_seq_segurado
	and	e.nr_seq_previa_pos	= g.nr_sequencia
	and	(
			(
				f.dt_atualizacao_nrec between dt_referencia_month_w and dt_referencia_fm_w
			)
...
union all
	...
	-- Taxa de intercambio de protocolos de contas medicas
	select	'T' ie_tipo_contab,
		CASE WHEN coalesce(d.nr_seq_material,0)=0 THEN 'P'  ELSE 'M' END ,
		t.nr_sequencia,
		null cd_conta_deb_provisao,
		null cd_conta_deb_taxa_prov,
		null cd_conta_deb_dif,
		null cd_conta_deb_ndc_prov,
		c.cd_conta_deb cd_conta_deb_taxa_adm,
		null cd_conta_cred_provisao,
		null cd_conta_cred_taxa_prov,
		null cd_conta_cred_dif,
		null cd_conta_cred_ndc_prov,
		c.cd_conta_cred cd_conta_cred_taxa_adm,
		0 vl_provisao,
		0 vl_administracao,
		0 vl_diferenca,
		0 vl_reembolso,
		0 vl_contab_ajuste,
		CASE WHEN f.ie_status_faturamento_new='C' THEN -1 WHEN f.ie_status_faturamento_new='N' THEN -1 WHEN f.ie_status_faturamento_new='U' THEN -1  ELSE 1 END  * coalesce(c.vl_provisao,0) vl_taxa_adm_prov,
		case when f.dt_atualizacao_nrec between dt_referencia_month_w and dt_referencia_fm_w then trunc(f.dt_atualizacao_nrec)
		else trunc(e.dt_mes_competencia) end dt_referencia_mens,
		null cd_historico_provisao,
		null cd_historico_taxa_prov,
		null cd_historico_dif,
		null cd_historico_ajuste,
		null cd_historico_ndc_prov,
		c.cd_historico cd_historico_taxa_adm,
		b.nr_sequencia,
		b.nr_seq_segurado,
		b.nr_seq_protocolo,
		b.nr_seq_prestador_exec,
		...
		coalesce((	select	max(r.nm_prestador_pgto)
			...
			and	d.nr_sequencia	= e.nr_seq_conta_pos ), b.nr_seq_prestador_exec) nm_prestador_pgto,
		substr(pls_obter_dados_segurado(b.nr_seq_segurado,'N'),1,255) nm_segurado,
		d.nr_sequencia,
		null,
		null nr_lote_contabil_prov,
		trunc(e.dt_mes_competencia),
		null nr_id_pos_estab_contab,
		null nr_id_log_pos_estab,
		c.oid nr_id_pos_taxa_contab,
		b.nr_seq_prestador nr_seq_prestador_solic,
		b.nr_seq_prestador_exec nr_seq_prestador_exec,
		r.nr_seq_prestador_pgto nr_seq_prestador_pgto,
		a.nr_seq_prestador nr_seq_prestador_atend,
		null nr_id_pos_estab_prev,
		null nr_seq_prev,
		null vl_provisao_prev,
		null cd_conta_cred_prev,
		null cd_conta_deb_prev,
		null cd_historico_prev,
		null nr_lote_contab_fat_prev,
		null ie_status_faturamento_new,
		null ie_estorno,
		null nr_lote_contab_previa_prov
	FROM pls_segurado s, pls_conta_medica_resumo r, pls_log_pos_estabelecido f, pls_conta_pos_estab_contab e, pls_conta b, pls_protocolo_conta a, pls_conta_pos_estabelecido d
LEFT OUTER JOIN pls_conta_pos_estab_taxa t ON (d.nr_sequencia = t.nr_seq_conta_pos_estab)
LEFT OUTER JOIN pls_conta_pos_taxa_contab c ON (t.nr_sequencia = c.nr_seq_pos_estab_taxa)
WHERE b.nr_sequencia		= d.nr_seq_conta and a.nr_sequencia		= b.nr_seq_protocolo and s.nr_sequencia 		= b.nr_seq_segurado   and r.nr_seq_conta		= b.nr_sequencia and r.nr_sequencia		= c.nr_seq_conta_resumo and c.nr_seq_conta_resumo	= e.nr_seq_conta_resumo and e.nr_seq_conta_pos	= d.nr_sequencia and d.nr_sequencia		= f.nr_seq_conta_pos and b.nr_sequencia		= f.nr_seq_conta and (
			(
				f.dt_atualizacao_nrec between dt_referencia_month_w and dt_referencia_fm_w
			)
			or (
  ...
  and ((ie_status_prov_pagto_w <> 'F') or (ie_status_prov_pagto_w = 'F' and b.ie_status = 'F')) and coalesce(f.nr_lote_contabil, 0) = 0 and a.cd_estabelecimento	= cd_estabelecimento_w and a.ie_situacao		!= 'RE' and coalesce(d.ie_situacao,'A')	= 'A' and coalesce(d.ie_status_faturamento, 'A') <> 'A' and coalesce((	select	max(r.ie_situacao)
			from	pls_conta_medica_resumo		r,
				pls_conta_pos_estab_contab 	e
			where	b.nr_sequencia	= r.nr_seq_conta
			and	r.nr_sequencia	= e.nr_seq_conta_resumo
			and	d.nr_sequencia	= e.nr_seq_conta_pos),'X')	<> 'I' and ie_lote_taxa_adm_w	= 'P' and a.ie_tipo_protocolo	= 'C'
	
union all

	-- Taxa de intercambio de protocolos de intercambio
	select	'T' ie_tipo_contab,
		CASE WHEN coalesce(d.nr_seq_material,0)=0 THEN 'P'  ELSE 'M' END ,
		t.nr_sequencia,
		null cd_conta_deb_provisao,
		...
		d.nr_seq_material,
		b.nr_seq_plano,
		pls_obter_dados_prestador(b.nr_seq_prestador_exec, 'N') nm_prestador_pgto,
		substr(pls_obter_dados_segurado(b.nr_seq_segurado,'N'),1,255) nm_segurado,
		d.nr_sequencia,
		null,
		null nr_lote_contabil_prov,
		...
		null ie_estorno,
		null nr_lote_contab_previa_prov
	FROM pls_segurado s, pls_log_pos_estabelecido f, pls_conta_pos_estab_contab e, pls_conta b, pls_protocolo_conta a, pls_conta_pos_estabelecido d
LEFT OUTER JOIN pls_conta_pos_estab_taxa t ON (d.nr_sequencia = t.nr_seq_conta_pos_estab)
LEFT OUTER JOIN pls_conta_pos_taxa_contab c ON (t.nr_sequencia = c.nr_seq_pos_estab_taxa)
WHERE b.nr_sequencia		= d.nr_seq_conta and a.nr_sequencia		= b.nr_seq_protocolo and s.nr_sequencia 		= b.nr_seq_segurado   and e.nr_seq_conta_pos	= d.nr_sequencia and d.nr_sequencia		= f.nr_seq_conta_pos and b.nr_sequencia		= f.nr_seq_conta and (
			(
				f.dt_atualizacao_nrec between dt_referencia_month_w and dt_referencia_fm_w
			)
			or (
  ...
  and ((ie_status_prov_pagto_w <> 'F') or (ie_status_prov_pagto_w = 'F' and b.ie_status = 'F')) and coalesce(f.nr_lote_contabil, 0) = 0 and a.cd_estabelecimento	= cd_estabelecimento_w and a.ie_situacao		!= 'RE' and coalesce(d.ie_situacao,'A')	= 'A' and coalesce(d.ie_status_faturamento, 'A') <> 'A' and ie_lote_taxa_adm_w	= 'P' and a.ie_tipo_protocolo	= 'I'
	 
union all

	/* Previsao do pos*/

	select	'P' ie_tipo_contab,
		CASE WHEN coalesce(e.nr_seq_material,0)=0 THEN 'P'  ELSE 'M' END ,
		e.nr_sequencia, --Validar
		e.cd_conta_deb_provisao,
		null,
		null,
		null,
		...
		null,
		null cd_conta_cred_taxa_adm,
		coalesce(e.vl_provisao,0) vl_provisao,
		0 vl_administracao,
		0 vl_diferenca,
		0 vl_reembolso,
		0 vl_contab_ajuste,
		0 vl_taxa_adm_prov,
		trunc(e.dt_mes_competencia) dt_referencia_mens,
		e.cd_historico_provisao,
		null,
		null,
		null,
		...
		null ie_estorno,
		null nr_lote_contab_previa_prov
	from	pls_conta_pos_estab_prev 	e,
		pls_conta			b,
		pls_protocolo_conta		a,
		pls_segurado			s
	where	b.nr_sequencia				= e.nr_seq_conta
	and	a.nr_sequencia				= b.nr_seq_protocolo
	and	s.nr_sequencia 				= b.nr_seq_segurado
	and	e.dt_mes_competencia 			between dt_referencia_month_w and dt_referencia_fm_w
	and	a.cd_estabelecimento			= cd_estabelecimento_w
	and	b.ie_status 				<> 'C' --Conta nao cancelada
	and	e.ie_status_faturamento			in ('L', 'P') -- Liberado para faturamento
	and	coalesce(e.nr_lote_contabil_prov, 0)		= 0
	and	a.ie_situacao				<> 'RE'
	and	ie_status_prov_pagto_w 			<> 'F'
	and	(
			(
				b.dt_fechamento_conta	> dt_referencia_fm_w
			and	
				b.ie_status = 'F'
			)		 --Se a conta estiver fechada, precisa ser em mes posterior a geracao do lote
		or (
				b.ie_status <> 'F'
			)
		)
	...
	and 	not exists	(select 1
				from	pls_conta_pos_estab_contab p,
					pls_conta_pos_estabelecido d,
					pls_log_pos_estabelecido f
				where	p.nr_seq_conta 		= e.nr_seq_conta
				and	p.nr_seq_previa_pos 	= e.nr_sequencia
				and	p.nr_seq_conta_pos 	= d.nr_sequencia
				and	f.nr_seq_conta 		= p.nr_seq_conta
				and	f.nr_seq_conta_pos 	= d.nr_sequencia
				and	((f.dt_atualizacao_nrec between dt_referencia_month_w and dt_referencia_fm_w)
					or (f.dt_atualizacao_nrec > p.dt_mes_competencia and p.dt_mes_competencia between dt_referencia_month_w and dt_referencia_fm_w))
				and	coalesce(f.ie_tipo_registro,'H') = 'H' -- Historico
				and	coalesce(f.nr_lote_contabil, 0) = 0
...
union all
	...
	select	'P' ie_tipo_contab,
		...
		null,
		null cd_conta_cred_taxa_adm,
		coalesce(e.vl_provisao,0) * -1 vl_provisao,
		0 vl_administracao,
		0 vl_diferenca,
		0 vl_reembolso,
		0 vl_contab_ajuste,
		0 vl_taxa_adm_prov,
		trunc(b.dt_fechamento_conta) dt_referencia_mens,
		e.cd_historico_provisao,
		null,
		null,
		null,
	...
	and	b.ie_status				= 'F'
	and	ie_gerar_previa_pos_w			= 'S'
	and	b.dt_fechamento_conta			between dt_referencia_month_w and dt_referencia_fm_w
	and	e.dt_mes_competencia			not between dt_referencia_month_w and dt_referencia_fm_w
	and	coalesce(e.nr_lote_contabil_prov, 0)		<> 0
	and	coalesce(e.nr_lote_contab_faturamento, 0) 	= 0
	and 	(not exists (select	1
				from	pls_conta_pos_estab_contab p
				where	p.nr_seq_conta 		= e.nr_seq_conta
				and	p.nr_seq_previa_pos 	= e.nr_sequencia)
		...
		exists 		(select 1
				...
				and	f.nr_seq_conta_pos 	= d.nr_sequencia
				and	((f.dt_atualizacao_nrec between dt_referencia_month_w and dt_referencia_fm_w)
					or (f.dt_atualizacao_nrec > e.dt_mes_competencia and e.dt_mes_competencia between dt_referencia_month_w and dt_referencia_fm_w))
				and 	coalesce(f.ie_status_faturamento_ant, 'X') = 'X'
...
union
	...
	select	'PP' --Provisao da previa do pos
	
	where (ie_lote_taxa_fat_w	not in ('P','A')
	and	ie_lote_reembolso_fat_w	not in ('P','A'))
	and	ie_tipo_contab_w = 'P'
...
if (ie_exclusao_p <> 'S') then
        ...
        if (cd_tipo_lote_contabil_w <> 43) then
...
select	max(coalesce(ie_gerar_previa_pos, 'N'))
...
if (ie_exclusao_p = 'S') then
	...
	update	pls_conta_pos_estab_prev
	set	nr_lote_contab_faturamento 	= 0,
		dt_atualizacao			= clock_timestamp(),
		nm_usuario 			= nm_usuario_p
	...
	fetch c_faturamento into
		...
		cd_conta_dif_deb_w,
		cd_conta_ndc_deb_w,
		cd_conta_taxa_adm_deb_w,
		cd_conta_prov_cred_w,
		cd_conta_taxa_cred_w,
		cd_conta_dif_cred_w,
		cd_conta_ndc_cred_w,
		cd_conta_taxa_adm_cred_w,
		vl_provisao_w,
		vl_administracao_w,
		vl_diferenca_w,
		vl_reembolso_w,
		...
		nr_id_pos_taxa_adm_w,
		nr_seq_prestador_solic_w,
		nr_seq_prestador_exec_w,
		nr_seq_prestador_pag_w,
		nr_seq_prestador_atend_w,
		nr_id_pos_estab_prev_w,
		nr_seq_prev_w,
		vl_prev_provisao_w,
		cd_conta_cred_prev_w,
		cd_conta_deb_prev_w,
		cd_historico_prev_w,
		nr_lote_contab_fat_prev_w,
		ie_status_faturamento_new_w,
		ie_estorno_w,
		nr_lote_contab_prev_prov_w;
	EXIT WHEN NOT FOUND; /* apply on c_faturamento */
		begin
		
		select	coalesce(max('S'), 'N')
		into STRICT	ie_estorno_anterior_w
		from 	pls_log_pos_estabelecido f
		where 	nr_seq_conta_pos_w	= f.nr_seq_conta_pos
		and	nr_seq_conta_w		= f.nr_seq_conta
		and	f.nr_sequencia 		< nr_seq_log_w
		and	f.dt_atualizacao_nrec between dt_referencia_month_w and dt_referencia_fm_w
		and	coalesce(f.ie_tipo_registro,'H') = 'H'
		and	coalesce(f.nr_lote_contabil, nr_lote_contabil_p) in (0, nr_lote_contabil_p)
		and	f.ie_status_faturamento_new in ('C','N','U')
		and	ie_status_faturamento_new_w not in ('C', 'N', 'U');
		...
		EXIT WHEN NOT FOUND; /* apply on c_tipo_movimento */
			...
			nm_tabela_w		:= '';
			nm_atributo_w		:= '';
			nr_seq_info_ctb_w	:= null;
			
			if (ie_movimento_w = 'P') then /* Provisao */
				/* Movimentos que nao sao previa 
				   OU ja tem um numero gravado na realizacao do ajuste e portanto precisa contabilizar o valor inteiro para qualquer alteracao futura 
				   OU existe um estorno que vai ser contabilizado no mesmo lote contabil, entao contabiliza o valor inteiro
				   OU eh um estorno, e contabiliza o valor inteiro tambem
				   
				   Se nao for ajuste da previa, entra pela primeira verificacao, caso seja previa precisa se encaixar em uma das demais verificacoes
				   */
				if 	((vl_prev_provisao_w = 0 and coalesce(nr_seq_prev_w, 0) = 0)
					or (coalesce(nr_lote_contab_fat_prev_w, 0) <> 0)
					or (ie_estorno_anterior_w = 'S')
					or (ie_estorno_w = 'S')) then
				...
				/* Existe previa e pos com valores diferentes, ainda nao foi gravado o lote de ajuste da previa, e ja foi gravado o lote contabil da previa
				
				So vai cair nesse if quando for ajuste da previa, pois no union "normal" traz o campo nr_lote_contab_prev_prov = 0
				 */
				elsif (vl_prev_provisao_w <> vl_provisao_w and coalesce(nr_lote_contab_fat_prev_w, 0) = 0) and  coalesce(nr_lote_contab_prev_prov_w, 0) <> 0 then
					cd_conta_debito_w 	:= cd_conta_deb_prev_w;
					cd_conta_credito_w	:= cd_conta_cred_prev_w;
					cd_historico_w		:= cd_historico_prev_w;
					vl_contabil_w		:= vl_provisao_w - vl_prev_provisao_w;
					nm_tabela_w		:= 'PLS_CONTA_POS_ESTAB_CONTAB';
					nm_atributo_w		:= 'VL_AJUSTE';
					nr_seq_info_ctb_w	:= 32;
				/* Existe previa e pos, a previa ainda nao foi contabilizada */

				elsif (coalesce(nr_seq_prev_w, 0) <> 0 and coalesce(nr_lote_contab_prev_prov_w, 0) = 0) then
					cd_conta_debito_w	:= cd_conta_prov_deb_w;
					cd_conta_credito_w	:= cd_conta_prov_cred_w;				
					cd_historico_w		:= cd_historico_prov_w;
					vl_contabil_w		:= vl_provisao_w;
			...
			elsif (ie_movimento_w = 'PP') then /* Provisao da previsao do pos */
				cd_conta_debito_w	:= cd_conta_prov_deb_w;
				cd_conta_credito_w	:= cd_conta_prov_cred_w;				
				cd_historico_w		:= cd_historico_prov_w;
				vl_contabil_w		:= vl_provisao_w;
				nm_tabela_w		:= 'PLS_CONTA_POS_ESTAB_PREV';
				nm_atributo_w		:= 'VL_LIBERADO';
				nr_seq_info_ctb_w	:= 32;
			...
			elsif (ie_movimento_w = 'D') then /* Diferenca */
			...
			elsif (ie_movimento_w = 'TA') then /* Taxa administrativa */
				cd_conta_debito_w	:= cd_conta_taxa_adm_deb_w;
				cd_conta_credito_w	:= cd_conta_taxa_adm_cred_w;
				cd_historico_w		:= cd_historico_taxa_adm_w;				
				vl_contabil_w		:= vl_taxa_adm_prov_w;
				nm_tabela_w		:= 'PLS_CONTA_POS_TAXA_CONTAB';
				nm_atributo_w		:= 'VL_TAXA_ADM_PROV';
				nr_seq_info_ctb_w	:= 32;
			end if;
			...
			if (coalesce(cd_conta_debito_w::text, '') = '') or (coalesce(cd_conta_credito_w::text, '') = '') then
				...
				if (coalesce(cd_conta_debito_w::text, '') = '') then				
					...
					values (nextval('w_pls_movimento_sem_conta_seq'),
						...
						ie_proc_mat_w,
						'D',
						wheb_mensagem_pck.get_texto(300343), -- Provisao de faturamento
						cd_tipo_lote_contabil_w);
				end if;
...
union all
				...
				if (cd_conta_debito_w IS NOT NULL AND cd_conta_debito_w::text <> '') then
					...
					if (ie_centro_custo_deb_w = 'S') then
						SELECT * FROM pls_obter_centro_custo(	'D', nr_seq_plano_w, cd_estabelecimento_w, '', '', ie_regulamentacao_w, nr_seq_segurado_w, '', cd_centro_debito_w, nr_seq_regra_cc_w, null, 				--nr_seq_tipo_lanc_p
									...
									) INTO STRICT cd_centro_debito_w, nr_seq_regra_cc_w;
					end if;
					...
					if (item_mov_contabil_w.vl_movimento <> 0) then
				...
				if (cd_conta_credito_w IS NOT NULL AND cd_conta_credito_w::text <> '') then
					...
					if (ie_centro_custo_cred_w = 'S') then
						SELECT * FROM pls_obter_centro_custo(	'D', nr_seq_plano_w, cd_estabelecimento_w, '', '', ie_regulamentacao_w, nr_seq_segurado_w, '', cd_centro_credito_w, nr_seq_regra_cc_w, null, 				--nr_seq_tipo_lanc_p
									null, 				--nr_seq_bonificacao_p
									null, 				--nr_seq_sca_p
									nr_seq_prestador_pag_w, nr_seq_prestador_exec_w, nr_seq_prestador_atend_w, nr_seq_prestador_solic_w, null				-- nr_seq_evento_p
									) INTO STRICT cd_centro_credito_w, nr_seq_regra_cc_w;
					end if;
		...
		if (ie_tipo_contab_w = 'L') then
			...
			if (coalesce(nr_lote_contab_fat_prev_w, 0) = 0) then
				pls_conta_pos_estab_prev_fat_w.nr_id(pls_conta_pos_estab_prev_w.nr_id.count) 	:= nr_seq_prev_w;

				forall i in pls_conta_pos_estab_prev_fat_w.nr_id.first .. pls_conta_pos_estab_prev_fat_w.nr_id.last
					update	pls_conta_pos_estab_prev	a
					set	a.nr_lote_contab_faturamento	= nr_lote_contabil_p
					where	nr_sequencia			= coalesce(pls_conta_pos_estab_prev_fat_w.nr_id(i), 0);
				commit;
				pls_conta_pos_estab_prev_fat_w.nr_id.delete;
		...
		elsif (ie_tipo_contab_w = 'P') then
			pls_conta_pos_estab_prev_w.nr_id(pls_conta_pos_estab_prev_w.nr_id.count) 	:= nr_id_pos_estab_prev_w;
			if (pls_conta_pos_estab_prev_w.nr_id.count >= 1000) then
				begin
				forall i in pls_conta_pos_estab_prev_w.nr_id.first .. pls_conta_pos_estab_prev_w.nr_id.last
					update	pls_conta_pos_estab_prev	a
					set	a.nr_lote_contabil_prov		= nr_lote_contabil_p
					where	rowid				= pls_conta_pos_estab_prev_w.nr_id(i);
				commit;
				pls_conta_pos_estab_prev_w.nr_id.delete;
				end;
			end if;	
		else
			pls_conta_pos_taxa_contab_w.nr_id(pls_conta_pos_taxa_contab_w.nr_id.count)	:= nr_id_pos_taxa_adm_w;
			...
			if (pls_conta_pos_taxa_contab_w.nr_id.count >= 1000) then
				...
				forall i in pls_conta_pos_taxa_contab_w.nr_id.first .. pls_conta_pos_taxa_contab_w.nr_id.last
					update	pls_conta_pos_taxa_contab	a
					set	a.nr_lote_contabil_prov		= nr_lote_contabil_p
	...
	if (pls_conta_pos_estab_prev_w.nr_id.count > 0) then
		begin
		forall i in pls_conta_pos_estab_prev_w.nr_id.first .. pls_conta_pos_estab_prev_w.nr_id.last
			update	pls_conta_pos_estab_prev	a
			set	a.nr_lote_contabil_prov		= nr_lote_contabil_p
	...
	if (pls_conta_pos_estab_prev_fat_w.nr_id.count > 0) then
		begin
		forall i in pls_conta_pos_estab_prev_fat_w.nr_id.first .. pls_conta_pos_estab_prev_fat_w.nr_id.last
			update	pls_conta_pos_estab_prev	a
			set	a.nr_lote_contab_faturamento	= nr_lote_contabil_p
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ctb_pls_contab_provisao_fat ( nr_lote_contabil_p bigint, nm_usuario_p text, ie_exclusao_p text, ds_retorno_p INOUT text) FROM PUBLIC;
