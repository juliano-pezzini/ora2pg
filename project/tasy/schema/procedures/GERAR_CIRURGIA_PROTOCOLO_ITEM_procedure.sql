...
CREATE OR REPLACE PROCEDURE gerar_cirurgia_protocolo_item (nr_cirurgia_p bigint, nr_seq_protocolo_p bigint, nr_seq_item_p bigint, ie_tipo_p text, dt_inicio_p timestamp, nr_seq_pepo_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ds_just_alergico_p text default null) AS $body$
DECLARE
...
ie_lado_disp_prot_w	varchar(1);
ie_acao_disp_prot_w	varchar(15);
nr_seq_sv_protocolo_w	bigint;
vl_padrao_sv_protocolo_w varchar(30);
nr_seq_sv_w		bigint;
nr_seq_mon_resp_w	bigint;
nr_seq_anal_w		bigint;
nr_atendimento_w	bigint;
nm_tabela_w		varchar(255);
nr_sequencia_www	bigint;
ie_inicio_equip_w	varchar(1);
nr_atend_monit_hemod_w	bigint; --atend_monit_hemod.nr_sequencia%type;
nr_seq_derivado_w 	protocolo_hemoderivados.nr_seq_derivado%type;
qt_material_w 		protocolo_hemoderivados.qt_material%type;
...
C02 CURSOR FOR
	...
	where	nr_seq_protocolo	=	nr_seq_protocolo_p
	and	cd_equipamento		= 	nr_seq_item_p
	and	ie_tipo_p		= 	'E'
	order by nr_sequencia;
...
C08 CURSOR(cd_pessoa_fisica_paciente_p text) FOR
...
C09 CURSOR(cd_pessoa_fisica_paciente_p text) FOR
...
update evento_cirurgia_paciente set ie_protocolo = 'S', nr_protocolo_pepo = nr_seq_protocolo_p where nr_sequencia = nr_seq_evento_w;
open C01;
loop
fetch C01 into	
	ie_posicao_w;
...
EXIT WHEN NOT FOUND; /* apply on C04 */
	...
	nr_sequencia_ww := gerar_agentes_medicamentos(cd_estabelecimento_p, nr_seq_agente_w, nr_cirurgia_p, cd_material_w, cd_pessoa_fisica_w, nm_usuario_p, nr_sequencia_ww, null, null, null, nr_seq_pepo_w, null, null, ds_just_alergico_p);
	...
	update cirurgia_agente_anestesico set nr_protocolo_pepo = nr_seq_protocolo_p, nr_seq_evento = nr_seq_evento_w where nr_sequencia = nr_sequencia_ww;
...
EXIT WHEN NOT FOUND; /* apply on C06 */
	begin
	CALL Registrar_disp_protocolo_cir(nr_seq_dispositivo_w,ie_lado_disp_prot_w,ie_acao_disp_prot_w,'N',nr_cirurgia_p,nm_usuario_p,nr_seq_pepo_p,dt_inicio_p);
	end;
end loop;
close C06;

select nextval('atendimento_sinal_vital_seq')
into STRICT nr_seq_sv_w
;

select nextval('atendimento_monit_resp_seq')
into STRICT	nr_seq_mon_resp_w
;

select nextval('atend_anal_bioq_port_seq')
into STRICT   nr_seq_anal_w
;

select nextval('atend_monit_hemod_seq')
into STRICT   nr_atend_monit_hemod_w
;

open C07;
loop
fetch C07 into	
	nr_seq_sv_protocolo_w,
	vl_padrao_sv_protocolo_w;
EXIT WHEN NOT FOUND; /* apply on C07 */
	begin
	SELECT * FROM registrar_sv_pepo(nr_cirurgia_p, nr_atendimento_w, dt_inicio_p, nr_seq_sv_w, nr_seq_mon_resp_w, nr_seq_anal_w, nr_seq_sv_protocolo_w, vl_padrao_sv_protocolo_w, nm_usuario_p, nr_seq_pepo_p, nm_tabela_w, nr_sequencia_www, null, 'N', nr_atend_monit_hemod_w) INTO STRICT nm_tabela_w, nr_sequencia_www;
	end;
end loop;
close C07;
...
if (ie_tipo_evolucao_w IS NOT NULL AND ie_tipo_evolucao_w::text <> '') then
	open C08(cd_pessoa_fisica_paciente_w);
	loop
	fetch C08 into	
...
open C09(cd_pessoa_fisica_paciente_w);
loop
fetch C09 into	
		nr_seq_derivado_w,
		qt_material_w,
...
EXIT WHEN NOT FOUND; /* apply on C09 */
	...
	insert into cirurgia_agente_anestesico(
		...
		values (
			...
			nr_seq_evento_w,
			nr_seq_protocolo_p);
		if (qt_dose_ww IS NOT NULL AND qt_dose_ww::text <> '') then
			insert into CIRURGIA_AGENTE_ANEST_OCOR(
				nr_sequencia,
				dt_atualizacao,
				nm_usuario,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_cirurgia_protocolo_item (nr_cirurgia_p bigint, nr_seq_protocolo_p bigint, nr_seq_item_p bigint, ie_tipo_p text, dt_inicio_p timestamp, nr_seq_pepo_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ds_just_alergico_p text default null) FROM PUBLIC;
