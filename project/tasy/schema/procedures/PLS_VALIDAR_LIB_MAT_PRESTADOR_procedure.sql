...
CREATE OR REPLACE PROCEDURE pls_validar_lib_mat_prestador ( nr_seq_prestador_p bigint, nr_seq_prest_fornec_p bigint, nr_seq_material_p bigint, dt_atendimento_p timestamp, cd_estabelecimento_p bigint, nr_seq_prestador_exec_p bigint, ie_internacao_p text, ie_proc_liberado_p INOUT text, nr_seq_regra_p INOUT bigint) AS $body$
DECLARE


/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: Validar se o material esta liberado para o prestador
-------------------------------------------------------------------------------------------------------------------
...
Pontos de atencao:
	QUALQUER ALTERACAO FEITA NESTA PROCEDURE TAMBEM DEVERA
	SER FEITA NA FUNCTION  'PLS_OBTER_SE_MAT_PRESTADOR'
-------------------------------------------------------------------------------------------------------------------

Referencias:
	PLS_GERAR_ITENS_LOTE_EXECUCAO
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
...
C01 CURSOR FOR
	...
	and	dt_atendimento_p between trunc(coalesce(a.dt_inicio_vigencia, dt_atendimento_p), 'dd') and fim_dia(coalesce(a.dt_fim_vigencia, dt_atendimento_p))
	and (nr_seq_tipo_prestador = coalesce(nr_seq_tipo_prestador_w,nr_seq_tipo_prestador) 	or coalesce(a.nr_seq_tipo_prestador::text, '') = '')
	and (a.ie_internado = coalesce(ie_internacao_p, a.ie_internado) or (coalesce(a.ie_internado::text, '') = '') or (a.ie_internado = 'N'))
	and	((exists (SELECT	1
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_validar_lib_mat_prestador ( nr_seq_prestador_p bigint, nr_seq_prest_fornec_p bigint, nr_seq_material_p bigint, dt_atendimento_p timestamp, cd_estabelecimento_p bigint, nr_seq_prestador_exec_p bigint, ie_internacao_p text, ie_proc_liberado_p INOUT text, nr_seq_regra_p INOUT bigint) FROM PUBLIC;
