...
CREATE OR REPLACE PROCEDURE gerar_os_reteste_proj_migr ( nr_seq_projeto_p bigint, dt_referencia_p timestamp, nm_usuario_p text) AS $body$
DECLARE
...
where	nr_seq_ordem_serv = nr_seq_os_cor_teste_w
and	nr_seq_tipo in (68,53,49)
order by
	dt_historico desc;
...
if (nr_seq_projeto_p IS NOT NULL AND nr_seq_projeto_p::text <> '') and (nm_usuario_p IS NOT NULL AND nm_usuario_p::text <> '') then
	...
	and	ie_tipo_ordem = 'T';*/
	/*select	nvl(max(nr_seq_os),0)
	into	nr_seq_os_revisao_w
	...
	values (
		...
		1272,
		41,
		'Migração: reteste do projeto ' || obter_form_funcao(cd_funcao_projeto_w),
		'A',
		ds_dano_w,
		cd_funcao_projeto_w,
		nr_seq_grupo_des_w,
	...
	open c00;
	loop
	fetch c00 into nr_seq_os_cor_teste_w;
	EXIT WHEN NOT FOUND; /* apply on c00 */
		...
		where	nr_seq_ordem = nr_seq_os_cor_teste_w;
		end;
	end loop;
	close c00;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_os_reteste_proj_migr ( nr_seq_projeto_p bigint, dt_referencia_p timestamp, nm_usuario_p text) FROM PUBLIC;
