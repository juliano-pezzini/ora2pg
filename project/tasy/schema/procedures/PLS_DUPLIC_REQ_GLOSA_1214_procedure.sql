...
CREATE OR REPLACE PROCEDURE pls_duplic_req_glosa_1214 ( nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE

				
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade:	Duplicar a requisicao passada por parametro apenas com os itens que foram glosados com a Glosa 1214 - Credenciado
		e, por fim, consistir a requisicao gerada
nao habilitado a realizar o procedimento. Caso o cabecalho da requisicao possuir glosa ou ocorrencia, a requisicao nao e duplicada.
-------------------------------------------------------------------------------------------------------------------
...
ds_historico_w			pls_requisicao_historico.ds_historico%type;
...
-- Verificando se existe algum procedimento com a Glosa '1214' e nenhuma outra Glosa ou ocorrencia.
for r_c05_w in C05 loop
	select	count(1)
	into STRICT	qt_glosa_w
	from	pls_requisicao_glosa
	where	nr_seq_req_proc		= r_c05_w.nr_sequencia
	and	nr_seq_motivo_glosa	= 514 --Glosa 1214
	and	0 =	(SELECT	count(1)
			from	pls_requisicao_glosa
			where	nr_seq_req_proc	= r_c05_w.nr_sequencia
			and	nr_seq_motivo_glosa	<> 514)
	and	0 =	(select	count(1)
			from	pls_ocorrencia_benef
			where	nr_seq_proc	= r_c05_w.nr_sequencia);

	qt_glosa_total_w	:= qt_glosa_total_w + qt_glosa_w;
end loop;

-- Verificando se existe algum material com a Glosa '1214' e nenhuma outra Glosa ou ocorrencia.
for r_c10_w in C10 loop
	...
	and	0 =	(select	count(1)
			from	pls_ocorrencia_benef
			where	nr_seq_mat	= r_c10_w.nr_sequencia);

	qt_glosa_total_w	:= qt_glosa_total_w + qt_glosa_w;
end loop;

--Conta quantas glosas tem no cabecalho.
select	count(1)
into STRICT	qt_glosa_w
from	pls_requisicao_glosa
where	nr_seq_requisicao	= nr_seq_requisicao_p
...
-- Se nao encontrar glosas no cabecalho entao procurar por ocorrencias, pois, se houver, tambem nao pode gerar a nova requisicao.
...
-- A requisicao so e duplicada se houverem itens com a glosa '1214' e nao houver glosa ou ocorrencia no cabecalho.
if (qt_glosa_total_w	> 0) and (qt_glosa_w	= 0) then
	...
	if (coalesce(nr_seq_requisicao_p,0)	<> 0) then
		...
		insert	into pls_requisicao(nr_sequencia, dt_requisicao, dt_atualizacao,
			...
			dt_nasc_recem_nascido, ie_tipo_atendimento, dt_internacao_sugerida,
			...
			nr_seq_cbo_saude, ie_saude_ocupacional, ie_regime_atendimento, ie_cobertura_especial)
		(SELECT	nr_seq_requisicao_w, clock_timestamp(), clock_timestamp(),
			nm_usuario_p, clock_timestamp(), nm_usuario_p,
			a.nr_seq_segurado, a.cd_medico_solicitante, 'P',
			nr_seq_plano_w, cd_estabelecimento_p, a.ie_tipo_guia,
			1, 'M', a.nr_seq_motivo_inclusao,
			a.nr_seq_prestador, a.qt_dia_solicitado, a.qt_dia_autorizado,
			a.cd_especialidade, a.ie_tipo_consulta, a.nm_recem_nascido,
			a.dt_nasc_recem_nascido, a.ie_tipo_atendimento, a.dt_internacao_sugerida,
			a.ie_carater_atendimento, a.ds_indicacao_clinica, a.nr_seq_clinica,
			a.ie_regime_internacao, a.nr_seq_tipo_acomodacao, a.dt_entrada_hospital,
			a.dt_alta_hospital, a.nr_sequencia, a.ie_tipo_saida,
			a.ds_biometria, a.ie_tipo_processo, a.nr_seq_uni_exec,
			a.ie_consulta_urgencia, null, null,
			a.ds_observacao, nm_usuario_p, a.ie_origem_solic,
			a.ie_tipo_intercambio, a.ie_tipo_gat, a.ie_recem_nascido,
			a.cd_senha_externa, a.dt_valid_senha_ext, null,
			null, c.cd_guia, b.nr_seq_guia,
			null, cd_matricula_estipulante_w, a.ie_pcmso,
			a.nr_seq_cbo_saude, a.ie_saude_ocupacional, a.ie_regime_atendimento, a.ie_cobertura_especial
		...
		for r_c05_w in C05 loop
			...
			if (qt_glosa_w > 0) then
				...
				insert	into pls_requisicao_proc(
					nr_sequencia, nr_seq_requisicao, cd_procedimento,
		...
		for r_c10_w in C10 loop
			...
			into STRICT	qt_glosa_w
			from	pls_requisicao_glosa
			where	nr_seq_req_proc	= r_c10_w.nr_sequencia
			and	nr_seq_motivo_glosa	= 514 --Glosa 1214
			and	0 =	(SELECT	count(1)
					from	pls_requisicao_glosa
					where	nr_seq_req_proc	= r_c10_w.nr_sequencia
		...
		for r_c07_w in C07 loop
			...
			for r_c11_w in C11 loop
				...
				(SELECT	r_c11_w.cd_diagnostico_imagem, r_c11_w.cd_doenca, r_c11_w.cd_finalidade_tratamento,
					...
					nextval('pls_lote_anexo_diag_aut_seq')
				);
			end loop;
		end loop;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_duplic_req_glosa_1214 ( nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
