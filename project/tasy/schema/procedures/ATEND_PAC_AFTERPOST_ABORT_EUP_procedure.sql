...
if (ie_continuar_execucao_w = 'S') and (ie_modo_edicao_p = 'S') and (ie_perm_alt_medico_resp_p = 'N') and (cd_medico_resp_old_p IS NOT NULL AND cd_medico_resp_old_p::text <> '') then
...
if (cd_medico_resp_p IS NOT NULL AND cd_medico_resp_p::text <> '') then
	...
	if (ie_continuar_execucao_w = 'S') then
	...
	if (ie_continuar_execucao_w = 'S') and (ie_vinculo_medico_inst_p IS NOT NULL AND ie_vinculo_medico_inst_p::text <> '') then
	...
	if (ie_continuar_execucao_w = 'S') and (ie_consis_medico_tipo_atend_p = 'N') then
	...
	if (ie_continuar_execucao_w = 'S') and (ie_consis_medico_estab_p = 'S') then
...
if (coalesce(nr_seq_classificacao_p,0) > 0) then	
	...
	if (ie_continuar_execucao_w = 'S') and
		((ie_modo_edicao_p = 'S' AND ie_consistir_classif_atend_p = 'S') or
	...
	if (ie_continuar_execucao_w = 'S') and (ie_classif_obriga_indic_w IS NOT NULL AND ie_classif_obriga_indic_w::text <> '') and (nr_seq_indicacao_p = 0) then
...
if (ie_continuar_execucao_w = 'S') and (cd_medico_referido_p IS NOT NULL AND cd_medico_referido_p::text <> '') then
...
if (cd_pessoa_responsavel_p IS NOT NULL AND cd_pessoa_responsavel_p::text <> '') then
	if (ie_continuar_execucao_w = 'S') and (ie_resp_sem_cpf_p <> 'S') then
		...
		if (coalesce(ie_resp_brasileiro_w,'N') <> 'S') or (qt_idade_responsavel_w >= coalesce(qt_idade_min_cons_cpf_p,0)) then
			...
			if (ie_resp_sem_cpf_p = 'N') and (coalesce(nr_cpf_resp_w::text, '') = '') then
				...
				ie_continuar_execucao_w := 'N';
			elsif (ie_continuar_execucao_w = 'S') and (ie_resp_sem_cpf_p = 'E') then	
			...
			elsif (ie_continuar_execucao_w = 'S') and (ie_resp_sem_cpf_p = 'B') then	
	...
	if (ie_continuar_execucao_w = 'S') and (ie_resp_menor_idade_p = 'S') then
	...
	if (ie_continuar_execucao_w = 'S') and
		((ie_permite_resp_paciente_p = 'N') or
	...
	if (ie_continuar_execucao_w = 'S') and (ie_obriga_resp_p = 'S') and (coalesce(ie_responsavel_p::text, '') = '') then
		ds_msg_erro_w := substr(obter_texto_tasy(236024, wheb_usuario_pck.get_nr_seq_idioma),1,255);
		ie_continuar_execucao_w := 'N';
	end if;
else
	if (ie_continuar_execucao_w = 'S') and (ie_qt_max_min_idade_p IS NOT NULL AND ie_qt_max_min_idade_p::text <> '') and
		((ie_exige_resp_atend_intern_p = 'N') or 
		 (ie_exige_resp_atend_intern_p = 'S' AND ie_tipo_atendimento_p = 1)) and (cd_pessoa_fisica_p IS NOT NULL AND cd_pessoa_fisica_p::text <> '') then
...
if (cd_pessoa_fisica_p IS NOT NULL AND cd_pessoa_fisica_p::text <> '') then	
	if (ie_continuar_execucao_w = 'S') and (coalesce(nr_seq_agenda_p,0) > 0) and (cd_tipo_agenda_p in (1,2)) then
	...
	if (ie_continuar_execucao_w = 'S') and (ie_verifica_atend_vali_p = 'S') and (coalesce(nr_atend_origem_pa_p,0) > 0) then
	...
	if (ie_continuar_execucao_w = 'S') and (dt_entrada_p IS NOT NULL AND dt_entrada_p::text <> '') then
...
if (ie_continuar_execucao_w = 'S') and (ie_proced_atend_pa_p IS NOT NULL AND ie_proced_atend_pa_p::text <> '') and (coalesce(nr_atend_origem_pa_p,0) > 0) then
...
if (ie_continuar_execucao_w = 'S') and (ie_novo_atend_alta_anterior_p = 'R') and (cd_pessoa_fisica_p IS NOT NULL AND cd_pessoa_fisica_p::text <> '') then
...
if (ie_continuar_execucao_w = 'S') and (ie_consis_tipo_convenio_w = 'S') and
...
if (ie_continuar_execucao_w = 'S') and (ie_novo_p = 'S') then
