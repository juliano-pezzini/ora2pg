...
CREATE OR REPLACE PROCEDURE estornar_propaci_prescr_gas ( nr_prescricao_p bigint, nr_seq_prescr_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
c01 CURSOR FOR
	...
	and	nr_sequencia_gas	= nr_seq_prescr_p
	...
	fetch c01 into	nr_sequencia_w,
		...
		if (nr_seq_gerada_w IS NOT NULL AND nr_seq_gerada_w::text <> '') then
			...
			set	nr_interno_conta  = NULL,
				...
				ds_observacao	 = Wheb_mensagem_pck.get_Texto(305579, 'DS_OBSERVACAO_W='|| ds_observacao ) /*ds_observacao || ' Estorno gerado a partir do cancelamento da prescricao'*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE estornar_propaci_prescr_gas ( nr_prescricao_p bigint, nr_seq_prescr_p bigint, nm_usuario_p text) FROM PUBLIC;
