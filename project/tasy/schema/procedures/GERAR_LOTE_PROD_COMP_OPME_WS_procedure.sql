...
CREATE OR REPLACE PROCEDURE gerar_lote_prod_comp_opme_ws ( nr_lote_producao_p bigint, cd_material_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
select	max(b.cd_material),
	max(coalesce(b.qt_material,0)),
	max(b.cd_unidade_medida_dose),
	max(b.nr_seq_componente),
	max(b.qt_dose),
	max(b.ie_necessita_disp)
into STRICT    cd_material_comp_w,
	qt_material_w,
...
if (cd_unidade_medida_dose_w IS NOT NULL AND cd_unidade_medida_dose_w::text <> '') then
	insert into lote_producao_comp(nr_lote_producao,
		...
		values (nr_lote_producao_p,
			...
			nm_usuario_p,
			0,
			coalesce(qt_material_w,0) * coalesce(qt_prevista_w,0),
			0, 0, 0, 0,
			qt_material_w,
			qt_dose_w,
			cd_unidade_medida_dose_w,
	...
	CALL wheb_mensagem_pck.Exibir_Mensagem_Abort(197594);
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_lote_prod_comp_opme_ws ( nr_lote_producao_p bigint, cd_material_p bigint, nm_usuario_p text) FROM PUBLIC;
