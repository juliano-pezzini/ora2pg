...
CREATE OR REPLACE PROCEDURE gerar_retorno_sicoob_400 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
c01 CURSOR FOR
SELECT	trim(both substr(ds_string,63,12)),
	...
	(substr(ds_string,182,07))::numeric /100,
	coalesce(somente_numero(substr(ds_string,176,6)),0),
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	--if  (nr_titulo_w is null) then /* Quando não há título, busca pelo nosso numero */
		select	max(nr_titulo),
	...
	if (nr_titulo_w IS NOT NULL AND nr_titulo_w::text <> '') then
		...
		ds_observacao_w := substr(wheb_mensagem_pck.get_texto(302918, 'ds_titulo_w=' || ds_titulo_w),1,4000);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_retorno_sicoob_400 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
