...
CREATE OR REPLACE PROCEDURE envia_email_proximo_aprov_cot ( nr_seq_aprovacao_p bigint, nr_seq_proc_aprov_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
from	(select	b.ds_email
	...
	and	not exists (select	1
			...
			and	((coalesce(PKG_DATE_UTILS.start_of(x.dt_inicio_limite, 'dd', 0),PKG_DATE_UTILS.start_of(clock_timestamp(), 'dd', 0)) <= PKG_DATE_UTILS.start_of(clock_timestamp(), 'dd', 0)) and (PKG_DATE_UTILS.start_of(x.dt_limite, 'dd', 0) >= PKG_DATE_UTILS.start_of(clock_timestamp(), 'dd', 0))))
...
union all
	...
	and	((coalesce(PKG_DATE_UTILS.start_of(c.dt_inicio_limite, 'dd', 0),PKG_DATE_UTILS.start_of(clock_timestamp(), 'dd', 0)) <= PKG_DATE_UTILS.start_of(clock_timestamp(), 'dd', 0)) and (PKG_DATE_UTILS.start_of(c.dt_limite, 'dd', 0) >= PKG_DATE_UTILS.start_of(clock_timestamp(), 'dd', 0)))
	and	a.nr_sequencia = nr_seq_aprovacao_p
	and	a.nr_seq_proc_aprov = nr_seq_proc_aprov_p
...
if (ie_origem_processo_w = 'C') then
	begin

	ie_origem_processo_w := 'CC';
...
and	obter_se_envia_email_regra(nr_documento_w, ie_origem_processo_w, ie_tipo_mensagem, cd_estabelecimento) = 'S';

if (nr_seq_regra_w > 0) and (ie_origem_processo_w = 'CC') and
	...
	ds_assunto_w := substr(replace_macro(ds_assunto_w,'@dt_cotacao',PKG_DATE_FORMATERS.to_varchar(dt_cotacao_w, 'shortDate', 8031,PKG_DATE_FORMATERS.getCalendar(cd_estabelecimento_p,nm_usuario_p))),1,255);
	...
	ds_mensagem_w := substr(replace_macro(ds_mensagem_w,'@dt_cotacao',PKG_DATE_FORMATERS.to_varchar(dt_cotacao_w, 'shortDate', 8031,PKG_DATE_FORMATERS.getCalendar(cd_estabelecimento_p,nm_usuario_p))),1,4000);
	...
	if (ie_momento_envio_w = 'A') then
		...
		exception
		when others then
			/*gravar__log__tasy(91501,'Falha ao enviar e-mail compras - Evento: 83 - Seq. Regra: ' || nr_seq_regra_w,nm_usuario_p);*/

			CALL gerar_historico_cotacao(
				nr_documento_w,
				wheb_mensagem_pck.get_texto(299745),
				wheb_mensagem_pck.get_texto(300025, 'NR_SEQ_REGRA=' || nr_seq_regra_w),
				'S',
				nm_usuario_p);
		end;
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE envia_email_proximo_aprov_cot ( nr_seq_aprovacao_p bigint, nr_seq_proc_aprov_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
