...
CREATE OR REPLACE PROCEDURE vipe_obter_medic ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_exibir_hor_realizados_p text, ie_exibir_hor_suspensos_p text, ie_regra_inclusao_p text, ie_data_lib_prescr_p text, ie_lib_pend_rep_p text, ie_exibir_suspensos_p text, ie_agrupar_acm_sn_p text, ie_agrupar_dose_esp_p text, ie_diluicao_acm_sn_gestao_p text, ie_mostrar_substituido_p text, ie_prescr_setor_p text, cd_setor_paciente_p bigint) AS $body$
DECLARE
...
c01 CURSOR FOR 
SELECT	CASE WHEN ie_acm_sn='S' THEN  CASE WHEN obter_se_agrupar_acm_sn_adep(nr_prescricao,nr_seq_material,ie_agrupar_acm_sn_p)='N' THEN  nr_prescricao END  END , 
	CASE WHEN ie_acm_sn='S' THEN  CASE WHEN obter_se_agrupar_acm_sn_adep(nr_prescricao,nr_seq_material,ie_agrupar_acm_sn_p)='N' THEN  nr_seq_material END  END , 
	cd_material, 
	ds_material, 
	ie_acm_sn, 
	cd_intervalo, 
...
group by 
	CASE WHEN ie_acm_sn='S' THEN  CASE WHEN obter_se_agrupar_acm_sn_adep(nr_prescricao,nr_seq_material,ie_agrupar_acm_sn_p)='N' THEN  nr_prescricao END  END , 
	CASE WHEN ie_acm_sn='S' THEN  CASE WHEN obter_se_agrupar_acm_sn_adep(nr_prescricao,nr_seq_material,ie_agrupar_acm_sn_p)='N' THEN  nr_seq_material END  END ,	 
	cd_material, 
	ds_material, 
	ie_acm_sn,	 
...
from	( 
	SELECT	a.nr_prescricao, 
		...
		x.cd_unidade_medida_dose, 
		x.ie_via_aplicacao, 
		CASE WHEN x.ie_acm='S' THEN 'ACM '  ELSE CASE WHEN x.ie_se_necessario='S' THEN 'SN '  ELSE '' END  END  || substr(obter_desc_intervalo_prescr(x.cd_intervalo),1,15) ds_intervalo, 
		coalesce(x.qt_total_dias_lib,0) qt_total_dias_lib 
	from	material y, 
		prescr_material x, 
		prescr_medica a 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE vipe_obter_medic ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_exibir_hor_realizados_p text, ie_exibir_hor_suspensos_p text, ie_regra_inclusao_p text, ie_data_lib_prescr_p text, ie_lib_pend_rep_p text, ie_exibir_suspensos_p text, ie_agrupar_acm_sn_p text, ie_agrupar_dose_esp_p text, ie_diluicao_acm_sn_gestao_p text, ie_mostrar_substituido_p text, ie_prescr_setor_p text, cd_setor_paciente_p bigint) FROM PUBLIC;
