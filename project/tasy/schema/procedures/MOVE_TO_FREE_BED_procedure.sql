...
CREATE OR REPLACE PROCEDURE move_to_free_bed ( nm_usuario_p text, nr_atendimento_p bigint, nr_enc_unity_code_p bigint, dt_entrada_unidade_p timestamp default null, ds_retorno_p INOUT text DEFAULT NULL) AS $body$
DECLARE
...
if (coalesce(nr_atendimento_p,0) > 0) and (coalesce(nr_enc_unity_code_p,0) >0) then

	nr_seq_atepacu_atend_w :=	obter_atepacu_paciente(nr_atendimento_p, 'A');
 
	if (coalesce(nr_seq_atepacu_atend_w,0) > 0)  then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE move_to_free_bed ( nm_usuario_p text, nr_atendimento_p bigint, nr_enc_unity_code_p bigint, dt_entrada_unidade_p timestamp default null, ds_retorno_p INOUT text DEFAULT NULL) FROM PUBLIC;
