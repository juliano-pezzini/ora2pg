...
CREATE OR REPLACE PROCEDURE ptu_imp_fatura_v110 ( ds_conteudo_p text, ie_tipo_arquivo_p bigint, cd_estabelecimento_p bigint, nr_linha_p bigint, nm_usuario_p text, nr_seq_fatura_p INOUT bigint, nr_seq_fat_imp_p INOUT ptu_fatura.nr_sequencia%type) AS $body$
DECLARE
...
if (ds_registro_w	= '502') then
	...
	if (dt_conhecimento_w IS NOT NULL AND dt_conhecimento_w::text <> '') then
		...
		-- Montar a carteirinha do beneficiário
		...
		from (	-- busca por pessoa juridica
			...
			and	ac.cd_cnpj_executante	= nr_cnpj_cpf_w
			and	tp_pessoa_w		= 'J'
			...
			and	ac.nr_guia_operadora	= nr_nota_w
...
union all
			...
			and	ac.nr_guia_prestador	= nr_guia_tiss_prestador_w
			and	ac.nr_carteira_benef	= nr_carteira_benef_w
			and	ac.nr_guia_operadora	= nr_nota_w) x;
	end if;
...
if (ds_registro_w = '503') then
	...
	-- Versão PTU 11.0 foram descartados
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_imp_fatura_v110 ( ds_conteudo_p text, ie_tipo_arquivo_p bigint, cd_estabelecimento_p bigint, nr_linha_p bigint, nm_usuario_p text, nr_seq_fatura_p INOUT bigint, nr_seq_fat_imp_p INOUT ptu_fatura.nr_sequencia%type) FROM PUBLIC;
