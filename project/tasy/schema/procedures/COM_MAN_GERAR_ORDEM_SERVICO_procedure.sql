...
CREATE OR REPLACE PROCEDURE com_man_gerar_ordem_servico ( nr_seq_localizacao_p bigint, nr_seq_equipamento_p bigint, cd_pessoa_solicitante_p text, dt_ordem_servico_p timestamp, ie_prioridade_p text, ie_parado_p text, ds_dano_breve_p text, nm_usuario_p text, ds_dano_p text, ie_tipo_ordem_p text, ie_status_ordem_p text, ie_classificacao_p text, nr_seq_estagio_p bigint, nr_grupo_planej_p bigint, nr_grupo_trabalho_p bigint, nr_seq_cliente_p bigint, nr_ordem_servico_p INOUT text) AS $body$
DECLARE
...
and	((coalesce(a.dt_fim_atuacao::text, '') = '') or (a.dt_fim_atuacao > clock_timestamp()));

select	substr(obter_desc_canal_atuacao(nr_seq_canal_w),1,60)
into STRICT	ds_canal_w
;

ds_dano_w := substr((ds_dano_p || chr(13) || chr(10) || ds_canal_w),1,4000);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE com_man_gerar_ordem_servico ( nr_seq_localizacao_p bigint, nr_seq_equipamento_p bigint, cd_pessoa_solicitante_p text, dt_ordem_servico_p timestamp, ie_prioridade_p text, ie_parado_p text, ds_dano_breve_p text, nm_usuario_p text, ds_dano_p text, ie_tipo_ordem_p text, ie_status_ordem_p text, ie_classificacao_p text, nr_seq_estagio_p bigint, nr_grupo_planej_p bigint, nr_grupo_trabalho_p bigint, nr_seq_cliente_p bigint, nr_ordem_servico_p INOUT text) FROM PUBLIC;
