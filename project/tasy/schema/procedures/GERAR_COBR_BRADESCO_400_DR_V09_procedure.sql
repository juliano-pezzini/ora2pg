...
CREATE OR REPLACE PROCEDURE gerar_cobr_bradesco_400_dr_v09 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_titulo_w IS NOT NULL AND nr_titulo_w::text <> '') then
		...
		CALL fin_gerar_log_controle_banco(3,substr(wheb_mensagem_pck.get_texto(303847, 'DS_TITULO_W=' || ds_titulo_w),1,4000),nm_usuario_p,'N');
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_cobr_bradesco_400_dr_v09 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
