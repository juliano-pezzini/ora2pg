...
CREATE OR REPLACE PROCEDURE atualiza_dados_nfse ( nr_nfe_imp_p text, cd_verificacao_nfse_p text, dt_emissao_nfe_p text, cd_serie_nf_p text, nr_nota_fiscal_p text, cd_cgc_emitente_p text, dt_emissao_rps_p text) AS $body$
DECLARE
...
cd_interno_w	:= substr(obter_conversao_interna(cd_cgc_emitente_w,'NOTA_FISCAL','CD_SERIE_NF',coalesce(trim(both cd_serie_nf_p),0)),1,40);
...
if (coalesce(cd_interno_w::text, '') = '') then
	...
	and ((dt_atualizacao_estoque IS NOT NULL AND dt_atualizacao_estoque::text <> '' AND ie_situacao = '1') or ((coalesce(dt_atualizacao_estoque::text, '') = '') and (ie_situacao = '8')));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE atualiza_dados_nfse ( nr_nfe_imp_p text, cd_verificacao_nfse_p text, dt_emissao_nfe_p text, cd_serie_nf_p text, nr_nota_fiscal_p text, cd_cgc_emitente_p text, dt_emissao_rps_p text) FROM PUBLIC;
