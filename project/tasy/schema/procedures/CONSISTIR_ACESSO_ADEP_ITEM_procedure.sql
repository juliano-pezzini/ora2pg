...
CREATE OR REPLACE PROCEDURE consistir_acesso_adep_item ( cd_estabelecimento_p bigint, nr_atendimento_p bigint, cd_paciente_p text, cd_pessoa_fisica_p text, nm_usuario_p text, dt_logon_p timestamp, nr_seq_item_pront_p bigint, ie_permite_p INOUT text, nr_seq_mensagem_p INOUT bigint) AS $body$
DECLARE
			...
			EXIT WHEN NOT FOUND; /* apply on c01 */
				...
				if (ie_acesso_w = 'T') then 
					...
					if (IE_CONSISTE_JUSTIFICATIVA_w	= 'P') then 
						...
						and	i.nr_seq_item	= nr_seq_item_pront_p 
						and	a.dt_alta is null 
						and	i.nr_seq_justificativa is not null 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE consistir_acesso_adep_item ( cd_estabelecimento_p bigint, nr_atendimento_p bigint, cd_paciente_p text, cd_pessoa_fisica_p text, nm_usuario_p text, dt_logon_p timestamp, nr_seq_item_pront_p bigint, ie_permite_p INOUT text, nr_seq_mensagem_p INOUT bigint) FROM PUBLIC;
