...
CREATE OR REPLACE PROCEDURE gerar_pagto_febraban_240_agrup ( nr_seq_envio_p bigint, nm_usuario_p text) AS $body$
DECLARE


/* PADRAO CNAB 240 VERSAO 084 DE 07/2010 */
...
SELECT	cd_banco,
	...
	ds_bairro,
	cd_camara
from (SELECT	CASE WHEN coalesce(e.cd_cgc,coalesce(e.cd_pessoa_fisica,'0'))='0' THEN b.cd_banco  ELSE somente_numero(substr(obter_conta_pessoa(coalesce(e.cd_cgc,e.cd_pessoa_fisica),CASE WHEN coalesce(e.cd_cgc::text, '') = '' THEN 'F'  ELSE 'J' END ,'B'),1,3)) END  cd_banco,
		CASE WHEN coalesce(e.cd_cgc,coalesce(e.cd_pessoa_fisica,'0'))='0' THEN lpad(substr(b.cd_agencia_bancaria,1,5) || substr(b.ie_digito_agencia,1,1),6,'0')  ELSE lpad(substr(obter_conta_pessoa(coalesce(e.cd_cgc,e.cd_pessoa_fisica),CASE WHEN coalesce(e.cd_cgc::text, '') = '' THEN 'F'  ELSE 'J' END ,'A'),1,5) || substr(obter_conta_pessoa(coalesce(e.cd_cgc,e.cd_pessoa_fisica),CASE WHEN coalesce(e.cd_cgc::text, '') = '' THEN 'F'  ELSE 'J' END ,'DA'),1,1),6,'0') END  cd_agencia_bancaria,
		rpad(elimina_caractere_especial(substr(obter_nome_pf_pj(c.cd_pessoa_fisica,c.cd_cgc),1,30)),30,' ') nm_pessoa,
		b.vl_escritural,
		b.vl_acrescimo,
		b.vl_desconto,
		b.vl_despesa,
		lpad(coalesce(d.nr_cpf,c.cd_cgc),14,'0') nr_inscricao,
		CASE WHEN coalesce(e.cd_cgc,coalesce(e.cd_pessoa_fisica,'0'))='0' THEN b.nr_conta  ELSE substr(obter_conta_pessoa(coalesce(e.cd_cgc,e.cd_pessoa_fisica),CASE WHEN coalesce(e.cd_cgc::text, '') = '' THEN 'F'  ELSE 'J' END ,'C'),1,255) END  nr_conta,
		CASE WHEN coalesce(e.cd_cgc,coalesce(e.cd_pessoa_fisica,'0'))='0' THEN substr(b.ie_digito_conta,1,1)  ELSE coalesce(substr(obter_conta_pessoa(coalesce(e.cd_cgc,e.cd_pessoa_fisica),CASE WHEN coalesce(e.cd_cgc::text, '') = '' THEN 'F'  ELSE 'J' END ,'DC'),1,1),'0') END  ie_digito_conta,
		b.vl_juros,
		b.vl_multa,
		CASE WHEN coalesce(c.cd_pessoa_fisica::text, '') = '' THEN '2'  ELSE '1' END  ie_tipo_inscricao,
		rpad(coalesce(substr(obter_dados_pf_pj(c.cd_pessoa_fisica,c.cd_cgc,'R'),1,30),' '),30,' ') ds_endereco,
		lpad(coalesce(substr(obter_dados_pf_pj(c.cd_pessoa_fisica,c.cd_cgc,'NR'),1,5),'0'),5,'0') nr_endereco,
		rpad(coalesce(substr(obter_dados_pf_pj(c.cd_pessoa_fisica,c.cd_cgc,'CO'),1,15),' '),15,' ') ds_complemento,
		rpad(coalesce(substr(obter_dados_pf_pj(c.cd_pessoa_fisica,c.cd_cgc,'CI'),1,20),' '),20,' ') ds_municipio,
		lpad(coalesce(substr(obter_dados_pf_pj(c.cd_pessoa_fisica,c.cd_cgc,'CEP'),1,8),'0'),8,'0') cd_cep,
		rpad(coalesce(substr(obter_dados_pf_pj(c.cd_pessoa_fisica,c.cd_cgc,'UF'),1,2),' '),2,' ') ie_estado,
		rpad(coalesce(substr(obter_dados_pf_pj(c.cd_pessoa_fisica,c.cd_cgc,'B'),1,15),' '),15,' ') ds_bairro,
...
LEFT OUTER JOIN titulo_pagar_favorecido e ON (c.nr_titulo = e.nr_titulo)
LEFT OUTER JOIN pessoa_fisica d ON (c.cd_pessoa_fisica = d.cd_pessoa_fisica)
WHERE b.nr_titulo		= c.nr_titulo and b.ie_tipo_pagamento	= ie_tipo_pagamento_w and a.nr_sequencia		= b.nr_seq_escrit and a.nr_sequencia		= nr_seq_envio_p ) alias71
group by	cd_banco,
	cd_agencia_bancaria,
	nm_pessoa,
	nr_titulo,
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	EXIT WHEN NOT FOUND; /* apply on c02 */
		...
		ds_conteudo_w	:=	lpad(cd_banco_cobr_w,3,'0') ||
					...
					' ' ||
					nm_pessoa_w ||
					rpad(coalesce(to_char(nr_titulo_w),' '),20,' ') ||
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_pagto_febraban_240_agrup ( nr_seq_envio_p bigint, nm_usuario_p text) FROM PUBLIC;
