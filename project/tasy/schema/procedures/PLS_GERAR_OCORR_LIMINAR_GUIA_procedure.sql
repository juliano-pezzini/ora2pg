...
CREATE OR REPLACE PROCEDURE pls_gerar_ocorr_liminar_guia ( nr_seq_segurado_p bigint, nr_seq_processo_p bigint, nr_seq_guia_p bigint, nm_usuario_p text, nr_seq_regra_p INOUT bigint) AS $body$
DECLARE


/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: Verificar as liminares cadastradas para o beneficiário na função OPS - Processo Judicial e, caso exista ocorrência para liminar judicial na função OPS - Glosas e Ocorrências, gera ocorrência na guia.
-------------------------------------------------------------------------------------------------------------------
	...
	if (ie_estagio_w	in (1,2,4,10,12)) then
						...
						EXIT WHEN NOT FOUND; /* apply on C02 */
							...
							if (qt_reg_proc_w	> 0) then
								if (ie_tipo_processo_w = 'I') and (ie_tipo_intercambio_w = 'I') then
									...
									CALL pls_inserir_ocorr_guia_benef(	nr_seq_ocorrencia_w, nr_seq_regra_w, nr_seq_guia_w,
		...
		if (ie_tipo_processo_w = 'I') and (ie_tipo_intercambio_w = 'I') and (ie_liminar_intercambio_w = 'S') then
			...
			if (qt_ped_w > 0) then
				...
				CALL pls_guia_gravar_historico(nr_seq_guia_w, 2, 'A guia teve o campo "Liminar" marcado, devido a liminar nº: ' || nr_seq_processo_w, null, nm_usuario_p);
			else
				select	count(1)
				into STRICT	qt_ped_w
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_ocorr_liminar_guia ( nr_seq_segurado_p bigint, nr_seq_processo_p bigint, nr_seq_guia_p bigint, nm_usuario_p text, nr_seq_regra_p INOUT bigint) FROM PUBLIC;
