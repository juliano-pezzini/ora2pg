...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_c195_icmsipi ( nr_seq_controle_p bigint, nr_seq_nota_p bigint, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
type reg_c_reg_C195 is table of fis_efd_icmsipi_C195%RowType;
...
fetch c_reg_C195 bulk collect into vetRegC195 limit 1000;
	...
	ds_linha_w := substr(	ds_sep_w || vetRegC195[i].cd_reg ||
				ds_sep_w || vetRegC195[i].cd_obs ||
				ds_sep_w || vetRegC195[i].ds_txt_compl || ds_sep_w, 1, 8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro		:= vetRegC195[i].cd_reg;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_c195_icmsipi ( nr_seq_controle_p bigint, nr_seq_nota_p bigint, nr_linha_p INOUT bigint ) FROM PUBLIC;
