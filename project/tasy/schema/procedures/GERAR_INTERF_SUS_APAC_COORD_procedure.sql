...
CREATE OR REPLACE PROCEDURE gerar_interf_sus_apac_coord ( dt_inicial_p timestamp, dt_final_p timestamp, nm_usuario_p text) AS $body$
DECLARE
...
coalesce(CASE WHEN ie_exporta_resp_w='C' THEN substr(obter_compl_pf(c.cd_pessoa_fisica, 3,'N'),1,60)  ELSE substr(obter_nome_pf(c.cd_pessoa_responsavel),1,60) END , coalesce(substr(obter_compl_pf(c.cd_pessoa_fisica, 5,'N'),1,60),substr(obter_compl_pf(c.cd_pessoa_fisica, 4,'N'),1,60))),
		...
		replace(elimina_acentos(upper(substr(obter_compl_pf(c.cd_pessoa_fisica, 1, 'B'),1,30))),'Ç','C') ds_bairro,
	...
	and	trunc(a.dt_competencia) between dt_inicial_p and dt_final_p
...
EXIT WHEN NOT FOUND; /* apply on C03 */
	...
	delete from w_susapac_interf where nr_interno_conta = nr_interno_conta_w;
	delete from w_susapac_interf_coord where nr_interno_conta = nr_interno_conta_w;
	end;
end loop;
close C03;
...
if (coalesce(nr_regra_exp_par_w,0) <> 0) then
	begin
	select	a.cd_orgao_emissor_apac,
		a.cd_unidade_apac,
		coalesce(a.cd_unidade_solic_apac,''),
		coalesce(a.ie_exporta_resp,'C'),
	...
	into STRICT	cd_orgao_emissor_apac_w,
		...
		ie_exporta_resp_w,
		ie_exp_cnes_prest_w
	from	sus_regra_exp_param_apac a
	where	a.nr_sequencia		= nr_regra_exp_par_w
...
exception
	...
	/*'Dados dos parâmetros APAC não encontrado'*/
...
exception
	...
	/*'Falta a informação da localidade do Hospital. ' || chr(13) ||
				'Verificar a função Empresas/Estabelecimento/Conta/CC, pasta estabelecimento, campo Localidade.'*/
end;
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	insert into w_susapac_interf_coord(nr_sequencia,
			nr_interno_conta,
			dt_atualizacao,
			nm_usuario,
			dt_atualizacao_nrec,
			...
			dt_envio_coord,
			nm_usuario_envio)
		values (nextval('w_susapac_interf_coord_seq'),
			nr_interno_conta_w,
			clock_timestamp(),
			nm_usuario_p,
			clock_timestamp(),
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_interf_sus_apac_coord ( dt_inicial_p timestamp, dt_final_p timestamp, nm_usuario_p text) FROM PUBLIC;
