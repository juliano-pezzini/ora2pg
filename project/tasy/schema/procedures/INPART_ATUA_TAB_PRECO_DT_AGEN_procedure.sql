...
CREATE OR REPLACE PROCEDURE inpart_atua_tab_preco_dt_agen (cd_material_p bigint, dt_vigencia_p timestamp, cd_tab_preco_mat_p bigint, cd_estabelecimento_p bigint, vl_preco_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (coalesce(vl_preco_p,0) > vl_preco_venda_w) then
	...
	if (qt_existe_w = 0) then
		...
		and	cd_tab_preco_mat	= cd_tab_preco_mat_p
		and	cd_material		= cd_material_p
		and	dt_inicio_vigencia 	= trunc(dt_vigencia_p);   -- Retirado após as conversas por telefone com Daniela Z e retorno da Lilian (Sírio) na OS
		select	count(*)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE inpart_atua_tab_preco_dt_agen (cd_material_p bigint, dt_vigencia_p timestamp, cd_tab_preco_mat_p bigint, cd_estabelecimento_p bigint, vl_preco_p bigint, nm_usuario_p text) FROM PUBLIC;
