...
CREATE OR REPLACE PROCEDURE ptu_gerar_env_arq_a700_fat_v62 ( nr_seq_fatura_p bigint, cd_estabelecimento_p text, nm_usuario_p text) AS $body$
DECLARE
...
qt_tot_r704_ww			ptu_nota_servico.qt_procedimento%type := 0;
qt_tot_r704_www			varchar(11);
...
ie_tp_consulta_w		ptu_nota_cobranca.tp_consulta%type;
...
nr_seq_lote_fatutamento_w	pls_lote_faturamento.nr_sequencia%type;

C00 CURSOR FOR 
	SELECT	'0976', --lpad(nvl(cd_unimed_destino,'0'),4,'0'), 
		lpad(coalesce(cd_unimed_origem,'0'),4,'0'), 
		to_char(dt_geracao,'yyyymmdd'), 
		coalesce(dt_emissao_fatura,dt_emissao_ndc), 
		to_char(trunc(coalesce(dt_emissao_fatura,dt_emissao_ndc),'month'),'yyyymmdd'), 
		to_char(last_day(coalesce(dt_emissao_fatura,dt_emissao_ndc)),'yyyymmdd'), 
		'18' 
	from	ptu_fatura 
...
C01 CURSOR FOR 
	...
	and	b.nr_sequencia		= c.nr_seq_pls_fatura 
	and	a.nr_sequencia		= nr_seq_lote_fatutamento_w 
	order by 
		b.nr_sequencia, 
		c.nr_sequencia;
...
C02 CURSOR FOR 
	SELECT	lpad(coalesce(elimina_caractere_especial(to_char(nr_lote)),'0'),8,'0'), 
		lpad(coalesce(substr(elimina_caractere_especial(to_char(nr_nota)),1,20),'0'),20,'0'), 
		...
		nr_sequencia, 
		rpad(coalesce(to_char(dt_internacao,'yyyy/mm/ddhh:mi:ss') || CASE WHEN coalesce(dt_internacao::text, '') = '' THEN ' '  ELSE ds_meridiano_w END ,' '),21,' '), 
		rpad(coalesce(to_char(dt_alta,'yyyy/mm/ddhh:mi:ss') || CASE WHEN coalesce(dt_alta::text, '') = '' THEN ' '  ELSE ds_meridiano_w END ,' '),21,' '), 
...
C03 CURSOR FOR 
	SELECT	lpad(elimina_caractere_especial(a.nr_lote),8,'0') nr_lote_703, 
		...
		rpad(coalesce(b.nr_declara_vivo,' '),15,' '), 
		rpad(coalesce(b.nr_declara_vivo,' '),15,' '), 
		rpad(coalesce(b.nr_declara_vivo,' '),15,' '), 
		rpad(coalesce(b.nr_declara_vivo,' '),15,' '), 
		rpad(coalesce(b.nr_declara_vivo,' '),15,' '), 
...
C04 CURSOR FOR 
	SELECT	lpad(coalesce(elimina_caractere_especial(a.nr_lote),'0'),8,'0'), 
		CASE WHEN 	lpad(coalesce(substr(elimina_caractere_especial(to_char(a.nr_nota)),length(to_char(a.nr_nota)) - 10,length(to_char(a.nr_nota))),'0'),11,'0')='00000000000' THEN  			lpad(coalesce(substr(elimina_caractere_especial(to_char(a.nr_nota)),1,11),'0'),11,'0')  ELSE lpad(coalesce(substr(elimina_caractere_especial(to_char(a.nr_nota)),length(to_char(a.nr_nota)) - 10,length(to_char(a.nr_nota))),'0'),11,'0') END , 
		...
		lpad(coalesce(a.qt_procedimento,0),4,0), 
		...
		rpad(coalesce(substr(a.cd_rec_prestador,1,10),' '),10,' ') 
	from	ptu_nota_servico		a, 
		ptu_nota_cobranca		b 
	where	a.nr_seq_nota_cobr		= b.nr_sequencia 
	and	coalesce(a.qt_procedimento,0) > 0	 
	--and	((a.dt_pgto_prestador is null) or (trunc(a.dt_pgto_prestador) between trunc(dt_mes_competencia_w) and fim_dia(last_day(dt_mes_competencia_w)))) 
	and	b.nr_sequencia		= nr_seq_nota_cobranca_w;

C05 CURSOR FOR 
	SELECT	lpad(coalesce(elimina_caractere_especial(a.nr_lote),'0'),8,'0'), 
		lpad(coalesce(substr(elimina_caractere_especial(to_char(a.nr_nota)),length(to_char(a.nr_nota)) - 10,length(to_char(a.nr_nota))),'0'),11,'0'), 
		coalesce(to_char(a.ie_tipo_complemento),' '), 
		rpad(coalesce(elimina_caractere_especial(elimina_acentuacao(replace(a.ds_complemento,chr(10),''))),' '),100,' '), 
		rpad(coalesce(elimina_caractere_especial(elimina_acentuacao(a.especif_material)),' '),500,' ') 
...
EXIT WHEN NOT FOUND; /* apply on C00 */
	...
	into STRICT	nr_seq_lote_fatutamento_w 
	from	ptu_fatura		c, 
		pls_fatura		b, 
		pls_lote_faturamento	a 
	where	a.nr_sequencia		= b.nr_seq_lote 
	...
	update	ptu_fatura 
	set	nr_seq_geracao	= nr_seq_lote_fatutamento_w 
	where	nr_sequencia	= nr_seq_fatura_p;
		 
	ds_conteudo_w	:=	lpad(to_char(nr_seq_registro_w),8,'0') || '701' || cd_unimed_destino_w || cd_unimed_origem_w || dt_geracao_w || lpad(substr(nr_seq_lote_fatutamento_w,1,4),4,0) || 
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		EXIT WHEN NOT FOUND; /* apply on C02 */
			...
			if (qt_registro_w > 0) and (trunc(dt_atendimento_ww) <= trunc(to_date('31/08/2007', 'dd/mm/yyyy'))) then 
				ie_tipo_saida_spdat_w := ' ';
			end if;
			...
			ds_conteudo_w :=	lpad(to_char(nr_seq_registro_w),8,'0') 	|| '702'			|| nr_lote_w			|| 
						lpad(' ',11,' ')	 		|| lpad(' ',1,' ') 		|| cd_unimed_destino_w 		|| 
						lpad(' ',3,' ')				|| cd_usuario_plano_w 		|| nm_beneficiario_w		|| 
						dt_atendimento_w 			|| lpad(' ',1,' ')		|| lpad(' ',1,' ')		|| 
						lpad(' ',2,' ')				|| cd_cid_w 			|| lpad(' ',14,' ') 		|| 
						ie_paciente_w 				|| ie_tipo_saida_spdat_w 	|| ie_tipo_atendimento_w 	|| 
						lpad(' ',11,' ')	 		|| nr_nota_w 			|| nr_guia_principal_w 		|| 
						dt_internacao_702_w 			|| dt_alta_702_w 		|| tp_nota_w 			|| 
						...
						motivo_encerram_w 			|| nr_cnpj_cpf_req_w 		|| nm_prest_req_w 		|| 
						sg_cons_prof_req_702_w 			|| nr_cons_prof_req_702_w 	|| sg_uf_cons_req_702_w 	|| 
						nr_cbo_req_w 				|| dt_protocolo_w 		|| id_rn_w 			|| 
						ie_carater_atendimento_w 		|| ie_tp_consulta_w;
			...
			EXIT WHEN NOT FOUND; /* apply on C03 */
				...
				ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '703' || nr_lote_703_w || lpad(' ',11,' ') || 
						cd_unimed_hospital_w || cd_hospital_w || nm_hospital_w || ie_tipo_acomodacao_w || lpad(' ',42,' ') || 
						tx_mult_amb_w || '  ' || cd_cgc_hospital_w || ie_tipo_internacao_w || lpad(' ',19,' ') || 
						ie_faturamento_w || cd_cid_obito_w || lpad(' ',7,' ') || nr_declara_vivo_1_w || nr_declara_vivo_2_w || nr_declara_vivo_3_w || 
						nr_declara_vivo_4_w || nr_declara_vivo_5_w || ' ' || nr_declara_obito_w || cd_cid_obito_1_w || cd_cid_obito_2_w || 
						cd_cid_obito_3_w || cd_cid_obito_4_w || cd_cid_obito_5_w || nr_declara_obito_1_w || nr_declara_obito_2_w || 
						nr_declara_obito_3_w || nr_declara_obito_4_w || nr_declara_obito_5_w || nr_nota_703_w || reg_internacao_w;
			...
			EXIT WHEN NOT FOUND; /* apply on C04 */
				...
				qt_tot_r704_ww	:= qt_tot_r704_ww + qt_procedimento_w;
				 
				qt_procedimento_ww	:= lpad(trunc(coalesce(qt_procedimento_w,0)),4,'0') || rpad(elimina_caractere_especial(coalesce(qt_procedimento_w,0)-trunc(coalesce(qt_procedimento_w,0))),4,'0');
				 
				ds_conteudo_w	:=	lpad(to_char(nr_seq_registro_w),8,'0') 	|| '704' 			|| nr_lote_704_w 		|| 
							lpad(' ',11,' ') 			|| cd_unimed_prestador_w 	|| cd_prestador_w 		|| 
							nm_prestador_w 				|| ie_tipo_participacao_w 	|| dt_procedimento_w 		|| 
							ie_tipo_tabela_w 			|| cd_procedimento_w 		|| qt_procedimento_ww 		|| 
							vl_procedimento_w 			|| vl_custo_operacional_w 	|| vl_filme_w 			|| 
							cd_porte_anestesico_w 			|| lpad(' ',13,' ') 		|| cd_unimed_pre_req_w 		|| 
							cd_prestador_req_w 			|| ie_via_acesso_w 		|| lpad(' ',42,' ') 		|| 
							cd_especialidade_w 			|| dt_pgto_prestador_w 		|| ie_tipo_prestador_w 		|| 
							ie_rede_propria_w 			|| ie_tipo_pessoa_prestador_w 	|| nr_cgc_cpf_w 		|| 
							ie_pacote_w 				|| 'S' 				|| cd_ato_w 			|| 
							tx_procedimento_w 			|| nr_seq_nota_w 		|| ds_hora_procedimento_w 	|| 
							cd_cnes_prest_w 			|| sg_cons_prof_prest_w 	|| nr_cons_prof_prest_w 	|| 
							sg_uf_cons_prest_w 			|| lpad(' ',2,' ')		|| lpad(' ',14,' ')		|| 
							lpad(' ',40,' ')			|| lpad(' ',12,' ')		|| lpad(' ',15,' ')		|| 
							lpad(' ',2,' ')				|| nm_profissional_prestador_w 	|| ie_reembolso_w 		|| 
							nr_nota_704_w 				|| hr_final_w 			|| id_acres_urg_emer_w 		|| 
							nr_cbo_exec_w 				|| tec_utilizada_w 		|| dt_autoriz_w 		|| 
							dt_solicitacao_w 			|| unidade_medida_w 		|| nr_reg_anvisa_w 		|| 
							cd_munic_w 				|| lpad(' ',30,' ')		|| dt_pgto_prestador_w		|| 
							nr_cnpj_fornecedor_w			|| cd_rec_prestador_w		|| cd_ref_material_fab_w;
						 
				vl_tot_serv_w	:= coalesce(vl_tot_serv_w,0)		+ coalesce(vl_procedimento_w,0);
				vl_tot_co_w	:= coalesce(vl_tot_co_w,0)		+ coalesce(vl_custo_operacional_w,0);
				vl_tot_filme_w	:= coalesce(vl_tot_filme_w,0)	+ coalesce(vl_filme_w,0);
			...
			EXIT WHEN NOT FOUND; /* apply on C05 */
				...
				ds_conteudo_w	:= lpad(to_char(nr_seq_registro_w),8,'0') || '705' || nr_lote_705_w || lpad(' ',11,' ') || 
						ie_tipo_complemento_w || ds_complemento_w || nr_nota_705_w;-- || especif_material_w; 
...
select	lpad(trunc(to_char(qt_tot_r704_ww)),7,'0') || rpad(elimina_caractere_especial(qt_tot_r704_ww-trunc(qt_tot_r704_ww)),4,'0') 
into STRICT	qt_tot_r704_www
;
 
ds_conteudo_w	:=	lpad(to_char(nr_seq_registro_w),8,'0') || '709' || lpad(to_char(qt_tot_r702_w),5,'0') || lpad(to_char(qt_tot_r703_w),5,'0') || 
			lpad(coalesce(qt_tot_r704_w,'0'),5,'0') || lpad(to_char(qt_tot_r705_w),5,'0') || lpad(' ',10,' ') || qt_tot_r704_www || 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_env_arq_a700_fat_v62 ( nr_seq_fatura_p bigint, cd_estabelecimento_p text, nm_usuario_p text) FROM PUBLIC;
