...
CREATE OR REPLACE PROCEDURE pls_gerar_env_arq_fund_302_txt ( nr_seq_pls_fatura_p pls_fatura.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
for r_C01_w in C01( nr_seq_pls_fatura_p ) loop
	...
	for r_C02_w in C02( r_C01_w.nr_sequencia ) loop
		...
		if (length(r_C02_w.nr_documento) > 20) then
			nr_documento_w := substr(r_C02_w.nr_documento, length(r_C02_w.nr_documento) - 19, 20);
		else
			nr_documento_w := lpad(r_C02_w.nr_documento,20,0);
		end if;
		...
		ds_conteudo_w	:= 	lpad(coalesce(r_C02_w.cd_unimed,' '),3,' ') ||
					...
					lpad(coalesce(replace(r_C02_w.tx_adm, ',', ''),'0'),6,'0') ||
					...
					lpad(coalesce(r_C02_w.cd_tipo_atendimento,'0'),2,'0') ||
					...
					lpad(coalesce(r_C02_w.cd_registro_ans,' '),6,' ') ||
					lpad(coalesce(r_C02_w.cd_cbos,' '),7,' ') ||
					lpad(coalesce(r_C02_w.ie_atendimento_rn,' '),1,' ') ||
					lpad(coalesce(r_C02_w.ie_indicacao_acidente,' '),1,' ') ||
					lpad(coalesce(r_C02_w.tx_reducao_acrescimo,' '),5,' ') ||
					lpad(substr(replace(replace(campo_mascara_virgula(coalesce(replace(r_C02_w.vl_unitario,'.',''),' ')),',',''),'.',''),1,8),8,' ') ||
					lpad(coalesce(r_C02_w.cd_unidade_medida,' '),3,' ') ||
					lpad(coalesce(r_C02_w.dt_inicio_faturamento,' '),8,' ') ||
					lpad(coalesce(r_C02_w.dt_fim_faturamento,' '),8,' ') ||
					lpad(coalesce(nr_documento_w,0),20,0) ||
					lpad(coalesce(r_C02_w.cd_servico,' '),10,' ')||
					lpad(substr(replace(replace(campo_mascara_virgula(coalesce(replace(r_C02_w.vl_servico,'.',''),'0')),',',''),'.',''),1,13),13,'0') ||
					coalesce(r_C02_w.ie_taxa_adm, ' ');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_env_arq_fund_302_txt ( nr_seq_pls_fatura_p pls_fatura.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
