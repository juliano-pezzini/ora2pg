...
CREATE OR REPLACE PROCEDURE pls_gerar_alteracao_sib_flex ( nr_seq_lote_sib_p bigint, ie_gerar_todos_p text, nm_usuario_p text, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
CALL pls_gerar_w_alt_sib_flex(nr_seq_lote_sib_p,nm_usuario_p,cd_estabelecimento_p);
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	if (coalesce(nr_cco_w,0) > 0) or (coalesce(cd_cco_w,0) > 0) then	
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_alteracao_sib_flex ( nr_seq_lote_sib_p bigint, ie_gerar_todos_p text, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
