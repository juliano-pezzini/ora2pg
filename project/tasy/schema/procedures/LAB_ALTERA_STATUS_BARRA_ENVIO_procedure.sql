...
CREATE OR REPLACE PROCEDURE lab_altera_status_barra_envio ( nr_prescricao_p text, nm_usuario_p text, ie_status_atend_p bigint, Ds_justificativa_alt_p text, cd_equipamento_pda_p text, cd_perfil_p bigint default null, ds_lista_status_perm_p text default null) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	from	prescr_proc_controle_etiq
	where	((ie_status_atend	<> ie_status_atend_p) or (coalesce(ie_status_atend::text, '') = ''))
	...
	if (qt_prescr_proc_w = 0) then
		...
		set	ie_status_atend	= ie_status_atend_p,
			...
			nm_usuario = nm_usuario_p,
			dt_atualizacao = clock_timestamp(),
			cd_equip_pda = cd_equipamento_pda_p
		where	nr_sequencia	= nr_seq_prescr_w
		and	nr_prescricao	= nr_prescricao_w
		and	((ie_status_atend	< ie_status_atend_p AND ie_status_atend > 10)
		or	(((ds_lista_status_perm_p IS NOT NULL AND ds_lista_status_perm_p::text <> '')
		and (ie_status_atend_p = 16)
...
, nm_usuario_p, 'N', nr_seq_proc_interno_w, null, null, ie_acm_sn_w, null, 'N', null, null, cd_perfil_p, 1113, null, null, null, null, null, null, null, null, null, null, null, '', '', cd_evolucao_w, null, null, ds_msg_w) INTO STRICT cd_evolucao_w, ds_msg_w;
			elsif (ie_status_atend_p < 20) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE lab_altera_status_barra_envio ( nr_prescricao_p text, nm_usuario_p text, ie_status_atend_p bigint, Ds_justificativa_alt_p text, cd_equipamento_pda_p text, cd_perfil_p bigint default null, ds_lista_status_perm_p text default null) FROM PUBLIC;
