...
CREATE OR REPLACE PROCEDURE sup_gerar_saldo_projeto ( nr_seq_projeto_p bigint, dt_referencia_p timestamp, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
vl_inicio_proj_w		double precision;
...
if (coalesce(dt_referencia_p::text, '') = '') then
	...
	select	PKG_DATE_UTILS.ADD_MONTH(pkg_date_utils.start_of(a.dt_mesano_referencia,'MONTH',0),1,0)
	...
	vl_saldo_w	:= ((vl_saldo_anterior_w + vl_entrada_w + vl_lancamentos_fin_w + vl_transf_saida_w) - vl_saida_w - vl_transf_saida_w - vl_tributo_subtrai_w);
	...
	select	nextval('projeto_recurso_saldo_seq')
	into STRICT	nr_sequencia_w
	;

	insert into projeto_recurso_saldo(
		nr_sequencia,
		dt_atualizacao,
		nm_usuario,
		dt_atualizacao_nrec,
	...
	values (	nr_sequencia_w,
		...
		coalesce(vl_entrada_w,0),
		coalesce(vl_saida_w,0),
		coalesce(vl_saldo_w,0),
		coalesce(vl_saldo_anterior_w,0),
	...
	dt_mesano_referencia_w	:= pkg_date_utils.start_of(dt_referencia_p,'MONTH',0);

	select	coalesce(sum(a.vl_inicio_projeto),0)
	into STRICT	vl_inicio_proj_w
	from	projeto_recurso a
	where	((ie_param_25_w = 'N' AND a.nr_sequencia = nr_seq_projeto_p) or
		((ie_param_25_w = 'S') and ((a.nr_sequencia = nr_seq_projeto_p) or (a.nr_sequencia in (SELECT nr_sequencia from projeto_recurso where nr_seq_superior = nr_seq_projeto_p)))));
	...
	and	a.nr_titulo = b.nr_titulo
	and	b.ie_situacao <> 'T'
	and	trunc(dt_baixa) between pkg_date_utils.start_of(dt_referencia_p,'MONTH',0) and trunc(PKG_DATE_UTILS.END_OF(dt_referencia_p, 'MONTH', 0));
	...
	and	a.nr_titulo = b.nr_titulo
	and	b.ie_situacao <> '5'
	and	trunc(dt_recebimento) between pkg_date_utils.start_of(dt_referencia_p,'MONTH',0) and trunc(PKG_DATE_UTILS.END_OF(dt_referencia_p, 'MONTH', 0));
	...
	if (ie_considerar_trib_saldo_w = 'S') then
		...
		and	a.nr_sequencia in (	SELECT 	x.nr_sequencia
						...
						and	x.nr_sequencia in (	select	t.nr_seq_nota_fiscal
										...
										and	tb.nr_titulo = t.nr_titulo
										and	t.ie_situacao <> 'T'
										and	trunc(tb.dt_baixa) between pkg_date_utils.start_of(dt_referencia_p,'MONTH',0) and trunc(PKG_DATE_UTILS.END_OF(dt_referencia_p, 'MONTH', 0))
										having	coalesce(sum(tb.vl_pago),0) <> 0
										group by t.nr_seq_nota_fiscal));

	end if;

	vl_saldo_w 	:= ((vl_inicio_proj_w + vl_entrada_w + vl_lancamentos_fin_w + vl_transf_entrada_w) - vl_saida_w - vl_transf_saida_w - vl_tributo_subtrai_w);
	vl_entrada_w	:= vl_entrada_w + vl_transf_entrada_w;
	vl_saida_w	:= vl_saida_w + vl_transf_saida_w + vl_tributo_subtrai_w;

	select	obter_vl_comprometido_proj_rec(nr_seq_projeto_p, trunc(dt_referencia_p,'mm'),'M')
	into STRICT	vl_comprometido_w
	;
			...
			and	a.nr_sequencia in (	SELECT 	x.nr_sequencia
							...
							and	x.nr_sequencia in (	select	t.nr_seq_nota_fiscal
											...
											where	((ie_param_25_w = 'N' AND t.nr_seq_proj_rec = nr_seq_projeto_p) or
												((ie_param_25_w = 'S') and ((t.nr_seq_proj_rec = nr_seq_projeto_p) or (t.nr_seq_proj_rec in (select nr_sequencia from projeto_recurso where nr_seq_superior = nr_seq_projeto_p)))))
											and	coalesce(t.dt_liquidacao::text, '') = ''
											and	pkg_date_utils.start_of(t.dt_emissao,'MONTH',0) = pkg_date_utils.start_of(dt_referencia_p,'MONTH',0)
											and	t.ie_situacao = 'A'));
	...
	insert into projeto_recurso_saldo(
		...
		values (	nr_sequencia_w,
			...
			coalesce(vl_saida_w,0),
			coalesce(vl_saldo_w,0),
			coalesce(vl_inicio_proj_w,0),
			coalesce(vl_comprometido_w,0));

	vl_anterior_diario_w	:= vl_inicio_proj_w;
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		from	titulo_pagar_baixa a,
			titulo_pagar b
		where	b.nr_seq_proj_rec = nr_seq_projeto_p
		and	a.nr_titulo = b.nr_titulo
		and	b.ie_situacao <> 'T'
		and	trunc(dt_baixa) = trunc(dt_transacao_w);
		...
		vl_entrada_diario_w	:= (vl_entrada_diario_w + vl_transf_entrada_w);
		vl_saida_diario_w	:= (vl_saida_diario_w + vl_transf_saida_w + vl_tributo_subtrai_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE sup_gerar_saldo_projeto ( nr_seq_projeto_p bigint, dt_referencia_p timestamp, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
