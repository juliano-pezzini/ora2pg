...
CREATE OR REPLACE PROCEDURE gerar_analise_prod_migr_co_mes (nm_usuario_colab_p text) AS $body$
DECLARE
...
BEGIN
if (nm_usuario_colab_p IS NOT NULL AND nm_usuario_colab_p::text <> '') then
	begin
	/* obter análise mês / colaborador - caso existir */
	...
	and	ie_analise = 'CM'
	and	nm_usuario_colab = nm_usuario_colab_p;

	/* gerar análise mês / colaborador - caso não existir */
	...
	if (nr_seq_analise_mes_w = 0) then
		...
		where	w.nr_sequencia = (
				...
				and	x.ie_analise = 'C'
				and	x.nm_usuario_colab = nm_usuario_colab_p);
		...
		and	w.ie_analise = 'C'
		and	w.nm_usuario_colab = nm_usuario_colab_p;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_analise_prod_migr_co_mes (nm_usuario_colab_p text) FROM PUBLIC;
