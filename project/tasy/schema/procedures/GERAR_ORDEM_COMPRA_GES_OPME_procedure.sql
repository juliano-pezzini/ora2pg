...
CREATE OR REPLACE PROCEDURE gerar_ordem_compra_ges_opme ( nm_usuario_p text, cd_estabelecimento_p bigint, cd_pessoa_fisica_p text, nr_seq_protocolo_p bigint ) AS $body$
DECLARE
...
if (coalesce(cond_pag_padrao_w::text, '') = '') then 
	CALL Wheb_mensagem_pck.exibir_mensagem_abort(263044);
end if;
...
if (coalesce(moeda_padrao_w::text, '') = '') then 
	CALL Wheb_mensagem_pck.exibir_mensagem_abort(263045);
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_ordem_compra_ges_opme ( nm_usuario_p text, cd_estabelecimento_p bigint, cd_pessoa_fisica_p text, nr_seq_protocolo_p bigint ) FROM PUBLIC;
