...
CREATE OR REPLACE PROCEDURE gerar_interf_sus_bpa_rp_reenv ( nr_seq_protocolo_p bigint, nr_folha_bpa_p bigint, nr_folha_bpi_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
WHERE a.nr_seq_protocolo	= nr_seq_protocolo_p and a.nr_atendimento	= b.nr_atendimento and a.nr_interno_conta	= c.nr_interno_conta   and c.ie_origem_proced	= 7 and coalesce(c.cd_motivo_exc_conta::text, '') = '' and sus_obter_tiporeg_proc(c.cd_procedimento, c.ie_origem_proced, 'C', 1) = 2 and sus_consiste_proc_comp(b.nr_atendimento, a.nr_interno_conta, c.cd_procedimento) = 'S' and a.nr_interno_conta	= f.nr_interno_conta and f.ie_status_ajuste	= 'A' and f.ie_situacao_retorno	= 'G' and f.dt_envio_conta	= dt_envio_conta_w order by	cd_cns_medico_exec, 
		dt_atendimento, 
		cd_procedimento, 
		cd_cbo, 
		nr_idade_pac;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_interf_sus_bpa_rp_reenv ( nr_seq_protocolo_p bigint, nr_folha_bpa_p bigint, nr_folha_bpi_p bigint, nm_usuario_p text) FROM PUBLIC;
