...
CREATE OR REPLACE PROCEDURE gerar_rec_amb_onc_agrup (( nr_seq_paciente_p bigint, nr_ciclo_inicial_p bigint, nr_ciclo_final_p bigint, nr_atendimento_p bigint, nm_usuario_p text, ie_gerado_p out text) is cd_material_w bigint) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	begin
		if	((coalesce(nr_seq_receita_w::text, '') = '') or (nr_dias_receita_w <> nr_dias_receita_ant_w)) then
			...
			insert	into fa_receita_farmacia(	nr_sequencia,
					...
					values (	nr_seq_receita_w,
								...
								clock_timestamp(),
								CASE WHEN coalesce(nr_dias_receita_w,0)=0 THEN null  ELSE clock_timestamp() + nr_dias_receita_w END ,
...
;
		...
		insert into fa_receita_farmacia_item(	nr_sequencia,	
				...
				values (		nextval('fa_receita_farmacia_item_seq'),
							...
							ds_dose_diferenciada_w,
							clock_timestamp(),
							CASE WHEN coalesce(qt_dias_util_w,0)=0 THEN null  ELSE clock_timestamp() + qt_dias_util_w END );
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_rec_amb_onc_agrup (( nr_seq_paciente_p bigint, nr_ciclo_inicial_p bigint, nr_ciclo_final_p bigint, nr_atendimento_p bigint, nm_usuario_p text, ie_gerado_p out text) is cd_material_w bigint) FROM PUBLIC;
