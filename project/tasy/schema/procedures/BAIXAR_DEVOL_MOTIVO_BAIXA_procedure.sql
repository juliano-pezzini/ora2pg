...
CREATE OR REPLACE PROCEDURE baixar_devol_motivo_baixa ( nr_devolucao_p bigint, cd_tipo_baixa_p bigint, nr_sequencia_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (ie_atualiza_estoque_w = 'S') or (ie_atualiza_conta_w = 'S') then 
	-- Para baixar por motivo, o tipo de baixa não deve atualizar a conta ou o estoque! 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE baixar_devol_motivo_baixa ( nr_devolucao_p bigint, cd_tipo_baixa_p bigint, nr_sequencia_p bigint, nm_usuario_p text) FROM PUBLIC;
