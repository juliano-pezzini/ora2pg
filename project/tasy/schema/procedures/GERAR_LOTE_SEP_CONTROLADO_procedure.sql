...
CREATE OR REPLACE PROCEDURE gerar_lote_sep_controlado ( nr_prescricao_p bigint, nr_sequencia_p bigint, nr_seq_horario_p bigint, ie_so_aprazado_p text, nm_usuario_p text, ie_origem_lote_p text) AS $body$
DECLARE
...
C01 CURSOR FOR  --especifico para medicamentos controlados
	...
	and	obter_se_medic_controlado(b.cd_material) = 'S'
	...
	and	((coalesce(ie_itens_associados_w,'S') = 'N') or
		(coalesce(ie_itens_associados_w,'S') = 'S' and ((coalesce(b.nr_seq_superior::text, '') = '') or (exists (	SELECT	1
																							...
																							and 	obter_se_medic_controlado(d.cd_material) = 'N')))))
	and	((ie_gerar_acm_w = 'S') or (ie_gerar_acm_w = 'N' and coalesce(c.ie_acm, 'N') = 'N') or ((ie_gerar_lote_acm_sn_uti_w = 'S') and (obter_classif_setor(a.cd_setor_atendimento) = 4)) or (obter_se_hor_dose_especial(b.nr_sequencia) = 'S'))
	and	((ie_gerar_sn_w = 'S') or (ie_gerar_sn_w = 'N' and coalesce(c.ie_se_necessario, 'N') = 'N') or ((ie_gerar_lote_acm_sn_uti_w = 'S') and (obter_classif_setor(a.cd_setor_atendimento) = 4)) or (obter_se_hor_dose_especial(b.nr_sequencia) = 'S'))
...
c10 CURSOR FOR
	...
	and	(b.nr_seq_classif IS NOT NULL AND b.nr_seq_classif::text <> '')
	and	coalesce(c.nr_sequencia_solucao::text, '') = ''
	and obter_se_associado(a.nr_prescricao, b.nr_seq_superior, nr_seq_prescr_mat_w) = 'S'
...
union all
 /*apenas para diluicao Prescricao por solucao*/
	SELECT  b.cd_material,
		b.nr_sequencia,
		b.cd_unidade_medida,
		b.qt_dispensar,
	...
	and	a.nr_prescricao = nr_prescricao_p
	and	x.cd_material	= c.cd_material
	and	a.nr_prescricao	= s.nr_prescricao
	...
	and	(b.nr_seq_classif IS NOT NULL AND b.nr_seq_classif::text <> '')
	and	(c.nr_sequencia_solucao IS NOT NULL AND c.nr_sequencia_solucao::text <> '')
	and	c.nr_sequencia_diluicao = nr_seq_prescr_mat_w
	and	c.cd_motivo_baixa = 0
	and	coalesce(b.ie_gerar_lote,'S') = 'S'
	and	coalesce(c.ie_gerar_lote,'S') = 'S'
	and	coalesce(a.dt_suspensao::text, '') = ''
	...
	and	(b.nr_seq_turno IS NOT NULL AND b.nr_seq_turno::text <> '')
	and	b.nr_seq_turno = nr_seq_turno_w
	and	((ie_gerar_lote_null_w = 'S') or (c.ds_horarios IS NOT NULL AND c.ds_horarios::text <> '') or (ie_origem_lote_p in ('AIP','AIS')))
	and	((VarNaoGerarSoPrimeiraEtapa_w = 'N') or (coalesce(b.nr_etapa_sol,1) = 1)  or (ie_origem_lote_p = 'GASR'))
	and	((ie_gerar_acm_w = 'S') or (ie_gerar_acm_w = 'N' and coalesce(c.ie_acm, 'N') = 'N') or ((ie_gerar_lote_acm_sn_uti_w = 'S') and (obter_classif_setor(a.cd_setor_atendimento) = 4)) or (obter_se_hor_dose_especial(b.nr_sequencia) = 'S'))
	and	((ie_gerar_sn_w = 'S') or (ie_gerar_sn_w = 'N' and coalesce(c.ie_se_necessario, 'N') = 'N') or ((ie_gerar_lote_acm_sn_uti_w = 'S') and (obter_classif_setor(a.cd_setor_atendimento) = 4)) or (obter_se_hor_dose_especial(b.nr_sequencia) = 'S'))
	and	((ie_origem_lote_p <> 'AIS') or	(ie_origem_lote_p = 'AIS' AND b.ie_aprazado = 'S'))
...
if (ie_setor_atual_w = 'S') and (ie_origem_lote_p in ('AIP','GAI', 'AIS')) then
...
values (nextval('ap_lote_log_seq'),
	...
	' ie_gerar_lote_null_w='           || ie_gerar_lote_null_w           || ' ie_gerar_acm_w='             || ie_gerar_acm_w             ||
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if	((ie_gerar_ap_lote_w = 'S') or ((ie_origem_lote_p = 'AIP') or (ie_origem_lote_p = 'AIS'))) and
		((nr_seq_turno_w <> nr_seq_turno_ant_w AND ie_mesmo_turno_w <> 'U') or (ie_mesmo_turno_w = 'N') or (nr_seq_classif_w <> nr_seq_classif_ant_w) or
		((PKG_DATE_UTILS.start_of(dt_horario_w,'dd',0) <> PKG_DATE_UTILS.start_of(dt_horario_ant_w,'dd',0)) and (ie_gerar_novo_lote_turno_dia_w = 'S') and (ie_mesmo_turno_w <> 'U')) or
		(((dt_horario_w - dt_inicio_turno_w) >= 1) and (ie_novo_lote_dia_w = 'S') and (ie_mesmo_turno_w <> 'U')) or (cd_local_estoque_w <> cd_local_estoque_ant_w) or (ie_gerou_w = 'N')) then
		begin
		OPEN C02;
		LOOP
			FETCH C02 INTO
		...
		if (hr_hora_w < hr_inicio_turno_w) then
			...
			dt_inicio_turno_w	:= pkg_date_utils.get_time(dt_horario_w, REPLACE(hr_inicio_turno_w,'24:','00:') ||':00');
		end if;
		...
		values ( nr_sequencia_w,
			...
			'CL',
			nr_atendimento_w);
	     if (ie_send_integration_w ='S') then               CALL acceptance_submission_cancel(nr_sequencia_w,clock_timestamp(),null,'A',nr_prescricao_p,nm_usuario_p);
	     end if;
		end;
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_lote_sep_controlado ( nr_prescricao_p bigint, nr_sequencia_p bigint, nr_seq_horario_p bigint, ie_so_aprazado_p text, nm_usuario_p text, ie_origem_lote_p text) FROM PUBLIC;
