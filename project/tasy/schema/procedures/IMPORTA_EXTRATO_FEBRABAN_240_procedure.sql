...
CREATE OR REPLACE PROCEDURE importa_extrato_febraban_240 ( nr_seq_extrato_p bigint) AS $body$
DECLARE
...
/* Saldos inicial e final sem lancamentos */
...
SELECT	a.nr_sequencia,
	to_date(substr(a.ds_conteudo,143,8),'dd/mm/yyyy') dt_lancto,
	somente_numero(substr(a.ds_conteudo,151,18)) / 100 vl_lancto,
	substr(ds_conteudo,169,1) ie_cred_deb,
	substr(ds_conteudo,59,12) nr_conta,
	substr(ds_conteudo,71,1) digito_conta
from	W_interf_concil a
where	substr(a.ds_conteudo,8,1)	= '1'
...
WHERE (a.nr_sequencia = nr_seq_conta_w or
			...
			and	x.nr_sequencia		= nr_seq_conta_w)) and somente_numero(a.cd_conta)||somente_numero(a.ie_digito_conta) = somente_numero(nr_conta_w)||somente_numero(coalesce(trim(both digito_conta_w),a.ie_digito_conta)) and cd_banco				= cd_banco_w and coalesce(a.ie_situacao,'A')			= 'A' and coalesce(b.ie_classif_conta,'CC') <> 'CA';
...
WHERE (a.nr_sequencia = nr_seq_conta_w or
				...
				where	x.ie_digito_conta	= a.ie_digito_conta
				and	x.cd_conta		= a.cd_conta
				and	x.nr_sequencia		= nr_seq_conta_w)) and somente_numero(a.ie_digito_conta)	= somente_numero(coalesce(trim(both digito_conta_w),a.ie_digito_conta)) and somente_numero(a.cd_conta)		= somente_numero(nr_conta_w) and cd_banco				= cd_banco_w and coalesce(a.ie_situacao,'A')			= 'A' and coalesce(b.ie_classif_conta,'CC') <> 'CA';
...
WHERE (a.nr_sequencia = nr_seq_conta_w or
				...
				where	x.ie_digito_conta	= a.ie_digito_conta
				and	x.cd_conta		= a.cd_conta
				and	x.nr_sequencia		= nr_seq_conta_w)) and somente_numero(a.ie_digito_conta)	= somente_numero(coalesce(trim(both substr(nr_conta_w,13,1)),a.ie_digito_conta)) and somente_numero(a.cd_conta)		= somente_numero(substr(nr_conta_w,1,12)) and cd_banco				= cd_banco_w and coalesce(a.ie_situacao,'A')			= 'A' and coalesce(b.ie_classif_conta,'CC') <> 'CA';
...
/* Importar os saldos inicial e final das contas que nao possuem lancamento */
...
EXIT WHEN NOT FOUND; /* apply on c02 */
	...
	and	substr(a.ds_conteudo,8,1)	= '5'
	and	a.nr_seq_conta		= nr_seq_conta_W;
...
WHERE (a.nr_sequencia = nr_seq_conta_w or
		...
		select	to_date(substr(a.ds_conteudo,143,8),'dd/mm/yyyy') dt_lancto,
			somente_numero(substr(a.ds_conteudo,151,18)) / 100 vl_lancto,
			substr(a.ds_conteudo,169,1) ie_deb_cred
		into STRICT	dt_saldo_final_w,
			vl_saldo_final_w,
			ie_deb_cred_saldo_fin_w
		from	w_interf_concil a
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE importa_extrato_febraban_240 ( nr_seq_extrato_p bigint) FROM PUBLIC;
