...
CREATE OR REPLACE PROCEDURE agenda_paciente_atribdblclick ( nr_seq_agenda_p bigint, cd_agenda_p bigint, cd_pessoa_fisica_p text, dt_agenda_p timestamp, hr_inicio_p timestamp, cd_estabelecimento_p bigint, cd_perfil_p bigint, nm_usuario_p text, ie_se_perm_pf_classif_p INOUT text, nm_pessoa_fisica_p INOUT text, dt_nascimento_pac_p INOUT timestamp, qt_idade_paciente_p INOUT text, qt_idade_meses_p INOUT text, nr_telefone_p INOUT text, cd_procedencia_p INOUT text, ds_sexo_p INOUT text, cd_convenio_p INOUT bigint, cd_categoria_p INOUT text, cd_usuario_convenio_p INOUT text, dt_validade_carteira_p INOUT timestamp, nr_doc_convenio_p INOUT text, cd_tipo_acomodacao_p INOUT bigint, cd_plano_p INOUT text, cd_usuario_plano_p INOUT text, dt_validade_p INOUT timestamp, qt_altura_cm_p INOUT bigint, qt_peso_p INOUT bigint, ds_email_p INOUT text) AS $body$
DECLARE
...
ie_atualiza_procedencia_w	varchar(1);
...
if (cd_agenda_p > 0) then 
	...
	if (cd_pessoa_fisica_p IS NOT NULL AND cd_pessoa_fisica_p::text <> '') then 
		...
		dt_nascimento_pac_w	:= obter_data_nascto_pf(cd_pessoa_fisica_p);
		...
		ds_email_w		:= substr(obter_compl_pf(cd_pessoa_fisica_p, 1, 'M'),1,255);
		ie_atualiza_procedencia_w := Obter_Param_Usuario(820, 368, cd_perfil_p, nm_usuario_p, cd_estabelecimento_p, ie_atualiza_procedencia_w);
		if (ie_atualiza_procedencia_w = 'S') then 
		...
		qt_altura_cm_p		:= obter_dados_pf(cd_pessoa_fisica_p, 'AL');
		qt_peso_p		:= obter_dados_pf(cd_pessoa_fisica_p, 'KG');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE agenda_paciente_atribdblclick ( nr_seq_agenda_p bigint, cd_agenda_p bigint, cd_pessoa_fisica_p text, dt_agenda_p timestamp, hr_inicio_p timestamp, cd_estabelecimento_p bigint, cd_perfil_p bigint, nm_usuario_p text, ie_se_perm_pf_classif_p INOUT text, nm_pessoa_fisica_p INOUT text, dt_nascimento_pac_p INOUT timestamp, qt_idade_paciente_p INOUT text, qt_idade_meses_p INOUT text, nr_telefone_p INOUT text, cd_procedencia_p INOUT text, ds_sexo_p INOUT text, cd_convenio_p INOUT bigint, cd_categoria_p INOUT text, cd_usuario_convenio_p INOUT text, dt_validade_carteira_p INOUT timestamp, nr_doc_convenio_p INOUT text, cd_tipo_acomodacao_p INOUT bigint, cd_plano_p INOUT text, cd_usuario_plano_p INOUT text, dt_validade_p INOUT timestamp, qt_altura_cm_p INOUT bigint, qt_peso_p INOUT bigint, ds_email_p INOUT text) FROM PUBLIC;
