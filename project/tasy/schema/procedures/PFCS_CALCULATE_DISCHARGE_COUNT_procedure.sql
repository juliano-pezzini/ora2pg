...
CREATE OR REPLACE PROCEDURE pfcs_calculate_discharge_count ( nr_seq_indicator_p bigint, cd_estabelecimento_p text, nm_usuario_p text) AS $body$
DECLARE
...
SELECT eid.ds_value nr_encounter,
  ...
  /* Cursor to calculate discharge count for tasy */
	...
	cur_get_tot_disc_count_tasy CURSOR FOR
...
BEGIN
  ...
  select ie_table_origin,
    ...
    qt_total_w := 0;
	 for r_c01 in cur_get_tot_disc_count_tasy loop
      begin
		qt_total_w := qt_total_w + 1;
		select nextval('pfcs_panel_detail_seq') into STRICT pfcs_panel_detail_seq_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pfcs_calculate_discharge_count ( nr_seq_indicator_p bigint, cd_estabelecimento_p text, nm_usuario_p text) FROM PUBLIC;
