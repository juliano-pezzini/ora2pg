...
CREATE OR REPLACE PROCEDURE pls_oc_cta_tratar_val_28 ( dados_regra_p pls_tipos_ocor_pck.dados_regra, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, dados_forma_geracao_ocor_p pls_tipos_ocor_pck.dados_forma_geracao_ocor, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
C01 CURSOR(	nr_seq_oc_cta_comb_p	dados_regra_p.nr_sequencia%type) FOR
	SELECT	a.nr_sequencia	nr_seq_validacao,
		a.ie_val_vl_apres_a_maior,
		coalesce(a.vl_tolerancia, 0) vl_tolerancia,
		coalesce(a.pr_tolerancia,0) pr_tolerancia
	from	pls_oc_cta_val_vl_ap_maior a
...
if (dados_regra_p.nr_sequencia IS NOT NULL AND dados_regra_p.nr_sequencia::text <> '') then
	...
	-- Se for ações de aceitar valores ou ajustar, leva em consideração o valor liberado para verificar a glosa.
	-- Isso acontece porque, por exemplo, no caso de aceitar valor apresentado que é R$500,00  e o calculado de R$176,00, a glosa ficaria ativa sempre
	if (dados_forma_geracao_ocor_p.cd_acao_analise in (1, 2, 3, 4, 5)) then
		ie_usa_vl_liberado_w := true;
	else
		ie_usa_vl_liberado_w := false;
	end if;
	...
	for	r_C01_w in C01(dados_regra_p.nr_sequencia) loop
		...
		if (r_C01_w.ie_val_vl_apres_a_maior = 'S') then
			...
			for	r_C02_w in C02(nr_id_transacao_p) loop
				...
				if (ie_usa_vl_liberado_w = true) then
					-- usa somente o valor liberado para esta ação
					vl_calculado_comparacao_w := r_C02_w.vl_liberado;
				else
					if (r_C01_w.vl_tolerancia > 0) then
					...
					elsif (r_C01_w.pr_tolerancia > 0) then
						vl_calculado_comparacao_w := r_C02_w.vl_calculado + dividir(r_C01_w.pr_tolerancia*r_C02_w.vl_calculado,100);
					else
						vl_calculado_comparacao_w := r_C02_w.vl_calculado;
					end if;
				end if;

				-- se o apresentado for maior que o calculado
				if (r_C02_w.vl_apresentado > vl_calculado_comparacao_w) then

					ie_gera_ocor_w := 'S';

					-- só não gera a ocorrência se o valor calculado for zero e o parâmetro indicar que não deve gerar ocorrência
					if (r_C02_w.vl_calculado = 0 and ie_gerar_ocor_valor_zerado_w = 'N') then
						ie_gera_ocor_w := 'N';
					end if;
				end if;
				...
				if (ie_gera_ocor_w = 'S') then
					ds_observacao_w(i) := 	'Valor apresentado superior ao valor calculado. ' || pls_util_pck.enter_w ||
								'Valor apresentado: ' || r_C02_w.vl_apresentado || pls_util_pck.enter_w ||
								'Valor calculado: ' || vl_calculado_comparacao_w;
				end if;
			...
			for	r_C03_w in C03(nr_id_transacao_p) loop
				...
				if (ie_usa_vl_liberado_w = true) then
					-- usa somente o valor liberado para esta ação
					vl_calculado_comparacao_w := r_C03_w.vl_liberado;
				else
					if (r_C01_w.vl_tolerancia > 0) then
						-- para comparação é utilizado o valor calculado juntamente com o valor de tolerancia
						vl_calculado_comparacao_w := (r_C03_w.vl_calculado + r_C01_w.vl_tolerancia);
					elsif (r_C01_w.pr_tolerancia > 0) then
						vl_calculado_comparacao_w := r_C03_w.vl_calculado + dividir(r_C01_w.pr_tolerancia*r_C03_w.vl_calculado,100);
					else
						vl_calculado_comparacao_w := r_C03_w.vl_calculado;
					end if;
				end if;

				-- se o apresentado for maior que o calculado
				if (r_C03_w.vl_apresentado > vl_calculado_comparacao_w) then

					ie_gera_ocor_w := 'S';

					-- só não gera a ocorrência se o valor calculado for zero e o parâmetro indicar que não deve gerar ocorrência
					if (r_C03_w.vl_calculado = 0 and ie_gerar_ocor_valor_zerado_w = 'N') then
						ie_gera_ocor_w := 'N';
					end if;
				end if;
				...
				if (ie_gera_ocor_w = 'S') then
					ds_observacao_w(i) := 	'Valor apresentado superior ao valor calculado. ' || pls_util_pck.enter_w ||
								'Valor apresentado: ' || r_C03_w.vl_apresentado || pls_util_pck.enter_w ||
								'Valor calculado: ' || vl_calculado_comparacao_w;
				end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_oc_cta_tratar_val_28 ( dados_regra_p pls_tipos_ocor_pck.dados_regra, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, dados_forma_geracao_ocor_p pls_tipos_ocor_pck.dados_forma_geracao_ocor, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
