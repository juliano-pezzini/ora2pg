...
CREATE OR REPLACE PROCEDURE glosa_mat_percentual (nr_sequencia_p bigint, pr_glosa_p bigint, cd_convenio_glosa_p bigint, cd_categoria_glosa_p text) AS $body$
DECLARE
...
vl_tabela_original_ww		double precision;
...
--vl_material_ww	:= trunc(dividir((vl_material_w * pr_glosa_w),100),4);

--vl_material_ww	:= trunc(dividir((vl_unitario_w * qt_material_w * pr_glosa_w),100),4);
vl_material_ww	:= trunc((vl_unitario_w * qt_material_w * pr_glosa_w) / 100,4);
--vl_unitario_ww	:= trunc(dividir(vl_material_ww,qt_material_w),4);
if (qt_material_w <> 0) then
	vl_unitario_ww	:= trunc((vl_material_ww / qt_material_w),4);
else
	vl_unitario_ww	:= trunc(dividir(vl_material_ww,qt_material_w),4);
end if;
...
update material_atend_paciente
set	ie_valor_informado	= 'S',
	--vl_material		= vl_material_w - vl_material_ww,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE glosa_mat_percentual (nr_sequencia_p bigint, pr_glosa_p bigint, cd_convenio_glosa_p bigint, cd_categoria_glosa_p text) FROM PUBLIC;
