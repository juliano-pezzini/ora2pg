...
CREATE OR REPLACE PROCEDURE gerar_transf_pac_leitos ( nr_atendimento_p bigint, cd_setor_atendimento_p bigint, cd_unidade_basica_p text, cd_unidade_compl_p text, cd_tipo_acomodacao_p bigint, nr_acompanhantes_p bigint, nr_seq_motivo_transf_p bigint, ds_observacao_p text, nm_usuario_p text, dt_entrada_unidade_p timestamp) AS $body$
DECLARE
...
ds_leito_w				varchar(30);
ds_leito_ant_w			varchar(30);
...
union all
	...
	and		not exists (
						...
						from	cirurgia x
						where	x.nr_prescricao = a.nr_prescricao)
	and 	((ie_susp_rep_transf_w <> 'R') or (OBTER_SE_SUSP_TRANSFER(cd_setor_atend_regra_w, 'N') = 'S'))
...
union all
	...
	and		not exists (
						...
						from	cirurgia x
						where	x.nr_prescricao = a.nr_prescricao)
	and 	((ie_susp_rep_transf_w <> 'R') or (OBTER_SE_SUSP_TRANSFER(cd_setor_atend_regra_w, 'M') = 'S'))
...
union all
	...
	and 	((ie_susp_rep_transf_w <> 'R') or (OBTER_SE_SUSP_TRANSFER_PROC(cd_setor_atend_regra_w, b.nr_seq_proc_interno) = 'S'))
...
union all
	...
	and		not exists (
						...
						and (coalesce(y.cd_motivo_baixa,0) <> 0))
	and 	((ie_susp_rep_transf_w <> 'R') or (OBTER_SE_SUSP_TRANSFER(cd_setor_atend_regra_w, 'R') = 'S'))
...
union all
	...
	and		not exists (
						...
						and (coalesce(y.cd_motivo_baixa,0) <> 0))
	and 	((ie_susp_rep_transf_w <> 'R') or (OBTER_SE_SUSP_TRANSFER(cd_setor_atend_regra_w, 'G') = 'S'));
...
ds_leito_ant_w 	:= substr(cd_unidade_basica_ww || ' - ' || cd_unidade_compl_ww,1,10);
CALL gerar_int_dankia_pck.dankia_gerar_movimentacao(nr_atendimento_p, cd_setor_atendimento_p, cd_setor_atual_w, ds_leito_w, ds_leito_ant_w, cd_estabelecimento_w, nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_transf_pac_leitos ( nr_atendimento_p bigint, cd_setor_atendimento_p bigint, cd_unidade_basica_p text, cd_unidade_compl_p text, cd_tipo_acomodacao_p bigint, nr_acompanhantes_p bigint, nr_seq_motivo_transf_p bigint, ds_observacao_p text, nm_usuario_p text, dt_entrada_unidade_p timestamp) FROM PUBLIC;
