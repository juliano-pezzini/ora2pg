...
CREATE OR REPLACE PROCEDURE pls_gerar_estima_copartic_guia ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
c_regras_copart CURSOR FOR 	-- Pega as regras de coparticipacao do contrato (1a Parte do union) , do intercambio (2a parte do union), ou do plano caso nao exista no contrato (3a Parte do union) 
...
select	max(a.nr_seq_segurado),
	max(a.cd_guia),
	max(a.ie_tipo_guia),
	max(trunc(coalesce(a.dt_autorizacao, a.dt_solicitacao))),
	...
	max(pls_obter_internado_guia(a.nr_sequencia, 'A')),
...
-- Obter o tipo do beneficiario da guia
...
-- Caso ja exista uma estimativa de coparticipacao para a guia em questao, deve ser atulizado para a mais recente
delete	from pls_guia_coparticipacao
where	nr_seq_guia = nr_seq_guia_p;

for r_proc_guia_w in c_proc_guia loop
	ie_depende_regra_w	:= 'N';
	ie_liberado_w		:= 'N';
	
	if (r_proc_guia_w.qt_autorizada	= 0) then
		qt_autorizada_w	:= r_proc_guia_w.qt_solicitada;
	else
		qt_autorizada_w	:= r_proc_guia_w.qt_autorizada;
	end if;
	
	-- Obter a estrutura do procedimento selecionado no cursor
	SELECT * FROM pls_obter_estrut_proc(	r_proc_guia_w.cd_procedimento, r_proc_guia_w.ie_origem_proced, 	-- IN
	...
	if (ie_calculo_coparticipacao_w	= 'R') and (r_proc_guia_w.nr_seq_pacote IS NOT NULL AND r_proc_guia_w.nr_seq_pacote::text <> '') then
		...
		if (ie_tipo_valor_w	= 1) then
			...
			SELECT * FROM pls_define_preco_pacote(cd_estabelecimento_p, nr_seq_prestador_w, nr_seq_tipo_acomodacao_w, 			-- IN
						dt_autorizacao_w, r_proc_guia_w.cd_procedimento, r_proc_guia_w.ie_origem_proced, 	-- IN
						...
						null, nr_seq_segurado_w, null, 'N', null, 1) INTO STRICT 
	...
	if (ie_gerar_copartic_w = 'N') then
		...
		for r_regras_copart_w in c_regras_copart loop
			if (r_regras_copart_w.ie_tipo_atendimento = ie_tipo_atend_w) or (r_regras_copart_w.ie_tipo_atendimento = 'A')	then
				...
				--So entra no if para o valor de coparticipacao fixo
				if (ie_tipo_guia_w in ('1','8')) and (cont_w > 0) then
					...
					where	nr_seq_guia_proc = r_proc_guia_w.nr_sequencia;
					
					if (coalesce(ie_liberado_w,'N') = 'S') and (coalesce(r_regras_copart_w.vl_coparticipacao,0) > 0) and (qt_coparticipacao_w > 0) then
						vl_coparticipacao_w	:= r_regras_copart_w.vl_coparticipacao;

						insert	into pls_guia_coparticipacao(nr_sequencia, dt_atualizacao, nm_usuario,
							 dt_atualizacao_nrec, nm_usuario_nrec, nr_seq_guia,
							 nr_seq_guia_proc, vl_coparticipacao, nr_seq_regra,
							 nr_seq_regra_copartic)
						values (nextval('pls_guia_coparticipacao_seq'), clock_timestamp(), nm_usuario_p,
							 clock_timestamp(), nm_usuario_p, nr_seq_guia_p,
							 null, vl_coparticipacao_w, nr_seq_regra_w,
				...
				for r_copartic_proc_w in c_copartic_proc(	r_proc_guia_w.cd_procedimento, r_proc_guia_w.ie_origem_proced, cd_grupo_w,
					...
					if (r_copartic_proc_w.nr_seq_grupo_servico IS NOT NULL AND r_copartic_proc_w.nr_seq_grupo_servico::text <> '') then
						...
						ie_proc_grupo_serv_w	:= pls_se_grupo_preco_servico(r_copartic_proc_w.nr_seq_grupo_servico, r_proc_guia_w.cd_procedimento, r_proc_guia_w.ie_origem_proced);
				...
				if (coalesce(ie_liberado_w,'N') = 'S') then
					...
					if (qt_registros_w = 0) then
						...
						if (r_regras_copart_w.vl_maximo	> 0) and (vl_copartic_unit_w > r_regras_copart_w.vl_maximo) then
						...
						if (coalesce(vl_coparticipacao_w,0) > 0) then
							...
							values (nextval('pls_guia_coparticipacao_seq'), clock_timestamp(), nm_usuario_p,
								 clock_timestamp(), nm_usuario_p, nr_seq_guia_p,
								 r_proc_guia_w.nr_sequencia, vl_coparticipacao_w, nr_seq_regra_w,
								 r_regras_copart_w.nr_seq_regra_coparticipacao);
						end if;
					end if;
					ie_liberado_w	:= 'N';
...
for r_mat_guia_w in c_mat_guia loop
	ie_liberado_w	:= 'N';

	if (r_mat_guia_w.qt_autorizada	= 0) then
		qt_autorizada_w	:= r_mat_guia_w.qt_solicitada;
		
	else
		qt_autorizada_w	:= r_mat_guia_w.qt_autorizada;
	end if;
	...
	begin
		...
		into STRICT	ie_tipo_despesa_w,
			nr_seq_estrut_mat_w
		from	pls_material
		where	nr_sequencia	= r_mat_guia_w.nr_seq_material;
	exception
	when others then
	...
	SELECT * FROM pls_define_preco_material(	cd_estabelecimento_p, nr_seq_prestador_w, dt_autorizacao_w, r_mat_guia_w.nr_sequencia, 4, ie_tipo_despesa_w, null, ie_calculo_copartic_w, nr_seq_outorgante_w, nr_seq_segurado_w, null, r_mat_guia_w.nr_seq_prest_fornec, nr_seq_categoria_w, nr_seq_tipo_acomodacao_w, nr_seq_tipo_atendimento_w, nr_seq_clinica_w, ie_tipo_guia_w, null, null, 'N', nm_usuario_p, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, vl_material_w, dt_ult_vigencia_w, nr_seq_material_preco_w, vl_material_simpro_w, vl_material_brasindice_w, vl_material_tabela_w, dados_regra_preco_material_w, null, ie_regime_atendimento_w, ie_saude_ocupacional_w) INTO STRICT _ora2pg_r;
 vl_material_w := _ora2pg_r.vl_material_p; dt_ult_vigencia_w := _ora2pg_r.dt_ult_vigencia_p; nr_seq_material_preco_w := _ora2pg_r.nr_seq_material_preco_p; vl_material_simpro_w := _ora2pg_r.vl_material_simpro_p; vl_material_brasindice_w := _ora2pg_r.vl_material_brasindice_p; vl_material_tabela_w := _ora2pg_r.vl_material_tabela_p; dados_regra_preco_material_w := _ora2pg_r.dados_regra_preco_material_p;
	...
	vl_liberado_w 	:= (vl_material_w * r_mat_guia_w.qt_autorizada);
		...
		for r_regras_copart_w in c_regras_copart loop
			if (r_regras_copart_w.ie_tipo_atendimento = ie_tipo_atend_w) or (r_regras_copart_w.ie_tipo_atendimento = 'A')then
				...
				if (ie_tipo_guia_w in ('1','8')) and (cont_w > 0) then
					...
					into STRICT	qt_coparticipacao_w
					from	pls_guia_coparticipacao
					where	nr_seq_guia_mat = r_mat_guia_w.nr_sequencia;
				...
				for r_copartic_mat_w in c_copartic_mat(r_mat_guia_w.nr_seq_material, ie_tipo_despesa_w, r_regras_copart_w.nr_seq_tipo_coparticipacao) loop
					ie_estrut_mat_w	:= 'S';
					if (r_copartic_mat_w.nr_seq_estrut_mat IS NOT NULL AND r_copartic_mat_w.nr_seq_estrut_mat::text <> '') then
						if (pls_obter_se_mat_estrutura(r_mat_guia_w.nr_seq_material, r_copartic_mat_w.nr_seq_estrut_mat) = 'N') then
							ie_estrut_mat_w	:= 'N';
						end if;
					end if;
				...
				if (coalesce(ie_liberado_w,'N') = 'S') then
					...
					if (qt_registros_w = 0) then
						...
						if (r_regras_copart_w.vl_maximo	> 0) and (vl_copartic_unit_w	> r_regras_copart_w.vl_maximo) then
							vl_coparticipacao_w	:= (r_regras_copart_w.vl_maximo * r_mat_guia_w.qt_autorizada);
							vl_copartic_unit_w	:= r_regras_copart_w.vl_maximo;
						else
							vl_coparticipacao_w	:= ((coalesce(vl_liberado_w,0) * r_regras_copart_w.tx_coparticipacao) / 100);
						end if;
						...
						if (coalesce(vl_coparticipacao_w,0) > 0) then
							insert into pls_guia_coparticipacao(nr_sequencia, dt_atualizacao, nm_usuario,
								 dt_atualizacao_nrec, nm_usuario_nrec, nr_seq_guia,
								 nr_seq_guia_mat, vl_coparticipacao, nr_seq_regra,
								 nr_seq_regra_copartic)
							values (nextval('pls_guia_coparticipacao_seq'), clock_timestamp(), nm_usuario_p,
								 clock_timestamp(), nm_usuario_p, nr_seq_guia_p,
								 r_mat_guia_w.nr_sequencia, vl_coparticipacao_w, nr_seq_regra_w,
								 r_regras_copart_w.nr_seq_regra_coparticipacao);
						end if;
					end if;
					ie_liberado_w	:= 'N';
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_estima_copartic_guia ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
