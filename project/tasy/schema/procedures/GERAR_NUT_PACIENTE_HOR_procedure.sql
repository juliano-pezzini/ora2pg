...
CREATE OR REPLACE PROCEDURE gerar_nut_paciente_hor ( nr_prescricao_p bigint, nr_atendimento_p bigint, nr_seq_nut_pac_p bigint, dt_horario_p timestamp, ie_aprazando_p text, cd_estabelecimento_p bigint, cd_perfil_p bigint, nm_usuario_p text) AS $body$
DECLARE
			...
			if (ie_aprazar_w = 'S') then
				...
				if (ie_agora_impressao_w = 'S') then
					...
					if (hr_turno_agora_w > hr_final_turno_agora_w) then
						if (to_char(dt_horario_p,'dd/mm/yyyy') > to_char(dt_inicio_prescr_w,'dd/mm/yyyy')) then
							dt_limite_agora_w := trunc(ESTABLISHMENT_TIMEZONE_UTILS.dateAtTime(dt_horario_p-1, coalesce(hr_turno_agora_w, '00:00')), 'mi') - (qt_min_antes_atend_w/1440);
						else
							dt_limite_agora_w := trunc(ESTABLISHMENT_TIMEZONE_UTILS.dateAtTime(dt_horario_p, coalesce(hr_turno_agora_w, '00:00')), 'mi') - (qt_min_antes_atend_w/1440);
						end if;
					else
						dt_limite_agora_w := trunc(ESTABLISHMENT_TIMEZONE_UTILS.dateAtTime(dt_horario_p, coalesce(hr_turno_agora_w, '00:00')), 'mi') - (qt_min_antes_atend_w/1440);
					end if;
				...
				if (nr_prescr_atual_w <> nr_prescricao_w) and (coalesce(nr_seq_horario_w,0) > 0) then
					...
					CALL Gerar_Lote_Atend_Prescricao(nr_prescricao_w, nr_seq_item_w, nr_seq_horario_w, 'N', nm_usuario_p, 'AIP');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_nut_paciente_hor ( nr_prescricao_p bigint, nr_atendimento_p bigint, nr_seq_nut_pac_p bigint, dt_horario_p timestamp, ie_aprazando_p text, cd_estabelecimento_p bigint, cd_perfil_p bigint, nm_usuario_p text) FROM PUBLIC;
