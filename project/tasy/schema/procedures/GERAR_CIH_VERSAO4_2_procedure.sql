...
nr_declaracao_obito_w		varchar(8);
...
SELECT a.ie_tipo_reg_interface,  
	...
	nr_declaracao_obito, 
	qt_filhos_vivos, 
	Somente_Numero(elimina_aspas(replace(elimina_caracteres_especiais(doc_nasc1),'+',''))), 
	Somente_Numero(elimina_aspas(replace(elimina_caracteres_especiais(doc_nasc2),'+',''))), 
	Somente_Numero(elimina_aspas(replace(elimina_caracteres_especiais(doc_nasc3),'+',''))), 
	Somente_Numero(elimina_aspas(replace(elimina_caracteres_especiais(doc_nasc4),'+',''))), 
	Somente_Numero(elimina_aspas(replace(elimina_caracteres_especiais(doc_nasc5),'+',''))), 
...
and (ie_tipo_atendimento = 8 or coalesce(ie_somente_ambulatorial_w,'N') = 'N') 
...
and	coalesce(ie_tipo_reg_interface,1) = 2 
...
and (ie_tipo_atendimento	= 8 or 
	 coalesce(ie_somente_ambulatorial_w,'N') = 'N') 
...
select	max(IE_SOMENTE_INTERNADO), 
	coalesce(max(IE_SOMENTE_AMBULATORIAL),'N'), 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_cih_versao4_2 (nr_seq_lote_p bigint, ie_consiste_municipio_ibge_p text, ie_gerar_atend_sem_alta_uti_p text, ie_restringer_estab_atend_p text, ie_gerar_pac_cronico_p text, ds_motivo_alta_p text, ie_gera_atend_lote_p text, ds_versao_p text, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
