...
CREATE OR REPLACE PROCEDURE ptu_gerar_arq_mov_prestador_50 (nr_seq_lote_p bigint, cd_interface_p text, nm_usuario_p text, ds_local_p text, nm_arquivo_p text) AS $body$
DECLARE

 
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
Finalidade: Gerar arquivo A400 de acordo com a versão PTU 5.0 
------------------------------------------------------------------------------------------------------------------- 
Locais de chamada direta: 
[X] Objetos do dicionário [ ] Tasy (Delphi/Java) [ ] Portal [ ] Relatórios [ ] Outros: 
 ------------------------------------------------------------------------------------------------------------------ 
Pontos de atenção:	NÃO ADICIONAR MAIS NENHUMA FUNCTION 
		OU 
		TRATAMENTO AQUI DENTRO 
 
PERMITIDO:	LPAD	RPAD	NVL	SUBSTR	TO_CHAR 
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/ 
...
id_autoriz_divulga_w		varchar(1);
ie_408_w			varchar(1) := 'S';
...
c01 CURSOR FOR 
	SELECT	nr_sequencia, 
		...
		lpad(coalesce(nr_crm,'0'),8,'0'), 
		rpad(coalesce(uf_crm,' '),2,' '), 
		rpad(coalesce(nm_fantasia,' '),40,' '), 
		...
		rpad(coalesce(nm_diretor_tecnico,' '),40,' '), 
		...
		CASE WHEN ie_tipo_prestador=1 THEN 'N' WHEN ie_tipo_prestador=5 THEN 'N' WHEN ie_tipo_prestador=7 THEN 'N'  ELSE CASE WHEN coalesce(ie_instituicao_acred_ptu::text, '') = '' THEN 'N'  ELSE 'S' END  END  ie_prest_acred, 
...
C02 CURSOR FOR 
	SELECT	z.nr_sequencia, 
		...
		rpad(coalesce(z.ds_email,' '),40,' '), 
		rpad(coalesce(z.ds_endereco_web,' '),50,' '), 
		rpad(coalesce(z.cd_cnes,'9999999'),7,' '), 
...
ds_conteudo_w :=	lpad(nr_seq_registro_w,8,'0') || '401' || cd_unimed_destino_w || cd_unimed_origem_w || dt_geracao_w || '20' || id_ope_prest_w || 
			nr_registro_ans_w;
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	ds_conteudo_w :=	lpad(nr_seq_registro_w,8,'0') || '402' || tp_prest_w || cd_prest_w || cd_cgc_cpf_w || cd_insc_est_w || nr_conselho_w || 
				cd_uf_conselho_w || lpad(' ',40,' ') || nm_fantasia_w || tp_vinculo_w || cd_espec_1_w || cd_atua_1_w || cd_espec_2_w || cd_atua_2_w || 
				...
				dt_ini_contrato_w || nr_registro_ans_w || nm_diretor_tecnico_w || lpad(' ',8,' ') || tp_disponibilidade_w || id_tab_propria_w || 
				...
				id_guia_medico_atua_2_w || prest_acred_w || particip_notivisa_w || particip_qualiss_ans_w || nr_rce_espec_1_w || nr_rce_espec_2_w || 
				nr_rce_2_espec_1_w || nr_rce_2_espec_2_w || nr_rce_atua_1_w || nr_rce_atua_2_w || nr_rce_2_atua_1_w || nr_rce_2_atua_2_w || 
				indic_pos_grad_w || ' ' || nm_prest_w || id_publica_ans_w || nr_cbo_w || indic_residencia_w || cd_uni_prestadora_w || 
				id_login_wsd_tiss_w || id_cadu_w || id_inativo_w || sg_conselho_w;
	...
	EXIT WHEN NOT FOUND; /* apply on C02 */
		...
		ds_conteudo_w :=	lpad(nr_seq_registro_w,8,'0') || '403' || tp_end_w || ds_end_w || nr_end_w || lpad(' ',15,' ') || ds_bairro_w || cd_munic_w || 
					nr_cep_w || nr_ddd_w || nr_fone_1_w || nr_fone_2_w || nr_fax_w || ds_email_w || ds_endereco_web_w || cd_cnes_w || 
					...
					nr_latitude_w || nr_longitude_w || nr_leitos_intermed_w;
	...
	begin 
	select	lpad(coalesce(cd_cgc_cpf,'0'),15,'0'), 
		rpad(coalesce(ie_instituicao_acred_ptu,' '),5,' '), 
		coalesce(ie_nivel_acreditacao_ptu,'0'), 
		coalesce(id_autoriz_divulga,' ') 
	into STRICT	cd_cgc_cpf_w, 
		inst_acred_w, 
		nivel_acred_w, 
		id_autoriz_divulga_w 
	from	ptu_prestador 
	where	nr_sequencia	= nr_seq_ptu_prestador_w 
	and	(CASE WHEN (ie_instituicao_acred_ptu IS NOT NULL AND ie_instituicao_acred_ptu::text <> '') THEN 'S' ELSE 'N' END) = 'S';
	exception 
	when others then 
		ie_408_w := 'N';
	end;
	...
	if (ie_408_w = 'S') then 
		...
		ds_conteudo_w :=	lpad(nr_seq_registro_w,8,'0') || '408' || cd_cgc_cpf_w || inst_acred_w || nivel_acred_w || id_autoriz_divulga_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_arq_mov_prestador_50 (nr_seq_lote_p bigint, cd_interface_p text, nm_usuario_p text, ds_local_p text, nm_arquivo_p text) FROM PUBLIC;
