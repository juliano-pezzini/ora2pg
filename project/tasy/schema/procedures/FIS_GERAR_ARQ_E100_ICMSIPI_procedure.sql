...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_e100_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
fetch c_reg_E100 bulk collect      into vetRegE100 limit 1000;
	...
	ds_linha_w := substr(ds_sep_w || vetRegE100[i].cd_reg ||
			     ds_sep_w || to_char(vetRegE100[i].dt_ini,'ddmmyyyy') ||
			     ds_sep_w || to_char(vetRegE100[i].dt_fin,'ddmmyyyy') || ds_sep_w,
			     1,
			     8000);
	...
	nr_linha_w := fis_gerar_arq_e110_icmsipi(	nr_seq_controle_p, nr_linha_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_e100_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) FROM PUBLIC;
