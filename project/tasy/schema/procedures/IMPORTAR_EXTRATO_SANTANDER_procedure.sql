...
CREATE OR REPLACE PROCEDURE importar_extrato_santander ( nr_seq_extrato_p bigint, nm_usuario_p text, ds_arquivo_p text) AS $body$
DECLARE
...
tipo_operacao_w     varchar(1);
vl_alteracao_valor_w  varchar(12);
...
c04 CURSOR FOR
SELECT  substr(a.ds_conteudo,63,1) tipo_operacao,
        TO_CHAR(substr(a.ds_conteudo,64,12), 'fm9999999999,99') vl_alteracao_valor
from	w_extrato_cartao_cr a
where	somente_numero(substr(a.ds_conteudo,114,12)) = ds_comprovante_w
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	EXIT WHEN NOT FOUND; /* apply on c02 */
		...
		EXIT WHEN NOT FOUND; /* apply on c03 */
            ...
            fetch  c04 into
                tipo_operacao_w,
                vl_alteracao_valor_w;
            EXIT WHEN NOT FOUND; /* apply on c04 */

            if (tipo_operacao_w = '+') then
                vl_parcela_w := vl_parcela_w + vl_alteracao_valor_w;
            elsif (tipo_operacao_w = '-') then
                vl_parcela_w := vl_parcela_w - vl_alteracao_valor_w;
            end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE importar_extrato_santander ( nr_seq_extrato_p bigint, nm_usuario_p text, ds_arquivo_p text) FROM PUBLIC;
