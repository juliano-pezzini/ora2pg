...
CREATE OR REPLACE PROCEDURE atualizar_dose_onc_prot_mat (nr_seq_paciente_p bigint, nr_seq_material_p bigint default null) AS $body$
DECLARE
...
/* atualizar todos os medicamentos com dose padrão */
...
if (coalesce(ie_atualizar_ciclos_w,'N')	<> 'N')then 
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		begin 
		CALL atualizar_dose_onc_ciclo(nr_seq_atendimento_w,'S',nr_seq_material_p);
		end;
	end loop;
	close C01;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE atualizar_dose_onc_prot_mat (nr_seq_paciente_p bigint, nr_seq_material_p bigint default null) FROM PUBLIC;
