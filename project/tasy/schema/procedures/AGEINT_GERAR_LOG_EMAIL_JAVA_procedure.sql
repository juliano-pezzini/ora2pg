...
CREATE OR REPLACE PROCEDURE ageint_gerar_log_email_java (nr_seq_ageint_p bigint, ds_destinatario_p text, ds_Assunto_p text, nm_usuario_p text, ie_tipo_email_p text, ds_email_cc_p text) AS $body$
DECLARE
...
/*Inicio - Gravação dos históricos de envio para as agendas.*/
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	begin
	/*Grava histórico para as agendas de exames*/
	...
	if (nr_seq_agenda_exame_w IS NOT NULL AND nr_seq_agenda_exame_w::text <> '') then
		...
		insert into agenda_paciente_envio(
				...
				ie_tipo_email_p);
		commit;
		/*Gera update com o conteúdo long de uma tabela para outra*/

		CALL COPIA_CAMPO_LONG_DE_PARA_NOVO(	'W_AGE_EMAIL_' || replace(replace(Elimina_Caracteres_Especiais(nm_usuario_p),' ',''),'.',''),
										'DS_CONTEUDO',
										'WHERE 1=1',
										'',
	...
	/*Grava histórico para as agendas de consultas/serviços*/
...
/*Fim - Gravação dos históricos de envio para as agendas.*/

/*Inicio - Gravação de logs de envio do e-mail para a Agenda Integrada.*/
...
CALL COPIA_CAMPO_LONG_DE_PARA_NOVO(	'W_AGE_EMAIL_' || replace(replace(Elimina_Caracteres_Especiais(nm_usuario_p),' ',''),'.',''),
								...
								'L');
/*Fim - Gravação de logs de envio do e-mail para a Agenda Integrada.*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ageint_gerar_log_email_java (nr_seq_ageint_p bigint, ds_destinatario_p text, ds_Assunto_p text, nm_usuario_p text, ie_tipo_email_p text, ds_email_cc_p text) FROM PUBLIC;
