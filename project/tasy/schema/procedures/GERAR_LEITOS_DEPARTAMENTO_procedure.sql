...
CREATE OR REPLACE PROCEDURE gerar_leitos_departamento ( nm_usuario_p text, cd_estab_ocupacao_p bigint, cd_perfil_p bigint, cd_setor_atendimento_p bigint, ie_param_266_p text, ie_regr_pront_p text, cd_departamento_p bigint) AS $body$
DECLARE
...
cd_classif_detento_w				varchar(255); -- Parâmetro [191] 
cd_classif_onc_w					varchar(255); -- Parâmetro [211] 
cd_classif_sigilo_w					varchar(255); -- Parâmetro [217] 
IE_ALTA_MEDICO_w					varchar(1); -- Parâmetro [119] 
IE_LEGENDA_AP_w						varchar(1); -- Parâmetro [148] 
IE_ALTA_MEDICO_PRIORIDADE_w			varchar(1); -- Parâmetro [213] 
IE_PESSOA_STATUS_w					varchar(1); -- Parâmetro [105] 
IE_PRECAUCAO_PEP_w					varchar(1); -- Parâmetro [187] 
---------------------------------------------------------------------------------------- 
c01 CURSOR FOR 
	...
	and  obter_se_contido(b.cd_classif_setor , '3,4,11,12') = 'S' 
...
c02 CURSOR FOR	 
 ...
 WHERE  obter_ultimo_depto_medico(nr_atendimento) = cd_departamento_p 
 AND	  cd_setor_atendimento not in (WITH RECURSIVE cte AS (
 
       SELECT regexp_substr(obter_setores_departamento(cd_departamento_p),'[^,]+', 1, level)  
       regexp_substr(obter_setores_departamento(cd_departamento_p), '[^,]+', 1, level) is not null 
       UNION ALL
 ...
 AND   ((coalesce(cd_estabelecimento::text, '') = '') OR (cd_estabelecimento = 1) OR ('N' = 'S')) 
 AND	  ((o.ie_status_unidade <> 'L' AND 'N' = 'S') OR ('N' = 'N')) 
...
BEGIN 
 ...
 open c01;
  ...
  fetch c01 into c01_w;
   ...
   begin 
    CALL gerar_wocupacao_leito(nm_usuario_p, 
           cd_estab_ocupacao_p, 
           cd_perfil_p, 
...
;	
    ...
    select max((obter_status_leito_ocupacao( 
      ...
      nr_seq_interno_w, 
      dt_saida_temporaria_w, 
      dt_retorno_saida_temporaria_w))::numeric ) 
    into STRICT	nr_status_w 
;
     
    select max((Obter_imagem_leito_ocup_hosp( 
         obter_status_leito_ocupacao( 
          ...
          ie_interditado_radiacao_w, 
          DT_PREVISTO_ALTA_ww, 
          obter_se_alta_tesouraria(cd_setor_atendimento_w, cd_unidade_basica_w, cd_unidade_compl_w), 
          null, 
          nr_seq_interno_w, 
          dt_saida_temporaria_w, 
          dt_retorno_saida_temporaria_w )))::numeric ) 
...
;		
    ...
    select	max(SUBSTR(obter_profissional_atend(nr_atendimento_w,'P','N'),1,60)), 
      ...
      max(SUBSTR(obter_idade_pf(cd_pessoa_fisica_w, clock_timestamp(), 'A'),1,100)), 
...
;
    ...
    if (IE_PRECAUCAO_PEP_w = 'S') then 
     select 	max(SUBSTR(obter_precaucao_isolamento(nr_atendimento_w,'MO'),1,200)), 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_leitos_departamento ( nm_usuario_p text, cd_estab_ocupacao_p bigint, cd_perfil_p bigint, cd_setor_atendimento_p bigint, ie_param_266_p text, ie_regr_pront_p text, cd_departamento_p bigint) FROM PUBLIC;
