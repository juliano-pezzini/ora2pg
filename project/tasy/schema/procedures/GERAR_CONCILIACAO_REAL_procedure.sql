...
CREATE OR REPLACE PROCEDURE gerar_conciliacao_real (nr_seq_extrato_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
select	somente_numero(substr(ds_conteudo,151,16)) || ',' || somente_numero(substr(ds_conteudo,167,2)),
	substr(ds_conteudo,143,8)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_conciliacao_real (nr_seq_extrato_p bigint, nm_usuario_p text) FROM PUBLIC;
