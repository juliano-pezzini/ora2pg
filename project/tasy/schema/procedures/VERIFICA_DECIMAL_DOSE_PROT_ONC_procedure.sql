...
CREATE OR REPLACE PROCEDURE verifica_decimal_dose_prot_onc ( nr_seq_paciente_p bigint, nr_seq_material_p bigint, ds_erro_p INOUT text, qt_dose_prescr_p bigint default null, qt_dose_p bigint default null) AS $body$
DECLARE


cd_setor_atendimento_w	paciente_setor.cd_setor_atendimento%type;
cd_material_w			paciente_protocolo_medic.cd_material%type;
cd_unidade_medida_w		paciente_protocolo_medic.cd_unidade_medida%type;
qt_dose_w				paciente_protocolo_medic.qt_dose%type;
...
BEGIN
	...
	select 	max(cd_material),
			max(coalesce(cd_unid_med_prescr, cd_unidade_medida)),
			max(coalesce(qt_dose_prescr, qt_dose))
	...
	qt_dose_w := coalesce(qt_dose_prescr_p, coalesce(qt_dose_p, qt_dose_w));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE verifica_decimal_dose_prot_onc ( nr_seq_paciente_p bigint, nr_seq_material_p bigint, ds_erro_p INOUT text, qt_dose_prescr_p bigint default null, qt_dose_p bigint default null) FROM PUBLIC;
