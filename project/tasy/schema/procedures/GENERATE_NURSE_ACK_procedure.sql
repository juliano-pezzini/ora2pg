...
CREATE OR REPLACE PROCEDURE generate_nurse_ack (ie_suspensao_p text, ie_tipo_item_p text, nr_atendimento_p bigint, nr_seq_cpoe_p bigint, cd_item_p text) AS $body$
DECLARE
...
c08 CURSOR FOR
	...
	from	prescr_solucao b,
			prescr_medica a
	where (a.nr_prescricao, b.nr_seq_dialise) in (SELECT c.nr_prescricao, c.nr_sequencia
...
*/
	if (adep_obtain_acknowledged(ie_tipo_item_p, nr_seq_cpoe_p) = 'N') then
		case
		when ie_tipo_item_p = 'M' or ie_tipo_item_p = 'MAT' or ie_tipo_item_p = 'IA' then /* suplementos orais - medicamentos - materiais */
		  if (ie_suspensao_p = 'N') then
			update cpoe_inf_adic
			set dt_ack_nurse = clock_timestamp(),
			  nm_user_ack = nm_user_w
			where nr_seq_mat_cpoe = nr_seq_cpoe_p;
		  else
			update cpoe_inf_adic
			set dt_ack_nurse_susp = clock_timestamp(),
			  nm_user_ack_susp = nm_user_w
			where nr_seq_mat_cpoe = nr_seq_cpoe_p;
		  end if;
		...
		for c01_w in c01 loop
			...
			values (
				  ...
				  CASE WHEN ie_suspensao_p='S' THEN  64  ELSE 63 END ,
				  ie_tipo_item_p,
				  nr_atendimento_p,
				  cd_item_p);
		end loop;
			...
			where nr_seq_mat_cpoe = nr_seq_cpoe_p;
			  ...
			  for c02_w in c02 loop
				...
				values (
					  ...
					  1,
					  obter_dados_usuario_opcao(nm_user_w, 'C'),
					  CASE WHEN ie_suspensao_p='S' THEN  60  ELSE 59 END ,
					  clock_timestamp(),
					  'S');
			  end loop;
		...
		when ie_tipo_item_p = 'LD' or ie_tipo_item_p = 'S' then /* Leite */
		  if (ie_suspensao_p = 'N') then
			update cpoe_inf_adic
			set dt_ack_nurse = clock_timestamp(),
			  nm_user_ack = nm_user_w
		...
		when ie_tipo_item_p = 'NAN' or ie_tipo_item_p = 'NPN' or ie_tipo_item_p = 'NPP' then /* npt */
		  if (ie_suspensao_p = 'N') then
			update cpoe_inf_adic
			set dt_ack_nurse = clock_timestamp(),
			  nm_user_ack = nm_user_w
		...
		when ie_tipo_item_p = 'O' then /* gasoterapia */
		  ...
		  for c06_w in c06 loop
			...
			values (
				  ...
				  nr_seq_cpoe_p,
				  clock_timestamp(),
				  CASE WHEN ie_suspensao_p='S' THEN  'SA'  ELSE 'NA' END );
		  end loop;
	...
	CALL wl_alterar_fim_previsto(nr_seq_worklist_w,clock_timestamp() + interval '1 days', clock_timestamp(), nm_user_w, obter_desc_expressao(1079594), null);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE generate_nurse_ack (ie_suspensao_p text, ie_tipo_item_p text, nr_atendimento_p bigint, nr_seq_cpoe_p bigint, cd_item_p text) FROM PUBLIC;
