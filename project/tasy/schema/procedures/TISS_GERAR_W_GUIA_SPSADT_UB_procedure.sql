...
CREATE OR REPLACE PROCEDURE tiss_gerar_w_guia_spsadt_ub (nr_sequencia_autor_p bigint, nr_seq_protocolo_p bigint, nr_interno_conta_p bigint, ds_dir_padrao_p text, nr_seq_exame_p bigint, nr_prescr_tiss_p bigint, nr_seq_med_fatur_p text, nr_atend_conta_tiss_p text, nm_usuario_p text, ie_setor_prescr_p text) AS $body$
DECLARE
...
SELECT	a.nr_seq_exame,
	coalesce(coalesce(coalesce(a.cd_procedimento, b.cd_procedimento),
		Obter_Cod_Proc_Interno_conv(b.nr_proc_interno, e.cd_convenio, e.cd_categoria, clock_timestamp(), b.cd_estabelecimento)
		),
	    Lab_Obter_Dados_Exame(a.NR_SEQ_EXAME_LAB, 'P')),
	substr(coalesce(Obter_Med_Exame_Externo(a.nr_sequencia),
		...
		EXIT WHEN NOT FOUND; /* apply on c14 */
			...
			EXIT WHEN NOT FOUND; /* apply on c04 */
				...
				if (qt_proc_guia_w = 1) then
					...
					SELECT * FROM tiss_obter_regra_prest_solic(	cd_convenio_w, cd_estabelecimento_w, cd_setor_entrada_w, ie_clinica_w, cd_procedimento_w, ie_tipo_atendimento_w, 'N', ie_solicitante_w, cd_cgc_solic_regra_w, cd_medico_w, cd_medico_solic_w, ds_irrelevante_w, ds_irrelevante_w, ds_irrelevante_w, cd_categoria_conv_w, clock_timestamp(), null, null, null, nr_seq_regra_solic_w) INTO STRICT ie_solicitante_w, cd_cgc_solic_regra_w, cd_medico_solic_w, ds_irrelevante_w, ds_irrelevante_w, nr_seq_regra_solic_w;
			...
			EXIT WHEN NOT FOUND; /* apply on c15 */
				...
				EXIT WHEN NOT FOUND; /* apply on c07 */
					...
					if (ie_desc_proc_interno_w = 'S') then
						...
						ds_procedimento_w := coalesce(ds_proc_procedimento_w, ds_procedimento_w);
					end if;
					...
					if (coalesce(IE_REGRA_W, 'S') = 'S') then
						...
						if (qt_proc_guia_w = 1) then
							...
							select	coalesce(max(obter_se_medico_conveniado(cd_estabelecimento_w, cd_medico_solic_w, cd_convenio_w, null, null, null,null,null,null, null, null)), 'N')
							into STRICT	ie_conveniado_w
							;
							...
							if (ie_solicitante_w	= 'M') or (ie_solicitante_w	= 'MC' AND ie_conveniado_w	= 'S') then
								...
								cd_interno_solic_w		:= tiss_obter_codigo_prestador(cd_convenio_w, cd_estabelecimento_w, cd_medico_solic_w, null, cd_setor_entrada_w, 'CI',null,ie_tipo_atendimento_w,cd_categoria_conv_w);
						...
						if (coalesce(qt_solicitada_w,0) <> 0) or (coalesce(qt_autorizada_w,0) <> 0) or (ie_emitir_spsadt_zerado_w = 'S') then
							...
							/* Concatena a observação de cada procedimento.*/

							update 	w_tiss_guia
							set	ds_observacao	= ds_observacao ||'  '|| ds_observacao_w
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE tiss_gerar_w_guia_spsadt_ub (nr_sequencia_autor_p bigint, nr_seq_protocolo_p bigint, nr_interno_conta_p bigint, ds_dir_padrao_p text, nr_seq_exame_p bigint, nr_prescr_tiss_p bigint, nr_seq_med_fatur_p text, nr_atend_conta_tiss_p text, nm_usuario_p text, ie_setor_prescr_p text) FROM PUBLIC;
