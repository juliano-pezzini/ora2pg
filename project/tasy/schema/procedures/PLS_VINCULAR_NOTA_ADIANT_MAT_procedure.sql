...
CREATE OR REPLACE PROCEDURE pls_vincular_nota_adiant_mat ( nr_seq_conta_mat_p bigint, nr_seq_nota_fiscal_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
vl_itens_w		pls_conta_proc.vl_procedimento_imp%type;
vl_item_w		pls_conta_mat.vl_material_imp%type;
...
from (	SELECT CASE WHEN ie_status='S' THEN a.vl_liberado WHEN ie_status='L' THEN a.vl_liberado  ELSE a.vl_procedimento_imp END  vl_apres
	from   pls_conta_proc a
	where  a.nr_seq_nota_fiscal = nr_seq_nota_fiscal_p
	
union all

	SELECT CASE WHEN ie_status='S' THEN a.vl_liberado WHEN ie_status='L' THEN a.vl_liberado  ELSE a.vl_material_imp END  vl_apres
	from   pls_conta_mat a
	where  a.nr_seq_nota_fiscal =nr_seq_nota_fiscal_p) alias2;
	
select 	CASE WHEN ie_status='S' THEN a.vl_liberado WHEN ie_status='L' THEN a.vl_liberado  ELSE a.vl_material_imp END ,
	nr_seq_conta,
	nr_seq_analise
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_vincular_nota_adiant_mat ( nr_seq_conta_mat_p bigint, nr_seq_nota_fiscal_p bigint, nm_usuario_p text) FROM PUBLIC;
