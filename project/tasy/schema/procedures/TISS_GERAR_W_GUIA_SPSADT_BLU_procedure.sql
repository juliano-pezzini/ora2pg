...
CREATE OR REPLACE PROCEDURE tiss_gerar_w_guia_spsadt_blu (nr_sequencia_autor_p bigint, nr_seq_protocolo_p bigint, nr_interno_conta_p bigint, ds_dir_padrao_p text, nr_seq_exame_p bigint, nr_prescr_tiss_p bigint, nr_seq_med_fatur_p text, nr_atend_conta_tiss_p text, nm_usuario_p text, ie_setor_prescr_p text) AS $body$
DECLARE


/*
OS250320 em 17/09/2010 - Unimed Blumenau
Gerar a guia de Sp/SADT no faturamento médico, apenas se o tipo de atendimento for <> de 'Internado' e gera o contratado executante conforme informado no faturamento médico.
Se informado prestador gera ele no campo 30, se não informado gera o próprio médico.
*/
...
uf_crm_w				varchar(002);
cd_cbo_saude_w			varchar(020);
ie_tipo_atendimento_w		varchar(020);
...
WHERE j.nr_atendimento				= e.nr_atendimento and d.cd_estabelecimento				= j.cd_estabelecimento and n.cd_convenio					= e.cd_convenio and e.nr_atendimento				= c.nr_atendimento and c.nr_sequencia					= a.nr_seq_pedido and c.nr_sequencia					= nr_seq_exame_p and ((coalesce((obter_tipo_procedimento(
		...
		,'C'))::numeric ,999)	= cd_tipo_proc_w) or (coalesce(cd_tipo_Proc_w::text, '') = '') ) group by e.cd_senha,
	 coalesce(e.nr_doc_convenio,'Não informada'),
	 e.dt_final_vigencia,
	 e.dt_inicio_vigencia,
	 a.nr_seq_exame,
				...
				if (qt_proc_guia_w = 1) then
					...
					if (coalesce(ie_solicitante_w,'H') = 'MC') then
						...
						if (coalesce(ie_conveniado_w,'S') = 'S') then
							...
							cd_cgc_w		:= tiss_obter_codigo_prestador(cd_convenio_w, cd_estabelecimento_w, cd_medico_solic_w, null, null, 'CI',null,ie_tipo_atendimento_w);
	...
	EXIT WHEN NOT FOUND; /* apply on c08 */
		...
		EXIT WHEN NOT FOUND; /* apply on c09 */
			...
			if (qt_proc_guia_w = 1) then
				...
				if (ie_medico_solic_atend_w = 'S') then
					select	coalesce(max(cd_medico_resp),cd_medico_convenio_w)
					into STRICT	cd_medico_convenio_w
					from 	atendimento_paciente
					where	nr_atendimento	= nr_atendimento_w;
				elsif (ie_medico_solic_atend_w = 'N') then
					cd_medico_convenio_w	:= null;
				end if;					
				...
				select	cd_cnpj_exec,
					cd_medico
				into STRICT	cd_cnpj_exec_w,
					cd_medico_w
				from	pep_med_fatur
				where	nr_sequencia	= nr_seq_med_fatur_p;	

				SELECT * FROM TISS_OBTER_REGRA_PRESTADOR(	cd_convenio_w, cd_estabelecimento_w, ie_tipo_atendimento_w, null, ie_clinica_w, cd_cnpj_exec_w, null, cd_medico_w, cd_medico_w, cd_categoria_conv_w, null, null, '4',  --honorário
								'S',  --honorário
				...
				if (nr_seq_regra_w IS NOT NULL AND nr_seq_regra_w::text <> '') then
					...
					if (coalesce(cd_cgc_prest_regra_prest_w,'X') <> 'X') then
						cd_cgc_executor_w	:= cd_cgc_prest_regra_prest_w;
						nm_contratado_w		:= substr(obter_nome_pf_pj(null, cd_cgc_prest_regra_prest_w),1,255);
						cd_medico_exec_w	:= null;
						nr_cpf_w		:= obter_dados_pf(cd_medico_w, 'CPF'); --CPF do executor complementar
					elsif (coalesce(cd_medico_exec_regra_prest_w,'X') <> 'X') then
						cd_cgc_executor_w	:= null;
						cd_medico_exec_w	:= cd_medico_exec_regra_prest_w;
					...
					if (ie_medico_exec_w = 'S') then
						cd_medico_exec_w	:= cd_medico_w;
						nr_cpf_w		:= obter_dados_pf(cd_medico_w, 'CPF');
						nm_contratado_w		:= obter_nome_medico(cd_medico_w, 'N');
						cd_cgc_executor_w	:= null;
					end if;
					...
					insert	into w_tiss_contratado_exec(nr_sequencia,
						...
						SELECT	nextval('w_tiss_contratado_exec_seq'),
							...
							nm_usuario_p,
							nr_seq_guia_w,
							tiss_obter_regra_campo(4, 'CD_INTERNO', cd_convenio_w, cd_cgc_executor_w, ie_tipo_atendimento_w, cd_categoria_conv_w,'N',0, cd_estabelecimento_w, null, null, null, null),
							cd_interno_contratado_w,
							tiss_obter_regra_campo(4, 'CD_INTERNO', cd_convenio_w, nr_cpf_w, ie_tipo_atendimento_w, cd_categoria_conv_w,'N',0, cd_estabelecimento_w, null, null, null, null),
							nm_contratado_w,
							'81' ds_tipo_logradouro,
							CASE WHEN coalesce(cd_cgc_executor_w::text, '') = '' THEN obter_dados_pf_pj(cd_medico_exec_w,cd_cgc_executor_w,'E')  ELSE obter_dados_pf_pj(null,cd_cgc_executor_w,'E') END ,
							CASE WHEN coalesce(cd_cgc_executor_w::text, '') = '' THEN obter_dados_pf_pj(cd_medico_exec_w,cd_cgc_executor_w,'CI')  ELSE obter_dados_pf_pj(null,cd_cgc_executor_w,'CI') END ,

							CASE WHEN coalesce(cd_cgc_executor_w::text, '') = '' THEN obter_dados_pf_pj(cd_medico_exec_w,cd_cgc_executor_w,'UF')  ELSE obter_dados_pf_pj(null,cd_cgc_executor_w,'UF') END ,
							substr(lpad(OBTER_MUNICIPIO_IBGE(somente_numero(CASE WHEN coalesce(cd_cgc_executor_w::text, '') = '' THEN obter_dados_pf_pj(cd_medico_exec_w,cd_cgc_executor_w,'CEP')  ELSE obter_dados_pf_pj(null,cd_cgc_executor_w,'CEP') END )),7,'0'),1,25),
							lpad(to_char(somente_numero(CASE WHEN coalesce(cd_cgc_executor_w::text, '') = '' THEN obter_dados_pf_pj(cd_medico_exec_w,cd_cgc_executor_w,'CEP')  ELSE obter_dados_pf_pj(null,cd_cgc_executor_w,'CEP') END )),8,'0'),
							null,
							tiss_obter_regra_campo(4, 'NM_EXECUTOR', cd_convenio_w, obter_nome_medico(b.cd_medico, 'N'), ie_tipo_atendimento_w,
										cd_categoria_conv_w,'N',0, cd_estabelecimento_w, null, null, null, null),
							tiss_obter_regra_campo(4, 'SG_CONSELHO', cd_convenio_w, obter_dados_medico(b.cd_medico, 'SGCRM'), ie_tipo_atendimento_w,
										cd_categoria_conv_w,'N',0, cd_estabelecimento_w, null, null, null, null),
							tiss_obter_regra_campo(4, 'NR_CRM', cd_convenio_w, obter_dados_medico(b.cd_medico, 'CRM'), ie_tipo_atendimento_w,
										cd_categoria_conv_w,'N',0, cd_estabelecimento_w, null, null, null, null),
							tiss_obter_regra_campo(4, 'UF_CRM', cd_convenio_w, obter_dados_medico(b.cd_medico, 'UFCRM'), ie_tipo_atendimento_w,
										cd_categoria_conv_w,'N',0, cd_estabelecimento_w, null, null, null, null),
							substr(obter_dados_pf(b.cd_medico, 'CBOS'),1,20),
							ie_funcao_w
					...
					if (IE_EMITE_HONOR_SADT_PEP_w = 'N') then
						insert	into w_tiss_contratado_exec(nr_sequencia,
							dt_atualizacao,
							nm_usuario,
							nr_seq_guia,
						...
						SELECT	nextval('w_tiss_contratado_exec_seq'),
							...
							nm_usuario_p,
							nr_seq_guia_w,
							coalesce(cd_cnpj_exec,coalesce(tiss_obter_regra_campo(4, 'CD_INTERNO', cd_convenio_w, coalesce(nr_cpf_w, TISS_OBTER_CODIGO_PRESTADOR(cd_convenio_w, cd_estabelecimento_w, b.cd_medico, null, (null)::numeric , 'CI', null,ie_tipo_atendimento_w,cd_categoria_conv_w)), ie_tipo_atendimento_w, cd_categoria_conv_w,'N',0, cd_estabelecimento_w, null, null, null, null),Obter_Cpf_Pessoa_Fisica(b.cd_medico))),
							null,
							tiss_obter_regra_campo(4, 'CD_INTERNO', cd_convenio_w, coalesce(Obter_Cpf_Pessoa_Fisica(b.cd_medico), TISS_OBTER_CODIGO_PRESTADOR(cd_convenio_w, cd_estabelecimento_w, b.cd_medico, null, (null)::numeric , 'CI', null,ie_tipo_atendimento_w,cd_categoria_conv_w)), ie_tipo_atendimento_w, cd_categoria_conv_w,'N',0, cd_estabelecimento_w, null, null, null, null),
							substr(obter_nome_pf_pj(b.cd_medico,cd_cnpj_exec),1,100),
							'81' ds_tipo_logradouro,
							CASE WHEN coalesce(cd_cnpj_exec::text, '') = '' THEN obter_dados_pf_pj(b.cd_medico,cd_cnpj_exec,'E')  ELSE obter_dados_pf_pj(null,cd_cnpj_exec,'E') END ,
							CASE WHEN coalesce(cd_cnpj_exec::text, '') = '' THEN obter_dados_pf_pj(b.cd_medico,cd_cnpj_exec,'CI')  ELSE obter_dados_pf_pj(null,cd_cnpj_exec,'CI') END ,

							CASE WHEN coalesce(cd_cnpj_exec::text, '') = '' THEN obter_dados_pf_pj(b.cd_medico,cd_cnpj_exec,'UF')  ELSE obter_dados_pf_pj(null,cd_cnpj_exec,'UF') END ,
							substr(lpad(OBTER_MUNICIPIO_IBGE(somente_numero(CASE WHEN coalesce(cd_cnpj_exec::text, '') = '' THEN obter_dados_pf_pj(b.cd_medico,cd_cnpj_exec,'CEP')  ELSE obter_dados_pf_pj(null,cd_cnpj_exec,'CEP') END )),7,'0'),1,25),
							lpad(to_char(somente_numero(CASE WHEN coalesce(cd_cnpj_exec::text, '') = '' THEN obter_dados_pf_pj(b.cd_medico,cd_cnpj_exec,'CEP')  ELSE obter_dados_pf_pj(null,cd_cnpj_exec,'CEP') END )),8,'0'),
							null,
							tiss_obter_regra_campo(4, 'NM_EXECUTOR', cd_convenio_w, obter_nome_medico(b.cd_medico, 'N'), ie_tipo_atendimento_w,
										cd_categoria_conv_w,'N',0, cd_estabelecimento_w, null, null, null, null),
							tiss_obter_regra_campo(4, 'SG_CONSELHO', cd_convenio_w, obter_dados_medico(b.cd_medico, 'SGCRM'), ie_tipo_atendimento_w,
						...
						SELECT	nextval('w_tiss_contratado_exec_seq'),
							...
							substr(obter_nome_pf(b.cd_medico),1,100),
							'81' ds_tipo_logradouro,
							(obter_compl_pf(b.cd_medico, 1, 'EN') || ' ' || obter_compl_pf(b.cd_medico, 1, 'NR') || ' ' || obter_compl_pf(b.cd_medico, 1, 'CO')),
							obter_compl_pf(b.cd_medico, 1, 'CI'),
							obter_compl_pf(b.cd_medico, 1, 'UF'),
							substr(lpad(OBTER_MUNICIPIO_IBGE(somente_numero(obter_compl_pf(b.cd_medico, 1, 'CEP'))),7,'0'),1,25),
							lpad(to_char(somente_numero(obter_compl_pf(b.cd_medico, 1, 'CEP'))),8,'0'),
							null,
							tiss_obter_regra_campo(4, 'NM_EXECUTOR', cd_convenio_w, obter_nome_medico(b.cd_medico, 'N'), ie_tipo_atendimento_w,
										cd_categoria_conv_w,'N',0, cd_estabelecimento_w, null, null, null, null),
							tiss_obter_regra_campo(4, 'SG_CONSELHO', cd_convenio_w, obter_dados_medico(b.cd_medico, 'SGCRM'), ie_tipo_atendimento_w,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE tiss_gerar_w_guia_spsadt_blu (nr_sequencia_autor_p bigint, nr_seq_protocolo_p bigint, nr_interno_conta_p bigint, ds_dir_padrao_p text, nr_seq_exame_p bigint, nr_prescr_tiss_p bigint, nr_seq_med_fatur_p text, nr_atend_conta_tiss_p text, nm_usuario_p text, ie_setor_prescr_p text) FROM PUBLIC;
