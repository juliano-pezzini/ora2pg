...
CREATE OR REPLACE PROCEDURE gerar_tabela_w_nfe_export ( dt_inicio_p timestamp, dt_final_p timestamp, nm_usuario_p text, ie_setor_usuario_p text, nr_nota_fiscal_p bigint, cd_estabelecimento_p bigint, ds_filtro_p text, nr_seq_lote_p bigint) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	ds_comando_w	:= 'select count(*) from nota_fiscal where nr_sequencia = :nr_sequencia ' ||
			   ' and ((nvl(ie_status_envio, :ie_status_envio) = :ie_status_envio) or (nvl(ie_status_envio, ' || '''E''' || ') = ' || '''E''' || '))' ||
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_tabela_w_nfe_export ( dt_inicio_p timestamp, dt_final_p timestamp, nm_usuario_p text, ie_setor_usuario_p text, nr_nota_fiscal_p bigint, cd_estabelecimento_p bigint, ds_filtro_p text, nr_seq_lote_p bigint) FROM PUBLIC;
