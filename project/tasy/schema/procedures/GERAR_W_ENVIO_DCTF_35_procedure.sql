...
CREATE OR REPLACE PROCEDURE gerar_w_envio_dctf_35 ( nr_seq_lote_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
SELECT	distinct
	...
	0 ie_pj_regime_especial,
	0 ie_pj_dctf_semestral,
	max(CASE WHEN(SELECT 	count(*) 				from   	estabelecimento ew				where  	ew.cd_estabelecimento = d.cd_estabelecimento				and    	coalesce(ew.ie_scp, 'N') = 'S')=0 THEN  									/*Nao SPC*/
 0  ELSE /*Validacao se o DARF esta nos aceitos*/
 CASE WHEN fis_codigo_aceito_darf_scp(d.cd_darf)='S' THEN  1  ELSE CASE WHEN fis_codigo_aceito_darf_scp(d.cd_darf || 																		 lpad(CASE WHEN coalesce(coalesce(i.cd_variacao,f.cd_variacao)::text, '') = '' THEN 																		 obter_cd_darf_variacao(g.cd_tributo,'V')  ELSE coalesce(i.cd_variacao,f.cd_variacao) END ,2,0))='S' THEN  1  ELSE 0 END  END  END ) ie_spc_inc,
	' ' cd_natureza_juridica,
	' ' ds_logradouro,
	' ' ds_numero,
	' ' ds_complemento,
...
select	distinct 
	...
	0 ie_pj_regime_especial,
	0 ie_pj_dctf_semestral,
	max(CASE WHEN(select 	count(*) 				from   	estabelecimento ew, 						dctf_lote_darf lw,						darf dw				where  	ew.cd_estabelecimento = dw.cd_estabelecimento				and		lw.nr_seq_darf = dw.nr_sequencia				and    	coalesce(ew.ie_scp, 'N') = 'S'				and    	lw.nr_seq_lote = a.nr_sequencia)=0 THEN  									/*Nao SPC*/
 0  ELSE /*Validacao se o DARF esta nos aceitos*/
 CASE WHEN fis_codigo_aceito_darf_scp(d.cd_darf)='S' THEN  1  ELSE CASE WHEN fis_codigo_aceito_darf_scp(d.cd_darf || 																		 lpad(d.cd_variacao,2,'0'))='S' THEN  1  ELSE 0 END  END  END ) ie_spc_inc,
	' ' cd_natureza_juridica,
	' ' ds_logradouro,
	' ' ds_numero,
	' ' ds_complemento,
...
EXIT WHEN NOT FOUND; /* apply on c04 */
	...
	/*
	programa DCTF mensal 1.6, da receita considera apenas a posicao 55. Por isso, esta alteracao muda '07', por exemplo, para '70'
	*/
	insert into w_envio_dctf(nr_sequencia,
		nm_usuario,
		dt_atualizacao,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_w_envio_dctf_35 ( nr_seq_lote_p bigint, nm_usuario_p text) FROM PUBLIC;
