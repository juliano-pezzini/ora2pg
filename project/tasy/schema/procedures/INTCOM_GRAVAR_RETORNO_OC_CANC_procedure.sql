...
CREATE OR REPLACE PROCEDURE intcom_gravar_retorno_oc_canc ( nr_seq_registro_p bigint, ie_status_p text, ie_sistema_origem_p text, ds_erro_p text, ds_retorno_p text, ds_xml_p text, nm_usuario_p text) AS $body$
DECLARE
...
if (ie_status_p = 'P') then
	...
	values (	nextval('ordem_compra_hist_seq'),
		...
		nm_usuario_p,
		clock_timestamp(),
		WHEB_MENSAGEM_PCK.get_texto(280466),
		WHEB_MENSAGEM_PCK.get_texto(280468),
		clock_timestamp(),
		nm_usuario_p,
		'S',
		clock_timestamp());
	...
	values (	nextval('ordem_compra_hist_seq'),
		...
		nm_usuario_p,
		clock_timestamp(),
		WHEB_MENSAGEM_PCK.get_texto(280470),
		WHEB_MENSAGEM_PCK.get_texto(280471) || ds_erro_p,
		clock_timestamp(),
		nm_usuario_p,
		'S',
		clock_timestamp());
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE intcom_gravar_retorno_oc_canc ( nr_seq_registro_p bigint, ie_status_p text, ie_sistema_origem_p text, ds_erro_p text, ds_retorno_p text, ds_xml_p text, nm_usuario_p text) FROM PUBLIC;
