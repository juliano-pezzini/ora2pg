...
CREATE OR REPLACE PROCEDURE ratear_item_ordem_compra ( nr_ordem_compra_p bigint, nr_item_oci_p bigint, nr_seq_criterio_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text, dt_vigencia_p timestamp) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on c00 */
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		if (vl_total_item_ordem_w	> 0) then
			vl_rateio_w	:= dividir((vl_total_item_ordem_w * pr_rateio_w), 100);
		else
			vl_rateio_w	:= 0;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ratear_item_ordem_compra ( nr_ordem_compra_p bigint, nr_item_oci_p bigint, nr_seq_criterio_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text, dt_vigencia_p timestamp) FROM PUBLIC;
