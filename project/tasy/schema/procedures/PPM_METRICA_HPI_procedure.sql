...
CREATE OR REPLACE PROCEDURE ppm_metrica_hpi ( nr_seq_metrica_p bigint, nr_seq_objetivo_metrica_p bigint, cd_pessoa_fisica_p text, nm_usuario_p text, dt_referencia_p timestamp) AS $body$
DECLARE
...
if (nr_seq_diretoria_w IS NOT NULL AND nr_seq_diretoria_w::text <> '') then
	select	round((avg(pmo_obter_dados_bsc(y.nr_sequencia, 'HPI', 'PROJ')))::numeric, 2) hpi
...
CALL ppm_gravar_resultado(nr_seq_objetivo_metrica_p,dt_referencia_p,resultado_w, hr_real_w, hr_prev_w, nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ppm_metrica_hpi ( nr_seq_metrica_p bigint, nr_seq_objetivo_metrica_p bigint, cd_pessoa_fisica_p text, nm_usuario_p text, dt_referencia_p timestamp) FROM PUBLIC;
