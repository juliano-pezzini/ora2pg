...
CREATE OR REPLACE PROCEDURE gerar_pagto_febraban_240_v89 ( nr_seq_envio_p bigint, nm_usuario_p text) AS $body$
DECLARE


/* Layout CNAB 240 FEBRABAN (de 23/07/2018)
Rotina criada para atender as necessidades da OS 1973395.  Baseada na estrutura da procedure GERAR_PAGTO_FEBRABAN_240_V87*/
...
EXIT WHEN NOT FOUND; /* apply on c03 */
	...
	EXIT WHEN NOT FOUND; /* apply on c04 */
		...
		/* segmento O */
			...
			if (ie_tipo_titulo_w = 16 or ie_tipo_titulo_w = 29) then
				...
				/* segmento W - GRU*/
				...
				elsif (ie_tipo_titulo_w = 29) then
				  ...
				  ds_conteudo_w	:=	lpad(cd_banco_pagto_w,3,'0')												|| --  1 3
								...
								'1' 																			|| -- 15 15
								'9'																				|| -- 16 16
								rpad((coalesce(nr_documento_w,' ')), 20,' ') 										|| -- 17 36
								to_char(dt_emissao_w,'mmyyyy')													|| -- 37 42
								lpad(nr_inscricao_w,14,'0') 													|| -- 43 56
								lpad(somente_numero(to_char(coalesce(vl_escritural_w,0),'999999999999.00')),14,'0') 	|| -- 57 70
								lpad(somente_numero(to_char(coalesce(vl_desconto_w,0),'999999999999.00')),14,'0') 	|| -- 71 84
								rpad(' ',12,' ')																|| -- 85 96
								rpad(' ',14,' ')																|| -- 97 110
								lpad(somente_numero(to_char(coalesce(vl_multa_w,0),'999999999999.00')),14,'0') 		|| -- 111 124
								lpad(somente_numero(to_char(coalesce(vl_juros_w,0),'999999999999.00')),14,'0') 		|| -- 125 138
								rpad(' ',14,' ')																|| -- 139 152
								rpad(' ',24,' ')																|| -- 153 176
								'88'																			|| -- 177 178
								rpad(' ',48,' ')																|| -- 179 228
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_pagto_febraban_240_v89 ( nr_seq_envio_p bigint, nm_usuario_p text) FROM PUBLIC;
