...
CREATE OR REPLACE PROCEDURE pls_lanc_auto_requisicao ( nr_seq_requisicao_p bigint, nr_seq_proc_p bigint, nr_seq_mat_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ie_tipo_lanc_anexo_p text) AS $body$
DECLARE
...
qt_item_lancamento_w		double precision;
...
nr_seq_item_requisicao_w	pls_requisicao_proc.nr_sequencia%type;
ie_tipo_anexo_w			pls_requisicao_proc.ie_tipo_anexo%type;
vl_parametro_w			varchar(10);
ie_tipo_guia_w			pls_regra_lanc_automatico.ie_tipo_guia%type;
ie_tipo_atendimento_w		pls_regra_lanc_automatico.ie_tipo_atendimento%type;
nr_seq_material_forn_w		pls_regra_lanc_aut_item.nr_seq_material_forn%type;
nr_seq_mat_forn_preco_w		pls_regra_lanc_aut_item.nr_sequencia%type;
ie_consist_lanc_aut_w		pls_param_requisicao.ie_consist_lanc_aut%type;
qt_item_existente_req_w		bigint;
qt_solicitado_w			pls_requisicao_proc.qt_solicitado%type;
nr_seq_req_proc_w		pls_requisicao_proc.nr_sequencia%type;
ie_requisicao_portal_w		varchar(1)	:= 'N';	-- variavel para controlar se a requisicao veio do portal ou do tasy.
...
C01 CURSOR FOR
	...
	and (ie_regime_internacao	= ie_regime_internacao_w 	or coalesce(ie_regime_internacao::text, '') = '')
...
BEGIN
select 	CASE WHEN coalesce(ie_tipo_lanc_anexo_p::text, '') = '' THEN  'N'  ELSE 'S' END
into STRICT	ie_requisicao_portal_w
;

begin
	select	coalesce(ie_consist_lanc_aut,'N')
	into STRICT	ie_consist_lanc_aut_w
	from	pls_param_requisicao;	
exception
when others then
	ie_consist_lanc_aut_w	:= 'N';
end;

if (ie_requisicao_portal_w	= 'S') then
	vl_parametro_w	:= ie_tipo_lanc_anexo_p;
else
	vl_parametro_w := Obter_Param_Usuario(1271, 25, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_p, vl_parametro_w);
end if;	
	...
	if (nr_seq_regra_w	> 0) then
		...
		EXIT WHEN NOT FOUND; /* apply on C02 */
			...
			if (cd_procedimento_item_w IS NOT NULL AND cd_procedimento_item_w::text <> '') then
				if (ie_consist_lanc_aut_w	in ('S', 'M')) then
					qt_item_existente_req_w		:= 0;
					
					select 	count(1)
					into STRICT	qt_item_existente_req_w
					from	pls_requisicao_proc
					where	nr_seq_requisicao	= nr_seq_requisicao_p
					and	cd_procedimento		= cd_procedimento_item_w
					and	ie_origem_proced	= ie_origem_proced_item_w;

					if (qt_item_existente_req_w	= 0) then	
						select	nextval('pls_requisicao_proc_seq')
						into STRICT	nr_seq_item_requisicao_w
						;
						...
						insert	into pls_requisicao_proc(nr_sequencia, dt_atualizacao, nm_usuario,
								 ...
								 qt_solicitado, ie_status, ie_estagio,
								 nr_seq_proc_princ, nr_seq_mat_princ)
						values (nr_seq_item_requisicao_w, clock_timestamp(), nm_usuario_p,
								 clock_timestamp(), nm_usuario_p, nr_seq_requisicao_p,
								 cd_procedimento_item_w, ie_origem_proced_item_w, 0,
								 coalesce(qt_item_lancamento_w,1), 'U', 'AE',
								 nr_seq_proc_p, nr_seq_mat_p);

						CALL pls_atualiza_valor_proc_aut(nr_seq_item_requisicao_w, 'R', nm_usuario_p);

						if (vl_parametro_w	= 'LR') then
							CALL pls_atualizar_tipo_anexo_item(	nr_seq_item_requisicao_w, null, null,
							...
							if (ie_requisicao_portal_w	= 'S') then
								...
								from	pls_requisicao_proc
								where	nr_sequencia	= nr_seq_item_requisicao_w;
								
								CALL pls_gerar_anexo_req_item_web(	nr_seq_requisicao_p, nr_seq_item_requisicao_w, 'P',
												ie_tipo_anexo_w, null, null,
												null, null, coalesce(qt_item_lancamento_w,1),
												null, null, null,
						...
						if (coalesce(ie_tipo_processo_w,'X')	= 'I') then
							CALL pls_gerar_de_para_req_intercam(	nr_seq_item_requisicao_w, '', null,
											null, null, null,
											null, cd_estabelecimento_p, nm_usuario_p);
						end if;
					else
						if (ie_consist_lanc_aut_w	= 'M') then							
							...
							if (nr_seq_req_proc_w IS NOT NULL AND nr_seq_req_proc_w::text <> '') then								
								select 	coalesce(qt_solicitado, 0)	
								into STRICT	qt_solicitado_w
								from	pls_requisicao_proc
								where	nr_sequencia	= nr_seq_req_proc_w;
									
								if (coalesce(qt_item_lancamento_w,1) > qt_solicitado_w) then
									update	pls_requisicao_proc
									set	qt_solicitado		= coalesce(qt_item_lancamento_w,1),
										nr_seq_proc_princ	= nr_seq_proc_p,
										nr_seq_mat_princ	= nr_seq_mat_p
									where	nr_sequencia		= nr_seq_req_proc_w;
									
									CALL pls_atualiza_valor_proc_aut(nr_seq_req_proc_w, 'R', nm_usuario_p);
									
									if (vl_parametro_w	= 'LR') then
										CALL pls_atualizar_tipo_anexo_item(	nr_seq_req_proc_w, null, null,
														null, cd_procedimento_item_w, ie_origem_proced_item_w,
														null, nm_usuario_p);
										...
										if (ie_requisicao_portal_w	= 'S') then
											...
											CALL pls_gerar_anexo_req_item_web(	nr_seq_requisicao_p, nr_seq_req_proc_w, 'P',
															ie_tipo_anexo_w, null, null,
															null, null, coalesce(qt_item_lancamento_w,1),
															null, null, null,
															nm_usuario_p, null, null);
			...
			if (nr_seq_material_item_w IS NOT NULL AND nr_seq_material_item_w::text <> '') then
				...
				elsif (nr_seq_material_forn_w IS NOT NULL AND nr_seq_material_forn_w::text <> '') then
					begin
						select	max(b.nr_sequencia)
						into STRICT	nr_seq_mat_forn_preco_w
						from	pls_material_fornec a,
							pls_material_fornec_preco b
						where	a.nr_sequencia 		= b.nr_seq_mat_fornec
						and	a.nr_seq_material	= nr_seq_material_item_w
						and	a.nr_seq_fornecedor	= nr_seq_material_forn_w
						and (trunc(clock_timestamp()) between b.dt_inicio_vigencia	and coalesce(b.dt_fim_vigencia,clock_timestamp()));
					exception
					when others then
						nr_seq_mat_forn_preco_w		:= 0;
					end;

					if (coalesce(nr_seq_mat_forn_preco_w,0) > 0) then
						begin
							select	vl_preco
							into STRICT	vl_preco_w
							from	pls_material_fornec_preco
							where	nr_sequencia	= nr_seq_mat_forn_preco_w;
						exception
						when others then
							vl_preco_w	:= 0;
						end;
				...
				if (ie_consist_lanc_aut_w	= 'S') then
					qt_item_existente_req_w	:= 0;
					...
					if (qt_item_existente_req_w	= 0) then				
						...
						insert	into pls_requisicao_mat(nr_sequencia, dt_atualizacao, nm_usuario,
							 dt_atualizacao_nrec, nm_usuario_nrec, nr_seq_requisicao,
							 qt_solicitado, qt_material, ie_status,
							 nr_seq_material, nr_seq_prest_fornec, vl_material,
							 ie_estagio, nr_seq_material_forn, nr_seq_proc_princ,
							 nr_seq_mat_princ)
						values (nr_seq_requisicao_w, clock_timestamp(), nm_usuario_p,
							 clock_timestamp(), nm_usuario_p, nr_seq_requisicao_p,
							 coalesce(qt_item_lancamento_w,1), 0, 'U',
							 nr_seq_material_item_w, nr_seq_prest_fornec_w,	vl_preco_w,
							 'AE', nr_seq_material_forn_w, nr_seq_proc_p,
							 nr_seq_mat_p);							
						
						if (coalesce(nr_seq_prest_fornec_w::text, '') = '') then
							CALL pls_atualiza_valor_mat_aut(nr_seq_requisicao_w, 'R', nm_usuario_p);
						end if;

						if (vl_parametro_w	= 'LR') then
							CALL pls_atualizar_tipo_anexo_item(	null, nr_seq_requisicao_w, null,
											null, null, null,
							...
							if (ie_requisicao_portal_w	= 'S') then					
								...
								CALL pls_gerar_anexo_req_item_web(	nr_seq_requisicao_p, nr_seq_requisicao_w, 'M',
												ie_tipo_anexo_w, null, null,
												null, null, coalesce(qt_item_lancamento_w,1),
												null, null, null,
												nm_usuario_p, null, null);
						...
						if (coalesce(ie_tipo_processo_w,'X')	= 'I') then
							CALL pls_gerar_de_para_req_intercam(	'', nr_seq_requisicao_w, null,
											null, null, null,
											null, cd_estabelecimento_p, nm_usuario_p);
						end if;						
					end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_lanc_auto_requisicao ( nr_seq_requisicao_p bigint, nr_seq_proc_p bigint, nr_seq_mat_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ie_tipo_lanc_anexo_p text) FROM PUBLIC;
