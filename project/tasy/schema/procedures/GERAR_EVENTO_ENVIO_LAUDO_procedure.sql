...
CREATE OR REPLACE PROCEDURE gerar_evento_envio_laudo ( nr_seq_evento_p bigint, nm_usuario_p text, nr_atendimento_p bigint) AS $body$
DECLARE
...
C01 CURSOR FOR 
	...
	and	coalesce(ie_tipo_atend, coalesce(ie_tipo_atend_w,0))	= coalesce(ie_tipo_atend_w,0) 
...
ie_tipo_atend_w	:= coalesce(Obter_Tipo_Atendimento(nr_atendimento_p),0);
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (qt_corresp_w > 0) and (ie_pessoa_destino_w = '19') then 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_evento_envio_laudo ( nr_seq_evento_p bigint, nm_usuario_p text, nr_atendimento_p bigint) FROM PUBLIC;
