...
CREATE OR REPLACE PROCEDURE fis_calc_lalur_balancete ( nr_seq_lote_p bigint ) AS $body$
DECLARE
...
vl_BCIRPJ_w		double precision := 0.0;  -- 06  Base de Clculo da IRPJ
vl_IRPJAA9_w		double precision := 0.0;  -- 07 IRPJ Apurad8 - Alquota 15%
...
vl_IRPJ_dma_w		double precision := 0.0;  -- 10  IRPJ devido no mes anterior
vl_IRPJ_dm_w		double precision := 0.0;  -- 11  IRPJ devido no mes
vl_IRPJ_pagar_w		double precision := 0.0;  -- 12  IRPJ a Pagar
...
vl_IRPJ_dma2_w double precision := 0.0;
vl_IRPJ_dma_aux_w double precision := 0.0;
index_w smallint := 0;
...
EXIT WHEN NOT FOUND; /* apply on c03 */
	...
	if (c03_w.ie_anual_trimestral = 'T') then
		begin
		nr_mes_ref_w := (PKG_DATE_UTILS.extract_field('MONTH',fim_mes(PKG_DATE_UTILS.ADD_MONTH(PKG_DATE_UTILS.start_of(dt_referencia_w, 'Q', 0), 2, 0))))::numeric;
		end;
	else
		begin
	...
	if (c03_w.linha in (1,2,3,5)) then
		...
		EXIT WHEN NOT FOUND; /* apply on c04 */
			...
			if (c04_w.linha = 1) then
				...
				begin
				if (c03_w.ie_anual_trimestral = 'A') then
					begin
				...
				elsif (c03_w.ie_anual_trimestral = 'T') then
					begin
					if (vl_BCIRPJ_w <= 60000.0) then
						begin
						vl_auxiliar_w := 0.0;
						end;
					else
						begin
						vl_auxiliar_w := (vl_BCIRPJ_w - 60000.0) * 0.1; --10%
						end;
					end if;
					end;
				end if;
			...
			ds_comando_w := 'update fis_calculo_estrut set ' || nm_vl_mes || ' = ' || vl_auxiliar_w || ' where nr_sequencia = '|| c04_w.nr_seq_calc_estrut;
    ...
    begin
      while(index_w < mes_apuracao_ref_w) loop
        index_w := index_w + 1;
        vl_IRPJ_dma2_w := vl_IRPJ_dma2_w + vl_auxiliar2_w;
        vl_auxiliar2_w := 0;
        ...
        and   s.ie_lalur_lacs = 'LL'
        and 	position('Imposto de Renda Devido no Mes Antes de Retencoes e Pagamentos' in remover_acentuacao(i.ds_item)) > 0
    ...
    vl_IRPJ_dma_aux_w := vl_IRPJ_dma2_w;

    if vl_IRPJ_dma2_w < 0 then
    vl_IRPJ_dma2_w := 0;
    end if;
    ...
    if vl_IRPJ_dma2_w <> vl_IRPJ_dma_w then
		vl_IRPJ_dm_w := vl_IRPJAA9_w - vl_deducoes_parte1_w - vl_IRPJ_dma_w - vl_IRPJ_dma2_w;
    else
		...
		if (nr_seq_calc_estrut1_w IS NOT NULL AND nr_seq_calc_estrut1_w::text <> '') then
			ds_comando_w := 'update fis_calculo_estrut set ' || nm_vl_mes || ' = ' || vl_IRPJ_dma_w || ' where nr_sequencia = '|| nr_seq_calc_estrut1_w;
			ds_comando_w := replace(ds_comando_w, ',', '.');
			CALL exec_sql_dinamico('Tasy', ds_comando_w);
		end if;
		
		if (nr_seq_calc_estrut2_w IS NOT NULL AND nr_seq_calc_estrut2_w::text <> '') then
			ds_comando_w := 'update fis_calculo_estrut set ' || nm_vl_mes || ' = ' || vl_IRPJ_dm_w || ' where nr_sequencia = '|| nr_seq_calc_estrut2_w;
			ds_comando_w := replace(ds_comando_w, ',', '.');
			CALL exec_sql_dinamico('Tasy', ds_comando_w);
		end if;

    if (nr_seq_calc_estrut3_w IS NOT NULL AND nr_seq_calc_estrut3_w::text <> '') then
		ds_comando_w := 'update fis_calculo_estrut set ' || nm_vl_mes || ' = ' || vl_IRPJ_dma2_w || ' where nr_seq_item = '|| nr_seq_calc_estrut3_w || ' and nr_seq_lote = ' || nr_seq_lote_p;
		ds_comando_w := replace(ds_comando_w, ',', '.');
		CALL exec_sql_dinamico('Tasy', ds_comando_w);
		end if;
		...
		if	( (vl_IRPJ_dm_w) > 0 ) then
			...
			begin
			vl_IRPJ_pagar_w := vl_IRPJAA9_w - vl_deducoes_parte1_w - vl_deducoes_parte2_w - vl_IRPJ_dma_aux_w;
			end;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_calc_lalur_balancete ( nr_seq_lote_p bigint ) FROM PUBLIC;
