...
CREATE OR REPLACE PROCEDURE gerar_cobr_deb_150_v05 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE

/*         
Procedure para uso geral de envio de cobrança de débito automático. 
Interface padrão: 2233 - Cobrança Escritural - Procedure Envio 150 posições 
Origem : IN 760-2 item 3.1.3 - Manual de Débito Automático 
...
C01 CURSOR FOR 
	SELECT	lpad(coalesce(substr(x.cd_agencia_bancaria,1,4),'0'),4,'0') cd_agencia_bancaria, 
		...
		lpad(' ',26,' ') ds_mensagen,		 
		rpad(coalesce(substr(CASE WHEN coalesce(obter_pf_titulo_receber(b.nr_titulo, 'C')::text, '') = '' THEN  obter_pj_titulo_receber(b.nr_titulo, 'C')  ELSE obter_pf_titulo_receber(b.nr_titulo, 'C') END ,1,25),' '),25,' ') ds_ident_cliente_emp,		 
		lpad(' ',25,' ') ds_ident_cliente_emp_atual, 
		CASE WHEN coalesce(d.nr_seq_pagador::text, '') = '' THEN  					rpad(coalesce(coalesce(substr(OBTER_BANCO_PF_PJ(b.cd_pessoa_fisica,b.cd_cgc,'AC'),1,8),' ') || 					substr(OBTER_BANCO_PF_PJ(b.cd_pessoa_fisica,b.cd_cgc,'ADC'),1,1),'0'),14,' ')  ELSE rpad(coalesce(coalesce(substr(pls_obter_dados_pagador_fin(d.nr_seq_pagador,'C'),1,8),' ') || 					substr(pls_obter_dados_pagador_fin(d.nr_seq_pagador,'DC'),1,1),'0'),14,' ') END  ds_ident_cliente_banco 
...
ds_conteudo_w	:= 	'A'|| '1' || cd_convenio_w || upper(nm_empresa_w) || cd_banco_w || upper(nm_banco_w) || dt_geracao_w || 
			lpad(nr_seq_arquivo_w,6,'0') || '05' ||	rpad('DEBITO AUTOMATICO',17,' ') || ds_brancos_52_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_cobr_deb_150_v05 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
