...
CREATE OR REPLACE PROCEDURE pls_imp_camara_contest_v110 ( ds_conteudo_p text, nm_usuario_p text, nr_seq_camara_contest_p bigint, nr_seq_referencia_p INOUT bigint, cd_estabelecimento_p bigint, nr_seq_camara_cont_p INOUT bigint) AS $body$
DECLARE
...
vl_tot_aco_servico_w		double precision;
vl_tot_aco_taxa_w		double precision;

-- 998
nr_seq_camara_cont_w		ptu_camara_contestacao.nr_sequencia%type;

-- 999
dt_postagem_arquivo_w		ptu_camara_contestacao.dt_postagem_arquivo%type;


BEGIN

CALL pls_grava_log_proces_imp_a500('Inicio pls_imp_camara_contest_v110', 'Inicio pls_imp_camara_contest_v110', nm_usuario_p);
...
if (substr(ds_conteudo_p,9,3) = '551') then
	...
	cd_unimed_origem_w	:= (substr(ds_conteudo_p,16,4))::numeric;	-- Prestadora dos servicos	
	...
	begin
		if (nr_nota_cred_deb_number_w > 0) then
			nr_nota_credito_debito_a500_w := nr_nota_cred_deb_number_w;
		else
			nr_nota_credito_debito_a500_w := null;
		end if;
	exception
	...
	if (ie_tp_arquivo_w in (3,4,5,6)) then
		...
		if (qt_arq_fec_compl_w > 0) then
			if (ie_tp_arquivo_w in (3,4)) then
				CALL wheb_mensagem_pck.exibir_mensagem_abort(717186); -- Nao e permitido importar arquivos do tipo fechamento parcial aps importar arquivo do tipo fechamento complementar.
			else
	...
	if (ie_operacao_w = 'E') and (coalesce(nr_seq_pls_fatura_w::text, '') = '') and (ie_tp_arquivo_w = 1) then
		CALL wheb_mensagem_pck.exibir_mensagem_abort(1079340);
	end if;
	...
	if (coalesce(nr_seq_contestacao_w::text, '') = '') then	
		...
		set	vl_total_acordo		= coalesce(vl_total_acordo, 0) + coalesce(vl_tot_acordo_w, 0),
	...
	ie_pacote_w		:= trim(both substr(ds_conteudo_p,597,1));
	...
	nm_prestador_557_w	:= trim(both substr(ds_conteudo_p,104,60));
	...
	if 	trim(both upper(sg_cons_prof_558_w)) = 'CRESS' then
		sg_cons_prof_558_w := 'CRAS';
	end if;

	if 	trim(both upper(sg_cons_prof_558_w)) = 'CREFONO' then
		sg_cons_prof_558_w := 'CRFA';
	end if;
	...
	nm_profissional_558_w	:= trim(both substr(ds_conteudo_p,235,60));
	...
	if (coalesce(nr_seq_camara_cont_p,0) = 0) then
		select	max(nr_sequencia)
		into STRICT	nr_seq_camara_cont_w
		from	ptu_camara_contestacao
		where	upper(nm_usuario) = upper(nm_usuario_p);
	else
		nr_seq_camara_cont_w := nr_seq_camara_cont_p;
	end if;
	
	vl_tot_aco_servico_w	:= pls_format_valor_imp_ptu(substr(ds_conteudo_p, 45, 14));
	vl_tot_aco_taxa_w	:= pls_format_valor_imp_ptu(substr(ds_conteudo_p, 59, 14));
	
	update	ptu_camara_contestacao
	set	vl_total_acordo	= coalesce(vl_total_acordo, 0) + coalesce(vl_tot_aco_servico_w, 0) + coalesce(vl_tot_aco_taxa_w, 0)
	where	nr_sequencia = nr_seq_camara_cont_w;
	commit;
	...
	if (coalesce(nr_seq_camara_cont_w,0) > 0) or (nr_seq_camara_cont_w IS NOT NULL AND nr_seq_camara_cont_w::text <> '') then		
		update	ptu_camara_contestacao
	...
	if (ie_tipo_arquivo_w = 1) then
		dt_postagem_arquivo_w	:= to_date(substr(ds_conteudo_p,12,18), 'yyyy/mm/dd hh24:mi:ss');
		
		begin
			update	ptu_camara_contestacao
			set	dt_postagem_arquivo = dt_postagem_arquivo_w
			where	nr_sequencia = nr_seq_camara_cont_w;
			commit;
		exception
		when others then
...
CALL pls_grava_log_proces_imp_a500('Fim pls_imp_camara_contest_v110', 'Fim pls_imp_camara_contest_v110', nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_imp_camara_contest_v110 ( ds_conteudo_p text, nm_usuario_p text, nr_seq_camara_contest_p bigint, nr_seq_referencia_p INOUT bigint, cd_estabelecimento_p bigint, nr_seq_camara_cont_p INOUT bigint) FROM PUBLIC;
