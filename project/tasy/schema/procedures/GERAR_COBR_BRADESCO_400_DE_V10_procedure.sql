...
CREATE OR REPLACE PROCEDURE gerar_cobr_bradesco_400_de_v10 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
qt_registros_w			bigint	:= 1;
ie_gerar_cob_esc_prim_mens_w	pls_parametros_cr.ie_gerar_cob_esc_prim_mens%type 	:= null;
...
C01 CURSOR FOR
	...
	SELECT	lpad('0',5,'0') cd_agencia_debito,
		...
		coalesce(a.IE_EMISSAO_BLOQUETO,'2') cd_condicao,
		'N' ie_emite_papeleta,
		' ' ie_rateio,
		'1' ie_endereco,
		...
		lpad(replace(to_char(coalesce(b.vl_titulo,0), 'fm00000000000.00'),'.',null),13,'0') vl_titulo,
		...
		'00' ie_instrucao2,
		substr(lpad(CASE WHEN coalesce(elimina_caracteres_especiais(c.vl_acrescimo),0)=0 THEN '0'  ELSE elimina_caracteres_especiais(c.vl_acrescimo) END ,13,'0'),1,13) vl_acrescimo,
		CASE WHEN coalesce(elimina_caracteres_especiais(c.vl_desconto),0)=0 THEN '000000'  ELSE to_char(clock_timestamp(),'ddmmyy') END  dt_desconto,
		substr(lpad(CASE WHEN coalesce(elimina_caracteres_especiais(c.vl_desconto),0)=0 THEN '0'  ELSE elimina_caracteres_especiais(c.vl_desconto) END ,13,'0'),1,13) vl_desconto,
		lpad('0',13,'0') vl_iof,
		...
		substr(substr(CASE WHEN coalesce(d.nr_sequencia::text, '') = '' THEN			obter_dados_pf_pj(b.cd_pessoa_fisica, b.cd_cgc, 'E')  ELSE pls_obter_compl_pagador(d.nr_seq_pagador,'E') END ,1,10)  || ' ' ||
		...
		substr(CASE WHEN coalesce(d.nr_sequencia::text, '') = '' THEN 			obter_dados_pf_pj(b.cd_pessoa_fisica, b.cd_cgc, 'UF')  ELSE pls_obter_compl_pagador(d.nr_seq_pagador,'UF') END ,1,2),1,40) ds_endereco_sacado,
		...
		substr(lpad(coalesce(to_char(a.nr_sequencia),'0'),6,'0'),1,6) nr_seq_arquivo
...
and		upper(a.ds_objeto)		= upper('gerar_cobr_bradesco_400_de_v10');
...
select	lpad(coalesce(substr(c.cd_convenio_banco,1,20),'0'),20,'0'),
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	ds_conteudo_w	:= 	'1' ||  --01
				...
				coalesce(cd_cep_w,'        ') || 	--327 a 331 e 332 a 334
				rpad(coalesce(nm_avalista_w,' '),60,' ') || 	--335 a 394
	...
	if 	(ds_mensagem_1_w IS NOT NULL AND ds_mensagem_1_w::text <> '') or
		(ds_mensagem_2_w IS NOT NULL AND ds_mensagem_2_w::text <> '') or
		(ds_mensagem_3_w IS NOT NULL AND ds_mensagem_3_w::text <> '') or
		(ds_mensagem_4_w IS NOT NULL AND ds_mensagem_4_w::text <> '') then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_cobr_bradesco_400_de_v10 ( nr_seq_cobr_escrit_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
