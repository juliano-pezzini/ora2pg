...
CREATE OR REPLACE PROCEDURE gerar_lote_sep_alto_risco ( nr_prescricao_p bigint, nr_sequencia_p bigint, nr_seq_horario_p bigint, ie_so_aprazado_p text, nm_usuario_p text, ie_origem_lote_p text) AS $body$
DECLARE
...
C01 CURSOR FOR  --especifico para medicamentos de alto risco
	...
	and	obter_se_material_risco(a.cd_setor_atendimento,b.cd_material) = 'S'
	...
	and	((coalesce(ie_itens_associados_w,'S') = 'N') or ((coalesce(ie_itens_associados_w,'S') = 'S') and (coalesce(b.nr_seq_superior::text, '') = '')) or ((coalesce(ie_itens_associados_w,'S') = 'S') and (c.nr_seq_recomendacao IS NOT NULL AND c.nr_seq_recomendacao::text <> '')))
...
union all
  -- especifico para solucoes
	...
	or	exists (select 1 
				...
				and 	obter_se_material_risco(a.cd_setor_atendimento,w.cd_material) = 'S'))
	and	b.qt_dispensar_hor > 0
	and	coalesce(b.nr_seq_lote::text, '') = ''
...
c10 CURSOR FOR
	...
	and	coalesce(b.nr_seq_lote::text, '') = ''
	and	((coalesce(c.nr_sequencia_solucao::text, '') = '') or (c.nr_sequencia_solucao = nr_seq_prescr_mat_w))
...
union all
	...
	and	a.nr_prescricao = nr_prescricao_p
	--and c.nr_sequencia_solucao = nr_seq_prescr_mat_w
	and	b.nr_seq_superior = nr_seq_prescr_mat_w
	and	coalesce(b.nr_seq_lote::text, '') = ''
...
values (	nextval('ap_lote_log_seq'),
	...
	' ie_gerar_lote_null_w='           || ie_gerar_lote_null_w           || ' ie_novo_lote_dia_w='         || ie_novo_lote_dia_w         ||
	' nr_seq_regra_alto_risco_w='      || nr_seq_regra_alto_risco_w      || ' ie_mesmo_turno_w='           || ie_mesmo_turno_w           ||
	' ie_itens_associados_w='          || ie_itens_associados_w,
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (ie_gerar_ap_lote_w = 'S') then
		if	((nr_seq_turno_w <> nr_seq_turno_ant_w) or (nr_seq_classif_w <> nr_seq_classif_ant_w) or
			...
			(nr_seq_turno_w = nr_seq_turno_ant_w AND ie_mesmo_turno_w = 'N')) then
			begin
			OPEN C02;
			LOOP
				FETCH C02 INTO
			...
			fetch C10 into	
				...
				EXIT WHEN NOT FOUND; /* apply on C10 */
					...
					values (	nextval('ap_lote_log_seq'),
						...
						' qt_dispensar_hor_w='				|| qt_dispensar_hor_w,
						substr(dbms_utility.format_call_stack,1,2000));	
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_lote_sep_alto_risco ( nr_prescricao_p bigint, nr_sequencia_p bigint, nr_seq_horario_p bigint, ie_so_aprazado_p text, nm_usuario_p text, ie_origem_lote_p text) FROM PUBLIC;
