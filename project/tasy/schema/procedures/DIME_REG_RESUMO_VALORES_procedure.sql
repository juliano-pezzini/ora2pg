...
CREATE OR REPLACE PROCEDURE dime_reg_resumo_valores ( nr_seq_controle_p bigint, cd_estabelecimento_p text, nm_usuario_p text, dt_referencia_p timestamp, ds_separador_p text, qt_linha_p INOUT bigint, nr_sequencia_p INOUT bigint ) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	if (nr_item_resumo_w = '3010') then
		vl_resumo_w := coalesce(vl_contabil_entrada_w,0);
	elsif (nr_item_resumo_w = '3020') then
		vl_resumo_w := coalesce(vl_base_calculo_entrada_w,0);
	elsif (nr_item_resumo_w = '3030') then
		vl_resumo_w := coalesce(vl_imposto_cred_entrada_w,0);
	elsif (nr_item_resumo_w = '3040') then
		vl_resumo_w := 0;
	elsif (nr_item_resumo_w = '3050') then
		vl_resumo_w := coalesce(vl_outras_entrada_w,0);
	elsif (nr_item_resumo_w = '3060') then
		vl_resumo_w := coalesce(vl_contabil_saida_w,0);
	elsif (nr_item_resumo_w = '3070') then
		vl_resumo_w := coalesce(vl_base_calculo_saida_w,0);
	elsif (nr_item_resumo_w = '3080') then
		vl_resumo_w := coalesce(vl_imposto_cred_saida_w,0);
	elsif (nr_item_resumo_w = '3090') then
		vl_resumo_w := 0;
	elsif (nr_item_resumo_w = '3100') then
		vl_resumo_w := coalesce(vl_outras_saida_w,0);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dime_reg_resumo_valores ( nr_seq_controle_p bigint, cd_estabelecimento_p text, nm_usuario_p text, dt_referencia_p timestamp, ds_separador_p text, qt_linha_p INOUT bigint, nr_sequencia_p INOUT bigint ) FROM PUBLIC;
