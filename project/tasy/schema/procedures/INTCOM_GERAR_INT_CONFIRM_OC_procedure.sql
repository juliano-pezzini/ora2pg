-- Generated by Ora2Pg, the Oracle database Schema converter, version 23.1
-- Copyright 2000-2022 Gilles DAROLD. All rights reserved.
-- DATASOURCE: dbi:Oracle:host=srv-dbora-03.whebdc.com.br;service_name=DEV_1815;port=1521

SET client_encoding TO 'UTF8';





CREATE OR REPLACE PROCEDURE intcom_gerar_int_confirm_oc ( nr_ordem_compra_p bigint, ie_tipo_integracao_p text, nm_usuario_p text, nr_sequencia_p INOUT bigint) AS $body$
DECLARE

 
nr_seq_registro_w		bigint;				
ds_tipo_operacao_w	varchar(30);
ds_login_w		varchar(255);
ds_senha_w		varchar(255);
nr_documento_externo_w	varchar(255);
nr_sequencia_w		bigint;


BEGIN 
 
select	CASE WHEN ie_tipo_integracao_p='1' THEN 'WCG' WHEN ie_tipo_integracao_p='2' THEN 'WDG' WHEN ie_tipo_integracao_p='3' THEN 'WEG' END  
into STRICT	ds_tipo_operacao_w
;
 
select	nextval('registro_integr_compras_seq') 
into STRICT	nr_seq_registro_w
;
 
select	a.ds_login_integr_compras_ws, 
	a.ds_senha_integr_compras_ws, 
	nr_documento_externo 
into STRICT	ds_login_w, 
	ds_senha_w, 
	nr_documento_externo_w 
from	parametro_compras a, 
	ordem_compra b 
where	a.cd_estabelecimento	= b.cd_estabelecimento 
and	b.nr_ordem_compra	= nr_ordem_compra_p;
 
insert into registro_integr_compras( 
	nr_sequencia, 
	dt_atualizacao, 
	nm_usuario, 
	dt_atualizacao_nrec, 
	nm_usuario_nrec, 
	ie_tipo_operacao, 
	nr_ordem_compra) 
values (	nr_seq_registro_w, 
	clock_timestamp(), 
	nm_usuario_p, 
	clock_timestamp(), 
	nm_usuario_p,	 
	ds_tipo_operacao_w, 
	nr_ordem_compra_p);
	 
select	nextval('registro_integr_com_xml_seq') 
into STRICT	nr_sequencia_w
;
 
insert into registro_integr_com_xml( 
	nr_sequencia, 
	dt_atualizacao, 
	nm_usuario, 
	dt_atualizacao_nrec, 
	nm_usuario_nrec, 
	nr_seq_registro, 
	ie_status, 
	ie_operacao, 
	ie_sistema_origem, 
	ds_erro, 
	ds_retorno, 
	ie_tipo_operacao, 
	ds_login, 
	ds_senha, 
	id_pdc) 
values (	nr_sequencia_w, 
	clock_timestamp(), 
	nm_usuario_p, 
	clock_timestamp(), 
	nm_usuario_p, 
	nr_seq_registro_w, 
	'N', 
	'E', 
	'TASY', 
	null, 
	null, 
	'WBS', 
	ds_login_w, 
	ds_senha_w, 
	nr_documento_externo_w);
 
insert into ordem_compra_hist(	 
	nr_sequencia, 
	nr_ordem_compra, 
	dt_atualizacao, 
	nm_usuario, 
	dt_historico, 
	ds_titulo, 
	ds_historico, 
	dt_atualizacao_nrec, 
	nm_usuario_nrec, 
	ie_tipo, 
	dt_liberacao) 
values (	nextval('ordem_compra_hist_seq'), 
	nr_ordem_compra_p, 
	clock_timestamp(), 
	nm_usuario_p, 
	clock_timestamp(), 
	wheb_mensagem_pck.get_texto(303518), 
	wheb_mensagem_pck.get_texto(303523), 
	clock_timestamp(), 
	nm_usuario_p, 
	'S', 
	clock_timestamp());
commit;
 
nr_sequencia_p := nr_sequencia_w;
 
end;
$body$
LANGUAGE PLPGSQL
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE intcom_gerar_int_confirm_oc ( nr_ordem_compra_p bigint, ie_tipo_integracao_p text, nm_usuario_p text, nr_sequencia_p INOUT bigint) FROM PUBLIC;

