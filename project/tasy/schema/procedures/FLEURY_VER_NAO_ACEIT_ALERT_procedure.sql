...
CREATE OR REPLACE PROCEDURE fleury_ver_nao_aceit_alert (nr_prescricao_p bigint, nr_seq_prescr_p bigint, nr_seq_exame_p bigint, nr_seq_material_p bigint, ds_resultado_p text, qt_resultado_p bigint, pr_resultado_p bigint, possui_result_nao_aceit_p INOUT text) AS $body$
DECLARE
...
if (ie_formato_resultado_w = 'P') then 
	...
	where ((ie_sexo = coalesce(ie_sexo_w,'0')) or (ie_sexo = '0')) 
	 ...
	 and (((coalesce(trunc((qt_dias_w / 365.25),2),0) between qt_idade_min and qt_idade_max) and (ie_periodo = 'A')) or 
		 (((coalesce((qt_dias_w / 365.25),0) * 12) between qt_idade_min and qt_idade_max) and (ie_periodo = 'M')) or 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fleury_ver_nao_aceit_alert (nr_prescricao_p bigint, nr_seq_prescr_p bigint, nr_seq_exame_p bigint, nr_seq_material_p bigint, ds_resultado_p text, qt_resultado_p bigint, pr_resultado_p bigint, possui_result_nao_aceit_p INOUT text) FROM PUBLIC;
