...
CREATE OR REPLACE PROCEDURE obter_consentimento_ageint ( nr_seq_ageint_p bigint, cd_estabelecimento_p bigint, cd_perfil_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR 
	...
	and	coalesce(ie_parametro_w, 'N') = 'S';
...
BEGIN 
ie_parametro_w  := obter_param_usuario(869, 399, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_p, ie_parametro_w );
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE obter_consentimento_ageint ( nr_seq_ageint_p bigint, cd_estabelecimento_p bigint, cd_perfil_p bigint, nm_usuario_p text) FROM PUBLIC;
