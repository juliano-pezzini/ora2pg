...
CREATE OR REPLACE PROCEDURE pls_gerar_tit_rec_liq_mens_spr ( nr_titulo_p bigint, nr_seq_baixa_p bigint, nm_usuario_p text, nr_titulo_contab_p INOUT bigint) AS $body$
DECLARE
...
Pontos de atenção:
	IE_TIPO_LANCAMENTO
		(BA,PR,EA)(Baixa antecipada (Antecip. Total),Pró-rata,Estorno antecipação)
...
vl_titulos_mensalidade_w		double precision;
qt_prop_tit_mens_w			double precision;
vl_lanc_ajuste_w			double precision;
...
WHERE c.nr_sequencia	= d.nr_seq_mensalidade_seg and ((b.ie_cancelamento in ('C','E'))
	or (c.nr_sequencia	= e.nr_seq_mensalidade_seg)) and b.nr_sequencia	= c.nr_seq_mensalidade and e.ie_status_coparticipacao  <> 'N' and b.nr_sequencia	= a.nr_seq_mensalidade and d.ie_tipo_item	= '3' and a.nr_titulo	= nr_titulo_p
...
if (nr_titulo_p IS NOT NULL AND nr_titulo_p::text <> '') and (nr_seq_baixa_p IS NOT NULL AND nr_seq_baixa_p::text <> '') then
	...
	if (nr_lote_contabilizado_w IS NOT NULL AND nr_lote_contabilizado_w::text <> '') then
		...
		into STRICT	vl_titulos_mensalidade_w
		from	titulo_receber
		where	nr_seq_mensalidade	= nr_seq_mensalidade_w;

		qt_prop_tit_mens_w := dividir(vl_titulo_w, vl_titulos_mensalidade_w);
		...
		EXIT WHEN NOT FOUND; /* apply on c_itens_mensalidade */
			...
			vl_item_prop_w	:= ((dividir_sem_round(vl_item_w,vl_titulo_w) * (vl_recebido_w + vl_trib_total_w)) - vl_tributo_w) * qt_prop_tit_mens_w;

			if (ie_origem_baixa_w = 'ET' and coalesce(nr_seq_liq_origem_w,0) <> 0) then
				select	count(*)
				into STRICT	qt_reg_rec_antec_w
				from	pls_titulo_rec_liq_mens
				where	nr_titulo	= nr_titulo_p
				and	nr_seq_baixa	= nr_seq_liq_origem_w
				and	ie_tipo_lancamento in ('BA','EA');
			end if;

			if (trunc(dt_recebimento_w,'month') < trunc(dt_referencia_mens_w,'month') or (qt_reg_rec_antec_w > 0)) and
				...
				while(i <= 3) loop
					...
					if (i = 1) then
						...
						vl_lancamento_w		:= vl_item_prop_w;
		...
		select	sum(x.vl_lancamento)
		into STRICT	vl_lanc_ajuste_w
		from (SELECT	sum(t.vl_lancamento) vl_lancamento
			from	pls_mensalidade_seg_item i,
				pls_titulo_rec_liq_mens t
			where	t.nr_seq_item_mens 	= i.nr_sequencia
			and	t.nr_titulo 		= nr_titulo_p
			and	t.nr_seq_baixa		= nr_seq_baixa_p
			and	i.ie_tipo_item	not in ('3', '6', '7')
			and	coalesce(i.nr_seq_rec_futura,0) = 0
			
union all

			SELECT	sum(t.vl_lancamento) vl_lancamento
			from	pls_titulo_rec_liq_mens 	t,
			...
			and	not exists (	select	1
						...
						where	i.nr_seq_conta		= a.nr_seq_conta
						and	coalesce(a.nr_seq_rec_futura,0) <> 0)
...
union all
			...
			and	t.nr_titulo 		= nr_titulo_p
			and	t.nr_seq_baixa		= nr_seq_baixa_p
			and	i.ie_tipo_item	in ('6', '7')
			and	exists (	select	1
					FROM i
LEFT OUTER JOIN pls_conta_pos_estabelecido a ON (i.nr_seq_conta = a.nr_seq_conta)
WHERE coalesce(a.nr_seq_rec_futura,0) = 0 )) x;

		vl_lanc_ajuste_w	:= coalesce(vl_lanc_ajuste_w,0);
		...
		EXIT WHEN NOT FOUND; /* apply on c_ajuste_item */
			...
			if (vl_recebido_w <> vet_ajuste_item.vl_lancamento) then
				...
				if (vet_ajuste_item.ie_tipo_lancamento in ('BA', 'EA')) and (vl_recebido_w <> (vet_ajuste_item.vl_lancamento + vl_lanc_ajuste_w)) then
					/*Não existe antecipação da coparticipação, por isso ela é contabilizada no mesmo momento da antecipação e deve somar à esse valor no momento de identificar diferenças*/

					update	pls_titulo_rec_liq_mens
					set	vl_lancamento	= vl_lancamento	+ (vl_recebido_w - (vet_ajuste_item.vl_lancamento + vl_lanc_ajuste_w))
					where	nr_sequencia	= vet_ajuste_item.nr_seq_liq_mens;

				elsif (vet_ajuste_item.ie_tipo_lancamento = 'PR') then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_tit_rec_liq_mens_spr ( nr_titulo_p bigint, nr_seq_baixa_p bigint, nm_usuario_p text, nr_titulo_contab_p INOUT bigint) FROM PUBLIC;
