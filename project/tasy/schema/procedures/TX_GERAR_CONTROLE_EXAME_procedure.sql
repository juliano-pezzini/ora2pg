...
CREATE OR REPLACE PROCEDURE tx_gerar_controle_exame (cd_pessoa_fisica_p text, nr_seq_protocolo_p bigint, dt_referencia_p timestamp, ds_itens_p text, ie_atualizar_todos_p text, nm_usuario_p text) AS $body$
DECLARE
...
c01 CURSOR FOR
	SELECT	ds_item
	from	tx_item_controle_exame where	((coalesce(nr_seq_protocolo_p::text, '') = '') or (Tx_obter_se_item_protocolo(nr_seq_protocolo_p,nr_sequencia) = 'S'))
...
delete 	FROM TX_EXAME_GRID_CONTROLE
where	ie_ordem = 1
...
select	nextval('tx_exame_grid_controle_seq')
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE tx_gerar_controle_exame (cd_pessoa_fisica_p text, nr_seq_protocolo_p bigint, dt_referencia_p timestamp, ds_itens_p text, ie_atualizar_todos_p text, nm_usuario_p text) FROM PUBLIC;
