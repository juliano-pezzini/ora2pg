...
CREATE OR REPLACE PROCEDURE hvc_gerar_retorno_deb_cc ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	SELECT	somente_numero(substr(ds_string,74,15))	nr_titulo,
		substr(ds_string,155,8) ds_dt_liquidacao,
		somente_numero(substr(ds_string,120,15))/100 ds_vl_liquidacao,
		substr(ds_string,231,2)	cd_ocorrencias,
		substr(ds_string,94,8) ds_dt_pagto_real
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_titulo_w IS NOT NULL AND nr_titulo_w::text <> '') then
		...
		and	coalesce(a.nr_seq_tipo,coalesce(nr_seq_tipo_w,2))	= coalesce(nr_seq_tipo_w,2)
		...
		if (dt_liquidacao_w = '00000000') then
			dt_liquidacao_w := null;
		else
			dt_liquidacao_w := coalesce(to_Date(dt_liquidacao_w,'ddmmyyyy'),clock_timestamp());
		end if;

		if (dt_pagto_real_w = '000000') then
			dt_pagto_real_w := null;
		else
			dt_pagto_real_w := coalesce(to_date(dt_pagto_real_w,'ddmmyyyy'),clock_timestamp());
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE hvc_gerar_retorno_deb_cc ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
