...
CREATE OR REPLACE PROCEDURE gerar_receita_oculos_red ( nr_sequencia_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
ds_esferica_w		varchar(60) := Lpad('ESF',60,' ');
vl_esf_od_w		varchar(20);
vl_esf_oe_w		varchar(20);
ds_cilindrica_w		varchar(20) := Lpad('CIL',20,' ');
...
ds_olho_direito_w	varchar(40) := Rpad('OD',40,' ');
ds_olho_esquerdo_w	varchar(40) := Rpad('OE',36,' ');
...
ds_receita_w	:= '{\rtf1\ansi\ansicpg1252\deff0\deflang1046{\fonttbl{\f0\fswiss\fprq2\fcharset0 Arial;}}{\*\generator Msftedit 
 
5.41.21.2507;}\viewkind4\uc1\pard\b\f0\fs22';
...
ds_tabela_w	:= '\par\tab\tab   ESF\tab\tab   CIL\tab\tab   EIXO\par 
OD\tab\tab   '||ds_graue_od_w||'\tab   '||ds_grauc_od_w||'\tab  '||ds_eixo_od_w||'\par 
OE\tab\tab   '||ds_graue_oe_w||'\tab   '||ds_grauc_oe_w||'\tab  '||ds_eixo_oe_w||'\b0\fs20\par';
...
if	((ie_adicao_w = 'S') and not((ie_receita_w = 'N') and (ie_estatica_w = 'N') and (ie_dinamica_w = 'N'))) then 
	begin 
	ds_receita_w	:= ds_receita_w || '\par ADIÇÃO '||vl_adicao_od_w||'\par';
	end;
 
end if;
 
ds_receita_w	:= ds_receita_w || '\fs20\par DP Medir c/ Pupilômetro';
 
if (ds_obs_receita_w IS NOT NULL AND ds_obs_receita_w::text <> '') then 
	ds_receita_w	:= ds_receita_w || '\fs20\par' || obter_desc_expressao(753406) || ' ' ||ds_obs_receita_w || ' ';
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_receita_oculos_red ( nr_sequencia_p bigint, nm_usuario_p text) FROM PUBLIC;
