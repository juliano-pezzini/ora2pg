...
C01 CURSOR FOR
	SELECT	qt_diarias_permitida,
		ie_guia_vinculada
	from	pls_validacao_aut_dia_perm
			...
			for r_C02_w in C02 loop
				begin
					...
					if (ie_gerar_ocorrencia_w	= 'S') then
						if (coalesce(r_C01_w.qt_diarias_permitida,0) > 0) then

							ie_gerar_ocorrencia_w	:= pls_obter_se_diarias_permitida(nr_seq_guia_p, null, null, r_C01_w.qt_diarias_permitida, r_C01_w.ie_guia_vinculada);
				...
				begin
					...
					if (ie_gerar_ocorrencia_w	= 'S') then
						...
						if (coalesce(r_C01_w.qt_diarias_permitida,0) > 0) then
							...
							ie_gerar_ocorrencia_w	:= pls_obter_se_diarias_permitida(null, nr_seq_requisicao_p, null, r_C01_w.qt_diarias_permitida, r_C01_w.ie_guia_vinculada);
				...
				begin
					...
					if (ie_gerar_ocorrencia_w	= 'S') then
						...
						if (coalesce(r_C01_w.qt_diarias_permitida,0) > 0) then
							...
							ie_gerar_ocorrencia_w	:= pls_obter_se_diarias_permitida(null, null, nr_seq_execucao_p, r_C01_w.qt_diarias_permitida, r_C01_w.ie_guia_vinculada);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_valida_ocor_aut_dia_perm ( nr_seq_ocor_combinada_p pls_ocor_aut_combinada.nr_sequencia%type, nr_seq_ocorrencia_p pls_ocorrencia.nr_sequencia%type, nr_seq_motivo_glosa_p tiss_motivo_glosa.nr_sequencia%type, nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, nr_seq_execucao_p pls_execucao_requisicao.nr_sequencia%type, ie_utiliza_filtro_p pls_ocor_aut_combinada.ie_utiliza_filtro%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type ) FROM PUBLIC;
