...
CREATE OR REPLACE PROCEDURE csmsf_gerar_ret_bradesco_400 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_titulo_w IS NOT NULL AND nr_titulo_w::text <> '') then 
		...
		values (	nr_seq_tit_rec_cobr_w, 
			...
			CASE WHEN dt_liquidacao_w='000000' THEN  clock_timestamp()  ELSE coalesce(trim(both dt_liquidacao_w),clock_timestamp()) END , 
			clock_timestamp(), 
			nm_usuario_p, 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE csmsf_gerar_ret_bradesco_400 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
