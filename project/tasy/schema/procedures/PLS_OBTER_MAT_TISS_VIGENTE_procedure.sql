...
CREATE OR REPLACE PROCEDURE pls_obter_mat_tiss_vigente ( nr_seq_material_p INOUT pls_material.nr_sequencia%type, dt_referencia_p timestamp, cd_mat_referencia_w text, ie_opcao_p text, ie_valida_vig_mat_p text, nr_versao_tiss_p pls_material_tiss.ds_versao_tiss%type) AS $body$
DECLARE
...
if (ie_opcao_p = 'O') and (coalesce(nr_seq_material_p::text, '') = '') then
	...
	begin
		cd_material_ops_w	:= (cd_mat_referencia_w )::numeric;
	exception
	when others then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_obter_mat_tiss_vigente ( nr_seq_material_p INOUT pls_material.nr_sequencia%type, dt_referencia_p timestamp, cd_mat_referencia_w text, ie_opcao_p text, ie_valida_vig_mat_p text, nr_versao_tiss_p pls_material_tiss.ds_versao_tiss%type) FROM PUBLIC;
