...
CREATE OR REPLACE PROCEDURE ptu_gerar_pedido_insistencia ( nr_seq_requisicao_p bigint, nr_seq_guia_p bigint, nr_seq_complemento_p bigint, ds_menssagem_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_execucao_p INOUT bigint) AS $body$
DECLARE
...
if (nr_seq_guia_p IS NOT NULL AND nr_seq_guia_p::text <> '') then
	...
	where	nr_seq_execucao		= nr_seq_execucao_p
	and	ie_tipo_resposta	= 'PA';
	exception
	when others then
		CALL wheb_mensagem_pck.exibir_mensagem_abort(179722);
	end;
...
select	replace(replace(ds_menssagem_p,chr(13),''),chr(10),'')
...
values (nextval('ptu_pedido_insistencia_seq'), ie_tipo_cliente_w, cd_unimed_executora_w,
	 ...
	 '00202', ds_menssagem_w, nr_seq_requisicao_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_pedido_insistencia ( nr_seq_requisicao_p bigint, nr_seq_guia_p bigint, nr_seq_complemento_p bigint, ds_menssagem_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_execucao_p INOUT bigint) FROM PUBLIC;
