...
CREATE OR REPLACE PROCEDURE gerar_sinal_vital_roche ( cd_pessoa_fisica_p text, dt_sinal_vital_p text, ds_lista_parametros_p text, ds_lista_parametros2_p text, nm_usuario_p text) AS $body$
DECLARE
...
dt_sinal_vital_w := coalesce(dt_sinal_vital_w, clock_timestamp());
...
if (nr_atendimento_w IS NOT NULL AND nr_atendimento_w::text <> '') then 
	...
	for i in 1..Vetor_w.count loop 
		...
		if (Vetor_w[i].nm_sinal_vital	= 'Glu') then 
			atualizar_valor_sv('ATENDIMENTO_SINAL_VITAL','QT_GLICEMIA_CAPILAR',((Vetor_w[i].vl_sinal_vital)::numeric ));
		 
    elsif (Vetor_w[i].nm_sinal_vital	= 'Glu2') then 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_sinal_vital_roche ( cd_pessoa_fisica_p text, dt_sinal_vital_p text, ds_lista_parametros_p text, ds_lista_parametros2_p text, nm_usuario_p text) FROM PUBLIC;
