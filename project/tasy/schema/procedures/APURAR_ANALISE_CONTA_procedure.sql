...
CREATE OR REPLACE PROCEDURE apurar_analise_conta ( nr_sequencia_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
vl_custo_dir_apoio_w      	analise_conta_movto.vl_custo_dir_apoio%type;
vl_custo_direto_w         	analise_conta_movto.vl_custo_direto%type;
vl_custo_excluido_w       	analise_conta_movto.vl_custo_excluido%type;
vl_custo_hm_w             	analise_conta_movto.vl_custo_hm%type;
vl_custo_indireto_w       	analise_conta_movto.vl_custo_indireto%type;
vl_custo_mao_obra_w       	analise_conta_movto.vl_custo_mao_obra%type;
vl_custo_operacional_w    	analise_conta_movto.vl_custo_operacional%type;
vl_custo_sadt_w           	analise_conta_movto.vl_custo_sadt%type;
vl_custo_variavel_w			analise_conta_movto.vl_custo_variavel%type;
nr_seq_proc_interno_w		analise_conta_movto.nr_seq_proc_interno%type;
nr_seq_exame_w				analise_conta_movto.nr_seq_exame%type;

nr_seq_local_analise_w		bigint;
ds_local_analise_w			varchar(80);
qt_local_analise_99_w		bigint;
ds_mensagem_w				varchar(2000);
ds_msg_aux_w				varchar(1500);
...
SELECT
	...
	coalesce(vl_custo_excluido,0),
	coalesce(vl_custo_hm,0),
...
C04 CURSOR FOR
	SELECT	somente_numero(a.vl_dominio),
		substr(obter_valor_dominio(cd_dominio, vl_dominio), 1, 80) ds_dominio
	from	valor_dominio a
	where	a.cd_dominio = 1
	and	not exists (	SELECT	1
				from	analise_conta_local x
				where	x.nr_sequencia = somente_numero(a.vl_dominio) )
order by	1;
...
delete 	from analise_conta_item
where 	nr_sequencia = nr_sequencia_p;
...
WHERE a.nr_interno_conta 	= b.nr_interno_conta and a.nr_sequencia 		= nr_sequencia_p    and qt_resumo		> 0 and ((coalesce(ds_filtro_setor_atendimento_w::text, '') = '') or
	...
	((ie_filtro_estrutura_conta_w = '1') and (obter_se_contido(somente_numero(b.cd_estrutura_conta),replace(ds_filtro_estrutura_conta_w,chr(39),''))= 'S')) or
	((ie_filtro_estrutura_conta_w = '2') and (not obter_se_contido(somente_numero(b.cd_estrutura_conta),replace(ds_filtro_estrutura_conta_w,chr(39),''))= 'S'))) and ((coalesce(ds_filtro_carater_cirurgia_w::text, '') = '') or
	((ie_filtro_carater_cirurgia_w = '1') and ( exists (	SELECT	1
									from	cirurgia x
									where	x.nr_cirurgia	= b.nr_cirugia
									and	obter_se_contido(x.ie_carater_cirurgia,
									replace(ds_filtro_carater_cirurgia_w,chr(39),''))= 'S'))) or 
	((ie_filtro_carater_cirurgia_w = '2') and (not  exists (	select	1
									from	cirurgia x
									where	x.nr_cirurgia	= b.nr_cirugia
									and	obter_se_contido(x.ie_carater_cirurgia,
									replace(ds_filtro_carater_cirurgia_w,chr(39),''))= 'S')))) group by
...
/*	fim apuracao da moda */
	...
	exception
		...
		loop
		fetch C04 into	
			nr_seq_local_analise_w,
			ds_local_analise_w;
		EXIT WHEN NOT FOUND; /* apply on C04 */
			begin
			ds_msg_aux_w := ds_msg_aux_w || nr_seq_local_analise_w || ' - ' || ds_local_analise_w || chr(10);
			end;
		end loop;
		close C04;
		
		if (ds_msg_aux_w IS NOT NULL AND ds_msg_aux_w::text <> '') then
			select	count(*)
			into STRICT	qt_local_analise_99_w
			from	analise_conta_local
			where	nr_sequencia = 99;
			
			if (coalesce(qt_local_analise_99_w,0) = 0) then
				ds_msg_aux_w	:= ds_msg_aux_w || '99 - ' || Wheb_mensagem_pck.get_texto(445686);
			end if;
			
			ds_mensagem_w := substr(Wheb_mensagem_pck.get_texto(445687) || chr(10) || ds_msg_aux_w || chr(10) || Wheb_mensagem_pck.get_texto(445688),1,2000);
			
			CALL Wheb_mensagem_pck.exibir_mensagem_abort(ds_mensagem_w);
		else	
			ds_mensagem_w	:= substr(sqlerrm,1,2000);
			CALL Wheb_mensagem_pck.exibir_mensagem_abort(ds_mensagem_w);
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE apurar_analise_conta ( nr_sequencia_p bigint, nm_usuario_p text) FROM PUBLIC;
