...
CREATE OR REPLACE PROCEDURE gerar_ontologia_res ( nm_tabela_p text, nm_atributo_p text, ie_ontologia_p text, nm_usuario_p text) AS $body$
DECLARE
...
if (ie_ontologia_p IS NOT NULL AND ie_ontologia_p::text <> '') then
	...
	if (nm_tabela_p IS NOT NULL AND nm_tabela_p::text <> '' AND nm_atributo_p IS NOT NULL AND nm_atributo_p::text <> '') then
		...
		where	upper(a.nm_tabela) = upper(nm_tabela_p)
		and	upper(a.nm_atributo) = upper(nm_atributo_p);
		...
		if (ie_componente_w = 'cb') then
			...
			if (cd_dominio_w IS NOT NULL AND cd_dominio_w::text <> '') then
				...
				and		a.table_name = upper(nm_tabela_p)
				and		c.column_name = upper(nm_atributo_p)
		...
		EXIT WHEN NOT FOUND; /* apply on CursorConsulta */
			begin
				...
				and		cd_valor_tasy = cd_valor_tasy_w;

				if ( ie_existe_w = 'N') then

					insert into res_Cadastro_ontologia_cli(
							nr_sequencia,
							dt_atualizacao,
							nm_usuario,
							dt_atualizacao_nrec,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_ontologia_res ( nm_tabela_p text, nm_atributo_p text, ie_ontologia_p text, nm_usuario_p text) FROM PUBLIC;
