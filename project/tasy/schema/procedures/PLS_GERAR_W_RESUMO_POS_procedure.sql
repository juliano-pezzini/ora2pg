...
CREATE OR REPLACE PROCEDURE pls_gerar_w_resumo_pos ( nr_seq_conta_p bigint, nr_seq_conta_proc_p bigint, nr_seq_conta_mat_p bigint, nr_seq_analise_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
union all
	...
	and 	nr_seq_conta_proc_ww = 0 
	and 	nr_seq_conta_mat_ww = 0 
	and	((b.ie_situacao	= 'A') or (coalesce(b.ie_situacao::text, '') = '')) 
...
C02 CURSOR FOR 
	SELECT	a.nr_sequencia, 
		...
		(SELECT	substr(max(x.ds_grau_participacao),1,255) 
		from	pls_grau_participacao	x 
		where  x.ie_situacao = 'A' 
		and   x.nr_sequencia = a.nr_seq_grau_partic), 
...
BEGIN 
nr_seq_conta_proc_ww	:= coalesce(nr_seq_conta_proc_p,0);
nr_seq_conta_mat_ww	:= coalesce(nr_seq_conta_mat_p,0);
...
select	a.ie_tipo_guia, 
	...
	pls_obter_dados_segurado(a.nr_seq_segurado,'N'), 
	a.nr_seq_guia, 
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	insert into w_pls_resumo_conta(nr_sequencia, nr_seq_conta, nr_seq_item, 
		...
		nr_seq_item_ref,qt_apresentado, ie_custo_operacional, 
		nr_seq_prestador_exec,nm_prestador,ie_faturamento,nr_seq_pos_estab_interc, 
	...
	values ( nr_seq_item_analise_w, nr_seq_conta_p, nr_seq_item_w, 
		...
		nr_seq_item_ref_w,qt_item_w, ie_custo_operacional_w, 
		nr_seq_prestador_exec_w,nm_prestador_w,'L',nr_seq_pos_estab_interc_w, 
	...
	if (ie_tipo_item_w = 'P') then 
		...
		EXIT WHEN NOT FOUND; /* apply on C02 */
			...
			where	nr_seq_proc_partic = nr_seq_partic_w  LIMIT 1;
			 
			if (qt_registro_w > 0) then 
				select	coalesce(max(vl_participante_pos),0) 
			...
			insert into w_pls_resumo_conta(nr_sequencia, nr_seq_conta, nr_seq_item, 
				...
				nr_seq_item_ref, ie_custo_operacional,ie_faturamento,nr_seq_pos_estab_interc, 
				cd_item_convertido, ds_item_convertido) 
			values ( nr_seq_item_analise_w, nr_seq_conta_p, nr_seq_item_w, 
				ie_tipo_item_w || ie_tipo_despesa_w, ie_via_acesso_w, dt_item_w, 
				cd_item_w, ie_origem_proced_w, tx_item_w, 	 
				...
				nr_seq_item_ref_w, ie_custo_operacional_w,'L',nr_seq_pos_estab_interc_w, 
				cd_item_convertido_w, ds_item_convertido_w);
			end;
		end loop;
		close C02;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_w_resumo_pos ( nr_seq_conta_p bigint, nr_seq_conta_proc_p bigint, nr_seq_conta_mat_p bigint, nr_seq_analise_p bigint, nm_usuario_p text) FROM PUBLIC;
