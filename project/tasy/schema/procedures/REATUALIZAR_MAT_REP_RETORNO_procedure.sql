...
CREATE OR REPLACE PROCEDURE reatualizar_mat_rep_retorno ( nr_seq_material_p bigint, nr_seq_retorno_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
nr_seq_mat_rep_old_w	bigint;
vl_repasse_old_w		double precision;
vl_liberado_old_w		double precision;
nr_seq_origem_w		bigint;
ie_permite_recalcular_w		varchar(255);
cd_estabelecimento_ativo_w		bigint;
IE_LIB_REPASSE_RET_w		varchar(1);
...
ie_permite_recalcular_w := obter_param_usuario(27, 214, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_ativo_w, ie_permite_recalcular_w);
select	max(a.nr_sequencia), 
	sum(a.vl_repasse), 
	sum(a.vl_liberado) 
into STRICT	nr_seq_mat_rep_old_w, 
	vl_repasse_old_w, 
	vl_liberado_old_w 
from	material_repasse a, 
	material_atend_paciente b 
...
if (ie_status_retorno_w <> 'F') and (coalesce(IE_LIB_REPASSE_RET_w,'A') = 'A') then 
...
if (coalesce(nr_seq_material_p::text, '') = '') then 
	/* Número de sequencia do material não foi encontrado! */
 
	CALL wheb_mensagem_pck.exibir_mensagem_abort(195127);
end if;
...
if (cont_w > 1) and (coalesce(ie_permite_recalcular_w,'N') = 'N')then 
...
EXIT WHEN NOT FOUND; /* apply on c05 */
	...
	if (coalesce(nr_repasse_origem_w, 0) > 0) and (ie_repasse_estorno_w = 'S') then 
		...
		nr_repasse_terceiro_w := obter_repasse_terceiro(	clock_timestamp(), nr_seq_terc_origem_w, nm_usuario_p, nr_seq_mat_origem_w, 'P', nr_repasse_terceiro_w, null, null);
...
CALL gravar_log_tasy(55889, WHEB_MENSAGEM_PCK.get_texto(298679, 'NR_SEQ_MATERIAL_P=' || nr_seq_material_p || ';NR_SEQ_RETORNO_P=' || nr_seq_retorno_p) || to_char(clock_timestamp(), 'dd/mm/yyyy hh24:mi:ss'), nm_usuario_p);
 
CALL gerar_material_repasse(	nr_seq_material_p, 
...
if (coalesce(IE_LIB_REPASSE_RET_w,'A') = 'N') then 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE reatualizar_mat_rep_retorno ( nr_seq_material_p bigint, nr_seq_retorno_p bigint, nm_usuario_p text) FROM PUBLIC;
