...
CREATE OR REPLACE PROCEDURE nut_gerar_requisicao ( nr_seq_ordem_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	SELECT 	distinct
		x.nr_seq_local,
		f.cd_centro_custo
	FROM local_estoque z, nut_local_estoque y, nut_local_estoque_mat x, nut_pac_opcao_rec d, nut_genero_alim c, nut_receita_comp b, nut_receita a, nut_cardapio_dia e
...
WHERE a.nr_sequencia 	   	= b.nr_seq_receita AND c.nr_sequencia 		= b.nr_seq_gen_alim AND ((ds_parametro_120_w = 'N' and d.nr_seq_cardapio_dia 	= e.nr_sequencia)
  		or (ds_parametro_120_w = 'S' and d.nr_seq_cardapio_dia 	= e.nr_sequencia and coalesce(e.nr_seq_cycle,0)>0)) AND a.nr_sequencia 		= d.nr_seq_receita AND x.cd_material		= c.cd_material and x.nr_seq_local 		= y.nr_sequencia and y.nr_seq_local_estoque 	= z.cd_local_estoque and z.cd_estabelecimento 	= cd_estabelecimento_p  AND e.nr_sequencia IN (SELECT nr_seq_cardapio
...
LEFT OUTER JOIN nut_local_refeicao f ON (e.nr_seq_local = f.nr_sequencia)
WHERE a.nr_sequencia 		= b.nr_seq_receita AND c.nr_sequencia 		= b.nr_seq_gen_alim AND ((ds_parametro_120_w = 'N' and d.nr_seq_cardapio_dia 	= e.nr_sequencia)
		or (ds_parametro_120_w = 'S' and d.nr_seq_cardapio_dia 	= e.nr_sequencia and coalesce(e.nr_seq_cycle,0)>0)) AND a.nr_sequencia 		= d.nr_seq_receita AND m.cd_material		= c.cd_material AND ((f.cd_estabelecimento = cd_estabelecimento_p) or (coalesce(f.cd_estabelecimento::text, '') = ''))  AND e.nr_sequencia		IN (SELECT nr_seq_cardapio
	   				    FROM   nut_ordem_prod_card
					    WHERE  nr_seq_ordem   = nr_seq_ordem_p) AND nr_seq_local_w		= (SELECT MAX(x.nr_seq_local) 
					   ...
					   WHERE  x.cd_material = c.cd_material
						   and x.nr_seq_local = y.nr_sequencia
                    					   ...
                    					   and z.cd_estabelecimento = cd_estabelecimento_p) AND coalesce(f.cd_centro_custo,0)	= coalesce(cd_centro_custo_w,0) GROUP BY	c.cd_material,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE nut_gerar_requisicao ( nr_seq_ordem_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
