...
CREATE OR REPLACE PROCEDURE pls_lib_fat_item_pos ( nr_seq_item_resumo_p bigint, nr_seq_item_ref_p bigint, nr_seq_analise_p bigint, nr_seq_grupo_atual_p bigint, nr_seq_motiv_lib_p bigint, ds_observacao_p text, ie_comitar_p text, ie_opcao_p text, cd_estabelecimento_p bigint, nm_usuario_p text ) AS $body$
DECLARE
...
if (coalesce(nr_seq_item_ref_p,0) > 0) then 
	...
	EXIT WHEN NOT FOUND; /* apply on C02 */
		...
		CALL pls_lib_fat_item_pos(	nr_seq_item_resumo_w, null, nr_seq_analise_p, 
					nr_seq_grupo_atual_p, nr_seq_motiv_lib_p, ds_observacao_p, 
					'N', null, cd_estabelecimento_p, nm_usuario_p );
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		if (ie_status_w in ('P', 'A', 'N')) then 
			...
			if (coalesce(nr_seq_motiv_lib_p,0) <> 0) then 
				...
				values (	nextval('pls_analise_parecer_item_seq'), nr_seq_item_w, nr_seq_motiv_lib_p, 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_lib_fat_item_pos ( nr_seq_item_resumo_p bigint, nr_seq_item_ref_p bigint, nr_seq_analise_p bigint, nr_seq_grupo_atual_p bigint, nr_seq_motiv_lib_p bigint, ds_observacao_p text, ie_comitar_p text, ie_opcao_p text, cd_estabelecimento_p bigint, nm_usuario_p text ) FROM PUBLIC;
