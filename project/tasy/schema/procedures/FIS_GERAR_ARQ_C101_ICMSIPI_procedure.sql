...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_c101_icmsipi ( nr_seq_controle_p bigint, nr_seq_nota_p bigint, nr_linha_p INOUT bigint) AS $body$
DECLARE
...
type reg_c_reg_C101 is table of fis_efd_icmsipi_C101%RowType;
...
fetch c_reg_C101 bulk collect into vetRegC101 limit 1000;
	...
	ds_linha_w := substr(ds_sep_w || vetRegC101[i].cd_reg ||
			     ds_sep_w || substr(sped_obter_campo_valor(vetRegC101[i].vl_fcp_uf_dest), 1, 30) ||
			     ds_sep_w || substr(sped_obter_campo_valor(vetRegC101[i].vl_icms_uf_dest), 1, 30) ||
			     ds_sep_w || substr(sped_obter_campo_valor(vetRegC101[i].vl_icms_uf_rem), 1, 30) || ds_sep_w,
			     1,
			     8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro :=  vetRegC101[i].cd_reg;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_c101_icmsipi ( nr_seq_controle_p bigint, nr_seq_nota_p bigint, nr_linha_p INOUT bigint) FROM PUBLIC;
