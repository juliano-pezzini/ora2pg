...
C01 CURSOR FOR
	...
	from	(	SELECT	nr_seq_congenere_cobr,
			...
			where	nr_seq_intercambio = nr_seq_contrato_int_w
...
union all
			...
			and	not exists (	select	1
						from	pls_contrato_regra_repasse
						where	nr_seq_intercambio = nr_seq_contrato_int_w)) alias16
...
if (ie_consistir_regra_contrato_p = 'S') and (nr_seq_contrato_int_w IS NOT NULL AND nr_seq_contrato_int_w::text <> '') then
	open C01;
	loop
	fetch C01 into	
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_repasse_int_web ( ie_tipo_acao_p text, dt_repasse_p text, nr_seq_congenere_p bigint, nr_seq_segurado_p bigint, nr_seq_repasse_p bigint, ie_consistir_regra_contrato_p text, ie_mes_posterior_repasse_p text, nm_usuario_p text, cd_estabelecimento_p bigint, nr_seq_repasse_gerado_p INOUT bigint) FROM PUBLIC;
