...
--
-- dblink wrapper to call function sus_consiste_fpo_unif as an autonomous transaction
--
CREATE EXTENSION IF NOT EXISTS dblink;

CREATE OR REPLACE PROCEDURE sus_consiste_fpo_unif ( dt_parametro_p timestamp, cd_procedimento_p bigint, ie_origem_proced_p bigint, qt_procedimento_p bigint, nr_atendimento_p bigint, cd_estabelecimento_p bigint, ie_tipo_atendimento_p bigint, cd_cbo_p text, ie_opcao_p text, ds_retorno_p INOUT text, cd_setor_atendimento_p bigint, cd_procedencia_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
BEGIN
	v_query := 'SELECT * FROM sus_consiste_fpo_unif_atx ( ' || quote_nullable(dt_parametro_p) || ',' || quote_nullable(cd_procedimento_p) || ',' || quote_nullable(ie_origem_proced_p) || ',' || quote_nullable(qt_procedimento_p) || ',' || quote_nullable(nr_atendimento_p) || ',' || quote_nullable(cd_estabelecimento_p) || ',' || quote_nullable(ie_tipo_atendimento_p) || ',' || quote_nullable(cd_cbo_p) || ',' || quote_nullable(ie_opcao_p) || ',' || quote_nullable(ds_retorno_p) || ',' || quote_nullable(cd_setor_atendimento_p) || ',' || quote_nullable(cd_procedencia_p) || ',' || quote_nullable(nm_usuario_p) || ' )';
...
CREATE OR REPLACE PROCEDURE sus_consiste_fpo_unif_atx ( dt_parametro_p timestamp, cd_procedimento_p bigint, ie_origem_proced_p bigint, qt_procedimento_p bigint, nr_atendimento_p bigint, cd_estabelecimento_p bigint, ie_tipo_atendimento_p bigint, cd_cbo_p text, ie_opcao_p text, ds_retorno_p INOUT text, cd_setor_atendimento_p bigint, cd_procedencia_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
vl_agenda_w		double precision;
ie_consiste_agenda_w	varchar(1) := 'N';
ie_carater_inter_sus_w	atendimento_paciente.ie_carater_inter_sus%type;
BEGIN

ie_consiste_agenda_w := coalesce(obter_valor_param_usuario(820, 421, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_p),'N');
...
if (ie_origem_proced_p = 7) and
	...
	qt_executado_w	:= coalesce(sus_obter_resultado_fpo(nr_seq_regra_w,1,'E'),0);
	vl_executado_w	:= coalesce(sus_obter_resultado_fpo(nr_seq_regra_w,2,'E'),0);
	...
	if (ie_consiste_agenda_w = 'S') and (obter_funcao_ativa = 820) then
		begin
		qt_agenda_w := coalesce(sus_obter_resultado_fpo(nr_seq_regra_w,1,'A'),0);
		vl_agenda_w := coalesce(sus_obter_resultado_fpo(nr_seq_regra_w,2,'A'),0);
		end;
	else
		begin
		qt_agenda_w := 0;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE sus_consiste_fpo_unif ( dt_parametro_p timestamp, cd_procedimento_p bigint, ie_origem_proced_p bigint, qt_procedimento_p bigint, nr_atendimento_p bigint, cd_estabelecimento_p bigint, ie_tipo_atendimento_p bigint, cd_cbo_p text, ie_opcao_p text, ds_retorno_p INOUT text, cd_setor_atendimento_p bigint, cd_procedencia_p bigint, nm_usuario_p text) FROM PUBLIC; -- REVOKE ALL ON PROCEDURE sus_consiste_fpo_unif_atx ( dt_parametro_p timestamp, cd_procedimento_p bigint, ie_origem_proced_p bigint, qt_procedimento_p bigint, nr_atendimento_p bigint, cd_estabelecimento_p bigint, ie_tipo_atendimento_p bigint, cd_cbo_p text, ie_opcao_p text, ds_retorno_p INOUT text, cd_setor_atendimento_p bigint, cd_procedencia_p bigint, nm_usuario_p text) FROM PUBLIC;
