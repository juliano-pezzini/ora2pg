...
CREATE OR REPLACE PROCEDURE gerar_retorno_caixa_400_v003 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
c01 CURSOR FOR
	SELECT	trim(both substr(ds_string,59,14)),
		(substr(ds_string,153,15))::numeric /100,
		(substr(ds_string,267,15))::numeric /100,
		(substr(ds_string,241,15))::numeric /100,
		(substr(ds_string,228,15))::numeric /100,
		(substr(ds_string,254,15))::numeric /100,
		'0',
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_retorno_caixa_400_v003 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
