...
CREATE OR REPLACE PROCEDURE plt_sincronizar_ccg ( nm_usuario_p text, nr_atendimento_p bigint, dt_validade_limite_p timestamp, dt_horario_p timestamp, nr_horario_p integer, ie_prescr_usuario_p text) AS $body$
DECLARE
...
SELECT	a.nr_prescricao,
	...
	c.nr_sequencia,
	SUBSTR(plt_obter_status_hor_proc_ccg(c.dt_fim_horario,c.dt_suspensao,c.ie_dose_especial,c.dt_lib_horario),1,15),
	c.cd_procedimento,
	z.ds_protocolo,
	...
	substr(obter_mat_med_assoc_ccg(a.nr_prescricao,c.nr_seq_procedimento,'S'),1,2000) ds_mat_med_assoc,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE plt_sincronizar_ccg ( nm_usuario_p text, nr_atendimento_p bigint, dt_validade_limite_p timestamp, dt_horario_p timestamp, nr_horario_p integer, ie_prescr_usuario_p text) FROM PUBLIC;
