...
CREATE OR REPLACE PROCEDURE cpoe_insert_npt_ped ( nr_atendimento_p bigint, nr_prescricao_p bigint, nr_seq_cpoe_dieta_p bigint, cd_estabelecimento_p bigint, cd_perfil_p bigint, nm_usuario_p text, ie_origem_inf_p text) AS $body$
DECLARE
...
ie_ajustar_sodio_w								cpoe_dieta.ie_ajustar_sodio%type;
qt_aporte_hidrico_diario_w						cpoe_dieta.qt_aporte_hidrico_diario%type;
qt_nec_hidrica_diaria_w							cpoe_dieta.qt_nec_hidrica_diaria%type;
qt_nec_kcal_dia_w								cpoe_dieta.qt_nec_kcal_dia%type;
qt_nec_kcal_kg_dia_w							cpoe_dieta.qt_nec_kcal_kg_dia%type;
qt_peso_calorico_w								cpoe_dieta.qt_peso_calorico%type;
nr_sequencia_elem_w								cpoe_npt_elemento.nr_sequencia%type;
...
where	nr_seq_npt_cpoe = nr_seq_cpoe_dieta_p
and 	nr_seq_elemento = nr_sequencia_elem_w;
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	values (
		...
		qt_volume_diario_w,
		ds_justificativa_w,
		replace(cpoe_obter_tipo_npt(cd_pessoa_fisica_w,nr_atendimento_p),'I','P'),
		qt_dia_npt_w,
		0,
		'N',
		'N',
	...
	open c02;
	loop
	fetch c02 into	nr_sequencia_elem_w,
			nr_seq_elemento_w,
			dt_atualizacao_w,
			nm_usuario_w,
			dt_atualizacao_nrec_w,
...
exception when others then
	CALL gravar_log_cpoe(substr('CPOE_INSERT_NPT_PED EXCEPTION:'|| substr(to_char(sqlerrm),1,2000)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE cpoe_insert_npt_ped ( nr_atendimento_p bigint, nr_prescricao_p bigint, nr_seq_cpoe_dieta_p bigint, cd_estabelecimento_p bigint, cd_perfil_p bigint, nm_usuario_p text, ie_origem_inf_p text) FROM PUBLIC;
