...
CREATE OR REPLACE PROCEDURE pls_obter_dados_end_prestador ( cd_pessoa_fisica_p text, cd_cgc_p text, ie_tipo_compl_prest_p text, ie_compl_pj_adic_p text, nr_seq_compl_adic_p bigint, nr_seq_tipo_compl_adic_p bigint, ds_endereco_p INOUT text, nr_endereco_p INOUT text, ds_complemento_p INOUT text, ds_bairro_p INOUT text, cd_cep_p INOUT text, nr_telefone_p INOUT text, ds_email_p INOUT text, ds_website_p INOUT text, nr_ddi_telefone_p INOUT text, nr_ddd_telefone_p INOUT text, nr_ramal_p INOUT bigint, cd_municipio_ibge_p INOUT text, sg_estado_p INOUT text, ds_fax_p INOUT text, nr_ddd_fax_p INOUT text, ds_municipio_p INOUT text, ds_fone_adic_p INOUT text) AS $body$
DECLARE
...
if (ie_tipo_complemento_w > 0) then
	...
	elsif (ie_tipo_compl_prest_p in ('PJF','PJC', 'PJA')) then
		if (ie_tipo_compl_prest_p = 'PJA') and (nr_seq_compl_adic_p IS NOT NULL AND nr_seq_compl_adic_p::text <> '') then
			select	max(ds_endereco),
			...
			select	substr(ds_endereco,1,255),
				...
				null,
				substr(nr_ddd_telefone,1,3),
				somente_numero(substr(nr_ramal_contato,1,5)),
				substr(cd_municipio_ibge,1,5),
				substr(sg_estado,1,6),
				substr(nr_fax,1,80),
				null,
				substr(ds_municipio,1,40)
			into STRICT	ds_endereco_p,
