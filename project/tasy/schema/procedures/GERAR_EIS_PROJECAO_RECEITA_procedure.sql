...
CREATE OR REPLACE PROCEDURE gerar_eis_projecao_receita ( dt_parametro_p timestamp, nm_usuario_p text) AS $body$
DECLARE
...
SELECT	a.cd_empresa, 
	...
	a.cd_centro_custo, 
	sum(a.vl_total) 
FROM ctb_grupo_conta c, eis_resultado_v a
LEFT OUTER JOIN conta_contabil b ON (a.cd_conta_contabil = b.cd_conta_contabil)
WHERE b.cd_grupo		= c.cd_grupo and c.ie_tipo		in ('R','C') and a.dt_receita		= dt_mesano_referencia_w and somente_numero(a.ie_protocolo) >= '2' and somente_numero(a.ie_protocolo) <= '10' group by 
	a.cd_empresa, 
	a.cd_estabelecimento, 
	a.cd_conta_contabil, 
	a.cd_centro_custo 
...
nr_sequencia_w := Gravar_Log_Indicador(615, 'Projecao_Receita', clock_timestamp(), PKG_DATE_UTILS.start_of(dt_parametro_p, 'dd', 0), nm_usuario_p, nr_sequencia_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_eis_projecao_receita ( dt_parametro_p timestamp, nm_usuario_p text) FROM PUBLIC;
