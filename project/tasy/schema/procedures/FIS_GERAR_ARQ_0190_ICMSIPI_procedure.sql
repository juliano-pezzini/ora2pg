...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_0190_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
open c_reg_0190;
loop
fetch c_reg_0190 bulk collect into vetReg0190 limit 1000;
	...
	ds_linha_w := substr(ds_sep_w || vetReg0190[i].cd_reg ||
			     ds_sep_w || vetReg0190[i].cd_unid ||
			     ds_sep_w || vetReg0190[i].ds_descr || ds_sep_w,
			     1,
			     8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro 		:=  vetReg0190[i].cd_reg;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_0190_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) FROM PUBLIC;
