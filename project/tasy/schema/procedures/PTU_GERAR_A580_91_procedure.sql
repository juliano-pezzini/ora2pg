...
CREATE OR REPLACE PROCEDURE ptu_gerar_a580_91 ( nr_seq_fat_geral_p ptu_fatura_geral.nr_sequencia%type, cd_interface_p interface.cd_interface%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
select	cd_uni_des,
	...
	'07', -- Alterado da leitura da fatura para fixo, pois poderia ocorrer de uma fatura geral ser criada em uma versão, e o layout de outra
	lpad(replace(replace(campo_mascara_virgula(vl_ir),',',''),'.',''),14,'0'),
	rpad(nr_documento,20,' '),
	rpad(doc_fiscal,20,' '),
	tp_documento,
	coalesce(id_cobranca, '9')
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_a580_91 ( nr_seq_fat_geral_p ptu_fatura_geral.nr_sequencia%type, cd_interface_p interface.cd_interface%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
