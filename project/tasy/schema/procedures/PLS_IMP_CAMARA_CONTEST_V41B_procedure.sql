...
CREATE OR REPLACE PROCEDURE pls_imp_camara_contest_v41b ( ds_conteudo_p text, nm_usuario_p text, nr_seq_camara_contest_p bigint, nr_seq_referencia_p INOUT bigint, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
if (substr(ds_conteudo_p,9,3) = '551') then 
	...
	nr_nota_w		:= trim(both somente_numero(substr(ds_conteudo_p,24,11)));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_imp_camara_contest_v41b ( ds_conteudo_p text, nm_usuario_p text, nr_seq_camara_contest_p bigint, nr_seq_referencia_p INOUT bigint, cd_estabelecimento_p bigint) FROM PUBLIC;
