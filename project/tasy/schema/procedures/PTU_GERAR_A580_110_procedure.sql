...
CREATE OR REPLACE PROCEDURE ptu_gerar_a580_110 ( nr_seq_fat_geral_p ptu_fatura_geral.nr_sequencia%type, cd_interface_p interface.cd_interface%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
select	cd_uni_des,
	...
	'08', -- Alterado da leitura da fatura para fixo, pois poderia ocorrer de uma fatura geral ser criada em uma versão, e o layout de outra
	lpad(replace(replace(campo_mascara_virgula(vl_ir),',',''),'.',''),14,'0'),
	rpad(nr_documento,20,' '),
	rpad(doc_fiscal,20,' '),
	tp_documento,
	coalesce(lpad(id_cobranca,2,'0'), '09')
...
ds_conteudo_w	:=	'00000001581' || cd_uni_des_w || cd_uni_ori_w || dt_geracao_w || nr_comp_w || lpad(' ',11,' ') || dt_ven_doc_w || dt_emi_doc_w || vl_tot_doc_w ||
					 nr_ver_tra_w || vl_ir_w || nr_documento_w || doc_fiscal_w || tp_documento_w || id_cobranca_w;
ds_arquivo_w	:=	ds_conteudo_w;
...
ds_conteudo_w	:=	'00000002998' || ds_hash_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_a580_110 ( nr_seq_fat_geral_p ptu_fatura_geral.nr_sequencia%type, cd_interface_p interface.cd_interface%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
