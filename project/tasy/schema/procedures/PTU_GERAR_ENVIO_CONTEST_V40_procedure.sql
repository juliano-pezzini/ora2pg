...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_contest_v40 ( nr_seq_camara_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE


-- 552
ds_conteudo_aux_w		varchar(600);
...
C01 CURSOR FOR
	SELECT	'553' ||
		...
		rpad(CASE WHEN coalesce(b.ds_parecer_glosa::text, '') = '' THEN  substr(replace(elimina_caractere_especial(Elimina_Acentuacao(a.ds_motivo)),'|',''),1,500)  ELSE replace(elimina_caractere_especial(Elimina_Acentuacao(b.ds_parecer_glosa)),'|','') END ,500, ' ')
...
union
	...
	SELECT	'553' ||
		lpad((substr(a.cd_motivo,1,4)),4,'0') ||
		rpad(replace(elimina_caractere_especial(Elimina_Acentuacao(a.ds_motivo)),'|',''),500,' ')
...
select	sum(coalesce(vl_reconhecido,0) + coalesce(vl_reconh_adic_co,0) +
	...
	coalesce(vl_reconh_co,0) + coalesce(vl_reconh_filme,0)) vl_rec,
	sum(coalesce(vl_cobrado,0) + coalesce(vl_cobr_adic_co,0) +
	coalesce(vl_cobr_adic_filme,0) + coalesce(vl_cobr_adic_serv,0) +
	coalesce(vl_cobr_co,0) + coalesce(vl_cobr_filme,0)) vl_cobr
...
select	lpad(nr_seq_registro_w,8,'0') ||
	...
	lpad(replace(replace(campo_mascara_virgula(coalesce(vl_contestacao_w,0)),'.',''),',',''),14,'0') ||
	lpad(replace(replace(campo_mascara_virgula(coalesce(vl_total_acordo,0)),'.',''),',',''),14,'0') ||
...
EXIT WHEN NOT FOUND; /* apply on C00 */
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		begin
		ds_conteudo_w	:= lpad(nr_seq_registro_w,8,'0') || replace(ds_conteudo_553_w,'¿',' ');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_contest_v40 ( nr_seq_camara_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
