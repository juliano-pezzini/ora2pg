...
CREATE OR REPLACE PROCEDURE gerar_retorno_itau_400 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
ds_motivo_rejeicao_w		varchar(8);
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_titulo_w IS NOT NULL AND nr_titulo_w::text <> '') then
		...
		CALL fin_gerar_log_controle_banco(3,substr('Nao foi importado o titulo ' || ds_titulo_w || ', pois nao foi encontrado no Tasy',1,4000),nm_usuario_p,'N');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_retorno_itau_400 ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
