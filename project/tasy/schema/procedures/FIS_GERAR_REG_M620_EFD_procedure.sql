...
CREATE OR REPLACE PROCEDURE fis_gerar_reg_m620_efd ( nr_seq_controle_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, dt_inicio_p timestamp, dt_fim_p timestamp, cd_empresa_p bigint, ds_separador_p text, qt_linha_p INOUT bigint, nr_sequencia_p INOUT bigint) AS $body$
DECLARE
...
c02 CURSOR FOR 
	SELECT	r.nr_nota_fiscal, 
		...
		sum(vl_glosa) * (aliquota_cofins_w/100) vl_glosa 
...
EXIT WHEN NOT FOUND; /* apply on C02 */
	...
	ds_linha_w	:= substr(	sep_w || 'M620'							|| 	--texto fixo 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_reg_m620_efd ( nr_seq_controle_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, dt_inicio_p timestamp, dt_fim_p timestamp, cd_empresa_p bigint, ds_separador_p text, qt_linha_p INOUT bigint, nr_sequencia_p INOUT bigint) FROM PUBLIC;
