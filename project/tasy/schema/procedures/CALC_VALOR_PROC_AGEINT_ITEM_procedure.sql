...
CREATE OR REPLACE PROCEDURE calc_valor_proc_ageint_item ( nr_seq_ageint_p bigint, cd_convenio_p bigint, cd_categoria_p text, cd_estabelecimento_p bigint, dt_inicio_agendamento_p timestamp, cd_plano_p text, nm_usuario_p text, cd_usuario_convenio_p text, cd_pessoa_fisica_p text, ie_tipo_Atendimento_p bigint, nr_seq_ageint_item_p bigint, nr_seq_ageint_item_adic_p bigint) AS $body$
DECLARE
...
if (nr_seq_ageint_item_adic_p IS NOT NULL AND nr_seq_ageint_item_adic_p::text <> '') then
	...
	select	nr_sequencia,
		...
		cd_estabelecimento
	into STRICT	nr_seq_item_ageint_w,
...
if (nr_seq_proc_interno_w	> 0) then
	...
	if (nr_seq_ageint_item_adic_p IS NOT NULL AND nr_seq_ageint_item_adic_p::text <> '') then
		...
		where	nr_seq_adic_item   = nr_seq_ageint_item_adic_p;
	else
		select	max(cd_convenio),
			max(cd_categoria),
			max(cd_plano)
	...
	ie_pacote_item_w	:= obter_Se_pacote_convenio(cd_procedimento_w, ie_origem_proced_w, cd_convenio_ww, cd_estabelecimento_w);
            ...
            if (ie_edicao_w			= 'N') and (coalesce(cd_edicao_ajuste_w,0) 	= 0) and (ie_pacote_item_w 		= 'N') then
                        ie_glosa_w        := 'T';
            end if;
	...
	if	((coalesce(ie_Regra_w,0) not in (1,2,5)) or (coalesce(ie_glosa_w,'') not in ('T','E','R','B','H','Z',''))) and (ie_tipo_convenio_w <> 1) then
		...
		SELECT * FROM Define_Preco_Procedimento(
			cd_estabelecimento_w, cd_convenio_w, cd_categoria_w, coalesce(dt_transferencia_w,dt_inicio_agendamento_p), CD_PROCEDIMENTO_w, 0, coalesce(ie_tipo_atendimento_p,0), 0, null, --medico
		...
		if (ie_calcula_lanc_auto_w	<> 'N') then
			...
			vl_lanc_autom_w	:= obter_valor_regra_lanc_aut(cd_estabelecimento_w, cd_convenio_w, cd_categoria_w, cd_procedimento_w,

ie_origem_proced_w,nr_seq_proc_interno_w, 34, 'T',null);
		end if;
	end if;
	...
	if (ie_pacote_w	= 'N') then
		...
		if (coalesce(nr_seq_ageint_item_adic_p::text, '') = '') then		
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE calc_valor_proc_ageint_item ( nr_seq_ageint_p bigint, cd_convenio_p bigint, cd_categoria_p text, cd_estabelecimento_p bigint, dt_inicio_agendamento_p timestamp, cd_plano_p text, nm_usuario_p text, cd_usuario_convenio_p text, cd_pessoa_fisica_p text, ie_tipo_Atendimento_p bigint, nr_seq_ageint_item_p bigint, nr_seq_ageint_item_adic_p bigint) FROM PUBLIC;
