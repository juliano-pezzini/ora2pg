...
CREATE OR REPLACE PROCEDURE ctb_gerar_acomp_valor_cenario ( cd_estab_p text, nr_seq_cenario_p bigint, cd_classif_conta_p text, cd_classif_centro_p text, cd_conta_contabil_p text, cd_centro_custo_p text, qt_min_nivel_conta_p bigint, qt_max_nivel_conta_p bigint, qt_min_nivel_centro_p bigint, qt_max_nivel_centro_p bigint, nm_usuario_p text, ie_ordem_inicio_p text, ie_informacao_p bigint, nr_seq_mes_contabil_p bigint, ie_sem_cenario_p text) AS $body$
DECLARE
...
and	b.dt_referencia between dt_inicial_cen_w and dt_final_cen_w
and (coalesce(cd_estab_p::text, '') = '' or substr(ctb_obter_se_elemento_contido(a.cd_estabelecimento, cd_estab_p),1,1) = 'S')
and	c.ie_tipo			= 'A'
and	c.ie_situacao		= 'A'
and	ie_sem_cenario_w	= 'S'
and	not exists (	select	1
			...
			and	y.nr_seq_mes_ref	= a.nr_seq_mes_ref
...
qt_commit_w		:= 0;
ie_sem_cenario_w	:= coalesce(ie_sem_cenario_p,'N');
	...
	if (ie_gerar_w	= 'S')  then
		...
		while(y >= qt_nivel_min_w) loop
			...
			if (coalesce(nr_seq_mes_contabil_p,0) <> 0) then
				...
				and (coalesce(cd_estab_p::text, '') = '' or substr(ctb_obter_se_elemento_contido(a.cd_estabelecimento, cd_estab_p),1,1) = 'S')
				and	b.dt_referencia between trunc(dt_mes_ref_w,'year') and fim_mes(dt_mes_ref_w);
			...
			if (qt_registro_w > 0) then
				...
				set	vl_orcado		= coalesce(vl_orcado,0) + vl_orcado_w,
					vl_contabil		= coalesce(vl_contabil,0) + vl_contabil_w,
					vl_contabil_acum	= coalesce(vl_contabil_acum,0) + vl_contabil_acum_w
				...
				if (qt_commit_w >= 3000) then
					...
					if (ie_ordem_inicio_p <> 'C' ) and (ie_debito_credito_w = 'D') and (y = qt_nivel_max_w) then
						...
						vl_contabil_acum_w	:= vl_contabil_acum_w * -1;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ctb_gerar_acomp_valor_cenario ( cd_estab_p text, nr_seq_cenario_p bigint, cd_classif_conta_p text, cd_classif_centro_p text, cd_conta_contabil_p text, cd_centro_custo_p text, qt_min_nivel_conta_p bigint, qt_max_nivel_conta_p bigint, qt_min_nivel_centro_p bigint, qt_max_nivel_centro_p bigint, nm_usuario_p text, ie_ordem_inicio_p text, ie_informacao_p bigint, nr_seq_mes_contabil_p bigint, ie_sem_cenario_p text) FROM PUBLIC;
