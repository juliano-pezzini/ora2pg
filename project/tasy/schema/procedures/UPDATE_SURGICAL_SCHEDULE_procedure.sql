...
CREATE OR REPLACE PROCEDURE update_surgical_schedule (nr_seq_agenda_p bigint, nr_minuto_duracao_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
for x IN REVERSE array_agendas_w.count..1 loop
	if (array_agendas_w[x].ie_alterou_inicio = 'S') then
	    ...
	    set	   dt_agenda = array_agendas_w[x].dt_agenda,
		   hr_inicio = array_agendas_w[x].hr_inicio
	    where  cd_agenda = array_agendas_w[x].cd_agenda
	    and	   nr_sequencia = array_agendas_w[x].nr_sequencia;
	    commit;
	end if;
end loop;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE update_surgical_schedule (nr_seq_agenda_p bigint, nr_minuto_duracao_p bigint, nm_usuario_p text) FROM PUBLIC;
