...
CREATE OR REPLACE PROCEDURE ptu_gerar_arq_mov_prestad_100 (nr_seq_lote_p bigint, cd_interface_p text, nm_usuario_p text, ds_local_p text, nm_arquivo_p text) AS $body$
DECLARE
...
ds_conteudo_w	:=	lpad(nr_seq_registro_w,8,'0') || '401' || cd_unimed_destino_w || cd_unimed_origem_w || dt_geracao_w || '27' || id_ope_prest_w ||
			nr_registro_ans_w;
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	ds_conteudo_w	:=	lpad(nr_seq_registro_w,8,'0') || '402' || tp_prest_w || cd_prest_w || cd_cgc_cpf_w || cd_insc_est_w || lpad(' ',8,' ') ||
				...
				ie_titulo_especialista_w || indic_residencia_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_arq_mov_prestad_100 (nr_seq_lote_p bigint, cd_interface_p text, nm_usuario_p text, ds_local_p text, nm_arquivo_p text) FROM PUBLIC;
