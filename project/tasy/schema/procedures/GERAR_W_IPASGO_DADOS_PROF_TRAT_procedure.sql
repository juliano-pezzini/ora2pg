...
CREATE OR REPLACE PROCEDURE gerar_w_ipasgo_dados_prof_trat ( nr_interno_conta_p bigint, dt_mesano_referencia_p timestamp, nm_usuario_p text, nr_seq_tipo_fatura_p bigint, qt_linha_arq_p INOUT bigint, qt_linha_atend_p INOUT bigint) AS $body$
DECLARE
...
select	sum(CASE WHEN Obter_Conversao_Externa(cd_cnpj_w,'W_IPASGO_DADOS_PROF_TRAT','CD_FUNCAO',a.ie_funcao)=6 THEN 0  ELSE a.vl_participante END ) vl_procedimento,	
	a.ie_funcao,
	a.cd_pessoa_fisica,
...
and	((cd_cnpj_estab_w not in ('02780488000142','02780488000223','02780488000304','26878439000105') and
	c.cd_tipo_procedimento not in (1,2,3,4,5,8,12,13,14,15,16,20,25,26,30,31,33,34,39,74,75,80,91,92,93,94,96,97,103)) or (cd_cnpj_estab_w in ('02780488000142','02780488000223','02780488000304','26878439000105') and c.nr_seq_grupo_rec = 6) or (Obter_Conversao_Externa(cd_cnpj_w,'W_IPASGO_DADOS_PROF_TRAT','CD_FUNCAO',a.ie_funcao) = 6))
...
SELECT	sum(CASE WHEN Obter_Conversao_Externa(cd_cnpj_w,'W_IPASGO_DADOS_PROF_TRAT','CD_FUNCAO',a.ie_funcao)=6 THEN 0 WHEN Obter_Conversao_Externa(cd_cnpj_w,'W_IPASGO_DADOS_PROF_TRAT','CD_FUNCAO',a.ie_funcao)=3 THEN     	CASE WHEN ie_somente_vl_medico_w='S' THEN  (coalesce(((b.qt_procedimento * 			Obter_preco_proc_ipasgo_atend(b.nr_atendimento,b.dt_conta,b.cd_procedimento,b.ie_origem_proced,b.nr_seq_proc_interno,a.ie_responsavel_credito,'M', b.nr_sequencia)) * 				(b.tx_procedimento/100)),0) *0.3)  ELSE (coalesce(((b.qt_procedimento * 			Obter_preco_proc_ipasgo_atend(b.nr_atendimento,b.dt_conta,b.cd_procedimento,b.ie_origem_proced,b.nr_seq_proc_interno,a.ie_responsavel_credito,'IPDE', b.nr_sequencia)) * 				(b.tx_procedimento/100)),0)*0.3) END   ELSE CASE WHEN ie_somente_vl_medico_w='S' THEN  (coalesce(((b.qt_procedimento * 			Obter_preco_proc_ipasgo_atend(b.nr_atendimento,b.dt_conta,b.cd_procedimento,b.ie_origem_proced,b.nr_seq_proc_interno,a.ie_responsavel_credito,'M', b.nr_sequencia)) * 				(b.tx_procedimento/100)),0) *0.2)  ELSE (coalesce(((b.qt_procedimento * 			Obter_preco_proc_ipasgo_atend(b.nr_atendimento,b.dt_conta,b.cd_procedimento,b.ie_origem_proced,b.nr_seq_proc_interno,a.ie_responsavel_credito,'IPDE', b.nr_sequencia)) * 				(b.tx_procedimento/100)),0)*0.2) END  END  ) vl_procedimento,	
	a.ie_funcao,
	a.cd_pessoa_fisica,
	sum(b.qt_procedimento),
	b.dt_procedimento,
...
for C01_w in C01 loop
	...
	cd_medico_convenio_w	:= c01_w.cd_medico_convenio;
	nr_doc_convenio_w	:= c01_w.nr_doc_convenio;
	
	nr_matricula_prestador_w	:= '00000000';
	...
	if (cd_tipo_fatura_w <> 4) and
		...
		if (coalesce(cd_tipo_procedimento_ww::text, '') = '') then
			begin
			ds_controle_ato_w[qt_reg_w].tipo	:= cd_tipo_procedimento_w;
			...
			for i in 1..ds_controle_ato_w.count loop
				begin
				if (ds_controle_ato_w[i].tipo = cd_tipo_procedimento_w) then
					begin
	...
	select	substr(max(obter_crm_medico(cd_pessoa_fisica)), 1, 255)
	...
	begin
	select	coalesce(max(cd_externo), nr_seq_conselho_w)
	into STRICT	nr_seq_conselho_w
	...
	if (coalesce(nr_doc_convenio_w,'X') <> 'X') then
		begin
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_w_ipasgo_dados_prof_trat ( nr_interno_conta_p bigint, dt_mesano_referencia_p timestamp, nm_usuario_p text, nr_seq_tipo_fatura_p bigint, qt_linha_arq_p INOUT bigint, qt_linha_atend_p INOUT bigint) FROM PUBLIC;
