...
CREATE OR REPLACE PROCEDURE gerar_ret_cobr_santander ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
c01 CURSOR FOR
SELECT	nr_sequencia,
	(substr(ds_string,41,12))::numeric  nr_titulo,
	...
	(substr(ds_string,194,15))::numeric  / 100 vl_despesa_bancaria
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_titulo_w IS NOT NULL AND nr_titulo_w::text <> '') then
		...
		select	(substr(ds_string,123,13) || ',' || substr(ds_string,136,2))::numeric ,
			...
			CASE WHEN coalesce(vl_despesa_bancaria_w,0)=0 THEN (substr(ds_string,108,13) || ',' || substr(ds_string,121,2))::numeric   ELSE vl_despesa_bancaria_w END ,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_ret_cobr_santander ( nr_seq_cobr_escrit_p bigint, nm_usuario_p text) FROM PUBLIC;
