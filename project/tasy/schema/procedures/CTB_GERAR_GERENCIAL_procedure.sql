...
C001 CURSOR FOR
	SELECT	s.cd_estabelecimento,
		...
		sum(CASE WHEN ie_normal_encerramento_w='E' THEN (s.vl_movimento - s.vl_encerramento)  ELSE s.vl_movimento END ) vl_movimento,
...
/*	Opcao Anderson 21/07/2006 OS 37439
	1 - Todas contas
	2 - Com Movimento
	3 - Com Saldo
*/
			...
			if (k > 0) then
				...
				if (ds_gerencial_w IS NOT NULL AND ds_gerencial_w::text <> '') and
					...
					values (nm_usuario_p,
						...
						cd_centro_ww,
						cd_classificacao_w,
						coalesce(ds_gerencial_w, WHEB_MENSAGEM_PCK.get_texto(298483)),
						vl_mes_01_w,
						vl_mes_02_w,
						vl_mes_03_w,
						vl_mes_04_w,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ctb_gerar_gerencial ( cd_empresa_p bigint, cd_estab_p text, dt_inicial_p timestamp, dt_final_p timestamp, cd_classif_conta_p text, cd_classif_centro_p text, cd_conta_contabil_p text, cd_centro_custo_p text, qt_min_nivel_conta_p bigint, qt_max_nivel_conta_p bigint, qt_min_nivel_centro_p bigint, qt_max_nivel_centro_p bigint, nm_usuario_p text, ie_ordem_inicio_p text, ie_apurar_result_p text, ie_diario_mensal_p text, ie_opcao_p bigint, ie_normal_encerramento_p text, ie_imprime_24_col_p text) FROM PUBLIC;
