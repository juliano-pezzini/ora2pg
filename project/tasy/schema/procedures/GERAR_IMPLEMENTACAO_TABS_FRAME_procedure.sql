...
CREATE OR REPLACE PROCEDURE gerar_implementacao_tabs_frame ( cd_funcao_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (cd_funcao_p IS NOT NULL AND cd_funcao_p::text <> '') and (nm_usuario_p IS NOT NULL AND nm_usuario_p::text <> '') then
	begin
	ds_package_frame_w	:= 'import br.com.' || obter_form_funcao(cd_funcao_p);
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		begin
		ds_tabs_imports_w		:= ds_tabs_imports_w || replace(ds_package_frame_w,'package','import') || '.' || nm_package_w || '.' || nm_classe_w || ';' || chr(10);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_implementacao_tabs_frame ( cd_funcao_p bigint, nm_usuario_p text) FROM PUBLIC;
