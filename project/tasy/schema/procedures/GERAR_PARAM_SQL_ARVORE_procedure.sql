...
CREATE OR REPLACE PROCEDURE gerar_param_sql_arvore (nr_seq_nivel_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
BEGIN
	...
	into STRICT	ds_comando_w,
		cd_funcao_w
	from	dic_objeto
	where	nr_sequencia = nr_seq_nivel_p;

	update 	dic_objeto
	set	nr_seq_apres 	= nr_seq_apres * -1
	where	nr_seq_obj_sup 	= nr_seq_nivel_p;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_param_sql_arvore (nr_seq_nivel_p bigint, nm_usuario_p text) FROM PUBLIC;
