...
CREATE OR REPLACE PROCEDURE consistir_bloq_agenda_servico (cd_agenda_p bigint, dt_horario_p timestamp, ie_dia_semana_p bigint, cd_Classificacao_p text, ie_bloqueio_p INOUT text) AS $body$
DECLARE
...
if (cd_agenda_p IS NOT NULL AND cd_agenda_p::text <> '') and (dt_horario_p IS NOT NULL AND dt_horario_p::text <> '') and (ie_dia_semana_p IS NOT NULL AND ie_dia_semana_p::text <> '') then
	...
	and	((coalesce(ie_classif_bloqueio, cd_classificacao_p)	= cd_classificacao_p)
	or (coalesce(cd_classificacao_p::text, '') = ''))
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE consistir_bloq_agenda_servico (cd_agenda_p bigint, dt_horario_p timestamp, ie_dia_semana_p bigint, cd_Classificacao_p text, ie_bloqueio_p INOUT text) FROM PUBLIC;
