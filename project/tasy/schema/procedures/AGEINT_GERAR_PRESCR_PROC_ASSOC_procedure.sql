...
CREATE OR REPLACE PROCEDURE ageint_gerar_prescr_proc_assoc (nr_prescricao_p bigint, nr_sequencia_p bigint, ie_proced_vinculado_p text default 'T') AS $body$
DECLARE
...
nr_seq_proc_interno_princ_w   prescr_procedimento.nr_seq_proc_interno%type;
ie_excluir_exame_princ_w	  proc_interno.ie_excluir_proc_princ%type := 'N';

C01 CURSOR FOR
	SELECT	nr_seq_acum_w + row_number() OVER () AS rownum,
		a.nr_seq_proc_int_adic,
		CASE WHEN coalesce(ie_gera_qt_proc_principal_w,'N')='S' THEN  p.qt_procedimento  ELSE a.qt_proc_adic END ,
		p.nm_usuario,
		p.cd_intervalo,
		p.ds_dado_clinico,
		m.cd_material_exame,
		...
		p.nr_seq_interno,
		p.ie_lado,
		a.DS_OBSERVACAO || '  ' || p.ds_observacao,
		a.cd_intervalo,
		p.ie_urgencia,
...
WHERE a.nr_seq_proc_interno	= p.nr_seq_proc_interno and p.nr_prescricao		= nr_prescricao_p and p.nr_sequencia		= nr_sequencia_p and coalesce(a.ie_agenda_integrada,'N')	= 'N' and coalesce(a.ie_somente_agenda_cir,'N') = 'N' and coalesce(a.cd_convenio,cd_convenio_w) = cd_convenio_w and Obter_conv_excluir_proc_assoc(a.nr_sequencia, cd_convenio_w)	= 'S' and ((coalesce(a.cd_convenio_excluir::text, '') = '') or (a.cd_convenio_excluir <> cd_convenio_w)) and ((coalesce(a.cd_categoria_convenio::text, '') = '') or (a.cd_categoria_convenio = cd_categoria_w)) and ((a.cd_plano_convenio	= cd_plano_w) or (coalesce(a.cd_plano_convenio::text, '') = '')) and ((coalesce(a.cd_medico_prescritor::text, '') = '') or (a.cd_medico_prescritor = p.cd_medico_exec)) and ((coalesce(a.cd_medico_excluir::text, '') = '') or (a.cd_medico_excluir <> p.cd_medico_exec)) and ((a.ie_origem_proc_filtro = ie_origem_proc_filtro_w) or (coalesce(a.ie_origem_proc_filtro::text, '') = ''))  and (not exists (
			SELECT	1
			from	prescr_procedimento b
			where	b.nr_prescricao	= nr_prescricao_p
			and	b.nr_seq_proc_interno	= a.nr_seq_proc_int_adic)) and (((coalesce(qt_idade_pac_w::text, '') = '') or (coalesce(a.qt_idade_min::text, '') = '' and coalesce(a.qt_idade_max::text, '') = '')) or
		((qt_idade_pac_w IS NOT NULL AND qt_idade_pac_w::text <> '') and (qt_idade_pac_w between coalesce(a.qt_idade_min,qt_idade_pac_w) and
		coalesce(a.qt_idade_max,qt_idade_pac_w)))) and ((coalesce(ie_proced_vinculado_p,'T')	= 'T') or (a.ie_proced_vinculado = ie_proced_vinculado_p)) and ((ie_sexo = ie_sexo_prescr_w) or (coalesce(ie_sexo::text, '') = ''));
...
C02 CURSOR FOR
	SELECT	nr_seq_acum_w + row_number() OVER () AS rownum,
		...
		p.nm_usuario,
		p.cd_intervalo,
		coalesce(Obter_setor_Atend_proc(cd_estabelecimento_w, a.cd_proc_adic,a.ie_orig_proced_adic, null, cd_setor_atendimento_w, p.nm_usuario,null,nr_atendimento_w),p.cd_setor_atendimento),
		p.ds_dado_clinico,
		Obter_data_prev_exec(dt_prescricao_w,p.dt_prev_execucao,Obter_setor_Atend_proc(cd_estabelecimento_w, a.cd_proc_adic,a.ie_orig_proced_adic, null, cd_setor_atendimento_w, p.nm_usuario,null,nr_atendimento_w), nr_prescricao_p, 'A'),
		m.cd_material_exame,
		a.nr_seq_exame,
		p.ie_origem_inf,
		nr_agrup_acum_w + rownum,
...
LEFT OUTER JOIN material_exame_lab m ON (a.nr_seq_material = m.nr_sequencia)
WHERE a.cd_procedimento 	= p.cd_procedimento and a.ie_origem_proced	= p.ie_origem_proced and p.nr_prescricao		= nr_prescricao_p and p.nr_sequencia		= nr_sequencia_p  and ((coalesce(a.nr_seq_Exame_filtro,0) = 0) or (a.nr_seq_Exame_filtro = p.nr_seq_exame)) and ((coalesce(a.cd_setor_atendimento::text, '') = '') or (a.cd_setor_atendimento = cd_setor_atendimento_w)) and not exists (
			SELECT	1
			from	prescr_procedimento b
			where	b.nr_prescricao	= nr_prescricao_p
			and	b.cd_procedimento	= a.cd_proc_adic) and (((coalesce(qt_idade_pac_w::text, '') = '') or (coalesce(a.qt_idade_min::text, '') = '' and coalesce(a.qt_idade_max::text, '') = '')) or
...
ie_gera_qt_proc_principal_w := obter_param_usuario(924, 517, cd_perfil_orig_w, nm_usuario_orig_w, 0, ie_gera_qt_proc_principal_w);
ie_prescr_proc_sem_lib_w := Obter_Param_Usuario(924, 530, cd_perfil_orig_w, nm_usuario_orig_w, cd_estabelecimento_w, ie_prescr_proc_sem_lib_w);
ie_sem_usuario_w := Obter_Param_Usuario(924, 658, cd_perfil_orig_w, nm_usuario_orig_w, cd_estabelecimento_w, ie_sem_usuario_w);
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (ie_sem_usuario_w = 'N') then
		...
		select 	Obter_setor_Atend_proc(cd_estabelecimento_w, cd_proced_adic_w ,ie_origem_proc_adic_w , null, cd_setor_atendimento_w, null, null,nr_atendimento_w)
		into STRICT	cd_setor_proc_w
		;
	end if;
	...
	if (ie_prescr_proc_sem_lib_w = 'S') then
		CALL Gerar_prescr_proc_sem_dt_lib(nr_prescricao_p,nr_Sequencia_w,cd_perfil_orig_w,'N',nm_usuario_orig_w);
	end if;	
	...
	if (Obter_Funcao_Ativa	= 916) then
		...
		if (nr_seq_regra_plano_w IS NOT NULL AND nr_seq_regra_plano_w::text <> '') then
			...
			if (ie_autorizacao_w IS NOT NULL AND ie_autorizacao_w::text <> '') then
				CALL Atualizar_Aut_Prescr_Proc_EUP(	nr_prescricao_p,
								nr_sequencia_w,
								ie_autorizacao_w,
								nm_usuario_orig_w);
			end if;
...
if (coalesce(nr_sequencia_w,0) > 0 and
    (nr_seq_proc_interno_princ_w IS NOT NULL AND nr_seq_proc_interno_princ_w::text <> '')) then
	...
	select obter_se_proc_int_exc_princ(nr_seq_proc_interno_princ_w)
	into STRICT	ie_excluir_exame_princ_w
	;

	if (ie_excluir_exame_princ_w = 'S') then
		delete
		from	prescr_procedimento
		where	nr_prescricao	= nr_prescricao_p
		and	nr_seq_proc_interno	= nr_seq_proc_interno_princ_w;
	end if;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ageint_gerar_prescr_proc_assoc (nr_prescricao_p bigint, nr_sequencia_p bigint, ie_proced_vinculado_p text default 'T') FROM PUBLIC;
