...
CREATE OR REPLACE PROCEDURE gerar_lote_sep_sn ( nr_prescricao_p bigint, nr_sequencia_p bigint, nr_seq_horario_p bigint, ie_so_aprazado_p text, nm_usuario_p text, ie_origem_lote_p text, ie_send_integration_p text default 'N') AS $body$
DECLARE
...
C01 CURSOR FOR  --especifico para itens SN
	...
	and	coalesce(c.ie_se_necessario, 'N') = 'S'
	...
	--and	(((nvl(b.ie_horario_especial,'N') = 'N') and (ie_gera_lote_orig_w = 'N')) or (ie_gera_lote_orig_w = 'S'))
	...
	(coalesce(ie_itens_associados_w,'S') = 'S' and ((coalesce(b.nr_seq_superior::text, '') = '') or (exists (	SELECT	1
																						...
																						and 	coalesce(d.ie_se_necessario, 'N') = 'N')))))
	...
	and	((ie_gera_lote_orig_w = 'S') or ((ie_origem_lote_p <> 'AIP') or (coalesce(b.ie_aprazado,'N') = 'S')))
	...
	if (ie_itens_associados_w = 'S') then
		...
		EXIT WHEN NOT FOUND; /* apply on C10 */
			...
			values (nextval('ap_lote_log_seq'),
				...
				' cd_local_estoque_ant_w = ' || cd_local_estoque_ant_w || ' nr_seq_mat_hor_w = ' || nr_seq_mat_hor_w ||
				' nr_prescricao_p = ' || nr_prescricao_p || ' nr_sequencia_p = ' || nr_sequencia_p || ' ie_so_aprazado_p = ' || ie_so_aprazado_p ||
				' nr_seq_lote = ' || nr_sequencia_w || ' nr_seq_prescr_mat_w = ' || nr_seq_prescr_mat_w || ' ie_mesmo_turno_w = ' || ie_mesmo_turno_w,
				substr(dbms_utility.format_call_stack,1,2000));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_lote_sep_sn ( nr_prescricao_p bigint, nr_sequencia_p bigint, nr_seq_horario_p bigint, ie_so_aprazado_p text, nm_usuario_p text, ie_origem_lote_p text, ie_send_integration_p text default 'N') FROM PUBLIC;
