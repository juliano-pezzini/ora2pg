...
CREATE OR REPLACE PROCEDURE fis_reg_ecf_u030 ( nr_seq_controle_p text, ds_separador_p text, cd_estabelecimento_p text, nm_usuario_p text, cd_empresa_p text, qt_linha_p INOUT bigint, nr_sequencia_p INOUT bigint, ie_scp_p text) AS $body$
DECLARE
...
c01 CURSOR FOR
	...
	and 	a.ie_lalur_lacs		= 'CS'
	and	a.ie_forma_apuracao 	= 'I'
	and	a.ie_anual_trimestral 	= ie_forma_apuracao_w
...
c02 CURSOR FOR
	...
	order by  d.ie_gerar_ecf, to_number( d.cd_codigo_ecf, '999.999' )asc; -- a.dt_referencia,
vet02	c02%RowType;
...
if (ie_forma_apuracao_w	= 'A') then
	tp_registro	:= 'U030'; --linha A00 - Anual
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	if (vet01.ie_forma_apuracao <> 'R') then
		...
		if (vet01.ie_anual_trimestral = 'A') then
			dt_inicio_w 		:= trunc(vet01.dt_mes_apuracao,'mm'); --dt_inicio_apuracao_w
			dt_fim_w			:= fim_mes(vet01.dt_mes_apuracao);
			ie_tipo_apuracao_w 	:= 'A';
			nr_seq_apuracao_w 	:= to_char(dt_fim_w, 'mm');
		else
			dt_inicio_w 		:= TRUNC(vet01.dt_mes_apuracao, 'Q');
			dt_fim_w			:= fim_mes(add_months(TRUNC(vet01.dt_mes_apuracao, 'Q'), 2));
...
SELECT * FROM fis_reg_ecf_U990(nr_seq_controle_p, ds_separador_p, cd_estabelecimento_p, nm_usuario_p, cd_empresa_p, qt_linha_p, nr_sequencia_p) INTO STRICT qt_linha_p, nr_sequencia_p;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_reg_ecf_u030 ( nr_seq_controle_p text, ds_separador_p text, cd_estabelecimento_p text, nm_usuario_p text, cd_empresa_p text, qt_linha_p INOUT bigint, nr_sequencia_p INOUT bigint, ie_scp_p text) FROM PUBLIC;
