...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_c113_icmsipi ( nr_seq_controle_p bigint, nr_seq_nota_p bigint, nr_linha_p INOUT bigint) AS $body$
DECLARE
...
type reg_c_reg_C113 is table of fis_efd_icmsipi_C113%RowType;
...
fetch c_reg_C113 bulk collect into vetRegC113 limit 1000;
	...
	ds_linha_w := substr(ds_sep_w || vetRegC113[i].cd_reg ||
	ds_sep_w || vetRegC113[i].cd_ind_oper ||
	ds_sep_w || vetRegC113[i].cd_ind_emit ||
	ds_sep_w || vetRegC113[i].cd_part ||
	ds_sep_w || vetRegC113[i].cd_mod ||
	ds_sep_w || vetRegC113[i].cd_ser ||
	ds_sep_w || vetRegC113[i].cd_sub ||
	ds_sep_w || vetRegC113[i].nr_doc ||
	ds_sep_w || to_char(vetRegC113[i].dt_doc,'ddmmyyyy')  ||
	ds_sep_w || vetRegC113[i].ds_chv_doce || ds_sep_w,
	1,
	8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro 		:= vetRegC113[i].cd_reg;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_c113_icmsipi ( nr_seq_controle_p bigint, nr_seq_nota_p bigint, nr_linha_p INOUT bigint) FROM PUBLIC;
