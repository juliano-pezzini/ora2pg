...
CREATE OR REPLACE PROCEDURE pls_gerar_proposta_benef ( nr_seq_solicitacao_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) is cd_pessoa_fisica_w varchar(10) RETURNS bigint AS $body$
DECLARE
...
if (nr_seq_titular_contrato_p IS NOT NULL AND nr_seq_titular_contrato_p::text <> '') then
	select	max(b.cd_cgc),
...
if (nr_seq_titular_solic_p IS NOT NULL AND nr_seq_titular_solic_p::text <> '') then
	select	max(b.nr_seq_pagador)
	into STRICT	nr_seq_pagador_titular_w
	from 	pls_proposta_beneficiario b,
		pls_inclusao_beneficiario a
	where 	a.nr_sequencia = nr_seq_titular_solic_p
...
if (nr_seq_titular_solic_w IS NOT NULL AND nr_seq_titular_solic_w::text <> '') then
	...
	if (coalesce(nr_seq_proposta_benef_tit_w::text, '') = '') then --Caso ainda nao tenha contratado o titular, a contratacao deve ser realizada pelo titular
...
--Caso nao existir vendedor informado, buscar da regra de lancamento automatico do contrato
...
--caso a proposta nao seja criada,, tenta associar com a proposta_web
...
if (qt_pagador_principal_w >1) then
	--Parametro 115 - Utilizar pagador principal do contrato mais especifico
	ie_pagador_princ_w := coalesce(obter_valor_param_usuario(1232,115, Obter_Perfil_Ativo, nm_usuario_p, cd_estabelecimento_p),'N');
	
	--caso o parametro esteja setado para 'S' e exista mais de um pagador principal na proposta 
...
EXIT WHEN NOT FOUND; /* apply on C03 */
	...
	if (ie_inclusao_portabilidade_w = 'S') then
		if (coalesce(nr_seq_portabilidade_w::text, '') = '') then
			CALL wheb_mensagem_pck.exibir_mensagem_abort(1071326, 'NR_SEQ_INCLUSAO_BENEF='||nr_seq_solicitacao_benef_w); --Nao foi gerada solicitacao de portabilidade para a inclusao #@NR_SEQ_INCLUSAO_BENEF#@. Favor verificar.
		end if;
		...
		if (qt_portabilidade_w > 0) then
			CALL wheb_mensagem_pck.exibir_mensagem_abort(1071328, 'NR_SEQ_INCLUSAO_BENEF='||nr_seq_solicitacao_benef_w); --A solicitacao de portabilidade da inclusao #@NR_SEQ_INCLUSAO_BENEF#@ esta pendente. Favor verificar.
		end if;
	end if;
	...
	if (ie_acao_regra_pagador_w = 'GP') then
		...
		if (coalesce(nr_seq_titular_contrato_w::text, '') = '') and (coalesce(nr_seq_titular_solic_w::text, '') = '') then
			...
			nr_seq_pagador_w	:= obter_pagador_titular(nr_seq_titular_contrato_w, nr_seq_titular_solic_w, nr_seq_proposta_w);
		end if;
	else
		ie_gerou_pagador_dependente_w	:= 'N';
	
		if	((nr_seq_titular_contrato_w IS NOT NULL AND nr_seq_titular_contrato_w::text <> '') or (nr_seq_titular_solic_w IS NOT NULL AND nr_seq_titular_solic_w::text <> '')) then
			nr_seq_pagador_ww	:= obter_pagador_titular(nr_seq_titular_contrato_w, nr_seq_titular_solic_w, nr_seq_proposta_w);
			
			if (nr_seq_pagador_ww IS NOT NULL AND nr_seq_pagador_ww::text <> '') then
				ie_gerou_pagador_dependente_w	:= 'S';
				nr_seq_pagador_w		:= nr_seq_pagador_ww;
			end if;
		end if;
		
		if	(nr_seq_subestipulante_w IS NOT NULL AND nr_seq_subestipulante_w::text <> '' AND ie_gerou_pagador_dependente_w = 'N') then
...
if (qt_benef_migrados_w > 0) then
	if (qt_benef_migrados_w = qt_benef_proposta_w) then
		...
		set	ie_tipo_proposta = 4, -- Migracao a um contrato existente
		...
		update	pls_proposta_adesao
		set	ie_tipo_proposta = 8, -- Migracao + Adesao a um contrato existente
			nr_seq_contrato_mig = nr_contrato_ant_w
		where	nr_sequencia = nr_seq_proposta_w;
	end if;
end if;	
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_proposta_benef ( nr_seq_solicitacao_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) is cd_pessoa_fisica_w varchar(10) FROM PUBLIC;
