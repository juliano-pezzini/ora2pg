...
CREATE OR REPLACE PROCEDURE pls_atualiza_conta_proc_fat ( nr_seq_conta_proc_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ie_excluir_fat_p text) AS $body$
DECLARE
...
if (ie_glosa_w <> 'S') then
	...
	if (qt_pos_estabelecido_w	> 0) then
		...
		if (ie_geracao_pos_estabelecido_w = 'C' ) then
			CALL pls_gerar_valor_pos_estab(nr_seq_conta_w,nm_usuario_p,'C',nr_seq_conta_proc_p,null,'A');
		end if;
	end if;
else
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_atualiza_conta_proc_fat ( nr_seq_conta_proc_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ie_excluir_fat_p text) FROM PUBLIC;
