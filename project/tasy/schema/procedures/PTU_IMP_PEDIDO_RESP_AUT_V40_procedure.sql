...
CREATE OR REPLACE PROCEDURE ptu_imp_pedido_resp_aut_v40 ( ds_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
			...
			if (ie_tipo_tabela_w	in ('0','1','4')) then 
				if (coalesce(nr_seq_pedido_aut_w,0)	<> 0) then 
					begin 
						...
						and	((ds_opme IS NOT NULL AND ds_opme::text <> '') and (trim(both upper(elimina_acentuacao(ds_opme)))	= trim(both ds_servico_w)) 
	...
	if	((qt_registros_w > 0) or (qt_registros_ww > 0)) then 
		...
		if (qt_registros_w > 0) and (qt_registros_ww > 0) then 
			...
			set	ie_estagio		= 10, 
				...
				qt_dia_autorizado	= qt_dia_solicitado 
			where	nr_sequencia		= nr_seq_guia_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_imp_pedido_resp_aut_v40 ( ds_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
