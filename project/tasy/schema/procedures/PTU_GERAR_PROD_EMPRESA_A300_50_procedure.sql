...
CREATE OR REPLACE PROCEDURE ptu_gerar_prod_empresa_a300_50 ( nr_seq_mov_plano_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C02 CURSOR FOR
	...
	and	(((b.dt_inicio_vigencia between dt_mov_inicio_w and dt_mov_fim_w) and ((coalesce(b.dt_fim_vigencia::text, '') = '') or (b.dt_fim_vigencia > dt_mov_inicio_w))) or /* Inclusao */
		(b.dt_fim_vigencia between dt_mov_inicio_w and dt_mov_fim_w AND b.dt_fim_vigencia IS NOT NULL AND b.dt_fim_vigencia::text <> '')) /* Exclusao */
...
union
	...
	and	c.nr_seq_classificacao	= nr_seq_classificacao_w
	and (e.nr_sequencia = nr_seq_contrato_lote_w or coalesce(nr_seq_contrato_lote_w::text, '') = '')
	and	((b.dt_inicio_vigencia between dt_mov_inicio_w and dt_mov_fim_w) and ((coalesce(b.dt_fim_vigencia::text, '') = '') or (b.dt_fim_vigencia > dt_mov_inicio_w)))
	and	((trunc(b.dt_inicio_vigencia,'dd') <> trunc(b.dt_fim_vigencia,'dd') and (b.dt_fim_vigencia IS NOT NULL AND b.dt_fim_vigencia::text <> '')) or coalesce(b.dt_fim_vigencia::text, '') = '') /* Nao entrar no lote o que foi incluido e excluido no mesmo mes */
...
union
 select	e.nr_contrato,
	...
	and	c.nr_seq_classificacao	= nr_seq_classificacao_w
	and (e.nr_sequencia = nr_seq_contrato_lote_w or coalesce(nr_seq_contrato_lote_w::text, '') = '')
	and	(b.dt_fim_vigencia between dt_mov_inicio_w and dt_mov_fim_w AND b.dt_fim_vigencia IS NOT NULL AND b.dt_fim_vigencia::text <> '')
	and	((trunc(b.dt_inicio_vigencia,'dd') <> trunc(b.dt_fim_vigencia,'dd') and (b.dt_fim_vigencia IS NOT NULL AND b.dt_fim_vigencia::text <> '')) or coalesce(b.dt_fim_vigencia::text, '') = '') /* Nao entrar no lote o que foi incluido e excluido no mesmo mes */
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_prod_empresa_a300_50 ( nr_seq_mov_plano_p bigint, nm_usuario_p text) FROM PUBLIC;
