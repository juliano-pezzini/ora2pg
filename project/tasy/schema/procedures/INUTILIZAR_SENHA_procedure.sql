...
CREATE OR REPLACE PROCEDURE inutilizar_senha ( cd_senha_p bigint, cd_fila_p bigint, nr_seq_senha_p bigint, nm_usuario_p text, nr_seq_motivo_inutilizacao_p bigint default null) AS $body$
DECLARE
...
ie_consiste_estab_w := Obter_Param_Usuario(10021, 78, Obter_perfil_Ativo, nm_usuario_p, cd_estabelecimento_w, ie_consiste_estab_w);
...
and	((ie_consiste_estab_w = 'S') or (cd_estabelecimento = cd_estabelecimento_w));
...
where	((nr_seq_senha_p > 0 and nr_sequencia = nr_seq_senha_p) or (coalesce(nr_seq_senha_p,0) = 0 and cd_senha_gerada = cd_senha_p and coalesce(nr_seq_fila_senha,nr_seq_fila_senha_origem) = cd_fila_p)) 
and	((ie_consiste_estab_w = 'S') or (cd_estabelecimento = cd_estabelecimento_w));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE inutilizar_senha ( cd_senha_p bigint, cd_fila_p bigint, nr_seq_senha_p bigint, nm_usuario_p text, nr_seq_motivo_inutilizacao_p bigint default null) FROM PUBLIC;
