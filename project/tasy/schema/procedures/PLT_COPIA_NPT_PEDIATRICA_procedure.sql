...
CREATE OR REPLACE PROCEDURE plt_copia_npt_pediatrica ( nr_seq_orig_p bigint, nr_prescricao_p bigint, nr_seq_regra_p bigint, nm_usuario_p text, ie_tipo_item_p text, ie_modificar_p text, cd_perfil_p bigint, cd_estabelecimento_p bigint, ie_estende_inc_p text) AS $body$
DECLARE
...
select	nr_prescricao, 
	CASE WHEN ie_npt_adulta='S' THEN  'NAN' WHEN ie_npt_adulta='N' THEN  'NPN' WHEN ie_npt_adulta='P' THEN 'NPP' END  
...
and	((ie_permite	= 'N') or (ie_estende_inc_p = 'N'));
...
if	((qt_inconsistencia_w	= 0) or (ie_modificar_p 	= 'S')) then 
	...
	SELECT	nr_nut_pac_w, 
		...
		ie_zinco, 
		nr_sequencia,		 
		CASE WHEN ie_npt_adulta='P' THEN coalesce(qt_kcal_kg_ped,0)  ELSE 0 END , 
		coalesce(qt_peso_calorico,0), 
		coalesce(qt_nec_hidrica_diaria_ped,0) 
	from	nut_pac 
	where	nr_sequencia	= nr_seq_orig_p;
	...
	close c03;
	CALL calcular_nut_pac_pediatrica(nr_nut_pac_w,nm_usuario_p,cd_estabelecimento_p);
	CALL Calcular_Agua_Nut_Pac_Pediat(nr_nut_pac_w,nm_usuario_p);
	CALL calcular_nut_pac_pediatrica(nr_nut_pac_w,nm_usuario_p,cd_estabelecimento_p);
	commit;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE plt_copia_npt_pediatrica ( nr_seq_orig_p bigint, nr_prescricao_p bigint, nr_seq_regra_p bigint, nm_usuario_p text, ie_tipo_item_p text, ie_modificar_p text, cd_perfil_p bigint, cd_estabelecimento_p bigint, ie_estende_inc_p text) FROM PUBLIC;
