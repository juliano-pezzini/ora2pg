...
CREATE OR REPLACE PROCEDURE gerar_prescr_hor ( nr_prescricao_p bigint, cd_perfil_p bigint, nm_ususario_p text) AS $body$
DECLARE
...
if (qt_dieta_w > 0) then
	CALL gerar_prescr_dieta_hor(nr_prescricao_p, nm_ususario_p);
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_prescr_hor ( nr_prescricao_p bigint, cd_perfil_p bigint, nm_ususario_p text) FROM PUBLIC;
