...
qt_regra_valid_aut_cpt_cid_w	bigint;
...
select	count(1) 
into STRICT	qt_regra_valid_aut_cpt_cid_w 
from	pls_validacao_aut_cpt_cid 
where	nr_seq_ocor_aut_combinada	= nr_seq_ocor_combinada_p 
and	ie_situacao			= 'A';
 
if (qt_regra_valid_aut_cpt_cid_w > 0) then 
		...
		for r_C01_w in C01 loop 
			begin 
				...
				if (ie_utiliza_filtro_p	= 'S') then 
					...
					SELECT * FROM pls_gerar_ocor_aut_filtro(	nr_seq_ocor_combinada_p, nr_seq_guia_p, nr_seq_requisicao_p, nr_seq_execucao_p, r_C01_w.nr_sequencia, null, null, null, null, r_C01_w.cd_procedimento, r_C01_w.ie_origem_proced, null, ie_gerou_ocor_cabecalho_w, nr_seq_prestador_w, nr_seq_ocorrencia_p, null, null, null, nm_usuario_p, ie_regra_w, ie_tipo_ocorrencia_w) INTO STRICT ie_regra_w, ie_tipo_ocorrencia_w;
				...
				if (ie_gerar_ocorrencia_w	= 'S') then 
					...
					ie_gerar_ocorrencia_w	:= pls_obter_se_ocorrencia_cpt(r_C01_w.nr_sequencia, 1, null, null);		
												 
					if (ie_gerar_ocorrencia_w	= 'S') then							 
						nr_seq_oc_benef_w := pls_inserir_ocorrencia(	nr_seq_segurado_w, nr_seq_ocorrencia_p, null, nr_seq_guia_p, null, r_C01_w.nr_sequencia, null, nr_seq_ocor_combinada_p, nm_usuario_p, null, nr_seq_motivo_glosa_p, 1, cd_estabelecimento_p, 'N', null, nr_seq_oc_benef_w, null, null, null, null);
		...
		for r_C02_w in C02 loop 
			begin 
				...
				if (ie_gerar_ocorrencia_w	= 'S') then 
					...
					ie_gerar_ocorrencia_w	:= pls_obter_se_ocorrencia_cpt(null, 1, r_C02_w.nr_sequencia, null);
		...
		for r_C03_w in C03 loop 
			begin 
				...
				if (ie_utiliza_filtro_p	= 'S') then 
					...
					SELECT * FROM pls_gerar_ocor_aut_filtro(	nr_seq_ocor_combinada_p, null, null, nr_seq_execucao_p, null, null, null, null, r_C03_w.nr_sequencia, r_C03_w.cd_procedimento, r_C03_w.ie_origem_proced, null, ie_gerou_ocor_cabecalho_w, nr_seq_prestador_w, nr_seq_ocorrencia_p, null, null, null, nm_usuario_p, ie_regra_w, ie_tipo_ocorrencia_w) INTO STRICT ie_regra_w, ie_tipo_ocorrencia_w;
				...
				if (ie_gerar_ocorrencia_w	= 'S') then 
					...
					ie_gerar_ocorrencia_w	:= pls_obter_se_ocorrencia_cpt(null, 1, null, r_C03_w.nr_sequencia);
					 
					if (ie_gerar_ocorrencia_w	= 'S') then						 
						nr_seq_oc_benef_w := pls_inserir_ocorrencia(	nr_seq_segurado_w, nr_seq_ocorrencia_p, null, null, null, r_C03_w.nr_sequencia, null, nr_seq_ocor_combinada_p, nm_usuario_p, null, nr_seq_motivo_glosa_p, 10, cd_estabelecimento_p, 'N', nr_seq_execucao_p, nr_seq_oc_benef_w, null, null, null, null);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_valida_ocor_aut_cpt_cid ( nr_seq_ocor_combinada_p pls_ocor_aut_combinada.nr_sequencia%type, nr_seq_ocorrencia_p pls_ocorrencia.nr_sequencia%type, nr_seq_motivo_glosa_p tiss_motivo_glosa.nr_sequencia%type, nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, nr_seq_execucao_p pls_execucao_requisicao.nr_sequencia%type, ie_utiliza_filtro_p pls_ocor_aut_combinada.ie_utiliza_filtro%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type ) FROM PUBLIC;
