...
CREATE OR REPLACE PROCEDURE ptu_gerar_cons_dados_benef_v50 ( cd_unimed_benef_p bigint, cd_usuario_plano_p text, nm_beneficiario_p text, sobrenome_benef_p text, dt_nascimento_p text, nr_versao_ptu_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_retorno_p INOUT bigint) AS $body$
DECLARE

/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade:  Gerar a transação do PTU que realiza a consulta de dados do beneficiário, através do SCS
-------------------------------------------------------------------------------------------------------------------
...
if	((length(cd_usuario_plano_p) < 17) or (length(cd_usuario_plano_p) > 17)) then
	CALL wheb_mensagem_pck.exibir_mensagem_abort(190565);
	--A carteirinha deve conter 17 dígitos !
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_cons_dados_benef_v50 ( cd_unimed_benef_p bigint, cd_usuario_plano_p text, nm_beneficiario_p text, sobrenome_benef_p text, dt_nascimento_p text, nr_versao_ptu_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_retorno_p INOUT bigint) FROM PUBLIC;
