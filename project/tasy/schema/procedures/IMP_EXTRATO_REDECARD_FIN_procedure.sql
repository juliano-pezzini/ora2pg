...
CREATE OR REPLACE PROCEDURE imp_extrato_redecard_fin (nr_seq_extrato_p bigint, nm_usuario_p text, ds_arquivo_p text) AS $body$
DECLARE
...
SELECT	nr_sequencia,
	...
	(substr(ds_conteudo,95,13) || ',' || substr(ds_conteudo,108,2))::numeric  vl_bruto,
	(substr(ds_conteudo,32,13) || ',' || substr(ds_conteudo,45,2))::numeric  vl_liquido,
	to_date(substr(ds_conteudo,24,8),'ddmmyyyy') dt_compra,
	(substr(ds_conteudo,125,2))::numeric  nr_parcela,
	(substr(ds_conteudo,128,2))::numeric  qt_parcelas
...
SELECT	nr_sequencia,
	...
	(substr(ds_conteudo,209,13) || ',' || substr(ds_conteudo,222,2))::numeric  vl_bruto,
	(substr(ds_conteudo,30,13) || ',' || substr(ds_conteudo,43,2))::numeric  vl_liquido,
	to_date(substr(ds_conteudo,22,8),'ddmmyyyy') dt_compra
from	w_extrato_cartao_cr
where	nr_seq_extrato			= nr_seq_extrato_p
and	substr(ds_conteudo,1,3) 	= '035' -- Despesas
and	obter_se_regra_importacao(nr_seq_grupo_w,substr(ds_conteudo,4,9)) = 'S'
and	substr(ds_conteudo,46,2)	in ('28','06')
order by nr_sequencia;
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	if (ie_tipo_registro_w = '034')  then
		...
		/*OS 1428551 - Pegar conta correte e conta ativa.*/
		...
		if (coalesce(nr_seq_conta_banco_w::text, '') = '') then
			/* Não foi encontrada a conta bancária de referência do arquivo importado no cadastro de bancos!
			Banco somente_numero(cd_banco_w)
			Agência somente_numero(cd_agencia_w)
...
set	dt_inicial		= dt_min_parcela_w,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE imp_extrato_redecard_fin (nr_seq_extrato_p bigint, nm_usuario_p text, ds_arquivo_p text) FROM PUBLIC;
