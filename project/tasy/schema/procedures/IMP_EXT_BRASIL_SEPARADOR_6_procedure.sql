...
CREATE OR REPLACE PROCEDURE imp_ext_brasil_separador_6 ( nr_seq_extrato_p bigint) AS $body$
DECLARE
...
c01 CURSOR FOR 
SELECT	replace(substr(obter_valor_campo_separador(a.ds_conteudo,2,';'),1,255),'"','') ds_dt_lancamento, 
	rtrim(replace(substr(obter_valor_campo_separador(a.ds_conteudo,4,';'),1,255),'"','')) ds_historico, 
	replace(substr(obter_valor_campo_separador(a.ds_conteudo,3,';'),1,255),'"','') nr_documento, 
	somente_numero(substr(obter_valor_campo_separador(a.ds_conteudo,5,';'),1,255)) / 100 vl_lancamento, 
	somente_numero(substr(obter_valor_campo_separador(a.ds_conteudo,6,';'),1,255)) / 100 ie_deb_cred 
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	if (coalesce(ds_dt_lancamento_w,'00000000') <> '00000000') then 
		...
		dt_lancamento_w	:= to_date(ds_dt_lancamento_w,'yyyy/mm/dd');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE imp_ext_brasil_separador_6 ( nr_seq_extrato_p bigint) FROM PUBLIC;
