...
CREATE OR REPLACE PROCEDURE gerar_ipasgo_dados_proc_trat_w ( nr_interno_conta_p bigint, dt_mesano_referencia_p timestamp, nm_usuario_p text, nr_seq_tipo_fatura_p bigint, qt_linha_arq_p INOUT bigint, qt_linha_atend_p INOUT bigint) AS $body$
DECLARE
...
SELECT	1,
	...
	b.ie_funcao_medico,
	coalesce((b.qt_procedimento * Obter_preco_proc_ipasgo_atend(b.nr_atendimento,b.dt_conta,b.cd_procedimento,b.ie_origem_proced,b.nr_seq_proc_interno,b.ie_responsavel_credito,'M')),0) vl_medico,
...
and	a.cd_tipo_procedimento not in (1,2,3,4,5,12,13,14,15,16,20,25,26,30,31,34,39,74,75,80,85,91,92,93,94,96,97,99,115,133,134)--add 93,99,133
and	somente_numero(b.cd_procedimento_convenio) not in (70025,70033)	--Estes sao alto custo devem sair no union 5
...
/*Anderson 15/03/2011 - Gerar taxa de box hora ao inves de proc ou honorario quando tipo de fatura for 8 (box hora)*/
...
/*Geliard 24/10/2012 para Hemolabor - Agrupar os procedimentos e ordenar pelo codigo*/
...
select	6,
	...
	b.nr_seq_proc_interno,
	b.ie_funcao_medico,
	coalesce(sum(b.qt_procedimento * Obter_preco_proc_ipasgo_atend(b.nr_atendimento,b.dt_conta,b.cd_procedimento,b.ie_origem_proced,b.nr_seq_proc_interno,b.ie_responsavel_credito,'M')),0) vl_medico,
	a.cd_tipo_procedimento,
...
and	a.cd_tipo_procedimento not in (1,2,3,4,5,8,12,13,14,15,16,20,25,26,30,31,33,34,39,74,75,80,85,91,92,93,94,96,97,99,103)-- add 85,93,99
and	somente_numero(b.cd_procedimento_convenio) not in (70025,70033)	--Estes sao alto custo devem sair no union 5
...
order by	nr_ato_ipasgo,
	...
	vl_ch_medico desc,
	cd_procedimento,	/*como via unica de acesso e os seguintes como mesma via do principal. OS597773 afreinert 27/05/2013*/
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (cd_tipo_fatura_w = 3) then	/*Alteracao para o Hemolabor 416225 - gerar mesmo ato para gerar mesma via de acesso, entao tem que ser a mesma data*/
	...
	if (coalesce(ie_funcao_medico_w,'0') <> '8') then
		...
		if (coalesce(nr_ato_ipasgo_w,0) = 0) then
			if	((dt_procedimento_w <> dt_procedimento_ww) or (coalesce(dt_procedimento_ww::text, '') = '')) then
				qt_ato_w	:= qt_ato_w + 1;
				nr_seq_proc_w	:= 0;
		...
		/*Buscar as conversoes meio externo - procedimento ou proc interno*/
		...
		if (cd_procedimento_w = 20010) and (coalesce(ie_funcao_parecer_w,'X') = ie_funcao_medico_w)  then
			qt_procedimento_w := 1;			
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_ipasgo_dados_proc_trat_w ( nr_interno_conta_p bigint, dt_mesano_referencia_p timestamp, nm_usuario_p text, nr_seq_tipo_fatura_p bigint, qt_linha_arq_p INOUT bigint, qt_linha_atend_p INOUT bigint) FROM PUBLIC;
