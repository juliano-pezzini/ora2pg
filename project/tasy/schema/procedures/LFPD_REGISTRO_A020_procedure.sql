...
CREATE OR REPLACE PROCEDURE lfpd_registro_a020 ( nr_seq_controle_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, dt_inicio_p timestamp, dt_fim_p timestamp, ds_separador_p text, qt_linha_p INOUT bigint, nr_sequencia_p INOUT bigint) AS $body$
DECLARE
...
c01 CURSOR FOR
	SELECT	'A020'																			cd_registro,
		...
		substr(CASE WHEN coalesce(n.cd_condicao_pagamento,0)=0 THEN  0  ELSE CASE WHEN obter_dados_nota_fiscal(n.nr_sequencia, '28')=0 THEN  0  ELSE 1 END  END , 1, 1)  					ie_pagto,
		0																				vl_subcontratado,
...
WHERE n.cd_operacao_nf = o.cd_operacao_nf  and coalesce(Obter_se_nota_entrada_saida(n.nr_sequencia),'E') = 'S' and to_char(n.dt_emissao,'dd/MM/yy') between to_date(dt_inicio_p,'dd/MM/yy') and to_date(dt_fim_p,'dd/MM/yy') and n.cd_estabelecimento = cd_estabelecimento_p and (n.dt_atualizacao_estoque IS NOT NULL AND n.dt_atualizacao_estoque::text <> '') and o.ie_servico = 'S' and n.vl_total_nota > 0 and n.ie_situacao = 1 and (m.cd_modelo_nf <> '3A' or coalesce(m.cd_modelo_nf::text, '') = '') and exists (SELECT 1
...
union
	...
	FROM operacao_nota o, natureza_operacao a, nota_fiscal n
LEFT OUTER JOIN modelo_nota_fiscal m ON (n.nr_seq_modelo = m.nr_sequencia)
WHERE n.cd_operacao_nf = o.cd_operacao_nf  and coalesce(Obter_se_nota_entrada_saida(n.nr_sequencia),'E') = 'E' and to_char(n.dt_emissao,'dd/MM/yy') between to_date(dt_inicio_p,'dd/MM/yy') and to_date(dt_fim_p,'dd/MM/yy') and n.cd_estabelecimento = cd_estabelecimento_p and (n.dt_atualizacao_estoque IS NOT NULL AND n.dt_atualizacao_estoque::text <> '') and o.ie_servico = 'S' and n.vl_total_nota > 0 and n.ie_situacao = 1 and (m.cd_modelo_nf <> '3A' or coalesce(m.cd_modelo_nf::text, '') = '') and exists (select 1
			from	nota_fiscal_item x
			where	x.nr_sequencia = n.nr_sequencia
			and		a.cd_natureza_operacao = x.cd_natureza_operacao
			and ((x.cd_material IS NOT NULL AND x.cd_material::text <> '') or (x.cd_procedimento IS NOT NULL AND x.cd_procedimento::text <> ''))  LIMIT 1);
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	select	coalesce(max(c.nr_seq_regra_lfpd),0)
	...
	if ie_gerar_w = 'S' then
		SELECT * FROM lfpd_registro_A025(nr_seq_controle_p, vet01.nr_seq_nota, nm_usuario_p, cd_estabelecimento_p, dt_inicio_p, dt_fim_p, ds_separador_p, nr_linha_w, nr_seq_registro_w) INTO STRICT nr_linha_w, nr_seq_registro_w;
	end if;
	SELECT * FROM lfpd_registro_A200(nr_seq_controle_p, nm_usuario_p, cd_estabelecimento_p, vet01.nr_seq_nota, dt_inicio_p, dt_fim_p, ds_separador_p, nr_linha_w, nr_seq_registro_w) INTO STRICT nr_linha_w, nr_seq_registro_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE lfpd_registro_a020 ( nr_seq_controle_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, dt_inicio_p timestamp, dt_fim_p timestamp, ds_separador_p text, qt_linha_p INOUT bigint, nr_sequencia_p INOUT bigint) FROM PUBLIC;
