...
CREATE OR REPLACE PROCEDURE plt_copia_suporte_nut (nr_prescricao_p bigint, nr_seq_regra_p bigint, ds_lista_p text, ie_modificar_p text, nm_usuario_p text, cd_perfil_p bigint, cd_estabelecimento_p bigint, ie_estende_inc_p text, ie_copia_agora_p text) AS $body$
DECLARE
...
ie_hora_sne_w := Obter_Param_Usuario(924, 146, cd_perfil_p, nm_usuario_p, cd_estabelecimento_p, ie_hora_sne_w);
...
while (ds_lista_w IS NOT NULL AND ds_lista_w::text <> '') loop
	...
	CALL PLT_consiste_extensao_item(dt_inicio_prescr_w, dt_validade_prescr_w, nr_prescr_lista_w, nr_seq_lista_w, 'SNE', nr_seq_regra_p, nm_usuario_p, cd_perfil_p, cd_estabelecimento_p);
		...
		if (qt_w > 0) then
			...
			if (ie_regra_geral_w = 'I') and (obter_se_intervalo_agora(cd_intervalo_w) = 'N') then
				...
				if (ie_operacao_w in ('F','V')) then
					...
					dt_ultimo_horario_w	:= plt_obter_ultimo_horario(nr_prescr_lista_w, nr_seq_lista_w, 'SNE', nm_usuario_p);
				...
				ds_horarios_w := substr(ds_horarios_w || ds_horarios2_w,1,2000);
			end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE plt_copia_suporte_nut (nr_prescricao_p bigint, nr_seq_regra_p bigint, ds_lista_p text, ie_modificar_p text, nm_usuario_p text, cd_perfil_p bigint, cd_estabelecimento_p bigint, ie_estende_inc_p text, ie_copia_agora_p text) FROM PUBLIC;
