...
CREATE OR REPLACE PROCEDURE intdisp_gerar_mat_hor_reapr ( nr_prescricao_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	and     c.ie_suspenso <> 'S'
	--and 	a.ie_classif_urgente = 'A'
	and     coalesce(a.dt_suspensao::text, '') = ''
	and		(a.qt_hor_reaprazamento IS NOT NULL AND a.qt_hor_reaprazamento::text <> '')
	and		(a.nm_usuario_reaprazamento IS NOT NULL AND a.nm_usuario_reaprazamento::text <> '')
	and	not exists (
		SELECT	1
		from	int_disp_mat_hor x
		where	x.nr_seq_lote = l.nr_sequencia)
	and exists(
	select	1
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (nr_seq_lote_w IS NOT NULL AND nr_seq_lote_w::text <> '') and (coalesce(ie_local_disp_eletronico_w,'N') = 'S') then
		begin
...
update	int_disp_mat_hor a
set		a.ie_tipo_movimentacao = 'EOD',
	a.dt_leitura  = NULL
where	a.nr_prescricao = nr_prescricao_p
and	exists (
	SELECT	1
	from	prescr_mat_hor b
	where	b.nr_sequencia = a.nr_seq_mat_hor
	and		b.nr_prescricao = nr_prescricao_p
	and		b.nr_seq_lote <> a.nr_seq_lote
	and		(b.qt_hor_reaprazamento IS NOT NULL AND b.qt_hor_reaprazamento::text <> '')
	and		(b.nm_usuario_reaprazamento IS NOT NULL AND b.nm_usuario_reaprazamento::text <> ''));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE intdisp_gerar_mat_hor_reapr ( nr_prescricao_p bigint, nm_usuario_p text) FROM PUBLIC;
