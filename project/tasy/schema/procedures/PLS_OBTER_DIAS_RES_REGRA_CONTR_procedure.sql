...
CREATE OR REPLACE PROCEDURE pls_obter_dias_res_regra_contr ( nr_seq_segurado_p bigint, nr_seq_motivo_cancel_p bigint, dt_rescisao_p timestamp, cd_estabelecimento_p bigint, nm_usuario_p text, dt_rescisao_nova_p INOUT timestamp) AS $body$
DECLARE
...
BEGIN
select	CASE WHEN coalesce(b.cd_pf_estipulante::text, '') = '' THEN 'PJ'  ELSE 'PF' END ,
	a.nr_seq_contrato
...
dt_rescisao_nova_p	:= dt_rescisao_nova_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_obter_dias_res_regra_contr ( nr_seq_segurado_p bigint, nr_seq_motivo_cancel_p bigint, dt_rescisao_p timestamp, cd_estabelecimento_p bigint, nm_usuario_p text, dt_rescisao_nova_p INOUT timestamp) FROM PUBLIC;
