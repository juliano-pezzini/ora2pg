...
CREATE OR REPLACE PROCEDURE dmed_mensal_reembolso_titular (nr_seq_dmed_p bigint, dt_referencia_p timestamp, cd_estabelecimento_p bigint, ie_cpf_p text, ie_idade_p text, ie_estrangeiro_p text) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	from	titulo_pagar p
	where	pkg_date_utils.start_of(p.dt_liquidacao,'YEAR',0) = pkg_date_utils.start_of(dt_referencia_p,'YEAR',0);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dmed_mensal_reembolso_titular (nr_seq_dmed_p bigint, dt_referencia_p timestamp, cd_estabelecimento_p bigint, ie_cpf_p text, ie_idade_p text, ie_estrangeiro_p text) FROM PUBLIC;
