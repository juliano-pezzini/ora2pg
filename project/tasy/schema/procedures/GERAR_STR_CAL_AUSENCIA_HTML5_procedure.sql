...
CREATE OR REPLACE PROCEDURE gerar_str_cal_ausencia_html5 (dia_base_p bigint, mes_base_p bigint, ds_string_p text, ds_string_w INOUT text) AS $body$
DECLARE
...
BEGIN
	...
	if (ds_string_p IS NOT NULL AND ds_string_p::text <> '') then
		...
		select to_char(PKG_DATE_UTILS.END_OF(PKG_DATE_UTILS.GET_DATE(mes_base_p),'MONTH'),'dd')
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_str_cal_ausencia_html5 (dia_base_p bigint, mes_base_p bigint, ds_string_p text, ds_string_w INOUT text) FROM PUBLIC;
