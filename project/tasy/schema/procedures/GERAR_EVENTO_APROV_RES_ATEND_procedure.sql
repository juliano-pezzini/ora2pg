...
CREATE OR REPLACE PROCEDURE gerar_evento_aprov_res_atend ( nr_seq_evento_p bigint, nm_usuario_p text, nr_prescricao_p bigint, nr_seq_prescr_p bigint, nr_seq_exame_p bigint, ie_trigger_p text, dt_aprovacao_p timestamp, cd_medico_resp_p text) AS $body$
DECLARE
...
ds_senha_w				varchar(60);
ds_senha_paciente_w		pessoa_fisica.ds_senha%type;
...
select	coalesce(max(ds_senha), '')
into STRICT	ds_senha_paciente_w
from	pessoa_fisica
where	cd_pessoa_fisica = cd_pessoa_fisica_w;

--gravar_log_tasy(89,nr_prescricao_p||' - '||nr_seq_prescr_p||' - '||nr_seq_exame_p||' - '||cd_material_exame_w||' - '||nr_seq_exame_nao_aceit_w||' - '||ds_resultado_nao_aceit_w,'teste');
...
ds_mensagem_w	:= substr(replace_macro(ds_mensagem_w,'@senha_atend',ds_senha_w),1,4000);
ds_mensagem_w	:= substr(replace_macro(ds_mensagem_w,'@cod_paciente',cd_pessoa_fisica_w),1,4000);
ds_mensagem_w	:= substr(replace_macro(ds_mensagem_w,'@senha_paciente',ds_senha_paciente_w),1,4000);
...
ds_titulo_w	:= substr(replace_macro(ds_titulo_w,'@senha_atend',ds_senha_w),1,4000);
ds_titulo_w	:= substr(replace_macro(ds_titulo_w,'@cod_paciente',cd_pessoa_fisica_w),1,4000);
ds_titulo_w	:= substr(replace_macro(ds_titulo_w,'@senha_paciente',ds_senha_paciente_w),1,4000);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_evento_aprov_res_atend ( nr_seq_evento_p bigint, nm_usuario_p text, nr_prescricao_p bigint, nr_seq_prescr_p bigint, nr_seq_exame_p bigint, ie_trigger_p text, dt_aprovacao_p timestamp, cd_medico_resp_p text) FROM PUBLIC;
