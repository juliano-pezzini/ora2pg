...
CREATE OR REPLACE PROCEDURE gerar_relat_mapa_ocupacao_v2 ( cd_estabelecimento_p bigint, nm_usuario_cor_p text, ie_todos_estab_p text ) AS $body$
DECLARE
...
and	obter_se_exibe_estab(
		...
		nm_usuario_cor_p,
		ie_todos_estab_p) = 'S'
order by	coalesce(nr_seq_apresentacao, 999);
...
C03 CURSOR FOR
	SELECT cd
	from	(
		SELECT		         TO_NUMBER(	obter_status_leito_ocupacao(
							(SUBSTR(Obter_Status_Isolamento_Ocup(cd_estabelecimento_p, nr_atendimento, ie_acompanhante, nm_usuario_cor_p),1,1)),
							SUBSTR(obter_classif_etaria(obter_estabelecimento_ativo,cd_pessoa_fisica,'C'),1,3),
							ie_sexo,
							ie_status_unidade,
							ie_temporario,
							dt_alta_medico,
							...
							CASE WHEN ie_legenda_ap='S' THEN a.dt_previsto_alta  ELSE NULL END ,
							obter_se_alta_tesouraria2(a.nr_atendimento, a.cd_setor_atendimento, a.cd_unidade_basica, a.cd_unidade_compl),
							a.nr_seq_interno)) cd
		--substr(' ' || substr(obter_status_leito(a.cd_setor_atendimento, a.cd_unidade_basica, a.cd_unidade_compl, cd_estabelecimento_p,'N'),1,10),1,10) cd
		from		ocupacao_unidade_v a
		where		a.cd_setor_atendimento = cd_setor_atendimento_w
		order by 	a.ds_ocupacao
		) alias8 LIMIT 32;
...
BEGIN
ie_legenda_ap := Obter_param_Usuario(44, 148, obter_perfil_ativo, nm_usuario_cor_p, cd_estabelecimento_p, ie_legenda_ap);
ie_alta_medico_w := Obter_param_Usuario(44, 119, obter_perfil_ativo, nm_usuario_cor_p, cd_estabelecimento_p, ie_alta_medico_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_relat_mapa_ocupacao_v2 ( cd_estabelecimento_p bigint, nm_usuario_cor_p text, ie_todos_estab_p text ) FROM PUBLIC;
