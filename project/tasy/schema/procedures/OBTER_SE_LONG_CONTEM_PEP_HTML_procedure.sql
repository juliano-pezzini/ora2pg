...
CREATE OR REPLACE PROCEDURE obter_se_long_contem_pep_html ( nm_tabela_p text, nm_coluna_p text, ds_restricao_where_p text, ds_parametros_sql_p text, ds_texto_busca_p text, ie_achou_p INOUT text) AS $body$
DECLARE
...
if	((dbms_lob.INSTR(ds_campo_clob_w,ds_texto_busca_w,1,1) > 0) or (dbms_lob.INSTR(ds_campo_clob_w,ds_texto_busca_w2,1,1) > 0)) then
	ie_achou_p := 'S';
end	if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE obter_se_long_contem_pep_html ( nm_tabela_p text, nm_coluna_p text, ds_restricao_where_p text, ds_parametros_sql_p text, ds_texto_busca_p text, ie_achou_p INOUT text) FROM PUBLIC;
