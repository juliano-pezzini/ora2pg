...
CREATE OR REPLACE PROCEDURE schematic4test_insertagroup (NR_SEQ_SCRIPT_P bigint, NR_SEQ_SNIPPET_P bigint, NM_USUARIO_P text, IE_TYPE_SCRIPT_P text) AS $body$
DECLARE
...
C01 CURSOR FOR  --Loop steps
  SELECT DISTINCT STEP2.NR_SEQ_EXECUTION NR_SEQ_EXECUTION, ACTION.DS_ACTION DS_ACTION, STEP2.NR_SEQ_ACTION NR_SEQ_ACTION, max(STEP2.NR_SEQUENCIA) NR_SEQUENCIA_STEP, STEP2.DS_VERSION DS_VERSION, STEP2.IE_SWITCH, STEP2.IE_JOBS, SNIPPET.NR_SEQUENCIA NR_SEQ_SNIPPET, STEP2.NR_SEQUENCIA NR_SEQ_AGRUPADOR
      INTO STRICT NR_SEQ_EXECUTION_W, DS_ACTION_W, NR_SEQ_ACTION_W, NR_SEQUENCIA_STEP_W, DS_VERSION_W, IE_SWITCH_W, IE_JOBS_W, NR_SEQ_SNIPPET_W, NR_SEQ_AGRUPADOR_W
  ...
  WHERE SNIPPET.NR_SEQUENCIA = NR_SEQ_SNIPPET_P
  AND coalesce(STEP2.NR_SEQ_SCRIPT::text, '') = ''
  AND SNIPPET.IE_TYPE_SCRIPT = IE_TYPE_SCRIPT_P
  GROUP BY STEP2.NR_SEQ_EXECUTION, ACTION.DS_ACTION, STEP2.NR_SEQ_ACTION, STEP2.DS_VERSION, STEP2.IE_SWITCH, STEP2.IE_JOBS, SNIPPET.NR_SEQUENCIA, STEP2.NR_SEQUENCIA
...
C02 CURSOR FOR  --Loop values
  ...
  WHERE STEP.NR_SEQ_SNIPPET = NR_SEQ_AGRUPADOR_W
...
BEGIN
    --procedure that insert snippet
	SELECT IE_TYPE_SCRIPT 
      INTO STRICT IE_TYPE_SCRIPT_W
    FROM SCHEM_TEST_SCRIPT
    WHERE NR_SEQUENCIA = NR_SEQ_SCRIPT_P;

	IF (IE_TYPE_SCRIPT_W = IE_TYPE_SCRIPT_P) THEN   
		...
		VALUES (nextval('schem_test_beholder_seq'), NM_USUARIO_P, clock_timestamp(), 'Robot', clock_timestamp(), 'SCHEMATIC4TEST_INSERTAGROUP', 
		'NR_SEQ_SCRIPT_P as '||NR_SEQ_SCRIPT_P, 
		'NR_SEQ_SNIPPET_P as '||NR_SEQ_SNIPPET_P);
		...
		BEGIN	
			...
			INSERT INTO SCHEM_TEST_STEP(NR_SEQUENCIA, NR_SEQ_EXECUTION, NM_USUARIO_NREC, NM_USUARIO, NR_SEQ_ACTION, NR_SEQ_SNIPPET, DS_VERSION, IE_SWITCH, IE_JOBS, NR_SEQ_SCRIPT, DT_ATUALIZACAO_NREC, DT_ATUALIZACAO, NR_SEQ_AGRUPADOR) 
              VALUES (NEWSEQUENCESTEP, NR_SEQ_EXECUTION_W, NM_USUARIO_P, NM_USUARIO_P, NR_SEQ_ACTION_W, NR_SEQ_SNIPPET_W, DS_VERSION_W , IE_SWITCH_W, IE_JOBS_W, NR_SEQ_SCRIPT_P, clock_timestamp(), clock_timestamp(), NR_SEQ_AGRUPADOR_W);
			COMMIT;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE schematic4test_insertagroup (NR_SEQ_SCRIPT_P bigint, NR_SEQ_SNIPPET_P bigint, NM_USUARIO_P text, IE_TYPE_SCRIPT_P text) FROM PUBLIC;
