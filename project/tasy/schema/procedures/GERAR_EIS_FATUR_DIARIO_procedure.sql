...
CREATE OR REPLACE PROCEDURE gerar_eis_fatur_diario ( dt_parametro_P timestamp, nm_usuario_p text) AS $body$
DECLARE
...
delete	from eis_fatur_diario
where	dt_referencia	= dt_referencia_w;
insert into eis_fatur_diario(
	dt_referencia,
	cd_estabelecimento,
	dt_atualizacao,
	nm_usuario,
...
from	eis_resultado
where	dt_referencia	between trunc(dt_referencia_w,'month') and last_day(dt_referencia_w)
group by
	dt_referencia_w,
	cd_estabelecimento,
	cd_convenio,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_eis_fatur_diario ( dt_parametro_P timestamp, nm_usuario_p text) FROM PUBLIC;
