...
CREATE OR REPLACE PROCEDURE ptu_imp_pedido_status_tran_v50 ( ds_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_origem_p INOUT bigint) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (substr(ds_conteudo_w,1,4)	<> 'FIM$') then 
		if (nr_seq_import_w	= 1) then 
			...
			nr_versao_w			:= trim(both substr(ds_conteudo_w,39,3));
...
if (nr_seq_requisicao_w IS NOT NULL AND nr_seq_requisicao_w::text <> '') then 
	CALL pls_requisicao_gravar_hist(nr_seq_requisicao_w, 'L', 'Recebido pedido de status da Unimed '||cd_unimed_exec_w, null, nm_usuario_p);
elsif (nr_seq_guia_w IS NOT NULL AND nr_seq_guia_w::text <> '') then 
	CALL pls_guia_gravar_historico(nr_seq_guia_w, 2, 'Recebido pedido de status da Unimed '||cd_unimed_exec_w, '', nm_usuario_p);
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_imp_pedido_status_tran_v50 ( ds_arquivo_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_origem_p INOUT bigint) FROM PUBLIC;
