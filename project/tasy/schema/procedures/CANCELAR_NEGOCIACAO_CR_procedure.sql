...
CREATE OR REPLACE PROCEDURE cancelar_negociacao_cr ( nr_seq_negociacao_p bigint, dt_cancelamento_p text, nm_usuario_p text, nr_seq_motivo_canc_p bigint, ds_observacao_p text) AS $body$
DECLARE
...
C03 CURSOR FOR 
	SELECT	a.nr_titulo, 
		b.nr_sequencia, 
		CASE WHEN  coalesce(ie_data_estorno_neg_w,'N')='S' THEN  coalesce(to_date(dt_cancelamento_p,'dd/mm/yyyy hh24:mi:ss'), b.dt_recebimento)  ELSE b.dt_recebimento END  
...
if (nr_seq_negociacao_p IS NOT NULL AND nr_seq_negociacao_p::text <> '') then 
	...
	if (trunc(dt_negociacao_w) > coalesce(to_date(dt_cancelamento_p,'dd/mm/yyyy hh24:mi:ss'),clock_timestamp())) then 
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		set	dt_liquidacao 	= coalesce(to_date(dt_cancelamento_p,'dd/mm/yyyy hh24:mi:ss'),clock_timestamp()) 
	...
	set	dt_cancelamento		= coalesce(to_date(dt_cancelamento_p,'dd/mm/yyyy hh24:mi:ss'),clock_timestamp()), 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE cancelar_negociacao_cr ( nr_seq_negociacao_p bigint, dt_cancelamento_p text, nm_usuario_p text, nr_seq_motivo_canc_p bigint, ds_observacao_p text) FROM PUBLIC;
