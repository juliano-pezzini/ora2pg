...
CREATE OR REPLACE PROCEDURE gerar_bordero_darf ( nr_seq_darf_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, cd_darf_p text, dt_vencimento_p timestamp) AS $body$
DECLARE
...
if (ie_selec_tit_outro_bordero_w = 'N') then
	...
	and	d.nr_seq_darf	= nr_seq_darf_p
	and	(t.nr_bordero IS NOT NULL AND t.nr_bordero::text <> '')  LIMIT 1;

	if (qt_registro_w > 0) then
		CALL wheb_mensagem_pck.exibir_mensagem_abort(212259);
	end if;
...
if vl_percomp_w > 0 then
	...
	-- Verificar o parametro do usuario, se for N apenas realizar UPDATE na tabela de titulos a pagar; se for S realizar INSERT na tabela BORDERO_TIT_PAGAR.
	if (ie_selec_tit_outro_bordero_w = 'N') then
		open c01;
		loop
		fetch c01 into	
		...
		EXIT WHEN NOT FOUND; /* apply on c01 */
			...
			set	nr_bordero		= nr_bordero_w,
				...
				vl_juros_bordero	= (vl_titulo_w * vl_juros_bordero_w / vl_soma_titulo_w),
				vl_multa_bordero	= (vl_titulo_w * vl_multa_bordero_w / vl_soma_titulo_w),
				vl_desconto_bordero	= (vl_titulo_w * vl_percomp_w / vl_soma_titulo_w)
	...
	if (ie_selec_tit_outro_bordero_w = 'N') then
		...
		EXIT WHEN NOT FOUND; /* apply on c01 */
			...
			set	nr_bordero		= nr_bordero_w,
				...
				vl_desconto_bordero	= vl_percomp_w
			where	nr_titulo		= nr_titulo_w;		
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_bordero_darf ( nr_seq_darf_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, cd_darf_p text, dt_vencimento_p timestamp) FROM PUBLIC;
