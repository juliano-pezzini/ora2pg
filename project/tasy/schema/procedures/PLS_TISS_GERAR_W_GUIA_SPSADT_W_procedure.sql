...
CREATE OR REPLACE PROCEDURE pls_tiss_gerar_w_guia_spsadt_w ( nr_sequencia_autor_p bigint, ds_dir_padrao_p text, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (coalesce(nr_sequencia_autor_p,0) > 0) then 
	...
	select	a.cd_guia, 
		...
		to_date(pls_obter_dados_cart_segurado(a.nr_seq_segurado,'D'),'dd/mm/yyyy'), 
		b.cd_pessoa_fisica, 
	...
	values (nextval('w_tiss_contratado_exec_seq'),clock_timestamp(),nm_usuario_p, 
		 nr_seq_guia_w, cgc_exec_w, nr_seq_prestador_w, 
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		if (qt_proc_guia_w	<= 5) then 
			dados_regra_preco_proc_w := pls_define_preco_proc(cd_estabelecimento_p, nr_seq_prestador_w, null, dt_autorizacao_w, null, cd_procedimento_w, ie_origem_proced_w, null, null, null, nr_seq_plano_w, 'P', 0, 0, null, 'N', null, '', '', '', '', '', '', null, '', '', '', '', 'A', 'X', null, ie_carater_internacao_w, dt_procedimento_w, null, null, null, dados_guia_w, null, dados_regra_preco_proc_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_tiss_gerar_w_guia_spsadt_w ( nr_sequencia_autor_p bigint, ds_dir_padrao_p text, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
