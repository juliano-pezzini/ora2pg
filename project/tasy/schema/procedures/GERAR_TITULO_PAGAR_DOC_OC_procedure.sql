...
CREATE OR REPLACE PROCEDURE gerar_titulo_pagar_doc_oc ( nr_sequencia_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
exception 
	when others then 
	CALL wheb_mensagem_pck.exibir_mensagem_abort(230297);
	/*r.aise_application_error(-20011,'Não foram localizados os parâmetros do contas a pagar!#@#@');*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_titulo_pagar_doc_oc ( nr_sequencia_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
