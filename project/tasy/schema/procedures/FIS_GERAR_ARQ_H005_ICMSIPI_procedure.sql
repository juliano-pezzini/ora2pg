...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_h005_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
fetch c_reg_H005 bulk collect into vetRegH005 limit 1000;
	...
	ds_linha_w := substr(ds_sep_w || vetRegH005[i].cd_reg ||
			     ds_sep_w || to_char(vetRegH005[i].dt_inv ,'ddmmyyyy') ||
			     ds_sep_w || vetRegH005[i].vl_inv     ||
			     ds_sep_w || vetRegH005[i].cd_mot_inv ||
			     ds_sep_w, 1, 8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro		:=  vetRegH005[i].cd_reg;

	-------> H010
	/* Gera registros do H010*/

        nr_linha_w := fis_gerar_arq_H010_icmsipi(nr_seq_controle_p, vetRegH005[i].dt_inv, nr_linha_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_h005_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) FROM PUBLIC;
