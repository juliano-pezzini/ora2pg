...
CREATE OR REPLACE PROCEDURE pfcs_mpl_lhs_tl_trans_review ( nr_seq_indicator_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text ) AS $body$
DECLARE
	...
	begin
		...
		begin
		 ...
		 := pfcs_pck_v2.pfcs_generate_results(
			vl_indicator_p => qt_total_w, ds_reference_value_p => r_c02.ds_department, nr_seq_indicator_p => nr_seq_indicator_p, nr_seq_operational_level_p => cd_estabelecimento_p, nm_usuario_p => nm_usuario_p, nr_seq_panel_p => nr_seq_panel_w);
	end;
	end loop;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pfcs_mpl_lhs_tl_trans_review ( nr_seq_indicator_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text ) FROM PUBLIC;
