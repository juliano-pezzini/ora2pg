...
CREATE OR REPLACE PROCEDURE qt_atualizar_dados_marc_ageint ( nr_seq_item_p bigint, dt_agenda_p timestamp, nm_usuario_p text, nr_seq_local_p bigint, cd_estabelecimento_p bigint, nr_minuto_duracao_p bigint, ie_gerar_p INOUT text, nr_seq_prof_p bigint default null) AS $body$
DECLARE
...
from	agenda_quimio
where	nr_seq_ageint_item	= nr_seq_item_p  LIMIT 1;
exception
	when others then
	qt_agendado_w	:= 0;
end;
...
if (ie_dia_quimio_w	= 'S') then
	...
	if (nr_seq_ageint_w	> 0) then
		ie_hor_concor_w	:= Obter_Se_Pac_Ageint_Concor(nr_seq_ageint_w, dt_agenda_p, nr_seq_item_p, qt_tempo_medic_w);
...
where	(dt_agenda_p < dt_Agenda or dt_agenda_p > dt_Agenda + (nr_duracao - 1) / 1440)
and	nr_seq_Ageint_item	= nr_seq_item_p  LIMIT 1;
exception
	when others then
	qt_agendado_w	:= 0;
	--ie_gerar_w		:= 'M';
...
if (ie_dia_quimio_w	= 'S') and (qt_agendado_w	= 0) then
	...
	if (qt_marcacao_w	> 0) then
		...
		from	w_agenda_quimio
		where	nr_seq_local	= nr_seq_local_p
		and		dt_horario		= dt_agenda_p
		and		ie_status		= 'L'  LIMIT 1;
		exception
			when others then
			qt_livre_w	:= 0;
		end;
		...
		and (ie_status = 'B' or ie_status = 'AG')
		and	nr_seq_local = nr_seq_local_p;
	
		if	(((dt_agenda_p + qt_tempo_medic_w / 1440) <= (dt_ultimo_horario_w + coalesce(nr_min_apres_local_w,nr_min_apres_w) / 1440)) and (qt_marcacao_w		= 0) and (qt_em_agenda_w		= 0) or (ie_perm_mesmo_horario_w = 'S')) and (qt_pendencia_marcada_w	= 0) and (ie_prof_marcado_w	= 'N') and (qt_livre_w		= 1) and (qt_bloqueio_w 		= 0) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE qt_atualizar_dados_marc_ageint ( nr_seq_item_p bigint, dt_agenda_p timestamp, nm_usuario_p text, nr_seq_local_p bigint, cd_estabelecimento_p bigint, nr_minuto_duracao_p bigint, ie_gerar_p INOUT text, nr_seq_prof_p bigint default null) FROM PUBLIC;
