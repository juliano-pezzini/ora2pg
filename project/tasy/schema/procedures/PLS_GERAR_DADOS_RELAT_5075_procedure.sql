...
CREATE OR REPLACE PROCEDURE pls_gerar_dados_relat_5075 ( nr_titulo_p bigint, nr_seq_lote_p bigint, nr_seq_lote_grupo_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
nr_seq_linha_item_copartic_w	bigint;
nr_seq_benef_inicio_pag_w	bigint;
cd_usuario_inicio_pag_w		varchar(30);
nm_segurado_inicio_pag_w	varchar(255);
...
qt_linhas_demonstrativo_w	integer;
i				integer;
dt_vencimento_original_w	titulo_receber.dt_vencimento%type;
...
union
	...
	select	nr_titulo,
		nr_seq_mensalidade
	from	pls_boleto
	where	nr_seq_lote_grupo = nr_seq_lote_grupo_p
	and	coalesce(nr_seq_lote_grupo_p,0) <> 0
	and	(nr_seq_mensalidade IS NOT NULL AND nr_seq_mensalidade::text <> '');

C02 CURSOR FOR	/* Dados ITENS DE MENSALIDADE - boleto PJ */
	SELECT nr_seq_plano,
	       nr_seq_tabela,
	       qt_idade_inicial,
	       ds_item,
...
C03 CURSOR FOR	/* Dados TRIBUTOS DO TÍTULO */
	SELECT	substr(obter_desc_tributo(a.cd_tributo), 1, 200) ds_tributo,
		...
		sum(coalesce(a.vl_tributo, 0)) vl_tributo,
		sum(coalesce(a.vl_base_calculo, 0)) vl_base_calculo,
		b.ie_tipo_tributo
...
C04 CURSOR FOR	/* Dados PLANO */
	...
	group by	d.ie_tipo_contratacao, coalesce(e.ie_participacao,d.ie_participacao), d.ds_plano,
			d.ie_regulamentacao, d.nr_protocolo_ans, d.cd_scpa;
...
C07 CURSOR FOR	/* Dados BENEFICIÁRIOS com coparticipação */
	...
	and	exists (SELECT	1
			...
			where	z.nr_seq_conta 			= y.nr_seq_conta
			and	z.nr_seq_mensalidade_seg 	= a.nr_sequencia
			and    	z.ie_tipo_item = 3
			and	y.ie_status_coparticipacao = 'S');

C08 CURSOR FOR	/* Dados ITENS DE COPARTICIPAÇÃO */
	SELECT	coalesce(d.dt_atendimento_referencia, d.dt_emissao) dt_emissao_conta,
	...
	and	b.ie_status_coparticipacao = 'S'
	group by
		d.nr_seq_prestador_exec,
		coalesce(d.dt_atendimento_referencia, d.dt_emissao),
		d.nr_sequencia,
		d.nr_seq_protocolo,
		d.cd_guia,
		d.cd_guia_referencia,
...
union all
	...
	select	coalesce(to_date(pls_obter_dados_conta_proc(b.nr_seq_conta_proc, 'DT')), coalesce(d.dt_atendimento_referencia, d.dt_emissao)) dt_emissao_conta,
		d.nr_sequencia nr_seq_conta,
		d.nr_seq_protocolo,
		d.cd_guia,
		d.cd_guia_referencia,
	...
	and	b.ie_status_coparticipacao = 'S'
	group by
		coalesce(to_date(pls_obter_dados_conta_proc(b.nr_seq_conta_proc, 'DT')), coalesce(d.dt_atendimento_referencia, d.dt_emissao)),
		d.nr_seq_prestador_exec,
		d.nr_sequencia,
		d.nr_seq_protocolo,
		d.cd_guia,
...
C09 CURSOR FOR	/* Dados DEMONSTRATIVO DE PAGAMENTO */
	SELECT	a.ie_tipo_item,
		substr(CASE WHEN a.ie_tipo_item='20' THEN  'LM - '||(	SELECT	ds_tipo_lancamento								from	pls_tipo_lanc_adic								where	nr_sequencia		= coalesce(a.nr_seq_tipo_lanc,0))  ELSE obter_valor_dominio(1930, a.ie_tipo_item) END ,1,255) ds_tipo_item,
		count(*) qt_item,
		sum(vl_item) vl_item
	from	pls_mensalidade_seg_item 	a,
		pls_mensalidade_segurado 	b
	...
	group by	a.ie_tipo_item, a.nr_seq_tipo_lanc
	order by
		ds_tipo_item;

C10 CURSOR(	nr_seq_mensalidade_pc	pls_mensalidade.nr_sequencia%type) FOR	/* Dados BENEFICIÁRIOS com coparticipação */
	SELECT	a.nr_sequencia nr_seq_mensalidade_seg,
		b.nr_sequencia nr_seq_segurado,
		substr(c.nm_pessoa_fisica, 1, 255) nm_segurado
	from	pls_mensalidade_segurado 	a,
...
C11 CURSOR(	nr_seq_mensalidade_seg_pc	pls_mensalidade_segurado.nr_sequencia%type) FOR
	SELECT	coalesce(pls_obter_dados_conta_proc(d.nr_sequencia, 'DT'), coalesce(c.dt_atendimento_referencia, c.dt_emissao)) dt_emissao_conta,
		...
		e.nr_sequencia nr_seq_mens_item_w
	...
	group by
		...
		coalesce(pls_obter_dados_conta_proc(d.nr_sequencia, 'DT'), coalesce(c.dt_atendimento_referencia, c.dt_emissao)),
		c.nr_sequencia,
		c.nr_seq_protocolo,
		c.cd_guia,
		c.cd_guia_referencia,
		pls_obter_dados_conta_proc(b.nr_seq_conta_proc,'CP'), d.cd_procedimento,
		e.nr_sequencia
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (ie_pf_pj_w = 'PJ') then
		qt_limite_linhas_w		:= 44;
	else
		qt_limite_linhas_w		:= 49;
	end if;
	...
	select	substr(b.ds_endereco || ' ' || b.nr_endereco || ' ' || b.ds_complemento,1,255),
		b.ds_bairro,
		b.cd_cep,
		b.ds_municipio,
		b.sg_estado
	into STRICT	ds_endereco_w,
		...
		ds_municipio_w,
		sg_estado_w
	from (	SELECT	ie_endereco_boleto
			from	pls_contrato_pagador
			where	nr_sequencia		= nr_seq_pagador_w) a,
		table(pls_gerar_boletos_mens_fat_pck.obter_dados_endereco_pagador(cd_pessoa_fisica_w,cd_cgc_w,a.ie_endereco_boleto)) b;
	...
	if (ie_pf_pj_w = 'PF') then
		...
		cd_usuario_plano_w	:= substr(pls_carteira_pag_se_seg(nr_seq_pagador_w, cd_pessoa_fisica_w), 1, 30);
	...
	if (ie_pf_pj_w = 'PF') then
		...
		EXIT WHEN NOT FOUND; /* apply on C05 */
			begin
			if (C05%rowcount = 1) then
				qt_linhas_extrato_w := qt_linhas_extrato_w + 3; /* considera a linha do título + descrição das colunas */
			end if;
			...
			insert into w_pls_relat_boleto(	nr_sequencia, dt_atualizacao, nm_usuario, dt_atualizacao_nrec, nm_usuario_nrec, ie_banda,
				...
				values (	nr_seq_linha_benef_w, clock_timestamp(), nm_usuario_p, clock_timestamp(), nm_usuario_p, 'B',
						...
						cd_usuario_plano_w, nm_segurado_w, dt_contratacao_w, ds_plano_w, cd_ans_w, ie_registro_w,
						ie_prorrogado_w);

			qt_linhas_extrato_w	:= qt_linhas_extrato_w + 3; -- considera a linha do beneficiário + total + linha em branco */
			...
			EXIT WHEN NOT FOUND; /* apply on C06 */
				...
				insert into w_pls_relat_boleto(	nr_sequencia, dt_atualizacao, nm_usuario, dt_atualizacao_nrec, nm_usuario_nrec, ie_banda,
							nr_titulo, ie_pf_pj, nr_seq_mensalidade_seg, ds_item, vl_item, ie_registro,
							ie_prorrogado)
					values (	nextval('w_pls_relat_boleto_seq'), clock_timestamp(), nm_usuario_p, clock_timestamp(), nm_usuario_p, 'I',
							nr_titulo_w, ie_pf_pj_w, nr_seq_mensalidade_seg_w, ds_item_w, vl_item_w, ie_registro_w,
							ie_prorrogado_w);
				...
				qt_itens_benef_w	:= qt_itens_benef_w + 1;
				end;
			end loop;
			close C06;
		...
		EXIT WHEN NOT FOUND; /* apply on C07 */
			...
			qt_linhas_extrato_w 	:= qt_linhas_extrato_w + 2; -- considera a linha do beneficiário + total */
			...
			EXIT WHEN NOT FOUND; /* apply on C08 */
				begin
				if (C08%rowcount = 1) then
					qt_linhas_extrato_w := qt_linhas_extrato_w + 1;
				end if;

				select	nextval('w_pls_relat_boleto_seq')
				into STRICT	nr_seq_linha_item_copartic_w
				;
				...
				insert into w_pls_relat_boleto(	nr_sequencia, dt_atualizacao, nm_usuario, dt_atualizacao_nrec, nm_usuario_nrec, ie_banda,
					...
					values (	nr_seq_linha_item_copartic_w, clock_timestamp(), nm_usuario_p, clock_timestamp(), nm_usuario_p, 'A',
							nr_titulo_w, ie_pf_pj_w, nr_seq_mensalidade_seg_w, nr_pagina_linha_w,
							dt_emissao_conta_w, nr_seq_conta_w, nr_seq_protocolo_w, cd_guia_w, cd_guia_referencia_w,
							nm_prestador_w, ds_procedimento_w, cd_procedimento_w, vl_procedimento_w, vl_coparticipacao_w, ie_registro_w,
							ie_prorrogado_w, qt_coparticipacao_w);
				...
				if (qt_linhas_contador_w > qt_limite_lin_extrato_w) then
					...
					update	w_pls_relat_boleto
					set	nr_pagina = nr_pagina_linha_w
					where	nr_sequencia = nr_seq_linha_item_copartic_w;

					nr_seq_benef_inicio_pag_w 	:= nr_seq_mensalidade_seg_w;
					cd_usuario_inicio_pag_w		:= cd_usuario_plano_w;
					nm_segurado_inicio_pag_w	:= nm_segurado_w;
				end if;
		...
		for r_c10_w in C10(nr_seq_mensalidade_w) loop
			...
			for r_c11_w in C11(r_c10_w.nr_seq_mensalidade_seg) loop
				...
				from ( 	SELECT x.nr_seq_item
					from 	  pls_mensalidade_item_conta x
					where 	  r_c11_w.nr_seq_mens_item_w = x.nr_seq_item
					group by x.nr_seq_item) alias1;
				...
				if (qt_linhas_contador_w > qt_limite_lin_extrato_w) then
					...
					nr_seq_benef_inicio_pag_w 	:= r_c10_w.nr_seq_mensalidade_seg;
					cd_usuario_inicio_pag_w		:= cd_usuario_plano_w;
					nm_segurado_inicio_pag_w	:= r_c10_w.nm_segurado;
				end if;
				end;
			end loop;
		...
		EXIT WHEN NOT FOUND; /* apply on C09 */
			begin
			if (C09%rowcount = 1) then
				qt_linhas_demonstrativo_w	:= qt_linhas_demonstrativo_w + 1;
				qt_linhas_extrato_w 		:= qt_linhas_extrato_w + 1;
			end if;
			...
			insert into w_pls_relat_boleto(	nr_sequencia, dt_atualizacao, nm_usuario, dt_atualizacao_nrec, nm_usuario_nrec, ie_banda,
					nr_titulo, ie_pf_pj, ie_tipo_item, ds_item, qt_item, vl_item,
					ie_registro, ie_prorrogado)
			values (	nextval('w_pls_relat_boleto_seq'), clock_timestamp(), nm_usuario_p, clock_timestamp(), nm_usuario_p, 'D',
					nr_titulo_w, ie_pf_pj_w, ie_tipo_item_w, ds_item_w, qt_item_w, vl_item_w,
					ie_registro_w, ie_prorrogado_w);

			qt_linhas_demonstrativo_w	:= qt_linhas_demonstrativo_w + 1;
			qt_linhas_extrato_w 		:= qt_linhas_extrato_w + 1;
					/*
			if	(length(ds_item_w) > 41) then
				qt_linhas_demonstrativo_w	:= qt_linhas_demonstrativo_w + 1;
				qt_linhas_extrato_w 		:= qt_linhas_extrato_w + 1;
			end if;		*/
			end;
		...
		EXIT WHEN NOT FOUND; /* apply on C02 */
			...
			insert into w_pls_relat_boleto(	nr_sequencia, dt_atualizacao, nm_usuario, dt_atualizacao_nrec, nm_usuario_nrec, ie_banda,
						nr_titulo, ie_pf_pj, ds_item, vl_item, qt_segurado, vl_total_item,
						ie_registro, ie_prorrogado)
				values (	nextval('w_pls_relat_boleto_seq'), clock_timestamp(), nm_usuario_p, clock_timestamp(), nm_usuario_p, 'I',
						nr_titulo_w, ie_pf_pj_w, ds_item_w, vl_unitario_w, qt_segurados_w, vl_total_item_w,
						ie_registro_w, ie_prorrogado_w);
		...
		EXIT WHEN NOT FOUND; /* apply on C03 */
			...
			insert into w_pls_relat_boleto(	nr_sequencia, dt_atualizacao, nm_usuario, dt_atualizacao_nrec, nm_usuario_nrec, ie_banda,
						nr_titulo, ie_pf_pj, vl_mensalidade, cd_cgc, ds_tributo, tx_tributo,
						vl_tributo, vl_base_calculo_trib, ie_tipo_tributo, ie_registro, ie_prorrogado)
				values (	nextval('w_pls_relat_boleto_seq'), clock_timestamp(), nm_usuario_p, clock_timestamp(), nm_usuario_p, 'T',
						nr_titulo_w, ie_pf_pj_w, vl_mensalidade_w, cd_cgc_w, ds_tributo_w, tx_tributo_w,
						vl_tributo_w, vl_base_calculo_trib_w, ie_tipo_tributo_w, ie_registro_w, ie_prorrogado_w);
		...
		EXIT WHEN NOT FOUND; /* apply on C04 */
			...
			insert into w_pls_relat_boleto(	nr_sequencia, dt_atualizacao, nm_usuario, dt_atualizacao_nrec, nm_usuario_nrec, ie_banda,
						nr_titulo, ie_pf_pj, cd_operadora_empresa, ie_tipo_contratacao, ie_participacao, ds_plano,
						cd_ans, ie_registro, ie_prorrogado)
				values (	nextval('w_pls_relat_boleto_seq'), clock_timestamp(), nm_usuario_p, clock_timestamp(), nm_usuario_p, 'O',
						nr_titulo_w, ie_pf_pj_w, cd_operadora_empresa_w, ie_tipo_contratacao_w, ie_participacao_w, ds_plano_w,
						cd_ans_w, ie_registro_w, ie_prorrogado_w);
	...
	if (ie_pf_pj_w = 'PF') then
		...
		if	(qt_benef_copartic_w > 0) then
			qt_linhas_total_w := qt_linhas_total_w + 3 + (qt_benef_copartic_w * 2)  + qt_itens_copartic_w;
		end if;

		if	(qt_benef_pos_estab_w > 0) then
			qt_linhas_total_w := qt_linhas_total_w + 3/* + (qt_benef_pos_estab_w * 4) + qt_itens_pos_estab_w;
		end if;*/
		qt_linhas_total_w := qt_linhas_extrato_w;

		if (qt_linhas_total_w <= qt_limite_linhas_w) then
			qt_linhas_brancas_w := qt_limite_linhas_w - qt_linhas_total_w;
		else
			qt_linhas_brancas_w := qt_limite_linhas_w - 3 - qt_linhas_demonstrativo_w;
			...
			for i in 1..qt_paginas_w loop
				...
				insert into w_pls_relat_boleto(	nr_sequencia, dt_atualizacao, nm_usuario, dt_atualizacao_nrec, nm_usuario_nrec, ie_banda,
							nr_titulo, ie_pf_pj, nr_pagina, nr_seq_mensalidade_seg, cd_usuario_plano, nm_pessoa,
							ie_registro, ie_prorrogado)
					values (	nextval('w_pls_relat_boleto_seq'), clock_timestamp(), nm_usuario_p, clock_timestamp(), nm_usuario_p, 'Q',
							nr_titulo_w, ie_pf_pj_w, nr_pagina_w, nr_seq_benef_inicio_pag_w, cd_usuario_inicio_pag_w, nm_segurado_inicio_pag_w,
							ie_registro_w, ie_prorrogado_w);
			end loop;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_dados_relat_5075 ( nr_titulo_p bigint, nr_seq_lote_p bigint, nr_seq_lote_grupo_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
