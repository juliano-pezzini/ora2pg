...
CREATE OR REPLACE PROCEDURE man_calcular_preco_ativ_prev ( nr_sequencia_p bigint) AS $body$
DECLARE
...
if (ie_calcula_valor_w = 'S') then
	...
	set	vl_cobranca		= vl_preco_w * coalesce(round((dividir(coalesce(qt_min_prev,0),60))::numeric,2),0),
		vl_cobranca_venda		= vl_venda_w * coalesce(round((dividir(coalesce(qt_min_prev,0),60))::numeric,2),0)
...
and	dt_referencia	= trunc(dt_prevista_w,'dd');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE man_calcular_preco_ativ_prev ( nr_sequencia_p bigint) FROM PUBLIC;
