...
CREATE TYPE filtros_not AS (	ds_filtros	varchar(255));
CREATE TYPE colunas AS (	nr_sequencia		bigint,
			cd_centro_controle_orig	centro_controle.cd_centro_controle%type,
			cd_centro_controle_dest	centro_controle.cd_centro_controle%type
			);


CREATE OR REPLACE PROCEDURE gerar_w_res_centro_holding ( cd_empresa_p bigint, cd_empresa_param_p bigint, cd_estab_p bigint, cd_centro_controle_p text, nr_meses_p bigint, cd_tabela_custo_p bigint, nr_seq_tabela_p bigint, ie_relatorio_p text, ie_centro_resultado_p text, ie_centro_superior_p text, cd_centro_superior_p text, nr_seq_ng_p text, cd_classificacao_p text, ie_tipo_relat_ng_p text, nr_seq_gng_p text, ie_tipo_centro_controle_p text, ie_tipo_gasto_p text, ie_totalizar_centro_p text, ie_zero_p text, ie_result_orc_p text, ie_somente_ativo_p text, ie_centro_atual_p text, ie_ordem_inversa_p text, nm_usuario_p text, nr_seq_unid_neg_p bigint, ie_somente_classif_ativa_p text, ie_resultado_orcado_p text, NR_SEQ_TABELA_ORC_p bigint) AS $body$
DECLARE
...
type reg_filtro_not is table of filtros_not index by integer;

/* Teste  */

type valores_adic is table of colunas index by integer;

valores_adic_w					valores_adic;
...
ds_tipo_centro_w				varchar(4000);
ds_centro_controle_w				varchar(4000);
ie_somente_centro_usuario_w			varchar(1);
nr_seq_unid_neg_w				unidade_negocio.nr_sequencia%type	:= coalesce(nr_seq_unid_neg_p,0);
ie_result_possib_gng_w	varchar(2);
ie_result_possib_ng_w	varchar(2);
cd_estab_w					estabelecimento.cd_estabelecimento%type	:= coalesce(cd_estab_p,0);
...
and	((coalesce(ie_somente_classif_ativa_p,'N') = 'N') or (coalesce(b.ie_situacao,'A')	= 'A'))
...
and 	((ie_somente_centro_usuario_w = 'N') or (substr(cus_obter_se_centro_usuario(a.cd_estabelecimento,a.cd_centro_controle, nm_usuario_p),1,1) = 'S'))
and	((coalesce(ds_filtro_ng_w,'0') = '0') or (obter_se_contido(a.nr_seq_ng,ds_filtro_ng_w) = ie_result_possib_ng_w))
and	((ie_result_possib_gng_w = 'N' and coalesce(a.nr_seq_ng::text, '') = '') or exists(	SELECT	1
										from	natureza_gasto z
										where	z.nr_sequencia = a.nr_seq_ng
										and	((coalesce(ds_filtro_gng_w,'0') = '0') or (obter_se_contido(z.nr_seq_gng,ds_filtro_gng_w) = ie_result_possib_gng_w))
										 LIMIT 1))
and     exists (select	1
		from	tabela_custo_acesso_v x
		where	x.cd_empresa = d.cd_empresa
		...
		and	((x.cd_empresa = cd_empresa_w) or (coalesce(cd_empresa_w,0) = 0)))
group by
...
union all
										...
										where	coalesce(a.nr_seq_ng::text, '') = ''
										and	ds_filtro_gng_w	= '0'
										and	ie_result_possib_gng_w = 'N'
										 LIMIT 1))
and     exists (select	1
		from	tabela_custo_acesso_v x
		where	x.cd_empresa = d.cd_empresa
...
from	(
	SELECT	/*+INDEX (A RESCECO_UK)*/		CASE WHEN ie_tipo_relat_ng_p='0' THEN e.nr_sequencia  ELSE '0' END  nr_seq_ng,
		CASE WHEN ie_tipo_relat_ng_p='0' THEN e.ds_natureza_gasto  ELSE '0' END  ds_natureza_gasto,
		f.nr_sequencia nr_seq_gng,
		f.ds_grupo_natureza_gasto,
		a.ie_tipo_gasto,
		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN 000 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-1)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_01,
		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -01 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-2)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_02,
		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -02 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-3)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_03,
		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -03 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-4)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_04,
		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -04 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-5)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_05,
		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -05 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-6)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_06,
		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -06 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-7)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_07,
		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -07 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-8)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_08,
		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -08 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-9)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_09,
		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -09 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-10)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_10,
		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -10 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-11)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_11,
		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -11 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-12)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_12
	from	grupo_natureza_gasto f,
		...
		resultado_centro_controle a
	where	a.cd_estabelecimento	= coalesce(d.cd_estabelecimento, a.cd_estabelecimento)
	and	((a.ie_tipo_gasto	= ie_tipo_gasto_p) or (coalesce(ie_tipo_gasto_p,'0') = '0'))
	and	d.cd_empresa		= b.cd_empresa
	and	a.cd_tabela_custo	= d.cd_tabela_custo
	and	a.nr_seq_tabela		= d.nr_sequencia
	and	b.cd_classificacao	= a.ie_classif_conta
	...
	and	d.dt_mes_referencia between dt_inicial_w and dt_final_w
	and	((coalesce(ds_filtro_ng_w, '0') = '0') or (obter_se_contido(e.nr_sequencia,ds_filtro_ng_w) = ie_result_possib_ng_w))
	and	((coalesce(ds_filtro_gng_w, '0') = '0') or (obter_se_contido(f.nr_sequencia,ds_filtro_gng_w) = ie_result_possib_gng_w))
	and     exists (select	1
			from	tabela_custo_acesso_v x
			where	x.cd_empresa = d.cd_empresa
			and	x.cd_estabelecimento = a.cd_estabelecimento
...
SELECT	coalesce(sum(y.vl_mes_12),0) vl_mes_12,
	...
	coalesce(sum(y.vl_mes_01),0) vl_mes_01
from	(
	SELECT	/*+INDEX (A RESCECO_UK)*/		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN 000 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-1)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_01,
		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -01 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-2)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_02,
		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -02 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-3)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_03,
		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -03 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-4)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_04,
		CASE WHEN d.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -04 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-5)*-1) END ,0) THEN a.vl_mes  ELSE 0 END  vl_mes_05,
...
from	(
	SELECT	d.cd_centro_controle,
		...
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN 000 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-1)*-1) END ,0) THEN a.vl_distribuido  ELSE 0 END  vl_mes_01,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -01 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-2)*-1) END ,0) THEN a.vl_distribuido  ELSE 0 END  vl_mes_02,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -02 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-3)*-1) END ,0) THEN a.vl_distribuido  ELSE 0 END  vl_mes_03,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -03 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-4)*-1) END ,0) THEN a.vl_distribuido  ELSE 0 END  vl_mes_04,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -04 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-5)*-1) END ,0) THEN a.vl_distribuido  ELSE 0 END  vl_mes_05,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -05 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-6)*-1) END ,0) THEN a.vl_distribuido  ELSE 0 END  vl_mes_06,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -06 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-7)*-1) END ,0) THEN a.vl_distribuido  ELSE 0 END  vl_mes_07,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -07 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-8)*-1) END ,0) THEN a.vl_distribuido  ELSE 0 END  vl_mes_08,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -08 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-9)*-1) END ,0) THEN a.vl_distribuido  ELSE 0 END  vl_mes_09,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -09 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-10)*-1) END ,0) THEN a.vl_distribuido  ELSE 0 END  vl_mes_10,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -10 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-11)*-1) END ,0) THEN a.vl_distribuido  ELSE 0 END  vl_mes_11,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -11 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-12)*-1) END ,0) THEN a.vl_distribuido  ELSE 0 END  vl_mes_12
	from	centro_controle d,
		centro_controle c,
		orcamento_distribuido a,
		tabela_custo b
	where	a.cd_centro_controle_dest	= cd_centro_controle_w
...
SELECT	3 ie_tipo_indicador,
	WHEB_MENSAGEM_PCK.get_texto(299224) ds_tipo_indicador,
	y.nr_seq_indicador,
	y.ds_indicador,
	coalesce(sum(y.vl_mes_12),0) vl_mes_12,
	coalesce(sum(y.vl_mes_11),0) vl_mes_11,
...
from 	(
	SELECT	a.nr_seq_indicador,
		b.ds_indicador,
		CASE WHEN c.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN 000 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-1)*-1) END ,0) THEN a.vl_indicador  ELSE 0 END  vl_mes_01,
		CASE WHEN c.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -01 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-2)*-1) END ,0) THEN a.vl_indicador  ELSE 0 END  vl_mes_02,
		CASE WHEN c.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -02 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-3)*-1) END ,0) THEN a.vl_indicador  ELSE 0 END  vl_mes_03,
		CASE WHEN c.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -03 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-4)*-1) END ,0) THEN a.vl_indicador  ELSE 0 END  vl_mes_04,
		CASE WHEN c.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -04 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-5)*-1) END ,0) THEN a.vl_indicador  ELSE 0 END  vl_mes_05,
		CASE WHEN c.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -05 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-6)*-1) END ,0) THEN a.vl_indicador  ELSE 0 END  vl_mes_06,
		CASE WHEN c.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -06 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-7)*-1) END ,0) THEN a.vl_indicador  ELSE 0 END  vl_mes_07,
		CASE WHEN c.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -07 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-8)*-1) END ,0) THEN a.vl_indicador  ELSE 0 END  vl_mes_08,
		CASE WHEN c.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -08 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-9)*-1) END ,0) THEN a.vl_indicador  ELSE 0 END  vl_mes_09,
		CASE WHEN c.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -09 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-10)*-1) END ,0) THEN a.vl_indicador  ELSE 0 END  vl_mes_10,
		CASE WHEN c.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -10 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-11)*-1) END ,0) THEN a.vl_indicador  ELSE 0 END  vl_mes_11,
		CASE WHEN c.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -11 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-12)*-1) END ,0) THEN a.vl_indicador  ELSE 0 END  vl_mes_12
	from	centro_controle e,
		centro_controle_indicador d,
		resultado_ind_cen_cont a,
		resultado_indicador b,
		tabela_custo c
	where 	c.dt_mes_referencia		between dt_inicial_w and dt_final_w
...
SELECT	4 ie_tipo_indicador,
	WHEB_MENSAGEM_PCK.get_texto(299228) ds_tipo_indicador,
	y.nr_seq_indicador,
	y.ds_indicador,
	coalesce(sum(y.vl_mes_12),0) vl_mes_12,
	coalesce(sum(y.vl_mes_11),0) vl_mes_11,
...
from 	(
	SELECT	a.nr_seq_indicador,
		b.ds_indicador,
		CASE WHEN z.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN 000 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-1)*-1) END ,0) THEN a.qt_indicador  ELSE 0 END  vl_mes_01,
		CASE WHEN z.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -01 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-2)*-1) END ,0) THEN a.qt_indicador  ELSE 0 END  vl_mes_02,
		CASE WHEN z.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -02 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-3)*-1) END ,0) THEN a.qt_indicador  ELSE 0 END  vl_mes_03,
		CASE WHEN z.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -03 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-4)*-1) END ,0) THEN a.qt_indicador  ELSE 0 END  vl_mes_04,
		CASE WHEN z.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -04 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-5)*-1) END ,0) THEN a.qt_indicador  ELSE 0 END  vl_mes_05,
		CASE WHEN z.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -05 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-6)*-1) END ,0) THEN a.qt_indicador  ELSE 0 END  vl_mes_06,
		CASE WHEN z.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -06 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-7)*-1) END ,0) THEN a.qt_indicador  ELSE 0 END  vl_mes_07,
		CASE WHEN z.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -07 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-8)*-1) END ,0) THEN a.qt_indicador  ELSE 0 END  vl_mes_08,
		CASE WHEN z.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -08 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-9)*-1) END ,0) THEN a.qt_indicador  ELSE 0 END  vl_mes_09,
		CASE WHEN z.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -09 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-10)*-1) END ,0) THEN a.qt_indicador  ELSE 0 END  vl_mes_10,
		CASE WHEN z.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -10 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-11)*-1) END ,0) THEN a.qt_indicador  ELSE 0 END  vl_mes_11,
		CASE WHEN z.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -11 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-12)*-1) END ,0) THEN a.qt_indicador  ELSE 0 END  vl_mes_12
	from 	resultado_ind_cen_cont a,
     		resultado_indicador b,
     		tabela_custo z
	where 	a.cd_centro_controle     	= cd_centro_controle_w
	and 	a.nr_seq_indicador		= b.nr_sequencia
	...
	and 	z.dt_mes_referencia		between dt_inicial_w and dt_final_w
  	...
  	and	a.qt_indicador <> 0
	and     exists (select	1
			from	tabela_custo_acesso_v x
			where	x.cd_empresa = z.cd_empresa
			and	x.cd_estabelecimento = a.cd_estabelecimento
...
SELECT 5 ie_tipo_indicador,
	WHEB_MENSAGEM_PCK.get_texto(299229) ds_tipo_indicador,
       	y.nr_seq_indicador,
       	y.ds_indicador,
	coalesce(sum(y.vl_mes_12),0) vl_mes_12,
	coalesce(sum(y.vl_mes_11),0) vl_mes_11,
...
from	(
	SELECT	c.cd_centro_controle nr_seq_indicador,
		...
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN 000 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-1)*-1) END ,0) THEN a.vl_taxa_custo  ELSE 0 END  	vl_mes_01,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -01 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-2)*-1) END ,0) THEN a.vl_taxa_custo  ELSE 0 END  	vl_mes_02,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -02 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-3)*-1) END ,0) THEN a.vl_taxa_custo  ELSE 0 END  	vl_mes_03,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -03 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-4)*-1) END ,0) THEN a.vl_taxa_custo  ELSE 0 END  	vl_mes_04,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -04 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-5)*-1) END ,0) THEN a.vl_taxa_custo  ELSE 0 END  	vl_mes_05,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -05 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-6)*-1) END ,0) THEN a.vl_taxa_custo  ELSE 0 END  	vl_mes_06,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -06 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-7)*-1) END ,0) THEN a.vl_taxa_custo  ELSE 0 END  	vl_mes_07,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -07 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-8)*-1) END ,0) THEN a.vl_taxa_custo  ELSE 0 END  	vl_mes_08,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -08 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-9)*-1) END ,0) THEN a.vl_taxa_custo  ELSE 0 END  	vl_mes_09,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -09 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-10)*-1) END ,0) THEN a.vl_taxa_custo  ELSE 0 END  	vl_mes_10,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -10 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-11)*-1) END ,0) THEN a.vl_taxa_custo  ELSE 0 END  	vl_mes_11,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -11 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-12)*-1) END ,0) THEN a.vl_taxa_custo  ELSE 0 END  	vl_mes_12,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN 000 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-1)*-1) END ,0) THEN a.vl_custo  ELSE 0 END  		vl_custo_01,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -01 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-2)*-1) END ,0) THEN a.vl_custo  ELSE 0 END  		vl_custo_02,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -02 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-3)*-1) END ,0) THEN a.vl_custo  ELSE 0 END  		vl_custo_03,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -03 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-4)*-1) END ,0) THEN a.vl_custo  ELSE 0 END  		vl_custo_04,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -04 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-5)*-1) END ,0) THEN a.vl_custo  ELSE 0 END  		vl_custo_05,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -05 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-6)*-1) END ,0) THEN a.vl_custo  ELSE 0 END  		vl_custo_06,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -06 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-7)*-1) END ,0) THEN a.vl_custo  ELSE 0 END  		vl_custo_07,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -07 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-8)*-1) END ,0) THEN a.vl_custo  ELSE 0 END  		vl_custo_08,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -08 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-9)*-1) END ,0) THEN a.vl_custo  ELSE 0 END  		vl_custo_09,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -09 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-10)*-1) END ,0) THEN a.vl_custo  ELSE 0 END  		vl_custo_10,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -10 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-11)*-1) END ,0) THEN a.vl_custo  ELSE 0 END  		vl_custo_11,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -11 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-12)*-1) END ,0) THEN a.vl_custo  ELSE 0 END  		vl_custo_12,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN 000 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-1)*-1) END ,0) THEN a.qt_disponibilidade  ELSE 0 END  	qt_disp_01,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -01 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-2)*-1) END ,0) THEN a.qt_disponibilidade  ELSE 0 END  	qt_disp_02,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -02 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-3)*-1) END ,0) THEN a.qt_disponibilidade  ELSE 0 END  	qt_disp_03,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -03 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-4)*-1) END ,0) THEN a.qt_disponibilidade  ELSE 0 END  	qt_disp_04,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -04 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-5)*-1) END ,0) THEN a.qt_disponibilidade  ELSE 0 END  	qt_disp_05,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -05 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-6)*-1) END ,0) THEN a.qt_disponibilidade  ELSE 0 END  	qt_disp_06,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -06 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-7)*-1) END ,0) THEN a.qt_disponibilidade  ELSE 0 END  	qt_disp_07,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -07 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-8)*-1) END ,0) THEN a.qt_disponibilidade  ELSE 0 END  	qt_disp_08,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -08 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-9)*-1) END ,0) THEN a.qt_disponibilidade  ELSE 0 END  	qt_disp_09,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -09 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-10)*-1) END ,0) THEN a.qt_disponibilidade  ELSE 0 END  	qt_disp_10,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -10 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-11)*-1) END ,0) THEN a.qt_disponibilidade  ELSE 0 END  	qt_disp_11,
		CASE WHEN b.dt_mes_referencia=PKG_DATE_UTILS.ADD_MONTH(dt_final_w,CASE WHEN ie_Ordem_inversa_p='N' THEN -11 WHEN ie_Ordem_inversa_p='S' THEN ((nr_meses_p-12)*-1) END ,0) THEN a.qt_disponibilidade  ELSE 0 END 	qt_disp_12,
		a.vl_custo,
...
c09 CURSOR FOR
SELECT	CASE WHEN cd_estab_w=0 THEN 0  ELSE cd_estabelecimento END  cd_estabelecimento,
	99999999 cd_centro_controle,
	substr(WHEB_MENSAGEM_PCK.get_texto(299243)|| ' ' || CASE WHEN cd_estab_p=0 THEN obter_nome_empresa(cd_empresa_w)  ELSE Obter_nome_fantasia_estab(cd_estab_w) END ,1,255) ds_centro_controle,
	ie_classif_conta,
	ds_classificacao,
	ds_natureza_gasto,
	ds_tipo_indicador,
...
group by CASE WHEN cd_estab_w=0 THEN 0  ELSE cd_estabelecimento END ,
	ie_classif_conta,
	ds_classificacao,
	ds_natureza_gasto,
	ds_tipo_indicador,
...
if	((coalesce(cd_empresa_param_p::text, '') = '') or (cd_empresa_param_p = '')) then
	cd_empresa_w := cd_empresa_p;
	else
	cd_empresa_w := cd_empresa_param_p;
end if;

if (position('-' in coalesce(ie_tipo_centro_controle_p,'X')) > 0) or (position('NOT' in coalesce(upper(ie_tipo_centro_controle_p),'X')) > 0) then
	filtros_not_w[0].ds_filtros	:= 'IE_TIPO_CENTRO_CONTROLE_P';
	else
	filtros_not_w[0].ds_filtros := 'X';
end if;

if (position('-' in coalesce(cd_centro_controle_p,'X')) > 0) or (position('NOT' in coalesce(upper(cd_centro_controle_p),'X')) > 0) then
	filtros_not_w[2].ds_filtros	:= 'CD_CENTRO_CONTROLE_P';
	else
	filtros_not_w[2].ds_filtros := 'X';
end if;
...
dt_inicial_w		:= PKG_DATE_UTILS.ADD_MONTH(dt_mes_referencia_w,-(nr_meses_p -1),0);
...
ds_tipo_centro_w	:= ie_tipo_centro_controle_p;
ds_centro_controle_w	:= cd_centro_controle_p;
ds_filtro_gng_w		:= nr_seq_gng_p;
ds_filtro_ng_w		:= nr_seq_ng_p;

/* Filtro da grupo de natureza de gasto */

if (coalesce(ds_filtro_gng_w,'0') = '0') then
	begin
	ds_filtro_gng_w		:= '0';
	ie_result_possib_gng_w	:= 'N';
	end;
else
	begin
	ie_result_possib_gng_w	:= 'S';
	if (position('-' in coalesce(ds_filtro_gng_w,'X')) + position(' NOT ' in coalesce(upper(ds_filtro_gng_w),'X')) > 0) then
		begin
		ie_result_possib_gng_w	:= 'N';
		end;
	end if;
...
/* Filtro da natureza de gasto */

if (coalesce(ds_filtro_ng_w,'0') = '0') then
	begin
	ds_filtro_ng_w		:= '0';
	ie_result_possib_ng_w	:= 'N';
	end;
else
	begin
	ie_result_possib_ng_w	:= 'S';
	if (position('-' in coalesce(ds_filtro_ng_w,'X')) + position(' NOT ' in coalesce(upper(ds_filtro_ng_w),'X')) > 0) then
		begin
		ie_result_possib_ng_w	:= 'N';
		end;
	end if;
...
if (ie_tipo_centro_controle_p IS NOT NULL AND ie_tipo_centro_controle_p::text <> '') then

	ie_pos_w			:= position('(' in ds_tipo_centro_w);
	ds_tipo_centro_w	:= substr(ds_tipo_centro_w, ie_pos_w+1,2000);
	ie_pos_w			:= position(')' in ds_tipo_centro_w);
	ds_tipo_centro_w	:= substr(ds_tipo_centro_w, 1, ie_pos_w -1);
	if (coalesce(ds_tipo_centro_w::text, '') = '') then
		ds_tipo_centro_w	:=	ie_tipo_centro_controle_p;
	end if;
...
EXIT WHEN NOT FOUND; /* apply on C00 */
	begin
	ie_somente_centro_usuario_w	:= substr(coalesce(obter_valor_param_usuario(927, 3, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_w),'N'),1,1);
	i	:= 0;
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		if (coalesce(ie_tipo_centro_controle_p,'X') <> 'X') and (coalesce(ie_tipo_centro_controle_p,'X') <> '0') and (ie_gerar_w = 'S') and (filtros_not_w[0].ds_filtros = 'X') then
			ie_gerar_w	:= obter_se_contido(somente_numero(vet01.ie_tipo_centro_controle), ds_tipo_centro_w);
		end if;

		if (coalesce(ie_tipo_centro_controle_p,'X') <> 'X') and (coalesce(ie_tipo_centro_controle_p,'X') <> '0') and (ie_gerar_w = 'S')and (filtros_not_w[0].ds_filtros = 'IE_TIPO_CENTRO_CONTROLE_P')	then

			select coalesce(max('N'),'S')
			into STRICT	ie_gerar_w
			
			where	obter_se_contido(somente_numero(vet01.ie_tipo_centro_controle), ds_tipo_centro_w) = 'S';
		...
		if (coalesce(cd_centro_controle_p,'X') <> 'X') and (ie_gerar_w = 'S')  then
			...
			if (cd_centro_controle_p = '99999999') then
				ie_gerar_w	:= 'S';
			elsif (filtros_not_w[2].ds_filtros = 'X') and (cd_centro_controle_p <> '99999999') then
				ie_gerar_w	:= obter_se_contido(vet01.cd_centro_controle, ds_centro_controle_w);
			end if;

			if (cd_centro_controle_p = '99999999') then
				ie_gerar_w	:= 'S';
			elsif (filtros_not_w[2].ds_filtros = 'CD_CENTRO_CONTROLE_P') and (cd_centro_controle_p <> '99999999') then
		...
		if (nr_seq_unid_neg_w <> 0) and (ie_gerar_w = 'S')  then
			ie_gerar_w	:= substr(cus_obter_se_centro_unid_neg(cd_estabelecimento_w, vet01.cd_centro_controle, nr_seq_unid_neg_w),1,1);
		end if;
	...
	for i in 1..centro_w.count loop
		...
		EXIT WHEN NOT FOUND; /* apply on C02 */
			...
			if (ie_gerar_w = 'S') then
				...
				if (vet02.ie_emite	= 'G') then
					...
					if (vet02.ie_emite = 'C') then
						...
						EXIT WHEN NOT FOUND; /* apply on C05 */
							...
							valores_adic_w[nr_seq_w].nr_sequencia			:= nr_seq_w;
							valores_adic_w[nr_seq_w].cd_centro_controle_orig	:= vet05.cd_centro_controle;
							valores_adic_w[nr_seq_w].cd_centro_controle_dest	:= centro_w[i].cd_centro_controle;
				...
				/* Acumular a Classificação se ela for a receita Liquida, para o cálculo do percentual */
				...
				if (vet02.ie_emite_perc = 'S') then
					...
					valores_w[nr_seq_w].ie_classif_conta		:= vet02.cd_classificacao;
					valores_w[nr_seq_w].ie_emite			:= vet02.ie_emite;
					valores_w[nr_seq_w].ds_natureza_gasto		:= WHEB_MENSAGEM_PCK.get_texto(299231); /* % Sobre Receita Líquida */
		...
		close C02; /* Fim Classificações DRE*/
...
for nr_seq_w in 1..valores_w.count loop
	...
	if (ie_gerar_w = 'S') then
		begin
		vl_orcado_w := 0;
		if (valores_w[nr_seq_w].ie_emite = 'G') and (valores_w[nr_seq_w]coalesce(.ie_tipo_indicador::text, '') = '') then
			begin
			select 	coalesce(sum(a.vl_mes),0) vl_mes
			into STRICT	vl_orcado_w
			from 	resultado_centro_controle a
			where	a.cd_estabelecimento	= valores_w[nr_seq_w].cd_estabelecimento
			and	a.cd_centro_controle	= valores_w[nr_seq_w].cd_centro_controle
			and	a.ie_classif_conta	= valores_w[nr_seq_w].ie_classif_conta
			and	a.nr_seq_ng		= coalesce(valores_w[nr_seq_w].nr_seq_ng,a.nr_seq_ng)
			and	a.nr_seq_tabela		= nr_seq_tabela_orc_p;
			end;
		elsif (valores_w[nr_seq_w].ie_emite = 'C') and (valores_w[nr_seq_w]coalesce(.ie_tipo_indicador::text, '') = '') then
			begin

			begin
			select	sum(a.vl_distribuido) vl_mes
			into STRICT	vl_orcado_w
			from	tabela_parametro e,
				centro_controle d,
				centro_controle c,
				orcamento_distribuido a
			where	1 = 1
			and	a.cd_estabelecimento		= coalesce(d.cd_estabelecimento, a.cd_estabelecimento)
			and	a.cd_centro_controle		= d.cd_centro_controle
			and	a.cd_centro_controle		= valores_adic_w[nr_seq_w].cd_centro_controle_orig
			and	d.cd_classif_result		= valores_w[nr_seq_w].ie_classif_conta
			and	a.cd_centro_controle_dest	= c.cd_centro_controle
			and	a.cd_centro_controle_dest	= valores_adic_w[nr_seq_w].cd_centro_controle_dest
			and	e.nr_seq_tabela			= nr_seq_tabela_orc_p
			and	a.nr_seq_tabela			= e.nr_seq_tabela_param
			...
			when others then
				...
				and	d.cd_classif_result		= valores_w[nr_seq_w].ie_classif_conta
				and	a.cd_centro_controle_dest	= c.cd_centro_controle
				and	a.cd_centro_controle_dest	= valores_w[nr_seq_w].cd_centro_controle
				and	e.nr_seq_tabela			= nr_seq_tabela_orc_p
				and	a.nr_seq_tabela			= e.nr_seq_tabela_param
				and	nr_seq_parametro		= 1;
				exception
		...
		elsif (valores_w[nr_seq_w].ie_emite = 'S') and (valores_w[nr_seq_w]coalesce(.ie_tipo_indicador::text, '') = '') then
			begin
...
if (ie_totalizar_centro_p = 'S') then
	...
	EXIT WHEN NOT FOUND; /* apply on C09 */
		...
		if (coalesce(vet09.ie_classif_conta,0) <> 0) then
			...
			if (ie_receita_liquida_w = 'S') and (vet09.ds_classificacao IS NOT NULL AND vet09.ds_classificacao::text <> '') then
				begin
			...
			if (ie_emite_perc_w = 'S') and (coalesce(vet09.ds_natureza_gasto::text, '') = '') and (coalesce(vet09.ds_tipo_indicador::text, '') = '') and (vet09.ds_classificacao IS NOT NULL AND vet09.ds_classificacao::text <> '') then
				...
				valores_w[nr_seq_classif_w].vl_total		:= vet09.vl_total;
				valores_w[nr_seq_classif_w].vl_media		:= vet09.vl_media;
				...
				vet09.vl_total			:= 	coalesce(rec_liquida_w[01].vl_mes,0) +
									coalesce(rec_liquida_w[02].vl_mes,0) +
									coalesce(rec_liquida_w[03].vl_mes,0) +
									coalesce(rec_liquida_w[04].vl_mes,0) +
									coalesce(rec_liquida_w[05].vl_mes,0) +
				...
				vet09.vl_media			:= dividir(vet09.vl_total, nr_meses_p);

				vet09.vl_total			:= dividir_sem_round(valores_w[nr_seq_classif_w].vl_total, vet09.vl_total) * 100;

				vet09.vl_media			:= dividir_sem_round(valores_w[nr_seq_classif_w].vl_media, vet09.vl_media) * 100;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_w_res_centro_holding ( cd_empresa_p bigint, cd_empresa_param_p bigint, cd_estab_p bigint, cd_centro_controle_p text, nr_meses_p bigint, cd_tabela_custo_p bigint, nr_seq_tabela_p bigint, ie_relatorio_p text, ie_centro_resultado_p text, ie_centro_superior_p text, cd_centro_superior_p text, nr_seq_ng_p text, cd_classificacao_p text, ie_tipo_relat_ng_p text, nr_seq_gng_p text, ie_tipo_centro_controle_p text, ie_tipo_gasto_p text, ie_totalizar_centro_p text, ie_zero_p text, ie_result_orc_p text, ie_somente_ativo_p text, ie_centro_atual_p text, ie_ordem_inversa_p text, nm_usuario_p text, nr_seq_unid_neg_p bigint, ie_somente_classif_ativa_p text, ie_resultado_orcado_p text, NR_SEQ_TABELA_ORC_p bigint) FROM PUBLIC;
