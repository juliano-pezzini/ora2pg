...
CREATE OR REPLACE PROCEDURE obter_proc_tab_interno_autor (nr_seq_interno_p bigint, nr_prescricao_p bigint, nr_seq_autor_p bigint, nr_atendimento_p bigint, nr_interno_conta_p bigint, ds_procedimento_p text, cd_procedimento_p INOUT bigint, ie_origem_proced_p INOUT bigint, ds_proc_p INOUT text, cd_setor_item_p bigint default null, dt_conta_p timestamp default null, cd_convenio_p bigint default null) AS $body$
DECLARE
...
nr_seq_proc_interno_w		autorizacao_convenio.nr_seq_proc_interno%type;
...
/*Este cursor deve ser igual ao C01, porém para verificar se existem as regras para as prioridades do C03*/
					...
					if (ie_encontrou_edicao_w = 'N') then 
						...
						if (coalesce(cd_proc_prior_w,0) > 0) then 
							...
							if (ie_origem_proced_edicao_w = 5) then 
								...
								and	trunc(coalesce(dt_conta_p,clock_timestamp()),'dd') between coalesce(dt_inicio_vigencia,clock_timestamp() - interval '3650 days') and coalesce(dt_final_vigencia, coalesce(dt_conta_p,clock_timestamp()));
							end if;
...
if (nr_seq_autor_p > 0) then 
	update 	autorizacao_convenio 
	set	nr_seq_proc_interno = nr_seq_interno_p 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE obter_proc_tab_interno_autor (nr_seq_interno_p bigint, nr_prescricao_p bigint, nr_seq_autor_p bigint, nr_atendimento_p bigint, nr_interno_conta_p bigint, ds_procedimento_p text, cd_procedimento_p INOUT bigint, ie_origem_proced_p INOUT bigint, ds_proc_p INOUT text, cd_setor_item_p bigint default null, dt_conta_p timestamp default null, cd_convenio_p bigint default null) FROM PUBLIC;
