...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_0450_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
fetch c_reg_0450 bulk collect into vetReg0450 limit 1000;
	...
	ds_linha_w := substr(ds_sep_w || vetReg0450[i].cd_reg ||
			     ds_sep_w || vetReg0450[i].cd_inf ||
			     ds_sep_w || vetReg0450[i].ds_txt || ds_sep_w,
			     1,
			     8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro 		:=  vetReg0450[i].cd_reg;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_0450_icmsipi ( nr_seq_controle_p bigint, nr_linha_p INOUT bigint ) FROM PUBLIC;
