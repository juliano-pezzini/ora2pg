...
CREATE OR REPLACE PROCEDURE dmed_anual_arquivo_dtop ( nr_sequencia_p bigint, cd_pessoa_titular_p bigint, nm_usuario_p text, nr_cpf_titular_p text, nr_linha_p INOUT bigint) AS $body$
DECLARE
...
c_dependente CURSOR FOR
	...
	from	(SELECT	p.cd_pessoa_fisica cd_pessoa_beneficiario, 
		...
		order by
			...
			to_char(p.dt_nascimento,'yyyymmdd') asc) alias9;
	...
	for z in 1..s_array.count loop
		...
		if (nr_cpf_titular_p = nr_cpf_beneficiario_w) and (pkg_date_utils.add_month(dt_nascimento_w, 18 * 12,0) < clock_timestamp()) then
			...
			select	sum(t.vl_pago) vl_pago_beneficiario,
				max(dmed_obter_grau_parente(t.nr_documento, t.cd_pessoa_beneficiario))
			...
			if (vl_pago_w > 0) then
				...
				ds_arquivo_w 	:= 'DTOP' || '|' || nr_cpf_beneficiario_w || '|' || dt_nascimento_arquivo_w || '|' || nm_beneficiario_w || '|' || ie_dependencia_w ||
							'|' || vl_pago_w || '|';
                        else
				ds_arquivo_w 	:= 'DTOP' || '|' || nr_cpf_beneficiario_w || '|' || dt_nascimento_arquivo_w || '|' || nm_beneficiario_w || '|' || ie_dependencia_w ||
							'|' || '' || '|';
			...
			from	pls_segurado	a
			where	a.cd_pessoa_fisica	= cd_pessoa_beneficiario_w;
			
			select count(*)
			into STRICT   qtd_w
			from (SELECT	1 order_union,
			...
			and	t.cd_pessoa_beneficiario in (select	f.cd_pessoa_fisica
								...
								where	coalesce(f.nr_cpf,'0')	= coalesce(nr_cpf_beneficiario_w,coalesce(f.nr_cpf,'0'))
								and	f.cd_pessoa_fisica = cd_pessoa_beneficiario_w)
			group by
				p.nr_cpf,
				p.nm_pessoa_fisica
...
union
			...
			order by
				order_union,
				cpf_cnpj_benef) alias19;
				
			if ((vl_pago_w > 0) or (coalesce(qtd_w,0) > 0)) then	
			...
			nr_linha_w := dmed_anual_arquivo_rdtop(	nr_sequencia_p, cd_pessoa_beneficiario_w, nm_usuario_p, nr_cpf_beneficiario_w, nr_linha_w);
		end if;
		end;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dmed_anual_arquivo_dtop ( nr_sequencia_p bigint, cd_pessoa_titular_p bigint, nm_usuario_p text, nr_cpf_titular_p text, nr_linha_p INOUT bigint) FROM PUBLIC;
