...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_k280_icmsipi ( nr_seq_controle_p bigint, nr_seq_superior_p bigint, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
type reg_c_reg_K280 is table of fis_efd_icmsipi_K280%RowType;
...
fetch c_reg_K280 bulk collect into vetRegK280 limit 1000;
	...
	ds_linha_w := substr(ds_sep_w || vetRegK280[i].cd_reg ||
			     ds_sep_w || to_char(vetRegK280[i].dt_est ,'ddmmyyyy') ||
			     ds_sep_w || vetRegK280[i].cd_item ||
			     ds_sep_w || substr(sped_obter_campo_valor(vetRegK280[i].qt_cor_pos ), 1, 30) ||
			     ds_sep_w || substr(sped_obter_campo_valor(vetRegK280[i].qt_cor_neg ), 1, 30) ||
			     ds_sep_w || vetRegK280[i].cd_ind_est ||
			     ds_sep_w || vetRegK280[i].cd_part || ds_sep_w,
			     1,
			     8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro		:=  vetRegK280[i].cd_reg;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_k280_icmsipi ( nr_seq_controle_p bigint, nr_seq_superior_p bigint, nr_linha_p INOUT bigint ) FROM PUBLIC;
