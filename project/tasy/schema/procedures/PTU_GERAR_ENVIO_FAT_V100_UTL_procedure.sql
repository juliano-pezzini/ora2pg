...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_fat_v100_utl ( nr_seq_fatura_p ptu_fatura.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, ds_local_windows_utl_p INOUT text ) AS $body$
DECLARE
...
C01 CURSOR FOR
	SELECT	lpad(coalesce(elimina_caractere_especial(to_char(nr_lote)),'0'),8,'0'),
		...
		rpad(coalesce(nm_prest_exec,' '),70,' '),
		coalesce(id_aviso,' '),
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_fat_v100_utl ( nr_seq_fatura_p ptu_fatura.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, ds_local_windows_utl_p INOUT text ) FROM PUBLIC;
