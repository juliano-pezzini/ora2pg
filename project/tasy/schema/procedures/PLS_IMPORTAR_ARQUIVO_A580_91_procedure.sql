...
CREATE OR REPLACE PROCEDURE pls_importar_arquivo_a580_91 ( ds_conteudo_p text, nr_seq_fat_geral_p ptu_fatura_geral.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, nr_seq_regra_tit_fat_p pls_regra_tit_fat_geral.nr_sequencia%type) AS $body$
DECLARE
...
if (substr(ds_conteudo_p,9,3) = '581') then
	...
	id_cobranca_w	:= trim(both substr(ds_conteudo_p,130,1));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_importar_arquivo_a580_91 ( ds_conteudo_p text, nr_seq_fat_geral_p ptu_fatura_geral.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, nr_seq_regra_tit_fat_p pls_regra_tit_fat_geral.nr_sequencia%type) FROM PUBLIC;
