...
CREATE OR REPLACE PROCEDURE pls_lib_interface_a1200_v70 () AS $body$
BEGIN
...
values (2750, 'A1200 - Pacotes e Tabelas Contratualizadas - 7.0', 'C:\', clock_timestamp(), 'wcbernardino', 'N', 'PTU7.0', 'select ds_conteudo from w_ptu_envio_arq where nm_usuario = :nm_usuario_cor order by nr_seq_apres', null, '', '', '');
exception
when others then
	null;
end;
...
values (2750, 1, 'Conteúdo', 'N', 'V', clock_timestamp(), 'wcbernardino', '1', '');
exception
when others then
	null;
end;
...
values (2750, 1, 1, '', 'DS_CONTEUDO', 'VARCHAR2', 255, clock_timestamp(), 'wcbernardino', null, '', '', null, 'S', '', 'N', 'S', 'N', 'S', '');
exception
when others then
	null;
end;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_lib_interface_a1200_v70 () FROM PUBLIC;
