...
CREATE OR REPLACE PROCEDURE gerar_tributo_titulo ( nr_titulo_p bigint, nm_usuario_p text, ie_baixa_titulo_p text, nr_bordero_p bigint, dt_baixa_bordero_p timestamp, nr_seq_escrit_p bigint, dt_baixa_escrit_p timestamp, dt_baixa_p timestamp, cd_estabelecimento_p bigint, vl_pago_tit_p bigint) AS $body$
DECLARE
...
ie_valor_base_titulo_nf_w	varchar(1);
vl_base_trib_nf_w		double precision;
...
ie_gerar_trib_unif_w	varchar(1) := 'N';
ie_permite_trib_tit_adiant_w	varchar(1);
...
c01 CURSOR FOR
	SELECT	a.cd_tributo,
		...
		coalesce(a.ie_valor_base_titulo_nf, 'N'),
		coalesce(a.ie_data_irpf,'A')
	from	tributo a
	...
	and	( (ie_tipo_titulo_w		not in ('4','5','27')) or
	      ((coalesce(ie_permite_trib_tit_adiant_w,'N') = 'S') and (ie_tipo_titulo_w		not in ('4','27')))
		)
...
/* francisco - os80633 - 25/02/2008 */
	...
	if (qt_registros_w > 0) then
	    ...
	    CALL wheb_mensagem_pck.exibir_mensagem_abort(352356);
...
EXIT WHEN NOT FOUND; /* apply on c02 */
	...
	and coalesce(a.ds_emp_retencao,'X') = coalesce(ds_emp_retencao_w,'X')
	and	PKG_DATE_UTILS.start_of(a.dt_imposto,'month',0)	= PKG_DATE_UTILS.start_of(CASE WHEN ie_tipo_data_w='E' THEN dt_emissao_w WHEN ie_tipo_data_w='V' THEN dt_venc_titulo_w END ,'month',0);
	...
	if (qt_registro_w = 0) then
		...
		values (	nr_seq_imposto_w,
			...
			clock_timestamp(),
			nm_usuario_p,
			PKG_DATE_UTILS.start_of(CASE WHEN ie_tipo_data_w='E' THEN dt_emissao_w WHEN ie_tipo_data_w='V' THEN dt_venc_titulo_w END ,'dd',0),
			vl_base_tributo_pf_w,
...
ie_permite_trib_tit_adiant_w := obter_param_usuario(851, 260, obter_perfil_ativo, nm_usuario_p, cd_estabelecimento_w, ie_permite_trib_tit_adiant_w);
	...
	if (coalesce(pr_aliquota_w,0) > 0) then
		...
		if (ie_tipo_tributo_w = 'IR') and (cd_pf_titulo_w IS NOT NULL AND cd_pf_titulo_w::text <> '') then
			...
			if (coalesce(nr_seq_nota_fiscal_w,0) = 0) then
				...
				and	a.nr_titulo		= nr_titulo_p
				and	a.ie_pago_prev		= 'V'
				and (b.ie_baixa_titulo	= 'N' or (coalesce(b.ie_saldo_tit_pagar,'S') = 'N' or ie_trib_atualiza_saldo_w = 'N'));
				...
				vl_base_tributo_w	:= (obter_dados_tit_pagar(nr_titulo_p,'V'))::numeric  - vl_inss_w;
			end if;
			...
			ie_irpf_w	:= 'S';
			if (coalesce(nr_seq_nota_fiscal_w,0) = 0) then
				vl_base_tributo_w	:= (obter_dados_tit_pagar(nr_titulo_p,'V'))::numeric;
			end if;
		else
			vl_base_tributo_w	:= vl_titulo_original_w;
			if ( coalesce(PR_REDUCAO_BASE_W,0) > 0 ) then /*AAMFIRMO OS 560487 18/04/2013 */
				vl_base_tributo_w := ( vl_base_tributo_w - ( (PR_REDUCAO_BASE_W/100) * vl_base_tributo_w ) );
			end if;
		end if;
		...
		if (ie_baixa_titulo_w = 'S') then
			...
			if (vl_saldo_titulo_w + vl_adiantamento_w = vl_base_tributo_w) then
				...
				vl_desdobrado_w		:= 0;
				if (nr_titulo_original_w > 0) and (ie_acumular_trib_liq_w = 'N') then	-- edgar 06/05/2009, os 138996, so somar o valor desdobrado caso esteja parametrizado para nao acumular somente na ultima baixa
			...
			if (coalesce(ie_valor_base_titulo_nf_w,'N') = 'S') and (coalesce(nr_seq_nota_fiscal_w,0) > 0) then

				select	max(vl_base_calculo)
				into STRICT	vl_base_trib_nf_w
				from	nota_fiscal_trib
				where	nr_sequencia	= nr_seq_nota_fiscal_w;

				if (coalesce(vl_base_trib_nf_w,0) <> 0) then
					vl_base_tributo_w	:= vl_base_trib_nf_w;
				end if;
			elsif (coalesce(ie_valor_base_titulo_nf_w,'N') = 'M') and (coalesce(nr_seq_nota_fiscal_w,0) > 0) then
				begin
			...
			elsif (coalesce(ie_valor_base_titulo_nf_w,'N') = 'V') and (coalesce(nr_seq_nota_fiscal_w,0) > 0) then
			...
			and (a.nr_seq_escrit = nr_seq_escrit_p or
				PKG_DATE_UTILS.start_of(CASE WHEN ie_vencimento_w='V' THEN  b.dt_vencimento_atual WHEN ie_vencimento_w='C' THEN  coalesce(b.dt_contabil, b.dt_emissao) WHEN ie_vencimento_w='B' THEN  dt_venc_titulo_w WHEN ie_vencimento_w='S' THEN  CASE WHEN coalesce(nr_seq_escrit_p::text, '') = '' THEN CASE WHEN coalesce(nr_bordero_p::text, '') = '' THEN b.dt_vencimento_atual  ELSE dt_venc_titulo_w END   ELSE dt_venc_titulo_w END   ELSE b.dt_emissao END , 'month', 0) 	= PKG_DATE_UTILS.start_of(dt_venc_tributo_w, 'month', 0))
		...
		AND ((b.nr_titulo		= nr_titulo_p  AND coalesce(vl_base_calculo,0) <> 0)
			OR (a.nr_titulo			<> nr_titulo_p
			AND	((coalesce(b.cd_cgc,0)		= coalesce(cd_cgc_titulo_w,0)) and
			 	coalesce(b.cd_pessoa_fisica,0) 	= coalesce(cd_pf_titulo_w,0))
			AND	PKG_DATE_UTILS.start_of(a.dt_imposto,'month',0)	= PKG_DATE_UTILS.start_of(CASE WHEN ie_tipo_data_w='E' THEN dt_emissao_w WHEN ie_tipo_data_w='V' THEN dt_venc_titulo_w END ,'month',0)));


		if (coalesce(ie_data_irpf_w,'A')	= 'B') then

			dt_competencia_w	:= coalesce(dt_baixa_p,clock_timestamp());
		...
		if (ie_tipo_tributo_w = 'INSS') and (vl_base_retido_outro_w > 0) and (vl_total_base_w > vl_base_retido_outro_w )then

			select	coalesce(sum(b.vl_titulo),0) vl_total_base
			into STRICT	vl_total_base_w
			FROM tributo c, titulo_pagar_imposto a, titulo_pagar b
LEFT OUTER JOIN pessoa_juridica e ON (b.cd_cgc = e.cd_cgc)
WHERE a.nr_titulo		= b.nr_titulo and b.ie_situacao	in ('A','L') and a.ie_pago_prev	= 'V' and a.cd_tributo		= c.cd_tributo and coalesce(c.ie_baixa_titulo, 'N')	= 'N'  --
  and OBTER_EMPRESA_ESTAB(b.cd_estabelecimento) = cd_empresa_w and a.cd_tributo			= cd_tributo_w and (
					(
						(b.cd_cgc		= cd_cgc_titulo_w) or
						((e.cd_cnpj_raiz		= cd_cnpj_raiz_w) and (ie_cnpj_w = WHEB_MENSAGEM_PCK.get_texto(803917)))
					)
					or (b.cd_pessoa_fisica		= cd_pf_titulo_w)
				) and CASE WHEN c.ie_vencimento='V' THEN  b.dt_vencimento_atual WHEN c.ie_vencimento='C' THEN  coalesce(b.dt_contabil, b.dt_emissao)  ELSE b.dt_emissao END  between dt_venc_inicio_w and dt_venc_fim_w and ((ie_restringe_estab_w	= 'N') or (b.cd_estabelecimento	= cd_estabelecimento_w) or (b.cd_estab_financeiro	= cd_estabelecimento_w)) and coalesce(c.ie_baixa_titulo, 'N')	= ie_baixa_titulo_w and (
					(
						(ie_apuracao_piso_w = 'N') or (ie_apuracao_piso_w = substr(OBTER_SE_BASE_CALCULO(a.nr_titulo, 'TP'),1,1))
					)
					or
					(
						(ie_baixa_titulo_w	= 'S') and (coalesce(c.ie_baixa_titulo, 'N') = 'S')
					)
				);
		...
		if (ie_baixa_titulo_w = 'N') then
			...
			if (coalesce(obter_se_tit_pagar_unificado(nr_titulo_p),'N') = 'S') and (coalesce(philips_param_pck.get_cd_pais,1) = 2) then --Mexico
				select coalesce(sum(a.vl_base_calculo),0),
					coalesce(sum(a.vl_imposto),0)
				into STRICT vl_base_calculo_unifi_w,
					vl_imposto_unifi_w
				from titulo_pagar_imposto a,
					titulo_pagar b
				where a.nr_titulo = b.nr_titulo
			...
			if ((coalesce(philips_param_pck.get_cd_pais,1) = 2) and (coalesce(obter_se_tit_pagar_unificado(nr_titulo_p),'N') = 'S') and (coalesce(ie_gerar_trib_unif_w,'N') = 'S')) or
				/*se for mexico, se nao for processo de unificacao, deve gerar o imposto conforme ocorria antes dessa tratativa. */

			   ((coalesce(philips_param_pck.get_cd_pais,1) = 2) and (coalesce(obter_se_tit_pagar_unificado(nr_titulo_p),'N') = 'N')) or
			   /*se nao for mexico, gera normal tambem conforme ocorria antes dessa tratativa.*/

			   (coalesce(philips_param_pck.get_cd_pais,1) <> 2) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_tributo_titulo ( nr_titulo_p bigint, nm_usuario_p text, ie_baixa_titulo_p text, nr_bordero_p bigint, dt_baixa_bordero_p timestamp, nr_seq_escrit_p bigint, dt_baixa_escrit_p timestamp, dt_baixa_p timestamp, cd_estabelecimento_p bigint, vl_pago_tit_p bigint) FROM PUBLIC;
