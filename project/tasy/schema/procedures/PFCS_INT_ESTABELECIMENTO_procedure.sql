...
CREATE OR REPLACE PROCEDURE pfcs_int_estabelecimento (cd_cgc_p text, cd_base_cgc_p text, nm_estabelecimento_p text, ie_situacao_p text, nm_usuario_p text) AS $body$
DECLARE
...
if (cd_cgc_p IS NOT NULL AND cd_cgc_p::text <> '' AND nm_usuario_p IS NOT NULL AND nm_usuario_p::text <> '') then
	...
	json_w.put('CD_EMPRESA', cd_base_cgc_p);
	json_w.put('NOME_EMPRESA', nm_estabelecimento_p);
	...
	json_data_w := bifrost.send_integration_content('PFCS.establishment.event', json_data_w, nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pfcs_int_estabelecimento (cd_cgc_p text, cd_base_cgc_p text, nm_estabelecimento_p text, ie_situacao_p text, nm_usuario_p text) FROM PUBLIC;
