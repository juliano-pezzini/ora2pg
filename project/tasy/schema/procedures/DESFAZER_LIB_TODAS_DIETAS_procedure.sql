...
CREATE OR REPLACE PROCEDURE desfazer_lib_todas_dietas ( ds_lista_dieta_p text, nm_usuario_p text) AS $body$
DECLARE
	...
	begin
	 ...
	 while(ds_lista_dieta_w IS NOT NULL AND ds_lista_dieta_w::text <> '')	loop
		...
		if (nr_seq_sequencia_w > 0)then
			begin
			CALL desfazer_liberacao_dieta(nr_seq_sequencia_w,nm_usuario_p);
			end;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE desfazer_lib_todas_dietas ( ds_lista_dieta_p text, nm_usuario_p text) FROM PUBLIC;
