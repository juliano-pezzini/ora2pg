...
CREATE OR REPLACE PROCEDURE ptu_gerar_pedido_compl_aut_v50 ( nr_seq_guia_p bigint, nr_seq_requisicao_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
if (ie_anexo_guia_w	= 'S') then 
	CALL ptu_inserir_dados_anexo_pedido(nr_seq_guia_p, nr_seq_requisicao_p, null, nr_seq_pedido_novo_w, nm_usuario_p);
end if;
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	if (nr_seq_requisicao_p IS NOT NULL AND nr_seq_requisicao_p::text <> '') then 
		...
		values (nr_seq_pedido_compl_serv_w, nr_seq_pedido_novo_w, coalesce(ie_tipo_tabela_proc_w, ie_tipo_tabela_w), 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_pedido_compl_aut_v50 ( nr_seq_guia_p bigint, nr_seq_requisicao_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
