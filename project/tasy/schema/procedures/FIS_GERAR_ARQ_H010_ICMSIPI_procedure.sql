...
CREATE OR REPLACE PROCEDURE fis_gerar_arq_h010_icmsipi ( nr_seq_controle_p bigint, dt_inv_p timestamp, nr_linha_p INOUT bigint ) AS $body$
DECLARE
...
type reg_c_reg_H010 is table of fis_efd_icmsipi_H010%RowType;
...
fetch c_reg_H010 bulk collect into vetRegH010 limit 1000;
	...
	ds_linha_w := substr(ds_sep_w || vetRegH010[i].cd_reg ||
			     ds_sep_w || vetRegH010[i].cd_item ||
			     ds_sep_w || vetRegH010[i].cd_unid ||
			     ds_sep_w || substr(sped_obter_campo_valor(vetRegH010[i].qt_item ), 1, 30) ||
			     ds_sep_w || substr(sped_obter_campo_valor(vetRegH010[i].vl_unit ), 1, 30) ||
			     ds_sep_w || substr(sped_obter_campo_valor(vetRegH010[i].vl_item ), 1, 30) ||
			     ds_sep_w || vetRegH010[i].cd_ind_prop ||
			     ds_sep_w || vetRegH010[i].cd_part ||
			     ds_sep_w || vetRegH010[i].ds_txt_compl ||
			     ds_sep_w || vetRegH010[i].cd_cta ||
			     ds_sep_w || substr(sped_obter_campo_valor(vetRegH010[i].vl_item_ir ), 1, 30) || ds_sep_w,
			     1,
			     8000);
	...
	fis_registros_w[qt_cursor_w].cd_registro		:=  vetRegH010[i].cd_reg;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_gerar_arq_h010_icmsipi ( nr_seq_controle_p bigint, dt_inv_p timestamp, nr_linha_p INOUT bigint ) FROM PUBLIC;
