...
CREATE OR REPLACE PROCEDURE ptu_gerar_envio_fatura_v60 ( nr_seq_fatura_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
C02 CURSOR FOR 
	SELECT	lpad(elimina_caractere_especial(a.nr_lote),8,'0') nr_lote_503, 
		...
		rpad(coalesce(a.nm_medico_auditor,' '),40,' '), 
		rpad(coalesce(a.nr_crm_auditor,' '),15,' '), 
		rpad(coalesce(a.nm_enfer_auditor,' '),40,' '), 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_envio_fatura_v60 ( nr_seq_fatura_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
