...
CREATE OR REPLACE PROCEDURE ptu_processa_resp_pedido_aut ( nr_seq_resp_autorizacao_p ptu_resposta_autorizacao.nr_sequencia%type, nr_seq_execucao_p ptu_resposta_autorizacao.nr_seq_execucao%type, dt_validade_p text, cd_unimed_benef_p ptu_resposta_autorizacao.cd_unimed_beneficiario%type, ds_observacao_p ptu_resposta_autorizacao.ds_observacao%type, nm_usuario_p text) AS $body$
DECLARE
...
if (nr_seq_guia_w IS NOT NULL AND nr_seq_guia_w::text <> '') then 
	...
	and	nr_seq_guia	= nr_seq_guia_w;
	/* 
	O valor que vier no campo ie_autorizado_w irá valer para todos os itens. 
	Ex: se um item vier em auditoria todos os itens do xml serão em auditoria. 
	*/
	...
	if	((qt_registros_w > 0) or (qt_registros_ww > 0)) then 
		...
		if (qt_registros_w > 0) and (qt_registros_ww > 0) then 
			if (coalesce(ie_gerar_senha_interna_w,'S')	= 'S') then 
				...
				dt_validade_senha_w := dt_validade_w;
			else 
				cd_senha_w	:= null;
	...
	CALL pls_guia_gravar_historico(nr_seq_guia_w,2,substr('Recebida e processada a resposta de pedido de autorização da Unimed '||cd_unimed_benef_p||' com a mensagem: '||chr(10)||ds_observacao_w,1,4000),'',nm_usuario_p);
	...
	CALL pls_requisicao_gravar_hist(nr_seq_requisicao_w,'L',substr('Recebida e processada a resposta de pedido de autorização da Unimed '||cd_unimed_benef_p||' com a mensagem: '||chr(10)||ds_observacao_w,1,4000),null,nm_usuario_p);
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_processa_resp_pedido_aut ( nr_seq_resp_autorizacao_p ptu_resposta_autorizacao.nr_sequencia%type, nr_seq_execucao_p ptu_resposta_autorizacao.nr_seq_execucao%type, dt_validade_p text, cd_unimed_benef_p ptu_resposta_autorizacao.cd_unimed_beneficiario%type, ds_observacao_p ptu_resposta_autorizacao.ds_observacao%type, nm_usuario_p text) FROM PUBLIC;
