...
c001 CURSOR FOR
	SELECT	CASE WHEN coalesce(ie_quebra_mes_p,'N')='S' THEN r.dt_referencia  ELSE to_date(null) END  dt_referencia,
		...
		sum(CASE WHEN r.dt_referencia=trunc(dt_final_w, 'month') THEN  vl_saldo  ELSE 0 END ),
		CASE WHEN cd_estab_w=0 THEN null  ELSE s.cd_estabelecimento END  cd_estabelecimento
	...
	group by CASE WHEN coalesce(ie_quebra_mes_p,'N')='S' THEN r.dt_referencia  ELSE to_date(null) END ,
		...
		s.cd_centro_custo,
		d.ie_debito_credito,
		CASE WHEN cd_estab_w=0 THEN null  ELSE s.cd_estabelecimento END
...
dt_final_w	:= coalesce(dt_final_p, fim_mes(dt_inicial_w));
vl_saldo_ant_w	:= null;
	...
	if (ie_gerar_w	= 'S')  then
		...
		loop
			...
			if (vl_saldo_w <> 0) then
				vl_saldo_ant_w	:= coalesce(vl_saldo_w,0);
			end if;

			if ((vl_saldo_w = 0) and (coalesce(vl_saldo_ant_w,0) <> 0)) then
