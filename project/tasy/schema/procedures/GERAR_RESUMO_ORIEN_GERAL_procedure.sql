...
CREATE OR REPLACE PROCEDURE gerar_resumo_orien_geral (cd_estabelecimento_p bigint, nr_seq_consulta_p bigint, cd_pessoa_fisica_p text, ie_mostra_todos_p text, nr_seq_item_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	begin 
	ds_orientacao_w	:=	'';
	ds_orientacao_w	:= 	ds_orientacao_w || upper(expressao1_w) || ' ' || dt_registro_w || ds_espaco_20_w || 
	...
	CALL gravar_registro_resumo_oft(ds_orientacao_w,nr_seq_item_p,nm_usuario_p,to_date(dt_registro_w,'dd/mm/yyyy hh24:mi:ss'));
	end;
end loop;
close C01;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_resumo_orien_geral (cd_estabelecimento_p bigint, nr_seq_consulta_p bigint, cd_pessoa_fisica_p text, ie_mostra_todos_p text, nr_seq_item_p bigint, nm_usuario_p text) FROM PUBLIC;
