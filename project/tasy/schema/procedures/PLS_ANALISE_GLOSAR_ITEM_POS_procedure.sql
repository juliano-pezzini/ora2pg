...
CREATE OR REPLACE PROCEDURE pls_analise_glosar_item_pos ( nr_seq_analise_p bigint, nr_seq_conta_p bigint, nr_seq_conta_proc_p bigint, nr_seq_conta_mat_p bigint, nr_seq_conta_pos_p bigint, qt_liberar_p bigint, nr_seq_mot_liberacao_p bigint, ds_parecer_p text, cd_estabelecimento_p bigint, nr_seq_grupo_atual_p bigint, ie_obriga_parecer_p text, ie_obriga_ocorrencia_p text, nm_usuario_p text, ie_acao_glosar_p text default 'N') AS $body$
DECLARE
...
Pontos de atenção:
	...
	ALTERAÇÕES NRSSA ROTINA DEVEM SER REFLETIDAS NA PLS_ANALISE_GLOSA_IT_NOVO_POS, POIS
	AQUELA ROTINA FOI CRIADA À PARTIR DESTA PARA CONTEMPLAR NOVA GERAÇÃO DE PÓS-ESTABELECIDO.
...
qt_liberar_w			pls_conta_proc.qt_procedimento%type;
qt_ocor_pagto_w			integer := 0;
...
if (nr_seq_conta_w IS NOT NULL AND nr_seq_conta_w::text <> '') then
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		and	ie_situacao		= 'A'
		and	nr_seq_conta_pos_estab	= nr_seq_conta_pos_p;

		if (cd_ocorrencia_w IS NOT NULL AND cd_ocorrencia_w::text <> '') and (qt_ocorrencia_w	= 0) then
			...
			nr_seq_ocor_benef_w := pls_inserir_ocorrencia(	nr_seq_segurado_w, nr_seq_ocorrencia_w, null, null, null, null, null, null, nm_usuario_p, 'Ocorrência inserida pelo auditor '||obter_nome_usuario(nm_usuario_p), nr_seq_glosa_vinculada_w, ie_tipo_item_ww, cd_estabelecimento_p, 'N', null, nr_seq_ocor_benef_w, nr_seq_conta_pos_p, null, 'S', null);
	...
	if (ie_item_w = 'S') and
		...
		--Identifica se item tem glosa qu einterfere pagamento.
		--Caso retornar zero, então a(s) glosa(s) apenas interferem no faturamento
		select 	count(1)
		into STRICT	qt_ocor_pagto_w
		from	pls_ocorrencia_benef a,
				pls_ocorrencia b
		where	a.nr_seq_conta_pos_estab = nr_seq_conta_pos_p
		and		a.ie_situacao = 'A'
		and		a.nr_seq_ocorrencia = b.nr_sequencia
		and 	b.ie_glosar_pagamento = 'S';

		if (coalesce(ie_vl_pag_prestador_w,'N') = 'N') or (coalesce(qt_liberar_p,0) <= 0) or ( qt_ocor_pagto_w = 0 ) then
			...
			update	pls_conta_pos_estabelecido
			set	qt_item				= qt_liberada_w, --aaschlote OS 923624
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_analise_glosar_item_pos ( nr_seq_analise_p bigint, nr_seq_conta_p bigint, nr_seq_conta_proc_p bigint, nr_seq_conta_mat_p bigint, nr_seq_conta_pos_p bigint, qt_liberar_p bigint, nr_seq_mot_liberacao_p bigint, ds_parecer_p text, cd_estabelecimento_p bigint, nr_seq_grupo_atual_p bigint, ie_obriga_parecer_p text, ie_obriga_ocorrencia_p text, nm_usuario_p text, ie_acao_glosar_p text default 'N') FROM PUBLIC;
