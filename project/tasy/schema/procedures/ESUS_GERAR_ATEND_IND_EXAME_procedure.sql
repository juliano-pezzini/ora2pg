...
CREATE OR REPLACE PROCEDURE esus_gerar_atend_ind_exame ( cd_profissional_p text, cd_cbo_p text, cd_sec_profissional_p text, cd_sec_cbo_p text, cd_ter_profissional_p text, cd_ter_cbo_p text, nr_atendimento_p bigint, dt_atendimento_p timestamp, nr_seq_ficha_p bigint, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE

					 
cd_procedimento_w			esus_atend_ind_exame.cd_procedimento%type;
ie_origem_proced_w			esus_atend_ind_exame.ie_origem_proced%type;
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	where	nr_seq_atend_ind = nr_seq_ficha_p 
	...
	if (qt_proc_gerado_w = 0) then 
		...
		values (	nm_usuario_p, 
			...
			ie_origem_proced_w, 
			nr_seq_ficha_p, 
			nextval('esus_atend_ind_exame_seq'));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE esus_gerar_atend_ind_exame ( cd_profissional_p text, cd_cbo_p text, cd_sec_profissional_p text, cd_sec_cbo_p text, cd_ter_profissional_p text, cd_ter_cbo_p text, nr_atendimento_p bigint, dt_atendimento_p timestamp, nr_seq_ficha_p bigint, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
