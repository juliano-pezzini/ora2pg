...
CREATE OR REPLACE PROCEDURE ptu_envio_ped_autorizacao_v60 ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, nr_versao_ptu_p ptu_pedido_autorizacao.nr_versao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, nr_seq_pedido_novo_p INOUT ptu_pedido_autorizacao.nr_sequencia%type) AS $body$
DECLARE
...
if (nr_seq_requisicao_p IS NOT NULL AND nr_seq_requisicao_p::text <> '') then 
	...
	nr_seq_prestador_req_w		:= 	cd_medico_solicitante_w;
	...
	exception 
	when others then 
		nr_seq_prest_alto_custo_w	:= nr_seq_prestador_w;
	end;
	 
	if (nr_seq_prest_alto_custo_w IS NOT NULL AND nr_seq_prest_alto_custo_w::text <> '') then 
		cd_unimed_prestador_w		:=	cd_cooperativa_w;
		nm_prestador_alto_custo_w  := substr(pls_obter_dados_prestador(nr_seq_prestador_w,'N'),1,25);
	end if;
	...
	if	((ie_tipo_segurado_w IS NOT NULL AND ie_tipo_segurado_w::text <> '') and (coalesce(cd_carteirinha_w::text, '') = '')) or (coalesce(ie_tipo_segurado_w,'X') = 'T') then 
		begin 
			select	coalesce(nr_cartao_intercambio,cd_usuario_plano) 
			into STRICT	cd_carteirinha_w 
			from	pls_segurado_carteira 
...
if (ie_alto_custo_w  in ('3','2')) or (ie_tipo_guia_w  = 1)then 
  begin 
  ...
  cd_uni_prest_alto_custo_w  := null;
...
values (nr_seq_pedido_novo_w, dt_execucao_w, ie_tipo_cliente_w, 
	...
	nm_prestador_alto_custo_w, nr_versao_ptu_p, coalesce(ie_recem_nascido_w,'N'), 
	...
	ie_liminar_w, cd_municipio_ibge_w) returning nr_sequencia, nr_seq_execucao into nr_seq_pedido_novo_p, nr_seq_controle_exec_w;
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	if (nr_seq_requisicao_p IS NOT NULL AND nr_seq_requisicao_p::text <> '') then 
		...
		insert	into ptu_pedido_aut_servico(nr_sequencia,cd_servico, ds_opme, 
			...
			qt_dosagem_total, qt_unidade_medida) 
		values (nr_seq_pedido_aut_serv_w, cd_procedimento_w, ds_procedimento_ptu_w, 
			...
			qt_dosagem_total_w, qt_unidade_medida_w);
	elsif (nr_seq_guia_p IS NOT NULL AND nr_seq_guia_p::text <> '') then 
...
EXIT WHEN NOT FOUND; /* apply on c02 */
	...
	if (cd_material_w IS NOT NULL AND cd_material_w::text <> '') then 
		...
		if (nr_seq_requisicao_p IS NOT NULL AND nr_seq_requisicao_p::text <> '') then 
			...
			--cd_ref_fabricante_imp_w será utilizado o nr_sequencia temporariamente 
			...
			values (nr_seq_pedido_aut_serv_w, cd_servico_w, ds_material_ptu_w, 
				...
				trim(both somente_numero(coalesce(coalesce(nr_registro_anvisa_w, cd_anvisa_w), nr_registro_anvisa_gen_w))), coalesce(coalesce(cd_ref_fabricante_imp_w, cd_ref_fabricante_w), cd_ref_fabricante_imp_gen_w), CASE WHEN ie_tipo_anexo_w='OP' THEN 3 WHEN ie_tipo_anexo_w='QU' THEN 1 WHEN ie_tipo_anexo_w='RA' THEN 2  ELSE 9 END , 
...
/*Chamada da rotina para verificar se possui liminar para o beneficiário de intercâmbio*/
