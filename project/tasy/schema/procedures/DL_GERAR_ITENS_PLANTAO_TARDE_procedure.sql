...
CREATE OR REPLACE PROCEDURE dl_gerar_itens_plantao_tarde ( nr_seq_distribuicao_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, vl_distribuicao_p bigint) AS $body$
DECLARE
...
if (dl_obter_dados_socio(nr_seq_socio_w,'T') = 'A') then 
	...
	/* dominio DL - Tipo de item para calculo da distribuicao / Plantoes da tarde */
	...
	if (coalesce(nr_seq_tipo_distrib_w::text, '') = '') then -- afstringari 236244 28/07/2010 
		...
		ds_msg_tp_distrib_w	:= substr(dl_obter_msg_tp_distrib(1,3),1,255);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dl_gerar_itens_plantao_tarde ( nr_seq_distribuicao_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, vl_distribuicao_p bigint) FROM PUBLIC;
