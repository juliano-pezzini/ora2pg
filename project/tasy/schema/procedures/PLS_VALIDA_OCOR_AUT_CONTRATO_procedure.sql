...
C01 CURSOR FOR 
	SELECT	qt_tempo_contratacao, 
		ie_tipo_tempo 
	from	pls_validacao_aut_contrato 
...
if (nr_seq_ocor_combinada_p IS NOT NULL AND nr_seq_ocor_combinada_p::text <> '') then 
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		if (ie_tipo_tempo_w	= 'D') then 
			dt_regra_w	:= trunc(dt_inclusao_operadora_w + (qt_tempo_contratacao_w));
		elsif (ie_tipo_tempo_w	= 'M') then 
			dt_regra_w	:= (add_months(dt_inclusao_operadora_w, + qt_tempo_contratacao_w));
		elsif (ie_tipo_tempo_w	= 'A') then 
			dt_regra_w	:= (add_months(dt_inclusao_operadora_w, + qt_tempo_contratacao_w * 12)); /* Vezes 12 meses ao ano */
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_valida_ocor_aut_contrato ( nr_seq_ocor_combinada_p bigint, nr_seq_ocorrencia_p bigint, nr_seq_segurado_p bigint, nr_seq_motivo_glosa_p bigint, nr_seq_guia_p bigint, nr_seq_requisicao_p bigint, nr_seq_execucao_p bigint, ie_utiliza_filtro_p text, nr_seq_param1_p bigint, nr_seq_param2_p bigint, nr_seq_param3_p bigint, nr_seq_param4_p bigint, nr_seq_param5_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
