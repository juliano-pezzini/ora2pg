...
if (nr_seq_nota_fiscal_w IS NOT NULL AND nr_seq_nota_fiscal_w::text <> '') then 
	...
	CALL wheb_mensagem_pck.exibir_mensagem_abort(265325,'');
...
nr_seq_nota_fiscal_w := pls_gerar_notas_lote_prod(	nr_seq_lote_p, cd_serie_nf_p, cd_operacao_nf_p, dt_emissao_p, cd_natureza_operacao_p, ds_observacao_p, ds_complemento_p, dt_base_venc_p, nr_nota_fiscal_p, nm_usuario_p, 'S', nr_seq_prestador_p, nr_seq_prestador_pgto_p, cd_estabelecimento_p, ie_base_venc_p, nr_seq_nota_fiscal_w);
...
for r_c01_w in c01 loop 
	...
	ds_consistencia_w := vincular_titulo_pagar_nf(	nr_seq_nota_fiscal_w, r_c01_w.nr_titulo, nm_usuario_p, 'A', 'S', ds_consistencia_w);
end loop;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_vincular_nota_lote_prod ( nr_seq_lote_p pls_lote_pagamento.nr_sequencia%type, nr_seq_prestador_pgto_p pls_prestador.nr_sequencia%type, nr_seq_prestador_p pls_prestador.nr_sequencia%type, cd_serie_nf_p nota_fiscal.cd_serie_nf%type, cd_operacao_nf_p nota_fiscal.cd_operacao_nf%type, dt_emissao_p timestamp, cd_natureza_operacao_p bigint, ds_observacao_p text, ds_complemento_p text, dt_base_venc_p timestamp, nr_nota_fiscal_p nota_fiscal.nr_nota_fiscal%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, ie_base_venc_p text, nr_seq_nota_fiscal_p INOUT nota_fiscal.nr_sequencia%type) FROM PUBLIC;
