...
CREATE OR REPLACE PROCEDURE gerar_nut_elem_pac_adulto ( NR_SEQUENCIA_P bigint, NM_USUARIO_P text) AS $body$
DECLARE
		...
		if (nr_sequencia_w = 0) then
			...
			EXIT WHEN NOT FOUND; /* apply on C02 */
				...
				insert into nut_pac_elem_mat(
					...
					qt_vol_2_fase, qt_vol_3_fase, qt_vol_4_fase, nr_seq_elem_mat, nr_seq_nut_pac, cd_material,cd_unidade_medida)
				values (	nr_sequencia_ww, nr_sequencia_w, clock_timestamp(),
					nm_usuario_p, null, null,
					null, null, null, nr_sequencia_www, nr_sequencia_p, cd_material_w, obter_unid_med_usua('ml'));
			end Loop;
			close C02;
			end;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_nut_elem_pac_adulto ( NR_SEQUENCIA_P bigint, NM_USUARIO_P text) FROM PUBLIC;
