...
CREATE OR REPLACE PROCEDURE atualizar_acao_catraca ( nr_controle_acesso_p bigint, dt_acao_p timestamp, ie_entrada_saida_p text, ds_bloqueio_p text, ds_evento_p text, cd_pessoa_fisica_p text, nm_usuario_p text, ds_cracha_p text default null) AS $body$
DECLARE


ie_tipo_visita_w	varchar(5);
nr_atendimento_w	bigint;
dt_entrada_acomp_w	timestamp;
nr_seq_atend_visita_w	bigint;
ie_entrada_saida_w	varchar(10);		
nr_seq_controle_w	atendimento_visita.nr_seq_controle%type := 0;
ie_controle_cracha_w	varchar(1);
...
BEGIN
select upper(CASE WHEN coalesce(ie_entrada_saida_p,'N')='S' THEN 'SAIDA' WHEN coalesce(ie_entrada_saida_p,'N')='E' THEN 'ENTRADA'  ELSE 'N' END )
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE atualizar_acao_catraca ( nr_controle_acesso_p bigint, dt_acao_p timestamp, ie_entrada_saida_p text, ds_bloqueio_p text, ds_evento_p text, cd_pessoa_fisica_p text, nm_usuario_p text, ds_cracha_p text default null) FROM PUBLIC;
