...
CREATE OR REPLACE PROCEDURE ptu_inserir_dados_anexo_req ( nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, nr_seq_pedido_anexo_p ptu_pedido_aut_anexo.nr_sequencia%type, ds_biometria_p pls_lote_anexo_guias_aut.ds_biometria%type, nm_usuario_p text) AS $body$
DECLARE

/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
finalidade: Inserir os dados do anexo importado para as tabelas de anexo da requisição 
------------------------------------------------------------------------------------------------------------------- 
...
if (nr_seq_requisicao_p IS NOT NULL AND nr_seq_requisicao_p::text <> '') then 
	...
	if (coalesce(nr_seq_lote_anexo_guia_w::text, '') = '') then 
		...
		values (nr_seq_lote_anexo_guia_w, null, clock_timestamp(), 
			 ...
			 nr_seq_requisicao_p, ie_tipo_anexo_w, null, 
	...
	for	r_c01_w in c01	loop 
		...
		(SELECT clock_timestamp(), nm_usuario_p, clock_timestamp(), 
			 ...
			 nextval('pls_lote_anexo_proc_aut_seq'), dt_prevista_w 
		 from	 pls_requisicao_proc 
	...
	for	r_c02_w in c02	loop 
		...
		(SELECT clock_timestamp(), clock_timestamp(), 
			 ...
			 nr_sequencia, nextval('pls_lote_anexo_mat_aut_seq'), nr_seq_material, 
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_inserir_dados_anexo_req ( nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, nr_seq_pedido_anexo_p ptu_pedido_aut_anexo.nr_sequencia%type, ds_biometria_p pls_lote_anexo_guias_aut.ds_biometria%type, nm_usuario_p text) FROM PUBLIC;
