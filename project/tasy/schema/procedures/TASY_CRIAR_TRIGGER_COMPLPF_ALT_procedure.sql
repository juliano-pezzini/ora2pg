...
CREATE OR REPLACE PROCEDURE tasy_criar_trigger_complpf_alt () AS $body$
DECLARE
...
ds_comando_c001_w	varchar(4000) := '' ||
		...
		'	where	upper(table_name) = ''COMPL_PESSOA_FISICA''' ||
		'	and	obter_se_atrib_constraint(table_name,column_name,''PK'') = ''N'''||
		'	and	upper(column_name)	not in (''DT_ATUALIZACAO'',''NM_USUARIO'',''IE_TIPO_COMPLEMENTO'',''DS_OBSERVACAO'')';
...
ds_script_trigger_w :=  ' create or replace trigger compl_pessoa_fisica_update ' || ds_enter_w||
			' before update on COMPL_PESSOA_FISICA ' || ds_enter_w||
			...
			' cd_estabelecimento_w	number(10);'|| ds_enter_w||
			...
			'	select	cd_estabelecimento 					' || ds_enter_w || 
			'	into	cd_estabelecimento_w 					' || ds_enter_w || 
			'	from	pessoa_fisica						' || ds_enter_w || 
			'	where	cd_pessoa_fisica	= :new.cd_pessoa_fisica;	' || ds_enter_w ||
...
while( DBMS_SQL.FETCH_ROWS(C001) > 0 ) loop
	...
	ds_script_trigger_ww	:= ds_script_trigger_ww||'      gerar_solicitacao_alteracao(:old.'||nm_atributo_w||','||':new.'||nm_atributo_w||','||ds_aspa_w||
								nm_atributo_w||ds_aspa_w||','||':new.cd_pessoa_fisica'||',cd_estabelecimento_w, :new.nm_usuario);'||ds_enter_w;
	end;
end loop;
...
ds_script_trigger_www	:=	ds_script_trigger_www||	'	select	decode(count(1),0,''N'',''S'')	' || ds_enter_w||
							...
							'	and	((dt_rescisao_contrato is null)  or (dt_rescisao_contrato > sysdate));'||ds_enter_w||ds_enter_w||
							'	select	decode(count(1),0,''N'',''S'')	' || ds_enter_w||
							'	into	ie_pagador_w	' 		|| ds_enter_w||
							'	from	pls_contrato_pagador		' || ds_enter_w||
							'	where	cd_pessoa_fisica	= :new.cd_pessoa_fisica'||ds_enter_w||
							'	and	((dt_rescisao is null)  or (dt_rescisao > sysdate));'||ds_enter_w||ds_enter_w||
...
while( DBMS_SQL.FETCH_ROWS(C001) > 0 ) loop
	...
	ds_script_trigger_www	:= ds_script_trigger_www||'      gerar_solicitacao_alt_regra(:old.'||nm_atributo_w||','||':new.'||nm_atributo_w||','||ds_aspa_w||
								nm_atributo_w||ds_aspa_w||','||':new.cd_pessoa_fisica,''COMPL_PESSOA_FISICA'',wheb_usuario_pck.get_cd_funcao,:new.ie_tipo_complemento,r_c01_w.ie_tipo_segurado'||
								',ie_estipulante_w,ie_pagador_w,cd_estabelecimento_w, :new.nm_usuario,ie_alterar_w);'||ds_enter_w;
...
ds_script_trigger_wwww	:= '		end loop;' || ds_enter_w ||
			   '	end if;'||ds_enter_w||
			...
			'		tasy_gerar_solicitacao_compl(:new.cd_pessoa_fisica,:new.ie_tipo_complemento);'||ds_enter_w||
			'		if	(vl_parametro_w = ''S'') then	'|| ds_enter_w ||		
...
qt_caracteres_w := length(ds_script_vetor[1].ds) + coalesce(length(ds_script_vetor[2].ds),0) + coalesce(length(ds_script_vetor[3].ds),0) + coalesce(length(ds_script_vetor[4].ds),0) + coalesce(length(ds_script_vetor[5].ds),0);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE tasy_criar_trigger_complpf_alt () FROM PUBLIC;
