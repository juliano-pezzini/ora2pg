...
CREATE OR REPLACE PROCEDURE ptu_gerar_resp_cons_benef_v50 ( nr_seq_consulta_p bigint, cd_usuario_plano_p text, nm_beneficiario_p text, sobrenome_benef_p text, dt_nasc_benef_p timestamp, nr_versao_ptu_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_retorno_p INOUT bigint) AS $body$
DECLARE
...
cd_rede_atendimento_w		varchar(4);
ie_regulamentacao_w			smallint;
ie_regulamentacao_cad_w		varchar(2);
...
C01 CURSOR FOR 
	...
	and (b.cd_usuario_plano	= cd_carteirinha_w) 
		...
		if (qt_registro_w > 0) then 
			...
			EXIT WHEN NOT FOUND; /* apply on C02 */
				...
				fetch C03 into	 
					...
					ie_abrange_w, 
					ie_regulamentacao_cad_w, 
					cd_rede_atendimento_w;
				EXIT WHEN NOT FOUND; /* apply on C03 */
					begin 
					ie_regulamentacao_w := null;
					if (ie_regulamentacao_cad_w = 'P') then 
						ie_regulamentacao_w := 3;
					elsif (ie_regulamentacao_cad_w = 'A') then 
						ie_regulamentacao_w := 2;
					elsif (ie_regulamentacao_cad_w = 'R') then 
					...
					exception 
					when others then 
						ds_tipo_acomodacao_w	:= 'Nao especificada';
					end;
					...
					values (nextval('ptu_resp_nomes_benef_seq'), nr_seq_retorno_p, nm_beneficiario_w, 
						...
						cd_rede_atendimento_w, ie_regulamentacao_w);
					end;
				end loop;
				close C03;
				end;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_gerar_resp_cons_benef_v50 ( nr_seq_consulta_p bigint, cd_usuario_plano_p text, nm_beneficiario_p text, sobrenome_benef_p text, dt_nasc_benef_p timestamp, nr_versao_ptu_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_retorno_p INOUT bigint) FROM PUBLIC;
