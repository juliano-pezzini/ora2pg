...
CREATE OR REPLACE PROCEDURE pls_importar_arquivo_a560 ( ds_conteudo_p text, nr_seq_camara_contest_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) AS $body$
DECLARE
...
union
		...
		select	CASE WHEN (substr(ds_conteudo_p,47,11))::numeric =0 THEN null  ELSE (substr(ds_conteudo_p,47,11))::numeric  END
		into STRICT	nr_nota_credito_debito_a500_w
		;
		...
		select	CASE WHEN (substr(ds_conteudo_p,12,11))::numeric =0 THEN null  ELSE (substr(ds_conteudo_p,12,11))::numeric  END ,
			CASE WHEN (substr(ds_conteudo_p,45,11))::numeric =0 THEN null  ELSE (substr(ds_conteudo_p,45,11))::numeric  END
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_importar_arquivo_a560 ( ds_conteudo_p text, nr_seq_camara_contest_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
