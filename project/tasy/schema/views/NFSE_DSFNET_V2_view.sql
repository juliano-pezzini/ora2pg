...
CREATE OR REPLACE VIEW nfse_dsfnet_v2 (nr_seq_nota, nr_seq_transmissao, assinatura, inscricaomunicipalprestador, razaosocialprestador, tiporps, serierps, numerorps, dataemissaorps, situacaorps, serierpssubstituido, numerorpssubstituido, numeronfsesubstituida, dataemissaonfsesubstituida, serieprestacao, inscricaomunicipaltomador, cpfcnpjtomador, razaosocialtomador, tipologradourotomador, logradourotomador, numeroenderecotomador, complementoenderecotomador, tipobairrotomador, bairrotomador, cidadetomador, cidadetomadordescricao, ceptomador, emailtomador, codigoatividade, aliquotaatividade, tiporecolhimento, municipioprestacao, municipioprestacaodescricao, operacao, tributacao, valorpis, valorcofins, valorinss, valorir, valorcsll, aliquotapis, aliquotacofins, aliquotainss, aliquotair, aliquotacsll, descricaorps, dddprestador, telefoneprestador, dddtomador, telefonetomador, motcancelamento, cpfcnpjintermediario, deducaopor, tipodeducao, cpfcnpjreferencia, numeronfreferencia, valortotalreferencia, nrseqtransmissao, percentualdeduzir, valordeduzir) AS select	a.nr_sequencia nr_seq_nota,
        n.nr_seq_transmissao nr_seq_transmissao,
	...
	99 SeriePrestacao,
	CASE WHEN 		obter_desc_municipio_ibge(			CASE WHEN 	obter_municipio_ibge(obter_dados_pf_pj(cd_pessoa_fisica, cd_cgc, 'CEP'))=0 THEN  				obter_dados_pf_pj(cd_pessoa_fisica, cd_cgc, 'CDM')  ELSE obter_municipio_ibge(obter_dados_pf_pj(cd_pessoa_fisica, cd_cgc, 'CEP')) END )=obter_desc_municipio_ibge(			CASE WHEN 	obter_municipio_ibge(obter_dados_pf_pj('', cd_cgc_emitente, 'CEP'))=0 THEN  				obter_dados_pf_pj('', cd_cgc_emitente, 'CDM')  ELSE obter_municipio_ibge(obter_dados_pf_pj('', cd_cgc_emitente, 'CEP')) END ) THEN  		CASE WHEN cd_cgc='' THEN  coalesce(lpad(obter_dados_pf(cd_pessoa_fisica, 'ISS'), 11, '0'), '00000000000')  ELSE coalesce(lpad(substr(obter_dados_pf_pj(cd_pessoa_fisica, cd_cgc, 'IM'), 1, 30), 11, '0'), '00000000000') END   ELSE '00000000000' END  InscricaoMunicipalTomador,	
	...
	CASE WHEN substr(obter_dados_pf_pj(cd_pessoa_fisica,cd_cgc,'UF'),1,2)='IN' THEN '0009999'  ELSE lpad(obter_siafi_municipio_ibge(			CASE WHEN 				obter_municipio_ibge(nfe_elimina_caractere_especial(obter_dados_pf_pj(cd_pessoa_fisica, cd_cgc, 'CEP')))=0 THEN  				obter_dados_pf_pj(cd_pessoa_fisica, cd_cgc, 'CDM')  ELSE obter_municipio_ibge(nfe_elimina_caractere_especial(obter_dados_pf_pj(cd_pessoa_fisica, cd_cgc, 'CEP'))) END 			), 8, '0') END  CidadeTomador,
	substr(
	CASE WHEN substr(obter_dados_pf_pj(cd_pessoa_fisica,cd_cgc,'UF'),1,2)='IN' THEN obter_dados_pf_pj( cd_pessoa_fisica, cd_cgc, 'PAIS')  ELSE obter_desc_municipio_ibge(			CASE WHEN 				obter_municipio_ibge(nfe_elimina_caractere_especial(obter_dados_pf_pj(cd_pessoa_fisica, cd_cgc, 'CEP')))=0 THEN  				obter_dados_pf_pj(cd_pessoa_fisica, cd_cgc, 'CDM')  ELSE obter_municipio_ibge(nfe_elimina_caractere_especial(obter_dados_pf_pj(cd_pessoa_fisica, cd_cgc, 'CEP'))) END 		) END 
	, 1, 50) CidadeTomadorDescricao,			
	lpad(somente_numero(substr(obter_dados_pf_pj(cd_pessoa_fisica, cd_cgc, 'CEP'), 1, 15)), 8, '0') CEPTomador,
  coalesce(substr(nfe_elimina_caractere_especial(obter_dados_pf_pj(a.cd_pessoa_fisica, a.cd_cgc,'M')), 1, 80), '-') EmailTomador,
	--nvl(substr(nfe_elimina_caractere_especial(obter_email_pessoa(obter_estabelecimento_ativo,cd_pessoa_fisica, cd_cgc,obter_usuario_ativo)), 1, 80), '-') EmailTomador,

	--nvl(nvl(obter_dados_grupo_servico_item(obter_item_servico_proced(obter_procedimento_nfse(a.nr_sequencia,'P'), obter_procedimento_nfse(a.nr_sequencia,'O')), 'P'),
	...
	--    obter_dados_pf_pj_estab(a.cd_estabelecimento, NULL, a.cd_cgc_emitente, 'ATIV'))
	...
	obter_siafi_municipio_ibge(obter_municipio_ibge(obter_dados_pf_pj('', cd_cgc_emitente, 'CEP'))) MunicipioPrestacao,	
	nfe_elimina_caractere_especial(obter_desc_municipio_ibge(
		CASE WHEN 	obter_municipio_ibge(obter_dados_pf_pj('', cd_cgc_emitente, 'CEP'))=0 THEN  			obter_dados_pf_pj('', cd_cgc_emitente, 'CDM')  ELSE obter_municipio_ibge(obter_dados_pf_pj('', cd_cgc_emitente, 'CEP')) END )) MunicipioPrestacaoDescricao, 
	'A' Operacao,
	CASE WHEN nfse_obter_regra('TP',a.cd_estabelecimento)='1' THEN  'T' WHEN nfse_obter_regra('TP',a.cd_estabelecimento)='2' THEN  'T' WHEN nfse_obter_regra('TP',a.cd_estabelecimento)='3' THEN  'C' WHEN nfse_obter_regra('TP',a.cd_estabelecimento)='4' THEN  'F' WHEN nfse_obter_regra('TP',a.cd_estabelecimento)='T' THEN  'T' WHEN nfse_obter_regra('TP',a.cd_estabelecimento)='C' THEN 'C'  WHEN nfse_obter_regra('TP',a.cd_estabelecimento)='F' THEN 'F' WHEN nfse_obter_regra('TP',a.cd_estabelecimento)='G' THEN 'G' WHEN nfse_obter_regra('TP',a.cd_estabelecimento)='H' THEN 'H' WHEN nfse_obter_regra('TP',a.cd_estabelecimento)='M' THEN 'M'  ELSE 'K' END  Tributacao,
	...
	t.nr_sequencia NrSeqTransmissao,
