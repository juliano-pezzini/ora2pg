...
select	2					tp_registro,
	...
	CASE WHEN n.ie_situacao=1 THEN 'I' WHEN n.ie_situacao=3 THEN CASE WHEN n.ie_status_envio IS NULL THEN 'I'  ELSE 'C' END   ELSE 'C' END 	ie_situacao,
	CASE WHEN n.ie_situacao=1 THEN n.vl_mercadoria  ELSE 0 END  vl_servico,
	n.vl_descontos			vl_descontos,
	o.nr_codigo_serv_prest		cd_servico,
	0					vl_aliquota,
	to_char(CASE WHEN coalesce((	select	count(*)			from	nota_fiscal_trib a,				tributo c			where	a.cd_tributo 	 = c.cd_tributo			and	c.ie_tipo_tributo= 'ISS'			and 	a.nr_sequencia  = n.nr_sequencia),0)=0 THEN 2  ELSE 1 END ) ie_iss_retido,
	CASE WHEN coalesce((substr(obter_dados_pf_pj(null,n.cd_cgc,'P'),1,10))::numeric ,0)=0 THEN 			CASE WHEN n.cd_pessoa_fisica IS NULL THEN 2  ELSE 1 END  WHEN coalesce((substr(obter_dados_pf_pj(null,n.cd_cgc,'P'),1,10))::numeric ,0)=1 THEN 			CASE WHEN n.cd_pessoa_fisica IS NULL THEN 2  ELSE 1 END   ELSE 3 END  ie_cpf_cnpj,
	CASE WHEN coalesce((substr(obter_dados_pf_pj(null,n.cd_cgc,'P'),1,10))::numeric ,1)=1 THEN 		coalesce(substr(obter_cpf_pessoa_fisica(n.cd_pessoa_fisica),1,14),n.cd_cgc)  ELSE '00000000000000' END  cd_cpf_cnpj,
...
select	9					tp_registro,
	...
	''					ds_email,
	' '					ds_servicos,
	sum(CASE WHEN n.ie_situacao=1 THEN n.vl_mercadoria  ELSE 0 END )	vl_total_servicos,
