...
select
	...
	substr(b.cd_usuario_convenio,1,13)
					cd_usuario_convenio,
	CASE WHEN coalesce(obter_area_procedimento(c.cd_item, 1),0)=3 THEN  to_char(b.cd_cgc_hospital)  ELSE CASE WHEN c.ie_responsavel_credito='H' THEN  to_char(b.cd_cgc_hospital) WHEN c.ie_responsavel_credito='M' THEN  CASE WHEN c.cd_medico_executor IS NULL THEN  to_char(c.cd_cgc_prestador)  ELSE c.nr_cpf_executor END  WHEN c.ie_responsavel_credito='HA' THEN  CASE WHEN c.cd_medico_executor IS NULL THEN  to_char(c.cd_cgc_prestador)  ELSE c.nr_cpf_executor END  WHEN c.ie_responsavel_credito='H I' THEN  CASE WHEN c.cd_medico_executor IS NULL THEN  to_char(c.cd_cgc_prestador)  ELSE c.nr_cpf_executor END  WHEN c.ie_responsavel_credito='RI' THEN  CASE WHEN c.cd_medico_executor IS NULL THEN  to_char(c.cd_cgc_prestador)  ELSE c.nr_cpf_executor END   ELSE to_char(c.cd_prestador) END  END  cd_prestador,
	3				tp_prestador,
	LOCALTIMESTAMP				dt_referencia,
	c.nr_seq_conta_convenio		nr_interno_conta,
	0				vl_total_conta,
