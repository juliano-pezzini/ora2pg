...
SELECT     	'ITEM DA CONTA         ' 	DS_REGISTRO,
		...
		' '				UF_CRM_REQ,
           	...
           	substr(CASE WHEN c.cd_simpro IS NULL THEN ELIMINA_ACENTUACAO(upper(c.DS_ITEM))  ELSE obter_desc_material_simpro(c.cd_item) END ,1,50) NM_ITEM,
...
WHERE c.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA and w.nr_seq_protocolo = x.nr_seq_protocolo  and c.ie_tipo_item	= 1 and c.qt_item		<> 0 and coalesce(Z.ie_entra_conta,'S') = 'S' and c.cd_medico_executor is not null GROUP BY	somente_numero(coalesce(x.cd_interno,0)),
		c.NR_SEQ_PROTOCOLO,
           		c.NR_INTERNO_CONTA,
           		c.CD_ITEM_CONVENIO,
           		substr(CASE WHEN c.cd_simpro IS NULL THEN ELIMINA_ACENTUACAO(upper(c.DS_ITEM))  ELSE obter_desc_material_simpro(c.cd_item) END ,1,50),
           		CASE WHEN W.IE_CARATER_INTER='U' THEN 'S'  ELSE 'N' END ,
           		CASE WHEN c.pr_hora_extra=0 THEN 'N' WHEN c.pr_hora_extra=1 THEN 'N' WHEN c.pr_hora_extra IS NULL THEN 'N'  ELSE 'S' END ,
           		TRUNC(c.DT_ITEM,'dd'),
...
SELECT     	'ITEM DA CONTA         ' 	DS_REGISTRO,
		...
		' '				UF_CRM_REQ,
           	...
           	CASE WHEN c.pr_hora_extra=0 THEN 'N' WHEN c.pr_hora_extra=1 THEN 'N' WHEN c.pr_hora_extra IS NULL THEN 'N'  ELSE 'S' END 
                       			IE_EXTRA,
           	CASE WHEN ie_tipo_atendimento=1 THEN  TRUNC(DT_entrada,'dd')  ELSE TRUNC(c.DT_ITEM,'dd') END   		DT_ITEM,
           	c.CD_FUNCAO_EXECUTOR       	IE_FUNCAO_MEDICO,
           	SUM(c.QT_ITEM)             	QT_ITEM,
		...
		' '				ie_tipo_doc_paciente,
		' '				nr_doc_paciente,
		CASE WHEN c.cd_grupo_mat IS NULL THEN  ' '  ELSE CASE WHEN c.cd_grupo_mat=1 THEN  '2'  ELSE CASE WHEN c.cd_grupo_mat=7 THEN  '2'  ELSE '1' END  END  END  ie_matmed,
		0				nr_crm_resp,
		' '				uf_crm_resp,
		' '				nr_cpf_resp,
		substr(c.nr_doc_convenio,1,10)	cd_senha,
...
GROUP BY	somente_numero(coalesce(x.cd_interno,0)),
		c.NR_SEQ_PROTOCOLO,
           	...
           	CASE WHEN W.IE_CARATER_INTER='U' THEN 'S'  ELSE 'N' END ,
           	CASE WHEN c.pr_hora_extra=0 THEN 'N' WHEN c.pr_hora_extra=1 THEN 'N' WHEN c.pr_hora_extra IS NULL THEN 'N'  ELSE 'S' END ,
           	CASE WHEN ie_tipo_atendimento=1 THEN  TRUNC(DT_entrada,'dd')  ELSE TRUNC(c.DT_ITEM,'dd') END ,
           	c.CD_FUNCAO_EXECUTOR,
           	c.NR_SEQ_PROTOCOLO,
		CASE WHEN c.cd_grupo_mat IS NULL THEN  ' '  ELSE CASE WHEN c.cd_grupo_mat=1 THEN  '2'  ELSE CASE WHEN c.cd_grupo_mat=7 THEN  '2'  ELSE '1' END  END  END ,
		substr(c.nr_doc_convenio,1,10),
		c.cd_especialidade,
		w.ie_tipo_atendimento,
		coalesce(somente_numero(x.nr_protocolo),0),
