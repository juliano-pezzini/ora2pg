...
SELECT		'CABECALHO CONTA   '			DS_REGISTRO, 
		...
		SOMENTE_NUMERO(OBTER_TIPO_ACOMOD_ATEND(B.NR_ATENDIMENTO,'c')) 
							CD_ACOMODACAO, 
		...
		CASE WHEN substr(Obter_se_proced_cirurg_sas(B.nr_interno_conta, B.nr_seq_protocolo),1,1)='S' THEN 2  ELSE B.IE_TIPO_ATENDIMENTO END  
							IE_TIPO_ATENDIMENTO, 
		substr(obter_mascara_cid(B.CD_CID_PRINCIPAL),1,6) 
							CD_CID, 
		substr(Obter_Codigo_Item_Convenio(B.cd_convenio, 1, B.CD_PROC_PRINCIPAL, 1, 'S', 0, 
						B.cd_medico_resp, B.ie_tipo_atendimento),1,8)  
							CD_PROC_PRINCIPAL, 
...
SELECT   	'ITEM DA CONTA     '	 	DS_REGISTRO, 
	    ...
	    CASE WHEN W.IE_CARATER_INTER='U' THEN 'S'  ELSE 'N' END 	IE_EMERGENCIA, 
      	...
      	TRUNC(W.DT_ENTRADA,'dd') 		DT_ITEM, 
    		TO_CHAR(coalesce(C.CD_FUNCAO_EXECUTOR,0)) 	IE_FUNCAO_MEDICO, 
...
WHERE C.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA  /*and		nvl(Z.ie_entra_conta,'S') = 'S'*/
  and C.ie_tipo_item	= 1 and C.cd_medico_executor is not null GROUP BY	C.NR_SEQ_PROTOCOLO, 
      	C.NR_INTERNO_CONTA, 
      	C.CD_ITEM_CONVENIO, 
      	substr(ELIMINA_ACENTUACAO(upper(C.DS_ITEM)),1,50), 
...
and (C.ie_tipo_item	= 2 or (C.ie_tipo_item	= 1 and C.cd_medico_executor is null)) 
...
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA    /*and		nvl(Z.ie_entra_conta,'S') = 'S'*/
...
LEFT OUTER JOIN estrutura_procedimento_v x ON (E.CD_ITEM = X.CD_PROCEDIMENTO AND E.IE_ORIGEM_PROCED = X.IE_ORIGEM_PROCED)
LEFT OUTER JOIN regra_honorario z ON (E.IE_RESPONSAVEL_CREDITO = Z.cd_regra)
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA and B.nr_seq_protocolo	= f.nr_seq_protocolo    /*and		nvl(Z.ie_entra_conta,'S') = 'S'*/
