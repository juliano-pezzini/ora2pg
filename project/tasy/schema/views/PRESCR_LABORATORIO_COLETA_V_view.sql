...
WHERE A.CD_MOTIVO_BAIXA = 0 and ((A.dt_integracao_coleta 	is null) or (A.dt_integracao_suspenso 	is null)) and A.cd_procedimento		= p.cd_procedimento and A.ie_origem_proced		= p.ie_origem_proced and coalesce(dt_liberacao_medico, dt_liberacao) is not null AND A.NR_PRESCRICAO 		= B.NR_PRESCRICAO AND B.CD_PESSOA_FISICA 		= C.CD_PESSOA_FISICA   AND B.CD_MEDICO 			= D.CD_PESSOA_FISICA    AND G.NR_SEQ_INTERNO		= obter_atepacu_paciente(B.nr_atendimento,'A') AND G.CD_SETOR_ATENDIMENTO		= s.CD_SETOR_ATENDIMENTO and coalesce(A.IE_AMOSTRA,'N')		= 'S' AND A.CD_SETOR_ATENDIMENTO in (SELECT s.CD_SETOR_ATENDIMENTO 
	FROM SETOR_ATENDIMENTO S 
	WHERE s.NM_USUARIO_BANCO IN (SELECT USER )) AND F.NR_ATENDIMENTO 		= B.NR_ATENDIMENTO AND T.NR_ATENDIMENTO 		= B.NR_ATENDIMENTO AND F.DT_INICIO_VIGENCIA 		= 
