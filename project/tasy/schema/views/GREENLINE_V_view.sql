...
CREATE OR REPLACE VIEW greenline_v (ds_registro, tp_registro, ie_servico, cd_registro, cd_origem, nr_lote, nr_interno_conta, nr_parcela, cd_convenio, nm_convenio, tp_prestador, cd_prestador, nm_prestador, dt_geracao, nr_remessa, cd_plano, cd_paciente, cd_autorizacao, cd_acomodacao, dt_entrada, dt_saida, dt_inicio_cobranca, dt_fim_cobranca, ie_tipo_atendimento, cd_cid, cd_proc_principal, cd_medico_convenio, nr_crm, uf_crm, nr_crm_req, uf_crm_req, nr_seq_item, cd_item, nm_item, ie_emergencia, ie_extra, dt_item, ie_funcao_medico, qt_item, vl_item, ie_cirurgia_multipla, vl_total_item, qt_item_conta, nm_paciente, nr_fatura, qt_conta, vl_total_lote, nr_seq_protocolo, ds_espaco, ie_tipo_guia, nr_registro, ds_ma_me_tx, ie_tipo_atend_tasy, cd_especialidade) AS SELECT	'HEADER' 			DS_REGISTRO,
	1 				TP_REGISTRO,
	'CSMH' 			IE_SERVICO,
	0 				CD_REGISTRO,
	(coalesce(A.CD_INTERNO,0))::numeric  	CD_ORIGEM,
	0 				NR_LOTE,
	A.NR_SEQ_PROTOCOLO 		NR_INTERNO_CONTA,
	0 				NR_PARCELA,
...
SELECT	'CABECALHO CONTA' 		DS_REGISTRO,
	...
	substr(somente_numero(B.CD_USUARIO_CONVENIO),1,18) CD_PACIENTE,
...
SELECT	'ITEM DA CONTA' 		DS_REGISTRO,
	...
	' ' 				DS_ESPACO,
	CASE WHEN W.ie_tipo_atendimento=1 THEN 'GIH' WHEN W.ie_tipo_atendimento=3 THEN 'GCMM' WHEN W.ie_tipo_atendimento=7 THEN 'GEPM' WHEN W.ie_tipo_atendimento=8 THEN 'GAEP'  ELSE '' END  IE_TIPO_GUIA,
	1 				NR_REGISTRO,
	''				DS_MA_ME_TX,
...
WHERE C.NR_INTERNO_CONTA		= W.NR_INTERNO_CONTA  and coalesce(Z.ie_entra_conta,'S')  	= 'S' and C.ie_tipo_item             	= 1 and C.qt_item                  	<> 0 and C.cd_medico_executor is not null GROUP BY C.NR_SEQ_PROTOCOLO,
	 C.NR_INTERNO_CONTA,
	 C.CD_ITEM_CONVENIO,
	 ...
	 C.NR_SEQ_PROTOCOLO,
	 CASE WHEN W.ie_tipo_atendimento=1 THEN 'GIH' WHEN W.ie_tipo_atendimento=3 THEN 'GCMM' WHEN W.ie_tipo_atendimento=7 THEN 'GEPM' WHEN W.ie_tipo_atendimento=8 THEN 'GAEP'  ELSE '' END ,
...
SELECT	'ITEM DA CONTA' 		DS_REGISTRO,
	...
	CASE WHEN W.ie_tipo_atendimento=1 THEN 'GIH' WHEN W.ie_tipo_atendimento=3 THEN 'GCMM' WHEN W.ie_tipo_atendimento=7 THEN 'GEPM' WHEN W.ie_tipo_atendimento=8 THEN 'GAEP'  ELSE '' END  IE_TIPO_GUIA,
	1 				NR_REGISTRO,
	CASE WHEN C.ie_total_interf=2 THEN '4' WHEN C.ie_total_interf=3 THEN '5' WHEN C.ie_total_interf=7 THEN '2'  ELSE '3' END  DS_MA_ME_TX,
	W.ie_tipo_atendimento 	ie_tipo_atend_tasy,
	0 				CD_ESPECIALIDADE
...
GROUP BY C.NR_SEQ_PROTOCOLO,
	 ...
	 C.NR_SEQ_PROTOCOLO,
	 CASE WHEN W.ie_tipo_atendimento=1 THEN 'GIH' WHEN W.ie_tipo_atendimento=3 THEN 'GCMM' WHEN W.ie_tipo_atendimento=7 THEN 'GEPM' WHEN W.ie_tipo_atendimento=8 THEN 'GAEP'  ELSE '' END ,
	 CASE WHEN C.ie_total_interf=2 THEN '4' WHEN C.ie_total_interf=3 THEN '5' WHEN C.ie_total_interf=7 THEN '2'  ELSE '3' END ,
	 W.ie_tipo_atendimento
having	 SUM(C.QT_ITEM) <> 0
...
SELECT	'TOTAL DA CONTA' 		DS_REGISTRO,
	...
	0 				QT_ITEM_CONTA,
	substr(ELIMINA_ACENTUACAO(UPPER(B.NM_PACIENTE)),1,40) NM_PACIENTE,
	' ' 				NR_FATURA,
	0 				QT_CONTA,
	0 				VL_TOTAL_LOTE,
	B.NR_SEQ_PROTOCOLO 		NR_SEQ_PROTOCOLO,
