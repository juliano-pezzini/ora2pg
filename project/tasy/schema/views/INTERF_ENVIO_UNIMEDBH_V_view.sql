...
CREATE OR REPLACE VIEW interf_envio_unimedbh_v (nr_seq_protocolo, cd_convenio, tp_registro, ie_disquete, cd_tipo_registro, nr_lote, cd_prestador, nr_seq_conta, ie_tipo_prestador, cd_cgc, nm_prestador, dt_remessa, nr_seq_remessa, nr_reg_remessa, nm_paciente, ie_sexo, dt_nascimento, cd_cliente, nr_conta, ie_tipo_paciente, cd_tipo_atendimento, cd_classe_atendimento, dt_item, cd_cid, nr_crm, sg_uf_crm, nm_medico_requisitante, cd_medico_requisitante, dt_requisicao, nr_guia, dt_guia, nm_responsavel_guia, cd_proced_guia, ds_proced_guia, qt_proced_guia, nr_seqproc_guia, nr_seq_item, ds_item, cd_item, qt_item, qt_gus, cd_cgc_convenio, nr_prontuario, cd_plano, cd_dependente, dt_validade_carteira, nr_identidade, cd_tipo_acomodacao, dt_entrada, dt_alta, cd_motivo_alta, tp_guia, cd_medico_convenio, ie_funcao_medico, qt_porte_anestesico, cd_grupo_gasto, ds_unidade_medida, qt_ch, vl_unitario_ch, vl_unitario, vl_item, pr_funcao_partic, ie_via_acesso, pr_via_acesso, pr_hora_extra, ie_local_medicado, cd_brasindice, cd_item_convenio, cd_procedimento_ref, cd_senha, cd_regional, nm_convenio, ds_espaco, ie_estado_civil, nr_cpf_paciente, cd_nacionalidade, cd_cid_secundario, nr_planilha, vl_tot_honorarios, vl_tot_diarias, vl_tot_taxas, vl_tot_medicamentos, vl_tot_materiais, vl_tot_sadt, vl_tot_pacote, vl_tot_ortese, vl_tot_protese, qt_tot_diarias, qt_tot_participantes, cd_proc_orig_pacote, cd_cgc_cpf_resp_cred, nr_doc_convenio, nr_crm_requisitante, ie_tipo_protocolo, ie_tipo_atendimento, cd_grupo_proc, cd_setor_atendimento, ie_clinica, dt_periodo_inicial, dt_periodo_final, cd_interno, nr_seq_indicacao) AS SELECT	
		...
		'' NR_GUIA,LOCALTIMESTAMP DT_GUIA, 
		'' NM_RESPONSAVEL_GUIA, 
		'' CD_PROCED_GUIA, 
		'' DS_PROCED_GUIA, 
		0 QT_PROCED_GUIA, 
...
SELECT   	a.NR_SEQ_PROTOCOLO,a.CD_CONVENIO CD_CONVENIO,2 TP_REGISTRO, 
		...
		a.cd_classe_atendimento		CD_CLASSE_ATENDIMENTO, 
      	LOCALTIMESTAMP         	DT_ITEM, 
      	a.cd_cid_principal      CD_CID, 
     	CASE WHEN a.ie_medico_credenciado='N' THEN a.nr_crm_medico_resp||calcula_digito_unimed_bh( 			'Medico_Unimed',to_char(somente_numero(a.nr_crm_medico_resp)))  ELSE 0 END  NR_CRM, 
      	a.uf_crm_medico_resp     SG_UF_CRM, 
      	upper(a.nm_medico_resp)  		NM_MEDICO_REQUISITANTE, 
		CASE WHEN a.ie_medico_credenciado='S' THEN a.cd_conv_medico_resp  ELSE 0 END 	CD_MEDICO_REQUISITANTE, 
      	coalesce(c.dt_autorizacao,a.dt_entrada) DT_REQUISICAO, 
		...
		a.DT_PERIODO_FINAL, 
		coalesce(a.cd_interno, '00644006002') cd_interno, 
		somente_numero(substr(obter_tipo_indicacao(a.nr_atendimento,'C'),1,10)) nr_seq_indicacao 
...
SELECT	a.NR_SEQ_PROTOCOLO,a.CD_CONVENIO CD_CONVENIO,4 TP_REGISTRO, 
		...
		coalesce(CASE WHEN a.cd_medico_executor IS NULL THEN a.cd_interno  ELSE CASE WHEN substr(a.cd_medico_exec_conv,1,5)='00644' THEN null  ELSE a.cd_medico_exec_conv END  END , '00615006014') cd_prestador, 
      	0               NR_SEQ_CONTA, 
      	''              IE_TIPO_PRESTADOR, 
      	a.cd_cgc_hospital       CD_CGC, 
      	''              NM_PRESTADOR, 
		...
		CASE WHEN coalesce(CASE WHEN a.cd_medico_executor IS NULL THEN a.cd_interno  ELSE CASE WHEN substr(a.cd_medico_exec_conv,1,5)='00644' THEN null  ELSE a.cd_medico_exec_conv END  END , '00615006014')='00615006014' THEN  '3'  ELSE a.cd_grupo_gasto END 	CD_GRUPO_GASTO, 
		...
		coalesce(a.pr_via_acesso,0) IE_VIA_ACESSO, 
...
group by 
		...
		b.DT_PERIODO_INICIAL, 
		b.DT_PERIODO_FINAL, 
		coalesce(CASE WHEN a.cd_medico_executor IS NULL THEN a.cd_interno  ELSE CASE WHEN substr(a.cd_medico_exec_conv,1,5)='00644' THEN null  ELSE a.cd_medico_exec_conv END  END , '00615006014'), 
      	a.cd_cgc_hospital, 
      	a.nr_remessa, 
      	a.nr_interno_conta, 
      	upper(a.ds_item), 
		...
		CASE WHEN coalesce(CASE WHEN a.cd_medico_executor IS NULL THEN a.cd_interno  ELSE CASE WHEN substr(a.cd_medico_exec_conv,1,5)='00644' THEN null  ELSE a.cd_medico_exec_conv END  END , '00615006014')='00615006014' THEN  '3'  ELSE a.cd_grupo_gasto END , 
		a.cd_UNIDADE_MEDIDA, 
		a.qt_ch_item, 
		a.vl_unitario_ch, 
		a.pr_funcao_participante, 
		coalesce(a.pr_via_acesso,0), 
...
SELECT   	a.NR_SEQ_PROTOCOLO,a.CD_CONVENIO CD_CONVENIO,5 TP_REGISTRO, 
		...
		sum(CASE WHEN b.cd_grupo_gasto='2' THEN 0  ELSE b.vl_honorario END ) vl_tot_honorarios, 
		...
		sum(CASE WHEN b.cd_grupo_gasto='2' THEN b.vl_total_item  ELSE 0 END ) vl_tot_sadt, 
...
SELECT   	a.NR_SEQ_PROTOCOLO,a.CD_CONVENIO CD_CONVENIO,9 TP_REGISTRO, 
      	...
      	'' CD_CLIENTE, 99999999	NR_CONTA, '' IE_TIPO_PACIENTE, 
