...
SELECT		'CABECALHO CONTA      '		DS_REGISTRO,
		...
		coalesce(B.DT_ALTA,B.DT_PERIODO_FINAL)
						DT_FIM_COBRANCA,
		obter_conversao_externa(B.cd_cgc_convenio, 'ABRAMGE_II_V', 'IE_TIPO_ATENDIMENTO', coalesce(B.IE_CLINICA,0))	IE_TIPO_ATENDIMENTO,
		...
		CASE WHEN B.cd_senha_guia=B.nr_doc_convenio THEN  null  ELSE B.cd_senha_guia END  cd_senha_atend,
...
SELECT     	'ITEM DA CONTA         ' 	DS_REGISTRO,
           	...
           	LOCALTIMESTAMP                 	DT_INICIO_COBRANCA,
           	LOCALTIMESTAMP                 	DT_FIM_COBRANCA,
           	obter_conversao_externa(w.cd_cgc_convenio, 'ABRAMGE_II_V', 'IE_TIPO_ATENDIMENTO', 0) IE_TIPO_ATENDIMENTO,
           	null                     	CD_CID,
           	0                        	CD_PROC_PRINCIPAL,
		' '				CD_MEDICO_CONVENIO,
           	' '                       	NR_CRM,
		...
		CASE WHEN w.cd_senha_guia=w.nr_doc_convenio THEN  null  ELSE w.cd_senha_guia END  cd_senha_atend,
		0				tp_internacao
...
WHERE c.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA and w.nr_seq_protocolo = x.nr_seq_protocolo  and c.ie_tipo_item	= 1 and c.qt_item		<> 0 AND c.cd_item in (select x.cd_procedimento
		from estrutura_procedimento_v x
		where x.cd_area_procedimento in (1,4)
		
union

		select x.cd_procedimento
		from estrutura_procedimento_v x
		where x.cd_area_procedimento in (11,13)
		and   c.nr_seq_proc_pacote is not null) GROUP BY	somente_numero(coalesce(x.cd_interno,0)),
		c.NR_SEQ_PROTOCOLO,
		obter_conversao_externa(w.cd_cgc_convenio, 'ABRAMGE_II_V', 'IE_TIPO_ATENDIMENTO', 0),
           		c.NR_INTERNO_CONTA,
           		c.CD_ITEM_CONVENIO,
           		substr(ELIMINA_ACENTUACAO(upper(c.DS_ITEM)),1,50),
           		CASE WHEN W.IE_CARATER_INTER='U' THEN 'S'  ELSE 'N' END ,
		...
		coalesce(somente_numero(x.nr_protocolo),0),
		coalesce(somente_numero(c.nr_doc_convenio),0),
		CASE WHEN w.cd_senha_guia=w.nr_doc_convenio THEN  null  ELSE w.cd_senha_guia END ,
		w.cd_senha_guia
...
SELECT     	'ITEM DA CONTA         ' 	DS_REGISTRO,
           	...
           	obter_conversao_externa(w.cd_cgc_convenio, 'ABRAMGE_II_V', 'IE_TIPO_ATENDIMENTO', 0) IE_TIPO_ATENDIMENTO,
           	null                     	CD_CID,
        	CASE WHEN c.nr_seq_item=c.nr_seq_proc_pacote THEN  c.cd_item  ELSE 0 END  CD_PROC_PRINCIPAL,
		' '				CD_MEDICO_CONVENIO,
           	' '                       	NR_CRM,
           	' '                       	UF_CRM,
		' '				NR_CRM_REQ,
		' '				UF_CRM_REQ,
           	...
           	CASE WHEN c.pr_hora_extra=0 THEN 'N' WHEN c.pr_hora_extra=1 THEN 'N' WHEN c.pr_hora_extra IS NULL THEN 'N'  ELSE 'S' END 
                       				IE_EXTRA,
           	TRUNC(W.DT_ENTRADA,'dd')  		DT_ITEM,
           	coalesce(c.CD_FUNCAO_EXECUTOR,0)        IE_FUNCAO_MEDICO,
           	SUM(c.QT_ITEM)               	QT_ITEM,
           	sum(CASE WHEN c.nr_seq_item=c.nr_seq_proc_pacote THEN  coalesce(c.vl_honorario,0) + coalesce(c.vl_custo_oper,0)  ELSE c.vl_total_item END ) VL_ITEM,
           	0                        	IE_CIRURGIA_MULTIPLA,
           	0		      		VL_TOTAL_ITEM,
           	0	      			QT_ITEM_CONTA,
	     	' '				NM_PACIENTE,
		...
		substr(c.nr_doc_convenio,1,10)	cd_senha,
		c.cd_especialidade		cd_especialidade,
		sum(CASE WHEN c.nr_seq_item=c.nr_seq_proc_pacote THEN  coalesce(c.vl_honorario,0) + coalesce(c.vl_custo_oper,0)  ELSE 0 END ) vl_proc_principal,
		0				ie_tipo_cirurgia,
		0				cd_unidade_atend,
		0				qt_diaria_enfermaria,
		0				qt_diaria_uti,
...
and (c.ie_tipo_item	= 2 or (c.ie_tipo_item	= 1 AND	c.cd_item not in (select x.cd_procedimento
		from estrutura_procedimento_v x
		where x.cd_area_procedimento in (1,4)
...
union
		...
		from estrutura_procedimento_v x
		where x.cd_area_procedimento in (11,13)
		and   c.nr_seq_proc_pacote is not null)))
and		c.qt_item		<> 0
GROUP BY	somente_numero(coalesce(x.cd_interno,0)),
		c.NR_SEQ_PROTOCOLO,
		obter_conversao_externa(w.cd_cgc_convenio, 'ABRAMGE_II_V', 'IE_TIPO_ATENDIMENTO', 0),
		CASE WHEN c.nr_seq_item=c.nr_seq_proc_pacote THEN  c.cd_item  ELSE 0 END ,
           	c.NR_INTERNO_CONTA,
           	c.CD_ITEM_CONVENIO,
           	substr(ELIMINA_ACENTUACAO(upper(c.DS_ITEM)),1,50),
           	CASE WHEN W.IE_CARATER_INTER='U' THEN 'S'  ELSE 'N' END ,
...
SELECT     	'TOTAL DA CONTA        ' 	DS_REGISTRO,
		...
		B.UF_CRM_MEDICO_RESP		UF_CRM_REQ,
           	...
           	0				VL_ITEM,
           	0				IE_CIRURGIA_MULTIPLA,
           	sum(CASE WHEN e.cd_area_proc=1 THEN (coalesce(e.vl_honorario,0) + coalesce(e.vl_custo_oper,0)) WHEN e.cd_area_proc=4 THEN (coalesce(e.vl_honorario,0) + coalesce(e.vl_custo_oper,0)) WHEN e.cd_area_proc=11 THEN (coalesce(e.vl_honorario,0) + coalesce(e.vl_custo_oper,0)) WHEN e.cd_area_proc=13 THEN (coalesce(e.vl_honorario,0) + coalesce(e.vl_custo_oper,0))  ELSE e.vl_total_item END ) +
			sum(CASE WHEN e.nr_seq_item=e.nr_seq_proc_pacote THEN  coalesce(e.vl_honorario,0) + coalesce(e.vl_custo_oper,0)  ELSE 0 END ) VL_TOTAL_ITEM,
           	0			QT_ITEM_CONTA,
...
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA and B.nr_seq_protocolo	= y.nr_seq_protocolo  GROUP BY	somente_numero(coalesce(y.cd_interno,0)),
		B.NR_SEQ_PROTOCOLO,
           	B.NR_INTERNO_CONTA,
		substr(ELIMINA_ACENTUACAO(UPPER(B.NM_PACIENTE)),1,40),
		...
		coalesce(somente_numero(y.nr_protocolo),0),
		coalesce(somente_numero(B.nr_doc_convenio),0),
		CASE WHEN B.cd_senha_guia=B.nr_doc_convenio THEN  null  ELSE B.cd_senha_guia END ,
		B.cd_senha_guia
