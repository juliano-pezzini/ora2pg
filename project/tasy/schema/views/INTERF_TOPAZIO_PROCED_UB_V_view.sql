...
CREATE OR REPLACE VIEW interf_topazio_proced_ub_v (nr_seq_protocolo, cd_convenio, tp_registro, ie_disquete, cd_tipo_registro, cd_federacao, nr_lote, cd_prestador, nr_seq_conta, nm_singular, ie_prestador, cd_cgc_hospital, nm_prestador, dt_remessa, nr_remessa, ds_espaco, nr_registro, nm_paciente, ie_sexo, dt_nascimento, nr_prontuario, cd_plano, cd_usuario, nm_convenio, qt_zeros, ie_tipo_nota, nr_nota, nr_conta_ambulat, tp_conta_origem, nr_conta_origem, cd_natureza, dt_competencia, ie_tipo_paciente, cd_tipo_atendimento, cd_classe_atend, cd_tipo_acomodacao, dt_entrada, hr_entrada, dt_saida, hr_saida, cd_motivo_alta, cd_cid, nr_crm, ds_uf_crm, nm_medico_resp, nr_crm_req, ds_uf_req, nm_medico_req, cd_medico_req, dt_req_assinada, cd_tipo_nasc, qt_eventos_nasc, qt_eventos_obito, nr_seq_autorizacao, cd_proc_autorizado, ds_proc_autorizado, cd_acomod_autorizado, qt_proc_autorizado, nr_guia, dt_guia, ds_senha, nm_autorizador, nr_seq_item_conta, cd_prest_exec, dt_procedimento, hr_procedimento, cd_funcao_executor, nr_porte_anestesico, ds_item, nr_crm_req_exa, nr_guia_exame, cd_prest_cobranca, cd_item, cd_unidade_medida, qt_item, qt_ch, vl_ch, vl_unitario, vl_total_item, ds_local_uso, cd_proc_referencia, cd_via_acesso, ie_tipo_item, qt_diarias_normais, qt_diarias_uti, qt_participantes, vl_honorarios, vl_diarias_normais, vl_diarias_uti, vl_tot_servicos, vl_tot_medicamentos, vl_tot_materiais, vl_sadt, vl_filmes, vl_tot_conta, nr_fatura, qt_conta_lote, vl_total_lote, qt_conta_remessa, vl_total_remessa, nr_interno_conta, dt_validade_carteira, nr_cns, qt_nasc_prematuro, qt_obito_nasc_precoce, qt_obito_nasc_tardio, ie_motivo_obito_mulher, ie_regime_internacao, tp_internacao, ie_motivo_saida_internacao, id_int_obstetricia_1, id_int_obstetricia_2, id_int_obstetricia_3, id_int_obstetricia_4, id_int_obstetricia_5, id_int_obstetricia_6, id_int_obstetricia_7, id_int_obstetricia_8, id_int_obstetricia_9, tp_doenca, tempo_doenca, ie_periodo_doenca, indicacao_acidente, cd_cid_diagnostico_2, cd_cid_diagnostico_3, cd_cid_diagnostico_4, tp_consulta, tp_saida_consulta_sadt, cd_cid_obito, nr_declaracao_obito, hr_fim_realiz_proc, fator_participacao, vl_total_gases_med, tp_faturamento, cd_proc_principal, ie_ordem_exames, nr_conta_ambulat2) AS SELECT 	distinct
	...
	somente_numero(coalesce(obter_guia_prescricao(coalesce(K.nr_prescricao,0)), 
			coalesce(somente_numero(K.nr_doc_convenio),0))) nr_nota, 
 	somente_numero(coalesce(K.nr_prescricao,0)) nr_conta_ambulat,	 
 	CASE WHEN d.ie_tipo_guia='I' THEN 'I'  ELSE 'E' END  tp_conta_origem, 
 	somente_numero(d.cd_autorizacao) 	nr_conta_origem, 
 	0 				cd_natureza, 
	...
	1				ie_ordem_exames, 
	somente_numero(coalesce(obter_guia_prescricao(coalesce(K.nr_prescricao,0)), 
			coalesce(somente_numero(K.nr_doc_convenio),0))) nr_conta_ambulat2 
FROM 	convenio 		c, 
...
SELECT 	distinct 
	...
	CASE WHEN b.dt_alta IS NULL THEN '16'  ELSE coalesce(substr(obter_conversao_externa(b.cd_cgc_convenio,'INTERF_ENVIO_TOPAZIO_UB_V','CD_MOTIVO_ALTA',coalesce(b.cd_motivo_alta,0)),1,30),'16') END  
					cd_motivo_alta, 
	...
	0				cd_proc_principal, 
	1				ie_ordem_exames, 
	somente_numero(coalesce(K.nr_prescricao,0)) nr_conta_ambulat2 
...
SELECT 
	...
	'0' 				ie_tipo_nota, 
	coalesce(somente_numero(b.nr_doc_convenio),0) NR_NOTA, 
	somente_numero(coalesce(b.nr_prescricao,0)) nr_conta_ambulat, 
	CASE WHEN d.ie_tipo_guia='I' THEN 'I'  ELSE 'E' END  tp_conta_origem, 
	somente_numero(d.cd_autorizacao) 	nr_conta_origem, 
	0 				cd_natureza, 
	LOCALTIMESTAMP 			dt_competencia, 
	...
	' ' 				nm_autorizador, 
	0 				nr_seq_item_conta, 
	substr(obter_cod_prestador(a.cd_interno,b.cd_setor_atendimento),1,12) 
					cd_prest_exec, 
	b.dt_item 			dt_procedimento, 
	b.dt_item 			hr_procedimento, 
	CASE WHEN b.ie_tipo_item=2 THEN ' '  ELSE substr(obter_conversao_externa(c.cd_cgc_convenio,'INTERF_ENVIO_TOPAZIO_UB_V','CD_FUNCAO_EXECUTOR', to_char(b.cd_funcao_executor)),1,1) END  
					cd_funcao_executor, 
	...
	coalesce(Somente_Numero(coalesce(SUBSTR(b.nr_crm_requisitante,1,5),b.nr_crm_executor)),0) 
					nr_crm_req_exa, 
	CASE WHEN Obter_se_setor_proprio(b.cd_setor_atendimento)='N' THEN coalesce(somente_numero(b.nr_doc_convenio),0)  ELSE 0 END  
			 		nr_guia_exame, 
	/*nvl(trim(SUBSTR(b.cd_medico_exec_conv,1,12)),' ')*/
 
	CASE WHEN Obter_se_setor_proprio(b.cd_setor_atendimento)='N' THEN substr(obter_cod_prestador(a.cd_interno,b.cd_setor_atendimento),1,12)  ELSE ' ' END  
					cd_prest_cobranca, 
	...
	/*SUM(vl_total_item) vl_total_item,*/
	 ...
	 ' ' 				ds_local_uso, 
	somente_numero(substr(Obter_Proc_Ref(b.nr_seq_ITEM, 'C'),1,30)) 
					cd_proc_referencia, 
	coalesce(substr(obter_conversao_externa(c.cd_cgc_convenio,'INTERF_ENVIO_TOPAZIO_UB_V','CD_VIA_ACESSO',coalesce(b.ie_via_acesso_inf,0)),1,1),'0') 
					cd_via_acesso, 
	...
	0				cd_proc_principal, 
	1				ie_ordem_exames, 
	somente_numero(coalesce(b.nr_prescricao,0)) nr_conta_ambulat2 
...
WHERE c.nr_interno_conta 	= 	d.nr_interno_conta and b.nr_seq_protocolo 	= 	a.nr_seq_protocolo AND b.nr_seq_protocolo 	= 	c.nr_seq_protocolo AND b.nr_interno_conta 	= 	c.nr_interno_conta and (b.ie_total_interf 	= 	4) AND coalesce(b.nr_seq_proc_pacote,b.nr_seq_item) = b.nr_seq_item  --AND 	NVL(z.ie_entra_conta,'S') = 	'S' 
  AND b.qt_item  		<> 	0 AND coalesce(b.ie_tipo_item,0) 	<> 2 AND b.cd_item NOT IN (SELECT x.cd_procedimento 
	...
	somente_numero(coalesce(b.nr_prescricao,0)), 
	somente_numero(coalesce(b.nr_prescricao,0)), 
 	CASE WHEN d.ie_tipo_guia='I' THEN 'I'  ELSE 'E' END , 
 	somente_numero(d.cd_autorizacao), 
 	b.dt_item, 
 	b.dt_item, 
 	CASE WHEN b.ie_tipo_item=2 THEN ' '  ELSE substr(obter_conversao_externa(c.cd_cgc_convenio,'INTERF_ENVIO_TOPAZIO_UB_V','CD_FUNCAO_EXECUTOR', to_char(b.cd_funcao_executor)),1,1) END , 
...
SELECT 	distinct 
	...
	0 					qt_zeros, 
	'0' 					ie_tipo_nota, 
	somente_numero(coalesce(K.nr_prescricao,0))	NR_NOTA, 
	somente_numero(coalesce(K.nr_prescricao,0))	nr_conta_ambulat, 
	' ' 					tp_conta_origem, 
	0 					nr_conta_origem, 
	0 					cd_natureza, 
