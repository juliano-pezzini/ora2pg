...
--	 decode(nvl(e.tx_conversao_qtde,0),0,1,e.tx_conversao_qtde)) vl_unitario_convenio,
	...
	substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'CLAB'),1,6) cd_lab_brasindice,
	substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'DLAB'),1,40) ds_lab_brasindice,
	CASE WHEN obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'CMED') IS NULL THEN 'N'  ELSE 'S' END  ie_brasindice,
	substr(Obter_Dados_Material_Simpro(a.nr_seq_mat_simp, 'CF'),1,10) ds_fabricante,
	a.cd_cgc_fornecedor,
	...
	CASE WHEN a.nr_seq_mat_simp IS NULL THEN  substr(substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'DLAB'),1,6),1,40)  ELSE substr(Obter_Dados_Material_Simpro(a.nr_seq_mat_simp, 'CF'),1,10) END  ds_fabric_bras_simpro,
	CASE WHEN a.nr_seq_mat_simp IS NULL THEN  substr(substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'DLAB'),1,6),1,40)  ELSE substr(Obter_Dados_Material_Simpro(a.nr_seq_mat_simp, 'CF'),1,10) END  ds_fabric_bras_simpro_data,
	substr(coalesce(obter_dados_lote_fornec(a.nr_seq_lote_fornec,'DF'),
		   coalesce( obter_nome_pf_pj(null,a.cd_cgc_fornecedor),
			coalesce(substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'DLAB'),1,6),
			    Obter_Dados_Material_Simpro(a.nr_seq_mat_simp, 'CF')))),1,80) ds_fornec_fabric_bras_simpro,
	substr(substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'DMED'),1,6),1,100) ds_material_bras,
	...
	substr(coalesce(CASE WHEN a.nr_seq_mat_simp IS NULL THEN  substr(substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'DLAB'),1,6),1,40)  ELSE substr(Obter_Dados_Material_Simpro(a.nr_seq_mat_simp, 'CF'),1,10) END ,
	...
	CASE WHEN Obter_Codigo_Simpro2(a.cd_material,a.cd_convenio,c.cd_estabelecimento,a.dt_atendimento,a.nr_seq_conv_simp)=0 THEN  coalesce(substr(Obter_medic_Brasindice(coalesce(d.cd_estabelecimento,1),			a.cd_material,a.dt_atendimento),1,20),' ')  ELSE to_char(Obter_Codigo_Simpro2(a.cd_material,a.cd_convenio,c.cd_estabelecimento,a.dt_atendimento,a.nr_seq_conv_simp)) END  cd_simpro_brasindice,
	e.cd_grupo cd_grupo_convenio,
	a.cd_cgc_prestador,
	substr(obter_razao_social(a.cd_cgc_prestador),1,200) ds_cgc_prestador,	
	substr(Obter_Prestador_Convenio(a.cd_cgc_prestador,a.cd_convenio, a.cd_categoria,null),1,30) cd_prestador_convenio,	
	...
	substr(coalesce(OBTER_DESC_MAT_FABRICANTE(b.cd_material), 
		CASE WHEN a.nr_seq_mat_simp IS NULL THEN  substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'DLAB'),1,6)  ELSE Obter_Dados_Material_Simpro(a.nr_seq_mat_simp, 'CF') END ),1,150) ds_fabric_cad_mat_bras_simpro,
	a.cd_material_tiss,
	...
	substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'DMED'),1,100)  || '  ' ||
				substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'DAPR'),1,98)	ds_mat_bras_apresent,
	a.nr_seq_proc_princ,
	...
	substr(CASE WHEN d.ie_status_acerto=2 THEN 		CASE WHEN obter_estrutura_material(a.cd_material,'G')=15 THEN obter_nf_conta(d.nr_interno_conta,2) END  END ,1,20) nr_nota_fiscal_opme,
	...
	CASE WHEN a.ie_origem_preco=2 THEN  coalesce(substr(Obter_Desc_Tabela_Mat(a.cd_tab_preco_material,c.cd_estabelecimento),1,30),'Não informado')  ELSE substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'DLAB'),1,40) END  ds_lab_bras_aherj,
	...
	substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'CAPR'),1,6) cd_apr_brasindice,
	substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'DMED'),1,100) ds_material_brasindice,
	...
	CASE WHEN coalesce(Obter_Dados_Brasindice2(d.cd_estabelecimento,a.cd_material, a.dt_atendimento, a.cd_convenio, 'CMED', a.cd_categoria,		obter_tipo_atendimento(a.nr_atendimento), (Obter_dados_lote_fornec(a.nr_seq_lote_fornec,'SMA'))::numeric ),0)=0 THEN  		Obter_Codigo_Simpro2(a.cd_material,a.cd_convenio,c.cd_estabelecimento,a.dt_atendimento,a.nr_seq_conv_simp)  ELSE substr(Obter_Dados_Brasindice2(d.cd_estabelecimento,a.cd_material, a.dt_atendimento, a.cd_convenio,'CMED', a.cd_categoria,		obter_tipo_atendimento(a.nr_atendimento), (Obter_dados_lote_fornec(a.nr_seq_lote_fornec,'SMA'))::numeric ),1,15) END  cd_brasindice_simpro,
	CASE WHEN coalesce(Obter_Dados_Brasindice2(d.cd_estabelecimento,a.cd_material, a.dt_atendimento, a.cd_convenio, 'CMED', a.cd_categoria, obter_tipo_atendimento(a.nr_atendimento),		(Obter_dados_lote_fornec(a.nr_seq_lote_fornec,'SMA'))::numeric ),0)=0 THEN 		substr(Obter_Desc_Simpro(Obter_Codigo_Simpro2(a.cd_material,a.cd_convenio,c.cd_estabelecimento,a.dt_atendimento,a.nr_seq_conv_simp)),1,100)  ELSE substr(Obter_Dados_Brasindice2(d.cd_estabelecimento,a.cd_material,a.dt_atendimento,a.cd_convenio,'DMED', a.cd_categoria,		obter_tipo_atendimento(a.nr_atendimento), (Obter_dados_lote_fornec(a.nr_seq_lote_fornec,'SMA'))::numeric ),1,100) END  ds_brasindice_simpro,
	...
	CASE WHEN a.ie_origem_preco=1 THEN   substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'DAPR'),1,98)  ELSE b.ds_material END  ds_bras_apresent_ou_material,
	CASE WHEN a.ie_origem_preco=1 THEN 		substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'DLAB'),1,40)  ELSE substr(obter_nome_pf_pj(null,obter_fornecedor_ultima_compra(a.cd_material,obter_estab_atend(a.nr_atendimento))),1,100) END  ds_lab_bras_ou_ds_for_ult_com,
	...
	CASE WHEN a.ie_origem_preco=1 THEN  substr(Obter_Dados_Brasindice2(d.cd_estabelecimento,a.cd_material, a.dt_atendimento, a.cd_convenio,'CMED', a.cd_categoria,		obter_tipo_atendimento(a.nr_atendimento), (Obter_dados_lote_fornec(a.nr_seq_lote_fornec,'SMA'))::numeric ),1,15) WHEN a.ie_origem_preco=4 THEN  Obter_Codigo_Simpro2(a.cd_material,a.cd_convenio,c.cd_estabelecimento,a.dt_atendimento,a.nr_seq_conv_simp) END  cd_cod_simpro_ou_brasindice,
	...
	coalesce(CASE WHEN substr(lpad(Obter_Codigo_Simpro2(a.cd_material,a.cd_convenio,c.cd_estabelecimento,a.dt_atendimento,a.nr_seq_conv_simp),10,0),1,10)='0000000000' THEN  		substr(Obter_Dados_Brasindice2(coalesce(c.cd_estabelecimento, c.cd_estabelecimento_base), a.cd_material, a.dt_atendimento, a.cd_convenio, 'TISS', a.cd_categoria, 		obter_tipo_atendimento(a.nr_atendimento), (Obter_dados_lote_fornec(a.nr_seq_lote_fornec,'SMA'))::numeric ),1,15)  ELSE substr(lpad(Obter_Codigo_Simpro2(a.cd_material,a.cd_convenio,c.cd_estabelecimento,a.dt_atendimento,a.nr_seq_conv_simp),10,0),1,10) END ,to_char(a.cd_material)) cd_simpro_brasindice_tiss,
	...
	d.dt_periodo_final,
	coalesce(substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'DMED'),1,100), b.ds_material) ds_bras_material,
	...
	CASE WHEN a.ie_origem_preco=1 THEN   substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'DMED'),1,100)  || '  ' ||				substr(obter_dados_brasindice_seq(a.nr_seq_bras_preco, 'DAPR'),1,98)  ELSE b.ds_material END  ds_bras_apresent_material,
	...
	coalesce(CASE WHEN coalesce(e.ds_material,b.ds_material)=b.ds_material THEN  null  ELSE e.ds_material END ,
	...
	coalesce(CASE WHEN somente_numero(a.cd_material_convenio)=a.cd_material THEN  null WHEN somente_numero(a.cd_material_convenio)=0 THEN  null  ELSE somente_numero(a.cd_material_convenio) END , coalesce(CASE WHEN a.cd_material_tuss=0 THEN null  ELSE a.cd_material_tuss END , a.cd_material_tiss)) CD_MAT_CONVERSAO_TUSS_TISS,	
	CASE WHEN 	coalesce(TISS_OBTER_REGRA_QTDE_MAT(a.nr_sequencia),'M')='M' THEN (a.qt_material * CASE WHEN coalesce(e.tx_conversao_qtde,0)=0 THEN  1  ELSE e.tx_conversao_qtde END ) WHEN 	coalesce(TISS_OBTER_REGRA_QTDE_MAT(a.nr_sequencia),'M')='R' THEN (round(a.qt_material * CASE WHEN coalesce(e.tx_conversao_qtde,0)=0 THEN  1  ELSE e.tx_conversao_qtde END )) WHEN 	coalesce(TISS_OBTER_REGRA_QTDE_MAT(a.nr_sequencia),'M')='D' THEN (dividir_sem_round(a.qt_material,CASE WHEN coalesce(e.tx_conversao_qtde,0)=0 THEN 1  ELSE e.tx_conversao_qtde END )) WHEN 	coalesce(TISS_OBTER_REGRA_QTDE_MAT(a.nr_sequencia),'M')='S' THEN (ceil(a.qt_material * CASE WHEN coalesce(e.tx_conversao_qtde,0)=0 THEN  1  ELSE e.tx_conversao_qtde END )) END  qt_material_tiss,
	dividir(a.vl_material, CASE WHEN 	coalesce(TISS_OBTER_REGRA_QTDE_MAT(a.nr_sequencia),'M')='M' THEN (a.qt_material * CASE WHEN coalesce(e.tx_conversao_qtde,0)=0 THEN  1  ELSE e.tx_conversao_qtde END ) WHEN 	coalesce(TISS_OBTER_REGRA_QTDE_MAT(a.nr_sequencia),'M')='R' THEN (round(a.qt_material * CASE WHEN coalesce(e.tx_conversao_qtde,0)=0 THEN  1  ELSE e.tx_conversao_qtde END )) WHEN 	coalesce(TISS_OBTER_REGRA_QTDE_MAT(a.nr_sequencia),'M')='D' THEN (dividir_sem_round(a.qt_material,CASE WHEN coalesce(e.tx_conversao_qtde,0)=0 THEN 1  ELSE e.tx_conversao_qtde END )) WHEN 	coalesce(TISS_OBTER_REGRA_QTDE_MAT(a.nr_sequencia),'M')='S' THEN (ceil(a.qt_material * CASE WHEN coalesce(e.tx_conversao_qtde,0)=0 THEN  1  ELSE e.tx_conversao_qtde END )) END ) vl_unitario_tiss,
	Obter_Fab_Lab_Tipo_Mat(a.cd_material, a.nr_seq_bras_preco, a.nr_seq_mat_simp, a.nr_seq_tuss_mat_item, a.ie_origem_preco) ds_fab_lab_Tipo_Mat
...
WHERE a.cd_material = b.cd_material and a.cd_setor_atendimento 	= c.cd_setor_atendimento and a.nr_interno_conta	= d.nr_interno_conta    and ((coalesce(f.nr_seq_desconto,0) = 0) or
	 (coalesce(f.nr_seq_desconto,0) = (	select 	coalesce(max(t.nr_sequencia),0)
					...
					and	t.nr_sequencia	= f.nr_seq_desconto)));
