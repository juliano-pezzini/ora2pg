...
CREATE OR REPLACE VIEW hsj_interf_unimed_sdc_v (ie_tipo_registro, nr_sequencia, dt_remessa, nr_seq_protocolo, nr_interno_conta, cd_interno, cd_regional, nr_lote, cd_senha_guia, nr_doc_convenio, ie_tipo_documento, dt_emissao, dt_entrada, dt_alta, hr_entrada, hr_alta, cd_usuario_convenio, nm_usuario_convenio, uf_medico_solicitante, cd_medico_solicitante, cd_doenca_def, cd_motivo_alta, ie_carater_internacao, cd_procedimento_princ, cd_tipo_acomodacao, ie_clinica, cd_tipo_plano, dt_procedimento, hr_procedimento, ie_cirurgia_video, cd_medico_executor, ie_funcao_executor, cd_item, ie_cirurgia_multipla, ie_tipo_nascimento, ie_sexo, dt_nascimento, dt_mesano_referencia, ie_tipo_paciente, qt_item, vl_item, qt_filme, vl_filme, vl_custo_operacional, vl_medico, ds_zeros, ds_espaco, tp_obito, tp_transferencia, cd_cid_principal) AS SELECT	
   ...
   somente_numero(substr(b.nr_doc_convenio,1,7)) nr_doc_convenio, 
 	substr(Obter_Conversao_Externa(a.cd_cgc_convenio,'HSJ_INTERF_UNIMED_SDC_V','IE_TIPO_DOCUMENTO', coalesce(a.ie_tipo_atendimento,0)),1,80) ie_tipo_documento, 
	...
	CASE WHEN a.ie_tipo_atendimento=1 THEN  		CASE WHEN e.ie_obito='S' THEN 0  ELSE CASE WHEN e.ie_transferencia='S' THEN 0  ELSE a.cd_motivo_alta END  END   ELSE 0 END  cd_motivo_alta, 
 	...
 	substr(Obter_Conversao_Externa(c.CD_CGC_CONVENIO, 'HSJ_INTERF_UNIMED_SDC_V', 'CD_TIPO_ACOMODACAO', 
 	...
 	substr(Obter_Conversao_Externa(c.CD_CGC_CONVENIO, 'HSJ_INTERF_UNIMED_SDC_V', 'CD_TIPO_PLANO', a.cd_categoria_convenio),1,50) cd_tipo_plano, 
	...
	CASE WHEN coalesce(b.cd_funcao_executor,'0')='0' THEN  'H'  ELSE substr(Obter_Conversao_Externa(c.CD_CGC_CONVENIO, 'HSJ_INTERF_UNIMED_SDC_V', 			 'IE_FUNCAO_EXECUTOR', b.cd_funcao_executor),1,5) END  ie_funcao_executor,            
...
WHERE a.nr_interno_conta	= b.nr_interno_conta and a.nr_seq_protocolo	= d.nr_seq_protocolo and a.nr_seq_protocolo	= c.nr_seq_protocolo  group by 
	...
	substr(Obter_Conversao_Externa(c.CD_CGC_CONVENIO, 'HSJ_INTERF_UNIMED_SDC_V', 'CD_TIPO_PLANO', a.cd_categoria_convenio),1,50), 
 	a.ie_tipo_atendimento, 
	Obter_Conversao_Externa(a.cd_cgc_convenio,'HSJ_INTERF_UNIMED_SDC_V','IE_TIPO_DOCUMENTO', coalesce(a.ie_tipo_atendimento,0)), 
 	a.dt_entrada, 
	...
	CASE WHEN coalesce(b.cd_funcao_executor,'0')='0' THEN  'H'  ELSE substr(Obter_Conversao_Externa(c.CD_CGC_CONVENIO, 'HSJ_INTERF_UNIMED_SDC_V', 			 'IE_FUNCAO_EXECUTOR', b.cd_funcao_executor),1,5) END ,            
 	b.cd_item_convenio, 
