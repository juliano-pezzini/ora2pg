...
select	2						tp_registro, 
	...
	coalesce(CASE WHEN n.cd_pessoa_fisica IS NULL THEN substr(obter_compl_pj(n.cd_cgc,1,'M'),1,80)  ELSE substr(obter_compl_pf(n.cd_pessoa_fisica, 1, 'M'),1,60) END , substr(obter_dados_pf_pj(n.cd_pessoa_fisica, n.cd_cgc, 'M'),1,60)) ds_email, 
...
select	5				tp_registro, 
	...
	''					cd_pessoa_fisica, 
	2					IndCNPJ, 
	lpad(coalesce(n.cd_cgc,substr(obter_cgc_convenio(obter_convenio_nf(n.nr_sequencia)),1,14)),14,'0') CNPJ_Intermed, 
	lpad(substr(CASE WHEN obter_dados_pf_pj(null,coalesce(n.cd_cgc,obter_cgc_convenio(obter_convenio_nf(n.nr_sequencia))),'CDM')=355030 THEN  obter_dados_pf_pj(null,coalesce(n.cd_cgc,obter_cgc_convenio(obter_convenio_nf(n.nr_sequencia))),'IM')  ELSE '0' END ,1,8),8,'0') INSC_MUNICIPAL_Intermed, 
	rpad(substr((SELECT x.ds_email from pessoa_juridica_estab x where x.cd_cgc = coalesce(n.cd_cgc,obter_cgc_convenio(obter_convenio_nf(n.nr_sequencia))) and x.cd_estabelecimento = n.cd_estabelecimento),1,75),75,' ') DS_EMAIL_Intermed	 
from	nota_fiscal n 
where	1 = 1 
and	exists ( 
	select	1 
