...
and 		coalesce(C.ie_responsavel_credito,'X') in ('CRC','MCOO') 
...
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA   and coalesce(e.ie_responsavel_credito,'X') in ('CRC','MCOO') GROUP BY	B.NR_SEQ_PROTOCOLO, 
     	B.NR_INTERNO_CONTA, 
		substr(ELIMINA_ACENTUACAO(UPPER(B.NM_PACIENTE)),1,40), 
...
LEFT OUTER JOIN estrutura_procedimento_v x ON (E.CD_ITEM = X.CD_PROCEDIMENTO AND E.IE_ORIGEM_PROCED = X.IE_ORIGEM_PROCED)
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA and B.nr_seq_protocolo	= f.nr_seq_protocolo   and coalesce(e.ie_responsavel_credito,'X') in ('CRC','MCOO') GROUP BY	(substr(coalesce(f.CD_INTERNO,0),1,5))::numeric , 
		B.NR_SEQ_PROTOCOLO;
