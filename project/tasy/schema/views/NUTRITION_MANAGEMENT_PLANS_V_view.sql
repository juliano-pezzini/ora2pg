...
CREATE OR REPLACE VIEW nutrition_management_plans_v (ie_type_admission, dt_hospitalization, cd_current_medical_department, ds_current_medical_department, cd_current_ward, ds_current_ward, cd_patient_id, nm_patient_name, ie_sex, qt_age, ds_type_admission, ds_medical_department_doc, ds_ward_doc, nm_attending_doctor, ie_status, ds_status, nr_document, ie_nec_nutrition_manage, nr_atendimento) AS SELECT	1 ie_type_admission, /*Inpatient*/
	...
	substr(get_data_doc_nut_plan(b.nr_atendimento,'DSD'),1,255) ds_Medical_department_doc,
	substr(get_data_doc_nut_plan(b.nr_atendimento,'DSW'),1,255) ds_Ward_Doc,
	substr(get_data_doc_nut_plan(b.nr_atendimento,'NMAD'),1,255) nm_Attending_doctor,
	substr(get_data_doc_nut_plan(b.nr_atendimento,'IEST'),1,255) ie_Status,
	substr(get_data_doc_nut_plan(b.nr_atendimento,'DSST'),1,255) ds_Status,
	substr(get_data_doc_nut_plan(b.nr_atendimento,'SEQ'),1,255) nr_Document,
	substr(CASE WHEN get_data_doc_treatment_plan(b.nr_atendimento,'SEQ','1234')='1' THEN wheb_mensagem_pck.get_Texto(334313)  ELSE wheb_mensagem_pck.get_Texto(854006) END ,1,255) ie_nec_nutrition_manage,
	b.nr_atendimento
