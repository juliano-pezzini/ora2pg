...
CREATE OR REPLACE VIEW eis_billing_eval_statistics_v (dt_contabil, cd_estabelecimento, cd_setor_atendimento, ie_origem_proced, cd_procedimento, ie_periodo, nm_usuario, dt_atualizacao, cd_medico_executor, cd_equipamento, cd_convenio, ie_tipo_atendimento, qt_procedimento, vl_custo_operacional, vl_medico, vl_materiais, vl_anestesita, vl_custo, vl_tabela, vl_receita_vp, qt_dias_prazo, cd_conta_contabil, cd_medico_laudo, nr_seq_proc_interno, cd_medico_req, cd_medico_prescr, ie_complexidade_sus, nr_atendimento, cd_paciente, nm_paciente, cd_tipo_procedimento, ds_setor_atendimento, ds_procedimento, nm_medico_laudo, nm_medico_executor, nm_medico_req, nm_medico_prescr, ds_equipamento, ds_convenio, ds_tipo_atendimento, ds_tipo_procedimento, vl_total, ds_grupo_proc, ds_especialidade, ds_area_procedimento, ds_especialidade_medica, cd_grupo_proc, cd_especialidade, cd_area_procedimento, cd_especialidade_medica, ds_proc_interno, ds_procedimento_int, ds_tipo_convenio, cd_tipo_convenio, ds_estabelecimento, ds_pac_atend, ie_clinica, ds_clinica_atend) AS select	
	...
	substr(Obter_Desc_Espec_medica(c.cd_especialidade_medica),1,80) ds_especialidade_medica,
	c.cd_grupo_proc,
	c.cd_especialidade,
	...
	substr(Obter_Clinica_Atend(a.nr_atendimento, 'C'), 1,90) ie_clinica,
	substr(Obter_Clinica_Atend(a.nr_atendimento, 'D'), 1,90) ds_clinica_atend
FROM	procedimento b,
