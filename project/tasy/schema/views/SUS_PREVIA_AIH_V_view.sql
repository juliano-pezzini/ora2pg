...
GROUP BY	p.cd_procedimento,
		...
		p.dt_procedimento
 HAVING(sum(s.vl_medico)<> 0 or sum(s.vl_ato_medico) <> 0 or sum(s.vl_sadt) <> 0 or sum(s.vl_matmed) <> 0)
...
GROUP BY	p.cd_procedimento,
		...
		p.nr_interno_conta,
		a.cd_pessoa_fisica,
		p.dt_procedimento HAVING sum(a.vl_participante) <>0
;
