...
CREATE OR REPLACE VIEW unimed_cri_v (ie_tipo_registro, nr_seq_protocolo, nr_interno_conta, cd_interno, cd_regional, nr_lote, cd_senha_guia, nr_doc_convenio, ie_tipo_documento, dt_emissao, dt_entrada, dt_alta, hr_entrada, hr_alta, cd_usuario_convenio, nm_usuario_convenio, uf_medico_solicitante, cd_medico_solicitante, cd_doenca_def, cd_motivo_alta, ie_carater_internacao, cd_procedimento_princ, cd_tipo_acomodacao, ie_clinica, cd_tipo_plano, dt_procedimento, hr_procedimento, ie_cirurgia_video, cd_medico_executor, ie_funcao_executor, cd_item, ie_cirurgia_multipla, ie_tipo_nascimento, ie_sexo, dt_nascimento, dt_mesano_referencia, qt_item, vl_item, qt_filme, vl_filme, vl_custo_operacional, vl_medico, ds_zeros, ds_espaco, ds_reg_trailer) AS SELECT	1 ie_tipo_registro,
	...
	CASE WHEN ie_tipo_atendimento=1 THEN a.cd_motivo_alta  ELSE null END  cd_motivo_alta, 
 	...
 	CASE WHEN ie_tipo_atendimento=1 THEN  coalesce(a.ie_clinica,0)  ELSE null END  ie_clinica,           
	...
	CASE WHEN b.ie_responsavel_credito='RM' THEN b.cd_interno  ELSE CASE WHEN coalesce(b.cd_medico_exec_conv,'0')='0' THEN  		b.cd_interno  ELSE b.cd_medico_exec_conv END  END  cd_medico_executor,            
...
group by 
	...
	CASE WHEN ie_tipo_atendimento=1 THEN a.cd_motivo_alta  ELSE null END ,          
 	...
 	CASE WHEN ie_tipo_atendimento=1 THEN  coalesce(a.ie_clinica,0)  ELSE null END ,           
	...
	CASE WHEN b.cd_medico_executor IS NULL THEN to_char(a.dt_entrada,'hh24mi')  ELSE to_char(b.dt_item,'hh24mi') END , 
 	b.ie_video, 
	CASE WHEN b.ie_responsavel_credito='RM' THEN b.cd_interno  ELSE CASE WHEN coalesce(b.cd_medico_exec_conv,'0')='0' THEN  		b.cd_interno  ELSE b.cd_medico_exec_conv END  END ,            
