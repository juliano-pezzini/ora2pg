...
WHERE c.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA and w.nr_seq_protocolo = x.nr_seq_protocolo  and c.ie_tipo_item	= 1 and c.qt_item		<> 0 and ((coalesce(Z.ie_entra_conta,'S') = 'S') or (c.nr_seq_proc_pacote = c.nr_seq_item)) and c.cd_medico_executor is not null GROUP BY	somente_numero(coalesce(x.cd_interno,0)),
		c.NR_SEQ_PROTOCOLO,
           		c.NR_INTERNO_CONTA,
           		c.CD_ITEM_CONVENIO,
           		substr(ELIMINA_ACENTUACAO(upper(c.DS_ITEM)),1,50),
...
and		c.vl_total_item	<> 0
and		c.qt_item		<> 0
and		c.CD_CONVENIO = 17
and 		c.CD_ITEM in (32100043,32100051,32100060)
GROUP BY	somente_numero(coalesce(x.cd_interno,0)),
		c.NR_SEQ_PROTOCOLO,
           	c.NR_INTERNO_CONTA,
           	c.CD_ITEM_CONVENIO,
...
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA and B.nr_seq_protocolo	= y.nr_seq_protocolo    and ((coalesce(Z.ie_entra_conta,'S') = 'S') or (e.nr_seq_proc_pacote = e.nr_seq_item)) GROUP BY	somente_numero(coalesce(y.cd_interno,0)),
		B.NR_SEQ_PROTOCOLO,
           	B.NR_INTERNO_CONTA,
		substr(ELIMINA_ACENTUACAO(UPPER(B.NM_PACIENTE)),1,40),
...
LEFT OUTER JOIN estrutura_procedimento_v x ON (E.CD_ITEM = x.CD_PROCEDIMENTO AND E.IE_ORIGEM_PROCED = x.IE_ORIGEM_PROCED)
LEFT OUTER JOIN regra_honorario z ON (E.IE_RESPONSAVEL_CREDITO = Z.cd_regra)
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA and B.nr_seq_protocolo	= y.nr_seq_protocolo    and ((coalesce(Z.ie_entra_conta,'S') = 'S') or (e.nr_seq_proc_pacote = e.nr_seq_item) or (hvc_obter_se_custo_casu(e.cd_convenio,e.cd_item) = 'S')) GROUP BY	somente_numero(coalesce(y.cd_interno,0)),
		B.NR_SEQ_PROTOCOLO,
		coalesce(somente_numero(y.nr_protocolo),0);
