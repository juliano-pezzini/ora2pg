...
WHERE C.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA  and ((coalesce(Z.ie_entra_conta,'S') = 'S') or (C.NR_SEQ_PROC_PACOTE = C.NR_SEQ_ITEM)) and C.ie_tipo_item	= 1 and C.qt_item		<> 0 and C.cd_medico_executor is not null and C.CD_ITEM_CONVENIO <> '10000' GROUP BY	C.NR_SEQ_PROTOCOLO, 
      	C.NR_INTERNO_CONTA, 
      	C.CD_ITEM_CONVENIO, 
      	substr(ELIMINA_ACENTUACAO(upper(C.DS_ITEM)),1,50), 
