...
CREATE OR REPLACE VIEW envio_dctf_v (ie_tipo_registro, nr_seq_lote, nr_seq_registro, ie_tipo_declaracao, nr_cnpj, nr_versao, nm_empresarial, sg_uf, ie_situacao, nr_ano_declaracao, nr_mes_declaracao, dt_periodo_inicial, dt_periodo_final, dt_ocorrencia_evento, ie_tipo, nr_mes_ocorrencia, dt_inicio, dt_fim, nr_declaracao, nr_recibo, ie_forma_tributacao, ie_qualificacao_pj, ie_pj_balanco, ie_pj_scp, ie_pj_inativa, ie_pj_regime_especial, ie_pj_dctf_semestral, cd_natureza_juridica, ds_logradouro, ds_numero, ds_complemento, ds_bairro, ds_municipio, nr_cep, nr_ddd_telefone, nr_telefone, nr_ddd_fax, nr_fax, nr_caixa_postal, ds_uf_caixa_postal, nr_cep_caixa_postal, ds_email, nm_representante, nr_cpf_representante, nr_ddd_telefone_representante, nr_telefone_representante, nr_ramal_representante, nr_ddd_fax_representante, nr_fax_representante, ds_email_representante, nm_responsavel, nr_cpf_resp, ds_crc_resp, ds_uf_crc_resp, nr_ddd_telefone_resp, nr_telefone_resp, nr_ramal_resp, nr_ddd_fax_resp, nr_fax_resp, ds_email_resp, nr_grupo_tributo, cd_receita, ds_periodicidade, nr_ano_apuracao, dt_apuracao_mensal, dt_apuracao_diario, nr_ordem_estab, nr_cnpj_incorporacao, vl_debito, ie_balanco_reducao, ie_saldo_quotas, dt_periodo_apuracao, nr_cnpj_darf, cd_receita_darf, dt_vencimento, nr_referencia, vl_principal, vl_multa, vl_juros, vl_pago_debito, nr_trimestre_apuracao, nr_quota, qt_registros, ds_hashcode) AS SELECT	1						ie_tipo_registro,
	a.nr_sequencia					nr_seq_lote, 
	'DCTFM'						nr_seq_registro, 
	...
	CASE WHEN a.ie_finalidade_dctf=00 THEN '00000000'  ELSE LOCALTIMESTAMP END  dt_ocorrencia_evento, 
	' ' ie_tipo, 
	LOCALTIMESTAMP nr_mes_ocorrencia, 
	LOCALTIMESTAMP dt_inicio, 
	LOCALTIMESTAMP dt_fim, 
	0 nr_declaracao, 
	' ' nr_recibo, 
	...
	'0' cd_receita, 
	' ' ds_periodicidade, 
	LOCALTIMESTAMP nr_ano_apuracao, 
	' ' dt_apuracao_mensal, 
	' ' dt_apuracao_diario, 
	' ' nr_ordem_estab, 
	...
	0 ie_saldo_quotas, 
	LOCALTIMESTAMP dt_periodo_apuracao, 
	' ' nr_cnpj_darf, 
	'0' cd_receita_darf, 
...
SELECT	2						ie_tipo_registro, 
	...
	' ' sg_uf, 
	a.ie_finalidade_dctf				ie_situacao, 
	LOCALTIMESTAMP nr_ano_declaracao, 
	LOCALTIMESTAMP nr_mes_declaracao, 
	TRUNC(a.dt_referencia,'mm') 			dt_periodo_inicial, 
	trunc(fim_mes(a.dt_referencia),'dd')		dt_periodo_final, 
	CASE WHEN a.ie_finalidade_dctf=00 THEN '00000000'  ELSE LOCALTIMESTAMP END  dt_ocorrencia_evento, 
	'R01'						ie_tipo, 
...
and	c.nr_sequencia		= (select	max(x.nr_sequencia) 
				  from		estab_dctf x 
				  where	trunc(x.dt_inicio_vigencia,'dd') <= trunc(LOCALTIMESTAMP,'dd') 
...
/* R03 - Dados dos responsáveis pela PJ */
...
SELECT	distinct 5					ie_tipo_registro, 
	a.nr_sequencia					nr_seq_lote, 
	' ' nr_seq_registro, 
	0 ie_tipo_declaracao, 
	b.cd_cgc					nr_cnpj, 
...
WHERE a.cd_estabelecimento	= b.cd_estabelecimento AND a.nr_sequencia		= c.nr_seq_lote AND c.nr_seq_darf		= d.nr_sequencia and d.nr_sequencia		= e.nr_seq_darf  and (f.cd_tributo		= g.cd_tributo or i.cd_tributo = g.cd_tributo) and h.cd_estabelecimento	= b.cd_estabelecimento and e.nr_titulo		= i.nr_titulo and h.nr_sequencia		= (select	max(x.nr_sequencia) 
				  from		estab_dctf x 
				  where	trunc(x.dt_inicio_vigencia,'dd') <= trunc(LOCALTIMESTAMP,'dd') 
				  and		b.cd_estabelecimento	= x.cd_estabelecimento) group by a.nr_sequencia, 
...
LEFT OUTER JOIN titulo_pagar_imposto f ON (i.nr_seq_tributo = f.nr_sequencia)
WHERE a.cd_estabelecimento	= b.cd_estabelecimento AND a.nr_sequencia		= c.nr_seq_lote AND c.nr_seq_darf		= d.nr_sequencia and d.nr_sequencia		= e.nr_seq_darf and e.nr_titulo			= i.nr_titulo  and (f.cd_tributo		= g.cd_tributo or i.cd_tributo = g.cd_tributo) 
