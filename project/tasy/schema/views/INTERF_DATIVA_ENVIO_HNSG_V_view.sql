...
SELECT		'CABECALHO CONTA   '		DS_REGISTRO, 
		...
		substr(B.cd_senha_guia,1,10) 
						CD_AUTORIZACAO, 
		substr(Obter_Unidade_Atendimento(B.nr_atendimento, 'A', 'CTA'),1,20)		CD_ACOMODACAO, 
...
SELECT   	'ITEM DA CONTA     ' 	DS_REGISTRO, 
		...
		sum(CASE WHEN coalesce(c.vl_honorario,0) + coalesce(c.vl_custo_oper,0)=0 THEN  CASE WHEN c.ie_responsavel_credito IS NULL THEN  c.vl_total_item  ELSE 0 END   ELSE coalesce(c.vl_honorario,0) + coalesce(c.vl_custo_oper,0) + coalesce(c.vl_filme,0) END ) VL_ITEM, 
      	0            	IE_CIRURGIA_MULTIPLA, 
      	0		   		VL_TOTAL_ITEM, 
      	0	   			QT_ITEM_CONTA, 
	   	' '				NM_PACIENTE, 
...
SELECT   	'TOTAL DA CONTA    ' 	DS_REGISTRO, 
      	...
      	'CSMH'         	IE_SERVICO, 
      	3            	CD_REGISTRO, 
      	somente_numero(coalesce(cd_origem,0))	CD_ORIGEM, 
      	NR_SEQ_PROTOCOLO   	NR_LOTE, 
		...
		UF_CRM 				UF_CRM_REQ, 
      	...
      	0				QT_ITEM_CONTA, 
	   	substr(ELIMINA_ACENTUACAO(UPPER(NM_PACIENTE)),1,40) 
						NM_PACIENTE, 
      	' '	   			NR_FATURA, 
      	0				QT_CONTA, 
      	0				VL_TOTAL_LOTE, 
		...
		0				cd_leitora, 
		'F'				ie_fim_registro, 
		ie_tipo_atendimento		ie_tipo_atend_tasy, 
		nr_doc_convenio 
FROM (SELECT   	'ITEM DA CONTA     ' 	DS_REGISTRO, 
      	3            	TP_REGISTRO, 
      	'CSMH'         	IE_SERVICO, 
      	2            	CD_REGISTRO, 
      	somente_numero(coalesce(x.cd_interno,0))	CD_ORIGEM, 
...
GROUP BY	somente_numero(coalesce(x.cd_interno,0)), 
		...
		w.ie_tipo_atendimento, 
		w.nr_doc_convenio 
GROUP BY	somente_numero(coalesce(cd_origem,0)), 
		NR_SEQ_PROTOCOLO, 
      	NR_INTERNO_CONTA, 
		substr(ELIMINA_ACENTUACAO(UPPER(NM_PACIENTE)),1,40), 
		NR_CRM, 
		...
		ie_tipo_atendimento, 
		nr_doc_convenio 
 HAVING SUM(c.QT_ITEM) > 0) alias154 
...
GROUP BY	somente_numero(coalesce(cd_origem,0)), 
		NR_SEQ_PROTOCOLO 
 HAVING SUM(c.QT_ITEM) > 0) alias214 
...
GROUP BY	somente_numero(coalesce(x.cd_interno,0)), 
		...
		w.nr_doc_convenio 
GROUP BY	somente_numero(coalesce(cd_origem,0)), 
		NR_SEQ_PROTOCOLO HAVING SUM(c.QT_ITEM) > 0) alias273
;
