...
CREATE OR REPLACE VIEW conta_paciente_custo_v (ie_proc_mat, cd_setor_atendimento, cd_conta_contabil, ds_tipo_item, ds_item, ds_convenio, ds_setor_atendimento, ds_conta_contabil, qt_resumo, cd_unidade_medida, ie_apuracao_custo, vl_preco_medio, vl_item, vl_original, vl_result_pacote, vl_imposto, vl_medico, vl_medico_fora_conta, vl_custo_medio, vl_resultado, cd_item, vl_custo, dt_mesano_referencia, dt_referencia, nr_interno_conta, nr_atendimento, nr_seq_proc_pacote, ds_observacao, qt_exclusao_custo, qt_exclusao_conta, vl_custo_operacional, nr_seq_protocolo) AS select  'M' ie_proc_mat,
	...
	sum(coalesce(a.vl_medico_fora_conta,0)) vl_medico_fora_conta,
	...
	'' ds_observacao,
	coalesce(sum(a.qt_exclusao_custo),0) qt_exclusao_custo,
	coalesce(sum(a.qt_exclusao_conta),0) qt_exclusao_conta,
	sum(0) vl_custo_operacional,
	b.nr_seq_protocolo
...
select  'P' ie_proc_mat,
	...
	sum(coalesce(a.vl_original,0)) vl_original,
	(sum(coalesce(a.vl_procedimento,0)) - sum(coalesce(a.vl_original,0))) vl_result_pacote,
	sum(coalesce(a.vl_imposto,0)) vl_imposto,
	sum(coalesce(a.vl_medico,0)) vl_medico,
	sum(coalesce(a.vl_medico_fora_conta,0)) vl_medico_fora_conta,
	CASE WHEN sum(a.qt_resumo)=0 THEN 0  ELSE (sum(a.vl_custo) / sum(a.qt_resumo)) END  vl_custo_medio,
	...
	coalesce(sum(a.qt_exclusao_custo),0) qt_exclusao_custo,
	coalesce(sum(a.qt_exclusao_conta),0) qt_exclusao_conta,
	coalesce(sum(a.vl_custo_operacional),0) vl_custo_operacional,
	b.nr_seq_protocolo
