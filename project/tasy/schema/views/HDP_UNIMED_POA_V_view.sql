...
CREATE OR REPLACE VIEW hdp_unimed_poa_v (tp_registro, tp_ordem_reg, nr_seq_protocolo, cd_prestador, nr_seq_lote, nr_interno_conta, cd_serie_documento, dt_geracao, nr_documento, cd_unid_carteira, nr_carteira, dt_referencia, cd_motivo_alta, cd_cid, dt_entrada, dt_alta, dt_ano_guia, nr_guia, cd_classe_nota, cd_acomodacao, nm_beneficiario, ie_sexo, dt_nascimento, dt_validade_carteira, cd_setor_atendimento, cd_procedimento, qt_procedimento_autor, dt_procedimento, ie_urgencia, ie_adicional_urgencia, qt_procedimento, vl_procedimento, ie_tipo_nascimento, nr_seq_autorizacao, ie_tipo_percentual, ie_tipo_insumo, cd_insumo, qt_insumo, dt_insumo, qt_cobrada, vl_cobrado, cd_pacote, dt_emissao, qt_reg_nota, qt_reg_doc, qt_reg_proc, qt_reg_insumo, vl_total_lote, nr_seq_pacote, nr_lote, qt_nasc_vivos_termo, qt_nasc_mortos, qt_nasc_vivos_premat, qt_obito_nasc_precoc, qt_obito_nasc_tardio, ie_tipo_saida, ie_tipo_doenca, qt_tempo_doenca, ie_tipo_tempo_doenca, ie_acidente, cd_cid1, cd_cid2, cd_cid3, nm_prof_solicitante, ie_tipo_atendimento, ie_tipo_consulta, ds_observacoes, ie_gestacao, ie_aborto, ie_trans_gravidez, ie_compl_puerperio, ie_sala_parto, ie_compl_neonat, ie_baixo_peso, ie_parto_cesareo, ie_parto_normal, ie_obito_mulher, ie_nasc_vivos, ie_cid_obito, nr_decl_obito, ie_tipo_faturamento, cd_cobertura, nm_prest_exec, cd_tab_medica, hr_final, nm_prof_exec, ie_tec_utilizada, ie_tipo_perc, qt_pacote, nr_linha_ptu, ie_divide_honorario, cd_divisao_honorario, cd_prestador_divisao, nr_seq_versao, nr_regime_internacao, nr_grau_participacao, nr_dec_nasc_vivo_2345, cd_ato_medico_divisao, ie_nasc_vivos2, ie_nasc_vivos3, ie_nasc_vivos4, ie_nasc_vivos5, ie_regime_internacao, ds_indic_clinica, nr_guia_prestador) AS select	/* Header */
	...
	0 				cd_insumo,
	0		 		qt_insumo,
	to_char(LOCALTIMESTAMP,'ddmmyyyyhhmi') dt_insumo,
	0 				qt_cobrada,
	0	 			vl_cobrado,
	0 				cd_pacote,
	...
	'0005'				nr_seq_versao,
	0				nr_regime_internacao,
	0				nr_grau_participacao,
	' '				nr_dec_nasc_vivo_2345,
...
select	/* Nota */
	...
	2				tp_ordem_reg,
	c.nr_seq_protocolo		nr_seq_protocolo,
	LPAD(substr(cd_interno,1,8),8,0)	cd_prestador,
	0 				nr_seq_lote,
	0				nr_interno_conta,
	0				cd_serie_documento,
	LOCALTIMESTAMP	 			dt_geracao,
...
select	/* Documento  */
	...
	LPAD(substr(CASE WHEN b.cd_convenio=157 THEN coalesce(poa_obter_regra_prestador(b.ie_tipo_atendimento,d.ie_tipo_guia, b.nr_seq_protocolo, b.cd_medico_resp),'990048')  ELSE poa_obter_regra_prestador(b.ie_tipo_atendimento,d.ie_tipo_guia, b.nr_seq_protocolo, b.cd_medico_resp) END ,1,8),8,0)
					cd_prestador,
	0 				nr_seq_lote,
	...
	CASE WHEN b.ie_tipo_atendimento=8 THEN substr(obter_guia_conta(b.nr_interno_conta),1,8)				     WHEN b.ie_tipo_atendimento=1 THEN substr(obter_guia_conta(b.nr_interno_conta),1,8)				     WHEN b.ie_tipo_atendimento=7 THEN substr(b.nr_interno_conta,1,8)				     WHEN b.ie_tipo_atendimento=3 THEN substr(b.nr_interno_conta,1,8) END
					nr_documento,
	substr(b.cd_usuario_convenio,1,4) cd_unid_carteira,
	substr(b.cd_usuario_convenio,5,13) nr_carteira,
	LOCALTIMESTAMP				dt_referencia,
	CASE WHEN b.ie_tipo_atendimento=1 THEN 	CASE WHEN b.dt_alta IS NULL THEN 10  ELSE CASE WHEN b.dt_periodo_final=b.dt_alta THEN b.cd_motivo_alta  ELSE 10 END  END   ELSE b.cd_motivo_alta END  cd_motivo_alta,
	substr(b.cd_cid_principal,1,4)	cd_cid,
	b.dt_periodo_inicial		dt_entrada,
	b.dt_periodo_final		dt_alta,
	--DECODE(obter_tipo_guia_convenio(d.nr_atendimento),'E',NULL,'C',NULL,'A', NULL, TO_CHAR(NVL(hdp_obter_ano_guia_conv(b.nr_atendimento,b.cd_convenio),TRUNC(b.dt_periodo_inicial,'year')),'yyyy')) dt_ano_guia,
	/*decode(b.ie_tipo_atendimento,8,DECODE(obter_tipo_guia_convenio(d.nr_atendimento),'E',NULL,'C',NULL,'A', NULL, TO_CHAR(NVL(hdp_obter_ano_guia_conv(b.nr_atendimento,b.cd_convenio),TRUNC(b.dt_periodo_inicial,'year')),'yyyy'))
				    ,1,DECODE(obter_tipo_guia_convenio(d.nr_atendimento),'E',NULL,'C',NULL,'A', NULL, TO_CHAR(NVL(hdp_obter_ano_guia_conv(b.nr_atendimento,b.cd_convenio),TRUNC(b.dt_periodo_inicial,'year')),'yyyy'))
				    ,7,'0000'
				    ,3,'0000') dt_ano_guia,*/
	CASE WHEN b.ie_tipo_atendimento=8 THEN '20' ||substr(coalesce(obter_guia_conta(b.nr_interno_conta),to_char(LOCALTIMESTAMP,'yyyy')),					length(obter_guia_conta(b.nr_interno_conta)) -1,length(obter_guia_conta(b.nr_interno_conta)))				     WHEN b.ie_tipo_atendimento=1 THEN '20' ||substr(coalesce(obter_guia_conta(b.nr_interno_conta),to_char(LOCALTIMESTAMP,'yyyy')),					length(obter_guia_conta(b.nr_interno_conta)) -1,length(obter_guia_conta(b.nr_interno_conta)))				     WHEN b.ie_tipo_atendimento=7 THEN '0000'				     WHEN b.ie_tipo_atendimento=3 THEN '0000' END  dt_ano_guia,
	CASE WHEN b.ie_tipo_atendimento=7 THEN 00000000 WHEN b.ie_tipo_atendimento=3 THEN 00000000  ELSE somente_numero(substr(obter_guia_conta(b.nr_interno_conta),1,8)) END 
	nr_guia,
	CASE WHEN b.ie_tipo_atendimento=7 THEN '02' WHEN b.ie_tipo_atendimento=8 THEN  '03' WHEN b.ie_tipo_atendimento=3 THEN  '08' WHEN b.ie_tipo_atendimento=1 THEN  CASE WHEN b.ie_clinica=1 THEN  '04' WHEN b.ie_clinica=2 THEN  '05' WHEN b.ie_clinica=3 THEN  '06' WHEN b.ie_clinica=4 THEN  '11' END   ELSE CASE WHEN obter_tipo_guia_convenio(b.nr_atendimento)='IC' THEN '04' WHEN obter_tipo_guia_convenio(b.nr_atendimento)='I' THEN '05' WHEN obter_tipo_guia_convenio(b.nr_atendimento)='IO' THEN '06' WHEN obter_tipo_guia_convenio(b.nr_atendimento)='AI' THEN '03' WHEN obter_tipo_guia_convenio(b.nr_atendimento)='IP' THEN '11'  ELSE '08' END  END  cd_classe_nota,
	' ' cd_acomodacao,
	substr(b.nm_paciente,1,40)	nm_beneficiario,
	b.ie_sexo			ie_sexo,
	b.dt_nascimento			dt_nascimento,
	b.dt_validade_carteira		dt_validade_carteira,
	CASE WHEN b.ie_tipo_atendimento=8 THEN 042				     WHEN b.ie_tipo_atendimento=1 THEN 040				     WHEN b.ie_tipo_atendimento=7 THEN 042				     WHEN b.ie_tipo_atendimento=3 THEN 041 END 
	...
	CASE WHEN b.ie_tipo_atendimento=1 THEN '0'  ELSE CASE WHEN substr(obter_tipo_fatur_tiss(b.nr_interno_conta),1,1)='P' THEN  '4'  ELSE to_char(b.cd_motivo_alta) END  END 	ie_tipo_saida,
	...
	substr(d.nm_medico_solicitante,1,70)
					nm_prof_solicitante,
	CASE WHEN b.ie_tipo_atendimento=8 THEN 05				     WHEN b.ie_tipo_atendimento=1 THEN 00				     WHEN b.ie_tipo_atendimento=7 THEN 05				     WHEN b.ie_tipo_atendimento=3 THEN 04 END  		ie_tipo_atendimento,
	CASE WHEN b.ie_tipo_atendimento=8 THEN 0				     WHEN b.ie_tipo_atendimento=1 THEN 0				     WHEN b.ie_tipo_atendimento=7 THEN 0				     WHEN b.ie_tipo_atendimento=3 THEN 1 END  				ie_tipo_consulta,
	' '				ds_observacoes,
...
select	/*Procedimentos*/
	...
	LPAD(substr(Obter_Prestador_Convenio(d.cd_cgc_prestador, f.cd_convenio,null),1,8),8,0)
					cd_prestador,
	0 				nr_seq_lote,
	...
	CASE WHEN d.cd_grupo_proc=20203 THEN  0  ELSE CASE WHEN d.ie_responsavel_credito='P' THEN  d.vl_honorario  ELSE d.vl_total_item END  END 
					vl_procedimento,
	...
	0				qt_pacote,
	0				nr_linha_ptu,
	--decode(d.cd_grupo_proc, 20203, 'S', 'N') ie_divide_honorario,
	substr(obter_dados_unimed_poa('IE_DIVIDE_HONORARIO', somente_numero(coalesce(d.cd_item_convenio, d.cd_item)), d.cd_grupo_proc, c.cd_categoria_convenio, d.dt_item),1,1) ie_divide_honorario,
	--decode(d.cd_grupo_proc, 20203, 48, 0) cd_divisao_honorario,
	somente_numero(obter_dados_unimed_poa('CD_DIVISAO_HONORARIO', somente_numero(coalesce(d.cd_item_convenio, d.cd_item)), d.cd_grupo_proc, c.cd_categoria_convenio, d.dt_item)) cd_divisao_honorario,
	--decode(d.cd_grupo_proc, 20203, nvl(somente_numero(d.cd_medico_exec_conv),0), 0) cd_prestador_divisao,
	CASE WHEN substr(obter_dados_unimed_poa('CD_PRESTADOR_DIVISAO', somente_numero(coalesce(d.cd_item_convenio, d.cd_item)), d.cd_grupo_proc, c.cd_categoria_convenio, d.dt_item),1,1)='S' THEN  coalesce(somente_numero(d.cd_medico_exec_conv),0)  ELSE 0 END  cd_prestador_divisao,
	''				nr_seq_versao,
	0				nr_regime_internacao,
	12				nr_grau_participacao,
	' '				NR_DEC_NASC_VIVO_2345,
	-- CD_ATO_MEDICO_DIVISAO defaault 2
	somente_numero(obter_dados_unimed_poa('CD_ATO_MEDICO_DIVISAO', somente_numero(coalesce(d.cd_item_convenio, d.cd_item)), d.cd_grupo_proc, c.cd_categoria_convenio, d.dt_item)) cd_ato_medico_divisao,
	' '				ie_nasc_vivos2,
	' '				ie_nasc_vivos3,
	' '				ie_nasc_vivos4,
	' '				ie_nasc_vivos5,
...
and	somente_numero(coalesce(d.cd_grupo_gasto,0)) not in (10,11,12,13,20,99)
...
select	/* Insumos  (Taxas e diarias)  */
	...
	5				tp_ordem_reg,
	c.nr_seq_protocolo		nr_seq_protocolo,
	LPAD(substr(f.cd_interno,1,8),8,0)	cd_prestador,
	0 				nr_seq_lote,
	d.nr_interno_conta		nr_interno_conta,
	0				cd_serie_documento,
	LOCALTIMESTAMP 			dt_geracao,
	...
	to_char(d.dt_item,'ddmmyyyyhhmi') dt_insumo,
...
and	somente_numero(coalesce(d.cd_grupo_gasto,0)) in (10,11,12,13,20,99)
and 	coalesce(d.ie_responsavel_credito,'X') <> 'MMOPM'
and 	d.ie_tipo_item = 1
group by
	c.nr_seq_protocolo,
	...
	somente_numero(coalesce(d.cd_item_convenio,d.cd_item)),
	to_char(d.dt_item,'ddmmyyyyhhmi'),
	to_char(c.nr_atendimento)
...
select	/* Insumos (Materiais e Medicamentos) */
	...
	0 				nr_seq_autorizacao,
	0 				ie_tipo_percentual,
	CASE WHEN d.cd_item_convenio='00000009' THEN 28  ELSE (d.cd_grupo_gasto)::numeric  END 	ie_tipo_insumo, -- OS 518612
	somente_numero(coalesce(d.cd_item_convenio,d.cd_item))
					cd_insumo,
	sum(d.qt_item) 			qt_insumo,
	to_char(d.dt_item,'ddmmyyyyhhmi') dt_insumo,
...
where   c.nr_interno_conta = d.nr_interno_conta
and 	c.nr_seq_protocolo = f.nr_seq_protocolo
and	((d.nr_seq_proc_pacote is null) or (d.cd_item_convenio = 000000009))
and 	((d.ie_tipo_item = 2) or (d.cd_item_convenio = 000000009))
and 	coalesce(d.ie_responsavel_credito,'X') <> 'MMOPM'
group by
	c.nr_seq_protocolo,
	substr(f.cd_interno,1,8),
...
select	/* Doc Pacote,  no caso da Guia do Atendimento ser diferente da Guia do Pacote  */
	...
	6				tp_ordem_reg,
	b.nr_seq_protocolo		nr_seq_protocolo,
	LPAD(substr(coalesce(hdjb_obter_regra_prestador(b.ie_tipo_atendimento,d.ie_tipo_guia, b.nr_seq_protocolo, b.cd_medico_resp),'990048'),1,8),8,0)
					cd_prestador,
	0 				nr_seq_lote,
	b.nr_interno_conta		nr_interno_conta,
	1				cd_serie_documento,
	...
	substr(b.cd_usuario_convenio,5,13) nr_carteira,
	LOCALTIMESTAMP				dt_referencia,
		CASE WHEN b.ie_tipo_atendimento=1 THEN 	CASE WHEN b.dt_periodo_final=b.dt_alta THEN b.cd_motivo_alta  ELSE CASE WHEN b.dt_alta IS NULL THEN 10  ELSE b.cd_motivo_alta END  END   ELSE b.cd_motivo_alta END  cd_motivo_alta,
	substr(b.cd_cid_principal,1,4)	cd_cid,
	max(b.dt_entrada)		dt_entrada,
	max(b.dt_alta)			dt_alta,
	CASE WHEN d.ie_tipo_guia='E' THEN null WHEN d.ie_tipo_guia='C' THEN null WHEN d.ie_tipo_guia='A' THEN  null  ELSE to_char(trunc(b.dt_periodo_inicial,'year'),'yyyy') END  dt_ano_guia,
...
WHERE b.nr_seq_protocolo	= a.nr_seq_protocolo  and i.nr_interno_conta 	= b.nr_interno_conta and LOCALTIMESTAMP < to_date('01/07/2009','dd/mm/yyyy') 	-- Conforme OS 151051, à partir de 01/07/2009 esse registro não deve mais ir para o convênio
  and coalesce(d.nr_sequencia,0) =
	...
	substr(b.cd_usuario_convenio,5,13),
		CASE WHEN b.ie_tipo_atendimento=1 THEN 	CASE WHEN b.dt_periodo_final=b.dt_alta THEN b.cd_motivo_alta  ELSE CASE WHEN b.dt_alta IS NULL THEN 10  ELSE b.cd_motivo_alta END  END   ELSE b.cd_motivo_alta END ,
...
where   c.nr_interno_conta = d.nr_interno_conta
and	c.nr_seq_protocolo = f.nr_seq_protocolo
and	((d.nr_seq_proc_pacote is not null) and (somente_numero(coalesce(d.cd_grupo_gasto,0)) <> 35) and (d.cd_item_convenio <> 000000009))
and 	coalesce(d.ie_responsavel_credito,'X') <> 'MMOPM'
...
select	/* Trailler */
	...
	LOCALTIMESTAMP 			dt_emissao,
 	...
 	4 				qt_reg_proc,
	5 				qt_reg_insumo,
	sum(CASE WHEN e.nr_seq_proc_pacote IS NULL THEN 	CASE WHEN e.cd_grupo_proc=20203 THEN  0  ELSE CASE WHEN e.ie_responsavel_credito='P' THEN 	e.vl_honorario  ELSE e.vl_total_item END  END   ELSE CASE WHEN e.cd_item_convenio=000000009 THEN 	 e.vl_total_item  ELSE 0 END  END )
					vl_total_lote,
	0				nr_seq_pacote,
	0				nr_lote,
	0 				qt_nasc_vivos_termo,
