...
CREATE OR REPLACE VIEW abramge_hsr_v2 (ds_registro, tp_registro, ie_servico, cd_registro, cd_origem, nr_lote, nr_interno_conta, nr_parcela, cd_convenio, nm_convenio, tp_prestador, cd_prestador, nm_prestador, dt_geracao, nr_remessa, cd_plano, cd_paciente, cd_autorizacao, cd_acomodacao, dt_entrada, dt_saida, dt_inicio_cobranca, dt_fim_cobranca, ie_tipo_atendimento, cd_cid, cd_proc_principal, cd_medico_convenio, nr_crm, uf_crm, nr_crm_req, uf_crm_req, nr_seq_item, cd_item, nm_item, ie_emergencia, ie_extra, dt_item, ie_funcao_medico, qt_item, vl_item, ie_cirurgia_multipla, vl_total_item, qt_item_conta, nm_paciente, nr_fatura, qt_conta, vl_total_lote, nr_seq_protocolo, ds_espaco, nr_registro, ie_matmed, ds_ma_me_tx, ie_tipo_atend_esp, ie_tipo_atend_tasy, nr_doc_convenio, cd_senha) AS SELECT		'HEADER        '			DS_REGISTRO,
		1					TP_REGISTRO, 
		'CSMH'					IE_SERVICO, 
		0					CD_REGISTRO, 
		(substr(coalesce(A.CD_INTERNO,0),1,5))::numeric  CD_ORIGEM, 
		0					NR_LOTE, 
		0					NR_INTERNO_CONTA, 
		0					NR_PARCELA, 
		...
		substr(ELIMINA_ACENTUACAO(UPPER(A.NM_HOSPITAL)),1,30) 
							NM_PRESTADOR, 
...
SELECT		'CABECALHO CONTA   '			DS_REGISTRO, 
		...
		'CSMH'					IE_SERVICO, 
		1					CD_REGISTRO, 
		(substr(coalesce(B.CD_INTERNO,0),1,5))::numeric  CD_ORIGEM, 
		...
		somente_numero(substr(Obter_dados_categ_conv(B.nr_atendimento,'G'),1,8)) nr_doc_convenio, 
		substr(Obter_dados_categ_conv(B.nr_atendimento,'S'),1,10) cd_senha 
...
SELECT   	'ITEM DA CONTA     '	 	DS_REGISTRO, 
    	3	         	   	TP_REGISTRO, 
      	'CSMH' 	 	       	IE_SERVICO, 
      	2       				CD_REGISTRO, 
      	(substr(coalesce(W.CD_INTERNO,0),1,5))::numeric  CD_ORIGEM, 
     	...
     	0	      	  		IE_TIPO_ATENDIMENTO, 
      	...
      	' '       	     	CD_PROC_PRINCIPAL, 
		' '					CD_MEDICO_CONVENIO, 
   		substr(NR_CRM_EXECUTOR,1,6)  		NR_CRM, 
      	' '	    	       	UF_CRM, 
		' '					NR_CRM_REQ, 
		' '					UF_CRM_REQ, 
		max(C.NR_SEQ_ITEM) 			NR_SEQ_ITEM, 
      	C.CD_ITEM_CONVENIO			CD_ITEM, 
      	substr(ELIMINA_ACENTUACAO(upper(C.DS_ITEM)),1,40) 
							NM_ITEM, 
...
--and 		nvl(c.ie_responsavel_credito,'X') in ('CRC','MCOO') --Solicitado pelo cliente na OS 823070 
and 		coalesce(C.ie_responsavel_credito,'X') in ('RM','MCOO') 
GROUP BY	C.NR_SEQ_PROTOCOLO, 
		...
		substr(NR_CRM_EXECUTOR,1,6), 
      	C.NR_SEQ_PROTOCOLO, 
		W.ie_tipo_atendimento, 
		(substr(coalesce(W.CD_INTERNO,0),1,5))::numeric  
...
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA   --and 		nvl(e.ie_responsavel_credito,'X') in ('CRC','MCOO')--Solicitado pelo cliente na OS 823070 
  and coalesce(e.ie_responsavel_credito,'X') in ('RM','MCOO') GROUP BY	B.NR_SEQ_PROTOCOLO, 
     	B.NR_INTERNO_CONTA, 
		substr(ELIMINA_ACENTUACAO(UPPER(B.NM_PACIENTE)),1,40), 
		B.ie_tipo_atendimento, 
		(substr(coalesce(B.CD_INTERNO,0),1,5))::numeric  
...
SELECT 	'TRAILLER       ' 		DS_REGISTRO, 
      	...
      	'CSMH'  				IE_SERVICO, 
      	9   			      	CD_REGISTRO, 
      	(substr(coalesce(f.CD_INTERNO,0),1,5))::numeric  CD_ORIGEM, 
      	99999999		   		NR_LOTE, 
      	99999999	   			NR_INTERNO_CONTA, 
      	99    		      	NR_PARCELA, 
      	0 	           		CD_CONVENIO, 
...
LEFT OUTER JOIN estrutura_procedimento_v x ON (E.CD_ITEM = X.CD_PROCEDIMENTO AND E.IE_ORIGEM_PROCED = X.IE_ORIGEM_PROCED)
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA and B.nr_seq_protocolo	= f.nr_seq_protocolo   --and 		nvl(e.ie_responsavel_credito,'X') in ('CRC','MCOO')--Solicitado pelo cliente na OS 823070 
  and coalesce(e.ie_responsavel_credito,'X') in ('RM','MCOO') GROUP BY	(substr(coalesce(f.CD_INTERNO,0),1,5))::numeric , 
		B.NR_SEQ_PROTOCOLO;
