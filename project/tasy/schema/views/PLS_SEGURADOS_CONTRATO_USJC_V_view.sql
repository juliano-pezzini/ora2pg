...
CREATE OR REPLACE VIEW pls_segurados_contrato_usjc_v (tp_registro, tp_arquivo, cd_empresa, cd_unidade_destino, dt_geracao, ie_tipo_remetente, ie_tipo_carga, cd_versao_layout, ie_tipo_movimentacao, nm_beneficiario, ie_sexo, dt_nascimento, ie_estado_civil, nr_cpf, nr_identidade, nr_cbo, ds_endereco, ds_bairro, nr_cep, ds_cidade, ie_uf, ie_grau_parentesco, dt_admissao, dt_inclusao, dt_exclusao, cd_matricula, cd_unidade_atend, cd_unidade_neg, cd_codigo_ant, nr_proposta_contrato, nr_seq_segurado, ie_responsavel, cd_responsavel, nm_benefe_cartao, nm_internacional, cd_uni_origem_resp, cd_cart_uni_resp, cd_departamento, cd_secao, cd_setor, ie_padrao_cobertura, info_espec_1, info_espec_2, info_espec_3, info_espec_4, nr_pis_pasep, nr_cartao_sus, nm_mae, nm_pai, ie_abrangencia, ie_tipo_contratacao, nr_protocolo_ans, ds_email, cd_vendedor, ds_orgao_emissor, nm_pais, ie_motivo_inclusao, nr_registro_ans, nr_seq_motivo_can, dt_obito, cd_mod_mae, proposta_mae, nr_seq_seg_mae, ds_nacionalidade, ds_nat_doc_ident, sg_emissora_ci, dt_emissao_ci, nr_telefone, nr_cel_benef, nr_tipo_carencia, ie_acomodacao, dt_fim_carencia, qt_total_segurados, nr_seq_contrato, nr_ordem, reservado_1, reservado_2, reservado_3, reservado_4, reservado_5, reservado_6, reservado_7, reservado_8, reservado_9, reservado_10, reservado_11, reservado_12, reservado_13, cd_funcao, nr_declaracao_nasc) AS select	1										tp_registro,
	''										tp_arquivo, 
	CASE WHEN a.cd_cgc_estipulante IS NULL THEN obter_dados_pf(a.cd_pf_estipulante,'CSA')  ELSE (	select	max(x.CD_SISTEMA_ANT) 			FROM	pessoa_juridica	x 			where	x.cd_cgc	= a.cd_cgc_estipulante) END 		cd_empresa, 
	'0004'										cd_unidade_destino, 
	LOCALTIMESTAMP										dt_geracao, 
	'E'										ie_tipo_remetente, 
	CASE WHEN a.nr_seq_proposta IS NULL THEN 'C'  ELSE 'M' END 						ie_tipo_carga, 
...
from (	select	3										tp_registro, 
			...
			a.nr_proposta_ant								nr_proposta_contrato, 
			...
			' '										reservado_12, 
			' '										reservado_13, 
			b.cd_matricula_estipulante							cd_funcao, 
			to_char(d.cd_declaracao_nasc_vivo)						nr_declaracao_nasc 
		FROM compl_pessoa_fisica e, pessoa_fisica d, pls_plano c, pls_contrato a, pls_segurado b
...
WHERE b.nr_seq_plano			= c.nr_sequencia and b.nr_seq_contrato		= a.nr_sequencia and b.cd_pessoa_fisica		= d.cd_pessoa_fisica and e.cd_pessoa_fisica		= d.cd_pessoa_fisica    and e.ie_tipo_complemento		= 1 ) alias25 
