...
where	--ie_tipo_atend_real = 1 
	exists (	select 	1 
		...
		and	coalesce(x.ie_total_interf,0)	= 5) 
...
0, LOCALTIMESTAMP, '', '', '', '', '', LOCALTIMESTAMP, LOCALTIMESTAMP, '', 0, 0, 
  ...
  LOCALTIMESTAMP, 
  LOCALTIMESTAMP, 
  CASE WHEN a.ie_tipo_item=1 THEN sum(a.qt_item)  ELSE sum(a.qt_item) END  qt_procedimento, 
  ...
  CASE WHEN x.cd_setor_atendimento=111 THEN CASE WHEN a.nr_cpf_executor IS NULL THEN '85377174000392'  ELSE a.nr_cpf_executor END   ELSE '85377174000392' END  cd_cgc_cpf_resp_cred, 
  x.dt_entrada dt_procedimento, 
  a.cd_funcao_executor cd_atividade_prestador, 
  a.nr_porte_anestesico ie_porte_anestesico, 
  -- Retirado a linha abaixo em 24/04/2007 e substituída pela linha abaixo da linha retirada (OS 55549) 
  --decode(a.ie_tipo_item,1, min(a.dt_item), nvl(x.dt_alta,x.dt_periodo_final)) hr_atendimento, 
  x.dt_entrada hr_atendimento, 
  ' ' ie_cirurgia_multipla, 
  a.pr_funcao_participante tx_procedimento, 
  'N' ie_pagar_honorario, 
  CASE WHEN a.cd_classif_setor='4' THEN 'S'  ELSE 'N' END  ie_procedimento_uti , 0, 0, 0, 
  CASE WHEN a.ie_tipo_item=1 THEN a.cd_item  ELSE Campo_numerico(a.cd_item_convenio) END  cd_item, 
  '00', 
 '00000000000000000000000000000000000000000000' ds_zeros, 
  '                      ' ds_espaco, 
  ' ' nm_usuario_carteirinha, 
...
where	a.nr_interno_conta	= x.nr_interno_conta 
and	coalesce(a.ie_total_interf,0)	<> 5 
and 	a.cd_item_convenio <> '98020059' 
group by Campo_numerico(a.cd_item_convenio), 
	a.nr_interno_conta, 
	a.nr_seq_protocolo, 
	coalesce(a.nr_cpf_executor,a.cd_cgc_hospital), 
	CASE WHEN x.cd_setor_atendimento=111 THEN CASE WHEN a.nr_cpf_executor IS NULL THEN '85377174000392'  ELSE a.nr_cpf_executor END   ELSE '85377174000392' END , 
	dt_entrada, 
	...
	a.ie_emite_conta, 
	a.cd_classif_setor,	 
	CASE WHEN a.ie_tipo_item=1 THEN a.cd_item  ELSE Campo_numerico(a.cd_item_convenio) END , 
	a.ie_tipo_item, 
	 substr(x.nr_doc_convenio,1,8) 
...
union all
 ...
 where a.nr_seq_protocolo = b.nr_seq_protocolo 
  ...
  and b.cd_item_convenio <> '98020059' 
