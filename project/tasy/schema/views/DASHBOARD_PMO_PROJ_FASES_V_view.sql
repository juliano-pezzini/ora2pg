...
CREATE OR REPLACE VIEW dashboard_pmo_proj_fases_v (nr_sequencia, ds_titulo, area, nr_seq_area_fase, ds_fase, dt_inicio_prev, dt_inicio_real, dt_fim_prev, dt_fim_real, qt_hora_prev, qt_hora_real, pr_prev, pr_real) AS select   p.nr_sequencia,
         p.ds_titulo,
         ...
         obter_pr_real_proj_fase(p.nr_sequencia, e.nr_seq_area_fase) pr_real
FROM     proj_projeto p,
         proj_cronograma c,
         proj_cron_etapa e,
