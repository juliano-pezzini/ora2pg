...
CREATE OR REPLACE VIEW interf_dativa_envio_hbjp_v (ds_registro, tp_registro, ie_servico, cd_registro, cd_origem, nr_lote, nr_interno_conta, nr_parcela, cd_convenio, nm_convenio, tp_prestador, cd_prestador, nm_prestador, dt_geracao, nr_remessa, cd_plano, cd_paciente, cd_autorizacao, cd_senha, cd_acomodacao, dt_entrada, dt_saida, dt_inicio_cobranca, dt_fim_cobranca, ie_tipo_atendimento, cd_cid, cd_proc_principal, cd_medico_convenio, nr_crm, uf_crm, nr_crm_req, uf_crm_req, nr_seq_item, cd_item, nm_item, ie_emergencia, ie_extra, dt_item, ie_funcao_medico, qt_item, vl_item, ie_cirurgia_multipla, vl_total_item, qt_item_conta, nm_paciente, nr_fatura, qt_conta, vl_total_lote, nr_seq_protocolo, ds_espaco, nr_registro, cd_cgc_prestador, cd_motivo_alta, ie_laudo_especial, ie_laudo_prorrogacao, dt_autorizacao, dt_validade_carteirinha, nr_prontuario, nm_medico_resp, ie_tipo_doc_paciente, nr_doc_paciente, ie_matmed, nr_crm_resp, uf_crm_resp, cd_especialidade, vl_proc_principal, ie_tipo_cirurgia, cd_unidade_atend, qt_diaria_enfermaria, qt_diaria_uti, cd_leitora, ie_fim_registro, ie_tipo_atend_tasy, nr_doc_convenio) AS SELECT		'HEADER               ' 	DS_REGISTRO,
		1				TP_REGISTRO,
		'CSMH'				IE_SERVICO,
		0				CD_REGISTRO,
...
SELECT		'CABECALHO CONTA      '	DS_REGISTRO,
		...
		to_char(B.nr_interno_conta)	NR_INTERNO_CONTA,
		1				NR_PARCELA,
		0				CD_CONVENIO,
		' '				NM_CONVENIO,
		'J'				TP_PRESTADOR,
		...
		substr(B.CD_USUARIO_CONVENIO,1,18)
						CD_PACIENTE,
		substr(c.CD_AUTORIZACAO,1,10)
						CD_AUTORIZACAO,
		substr(coalesce(c.cd_senha,'0'),1,10)CD_SENHA,
...
SELECT     	'ITEM DA CONTA         ' 	DS_REGISTRO,
		...
		to_char(c.NR_INTERNO_CONTA)	NR_INTERNO_CONTA,
