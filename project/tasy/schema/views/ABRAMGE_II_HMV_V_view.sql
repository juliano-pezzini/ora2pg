...
SELECT     	'ITEM DA CONTA         ' 	DS_REGISTRO,
           	...
           	0                        	IE_TIPO_ATENDIMENTO,
           	null                     	CD_CID,
           	CASE WHEN c.nr_seq_item=c.nr_seq_proc_pacote THEN  somente_numero(c.cd_item_convenio)  ELSE 0 END  CD_PROC_PRINCIPAL,
		' '				CD_MEDICO_CONVENIO,
           	' '                       	NR_CRM,
           	' '                       	UF_CRM,
		' '				NR_CRM_REQ,
		...
		sum(CASE WHEN c.nr_seq_item=c.nr_seq_proc_pacote THEN  vl_total_item  ELSE 0 END ) vl_proc_principal,
...
WHERE c.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA and w.nr_seq_protocolo = x.nr_seq_protocolo  and c.ie_tipo_item	= 1 and c.qt_item		<> 0 and ((coalesce(Z.ie_entra_conta,'S') = 'S') or (c.nr_seq_proc_pacote = c.nr_seq_item)) and c.cd_medico_executor is not null GROUP BY	somente_numero(coalesce(x.cd_interno,0)),
		c.NR_SEQ_PROTOCOLO,
           		...
           		c.NR_SEQ_PROTOCOLO,
		somente_numero(c.nr_crm_executor),
		CASE WHEN c.nr_seq_item=c.nr_seq_proc_pacote THEN  somente_numero(c.cd_item_convenio)  ELSE 0 END ,
		c.uf_crm_executor,
