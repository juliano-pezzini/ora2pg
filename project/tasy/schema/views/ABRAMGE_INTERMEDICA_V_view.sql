...
WHERE C.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA  and coalesce(Z.ie_entra_conta,'S') = 'S' and C.ie_tipo_item	= 1 and C.cd_medico_executor is not null GROUP BY	C.NR_SEQ_PROTOCOLO,
           	C.NR_INTERNO_CONTA,
           	C.CD_ITEM_CONVENIO,
           	substr(upper(C.DS_ITEM),1,50),
           	CASE WHEN W.IE_CARATER_INTER='U' THEN 'S'  ELSE 'N' END ,
