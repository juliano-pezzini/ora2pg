...
CREATE OR REPLACE VIEW gama_v (ds_registro, tp_registro, ie_servico, cd_registro, cd_origem, nr_lote, nr_interno_conta, nr_parcela, cd_convenio, nm_convenio, tp_prestador, cd_prestador, nm_prestador, dt_geracao, nr_remessa, cd_plano, cd_paciente, cd_autorizacao, cd_acomodacao, dt_entrada, dt_saida, dt_inicio_cobranca, dt_fim_cobranca, ie_tipo_atendimento, cd_cid, cd_proc_principal, cd_medico_convenio, nr_crm, uf_crm, nr_crm_req, uf_crm_req, nr_seq_item, cd_item, nm_item, ie_emergencia, ie_extra, dt_item, ie_funcao_medico, qt_item, vl_item, ie_cirurgia_multipla, vl_total_item, qt_item_conta, nm_paciente, nr_fatura, qt_conta, vl_total_lote, nr_seq_protocolo, ds_espaco, nr_registro, ie_matmed, ds_ma_me_tx, ie_tipo_atend_esp, ie_tipo_atend_tasy) AS SELECT		'HEADER        ' 	DS_REGISTRO,
		...
		' '				CD_AUTORIZACAO, 
		0				CD_ACOMODACAO, 
		to_char(LOCALTIMESTAMP,'ddmmyyhhmi')	DT_ENTRADA, 
		to_char(LOCALTIMESTAMP,'ddmmyyhhmi')	DT_SAIDA, 
		to_char(LOCALTIMESTAMP,'ddmmyyhhmi')	DT_INICIO_COBRANCA, 
		to_char(LOCALTIMESTAMP,'ddmmyyhhmi')	DT_FIM_COBRANCA, 
...
SELECT		'CABECALHO CONTA   '	DS_REGISTRO, 
		...
		to_char(B.DT_ENTRADA,'ddmmyyhhmi') DT_ENTRADA, 
		CASE WHEN B.ie_tipo_atendimento='1' THEN to_char(coalesce(B.DT_ALTA,B.DT_PERIODO_FINAL),'ddmmyyhhmi')  ELSE to_char('0000000000') END  
						DT_SAIDA, 
		CASE WHEN B.ie_tipo_atendimento='1' THEN to_char(B.DT_PERIODO_INICIAL,'ddmmyyhhmi')  ELSE to_char('0000000000') END  								DT_INICIO_COBRANCA, 
		CASE WHEN B.ie_tipo_atendimento='1' THEN to_char(coalesce(B.DT_ALTA,B.DT_PERIODO_FINAL),'ddmmyyhhmi')  ELSE to_char('0000000000') END  
						DT_FIM_COBRANCA, 
