...
WHERE c.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA and w.nr_seq_protocolo = x.nr_seq_protocolo  and c.ie_tipo_item	= 1 and c.qt_item		<> 0 and ((coalesce(Z.ie_entra_conta,'S') = 'S') or ((c.NR_SEQ_PROC_PACOTE = c.NR_SEQ_ITEM) or (c.NR_SEQ_PROC_PACOTE is NULL))) and c.cd_medico_executor is not null GROUP BY	somente_numero(coalesce(x.cd_interno,0)),
		c.NR_SEQ_PROTOCOLO,
           		c.NR_INTERNO_CONTA,
           		c.CD_ITEM_CONVENIO,
           		substr(ELIMINA_ACENTUACAO(upper(c.DS_ITEM)),1,50),
		...
		w.cd_senha_guia,
		w.nm_paciente
having		SUM(c.QT_ITEM) <> 0 and SUM(CASE WHEN(coalesce(c.vl_honorario,0) + coalesce(c.vl_custo_oper,0) + coalesce(c.vl_filme,0))=0 THEN c.vl_total_item  ELSE (coalesce(c.vl_honorario,0) + coalesce(c.vl_custo_oper,0) + coalesce(c.vl_filme,0)) END ) > 0
...
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA and B.nr_seq_protocolo	= y.nr_seq_protocolo    and ((coalesce(Z.ie_entra_conta,'S') = 'S') or ((e.NR_SEQ_PROC_PACOTE = e.NR_SEQ_ITEM) or (e.NR_SEQ_PROC_PACOTE is NULL))) GROUP BY	somente_numero(coalesce(y.cd_interno,0)),
		B.NR_SEQ_PROTOCOLO,
           	B.NR_INTERNO_CONTA,
		substr(ELIMINA_ACENTUACAO(UPPER(B.NM_PACIENTE)),1,40),
