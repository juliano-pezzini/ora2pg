...
CREATE OR REPLACE VIEW eis_consumo_matmed_v (cd_centro_custo, cd_material, dt_mesano_referencia, cd_estabelecimento, cd_local_estoque, ie_consignado, qt_estoque, vl_estoque, cd_grupo_material, ds_grupo_material, cd_subgrupo_material, ds_subgrupo_material, cd_classe_material, ds_classe_material, ds_material, ds_centro_custo, ds_local_estoque, vl_custo_medio, ie_curva_abc, cd_operacao_estoque, ds_operacao_estoque, ie_tipo_requisicao, ds_tipo_requisicao, cd_acao, ds_acao, dt_movimento_estoque, nm_estabelecimento, nr_seq_familia, ds_familia) AS select	a.cd_centro_custo,
	a.cd_material_estoque cd_material,
	a.dt_mesano_referencia,
	a.cd_estabelecimento,
	a.cd_local_estoque,
	CASE WHEN coalesce(o.ie_consignado,0)=0 THEN 'N'  ELSE 'S' END  ie_consignado,
	CASE WHEN o.ie_consumo='A' THEN  a.qt_estoque WHEN o.ie_consumo='D' THEN  a.qt_estoque * -1  ELSE 0 END  qt_estoque,
	...
	a.cd_acao,
	CASE WHEN a.cd_acao=2 THEN wheb_mensagem_pck.get_texto(301937)  ELSE wheb_mensagem_pck.get_texto(301936) END  ds_acao,
...
FROM subgrupo_material s, operacao_estoque o, local_estoque l, grupo_material g, material e, classe_material c, movimento_estoque_v1 a
LEFT OUTER JOIN centro_custo b ON (a.cd_centro_custo = b.cd_centro_custo)
WHERE a.cd_material_estoque = e.cd_material and a.cd_operacao_estoque = o.cd_operacao_estoque  and a.cd_local_estoque = l.cd_local_estoque and o.ie_consumo in ('A','D') and e.cd_classe_material = c.cd_classe_material and c.cd_subgrupo_material = s.cd_subgrupo_material and s.cd_grupo_material = g.cd_grupo_material and a.dt_processo is not null;
