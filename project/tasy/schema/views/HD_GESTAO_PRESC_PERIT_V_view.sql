...
CREATE OR REPLACE VIEW hd_gestao_presc_perit_v (nm_paciente, nr_prescricao, nr_atendimento, nm_medico, ds_convenio, dt_inicio, dt_final, ds_tipo_hemodialise, ds_setor_atendimento, dt_liberacao, ie_tipo_dialise, cd_setor_atendimento, ie_conferida, cd_pessoa_fisica) AS (
select 	substr(obter_nome_pessoa_fisica(m.cd_pessoa_fisica,null),1,255) nm_paciente,
	...
	substr(obter_valor_dominio('2154', p.ie_tipo_peritoneal), 1, 255) ds_tipo_hemodialise,
...
and	((p.ie_tipo_peritoneal in (	
		select	ie_tipo_peritoneal
	   	from	hd_regra_prescricao a,
			hd_regra_presc_perit b
		...
		and	b.ie_situacao = 'A'
		and	a.ie_situacao = 'A')) or (p.ie_tipo_peritoneal in (	
		select	ie_tipo_peritoneal
	   	from	hd_regra_prescricao a,
			hd_regra_presc_perit b 
		where	a.cd_estabelecimento = wheb_usuario_pck.get_cd_estabelecimento
	...
	and	not exists (
			...
			and	b.cd_setor_atendimento = m.cd_setor_atendimento
			and	b.ie_situacao = 'A'
			and	a.ie_situacao = 'A')) or (not exists (
		select	1
		from	hd_regra_prescricao a,
			hd_regra_presc_perit b
		where	a.cd_estabelecimento = wheb_usuario_pck.get_cd_estabelecimento
