...
select 
    ...
    somente_numero(substr(coalesce(c.cd_item_convenio,coalesce(c.cd_item,'0')),1,8)) 
                        cd_servico, 
    CASE WHEN c.nr_seq_proc_pacote=c.nr_seq_item THEN  obter_valor_pacote(c.nr_seq_proc_pacote)  ELSE CASE WHEN C.CD_AREA_PROC=4 THEN SUM(C.VL_HONORARIO)  ELSE sum(c.vl_total_item) END  END    vl_servico, 
...
WHERE b.nr_sequencia        = 
	...
	c.nr_seq_item, 
	obter_valor_pacote(c.nr_seq_proc_pacote) 
