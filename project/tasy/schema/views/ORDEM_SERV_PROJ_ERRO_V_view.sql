...
CREATE OR REPLACE VIEW ordem_serv_proj_erro_v (qt_ordens_abertas, qt_ordens_processo, qt_ordens_cliente, qt_ordens_encerradas, dt_mes, dt_mes_order, nr_seq_proj) AS SELECT	SUM(qt_ordens_abertas) + SUM(qt_ordens_cliente) + SUM(qt_ordens_encerradas) qt_ordens_abertas,
...
UNION ALL
	...
	SELECT	0 qt_ordens_abertas, 
		0 qt_ordens_cliente, 
		coalesce(SUM(qt_ordens),0) qt_ordens_encerradas, 
		TRUNC(x.dt_fim_real,'month'), nr_seq_proj 
	FROM (SELECT	COUNT(*) qt_ordens, 
...
UNION ALL
	...
	SELECT	0 qt_ordens_abertas, 
		coalesce(SUM(qt_ordens),0) qt_ordens_cliente, 
		0 qt_ordens_encerradas, 
...
UNION ALL
	...
	SELECT	coalesce(SUM(qt_ordens),0) qt_ordens_abertas, 
		0 qt_ordens_cliente, 
		0 qt_ordens_encerradas, 
		TRUNC(x.dt_inicio_real,'month'), 
		nr_seq_Proj 
	FROM (SELECT	COUNT(*) qt_ordens, 
		...
		GROUP BY	d.nr_seq_Proj, TRUNC(a.dt_inicio_real,'month')) x 
	WHERE	TRUNC(x.dt_inicio_real) BETWEEN ADD_MONTHS(TRUNC(LOCALTIMESTAMP,'month'),-5) AND TRUNC(LOCALTIMESTAMP,'month') 
	GROUP BY	TRUNC(x.dt_inicio_real,'month'), nr_seq_proj) alias78 
