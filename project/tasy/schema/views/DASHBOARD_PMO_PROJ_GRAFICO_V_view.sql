...
CREATE OR REPLACE VIEW dashboard_pmo_proj_grafico_v (nr_seq_proj, dt_inicio_prev, dt_inicio_real, dt_fim_prev, dt_fim_real, pr_prev, pr_real) AS select  p.nr_sequencia nr_seq_proj,
		e.dt_inicio_prev dt_inicio_prev,
		 e.dt_inicio_real dt_inicio_real,
		 e.dt_fim_prev dt_fim_prev,
		 c.dt_fim_real dt_fim_real,
		 obter_pr_prev_etapa_proj(p.nr_sequencia ,e.dt_fim_prev) pr_prev,
		 obter_pr_real_etapa_proj(p.nr_sequencia, e.dt_fim_prev) pr_real
FROM     proj_projeto p,
		 proj_cronograma c,
		 proj_cron_etapa e
