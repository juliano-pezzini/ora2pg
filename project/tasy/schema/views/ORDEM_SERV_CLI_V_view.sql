...
CREATE OR REPLACE VIEW ordem_serv_cli_v (qt_ordens_abertas, qt_ordens_processo, qt_ordens_cliente, qt_ordens_encerradas, dt_mes, dt_mes_order, nr_seq_proj) AS SELECT	SUM(qt_ordens_abertas) + SUM(qt_ordens_cliente) + SUM(qt_ordens_encerradas) qt_ordens_abertas,
...
FROM (	SELECT	0 qt_ordens_abertas, 
	...
	WHERE	TRUNC(x.dt_fim_real,'month') < ADD_MONTHS(TRUNC(LOCALTIMESTAMP,'month'),-5) 
...
UNION ALL
	...
	FROM (SELECT	COUNT(*) qt_ordens, 
		...
		AND	a.nr_seq_estagio IN (SELECT  nr_sequencia 
...
UNION ALL
	...
	SELECT	SUM(qt_ordens) qt_ordens_abertas, 
		0 qt_ordens_cliente, 
		0 qt_ordens_encerradas, 
		TRUNC(x.dt_inicio_real,'month') dt_mes, 
		nr_seq_proj 
	FROM (SELECT	COUNT(*) qt_ordens, 
			TRUNC(a.dt_inicio_real,'month') dt_inicio_real,d.nr_seq_proj 
		FROM	man_ordem_servico_v a, 
