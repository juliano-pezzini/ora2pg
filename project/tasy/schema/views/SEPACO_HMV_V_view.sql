...
WHERE C.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA  and coalesce(Z.ie_entra_conta,'S') = 'S' AND C.cd_item in (select x.cd_procedimento 
...
SELECT   	'ITEM DA CONTA     ' 	DS_REGISTRO, 
		...
		' '				UF_CRM_REQ, 
      	...
      	C.CD_FUNCAO_EXECUTOR     	IE_FUNCAO_MEDICO, 
      	SUM(C.QT_ITEM)        	QT_ITEM, 
      	SUM(C.vl_total_item - coalesce(C.vl_filme,0))	VL_ITEM, 
      	0            	IE_CIRURGIA_MULTIPLA, 
      	0		   		VL_TOTAL_ITEM, 
      	0	   			QT_ITEM_CONTA, 
	   	' '				NM_PACIENTE, 
		...
		CASE WHEN C.ie_total_interf=2 THEN '1' WHEN C.ie_total_interf=3 THEN '2'  ELSE ' ' END  
						IE_MAT_MED, 
		CASE WHEN C.cd_item=90040001 THEN ' ' WHEN C.cd_item=90040004 THEN ' ' WHEN C.cd_item=90020001 THEN ' ' WHEN C.cd_item=90020003 THEN ' ' WHEN C.cd_item=90060001 THEN ' ' WHEN C.cd_item=90060002 THEN ' ' WHEN C.cd_item=90070001 THEN ' ' WHEN C.cd_item=90070003 THEN ' ' WHEN C.cd_item=90030003 THEN ' ' WHEN C.cd_item=93010024 THEN ' ' WHEN C.cd_item=91030001 THEN ' ' WHEN C.cd_item=92010005 THEN ' ' WHEN C.cd_item=92010006 THEN ' ' WHEN C.cd_item=91010004 THEN ' ' WHEN C.cd_item=91010005 THEN ' ' WHEN C.cd_item=91010006 THEN ' ' WHEN C.cd_item=91010007 THEN ' ' WHEN C.cd_item=91010008 THEN ' ' WHEN C.cd_item=91010009 THEN ' ' WHEN C.cd_item=91010010 THEN ' '  ELSE CASE WHEN C.ie_total_interf=2 THEN 'MA' WHEN C.ie_total_interf=3 THEN 'ME' WHEN C.ie_total_interf=5 THEN 'TX' WHEN C.ie_total_interf=6 THEN 'TX' WHEN C.ie_total_interf=7 THEN 'TX'  ELSE ' ' END  END  
						DS_MA_ME_TX, 
		' '				ie_tipo_atend_esp, 
		W.ie_tipo_atendimento		ie_tipo_atend_tasy 
...
AND		C.cd_item not in (select x.cd_procedimento 
		from estrutura_procedimento_v x 
