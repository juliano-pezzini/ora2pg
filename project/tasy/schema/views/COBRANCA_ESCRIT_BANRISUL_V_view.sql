...
CREATE OR REPLACE VIEW cobranca_escrit_banrisul_v (tp_registro, nr_seq_envio, nr_conta, nm_empresa, cd_agencia, nr_dac, dt_gravacao, nr_inscricao, cd_agencia_bancaria, cd_conta_corrente, cd_dac, nr_titulo, nr_carteira, nr_nosso_numero, cd_ocorrencia, nr_documento, dt_vencimento, vl_titulo, cd_agencia_cobranca, dt_emissao, ds_instrucao, dt_limite_desconto, vl_desconto, vl_abatimento, vl_iof, vl_juros_atraso, cd_inscricao_sacado, nr_inscricao_sacado, nm_sacado, ds_logradouro, ds_bairro, ds_cidade, sg_uf, nr_cep, nm_sacador_avalista, dt_mora, qt_dias_prazo, ds_instrucao1, ds_instrucao2, cd_instrucao) AS SELECT	0 				tp_registro,
...
SELECT	1				tp_registro, 
	...
	CASE WHEN c.cd_ocorrencia='06' THEN (select max(x.dt_instrucao) 				from	titulo_rec_cobr_instrucao x 				where	x.nr_seq_tit_rec_cobr = c.nr_sequencia) 					  ELSE coalesce(b.dt_pagamento_previsto, b.dt_vencimento) END  dt_vencimento, 
	coalesce(b.vl_saldo_titulo,0)		vl_titulo, 
	somente_numero(b.cd_agencia_bancaria) cd_agencia_cobranca, 
	b.dt_emissao			dt_emissao, 
	somente_numero(c.cd_instrucao) ds_instrucao, 
	LOCALTIMESTAMP				dt_limite_desconto, 
	...
	where	x.nr_seq_tit_rec_cobr = c.nr_sequencia) ds_instrucao1, 
	(select coalesce(max(x.cd_instrucao),'00') 
	from	titulo_rec_cobr_instrucao x 
	where	x.nr_seq_tit_rec_cobr = c.nr_sequencia 
	having	max(x.cd_instrucao) <> min(x.cd_instrucao)) ds_instrucao2, 
	' '	cd_instrucao 
