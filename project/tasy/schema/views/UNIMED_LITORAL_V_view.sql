...
CREATE OR REPLACE VIEW unimed_litoral_v (tp_reg, nr_linha, tp_registro, nr_seq_registro, cd_cgc_cpf_prestador, nr_relacao, nr_interno_conta, nr_seq_protocolo, nm_usuario_carteirinha, tp_documento, nr_documento, nr_seq_documento, dt_emissao, cd_usuario_convenio, cd_cgc_cpf_medico_solic, ie_origem_doc, ie_versao_cid, cd_cid, dt_internacao, dt_alta, ie_carater_intern, cd_motivo_alta, ie_clinica, cd_procedimento, dt_entrada, dt_saida, qt_procedimento, vl_procedimento, cd_cgc_cpf_med_exec, cd_cgc_cpf_resp_cred, dt_procedimento, cd_atividade_prestador, ie_porte_anestesico, hr_atendimento, ie_cirurgia_multipla, tx_procedimento, ie_pagar_honorario, ie_procedimento_uti, nr_total_linha, vl_total, qt_documento, cd_item, ie_tp_nasc, ie_tipo_plano, ie_natureza_juridica, cd_campo_quebra, ds_zeros, ds_espaco) AS SELECT 0 TP_REG,
	0 nr_linha, 
  '00' tp_registro, 
  0 nr_seq_registro, 
...
and	Verifica_Regra_Guia_Proc(a.nr_interno_conta, Campo_numerico(a.cd_item_convenio), a.nr_doc_convenio) = 'S' 
...
union all
 ...
 SELECT 13 TP_REG, 
  ...
  CASE WHEN a.ie_tipo_item=1 THEN  		CASE WHEN a.cd_tipo_item_interf=2 THEN coalesce(x.dt_alta,x.dt_periodo_final)  ELSE a.dt_item END  		  ELSE coalesce(x.dt_alta,x.dt_periodo_final) END  dt_procedimento, 
  a.cd_funcao_executor cd_atividade_prestador, 
  a.nr_porte_anestesico ie_porte_anestesico, 
  CASE WHEN a.ie_tipo_item=1 THEN  		CASE WHEN a.cd_tipo_item_interf=2 THEN coalesce(x.dt_alta,x.dt_periodo_final)  ELSE a.dt_item END  		  ELSE coalesce(x.dt_alta,x.dt_periodo_final) END  hr_atendimento, 
  ' ' ie_cirurgia_multipla, 
...
group by Campo_numerico(a.cd_item_convenio), 
	...
	coalesce(a.cd_cgc_cpf_resp_cred, a.cd_cgc_hospital), 
  	CASE WHEN a.ie_tipo_item=1 THEN  		CASE WHEN a.cd_tipo_item_interf=2 THEN coalesce(x.dt_alta,x.dt_periodo_final)  ELSE a.dt_item END  		  ELSE coalesce(x.dt_alta,x.dt_periodo_final) END , 
 ...
 SELECT 19 TP_REG, 
  ...
  0, '', '', 0, 
	sum(CASE WHEN b.cd_area_proc=1 THEN (b.vl_honorario + b.vl_custo_oper) WHEN b.cd_area_proc=4 THEN (b.vl_honorario + b.vl_custo_oper)  ELSE b.vl_total_item END  - 
		Obter_valor_Regra_Guia_Proc(b.nr_interno_conta,Campo_numerico(b.cd_item_convenio),b.nr_doc_convenio)) vl_total, 
 	0, 0, '00', 
  ' ' ie_tipo_plano, 
  ' ' ie_natureza_juridica, 
  10 cd_campo_quebra, 
