...
CREATE OR REPLACE VIEW san_hcf_util_hemocomponente_v (ie_situacao, nr_sangue, ds_derivado, ie_tipo_sangue, ie_fator_rh, ds_convenio, nr_guia, nr_atendimento, nm_paciente, ie_tratamento, nr_seq_reserva, nr_seq_derivado, nr_sequencia, nr_seq_emp_ent, nr_seq_emp_saida, nr_seq_inutil, nr_seq_transfusao, dt_movimento, dt_transfusao, dt_saida_estoque, dt_emprestimo) AS select	distinct
...
union
	...
	select 'Estoque', 
		...
		a.nr_seq_inutil, 
		a.nr_seq_transfusao, 
--		a.dt_producao dt_movimento,  OS62941. Arlete pediu para ser sempre a data de emprestimo. 25/09/2007 
		b.dt_emprestimo dt_movimento, 
		null dt_transfusao, 
		trunc(san_obter_data_saida_estoque(a.nr_sequencia, a.nr_seq_transfusao, null, null)) dt_saida_estoque, 
		null dt_emprestimo 
...
--	and	a.nr_seq_transfusao is null 
	and	a.nr_seq_reserva is null 
	and	a.nr_seq_inutil is null 
	and	not exists (	select	1 
				from	san_reserva_prod x 
				where	x.nr_seq_producao 	= a.nr_sequencia 
				and	((x.ie_status 		<> 'N') or (x.ie_status 		<> 'R' and coalesce(san_obter_status_reserva(x.nr_seq_reserva), 'C') <> 'C')) 
			  )) a;
