...
WHERE b.nr_sequencia        = 
    (select coalesce(min(x.nr_sequencia),b.nr_sequencia) 
        from w_interf_conta_autor x 
        where a.nr_interno_conta = x.nr_interno_conta) and a.nr_interno_conta    = c.nr_interno_conta and c.ie_tipo_item		   = 1 --and    c.cd_area_proc        in(1,2,3,4,9,11,12,13,14) 
 group by 
    a.nr_seq_protocolo, 
    a.nr_interno_conta, 
    substr(coalesce(c.nr_doc_convenio, 
...
FROM w_interf_conta_item c, w_interf_conta_cab a
LEFT OUTER JOIN w_interf_conta_autor b ON (a.nr_interno_conta = b.nr_interno_conta)
WHERE a.nr_interno_conta    = 6940240 and b.nr_sequencia        = 
    (select coalesce(min(x.nr_sequencia),b.nr_sequencia) 
        from w_interf_conta_autor x 
        where a.nr_interno_conta = x.nr_interno_conta) and a.nr_interno_conta    = c.nr_interno_conta and c.ie_tipo_item		   = 1 and c.cd_medico_executor	 is not null --and    c.cd_area_proc        in(1,2,4) 
