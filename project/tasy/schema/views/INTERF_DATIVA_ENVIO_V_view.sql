...
CREATE OR REPLACE VIEW interf_dativa_envio_v (ds_registro, tp_registro, ie_servico, cd_registro, cd_origem, nr_lote, nr_interno_conta, nr_parcela, cd_convenio, nm_convenio, tp_prestador, cd_prestador, nm_prestador, dt_geracao, nr_remessa, cd_plano, cd_paciente, cd_autorizacao, cd_acomodacao, dt_entrada, dt_saida, dt_inicio_cobranca, dt_fim_cobranca, ie_tipo_atendimento, cd_cid, cd_proc_principal, cd_medico_convenio, nr_crm, uf_crm, nr_crm_req, uf_crm_req, nr_seq_item, cd_item, nm_item, ie_emergencia, ie_extra, dt_item, ie_funcao_medico, qt_item, vl_item, ie_cirurgia_multipla, vl_total_item, qt_item_conta, nm_paciente, nr_fatura, qt_conta, vl_total_lote, nr_seq_protocolo, ds_espaco, nr_registro, cd_cgc_prestador, cd_motivo_alta, ie_laudo_especial, ie_laudo_prorrogacao, dt_autorizacao, dt_validade_carteirinha, nr_prontuario, nm_medico_resp, ie_tipo_doc_paciente, nr_doc_paciente, ie_matmed, nr_crm_resp, uf_crm_resp, cd_senha, cd_especialidade, vl_proc_principal, ie_tipo_cirurgia, cd_unidade_atend, qt_diaria_enfermaria, qt_diaria_uti, cd_leitora, ie_fim_registro, ie_tipo_atend_tasy, nr_doc_convenio) AS SELECT		'HEADER        ' 	DS_REGISTRO,
		...
		0				NR_INTERNO_CONTA, 
		0				NR_PARCELA, 
		somente_numero(coalesce(substr(A.CD_REGIONAL,1,5),0)) CD_CONVENIO, 
...
WHERE c.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA and w.nr_seq_protocolo = x.nr_seq_protocolo  and c.ie_tipo_item	= 1 and coalesce(Z.ie_entra_conta,'S') = 'S' AND c.cd_item in (select x.cd_procedimento 
		...
		where x.cd_area_procedimento in (1,4,11,13)) GROUP BY	somente_numero(coalesce(substr(x.cd_interno,1,5),0)), 
		c.NR_SEQ_PROTOCOLO, 
      		c.NR_INTERNO_CONTA, 
      		c.CD_ITEM_CONVENIO, 
      		substr(ELIMINA_ACENTUACAO(upper(c.DS_ITEM)),1,50), 
...
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA and B.nr_seq_protocolo	= y.nr_seq_protocolo    and coalesce(Z.ie_entra_conta,'S') = 'S' GROUP BY	somente_numero(coalesce(substr(y.cd_interno,1,5),0)), 
		B.NR_SEQ_PROTOCOLO, 
      	B.NR_INTERNO_CONTA, 
		substr(ELIMINA_ACENTUACAO(UPPER(B.NM_PACIENTE)),1,40), 
