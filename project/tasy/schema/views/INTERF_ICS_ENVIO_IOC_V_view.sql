...
select
        ...
        0                                        nr_seq_envio,
        0                                        nr_seq_reg,
	coalesce(somente_numero(a.cd_senha_guia),0)     		nr_senha,
...
select
	...
	coalesce(somente_numero(a.cd_senha_guia),0)     		nr_senha,
        ...
        somente_numero(substr(coalesce(c.cd_item_convenio,coalesce(c.cd_item,'0')),1,8))
                                                cd_servico,
        sum(CASE WHEN c.nr_seq_proc_pacote=c.nr_seq_item THEN  c.vl_total_item  ELSE CASE WHEN C.CD_AREA_PROC=4 THEN C.VL_HONORARIO  ELSE c.vl_total_item END  END ) vl_servico,
        c.dt_item                                dt_servico,
        ' '                                        cd_dente,
        ' '                                        ie_faces_dente,
        sum(CASE WHEN c.cd_area_proc=4 THEN 1  ELSE c.qt_item END )      qt_servico,
        c.dt_item                                hr_atendimento,
        047 cd_especialidade,
        2                                        cd_local_atend,
        ' '                                        cd_cid,
        ' '                                        ie_conselho,
        0                                        nr_conselho,
...
WHERE b.nr_sequencia                =
        (select coalesce(min(x.nr_sequencia),b.nr_sequencia)
                from w_interf_conta_autor x
                where a.nr_interno_conta = x.nr_interno_conta) and a.nr_interno_conta        = c.nr_interno_conta and c.cd_area_proc                in (1,2,3,4,7,9) group by
        a.nr_seq_protocolo,
        a.nr_interno_conta,
	coalesce(somente_numero(a.cd_senha_guia),0),
        somente_numero(substr(coalesce(c.cd_item_convenio,coalesce(c.cd_item,'0')),1,8)),
        c.dt_item
