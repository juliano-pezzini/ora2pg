...
CREATE OR REPLACE VIEW eis_ocupacao_tipo_convenio_v (dt_referencia, cd_setor_atendimento, cd_convenio, cd_tipo_acomodacao, ie_clinica, cd_estabelecimento, qt_leito_livre, nr_pac_partic, nr_pac_conv, nr_pac_sus, nr_pac_outros, nr_obito_partic, nr_obito_conv, nr_obito_sus, nr_obito_outros, nr_alta_partic, nr_alta_conv, nr_alta_sus, nr_alta_outros, nr_admissao_partic, nr_admissao_conv, nr_admissao_sus, nr_admissao_outros, nr_pac_total, nr_obito_total, nr_alta_total, nr_admissao_total, qt_leito_total) AS select 	a.dt_referencia,
	...
	CASE WHEN v.ie_tipo_convenio=2 THEN  nr_altas WHEN v.ie_tipo_convenio=5 THEN nr_altas WHEN v.ie_tipo_convenio=6 THEN nr_altas WHEN v.ie_tipo_convenio=7 THEN nr_altas WHEN v.ie_tipo_convenio=8 THEN nr_altas WHEN v.ie_tipo_convenio=9 THEN nr_altas  ELSE 0 END  nr_alta_conv,
	...
	CASE WHEN v.ie_tipo_convenio=4 THEN 0  ELSE nr_pacientes END  nr_pac_total,
	CASE WHEN v.ie_tipo_convenio=4 THEN 0  ELSE nr_obitos END  nr_obito_total,
	CASE WHEN v.ie_tipo_convenio=4 THEN 0  ELSE nr_altas END  nr_alta_total,
	CASE WHEN v.ie_tipo_convenio=4 THEN 0  ELSE nr_admissoes END  nr_admissao_total,
	(coalesce(qt_leito_livre,0) + CASE WHEN v.ie_tipo_convenio=4 THEN 0  ELSE nr_pacientes END ) qt_leito_total
FROM 	convenio v,
