...
CREATE OR REPLACE VIEW interf_sulamerica_envio_v (ds_registro, tp_registro, ie_servico, cd_registro, cd_origem, nr_lote, nr_interno_conta, nr_parcela, cd_convenio, nm_convenio, tp_prestador, cd_prestador, nm_prestador, dt_geracao, nr_remessa, cd_plano, cd_paciente, cd_autorizacao, cd_acomodacao, dt_entrada, dt_saida, dt_inicio_cobranca, dt_fim_cobranca, ie_tipo_atendimento, cd_cid, cd_proc_principal, cd_medico_convenio, nr_crm, uf_crm, nr_crm_req, uf_crm_req, ie_tipo_nascimento, cd_prestador_solic, nr_crm_solicitante, uf_crm_solicitante, cd_usuario_convenio, nr_seq_item, cd_item, nm_item, ie_emergencia, ie_extra, dt_item, ie_funcao_medico, qt_item, vl_item, ie_cirurgia_multipla, cd_unidade_medida, qt_embalagem, cd_prestador_princ, vl_total_item, qt_item_conta, nm_paciente, nr_fatura, qt_conta, vl_total_lote, nr_seq_protocolo, ds_espaco, nr_registro, cd_cgc_prestador, cd_motivo_alta, ie_laudo_especial, ie_laudo_prorrogacao, ie_matmed, nr_crm_resp, uf_crm_resp, cd_senha, cd_especialidade, vl_proc_principal, ie_tipo_cirurgia, cd_unidade_atend, qt_diaria_enfermaria, qt_diaria_uti, cd_leitora, ie_fim_registro, ie_tipo_atend_tasy, nr_doc_convenio) AS SELECT		'HEADER               ' 	DS_REGISTRO,
		1				TP_REGISTRO,
		'CSMH'				IE_SERVICO,
		0				CD_REGISTRO,
...
SELECT		'CABECALHO CONTA      '	DS_REGISTRO,
		...
		c.cd_medico_solicitante	CD_PRESTADOR_SOLIC,
		c.NR_CRM_SOLICITANTE		NR_CRM_SOLICITANTE,
		c.UF_CRM_SOLICITANTE		UF_CRM_SOLICITANTE,
...
SELECT     	'ITEM DA CONTA         ' 	DS_REGISTRO,
		...
		0				QT_EMBALAGEM,
		c.cd_cgc_prestador		CD_PRESTADOR_PRINC,
           	0		      		VL_TOTAL_ITEM,
           	0	      			QT_ITEM_CONTA,
	     	' '				NM_PACIENTE,
           	' '	      			NR_FATURA,
...
WHERE c.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA and w.nr_seq_protocolo = x.nr_seq_protocolo  and c.ie_tipo_item	= 1 and coalesce(Z.ie_entra_conta,'S') = 'S' AND c.cd_item in (select x.cd_procedimento
