...
CREATE OR REPLACE VIEW hm_abramge_ii_v (ds_registro, tp_registro, ie_servico, cd_registro, cd_origem, nr_lote, nr_interno_conta, nr_parcela, cd_convenio, nm_convenio, tp_prestador, cd_prestador, nm_prestador, dt_geracao, nr_remessa, cd_plano, cd_paciente, cd_autorizacao, cd_acomodacao, dt_entrada, dt_saida, dt_inicio_cobranca, dt_fim_cobranca, ie_tipo_atendimento, cd_cid, cd_proc_principal, cd_medico_convenio, nr_crm, uf_crm, nr_crm_req, uf_crm_req, nr_seq_item, cd_item, nm_item, ie_emergencia, ie_extra, dt_item, ie_funcao_medico, qt_item, vl_item, ie_cirurgia_multipla, vl_total_item, qt_item_conta, nm_paciente, nr_fatura, qt_conta, vl_total_lote, nr_seq_protocolo, ds_espaco, nr_registro, cd_cgc_prestador, cd_motivo_alta, ie_laudo_especial, ie_laudo_prorrogacao, dt_autorizacao, dt_validade_carteirinha, nr_prontuario, nm_medico_resp, ie_tipo_doc_paciente, nr_doc_paciente, ie_matmed, ds_ma_me_tx, nr_crm_resp, uf_crm_resp, nr_cpf_resp, cd_senha, cd_especialidade, vl_proc_principal, ie_tipo_cirurgia, cd_unidade_atend, qt_diaria_enfermaria, qt_diaria_uti, cd_leitora, ie_fim_registro, ie_tipo_atend_tasy, cd_interno_protocolo, nr_guia, cd_senha_guia, tp_internacao, cd_unidade_medida, ie_tipo_faturamento, ie_folha_sala) AS SELECT		'HEADER               ' 	DS_REGISTRO,
		1				TP_REGISTRO,
		'CSMH'				IE_SERVICO,
		0				CD_REGISTRO,
...
SELECT		'CABECALHO CONTA      '	DS_REGISTRO,
		...
		coalesce(B.DT_ALTA,B.DT_PERIODO_FINAL)
						DT_FIM_COBRANCA,
		CASE WHEN B.IE_CLINICA=2 THEN 7 WHEN B.IE_CLINICA=1 THEN 6 WHEN B.IE_CLINICA=3 THEN 8 WHEN B.IE_CLINICA=4 THEN 8 WHEN B.IE_CLINICA=6 THEN 8 WHEN B.IE_CLINICA=5 THEN 8  ELSE 8 END 	IE_TIPO_ATENDIMENTO,
...
SELECT     	'ITEM DA CONTA         ' 	DS_REGISTRO,
		...
		' '				nr_doc_paciente,
		' '				ie_matmed,
		CASE WHEN c.ie_total_interf=2 THEN 'MA' WHEN c.ie_total_interf=9 THEN 'MA' WHEN c.ie_total_interf=10 THEN 'MA' WHEN c.ie_total_interf=11 THEN 'MA' WHEN c.ie_total_interf=3 THEN 'ME'  ELSE ' ' END 
							DS_MA_ME_TX,
...
WHERE c.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA and w.nr_seq_protocolo = x.nr_seq_protocolo  and c.ie_tipo_item	= 1 and c.qt_item		<> 0 and coalesce(Z.ie_entra_conta,'S') = 'S' and c.cd_medico_executor is not null GROUP BY	somente_numero(coalesce(x.cd_interno,0)),
		c.NR_SEQ_PROTOCOLO,
           		c.NR_INTERNO_CONTA,
           		c.CD_ITEM_CONVENIO,
           		substr(ELIMINA_ACENTUACAO(upper(c.DS_ITEM)),1,50),
