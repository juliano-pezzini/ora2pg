...
CREATE OR REPLACE VIEW eis_escala_asg_ppp_v (cd_setor_atendimento, ds_estabelecimento, cd_convenio, ds_convenio, ds_setor_atendimento, ie_sexo, ds_sexo, nm_medico, cd_profissional, ie_faixa_etaria, ds_unidade, dt_avaliacao, cd_empresa, cd_estabelecimento, nr_atendimento, qt_ponto_total, ds_pontuacao, ie_avaliacao_global) AS select	coalesce(a.cd_setor_atendimento,obter_setor_atend_data(a.nr_atendimento, e.dt_avaliacao)) cd_setor_atendimento,
		...
		a.nr_atendimento, 
		e.qt_ponto_total, 
		substr(obter_desc_pontuacao_ASG_PPP(e.qt_ponto_total),1,255) ds_pontuacao, 
		e.ie_avaliacao_global 
FROM 	escala_asg_ppp e, 
		estabelecimento f, 
