...
SELECT		'CABECALHO CONTA      '		DS_REGISTRO,
		...
		coalesce(somente_numero(B.nr_doc_convenio),0)
						nr_guia,
		CASE WHEN B.ie_tipo_atendimento=1 THEN B.cd_senha_guia  ELSE ' ' END  cd_senha_guia,
...
SELECT     	'ITEM DA CONTA         ' 	DS_REGISTRO,
		...
		CASE WHEN c.cd_area_proc=1 THEN c.cd_especialidade WHEN c.cd_area_proc=11 THEN c.cd_especialidade  ELSE 0 END  cd_especialidade,
		0				vl_proc_principal,
...
WHERE c.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA and w.nr_seq_protocolo = x.nr_seq_protocolo  and c.ie_tipo_item	= 1 and c.qt_item		<> 0 AND c.cd_item in (select x.cd_procedimento
		...
		CASE WHEN c.cd_area_proc=1 THEN c.cd_especialidade WHEN c.cd_area_proc=11 THEN c.cd_especialidade  ELSE 0 END ,
		CASE WHEN c.pr_via_acesso=100 THEN 1 WHEN c.pr_via_acesso=70 THEN 2  ELSE 3 END ,
