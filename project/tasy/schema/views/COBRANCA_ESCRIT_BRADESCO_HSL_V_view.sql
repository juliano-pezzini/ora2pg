...
select	1				tp_registro, 
	...
	lpad(e.cd_carteira,4,'0') || 
		(select lpad(substr(somente_numero(x.cd_agencia_bancaria),1,4),5,'0') || lpad(x.cd_conta,7,'0') || coalesce(x.ie_digito_conta,'0') 
	...
	where	x.nr_seq_tit_rec_cobr = c.nr_sequencia)	ie_instrucao1, 
	(select coalesce(max(x.cd_instrucao),'00') 
	from	titulo_rec_cobr_instrucao x 
	where	x.nr_seq_tit_rec_cobr = c.nr_sequencia 
	having	max(x.cd_instrucao) <> min(x.cd_instrucao)) ie_instrucao2, 
	LOCALTIMESTAMP				dt_desconto, 
