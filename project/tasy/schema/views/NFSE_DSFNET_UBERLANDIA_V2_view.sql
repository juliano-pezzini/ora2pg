...
CREATE OR REPLACE VIEW nfse_dsfnet_uberlandia_v2 (nr_seq_nota, assinatura, inscricaomunicipalprestador, razaosocialprestador, tiporps, serierps, numerorps, dataemissaorps, situacaorps, serierpssubstituido, numerorpssubstituido, numeronfsesubstituida, dataemissaonfsesubstituida, serieprestacao, inscricaomunicipaltomador, cpfcnpjtomador, razaosocialtomador, tipologradourotomador, logradourotomador, numeroenderecotomador, complementoenderecotomador, tipobairrotomador, bairrotomador, cidadetomador, cidadetomadordescricao, ceptomador, emailtomador, codigoatividade, aliquotaatividade, tiporecolhimento, municipioprestacao, municipioprestacaodescricao, operacao, tributacao, valorpis, valorcofins, valorinss, valorir, valorcsll, aliquotapis, aliquotacofins, aliquotainss, aliquotair, aliquotacsll, descricaorps, dddprestador, telefoneprestador, dddtomador, telefonetomador, motcancelamento, cpfcnpjintermediario, deducaopor, tipodeducao, cpfcnpjreferencia, numeronfreferencia, valortotalreferencia, percentualdeduzir, valordeduzir) AS select	a.nr_sequencia nr_seq_nota,
	...
	99 SeriePrestacao,
	CASE WHEN 		obter_desc_municipio_ibge(			CASE WHEN 	obter_municipio_ibge(obter_dados_pf_pj(cd_pessoa_fisica, cd_cgc, 'CEP'))=0 THEN 				obter_dados_pf_pj(cd_pessoa_fisica, cd_cgc, 'CDM')  ELSE obter_municipio_ibge(obter_dados_pf_pj(cd_pessoa_fisica, cd_cgc, 'CEP')) END )=obter_desc_municipio_ibge(			CASE WHEN 	obter_municipio_ibge(obter_dados_pf_pj('', cd_cgc_emitente, 'CEP'))=0 THEN 				obter_dados_pf_pj('', cd_cgc_emitente, 'CDM')  ELSE obter_municipio_ibge(obter_dados_pf_pj('', cd_cgc_emitente, 'CEP')) END ) THEN 		CASE WHEN cd_cgc='' THEN  coalesce(lpad(obter_dados_pf(cd_pessoa_fisica, 'ISS'), 11, '0'), '00000000000')  ELSE coalesce(lpad(substr(obter_dados_pf_pj(cd_pessoa_fisica, cd_cgc, 'IM'), 1, 30), 11, '0'), '00000000000') END   ELSE '00000000000' END  InscricaoMunicipalTomador,
	...
	coalesce(substr(nfe_elimina_caractere_especial(obter_email_pessoa(obter_estabelecimento_ativo,cd_pessoa_fisica, cd_cgc,obter_usuario_ativo)), 1, 80), '-') EmailTomador,
	coalesce(coalesce(obter_dados_grupo_servico_item(obter_item_servico_proced(obter_procedimento_nfse(a.nr_sequencia,'P'), obter_procedimento_nfse(a.nr_sequencia,'O')), 'P'),
