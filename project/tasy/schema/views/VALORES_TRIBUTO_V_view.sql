...
CREATE OR REPLACE VIEW valores_tributo_v (vl_soma_trib_nao_retido, vl_soma_base_nao_retido, vl_soma_trib_adic, vl_soma_base_adic, cd_cgc, cd_pessoa_fisica, dt_tributo, dt_tributo_mes, cd_tributo, ie_origem_valores, vl_tributo, cd_cgc_emitente, vl_total_base, cd_estabelecimento, ie_base_calculo, vl_reducao, dt_criacao, nr_titulo, nr_repasse_terceiro, nr_seq_nota_fiscal, ie_baixa_titulo, cd_cnpj_raiz, cd_cnpj_emitente_raiz, cd_estab_financeiro, cd_darf, nr_seq_distribuicao, nr_seq_lote_protocolo, cd_empresa, nr_seq_pag_prest, nr_seq_trib_valor, ie_tipo_tributo) AS select	coalesce(sum(a.VL_NAO_RETIDO),0) vl_soma_trib_nao_retido,
	...
	trunc(CASE WHEN c.ie_vencimento='V' THEN  b.dt_vencimento_atual WHEN c.ie_vencimento='C' THEN  coalesce(b.dt_contabil, b.dt_emissao)  ELSE b.dt_emissao END ,'month') dt_tributo_mes,
	a.cd_tributo,
...
select	coalesce(sum(a.VL_NAO_RETIDO),0) vl_soma_trib_nao_retido,
	...
	trunc(CASE WHEN e.ie_vencimento='V' THEN  b.DT_VENCIMENTO WHEN e.ie_vencimento='C' THEN  coalesce(b.DT_VENCIMENTO, c.dt_mesano_referencia)  ELSE c.dt_mesano_referencia END ,'month') dt_tributo_mes,
	a.cd_tributo,
...
select	coalesce(sum(a.VL_TRIB_NAO_RETIDO), 0) vl_soma_trib_nao_retido,
	...
	trunc(b.dt_emissao,'month') dt_tributo_mes,
	a.cd_tributo,
...
select	coalesce(sum(a.VL_TRIB_NAO_RETIDO), 0) vl_soma_trib_nao_retido, -- Edgar OS 50572 tributos que tem a base de calculo conforme vencimentos da NF
	...
	trunc(a.dt_vencimento,'month') dt_tributo_mes,
	a.cd_tributo,
	'NFE' ie_origem_valores,
	coalesce(sum(a.vl_tributo),0) vl_tributo,
	b.cd_cgc_emitente,
...
select	coalesce(sum(a.VL_NAO_RETIDO),0) vl_soma_trib_nao_retido,
	...
	trunc(d.dt_baixa,'month') dt_tributo_mes,
	a.cd_tributo,
	'TP' ie_origem_valores,
	coalesce(sum(a.vl_imposto),0) vl_tributo,
	'' cd_cgc_emitente,
...
select	coalesce(sum(a.VL_NAO_RETIDO),0) vl_soma_trib_nao_retido,
	...
	trunc(CASE WHEN e.ie_vencimento='V' THEN  c.DT_VENCIMENTO WHEN e.ie_vencimento='C' THEN  coalesce(c.DT_VENCIMENTO, f.dt_mesano_ref)  ELSE f.dt_mesano_ref END ,'month') dt_tributo_mes,
	a.cd_tributo,
...
select	coalesce(sum(a.VL_NAO_RETIDO),0) vl_soma_trib_nao_retido,
	...
	trunc(CASE WHEN e.ie_vencimento='V' THEN  b.DT_VENCIMENTO WHEN e.ie_vencimento='C' THEN  coalesce(b.DT_VENCIMENTO, c.dt_mes_competencia)  ELSE c.dt_mes_competencia END ,'month') dt_tributo_mes,
	a.cd_tributo,
