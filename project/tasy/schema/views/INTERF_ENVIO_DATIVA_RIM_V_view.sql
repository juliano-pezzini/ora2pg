...
select     	'ITEM DA CONTA         ' 	ds_registro,
		...
		' '				uf_crm_req,
           	...
           	coalesce(c.cd_item_convenio, c.cd_item) cd_item,
...
WHERE c.nr_interno_conta = w.nr_interno_conta and w.nr_seq_protocolo = x.nr_seq_protocolo  and c.ie_tipo_item	= 1 and c.qt_item		<> 0 and coalesce(z.ie_entra_conta,'S') = 'S' and c.cd_item in (select x.cd_procedimento
		...
		where x.cd_area_procedimento in (1,4)) group by	somente_numero(coalesce(x.cd_interno,0)),
		c.nr_seq_protocolo,
         		c.nr_interno_conta,
           		coalesce(c.cd_item_convenio, c.cd_item) ,
           		substr(elimina_acentuacao(upper(c.ds_item)),1,50),
           		CASE WHEN w.ie_carater_inter='U' THEN 'S'  ELSE 'N' END ,
           		CASE WHEN c.pr_hora_extra=0 THEN 'N' WHEN c.pr_hora_extra=1 THEN 'N' WHEN c.pr_hora_extra IS NULL THEN 'N'  ELSE 'S' END ,
...
select     	'TOTAL DA CONTA        ' 	ds_registro,
		...
		b.uf_crm_medico_resp		uf_crm_req,
           	...
           	0				vl_item,
           	0				ie_cirurgia_multipla,
           	sum(CASE WHEN x.cd_area_procedimento=1 THEN (coalesce(e.vl_honorario,0) + coalesce(e.vl_custo_oper,0)) WHEN x.cd_area_procedimento=4 THEN (coalesce(e.vl_honorario,0) + coalesce(e.vl_custo_oper,0))  ELSE e.vl_total_item END ) vl_total_item,
           	0			qt_item_conta,
