...
union all
 ...
 SELECT 10 TP_REG, 
  ...
  a.nr_interno_conta, 
  a.nr_seq_protocolo, 
  coalesce(lpad(Obter_tipo_atend_tiss(c.nr_atendimento, a.nr_interno_conta),2,0),b.ie_tipo_guia) tp_documento, 
...
WHERE a.nr_atendimento = c.nr_atendimento  and coalesce(b.nr_sequencia,0)	= 
	  ...
	  a.nr_seq_protocolo, 
 	  coalesce(lpad(Obter_tipo_atend_tiss(c.nr_atendimento, a.nr_interno_conta),2,0),b.ie_tipo_guia), 
...
union all
 ...
 SELECT 11 TP_REG, 
  ...
  '', 0, '', '', 0, 0, 0, 0, 
  CASE WHEN a.ie_tipo_nascimento IS NULL THEN '00'  ELSE '0' || ie_tipo_nascimento END , 
  '00000000000000000000000000000000000000000000' ds_zeros, 
  '                      ' ds_espaco, 
  ' ' nm_usuario_carteirinha, 
  ' ' ie_natureza_juridica, 
...
union all
 ...
 SELECT 12 TP_REG, 
  ...
  a.nr_seq_protocolo, 
  '', 
  to_char(somente_numero(substr(x.nr_doc_convenio,1,8))) nr_documento, 
...
group by Campo_numerico(a.cd_item_convenio), 
	...
	a.cd_item, 
	  ...
	  to_char(somente_numero(substr(x.nr_doc_convenio,1,8))) 
...
0, LOCALTIMESTAMP, '', '', '', '', '', LOCALTIMESTAMP, LOCALTIMESTAMP, '', 0, 0, 
  ...
  CASE WHEN a.ie_responsavel_credito='RU' THEN a.cd_cgc_hospital  ELSE coalesce(a.nr_cpf_executor,a.cd_cgc_hospital) END  cd_cgc_cpf_exec, 
...
group by Campo_numerico(a.cd_item_convenio), 
	...
	a.nr_seq_protocolo, 
	obter_cpf_pessoa_fisica(a.CD_MEDICO_REQ) , 
	CASE WHEN a.ie_responsavel_credito='RU' THEN a.cd_cgc_hospital  ELSE coalesce(a.nr_cpf_executor,a.cd_cgc_hospital) END , 
