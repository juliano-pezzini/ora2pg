...
CREATE OR REPLACE VIEW ccb_especialista_v (nr_seq_grupo, nm_usuario, nm_pessoa_fisica, ds_email, cd_pessoa_fisica, ds_tipo_usuario) AS select 	a.nr_sequencia nr_seq_grupo,
...
select 	a.nr_sequencia nr_seq_grupo,
			substr(obter_usuario_pessoa(b.cd_responsavel),1,60) nm_usuario,
			OBTER_PF_USUARIO(substr(obter_usuario_pessoa(b.cd_responsavel),1,60),'') nm_pessoa_fisica,
			OBTER_EMAIL_PF(OBTER_PF_USUARIO(substr(obter_usuario_pessoa(b.cd_responsavel),1,60),'C')) ds_email,
			OBTER_PF_USUARIO(substr(obter_usuario_pessoa(b.cd_responsavel),1,60),'C') cd_pessoa_fisica,
			'Gerente' ds_tipo_usuario
from		grupo_desenvolvimento	a,
			gerencia_wheb				b
where	a.nr_seq_gerencia			= b.nr_sequencia
