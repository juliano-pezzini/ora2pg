...
LEFT OUTER JOIN estrutura_procedimento_v x ON (E.CD_ITEM = X.CD_PROCEDIMENTO AND E.IE_ORIGEM_PROCED = X.IE_ORIGEM_PROCED)
LEFT OUTER JOIN regra_honorario z ON (E.IE_RESPONSAVEL_CREDITO = Z.cd_regra)
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA and B.nr_seq_protocolo	= f.nr_seq_protocolo    and ((coalesce(Z.ie_entra_conta,'S') = 'S') or (e.nr_seq_proc_pacote = e.nr_seq_item)) GROUP BY	(coalesce(substr(f.CD_INTERNO,1,5),0))::numeric , 
		B.NR_SEQ_PROTOCOLO;
