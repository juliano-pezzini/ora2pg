...
select	2					tp_registro, 
	...
	(CASE WHEN n.ie_situacao=1 THEN  n.vl_mercadoria WHEN n.ie_situacao=3 THEN  CASE WHEN n.ie_status_envio IS NULL THEN  n.vl_mercadoria  ELSE 0 END   ELSE 0 END  - n.vl_descontos) vl_servico, 
	n.vl_descontos				vl_descontos,	 
	/*lpad(nvl(substr(elimina_caractere_especial(obter_dados_grupo_servico_item(obter_item_servico_proced(obter_procedimento_nfse(n.nr_sequencia,'P'), obter_procedimento_nfse(n.nr_sequencia,'O')),'CD')),1,5) 
	...
	replace(CASE WHEN nr_seq_protocolo IS NULL THEN  	 substr((select	substr(obter_nome_pf(a.cd_pessoa_fisica),1,150) 		from	atendimento_paciente a, 			conta_paciente c 		where	a.nr_atendimento = c.nr_atendimento 		and	c.nr_interno_conta = n.nr_interno_conta) || '  ' || 		(select	substr(obter_dados_pf(a.cd_pessoa_fisica,'CPF'),1,11) 		from	atendimento_paciente a, 			conta_paciente c 		where	a.nr_atendimento = c.nr_atendimento 		and	c.nr_interno_conta = n.nr_interno_conta) || ' |' || 		substr(Obter_select_concatenado( 			'select decode(a.cd_material, null, 
			...
			substr(obter_descricao_procedimento(a.cd_procedimento, ie_origem_proced),1,240), 
			substr(obter_desc_material(a.cd_material),1,100)) || chr(32) || chr(47) || chr(32) || b.ds_observacao 
		from nota_fiscal_item a, nota_fiscal b where a.nr_sequencia = b.nr_sequencia and a.nr_sequencia = ' || n.nr_sequencia,null,'|'),1,1000) END ,chr(10),'|') ds_servicos, 
	replace(n.ds_observacao,chr(10),'|')				ds_observacao, 
	0					vl_total_servicos, 
	0					vl_total_descontos, 
	0					qt_linhas, 
