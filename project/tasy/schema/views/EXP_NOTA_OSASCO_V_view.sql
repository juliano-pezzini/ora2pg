...
CREATE OR REPLACE VIEW exp_nota_osasco_v (tp_registro, ds_registro, nr_seq_linha, cd_autent, nr_inscricao_municipal, ds_tipo_rps, cd_serie_nf, nr_nota_fiscal, dt_emissao, ie_situacao, ie_situacao_nf, vl_servico, vl_descontos, cd_servico, vl_aliquota, ie_iss_retido, ie_cpf_cnpj, cd_cpf_cnpj, cd_cpf_tomador, cd_cnpj_tomador, nr_inscricao_munic_tomador, nr_inscricao_estad_tomador, nm_tomador, ds_tipo_logradouro, ds_endereco, nr_endereco, ds_complemento, ds_bairro, ds_municipio, sg_estado, cd_cep, ds_email, ds_servicos, ds_observacao, vl_total_servicos, vl_total_descontos, qt_linhas, nr_sequencia, cd_operacao_nf, cd_estabelecimento, ds_natureza_operacao, vl_deducoes, vl_total_deducoes, vl_nota_fiscal, vl_total_nota_fiscal, outros_impostos, imposto_ir, imposto_inss, imposto_cofins, imposto_pis, imposto_csll, cd_pessoa_fisica) AS select  1          tp_registro,
  'H'             ds_registro,
  null               nr_seq_linha,
  null               cd_autent,
...
select  2          tp_registro,
  'D'             ds_registro,
  lpad(ROWNUM, 6, 0) nr_seq_linha,
  n.nr_nota_fiscal  cd_autent,
  null          nr_inscricao_municipal,
  'RPS'          ds_tipo_rps,
  n.cd_serie_nf        cd_serie_nf,
  n.nr_nota_fiscal        nr_nota_fiscal,
  trunc(n.dt_emissao)        dt_emissao,
  CASE WHEN n.ie_situacao=1 THEN  'N'  ELSE 'C' END   ie_situacao,
  n.ie_situacao        ie_situacao_nf,
  replace(replace(to_char(n.vl_total_nota, '999999999D99'), ',', null), ' ', null) vl_servico,
  ...
  null          vl_aliquota,
  substr(obter_se_nf_retem_iss(n.nr_sequencia), 1, 1) ie_iss_retido,
  ...
  CASE WHEN substr(obter_cpf_pessoa_fisica(n.cd_pessoa_fisica), 1, 11) IS NULL THEN  null  ELSE substr(obter_cpf_pessoa_fisica(n.cd_pessoa_fisica), 1, 11) END           cd_cpf_tomador,
  CASE WHEN substr(n.cd_cgc, 1, 14) IS NULL THEN  null  ELSE substr(n.cd_cgc, 1, 14) END                     cd_cnpj_tomador,
  ...
  coalesce(CASE WHEN n.cd_pessoa_fisica IS NULL THEN CASE WHEN substr(obter_compl_pj(n.cd_cgc,1,'UF'),1,3)='N/D' THEN        substr(obter_dados_pf_pj(n.cd_pessoa_fisica, n.cd_cgc, 'UF'),1,2)  ELSE substr(obter_compl_pj(n.cd_cgc,1,'UF'),1,2) END   ELSE substr(obter_compl_pf(n.cd_pessoa_fisica, 1, 'UF'),1,2) END ,
    substr(obter_dados_pf_pj(n.cd_pessoa_fisica, n.cd_cgc, 'UF'),1,2)) sg_estado,
  lpad(coalesce(to_char(CASE WHEN n.cd_pessoa_fisica IS NULL THEN CASE WHEN substr(obter_compl_pj(n.cd_cgc,1,'CEP'),1,15)='N/D' THEN       to_char(somente_numero(substr(obter_dados_pf_pj(n.cd_pessoa_fisica, n.cd_cgc, 'CEP'),1,15)))  ELSE substr(obter_compl_pj(n.cd_cgc,1,'CEP'),1,15) END   ELSE substr(obter_compl_pf(n.cd_pessoa_fisica, 1, 'CEP'),1,15) END ),
    to_char(somente_numero(substr(obter_dados_pf_pj(n.cd_pessoa_fisica, n.cd_cgc, 'CEP'),1,15)))), 8, 0) cd_cep,
  coalesce(CASE WHEN n.cd_pessoa_fisica IS NULL THEN CASE WHEN substr(obter_compl_pj(n.cd_cgc,1,'M'),1,80)='N/D' THEN  null  ELSE substr(obter_compl_pj(n.cd_cgc,1,'M'),1,80) END   ELSE substr(obter_compl_pf(n.cd_pessoa_fisica, 1, 'M'),1,60) END ,
    substr(obter_dados_pf_pj(n.cd_pessoa_fisica, n.cd_cgc, 'M'),1,60)) ds_email,    
  CASE WHEN  n.ie_situacao=1 THEN   CASE WHEN     nr_seq_protocolo IS NULL THEN       substr(      (select  substr(obter_nome_pf(a.cd_pessoa_fisica),1,150)      from  atendimento_paciente a,         conta_paciente c       where  a.nr_atendimento = c.nr_atendimento      and  c.nr_interno_conta = n.nr_interno_conta)       ||  '   '  ||      (select  substr(obter_dados_pf(a.cd_pessoa_fisica,'CPF'),1,11)      from  atendimento_paciente a,         conta_paciente c       where  a.nr_atendimento = c.nr_atendimento      and  c.nr_interno_conta = n.nr_interno_conta)       || ' |'  ||      substr(obter_descricao_rps(n.cd_estabelecimento, n.nr_sequencia, 'DS_SERVICOS'), 1, 1000)            || '   ' || 'Periodo de fechamento ' ||       (select  c.dt_periodo_inicial      from  atendimento_paciente a,         conta_paciente c       where  a.nr_atendimento = c.nr_atendimento      and  c.nr_interno_conta = n.nr_interno_conta)       || ' a '  ||      (select  c.dt_periodo_final      from  atendimento_paciente a,         conta_paciente c       where  a.nr_atendimento = c.nr_atendimento      and  c.nr_interno_conta = n.nr_interno_conta),      1, 1000)  ELSE CASE WHEN  n.ie_situacao=1 THEN replace(replace(substr(obter_descricao_rps(n.cd_estabelecimento, n.nr_sequencia, 'DS_SERVICOS'), 1, 1000), chr(13), ' '), chr(10), ' ')   ELSE '' END  END   ELSE '' END  ds_servico,    
