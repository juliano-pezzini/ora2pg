...
SELECT		'CABECALHO CONTA   '		DS_REGISTRO, 
		...
		(obter_conversao_externa(B.cd_cgc_convenio, 'INTERF_DATIVA_ENVIO_CS_V', 'CD_MOTIVO_ALTA', coalesce(B.CD_MOTIVO_ALTA,0)))::numeric  CD_MOTIVO_ALTA, 
		'N'				IE_LAUDO_ESPECIAL, 
...
SELECT   	'ITEM DA CONTA     ' 	DS_REGISTRO, 
		...
		' '				ie_tipo_doc_paciente, 
		' '				nr_doc_paciente, 
		CASE WHEN c.ie_total_interf=2 THEN '1' WHEN c.ie_total_interf=3 THEN '2' WHEN c.ie_total_interf=9 THEN '1'  ELSE ' ' END  
						ie_matmed, 
		0				nr_crm_resp, 
		' '				uf_crm_resp, 
		substr(c.cd_senha_guia,1,10)	cd_senha, 
...
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA and B.nr_seq_protocolo	= y.nr_seq_protocolo    -- and		nvl(Z.ie_entra_conta,'S') = 'S' 
 GROUP BY	somente_numero(coalesce(y.cd_interno,0)), 
		B.NR_SEQ_PROTOCOLO, 
      	B.NR_INTERNO_CONTA, 
		substr(ELIMINA_ACENTUACAO(UPPER(B.NM_PACIENTE)),1,40), 
