...
CREATE OR REPLACE VIEW admissao_hospitalar_v2 (nr_atendimento, nr_atend_original, dt_entrada, cd_pessoa_fisica, cd_procedencia, ie_tipo_atendimento, cd_medico_resp, dt_alta, cd_motivo_alta, ie_permite_visita, ie_permite_visita_rel, ie_clinica, ie_clinica_ant, nr_seq_classificacao, dt_entrada_unidade, dt_saida_unidade, cd_setor_atendimento, cd_unidade_basica, cd_unidade_compl, cd_tipo_acomod_unid, cd_convenio, cd_categoria, cd_usuario_convenio, cd_empresa, cd_tipo_acomod_conv, nr_doc_convenio, ds_setor_atendimento, ds_convenio, ds_categoria, nm_medico, nm_paciente, nr_prontuario, dt_nascimento, cd_religiao, ie_carater_inter_sus, nr_seq_indicacao, cd_estabelecimento, nr_seq_tipo_acidente, dt_saida_real, nm_usuario_saida, dt_cancelamento, ds_nivel_urgencia, ie_paciente_isolado) AS SELECT  A.NR_ATENDIMENTO,
	A.NR_ATEND_ORIGINAL,
        A.DT_ENTRADA,
        A.CD_PESSOA_FISICA,
        A.CD_PROCEDENCIA,
...
and	B.nr_seq_interno	= (	select	coalesce(min(o.nr_seq_interno),0)
					from 	atend_paciente_unidade o
					where	o.nr_atendimento 	= A.nr_atendimento
	  				and 	o.dt_entrada_unidade	=
					(	select  min(h.dt_entrada_unidade)
						from 	Setor_Atendimento y,
							atend_paciente_unidade h
						where 	h.nr_atendimento 	= A.nr_atendimento
		  				and 	h.cd_setor_atendimento	= y.cd_setor_atendimento
