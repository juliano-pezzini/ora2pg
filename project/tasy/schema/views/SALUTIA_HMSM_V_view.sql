...
SELECT     	'ITEM DA CONTA         ' 	DS_REGISTRO,
		...
		' '				UF_CRM_REQ,
           	...
           	substr(HSM_Obter_Proc_Hor_Esp(c.dt_item, c.cd_item, c.CD_ITEM_CONVENIO),1,20)	CD_ITEM,
...
WHERE c.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA and w.nr_seq_protocolo = x.nr_seq_protocolo  and c.ie_tipo_item	= 1 and c.qt_item		<> 0 and coalesce(Z.ie_entra_conta,'S') = 'S' AND c.cd_item in (select x.cd_procedimento
		...
		c.NR_SEQ_PROTOCOLO,
           		c.NR_INTERNO_CONTA,
           		substr(HSM_Obter_Proc_Hor_Esp(c.dt_item, c.cd_item, c.CD_ITEM_CONVENIO),1,20),
           		substr(ELIMINA_ACENTUACAO(upper(c.DS_ITEM)),1,50),
           		CASE WHEN W.IE_CARATER_INTER='U' THEN 'S'  ELSE 'N' END ,
           		CASE WHEN c.pr_hora_extra=0 THEN 'N' WHEN c.pr_hora_extra=1 THEN 'N' WHEN c.pr_hora_extra IS NULL THEN 'N'  ELSE 'S' END ,
...
SELECT     	'ITEM DA CONTA         ' 	DS_REGISTRO,
		...
		' '				UF_CRM_REQ,
           	...
           	CASE WHEN W.IE_CARATER_INTER='U' THEN 'S'  ELSE 'N' END 
						IE_EMERGENCIA,
           	max(CASE WHEN c.pr_hora_extra=0 THEN 'N' WHEN c.pr_hora_extra=1 THEN 'N' WHEN c.pr_hora_extra IS NULL THEN 'N'  ELSE 'S' END )
                       				IE_EXTRA,
           	TRUNC(W.DT_ENTRADA,'dd')  		DT_ITEM,
           	0         	IE_FUNCAO_MEDICO,
           	1		               	QT_ITEM,
           	SUM(coalesce(c.vl_filme, 0))	VL_ITEM,
           	0                        	IE_CIRURGIA_MULTIPLA,
           	0		      		VL_TOTAL_ITEM,
           	0	      			QT_ITEM_CONTA,
	     	' '				NM_PACIENTE,
		...
		0					nr_crm_resp,
		''		uf_crm_resp,
		max(substr(c.nr_doc_convenio,1,10))	cd_senha,
		0		cd_especialidade,
		0				vl_proc_principal,
		max(CASE WHEN c.pr_via_acesso=100 THEN 1 WHEN c.pr_via_acesso=70 THEN 2  ELSE 3 END )
						ie_tipo_cirurgia,
		0				cd_unidade_atend,
		0				qt_diaria_enfermaria,
		0				qt_diaria_uti,
		...
		coalesce(somente_numero(x.nr_protocolo),0)
						cd_interno_protocolo,
		max(coalesce(somente_numero(c.nr_doc_convenio),0))
						nr_guia,
		w.cd_senha_guia		cd_senha_guia,
		0				tp_internacao
FROM w_interf_conta_header x, w_interf_conta_cab w, w_interf_conta_item c
LEFT OUTER JOIN regra_honorario z ON (c.ie_responsavel_credito = Z.cd_regra)
WHERE c.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA and w.nr_seq_protocolo = x.nr_seq_protocolo  and c.ie_tipo_item	= 1 and c.qt_item		<> 0 and coalesce(Z.ie_entra_conta,'S') = 'S' GROUP BY	somente_numero(coalesce(x.cd_interno,0)),
		c.NR_SEQ_PROTOCOLO,
         	c.NR_INTERNO_CONTA,
          	CASE WHEN W.IE_CARATER_INTER='U' THEN 'S'  ELSE 'N' END ,
           	TRUNC(W.DT_ENTRADA,'dd'),
...
GROUP BY	somente_numero(coalesce(x.cd_interno,0)),
		...
		coalesce(somente_numero(c.nr_doc_convenio),0),
		w.cd_senha_guia
having	SUM(c.QT_ITEM) <> 0 and sum(c.vl_filme) = 0 and SUM(c.vl_total_item) <> 0
...
SELECT     	'ITEM DA CONTA         ' 	DS_REGISTRO,
		...
		' '				UF_CRM_REQ,
           	...
           	c.CD_FUNCAO_EXECUTOR         	IE_FUNCAO_MEDICO,
           	SUM(c.QT_ITEM)               	QT_ITEM,
           	SUM(c.vl_total_item) - sum(c.vl_filme) 	VL_ITEM,
           	0                        	IE_CIRURGIA_MULTIPLA,
           	0		      		VL_TOTAL_ITEM,
           	0	      			QT_ITEM_CONTA,
	     	' '				NM_PACIENTE,
...
GROUP BY	somente_numero(coalesce(x.cd_interno,0)),
		...
		coalesce(somente_numero(c.nr_doc_convenio),0),
		w.cd_senha_guia
having	SUM(c.QT_ITEM) <> 0 and sum(c.vl_filme) <> 0
