...
and 	SUM(coalesce(c.vl_honorario,0) + coalesce(c.vl_custo_oper,0)) > 0 
...
SELECT   	'TOTAL DA CONTA    ' 	DS_REGISTRO, 
		...
		w.UF_CRM_MEDICO_RESP		UF_CRM_REQ, 
      	...
      	0				VL_ITEM, 
      	0				IE_CIRURGIA_MULTIPLA, 
      	SUM(coalesce(c.vl_honorario,0) + coalesce(c.vl_custo_oper,0)) VL_TOTAL_ITEM, 
      	0				QT_ITEM_CONTA, 
		...
		'F'				ie_fim_registro, 
		w.ie_tipo_atendimento		ie_tipo_atend_tasy, 
		w.nr_doc_convenio		nr_doc_convenio 
FROM w_interf_conta_header x, w_interf_conta_cab w, w_interf_conta_item c
LEFT OUTER JOIN regra_honorario z ON (c.ie_responsavel_credito = Z.cd_regra)
WHERE c.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA and w.nr_seq_protocolo = x.nr_seq_protocolo  and c.ie_tipo_item	= 1 and coalesce(Z.ie_entra_conta,'S') = 'S' AND c.cd_item in (select x.cd_procedimento 
		from estrutura_procedimento_v x 
...
SELECT   	'FIM DE LOTE      ' 	DS_REGISTRO, 
		...
		' '				UF_CRM_REQ, 
      	...
      	0	   			QT_ITEM_CONTA, 
	   	' '				NM_PACIENTE, 
      	' '	   			NR_FATURA, 
      	count(distinct c.nr_interno_conta) QT_CONTA, 
      	SUM(coalesce(c.vl_honorario,0) + coalesce(c.vl_custo_oper,0)) VL_TOTAL_ITEM, 
	   	w.NR_SEQ_PROTOCOLO		NR_SEQ_PROTOCOLO, 
		' '				DS_ESPACO, 
      	1            	NR_REGISTRO, 
...
GROUP BY	somente_numero(coalesce(x.cd_interno,0)), 
		w.NR_SEQ_PROTOCOLO      	 
GROUP BY	cd_origem, 
		NR_SEQ_PROTOCOLO HAVING SUM(c.QT_ITEM) > 0 
) alias165
;
