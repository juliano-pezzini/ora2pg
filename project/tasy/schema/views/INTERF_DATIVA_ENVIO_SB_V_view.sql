...
SELECT		'CABECALHO CONTA   '		DS_REGISTRO, 
		...
		CASE WHEN B.cd_convenio=8 THEN somente_numero(B.nr_doc_convenio) WHEN B.cd_convenio=20 THEN  somente_numero(B.nr_doc_convenio)  ELSE B.NR_INTERNO_CONTA END 	NR_INTERNO_CONTA, 
		1				NR_PARCELA, 
		0				CD_CONVENIO, 
		' '				NM_CONVENIO, 
		'J'				TP_PRESTADOR, 
		...
		substr(B.cd_senha_guia,1,10) 
						CD_AUTORIZACAO, 
		Obter_Unidade_Atendimento(B.nr_atendimento, 'A', 'CTA')		CD_ACOMODACAO, 
...
SELECT   	'ITEM DA CONTA     ' 	DS_REGISTRO, 
		...
		CASE WHEN w.cd_convenio=8 THEN somente_numero(w.nr_doc_convenio) WHEN w.cd_convenio=20 THEN  somente_numero(w.nr_doc_convenio)  ELSE c.NR_INTERNO_CONTA END 	NR_INTERNO_CONTA, 
...
WHERE c.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA and w.nr_seq_protocolo = x.nr_seq_protocolo  and c.ie_tipo_item	= 1 and coalesce(Z.ie_entra_conta,'S') = 'S' AND c.cd_item in (select x.cd_procedimento 
		...
		c.NR_SEQ_PROTOCOLO, 
      	CASE WHEN w.cd_convenio=8 THEN somente_numero(w.nr_doc_convenio) WHEN w.cd_convenio=20 THEN  somente_numero(w.nr_doc_convenio)  ELSE c.NR_INTERNO_CONTA END , 
      	c.CD_ITEM_CONVENIO, 
      	substr(ELIMINA_ACENTUACAO(upper(c.DS_ITEM)),1,50), 
      	CASE WHEN W.IE_CARATER_INTER='U' THEN 'S'  ELSE 'N' END , 
      	CASE WHEN c.pr_hora_extra=0 THEN 'N' WHEN c.pr_hora_extra=1 THEN 'N' WHEN c.pr_hora_extra IS NULL THEN 'N'  ELSE 'S' END , 
...
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA and B.nr_seq_protocolo	= y.nr_seq_protocolo    and coalesce(Z.ie_entra_conta,'S') = 'S' GROUP BY	somente_numero(coalesce(y.cd_interno,0)), 
		B.NR_SEQ_PROTOCOLO, 
      	CASE WHEN B.cd_convenio=8 THEN somente_numero(B.nr_doc_convenio) WHEN B.cd_convenio=20 THEN  somente_numero(B.nr_doc_convenio)  ELSE B.NR_INTERNO_CONTA END , 
		substr(ELIMINA_ACENTUACAO(UPPER(B.NM_PACIENTE)),1,40), 
