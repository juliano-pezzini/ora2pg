...
select
        ...
        0                                        nr_seq_envio,
        0                                        nr_seq_reg,
        substr(coalesce(obter_dados_categ_conv(a.nr_atendimento,'S'),'0000000'),1,7)
                                                nr_senha,
...
select
        ...
        0                                        nr_seq_envio,
        0                                        nr_seq_reg,
        substr(coalesce(x.cd_senha,'0000000'),1,7)
                                                nr_senha,
        0                                        nr_carteira,
        0                                        tp_atendimento,
        0                                        ie_causa,
        ...
        0                                        vl_matmed,
        ' '                                        ie_matmed
FROM procedimento_paciente x, w_interf_conta_item c, w_interf_conta_cab a
LEFT OUTER JOIN w_interf_conta_autor b ON (a.nr_interno_conta = b.nr_interno_conta)
WHERE c.nr_seq_item = x.nr_sequencia and b.nr_sequencia                =
        (select coalesce(min(x.nr_sequencia),b.nr_sequencia)
                from w_interf_conta_autor x
                where a.nr_interno_conta = x.nr_interno_conta) and a.nr_interno_conta        = c.nr_interno_conta and c.cd_area_proc                in (1,2,3,4,9,14) and obter_classif_material_proced(null, c.cd_item, c.ie_origem_proced) <> '2' group by
        a.nr_seq_protocolo,
        a.nr_interno_conta,
        substr(coalesce(x.cd_senha,'0000000'),1,7),
        somente_numero(substr(coalesce(c.cd_item_convenio,coalesce(c.cd_item,'0')),1,8)),
...
select
        ...
        0                                        nr_seq_envio,
        0                                        nr_seq_reg,
	substr(coalesce(obter_dados_categ_conv(a.nr_atendimento,'S'),'00000000'),1,7)     nr_senha,
        0                                        nr_carteira,
        0                                        tp_atendimento,
        0                                        ie_causa,
        ' '                                        ie_internado,
...
select
        ...
        ' '                                        nm_profissional,
        ' '                                        ie_profissional,
        substr(somente_numero(coalesce(c.cd_item_convenio,coalesce(c.cd_item,'0'))),1,5)
                                                cd_matmed,
...
WHERE b.nr_sequencia                =
        (select coalesce(min(x.nr_sequencia),b.nr_sequencia)
                from w_interf_conta_autor x
                where a.nr_interno_conta = x.nr_interno_conta) and a.nr_interno_conta        = c.nr_interno_conta and ((c.ie_tipo_item = 2) or (obter_classif_material_proced(null, c.cd_item, c.ie_origem_proced) = '2')) --and        c.cd_tipo_item_interf        in(4,5)
 group by
        a.nr_seq_protocolo,
        a.nr_interno_conta,
        substr(coalesce(c.nr_doc_convenio,
