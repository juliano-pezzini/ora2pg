...
where coalesce(a.cd_grupo_proc,0) not in (32100000,32120001,94010001) 
...
SELECT   	a.NR_SEQ_PROTOCOLO,a.CD_CONVENIO CD_CONVENIO,5 TP_REGISTRO, 
		...
		0					VL_UNITARIO_CH, 
		0					VL_UNITARIO, 
		sum(b.vl_total_item) + coalesce(sum(CASE WHEN b.cd_grupo_proc=32100000 THEN  coalesce(b.VL_CUSTO_OPER,0) WHEN b.cd_grupo_proc=32120001 THEN  coalesce(b.VL_CUSTO_OPER,0) WHEN b.cd_grupo_proc=94010001 THEN  coalesce(b.VL_CUSTO_OPER,0)  ELSE CASE WHEN b.ie_responsavel_credito='M' THEN  							CASE WHEN b.vl_total_item=0 THEN  0  ELSE CASE WHEN coalesce(b.vl_honorario,0)=0 THEN  0  ELSE CASE WHEN coalesce(b.vl_honorario,0)=b.vl_total_item THEN  coalesce(b.vl_custo_oper,0)  ELSE 0 END  END  END  END  END ),0) vl_item, 
		0					PR_FUNCAO_PARTIC, 
		0					IE_VIA_ACESSO, 
		0					PR_VIA_ACESSO, 
		0					PR_HORA_EXTRA, 
		...
		sum(CASE WHEN b.cd_grupo_gasto='8' THEN CASE WHEN b.cd_grupo_proc=94010001 THEN  b.vl_total_item  ELSE 0 END  + b.vl_custo_oper  ELSE 0 END ) vl_tot_pacote, 
		0 vl_tot_ortese, 
		0 vl_tot_protese, 
