...
CREATE OR REPLACE VIEW unimed_toledo_v (tp_registro, ie_tipo_prestador, cd_prestador, nm_prestador, nr_lote, dt_geracao, cd_tipo_documento, nr_documento, cd_item, ie_tipo_item, ds_item, qt_item, vl_item, vl_total_item, nr_cep, cd_tipo_prestador, nr_crm, dt_inicio_internacao, dt_alta, ie_padrao_internacao, cd_unimed_usuario, cd_contrato_usuario, cd_familia_usuario, ie_dependencia_usuario, ie_digito_ver_usuario, dt_exec_servico, hr_exec_servico, cd_cid, ie_detalhe_cid, cd_dig_verif_cid, cd_seq_doc_servico, cd_amb_lpm, cd_dig_verif_servico, qt_servico_executado, ie_porte_cirurgico, ie_partic_executante, cd_local_atendimento, vl_servico, cd_moeda_servico, vl_filme, cd_moeda_filme, nr_autorizacao, cd_fator_plano, ie_tipo_prest_executante, cd_prest_executante, ie_situacao_guia, nm_paciente, hr_saida_paciente, nr_seq_protocolo, nr_atendimento, nr_interno_conta, qt_doc_enviados, qt_serv_relacionados, qt_total_servicos_real, qt_total_cobrado_ch, qt_total_filme, cd_campo_quebra, pr_faturado, ie_utilizado, nr_guia, cd_cid_principal, cd_medico_solic, nm_medico_resp, qt_dias_internacao, ie_atend_emergencia, cd_tipo_acomodacao, cd_usuario_convenio, nm_empresa, dt_nascimento, ie_sexo, ie_tipo_nascimento, ie_status_nota, ie_tipo_registro, nr_identidade, ie_tipo_alta, ie_tipo_internamento, nr_guia_origem, nr_doc_convenio, cd_proc_principal, cd_conv_medico_resp, vl_unitario, dt_atendimento, hr_atendimento, ie_tipo_atendimento) AS select		0					tp_registro,
		''					ie_tipo_prestador, 
		''					cd_prestador, 
...
select		distinct 
		...
		substr(Obter_menor_guia_conta(a.nr_interno_conta),1,15) nr_doc_convenio, 
		a.CD_PROC_PRINCIPAL, 
...
select		5					tp_registro, 
		CASE WHEN b.cd_funcao_executor=5 THEN  'A' WHEN b.cd_funcao_executor=3 THEN  '1' WHEN b.cd_funcao_executor=4 THEN  '2'  ELSE 'P' END  ie_tipo_prestador, 
		substr(coalesce(cd_medico_exec_conv, 
			Obter_Conversao_Externa(b.CD_CGC_CONVENIO, 'UNIMED_TOLEDO_V', 'CD_PRESTADOR', b.cd_cgc_prestador)),1,20) cd_prestador, 
		''					nm_prestador, 
		0					nr_lote, 
		LOCALTIMESTAMP				dt_geracao, 
		CASE WHEN a.ie_tipo_atendimento=1 THEN 2 WHEN a.ie_tipo_atendimento=3 THEN 6 WHEN a.ie_tipo_atendimento=7 THEN 9  ELSE 2 END 	cd_tipo_documento, 
...
WHERE b.nr_interno_conta		= a.nr_interno_conta and b.ie_tipo_item			= 1    and a.nr_interno_conta		= c.nr_interno_conta and coalesce(b.nr_doc_convenio, Obter_maior_guia_conta(a.nr_interno_conta)) = c.cd_autorizacao and somente_numero(c.cd_autorizacao) > 0 group by	a.cd_usuario_convenio, 
		b.cd_item_convenio, 
		a.ie_tipo_atendimento, 
		CASE WHEN b.cd_funcao_executor=5 THEN  'A' WHEN b.cd_funcao_executor=3 THEN  '1' WHEN b.cd_funcao_executor=4 THEN  '2'  ELSE 'P' END , 
		b.ds_item, 
		...
		a.CD_PROC_PRINCIPAL, 
		substr(coalesce(cd_medico_exec_conv, 
			Obter_Conversao_Externa(b.CD_CGC_CONVENIO, 'UNIMED_TOLEDO_V', 'CD_PRESTADOR', b.cd_cgc_prestador)),1,20), 
		a.CD_CONV_MEDICO_RESP 
