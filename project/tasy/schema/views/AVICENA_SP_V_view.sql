...
CREATE OR REPLACE VIEW avicena_sp_v (ds_registro, tp_registro, ie_servico, cd_registro, cd_origem, nr_lote, nr_interno_conta, nr_parcela, cd_convenio, nm_convenio, tp_prestador, cd_prestador, nm_prestador, dt_geracao, nr_remessa, cd_plano, cd_paciente, cd_autorizacao, cd_acomodacao, dt_entrada, dt_saida, dt_inicio_cobranca, dt_fim_cobranca, ie_tipo_atendimento, cd_cid, cd_proc_principal, cd_medico_convenio, nr_crm, uf_crm, nr_crm_req, uf_crm_req, nr_seq_item, cd_item, nm_item, ie_emergencia, ie_extra, dt_item, ie_funcao_medico, qt_item, vl_item, ie_cirurgia_multipla, vl_total_item, qt_item_conta, nm_paciente, nr_fatura, qt_conta, vl_total_lote, nr_seq_protocolo, ds_espaco, nr_registro, ie_matmed, ds_ma_me_tx, ie_tipo_atend_esp, ie_tipo_atend_tasy, cd_especialidade) AS SELECT		'HEADER        ' 	DS_REGISTRO,
		1				TP_REGISTRO, 
		'CSMH'				IE_SERVICO, 
		0				CD_REGISTRO, 
...
SELECT		'CABECALHO CONTA   '	DS_REGISTRO, 
		...
		substr(B.CD_USUARIO_CONVENIO,1,8) 
						CD_PACIENTE,		 
		...
		CASE WHEN Length(coalesce(B.cd_cid_principal,' '))=4 THEN (substr(B.cd_cid_principal,1,3) || '.' || substr(B.cd_cid_principal,4,1)) WHEN Length(coalesce(B.cd_cid_principal,' '))=3 THEN (substr(B.cd_cid_principal,1,3))  ELSE B.cd_cid_principal END  	cd_cid, 
		...
		' '				IE_MATMED, 
		' '				DS_MA_ME_TX, 
		substr(to_char(Obter_Tipo_Atend_Abramge(B.nr_interno_conta)),1,1) 
						ie_tipo_atend_esp, 
		B.ie_tipo_atendimento	ie_tipo_atend_tasy, 
		B.IE_CLINICA			CD_ESPECIALIDADE 
...
SELECT   	'ITEM DA CONTA     ' 	DS_REGISTRO, 
		...
		' '				UF_CRM_REQ, 
      	...
      	substr(ELIMINA_ACENTUACAO(upper(C.DS_ITEM)),1,45) || CASE WHEN C.cd_grupo_mat=4 THEN  '   '  ELSE CASE WHEN C.cd_brasindice IS NULL THEN  '   '  ELSE '/' || substr(C.cd_brasindice, 9, 4) END  END  NM_ITEM, 
		...
		SUM(CASE WHEN C.NR_SEQ_PROC_PACOTE=C.NR_SEQ_ITEM THEN  C.vl_total_item  ELSE CASE WHEN(coalesce(C.vl_honorario,0) + coalesce(C.vl_custo_oper,0) + coalesce(C.vl_filme,0))=0 THEN C.vl_total_item  ELSE (coalesce(C.vl_honorario,0) + coalesce(C.vl_custo_oper,0) + coalesce(C.vl_filme,0)) END  END )		 
						VL_ITEM, 
      	0            	IE_CIRURGIA_MULTIPLA, 
      	0		   		VL_TOTAL_ITEM, 
      	0	   			QT_ITEM_CONTA, 
...
WHERE C.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA  and ((coalesce(Z.ie_entra_conta,'S') = 'S') or ((C.NR_SEQ_PROC_PACOTE = C.NR_SEQ_ITEM) or (C.NR_SEQ_PROC_PACOTE is NULL))) and C.ie_tipo_item	= 1 and C.qt_item		<> 0 and C.cd_medico_executor is not null GROUP BY	C.NR_SEQ_PROTOCOLO, 
      	C.NR_INTERNO_CONTA, 
      	C.CD_ITEM_CONVENIO, 
      	substr(ELIMINA_ACENTUACAO(upper(C.DS_ITEM)),1,45) || CASE WHEN C.cd_grupo_mat=4 THEN  '   '  ELSE CASE WHEN C.cd_brasindice IS NULL THEN  '   '  ELSE '/' || substr(C.cd_brasindice, 9, 4) END  END , 
      	CASE WHEN W.IE_CARATER_INTER='U' THEN 'S'  ELSE 'N' END , 
      	CASE WHEN C.pr_hora_extra=0 THEN 'N' WHEN C.pr_hora_extra=1 THEN 'N' WHEN C.pr_hora_extra IS NULL THEN 'N'  ELSE 'S' END , 
...
SELECT   	'ITEM DA CONTA     ' 	DS_REGISTRO, 
		...
		' '				UF_CRM_REQ, 
      	max(C.NR_SEQ_ITEM)     	NR_SEQ_ITEM, 
      	substr(coalesce(substr(CASE WHEN C.cd_grupo_mat=4 THEN  to_char(C.cd_simpro)  ELSE coalesce(C.cd_brasindice, C.cd_simpro) END ,1,8), C.CD_ITEM_CONVENIO),1,8) CD_ITEM, 
		substr(ELIMINA_ACENTUACAO(upper(C.DS_ITEM)),1,45) || CASE WHEN C.cd_grupo_mat=4 THEN  '   '  ELSE CASE WHEN C.cd_brasindice IS NULL THEN  '   '  ELSE '/' || substr(C.cd_brasindice, 9, 4) END  END  NM_ITEM, 
      	CASE WHEN W.IE_CARATER_INTER='U' THEN 'S'  ELSE 'N' END  
						IE_EMERGENCIA, 
      	CASE WHEN C.pr_hora_extra=0 THEN 'N' WHEN C.pr_hora_extra=1 THEN 'N' WHEN C.pr_hora_extra IS NULL THEN 'N'  ELSE 'S' END  
...
GROUP BY	C.NR_SEQ_PROTOCOLO, 
      	C.NR_INTERNO_CONTA, 
      	substr(coalesce(substr(CASE WHEN C.cd_grupo_mat=4 THEN  to_char(C.cd_simpro)  ELSE coalesce(C.cd_brasindice, C.cd_simpro) END ,1,8), C.CD_ITEM_CONVENIO),1,8), 
      	substr(ELIMINA_ACENTUACAO(upper(C.DS_ITEM)),1,45) || CASE WHEN C.cd_grupo_mat=4 THEN  '   '  ELSE CASE WHEN C.cd_brasindice IS NULL THEN  '   '  ELSE '/' || substr(C.cd_brasindice, 9, 4) END  END , 
      	CASE WHEN W.IE_CARATER_INTER='U' THEN 'S'  ELSE 'N' END , 
      	CASE WHEN C.pr_hora_extra=0 THEN 'N' WHEN C.pr_hora_extra=1 THEN 'N' WHEN C.pr_hora_extra IS NULL THEN 'N'  ELSE 'S' END , 
      	TRUNC(W.DT_ENTRADA,'dd'), 
...
SELECT   	'TOTAL DA CONTA    ' 	DS_REGISTRO, 
		...
		' '				UF_CRM_REQ, 
      	...
      	0				VL_ITEM, 
      	0				IE_CIRURGIA_MULTIPLA, 
      	sum(CASE WHEN e.cd_medico_executor IS NULL THEN e.vl_total_item  ELSE CASE WHEN e.NR_SEQ_PROC_PACOTE=e.NR_SEQ_ITEM THEN  e.vl_total_item  ELSE CASE WHEN(coalesce(e.vl_honorario,0) + coalesce(e.vl_custo_oper,0) + coalesce(e.vl_filme,0))=0 THEN e.vl_total_item  ELSE (coalesce(e.vl_honorario,0) + coalesce(e.vl_custo_oper,0) + coalesce(e.vl_filme,0)) END  END  END ) VL_TOTAL_ITEM, 
      	0				QT_ITEM_CONTA, 
...
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA    and ((coalesce(Z.ie_entra_conta,'S') = 'S') or ((e.NR_SEQ_PROC_PACOTE = e.NR_SEQ_ITEM) or (e.NR_SEQ_PROC_PACOTE is NULL))) GROUP BY	B.NR_SEQ_PROTOCOLO, 
      	B.NR_INTERNO_CONTA, 
		substr(ELIMINA_ACENTUACAO(UPPER(B.NM_PACIENTE)),1,40), 
...
LEFT OUTER JOIN estrutura_procedimento_v x ON (E.CD_ITEM = X.CD_PROCEDIMENTO AND E.IE_ORIGEM_PROCED = X.IE_ORIGEM_PROCED)
LEFT OUTER JOIN regra_honorario z ON (E.IE_RESPONSAVEL_CREDITO = Z.cd_regra)
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA and B.nr_seq_protocolo	= f.nr_seq_protocolo    and ((coalesce(Z.ie_entra_conta,'S') = 'S') or ((e.NR_SEQ_PROC_PACOTE = e.NR_SEQ_ITEM) or (e.NR_SEQ_PROC_PACOTE is NULL))) GROUP BY	(coalesce(f.CD_INTERNO,0))::numeric , 
		B.NR_SEQ_PROTOCOLO;
