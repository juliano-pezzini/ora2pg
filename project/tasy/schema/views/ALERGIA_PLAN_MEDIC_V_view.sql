...
CREATE OR REPLACE VIEW alergia_plan_medic_v (ds_tipo_alerta, nr_seq_alerta, cd_pessoa_fisica, nr_seq_alergeno, ds_alergeno, nr_seq_dcb, ds_dcb, nr_seq_ficha_tecnica, ds_ficha_tecnica, cd_material, ds_material, nr_seq_dcb_mat, ds_dcb_mat, cd_classe_mat, ds_classe_mat, nr_seq_reacao, ds_tipo_reacao, ds_medic_nao_cad, ie_confirmacao, ds_confirmacao, ie_nega, dt_liberacao, nr_seq_familia, ds_familia_mat) AS select	
...
where	((a.dt_fim is null) or (LOCALTIMESTAMP between coalesce(a.dt_inicio,LOCALTIMESTAMP - interval '1 days') and a.dt_fim)) 
and	a.dt_inativacao is null 
and	coalesce(a.ie_nega_alergias,'N')	= 'N';
