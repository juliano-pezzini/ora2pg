...
select
	...
	3				tp_prestador,
	CASE WHEN c.tp_nota=85 THEN 0  ELSE CASE WHEN a.ie_tipo_atendimento=1 THEN a.cd_motivo_alta  ELSE 0 END  END 
					cd_motivo_cobranca,
	a.dt_entrada			dt_entrada,
	a.dt_alta			dt_alta,
	substr(a.nr_doc_convenio,1,6)	nr_guia,
	coalesce((lpad(a.nr_seq_conta_convenio,10,'0'))::numeric ,
		(lpad(a.nr_interno_conta,10,'0'))::numeric ) nr_interno_conta,
	(sum(CASE WHEN c.tp_nota=76 THEN coalesce(c.vl_total_item,0) WHEN c.tp_nota=86 THEN coalesce(c.vl_total_item,0)  ELSE 0 END ) +
	sum(CASE WHEN c.tp_nota=77 THEN CASE WHEN coalesce(c.vl_total_item,0)=0 THEN coalesce(c.vl_custo_oper,0)  ELSE coalesce(c.vl_total_item,0) END  WHEN c.tp_nota=87 THEN CASE WHEN coalesce(c.vl_total_item,0)=0 THEN coalesce(c.vl_custo_oper,0)  ELSE coalesce(c.vl_total_item,0) END   ELSE 0 END ) +
	sum(CASE WHEN c.tp_nota=75 THEN CASE WHEN(coalesce(c.vl_total_item,0) + coalesce(c.vl_custo_oper,0))=0 THEN coalesce(c.vl_honorario,0)  ELSE (coalesce(c.vl_total_item,0) + coalesce(c.vl_custo_oper,0)) END  WHEN c.tp_nota=85 THEN CASE WHEN(coalesce(c.vl_total_item,0) + coalesce(c.vl_custo_oper,0))=0 THEN coalesce(c.vl_honorario,0)  ELSE (coalesce(c.vl_total_item,0) + coalesce(c.vl_custo_oper,0)) END   ELSE 0 END )) vl_total_conta,
	...
	sum(CASE WHEN c.tp_nota=76 THEN coalesce(c.vl_total_item,0) WHEN c.tp_nota=86 THEN coalesce(c.vl_total_item,0)  ELSE 0 END ) vl_total_despesa,
	...
	sum(CASE WHEN c.tp_nota=77 THEN CASE WHEN coalesce(c.vl_total_item,0)=0 THEN coalesce(c.vl_custo_oper,0)  ELSE coalesce(c.vl_total_item,0) END  WHEN c.tp_nota=87 THEN CASE WHEN coalesce(c.vl_total_item,0)=0 THEN coalesce(c.vl_custo_oper,0)  ELSE coalesce(c.vl_total_item,0) END   ELSE 0 END ) vl_total_servico,
	sum(CASE WHEN c.tp_nota=75 THEN CASE WHEN(coalesce(c.vl_total_item,0) + coalesce(c.vl_custo_oper,0))=0 THEN coalesce(c.vl_honorario,0)  ELSE (coalesce(c.vl_total_item,0) + coalesce(c.vl_custo_oper,0)) END  WHEN c.tp_nota=85 THEN CASE WHEN(coalesce(c.vl_total_item,0) + coalesce(c.vl_custo_oper,0))=0 THEN coalesce(c.vl_honorario,0)  ELSE (coalesce(c.vl_total_item,0) + coalesce(c.vl_custo_oper,0)) END   ELSE 0 END ) vl_total_honorario,
	0				nr_linha,
	LOCALTIMESTAMP				dt_item,
	0				cd_item_convenio,
	0				qt_dias_executado,
...
group by
	...
	coalesce((lpad(a.nr_seq_conta_convenio,10,'0'))::numeric ,
		(lpad(a.nr_interno_conta,10,'0'))::numeric )
...
select
	...
	' '				cd_usuario_convenio,
	' '				cd_cid,
	CASE WHEN c.ie_responsavel_credito='RM' THEN  '87701249000374'  ELSE to_char(c.cd_prestador) END  cd_prestador,
	3				tp_prestador,
	0 				cd_motivo_cobranca,
	LOCALTIMESTAMP				dt_entrada,
	LOCALTIMESTAMP				dt_alta,
	...
	CASE WHEN c.cd_funcao_executor=41 THEN c.ie_urgencia WHEN c.cd_funcao_executor=42 THEN c.ie_urgencia WHEN c.cd_funcao_executor=43 THEN c.ie_urgencia  ELSE CASE WHEN c.ie_video='S' THEN 15  ELSE c.ie_urgencia END  END 
				ie_urgencia,
	CASE WHEN(coalesce(c.vl_total_item,0)+coalesce(c.vl_custo_oper,0))=0 THEN coalesce(c.vl_honorario,0)  ELSE (coalesce(c.vl_total_item,0)+coalesce(c.vl_custo_oper,0)) END 
					vl_honorario,
	CASE WHEN(coalesce(c.vl_total_item,0) + coalesce(c.vl_custo_oper,0))=0 THEN coalesce(c.vl_honorario,0)  ELSE (coalesce(c.vl_total_item,0) + coalesce(c.vl_custo_oper,0)) END 
					vl_total_item,
	0				vl_total_matmed,
...
select
	...
	substr(a.cd_usuario_convenio,1,13)
					cd_usuario_convenio,
	substr(a.cd_cid_principal,1,6)	cd_cid,
	to_char(a.cd_interno)		cd_prestador,
	3				tp_prestador,
	...
	0				ie_urgencia,
	0				vl_honorario,
	sum(coalesce(c.vl_total_item,0))		vl_total_item,
	sum(CASE WHEN c.ie_responsavel_credito='MEDIC' THEN coalesce(c.vl_total_item,0)  ELSE 0 END )
					vl_total_matmed,
	' '				cd_brasindice,
	' '				cd_unidade_medida
from	w_interf_conta_item_ipe c,
...
select
	...
	0				vl_honorario,
	coalesce(c.vl_total_item,0)			vl_total_item,
	0				vl_total_matmed,
...
select
	...
	0				ie_urgencia,
	0				vl_honorario,
	sum(CASE WHEN coalesce(c.vl_total_item,0)=0 THEN coalesce(c.vl_custo_oper,0)  ELSE coalesce(c.vl_total_item,0) END )
					vl_total_item,
	sum(CASE WHEN c.cd_item_convenio=8001 THEN coalesce(c.vl_total_item,0) WHEN c.cd_item_convenio=32200005 THEN coalesce(c.vl_total_item,0)  ELSE 0 END )
					vl_total_matmed,
	' '				cd_brasindice,
	' '				cd_unidade_medida
from	w_interf_conta_item_ipe c,
...
select
	...
	c.ie_via_acesso			ie_via_acesso,
	c.ie_urgencia			ie_urgencia,
	CASE WHEN(coalesce(c.vl_total_item,0) + coalesce(c.vl_honorario,0))=0 THEN coalesce(c.vl_custo_oper,0)  ELSE CASE WHEN coalesce(c.vl_total_item,0)=0 THEN coalesce(c.vl_honorario,0)  ELSE coalesce(c.vl_total_item,0) END  END 
					vl_honorario,
	CASE WHEN coalesce(c.vl_total_item,0)=0 THEN coalesce(c.vl_custo_oper,0)  ELSE coalesce(c.vl_total_item,0) END 
					vl_total_item,
	0				vl_total_matmed,
	' '				cd_brasindice,
	' '				cd_unidade_medida
