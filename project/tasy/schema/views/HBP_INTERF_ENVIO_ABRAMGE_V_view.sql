...
CREATE OR REPLACE VIEW hbp_interf_envio_abramge_v (ds_registro, tp_registro, ie_servico, cd_registro, cd_origem, nr_lote, nr_interno_conta, nr_parcela, cd_convenio, nm_convenio, tp_prestador, cd_prestador, nm_prestador, dt_geracao, nr_remessa, cd_plano, cd_paciente, cd_autorizacao, cd_acomodacao, dt_entrada, dt_saida, dt_inicio_cobranca, dt_fim_cobranca, ie_tipo_atendimento, cd_cid, cd_proc_principal, cd_medico_convenio, nr_crm, uf_crm, nr_crm_req, uf_crm_req, nr_seq_item, cd_item, nm_item, ie_emergencia, ie_extra, dt_item, ie_funcao_medico, qt_item, vl_item, ie_cirurgia_multipla, vl_total_item, qt_item_conta, nm_paciente, nr_fatura, qt_conta, vl_total_lote, nr_seq_protocolo, ds_espaco, nr_registro, ie_matmed, ds_ma_me_tx, ie_tipo_atend_esp, ie_tipo_atend_tasy, nm_medico_executor, nr_crm_executor, uf_crm_executor) AS SELECT		'HEADER        '			DS_REGISTRO,
		1					TP_REGISTRO, 
		'CSMH'					IE_SERVICO, 
		0					CD_REGISTRO, 
		(coalesce(substr(A.CD_INTERNO,1,5),0))::numeric  CD_ORIGEM, 
		0					NR_LOTE, 
		0					NR_INTERNO_CONTA, 
		0					NR_PARCELA, 
		(coalesce(substr(A.CD_REGIONAL,1,5),0))::numeric  CD_CONVENIO, 
		substr(ELIMINA_ACENTUACAO(UPPER(A.NM_CONVENIO)),1,40) 
							NM_CONVENIO, 
		'J'					TP_PRESTADOR, 
...
SELECT   	'ITEM DA CONTA     '	 	DS_REGISTRO, 
		...
		substr(C.NM_MEDICO_EXECUTOR,1,42)	nm_medico_executor, 
		substr(C.NR_CRM_EXECUTOR,1,6)		nr_crm_executor, 
...
WHERE C.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA  and C.cd_grupo_proc <> 10006 and ((coalesce(Z.ie_entra_conta,'S') = 'S') or (C.nr_seq_proc_pacote = C.nr_seq_item)) and C.ie_tipo_item	= 1 and C.qt_item		<> 0 and C.cd_medico_executor is not null GROUP BY	C.NR_SEQ_PROTOCOLO, 
      	C.NR_INTERNO_CONTA, 
      	C.CD_ITEM_CONVENIO, 
      	substr(ELIMINA_ACENTUACAO(upper(C.DS_ITEM)),1,42), 
		...
		substr(C.NM_MEDICO_EXECUTOR,1,42), 
		substr(C.NR_CRM_EXECUTOR,1,6), 
		substr(C.UF_CRM_EXECUTOR,1,2) 
...
SELECT   	'TOTAL DA CONTA    ' 		DS_REGISTRO, 
		...
		' '					UF_CRM_REQ, 
      	...
      	0					VL_ITEM, 
      	0					IE_CIRURGIA_MULTIPLA, 
      	sum(CASE WHEN e.cd_medico_executor IS NULL THEN e.vl_total_item  ELSE CASE WHEN(coalesce(e.vl_honorario,0) + coalesce(e.vl_custo_oper,0) + coalesce(e.vl_filme,0))=0 THEN e.vl_total_item  ELSE (coalesce(e.vl_honorario,0) + coalesce(e.vl_custo_oper,0) + coalesce(e.vl_filme,0)) END  END ) 
							VL_TOTAL_ITEM, 
      	0					QT_ITEM_CONTA, 
...
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA    --and 		e.cd_grupo_proc <> 10006 
  and ((coalesce(Z.ie_entra_conta,'S') = 'S') or (e.nr_seq_proc_pacote = e.nr_seq_item)) GROUP BY	B.NR_SEQ_PROTOCOLO, 
     	B.NR_INTERNO_CONTA, 
		substr(ELIMINA_ACENTUACAO(UPPER(B.NM_PACIENTE)),1,40), 
...
SELECT 	'TRAILLER       ' 		DS_REGISTRO, 
      	...
      	'CSMH'  				IE_SERVICO, 
      	9   			      	CD_REGISTRO, 
      	(coalesce(substr(f.CD_INTERNO,1,5),0))::numeric  CD_ORIGEM, 
      	99999999		   		NR_LOTE, 
      	99999999	   			NR_INTERNO_CONTA, 
      	99    		      	NR_PARCELA, 
      	0 	           		CD_CONVENIO, 
...
LEFT OUTER JOIN estrutura_procedimento_v x ON (E.CD_ITEM = X.CD_PROCEDIMENTO AND E.IE_ORIGEM_PROCED = X.IE_ORIGEM_PROCED)
LEFT OUTER JOIN regra_honorario z ON (E.IE_RESPONSAVEL_CREDITO = Z.cd_regra)
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA and B.nr_seq_protocolo	= f.nr_seq_protocolo  and e.cd_grupo_proc <> 10006   and ((coalesce(Z.ie_entra_conta,'S') = 'S') or (e.nr_seq_proc_pacote = e.nr_seq_item)) GROUP BY	(coalesce(substr(f.CD_INTERNO,1,5),0))::numeric , 
		B.NR_SEQ_PROTOCOLO;
