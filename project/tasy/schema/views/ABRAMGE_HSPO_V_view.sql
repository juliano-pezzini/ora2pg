...
SELECT		'CABECALHO CONTA   '		DS_REGISTRO, 
		...
		CASE WHEN B.IE_TIPO_ATENDIMENTO=3 THEN null  ELSE coalesce(to_char(B.DT_ALTA,'ddmmyyhhmi'),to_char(B.DT_PERIODO_FINAL,'ddmmyyhhmi')) END  DT_SAIDA, 
		CASE WHEN B.IE_TIPO_ATENDIMENTO=3 THEN null  ELSE to_char(B.DT_PERIODO_INICIAL,'ddmmyy') END DT_INICIO_COBRANCA, 
		CASE WHEN B.IE_TIPO_ATENDIMENTO=3 THEN null  ELSE coalesce(to_char(B.DT_ALTA,'ddmmyy'),to_char(B.DT_PERIODO_FINAL,'ddmmyy')) END  DT_FIM_COBRANCA, 
...
WHERE C.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA  and coalesce(Z.ie_entra_conta,'S') = 'S' and C.ie_tipo_item	= 1 and C.qt_item		<> 0 and C.cd_medico_executor is not null and C.CD_ITEM_CONVENIO <> '10000' GROUP BY	C.NR_SEQ_PROTOCOLO, 
      	C.NR_INTERNO_CONTA, 
      	C.CD_ITEM_CONVENIO, 
      	substr(ELIMINA_ACENTUACAO(upper(C.DS_ITEM)),1,50), 
...
SELECT   	'ITEM DA CONTA     '	 	DS_REGISTRO, 
		...
		CASE WHEN C.ie_total_interf=2 THEN '1' WHEN C.ie_total_interf=9 THEN '1' WHEN C.ie_total_interf=10 THEN '1' WHEN C.ie_total_interf=11 THEN '1' WHEN C.ie_total_interf=3 THEN '2'  ELSE ' ' END  
							IE_MAT_MED, 
		CASE WHEN C.ie_total_interf=2 THEN 'MA' WHEN C.ie_total_interf=9 THEN 'MA' WHEN C.ie_total_interf=10 THEN 'MA' WHEN C.ie_total_interf=11 THEN 'MA' WHEN C.ie_total_interf=3 THEN 'ME' WHEN C.ie_total_interf=5 THEN ' ' WHEN C.ie_total_interf=6 THEN ' ' WHEN C.ie_total_interf=7 THEN ' '  ELSE ' ' END  
							DS_MA_ME_TX, 
		' '					ie_tipo_atend_esp, 
		W.ie_tipo_atendimento		ie_tipo_atend_tasy 
