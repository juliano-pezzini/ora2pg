...
CREATE OR REPLACE VIEW eis_gestao_exames_v (nr_prescricao, dt_prescricao, cd_pessoa_dia, cd_medico, nm_medico, dt_liberacao_medico, nm_usuario_original, nm_usuario, dt_emissao_setor_atend, dt_liberacao, cd_pessoa_fisica, dt_prev_execucao, nr_sequencia, dt_liberacao_laudo, ie_status_execucao, ie_carater_inter_sus, ds_carater_inter, ie_status_filtro, ds_status_execucao, cd_setor_atendimento, cd_procedimento, ie_origem_proced, nm_usuario_laudo, nm_usuario_aprovacao, nm_usuario_seg_aprov, cd_medico_exec, nm_medico_prev_exec, cd_medico_executor, nm_medico_executor, cd_medico_resp, nm_medico_laudo, ds_autorizacao, ds_observacao, ds_procedimento, ds_procedimento_cod, nr_seq_proced, nr_seq_propaci, ds_dado_clinico, ds_motivo_parada, ds_senha, cd_material_exame, nr_prontuario, ds_setor_atendimento, cd_setor_entrega, dt_baixa, dt_procedimento, ds_motivo_baixa, ds_arquivo, nm_convenio, ie_laudo_ant, dt_ditado, ds_procedencia, dt_inicio, ds_lado_proced, cd_estabelecimento, nm_usuario_exec, ie_mat_med, ds_urgencia, ds_tipo_atend, ds_idade, dt_nascimento, ds_convenio, cd_usuario_convenio, nr_crm, ds_uf_medico, ds_senha_laudo_medico, ie_executar_leito, ds_local, ie_obs_proced, dt_inicio_exame, qt_prescrito, qt_liberado, qt_exame, ds_status_agenda, ds_exame_lab, ds_material_exame, ds_interv_prescr, ds_interv_liber, ds_interv_exame, ds_proced_princ_int, ds_tipo_convenio, ie_tipo_convenio, cd_convenio, ie_tipo_atendimento, ds_hora_procedimento, cd_hora_procedimento, cd_tipo_procedimento, ds_tipo_procedimento, cd_especialidade, ds_especialidade, cd_grupo_proc, ds_grupo_proc, cd_empresa, cd_setor_prescricao, ds_setor_prescricao, cd_proc_exec, ie_origem_exec, ds_proc_exec, qt_filme, nr_seq_proc_interno, nr_seq_exame, nr_seq_tamanho_filme, ds_tamanho_filme, ds_hora_prescricao, cd_hora_prescricao, nm_paciente, cd_paciente, ds_interv_exec, nm_medico_externo) AS select   a.nr_prescricao,
	...
	z.cd_pessoa_fisica || ' - ' || to_char(a.dt_prescricao,'dd/mm/yyyy') cd_pessoa_dia,
	 ...
	 substr((select nm_pessoa_fisica
		...
		where cd_pessoa_fisica = a.cd_medico),1,60) nm_medico,
		 ...
		 z.ie_carater_inter_sus,
		 substr((select ds_carater_internacao
				 from sus_carater_internacao
				 where cd_carater_internacao = z.ie_carater_inter_sus),1,40) ds_carater_inter,
	 ...
	 substr(obter_nome_convenio( coalesce(b.cd_convenio, coalesce((select max(cd_convenio) from	atend_categoria_convenio where	nr_seq_interno	= a.nr_seq_atecaco), obter_convenio_atendimento(a.nr_atendimento)))),1,100) nm_convenio,
	...
	(select	max(dt_atualizacao)
	 ...
	 to_char(a.cd_estabelecimento) cd_estabelecimento,
         ...
         substr((select obter_idade(dt_nascimento,coalesce(to_CHAR(i.dt_obito,'dd/mm/yyyy'), LOCALTIMESTAMP),'AM')
	...
	(select to_char(dt_nascimento,'DD/MM/YYYY') from pessoa_fisica
	 where cd_pessoa_fisica = a.cd_pessoa_fisica) dt_nascimento, 
         substr(obter_nome_convenio( coalesce(b.cd_convenio, coalesce((select max(cd_convenio) from	atend_categoria_convenio where	nr_seq_interno	= a.nr_seq_atecaco), obter_convenio_atendimento(a.nr_atendimento)))),1,100) ds_convenio,
	 ...
	 substr(obter_desc_tipo_convenio(coalesce(b.cd_convenio, coalesce((select max(cd_convenio) from	atend_categoria_convenio where	nr_seq_interno	= a.nr_seq_atecaco), obter_convenio_atendimento(a.nr_atendimento)))),1,100) ds_tipo_convenio,
	 obter_tipo_convenio(coalesce(b.cd_convenio, coalesce((select max(cd_convenio) from	atend_categoria_convenio where	nr_seq_interno	= a.nr_seq_atecaco), obter_convenio_atendimento(a.nr_atendimento)))) ie_tipo_convenio,
	 coalesce(b.cd_convenio, coalesce((select max(cd_convenio) from	atend_categoria_convenio where	nr_seq_interno	= a.nr_seq_atecaco), obter_convenio_atendimento(a.nr_atendimento))) cd_convenio,	 
	 ...
	 obter_valor_dominio(95,w.cd_tipo_procedimento) ds_tipo_procedimento,
