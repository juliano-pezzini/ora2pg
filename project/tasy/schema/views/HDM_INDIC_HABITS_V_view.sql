...
CREATE OR REPLACE VIEW hdm_indic_habits_v (nr_day_of_week, nr_day, si_holiday, si_weekend, dt_day_start, dt_day_end, ds_day_of_week, nr_year, nr_month, nr_semester, nr_quarter, dt_month_start, dt_month_end, nr_seq_patient_group, si_sex, ds_sex, nm_age_range, ds_participant, nr_seq_dm_program, nr_seq_program, nm_program, nm_program_module, nr_seq_campaign, nm_campaign, nr_seq_dm_group, nm_group, nm_group_class, nr_seq_turma, nr_seq_disease_risk, nm_disease_risk, nr_seq_habit, si_exists, si_habit, nm_habit, si_frequency, ds_frequency, nr_seq_fact, ds_unique, nr_dif_person, ds_habit_exists, nr_seq_day_start, nr_seq_day_end) AS SELECT	/* Dados dia */
	...
	a.nr_dif_person,
	CASE WHEN k.si_exists='S' THEN  wheb_mensagem_pck.get_texto(309708)  ELSE wheb_mensagem_pck.get_texto(309709) END  ds_habit_exists,
	a.nr_seq_day_start,
	a.nr_seq_day_end
FROM hdm_indic_ft_habit_rd o, hdm_indic_dm_risk_disease n, hdm_indic_dm_habit k, hdm_indic_ft_habit_ag j, hdm_indic_dm_activ_group i, hdm_indic_ft_habit_cp h, hdm_indic_dm_campaign g, hdm_indic_ft_habit_pr f, hdm_indic_dm_program e, hdm_indic_dm_patient_group d, hdm_indic_dm_day c, hdm_indic_dm_month b, hdm_indic_ft_habit a
...
, pessoa_fisica pf
LEFT OUTER JOIN hdm_indic_ft_habit a ON (pf.cd_pessoa_fisica = a.nr_dif_person)
WHERE 1 = 1 AND f.nr_seq_dimension = e.nr_sequencia AND h.nr_seq_dimension = g.nr_sequencia AND j.nr_seq_dimension = i.nr_sequencia AND o.nr_seq_dimension = n.nr_sequencia AND a.nr_sequencia = f.nr_seq_fact AND a.nr_sequencia = h.nr_seq_fact AND a.nr_sequencia = j.nr_seq_fact AND a.nr_sequencia = o.nr_seq_fact AND a.nr_seq_habit = k.nr_sequencia AND a.nr_seq_patient_group = d.nr_sequencia AND b.nr_sequencia = a.nr_seq_month_start  AND c.nr_sequencia = a.nr_seq_day_start;
