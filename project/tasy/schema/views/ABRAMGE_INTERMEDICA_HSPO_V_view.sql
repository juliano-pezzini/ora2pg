...
SELECT		'CABECALHO CONTA      '	DS_REGISTRO,
		...
		substr(B.CD_USUARIO_CONVENIO,1,23)
						CD_PACIENTE,
...
WHERE C.NR_INTERNO_CONTA = W.NR_INTERNO_CONTA  and ((coalesce(Z.ie_entra_conta,'S') = 'S') or (C.NR_SEQ_PROC_PACOTE = C.NR_SEQ_ITEM)) and C.ie_tipo_item	= 1 and C.cd_medico_executor is not null GROUP BY	C.NR_SEQ_PROTOCOLO,
           	C.NR_INTERNO_CONTA,
           	C.CD_ITEM_CONVENIO,
           	substr(upper(C.DS_ITEM),1,50),
           	CASE WHEN W.IE_CARATER_INTER='U' THEN 'S'  ELSE 'N' END ,
...
having		sum(C.qt_item) <> 0 /*and SUM(decode((nvl(c.vl_honorario,0) + nvl(c.vl_custo_oper,0) + nvl(c.vl_filme,0)),0,c.vl_total_item,
		(nvl(c.vl_honorario,0) + nvl(c.vl_custo_oper,0) + nvl(c.vl_filme,0)))) > 0*/
union all
