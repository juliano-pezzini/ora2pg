...
CREATE OR REPLACE VIEW han_ipe_v (tp_registro, nr_seq_protocolo, nm_sistema, cd_cgc_hospital, qt_total_conta, qt_total_lancamento, cd_interno, nm_hospital, ds_espaco, ie_zeros, tp_nota, nr_folha, cd_usuario_convenio, cd_cid, cd_prestador, tp_prestador, cd_motivo_cobranca, dt_entrada, dt_alta, nr_guia, nr_interno_conta, vl_total_conta, nr_folha_ini_despesa, nr_folha_fim_despesa, vl_total_despesa, nr_folha_ini_servico, nr_folha_fim_servico, vl_total_servico, vl_total_honorario, nr_linha, dt_item, cd_item_convenio, qt_dias_executado, qt_ocorrencia_dia, cd_funcao_executor, ie_via_acesso, ie_urgencia, vl_honorario, vl_total_item, vl_total_matmed, cd_brasindice, cd_unidade_medida) AS Select
	...
	(coalesce(CASE WHEN a.cd_interno IS NULL THEN null  ELSE ('03'||a.cd_interno) END ,0))::numeric 	cd_interno,
...
group by
	...
	a.cd_cgc_hospital,
	b.qt_total_conta,
	(coalesce(CASE WHEN a.cd_interno IS NULL THEN null  ELSE ('03'||a.cd_interno) END ,0))::numeric ,
...
select
	...
	substr(coalesce(Obter_Guia_Conta(a.nr_interno_conta),coalesce(a.cd_senha_guia,a.nr_doc_convenio)),1,6)	nr_guia,
	...
	(sum(CASE WHEN c.tp_nota=76 THEN CASE WHEN c.ie_responsavel_credito='PC' THEN (qt_item * vl_unitario_item) WHEN c.ie_responsavel_credito='M' THEN (qt_item * vl_unitario_item)  ELSE c.vl_total_item END  WHEN c.tp_nota=86 THEN CASE WHEN c.ie_responsavel_credito='PC' THEN (qt_item * vl_unitario_item) WHEN c.ie_responsavel_credito='M' THEN (qt_item * vl_unitario_item)  ELSE c.vl_total_item END   ELSE 0 END ) +
	sum(CASE WHEN c.tp_nota=77 THEN CASE WHEN c.vl_total_item=0 THEN c.vl_honorario  ELSE c.vl_total_item END 			     WHEN c.tp_nota=87 THEN c.vl_total_item  ELSE 0 END ) +
	...
	sum(CASE WHEN c.tp_nota=76 THEN CASE WHEN c.ie_responsavel_credito='PC' THEN (qt_item * vl_unitario_item) WHEN c.ie_responsavel_credito='M' THEN (qt_item * vl_unitario_item)  ELSE c.vl_total_item END  WHEN c.tp_nota=86 THEN CASE WHEN c.ie_responsavel_credito='PC' THEN (qt_item * vl_unitario_item) WHEN c.ie_responsavel_credito='M' THEN (qt_item * vl_unitario_item)  ELSE c.vl_total_item END   ELSE 0 END ) vl_total_despesa,
	...
	sum(CASE WHEN c.tp_nota=77 THEN CASE WHEN c.vl_total_item=0 THEN c.vl_honorario  ELSE c.vl_total_item END 			     WHEN c.tp_nota=87 THEN c.vl_total_item  ELSE 0 END ) vl_total_servico,
...
group by 
	...
	a.dt_periodo_final,
	substr(coalesce(Obter_Guia_Conta(a.nr_interno_conta),coalesce(a.cd_senha_guia,a.nr_doc_convenio)),1,6),
	coalesce((substr(lpad(a.nr_seq_conta_convenio,10,'0'),6,5))::numeric ,
...
select
	...
	0				ie_urgencia,
	0				vl_honorario,
	sum(CASE WHEN c.ie_responsavel_credito='PC' THEN (qt_item * vl_unitario_item) WHEN c.ie_responsavel_credito='M' THEN (qt_item * vl_unitario_item)  ELSE c.vl_total_item END )
					vl_total_item,
...
select
	...
	CASE WHEN c.ie_video='S' THEN 15  ELSE c.ie_urgencia END  ie_urgencia,
	0				vl_honorario,
	CASE WHEN c.ie_responsavel_credito='M' THEN (qt_item * vl_unitario_item) WHEN c.ie_responsavel_credito='PC' THEN (qt_item * vl_unitario_item)  ELSE c.vl_total_item END  vl_total_item,
	0				vl_total_matmed,
...
select
	...
	sum(CASE WHEN c.vl_total_item=0 THEN c.vl_honorario  ELSE c.vl_total_item END )
					vl_total_item,
	sum(CASE WHEN c.cd_item_convenio=8001 THEN 		CASE WHEN c.vl_total_item=0 THEN c.vl_honorario  ELSE c.vl_total_item END  WHEN c.cd_item_convenio=32200005 THEN CASE WHEN c.vl_total_item=0 THEN c.vl_honorario  ELSE c.vl_total_item END   ELSE 0 END )
					vl_total_matmed,
	' '				cd_brasindice,
	' '				cd_unidade_medida
from	w_interf_conta_item_ipe c,
