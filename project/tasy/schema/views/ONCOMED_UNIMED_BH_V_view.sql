...
SELECT   	a.NR_SEQ_PROTOCOLO,a.CD_CONVENIO CD_CONVENIO,1 TP_REGISTRO, 
		...
		CASE WHEN a.ie_medico_credenciado='S' THEN a.cd_conv_medico_resp  ELSE 0 END 	CD_MEDICO_REQUISITANTE, 
      	...
      	coalesce(d.nr_doc_convenio,c.cd_autorizacao)       NR_GUIA, 
		...
		0 ie_clinica, 
		substr(obter_convenio_origem(a.cd_convenio,a.nr_atendimento,'D'),1,60) ds_convenio_origem, 
		0 nr_seq_indicacao, 
		0 nr_seq_proc_princ 
...
SELECT   	a.NR_SEQ_PROTOCOLO,a.CD_CONVENIO CD_CONVENIO,2 TP_REGISTRO, 
 ...
 --     	decode(a.ie_medico_credenciado,'N',a.nr_crm_medico_resp,0) NR_CRM, 
		...
		''					CD_ITEM_CONVENIO, 
		0					CD_PROCEDIMENTO_REF, 
		substr(OBTER_DADOS_CATEG_CONV(a.nr_atendimento,'S'),1,11) CD_SENHA, 
		''					cd_regional, 
		''					nm_convenio, 
		' ' 					ds_espaco, 
		a.ie_estado_civil 		ie_estado_civil, 
...
WHERE a.nr_interno_conta	= d.nr_interno_conta and c.nr_sequencia		= 
		...
		substr(OBTER_DADOS_CATEG_CONV(a.nr_atendimento,'S'),1,11), 
		a.ie_estado_civil, 
		a.nr_cpf_paciente, 
		a.cd_nacionalidade, 
      	a.cd_cid_secundario, 
...
SELECT	a.NR_SEQ_PROTOCOLO,a.CD_CONVENIO CD_CONVENIO,4 TP_REGISTRO, 
		...
		CASE WHEN a.ie_responsavel_credito='FM' THEN '00615006002'  ELSE coalesce(a.CD_INTERNO,0) END  CD_PRESTADOR, 
      	0               NR_SEQ_CONTA, 
      	''              IE_TIPO_PRESTADOR, 
      	a.cd_cgc_hospital       CD_CGC, 
      	''              NM_PRESTADOR, 
		...
		0				NR_SEQ_CONTA_ITEM, 
      	...
      	max(trunc(a.DT_ITEM, 'mi'))	       	DT_ITEM, 
      	''              	CD_CID, 
      	''              	NR_CRM, 
      	''              	SG_UF_CRM, 
		...
		0					CD_MOTIVO_ALTA, 
		''					TP_GUIA, 
		substr(CASE WHEN coalesce(a.cd_grupo_gasto,'0')='1' THEN a.cd_medico_exec_conv WHEN coalesce(a.cd_grupo_gasto,'0')='3' THEN  		a.cd_medico_exec_conv  ELSE '' END ,-10,9)||calcula_digito_unimed_bh('Medico_Unimed',substr( 
		CASE WHEN coalesce(a.cd_grupo_gasto,'0')='1' THEN a.cd_medico_exec_conv WHEN coalesce(a.cd_grupo_gasto,'0')='3' THEN a.cd_medico_exec_conv  ELSE '' END ,-6,5)) CD_MEDICO_CONVENIO, 
	...
	-- Alterado Wander 06/07/2004	 
		...
		round((sum(a.QT_ITEM) * round((dividir_sem_round(sum(a.vl_total_item),sum(a.QT_ITEM)))::numeric,4))::numeric, 2) vl_item, 
...
and	coalesce(a.cd_grupo_proc,0) not in (32100000,32120001,94010001) 
...
group by 
		...
		a.CD_CONVENIO, 
		CASE WHEN a.ie_responsavel_credito='FM' THEN '00615006002'  ELSE coalesce(a.CD_INTERNO,0) END , 
      	a.cd_cgc_hospital, 
      	a.nr_remessa, 
      	a.nr_interno_conta, 
      	a.ds_item, 
...
/*   Alterado por marcus em 08/03/2002 a pedido do Wander - Se Material se valor zero no enviar */
...
and	a.cd_grupo_proc in (32100000,32120001,94010001) 
group by 
		a.nr_sequencia, 
      	coalesce(a.nr_doc_convenio,c.nr_doc_convenio), 
		a.NR_SEQ_PROTOCOLO, 
...
and	a.cd_grupo_proc not in (32100000,32120001,94010001) 
group by 
		a.NR_SEQ_PROTOCOLO, 
      	coalesce(a.nr_doc_convenio,c.nr_doc_convenio), 
		a.CD_CONVENIO, 
...
SELECT   	a.NR_SEQ_PROTOCOLO,a.CD_CONVENIO CD_CONVENIO,5 TP_REGISTRO, 
		...
		0					VL_UNITARIO_CH, 
		0					VL_UNITARIO, 
--		sum(b.vl_total_item) + nvl(sum(decode(b.cd_grupo_proc, 32100000, nvl(b.VL_CUSTO_OPER,0), 32120001, nvl(b.VL_CUSTO_OPER,0), 
--							94010001, nvl(b.VL_CUSTO_OPER,0), decode(b.ie_responsavel_credito, 'M', 
--							decode(b.vl_total_item, 0, 0, decode(nvl(b.vl_honorario,0), 0, 0, decode(nvl(b.vl_honorario,0), b.vl_total_item, nvl(b.vl_custo_oper,0),0)))))),0) vl_item, 
		...
		sum(CASE WHEN b.cd_grupo_gasto='3' THEN  				CASE WHEN b.cd_grupo_proc=32100000 THEN b.vl_honorario WHEN b.cd_grupo_proc=32120001 THEN b.vl_honorario  ELSE b.vl_total_item END  WHEN b.cd_grupo_gasto='1' THEN b.vl_total_item  ELSE 0 END ) vl_tot_honorarios, 
		...
		sum(CASE WHEN b.cd_grupo_gasto='8' THEN CASE WHEN b.cd_grupo_proc=94010001 THEN  b.vl_total_item  ELSE 0 END   ELSE 0 END ) vl_tot_pacote, 
		0 vl_tot_ortese, 
		0 vl_tot_protese, 
