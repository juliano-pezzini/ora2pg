...
CREATE OR REPLACE VIEW titulo_receber_bloq_vc_v (nr_titulo, cd_estabelecimento, dt_atualizacao, nm_usuario, dt_emissao, dt_vencimento, dt_pagamento_previsto, vl_titulo, vl_saldo_titulo, vl_saldo_juros, vl_saldo_multa, cd_moeda, cd_portador, cd_tipo_portador, tx_juros, tx_multa, cd_tipo_taxa_juro, cd_tipo_taxa_multa, tx_desc_antecipacao, ie_situacao, ie_tipo_emissao_titulo, ie_origem_titulo, ie_tipo_titulo, ie_tipo_inclusao, cd_pessoa_fisica, nr_interno_conta, cd_cgc, cd_serie, nr_documento, nr_sequencia_doc, cd_banco, cd_agencia_bancaria, nr_bloqueto, dt_liquidacao, nr_lote_contabil, ds_observacao_titulo, nr_atendimento, nr_seq_protocolo, dt_contabil, nr_seq_conta_banco, dt_emissao_bloqueto, nr_seq_classe, nr_guia, nr_nosso_numero, dt_ref_conta, cd_convenio_conta, nr_seq_terc_conta, nr_seq_nf_saida, nr_processo_aih, cd_tipo_recebimento, nr_seq_controle_pessoa, nr_seq_trans_fin_contab, nr_seq_contrato, vl_desc_previsto, nr_seq_carteira_cobr, dt_integracao_externa, cd_estab_financeiro, nr_titulo_externo, nr_seq_mensalidade, nr_seq_mens_segurado, ie_pls, nr_seq_trans_fin_baixa, nr_nota_fiscal, nm_usuario_orig, dt_inclusao, ie_tipo_carteira, nr_seq_empresa, nr_seq_proj_rec, nr_seq_lote_empresa, nr_seq_negociacao_origem, nr_seq_pls_lote_camara, nr_seq_ptu_fatura, dt_ultima_devolucao, nr_seq_lote_enc_contas, ie_liq_inadimplencia, ie_negociado, nr_seq_guia, nr_titulo_dest, nr_seq_pag_conta_evento, vl_outras_despesas, ie_dig_nosso_numero, nr_seq_segurado_mens, nr_seq_eme_fatura, nr_seq_pls_lote_contest, nr_seq_pls_lote_disc, nr_seq_grupo_prod, nr_seq_cob_previa, nr_seq_pagador, dt_atual, ds_conta, dt_vencimento_atual, cd_moeda_banco, cd_banco_dig, nr_bloqueto_lido, nr_bloqueto_editado, cd_banco_estab, ds_banco, cd_conta, ie_digito_conta, cd_agencia_conta, ds_end_sacado, ds_endereco_sacado, ds_sacado, ds_nome_sacado, ds_cidade_sacado, ds_instrucao, cd_carteira, cd_agencia_dig_conta, cd_pessoa, banco) AS Select	distinct
	...
	h.cd_conta,
	h.ie_digito_conta,
	h.cd_agencia_bancaria || '/'|| h.cd_conta || '-' || h.ie_digito_conta  cd_agencia_conta,
	...
	CASE WHEN a.cd_cgc='' THEN  g.cd_cep || ' - ' || g.ds_municipio || ' - ' || g.sg_estado  ELSE c.cd_cep ||		' - ' || c.ds_municipio || ' - ' || c.sg_estado END  ds_cidade_sacado,
...
WHERE a.cd_moeda 		= d.cd_moeda and h.cd_banco		= e.cd_banco and h.cd_agencia_bancaria	= f.cd_agencia_bancaria;
