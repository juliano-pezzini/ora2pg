...
SELECT     	'ITEM DA CONTA         ' 	DS_REGISTRO,
		...
		' '				UF_CRM_REQ,
           	...
           	CASE WHEN c.pr_hora_extra=0 THEN 'N' WHEN c.pr_hora_extra=1 THEN 'N' WHEN c.pr_hora_extra IS NULL THEN 'N'  ELSE 'S' END 
                       				IE_EXTRA,
	        trunc(w.dt_periodo_inicial,'dd')    DT_ITEM,  /* Alterado por Wander em 11/10/2004. */
          --  	TRUNC(W.DT_ENTRADA,'dd')  		DT_ITEM,
           	c.CD_FUNCAO_EXECUTOR         	IE_FUNCAO_MEDICO,
           	SUM(c.QT_ITEM)               	QT_ITEM,
           	SUM(CASE WHEN coalesce(Z.cd_regra,'X')='M' THEN 0  ELSE coalesce(c.vl_honorario,0) + coalesce(c.vl_custo_oper,0) END )
							VL_ITEM,
           	0                        	IE_CIRURGIA_MULTIPLA,
           	0		      		VL_TOTAL_ITEM,
           	0	      			QT_ITEM_CONTA,
...
SELECT     	'ITEM DA CONTA         ' 	DS_REGISTRO,
		...
		coalesce(somente_numero(c.nr_doc_convenio),0)
						nr_guia,
		w.cd_senha_guia		cd_senha_guia,
		e.cd_unidade_medida_consumo,
		e.qt_conv_estoque_consumo
FROM w_interf_conta_header x, w_interf_conta_cab w, w_interf_conta_item c
LEFT OUTER JOIN material e ON (c.cd_item = e.cd_material)
LEFT OUTER JOIN regra_honorario z ON (c.ie_responsavel_credito = Z.cd_regra)
WHERE c.NR_INTERNO_CONTA 	= W.NR_INTERNO_CONTA and w.nr_seq_protocolo 	= x.nr_seq_protocolo          --  Alterado por Wander
     --  Alterado por Wander.
  and coalesce(Z.ie_entra_conta,'S') 	= 'S' and (c.ie_tipo_item		= 2 or (c.ie_tipo_item		= 1 and
		c.cd_item not in (select x.cd_procedimento
		from estrutura_procedimento_v x
		where x.cd_area_procedimento in (1,4,11,13)))) and c.vl_total_item	<> 0 and c.qt_item		<> 0 GROUP BY	somente_numero(coalesce(x.cd_interno,0)),
		c.NR_SEQ_PROTOCOLO,
           	c.NR_INTERNO_CONTA,
           	c.CD_ITEM_CONVENIO,
           	substr(ELIMINA_ACENTUACAO(upper(c.DS_ITEM)),1,50),
...
SELECT     	'TOTAL DA CONTA        ' 	DS_REGISTRO,
		...
		B.UF_CRM_MEDICO_RESP		UF_CRM_REQ,
           	...
           	0				VL_ITEM,
           	0				IE_CIRURGIA_MULTIPLA,
           	sum(CASE WHEN x.cd_area_procedimento=1 THEN (CASE WHEN coalesce(Z.cd_regra,'X')='M' THEN 0  ELSE coalesce(e.vl_honorario,0) + coalesce(e.vl_custo_oper,0) END ) WHEN x.cd_area_procedimento=4 THEN (CASE WHEN coalesce(Z.cd_regra,'X')='M' THEN 0  ELSE coalesce(e.vl_honorario,0) + coalesce(e.vl_custo_oper,0) END ) WHEN x.cd_area_procedimento=11 THEN (CASE WHEN coalesce(Z.cd_regra,'X')='M' THEN 0  ELSE coalesce(e.vl_honorario,0) + coalesce(e.vl_custo_oper,0) END ) WHEN x.cd_area_procedimento=13 THEN (CASE WHEN coalesce(Z.cd_regra,'X')='M' THEN 0  ELSE coalesce(e.vl_honorario,0) + coalesce(e.vl_custo_oper,0) END )  ELSE e.vl_total_item END ) VL_TOTAL_ITEM,
           	0			QT_ITEM_CONTA,
