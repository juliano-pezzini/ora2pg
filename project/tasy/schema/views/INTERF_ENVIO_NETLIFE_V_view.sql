...
CREATE OR REPLACE VIEW interf_envio_netlife_v (ds_registro, tp_registro, ie_servico, cd_registro, cd_origem, nr_lote, nr_interno_conta, nr_parcela, cd_convenio, nm_convenio, tp_prestador, cd_prestador, nm_prestador, dt_geracao, nr_remessa, cd_plano, cd_paciente, cd_autorizacao, cd_acomodacao, dt_entrada, dt_saida, dt_inicio_cobranca, dt_fim_cobranca, ie_tipo_atendimento, cd_cid, cd_proc_principal, cd_medico_convenio, nr_crm, uf_crm, nr_crm_req, uf_crm_req, nr_seq_item, cd_item, nm_item, ie_emergencia, ie_extra, dt_item, ie_funcao_medico, qt_item, vl_item, ie_cirurgia_multipla, vl_total_item, qt_item_conta, nm_paciente, nr_fatura, qt_conta, vl_total_lote, nr_seq_protocolo, ds_espaco, nr_registro, ie_matmed, ds_ma_me_tx, ie_tipo_atend_esp, ie_tipo_atend_tasy, cd_motivo_alta, ie_clinica, ie_responsavel_credito) AS SELECT		'HEADER        ' 			DS_REGISTRO,
		1				TP_REGISTRO, 
		'CSMH'				IE_SERVICO, 
		0				CD_REGISTRO, 
...
SELECT   	'ITEM DA CONTA     ' 	DS_REGISTRO, 
      	...
      	' '            	CD_PROC_PRINCIPAL, 
		' '				CD_MEDICO_CONVENIO, 
      	C.nr_crm_executor            	NR_CRM, 
      	C.uf_crm_executor            	UF_CRM, 
		' '				NR_CRM_REQ, 
		' '				UF_CRM_REQ, 
...
and		coalesce(C.cd_cgc_prestador,'0')	= '03601925000186' 
...
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA   and coalesce(e.cd_cgc_prestador,'0')	= '03601925000186' GROUP BY	B.NR_SEQ_PROTOCOLO, 
      		B.NR_INTERNO_CONTA, 
		substr(ELIMINA_ACENTUACAO(UPPER(B.NM_PACIENTE)),1,40), 
...
LEFT OUTER JOIN estrutura_procedimento_v x ON (E.CD_ITEM = X.CD_PROCEDIMENTO AND E.IE_ORIGEM_PROCED = X.IE_ORIGEM_PROCED)
WHERE B.NR_INTERNO_CONTA 	= E.NR_INTERNO_CONTA and B.nr_seq_protocolo		= f.nr_seq_protocolo   and coalesce(e.cd_cgc_prestador,'0')	= '03601925000186' GROUP BY	(coalesce(f.CD_INTERNO,0))::numeric , 
		B.NR_SEQ_PROTOCOLO;
