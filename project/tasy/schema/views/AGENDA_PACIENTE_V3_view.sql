...
CREATE OR REPLACE VIEW agenda_paciente_v3 (cd_estabelecimento, cd_agenda, dt_agenda_dia, ds_agenda, dt_agenda, hr_inicio, nr_minuto_duracao, cd_medico, cd_procedimento, nm_pessoa_contato, ds_observacao, cd_convenio, cd_plano, nr_cirurgia, nm_usuario, dt_atualizacao, ie_origem_proced, ie_ortese_protese, ie_cdi, ie_uti, ie_leito, ie_anestesia, ie_banco_sangue, ie_serv_especial, ie_video, cd_motivo_cancelamento, cd_pessoa_fisica, nm_paciente_pf, nm_paciente, dt_nascimento, nm_medico, nm_guerra, ds_convenio, ds_procedimento, nm_instrumentador, nm_circulante, ds_proc_cirurgia, cd_tipo_agenda, cd_setor_exclusivo, ds_cirurgia, qt_idade_paciente, cd_tipo_anestesia, ds_tipo_anestesia, ie_status_agenda, ds_status_agenda, ds_tipo_anestesia_3, ie_turno, cd_turno, nr_sequencia, ds_senha, cd_pediatra, cd_anestesista, ds_proc_conv, nr_telefone_pac, nr_atendimento, ds_idade, ds_idade_abrev, qt_anos, ds_turno, cd_unidade, nm_pediatra, nm_anestesista, ie_carater_cirurgia, ds_carater_cirurgia, nm_guerra_anest, nr_seq_apresent, cd_usuario_convenio, nm_usuario_orig, qt_idade_mes, nr_telefone, dt_agendamento, ie_autorizacao, ie_sexo, nr_telefone_celular, ds_idade_grid, nr_controle, ie_status_laudo, ie_equipamento, cd_autorizacao, nr_seq_age_cons, cd_medico_exec, nm_medico_exec, ds_proced_adic, nr_seq_classif_agenda, ie_uc, ie_situacao, cd_procedencia, cd_categoria, cd_tipo_acomodacao, nr_doc_convenio, dt_validade_carteira, ds_tipo_autorizacao, dt_confirmacao, nm_usuario_confirm, ie_lado, nr_seq_proc_interno, ie_tipo_classif, ds_status_paciente, ds_cor_fundo_status, ds_cor_fonte_status, ie_biopsia, ie_congelacao, ds_laboratorio, qt_min_padrao, dt_nascimento_pac, cd_doenca_cid, ds_doenca_cid, nr_seq_sala, ds_sala, nm_medico_externo, nr_seq_status_pac, dt_entrada_tasy, ie_tipo_atendimento, ie_consulta_anestesica, ie_pre_internacao, ie_reserva_leito, ie_tipo_anestesia, dt_chegada, cd_medico_req, qt_min_espera_tasy, qt_min_espera, nr_seq_pq_proc, ds_proc_pesquisa, ds_classif_agenda_pac, dt_chegada_fim, ie_arco_c, ds_cor_classif, nr_seq_indicacao, ds_indicacao, cd_pessoa_indicacao, nm_pessoa_indicacao, ds_categoria, ds_dia_semana, ie_encaixe) AS select 	a.cd_estabelecimento,
	  a.cd_agenda, 
		...
		b.dt_atualizacao, 
		b.ie_origem_proced, 
		obter_se_possui_opme(b.nr_sequencia) ie_ortese_protese, 
		b.ie_cdi, 
		b.ie_uti, 
		b.ie_leito, 
		b.ie_anestesia, 
    ...
    b.cd_pessoa_fisica, 
		substr(CASE WHEN b.IE_STATUS_AGENDA='B' THEN  		CASE WHEN b.ds_observacao IS NULL THEN  'Bloqueado'  ELSE 'Bloqueado('|| b.ds_observacao || ')' END   ELSE coalesce(p.nm_pessoa_fisica,b.nm_paciente) END ,1,200) nm_paciente_pf, 
    b.nm_paciente, 
    p.dt_nascimento, 
    ...
    substr(OBTER_NOME_MEDICO(b.cd_medico, 'G'),1,200) nm_guerra, 
		...
		b.nm_instrumentador, 
    	b.nm_circulante, 
    	substr(coalesce(b.ds_cirurgia, 
	  coalesce(substr(obter_Nome_Item_Convenio(b.cd_convenio, 1, 
    	...
    	CASE WHEN b.cd_tipo_anestesia IS NULL THEN  		ie_anestesia  ELSE Substr(obter_valor_dominio(36, b.cd_tipo_anestesia),1,15) END  ds_Tipo_Anestesia_3, 
		...
		b.cd_pediatra, 
		b.cd_anestesista, 
		CASE WHEN b.nr_seq_proc_interno IS NULL THEN  		substr(obter_Nome_Item_Convenio(b.cd_convenio, 1, 		b.cd_procedimento, b.ie_origem_proced,'N', null, null, null, null),1,80)  ELSE substr(Obter_Desc_Prescr_Proc(CD_PROCEDIMENTO,IE_ORIGEM_PROCED, nr_seq_proc_interno),1,240) END  ds_proc_conv, 
		...
		CASE WHEN b.cd_turno=0 THEN  'Manhã'  ELSE 'Tarde' END  ds_turno, 
		...
		CASE WHEN coalesce(b.qt_idade_paciente,0) + coalesce(b.qt_idade_mes,0)=0 THEN  		substr(obter_idade(p.dt_nascimento, LOCALTIMESTAMP, 'S'),1,20)  ELSE CASE WHEN coalesce(b.qt_idade_paciente,0)=0 THEN ''  ELSE coalesce(b.qt_idade_paciente,0) || ' A ' END  || 		CASE WHEN coalesce(b.qt_idade_mes,0)=0 THEN ''  ELSE coalesce(b.qt_idade_mes,0) || ' M' END  END  ds_idade_Grid, 
		obter_controle_prescricao(nr_atendimento, b.nr_sequencia) nr_controle, 
		substr(obter_status_laudo_agenda(b.nr_sequencia, b.nr_atendimento),1,100) ie_status_laudo, 
		ie_equipamento, 
