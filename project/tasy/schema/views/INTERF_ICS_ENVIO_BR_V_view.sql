...
select 
    ...
    CASE WHEN obter_dados_medico(c.cd_medico_executor,'SGCRM')='CRP' THEN 057  ELSE 058 END  
						cd_especialidade, 
    2                    cd_local_atend, 
    ' '                    cd_cid, 
    ' '                    ie_conselho, 
...
select 
    ...
    0                    cd_local_atend, 
    ' '                    cd_cid, 
    substr(CASE WHEN obter_dados_medico(c.cd_medico_executor,'SGCRM')='CRM' THEN 1  ELSE 6 END ,1,1)   
						ie_conselho, 
    somente_numero(substr(coalesce(c.nr_crm_executor,'0'),1,5)) 
                        nr_conselho,   
	substr(CASE WHEN obter_dados_medico(c.cd_medico_executor,'SGCRM')='CRP' THEN '08/'  ELSE c.uf_crm_executor END ,1,5) 
						ie_conselho_regional, 
    substr(c.nm_medico_executor,1,27)    nm_profissional, 
