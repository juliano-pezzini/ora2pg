...
select
	...
	3				tp_prestador,
	case
		when CASE WHEN a.ie_tipo_atendimento=1 THEN 75 WHEN a.ie_tipo_atendimento=8 THEN 85  ELSE 0 END  = 85 then 0
	...
	(sum(CASE WHEN c.tp_nota=76 THEN c.vl_total_item WHEN c.tp_nota=86 THEN c.vl_total_item  ELSE 0 END ) +
	...
	sum(CASE WHEN c.tp_nota=76 THEN c.vl_total_item WHEN c.tp_nota=86 THEN c.vl_total_item  ELSE 0 END ) vl_total_despesa,
...
select
	...
	a.cd_cgc_hospital		cd_cgc_hospital,
	0				qt_total_conta,
	(max(CASE WHEN c.tp_nota=76 THEN c.nr_linha WHEN c.tp_nota=86 THEN c.nr_linha END ) - coalesce(scm_buscar_qtd_taxas(c.nr_interno_conta, 'C', c.nr_folha),0)) qt_total_lancamento,
	0				cd_interno,
	' '				nm_hospital,
	' '				ds_espaco,
	0 				ie_zeros,
	...
	sum(CASE WHEN c.cd_brasindice IS NULL THEN  0  ELSE CASE WHEN to_char(c.cd_item_convenio)=to_char(c.cd_item) THEN  c.vl_total_item  ELSE 0 END  END )
					vl_total_matmed,
	' '				cd_brasindice,
	' '				cd_unidade_medida
from	w_interf_conta_item_ipe c,
...
select
	...
	' '				cd_usuario_convenio,
	' '				cd_cid,
	CASE WHEN c.ie_responsavel_credito='RM' THEN c.cd_cgc_hospital  ELSE c.ie_prestador END  cd_prestador,
	3				tp_prestador,
	0 				cd_motivo_cobranca,
	LOCALTIMESTAMP				dt_entrada,
	LOCALTIMESTAMP				dt_alta,
