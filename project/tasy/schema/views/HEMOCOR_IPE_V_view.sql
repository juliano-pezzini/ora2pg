...
select
	...
	sum(CASE WHEN c.tp_nota=75 THEN CASE WHEN c.vl_total_item=0 THEN c.vl_honorario  ELSE c.vl_total_item END  WHEN c.tp_nota=85 THEN CASE WHEN c.vl_total_item=0 THEN c.vl_honorario  ELSE c.vl_honorario END   ELSE 0 END )) vl_total_conta,
	('0'||min(CASE WHEN c.tp_nota=76 THEN c.nr_folha WHEN c.tp_nota=86 THEN c.nr_folha END ))::numeric 	nr_folha_ini_despesa,
	...
	sum(CASE WHEN c.tp_nota=75 THEN CASE WHEN c.ie_responsavel_credito='H' THEN c.vl_honorario  ELSE c.vl_total_item END  WHEN c.tp_nota=85 THEN CASE WHEN c.ie_responsavel_credito='H' THEN c.vl_honorario  ELSE c.vl_total_item END   ELSE 0 END ) vl_total_honorario,
	0				nr_linha,
	LOCALTIMESTAMP				dt_item,
	0				cd_item_convenio,
	0				qt_dias_executado,
...
select
	...
	CASE WHEN c.ie_responsavel_credito='H' THEN c.vl_honorario  ELSE c.vl_total_item END 
					vl_honorario,
...
select
	...
	sum(CASE WHEN c.CD_BRASINDICE IS NOT NULL THEN c.vl_total_item ELSE 0 END) 
					vl_total_matmed,
	' '				cd_brasindice,
	' '				cd_unidade_medida
from	w_interf_conta_item_ipe c,
