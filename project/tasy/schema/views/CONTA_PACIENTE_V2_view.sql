...
, 
	...
	A.ds_observacao ds_obs_item, 
	0 cd_local_estoque, 
	0 nr_seq_lote_fornec, 
	A.nr_seq_conta_origem, 
	A.nr_seq_ajuste_proc	nr_seq_regra_ajuste 
FROM area_procedimento f, especialidade_proc e, grupo_proc d, procedimento c, procedimento_paciente a
LEFT OUTER JOIN conta_paciente m ON (A.NR_INTERNO_CONTA = M.NR_INTERNO_CONTA)
, (a
LEFT OUTER JOIN proc_paciente_valor g ON (A.NR_SEQUENCIA = G.NR_SEQ_PROCEDIMENTO AND 1 = G.IE_TIPO_VALOR)
WHERE (A.CD_PROCEDIMENTO    = C.CD_PROCEDIMENTO) AND (A.IE_ORIGEM_PROCED   = C.IE_ORIGEM_PROCED) AND (C.CD_GRUPO_PROC     = D.CD_GRUPO_PROC) AND (D.CD_ESPECIALIDADE   = E.CD_ESPECIALIDADE) AND (E.CD_AREA_PROCEDIMENTO = F.CD_AREA_PROCEDIMENTO)    
UNION
...
, 
	...
	x.ds_observacao ds_obs_item, 
	x.cd_local_estoque, 
	x.nr_seq_lote_fornec, 
	x.nr_seq_conta_origem, 
	x.NR_SEQ_REGRA_AJUSTE_MAT	nr_seq_regra_ajuste 
FROM material w, material_atend_paciente x
LEFT OUTER JOIN conta_paciente m ON (X.NR_INTERNO_CONTA = M.NR_INTERNO_CONTA)
