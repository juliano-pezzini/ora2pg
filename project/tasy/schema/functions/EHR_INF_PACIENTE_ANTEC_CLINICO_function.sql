...
CREATE OR REPLACE FUNCTION ehr_inf_paciente_antec_clinico ( nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, nr_atendimento_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') RETURNS varchar AS $body$
DECLARE
...
and	nr_sequencia  in (	SELECT	max(x.nr_sequencia)
				...
				and	coalesce(x.IE_PACIENTE,'S')	= 'S'
...
ds_comando_w	:= 'Select '||ds_comando_w ||' ds '||chr(13) ||'From paciente_antec_clinico ';
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ehr_inf_paciente_antec_clinico ( nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, nr_atendimento_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') FROM PUBLIC;
