...
CREATE OR REPLACE FUNCTION obter_alerta_atend_lote ( nr_prescricao_p bigint, cd_material_p bigint) RETURNS varchar AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	where	ie_situacao	= 'A'
	and 	coalesce(ie_atend_lote_prescr,'N') = 'S'
...
select	max(a.cd_material),
		coalesce(max(a.ie_via_aplicacao),'0'),
	coalesce(max(a.cd_unidade_medida_dose),'0'),
	coalesce(max(b.cd_setor_atendimento),0),
	coalesce(max(obter_idade_pf(b.cd_pessoa_fisica,clock_timestamp(),'A')),0),
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_alerta_atend_lote ( nr_prescricao_p bigint, cd_material_p bigint) FROM PUBLIC;
