...
CREATE OR REPLACE FUNCTION obter_status_diag_opcao ( cd_pessoa_fisica_p text, cd_doenca_p text, ie_opcao_p text) RETURNS varchar AS $body$
DECLARE
...
if (dt_status_w < dt_diagnostico_w) then 
	ds_status_w	:= null;
	ie_status_w := null;
	dt_status_w := null;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_status_diag_opcao ( cd_pessoa_fisica_p text, cd_doenca_p text, ie_opcao_p text) FROM PUBLIC;
