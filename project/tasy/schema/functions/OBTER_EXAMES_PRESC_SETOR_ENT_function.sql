...
CREATE OR REPLACE FUNCTION obter_exames_presc_setor_ent ( nr_prescricao_p bigint, cd_setor_atendimento_p bigint, ie_opcao_p text, ie_quebra_linha_p text) RETURNS varchar AS $body$
DECLARE
...
c01 CURSOR FOR 
	...
	AND 	((a.cd_setor_atendimento = cd_setor_atendimento_p) OR (c.cd_setor_atendimento = cd_setor_atendimento_p) or (coalesce(d.cd_setor_entrega,d.cd_setor_atendimento) = cd_setor_atendimento_p)) 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_exames_presc_setor_ent ( nr_prescricao_p bigint, cd_setor_atendimento_p bigint, ie_opcao_p text, ie_quebra_linha_p text) FROM PUBLIC;
