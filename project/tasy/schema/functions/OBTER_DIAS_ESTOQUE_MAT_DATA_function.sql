...
CREATE OR REPLACE FUNCTION obter_dias_estoque_mat_data ( cd_estabelecimento_p bigint, cd_material_p bigint, dt_mesano_referencia_p timestamp, nm_usuario_p text default 'Tasy') RETURNS bigint AS $body$
DECLARE
...
select	coalesce(max(ie_saldo_requisicao),'C') 
into STRICT	ie_saldo_requisicao_w 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_dias_estoque_mat_data ( cd_estabelecimento_p bigint, cd_material_p bigint, dt_mesano_referencia_p timestamp, nm_usuario_p text default 'Tasy') FROM PUBLIC;
