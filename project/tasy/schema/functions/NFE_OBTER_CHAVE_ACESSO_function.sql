...
CREATE OR REPLACE FUNCTION nfe_obter_chave_acesso (nr_seq_nota_p bigint, ie_tipo_retorno_w text default 'CH') RETURNS varchar AS $body$
DECLARE
...
select	CASE WHEN obter_se_nota_entrada_nfe(nr_sequencia)='S' THEN obter_valor_dominio(3620,nfe_obter_dados_pj(cd_cgc,'UF'))  ELSE obter_valor_dominio(3620,nfe_obter_dados_pj(cd_cgc_emitente,'UF')) END || 
	to_char(dt_emissao,'yymm') || 
	CASE WHEN obter_se_nota_entrada_nfe(nr_sequencia)='S' THEN LPAD(cd_cgc,14,'0')  ELSE LPAD(cd_cgc_emitente,14,'0') END  || 
	55 || 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION nfe_obter_chave_acesso (nr_seq_nota_p bigint, ie_tipo_retorno_w text default 'CH') FROM PUBLIC;
