...
CREATE OR REPLACE FUNCTION ehr_inf_paciente_habito_vicio ( nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, nr_atendimento_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') RETURNS varchar AS $body$
DECLARE
...
union
				...
				and	coalesce(x.dt_inativacao::text, '') = '') 
/*and	((a.dt_fim is null) or (sysdate between nvl(a.dt_inicio,sysdate-1) and a.dt_fim))*/
...
ds_comando_w	:= 'Select '||ds_comando_w ||' ds '||chr(13) ||'From paciente_habito_vicio ';
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ehr_inf_paciente_habito_vicio ( nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, nr_atendimento_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') FROM PUBLIC;
