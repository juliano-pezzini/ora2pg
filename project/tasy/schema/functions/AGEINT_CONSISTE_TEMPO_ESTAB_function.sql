...
CREATE OR REPLACE FUNCTION ageint_consiste_tempo_estab ( cd_agenda_p bigint, hr_Agenda_p timestamp, nr_minuto_duracao_p bigint, nr_Seq_ageint_p bigint, nr_seq_ageint_item_p bigint, nm_usuario_p text) RETURNS varchar AS $body$
DECLARE
...
C01 CURSOR FOR 
	...
	where	((cd_estab_pri	= cd_estab_agenda_w) 
	or (cd_estab_seg	= cd_estab_agenda_w)) 
	and	ie_situacao	= 'A';
...
C05 CURSOR FOR 
	...
	and		a.nr_Seq_ageint			= nr_seq_ageint_p 
	and		d.cd_setor_exclusivo 	= cd_estabelecimento_w 
	and		b.nr_sequencia			<> nr_seq_ageint_item_p;	
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	begin 
	select	CASE WHEN cd_estab_pri=cd_estab_agenda_w THEN  cd_estab_seg  ELSE cd_estab_pri END , 
			qt_tempo 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ageint_consiste_tempo_estab ( cd_agenda_p bigint, hr_Agenda_p timestamp, nr_minuto_duracao_p bigint, nr_Seq_ageint_p bigint, nr_seq_ageint_item_p bigint, nm_usuario_p text) FROM PUBLIC;
