...
CREATE OR REPLACE FUNCTION obter_motivo_inativ_agecons ( nr_seq_agenda_p bigint, nr_seq_motivo_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (nr_seq_agenda_p IS NOT NULL AND nr_seq_agenda_p::text <> '') then
	...
	if (nm_usuario_status_w IS NOT NULL AND nm_usuario_status_w::text <> '') then
		...
		elsif (ds_motivo_cad_w IS NOT NULL AND ds_motivo_cad_w::text <> '') then
			ds_motivo_w := substr(ds_motivo_cad_w,1,255);
		else
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_motivo_inativ_agecons ( nr_seq_agenda_p bigint, nr_seq_motivo_p bigint) FROM PUBLIC;
