...
CREATE OR REPLACE FUNCTION obter_horario_imp_nutricao ( nr_prescricao_p bigint, nr_sequencia_p bigint, ie_agrupador_p bigint, ie_proc_mat_p text) RETURNS varchar AS $body$
DECLARE
...
C01 CURSOR FOR
	SELECT	padroniza_horario_prescr(b.ds_horarios, CASE WHEN substr(Obter_Se_horario_hoje(a.dt_prescricao,a.dt_primeiro_horario,b.hr_prim_horario),1,1)='N' THEN '01/01/2000 23:59:59'  ELSE to_char(coalesce(a.dt_primeiro_horario,a.dt_prescricao),'dd/mm/yyyy hh24:mi:ss') END )
	from	prescr_material b,
		prescr_medica a
	where	b.nr_prescricao	= nr_prescricao_p
	  and	a.nr_prescricao = b.nr_prescricao
	  and	ie_proc_mat_p	= 'M'
	  and	b.ie_agrupador	= ie_agrupador_p
	  and	padroniza_horario_prescr(b.ds_horarios, CASE WHEN substr(Obter_Se_horario_hoje(a.dt_prescricao,a.dt_primeiro_horario,b.hr_prim_horario),1,1)='N' THEN '01/01/2000 23:59:59'  ELSE to_char(coalesce(a.dt_primeiro_horario,a.dt_prescricao),'dd/mm/yyyy hh24:mi:ss') END ) is not null
	
Union

	SELECT	padroniza_horario_prescr(b.ds_horarios, CASE WHEN substr(Obter_Se_horario_hoje(a.dt_prescricao,a.dt_primeiro_horario,to_char(b.dt_prev_execucao,'hh24:mi')),1,1)='N' THEN '01/01/2000 23:59:59'  ELSE to_char(coalesce(a.dt_primeiro_horario,a.dt_prescricao),'dd/mm/yyyy hh24:mi:ss') END )
	from	prescr_Procedimento b,
		prescr_medica a
	where	a.nr_prescricao	= nr_prescricao_p
	  and	a.nr_prescricao = b.nr_prescricao
	  and	padroniza_horario_prescr(b.ds_horarios, CASE WHEN substr(Obter_Se_horario_hoje(a.dt_prescricao,a.dt_primeiro_horario,to_char(b.dt_prev_execucao,'hh24:mi')),1,1)='N' THEN '01/01/2000 23:59:59'  ELSE to_char(coalesce(a.dt_primeiro_horario,a.dt_prescricao),'dd/mm/yyyy hh24:mi:ss') END ) is not null
	  and	ie_proc_mat_p	= 'P';
...
exception
	when others then
	--Parametro Horário Noturno da Prescrição Inválido.(hh/hh)');
	CALL Wheb_mensagem_pck.exibir_mensagem_abort(261439);
end;
...
if (coalesce(ds_horario_w::text, '') = '') or (ie_se_necessario_w = 'S') then
	...
	while	(ds_horario_w IS NOT NULL AND ds_horario_w::text <> '') LOOP
		...
		if (substr(upper(ds_hora_w),1,2) = 'AA') then
			horario_w[i].dt_hora	:= To_date('01/01/2000 ' || substr(ds_hora_w,3,6), 'dd/mm/yyyy hh24:mi') + 2;
		elsif (substr(upper(ds_hora_w),1,1) = 'A') then
			horario_w[i].dt_hora	:= To_date('01/01/2000 ' || substr(ds_hora_w,2,5), 'dd/mm/yyyy hh24:mi') + 1;
		else
...
/* Selecionar os horários de toda prescrição */
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		while	(ds_horario_w IS NOT NULL AND ds_horario_w::text <> '') LOOP
			...
			k			:= 1;
			if (substr(upper(ds_hora_w),1,2) = 'AA') then
				dt_hora_w	:= To_date('03/01/2000 ' || substr(ds_hora_w,3,6), 'dd/mm/yyyy hh24:mi');
			elsif (substr(upper(ds_hora_w),1,1) = 'A') then
				dt_hora_w	:= To_date('02/01/2000 ' || substr(ds_hora_w,2,5), 'dd/mm/yyyy hh24:mi');
			else
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_horario_imp_nutricao ( nr_prescricao_p bigint, nr_sequencia_p bigint, ie_agrupador_p bigint, ie_proc_mat_p text) FROM PUBLIC;
