...
CREATE OR REPLACE FUNCTION lic_obter_vl_lance_item_lote ( nr_seq_licitacao_p bigint, nr_seq_lic_item_p bigint, nr_seq_fornec_p bigint) RETURNS bigint AS $body$
DECLARE
...
if (ie_lance_zerado_w = 'N') then
	...
	if (pr_diferenca_w <= 0) then
		...
		vl_retorno_w	:= vl_proposta_inicial_w - vl_diferenca_w;
	else
		vl_diferenca_w	:= abs(((vl_lote_inicial_w * pr_diferenca_w)/100));
		vl_retorno_w	:= vl_lote_inicial_w - vl_diferenca_w;
	end if;
else
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION lic_obter_vl_lance_item_lote ( nr_seq_licitacao_p bigint, nr_seq_lic_item_p bigint, nr_seq_fornec_p bigint) FROM PUBLIC;
