...
CREATE OR REPLACE FUNCTION eis_obter_ocup_convenio ( cd_convenio_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estabelecimento_p bigint ) RETURNS bigint AS $body$
DECLARE
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eis_obter_ocup_convenio ( cd_convenio_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estabelecimento_p bigint ) FROM PUBLIC;
