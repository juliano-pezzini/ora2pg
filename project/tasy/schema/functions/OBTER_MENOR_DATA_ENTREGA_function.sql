-- Generated by Ora2Pg, the Oracle database Schema converter, version 23.1
-- Copyright 2000-2022 Gilles DAROLD. All rights reserved.
-- DATASOURCE: dbi:Oracle:host=srv-dbora-03.whebdc.com.br;service_name=DEV_1815;port=1521

SET client_encoding TO 'UTF8';





CREATE OR REPLACE FUNCTION obter_menor_data_entrega ( nr_documento_p bigint, ie_tipo_documento_p text) RETURNS timestamp AS $body$
DECLARE


dt_retorno_w		timestamp;


BEGIN

if (ie_tipo_documento_p = 'O') then
	begin

	select	min(dt_prevista_entrega)
	into STRICT	dt_retorno_w
	from	ordem_compra_item_entrega
	where	nr_ordem_compra = nr_documento_p;

	end;
elsif (ie_tipo_documento_p = 'S') then
	begin

	select	min(dt_entrega_solicitada)
	into STRICT	dt_retorno_w
	from	solic_compra_item_entrega
	where	nr_solic_compra = nr_documento_p;

	end;
elsif (ie_tipo_documento_p = 'C') then
	begin

	select	min(dt_entrega)
	into STRICT	dt_retorno_w
	from	cot_compra_item_entrega
	where	nr_cot_compra = nr_documento_p;

	end;
end if;

return	dt_retorno_w;

end;
$body$
LANGUAGE PLPGSQL
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_menor_data_entrega ( nr_documento_p bigint, ie_tipo_documento_p text) FROM PUBLIC;

