...
CREATE OR REPLACE FUNCTION diops_obter_valor_resultado ( nr_seq_operadora_p bigint, nr_seq_transacao_p bigint, nr_seq_periodo_p bigint, nr_seq_conta_lucprej_p bigint) RETURNS bigint AS $body$
DECLARE
...
while(length(ds_origem_conta_w) > 0) loop 
	...
	if (cd_conta_w IS NOT NULL AND cd_conta_w::text <> '') then 
		...
		into STRICT	vl_lucro_prejuizo_w 
		from	ctb_mes_ref			b, 
			ctb_balancete_v			a 
	...
	vl_conta_w	:= coalesce(vl_lucro_prejuizo_w,0);
	if (ie_sinal_w = '-') then 
...
/* select diops_obter_valor_resultado(17, 5, 2, 6) from dual; */
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION diops_obter_valor_resultado ( nr_seq_operadora_p bigint, nr_seq_transacao_p bigint, nr_seq_periodo_p bigint, nr_seq_conta_lucprej_p bigint) FROM PUBLIC;
