...
CREATE OR REPLACE FUNCTION proj_obter_se_ata_lib_usuario ( nr_seq_ata_p bigint, nm_usuario_p text) RETURNS varchar AS $body$
DECLARE
...
BEGIN
ie_forma_acesso_ata_w := obter_param_usuario(4000, 106, obter_perfil_Ativo, nm_usuario_p, cd_estabelecimento_w, ie_forma_acesso_ata_w);
ie_visu_ata_sem_lib_w := obter_param_usuario(4000, 177, obter_perfil_Ativo, nm_usuario_p, cd_estabelecimento_w, ie_visu_ata_sem_lib_w);
...
if (coalesce(nr_seq_ata_p,0) > 0) then
	...
	if (coalesce(cd_pf_usuario_w,'0') <> '0') and	/*Se o usuário é o digitador da ATA, visualiza*/
		(cd_pf_digitador_w = cd_pf_usuario_w) then
		ds_retorno_w	:= 'S';
	end if;
	...
	if (ds_retorno_w = 'N') and
		...
		if (ie_forma_acesso_ata_w = 'N') then	/*Se não restringe, qualquer usuário pode visualizar*/
			ds_retorno_w	:= 'S';
		elsif (ie_forma_acesso_ata_w = 'S') then	/*Se não restringe, qualquer usuário pode visualizar*/
			select	CASE WHEN count(*)=0 THEN 'N'  ELSE 'S' END
			into STRICT	ds_retorno_w
			from	proj_ata_participante
			where	nr_seq_ata 		= nr_seq_ata_p
			and	cd_pessoa_participante 	= cd_pf_usuario_w;
		elsif (ie_forma_acesso_ata_w = 'D') then	/*Se depende da regra*/
			select	count(*)
			...
			if (ds_retorno_w = 'N') then	/*Se existir alguma regra geral, permite para todos*/
				select	CASE WHEN count(*)=0 THEN 'N'  ELSE 'S' END
				into STRICT	ds_retorno_w
				from	proj_ata_lib
				where	nr_seq_ata = nr_seq_ata_p
			...
			if (ds_retorno_w = 'N') then	/*Verifica se tem regra por cargo ou setor ou grupo de usuário*/
				select	CASE WHEN count(*)=0 THEN 'N'  ELSE 'S' END
				into STRICT	ds_retorno_w
				from	proj_ata_lib
				where	nr_seq_ata = nr_seq_ata_p
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION proj_obter_se_ata_lib_usuario ( nr_seq_ata_p bigint, nm_usuario_p text) FROM PUBLIC;
