...
CREATE OR REPLACE FUNCTION obter_envia_marca_padrao ( cd_estabelecimento_p bigint, cd_classe_material_p bigint, cd_material_p bigint) RETURNS varchar AS $body$
DECLARE
...
ie_envia_marca_padrao_w			varchar(1)	:= 'N';
ie_padronizado_w				varchar(15);
ie_estoque_w				varchar(15);

c01 CURSOR FOR
SELECT	coalesce(ie_envia_marca_padrao,'N')
...
loop
fetch C01 into
	ie_envia_marca_padrao_w;
EXIT WHEN NOT FOUND; /* apply on C01 */
	begin
	ie_envia_marca_padrao_w	:= ie_envia_marca_padrao_w;
	end;
end loop;
close C01;

return ie_envia_marca_padrao_w;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_envia_marca_padrao ( cd_estabelecimento_p bigint, cd_classe_material_p bigint, cd_material_p bigint) FROM PUBLIC;
