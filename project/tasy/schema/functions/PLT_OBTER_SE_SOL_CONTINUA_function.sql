...
CREATE OR REPLACE FUNCTION plt_obter_se_sol_continua ( nr_prescricao_p bigint, nr_seq_solucao_p bigint, ds_solucao_p text, ie_tipo_retorno_p text ) RETURNS bigint AS $body$
DECLARE


/*
ie_tipo_retorno_p
P - Prescrição com a mesma solução
S - Sequência da mesma solução
*/
...
C01 CURSOR FOR  --Buscar todas soluções do atendimento com o mesmo titulo
	...
	and	b.dt_validade_prescr > trunc(clock_timestamp() - interval '30 days')
...
if (nr_retorno_w		<> 0) and (ie_tipo_retorno_p	= 'S') then

	nr_retorno_w	:= nr_seq_solucao_ww;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION plt_obter_se_sol_continua ( nr_prescricao_p bigint, nr_seq_solucao_p bigint, ds_solucao_p text, ie_tipo_retorno_p text ) FROM PUBLIC;
