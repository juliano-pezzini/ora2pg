...
CREATE OR REPLACE FUNCTION obter_qt_material_fat ( cd_estabelecimento_p bigint, cd_material_p bigint, cd_convenio_p bigint, cd_categoria_p text, cd_setor_p bigint, qt_material_p bigint, cd_kit_material_p bigint, dt_entrada_p timestamp, dt_atendimento_p timestamp, nr_prescricao_p bigint, nr_sequencia_prescricao_p bigint) RETURNS bigint AS $body$
DECLARE
...
if (qt_regra_w > 0) then
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		if (ie_regra_disp_w = 'C') then
		...
		elsif (ie_regra_disp_w = 'L') then  /* Fabr?cio em 01/09/2008 OS 97666  conforme definido em reuni?o  Ed?lson/Ricardo/Alejandra/Fabr?cio  */
			begin
		...
		elsif (ie_regra_disp_w = 'P') then  /* Fabr?cio em 28/09/2009 OS 161770   */
			begin						
		...
		elsif (ie_regra_disp_w = 'D') then  /* Fabr?cio OS 480663  */
			begin
							
			qt_faturar_w:= dividir(qt_faturar_w , coalesce(nr_multiplo_qtde_w,1));
			
			end;
		elsif (ie_regra_disp_w = 'B') then  /* Fabr?cio OS 507022  */
			begin
			...
			begin
			-- ex: 233 ml  (1Fr tem 100 ml), deve se cobrar 300ml por?m baixar de estoque apenas 233ml
			if (mod(qt_material_p, qt_conv_estoque_consumo_w) <> 0) then --Se a divis?o n?o ? exata (Lan?amento de qtde completa)
				qt_faturar_w:= trunc(dividir(qt_material_p , qt_conv_estoque_consumo_w)); -- ex: 233/100 = 2,33     Trunc(2,33) = 2
				if (qt_faturar_w >= 0) then -- Se ? positivo
					qt_faturar_w:= qt_faturar_w + 1; --ex: 2 + 1 = 3
					qt_faturar_w:= qt_faturar_w * qt_conv_estoque_consumo_w; --ex: 3 x 100ml (que corresponde ? um frasco)
			...
			begin
			-- ex: 6 gts  (1Fr tem 400 gts (Estoque x Consumo)), deve se cobrar 20gts que equivale ? 1 ml, pois o conv?nio paga em ml, por?m o m?dico prescreve em ml, por?m baixar de estoque apenas 6 gts
			if (mod(qt_material_p, qt_limite_min_w) <> 0) then --Se a divis?o n?o ? exata (Lan?amento de qtde completa)
				qt_faturar_w:= trunc(dividir(qt_material_p , qt_limite_min_w)); -- ex: 6/20 = 0,3     Trunc(0,3) = 0
				qt_faturar_w:= qt_faturar_w + 1; --ex: 0 + 1 = 1
				qt_faturar_w:= qt_faturar_w * qt_limite_min_w; --ex: 1 x 20gts (que corresponde ? um ml)
			end if;
			end;
		elsif (ie_regra_disp_w = 'X') then
			begin
			/*Heckmann OS 226628  Pega a quantidade de lan?amento do item e verifica se encaixa em alguma regra de faixa de dosagem
			Ex.: tem regra para um item com faixa de 1 ? 147, o usu?rio lan?a qtd 50 na conta, o sistema cobra quantidade  igual a 147	*/
			qt_fat_w	:= 0;
			begin
			...
			begin
			-- Qauntidade Prescrita  OS 207305
			qt_faturar_w	:= qt_material_p;
			if (coalesce(nr_prescricao_p,0) > 0) and (coalesce(nr_sequencia_prescricao_p,0) > 0) then
			
				select 	coalesce(max(qt_material),qt_material_p)
				into STRICT	qt_faturar_w
			...
			/*Heckmann OS 300142  Pega a quantidade  prescrita do item e verifica se encaixa em alguma regra de faiza de dosagem
			Ex.: tem regra para um item com faixa de 1 ? 110, o prescreve qtd 10 mg , o sistema arredonda para 110 e divide
				pela regra de convers?o unidade de medida (Cadastro de Materiais / Farm?cia / Conv unid),
				ent?o, ter?amos:
					110/440 = 0,25
				esta seria a quantidade a ser cobrada do paciente
				por?m, a quantidade a ser baixada do estoque permanece:
					10/440 = 0,02*/
			if (coalesce(nr_prescricao_p,0) > 0) and (coalesce(nr_sequencia_prescricao_p,0) > 0) then
				
				begin
				select 	coalesce(max(qt_dose),qt_material_p),
					coalesce(cd_unidade_medida_dose,'UMS')
				into STRICT	qt_faturar_w,
					cd_unidade_medida_w
				from 	prescr_material
				where 	nr_prescricao = nr_prescricao_p
				and 	nr_sequencia = nr_sequencia_prescricao_p
				group by	cd_unidade_medida_dose;
				exception
					when others then
					qt_faturar_w		:= 0;
				end;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_qt_material_fat ( cd_estabelecimento_p bigint, cd_material_p bigint, cd_convenio_p bigint, cd_categoria_p text, cd_setor_p bigint, qt_material_p bigint, cd_kit_material_p bigint, dt_entrada_p timestamp, dt_atendimento_p timestamp, nr_prescricao_p bigint, nr_sequencia_prescricao_p bigint) FROM PUBLIC;
