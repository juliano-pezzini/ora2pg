...
CREATE OR REPLACE FUNCTION lab_obter_amostra_prescr (ds_valor_p text, nr_prescricao_p bigint, nr_seq_exame_p bigint, nr_seq_prescr_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (ds_valor_p = 'PM') then
	if (pkg_i18n.get_user_locale() = 'es_AR') then
	        ds_resultado_w := nr_prescricao_w || 'M' || nr_seq_mat_w;
	else
		ds_resultado_w := lpad(nr_prescricao_w || 'M' || nr_seq_mat_w,10,'0');
	end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION lab_obter_amostra_prescr (ds_valor_p text, nr_prescricao_p bigint, nr_seq_exame_p bigint, nr_seq_prescr_p bigint) FROM PUBLIC;
