...
CREATE OR REPLACE FUNCTION obter_erro_produto_gerencia (dt_parametro_p timestamp, nr_seq_gerencia_p bigint, ie_tipo_valor_p text, nr_seq_localizacao_p bigint) RETURNS bigint AS $body$
DECLARE
...
WHERE a.dt_ordem_servico between dt_ref_mes_w and dt_fim_mes_w  and ((nr_seq_gerencia_p = 0) or (b.nr_seq_gerencia	= nr_seq_gerencia_p)) and ((coalesce(nr_seq_localizacao_p,0) = 0) or (a.nr_seq_localizacao = nr_seq_localizacao_p));
		...
		*/
		if ( nr_seq_gerencia_p = 2) then
			...
			FROM man_ordem_servico a
LEFT OUTER JOIN grupo_desenvolvimento b ON (a.nr_seq_grupo_des = b.nr_sequencia)
WHERE a.dt_ordem_servico between dt_ref_mes_w and dt_fim_mes_w  and b.nr_seq_gerencia	!= 2 and ((coalesce(nr_seq_localizacao_p,0) = 0) or (a.nr_seq_localizacao = nr_seq_localizacao_p)) and exists (
				SELECT 	1
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_erro_produto_gerencia (dt_parametro_p timestamp, nr_seq_gerencia_p bigint, ie_tipo_valor_p text, nr_seq_localizacao_p bigint) FROM PUBLIC;
