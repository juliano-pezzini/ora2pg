...
CREATE OR REPLACE FUNCTION obter_valor_item_opme (nr_sequencia_p bigint, ie_opcao_p text) RETURNS bigint AS $body$
DECLARE
		...
		EXIT WHEN NOT FOUND; /* apply on c01 */
			...
			if (ie_achou_w = 'N') then 
				if (ie_opcao_p = 'U') then 
					select 	obter_preco_material(wheb_usuario_pck.get_cd_estabelecimento,cd_convenio_w,cd_categoria_w,clock_timestamp(),cd_material_w,0,ie_tipo_atendimento_w,0,null,0,0) 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_valor_item_opme (nr_sequencia_p bigint, ie_opcao_p text) FROM PUBLIC;
