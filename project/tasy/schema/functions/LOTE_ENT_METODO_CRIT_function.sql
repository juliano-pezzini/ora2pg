...
CREATE OR REPLACE FUNCTION lote_ent_metodo_crit (nr_seq_ficha_p bigint, nr_seq_exame_p bigint) RETURNS varchar AS $body$
DECLARE
...
IF (nr_prescricao_w IS NOT NULL AND nr_prescricao_w::text <> '') AND (nr_seq_exame_p IS NOT NULL AND nr_seq_exame_p::text <> '') AND (nr_seq_prescr_w IS NOT NULL AND nr_seq_prescr_w::text <> '') THEN
	...
	select	max(a.ds_metodo_criterio)
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION lote_ent_metodo_crit (nr_seq_ficha_p bigint, nr_seq_exame_p bigint) FROM PUBLIC;
