...
CREATE OR REPLACE FUNCTION obter_volume_total_diluicao ( NR_SEQUENCIA_P bigint, NR_PRESCRICAO_P bigint) RETURNS varchar AS $body$
DECLARE
...
if (qt_conv_ml_w > 0) and (upper(cd_unid_med_dose_mat_w) <> upper(obter_unid_med_usua('ml'))) then
	if (qt_conv_ml_w >= 1) then
		ds_conv_ml_w	:= to_char(qt_conv_ml_w)||' '||lower(obter_unid_med_usua('ml'));
	else
		ds_conv_ml_w	:= '0'||to_char(qt_conv_ml_w)||' '||lower(obter_unid_med_usua('ml'));
	end if;
end if;
...
if (qt_solucao_ww IS NOT NULL AND qt_solucao_ww::text <> '') then
	ds_volume_w	:= ' '||to_char(qt_solucao_ww) ||' '||lower(obter_unid_med_usua('ml'));
elsif (ie_aplicar_w = 'S') then
	begin
	if (ds_rediluir_w IS NOT NULL AND ds_rediluir_w::text <> '') then
		ds_volume_w	:= ' '||cd_volume_final_str_w ||' '||lower(obter_unid_med_usua('ml'));
	elsif (ds_diluir_w IS NOT NULL AND ds_diluir_w::text <> '') then
		ds_volume_w	:= ' '||to_char(qt_conv_ml_w + qt_ml_diluente_w) ||' '||lower(obter_unid_med_usua('ml'));
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_volume_total_diluicao ( NR_SEQUENCIA_P bigint, NR_PRESCRICAO_P bigint) FROM PUBLIC;
