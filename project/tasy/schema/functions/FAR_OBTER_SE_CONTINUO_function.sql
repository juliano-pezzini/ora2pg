...
CREATE OR REPLACE FUNCTION far_obter_se_continuo ( cd_pessoa_fisica_p text, cd_material_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (qt_medic_cont_w > 0) then
	...
	if (qt_pedido_w > 0) then
		...
		if ((dt_ult_compra_w+regra_w) < (clock_timestamp() + interval '10 days')) then
			ie_consiste_w:= 'S';
		end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION far_obter_se_continuo ( cd_pessoa_fisica_p text, cd_material_p bigint) FROM PUBLIC;
