...
CREATE OR REPLACE FUNCTION obter_status_hora_agenda (cd_agenda_p bigint, dt_agenda_p timestamp) RETURNS varchar AS $body$
DECLARE
...
for	i in 0 .. 23 loop
	...
	and	hr_inicio between hr_inicial_w and hr_final_w;
	...
	if (qt_agenda_w = 0)  then
		...
		and	hr_inicial_w >=  hr_inicio
		and	hr_inicio + (nr_minuto_duracao / 1440) >= hr_inicial_w
		and	hr_inicio + (nr_minuto_duracao / 1440) <= (trunc(dt_agenda_p, 'dd') + 1);

		select	count(*)
		into STRICT	qt_ocup_w
		from	agenda_paciente
		where	cd_agenda		= cd_agenda_p
		and	ie_status_agenda	in ('N')
		and	hr_inicial_w >=  hr_inicio
		and	hr_inicio + (nr_minuto_duracao / 1440) >= hr_inicial_w
		and	hr_inicio + (nr_minuto_duracao / 1440) <= (trunc(dt_agenda_p, 'dd') + 1);
		...
		if (qt_ocup_w > 0) then
			...
			if (qt_ocup_w = qt_confirmado_w) then
				ie_status_hora_w	:= 'F';
			else
				ie_status_hora_w	:= 'O';
			end if;
		...
		if (qt_agenda_w = qt_exec_w) then
			ie_status_hora_w	:= 'E';
		elsif (qt_agenda_w = qt_livres_w) then
			ie_status_hora_w	:= 'L';
		elsif	((qt_ocup_w + qt_bloq_w) > 0) and (qt_livres_w > 0) then
			ie_status_hora_w	:= 'N';
		elsif (qt_confirmado_w <> qt_ocup_w) and (qt_ocup_w > 0) and (qt_livres_w = 0) then
			ie_status_hora_w	:= 'O';
		elsif (qt_confirmado_w = qt_ocup_w) and (qt_ocup_w > 0) and (qt_livres_w = 0) then
			ie_status_hora_w	:= 'F';
		elsif (qt_bloq_w > 0) and (qt_livres_w = 0) then
			ie_status_hora_w	:= 'B';
		elsif (qt_reserv_w > 0) and (qt_livres_w = 0) then
			ie_status_hora_w	:= 'R';
		end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_status_hora_agenda (cd_agenda_p bigint, dt_agenda_p timestamp) FROM PUBLIC;
