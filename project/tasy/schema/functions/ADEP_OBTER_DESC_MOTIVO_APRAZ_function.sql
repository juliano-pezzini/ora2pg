-- Generated by Ora2Pg, the Oracle database Schema converter, version 23.1
-- Copyright 2000-2022 Gilles DAROLD. All rights reserved.
-- DATASOURCE: dbi:Oracle:host=srv-dbora-03.whebdc.com.br;service_name=DEV_1815;port=1521

SET client_encoding TO 'UTF8';





CREATE OR REPLACE FUNCTION adep_obter_desc_motivo_apraz ( nr_sequencia_p bigint) RETURNS varchar AS $body$
DECLARE


ds_motivo_w	varchar(60);
nr_seq_motivo_w	bigint;


BEGIN

if (nr_sequencia_p IS NOT NULL AND nr_sequencia_p::text <> '') then
	select	nr_seq_motivo
	into STRICT	nr_seq_motivo_w
	from	prescr_mat_alteracao
	where	nr_sequencia	= nr_sequencia_p;

	if (nr_seq_motivo_w IS NOT NULL AND nr_seq_motivo_w::text <> '') then
		select	coalesce(max(ds_motivo),'')
		into STRICT	ds_motivo_w
		from	adep_motivo_interrupcao
		where	nr_sequencia = nr_seq_motivo_w;
	end if;
end if;

return	ds_motivo_w;

end;
$body$
LANGUAGE PLPGSQL
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION adep_obter_desc_motivo_apraz ( nr_sequencia_p bigint) FROM PUBLIC;

