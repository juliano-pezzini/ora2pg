...
CREATE OR REPLACE FUNCTION cpoe_obter_desc_item ( nr_sequencia_p bigint, ie_tipo_item_p text) RETURNS varchar AS $body$
DECLARE
...
if (ie_tipo_item_p IS NOT NULL AND ie_tipo_item_p::text <> '') and (nr_sequencia_p IS NOT NULL AND nr_sequencia_p::text <> '') then 
	...
	if (ie_tipo_item_p in ('M','MAT','SOL')) then 
		...
		select	max(substr(obter_desc_san_derivado(nr_seq_derivado),1,255)) 
		into STRICT	ds_item_w 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION cpoe_obter_desc_item ( nr_sequencia_p bigint, ie_tipo_item_p text) FROM PUBLIC;
