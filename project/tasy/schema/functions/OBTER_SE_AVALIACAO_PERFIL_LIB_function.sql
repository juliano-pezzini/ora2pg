...
CREATE OR REPLACE FUNCTION obter_se_avaliacao_perfil_lib (cd_perfil_p bigint, nr_seq_avaliacao_p bigint, cd_setor_atendimento_p bigint, nr_atendimento_p bigint default 0) RETURNS varchar AS $body$
DECLARE
...
C01 CURSOR FOR 
	SELECT	coalesce(IE_PERMITE_LIBERAR,'S') 
	...
	and	coalesce(cd_setor_atendimento,obter_setor_ativo) = obter_setor_ativo 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_avaliacao_perfil_lib (cd_perfil_p bigint, nr_seq_avaliacao_p bigint, cd_setor_atendimento_p bigint, nr_atendimento_p bigint default 0) FROM PUBLIC;
