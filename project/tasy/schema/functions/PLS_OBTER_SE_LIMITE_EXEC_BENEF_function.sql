...
CREATE OR REPLACE FUNCTION pls_obter_se_limite_exec_benef ( nr_seq_ocor_lim_benef_p bigint, nr_seq_guia_p bigint, nr_seq_requisicao_p bigint, nr_seq_execucao_p bigint, nr_seq_segurado_p bigint, cd_procedimento_p bigint, ie_origem_proced_p bigint, ie_preco_p text, qt_solicitada_p bigint, ie_valida_execucao_p text, nr_seq_param2_p bigint, nr_seq_param3_p bigint, nr_seq_param4_p bigint, nr_seq_param5_p bigint) RETURNS varchar AS $body$
DECLARE
...
C01 CURSOR FOR 
	...
	from	pls_limite_benef_item 
	where	nr_seq_ocor_lim_benef	= nr_seq_ocor_lim_benef_p 
...
begin 
	select	coalesce(qt_limite_execucao,0), 
		ie_preco 
	into STRICT	qt_limite_execucao_w, 
		ie_preco_regra_w 
	from	pls_ocorrencia_lim_benef 
	where	nr_sequencia	= nr_seq_ocor_lim_benef_p 
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		if (ie_grupo_servico_w	= 'S') then 
			if (nr_seq_guia_p IS NOT NULL AND nr_seq_guia_p::text <> '') then 
				...
				if (ie_valida_execucao_p = 'N') then 
				...
				elsif (ie_valida_execucao_p = 'S') then				 
					select	sum(b.qt_item) 
					into STRICT	qt_registros_w 
					from	pls_guia_plano		e, 
						pls_plano		d, 
						pls_requisicao		c, 
					...
					and	b.nr_seq_guia		= e.nr_sequencia 
					and	(((ie_preco_regra_w IS NOT NULL AND ie_preco_regra_w::text <> '') 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION pls_obter_se_limite_exec_benef ( nr_seq_ocor_lim_benef_p bigint, nr_seq_guia_p bigint, nr_seq_requisicao_p bigint, nr_seq_execucao_p bigint, nr_seq_segurado_p bigint, cd_procedimento_p bigint, ie_origem_proced_p bigint, ie_preco_p text, qt_solicitada_p bigint, ie_valida_execucao_p text, nr_seq_param2_p bigint, nr_seq_param3_p bigint, nr_seq_param4_p bigint, nr_seq_param5_p bigint) FROM PUBLIC;
