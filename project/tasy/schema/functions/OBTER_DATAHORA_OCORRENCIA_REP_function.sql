...
CREATE OR REPLACE FUNCTION obter_datahora_ocorrencia_rep ( ds_horarios_p text, nr_ocorrencia_p bigint) RETURNS timestamp AS $body$
DECLARE
...
dt_retorno_w	:= to_date(substr(to_char(clock_timestamp(),'dd/mm/yyyy hh24:mi:ss'),1,11) || hr_horario_ww ||
		':00','dd/mm/yyyy hh24:mi:ss');


if (dt_retorno_w < clock_timestamp()) then
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_datahora_ocorrencia_rep ( ds_horarios_p text, nr_ocorrencia_p bigint) FROM PUBLIC;
