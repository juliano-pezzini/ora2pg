...
CREATE OR REPLACE FUNCTION obter_media_consumo_loc_transf ( cd_estabelecimento_p bigint, cd_local_estoque_p bigint, cd_material_p bigint, ie_tipo_p text) RETURNS bigint AS $body$
DECLARE
...
select	coalesce(sum(a.qt_estoque),0) qt_estoque, 
		coalesce(sum(a.vl_estoque),0) vl_consumo 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_media_consumo_loc_transf ( cd_estabelecimento_p bigint, cd_local_estoque_p bigint, cd_material_p bigint, ie_tipo_p text) FROM PUBLIC;
