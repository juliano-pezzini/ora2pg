...
CREATE OR REPLACE FUNCTION obter_se_limite_sinal_vital ( nr_atendimento_p bigint, nm_campo_p text, vl_sinal_p bigint, nm_usuario_p text) RETURNS varchar AS $body$
DECLARE
...
if (nm_campo_p = 'QT_PESO') then 
	...
	nr_seq_sinal_w:= 4;
elsif (nm_campo_p = 'QT_FREQ_CARDIAC') then 
	nr_seq_sinal_w:= 9;	
elsif (nm_campo_p = 'QT_PA_SISTOLICA') then 
	nr_seq_sinal_w:= 1;				
elsif (nm_campo_p = 'QT_PA_DIASTOLIC') then 
	nr_seq_sinal_w:=2;
end if;
...
if (nr_sequencia_sv_w IS NOT NULL AND nr_sequencia_sv_w::text <> '') and 
	((Vl_Sinal_P < vl_minimo_aviso_sv_w) or (Vl_Sinal_P > vl_maximo_aviso_sv_w))or 
	((Vl_Sinal_P < vl_minimo_permitido_sv_w) or (Vl_Sinal_P > vl_maximo_permitido_sv_w)) then 
	ds_retorno_w	:= 'S';
end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_limite_sinal_vital ( nr_atendimento_p bigint, nm_campo_p text, vl_sinal_p bigint, nm_usuario_p text) FROM PUBLIC;
