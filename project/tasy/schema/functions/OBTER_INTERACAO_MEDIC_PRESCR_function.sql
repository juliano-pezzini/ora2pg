...
CREATE OR REPLACE FUNCTION obter_interacao_medic_prescr ( nr_prescricao_p bigint) RETURNS varchar AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	EXIT WHEN NOT FOUND; /* apply on C02 */
		begin
		if (coalesce(length(ds_interacao_ww),0) < 4000) then
			begin
			...
			if (ds_severidade_w IS NOT NULL AND ds_severidade_w::text <> '') then
					if (coalesce(ds_interacao_ww::text, '') = '') then
						ds_interacao_ww :=  substr(upper(ds_severidade_w) || chr(13) || chr(10) || ds_interacao_w,1,4000);
					else
						ds_interacao_ww :=  substr(ds_interacao_ww  || chr(13) || chr(10) ||  upper(ds_severidade_w) || chr(13) || chr(10) || ds_interacao_w,1,4000);
					end if;
			end if;
	...
	if (ds_interacao_ww IS NOT NULL AND ds_interacao_ww::text <> '') and (coalesce(length(ds_retorno_w),0) < 4000) then
		if (coalesce(ds_retorno_w::text, '') = '') then
			ds_retorno_w := ds_interacao_ww;
		else
			ds_retorno_w := substr(ds_retorno_w ||chr(13) || chr(10) || ds_interacao_ww,1,4000);
		end if;
	end if;
	...
	if (varMostraReacoes_w = 'T') and (ds_retorno_w IS NOT NULL AND ds_retorno_w::text <> '') then
		...
		if (ds_reacao_w IS NOT NULL AND ds_reacao_w::text <> '') then
			ds_retorno_w := substr(ds_retorno_w ||chr(13) || chr(10) || Wheb_mensagem_pck.get_texto(351087)||': ' ||ds_reacao_w,1,4000);
		end if;
	end if;
...
EXIT WHEN NOT FOUND; /* apply on C03 */
	...
	if (ds_interacao_alimento_w IS NOT NULL AND ds_interacao_alimento_w::text <> '') then
		ds_retorno_w	:= substr(ds_retorno_w || chr(13) || chr(10) || Wheb_mensagem_pck.get_texto(351088, 'DS_MATERIAL_WW='||ds_material_ww||';DS_INTERACAO_ALIMENTO_W='||ds_interacao_alimento_w),1,4000);
	end if;
end loop;
close C03;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_interacao_medic_prescr ( nr_prescricao_p bigint) FROM PUBLIC;
