...
CREATE OR REPLACE FUNCTION pls_oc_cta_obter_rest_val_23 ( dados_validacao_p pls_tipos_ocor_pck.dados_regra_val_util_item, dados_regra_p pls_tipos_ocor_pck.dados_regra, dados_conta_proc_p pls_tipos_ocor_pck.dados_conta_proc, dados_conta_mat_p pls_tipos_ocor_pck.dados_conta_mat, ie_proc_mat_p text, nr_seq_selecao_p pls_oc_cta_selecao_ocor_v.nr_sequencia%type, bind_sql_valor_p INOUT sql_pck.t_dado_bind) AS $body$
DECLARE


/* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade:	Obter a restrição a ser aplicada na validação 23 - Validar utilização de itens.
-------------------------------------------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------------------------------------------
Observações:
	* Esta rotina foi origininada da duplicação da procedure PLS_OC_CTA_OBTER_REST_VAL_UTIT.
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
...
if (dados_validacao_p.ie_tipo_qtde = 'G') then
	...
	if (dados_regra_p.ie_evento = 'IMP') then
		if (ie_proc_mat_p = 'P') then
			...
			bind_sql_valor_p := sql_pck.bind_variable(':cd_guia_proc_imp', dados_conta_proc_p.cd_guia_imp, bind_sql_valor_p);
			...
			bind_sql_valor_p := sql_pck.bind_variable(':cd_guia_mat_imp', dados_conta_mat_p.cd_guia_imp, bind_sql_valor_p);
		end if;
	else
		if (ie_proc_mat_p = 'P') then
			retorno_w.ds_restricao_proc :=	retorno_w.ds_restricao_proc || pls_tipos_ocor_pck.enter_w ||
							'and	outro_proc.cd_guia_referencia = :cd_guia_referencia_proc ';
			bind_sql_valor_p := sql_pck.bind_variable(':cd_guia_referencia_proc', dados_conta_proc_p.cd_guia_referencia, bind_sql_valor_p);
		elsif (ie_proc_mat_p = 'M') then
			retorno_w.ds_restricao_mat :=	retorno_w.ds_restricao_mat || pls_tipos_ocor_pck.enter_w ||
							'and	outro_mat.cd_guia_referencia = :cd_guia_referencia_mat ';
			bind_sql_valor_p := sql_pck.bind_variable(':cd_guia_referencia_mat', dados_conta_mat_p.cd_guia_referencia, bind_sql_valor_p);
		end if;
	end if;
...
if (dados_validacao_p.ie_somar_estrutura = 'S') then
	...
	if (dados_regra_p.ie_evento = 'IMP') then
		if (ie_proc_mat_p = 'P') then
			...
			bind_sql_valor_p := sql_pck.bind_variable(':cd_procedimento_imp', dados_conta_proc_p.cd_procedimento_imp, bind_sql_valor_p);
		elsif (ie_proc_mat_p = 'M') then
			retorno_w.ds_restricao_mat :=	retorno_w.ds_restricao_mat || pls_tipos_ocor_pck.enter_w ||
		...
		if (ie_proc_mat_p = 'P') then
			...
			bind_sql_valor_p := sql_pck.bind_variable(':ie_origem_proced', dados_conta_proc_p.ie_origem_proced, bind_sql_valor_p);
			bind_sql_valor_p := sql_pck.bind_variable(':cd_procedimento', dados_conta_proc_p.cd_procedimento, bind_sql_valor_p);
		elsif (ie_proc_mat_p = 'M') then
			retorno_w.ds_restricao_mat :=	retorno_w.ds_restricao_mat || pls_tipos_ocor_pck.enter_w ||
							'and	outro_mat.nr_seq_material = :nr_seq_material ';
			bind_sql_valor_p := sql_pck.bind_variable(':nr_seq_material', dados_conta_mat_p.nr_seq_material, bind_sql_valor_p);
...
if (ie_proc_mat_p = 'P') then
	...
	-- pessoas que o item que está sendo consistido. Por exemplo: Se for para verificar "Beneficiário"  e "Mesmo prestador executor" então só podem ser contados
	-- os itens que forem de contas do mesmo beneficiário e executadas pelo mesmo prestador.
	-- o prestador está sendo verificado nessa função por motivos de performance e o segurado é verificado em pls_oc_cta_obter_rest_val_23
		-- se for procedimento
		-- se o prestador não for igual desconsidera
		retorno_w.ds_restricao_proc :=	retorno_w.ds_restricao_proc || pls_tipos_ocor_pck.enter_w ||
							'and	outro_proc.nr_seq_prestador_exec = :nr_seq_prestador_exec ';
			bind_sql_valor_p := sql_pck.bind_variable(':nr_seq_prestador_exec', dados_conta_proc_p.nr_seq_prestador_exec, bind_sql_valor_p);
	...
	if (dados_validacao_p.ie_tipo_qtde = 'A') then
		-- Aqui verifica os itens lançados em um período de X Anos conforme definido pelo usuário no campo QT_TIPO_QUANTIDADE.
		-- Obs: deve se ter informação neste campo para que o filtro seja aplicado.
		...
		-- após o procedimento dentro do período informado na regra do campo QT_TIPO_QUANTIDADE. Se não considerar lançamentos posteriores
		...
		if (dados_validacao_p.ie_qt_lib_posterior = 'S') then
			...
			-- calcula as datas do período que devem ser consideradas
			...
			retorno_w.ds_restricao_proc :=	retorno_w.ds_restricao_proc || pls_tipos_ocor_pck.enter_w ||
								'and	outro_proc.dt_procedimento between :dt_val_inic_w and :dt_val_fim_w ';
			bind_sql_valor_p := sql_pck.bind_variable(':dt_val_inic_w', dt_val_inic_w, bind_sql_valor_p);
			bind_sql_valor_p := sql_pck.bind_variable(':dt_val_fim_w', dt_val_fim_w, bind_sql_valor_p);
	...
	-- Meses
		...
		-- Aqui verifica o período em meses na qual a ocorrência busca os itens lançados.
		-- Se for para contar atendimentos posteriores ao que está sendo consistido no momento, então deve considerar atendimentos que foram lançados
		-- após o procedimento dentro do período informado na regra do campo QT_TIPO_QUANTIDADE. Se não considerar lançamentos posteriores
		-- então busca os procedimentos executados até a data do procedimento.
		if (dados_validacao_p.ie_qt_lib_posterior = 'S') then
	...
	-- Dias
		...
		if (dados_validacao_p.ie_qt_lib_posterior = 'S') then
			...
			retorno_w.ds_restricao_proc :=	retorno_w.ds_restricao_proc || pls_tipos_ocor_pck.enter_w ||
								'and	outro_proc.dt_procedimento_real between :dt_val_inic_w and :dt_val_fim_w ';
			bind_sql_valor_p := sql_pck.bind_variable(':dt_val_inic_w', dt_val_inic_w, bind_sql_valor_p, sql_pck.b_data_hora);
			bind_sql_valor_p := sql_pck.bind_variable(':dt_val_fim_w', dt_val_fim_w, bind_sql_valor_p, sql_pck.b_data_hora);
	...
	-- Horario
		...
		-- Aqui é verificado o período que será buscado os itens por dia e horas. Está sendo verificado a data do item diminuindo ou adicionando a quantidade de dias informados no campo.
		-- calcula as datas do período que devem ser consideradas
		dt_val_inic_w := dados_conta_proc_p.dt_procedimento;
		retorno_w.ds_restricao_proc :=	retorno_w.ds_restricao_proc || pls_tipos_ocor_pck.enter_w ||
								'and	outro_proc.dt_procedimento = :dt_val_inic_w  ';
		bind_sql_valor_p := sql_pck.bind_variable(':dt_val_inic_w', dt_val_inic_w, bind_sql_valor_p);
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION pls_oc_cta_obter_rest_val_23 ( dados_validacao_p pls_tipos_ocor_pck.dados_regra_val_util_item, dados_regra_p pls_tipos_ocor_pck.dados_regra, dados_conta_proc_p pls_tipos_ocor_pck.dados_conta_proc, dados_conta_mat_p pls_tipos_ocor_pck.dados_conta_mat, ie_proc_mat_p text, nr_seq_selecao_p pls_oc_cta_selecao_ocor_v.nr_sequencia%type, bind_sql_valor_p INOUT sql_pck.t_dado_bind) FROM PUBLIC;
