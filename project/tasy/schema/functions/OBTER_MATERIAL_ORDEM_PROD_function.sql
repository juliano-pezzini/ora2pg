...
CREATE OR REPLACE FUNCTION obter_material_ordem_prod (nr_seq_ordem_p bigint, ie_opcao_p text, nr_sequencia_p can_ordem_item_prescr.nr_sequencia%type default null) RETURNS varchar AS $body$
DECLARE
...
select	coalesce(nr_sequencia_p, min(nr_sequencia))
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_material_ordem_prod (nr_seq_ordem_p bigint, ie_opcao_p text, nr_sequencia_p can_ordem_item_prescr.nr_sequencia%type default null) FROM PUBLIC;
