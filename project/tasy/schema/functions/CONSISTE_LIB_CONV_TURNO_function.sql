...
CREATE OR REPLACE FUNCTION consiste_lib_conv_turno (cd_agenda_p bigint, cd_convenio_p bigint, hr_inicial_p timestamp, cd_categoria_p text, nr_seq_origem_p bigint default null) RETURNS varchar AS $body$
DECLARE
...
and		coalesce(cd_categoria_padrao,cd_categoria_p) = cd_categoria_p;

cd_categoria_w	:= coalesce(cd_categoria_p,'X');
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION consiste_lib_conv_turno (cd_agenda_p bigint, cd_convenio_p bigint, hr_inicial_p timestamp, cd_categoria_p text, nr_seq_origem_p bigint default null) FROM PUBLIC;
