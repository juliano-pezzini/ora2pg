...
CREATE OR REPLACE FUNCTION obter_desc_unid_rn ( nr_atendimento_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (coalesce(nr_atendimento_p,0) > 0) then
	...
	begin
	select	max(e.cd_unidade_basica||' '||e.cd_unidade_compl)
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_desc_unid_rn ( nr_atendimento_p bigint) FROM PUBLIC;
