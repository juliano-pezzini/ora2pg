...
CREATE OR REPLACE FUNCTION obter_tempo_prioridade_pep_lab (nr_prescricao_p bigint, nr_seq_prescricao_p bigint, nr_seq_prescricao_med_p bigint, ds_titulo_laudo_p text) RETURNS varchar AS $body$
DECLARE
...
nr_seq_prioridade_w := obter_prioridade_pep(nr_prescricao_p,nr_seq_prescricao_p,ds_titulo_laudo_p);
  ...
  select coalesce(Obter_data_aprov_lab(nr_seq_prescricao_med_p, nr_seq_prescricao_p),to_date('01/01/0001','dd/MM/yyyy'))
  into STRICT    dt_execucao_w
;

  if (dt_execucao_w = to_date('01/01/0001','dd/MM/yyyy')) then
    return 0;
  end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_tempo_prioridade_pep_lab (nr_prescricao_p bigint, nr_seq_prescricao_p bigint, nr_seq_prescricao_med_p bigint, ds_titulo_laudo_p text) FROM PUBLIC;
