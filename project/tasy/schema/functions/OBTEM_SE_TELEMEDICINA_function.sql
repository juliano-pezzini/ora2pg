...
CREATE OR REPLACE FUNCTION obtem_se_telemedicina (NR_SEQ_AGECONS_P integer) RETURNS varchar AS $body$
DECLARE
...
if (obter_se_util_integra(coalesce(wheb_usuario_pck.get_cd_estabelecimento,1), 'TELE') = 'S') then
   ...
   INNER JOIN AGENDA_CLASSIF B ON A.IE_CLASSIF_AGENDA = B.CD_CLASSIFICACAO
	...
	IF (IE_UTILIZA_TELEMEDICINA_W = 'S') THEN
	  SELECT (CASE WHEN (MAX(DS_URL_TELEMEDICINA) IS NOT NULL AND (MAX(DS_URL_TELEMEDICINA))::text <> '') THEN 'S' ELSE 'N' END) IE_INTEGRA_TELEMEDICINA
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obtem_se_telemedicina (NR_SEQ_AGECONS_P integer) FROM PUBLIC;
