...
c02 CURSOR FOR
	...
	and    	c.dt_horario 		between(dt_horario_w) + (1/86000) and (dt_horario_w + 1) - (1/86000)
...
BEGIN
	...
	if (ds_horarios_w <> 'ACM') and (ds_horarios_w <> 'SN') then
		...
		select	reverter_horarios(padroniza_horario_prescr(ds_horarios_w, to_char(dt_inicio_prescr_w,'dd/mm/yyyy hh24:mi:ss')) || ' ')
...
||to_char(coalesce(qt_total_dose_adm_w,0));
				...
				if (ds_retorno_w = 'S') then
					...
					if (hr_dose_especial_ww IS NOT NULL AND hr_dose_especial_ww::text <> '') and (coalesce(qt_dose_especial_w,0) > 0) and
					   (hr_dose_especial_www between(dt_horario_w) + (1/86000) and (dt_horario_w + 1) - (1/86000)) then
