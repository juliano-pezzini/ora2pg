...
CREATE OR REPLACE FUNCTION pls_oc_cta_val_23_mat_imp ( dados_validacao_p pls_ocor_imp_pck.dados_regra_val_util_item, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, dados_conta_mat_p pls_ocor_imp_pck.dados_conta_mat, nr_seq_protocolo_p pls_protocolo_conta_imp.nr_sequencia%type) RETURNS varchar AS $body$
DECLARE


ds_observacao_w		varchar(32000);
dados_outro_mat_w	pls_ocor_imp_pck.dados_conta_mat;
...
if (nr_id_transacao_p IS NOT NULL AND nr_id_transacao_p::text <> '') then
	...
	bind_sql_valor_w := pls_oc_cta_obter_restr_23_imp(	dados_validacao_p, null, dados_conta_mat_p, 'M', bind_sql_valor_w);
	...
	ds_sql_w :=	'select	outro_mat.nr_seq_conta, 		' || pls_util_pck.enter_w ||
			...
			'	outro_mat.qt_executado qt_material, 	' || pls_util_pck.enter_w ||
			ds_campo_sql_w ||
			...
			'from	pls_conta_mat_ocor_imp_v outro_mat, 	' || pls_util_pck.enter_w ||
			'	pls_conta_imp conta 			' || pls_util_pck.enter_w ||
			'where	1 = 1 					' || pls_util_pck.enter_w ||
			'and	outro_mat.nr_seq_conta = conta.nr_sequencia '|| pls_util_pck.enter_w ||
			'and	conta.nr_seq_protocolo = '|| nr_seq_protocolo_p ||''|| pls_util_pck.enter_w ||
			dados_restricao_w.ds_restricao_mat 		  || pls_util_pck.enter_w ||
			' union all ' 					 || pls_util_pck.enter_w ||
			'select	outro_mat.nr_seq_conta, 		' || pls_util_pck.enter_w ||
			'	outro_mat.nr_sequencia, 		' || pls_util_pck.enter_w ||
			'	outro_mat.cd_guia_referencia, 		' || pls_util_pck.enter_w ||
			...
			'where	1 = 1 					' || pls_util_pck.enter_w ||
			dados_restricao_w.ds_restricao_mat 		  || pls_util_pck.enter_w ||
			'order by dt_atendimento, qt_material desc ';
	...
	loop
		...
		if (ie_valido_extra_w = 'S') then
			...
			ds_observacao_w := substr(ds_observacao_w || pls_util_pck.enter_w ||
					   ...
					   ' | Mat: ' || dados_outro_mat_w.nr_seq_conta_mat ||
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION pls_oc_cta_val_23_mat_imp ( dados_validacao_p pls_ocor_imp_pck.dados_regra_val_util_item, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, dados_conta_mat_p pls_ocor_imp_pck.dados_conta_mat, nr_seq_protocolo_p pls_protocolo_conta_imp.nr_sequencia%type) FROM PUBLIC;
