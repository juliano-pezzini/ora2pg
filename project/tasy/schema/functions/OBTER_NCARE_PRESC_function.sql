...
CREATE OR REPLACE FUNCTION obter_ncare_presc ( CD_PESSOA_FISICA_p text, DT_ATUALIZACAO_p timestamp, start_time_p timestamp, end_time_p timestamp ) RETURNS varchar AS $body$
DECLARE

    ds_others_w varchar(2500);

BEGIN
  select   rtrim( XMLAGG(XMLELEMENT(name E,prescript, chr(10))).EXTRACT['//text()'].getclobval(), ',')
...
WHERE
   ...
   and to_date(c.DT_INICIO,'dd-mm-yy')=to_date(DT_ATUALIZACAO_p,'dd-mm-yy')) alias29
;
 RETURN ds_others_w;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_ncare_presc ( CD_PESSOA_FISICA_p text, DT_ATUALIZACAO_p timestamp, start_time_p timestamp, end_time_p timestamp ) FROM PUBLIC;
