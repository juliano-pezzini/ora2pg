...
CREATE OR REPLACE FUNCTION obter_lib_proc_hor_gpt ( ie_tipo_usuario_p text, nm_tabela_p text, nr_seq_item_p bigint) RETURNS varchar AS $body$
DECLARE
...
BEGIN
  ...
  if (nr_prescricao_w IS NOT NULL AND nr_prescricao_w::text <> '') then
    begin
		if (nm_tabela_p = 'CPOE_ANATOMIA_PATOLOGICA') then
		  ...
		  and nr_sequencia          = nr_seq_item_p;       	
			...
			if (ie_liberacao_w = 'N') then
				...
				where (b.nr_prescricao, b.nr_sequencia_proc) in (
					...
					where	y.nr_seq_proc_cpoe = nr_seq_item_p
					and 	y.nr_prescricao = nr_prescricao_w
					and 	coalesce(y.dt_suspensao::text, '') = ''
				)
				and		(
					(ie_tipo_usuario_p = 'E' and coalesce(dt_lib_enfermagem::text, '') = '') or (ie_tipo_usuario_p = 'F' and coalesce(dt_lib_farmacia::text, '') = '' and (dt_lib_enfermagem IS NOT NULL AND dt_lib_enfermagem::text <> ''))
				)
				and 	coalesce(b.ie_checar_adep, 'N') = 'N'
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_lib_proc_hor_gpt ( ie_tipo_usuario_p text, nm_tabela_p text, nr_seq_item_p bigint) FROM PUBLIC;
