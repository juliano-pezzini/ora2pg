...
CREATE OR REPLACE FUNCTION obter_desc_ativ_desenv_trad ( nr_seq_atividade_p bigint, nr_seq_localizacao_p bigint) RETURNS varchar AS $body$
DECLARE
...
cd_expressao_w			des_ativ_exec.cd_exp_atividade%type;


BEGIN
if (nr_seq_atividade_p IS NOT NULL AND nr_seq_atividade_p::text <> '') then
	select	max(ds_atividade),
		max(cd_exp_atividade)
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_desc_ativ_desenv_trad ( nr_seq_atividade_p bigint, nr_seq_localizacao_p bigint) FROM PUBLIC;
