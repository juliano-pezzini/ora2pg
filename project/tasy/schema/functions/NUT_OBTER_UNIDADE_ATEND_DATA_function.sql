...
CREATE OR REPLACE FUNCTION nut_obter_unidade_atend_data ( nr_atendimento_p bigint, cd_setor_atendimento_p bigint, dt_referencia_p timestamp) RETURNS varchar AS $body$
DECLARE
...
if (coalesce(nr_atendimento_p,0) > 0) then 
	...
	and 	nr_seq_interno = (	SELECT 	min(coalesce(nr_seq_interno,0)) 
					...
					and	b.dt_saida_unidade >= dt_referencia_p 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION nut_obter_unidade_atend_data ( nr_atendimento_p bigint, cd_setor_atendimento_p bigint, dt_referencia_p timestamp) FROM PUBLIC;
