...
CREATE OR REPLACE FUNCTION obter_se_proc_mesmo_nivel ( nr_seq_aprovacao_p bigint, nr_seq_proc_aprov_p bigint, cd_estabelecimento_p bigint, cd_pessoa_fisica_p text, ie_somente_pendente_p text default 'S') RETURNS varchar AS $body$
DECLARE
...
/* Processo por Nivel, ou Nivel com valores */
...
if (ie_aprovacao_nivel_w in ('S', 'A')) then
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		if (ie_mesmo_nivel_w = 'N') then
			...
			/*Identifica que o aprovador e do mesmo nivel do processo que este pendente
...
SECURITY DEFINER
 IMMUTABLE;
-- REVOKE ALL ON FUNCTION obter_se_proc_mesmo_nivel ( nr_seq_aprovacao_p bigint, nr_seq_proc_aprov_p bigint, cd_estabelecimento_p bigint, cd_pessoa_fisica_p text, ie_somente_pendente_p text default 'S') FROM PUBLIC;
