...
CREATE OR REPLACE FUNCTION ehr_paciente_historia_familiar ( nr_seq_reg_elemento_p bigint, nr_seq_entidade_p bigint, nr_seq_elemento_p bigint) RETURNS varchar AS $body$
DECLARE
...
ds_comando_w	:= 'Select '||ds_comando_w ||' ds '||chr(13) ||'From paciente_antec_clinico '|| chr(13) || ' where nvl(ie_paciente,''S'') = ''N''';
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ehr_paciente_historia_familiar ( nr_seq_reg_elemento_p bigint, nr_seq_entidade_p bigint, nr_seq_elemento_p bigint) FROM PUBLIC;
