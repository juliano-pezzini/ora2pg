...
CREATE OR REPLACE FUNCTION obter_periodo_os_defeito ( nr_seq_ordem_serv_p bigint) RETURNS varchar AS $body$
DECLARE
...
BEGIN
    ...
    from (
      SELECT 
        max(rel.target_response) target_response
        ,max(rel.target_resolution) target_resolution
        ,max(sum(rel.stage_time)) so_resolution_time
      from (
        SELECT 
	  ...
	  order by coalesce(msr.ie_prioridade, ' '), msr.nr_seq_sla
      ) rel
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_periodo_os_defeito ( nr_seq_ordem_serv_p bigint) FROM PUBLIC;
