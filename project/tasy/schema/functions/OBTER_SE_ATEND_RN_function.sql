...
CREATE OR REPLACE FUNCTION obter_se_atend_rn (nr_atendimento_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (qt_reg_w	> 0) then
	...
	if (qt_reg_w	> 0) then
		...
		if (qt_idade_w IS NOT NULL AND qt_idade_w::text <> '') and (qt_idade_w	= 0) then
			return	'S';
		end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_atend_rn (nr_atendimento_p bigint) FROM PUBLIC;
