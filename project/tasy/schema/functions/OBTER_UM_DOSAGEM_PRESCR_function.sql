...
CREATE OR REPLACE FUNCTION obter_um_dosagem_prescr ( nr_prescricao_p bigint, nr_sequencia_p bigint) RETURNS varchar AS $body$
DECLARE
...
if	((qt_dose_w IS NOT NULL AND qt_dose_w::text <> '') and (coalesce(pkg_i18n.get_user_locale, 'pt_BR') = 'pt_BR') and (cd_unidade_medida_dose_w = 'UI') and (obter_se_campo_numerico(qt_dose_w) = 'S')) then
	qt_decimais_dose_w := 0;
	
	if (trunc(qt_dose_w) <> qt_dose_w) then
		select  least(length(trunc(reverse(to_char(abs(qt_dose_w))))), 5)
		into STRICT    qt_decimais_dose_w
		;
	end if;
	
	qt_dose_w	:= formata_casas_decimais(qt_dose_w, qt_decimais_dose_w);
end if;

/* Ivan em 08/02/2008 OS82257 - Incluida consistencia para comparar o qt_dosagem_w somente no agrupador 1 */
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_um_dosagem_prescr ( nr_prescricao_p bigint, nr_sequencia_p bigint) FROM PUBLIC;
