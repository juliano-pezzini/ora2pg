...
CREATE OR REPLACE FUNCTION html_obter_desc_grupo_ou_perg ( nr_seq_grupo_p bigint, nr_seq_pergunta_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (nr_seq_grupo_p IS NOT NULL AND nr_seq_grupo_p::text <> '') then
	...
	select	obter_char_repetido(160, 8) || ds_pergunta
	into STRICT	ds_result_w
	from	avf_pergunta
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION html_obter_desc_grupo_ou_perg ( nr_seq_grupo_p bigint, nr_seq_pergunta_p bigint) FROM PUBLIC;
