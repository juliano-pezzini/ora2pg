...
CREATE OR REPLACE FUNCTION obter_total_tit_transferidos (cd_pessoa_p bigint, tp_pessoa_p bigint, nm_usuario_p text) RETURNS bigint AS $body$
DECLARE
 vl_transferido double precision;

BEGIN
	select coalesce(sum(a.vl_titulo),0)
	into STRICT vl_transferido
	from	titulo_pagar a
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_total_tit_transferidos (cd_pessoa_p bigint, tp_pessoa_p bigint, nm_usuario_p text) FROM PUBLIC;
