...
CREATE OR REPLACE FUNCTION obter_se_permite_alterar_eif ( nr_seq_escala_p bigint) RETURNS varchar AS $body$
DECLARE
...
select	coalesce(max(ie_permite_insercao),'S') 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_permite_alterar_eif ( nr_seq_escala_p bigint) FROM PUBLIC;
