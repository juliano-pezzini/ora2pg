...
CREATE OR REPLACE FUNCTION obter_se_gqa_regra_liberada ( nr_seq_gqa_regra_p bigint) RETURNS varchar AS $body$
DECLARE
...
BEGIN
	if (nr_seq_gqa_regra_p IS NOT NULL AND nr_seq_gqa_regra_p::text <> '') then
		select	count(*)
		into STRICT	qt_registro_w
		from	gqa_pendencia_regra_lib
		where	nr_seq_gqa_regra = nr_seq_gqa_regra_p
		and		ie_situacao = 'A';
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_gqa_regra_liberada ( nr_seq_gqa_regra_p bigint) FROM PUBLIC;
