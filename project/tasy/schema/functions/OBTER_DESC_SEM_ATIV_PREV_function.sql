...
CREATE OR REPLACE FUNCTION obter_desc_sem_ativ_prev ( nr_seq_localizacao_p bigint) RETURNS varchar AS $body$
DECLARE
...
BEGIN
ds_retorno_w	:= wheb_mensagem_pck.get_texto(337219);
...
if (nr_seq_localizacao_p IS NOT NULL AND nr_seq_localizacao_p::text <> '') then
	...
	if (cd_cnpj_w IS NOT NULL AND cd_cnpj_w::text <> '') then
		...
		if (nr_seq_idioma_w IS NOT NULL AND nr_seq_idioma_w::text <> '') then
			ds_retorno_w	:= obter_desc_expressao_idioma(	668408,
									ds_retorno_w,
									nr_seq_idioma_w);
		end if;
	end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_desc_sem_ativ_prev ( nr_seq_localizacao_p bigint) FROM PUBLIC;
