...
CREATE OR REPLACE FUNCTION man_obter_min_com ( cd_estabelecimento_p bigint, dt_inicio_p timestamp, dt_fim_p timestamp) RETURNS bigint AS $body$
DECLARE
...
else	/*Se as datas são no mesmo dia, apenas contabiliza os minutos*/
	begin
	if (pkg_date_utils.IS_BUSINESS_DAY(dt_ref_w) = 0) then
		qt_min_w		:= 0;
	elsif (obter_se_feriado(cd_estabelecimento_p, dt_ref_w) > 0) then
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION man_obter_min_com ( cd_estabelecimento_p bigint, dt_inicio_p timestamp, dt_fim_p timestamp) FROM PUBLIC;
