-- Generated by Ora2Pg, the Oracle database Schema converter, version 23.1
-- Copyright 2000-2022 Gilles DAROLD. All rights reserved.
-- DATASOURCE: dbi:Oracle:host=srv-dbora-03.whebdc.com.br;service_name=DEV_1815;port=1521

SET client_encoding TO 'UTF8';





CREATE OR REPLACE FUNCTION pls_obter_tipo_intercamb_coop ( nr_seq_requisicao_p bigint, nr_seq_guia_p bigint) RETURNS varchar AS $body$
DECLARE


ds_retorno_w			varchar(2);
nr_seq_segurado_w		bigint;
nr_seq_congenere_w		bigint;


BEGIN

if (nr_seq_requisicao_p IS NOT NULL AND nr_seq_requisicao_p::text <> '') then
	select	nr_seq_segurado
	into STRICT	nr_seq_segurado_w
	from	pls_requisicao
	where	nr_sequencia	= nr_seq_requisicao_p;

	select	max(a.nr_seq_congenere)
	into STRICT	nr_seq_congenere_w
	from	pls_segurado_carteira	b,
		pls_segurado		a
	where	a.nr_sequencia	= nr_seq_segurado_w
	and	a.nr_sequencia	= b.nr_seq_segurado;
elsif (nr_seq_guia_p IS NOT NULL AND nr_seq_guia_p::text <> '') then
	select	nr_seq_segurado
	into STRICT	nr_seq_segurado_w
	from	pls_guia_plano
	where	nr_sequencia	= nr_seq_guia_p;

	select	max(a.nr_seq_congenere)
	into STRICT	nr_seq_congenere_w
	from	pls_segurado_carteira	b,
		pls_segurado		a
	where	a.nr_sequencia	= nr_seq_segurado_w
	and	a.nr_sequencia	= b.nr_seq_segurado;
end if;

select	coalesce(ie_tipo_intercambio,'I')
into STRICT	ds_retorno_w
from	pls_cong_autor_intercambio
where	nr_seq_congenere	= nr_seq_congenere_w;

return	ds_retorno_w;

end;
$body$
LANGUAGE PLPGSQL
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION pls_obter_tipo_intercamb_coop ( nr_seq_requisicao_p bigint, nr_seq_guia_p bigint) FROM PUBLIC;

