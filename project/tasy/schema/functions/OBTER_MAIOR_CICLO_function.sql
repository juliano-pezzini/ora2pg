...
CREATE OR REPLACE FUNCTION obter_maior_ciclo ( nr_sequencia_p bigint) RETURNS varchar AS $body$
DECLARE
...
retorno_ds      	varchar(1000);
nr_ciclos_setor_w	bigint;

c01 CURSOR FOR
	SELECT  CASE WHEN PM.DS_CICLOS_APLICACAO='' THEN ''  ELSE PM.DS_CICLOS_APLICACAO||',' END  ||
	CASE WHEN SOL.DS_CICLOS_APLICACAO='' THEN ''  ELSE SOL.DS_CICLOS_APLICACAO||',' END  ||
	CASE WHEN PP.DS_CICLOS_APLICACAO='' THEN ''  ELSE PP.DS_CICLOS_APLICACAO||',' END  ||
	CASE WHEN PR.DS_CICLOS_APLICACAO='' THEN ''  ELSE PR.DS_CICLOS_APLICACAO||',' END
	DS_CICLOS_APLICACAO FROM paciente_setor ps
...
BEGIN
	...
	begin
		ds_retorno_w := substr(ds_retorno_w || retorno_ds, 1, 1000);
	end;
	end loop;
	close C01;
	...
	while	posicao_w > 0 loop
		begin
			posicao_w	:= position(',' in ds_retorno_w);
			z := z + 1;
			ciclos_w[z].ds_ciclo	:= trim(both substr(ds_retorno_w,1,posicao_w - 1));
	...
	if (coalesce(Maior_ds::text, '') = '') or (Maior_ds = '') then
		...
		into STRICT	nr_ciclos_setor_w
		from	paciente_setor
		where	nr_seq_paciente = nr_sequencia_p;

		if (nr_ciclos_setor_w > 0) then
			Maior_ds := to_char(nr_ciclos_setor_w);
		else
			Maior_ds := '99999';
		end if;
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION obter_maior_ciclo ( nr_sequencia_p bigint) FROM PUBLIC;
