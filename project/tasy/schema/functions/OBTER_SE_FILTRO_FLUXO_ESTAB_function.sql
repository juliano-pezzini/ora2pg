...
CREATE OR REPLACE FUNCTION obter_se_filtro_fluxo_estab ( nr_seq_conta_banco_p bigint, nr_seq_cheque_p bigint, nr_seq_caixa_rec_p bigint, nm_usuario_p text, nr_seq_caixa_p bigint, cd_estabelecimento_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (qt_banco_w > 0) or (qt_caixa_w > 0) or (qt_estab_w > 0) then
	...
	if	((qt_banco_ok_w > 0) or (qt_caixa_ok_w > 0) or
		((qt_estab_ok_w > 0) and (qt_caixa_w = 0) and (qt_banco_w = 0))) and
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_filtro_fluxo_estab ( nr_seq_conta_banco_p bigint, nr_seq_cheque_p bigint, nr_seq_caixa_rec_p bigint, nm_usuario_p text, nr_seq_caixa_p bigint, cd_estabelecimento_p bigint) FROM PUBLIC;
