...
CREATE OR REPLACE FUNCTION eis_obter_porte_proc_anest (nr_cirurgia_p bigint, nr_seq_agenda_p bigint, nr_seq_interno_p bigint default null) RETURNS varchar AS $body$
DECLARE
...
if (cd_porte_w IS NOT NULL AND cd_porte_w::text <> '') then 
	ds_retorno_w := substr('Proc: '||cd_porte_w,1,255);
end if;	
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eis_obter_porte_proc_anest (nr_cirurgia_p bigint, nr_seq_agenda_p bigint, nr_seq_interno_p bigint default null) FROM PUBLIC;
