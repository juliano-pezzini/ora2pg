...
CREATE OR REPLACE FUNCTION obter_estrutura_substancia ( cd_substacia_atc_p text) RETURNS varchar AS $body$
DECLARE
...
ds_retorno_w	:= ds_grupo_anatomico_w || chr(13) || chr(10);
ds_retorno_w	:= ds_retorno_w ||'   '|| ds_classe_terapeutica_w ||chr(13) || chr(10);
ds_retorno_w	:= ds_retorno_w ||'      '|| ds_subclasse_terapeutica_w ||chr(13) || chr(10);
ds_retorno_w	:= ds_retorno_w ||'         '|| ds_grupo_quimico_w ||chr(13) || chr(10);
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_estrutura_substancia ( cd_substacia_atc_p text) FROM PUBLIC;
