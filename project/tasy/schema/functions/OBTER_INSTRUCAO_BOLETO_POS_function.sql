...
CREATE OR REPLACE FUNCTION obter_instrucao_boleto_pos ( nr_titulo_p bigint, cd_banco_p bigint, nr_linha_p bigint, ie_posicao_p text, nr_seq_conta_proc_p bigint, nr_seq_conta_mat_p bigint) RETURNS varchar AS $body$
DECLARE
...
/*INICIO - Macros replicadas da function OBTER_INSTRUCAO_BOLETO_TAM. Apenas copiado de la pra ca.
Caso ocorra defeito de mensagem incorreta, deve ser verificado o autor das mesmas na OBTER_INSTRUCAO_BOLETO_TAM*/
...
if (nr_titulo_p IS NOT NULL AND nr_titulo_p::text <> '') and (cd_banco_p IS NOT NULL AND cd_banco_p::text <> '') and (nr_linha_p IS NOT NULL AND nr_linha_p::text <> '') then
	...
	/*Essa macro é fixa conforme OS 799927. Nao existia essa macro nessa rotina, apenas na OBTER_INSTRUCAO_BOLETO_TAM, e repliquei a mesma para ca. */
	...
	and	a.nr_seq_segurado		= nr_seq_segurado_w
	and	a.nr_seq_mensalidade	= nr_seq_mensalidade_w
	and	PKG_DATE_UTILS.start_of(c.dt_contrato,'mm', 0) 	=  PKG_DATE_UTILS.start_of(PKG_DATE_UTILS.ADD_MONTH(clock_timestamp(),1, 0),'mm', 0);
	exception when others then
	...
	ds_coparticipacao_w	:= ' COPARTICIPAÇÃO '; /*Essa macro é fixa conforme OS 799927. Nao existia essa macro nessa rotina, apenas na OBTER_INSTRUCAO_BOLETO_TAM, e repliquei a mesma para ca. */
...
/*fim 950304*/
	...
	if (ds_instrucao_w IS NOT NULL AND ds_instrucao_w::text <> '') then
		...
		ds_instrucao_w	:= replace_macro(ds_instrucao_w,'@MES_ANO_EMISSAO',substr(obter_data_extenso(dt_emissao_w,7),1,255));
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_instrucao_boleto_pos ( nr_titulo_p bigint, cd_banco_p bigint, nr_linha_p bigint, ie_posicao_p text, nr_seq_conta_proc_p bigint, nr_seq_conta_mat_p bigint) FROM PUBLIC;
