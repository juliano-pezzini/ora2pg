...
CREATE OR REPLACE FUNCTION pls_obter_se_valor_calc_origem ( nr_seq_conta_p bigint, ie_origem_valor_p text, vl_minimo_p bigint) RETURNS varchar AS $body$
DECLARE


vl_calculado_proc_w		double precision:= 0;
vl_calculado_mat_w		double precision:= 0;
...
if (nr_seq_conta_p IS NOT NULL AND nr_seq_conta_p::text <> '') then
	...
	if (ie_origem_valor_p = 'G') then
		...
		if (coalesce(nr_Seq_segurado_w,0) > 0) then
			...
			and	coalesce(a.cd_guia_referencia,a.cd_guia)	= coalesce(cd_guia_referencia_w,cd_guia_w)
			and	a.ie_glosa	= 'N';
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION pls_obter_se_valor_calc_origem ( nr_seq_conta_p bigint, ie_origem_valor_p text, vl_minimo_p bigint) FROM PUBLIC;
