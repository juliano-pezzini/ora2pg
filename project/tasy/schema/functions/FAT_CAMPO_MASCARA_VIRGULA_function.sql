...
CREATE OR REPLACE FUNCTION fat_campo_mascara_virgula (vl_campo_p bigint) RETURNS varchar AS $body$
DECLARE
...
IF (vl_campo_p IS NOT NULL AND vl_campo_p::text <> '') THEN
   ...
   IF (coalesce(PHILIPS_PARAM_PCK.GET_NR_SEQ_IDIOMA, 1) <> 1 AND (PKG_I18N.GET_USER_LOCALE <> 'es_BO') AND (PKG_I18N.GET_USER_LOCALE <> 'es_AR')) THEN
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION fat_campo_mascara_virgula (vl_campo_p bigint) FROM PUBLIC;
