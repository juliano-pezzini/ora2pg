...
CREATE OR REPLACE FUNCTION qua_obter_result_avaliacao ( nr_seq_avaliacao_p bigint, nr_seq_item_p bigint, nr_seq_tipo_avaliacao_p bigint, nr_seq_evento_aval_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (ie_tipo_item_w in ('Z','C','D','A','O','R')) then
	...
	from	qua_evento_pac_aval_result a,
		qua_evento_pac_aval b
	where	b.nr_sequencia = nr_seq_avaliacao_p
	and	b.nr_seq_evento = nr_seq_evento_aval_p
	and	a.nr_seq_evento_aval = b.nr_sequencia
	...
	if (ie_tipo_item_w = 'B') then
		if (qt_resultado_aval_w = 1) then
			ds_resultado_w	:= substr(wheb_mensagem_pck.get_texto(300971),1,10); /*Sim*/
		else
			ds_resultado_w	:= substr(wheb_mensagem_pck.get_texto(300972),1,10); /*Não*/
		end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION qua_obter_result_avaliacao ( nr_seq_avaliacao_p bigint, nr_seq_item_p bigint, nr_seq_tipo_avaliacao_p bigint, nr_seq_evento_aval_p bigint) FROM PUBLIC;
