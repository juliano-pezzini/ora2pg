...
CREATE OR REPLACE FUNCTION obter_setor_atend_proc_estab ( cd_procedimento_p bigint, ie_origem_proced_p bigint, cd_setor_atend_p bigint, cd_setor_origem_p bigint, nm_usuario_p text, nr_seq_proc_interno_p bigint, nr_atendimento_p bigint) RETURNS bigint AS $body$
DECLARE
...
if (coalesce(cd_setor_atendimento_w::text, '') = '') and (nr_seq_proc_interno_p > 0) then
	...
	select	Obter_Setor_exec_proc_interno(nr_seq_proc_interno_p,cd_setor_origem_p, ie_tipo_atendimento_w, cd_convenio_w, cd_categoria_w)
	into STRICT	cd_setor_atendimento_w
	;
end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_setor_atend_proc_estab ( cd_procedimento_p bigint, ie_origem_proced_p bigint, cd_setor_atend_p bigint, cd_setor_origem_p bigint, nm_usuario_p text, nr_seq_proc_interno_p bigint, nr_atendimento_p bigint) FROM PUBLIC;
