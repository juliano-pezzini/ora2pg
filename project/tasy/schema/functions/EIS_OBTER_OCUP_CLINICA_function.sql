...
CREATE OR REPLACE FUNCTION eis_obter_ocup_clinica ( ie_clinica_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estabelecimento_p bigint ) RETURNS bigint AS $body$
DECLARE
...
and (coalesce(ie_clinica_p::text, '') = '') then 
	...
	and	ie_periodo = 'D' 
	and	cd_estabelecimento = cd_estabelecimento_p 
	and	((ie_clinica = ie_clinica_p) or (coalesce(ie_clinica_p::text, '') = ''));
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eis_obter_ocup_clinica ( ie_clinica_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estabelecimento_p bigint ) FROM PUBLIC;
