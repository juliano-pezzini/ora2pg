...
CREATE OR REPLACE FUNCTION allow_outpatient_same_day ( nr_seq_tipo_admissao_p subtipo_episodio.nr_seq_tipo_admissao%type, nr_case_p bigint, nr_atendimento_p bigint, dt_entrada_p timestamp) RETURNS varchar AS $body$
DECLARE
...
if ((nr_seq_tipo_admissao_p IS NOT NULL AND nr_seq_tipo_admissao_p::text <> '') and
	...
	if ((ie_tipo_episodio_w = 1) and (coalesce(pkg_i18n.get_user_locale, 'pt_BR') in ('de_DE', 'de_AT'))) then
		
		select	coalesce(max('S'), 'N')
		into STRICT	ie_has_rule_w
		from 	subtipo_episodio sep,
				tipo_admissao_fat taf
		where 	sep.nr_seq_tipo_admissao = taf.nr_sequencia
		and		sep.nr_seq_tipo_episodio = nr_seq_tipo_episodio_w	
		and 	sep.nr_seq_tipo_admissao = nr_seq_tipo_admissao_p
		and		sep.ie_permite_internar <> 3;
		
		if (ie_has_rule_w = 'S') then
			...
			for C01_W in C01(nr_case_p, nr_atendimento_p) loop
				...
				if (ie_permite_internar_w IS NOT NULL AND ie_permite_internar_w::text <> '') then
						...
						if (ie_permite_internar_w = 1 and (nr_seq_tipo_admissao_p <> C01_W.nr_seq_tipo_admissao_fat and C01_W.ie_tipo_atendimento <> 8)
							and trunc(dt_entrada_p) > trunc(C01_W.dt_entrada)) then
							 ds_retorno_w := 'N';
						elsif   (ie_permite_internar_w = 2 and ((nr_seq_tipo_admissao_p <> C01_W.nr_seq_tipo_admissao_fat and C01_W.ie_tipo_atendimento <> 8) or
									trunc(dt_entrada_p) = trunc(C01_W.dt_entrada)) and trunc(dt_entrada_p) >= trunc(C01_W.dt_entrada)) then
							 ds_retorno_w := 'N';
						end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION allow_outpatient_same_day ( nr_seq_tipo_admissao_p subtipo_episodio.nr_seq_tipo_admissao%type, nr_case_p bigint, nr_atendimento_p bigint, dt_entrada_p timestamp) FROM PUBLIC;
