...
CREATE OR REPLACE FUNCTION obter_status_hor_proced_lib ( dt_administracao_p timestamp, dt_suspensao_p timestamp, ie_dose_especial_p text, dt_inicio_proc_p timestamp, dt_lib_horario_p timestamp) RETURNS varchar AS $body$
DECLARE
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_status_hor_proced_lib ( dt_administracao_p timestamp, dt_suspensao_p timestamp, ie_dose_especial_p text, dt_inicio_proc_p timestamp, dt_lib_horario_p timestamp) FROM PUBLIC;
