...
CREATE OR REPLACE FUNCTION obter_se_hor_validade_rec ( nr_prescricao_p bigint, nr_seq_item_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (ds_horarios_w IS NOT NULL AND ds_horarios_w::text <> '') then
	while(ds_horarios_w IS NOT NULL AND ds_horarios_w::text <> '') LOOP
		...
		if (dt_horario_w < dt_prescr_w) then
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_hor_validade_rec ( nr_prescricao_p bigint, nr_seq_item_p bigint) FROM PUBLIC;
