...
CREATE OR REPLACE FUNCTION ageweb_obter_se_medico_turno (nr_seq_agenda_p bigint, cd_medico_p text) RETURNS varchar AS $body$
DECLARE
...
if (qt_reg_w > 0) then
	...
	and	((dt_dia_semana = ie_dia_semana_w) or ((dt_dia_semana = 9) and (pkg_date_utils.is_business_day(hr_inicio_w) = 1)))
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ageweb_obter_se_medico_turno (nr_seq_agenda_p bigint, cd_medico_p text) FROM PUBLIC;
