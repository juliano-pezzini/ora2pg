...
CREATE OR REPLACE FUNCTION qt_obter_regra_gerac_ordem ( cd_material_p bigint, ie_via_aplicacao_p text, qt_dose_p bigint, cd_inidade_medida_p text) RETURNS varchar AS $body$
DECLARE
...
if (cd_material_p IS NOT NULL AND cd_material_p::text <> '')	and (ie_via_aplicacao_p IS NOT NULL AND ie_via_aplicacao_p::text <> '')	and (qt_dose_p IS NOT NULL AND qt_dose_p::text <> '')	and (cd_inidade_medida_p IS NOT NULL AND cd_inidade_medida_p::text <> '') then
    ...
    if (qt_regra_w > 0) then
    	...
    	from	regra_geracao_ord_chegada
    	where	cd_material = cd_material_p
    	/*and		nvl(ie_via_aplicacao,ie_via_aplicacao_p) = ie_via_aplicacao_p    solicitação de retirada pelo cliente OS 594695 histórico 31/07/2013*/
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION qt_obter_regra_gerac_ordem ( cd_material_p bigint, ie_via_aplicacao_p text, qt_dose_p bigint, cd_inidade_medida_p text) FROM PUBLIC;
