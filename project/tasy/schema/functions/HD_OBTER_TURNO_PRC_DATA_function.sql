...
CREATE OR REPLACE FUNCTION hd_obter_turno_prc_data ( cd_pessoa_fisica_p text, dt_referencia_p timestamp, ie_opcao_p text) RETURNS varchar AS $body$
DECLARE

-- C - Código
-- D - Descrição
-- G - Descrição (Cadastro)
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION hd_obter_turno_prc_data ( cd_pessoa_fisica_p text, dt_referencia_p timestamp, ie_opcao_p text) FROM PUBLIC;
