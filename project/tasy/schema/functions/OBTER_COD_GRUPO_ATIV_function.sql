...
CREATE OR REPLACE FUNCTION obter_cod_grupo_ativ ( cd_estabelecimento_p bigint, nr_seq_nota_p text, ie_opcao_p text) RETURNS varchar AS $body$
DECLARE
...
union all
			...
			and	i.cd_estabelecimento = cd_estabelecimento_p
			and	i.nr_sequencia = nr_seq_nota_p
			and	(p.nr_seq_item_serv IS NOT NULL AND p.nr_seq_item_serv::text <> '')) alias6;
...
if (cd_material_w IS NOT NULL AND cd_material_w::text <> '') then
	...
	if (cod_serv_prest_proc_w IS NOT NULL AND cod_serv_prest_proc_w::text <> '') then
		...
		if (qt_possui_w > 0) then
			...
			select	max(coalesce(b.cd_prefeitura, b.cd_item_servico)),
				max(coalesce(substr(b.ds_item_servico, 1, 255), 0)),
				max(coalesce(a.cd_servico,0)),
				max(b.cd_item_servico)
			into STRICT	cd_item_servico_w,
				ds_item_servico_w,
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_cod_grupo_ativ ( cd_estabelecimento_p bigint, nr_seq_nota_p text, ie_opcao_p text) FROM PUBLIC;
