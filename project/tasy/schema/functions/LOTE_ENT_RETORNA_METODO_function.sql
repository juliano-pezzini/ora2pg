...
CREATE OR REPLACE FUNCTION lote_ent_retorna_metodo (nr_seq_ficha_p bigint, nr_seq_exame_p bigint) RETURNS varchar AS $body$
DECLARE
...
IF (nr_prescricao_w IS NOT NULL AND nr_prescricao_w::text <> '') AND (nr_seq_exame_p IS NOT NULL AND nr_seq_exame_p::text <> '') AND (nr_seq_prescr_w IS NOT NULL AND nr_seq_prescr_w::text <> '') THEN
	...
	if (nr_seq_metodo_w IS NOT NULL AND nr_seq_metodo_w::text <> '') then
		...
		select 	substr(Obter_Metodo_Exame_Lab(nr_seq_metodo_w,2),1,255)
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION lote_ent_retorna_metodo (nr_seq_ficha_p bigint, nr_seq_exame_p bigint) FROM PUBLIC;
