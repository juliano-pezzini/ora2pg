...
CREATE OR REPLACE FUNCTION obter_se_os_usuario ( nr_ordem_servico_p bigint, nm_usuario_p text, ie_opcao_p text) RETURNS varchar AS $body$
DECLARE
...
if (ie_opcao_p = 'MU') then
	select	obter_se_usuario_exec(nr_ordem_servico_p, nm_usuario_p, 0)
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_os_usuario ( nr_ordem_servico_p bigint, nm_usuario_p text, ie_opcao_p text) FROM PUBLIC;
