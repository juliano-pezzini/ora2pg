...
CREATE OR REPLACE FUNCTION hupa_obter_valor_ultima_compra ( cd_estabelecimento_p bigint, qt_dia_p bigint, cd_material_p bigint, cd_local_estoque_p bigint, ie_tipo_p text) RETURNS bigint AS $body$
DECLARE
...
if (ie_forma_preco_ult_compra_w = 'S') then 
	...
	if (coalesce(cd_local_estoque_p, 0) = 0) then 
		...
		and	b.nr_sequencia in (	SELECT max(b.nr_sequencia) 
						from	natureza_operacao o, 
						operacao_nota p, 
						nota_fiscal b, 
						nota_fiscal_item a 
	...
	and 	substr(obter_tipo_tributo(cd_tributo),1,3) = 'IPI';
...
vl_retorno_w := coalesce(vl_retorno_w,0) + coalesce(vl_ipi_w,0);
 
--vl_retorno_w := nvl(vl_retorno_w,0); 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION hupa_obter_valor_ultima_compra ( cd_estabelecimento_p bigint, qt_dia_p bigint, cd_material_p bigint, cd_local_estoque_p bigint, ie_tipo_p text) FROM PUBLIC;
