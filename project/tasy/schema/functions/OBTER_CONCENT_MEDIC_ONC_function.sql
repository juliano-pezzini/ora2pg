...
CREATE OR REPLACE FUNCTION obter_concent_medic_onc ( nr_seq_atendimento_p bigint, nr_seq_material_p bigint, ie_opcao_p text) RETURNS varchar AS $body$
DECLARE
...
select	max(cd_material),
		...
		max(coalesce(obter_conversao_ml(cd_material, qt_dose_prescricao,CD_UNID_MED_PRESCR),0)),
...
select	max(coalesce(CASE WHEN upper(CD_UNID_MED_BASE_CONC)=obter_unid_med_usua('ML') THEN  coalesce(QT_CONVERSAO_MG,0)  ELSE dividir(QT_CONVERSAO_MG, qt_conversao_w) END ,0))
into STRICT	qt_concentracao_w
from	material
where	cd_material	= cd_material_w;

select	max(obter_idade(c.dt_nascimento,coalesce(c.dt_obito,clock_timestamp()),'M')),
...
select	max(coalesce(qt_concentracao,0)),
		max(coalesce(qt_concentracao_min,0)),
...
if	--(qt_dose_medic_w > 0) and
	...
	select  max(coalesce(obter_conversao_ml(cd_material, qt_dose_prescricao,CD_UNID_MED_PRESCR),0)),
	...
	if (qt_dose_medic_w > 0) or (qt_dose_reconst_w > 0) then
		...
		if (ie_agrupador_w <> 7) then
			...
			select  max(coalesce(obter_conversao_ml(cd_material, qt_dose_prescricao,CD_UNID_MED_PRESCR),0)),
					max(coalesce(qt_dose_prescricao,0))
		...
		if	((qt_concent_ml_w > qt_concentracao_ml_w) or (qt_concent_ml_w < qt_concentracao_min_ml_w)) then
			...
			if (ie_opcao_p	=  'M') then
				if (qt_concent_ml_w > qt_concentracao_ml_w) then
					retorno_w	:= substr(obter_desc_expressao(727708)/*' A concentracao atual e maior que a maxima permitida: '*/
|| ds_concentracao_ml_w ||' '||cd_unidade_medida_dose_w || obter_desc_expressao(296059) ||ds_unid_med_mat_prescr_w||'.',1,255);
				elsif (qt_concent_ml_w < qt_concentracao_min_ml_w) then
					retorno_w	:= substr(obter_desc_expressao(727724)/*' A concentracao atual e menor que a minima permitida: '*/
|| ds_concentracao_min_ml_w ||' '||cd_unidade_medida_dose_w || obter_desc_expressao(296059) ||ds_unid_med_mat_prescr_w||'.',1,255);
				end if;
			else
			   retorno_w	:= 'S';
			end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_concent_medic_onc ( nr_seq_atendimento_p bigint, nr_seq_material_p bigint, ie_opcao_p text) FROM PUBLIC;
