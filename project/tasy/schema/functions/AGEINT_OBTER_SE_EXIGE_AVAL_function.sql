...
CREATE OR REPLACE FUNCTION ageint_obter_se_exige_aval (nr_seq_tipo_aval_p bigint, nr_seq_ageint_p bigint, nr_seq_proc_interno_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (ie_exige_aval_w = 'S') then
	select	CASE WHEN count(*)=0 THEN 'S'  ELSE 'N' END
	into STRICT	ie_retorno_w
	from	med_avaliacao_paciente
	where	nr_seq_tipo_avaliacao = nr_seq_tipo_aval_p
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ageint_obter_se_exige_aval (nr_seq_tipo_aval_p bigint, nr_seq_ageint_p bigint, nr_seq_proc_interno_p bigint) FROM PUBLIC;
