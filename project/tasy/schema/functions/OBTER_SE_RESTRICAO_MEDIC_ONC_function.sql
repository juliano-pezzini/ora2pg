...
CREATE OR REPLACE FUNCTION obter_se_restricao_medic_onc (cd_material_p bigint) RETURNS varchar AS $body$
DECLARE
...
ie_tipo_material_w integer;

c01 CURSOR FOR
SELECT	max(ds_mensagem)
from	onc_regra_material
where
	coalesce(cd_material,coalesce(cd_material_p,0)) = coalesce(cd_material_p,0)
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_restricao_medic_onc (cd_material_p bigint) FROM PUBLIC;
