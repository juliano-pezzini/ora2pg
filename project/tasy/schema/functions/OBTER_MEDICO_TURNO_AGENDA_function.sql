...
CREATE OR REPLACE FUNCTION obter_medico_turno_agenda (cd_agenda_p bigint, dt_agenda_p timestamp) RETURNS varchar AS $body$
DECLARE
...
if (nr_seq_medico_exec_w IS NOT NULL AND nr_seq_medico_exec_w::text <> '') then
	select	cd_medico
	into STRICT	cd_medico_w
	from	agenda_medico
	where	nr_sequencia	= nr_seq_medico_exec_w
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_medico_turno_agenda (cd_agenda_p bigint, dt_agenda_p timestamp) FROM PUBLIC;
