...
CREATE OR REPLACE FUNCTION rxt_tratamento_ag_aprov_med ( nr_seq_tratamento_p rxt_tratamento.nr_sequencia%TYPE ) RETURNS varchar AS $body$
DECLARE
...
BEGIN
    ...
    AND EXISTS (
        ...
        AND (fase.dt_lib_prim_fisico IS NOT NULL AND fase.dt_lib_prim_fisico::text <> '')
        AND coalesce(rxt_obter_se_trat_liberado(fase.nr_sequencia)::text, '') = ''
        AND coalesce(fase.dt_aprovacao_medico::text, '') = ''
        )
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION rxt_tratamento_ag_aprov_med ( nr_seq_tratamento_p rxt_tratamento.nr_sequencia%TYPE ) FROM PUBLIC;
