...
CREATE OR REPLACE FUNCTION rxt_tratamento_pend_imagens ( nr_seq_tratamento_p rxt_tratamento.nr_sequencia%TYPE ) RETURNS varchar AS $body$
DECLARE
...
BEGIN
    ...
    AND (tratamento.ie_checkfilm_done = 'N' OR coalesce(tratamento.ie_checkfilm_done::text, '') = '')
    AND (SELECT COUNT(*)
        FROM rxt_agenda
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION rxt_tratamento_pend_imagens ( nr_seq_tratamento_p rxt_tratamento.nr_sequencia%TYPE ) FROM PUBLIC;
