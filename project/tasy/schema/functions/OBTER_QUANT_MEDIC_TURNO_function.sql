...
CREATE OR REPLACE FUNCTION obter_quant_medic_turno (nr_prescricao_p bigint, cd_material_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (hr_prim_turno_w IS NOT NULL AND hr_prim_turno_w::text <> '') then 
	...
	LOOP 
		...
		if (ie_agrupador_w	= 4) then 
			...
			while	(ds_horarios_w IS NOT NULL AND ds_horarios_w::text <> '') LOOP 
				begin 
				select	position(wheb_mensagem_pck.get_texto(309283) in ds_horarios_w) -- das 
				into STRICT	k 
				;
				if (k > 0) then 
		...
		if (ie_agrupador_w	= 8) then 
			...
			if (position(wheb_mensagem_pck.get_texto(309283) in ds_horarios_dif_w) > 0) then -- das 
...
ds_retorno_w	:= wheb_mensagem_pck.get_texto(309281) || ' '||qt_tarde_w||' - ' || wheb_mensagem_pck.get_texto(309280) || ' '||qt_noite_w ||' - ' || wheb_mensagem_pck.get_texto(309282) || ' '||qt_manha_w; -- Tarde	-- Noite	-- Manhã 
if (coalesce(hr_ter_turno_w::text, '') = '') then
	ds_retorno_w	:= wheb_mensagem_pck.get_texto(309281) || ' '||qt_tarde_w||' - ' || wheb_mensagem_pck.get_texto(309282) || ' '||qt_manha_w; -- Tarde	-- Manhã 
end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_quant_medic_turno (nr_prescricao_p bigint, cd_material_p bigint) FROM PUBLIC;
