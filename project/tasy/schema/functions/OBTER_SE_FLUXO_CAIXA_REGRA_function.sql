...
CREATE OR REPLACE FUNCTION obter_se_fluxo_caixa_regra ( nr_seq_regra_p bigint, dt_referencia_p timestamp, cd_estabelecimento_p bigint, ie_opcao_p text) RETURNS varchar AS $body$
DECLARE
...
EXIT WHEN NOT FOUND or ds_retorno_w = 'N';  /* apply on c01 */
	...
	if (ie_regra_exclusao_w = 'TP') then
		...
		select	count(*),
			coalesce(sum(a.vl_saldo_titulo),0)
		into STRICT	cont_w,
			vl_saldo_titulo_w
		from	titulo_pagar a
		where	a.cd_estabelecimento	= cd_estabelecimento_p
		and	a.ie_origem_titulo		= coalesce(ie_origem_tit_pagar_w, a.ie_origem_titulo)
		and	coalesce(a.cd_pessoa_fisica, 'X')	= coalesce(cd_pessoa_fisica_w, coalesce(a.cd_pessoa_fisica, 'X'))
		and	coalesce(a.cd_cgc, 'X') 		= coalesce(cd_cgc_w, coalesce(a.cd_cgc, 'X'))
		and	a.vl_titulo			between coalesce(vl_minimo_w, a.vl_titulo) and coalesce(vl_maximo_w, a.vl_titulo)
		and	coalesce(a.nr_seq_classe,0)	= coalesce(nr_seq_classe_tit_pagar_w, coalesce(a.nr_seq_classe, 0))
		and	exists (SELECT 	1
		...
		where	a.cd_estabelecimento	= cd_estabelecimento_p
		and	a.ie_origem_titulo	= coalesce(ie_origem_tit_rec_w, a.ie_origem_titulo)
		and	coalesce(a.cd_pessoa_fisica, 'X') = coalesce(cd_pessoa_fisica_w, coalesce(a.cd_pessoa_fisica, 'X'))
		and	coalesce(a.cd_cgc, 'X') 	= coalesce(cd_cgc_w, coalesce(a.cd_cgc, 'X'))
		and	a.vl_titulo		between coalesce(vl_minimo_w, a.vl_titulo) and coalesce(vl_maximo_w, a.vl_titulo)
		and	exists (SELECT 	1
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_fluxo_caixa_regra ( nr_seq_regra_p bigint, dt_referencia_p timestamp, cd_estabelecimento_p bigint, ie_opcao_p text) FROM PUBLIC;
