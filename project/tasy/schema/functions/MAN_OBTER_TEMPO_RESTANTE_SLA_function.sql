...
CREATE OR REPLACE FUNCTION man_obter_tempo_restante_sla ( nr_seq_ordem_serv_p bigint, nr_seq_sla_p bigint) RETURNS bigint AS $body$
DECLARE
...
if (nr_seq_ordem_serv_p IS NOT NULL AND nr_seq_ordem_serv_p::text <> '') then
	...
	from	(SELECT	max(qt_min_termino) qt_min_termino,
			coalesce(sum(qt_minutos), 0) qt_minutos
		from	(select	msr.qt_min_termino,
				man_obter_tempo_com(ml.cd_estabelecimento, mose.dt_atualizacao,
					coalesce((	select	min(a.dt_atualizacao)
						from	man_ordem_serv_estagio a
						where	a.nr_seq_ordem  = mos.nr_sequencia
						and	a.dt_atualizacao > mose.dt_atualizacao), clock_timestamp()), 'MC') qt_minutos
			from	man_sla ms,
				man_sla_regra msr,
				man_sla_loc msl,
				man_localizacao ml,
			...
			and	mos.ie_classificacao = msr.ie_classificacao
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION man_obter_tempo_restante_sla ( nr_seq_ordem_serv_p bigint, nr_seq_sla_p bigint) FROM PUBLIC;
