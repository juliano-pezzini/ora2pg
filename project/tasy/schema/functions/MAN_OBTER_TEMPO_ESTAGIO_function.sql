...
--
-- dblink wrapper to call function man_obter_tempo_estagio as an autonomous transaction
--
CREATE EXTENSION IF NOT EXISTS dblink;

CREATE OR REPLACE FUNCTION man_obter_tempo_estagio ( nr_sequencia_p bigint, nr_seq_ordem_p bigint, ie_tipo_p text) RETURNS bigint AS $body$
DECLARE
	...
	v_ret	bigint;
BEGIN
	v_query := 'SELECT * FROM man_obter_tempo_estagio_atx ( ' || quote_nullable(nr_sequencia_p) || ',' || quote_nullable(nr_seq_ordem_p) || ',' || quote_nullable(ie_tipo_p) || ' )';
...
CREATE OR REPLACE FUNCTION man_obter_tempo_estagio_atx ( nr_sequencia_p bigint, nr_seq_ordem_p bigint, ie_tipo_p text) RETURNS bigint AS $body$
DECLARE
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION man_obter_tempo_estagio ( nr_sequencia_p bigint, nr_seq_ordem_p bigint, ie_tipo_p text) FROM PUBLIC; -- REVOKE ALL ON FUNCTION man_obter_tempo_estagio_atx ( nr_sequencia_p bigint, nr_seq_ordem_p bigint, ie_tipo_p text) FROM PUBLIC;
