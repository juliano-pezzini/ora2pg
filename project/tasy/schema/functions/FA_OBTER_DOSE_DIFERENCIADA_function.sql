...
CREATE OR REPLACE FUNCTION fa_obter_dose_diferenciada ( ds_dose_dif_p text, nr_dose_p bigint) RETURNS bigint AS $body$
DECLARE
...
if (ds_dose_dif_p IS NOT NULL AND ds_dose_dif_p::text <> '') then
	...
	return	vetor_w[nr_dose_p].qt_dose;
else
	return	'';
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION fa_obter_dose_diferenciada ( ds_dose_dif_p text, nr_dose_p bigint) FROM PUBLIC;
