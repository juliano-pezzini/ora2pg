...
CREATE OR REPLACE FUNCTION obter_anterior_resultado_lab (cd_pessoa_fisica_p bigint, seq_grid_p bigint, nr_seq_exame_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (nr_seq_resultado_atual_w > 0) then
	...
	and	a.cd_pessoa_fisica	= cd_pessoa_fisica_p
	and	c.nr_seq_exame		= nr_seq_exame_p
	and (coalesce(c.dt_digitacao,a.dt_resultado) < dt_digitacao_w)
	and (coalesce(coalesce(CASE WHEN c.ds_resultado='0' THEN ''  ELSE CASE WHEN d.ie_formato_resultado='V' THEN ''  ELSE c.ds_resultado END  END ,	coalesce(to_char(c.qt_resultado),to_char(CASE WHEN c.pr_resultado=0 THEN ''  ELSE c.pr_resultado END ))),c.ds_resultado)) is not null;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_anterior_resultado_lab (cd_pessoa_fisica_p bigint, seq_grid_p bigint, nr_seq_exame_p bigint) FROM PUBLIC;
