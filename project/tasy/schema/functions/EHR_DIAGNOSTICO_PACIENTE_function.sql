...
CREATE OR REPLACE FUNCTION ehr_diagnostico_paciente ( nr_seq_reg_elemento_p bigint, nr_seq_entidade_p bigint, nr_seq_elemento_p bigint) RETURNS varchar AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	begin
	ds_comando_ww	:= ds_comando_w || chr(13) || ' where  nr_atendimento ='||to_char(nr_atendimento_w) ||
	'  and dt_diagnostico = to_date('||guampa_w||to_char(dt_diagnostico_w,'dd/mm/yyyy hh24:mi:ss')||guampa_w||',''dd/mm/yyyy hh24:mi:ss'')   '||
	'  and cd_doenca = '''||cd_doenca_w||guampa_w;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ehr_diagnostico_paciente ( nr_seq_reg_elemento_p bigint, nr_seq_entidade_p bigint, nr_seq_elemento_p bigint) FROM PUBLIC;
