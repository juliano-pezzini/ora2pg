...
CREATE OR REPLACE FUNCTION ageint_obter_valores_itens (nr_seq_item_p bigint, ie_tipo_valor_p text, ie_exame_assoc_p text, ie_tipo_agendamento_p text) RETURNS bigint AS $body$
DECLARE
...
valor_item_assoc_w		double precision;
...
if (ie_tipo_agendamento_p = 'E') and (OBTER_CALC_ITEM_ASSOC_AGEINT(nr_seq_proc_interno_w) = 'S') then

	select	sum(vl_exame)
	into STRICT	valor_item_assoc_w
	from	ageint_exame_associado
	where	nr_seq_ageint_item = nr_seq_item_p;

	nr_valor_item_w := valor_item_w + valor_item_assoc_w;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ageint_obter_valores_itens (nr_seq_item_p bigint, ie_tipo_valor_p text, ie_exame_assoc_p text, ie_tipo_agendamento_p text) FROM PUBLIC;
