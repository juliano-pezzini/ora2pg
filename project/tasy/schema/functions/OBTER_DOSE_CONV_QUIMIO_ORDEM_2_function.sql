...
CREATE OR REPLACE FUNCTION obter_dose_conv_quimio_ordem_2 ( cd_material_p bigint, qt_dose_origem_p bigint, cd_unid_medida_ori_p text, cd_unid_medida_dest_p text, nr_seq_ordem_p bigint) RETURNS bigint AS $body$
DECLARE


-- ------------------------------------------------------------------------------------------
-- ao realizar alteracao aqui e necessario verificar tambem a OBTER_DOSE_CONV_QUIMIO_ORDEM
-- criada para realizar o mesmo processo, porem com casas ilimitadas durante a conversao
-- ------------------------------------------------------------------------------------------
cd_unid_medida_ori_w	varchar(30);
cd_unid_medida_dest_w	varchar(30);
qt_dose_convertida_w	bigint;
cd_unid_med_cons_w		varchar(30);
qt_conversao_w			bigint;
cd_estabelecimento_w	smallint := wheb_usuario_pck.get_cd_estabelecimento;
ie_conv_ml_w			varchar(15);
qt_diluicao_w			bigint;
qt_dose_medic_w			bigint;
cd_um_medic_w			varchar(30);
cd_unidade_medida_consumo_w	varchar(30);
qt_dose_convert_w		bigint;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_dose_conv_quimio_ordem_2 ( cd_material_p bigint, qt_dose_origem_p bigint, cd_unid_medida_ori_p text, cd_unid_medida_dest_p text, nr_seq_ordem_p bigint) FROM PUBLIC;
