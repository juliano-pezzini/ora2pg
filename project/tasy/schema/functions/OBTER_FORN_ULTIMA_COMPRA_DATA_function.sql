...
CREATE OR REPLACE FUNCTION obter_forn_ultima_compra_data ( cd_material_p bigint, cd_estabelecimento_p bigint, dt_referencia_p timestamp) RETURNS varchar AS $body$
DECLARE
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_forn_ultima_compra_data ( cd_material_p bigint, cd_estabelecimento_p bigint, dt_referencia_p timestamp) FROM PUBLIC;
