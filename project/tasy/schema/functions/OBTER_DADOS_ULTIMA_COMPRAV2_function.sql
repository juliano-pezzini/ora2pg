...
CREATE OR REPLACE FUNCTION obter_dados_ultima_comprav2 ( cd_estabelecimento_p bigint, cd_material_p bigint, ie_retorno_p text) RETURNS varchar AS $body$
DECLARE
...
DM	= descrição da marca do material da última compra
*/
...
if (ie_inf_ultima_compra_w = 'S') then
	...
	elsif (ie_retorno_p = 'DM') then
		vl_retorno_w 	:= to_char(ds_marca_w);
	end if;
	end;
end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_dados_ultima_comprav2 ( cd_estabelecimento_p bigint, cd_material_p bigint, ie_retorno_p text) FROM PUBLIC;
