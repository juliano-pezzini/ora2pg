...
CREATE OR REPLACE FUNCTION obter_cor_curativo_html ( cd_pessoa_ferida_p bigint, nr_seq_area_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (cd_pessoa_ferida_p IS NOT NULL AND cd_pessoa_ferida_p::text <> '') and (nr_seq_area_p IS NOT NULL AND nr_seq_area_p::text <> '') then
	...
	if (nr_seq_ferida_w IS NOT NULL AND nr_seq_ferida_w::text <> '') then
		...
		if (ie_pend_curativo_w = 'S') then
			...
			ds_cor_w := obter_cor_cadastro_html5(5296);
			...
			if (qt_score_w	>=1) and (qt_score_w	<=12) then
				ds_cor_w := obter_cor_cadastro_html5(5279);
			elsif (qt_score_w	>=13) and (qt_score_w	<=64) then
				ds_cor_w := obter_cor_cadastro_html5(5280);
			elsif (qt_score_w	>= 65) then
				ds_cor_w := obter_cor_cadastro_html5(5281);
			end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_cor_curativo_html ( cd_pessoa_ferida_p bigint, nr_seq_area_p bigint) FROM PUBLIC;
