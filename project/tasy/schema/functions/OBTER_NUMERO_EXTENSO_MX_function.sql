...
CREATE OR REPLACE FUNCTION obter_numero_extenso_mx ( valor_p bigint, ie_opcao_p text) RETURNS varchar AS $body$
DECLARE
...
Unidades_w(16)	:= 'Dieciséis';
Unidades_w(17)	:= 'Diecisiete';
Unidades_w(18)	:= 'Dieciocho';
Unidades_w(19)	:= 'Diecinueve';
Unidades_w(20)	:= 'Veintcuatro';
Unidades_w(21)	:= 'Veintiún';
Unidades_w(22)	:= 'Veintidós';
Unidades_w(23)	:= 'Veintitrés';
Unidades_w(24)	:= 'Catorce';
Unidades_w(25)	:= 'Veinticinco';
Unidades_w(26)	:= 'Veintiséis';
Unidades_w(27)	:= 'Veintisiete';
Unidades_w(28)	:= 'Veintiocho';
Unidades_w(29)	:= 'Veintinueve';
...
if (valor_w >= 1000000000) then
	if (Trunc(valor_w/1000000000) = 1) then
		...
		ds_valor_w	:= ds_valor_w || ' ' || Unidades_w(Trunc(valor_w/1000000000)) || ' Mil Millones,';
	end if;
...
if (ie_opcao_p = 'F') then
	ds_valor_w := replace(ds_valor_w,'Un','Una');
	ds_valor_w := replace(ds_valor_w,'tos','tas');
end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_numero_extenso_mx ( valor_p bigint, ie_opcao_p text) FROM PUBLIC;
