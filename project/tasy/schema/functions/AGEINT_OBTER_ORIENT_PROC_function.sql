...
CREATE OR REPLACE FUNCTION ageint_obter_orient_proc ( cd_procedimento_p bigint, ie_origem_proced_p bigint, cd_convenio_p bigint, nr_seq_proc_interno_p bigint, nr_atendimento_p bigint, nr_seq_tipo_classif_pac_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, nr_seq_ageint_item_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (coalesce(nr_seq_proc_interno_p,0) > 0) then
	...
	if (ie_forma_orientacao_w	= 'S') then
		ds_orientacao_w := 	substr(ds_orientacao_w || chr(13) || chr(10) ||
					...
					ds_orient_paciente_w,1,4000);
	/*else
		ds_orientacao_w := 	substr(ds_orientacao_w || chr(13) || chr(10) || 
					ds_orient_paciente_w || chr(13) || chr(10) || 
					ds_orient_pac_preparo_w,1,4000);*/
	else
		ds_orientacao_w	:= '';
	end if;
end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ageint_obter_orient_proc ( cd_procedimento_p bigint, ie_origem_proced_p bigint, cd_convenio_p bigint, nr_seq_proc_interno_p bigint, nr_atendimento_p bigint, nr_seq_tipo_classif_pac_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, nr_seq_ageint_item_p bigint) FROM PUBLIC;
