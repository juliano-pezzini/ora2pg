...
CREATE OR REPLACE FUNCTION obter_desc_mat_req_material ( nr_requisicao_p bigint, cd_material_p bigint, ie_localizacao_p text, cd_local_estoque_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (ie_localizacao_p = 'S') then
	Select ' [' || substr(obter_localizacao_material(cd_material_p, cd_local_estoque_p),1,80) || ']'
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_desc_mat_req_material ( nr_requisicao_p bigint, cd_material_p bigint, ie_localizacao_p text, cd_local_estoque_p bigint) FROM PUBLIC;
