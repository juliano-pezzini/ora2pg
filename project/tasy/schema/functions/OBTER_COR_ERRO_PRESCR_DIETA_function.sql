...
CREATE OR REPLACE FUNCTION obter_cor_erro_prescr_dieta ( nr_prescricao_p bigint, nr_seq_dieta_p bigint, nr_cor_padrao_p bigint, cd_estabelecimento_p bigint, cd_perfil_p bigint) RETURNS varchar AS $body$
DECLARE
...
BEGIN
if (nr_prescricao_p IS NOT NULL AND nr_prescricao_p::text <> '') and (nr_seq_dieta_p IS NOT NULL AND nr_seq_dieta_p::text <> '') and (nr_cor_padrao_p IS NOT NULL AND nr_cor_padrao_p::text <> '') then
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_cor_erro_prescr_dieta ( nr_prescricao_p bigint, nr_seq_dieta_p bigint, nr_cor_padrao_p bigint, cd_estabelecimento_p bigint, cd_perfil_p bigint) FROM PUBLIC;
