...
CREATE OR REPLACE FUNCTION ageint_consiste_sugest_salas ( cd_agenda_p bigint, hr_Agenda_p timestamp, nr_minuto_duracao_p bigint, nr_Seq_ageint_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ie_consiste_regra_p text default 'N') RETURNS varchar AS $body$
DECLARE
...
from 	ageint_sugestao_horarios b
where 	b.nr_seq_agenda					= nr_Seq_Ageint_p
...
if (hr_max_agenda_w IS NOT NULL AND hr_max_agenda_w::text <> '') then
	...
	from	ageint_sugestao_horarios a
	where	a.nr_seq_agenda		= nr_Seq_ageint_p
	and	a.nm_usuario		= nm_usuario_p
	and	a.hr_Agenda		= hr_max_agenda_w;
	--and	nvl(a.ie_Gerado, 'N')	= 'N';
...
if (coalesce(qt_tempo_sala_w,0) = 0) and (ie_consiste_regra_p = 'S') then
	ds_retorno_max_w := 'C';
end if;
...
if (ds_retorno_min_w	= 'N') or (ds_retorno_max_w	= 'N') then
	ds_Retorno_w	:= 'N';
elsif (ds_retorno_min_w	= 'C') and (ds_retorno_max_w	= 'C') then
	ds_Retorno_w	:= 'C';
end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ageint_consiste_sugest_salas ( cd_agenda_p bigint, hr_Agenda_p timestamp, nr_minuto_duracao_p bigint, nr_Seq_ageint_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ie_consiste_regra_p text default 'N') FROM PUBLIC;
