...
--
-- dblink wrapper to call function ehr_obter_valor_macro_html5 as an autonomous transaction
--
CREATE EXTENSION IF NOT EXISTS dblink;

CREATE OR REPLACE FUNCTION ehr_obter_valor_macro_html5 (cd_pessoa_fisica_p text, nr_atendimento_p bigint, ie_macro_p text) RETURNS SYS_REFCURSOR AS $body$
DECLARE
	...
	v_ret	SYS_REFCURSOR;
BEGIN
	v_query := 'SELECT * FROM ehr_obter_valor_macro_html5_atx ( ' || quote_nullable(cd_pessoa_fisica_p) || ',' || quote_nullable(nr_atendimento_p) || ',' || quote_nullable(ie_macro_p) || ' )';
	SELECT * INTO v_ret FROM dblink(v_conn_str, v_query) AS p (ret SYS_REFCURSOR);
	RETURN v_ret;
...
CREATE OR REPLACE FUNCTION ehr_obter_valor_macro_html5_atx (cd_pessoa_fisica_p text, nr_atendimento_p bigint, ie_macro_p text) RETURNS SYS_REFCURSOR AS $body$
DECLARE
...
if (ie_macro_p	= 'NOME') then
	...
	EXIT WHEN NOT FOUND; /* apply on C06 */
		...
		if (ds_dias_ciclos_w is  null) then	
...
open cursor_retorno for
    SELECT substr(ds_retorno_w , 1, 255) as "DS_RETORNO_W",
    dt_nascimento_w as "DT_NASCIMENTO_W"
;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ehr_obter_valor_macro_html5 (cd_pessoa_fisica_p text, nr_atendimento_p bigint, ie_macro_p text) FROM PUBLIC; -- REVOKE ALL ON FUNCTION ehr_obter_valor_macro_html5_atx (cd_pessoa_fisica_p text, nr_atendimento_p bigint, ie_macro_p text) FROM PUBLIC;
