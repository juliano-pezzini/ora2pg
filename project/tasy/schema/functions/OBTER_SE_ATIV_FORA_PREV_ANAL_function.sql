...
CREATE OR REPLACE FUNCTION obter_se_ativ_fora_prev_anal ( nr_seq_ordem_serv_p bigint, qt_porcent_toleravel_p bigint, qt_porcent_consistir_p bigint) RETURNS varchar AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	and	not exists (SELECT 1
			...
			where 	a.nr_seq_ativ_prev = b.nr_sequencia
			and	(b.NR_SEQ_MOTIVO_ASSERT IS NOT NULL AND b.NR_SEQ_MOTIVO_ASSERT::text <> ''));
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_ativ_fora_prev_anal ( nr_seq_ordem_serv_p bigint, qt_porcent_toleravel_p bigint, qt_porcent_consistir_p bigint) FROM PUBLIC;
