...
CREATE OR REPLACE FUNCTION rp_obter_se_ind_possui_tre ( nr_seq_ind_p bigint, dt_inicio_p timestamp, dt_fim_p timestamp, cd_pessoa_fisica_p text) RETURNS varchar AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	where	nr_sequencia = nr_seq_ind_p;
...
while(dt_inicio_w <= dt_fim_p) loop
	...
	EXIT WHEN NOT FOUND or (ds_retorno_w = 'S');  /* apply on C01 */
		...
		if (obter_cod_dia_semana(dt_inicio_w) = ie_dia_semana_w) or (ds_retorno_w = 'N') then
			...
			and		((hr_horario_w between d.dt_inicio_vigencia and fim_dia(d.dt_fim_vigencia)) or ((coalesce(d.dt_inicio_vigencia::text, '') = '') or (coalesce(d.dt_fim_vigencia::text, '') = '')));
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION rp_obter_se_ind_possui_tre ( nr_seq_ind_p bigint, dt_inicio_p timestamp, dt_fim_p timestamp, cd_pessoa_fisica_p text) FROM PUBLIC;
