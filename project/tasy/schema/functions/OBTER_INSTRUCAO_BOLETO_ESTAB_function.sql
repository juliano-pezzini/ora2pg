...
CREATE OR REPLACE FUNCTION obter_instrucao_boleto_estab ( nr_titulo_p bigint, cd_banco_p bigint, nr_linha_p bigint, cd_estabelecimento_p bigint) RETURNS varchar AS $body$
DECLARE
...
/*INICIO Essa macro é fixa conforme OS 799927. Nao existia essa macro nessa rotina, apenas na OBTER_INSTRUCAO_BOLETO_TAM, e repliquei a mesma para ca. */
...
union
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		if (coalesce(ds_msg_reajuste_w::text, '') = '') then
			ds_msg_reajuste_w := substr(ds_msg_reajuste_w2,1,2000);
		else
...
/*fim 950304*/
	...
	if (ds_instrucao_w IS NOT NULL AND ds_instrucao_w::text <> '') then
		...
		if (vl_reajuste_pls_etaria_w > 0) then
			ds_instrucao_w	:= replace_macro(ds_instrucao_w,'@VL_MENSALIDADE_PLS',campo_mascara_virgula(vl_mensalidade_pls_w + vl_reajuste_pls_etaria_w));
		else
		...
		ds_instrucao_w	:= replace_macro(ds_instrucao_w,'@DT_REAJUSTE_COBR',to_char(dt_reajuste_cobr_w,'mm/yyyy'));
		ds_instrucao_w	:= replace_macro(ds_instrucao_w,'@DT_REAJUSTE_MUD',to_char(dt_reajuste_mud_w,'mm/yyyy'));
		ds_instrucao_w	:= replace_macro(ds_instrucao_w,'@DT_REAJUSTE_VAR',to_char(dt_reajuste_var_w,'mm/yyyy'));
		ds_instrucao_w	:= replace_macro(ds_instrucao_w,'@DT_REAJUSTE_SCA',to_char(dt_reajuste_sca_w,'mm/yyyy'));
		...
		ds_instrucao_w	:= substr(replace_macro(ds_instrucao_w,'@DS_MENSAGEM_REAJUSTE',substr(ds_msg_reajuste_w,1,200)),1,255);
		ds_instrucao_w	:= substr(replace_macro(ds_instrucao_w,'@DS_MENSAGEM_2REAJUSTE',substr(ds_msg_reajuste_w,201,254)),1,255);
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_instrucao_boleto_estab ( nr_titulo_p bigint, cd_banco_p bigint, nr_linha_p bigint, cd_estabelecimento_p bigint) FROM PUBLIC;
