...
CREATE OR REPLACE FUNCTION fn_get_ontology_code ( cd_seq_record_p text, ie_group_p text, ds_type_p text ) RETURNS varchar AS $body$
DECLARE
...
BEGIN
    ...
    ELSIF ds_type_p = 'TABLE' THEN
        ...
        WHERE
            phi.ie_grupo = ie_group_p
            AND ot.nr_sequencia = (cd_seq_record_p)::numeric
    ...
    ELSIF ds_type_p = 'ATTRIBUTE_GROUP' THEN
        ...
        WHERE
            phi.ie_grupo = ie_group_p
            AND otag.nr_sequencia = (cd_seq_record_p)::numeric
            AND phi.ie_ontologia = 'SNO'
    ...
    ELSE
        ...
        WHERE
            phi.ie_grupo = ie_group_p
            AND ota.nr_sequencia = (cd_seq_record_p)::numeric
            AND phi.ie_ontologia = 'SNO'
            AND coalesce(phi.cd_valor_tasy::text, '') = ''  LIMIT 1;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION fn_get_ontology_code ( cd_seq_record_p text, ie_group_p text, ds_type_p text ) FROM PUBLIC;
