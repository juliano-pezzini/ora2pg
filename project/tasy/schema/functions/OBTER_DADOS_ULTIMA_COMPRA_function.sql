...
CREATE OR REPLACE FUNCTION obter_dados_ultima_compra ( cd_estabelecimento_p bigint, cd_material_p bigint, ie_retorno_p text) RETURNS varchar AS $body$
DECLARE
...
if (ie_inf_ultima_compra_w = 'S') then
	...
	if (nr_seq_ultima_compra_w > 0) then
		/*Identifica se a nota e mesmo de operacao que atualiza ultima compra, e se nao for nao entra no proximo if abaixo*/
	...
	if (ie_retorno_p = 'UC') then
		...
		vl_retorno_w	:= PKG_DATE_FORMATERS_TZ.TO_VARCHAR(dt_entrada_saida_w,'shortDate', ESTABLISHMENT_TIMEZONE_UTILS.getTimeZone);
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_dados_ultima_compra ( cd_estabelecimento_p bigint, cd_material_p bigint, ie_retorno_p text) FROM PUBLIC;
