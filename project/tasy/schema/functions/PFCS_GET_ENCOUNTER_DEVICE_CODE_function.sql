...
CREATE OR REPLACE FUNCTION pfcs_get_encounter_device_code ( nr_encounter bigint, ds_filter_type text, cd_estabelecimento_p text ) RETURNS bigint AS $body$
DECLARE


nr_device_code bigint;
...
select
        eq.cd_equipamento cd_equipamento into STRICT nr_device_code
			from
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION pfcs_get_encounter_device_code ( nr_encounter bigint, ds_filter_type text, cd_estabelecimento_p text ) FROM PUBLIC;
