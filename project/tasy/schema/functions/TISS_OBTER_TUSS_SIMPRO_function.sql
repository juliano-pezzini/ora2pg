...
CREATE OR REPLACE FUNCTION tiss_obter_tuss_simpro (cd_material_p bigint, cd_estabelecimento_p bigint, cd_convenio_p bigint, dt_referencia_p timestamp, nr_seq_marca_p bigint default 0) RETURNS bigint AS $body$
DECLARE


cd_simpro_w		bigint := null;
cd_tuss_w		bigint := null;
...
if (coalesce(cd_simpro_w::text, '') = '') then -- código SEM estalecimento
...
if (cd_simpro_w IS NOT NULL AND cd_simpro_w::text <> '') then
	...
	and	dt_vigencia 	=
				...
				where 	cd_simpro = cd_simpro_w
				and     coalesce(cd_estabelecimento, coalesce(cd_estab_simpro_w, 0)) = coalesce(cd_estab_simpro_w, 0)
				and 	dt_vigencia <= dt_referencia_p);
end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION tiss_obter_tuss_simpro (cd_material_p bigint, cd_estabelecimento_p bigint, cd_convenio_p bigint, dt_referencia_p timestamp, nr_seq_marca_p bigint default 0) FROM PUBLIC;
