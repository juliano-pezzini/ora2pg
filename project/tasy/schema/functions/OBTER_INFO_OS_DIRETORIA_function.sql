...
CREATE OR REPLACE FUNCTION obter_info_os_diretoria (dt_parametro_p timestamp, nr_seq_diretoria_p bigint, ie_tipo_inf_p text, ie_tipo_valor_p text) RETURNS bigint AS $body$
DECLARE
...
if (ie_tipo_inf_p in ('QT','PR','PRGD','PRIN','PREX','PRDCT', 'PRJIN','PRINSJ','PRPHI','PRVEV','PRDJAVA','PRDDELPHI','PRDJVCLI','PRDJVPHI','PRDDECLI','PRDDEPHI'))   then
	...
	and		b.nr_seq_diretoria	= nr_seq_diretoria_p;
	...
	and		a.dt_ordem_servico between dt_ref_mes_w and dt_fim_mes_w
	and		g.nr_seq_diretoria	= nr_seq_diretoria_p
...
WHERE a.dt_ordem_servico between dt_ref_mes_w and dt_fim_mes_w and a.nr_seq_gerencia		= g.nr_sequencia and g.nr_seq_diretoria	= nr_seq_diretoria_p  and coalesce(ie_terceiro,'S')	= 'S';
	...
	FROM gerencia_wheb g, os_recebida_gerencia_desenv_v a
LEFT OUTER JOIN man_localizacao b ON (a.nr_seq_localizacao = b.nr_sequencia)
WHERE a.dt_ordem_servico between dt_ref_mes_w and dt_fim_mes_w and a.nr_seq_gerencia		= g.nr_sequencia and g.nr_seq_diretoria	= nr_seq_diretoria_p  and coalesce(ie_terceiro,'S')	= 'N';
...
WHERE a.nr_seq_grupo_des = b.nr_sequencia and c.nr_sequencia = a.nr_seq_estagio and b.nr_seq_gerencia		= g.nr_sequencia and g.nr_seq_diretoria	= nr_seq_diretoria_p  and coalesce(m.ie_terceiro,'S')	= 'S' and coalesce(c.ie_desenv,'X') = 'S' and a.ie_status_ordem <> '3';
	...
	FROM gerencia_wheb g, man_estagio_processo c, grupo_desenvolvimento b, man_ordem_servico a
LEFT OUTER JOIN man_localizacao m ON (a.nr_seq_localizacao = m.nr_sequencia)
WHERE a.nr_seq_grupo_des = b.nr_sequencia and c.nr_sequencia = a.nr_seq_estagio and b.nr_seq_gerencia		= g.nr_sequencia and g.nr_seq_diretoria	= nr_seq_diretoria_p  and coalesce(m.ie_terceiro,'S')	= 'N' and coalesce(c.ie_desenv,'X') = 'S' and a.ie_status_ordem <> '3';
...
WHERE c.nr_sequencia = b.nr_seq_estagio and b.nr_sequencia = a.nr_sequencia and a.nr_seq_gerencia		= g.nr_sequencia and g.nr_seq_diretoria	= nr_seq_diretoria_p  and coalesce(m.ie_terceiro,'S')	= 'S' and coalesce(c.ie_desenv,'X') <> 'S' and b.ie_status_ordem <> '3' and c.ie_acao = 2;
	...
	and	b.ie_situacao = 'A' --
	and	a.nr_seq_diretoria = nr_seq_diretoria_p
	...
	and	exists (	SELECT	1
				...
				and	w.nr_seq_diretoria	= nr_seq_diretoria_p
				and	b.ie_situacao		= 'A'
				and	w.ie_situacao		= 'A'
	...
	and		x.nr_seq_diretoria	= nr_seq_diretoria_p
...
b.nr_seq_grupo_des = c.nr_sequencia and c.nr_seq_gerencia = g.nr_sequencia and g.nr_seq_diretoria = :nr_seq_diretoria and a.dt_alteracao between :dt_inicio and :dt_fim', 'nr_seq_diretoria='||nr_seq_diretoria_p||ds_sep_bv_w||'dt_inicio='||dt_ref_mes_w||ds_sep_bv_w||'dt_fim='||dt_fim_mes_w, qt_total_os_w);
	...
	FROM (	
		...
		and	not exists (
						...
						and	nr_seq_tipo = 129)
		and	a.nr_seq_classif <> 49) alias13
...
UNION ALL
	...
	FROM (	
		...
		and	not exists (
						...
						where	a.nr_sequencia = x.nr_seq_ordem_serv
						and	nr_seq_tipo = 129)
		and	a.nr_seq_classif <> 49) alias21) alias22;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_info_os_diretoria (dt_parametro_p timestamp, nr_seq_diretoria_p bigint, ie_tipo_inf_p text, ie_tipo_valor_p text) FROM PUBLIC;
