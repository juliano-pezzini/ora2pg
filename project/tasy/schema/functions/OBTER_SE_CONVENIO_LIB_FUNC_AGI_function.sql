...
CREATE OR REPLACE FUNCTION obter_se_convenio_lib_func_agi (cd_convenio_p bigint, cd_funcao_p bigint, ie_opcao_p text, dt_referencia_p timestamp, nm_usuario_p text) RETURNS varchar AS $body$
DECLARE
...
ie_conv_multi_estab_w varchar(1);
...
begin
	...
	WHERE	a.cd_estabelecimento	= cd_estabelecimento_usu_w
...
begin
	...
	where 	b.cd_estabelecimento = cd_estabelecimento_usu_w
...
if (cd_funcao_p = 869) then
	...
	where coalesce(cd_estabelecimento, cd_estabelecimento_usu_w) = cd_estabelecimento_usu_w;

	if (ie_conv_estab_w = 'S') and (qt_registro_w = 0) and (ie_liberado_w = 'S') then
		ie_liberado_w := 'N';
	end if;
else
	ie_considera_tab_w := Obter_Param_Usuario(916, 669, obter_perfil_ativo, wheb_usuario_pck.get_nm_usuario, cd_estabelecimento_usu_w, ie_considera_tab_w);
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_convenio_lib_func_agi (cd_convenio_p bigint, cd_funcao_p bigint, ie_opcao_p text, dt_referencia_p timestamp, nm_usuario_p text) FROM PUBLIC;
