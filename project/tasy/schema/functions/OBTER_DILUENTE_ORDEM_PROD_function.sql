...
CREATE OR REPLACE FUNCTION obter_diluente_ordem_prod ( nr_seq_ordem_p bigint, ie_opcao_p text) RETURNS varchar AS $body$
DECLARE
...
/* ie_opcao_p M - Material, D - dose U - unidade medida   C - Código */
...
Select	max(cd_material),
	substr(obter_desc_material(max(cd_material)),1,60),
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_diluente_ordem_prod ( nr_seq_ordem_p bigint, ie_opcao_p text) FROM PUBLIC;
