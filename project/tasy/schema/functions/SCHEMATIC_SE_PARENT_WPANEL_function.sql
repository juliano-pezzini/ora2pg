...
CREATE OR REPLACE FUNCTION schematic_se_parent_wpanel (nr_sequencia_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (ie_tipo_componente_sup_w = 'WPANEL') then
	return	'S';
else
	select	SCHEMATIC_SE_PARENT_WPANEL(nr_seq_obj_sup_w)
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION schematic_se_parent_wpanel (nr_sequencia_p bigint) FROM PUBLIC;
