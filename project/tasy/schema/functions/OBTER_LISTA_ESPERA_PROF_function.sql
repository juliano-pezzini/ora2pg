...
CREATE OR REPLACE FUNCTION obter_lista_espera_prof ( nr_seq_lista_p bigint, ie_type_p text) RETURNS varchar AS $body$
DECLARE
...
c01 CURSOR FOR
	SELECT  obter_nome_pessoa_fisica(prof.cd_profissional, null) ds_names
	from    agenda_lista_espera_prof prof,
		funcao_medico fm
	where   prof.nr_seq_lista = nr_seq_lista_p
	and     prof.cd_funcao = fm.cd_funcao
...
if (ie_type_p = 'M')then
  ...
  if (ds_cirurgiao_w IS NOT NULL AND ds_cirurgiao_w::text <> '') then
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_lista_espera_prof ( nr_seq_lista_p bigint, ie_type_p text) FROM PUBLIC;
