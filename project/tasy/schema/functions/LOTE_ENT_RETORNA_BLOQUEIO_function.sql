...
CREATE OR REPLACE FUNCTION lote_ent_retorna_bloqueio (nr_seq_ficha_p bigint, nr_seq_exame_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (nr_prescricao_w IS NOT NULL AND nr_prescricao_w::text <> '') and (nr_seq_exame_p IS NOT NULL AND nr_seq_exame_p::text <> '') and (nr_seq_prescr_w IS NOT NULL AND nr_seq_prescr_w::text <> '') then
	...
	SELECT	max(SUBSTR(lab_obter_desc_motivo_bloqueio(a.nr_seq_motivo_bloqueio),1,100))
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION lote_ent_retorna_bloqueio (nr_seq_ficha_p bigint, nr_seq_exame_p bigint) FROM PUBLIC;
