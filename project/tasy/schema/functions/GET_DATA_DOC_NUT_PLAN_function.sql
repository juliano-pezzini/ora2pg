...
CREATE OR REPLACE FUNCTION get_data_doc_nut_plan ( nr_atendimento_p bigint, si_option_return_p text) RETURNS varchar AS $body$
DECLARE
...
DSST	DS_STATUS*/
	...
	if (coalesce(nr_sequencia_w,0) = 0) then
		...
		select	CASE WHEN ie_status='W' THEN 'P'  ELSE CASE WHEN coalesce(dt_liberacao::text, '') = '' THEN 'C'  ELSE 'I' END  END ,
			CASE WHEN ie_status='W' THEN wheb_mensagem_pck.get_Texto(1152692)  ELSE CASE WHEN coalesce(dt_liberacao::text, '') = '' THEN wheb_mensagem_pck.get_Texto(1147368)  ELSE wheb_mensagem_pck.get_Texto(1147369) END  END  /*Not applicable or Created or Issued*/
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION get_data_doc_nut_plan ( nr_atendimento_p bigint, si_option_return_p text) FROM PUBLIC;
