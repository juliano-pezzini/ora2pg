...
CREATE OR REPLACE FUNCTION obter_dados_unid_saude_colo ( nr_seq_sisicolo_p bigint , ie_opcao_p text ) RETURNS varchar AS $body$
DECLARE
...
if (nr_seq_unidade_saude_w <> 0)	then 
	begin 
		...
		exception 
			when others then 
			CALL WHEB_MENSAGEM_PCK.Exibir_Mensagem_Abort(278874);
			end;
...
elsif (cd_cnpj_unidade_saude_w <> '') 	then 
	begin 
	if (ie_opcao_p = 'UF') then 
		select	substr(obter_dados_pf_pj(null,cd_cnpj_unidade_saude_w,'UF'),1,2) 
		into STRICT	ds_retorno_w 
		;
	elsif (ie_opcao_p = 'CDM') then 
		select	substr(obter_dados_pf_pj(null,cd_cnpj_unidade_saude_w,'CDM')||calcula_digito('MODULO10',obter_dados_pf_pj(null,cd_cnpj_unidade_saude_w,'CDM')),1,7) 
		into STRICT	ds_retorno_w 
		;
	elsif (ie_opcao_p = 'CNES') then 
		...
		select	substr(obter_nome_pf_pj(null,cd_cnpj_unidade_saude_w),1,40) 
		into STRICT	ds_retorno_w 
		;
	end if;
	end;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_dados_unid_saude_colo ( nr_seq_sisicolo_p bigint , ie_opcao_p text ) FROM PUBLIC;
