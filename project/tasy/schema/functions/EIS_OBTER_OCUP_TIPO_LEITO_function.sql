...
CREATE OR REPLACE FUNCTION eis_obter_ocup_tipo_leito ( cd_classif_setor_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estabelecimento_p bigint) RETURNS bigint AS $body$
DECLARE
...
and (coalesce(cd_classif_setor_p::text, '') = '') then
	...
	and	((cd_classif_setor = cd_classif_setor_p) or (coalesce(cd_classif_setor_p::text, '') = ''));
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eis_obter_ocup_tipo_leito ( cd_classif_setor_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estabelecimento_p bigint) FROM PUBLIC;
