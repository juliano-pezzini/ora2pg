...
CREATE OR REPLACE FUNCTION ehr_inf_esc_risco_insuf_renal ( nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, nr_atendimento_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') RETURNS varchar AS $body$
DECLARE
...
and	nr_sequencia in (	SELECT	max(x.nr_sequencia) 
				from	escala_risco_insuf_renal x 
				where	nr_atendimento		= nr_atendimento_p 
...
ds_comando_w	:= 'Select '||ds_comando_w ||' ds '||chr(13) ||'From escala_risco_insuf_renal ';
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ehr_inf_esc_risco_insuf_renal ( nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, nr_atendimento_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') FROM PUBLIC;
