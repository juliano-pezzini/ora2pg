...
CREATE OR REPLACE FUNCTION obter_nr_sequencia_cpoe (nr_sequencia_p bigint, nr_prescricao_p bigint, ie_tipo_item_p text) RETURNS bigint AS $body$
DECLARE
...
if (coalesce(nr_prescricao_p,0) > 0) and (coalesce(nr_sequencia_p,0) > 0 ) and (ie_tipo_item_p IS NOT NULL AND ie_tipo_item_p::text <> '')then
	...
	elsif (ie_tipo_item_p = 'P') or
			((ie_tipo_item_p = 'AP') )then	--'Procedimento, Anatomia Patológica';
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_nr_sequencia_cpoe (nr_sequencia_p bigint, nr_prescricao_p bigint, ie_tipo_item_p text) FROM PUBLIC;
