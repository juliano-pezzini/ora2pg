...
CREATE OR REPLACE FUNCTION obter_volume_diluicao ( NR_SEQUENCIA_P bigint, NR_PRESCRICAO_P bigint) RETURNS varchar AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	if (cd_material_w IS NOT NULL AND cd_material_w::text <> '') and (qt_dose_w IS NOT NULL AND qt_dose_w::text <> '') and (cd_unid_med_dose_w IS NOT NULL AND cd_unid_med_dose_w::text <> '') and (ie_agrupador_w = 3) then
		...
		if (ds_reconstituir_w IS NOT NULL AND ds_reconstituir_w::text <> '') then
			ds_aux_diluir_w	:= wheb_mensagem_pck.get_texto(306853); --  da reconstituição
		end if;
		...
		if (coalesce(ds_conv_ml_w::text, '') = '') and (coalesce(ds_diluir_w::text, '') = '') then
			...
			ds_diluir_w := 	ds_diluir_w || wheb_mensagem_pck.get_texto(306860) || ' '||  wheb_mensagem_pck.get_texto(306869) || ' ' || -- Diluir a solução em
				ds_dose_diluicao_w||' ' || wheb_mensagem_pck.get_texto(306844) || ' '||ds_material_w || chr(13) || chr(10); -- de
		end if;
		end;
	end if;
	...
	if (cd_material_w IS NOT NULL AND cd_material_w::text <> '') and (qt_dose_w IS NOT NULL AND qt_dose_w::text <> '') and (cd_unid_med_dose_w IS NOT NULL AND cd_unid_med_dose_w::text <> '') and (ie_agrupador_w = 7) then
		...
		ds_rediluir_w := ds_rediluir_w||wheb_mensagem_pck.get_texto(306848) || ' '||replace(qt_solucao_str_w,'.',',')||' '||obter_unid_med_usua('ml')||' ' || wheb_mensagem_pck.get_texto(309596) || ' '|| -- Rediluir	-- da solução em
				 ds_dose_diluicao_w ||' ' || wheb_mensagem_pck.get_texto(306844) || ' '||ds_material_w || chr(13) || chr(10); -- de
		end;
	end if;
	end;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_volume_diluicao ( NR_SEQUENCIA_P bigint, NR_PRESCRICAO_P bigint) FROM PUBLIC;
