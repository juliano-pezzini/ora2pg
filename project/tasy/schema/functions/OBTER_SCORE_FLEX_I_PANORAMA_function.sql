...
CREATE OR REPLACE FUNCTION obter_score_flex_i_panorama ( cd_setor_atendimento_p bigint, nr_atendimento_p bigint, ie_opcao_p text) RETURNS varchar AS $body$
DECLARE
...
C02 CURSOR FOR
	SELECT	b.nm_tabela,
			...
			b.nm_atributo_inf,
			a.nr_seq_escala,
			substr(obter_desc_regra_score_flex_I(nr_seq_score_flex,nr_seq_escala),1,255) ds_score_flex,
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_score_flex_i_panorama ( cd_setor_atendimento_p bigint, nr_atendimento_p bigint, ie_opcao_p text) FROM PUBLIC;
