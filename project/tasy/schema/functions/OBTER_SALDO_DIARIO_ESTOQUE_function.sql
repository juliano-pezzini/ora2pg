...
CREATE OR REPLACE FUNCTION obter_saldo_diario_estoque ( dt_saldo_p timestamp, cd_estabelecimento_p bigint, cd_local_estoque_p text, cd_material_p bigint) RETURNS bigint AS $body$
DECLARE
...
C01 CURSOR FOR
	SELECT	pkg_date_utils.start_of(a.dt_movimento_estoque, 'DD', 0) dt_movimento_estoque,
		ie_entrada_saida,
		sum(CASE WHEN a.cd_acao='1' THEN  qt_estoque  ELSE qt_estoque * -1 END )
	...
	group by pkg_date_utils.start_of(a.dt_movimento_estoque, 'DD', 0),
		ie_entrada_saida
	order by 1;
...
dt_fim_w		:= pkg_date_utils.end_of(dt_saldo_p, 'DAY', 0);
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_saldo_diario_estoque ( dt_saldo_p timestamp, cd_estabelecimento_p bigint, cd_local_estoque_p text, cd_material_p bigint) FROM PUBLIC;
