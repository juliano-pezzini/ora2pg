...
CREATE OR REPLACE FUNCTION obter_se_mantem_dur_agecons (cd_agenda_p bigint, nr_seq_agenda_p bigint, dt_agenda_p timestamp, nr_min_duracao_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (cd_agenda_p IS NOT NULL AND cd_agenda_p::text <> '') and (nr_seq_agenda_p IS NOT NULL AND nr_seq_agenda_p::text <> '') and (dt_agenda_p IS NOT NULL AND dt_agenda_p::text <> '') and (nr_min_duracao_p IS NOT NULL AND nr_min_duracao_p::text <> '') then
	...
	and	dt_agenda_p + nr_min_duracao_p / 1440 > dt_agenda;
end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_mantem_dur_agecons (cd_agenda_p bigint, nr_seq_agenda_p bigint, dt_agenda_p timestamp, nr_min_duracao_p bigint) FROM PUBLIC;
