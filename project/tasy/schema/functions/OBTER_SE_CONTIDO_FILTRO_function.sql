...
CREATE OR REPLACE FUNCTION obter_se_contido_filtro (qt_valor_p text, ds_possib_p text) RETURNS varchar AS $body$
DECLARE
...
if ( qt_pos_separador_w = 0 ) and ( trim(both ds_possib_w) = chr(39)||qt_valor_p||chr(39) ) then
	ds_result_w	:= 'S';
else
	while( qt_pos_separador_w > 0 )  and ( qt_controle_w < 999 ) loop
		if (substr(ds_possib_w,2,qt_pos_separador_w-3) = qt_valor_p ) then
			ds_result_w	:= 'S';
			qt_controle_w	:= 999;
		else
	...
	if (ds_result_w	= 'N')  and (ds_possib_w = chr(39)||qt_valor_p||chr(39)) then
		ds_result_w	:= 'S';
	end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_contido_filtro (qt_valor_p text, ds_possib_p text) FROM PUBLIC;
