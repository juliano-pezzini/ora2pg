...
CREATE OR REPLACE FUNCTION gpi_obter_classif_etapa ( nr_sequencia_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (estrutura_w.Count = 1) then
	...
	FOR i IN REVERSE estrutura_w.Count-1..0 loop
		begin
		cd_classificacao_w	:= cd_classificacao_w || trim(both to_char(estrutura_w[i].nr_seq_apres,'009')) || '.';
		end;
	end loop;
	end;
end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION gpi_obter_classif_etapa ( nr_sequencia_p bigint) FROM PUBLIC;
