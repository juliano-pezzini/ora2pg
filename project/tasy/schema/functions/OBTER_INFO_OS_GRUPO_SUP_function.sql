...
CREATE OR REPLACE FUNCTION obter_info_os_grupo_sup ( dt_parametro_p timestamp, nr_seq_grupo_sup_p bigint, ie_tipo_inf_p text, ie_tipo_valor_p text ) RETURNS bigint AS $body$
DECLARE
...
IE_TIPO_INF_P:
	...
	QT - Qtde total de OSs recebidas no mês (não é a quantidade atual de OSs no grupo)
	QTDU - Qtde de OSs de dúvida recebidas no mês (também não é a quantidade atual)
	QTI - Qtde de OSs com insatisfação
	...
	PRDU - % de OSs de dúvida
	PRI - % de insatisfações
...
if (ie_tipo_inf_p = 'QT') then
	...
	and	((nr_seq_grupo_sup_p = nr_seq_grupo_sup) or (nr_seq_grupo_sup_p = 0));
...
if (ie_tipo_inf_p in ('EN')) then
	...
	if (ie_tipo_valor_p = 'D') then
		...
		where	((nr_seq_grupo_sup = nr_seq_grupo_sup_p) or (nr_seq_grupo_sup_p = 0))
...
if (ie_tipo_inf_p in ('NEW_OS')) then
	if (ie_tipo_valor_p = 'D') then
		...
		where	trunc(a.dt_ordem_servico) = trunc(dt_parametro_p)
...
union all
		...
		and (	coalesce(nr_seq_grupo_sup_p, 0) = 0
			or	nr_seq_grupo_sup_p = a.nr_seq_grupo_sup);
...
if (ie_tipo_inf_p in ('PRI','ENS')) then
	...
	where	((coalesce(nr_seq_gerencia_insatisf, nr_seq_grupo_sup) = nr_seq_grupo_sup_p) or (nr_seq_grupo_sup_p = 0))
	and	dt_fim_real between dt_ref_mes_w and dt_fim_mes_w;
...
if (ie_tipo_inf_p in ('QTDU', 'PRDU')) then
	...
	if (ie_tipo_inf_p = 'PRDU') then
		...
		from	man_ordem_servico x,
			...
			gerencia_wheb z,
			grupo_suporte k
		where	x.nr_seq_grupo_sup = nr_seq_grupo_sup_p
...
if (ie_tipo_inf_p = 'QTPESSOAS') then
	...
	and	g.nr_sequencia = nr_seq_grupo_sup_p;
...
if (ie_tipo_inf_p in ('QTMETAMES', 'QTMETADIA', 'QTMETAHORA', 'QTMETADIAHORA', 'QTMETAMESHORA')) then
	...
	from	usuario_grupo_sup u,
		usuario_nivel_sup u_niv,
		nivel_grupo_sup niv
	where	u.nm_usuario_grupo = u_niv.nm_usuario_sup
	and	u_niv.nr_seq_nivel = niv.nr_sequencia
	and	u.nr_seq_grupo = nr_seq_grupo_sup_p
...
if (ie_tipo_inf_p = 'QTOSDIAANT') then
	...
	select	coalesce(qt_informacao, 0)
	...
	and	nr_seq_grupo = nr_seq_grupo_sup_p;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_info_os_grupo_sup ( dt_parametro_p timestamp, nr_seq_grupo_sup_p bigint, ie_tipo_inf_p text, ie_tipo_valor_p text ) FROM PUBLIC;
