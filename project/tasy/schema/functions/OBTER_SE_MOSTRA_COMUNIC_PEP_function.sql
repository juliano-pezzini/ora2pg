...
CREATE OR REPLACE FUNCTION obter_se_mostra_comunic_pep ( nr_sequencia_p bigint, nm_usuario_p text) RETURNS varchar AS $body$
DECLARE
...
if (nm_usuario_p	= nm_usuario_w) then
	...
	if (ds_perfil_adic_w IS NOT NULL AND ds_perfil_adic_w::text <> '') and (ie_mostra_w <> 'S') then
		...
		while(cd_posicao_w <> '0') and (ie_mostra_w <> 'S') and (qt_controle_w < 500) loop
			...
			cd_perfil_w	:= (substr(ds_perfil_adic_w,1,cd_posicao_w-1))::numeric;
	...
	if (ds_setor_adicional_w IS NOT NULL AND ds_setor_adicional_w::text <> '') and (ie_mostra_w <> 'S') then
		...
		while(cd_posicao_w <> '0') and (ie_mostra_w <> 'S') and (qt_controle_w < 500) loop
			...
			cd_setor_w	:= (substr(ds_setor_adicional_w,1,cd_posicao_w-1))::numeric;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_mostra_comunic_pep ( nr_sequencia_p bigint, nm_usuario_p text) FROM PUBLIC;
