...
CREATE OR REPLACE FUNCTION obter_se_exibe_alerta_medico ( nm_usuario_p text, cd_funcao_ativa_p bigint) RETURNS varchar AS $body$
DECLARE
...
and ((d.cd_perfil = cd_perfil_w) or (coalesce(d.cd_perfil::text, '') = ''));
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_exibe_alerta_medico ( nm_usuario_p text, cd_funcao_ativa_p bigint) FROM PUBLIC;
