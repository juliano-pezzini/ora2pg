...
CREATE OR REPLACE FUNCTION cm_obter_qt_cota_disp ( nr_requisicao_p bigint, nr_seq_item_req_p bigint, qt_conjunto_p bigint) RETURNS bigint AS $body$
DECLARE
...
if (qt_regra_w > 0) then
	...
	if (ie_periodo_w = 'D') then
		...
		if	((qt_conjunto_regra_w - (qt_conj_atendido_w + coalesce(qt_conjunto_p,0))) < 1) then
			qt_retorno_w	:= 0;
		else
			qt_retorno_w	:= (qt_conjunto_regra_w - (qt_conj_atendido_w + coalesce(qt_conjunto_p,0)));
		end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION cm_obter_qt_cota_disp ( nr_requisicao_p bigint, nr_seq_item_req_p bigint, qt_conjunto_p bigint) FROM PUBLIC;
