...
CREATE OR REPLACE FUNCTION ehr_tratamento_anterior (nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') RETURNS varchar AS $body$
DECLARE
...
and	nr_sequencia  in (	SELECT	max(x.nr_sequencia)
				from	historico_saude_tratamento x
				where	cd_pessoa_fisica	= cd_pessoa_fisica_p
...
ds_comando_w	:= 'Select '||ds_comando_w ||' ds '||chr(13) ||'From historico_saude_tratamento ';
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ehr_tratamento_anterior (nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') FROM PUBLIC;
