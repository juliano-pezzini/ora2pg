...
CREATE OR REPLACE FUNCTION obter_texto_ci_autor (nr_sequencia_autor_p bigint, nr_seq_regra_p bigint, ie_opcao_p text) RETURNS varchar AS $body$
DECLARE


/*ie_opcao_p			
T	Retorno o título da mensagem
C	Retorno o conteúdo da mensagem
*/
ds_retorno_w			varchar(4000);
ds_conteudo_w			text;--varchar2(4000);
...
ds_conteudo_w	:= replace_macro(ds_conteudo_w,'@dt_validade_guia',substr(dt_validade_guia_w,1,2000));
...
ds_titulo_w	:= replace_macro(ds_titulo_w,'@dt_nascimento_pac', to_char(obter_data_nascto_pf(cd_pessoa_fisica_w),'dd/mm/yyyy'));
...
ds_titulo_w	:= replace_macro(ds_titulo_w,'@dt_exec_prescr', to_char(dt_baixa_prescr_w,'dd/mm/yyyy hh24:mi:ss'));
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_texto_ci_autor (nr_sequencia_autor_p bigint, nr_seq_regra_p bigint, ie_opcao_p text) FROM PUBLIC;
