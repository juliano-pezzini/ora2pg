...
CREATE OR REPLACE FUNCTION lab_obter_valor_ref_patol (nr_prescricao_p bigint, nr_seq_patologia_p bigint, nr_seq_prescr_p bigint, nr_seq_exame_p bigint, ie_opcao_p bigint) RETURNS varchar AS $body$
DECLARE
...
from ( 
	SELECT 	qt_minima, 
		...
		qt_percent_max, 
		ds_observacao 
	from 	lote_ent_patol_exam_ref 
	where 	((ie_sexo = ie_sexo_w) or (ie_sexo = '0')) 
	and 	nr_seq_exame = nr_seq_exame_p 
	and		nr_seq_patologia = nr_seq_patologia_p 
	...
	order by NR_SEQ_PRIORIDADE, coalesce(nr_seq_material, 9999999999), coalesce(nr_seq_metodo, 9999999999), ie_sexo, CASE WHEN ie_periodo='D' THEN 1 WHEN ie_periodo='M' THEN 2  ELSE 3 END ) alias34 LIMIT 1;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION lab_obter_valor_ref_patol (nr_prescricao_p bigint, nr_seq_patologia_p bigint, nr_seq_prescr_p bigint, nr_seq_exame_p bigint, ie_opcao_p bigint) FROM PUBLIC;
