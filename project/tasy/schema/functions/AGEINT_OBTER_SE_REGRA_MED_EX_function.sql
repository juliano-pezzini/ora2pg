...
CREATE OR REPLACE FUNCTION ageint_obter_se_regra_med_ex ( cd_medico_p text, cd_agenda_p bigint, nr_seq_proc_interno_p bigint, nr_seq_ageint_p bigint, cd_procedimento_p bigint, ie_origem_proced_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (coalesce(nr_seq_proc_interno_p,0)	> 0) then
	...
	AND	((cd_procedimento =  cd_procedimento_w and ie_origem_proced = ie_origem_proced_w ) OR (nr_seq_proc_interno = nr_seq_proc_interno_p) ) and
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ageint_obter_se_regra_med_ex ( cd_medico_p text, cd_agenda_p bigint, nr_seq_proc_interno_p bigint, nr_seq_ageint_p bigint, cd_procedimento_p bigint, ie_origem_proced_p bigint) FROM PUBLIC;
