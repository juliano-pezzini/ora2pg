...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (ds_orientacao_usuario_w IS NOT NULL AND ds_orientacao_usuario_w::text <> '') and (coalesce(ie_mostra_obs_usuario_p,'N') = 'S') then
		ds_retorno_w := SUBSTR(Obter_Desc_Expressao(294897) || ': '  || ds_orientacao_usuario_w || CHR(10),1,4000);
	end if;
	...
	if (ds_orientacao_medicamento_w IS NOT NULL AND ds_orientacao_medicamento_w::text <> '') and (coalesce(ie_mostra_obs_medicamento_p,'N') = 'S') then
		ds_retorno_w := SUBSTR(ds_retorno_w || Obter_Desc_Expressao(774601) || ': ' || ds_orientacao_medicamento_w || CHR(10),1,4000);
	end if;

	if (ie_mostra_obs_paciente_p IS NOT NULL AND ie_mostra_obs_paciente_p::text <> '') and (coalesce(ie_mostra_obs_paciente_p,'N') = 'S') then
		ds_retorno_w := SUBSTR(ds_retorno_w || Obter_Desc_Expressao(710639) || ' ' || ds_orientacao_paciente_w || CHR(10),1,4000);
	end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_orientacao_exame_id_sexo ( nr_seq_exame_p bigint, nr_prescricao_p bigint, cd_estabelecimento_p bigint, ie_mostra_obs_tecnica_p text default null, ie_mostra_obs_usuario_p text default null, ie_mostra_obs_medicamento_p text default null, ie_mostra_obs_paciente_p text default null) FROM PUBLIC;
