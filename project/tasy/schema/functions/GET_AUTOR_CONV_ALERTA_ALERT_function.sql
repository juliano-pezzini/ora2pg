...
CREATE OR REPLACE FUNCTION get_autor_conv_alerta_alert ( nr_seq_atend_alerta_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) RETURNS varchar AS $body$
DECLARE


ds_alert_w		varchar(2000);
nr_seq_tipo_alerta_w	autor_convenio_alerta.nr_seq_tipo_alerta%type;
nr_seq_alerta_w 	autor_convenio_alerta.nr_seq_alerta%type;
dt_alerta_w		autor_convenio_alerta.dt_alerta%type;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION get_autor_conv_alerta_alert ( nr_seq_atend_alerta_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
