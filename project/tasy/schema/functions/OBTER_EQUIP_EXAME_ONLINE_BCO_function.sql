...
CREATE OR REPLACE FUNCTION obter_equip_exame_online_bco ( nr_seq_exame_p bigint, cd_equipamento_p bigint, ie_opcao_p text, ie_tipo_atendimento_p bigint, cd_estabelecimento_p bigint) RETURNS varchar AS $body$
DECLARE
...
where a.cd_equipamento 	= b.cd_equipamento
  ...
  and (to_char(clock_timestamp(),'hh24'))::numeric  >= somente_numero(c.ds_hora_inicio)
  and DS_FORMA_ENVIO_EQUIP = 'BCO'
  and (to_char(clock_timestamp(),'hh24'))::numeric  <= somente_numero(c.ds_hora_fim);
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_equip_exame_online_bco ( nr_seq_exame_p bigint, cd_equipamento_p bigint, ie_opcao_p text, ie_tipo_atendimento_p bigint, cd_estabelecimento_p bigint) FROM PUBLIC;
