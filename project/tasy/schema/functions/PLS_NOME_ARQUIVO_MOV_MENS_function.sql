...
CREATE OR REPLACE FUNCTION pls_nome_arquivo_mov_mens ( nr_seq_lote_p pls_mov_mens_lote.nr_sequencia%type, nr_seq_operadora_p pls_mov_mens_operadora.nr_sequencia%type) RETURNS varchar AS $body$
DECLARE


nm_arquivo_w		varchar(255);
cd_operadora_origem_w	pls_congenere.nr_seq_congenere%type;
cd_operadora_w		pls_mov_mens_operadora.nr_seq_congenere%type;
...
from	pls_mov_mens_lote      a,
	pls_mov_mens_regra     b
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION pls_nome_arquivo_mov_mens ( nr_seq_lote_p pls_mov_mens_lote.nr_sequencia%type, nr_seq_operadora_p pls_mov_mens_operadora.nr_sequencia%type) FROM PUBLIC;
