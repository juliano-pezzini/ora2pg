...
CREATE OR REPLACE FUNCTION obter_pr_real_prog_fase ( nr_seq_programa_p bigint, nr_seq_area_fase_p bigint ) RETURNS bigint AS $body$
DECLARE
...
BEGIN
  ...
  select ( select dividir(t.per_exec * 100,t.qt_hora_prev) pr_realizado
           from ( select sum(pe.qt_hora_prev) qt_hora_prev,
                    ...
                    and    pr.nr_sequencia = nr_seq_programa_p
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_pr_real_prog_fase ( nr_seq_programa_p bigint, nr_seq_area_fase_p bigint ) FROM PUBLIC;
