...
if ((ie_tipo_p = 'EN') or (ie_tipo_p = 'EV')) then
	...
	and		cd_medico_dest = cd_medico_dest_p;
...
if ( ie_tipo_p in ('SV','SE','SH','EQ') ) then
	...
	select	COALESCE(max(ie_confirma_encaminhamento),'X')
	into STRICT 	ie_confirma_encaminhamento_w
	from	regra_regulacao_itens 	
	...
	select	COALESCE(max(ie_confirma_encaminhamento),'X')
	into STRICT 	ie_confirma_encaminhamento_w
	from	regra_regulacao_itens a 	
	where	a.nr_sequencia = ( SELECT max(b.nr_sequencia)
							   from		regra_regulacao_itens b
...
if (COALESCE(ie_opcao_p,'C') = 'C') then
    ...
    return_w := ie_confirma_encaminhamento_w;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_conf_regulacao (ie_tipo_p text, cd_especialidade_p bigint default null, cd_medico_dest_p text default null, cd_material_exame_p text default null, nr_seq_exame_p bigint default null, ie_origem_proced_p bigint default null, nr_seq_motivo_solic_ext_p bigint default null, cd_material_p bigint default null, nr_seq_proc_interno_p bigint default null, nr_seq_exame_lab_p bigint default null, cd_procedimento_p bigint default null, cd_tipo_solicitacao_p text default null, cd_tipo_vaga_p text default null, ie_opcao_p text default 'P', ie_classificacao_p text default null, nr_seq_equip_control_p bigint default null) FROM PUBLIC;
