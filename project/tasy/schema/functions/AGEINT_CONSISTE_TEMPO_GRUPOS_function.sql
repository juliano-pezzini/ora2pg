...
CREATE OR REPLACE FUNCTION ageint_consiste_tempo_grupos ( cd_agenda_p bigint, hr_Agenda_p timestamp, nr_minuto_duracao_p bigint, nr_Seq_ageint_p bigint, nr_seq_ageint_item_p bigint, nm_usuario_p text) RETURNS varchar AS $body$
DECLARE
...
C05 CURSOR FOR
	...
	and		c.nr_seq_grupo_selec	= cd_exame_w
	and		a.nr_Seq_ageint			= nr_seq_ageint_p
	and		b.nr_sequencia			<> nr_seq_ageint_item_p;

C03 CURSOR FOR
	SELECT	nr_seq_grupo_selec
...
C04 CURSOR FOR
	...
	from	ageint_exame_adic_item
	where	nr_seq_item			<> nr_seq_ageint_item_p
	and		nr_seq_grupo_selec	= cd_exame_w
	order by nr_seq_proc_interno;
...
/*
cursor das regras a qual o grupo do item se aplica
*/
open C01;
loop
fetch C01 into
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	/*
	cursor das marcacoes com os grupos ao qual a regra se aplica
	*/
	open C02;
	loop
	fetch C02 into
		...
		EXIT WHEN NOT FOUND; /* apply on C03 */
			...
			if (ds_retorno_w	<> 'N') then
				...
				*/
				...
				EXIT WHEN NOT FOUND; /* apply on C01 */
					begin
					select	CASE WHEN cd_grupo_pri=nr_seq_grupo_adic_w THEN  cd_grupo_seg  ELSE cd_grupo_pri END ,
							qt_tempo
					into STRICT	cd_exame_w,
							qt_tempo_w
					from	ageint_tempo_entre_grupos
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ageint_consiste_tempo_grupos ( cd_agenda_p bigint, hr_Agenda_p timestamp, nr_minuto_duracao_p bigint, nr_Seq_ageint_p bigint, nr_seq_ageint_item_p bigint, nm_usuario_p text) FROM PUBLIC;
