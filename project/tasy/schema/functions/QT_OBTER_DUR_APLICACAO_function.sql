...
CREATE OR REPLACE FUNCTION qt_obter_dur_aplicacao (ds_dia_ciclo_p text, nr_seq_prot_medic_p bigint, cd_protocolo_p bigint, nr_seq_atendimento_p bigint, dt_agenda_p timestamp, nr_seq_pendencia_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) RETURNS bigint AS $body$
DECLARE
...
if (qt_tempo_agenda_w IS NOT NULL AND qt_tempo_agenda_w::text <> '') then
	...
	if (ie_tempo_medic_w = 'C') then
		...
		and		ESTABLISHMENT_TIMEZONE_UTILS.startOfDay(coalesce(b.dt_real, b.dt_prevista))	= ESTABLISHMENT_TIMEZONE_UTILS.startOfDay(dt_agenda_p);
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION qt_obter_dur_aplicacao (ds_dia_ciclo_p text, nr_seq_prot_medic_p bigint, cd_protocolo_p bigint, nr_seq_atendimento_p bigint, dt_agenda_p timestamp, nr_seq_pendencia_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
