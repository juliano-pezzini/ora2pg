...
CREATE OR REPLACE FUNCTION obter_total_tit_cancelados (cd_pessoa_p bigint, tp_pessoa_p bigint, nm_usuario_p text) RETURNS bigint AS $body$
DECLARE
 vl_cancelado double precision;

BEGIN
	select coalesce(sum(a.vl_titulo),0)
	into STRICT vl_cancelado
	from	titulo_pagar a
	where	a.ie_situacao = 'C'
		and ((tp_pessoa_p = 0
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_total_tit_cancelados (cd_pessoa_p bigint, tp_pessoa_p bigint, nm_usuario_p text) FROM PUBLIC;
