...
CREATE OR REPLACE FUNCTION obter_prio_leg_fonte_agenda ( nr_seq_agenda_p bigint) RETURNS varchar AS $body$
DECLARE
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (ie_status_legenda_w = 'LI') then
		...
		ds_cor_w := ds_cor_w;
		goto final;
	elsif (ie_status_legenda_w = 'PS') and (dt_chegada_fim_w IS NOT NULL AND dt_chegada_fim_w::text <> '') then
		ds_cor_w := ds_cor_w;
		goto final;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_prio_leg_fonte_agenda ( nr_seq_agenda_p bigint) FROM PUBLIC;
