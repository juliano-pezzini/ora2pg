...
CREATE OR REPLACE FUNCTION obter_nome_usuario_exec_fim (NR_ORDEM_SERVICO_P bigint, ie_tipo_p text) RETURNS varchar AS $body$
DECLARE
...
if (NR_ORDEM_SERVICO_P IS NOT NULL AND NR_ORDEM_SERVICO_P::text <> '') then
	begin
	select coalesce(max(nm_usuario_exec), obter_desc_expressao(327119)/*'Não Informado'*/
)
	...
	and		exists (SELECT 1
					...
					where	a.nm_usuario_exec = c.nm_usuario_grupo
					and 	coalesce(IE_FUNCAO_USUARIO,'P') = 'P');
	...
	if (nr_seq_exec_w = 0) then
		...
		and		a.nr_seq_ordem 	= nr_ordem_servico_p
		and		coalesce(a.dt_fim_execucao::text, '') = ''
		and	exists (SELECT 1 from usuario_grupo_des c where	a.nm_usuario_exec = c.nm_usuario_grupo)
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_nome_usuario_exec_fim (NR_ORDEM_SERVICO_P bigint, ie_tipo_p text) FROM PUBLIC;
