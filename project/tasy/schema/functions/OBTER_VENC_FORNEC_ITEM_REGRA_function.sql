...
CREATE OR REPLACE FUNCTION obter_venc_fornec_item_regra ( nr_cot_compra_p bigint, nr_item_cot_compra_p bigint) RETURNS bigint AS $body$
DECLARE
...
ie_preco_duplicado_cotacao_w		parametro_compras.ie_preco_duplicado_cotacao%type;
...
if (ie_preco_duplicado_cotacao_w = 'S') and (Obter_Se_Cotacao_Preco_Igual(nr_seq_item_fornec_w) > 0) then 
	begin 
		select	coalesce(obter_forn_maior_venc(nr_cot_compra_p,nr_item_cot_compra_p),0) 
		...
		if nr_seq_cot_forn_w > 0 then 
			...
			and	i.nr_item_cot_compra	= nr_item_cot_compra_p 
			and	i.vl_unitario_material	> 0;		
			 
			if nr_seq_item_fornec_venc_w > 0 then 
				nr_seq_item_fornec_w:=nr_seq_item_fornec_venc_w;
			end if;
		end if;	
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_venc_fornec_item_regra ( nr_cot_compra_p bigint, nr_item_cot_compra_p bigint) FROM PUBLIC;
