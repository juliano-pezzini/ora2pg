...
CREATE OR REPLACE FUNCTION ptu_obter_qtd_trans_interc ( nr_seq_guia_p bigint, nr_seq_requisicao_p bigint, nr_seq_execucao_p bigint, nr_seq_origem_p bigint, cd_unimed_exec_p bigint, cd_unimed_origem_p bigint, ie_tipo_transacao_p text ) RETURNS bigint AS $body$
DECLARE
...
if (nr_seq_execucao_p IS NOT NULL AND nr_seq_execucao_p::text <> '') then
	...
	if (cd_unimed_exec_p IS NOT NULL AND cd_unimed_exec_p::text <> '') then
		ds_comando_w	:= ds_comando_w || 'and cd_unimed_executora	= ' || cd_unimed_exec_p;
	end if;

	if (cd_unimed_origem_p IS NOT NULL AND cd_unimed_origem_p::text <> '') then
		ds_comando_w	:= ds_comando_w || 'and cd_unimed_beneficiario	= ' || cd_unimed_origem_p;
	end if;
	...
	vl_retorno_w	:= qt_registros_w;
elsif (coalesce(nr_seq_execucao_p::text, '') = '' and
	coalesce(nr_seq_origem_p::text, '') = '' and
	coalesce(nr_seq_guia_p::text, '') = '' and
	coalesce(nr_seq_requisicao_p::text, '') = '') then

	if ((cd_unimed_exec_p IS NOT NULL AND cd_unimed_exec_p::text <> '') and coalesce(cd_unimed_origem_p::text, '') = '') then
		ds_comando_w	:= ds_comando_w || 'where cd_unimed_executora	= ' || cd_unimed_exec_p;
	elsif (coalesce(cd_unimed_exec_p::text, '') = '' and (cd_unimed_origem_p IS NOT NULL AND cd_unimed_origem_p::text <> '')) then
		ds_comando_w	:= ds_comando_w || 'where cd_unimed_beneficiario	= ' || cd_unimed_origem_p;
	elsif (cd_unimed_exec_p IS NOT NULL AND cd_unimed_exec_p::text <> '' AND cd_unimed_origem_p IS NOT NULL AND cd_unimed_origem_p::text <> '') then
		ds_comando_w	:= ds_comando_w || 'where cd_unimed_executora	= ' || cd_unimed_exec_p || ' and cd_unimed_beneficiario	= ' || cd_unimed_origem_p;
	end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ptu_obter_qtd_trans_interc ( nr_seq_guia_p bigint, nr_seq_requisicao_p bigint, nr_seq_execucao_p bigint, nr_seq_origem_p bigint, cd_unimed_exec_p bigint, cd_unimed_origem_p bigint, ie_tipo_transacao_p text ) FROM PUBLIC;
