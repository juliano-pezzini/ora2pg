...
CREATE OR REPLACE FUNCTION obter_agendamentos_medic_email (dt_agenda_p timestamp, cd_agenda_p bigint, dt_agenda_fim_p timestamp, cd_medico_p bigint ) RETURNS varchar AS $body$
DECLARE
...
c01 CURSOR FOR 
	SELECT	substr(wheb_mensagem_pck.get_texto(791362) || ' ' || Obter_Nome_Agenda(cd_agenda),1,100) ds_agenda, 
		...
		wheb_mensagem_pck.get_texto(793494) || ': ' || to_char(hr_inicio, pkg_date_formaters.localize_mask('timestamp', pkg_date_formaters.getUserLanguageTag(wheb_usuario_pck.get_cd_estabelecimento, wheb_usuario_pck.get_nm_usuario)))||' - '||CASE WHEN ie_status_agenda='II' THEN wheb_mensagem_pck.get_texto(802664) WHEN ie_status_agenda='B' THEN wheb_mensagem_pck.get_texto(802665) END  hr_inicio, 
	...
	and	cd_medico_exec = cd_medico_p 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_agendamentos_medic_email (dt_agenda_p timestamp, cd_agenda_p bigint, dt_agenda_fim_p timestamp, cd_medico_p bigint ) FROM PUBLIC;
