...
CREATE TYPE campos AS (cd_procedimento 	double precision);


CREATE OR REPLACE FUNCTION analise_obter_se_conta_proced ( nr_interno_conta_p bigint, ds_filtro_procedimento_p text) RETURNS varchar AS $body$
DECLARE
...
while(length(ds_filtro_procedimento_w) > 0) loop
	...
	if (position(',' in ds_filtro_procedimento_w)	>0)  then
		Vetor_w[i].cd_procedimento	:= somente_numero(substr(ds_filtro_procedimento_w,1,position(',' in ds_filtro_procedimento_w) ));
		ds_filtro_procedimento_w	:= substr(ds_filtro_procedimento_w,position(',' in ds_filtro_procedimento_w)+1,40000);

	else
		Vetor_w[i].cd_procedimento	:= somente_numero(ds_filtro_procedimento_w);
...
for j in 1..Vetor_w.count loop
	...
	and	cd_procedimento		= Vetor_w[j].cd_procedimento;
	exception
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION analise_obter_se_conta_proced ( nr_interno_conta_p bigint, ds_filtro_procedimento_p text) FROM PUBLIC;
