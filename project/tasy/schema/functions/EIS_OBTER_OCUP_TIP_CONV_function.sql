...
CREATE OR REPLACE FUNCTION eis_obter_ocup_tip_conv ( ie_tipo_convenio_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estabelecimento_p bigint ) RETURNS bigint AS $body$
DECLARE
...
and (coalesce(ie_tipo_convenio_p::text, '') = '') then 
	...
	from	eis_ocupacao_convenio_v2 a 
	...
	and	ie_periodo = 'D' 
	and	cd_estabelecimento = cd_estabelecimento_p 
	and	((ie_tipo_convenio = ie_tipo_convenio_p) or (coalesce(ie_tipo_convenio_p::text, '') = ''));
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eis_obter_ocup_tip_conv ( ie_tipo_convenio_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estabelecimento_p bigint ) FROM PUBLIC;
