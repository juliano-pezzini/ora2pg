...
CREATE OR REPLACE FUNCTION eis_obter_ocup_especialidade ( cd_especialidade_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estabelecimento_p bigint ) RETURNS bigint AS $body$
DECLARE
...
and (coalesce(cd_especialidade_p::text, '') = '') then 
	...
	from	eis_ocupacao_especialidade_v2 a 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eis_obter_ocup_especialidade ( cd_especialidade_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estabelecimento_p bigint ) FROM PUBLIC;
