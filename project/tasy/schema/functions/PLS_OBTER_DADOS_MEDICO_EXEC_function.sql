...
CREATE OR REPLACE FUNCTION pls_obter_dados_medico_exec ( nr_seq_item_p pls_conta_proc.nr_sequencia%type, ie_mat_proc_p text) RETURNS PLS_UTIL_PCK.DADOS_MEDICO_EXECUTOR_CTA AS $body$
DECLARE
...
Finalidade:	Obtem informações sobre o médico executor da conta.
	...
	Também deve ser ajustada nesse caso, a pls_obter_dados_medic_exec_imp, que é
	a rotina equivalente a essa, utilizada na importação do XML.
-------------------------------------------------------------------------------------------------------------------
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION pls_obter_dados_medico_exec ( nr_seq_item_p pls_conta_proc.nr_sequencia%type, ie_mat_proc_p text) FROM PUBLIC;
