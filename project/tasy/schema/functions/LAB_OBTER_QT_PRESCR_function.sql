...
CREATE OR REPLACE FUNCTION lab_obter_qt_prescr ( nr_prescricao_p bigint) RETURNS varchar AS $body$
DECLARE
...
WHERE a.nr_prescricao		= b.nr_prescricao and coalesce(b.nr_seq_origem::text, '') = '' and a.nr_prescricao	= nr_prescricao_p and a.dt_prescricao		between dt_referencia_w and clock_timestamp() + interval '2 days'  and (coalesce(a.dt_liberacao, a.dt_liberacao_medico) IS NOT NULL AND (coalesce(a.dt_liberacao, a.dt_liberacao_medico))::text <> '') and b.ie_suspenso		= 'N' group by coalesce(nm_curto,'Proc');
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION lab_obter_qt_prescr ( nr_prescricao_p bigint) FROM PUBLIC;
