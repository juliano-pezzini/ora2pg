...
--
-- dblink wrapper to call function ehr_paciente_vacina as an autonomous transaction
--
CREATE EXTENSION IF NOT EXISTS dblink;

CREATE OR REPLACE FUNCTION ehr_paciente_vacina (nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') RETURNS varchar AS $body$
DECLARE
	...
	v_ret	varchar;
BEGIN
	v_query := 'SELECT * FROM ehr_paciente_vacina_atx ( ' || quote_nullable(nr_seq_elemento_p) || ',' || quote_nullable(cd_pessoa_fisica_p) || ',' || quote_nullable(ie_inf_entidade_p) || ',' || quote_nullable(ie_atendimento_p) || ',' || quote_nullable(ie_profissional_p) || ' )';
...
CREATE OR REPLACE FUNCTION ehr_paciente_vacina_atx (nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') RETURNS varchar AS $body$
DECLARE
...
ds_comando_w	:= 'Select '||ds_comando_w ||' ds '||chr(13) ||'From PACIENTE_VACINA ';
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ehr_paciente_vacina (nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') FROM PUBLIC; -- REVOKE ALL ON FUNCTION ehr_paciente_vacina_atx (nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') FROM PUBLIC;
