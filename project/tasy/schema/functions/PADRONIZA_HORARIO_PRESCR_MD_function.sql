...
CREATE OR REPLACE FUNCTION padroniza_horario_prescr_md ( ds_horario_p text, dt_hora_inicio_p text) RETURNS varchar AS $body$
DECLARE
...
if (coalesce(length(dt_hora_inicio_w),0) < 12) then
	dt_hora_inicio_w := substr(coalesce(dt_hora_inicio_w, clock_timestamp()),1,10)||' 00:00:01';
end if;
...
ds_horarios_w := trim(both ds_horarios_w);
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION padroniza_horario_prescr_md ( ds_horario_p text, dt_hora_inicio_p text) FROM PUBLIC;
