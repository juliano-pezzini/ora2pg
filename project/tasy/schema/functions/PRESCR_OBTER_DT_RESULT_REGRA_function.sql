...
CREATE OR REPLACE FUNCTION prescr_obter_dt_result_regra (nr_seq_prescricao_p bigint, nr_prescricao_p bigint) RETURNS timestamp AS $body$
DECLARE
...
BEGIN 
		...
		if (ie_data_resultado_w = 'P') then 
			...
			if (ds_hora_fixa_w IS NOT NULL AND ds_hora_fixa_w::text <> '') then 
				...
				select	(trunc(dt_coleta_w) + (ds_hora_fixa_w/24)) + qt_dia_entrega_w + (CASE WHEN qt_min_entrega_w=0 THEN 0  ELSE qt_min_entrega_w/1440 END ) 
				into STRICT	dt_resultado_w 
				from 	prescr_medica 
				where 	nr_prescricao = nr_prescricao_p;		
			else 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION prescr_obter_dt_result_regra (nr_seq_prescricao_p bigint, nr_prescricao_p bigint) FROM PUBLIC;
