...
CREATE OR REPLACE FUNCTION obter_exames_prescricao ( nr_prescricao_p bigint, cd_setor_atendimento_p bigint, ie_opcao_p text, ie_quebra_linha_p text) RETURNS varchar AS $body$
DECLARE
...
c01 CURSOR FOR 
	...
	or (d.cd_setor_entrega = cd_setor_atendimento_p)) 
...
/* Opções 
	...
	CDLCI	- Código / Descrição / Lado / Código interno 
	DE 	- Descrição Exame (Separado por vírgulas) 
*/
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (ie_opcao_p = 'CQD') then 
		...
		Resultado_w || '(' || to_char(dt_prev_execucao_w,'dd/mm/yyyy hh24:mi:ss') || ')' || ' ' || 
        	to_char(qt_procedimento_w, '9990') || ' - ' || 
		ds_procedimento_w || quebra_w;
	elsif (ie_opcao_p = 'CDLCI') and (qt_lengt_w < 1700) then 
			Resultado_w	:= 	 
			Resultado_w || rpad(substr('(' || cd_procedimento_w || ')' || ' ' ||	       
			ds_procedimento_w,1,40),40) || ' ' || rpad(coalesce(ie_lado_w,' '),4) || ' ' || nr_seq_proc_interno_w || quebra_w;
	elsif (ie_opcao_p = 'DE') and (qt_lengt_w < 1700) then	 
		select 	a.nm_exame 
		into STRICT	nm_exame_w 
		from	exame_laboratorio a 
		where	a.nr_seq_exame	= nr_seq_exame_w 
		and	ie_urgencia_w 	= 'N';
		if (nm_exame_w IS NOT NULL AND nm_exame_w::text <> '') then 
			Resultado_w	:= Resultado_w || nm_exame_w || ', ';
		end if;
	end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_exames_prescricao ( nr_prescricao_p bigint, cd_setor_atendimento_p bigint, ie_opcao_p text, ie_quebra_linha_p text) FROM PUBLIC;
