...
CREATE OR REPLACE FUNCTION obter_valor_auditoria (nr_seq_auditoria_p bigint, nr_interno_conta_p bigint) RETURNS bigint AS $body$
DECLARE
	...
	if (ie_tipo_auditoria_w = 'I') then -- Auditoria Interna
		...
		if (qt_ajuste_w IS NOT NULL AND qt_ajuste_w::text <> '') then
			...
			if (ie_tipo_w = 1) then
				...
				vl_total_w	:=  vl_total_w + round((vl_unitario_w * qt_dif_w)::numeric, 2);
				...
				vl_total_w	:=  vl_total_w + round((vl_preco_w * qt_dif_w)::numeric, 2);
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_valor_auditoria (nr_seq_auditoria_p bigint, nr_interno_conta_p bigint) FROM PUBLIC;
