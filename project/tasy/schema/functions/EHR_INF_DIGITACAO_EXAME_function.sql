...
CREATE OR REPLACE FUNCTION ehr_inf_digitacao_exame ( nr_seq_elemento_p bigint, cd_pessoa_fisica_p text, nr_atendimento_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') RETURNS varchar AS $body$
DECLARE
...
and	a.nr_seq_resultado in (	SELECT	max(x.nr_seq_resultado) 
...
ds_comando_w	:= 'Select '||ds_comando_w ||' ds '||chr(13) ||'From exame_lab_resultado ';
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	begin 
	ds_comando_ww	:= ds_comando_w || chr(13) || 'where nr_seq_resultado = '||to_char(nr_sequencia_w);
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ehr_inf_digitacao_exame ( nr_seq_elemento_p bigint, cd_pessoa_fisica_p text, nr_atendimento_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') FROM PUBLIC;
