...
CREATE OR REPLACE FUNCTION obter_venc_tributo ( nr_sequencia_p bigint, cd_tributo_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (coalesce(nr_seq_lote_res_pls_w,0) <> 0) then /* se possuir a sequência do lote de resumo, então a origem do título é pls - contas médicas, senão a origem é nota fiscal (paulo - os 171927) */
...
if (cd_cond_pagto_w IS NOT NULL AND cd_cond_pagto_w::text <> '') then
	...
	/* data base do vencimento não deve ser final de semana */

	if (pkg_date_utils.get_WeekDay(dt_imposto_w) = 7) then
		-- não deixar passar par ao mes seguinte
...
ds_retorno_w := to_date(substr(dt_imposto_w,1,10),'dd/mm/rrrr');
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_venc_tributo ( nr_sequencia_p bigint, cd_tributo_p bigint) FROM PUBLIC;
