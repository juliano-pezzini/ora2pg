...
CREATE OR REPLACE FUNCTION obter_conteudo_email_gv_html5 ( nr_seq_gv_p bigint, cd_declaracao_p text, cd_estabelecimento_p bigint, nm_usuario_p text) RETURNS text AS $body$
DECLARE
	...
	if (cd_declaracao_p IS NOT NULL AND cd_declaracao_p::text <> '') then
		if (nr_seq_agenda_w IS NOT NULL AND nr_seq_agenda_w::text <> '') then
			...
			if (ie_agenda_paciente_w = 'S') then
				ds_declaracao_w := to_char(OBTER_CONTEUDO_EMAIL_HTML5(nr_seq_agenda_w, cd_declaracao_p));
			end if;
		else
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_conteudo_email_gv_html5 ( nr_seq_gv_p bigint, cd_declaracao_p text, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
