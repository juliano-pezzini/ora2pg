...
CREATE OR REPLACE FUNCTION obter_saldo_disp_estoque ( cd_estabelecimento_p bigint, cd_material_p bigint, cd_local_estoque_p bigint, dt_mesano_referencia_p timestamp, nr_seq_lote_p bigint default null, ie_virada_p text default 'N') RETURNS bigint AS $body$
DECLARE
...
qt_saldo_w2	 double precision;
ds_local_estoque_w	varchar(2000);
ds_local_estoque_w2	varchar(2000);
...
qt_mat_ap_lote_w		ap_lote_item_consistido.qt_material%type;
qt_mat_lote_prod_w		ap_lote_item_consistido.qt_material%type;
...
dt_mesano_referencia_w := PKG_DATE_UTILS.start_of(coalesce(dt_mesano_referencia_p,clock_timestamp()), 'month', 0);
...
if (ie_estoque_lote_w = 'N') or (nr_seq_lote_w <= 0) then
	...
	begin
		...
		if (ie_tipo_local_w = 2)  or (ie_baixa_disp_w = 'S') then
			if (coalesce(ds_local_estoque_w::text, '') = '') then
				ds_local_estoque_w := c01_row.cd_local_estoque;
			else
				ds_local_estoque_w := ds_local_estoque_w ||','|| c01_row.cd_local_estoque;
			end if;
		end if;
		
		select 	max(cd_local_estoque)
		into STRICT		cd_local_estoque_kit_w
		from		kit_estoque
		where	cd_local_estoque = cd_local_estoque_w
		...
		qt_saldo_w := obter_saldo_estoque(cd_estabelecimento_p, cd_material_w, cd_local_estoque_w, dt_mesano_referencia_w, qt_saldo_w);
		
		if (qt_saldo_w2 = 0) then
			qt_saldo_w2 := qt_saldo_w;
		else
			qt_saldo_w2 := qt_saldo_w + qt_saldo_w2;
		end if;

	end;
	end loop;
	
	qt_saldo_w := qt_saldo_w2;

		if (ie_virada_p = 'N') then
			qt_kit_w	:=	qt_kit_w + sup_obter_qt_material_kit(cd_estabelecimento_p, cd_local_estoque_w, cd_material_w,null,ds_local_estoque_w);
		end if;
	...
	qt_cirurgia_w		:= 0;
		if (ie_tipo_local_w = 2) or (ie_baixa_disp_w = 'S') then /*Baixa do disponivel se o local for direto ou se o campo IE_DISP_ESTOQUE estiver marcada*/
			begin
			if (ie_disp_cirurgia_w = 'S') and (coalesce(ie_virada_p, 'N') = 'N') then
				...
				from	setor_atendimento c,
					...
					prescr_material a,
					prescr_medica m,
					table(lista_pck.obter_lista(ds_local_estoque_w))  y
			...
			/*Busca as quantidades que estao em cirurgia no local de consistencia, se nao ter local de consistencia, busca do local do setor da cirurgia*/
				...
				into STRICT	qt_cirurgia_w
				from (  SELECT	/*+ index (a PRESMAT_I3)*/						coalesce(sum(qt_total_dispensar),0) qt_cirurgia
					from	setor_atendimento c,
						cirurgia b,
						material x,
						prescr_material a,
...
union all
					...
					SELECT	/*+ index (a PRESMAT_I3)*/						coalesce(sum(qt_total_dispensar),0) qt_cirurgia
					from	setor_atendimento c,
						cirurgia b,
						material x,
						prescr_material a,
					...
					and	coalesce(a.ie_baixa_estoque_cir::text, '') = ''
					and coalesce(a.qt_total_dispensar,0) > 0
					group by 		coalesce(qt_total_dispensar,0)	) alias20;
			else
				qt_cirurgia_w := 0;
			end if;
			
			if (ie_disp_esp_cirurgia_w = 'S') and (coalesce(ie_virada_p, 'N') = 'N') then
			...
			elsif (ie_disp_esp_cirurgia_w = 'C') and (coalesce(ie_virada_p, 'N') = 'N') then
				select 	sum(qt_cirurgia)
				into STRICT	qt_esp_cirurgia_w
...
union all
					...
					AND	a.cd_setor_atendimento	= b.cd_setor_atendimento
					AND	d.cd_material		= e.cd_material		
					and	d.cd_local_estoque = y.nr_registro
					AND	e.cd_material_estoque	= cd_material_w
					AND	d.cd_motivo_baixa	= 0
					AND	d.ie_status_cirurgia	IN ('CB','AD')
					AND	coalesce(d.ie_baixa_estoque_cir::text, '') = '' 
					and coalesce(d.qt_total_dispensar,0) > 0
					group by coalesce(obter_quantidade_convertida(d.cd_material, d.qt_total_dispensar, d.cd_unidade_medida, 'UME'),0) ) alias23;
			end if;
			

			/*Busca as quantidades que estao prescricao de emergencia. indice definido, motivo OS276540*/

			if (ie_disp_prescr_eme_w = 'S') and (coalesce(ie_virada_p, 'N') = 'N') then
				select	/*+ index (a PRESMAT_I3)*/					coalesce(sum(qt_total_dispensar),0)
			...
			if (ie_disp_ag_cirur_w = 'S') and (coalesce(ie_virada_p, 'N') = 'N') then
		...
		if (ie_disp_sep_gedipa_w = 'S') and (coalesce(ie_virada_p, 'N') = 'N') then
		
			/* Alteracao realizada por Daniel e Hudson */
		...
		if (ie_disp_quimioterapia_w = 'S') and (coalesce(ie_virada_p, 'N') = 'N') then
			...
			and	c.nr_seq_cabine in (
					...
					table(lista_pck.obter_lista(ds_local_estoque_w))  y	
					where	x.cd_local_estoque = y.nr_registro);
					
			qt_quimioterapia_w	:= dividir(coalesce(qt_quimioterapia_w,0) , coalesce(qt_conv_estoque_w,1));
		end if;

		if (ie_disp_req_trans_w = 'S') and (coalesce(ie_virada_p, 'N') = 'N') then
		...
		if (ie_disp_req_trans_estab_w = 'S') and (coalesce(ie_virada_p, 'N') = 'N') then
			...
			from (	SELECT obter_quantidade_convertida(b.cd_material, SUM(c.qt_prevista_entrega) - max(obter_qt_oci_trans_nota(a.nr_ordem_compra, b.nr_item_oci,'S')), b.cd_unidade_medida_compra, 'UME') qt_estoque
				...
				and	a.cd_local_transf = y.nr_registro
				and	exists (select	1
						from	material x
						where	x.cd_material_estoque = cd_material_w
						and	x.cd_material = b.cd_material)
				group by a.nr_ordem_compra, b.nr_item_oci, b.cd_material, b.cd_unidade_medida_compra) alias19;
		end if;
		
		if (ie_disp_ap_lote_w = 'S') and (coalesce(ie_virada_p, 'N') = 'N') then
			select	coalesce(max(sum(b.qt_material)),0)
			...
			qt_mat_ap_lote_w := dividir(coalesce(qt_mat_ap_lote_w,0),coalesce(qt_conv_estoque_w,1));
		end if;
		
		if (ie_disp_lote_prod_w = 'S') and (coalesce(ie_virada_p, 'N') = 'N') then
			select	coalesce(max(sum(a.qt_prevista)),0)
			into STRICT	qt_mat_lote_prod_w
			...
			qt_mat_lote_prod_w := dividir(coalesce(qt_mat_lote_prod_w,0),coalesce(qt_conv_estoque_w,1));
		end if;
		...
		if (PKG_DATE_UTILS.start_of(dt_mesano_referencia_p, 'month', 0) = PKG_DATE_UTILS.start_of(clock_timestamp(), 'month', 0)) or (PKG_DATE_UTILS.start_of(dt_mesano_referencia_p, 'month', 0)	< clock_timestamp() - interval '2000 days') then
			qt_estoque_w	:=	qt_estoque_w +
						...
						coalesce(qt_mat_ap_lote_w, 0) - 
						coalesce(qt_mat_lote_prod_w, 0);
		else
			qt_estoque_w	:=	qt_estoque_w +
						coalesce(qt_saldo_w,0);
		end if;
	...
	for c02_row in c02 loop
		begin
		cd_local_estoque_w := c02_row.cd_local_estoque;

		qt_emprestimo_w		:= 0;
		qt_cirurgia_w		:= 0;

		qt_saldo_w := obter_saldo_estoque_lote(cd_estabelecimento_p, cd_material_w, cd_local_estoque_w, dt_mesano_referencia_w, nr_seq_lote_w);
		...
		if (ie_disp_req_trans_estab_w = 'S') and (coalesce(ie_virada_p, 'N') = 'N') then
			...
			from (	SELECT obter_quantidade_convertida(b.cd_material, SUM(c.qt_prevista_entrega) - max(obter_qt_oci_trans_nota(a.nr_ordem_compra, b.nr_item_oci,'S')), b.cd_unidade_medida_compra, 'UME') qt_estoque
				...
				and	exists (select	1
						...
						where	x.cd_material_estoque = cd_material_w
						and	x.cd_material = b.cd_material)
				group by a.nr_ordem_compra, b.nr_item_oci, b.cd_material, b.cd_unidade_medida_compra) alias17;
		end if;
		...
		if (ie_disp_ap_lote_w = 'S') and (coalesce(ie_virada_p, 'N') = 'N') then
			...
			from	ap_lote_item_consistido b,
				...
				prescr_medica c
			where	b.cd_material = cd_material_w
			and	b.nr_lote_fornec = nr_seq_lote_w
			and	b.cd_local_estoque = cd_local_estoque_w
			and	c.cd_estabelecimento = cd_estabelecimento_p
			and	a.ie_status_lote = 'CO'
			and	a.nr_sequencia = b.nr_lote_ap
		...
		if (ie_disp_lote_prod_w = 'S') and (coalesce(ie_virada_p, 'N') = 'N') then
			...
			from	lote_producao_comp a,
					lote_producao b
			where	a.nr_seq_lote_fornec = nr_seq_lote_w
			and		a.cd_material = cd_material_w
			and		a.cd_local_estoque = cd_local_estoque_w
			and		b.cd_estabelecimento = cd_estabelecimento_p
			and		coalesce(b.dt_confirmacao::text, '') = ''
		...
		if (ie_virada_p = 'N') then
			qt_kit_w	:=	sup_obter_qt_material_kit(cd_estabelecimento_p, cd_local_estoque_w, cd_material_w, nr_seq_lote_w);
		end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_saldo_disp_estoque ( cd_estabelecimento_p bigint, cd_material_p bigint, cd_local_estoque_p bigint, dt_mesano_referencia_p timestamp, nr_seq_lote_p bigint default null, ie_virada_p text default 'N') FROM PUBLIC;
