...
--
-- dblink wrapper to call function ehr_historico_saude_internacao as an autonomous transaction
--
CREATE EXTENSION IF NOT EXISTS dblink;

CREATE OR REPLACE FUNCTION ehr_historico_saude_internacao (nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') RETURNS varchar AS $body$
DECLARE
	...
	v_ret	varchar;
BEGIN
	v_query := 'SELECT * FROM ehr_historico_saude_internacao_atx ( ' || quote_nullable(nr_seq_elemento_p) || ',' || quote_nullable(cd_pessoa_fisica_p) || ',' || quote_nullable(ie_inf_entidade_p) || ',' || quote_nullable(ie_atendimento_p) || ',' || quote_nullable(ie_profissional_p) || ' )';
...
CREATE OR REPLACE FUNCTION ehr_historico_saude_internacao_atx (nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') RETURNS varchar AS $body$
DECLARE
...
and	nr_sequencia in (	SELECT	max(x.nr_sequencia) 
				from	HISTORICO_SAUDE_INTERNACAO x 
				where	cd_pessoa_fisica	= cd_pessoa_fisica_p 
...
ds_comando_w	:= 'Select '||ds_comando_w ||' ds '||chr(13) ||'From HISTORICO_SAUDE_INTERNACAO ';
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ehr_historico_saude_internacao (nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') FROM PUBLIC; -- REVOKE ALL ON FUNCTION ehr_historico_saude_internacao_atx (nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N') FROM PUBLIC;
