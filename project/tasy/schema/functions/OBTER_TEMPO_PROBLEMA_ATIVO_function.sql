...
CREATE OR REPLACE FUNCTION obter_tempo_problema_ativo (nr_seq_problema_p bigint) RETURNS varchar AS $body$
DECLARE
...
BEGIN
	select	coalesce(max(coalesce(obter_dt_origem_problema(nr_sequencia,'O'),dt_inicio)),''),
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_tempo_problema_ativo (nr_seq_problema_p bigint) FROM PUBLIC;
