...
CREATE OR REPLACE FUNCTION obter_motivo_transf_agenda (nr_seq_agenda_p bigint, nr_seq_motivo_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (nr_seq_agenda_p IS NOT NULL AND nr_seq_agenda_p::text <> '') then
	...
	select	max(ds_motivo_copia_trans),
		'(' || max(nm_usuario_copia_trans) || chr(32) || wheb_mensagem_pck.get_texto(456028) || chr(32) || PKG_DATE_FORMATERS.TO_VARCHAR(max(dt_copia_trans), 'timestamp', WHEB_USUARIO_PCK.GET_CD_ESTABELECIMENTO, WHEB_USUARIO_PCK.GET_NM_USUARIO) || ')'
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_motivo_transf_agenda (nr_seq_agenda_p bigint, nr_seq_motivo_p bigint) FROM PUBLIC;
