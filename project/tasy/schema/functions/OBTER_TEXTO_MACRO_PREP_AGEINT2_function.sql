...
CREATE OR REPLACE FUNCTION obter_texto_macro_prep_ageint2 ( nr_seq_regra_prep_p bigint, nr_seq_ageint_item_p bigint, ds_texto_p text, ie_dia_p text default 'S') RETURNS text AS $body$
DECLARE
...
if (nr_seq_ageint_item_p IS NOT NULL AND nr_seq_ageint_item_p::text <> '') and (ds_texto_p IS NOT NULL AND ds_texto_p::text <> '') then
	...
	while(ind > 0) loop
		...
		ds_retorno_w := replace_macro_clob2(ds_retorno_w, lista_macros_w(ind), ''||qt_valor_w);
		ind := ind-1;
	end loop;
end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_texto_macro_prep_ageint2 ( nr_seq_regra_prep_p bigint, nr_seq_ageint_item_p bigint, ds_texto_p text, ie_dia_p text default 'S') FROM PUBLIC;
