...
CREATE OR REPLACE FUNCTION far_obter_preco_material ( cd_material_p bigint, cd_estabelecimento_p bigint, nr_seq_contrato_conv_p bigint) RETURNS bigint AS $body$
DECLARE
...
vl_preco_w := coalesce(vl_preco_venda_w,0) - (dividir((vl_preco_venda_w * coalesce(tx_ajuste_contrato_conv,0)),100));
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION far_obter_preco_material ( cd_material_p bigint, cd_estabelecimento_p bigint, nr_seq_contrato_conv_p bigint) FROM PUBLIC;
