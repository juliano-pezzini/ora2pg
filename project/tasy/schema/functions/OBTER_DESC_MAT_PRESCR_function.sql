...
CREATE OR REPLACE FUNCTION obter_desc_mat_prescr ( nr_prescricao_p bigint, nr_sequencia_p bigint, ie_localizacao_p text, cd_local_estoque_p bigint) RETURNS varchar AS $body$
DECLARE
...
select	substr(max(Obter_Descricao_mat_prescr(c.cd_material)),1,100),
...
if (ie_urgencia_w = 'S') then
	ds_retorno_w	:= '#' || replace(ds_retorno_w,'#','') || '#' || 'NI' ||'#P';
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_desc_mat_prescr ( nr_prescricao_p bigint, nr_sequencia_p bigint, ie_localizacao_p text, cd_local_estoque_p bigint) FROM PUBLIC;
