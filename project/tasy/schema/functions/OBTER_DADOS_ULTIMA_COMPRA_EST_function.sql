...
CREATE OR REPLACE FUNCTION obter_dados_ultima_compra_est ( cd_estabelecimento_p bigint, cd_material_p bigint, ie_retorno_p text, qt_dias_pesquisa_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (ie_inf_ultima_compra_w = 'S') then
	...
	if (nr_sequencia_w = 0) then
		...
		and	x.dt_emissao >= clock_timestamp() - qt_dias_pesquisa_p;
		end;
	end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_dados_ultima_compra_est ( cd_estabelecimento_p bigint, cd_material_p bigint, ie_retorno_p text, qt_dias_pesquisa_p bigint) FROM PUBLIC;
