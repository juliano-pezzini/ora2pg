...
CREATE OR REPLACE FUNCTION pls_obter_valor_segurado (nr_seq_segurado_p bigint, ie_opcao_p text) RETURNS bigint AS $body$
DECLARE


vl_sem_desconto_w		double precision	:= 0;
vl_desconto_w			double precision	:= 0;
vl_com_desconto_w		double precision	:= 0;
...
if (coalesce(nr_seq_segurado_p,0) <> 0) then
	...
	if (nr_seq_preco_w > 0) then
		select	coalesce(vl_preco_atual,0),
			...
			(coalesce(vl_preco_atual,0)+coalesce(vl_adaptacao,0) - coalesce(vl_desconto,0))
		into STRICT	vl_sem_desconto_w,
			vl_desconto_w,
			vl_com_desconto_w
		from	pls_segurado_preco
		where	nr_sequencia	= nr_seq_preco_w;

		if (ie_opcao_p	= 'VSD') then
			vl_retorno_w	:= vl_sem_desconto_w;
		elsif (ie_opcao_p	= 'VD') then
			vl_retorno_w	:= vl_desconto_w;
		elsif (ie_opcao_p	= 'VCD') then
			vl_retorno_w	:= vl_com_desconto_w;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION pls_obter_valor_segurado (nr_seq_segurado_p bigint, ie_opcao_p text) FROM PUBLIC;
