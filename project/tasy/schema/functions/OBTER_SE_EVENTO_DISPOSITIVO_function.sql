-- Generated by Ora2Pg, the Oracle database Schema converter, version 23.1
-- Copyright 2000-2022 Gilles DAROLD. All rights reserved.
-- DATASOURCE: dbi:Oracle:host=srv-dbora-03.whebdc.com.br;service_name=DEV_1815;port=1521

SET client_encoding TO 'UTF8';





CREATE OR REPLACE FUNCTION obter_se_evento_dispositivo ( nr_seq_disp_pac_p bigint, nr_seq_evento_p bigint ) RETURNS varchar AS $body$
DECLARE


nr_seq_dispositivo_w	bigint;
ie_evento_w		varchar(1);


BEGIN

select	nr_seq_dispositivo
into STRICT	nr_seq_dispositivo_w
from	atend_pac_dispositivo
where	nr_sequencia		= nr_seq_disp_pac_p;

select	coalesce(max('S'),'N')
into STRICT	ie_evento_w
from	dispositivo_evento
where	nr_seq_dispositivo	= nr_seq_dispositivo_w
and	nr_seq_evento		= nr_seq_evento_p;

return	ie_evento_w;

end;
$body$
LANGUAGE PLPGSQL
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_evento_dispositivo ( nr_seq_disp_pac_p bigint, nr_seq_evento_p bigint ) FROM PUBLIC;

