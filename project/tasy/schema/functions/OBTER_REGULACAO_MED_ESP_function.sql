...
CREATE OR REPLACE FUNCTION obter_regulacao_med_esp ( cd_especialidade_p bigint, cd_medico_dest_p text, ie_tipo_regra_p text, ie_grupo_p text default 'N', ie_somente_ref_p text default 'N') RETURNS bigint AS $body$
DECLARE
...
and		coalesce(a.cd_medico_dest, coalesce(cd_medico_dest_p,0))			= coalesce(cd_medico_dest_p,0)
and		coalesce(a.cd_especialidade, coalesce(cd_especialidade_p,0))		= coalesce(cd_especialidade_p,0)
order by   coalesce(a.cd_medico_dest,0),
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_regulacao_med_esp ( cd_especialidade_p bigint, cd_medico_dest_p text, ie_tipo_regra_p text, ie_grupo_p text default 'N', ie_somente_ref_p text default 'N') FROM PUBLIC;
