...
CREATE OR REPLACE FUNCTION eis_obter_ocup_conv_setor ( cd_convenio_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estabelecimento_p bigint, cd_setor_atendimento_p bigint) RETURNS bigint AS $body$
DECLARE
...
and (coalesce(cd_convenio_p::text, '') = '') then 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION eis_obter_ocup_conv_setor ( cd_convenio_p bigint, dt_inicial_p timestamp, dt_final_p timestamp, cd_estabelecimento_p bigint, cd_setor_atendimento_p bigint) FROM PUBLIC;
