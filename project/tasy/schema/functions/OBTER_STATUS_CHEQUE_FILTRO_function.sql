...
CREATE OR REPLACE FUNCTION obter_status_cheque_filtro ( nr_seq_cheque_p bigint, dt_parametro_p timestamp, ie_devolvido_cobranca_p text) RETURNS varchar AS $body$
DECLARE
...
if (dt_parametro_w	>= dt_cobranca_w) and (dt_cobranca_w > dt_status_w) and
	((coalesce(ie_devolvido_w,'N') = 'N') or (coalesce(ie_devolvido_cobranca_p,'N') = 'N')) then		-- enviado para cobrança
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_status_cheque_filtro ( nr_seq_cheque_p bigint, dt_parametro_p timestamp, ie_devolvido_cobranca_p text) FROM PUBLIC;
