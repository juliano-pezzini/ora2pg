...
CREATE OR REPLACE FUNCTION obter_total_tit_liquidados (cd_pessoa_p bigint, tp_pessoa_p bigint, nm_usuario_p text) RETURNS bigint AS $body$
DECLARE
 vl_liquidado double precision;

BEGIN
	select coalesce(sum(a.vl_titulo),0)
	into STRICT vl_liquidado
	from	titulo_pagar a
	where	a.ie_situacao = 'L'
	and ((tp_pessoa_p = 0
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_total_tit_liquidados (cd_pessoa_p bigint, tp_pessoa_p bigint, nm_usuario_p text) FROM PUBLIC;
