...
CREATE OR REPLACE FUNCTION obter_se_susp_transfer_proc ( cd_setor_atendimento_p bigint, nr_seq_proc_interno_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (nr_seq_regra_w > 0) then -- if there is rule for the department
	...
	if (nr_seq_regra_proc_w > 0) then
		...
		if (cont_rule_susp_proc_w > 0) then
			...
			select	count(*)
			into STRICT	qt_regra_espec_proc_w
			from 	CPOE_RULE_SUSP_PROC d
			where	d.NR_SEQ_RULE_SUSP_ITEMS = nr_seq_regra_proc_w
			and		d.ie_situacao = 'S'
			and		((coalesce(d.nr_seq_proc_interno::text, '') = '') or (d.nr_seq_proc_interno = nr_seq_proc_interno_p))
			and 	((coalesce(d.ie_tipo::text, '') = '') or (d.ie_tipo = obter_tipo_proc_interno(nr_seq_proc_interno_p)));
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_susp_transfer_proc ( cd_setor_atendimento_p bigint, nr_seq_proc_interno_p bigint) FROM PUBLIC;
