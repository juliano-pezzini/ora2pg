...
CREATE OR REPLACE FUNCTION obter_vl_ultima_compra_pf ( cd_estabelecimento_p bigint, qt_dia_p bigint, cd_material_p bigint, cd_local_estoque_p bigint, cd_pessoa_fisica_p text, ie_tipo_p text) RETURNS bigint AS $body$
DECLARE
...
/*ie_tipo_p
U - Vai buscar o valor unitário do item.
*/
BEGIN
ie_tipo_w			:= ie_tipo_p;
vl_retorno_w		:= 0;

if (ie_tipo_w in ('N','U')) then
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_vl_ultima_compra_pf ( cd_estabelecimento_p bigint, qt_dia_p bigint, cd_material_p bigint, cd_local_estoque_p bigint, cd_pessoa_fisica_p text, ie_tipo_p text) FROM PUBLIC;
