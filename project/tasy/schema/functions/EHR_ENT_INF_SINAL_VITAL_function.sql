...
CREATE OR REPLACE FUNCTION ehr_ent_inf_sinal_vital ( nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, nr_atendimento_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N', nr_seq_triagem_p bigint default 0) RETURNS varchar AS $body$
DECLARE
...
and coalesce(nr_seq_triagem_p,0) = 0
...
and		nr_sequencia in (select	max(x.nr_sequencia)
						 from	atendimento_sinal_vital x
						 where	x.cd_paciente = cd_pessoa_fisica_p
...
ds_comando_w	:= 'Select '||ds_comando_w ||' ds '||chr(13) ||'From atendimento_sinal_vital ';
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION ehr_ent_inf_sinal_vital ( nr_seq_elemento_p bigint, cd_pessoa_fisica_p bigint, nr_atendimento_p bigint, ie_inf_entidade_p text, ie_atendimento_p text, ie_profissional_p text default 'N', nr_seq_triagem_p bigint default 0) FROM PUBLIC;
