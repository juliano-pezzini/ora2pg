...
CREATE OR REPLACE FUNCTION obter_valor_orc_pac_adic ( nr_seq_orcamento_p bigint, ie_adicional_p text) RETURNS bigint AS $body$
DECLARE
...
if (nr_seq_orcamento_p IS NOT NULL AND nr_seq_orcamento_p::text <> '') then
	...
	and	coalesce(ie_adicional,'N') = ie_adicional_p;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_valor_orc_pac_adic ( nr_seq_orcamento_p bigint, ie_adicional_p text) FROM PUBLIC;
