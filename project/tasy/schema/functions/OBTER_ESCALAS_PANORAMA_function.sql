...
CREATE OR REPLACE FUNCTION obter_escalas_panorama ( cd_setor_atendimento_p bigint, nr_atendimento_p bigint, ie_opcao_p text) RETURNS varchar AS $body$
DECLARE
...
C02 CURSOR FOR
	SELECT	b.nm_tabela,
			...
			b.nm_atributo_inf,
			a.nr_seq_escala,
			substr(obter_desc_regra_score_flex(nr_seq_score_flex,nr_seq_escala),1,255) ds_score_flex,
			...
			EXIT WHEN NOT FOUND; /* apply on C02 */
				...
				if ( coalesce(nr_sequencia_w,0) > 0) then
					...
					if (ds_func_escala_suep_w IS NOT NULL AND ds_func_escala_suep_w::text <> '') then
						...
						if ( coalesce(ds_score_flex_w::text, '') = '' ) then
								
							ds_retorno_w := substr(ds_retorno_w || ds_escala_w ||': '|| vl_informacao_func_w || ' - '|| dt_referencia_w || ' <br> ',1,4000);
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_escalas_panorama ( cd_setor_atendimento_p bigint, nr_atendimento_p bigint, ie_opcao_p text) FROM PUBLIC;
