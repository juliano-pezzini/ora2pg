...
CREATE OR REPLACE FUNCTION obter_dados_compl_pf_data ( cd_pessoa_fisica_p text, ie_tipo_complemento_p bigint, dt_referencia_p timestamp, ds_atributo_p text) RETURNS varchar AS $body$
DECLARE
...
DBMS_SQL.BIND_VARIABLE(C010, 'ie_tipo_complemento_p', ie_tipo_complemento_p);
...
if (coalesce(nm_retorno_w::text, '') = '') then
	...
	DBMS_SQL.BIND_VARIABLE(C011, 'ie_tipo_complemento_p', ie_tipo_complemento_p);
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_dados_compl_pf_data ( cd_pessoa_fisica_p text, ie_tipo_complemento_p bigint, dt_referencia_p timestamp, ds_atributo_p text) FROM PUBLIC;
