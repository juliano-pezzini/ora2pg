...
CREATE OR REPLACE FUNCTION obter_status_proc_area_adep ( nr_seq_processo_p bigint, nr_seq_area_prep_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (nr_seq_processo_p IS NOT NULL AND nr_seq_processo_p::text <> '') and (nr_seq_area_prep_p IS NOT NULL AND nr_seq_area_prep_p::text <> '') then
	...
	select	max(ie_status_processo),
		max(ie_origem_processo)
	into STRICT	ie_status_processo_w,
		ie_origem_processo_w
	from	adep_processo
	where	nr_sequencia	= nr_seq_processo_p;

	if (ie_origem_processo_w = 'GGP') and (ie_status_processo_w = 'L') then
		ie_status_processo_w	:= 'P';
	end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_status_proc_area_adep ( nr_seq_processo_p bigint, nr_seq_area_prep_p bigint) FROM PUBLIC;
