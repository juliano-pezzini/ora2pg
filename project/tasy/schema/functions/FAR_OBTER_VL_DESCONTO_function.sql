...
CREATE OR REPLACE FUNCTION far_obter_vl_desconto ( cd_material_p bigint, cd_estabelecimento_p bigint, nr_seq_contrato_conv_p bigint) RETURNS bigint AS $body$
DECLARE
...
vl_preco_venda_w		:= coalesce(vl_preco_w,0) + (dividir((vl_preco_w * coalesce(tx_ajuste_w,0)),100));
vl_preco_minimo_w		:= coalesce(vl_preco_w,0) + (dividir((vl_preco_w * coalesce(tx_ajuste_minimo_w,0)),100));
vl_desconto_w 		:= (dividir((vl_preco_w * coalesce(tx_ajuste_contrato_conv,0)),100));
vl_preco_w 		:= coalesce(vl_preco_venda_w,0) - coalesce(vl_desconto_w,0);

if (vl_preco_w < vl_preco_minimo_w) then
	vl_desconto_w	:= (vl_preco_venda_w - vl_preco_minimo_w);
end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION far_obter_vl_desconto ( cd_material_p bigint, cd_estabelecimento_p bigint, nr_seq_contrato_conv_p bigint) FROM PUBLIC;
