...
CREATE OR REPLACE FUNCTION con_pr_conhecimento_tasy_coor (cd_consultor_p bigint, cd_coordenador_p text, ie_tipo_p text) RETURNS varchar AS $body$
DECLARE
...
if (ie_tipo_p = 'CO') then
	...
	and		c.cd_coordenador_padrao = cd_coordenador_p
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION con_pr_conhecimento_tasy_coor (cd_consultor_p bigint, cd_coordenador_p text, ie_tipo_p text) FROM PUBLIC;
