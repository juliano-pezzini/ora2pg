...
CREATE OR REPLACE FUNCTION obter_valor_orc_pac (nr_seq_orcamento_p bigint) RETURNS bigint AS $body$
DECLARE
...
if (nr_seq_orcamento_p IS NOT NULL AND nr_seq_orcamento_p::text <> '') then
	...
	if (2 = philips_param_pck.get_cd_pais) then
		...
		and	not exists (
				SELECT	1
				from	orcamento_proc_imposto x
				where	x.nr_seq_proc = a.nr_sequencia);

		select	coalesce(sum(a.vl_proc_imposto),0)
		into STRICT	vl_proc_imposto_w
		...
		and	not exists (
				SELECT	1
				from	orcamento_mat_imposto x
				where	x.nr_seq_mat = a.nr_sequencia);

		select	coalesce(sum(a.vl_mat_imposto),0)
		into STRICT	vl_mat_imposto_w
		from 	orcamento_paciente_mat	b,
	...
	if (vl_desconto_w	= 0) then
		...
		if (ie_desconto_pacote_w = 'S') then
			...
			select	coalesce(sum(vl_desconto),0)
			into STRICT	vl_desc_proced_pacote_w
			from	orcamento_paciente_proc
			where	nr_sequencia_orcamento = nr_seq_orcamento_p
...
vl_orcamento_w	:= round(vl_procedimento_w + vl_material_w - (vl_desconto_w  + vl_desconto_ww),2);
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_valor_orc_pac (nr_seq_orcamento_p bigint) FROM PUBLIC;
