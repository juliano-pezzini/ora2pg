...
CREATE OR REPLACE FUNCTION obter_status_servico_leito ( nr_seq_interno_p bigint, ie_opcao_p text ) RETURNS varchar AS $body$
DECLARE
...
and	a.nr_sequencia	=	(	SELECT	max(x.nr_sequencia)
					...
					and	x.dt_prevista between trunc(clock_timestamp(),'dd') and (trunc(clock_timestamp(),'dd') + (86399/86400)));
exception
	when others then
	ds_retorno_w	:= ' ';
end;
...
if (ie_opcao_p = 'D') and (coalesce(ds_retorno_w::text, '') = '') then
	ds_retorno_w := wheb_mensagem_pck.get_texto(309491, 'DS_STATUS_W=' || ds_status_w || ';' ||
							'DS_SERVICO_W=' || ds_servico_w || ';' ||
							'DT_PREVISTA_W=' || to_char(dt_prevista_w,'dd/mm/yy hh24:mi') || ';' ||
							'DT_INICIO_W=' || coalesce(to_char(dt_inicio_w,'dd/mm/yy hh24:mi'),wheb_mensagem_pck.get_texto(802901)));
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_status_servico_leito ( nr_seq_interno_p bigint, ie_opcao_p text ) FROM PUBLIC;
