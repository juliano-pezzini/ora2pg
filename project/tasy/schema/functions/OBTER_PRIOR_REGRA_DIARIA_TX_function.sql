...
CREATE OR REPLACE FUNCTION obter_prior_regra_diaria_tx ( nr_sequencia_p bigint, nr_seq_regra_p bigint) RETURNS bigint AS $body$
DECLARE
...
select	coalesce(min(CASE WHEN coalesce(ie_prioridade,0)=0 THEN  999  ELSE ie_prioridade END ),999)
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_prior_regra_diaria_tx ( nr_sequencia_p bigint, nr_seq_regra_p bigint) FROM PUBLIC;
