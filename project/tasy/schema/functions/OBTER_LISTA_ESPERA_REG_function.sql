...
CREATE OR REPLACE FUNCTION obter_lista_espera_reg ( nr_sequencia_p bigint, ie_opcao_p text) RETURNS varchar AS $body$
DECLARE
...
if (nr_sequencia_p IS NOT NULL AND nr_sequencia_p::text <> '') then
	...
	if ( ie_opcao_p = 'RE') then
		...
		If (nr_seq_regulacao_w IS NOT NULL AND nr_seq_regulacao_w::text <> '') then
			...
			Select  substr(obter_valor_dominio(8800,ie_tipo),1,255)
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_lista_espera_reg ( nr_sequencia_p bigint, ie_opcao_p text) FROM PUBLIC;
