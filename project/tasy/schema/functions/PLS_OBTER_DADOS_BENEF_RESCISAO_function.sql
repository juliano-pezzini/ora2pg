...
CREATE OR REPLACE FUNCTION pls_obter_dados_benef_rescisao ( nr_seq_notific_pagador_p pls_notificacao_pagador.nr_sequencia%type, ie_opcao_p text) RETURNS bigint AS $body$
DECLARE
...
c01 CURSOR(	nr_seq_notific_pagador_pc	pls_notificacao_pagador.nr_sequencia%type)	FOR
	SELECT	nr_sequencia
	from	pls_segurado	a
	where	(a.dt_rescisao IS NOT NULL AND a.dt_rescisao::text <> '')
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION pls_obter_dados_benef_rescisao ( nr_seq_notific_pagador_p pls_notificacao_pagador.nr_sequencia%type, ie_opcao_p text) FROM PUBLIC;
