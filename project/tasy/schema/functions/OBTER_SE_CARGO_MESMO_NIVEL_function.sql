...
CREATE OR REPLACE FUNCTION obter_se_cargo_mesmo_nivel ( cd_cargo_padrao_p bigint, cd_cargo_verificar_p bigint, cd_estabelecimento_p bigint) RETURNS varchar AS $body$
DECLARE
...
if (ie_aprovacao_nivel_w = 'S') then 
	...
	from	cargo 
	where	cd_cargo = cd_cargo_padrao_p;
 
	select	nr_nivel_aprovacao 
	into STRICT	nr_nivel_aprovacao_ww 
	from	cargo 
	where	cd_cargo = cd_cargo_verificar_p;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_cargo_mesmo_nivel ( cd_cargo_padrao_p bigint, cd_cargo_verificar_p bigint, cd_estabelecimento_p bigint) FROM PUBLIC;
