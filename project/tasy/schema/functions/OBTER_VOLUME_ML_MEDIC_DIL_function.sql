...
CREATE OR REPLACE FUNCTION obter_volume_ml_medic_dil ( nr_prescricao_p bigint, nr_sequencia_p bigint, ie_ceil_unitario_p text default 'N') RETURNS bigint AS $body$
DECLARE
...
C01 CURSOR FOR
	SELECT	nr_sequencia,
		CASE WHEN ie_ceil_unitario_p='N' THEN 			coalesce(obter_conversao_ml(cd_material,qt_dose,cd_unidade_medida_dose),0)  ELSE coalesce(obter_conversao_ml(cd_material,ceil(qt_unitaria),cd_unidade_medida),0) END ,
		cd_material,
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_volume_ml_medic_dil ( nr_prescricao_p bigint, nr_sequencia_p bigint, ie_ceil_unitario_p text default 'N') FROM PUBLIC;
