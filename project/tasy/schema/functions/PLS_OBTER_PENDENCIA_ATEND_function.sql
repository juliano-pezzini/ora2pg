...
CREATE OR REPLACE FUNCTION pls_obter_pendencia_atend ( cd_pessoa_p text, ie_tipo_pessoa text, nm_usuario_p text) RETURNS bigint AS $body$
DECLARE
...
ds_retorno_w := (coalesce(qt_incosistencia_w,0) + coalesce(qt_mensalidade_w,0) + coalesce(qt_guia_w,0) + coalesce(qt_sac_boletim_w,0) + coalesce(qt_notif_atend_w,0)
		+ coalesce(qt_liminar_w,0) + coalesce(qt_carencia_w,0) + coalesce(qt_pendencia_w,0) + coalesce(qt_cpt_w,0) + coalesce(qt_pendencia_rescisao_w,0));
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION pls_obter_pendencia_atend ( cd_pessoa_p text, ie_tipo_pessoa text, nm_usuario_p text) FROM PUBLIC;
