...
CREATE OR REPLACE FUNCTION obter_valor_regra_lanc_aut ( cd_estabelecimento_p bigint, cd_convenio_p bigint, cd_categoria_p text, cd_procedimento_p bigint, ie_origem_proced_p bigint, nr_seq_proc_interno_p bigint, nr_seq_evento_p bigint, ie_opcao_p text, nr_seq_exame_p bigint) RETURNS bigint AS $body$
DECLARE
    ...
    if (cd_procedimento_w IS NOT NULL AND cd_procedimento_w::text <> '') then
      ...
      if (ie_situacao_w = 'I') then
		...
		if (ie_classificacao_w		= 1 ) then
			...
			SELECT * FROM define_preco_servico(	cd_estabelecimento_p, cd_convenio_p, cd_categoria_w, dt_execucao_w, cd_procedimento_w, 0, 1, 0, null, null, 0, 0, null, vl_servico_w, nr_seq_ajuste_proc_w, null, 0, null, nr_seq_proc_interno_p, null, null, null, null, null, null, null, null, null, null) INTO STRICT vl_servico_w, nr_seq_ajuste_proc_w;
		end if;		
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_valor_regra_lanc_aut ( cd_estabelecimento_p bigint, cd_convenio_p bigint, cd_categoria_p text, cd_procedimento_p bigint, ie_origem_proced_p bigint, nr_seq_proc_interno_p bigint, nr_seq_evento_p bigint, ie_opcao_p text, nr_seq_exame_p bigint) FROM PUBLIC;
