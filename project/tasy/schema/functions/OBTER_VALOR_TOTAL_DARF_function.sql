...
CREATE OR REPLACE FUNCTION obter_valor_total_darf (nr_seq_darf_p bigint) RETURNS bigint AS $body$
DECLARE
...
if (ie_permite_juros_multas_tit_w = 'S') then 
	...
	select	sum(coalesce((SUBSTR(obter_dados_tit_pagar(a.nr_titulo,'V'),1,200))::numeric ,0)) + coalesce(vl_juros_w,0) + coalesce(vl_multa_w,0) 
	into STRICT	vl_titulo_w 
	from	darf_titulo_pagar a, 
		titulo_pagar b 
	where	a.nr_titulo = b.nr_titulo 
	and	nr_seq_darf = nr_seq_darf_p 
	group by coalesce(vl_juros_w,0), 
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_valor_total_darf (nr_seq_darf_p bigint) FROM PUBLIC;
