...
CREATE OR REPLACE FUNCTION hd_obter_valor_gpid ( nr_seq_dialise_p bigint, cd_pessoa_fisica_p text, qt_peso_pre_p bigint) RETURNS bigint AS $body$
DECLARE
...
if (qt_peso_ultima_w = 0) then
	...
	qt_gpid_w	:= qt_peso_pre_p - qt_peso_ultima_w;
end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION hd_obter_valor_gpid ( nr_seq_dialise_p bigint, cd_pessoa_fisica_p text, qt_peso_pre_p bigint) FROM PUBLIC;
