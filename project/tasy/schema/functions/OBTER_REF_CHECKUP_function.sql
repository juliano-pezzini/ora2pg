...
CREATE OR REPLACE FUNCTION obter_ref_checkup (cd_pessoa_fisica_p text, nr_sequencia_p bigint, qt_pessoa_p bigint, ie_opcao_p text) RETURNS varchar AS $body$
DECLARE
...
from	checkup_valor_padrao b
where	b.nr_seq_dado	= nr_sequencia_p
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_ref_checkup (cd_pessoa_fisica_p text, nr_sequencia_p bigint, qt_pessoa_p bigint, ie_opcao_p text) FROM PUBLIC;
