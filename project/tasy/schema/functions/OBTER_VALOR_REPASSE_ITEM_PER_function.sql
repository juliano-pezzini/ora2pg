...
CREATE OR REPLACE FUNCTION obter_valor_repasse_item_per ( nr_sequencia_p bigint, ie_proc_mat_p text, dt_inicial_p timestamp, dt_final_p timestamp) RETURNS bigint AS $body$
DECLARE
...
if (ie_proc_mat_p = 'P') then
	...
	from 	procedimento_repasse
	where 	nr_seq_procedimento = nr_sequencia_p
	and	coalesce(dt_liberacao,dt_inicial_p) between dt_inicial_p and fim_dia(dt_final_p)
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_valor_repasse_item_per ( nr_sequencia_p bigint, ie_proc_mat_p text, dt_inicial_p timestamp, dt_final_p timestamp) FROM PUBLIC;
