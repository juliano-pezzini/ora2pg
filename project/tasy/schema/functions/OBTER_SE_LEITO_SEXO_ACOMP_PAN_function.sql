...
CREATE OR REPLACE FUNCTION obter_se_leito_sexo_acomp_pan ( cd_setor_atendimento_p bigint, cd_unidade_basica_p text, cd_unidade_compl_p text, nr_atendimento_acomp_p bigint, DT_ACOMPANHANTE_P timestamp, cd_pessoa_fisica_p text) RETURNS varchar AS $body$
DECLARE
...
if (coalesce(nr_atendimento_acomp_p,0) > 0) then
	...
	if	(((ie_sexo_fixo_w = 'S') and (upper(ie_sexo_unid_w) = upper(ie_sexo_pf_w))) or
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_se_leito_sexo_acomp_pan ( cd_setor_atendimento_p bigint, cd_unidade_basica_p text, cd_unidade_compl_p text, nr_atendimento_acomp_p bigint, DT_ACOMPANHANTE_P timestamp, cd_pessoa_fisica_p text) FROM PUBLIC;
