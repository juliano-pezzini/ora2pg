...
CREATE OR REPLACE FUNCTION obter_conteudo_email_html5 ( nr_seq_agenda_p bigint, cd_declaracao_p text ) RETURNS text AS $body$
DECLARE
...
BEGIN
 ...
 END
        ...
        SELECT
            coalesce(coalesce(ds_declaracao_clob, ds_declaracao), ' ')
        ...
        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@numeroagenda', nr_seq_agenda_p), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@idade', qt_idade_paciente_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@procedimento', nm_procedimento_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@cirurgiao', nm_medico_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@paciente', nm_paciente_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@data', dt_procedimento_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@duracao', nr_minuto_duracao_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@cirurgia', nr_cirurgia_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@convenio', ds_convenio_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@docusuconv', cd_usuario_convenio_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@observacao', ds_observacao_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@procadic', ds_proc_adic_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@proclibrealizacao', ds_proc_adic2_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@procbloqconvenio', ds_proc_adic3_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@procpendautorhospital', ds_proc_adic4_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@procpendautorpac', ds_proc_adic5_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@coddescproced', ds_procedimento_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@anestesia', ds_anestesia_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@proc_interno', ds_proc_interno_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@dtchegadaprev', dt_chegada_prevista_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@usuario', nm_usuario_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@opme', ds_conteudo_email_opme_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@solic_medica', ds_email_solic_medica_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@equipamento', ds_conteudo_email_equip_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@cme', ds_conteudo_email_cme_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@bancodesangue', ds_conteudo_email_sangue_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@servicos', ds_conteudo_email_servicos_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@anestesista', nm_anestesista_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@dt_sem_hora', dt_procedimento_ww), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@sem_codigo_mat_med', ds_conteudo_sem_cod_matmed_w), 1, 32000)
;

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@sem_codigo_opme', ds_conteudo_sem_cod_opme_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@numero_reserva', ds_reserva_w), 1, 32000);

        ds_declaracao_w := substr(replace_macro(ds_declaracao_w, '@estab_agenda', ds_estabelecimento_agenda_w), 1, 32000);
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_conteudo_email_html5 ( nr_seq_agenda_p bigint, cd_declaracao_p text ) FROM PUBLIC;
