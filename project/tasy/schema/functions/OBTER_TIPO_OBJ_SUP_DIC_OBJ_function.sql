...
CREATE OR REPLACE FUNCTION obter_tipo_obj_sup_dic_obj ( cd_dominio_p bigint, ie_tipo_objeto_p text) RETURNS varchar AS $body$
DECLARE
...
if (ie_tipo_objeto_p IS NOT NULL AND ie_tipo_objeto_p::text <> '') then
	begin
	if (cd_dominio_p = 2666) then
		begin
		if (ie_tipo_objeto_p = 'P') then
			begin
			ie_tipo_objeto_w := 'WJP';
			end;
		elsif (ie_tipo_objeto_p in ('M','MI')) then
			begin
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_tipo_obj_sup_dic_obj ( cd_dominio_p bigint, ie_tipo_objeto_p text) FROM PUBLIC;
