...
CREATE OR REPLACE FUNCTION hsj_obter_valor_rel_conta2 ( nr_interno_conta_p bigint, nr_seq_propaci_p bigint, nr_seq_matpaci_p bigint, ie_emite_conta_p text, ie_tipo_valor_p text) RETURNS bigint AS $body$
DECLARE
		...
		IF (ie_tipo_registro_w = 'P') THEN
			...
			IF (ie_valor_informado_w  = 'N')  THEN
				...
				IF (vl_indice_w < 1) THEN
					vl_desconto_tab_w	:= vl_preco_unitario_w * (1 - vl_indice_w);
				ELSE
					vl_acresc_tab_w		:= (vl_preco_unitario_w * (vl_indice_w - 1));
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION hsj_obter_valor_rel_conta2 ( nr_interno_conta_p bigint, nr_seq_propaci_p bigint, nr_seq_matpaci_p bigint, ie_emite_conta_p text, ie_tipo_valor_p text) FROM PUBLIC;
