...
CREATE OR REPLACE FUNCTION lab_obter_result_ant_evento ( nr_prescricao_p bigint, nr_seq_exame_p bigint, ie_separador_p text, nr_seq_prescr_p bigint, ie_opcao_p bigint, ie_percent_p bigint default 0) RETURNS varchar AS $body$
DECLARE
...
c01 CURSOR FOR
	SELECT	obter_data_aprov_lab(b.nr_prescricao,c.nr_seq_prescr) dt_aprovacao,
		substr(coalesce(coalesce(CASE WHEN c.ds_resultado='0' THEN ''  ELSE CASE WHEN d.ie_formato_resultado='V' THEN ''  ELSE c.ds_resultado END  END ,	coalesce(CASE WHEN ie_percent_p=1 THEN c.pr_resultado  ELSE to_char(c.qt_resultado) END ,to_char(CASE WHEN c.pr_resultado=0 THEN ''  ELSE c.pr_resultado END ))),c.ds_resultado),1,100),
...
union all
	...
	where	b.nr_seq_resultado	= c.nr_seq_resultado
	  ...
	  --and	nvl(c.nr_seq_material,nr_seq_material_ex_w) = Obter_Material_Exame_Lab(null,e.cd_material_exame,1)
	  and	((qt_dias_prescr_w = 0) or (a.dt_prescricao between(trunc(dt_prescricao_exam_w) - qt_dias_prescr_w) and dt_prescricao_exam_w))
...
loop
	...
	if ((qt_exame_ant_w <= qt_result_anterior_w) or (qt_result_anterior_w = 0)) and (coalesce(length(ds_retorno_w),0) < 2000) then
		...
		if (ds_resultado_w IS NOT NULL AND ds_resultado_w::text <> '') then
			...
			if (ie_opcao_p = 1) then
				...
				ds_retorno_w := ds_retorno_w || rpad(nr_prescricao_w,10,' ') || '   '|| rpad(dt_coleta_w,18,' ') || '   '||  rpad(ds_resultado_w,20,' ') || '   '|| rpad(cd_unidade_medida_w,13,' ') || '   '|| rpad(ds_metodo_w,20,' ') || chr(13);
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION lab_obter_result_ant_evento ( nr_prescricao_p bigint, nr_seq_exame_p bigint, ie_separador_p text, nr_seq_prescr_p bigint, ie_opcao_p bigint, ie_percent_p bigint default 0) FROM PUBLIC;
