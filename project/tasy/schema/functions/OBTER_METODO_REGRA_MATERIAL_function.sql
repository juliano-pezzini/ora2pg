...
CREATE OR REPLACE FUNCTION obter_metodo_regra_material (nr_prescricao_p bigint, nr_seq_exame_p bigint, nr_seq_material_p bigint) RETURNS bigint AS $body$
DECLARE
...
and	((coalesce(nr_seq_material_p,0) = 0) or (nr_seq_material_p = b.nr_seq_material))
...
if (nr_seq_metodo_w = 0) then
	...
	and coalesce(b.ie_tipo_atendimento,ie_tipo_atendimento_w) = ie_tipo_atendimento_w
	and	a.nr_seq_exame = nr_seq_exame_p
	and	(((coalesce(coalesce(nr_seq_material_p,null)::text, '') = '') or (nr_seq_material_p = b.nr_seq_material) or coalesce(b.nr_seq_material::text, '') = ''))
	order by a.ie_prioridade;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_metodo_regra_material (nr_prescricao_p bigint, nr_seq_exame_p bigint, nr_seq_material_p bigint) FROM PUBLIC;
