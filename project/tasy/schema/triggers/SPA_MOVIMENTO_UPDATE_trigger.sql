...
DROP TRIGGER IF EXISTS spa_movimento_update ON spa_movimento CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_spa_movimento_update() RETURNS trigger AS $BODY$
declare
...
if (coalesce(cd_convenio_w,0) <> 0) and (coalesce(NEW.cd_convenio,0) <> coalesce(cd_convenio_w,0)) then 
...
-- REVOKE ALL ON FUNCTION trigger_fct_spa_movimento_update() FROM PUBLIC;

CREATE TRIGGER spa_movimento_update
	BEFORE UPDATE ON spa_movimento FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_spa_movimento_update();
