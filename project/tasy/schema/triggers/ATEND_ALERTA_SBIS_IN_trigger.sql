...
DROP TRIGGER IF EXISTS atend_alerta_sbis_in ON atendimento_alerta CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_atend_alerta_sbis_in() RETURNS trigger AS $BODY$
declare
ie_inativacao_w		varchar(1);
nr_log_seq_w		bigint;
cd_pessoa_fisica_w  log_alteracao_prontuario.cd_pessoa_fisica%type;
qt_reg_w		smallint;
...
-- REVOKE ALL ON FUNCTION trigger_fct_atend_alerta_sbis_in() FROM PUBLIC;

CREATE TRIGGER atend_alerta_sbis_in
	BEFORE INSERT OR UPDATE ON atendimento_alerta FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_atend_alerta_sbis_in();
