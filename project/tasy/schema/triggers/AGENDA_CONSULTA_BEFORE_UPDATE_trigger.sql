...
DROP TRIGGER IF EXISTS agenda_consulta_before_update ON agenda_consulta CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_agenda_consulta_before_update() RETURNS trigger AS $BODY$
declare
  ...
  nm_pessoa_montar_w        agenda_consulta.nm_paciente%type;
...
-- REVOKE ALL ON FUNCTION trigger_fct_agenda_consulta_before_update() FROM PUBLIC;

CREATE TRIGGER agenda_consulta_before_update
	BEFORE UPDATE ON agenda_consulta FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_agenda_consulta_before_update();
