...
DROP TRIGGER IF EXISTS banda_relatorio_atual ON banda_relatorio CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_banda_relatorio_atual() RETURNS trigger AS $BODY$
DECLARE
...
-- REVOKE ALL ON FUNCTION trigger_fct_banda_relatorio_atual() FROM PUBLIC;

CREATE TRIGGER banda_relatorio_atual
	BEFORE INSERT OR UPDATE OR DELETE ON banda_relatorio FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_banda_relatorio_atual();
