...
DROP TRIGGER IF EXISTS rl_ref_to_hospital_after ON rl_ref_to_hospital CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_rl_ref_to_hospital_after() RETURNS trigger AS $BODY$
declare
...
BEGIN
	if (wheb_usuario_pck.get_ie_executar_trigger = 'S') then
		...
		from	rl_history
		where	nr_seq_rl_ref_to_hosp = NEW.nr_sequencia;
...
-- REVOKE ALL ON FUNCTION trigger_fct_rl_ref_to_hospital_after() FROM PUBLIC;

CREATE TRIGGER rl_ref_to_hospital_after
	AFTER INSERT OR UPDATE ON rl_ref_to_hospital FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_rl_ref_to_hospital_after();
