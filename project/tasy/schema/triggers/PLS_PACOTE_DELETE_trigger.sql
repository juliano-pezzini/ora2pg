...
DROP TRIGGER IF EXISTS pls_pacote_delete ON pls_pacote CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_pacote_delete() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_pacote_delete() FROM PUBLIC;

CREATE TRIGGER pls_pacote_delete
	AFTER DELETE ON pls_pacote FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_pacote_delete();
