...
DROP TRIGGER IF EXISTS pls_pp_cta_combinada_atual ON pls_pp_cta_combinada CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_pp_cta_combinada_atual() RETURNS trigger AS $BODY$
declare

qt_registro_w	integer;

c01 CURSOR(	nr_seq_pp_combinada_pc	pls_pp_cta_combinada.nr_sequencia%type) FOR
	SELECT	nr_sequencia
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_pp_cta_combinada_atual() FROM PUBLIC;

CREATE TRIGGER pls_pp_cta_combinada_atual
	BEFORE INSERT OR UPDATE ON pls_pp_cta_combinada FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_pp_cta_combinada_atual();
