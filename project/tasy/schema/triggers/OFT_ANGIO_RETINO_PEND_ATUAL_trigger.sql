...
DROP TRIGGER IF EXISTS oft_angio_retino_pend_atual ON oft_angio_retino CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_oft_angio_retino_pend_atual() RETURNS trigger AS $BODY$
declare
...
if (ie_libera_exames_oft_w = 'S') then
	if (TG_OP = 'INSERT') or (TG_OP = 'UPDATE') then
		...
		if (NEW.dt_liberacao is null) then
         if (coalesce(NEW.ie_tipo_registro,'A') = 'A') then
            ie_tipo_w := 'AGRL';
         else
            ie_tipo_w := 'ARTL';
         end if;
      ...
      from     oft_consulta a where a.nr_sequencia = NEW.nr_seq_consulta;
		...
		exception
            ...
            where 	IE_TIPO_REGISTRO in ('AGRL','ARTL')
...
-- REVOKE ALL ON FUNCTION trigger_fct_oft_angio_retino_pend_atual() FROM PUBLIC;

CREATE TRIGGER oft_angio_retino_pend_atual
	AFTER INSERT OR UPDATE OR DELETE ON oft_angio_retino FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_oft_angio_retino_pend_atual();
