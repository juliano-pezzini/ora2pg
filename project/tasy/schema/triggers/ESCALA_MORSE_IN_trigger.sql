...
DROP TRIGGER IF EXISTS escala_morse_in ON escala_morse CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_escala_morse_in() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_escala_morse_in() FROM PUBLIC;

CREATE TRIGGER escala_morse_in
	BEFORE INSERT OR UPDATE ON escala_morse FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_escala_morse_in();
