...
DROP TRIGGER IF EXISTS cpoe_hemo_insert_update ON cpoe_hemoterapia CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_cpoe_hemo_insert_update() RETURNS trigger AS $BODY$
declare
...
BEGIN
  ...
  if (coalesce(NEW.cd_doenca_cid,'XPTO') <> coalesce(OLD.cd_doenca_cid,'XPTO')) then
    ds_log_cpoe_w	:= substr(ds_log_cpoe_w || ' cd_doenca_cid(' || coalesce(OLD.cd_doenca_cid,'<NULL>') || '/' || coalesce(NEW.cd_doenca_cid,'<NULL>')||'); ',1,2000);
  end if;
  ...
  if (coalesce(NEW.dt_programada, LOCALTIMESTAMP) <> coalesce(OLD.dt_programada, LOCALTIMESTAMP)) then
    ds_log_cpoe_w	:= substr(ds_log_cpoe_w || ' dt_programada(' || coalesce(PKG_DATE_FORMATERS_TZ.TO_VARCHAR(OLD.dt_programada, 'timestamp', ESTABLISHMENT_TIMEZONE_UTILS.getTimeZone),'<NULL>') || '/' || coalesce(PKG_DATE_FORMATERS_TZ.TO_VARCHAR(NEW.dt_programada, 'timestamp', ESTABLISHMENT_TIMEZONE_UTILS.getTimeZone),'<NULL>')||'); ',1,2000);
  end if;

  if (coalesce(NEW.dt_cirurgia, LOCALTIMESTAMP) <> coalesce(OLD.dt_cirurgia, LOCALTIMESTAMP)) then
    ds_log_cpoe_w	:= substr(ds_log_cpoe_w || ' dt_cirurgia(' || coalesce(PKG_DATE_FORMATERS_TZ.TO_VARCHAR(OLD.dt_cirurgia, 'timestamp', ESTABLISHMENT_TIMEZONE_UTILS.getTimeZone),'<NULL>') || '/' || coalesce(PKG_DATE_FORMATERS_TZ.TO_VARCHAR(NEW.dt_cirurgia, 'timestamp', ESTABLISHMENT_TIMEZONE_UTILS.getTimeZone),'<NULL>')||'); ',1,2000);
  end if;	
  ...
  if (ds_log_cpoe_w is not null) then
    ...
    insert into log_cpoe(nr_sequencia, nr_atendimento, dt_atualizacao, nm_usuario, nr_seq_hemoterapia, ds_log, ds_stack) values (nextval('log_cpoe_seq'), NEW.nr_atendimento, LOCALTIMESTAMP, NEW.nm_usuario, NEW.nr_sequencia, ds_log_cpoe_w, ds_stack_w);
  end if;

  if (wheb_usuario_pck.get_ie_executar_trigger = 'S' and pkg_i18n.get_user_locale = 'ja_JP') then
    CALL update_blood_admin_inst(NEW.nr_sequencia, 'U');
  end if;
  ...
  when others then
    ...
    values (			nextval('log_cpoe_seq'), 
              ...
              NEW.nm_usuario, 
              NEW.nr_sequencia, 
              'EXCEPTION CPOE_MAT_INSERT_UPDATE', 
              ds_stack_w);
end;
...
BEGIN
  BEGIN
	if (NEW.dt_liberacao is not null and ((OLD.dt_liberacao_enf is null and  NEW.dt_liberacao_enf is not null) or (OLD.dt_liberacao_farm is null and NEW.dt_liberacao_farm is not null))) then
		CALL cpoe_atualizar_inf_adic(NEW.nr_atendimento, NEW.nr_sequencia, 'H', NEW.dt_liberacao_enf, NEW.dt_liberacao_farm, null, null, null, null, null, NEW.nr_seq_cpoe_order_unit);
	end if;
  exception
  when others then
  	CALL gravar_log_cpoe('CPOE_HEMO_INSERT_UPDATE_AFTER - CPOE_ATUALIZAR_INF_ADIC - Erro: ' || substr(sqlerrm(SQLSTATE),1,1500) || ' :new.nr_sequencia '|| NEW.nr_sequencia, NEW.nr_atendimento);
  end;
  ...
  if (ie_use_integration_w = 'S')
    ...
    if (NEW.ie_tipo_hemoterap = 0) then
      select obter_cpoe_regra_ator('IP')
      into STRICT ie_order_integr_type_w
;
    elsif (NEW.ie_tipo_hemoterap = 1) then
...
;
    ...
    if (ie_order_integr_type_w = 'OI') then
      ...
      nr_entity_identifier_w := cpoe_hemo_order_json_pck.getCpoeIntegracaoHemo(NEW.nr_sequencia, NEW.nm_usuario, nr_entity_identifier_w);

      if (OLD.DT_LIBERACAO IS NULL AND NEW.DT_LIBERACAO IS NOT NULL) then

        CALL call_bifrost_content('prescription.hemotherapy.order.request','cpoe_hemo_order_json_pck.get_message_clob(' || NEW.nr_sequencia || ', ''NW'', ' || nr_entity_identifier_w || ')', NEW.nm_usuario);

      elsif (OLD.DT_LIB_SUSPENSAO IS NULL AND NEW.DT_LIB_SUSPENSAO IS NOT NULL ) then

        CALL call_bifrost_content('prescription.hemotherapy.order.request','cpoe_hemo_order_json_pck.get_message_clob(' || NEW.nr_sequencia || ', ''CA'', ' || nr_entity_identifier_w || ')', NEW.nm_usuario);
...
-- REVOKE ALL ON FUNCTION trigger_fct_cpoe_hemo_insert_update() FROM PUBLIC;

CREATE TRIGGER cpoe_hemo_insert_update
	AFTER INSERT OR UPDATE ON cpoe_hemoterapia FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_cpoe_hemo_insert_update();
