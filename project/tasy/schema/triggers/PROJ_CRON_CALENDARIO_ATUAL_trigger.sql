...
DROP TRIGGER IF EXISTS proj_cron_calendario_atual ON proj_cron_calendario CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_proj_cron_calendario_atual() RETURNS trigger AS $BODY$
declare
BEGIN
  BEGIN
BEGIN
...
-- REVOKE ALL ON FUNCTION trigger_fct_proj_cron_calendario_atual() FROM PUBLIC;

CREATE TRIGGER proj_cron_calendario_atual
	BEFORE INSERT OR UPDATE ON proj_cron_calendario FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_proj_cron_calendario_atual();
