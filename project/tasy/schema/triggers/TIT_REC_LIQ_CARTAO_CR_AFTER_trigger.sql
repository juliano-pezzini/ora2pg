...
DROP TRIGGER IF EXISTS tit_rec_liq_cartao_cr_after ON tit_rec_liq_cartao_cr CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_tit_rec_liq_cartao_cr_after() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_tit_rec_liq_cartao_cr_after() FROM PUBLIC;

CREATE TRIGGER tit_rec_liq_cartao_cr_after
	AFTER INSERT OR UPDATE ON tit_rec_liq_cartao_cr FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_tit_rec_liq_cartao_cr_after();
