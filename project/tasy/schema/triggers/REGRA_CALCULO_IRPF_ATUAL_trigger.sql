...
DROP TRIGGER IF EXISTS regra_calculo_irpf_atual ON regra_calculo_irpf CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_regra_calculo_irpf_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_regra_calculo_irpf_atual() FROM PUBLIC;

CREATE TRIGGER regra_calculo_irpf_atual
	BEFORE INSERT OR UPDATE ON regra_calculo_irpf FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_regra_calculo_irpf_atual();
