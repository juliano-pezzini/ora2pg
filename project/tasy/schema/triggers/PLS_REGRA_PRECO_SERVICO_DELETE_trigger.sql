...
DROP TRIGGER IF EXISTS pls_regra_preco_servico_delete ON pls_regra_preco_servico CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_regra_preco_servico_delete() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_regra_preco_servico_delete() FROM PUBLIC;

CREATE TRIGGER pls_regra_preco_servico_delete
	AFTER DELETE ON pls_regra_preco_servico FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_regra_preco_servico_delete();
