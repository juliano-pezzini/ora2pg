...
DROP TRIGGER IF EXISTS encaminhamento_pend_atual ON atend_encaminhamento CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_encaminhamento_pend_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_encaminhamento_pend_atual() FROM PUBLIC;

CREATE TRIGGER encaminhamento_pend_atual
	AFTER INSERT OR UPDATE OR DELETE ON atend_encaminhamento FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_encaminhamento_pend_atual();
