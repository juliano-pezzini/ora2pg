...
DROP TRIGGER IF EXISTS profissional_agenda_delete ON profissional_agenda CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_profissional_agenda_delete() RETURNS trigger AS $BODY$
DECLARE
...
-- REVOKE ALL ON FUNCTION trigger_fct_profissional_agenda_delete() FROM PUBLIC;

CREATE TRIGGER profissional_agenda_delete
	BEFORE DELETE ON profissional_agenda FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_profissional_agenda_delete();
