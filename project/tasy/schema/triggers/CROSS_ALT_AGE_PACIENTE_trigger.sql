...
DROP TRIGGER IF EXISTS cross_alt_age_paciente ON agenda_paciente CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_cross_alt_age_paciente() RETURNS trigger AS $BODY$
DECLARE
    json_w                     philips_json;
    json_data_w                text;
    existe_regra_envio_w       regra_envio_status_cross.cd_motivo%TYPE := 0;
    cd_motivo_w                regra_envio_status_cross.cd_motivo%TYPE;
    cd_tipo_agenda_paciente_w  agenda.cd_tipo_agenda%TYPE := 2;
    ...
    nm_usuario_w               agenda_paciente.nm_usuario%TYPE;
...
if (wheb_usuario_pck.get_ie_executar_trigger	= ie_exec_w)  then	
    ...
    WHERE
        ...
        AND cd_tipo_agenda = cd_tipo_agenda_paciente_w;
    ...
    IF existe_regra_envio_w > 0 THEN
        ...
        FROM
                 agenda a
            INNER JOIN agenda_paciente        ap ON a.cd_agenda = ap.cd_agenda
            INNER JOIN gercon_solic_consulta  ger ON ap.cd_agendamento_externo = ger.cd_agendamento_externo
        WHERE
            ap.nr_sequencia = NEW.nr_sequencia;
...
-- REVOKE ALL ON FUNCTION trigger_fct_cross_alt_age_paciente() FROM PUBLIC;

CREATE TRIGGER cross_alt_age_paciente
	AFTER UPDATE ON agenda_paciente FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_cross_alt_age_paciente();
