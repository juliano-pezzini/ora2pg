...
DROP TRIGGER IF EXISTS pls_pp_lanc_programado_atual ON pls_pp_lanc_programado CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_pp_lanc_programado_atual() RETURNS trigger AS $BODY$
declare
BEGIN 
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_pp_lanc_programado_atual() FROM PUBLIC;

CREATE TRIGGER pls_pp_lanc_programado_atual
	BEFORE INSERT OR UPDATE ON pls_pp_lanc_programado FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_pp_lanc_programado_atual();
