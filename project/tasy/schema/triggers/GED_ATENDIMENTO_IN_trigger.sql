...
DROP TRIGGER IF EXISTS ged_atendimento_in ON ged_atendimento CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_ged_atendimento_in() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_ged_atendimento_in() FROM PUBLIC;

CREATE TRIGGER ged_atendimento_in
	BEFORE INSERT OR UPDATE ON ged_atendimento FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_ged_atendimento_in();
