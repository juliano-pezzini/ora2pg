...
DROP TRIGGER IF EXISTS wl_regra_item_tp ON wl_regra_item CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_wl_regra_item_tp() RETURNS trigger AS $BODY$
...
-- REVOKE ALL ON FUNCTION trigger_fct_wl_regra_item_tp() FROM PUBLIC;

CREATE TRIGGER wl_regra_item_tp
	AFTER UPDATE ON wl_regra_item FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_wl_regra_item_tp();
