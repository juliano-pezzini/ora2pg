...
DROP TRIGGER IF EXISTS profissional_agenda_atual ON profissional_agenda CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_profissional_agenda_atual() RETURNS trigger AS $BODY$
DECLARE
...
-- REVOKE ALL ON FUNCTION trigger_fct_profissional_agenda_atual() FROM PUBLIC;

CREATE TRIGGER profissional_agenda_atual
	BEFORE INSERT OR UPDATE ON profissional_agenda FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_profissional_agenda_atual();
