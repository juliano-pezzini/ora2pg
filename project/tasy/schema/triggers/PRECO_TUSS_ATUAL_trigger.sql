...
DROP TRIGGER IF EXISTS preco_tuss_atual ON preco_tuss CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_preco_tuss_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_preco_tuss_atual() FROM PUBLIC;

CREATE TRIGGER preco_tuss_atual
	BEFORE INSERT OR UPDATE ON preco_tuss FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_preco_tuss_atual();
