...
DROP TRIGGER IF EXISTS paciente_ocorrencia_pend_atual ON paciente_ocorrencia CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_paciente_ocorrencia_pend_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_paciente_ocorrencia_pend_atual() FROM PUBLIC;

CREATE TRIGGER paciente_ocorrencia_pend_atual
	AFTER INSERT OR UPDATE ON paciente_ocorrencia FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_paciente_ocorrencia_pend_atual();
