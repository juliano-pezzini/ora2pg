...
DROP TRIGGER IF EXISTS pls_regra_preco_mat_delete ON pls_regra_preco_mat CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_regra_preco_mat_delete() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_regra_preco_mat_delete() FROM PUBLIC;

CREATE TRIGGER pls_regra_preco_mat_delete
	AFTER DELETE ON pls_regra_preco_mat FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_regra_preco_mat_delete();
