...
DROP TRIGGER IF EXISTS pac_antec_clinico_pend_atual ON paciente_antec_clinico CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pac_antec_clinico_pend_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pac_antec_clinico_pend_atual() FROM PUBLIC;

CREATE TRIGGER pac_antec_clinico_pend_atual
	AFTER INSERT OR UPDATE ON paciente_antec_clinico FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pac_antec_clinico_pend_atual();
