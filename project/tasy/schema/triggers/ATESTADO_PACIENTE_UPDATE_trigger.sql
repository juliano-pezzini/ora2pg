...
DROP TRIGGER IF EXISTS atestado_paciente_update ON atestado_paciente CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_atestado_paciente_update() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_atestado_paciente_update() FROM PUBLIC;

CREATE TRIGGER atestado_paciente_update
	BEFORE UPDATE ON atestado_paciente FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_atestado_paciente_update();
