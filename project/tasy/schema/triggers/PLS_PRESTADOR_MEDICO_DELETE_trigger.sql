...
DROP TRIGGER IF EXISTS pls_prestador_medico_delete ON pls_prestador_medico CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_prestador_medico_delete() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_prestador_medico_delete() FROM PUBLIC;

CREATE TRIGGER pls_prestador_medico_delete
	AFTER DELETE ON pls_prestador_medico FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_prestador_medico_delete();
