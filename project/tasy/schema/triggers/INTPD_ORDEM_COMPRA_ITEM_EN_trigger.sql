...
DROP TRIGGER IF EXISTS intpd_ordem_compra_item_en ON ordem_compra_item_entrega CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_intpd_ordem_compra_item_en() RETURNS trigger AS $BODY$
declare
...
if (TG_OP = 'INSERT' or TG_OP = 'UPDATE') and (dt_aprovacao_w is null) then
	...
	/* Grava o agendamento da informação para atualização do fluxo de caixa. */
		...
		CALL gravar_agend_fluxo_caixa(NEW.nr_ordem_compra,NEW.nr_sequencia,'OC', NEW.DT_PREVISTA_ENTREGA,'I',NEW.nm_usuario,'S');
	elsif (TG_OP = 'UPDATE') then
	/* Grava o agendamento da informação para atualização do fluxo de caixa. */

		CALL gravar_agend_fluxo_caixa(NEW.nr_ordem_compra,NEW.nr_sequencia,'OC', NEW.DT_PREVISTA_ENTREGA,'A',NEW.nm_usuario,'S');
	end if;
	...
	CALL gravar_agend_fluxo_caixa(OLD.nr_ordem_compra,OLD.nr_sequencia,'OC', OLD.DT_PREVISTA_ENTREGA,'E',OLD.nm_usuario,'S');
...
-- REVOKE ALL ON FUNCTION trigger_fct_intpd_ordem_compra_item_en() FROM PUBLIC;

CREATE TRIGGER intpd_ordem_compra_item_en
	BEFORE INSERT OR UPDATE OR DELETE ON ordem_compra_item_entrega FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_intpd_ordem_compra_item_en();
