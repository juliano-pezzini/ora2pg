...
DROP TRIGGER IF EXISTS preco_servico_atual ON preco_servico CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_preco_servico_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_preco_servico_atual() FROM PUBLIC;

CREATE TRIGGER preco_servico_atual
	BEFORE INSERT OR UPDATE ON preco_servico FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_preco_servico_atual();
