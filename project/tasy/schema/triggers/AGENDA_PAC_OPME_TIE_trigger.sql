...
DROP TRIGGER IF EXISTS agenda_pac_opme_tie ON agenda_pac_opme CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_agenda_pac_opme_tie() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_agenda_pac_opme_tie() FROM PUBLIC;

CREATE TRIGGER agenda_pac_opme_tie
	AFTER INSERT OR UPDATE OR DELETE ON agenda_pac_opme FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_agenda_pac_opme_tie();
