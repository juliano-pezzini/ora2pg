...
DROP TRIGGER IF EXISTS proj_calendario_padrao_atual ON proj_calendario_padrao CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_proj_calendario_padrao_atual() RETURNS trigger AS $BODY$
declare
BEGIN
  BEGIN
...
-- REVOKE ALL ON FUNCTION trigger_fct_proj_calendario_padrao_atual() FROM PUBLIC;

CREATE TRIGGER proj_calendario_padrao_atual
	BEFORE INSERT OR UPDATE ON proj_calendario_padrao FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_proj_calendario_padrao_atual();
