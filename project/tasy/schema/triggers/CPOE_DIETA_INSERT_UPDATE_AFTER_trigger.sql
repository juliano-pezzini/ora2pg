...
DROP TRIGGER IF EXISTS cpoe_dieta_insert_update_after ON cpoe_dieta CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_cpoe_dieta_insert_update_after() RETURNS trigger AS $BODY$
declare
...
BEGIN
  BEGIN
	BEGIN
		...
		if (coalesce(NEW.ie_tipo_dieta,'XPTO') <> coalesce(OLD.ie_tipo_dieta,'XPTO')) then
			ds_log_cpoe_w	:= substr(ds_log_cpoe_w || ' ie_tipo_dieta(' || coalesce(OLD.ie_tipo_dieta,'<NULL>') || '/' || coalesce(NEW.ie_tipo_dieta,'<NULL>')||'); ',1,2000);
		end if;
		...
		if (coalesce(NEW.cd_dieta,0) <> coalesce(OLD.cd_dieta,0)) then
			ds_log_cpoe_w	:= substr(ds_log_cpoe_w || ' cd_dieta(' || coalesce(OLD.cd_dieta,0) || '/' || coalesce(NEW.cd_dieta,0)||'); ',1,2000);
		end if;

		if (coalesce(NEW.cd_mat_prod1,0) <> coalesce(OLD.cd_mat_prod1,0)) then
			ds_log_cpoe_w	:= substr(ds_log_cpoe_w || ' cd_mat_prod1(' || coalesce(OLD.cd_mat_prod1,0) || '/' || coalesce(NEW.cd_mat_prod1,0)||'); ',1,2000);
		end if;
      ...
      ds_log_cpoe_w	:= substr(ds_log_cpoe_w || ' dt_alta_medico(' || coalesce(PKG_DATE_FORMATERS_TZ.TO_VARCHAR(OLD.dt_alta_medico, 'timestamp', establishment_timezone_utils.gettimezone),'<NULL>') || '/' || coalesce(            PKG_DATE_FORMATERS_TZ.TO_VARCHAR(NEW.dt_alta_medico, 'timestamp', establishment_timezone_utils.gettimezone),'<NULL>')||'); ',1,2000);
		...
		if (ds_log_cpoe_w is not null) then
			...
			insert into log_cpoe(nr_sequencia, nr_atendimento, dt_atualizacao, nm_usuario, nr_seq_dieta, ds_log, ds_stack) values (nextval('log_cpoe_seq'), NEW.nr_atendimento, LOCALTIMESTAMP, NEW.nm_usuario, NEW.nr_sequencia, ds_log_cpoe_w, ds_stack_w);
		end if;

	exception
	when others then
		ds_stack_w	:= substr(dbms_utility.format_call_stack,1,2000);
		ds_erro_w := substr('EXCEPTION CPOE_DIETA_INSERT_UPDATE_AFTER' || ': ' || sqlerrm(SQLSTATE), 1, 2000);
	...
	BEGIN
		if (NEW.dt_liberacao is not null and ((OLD.dt_liberacao_enf is null and  NEW.dt_liberacao_enf is not null) or (OLD.dt_liberacao_farm is null and NEW.dt_liberacao_farm is not null))) then
			CALL cpoe_atualizar_inf_adic(NEW.nr_atendimento, NEW.nr_sequencia, 'N', NEW.dt_liberacao_enf, NEW.dt_liberacao_farm, null, null, null, null, null, NEW.nr_seq_cpoe_order_unit);
		end if;
	exception
	when others then
		CALL gravar_log_cpoe('CPOE_DIETA_INSERT_UPDATE - CPOE_ATUALIZAR_INF_ADIC - Erro: ' || substr(sqlerrm(SQLSTATE),1,1500) || ' new.nr_sequencia '|| NEW.nr_sequencia, NEW.nr_atendimento);
	end;
	...
	BEGIN	
		...
		if (ie_use_integration_w = 'S')
			and (NEW.IE_TIPO_DIETA in ('J','O','E','S','L','P'))
			...
			if (NEW.IE_TIPO_DIETA in ('J','O','L')) then
				select obter_cpoe_regra_ator('D')
				into STRICT ie_order_integr_type_w
				;			
			elsif (NEW.IE_TIPO_DIETA in ('E','S')) then
				select obter_cpoe_regra_ator('E')
				into STRICT ie_order_integr_type_w
				;
			elsif (NEW.IE_TIPO_DIETA in ('P')) then
			...
			if (ie_order_integr_type_w = 'OI') then
				...
				nr_entity_identifier_w := cpoe_diet_json_pck.getCpoeIntegracaoDiet(NEW.nr_sequencia, NEW.nm_usuario, nr_entity_identifier_w);
			
				if (OLD.DT_LIBERACAO IS NULL AND NEW.DT_LIBERACAO IS NOT NULL) then
					
					CALL call_bifrost_content('prescription.diet.order.request','cpoe_diet_json_pck.get_message_clob(' || NEW.nr_sequencia || ', ''NW'', ' || nr_entity_identifier_w || ')', NEW.nm_usuario);
					
				elsif (OLD.DT_LIB_SUSPENSAO IS NULL AND NEW.DT_LIB_SUSPENSAO IS NOT NULL ) then
				
					CALL call_bifrost_content('prescription.diet.order.request','cpoe_diet_json_pck.get_message_clob(' || NEW.nr_sequencia || ', ''CA'', ' || nr_entity_identifier_w || ')', NEW.nm_usuario);
	...
	if (coalesce(pkg_i18n.get_user_locale, 'pt_BR') = 'ja_JP') then
        BEGIN
			select 	CASE WHEN e.ie_retrogrado IS NULL THEN  'N'  ELSE e.ie_retrogrado END ,
					is_special_order_rule('NU','A',wheb_usuario_pck.get_cd_estabelecimento)
			into STRICT 	ie_retrogrado_w,
			        ie_transmit_special_order 
			from	cpoe_dieta e
			where   e.nr_sequencia = NEW.nr_sequencia;
		    exception
			when others then
			        ie_retrogrado_w := 'N';
			        ie_transmit_special_order := 'N';
		end;	
		 if ((NEW.dt_liberacao  is not null) and (OLD.dt_liberacao is null) and (ie_retrogrado_w = 'N' or (ie_retrogrado_w = 'S' and ie_transmit_special_order = 'S'))) then
            if((trunc(coalesce(NEW.dt_inicio, LOCALTIMESTAMP)) = trunc(LOCALTIMESTAMP)) or (trunc(coalesce(NEW.dt_fim, LOCALTIMESTAMP)) = trunc(LOCALTIMESTAMP))) then         		
		         ds_param_integration_w := '{ '
					|| '"recordId" : "' ||NEW.nr_atendimento || '"' ||' , '
					|| '"fromJob" : "' || 'N' || '"'
					|| ' }';
		        CALL execute_bifrost_integration(268, ds_param_integration_w);
		    end if;
		end if;
	end if;
...
-- REVOKE ALL ON FUNCTION trigger_fct_cpoe_dieta_insert_update_after() FROM PUBLIC;

CREATE TRIGGER cpoe_dieta_insert_update_after
	AFTER INSERT OR UPDATE ON cpoe_dieta FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_cpoe_dieta_insert_update_after();
