...
DROP TRIGGER IF EXISTS pessoa_fisica_tie ON pessoa_fisica CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pessoa_fisica_tie() RETURNS trigger AS $BODY$
declare
...
SELECT 	a.nr_sequencia schedule_id,
        ...
        pkg_date_formaters.to_varchar((a.hr_inicio + (a.nr_minuto_duracao)::numeric /(24*60)), 'shortTime', wheb_usuario_pck.get_cd_estabelecimento, wheb_usuario_pck.get_nm_usuario) procedure_duration,
...
FROM (select NEW.cd_pessoa_fisica cd_pessoa_fisica,
...
WHERE a.cd_agenda = b.cd_agenda  and d.cd_pessoa_fisica = a.cd_pessoa_fisica and a.ie_status_agenda not in ('L', 'B', 'C') and a.hr_inicio between LOCALTIMESTAMP and LOCALTIMESTAMP + (obter_valor_param_usuario(410, 64, obter_perfil_ativo, wheb_usuario_pck.get_nm_usuario, wheb_usuario_pck.get_cd_estabelecimento))::numeric /24 and b.cd_tipo_agenda = 1
...
select  a.nr_sequencia schedule_id,
        ...
        coalesce((select     max(substr(obter_valor_dominio(3195,h.ie_status),1,255))
             ...
             and        i.nr_sequencia = a.nr_sequencia
             and        nr_seq_proc_servico =  6817), wheb_mensagem_pck.get_texto(94755)) surgical_services_status
FROM agenda_paciente_proc j, (select NEW.cd_pessoa_fisica cd_pessoa_fisica,
                NEW.nm_pessoa_fisica nm_pessoa_fisica,
                NEW.dt_nascimento dt_nascimento,
                NEW.nr_prontuario nr_prontuario,
                NEW.ie_tipo_sangue ie_tipo_sangue,
...
-- REVOKE ALL ON FUNCTION trigger_fct_pessoa_fisica_tie() FROM PUBLIC;

CREATE TRIGGER pessoa_fisica_tie
	AFTER INSERT OR UPDATE ON pessoa_fisica FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pessoa_fisica_tie();
