...
DROP TRIGGER IF EXISTS material_tuss_after ON material_tuss CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_material_tuss_after() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_material_tuss_after() FROM PUBLIC;

CREATE TRIGGER material_tuss_after
	AFTER INSERT OR UPDATE OR DELETE ON material_tuss FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_material_tuss_after();
