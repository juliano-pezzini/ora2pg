...
DROP TRIGGER IF EXISTS pepo_curativo_pend_atual ON pepo_curativo CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pepo_curativo_pend_atual() RETURNS trigger AS $BODY$
declare

qt_reg_w			smallint;
ie_tipo_w			varchar(10);
ie_pepo_curativo_w			varchar(10);
...
into STRICT	ie_pepo_curativo_w
...
if (coalesce(ie_pepo_curativo_w,'N') = 'S') then
...
-- REVOKE ALL ON FUNCTION trigger_fct_pepo_curativo_pend_atual() FROM PUBLIC;

CREATE TRIGGER pepo_curativo_pend_atual
	AFTER INSERT OR UPDATE ON pepo_curativo FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pepo_curativo_pend_atual();
