...
DROP TRIGGER IF EXISTS pac_acessorio_pend_atual ON paciente_acessorio CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pac_acessorio_pend_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pac_acessorio_pend_atual() FROM PUBLIC;

CREATE TRIGGER pac_acessorio_pend_atual
	AFTER INSERT OR UPDATE ON paciente_acessorio FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pac_acessorio_pend_atual();
