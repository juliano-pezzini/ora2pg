...
DROP TRIGGER IF EXISTS diagnostico_doenca_sbis_in ON diagnostico_doenca CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_diagnostico_doenca_sbis_in() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_diagnostico_doenca_sbis_in() FROM PUBLIC;

CREATE TRIGGER diagnostico_doenca_sbis_in
	BEFORE INSERT OR UPDATE ON diagnostico_doenca FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_diagnostico_doenca_sbis_in();
