...
DROP TRIGGER IF EXISTS pep_pac_ci_sbis_in ON pep_pac_ci CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pep_pac_ci_sbis_in() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pep_pac_ci_sbis_in() FROM PUBLIC;

CREATE TRIGGER pep_pac_ci_sbis_in
	BEFORE INSERT OR UPDATE ON pep_pac_ci FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pep_pac_ci_sbis_in();
