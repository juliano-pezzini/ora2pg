...
DROP TRIGGER IF EXISTS med_receita_sbis_in ON med_receita CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_med_receita_sbis_in() RETURNS trigger AS $BODY$
declare
...
;
    ...
    insert into log_alteracao_prontuario(nr_sequencia, 
                       ...
                       coalesce(obter_pessoa_atendimento(coalesce(NEW.nr_atendimento,NEW.nr_atendimento_hosp),'C'), NEW.cd_pessoa_fisica), 
...
-- REVOKE ALL ON FUNCTION trigger_fct_med_receita_sbis_in() FROM PUBLIC;

CREATE TRIGGER med_receita_sbis_in
	BEFORE INSERT OR UPDATE ON med_receita FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_med_receita_sbis_in();
