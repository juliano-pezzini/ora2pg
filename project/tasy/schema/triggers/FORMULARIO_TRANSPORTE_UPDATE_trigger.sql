...
DROP TRIGGER IF EXISTS formulario_transporte_update ON formulario_transporte CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_formulario_transporte_update() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_formulario_transporte_update() FROM PUBLIC;

CREATE TRIGGER formulario_transporte_update
	BEFORE UPDATE ON formulario_transporte FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_formulario_transporte_update();
