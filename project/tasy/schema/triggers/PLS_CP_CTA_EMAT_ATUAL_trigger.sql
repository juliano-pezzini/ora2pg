...
DROP TRIGGER IF EXISTS pls_cp_cta_emat_atual ON pls_cp_cta_emat CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_cp_cta_emat_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_cp_cta_emat_atual() FROM PUBLIC;

CREATE TRIGGER pls_cp_cta_emat_atual
	BEFORE INSERT OR UPDATE ON pls_cp_cta_emat FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_cp_cta_emat_atual();
