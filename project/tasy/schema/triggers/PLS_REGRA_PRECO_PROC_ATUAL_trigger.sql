...
DROP TRIGGER IF EXISTS pls_regra_preco_proc_atual ON pls_regra_preco_proc CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_regra_preco_proc_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_regra_preco_proc_atual() FROM PUBLIC;

CREATE TRIGGER pls_regra_preco_proc_atual
	BEFORE INSERT OR UPDATE ON pls_regra_preco_proc FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_regra_preco_proc_atual();
