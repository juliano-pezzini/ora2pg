...
DROP TRIGGER IF EXISTS titulo_rec_liq_valor_after ON titulo_rec_liq_valor CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_titulo_rec_liq_valor_after() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_titulo_rec_liq_valor_after() FROM PUBLIC;

CREATE TRIGGER titulo_rec_liq_valor_after
	AFTER INSERT OR UPDATE ON titulo_rec_liq_valor FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_titulo_rec_liq_valor_after();
