...
DROP TRIGGER IF EXISTS auditoria_propaci_insert ON auditoria_propaci CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_auditoria_propaci_insert() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_auditoria_propaci_insert() FROM PUBLIC;

CREATE TRIGGER auditoria_propaci_insert
	BEFORE INSERT ON auditoria_propaci FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_auditoria_propaci_insert();
