...
DROP TRIGGER IF EXISTS agenda_pac_sangue_tie ON agenda_pac_sangue CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_agenda_pac_sangue_tie() RETURNS trigger AS $BODY$
declare

json_w             philips_json;
json_data_w        text;
preorder_blood_w   varchar(5);
nr_seq_agenda_w    agenda_pac_sangue.nr_seq_agenda%type;
...
WHERE a.cd_agenda = b.cd_agenda  and d.cd_pessoa_fisica = a.cd_pessoa_fisica and a.ie_status_agenda not in ('L', 'B', 'C') and to_date(a.hr_inicio, 'dd/mm/yyyy hh24:mi:ss') between to_date(LOCALTIMESTAMP::text, 'dd/mm/yyyy hh24:mi:ss') and to_date(LOCALTIMESTAMP + (obter_valor_param_usuario(410, 64, obter_perfil_ativo, wheb_usuario_pck.get_nm_usuario, wheb_usuario_pck.get_cd_estabelecimento))::numeric /24, 'dd/mm/yyyy hh24:mi:ss') and b.cd_tipo_agenda = 1 and a.nr_sequencia = nr_seq_agenda_w
...
, classif_agenda_cirurgica f
LEFT OUTER JOIN agenda b ON (f.nr_sequencia = b.nr_seq_classif)
WHERE a.cd_agenda = b.cd_agenda  and d.cd_pessoa_fisica = a.cd_pessoa_fisica and j.nr_sequencia = a.nr_sequencia and a.dt_agenda >= LOCALTIMESTAMP and a.ie_status_agenda not in ('L', 'B', 'C') and to_date(a.hr_inicio, 'dd/mm/yyyy hh24:mi:ss') between to_date(LOCALTIMESTAMP::text, 'dd/mm/yyyy hh24:mi:ss') and to_date(LOCALTIMESTAMP + (obter_valor_param_usuario(410, 64, obter_perfil_ativo, wheb_usuario_pck.get_nm_usuario, wheb_usuario_pck.get_cd_estabelecimento))::numeric /24, 'dd/mm/yyyy hh24:mi:ss') and b.cd_tipo_agenda = 1 and a.nr_sequencia = nr_seq_agenda_w order by schedule_date, start_time, patient;
...
if (TG_OP = 'INSERT' or TG_OP = 'UPDATE') then
    ...
    preorder_blood_w := wheb_mensagem_pck.get_texto(94754);

elsif (TG_OP = 'DELETE') then

    nr_seq_agenda_w := OLD.nr_seq_agenda;

    select coalesce(max(wheb_mensagem_pck.get_texto(94754)),wheb_mensagem_pck.get_texto(94755))
    into STRICT preorder_blood_w
    from agenda_pac_sangue h
...
if (wheb_usuario_pck.get_nm_usuario is not null) then
	...
	for c01_w in c01 loop
		...
		json_w.put('preorder_blood', preorder_blood_w);
...
-- REVOKE ALL ON FUNCTION trigger_fct_agenda_pac_sangue_tie() FROM PUBLIC;

CREATE TRIGGER agenda_pac_sangue_tie
	AFTER INSERT OR UPDATE OR DELETE ON agenda_pac_sangue FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_agenda_pac_sangue_tie();
