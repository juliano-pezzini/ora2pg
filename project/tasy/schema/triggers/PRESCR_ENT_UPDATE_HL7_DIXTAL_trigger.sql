...
DROP TRIGGER IF EXISTS prescr_ent_update_hl7_dixtal ON prescr_material CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_prescr_ent_update_hl7_dixtal() RETURNS trigger AS $BODY$
declare
...
if (coalesce(OLD.ie_suspenso,'N') = 'N') and (NEW.ie_suspenso = 'S') and (NEW.ie_agrupador = 8) then 
...
-- REVOKE ALL ON FUNCTION trigger_fct_prescr_ent_update_hl7_dixtal() FROM PUBLIC;

CREATE TRIGGER prescr_ent_update_hl7_dixtal
	AFTER UPDATE ON prescr_material FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_prescr_ent_update_hl7_dixtal();
