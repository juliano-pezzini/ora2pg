...
DROP TRIGGER IF EXISTS pls_regra_simpro_preco_atual ON pls_regra_simpro_preco CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_regra_simpro_preco_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_regra_simpro_preco_atual() FROM PUBLIC;

CREATE TRIGGER pls_regra_simpro_preco_atual
	BEFORE INSERT OR UPDATE ON pls_regra_simpro_preco FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_regra_simpro_preco_atual();
