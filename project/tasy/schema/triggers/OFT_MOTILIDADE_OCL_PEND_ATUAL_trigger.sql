...
DROP TRIGGER IF EXISTS oft_motilidade_ocl_pend_atual ON oft_motilidade_ocular CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_oft_motilidade_ocl_pend_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_oft_motilidade_ocl_pend_atual() FROM PUBLIC;

CREATE TRIGGER oft_motilidade_ocl_pend_atual
	AFTER INSERT OR UPDATE OR DELETE ON oft_motilidade_ocular FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_oft_motilidade_ocl_pend_atual();
