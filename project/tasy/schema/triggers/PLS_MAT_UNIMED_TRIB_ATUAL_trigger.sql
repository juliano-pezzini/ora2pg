...
DROP TRIGGER IF EXISTS pls_mat_unimed_trib_atual ON pls_mat_unimed_trib CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_mat_unimed_trib_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_mat_unimed_trib_atual() FROM PUBLIC;

CREATE TRIGGER pls_mat_unimed_trib_atual
	BEFORE INSERT OR UPDATE ON pls_mat_unimed_trib FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_mat_unimed_trib_atual();
