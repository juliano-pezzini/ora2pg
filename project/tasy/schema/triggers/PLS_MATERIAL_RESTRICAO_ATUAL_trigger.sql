...
DROP TRIGGER IF EXISTS pls_material_restricao_atual ON pls_material_restricao CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_material_restricao_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_material_restricao_atual() FROM PUBLIC;

CREATE TRIGGER pls_material_restricao_atual
	BEFORE INSERT OR UPDATE ON pls_material_restricao FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_material_restricao_atual();
