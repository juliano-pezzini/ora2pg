...
DROP TRIGGER IF EXISTS pls_evento_regra_tit_pag_atual ON pls_evento_regra_tit_pagar CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_evento_regra_tit_pag_atual() RETURNS trigger AS $BODY$
declare

BEGIN

-- previnir que seja gravada a hora junto na regra, essa situação ocorre quando o usuário seleciona a data no campo via enter
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_evento_regra_tit_pag_atual() FROM PUBLIC;

CREATE TRIGGER pls_evento_regra_tit_pag_atual
	BEFORE INSERT OR UPDATE ON pls_evento_regra_tit_pagar FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_evento_regra_tit_pag_atual();
