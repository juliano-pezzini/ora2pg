...
DROP TRIGGER IF EXISTS lista_problema_pac_in ON lista_problema_pac CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_lista_problema_pac_in() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_lista_problema_pac_in() FROM PUBLIC;

CREATE TRIGGER lista_problema_pac_in
	BEFORE INSERT OR UPDATE ON lista_problema_pac FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_lista_problema_pac_in();
