...
DROP TRIGGER IF EXISTS prescr_rec_update_hl7_dixtal ON prescr_recomendacao CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_prescr_rec_update_hl7_dixtal() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_prescr_rec_update_hl7_dixtal() FROM PUBLIC;

CREATE TRIGGER prescr_rec_update_hl7_dixtal
	AFTER UPDATE ON prescr_recomendacao FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_prescr_rec_update_hl7_dixtal();
