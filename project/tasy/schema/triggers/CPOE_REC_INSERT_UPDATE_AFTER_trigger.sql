...
DROP TRIGGER IF EXISTS cpoe_rec_insert_update_after ON cpoe_recomendacao CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_cpoe_rec_insert_update_after() RETURNS trigger AS $BODY$
declare
...
BEGIN
  BEGIN
	...
	if (coalesce(NEW.cd_recomendacao,0) <> coalesce(OLD.cd_recomendacao,0)) then
		ds_log_cpoe_w	:= substr(ds_log_cpoe_w || ' cd_recomendacao(' || coalesce(OLD.cd_recomendacao,0) || '/' || coalesce(NEW.cd_recomendacao,0)||'); ',1,2000);
	end if;
	...
	if (ds_log_cpoe_w is not null) then
		...
		insert into log_cpoe(nr_sequencia, nr_atendimento, dt_atualizacao, nm_usuario, nr_seq_recomendacao, ds_log, ds_stack) values (nextval('log_cpoe_seq'), NEW.nr_atendimento, LOCALTIMESTAMP, NEW.nm_usuario, NEW.nr_sequencia, ds_log_cpoe_w, ds_stack_w);
	end if;
	...
	BEGIN
		if (NEW.dt_liberacao is not null and ((OLD.dt_liberacao_enf is null and  NEW.dt_liberacao_enf is not null) or (OLD.dt_liberacao_farm is null and NEW.dt_liberacao_farm is not null))) then
			CALL cpoe_atualizar_inf_adic(NEW.nr_atendimento, NEW.nr_sequencia, 'R', NEW.dt_liberacao_enf, NEW.dt_liberacao_farm, null, null, null, null, null, NEW.nr_seq_cpoe_order_unit);
		end if;
	exception
	when others then
		CALL gravar_log_cpoe('CPOE_REC_INSERT_UPDATE_AFTER - CPOE_ATUALIZAR_INF_ADIC - Erro: ' || substr(sqlerrm(SQLSTATE),1,1500) || ' :new.nr_sequencia '|| NEW.nr_sequencia, NEW.nr_atendimento);
	end;
  ...
  BEGIN	
		...
		if (ie_use_integration_w = 'S')
			...
			if (ie_order_integr_type_w = 'OI') then
				...
				nr_entity_identifier_w := recommendation_json_pck.getCpoeIntegracaoRecom(NEW.nr_sequencia, NEW.nm_usuario, nr_entity_identifier_w);
			
				if (OLD.DT_LIBERACAO IS NULL AND NEW.DT_LIBERACAO IS NOT NULL) then
					
					CALL call_bifrost_content('prescription.recommendation.order.request','recommendation_json_pck.get_recomm_message_clob(' || NEW.nr_sequencia || ', ''NW'', ' || nr_entity_identifier_w || ')', NEW.nm_usuario);
					
				elsif (OLD.DT_LIB_SUSPENSAO IS NULL AND NEW.DT_LIB_SUSPENSAO IS NOT NULL ) then
				
					CALL call_bifrost_content('prescription.recommendation.order.request','recommendation_json_pck.get_recomm_message_clob(' || NEW.nr_sequencia || ', ''CA'', ' || nr_entity_identifier_w || ')', NEW.nm_usuario);
...
-- REVOKE ALL ON FUNCTION trigger_fct_cpoe_rec_insert_update_after() FROM PUBLIC;

CREATE TRIGGER cpoe_rec_insert_update_after
	AFTER INSERT OR UPDATE ON cpoe_recomendacao FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_cpoe_rec_insert_update_after();
