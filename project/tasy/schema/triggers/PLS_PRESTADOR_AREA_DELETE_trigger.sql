...
DROP TRIGGER IF EXISTS pls_prestador_area_delete ON pls_prestador_area CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_prestador_area_delete() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_prestador_area_delete() FROM PUBLIC;

CREATE TRIGGER pls_prestador_area_delete
	AFTER DELETE ON pls_prestador_area FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_prestador_area_delete();
