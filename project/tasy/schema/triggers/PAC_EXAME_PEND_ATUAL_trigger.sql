...
DROP TRIGGER IF EXISTS pac_exame_pend_atual ON paciente_exame CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pac_exame_pend_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pac_exame_pend_atual() FROM PUBLIC;

CREATE TRIGGER pac_exame_pend_atual
	AFTER INSERT OR UPDATE ON paciente_exame FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pac_exame_pend_atual();
