...
DROP TRIGGER IF EXISTS pls_rec_regra_valor_atual ON pls_rec_regra_valor CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_rec_regra_valor_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_rec_regra_valor_atual() FROM PUBLIC;

CREATE TRIGGER pls_rec_regra_valor_atual
	BEFORE UPDATE ON pls_rec_regra_valor FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_rec_regra_valor_atual();
