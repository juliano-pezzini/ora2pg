...
DROP TRIGGER IF EXISTS agenda_pac_servico_atual ON agenda_pac_servico CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_agenda_pac_servico_atual() RETURNS trigger AS $BODY$
DECLARE
...
-- REVOKE ALL ON FUNCTION trigger_fct_agenda_pac_servico_atual() FROM PUBLIC;

CREATE TRIGGER agenda_pac_servico_atual
	BEFORE INSERT OR UPDATE ON agenda_pac_servico FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_agenda_pac_servico_atual();
