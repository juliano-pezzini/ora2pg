...
DROP TRIGGER IF EXISTS nut_paciente_update_hl7_dixtal ON nut_paciente CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_nut_paciente_update_hl7_dixtal() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_nut_paciente_update_hl7_dixtal() FROM PUBLIC;

CREATE TRIGGER nut_paciente_update_hl7_dixtal
	AFTER UPDATE ON nut_paciente FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_nut_paciente_update_hl7_dixtal();
