...
DROP TRIGGER IF EXISTS formulario_audicao_update ON formulario_audicao CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_formulario_audicao_update() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_formulario_audicao_update() FROM PUBLIC;

CREATE TRIGGER formulario_audicao_update
	BEFORE UPDATE ON formulario_audicao FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_formulario_audicao_update();
