...
DROP TRIGGER IF EXISTS banda_relat_campo_atual ON banda_relat_campo CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_banda_relat_campo_atual() RETURNS trigger AS $BODY$
DECLARE
...
END IF;
    ...
    BEGIN
        ...
        ds_comando_w := 'UPDATE RELATORIO '||
                        ...
                        '       NM_USUARIO = :1' ||
                        ' WHERE NR_SEQUENCIA = (SELECT NR_SEQ_RELATORIO'  ||
                        '                         FROM BANDA_RELATORIO' ||
                        '                        WHERE NR_SEQUENCIA = :2)';
        EXECUTE ds_comando_w USING coalesce(NEW.NM_USUARIO, OLD.NM_USUARIO), coalesce(NEW.NR_SEQ_BANDA, OLD.NR_SEQ_BANDA);
...
-- REVOKE ALL ON FUNCTION trigger_fct_banda_relat_campo_atual() FROM PUBLIC;

CREATE TRIGGER banda_relat_campo_atual
	BEFORE INSERT OR UPDATE OR DELETE ON banda_relat_campo FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_banda_relat_campo_atual();
