...
DROP TRIGGER IF EXISTS pls_evento_regra_tit_rec_atual ON pls_evento_regra_tit_rec CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_evento_regra_tit_rec_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_evento_regra_tit_rec_atual() FROM PUBLIC;

CREATE TRIGGER pls_evento_regra_tit_rec_atual
	BEFORE INSERT OR UPDATE ON pls_evento_regra_tit_rec FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_evento_regra_tit_rec_atual();
