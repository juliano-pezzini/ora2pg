...
DROP TRIGGER IF EXISTS pls_conta_mat_tiss_update ON pls_conta_mat CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_conta_mat_tiss_update() RETURNS trigger AS $BODY$
declare
...
if	(nvl(wheb_usuario_pck.get_ie_lote_contabil,'N') = 'N') and
	...
	-- Se a conta já estava com status fechado e mudou um dos valores do item
	if	(:old.vl_liberado <> :new.vl_liberado) or
		(:old.vl_material <> :new.vl_material) or
		(:old.vl_glosa <> :new.vl_glosa) or
		(:old.vl_material_imp <> :new.vl_material_imp) then
		-- Verificar se conta foi fechada
		select	a.ie_status
		...
		if	(ie_status_conta_w = 'F') then
			...
			and	nr_seq_conta_mat	= :new.nr_sequencia
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_conta_mat_tiss_update() FROM PUBLIC;

CREATE TRIGGER pls_conta_mat_tiss_update
	BEFORE UPDATE ON pls_conta_mat FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_conta_mat_tiss_update();
