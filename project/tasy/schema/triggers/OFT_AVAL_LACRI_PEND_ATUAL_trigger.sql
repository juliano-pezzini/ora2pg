...
DROP TRIGGER IF EXISTS oft_aval_lacri_pend_atual ON oft_olho_seco CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_oft_aval_lacri_pend_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_oft_aval_lacri_pend_atual() FROM PUBLIC;

CREATE TRIGGER oft_aval_lacri_pend_atual
	AFTER INSERT OR UPDATE OR DELETE ON oft_olho_seco FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_oft_aval_lacri_pend_atual();
