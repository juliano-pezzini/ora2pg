...
DROP TRIGGER IF EXISTS agenda_pac_equip_delete ON agenda_pac_equip CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_agenda_pac_equip_delete() RETURNS trigger AS $BODY$
DECLARE
...
-- REVOKE ALL ON FUNCTION trigger_fct_agenda_pac_equip_delete() FROM PUBLIC;

CREATE TRIGGER agenda_pac_equip_delete
	BEFORE DELETE ON agenda_pac_equip FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_agenda_pac_equip_delete();
