...
DROP TRIGGER IF EXISTS pls_fatura_tp ON pls_fatura CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_fatura_tp() RETURNS trigger AS $BODY$
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_fatura_tp() FROM PUBLIC;

CREATE TRIGGER pls_fatura_tp
	AFTER UPDATE ON pls_fatura FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_fatura_tp();
