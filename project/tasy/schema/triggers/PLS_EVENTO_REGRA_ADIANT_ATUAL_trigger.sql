...
DROP TRIGGER IF EXISTS pls_evento_regra_adiant_atual ON pls_evento_regra_adiant CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_evento_regra_adiant_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_evento_regra_adiant_atual() FROM PUBLIC;

CREATE TRIGGER pls_evento_regra_adiant_atual
	BEFORE INSERT OR UPDATE ON pls_evento_regra_adiant FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_evento_regra_adiant_atual();
