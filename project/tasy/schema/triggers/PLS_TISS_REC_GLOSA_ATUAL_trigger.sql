...
DROP TRIGGER IF EXISTS pls_tiss_rec_glosa_atual ON pls_tiss_rec_glosa CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_tiss_rec_glosa_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_tiss_rec_glosa_atual() FROM PUBLIC;

CREATE TRIGGER pls_tiss_rec_glosa_atual
	BEFORE INSERT OR UPDATE ON pls_tiss_rec_glosa FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_tiss_rec_glosa_atual();
