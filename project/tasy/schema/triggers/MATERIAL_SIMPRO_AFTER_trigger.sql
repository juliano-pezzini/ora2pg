...
DROP TRIGGER IF EXISTS material_simpro_after ON material_simpro CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_material_simpro_after() RETURNS trigger AS $BODY$
declare

ds_retorno_integracao_w text;
event_w					varchar(25);
event_class_w			varchar(90);
...
if (qt_retorno_w = 'S')then
  /*Quando Incluir, alterar e excluir as informações da material_tuss chama a integração padrão*/
...
-- REVOKE ALL ON FUNCTION trigger_fct_material_simpro_after() FROM PUBLIC;

CREATE TRIGGER material_simpro_after
	AFTER INSERT OR UPDATE OR DELETE ON material_simpro FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_material_simpro_after();
