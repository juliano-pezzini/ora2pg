...
DROP TRIGGER IF EXISTS nascimento_pend_atual ON nascimento CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_nascimento_pend_atual() RETURNS trigger AS $BODY$
declare
  ...
  BEGIN
	...
	if 	((TG_OP = 'INSERT') OR (TG_OP = 'UPDATE')) then
      ...
      nr_atend_cons_pepa_p=> NEW.nr_seq_atend_cons_pepa
		...
		where 	nr_seq_registro = OLD.nr_seq_interno
...
-- REVOKE ALL ON FUNCTION trigger_fct_nascimento_pend_atual() FROM PUBLIC;

CREATE TRIGGER nascimento_pend_atual
	AFTER INSERT OR UPDATE OR DELETE ON nascimento FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_nascimento_pend_atual();
