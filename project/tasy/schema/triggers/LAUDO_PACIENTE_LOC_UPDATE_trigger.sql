...
DROP TRIGGER IF EXISTS laudo_paciente_loc_update ON laudo_paciente_loc CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_laudo_paciente_loc_update() RETURNS trigger AS $BODY$
declare
qt_regra_w		bigint;
...
-- REVOKE ALL ON FUNCTION trigger_fct_laudo_paciente_loc_update() FROM PUBLIC;

CREATE TRIGGER laudo_paciente_loc_update
	BEFORE UPDATE ON laudo_paciente_loc FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_laudo_paciente_loc_update();
