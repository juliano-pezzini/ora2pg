...
DROP TRIGGER IF EXISTS agenda_pac_equip_atual ON agenda_pac_equip CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_agenda_pac_equip_atual() RETURNS trigger AS $BODY$
declare
...
if (TG_OP = 'UPDATE') and (coalesce(NEW.ie_origem_inf,'I') = 'E') and (NEW.ie_tipo_exclusao is null)	then
	NEW.ie_tipo_exclusao := '2';
elsif (TG_OP = 'UPDATE') and (coalesce(NEW.ie_origem_inf,'I') = 'I') then
...
-- REVOKE ALL ON FUNCTION trigger_fct_agenda_pac_equip_atual() FROM PUBLIC;

CREATE TRIGGER agenda_pac_equip_atual
	BEFORE INSERT OR UPDATE ON agenda_pac_equip FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_agenda_pac_equip_atual();
