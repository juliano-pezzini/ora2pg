...
DROP TRIGGER IF EXISTS pls_dt_ref_material_tuss ON pls_material_tuss CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_dt_ref_material_tuss() RETURNS trigger AS $BODY$
declare
...
NEW.dt_inicio_vigencia_ref := pls_util_pck.obter_dt_vigencia_null( NEW.dt_vigencia_inicial, 'I');
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_dt_ref_material_tuss() FROM PUBLIC;

CREATE TRIGGER pls_dt_ref_material_tuss
	BEFORE INSERT OR UPDATE ON pls_material_tuss FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_dt_ref_material_tuss();
