...
DROP TRIGGER IF EXISTS rel_relatorio_document_atual ON relatorio_documentacao CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_rel_relatorio_document_atual() RETURNS trigger AS $BODY$
DECLARE
...
-- REVOKE ALL ON FUNCTION trigger_fct_rel_relatorio_document_atual() FROM PUBLIC;

CREATE TRIGGER rel_relatorio_document_atual
	BEFORE INSERT OR UPDATE OR DELETE ON relatorio_documentacao FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_rel_relatorio_document_atual();
