...
DROP TRIGGER IF EXISTS pls_prestador_crit_receb_atual ON pls_prestador_crit_receb CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_prestador_crit_receb_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_prestador_crit_receb_atual() FROM PUBLIC;

CREATE TRIGGER pls_prestador_crit_receb_atual
	BEFORE INSERT OR UPDATE ON pls_prestador_crit_receb FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_prestador_crit_receb_atual();
