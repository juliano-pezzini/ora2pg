...
DROP TRIGGER IF EXISTS paciente_alergia_pend_atual ON paciente_alergia CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_paciente_alergia_pend_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_paciente_alergia_pend_atual() FROM PUBLIC;

CREATE TRIGGER paciente_alergia_pend_atual
	AFTER INSERT OR UPDATE ON paciente_alergia FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_paciente_alergia_pend_atual();
