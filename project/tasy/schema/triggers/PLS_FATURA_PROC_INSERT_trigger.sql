...
DROP TRIGGER IF EXISTS pls_fatura_proc_insert ON pls_fatura_proc CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_fatura_proc_insert() RETURNS trigger AS $BODY$
declare
...
if (coalesce(wheb_usuario_pck.get_ie_lote_contabil,'N') = 'N') and (coalesce(wheb_usuario_pck.get_ie_atualizacao_contabil,'N') = 'N') then
	...
	if (nr_seq_lote_disc_w is null) then
		...
		where	nr_sequencia	= NEW.nr_seq_pos_proc;
	end if;
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_fatura_proc_insert() FROM PUBLIC;

CREATE TRIGGER pls_fatura_proc_insert
	BEFORE INSERT OR UPDATE ON pls_fatura_proc FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_fatura_proc_insert();
