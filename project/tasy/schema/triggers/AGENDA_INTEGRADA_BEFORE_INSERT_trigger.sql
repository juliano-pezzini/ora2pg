...
DROP TRIGGER IF EXISTS agenda_integrada_before_insert ON agenda_integrada CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_agenda_integrada_before_insert() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_agenda_integrada_before_insert() FROM PUBLIC;

CREATE TRIGGER agenda_integrada_before_insert
	BEFORE INSERT OR UPDATE ON agenda_integrada FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_agenda_integrada_before_insert();
