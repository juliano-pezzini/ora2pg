...
DROP TRIGGER IF EXISTS agenda_paciente_tie ON agenda_paciente CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_agenda_paciente_tie() RETURNS trigger AS $BODY$
declare
...
SELECT  a.nr_sequencia schedule_id,
        ...
        substr(obter_valor_dominio(83, a.ie_status_agenda),1,200) schedule_status,
        ...
        coalesce((select max(wheb_mensagem_pck.get_texto(94754)) from agenda_pac_opme h where h.nr_seq_agenda = a.nr_sequencia and h.dt_exclusao is null), wheb_mensagem_pck.get_texto(94755)) OPSM,
        ...
        (select max(h.ds_classif_paciente) from tipo_classificao_paciente h where h.nr_sequencia = a.nr_seq_tipo_classif_pac) patient_classification,
        ...
        coalesce((select     max(wheb_mensagem_pck.get_texto(94754))
             ...
             where      a.nr_sequencia = h.nr_seq_agenda
...
FROM pessoa_fisica d, (select NEW.nr_sequencia nr_sequencia,
                NEW.cd_agenda cd_agenda,
                NEW.ie_status_agenda ie_status_agenda,
                NEW.dt_agenda dt_agenda,
                NEW.dt_agendamento dt_agendamento,
                NEW.cd_procedimento cd_procedimento,
                NEW.ie_origem_proced ie_origem_proced,
                NEW.nr_reserva nr_reserva,
                NEW.dt_chegada_prev dt_chegada_prev,
                NEW.hr_inicio hr_inicio,
                NEW.qt_idade_gestacional qt_idade_gestacional,
                NEW.qt_idade_paciente qt_idade_paciente,
                NEW.cd_medico cd_medico,
                NEW.nr_atendimento nr_atendimento,
                NEW.cd_convenio cd_convenio,
                NEW.cd_usuario_convenio cd_usuario_convenio,
                NEW.ie_reserva_leito ie_reserva_leito,
                NEW.nr_telefone nr_telefone,
                NEW.nr_minuto_duracao nr_minuto_duracao,
                NEW.cd_pessoa_fisica cd_pessoa_fisica,
                NEW.ie_anestesia ie_anestesia,
                NEW.ie_carater_cirurgia ie_carater_cirurgia,
                NEW.nr_seq_tipo_classif_pac nr_seq_tipo_classif_pac,
                NEW.nr_seq_cobertura nr_seq_cobertura
        ) a
...
WHERE a.cd_agenda = b.cd_agenda  and d.cd_pessoa_fisica = a.cd_pessoa_fisica and a.ie_status_agenda <> 'L' and to_date(a.hr_inicio, 'dd/mm/yyyy hh24:mi:ss') between to_date(LOCALTIMESTAMP::text, 'dd/mm/yyyy hh24:mi:ss') and to_date(LOCALTIMESTAMP + (obter_valor_param_usuario(410, 64, obter_perfil_ativo, wheb_usuario_pck.get_nm_usuario, wheb_usuario_pck.get_cd_estabelecimento))::numeric /24, 'dd/mm/yyyy hh24:mi:ss') and b.cd_tipo_agenda = 1 
...
select  a.nr_sequencia schedule_id,
        ...
        a.dt_agendamento scheduling_date,
        a.cd_agenda schedule_code,
        i.cd_procedimento procedure_id,
        substr(obter_descricao_procedimento(i.cd_procedimento, i.ie_origem_proced),1,100) procedure_description,
        wheb_mensagem_pck.get_texto(163013) procedure_status,
        a.nr_reserva reservation,
        c.dt_entrada admission_date,
        a.dt_chegada_prev estimated_admission_date,
        ...
        coalesce((select     max(substr(obter_valor_dominio(3195,h.ie_status),1,255))
             ...
             where      a.nr_sequencia = h.nr_seq_agenda
             and        nr_seq_proc_servico =  6817), wheb_mensagem_pck.get_texto(94755)) surgical_services_status
FROM agenda_paciente_proc i, pessoa_fisica d, (select NEW.nr_sequencia nr_sequencia,
                NEW.cd_agenda cd_agenda,
                NEW.ie_status_agenda ie_status_agenda,
                NEW.dt_agenda dt_agenda,
                NEW.dt_agendamento dt_agendamento,
...
, classif_agenda_cirurgica f
LEFT OUTER JOIN agenda b ON (f.nr_sequencia = b.nr_seq_classif)
WHERE a.cd_agenda = b.cd_agenda  and d.cd_pessoa_fisica = a.cd_pessoa_fisica and i.nr_sequencia = a.nr_sequencia and a.dt_agenda >= LOCALTIMESTAMP and a.ie_status_agenda <> 'L' and to_date(a.hr_inicio, 'dd/mm/yyyy hh24:mi:ss') between to_date(LOCALTIMESTAMP::text, 'dd/mm/yyyy hh24:mi:ss') and to_date(LOCALTIMESTAMP + (obter_valor_param_usuario(410, 64, obter_perfil_ativo, wheb_usuario_pck.get_nm_usuario, wheb_usuario_pck.get_cd_estabelecimento))::numeric /24, 'dd/mm/yyyy hh24:mi:ss') and b.cd_tipo_agenda = 1  order by schedule_date, start_time, patient LIMIT 1;

BEGIN

if (wheb_usuario_pck.get_nm_usuario is not null and
  ((TG_OP = 'INSERT' and NEW.ie_status_agenda <> 'L') or (TG_OP = 'UPDATE' and OLD.ie_status_agenda <> 'L' and NEW.ie_status_agenda <> 'B'))) then
...
-- REVOKE ALL ON FUNCTION trigger_fct_agenda_paciente_tie() FROM PUBLIC;

CREATE TRIGGER agenda_paciente_tie
	AFTER INSERT OR UPDATE ON agenda_paciente FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_agenda_paciente_tie();
