...
DROP TRIGGER IF EXISTS pls_prestador_tributo_delete ON pls_prestador_tributo CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_prestador_tributo_delete() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_prestador_tributo_delete() FROM PUBLIC;

CREATE TRIGGER pls_prestador_tributo_delete
	AFTER DELETE ON pls_prestador_tributo FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_prestador_tributo_delete();
