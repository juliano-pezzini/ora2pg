...
DROP TRIGGER IF EXISTS agenda_pac_cme_tie ON agenda_pac_cme CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_agenda_pac_cme_tie() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_agenda_pac_cme_tie() FROM PUBLIC;

CREATE TRIGGER agenda_pac_cme_tie
	AFTER INSERT OR UPDATE ON agenda_pac_cme FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_agenda_pac_cme_tie();
