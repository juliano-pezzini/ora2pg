...
DROP TRIGGER IF EXISTS pls_regra_intercambio_atual ON pls_regra_intercambio CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_regra_intercambio_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_regra_intercambio_atual() FROM PUBLIC;

CREATE TRIGGER pls_regra_intercambio_atual
	BEFORE INSERT OR UPDATE ON pls_regra_intercambio FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_regra_intercambio_atual();
