...
DROP TRIGGER IF EXISTS intpd_ordem_compra_item_tr ON ordem_compra_item_trib CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_intpd_ordem_compra_item_tr() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_intpd_ordem_compra_item_tr() FROM PUBLIC;

CREATE TRIGGER intpd_ordem_compra_item_tr
	BEFORE INSERT OR UPDATE OR DELETE ON ordem_compra_item_trib FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_intpd_ordem_compra_item_tr();
