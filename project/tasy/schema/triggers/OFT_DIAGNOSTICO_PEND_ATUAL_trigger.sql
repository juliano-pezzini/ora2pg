...
DROP TRIGGER IF EXISTS oft_diagnostico_pend_atual ON oft_diagnostico CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_oft_diagnostico_pend_atual() RETURNS trigger AS $BODY$
declare
...
if (ie_libera_exames_oft_w = 'S') then
	if (TG_OP = 'INSERT') or (TG_OP = 'UPDATE') then
		...
		if (NEW.dt_liberacao is null) then
			if (coalesce(NEW.ie_paciente,'S') = 'S') then
				ie_tipo_w := 'ODL';
			else
				ie_tipo_w := 'ODF';
			end if;
...
-- REVOKE ALL ON FUNCTION trigger_fct_oft_diagnostico_pend_atual() FROM PUBLIC;

CREATE TRIGGER oft_diagnostico_pend_atual
	AFTER INSERT OR UPDATE OR DELETE ON oft_diagnostico FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_oft_diagnostico_pend_atual();
