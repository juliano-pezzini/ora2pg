...
DROP TRIGGER IF EXISTS kv_atestado_crianca_update ON kv_atestado_crianca CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_kv_atestado_crianca_update() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_kv_atestado_crianca_update() FROM PUBLIC;

CREATE TRIGGER kv_atestado_crianca_update
	BEFORE UPDATE ON kv_atestado_crianca FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_kv_atestado_crianca_update();
