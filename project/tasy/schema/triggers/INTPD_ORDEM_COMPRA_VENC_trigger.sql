...
DROP TRIGGER IF EXISTS intpd_ordem_compra_venc ON ordem_compra_venc CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_intpd_ordem_compra_venc() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_intpd_ordem_compra_venc() FROM PUBLIC;

CREATE TRIGGER intpd_ordem_compra_venc
	BEFORE INSERT OR UPDATE ON ordem_compra_venc FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_intpd_ordem_compra_venc();
