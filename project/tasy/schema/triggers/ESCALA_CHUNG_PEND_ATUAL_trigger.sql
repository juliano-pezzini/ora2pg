...
DROP TRIGGER IF EXISTS escala_chung_pend_atual ON escala_chung CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_escala_chung_pend_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_escala_chung_pend_atual() FROM PUBLIC;

CREATE TRIGGER escala_chung_pend_atual
	AFTER INSERT OR UPDATE ON escala_chung FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_escala_chung_pend_atual();
