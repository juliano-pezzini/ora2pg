...
DROP TRIGGER IF EXISTS pls_alt_autor_mat_log_bd ON pls_guia_plano_mat CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_alt_autor_mat_log_bd() RETURNS trigger AS $BODY$
declare
 
nm_machine_w		varchar(255);
ie_tipo_alteracao_w	varchar(1)	:= null;
ds_log_w		varchar(2000)	:= null;
qt_rotina_w		bigint	:= null;
BEGIN
  BEGIN 
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_alt_autor_mat_log_bd() FROM PUBLIC;

CREATE TRIGGER pls_alt_autor_mat_log_bd
	BEFORE UPDATE OR DELETE ON pls_guia_plano_mat FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_alt_autor_mat_log_bd();
