...
DROP TRIGGER IF EXISTS oft_acuidade_pend_atual ON oft_acuidade_visual CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_oft_acuidade_pend_atual() RETURNS trigger AS $BODY$
declare
...
-- REVOKE ALL ON FUNCTION trigger_fct_oft_acuidade_pend_atual() FROM PUBLIC;

CREATE TRIGGER oft_acuidade_pend_atual
	AFTER INSERT OR UPDATE OR DELETE ON oft_acuidade_visual FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_oft_acuidade_pend_atual();
