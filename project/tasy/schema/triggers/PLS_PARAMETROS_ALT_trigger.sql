...
DROP TRIGGER IF EXISTS pls_parametros_alt ON pls_parametros CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_pls_parametros_alt() RETURNS trigger AS $BODY$
declare

qt_registros_ant_w		integer	:= 0;
qt_registros_atual_w	integer := 0;
...
C01 CURSOR FOR
	SELECT	job
	from	job_v
	where	comando	like '%PLS_PROCES_CTA_ANALISE_COMPL%';
BEGIN
  BEGIN

--Verifica se é update para buscar os registros anteriores
if (TG_OP = 'UPDATE') then
	if (OLD.qt_filas_paralelo is not null) then
		qt_registros_ant_w	:= OLD.qt_filas_paralelo;
	end if;
end if;

if (NEW.qt_filas_paralelo is not null) then
	qt_registros_atual_w := NEW.qt_filas_paralelo;
end if;

--Verifica se o registro antigo é diferente do novo
if (qt_registros_ant_w <> qt_registros_atual_w) then
	...
	qt_registros_gerar_w	:= qt_registros_atual_w;
	qt_interval_w		:= 0.100;
...
-- REVOKE ALL ON FUNCTION trigger_fct_pls_parametros_alt() FROM PUBLIC;

CREATE TRIGGER pls_parametros_alt
	AFTER INSERT OR UPDATE ON pls_parametros FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_pls_parametros_alt();
