...
DROP TRIGGER IF EXISTS prescr_proced_upd_hl7_dixtal ON prescr_procedimento CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_prescr_proced_upd_hl7_dixtal() RETURNS trigger AS $BODY$
declare
...
if (coalesce(OLD.ie_suspenso,'N') = 'N') and (NEW.ie_suspenso = 'S') and (NEW.ie_tipo_proced <> 'G') and (NEW.nr_seq_solic_sangue is null) then 
	...
	if (ie_leito_monit = 'S') then 
		...
		ds_param_integ_hl7_w :=	'cd_pessoa_fisica=' || cd_pessoa_fisica_w || ds_sep_bv_w || 
						...
						'nr_seq_proc='	  || NEW.nr_sequencia   || ds_sep_bv_w;	
...
-- REVOKE ALL ON FUNCTION trigger_fct_prescr_proced_upd_hl7_dixtal() FROM PUBLIC;

CREATE TRIGGER prescr_proced_upd_hl7_dixtal
	AFTER UPDATE ON prescr_procedimento FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_prescr_proced_upd_hl7_dixtal();
