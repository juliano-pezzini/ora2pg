...
DROP TRIGGER IF EXISTS nota_fiscal_venc_trib_atual ON nota_fiscal_venc_trib CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_nota_fiscal_venc_trib_atual() RETURNS trigger AS $BODY$
declare
...
if (ie_forma_arred_nf_w = 'S') then
	BEGIN
	vl_base_calculo_w	:= coalesce(NEW.vl_base_calculo, 0) - coalesce(NEW.vl_reducao, 0);
...
-- REVOKE ALL ON FUNCTION trigger_fct_nota_fiscal_venc_trib_atual() FROM PUBLIC;

CREATE TRIGGER nota_fiscal_venc_trib_atual
	BEFORE INSERT OR UPDATE ON nota_fiscal_venc_trib FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_nota_fiscal_venc_trib_atual();
