...
CREATE OR REPLACE PROCEDURE vipe_gerar_horarios_pck.vipe_sincronizar_medic ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_exibir_hor_realizados_p text, ie_exibir_hor_suspensos_p text, ie_regra_inclusao_p text, ie_data_lib_prescr_p text, ie_exibir_suspensos_p text, ie_agrupar_acm_sn_p text, ie_agrupar_dose_esp_p text, ie_obs_diferenciar_p text, dt_horario_p timestamp, nr_horario_p integer, ie_mostrar_substituido_p text, ie_prescr_setor_p text, cd_setor_paciente_p bigint) AS $body$
DECLARE
	...
	ie_horario_especial_w	varchar(15);
	...
	SELECT	a.nr_prescricao,
		c.nr_seq_material,
		c.nr_sequencia,
		substr(CASE WHEN coalesce(x.dt_suspensao::text, '') = '' THEN obter_status_hor_medic_lib(c.dt_fim_horario,c.dt_suspensao,c.ie_dose_especial,c.nr_seq_processo,c.nr_seq_area_prep, c.dt_lib_horario)  ELSE CASE WHEN coalesce(c.dt_fim_horario::text, '') = '' THEN 'K'  ELSE 'A' END  END ,1,15),
		c.cd_material,
		...
		c.dt_horario,
		VIPE_obter_se_colore(a.ie_motivo_prescricao, x.nr_prescricao, x.nr_sequencia, 'M') ie_pintar,
		...
		coalesce(VIPE_obter_se_medic_composto(a.nr_prescricao, x.nr_sequencia, x.nr_agrupamento, 'P'),'N')
	from	material y,
		prescr_material x,
		prescr_mat_hor c,
		prescr_medica a
	...
	--and	((c.dt_horario between dt_horario_inicio_sinc_w and dt_horario_fim_sinc_w) or
	--	 (nvl(c.ie_horario_especial,'N') = 'S'))
	...
	group by
		...
		VIPE_obter_se_colore(a.ie_motivo_prescricao, x.nr_prescricao, x.nr_sequencia, 'M'),
		x.cd_intervalo,
		...
		c.ie_horario_adep,
		coalesce(VIPE_obter_se_medic_composto(a.nr_prescricao, x.nr_sequencia, x.nr_agrupamento, 'P'),'N')
	order by c.dt_horario, c.dt_suspensao;
...
BEGIN
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		if (nr_horario_w IS NOT NULL AND nr_horario_w::text <> '') then
			...
			if (ie_horario_especial_w	= 'N') and (ie_horario_adep_w	= 'S')  then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE vipe_gerar_horarios_pck.vipe_sincronizar_medic ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_exibir_hor_realizados_p text, ie_exibir_hor_suspensos_p text, ie_regra_inclusao_p text, ie_data_lib_prescr_p text, ie_exibir_suspensos_p text, ie_agrupar_acm_sn_p text, ie_agrupar_dose_esp_p text, ie_obs_diferenciar_p text, dt_horario_p timestamp, nr_horario_p integer, ie_mostrar_substituido_p text, ie_prescr_setor_p text, cd_setor_paciente_p bigint) FROM PUBLIC;
