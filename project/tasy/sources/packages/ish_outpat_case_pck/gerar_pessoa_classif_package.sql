...
/* internal procedure to generate data on the patient's classification table (PESSOA_CLASSIF) */

CREATE OR REPLACE PROCEDURE ish_outpat_case_pck.gerar_pessoa_classif ( reg_integracao_p INOUT gerar_int_padrao.reg_integracao_conv, cd_pessoa_fisica_p text, ie_classif_p text, vl_classif_p text, nr_seq_regra_p bigint, ie_conversao_p text, nm_usuario_p text, dt_atualizacao_p timestamp) AS $body$
DECLARE
...
intpd_processar_atributo(reg_integracao_p,'NM_USUARIO',nm_usuario_p,'N',nm_usuario_w);
...
if (reg_integracao_p.qt_reg_log = 0) then
	...
	if (pessoa_classif_w.nr_sequencia IS NOT NULL AND pessoa_classif_w.nr_sequencia::text <> '') and (coalesce(vl_classif_p,'a') <> 'x') then
		...
		pessoa_classif_w.nm_usuario		:=	nm_usuario_w;
		...
		pessoa_classif_w.nm_usuario		:=	nm_usuario_w;
		pessoa_classif_w.dt_atualizacao	:=	dt_atualizacao_p;
		pessoa_classif_w.nm_usuario_nrec	:=	nm_usuario_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ish_outpat_case_pck.gerar_pessoa_classif ( reg_integracao_p INOUT gerar_int_padrao.reg_integracao_conv, cd_pessoa_fisica_p text, ie_classif_p text, vl_classif_p text, nr_seq_regra_p bigint, ie_conversao_p text, nm_usuario_p text, dt_atualizacao_p timestamp) FROM PUBLIC;
