...
CREATE OR REPLACE FUNCTION phi_defect_measures_pck.get_internal_defect_density ( dt_referencia_p timestamp, ie_opcao_p text default 'R') RETURNS bigint AS $body$
DECLARE


qtd_internal_defects_w		double precision;
qtd_loc_w			double precision;
dt_ref_mes_w			timestamp;
dt_fim_mes_w			timestamp;
internal_defect_density_w	double precision;

/*
ie_opcao_p
D	= Internal Defect Number
L	= KPI Lines Kaloc
R	= Result
*/
...
select	count(distinct a.nr_sequencia) qtd_defects
into STRICT	qtd_internal_defects_w
...
internal_defect_density_w := (dividir_sem_round(qtd_internal_defects_w,qtd_loc_w)*1000);

if (ie_opcao_w = 'D') then
	return	qtd_internal_defects_w;
elsif (ie_opcao_w = 'L') then
	return	qtd_loc_w;
else
	return	internal_defect_density_w;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION phi_defect_measures_pck.get_internal_defect_density ( dt_referencia_p timestamp, ie_opcao_p text default 'R') FROM PUBLIC;
