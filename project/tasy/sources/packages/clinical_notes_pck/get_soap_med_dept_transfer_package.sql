...
CREATE OR REPLACE FUNCTION clinical_notes_pck.get_soap_med_dept_transfer ( nr_atendimento_p bigint, nr_sequencia_p bigint, ie_record_type_p text, ie_soap_type_p text, ie_stage_p bigint, cd_exp_title_p dic_expressao.cd_expressao%type default null) RETURNS varchar AS $body$
DECLARE
	...
	ds_special_clasif_w atend_paciente_unidade.ds_observacao%type;
    c01 CURSOR FOR
    SELECT *
...
BEGIN
    ...
    obter_nome_departamento_medico(coalesce(get_dept_transfer_japan_number(a.nr_atendimento,'DEST_DEPT',clock_timestamp()),a.cd_departamento),obter_departamento_setor(a.cd_setor_atendimento)),
    CASE WHEN coalesce(a.cd_unidade_compl::text, '') = '' THEN a.cd_unidade_basica  ELSE a.cd_unidade_basica || ' - ' || a.cd_unidade_compl END ,
    pkg_date_formaters.to_varchar(coalesce(get_dept_transfer_japan_date(a.nr_atendimento,clock_timestamp()),a.DT_ENTRADA_UNIDADE), 'timestamp', wheb_usuario_pck.get_cd_estabelecimento, wheb_usuario_pck.get_nm_usuario),
    substr(obter_desc_motivo_transf_pac(coalesce(get_dept_transfer_japan_number(a.nr_atendimento,'REASON_TRANSFER',clock_timestamp()),a.nr_seq_motivo_transf)),1,60),
	...
	(select max(ds_classificacao) ds from CLASSIF_ESPECIAL_PACIENTE x where x.nr_sequencia = (select max(nr_seq_classif_esp) from atendimento_paciente y where y.nr_atendimento = a.nr_atendimento))
    into STRICT
    ds_care_dept_w,
    ds_med_dept_w,
    ds_unit_w,
    ...
    if (ie_record_type_p = 'DEPT_TRANSFER') then
        if (r_c01.ie_field    = 'CARE_DEPT' and (ds_care_dept_w IS NOT NULL AND ds_care_dept_w::text <> ''))then
          if (coalesce(ds_content_w::text, '') = '' ) then
            ds_content_w    := '<b>' || coalesce(obter_desc_expressao(cd_exp_title_p),obter_desc_expressao(488808)) || '</b>';
        ...
        if (r_c01.ie_field   = 'PRIM_NURSE' and (nm_nurse_primary_w IS NOT NULL AND nm_nurse_primary_w::text <> ''))then
		  ...
		  if (r_c01.ie_field   = 'NR_SEQ_CLASSIF' and (ds_special_clasif_w IS NOT NULL AND ds_special_clasif_w::text <> ''))then
          if (coalesce(ds_content_w::text, '') = '' ) then
            ds_content_w    := '<b>' || coalesce(obter_desc_expressao(cd_exp_title_p),obter_desc_expressao(488808)) || '</b>';
            ds_content_w   := ds_content_w|| '</br>' || coalesce(r_c01.nm_field,r_c01.ds_expressao) || ' - '|| ds_special_clasif_w;
          else
            ds_content_w := ds_content_w|| '</br>' || coalesce(r_c01.nm_field,r_c01.ds_expressao) || ' - '|| ds_special_clasif_w;
          end if;
          end if;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION clinical_notes_pck.get_soap_med_dept_transfer ( nr_atendimento_p bigint, nr_sequencia_p bigint, ie_record_type_p text, ie_soap_type_p text, ie_stage_p bigint, cd_exp_title_p dic_expressao.cd_expressao%type default null) FROM PUBLIC;
