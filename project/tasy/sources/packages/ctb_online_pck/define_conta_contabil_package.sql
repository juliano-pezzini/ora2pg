...
CALL philips_contabil_pck.set_cd_sequencia_parametro(null);
  if (ie_origem_conta_p in ('3','8')) then
      begin
          begin
              select cd_empresa into STRICT cd_empresa_w from estabelecimento where cd_estabelecimento = cd_estabelecimento_p;
          exception
              when others then
                  cd_empresa_w := null;
          end;
          ...
          if (cd_pessoa_fisica_p IS NOT NULL AND cd_pessoa_fisica_p::text <> '') then
              begin
                  select Obter_Conta_Contab_PF(cd_empresa_w,
                                                cd_pessoa_fisica_p,
                                                CASE WHEN ie_origem_conta_p='3' THEN 'P' WHEN ie_origem_conta_p='8' THEN 'R' END ,
                                                dt_vigencia_p)
...
;
  ...
  if (ie_origem_conta_p = '2') then
      begin
          ...
          if (cd_material_p IS NOT NULL AND cd_material_p::text <> '') then
              ...
              define_Conta_Procedimento(cd_estabelecimento_p,
                                        cd_procedimento_p,
                                        ie_origem_proced_p,
                                        ie_tipo_conta_w,
                                        null,
  ...
  if (ie_origem_conta_p = '5') then
      select Obter_Conta_Convenio(cd_estabelecimento_p,
                                  cd_convenio_p,
                                  null,
                                  null,
                                  dt_vigencia_p,
      ...
      begin
          ...
          begin
              if (coalesce(cd_pessoa_fisica_p,'0') <> '0') then
                  select Obter_Conta_Contab_PF(cd_empresa_w,
                                                cd_pessoa_fisica_p,
                                                CASE WHEN ie_origem_conta_p='3' THEN 'P' WHEN ie_origem_conta_p='8' THEN 'R'  ELSE 'R' END ,
                                                dt_vigencia_p)
                  into STRICT   cd_conta_contabil_p
;
              else

                  select obter_conta_contab_pj(cd_empresa_w,
                                                cd_estabelecimento_p,
                                                cd_cgc_p,
                                                'R',
                                                coalesce(dt_vigencia_p,clock_timestamp()))
                  into STRICT   cd_conta_contabil_p
;
              end if;
          exception
  ...
  if (ie_origem_conta_p = '12') then
      begin
          ...
          select Obter_Conta_Convenio(cd_estabelecimento_p,
                                      ...
                                      cd_procedimento_p,
                                      ie_origem_proced_p)
          into STRICT   current_setting('ctb_online_pck.cd_conta_contabil_w')::ctb_regra_estab_dif.cd_conta_contabil%type
;

          if (coalesce(current_setting('ctb_online_pck.cd_conta_contabil_w')::ctb_regra_estab_dif.cd_conta_contabil%type,'0') = '0') then
              ...
              if (current_setting('ctb_online_pck.cd_conta_contabil_w')::ctb_regra_estab_dif.cd_conta_contabil%coalesce(type::text, '') = '') then
                  PERFORM set_config('ctb_online_pck.cd_conta_contabil_w', obter_conta_contab_pj(cd_empresa_p         => cd_empresa_w,
                                                               cd_estabelecimento_p => cd_estabelecimento_p,
                                                               cd_cgc_p             => cd_cgc_p,
                                                               ie_tipo_p            => 'R',
                                                               dt_vigencia_p        => dt_vigencia_p), false);
              end if;

          end if;

          cd_conta_contabil_p := current_setting('ctb_online_pck.cd_conta_contabil_w')::ctb_regra_estab_dif.cd_conta_contabil%type;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ctb_online_pck.define_conta_contabil (ie_origem_conta_p text, cd_estabelecimento_p bigint, cd_cgc_p text, cd_pessoa_fisica_p text, ie_tipo_convenio_p bigint, cd_convenio_p bigint, cd_material_p bigint, cd_procedimento_p bigint, ie_origem_proced_p bigint, cd_local_estoque_p bigint, cd_conta_contabil_p INOUT text, cd_centro_custo_p INOUT text, ds_observacao_p text, dt_vigencia_p timestamp, ie_regra_pacote_p text default null, nr_seq_motivo_solic_p bigint default null, cd_operacao_nf_p bigint default null, nr_seq_proc_interno_p nota_fiscal_item.nr_seq_proc_interno%type default null, nr_seq_proj_rec_p bigint default null, vl_item_regra_p bigint default null) FROM PUBLIC;
