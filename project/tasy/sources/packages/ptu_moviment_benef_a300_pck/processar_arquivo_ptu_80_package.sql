...
CREATE OR REPLACE PROCEDURE ptu_moviment_benef_a300_pck.processar_arquivo_ptu_80 ( nr_seq_lote_p bigint, arq_texto_p utl_file.file_type) AS $body$
DECLARE
...
BEGIN
	...
	for r_c01_w in current_setting('ptu_moviment_benef_a300_pck.c01')::CURSOR(nr_seq_lote_pc(nr_seq_lote_p) loop
		...
		ds_conteudo_w := lpad(r_c01_w.cd_unimed_destino,4,0) || lpad(r_c01_w.cd_unimed_origem,4,0) || to_char(r_c01_w.dt_geracao,'YYYYMMDD') ||
				...
				rpad(r_c01_w.ie_tipo_produto,2,' ') || '14';
		...
		for r_c02_w in current_setting('ptu_moviment_benef_a300_pck.c02')::CURSOR(nr_seq_mov_produto_pc(r_c01_w.nr_seq_mov_produto) loop
			ds_conteudo_w := lpad(r_c02_w.ds_reservado,4,' ') || lpad(coalesce(r_c02_w.cd_filial,0),3,0) || rpad(r_c02_w.ds_razao_social,40,' ') ||
					rpad(r_c02_w.nm_empr_abrev,18,' ') || lpad(r_c02_w.ie_tipo_pessoa,1,0) || lpad(r_c02_w.cd_cgc_cpf,15,0) ||
					...
					rpad(coalesce(r_c02_w.ds_bairro,' '),30,' ') || rpad(r_c02_w.cd_cep,8,' ') || rpad(r_c02_w.nm_cidade,30,' ') ||
					rpad(r_c02_w.sg_uf,2,' ') || lpad(coalesce(r_c02_w.nr_ddd,0),4,0) || '                ' ||
					...
					lpad(coalesce(r_c02_w.nr_fax,0),9,0) || lpad(coalesce(r_c02_w.nr_contrato,0),15,0) || lpad(coalesce(somente_numero(r_c02_w.nr_endereco),0),6,0);
			...
			for r_c03_w in current_setting('ptu_moviment_benef_a300_pck.c03')::CURSOR(nr_seq_mov_prod_empresa_pc(r_c02_w.nr_seq_mov_prod_empresa) loop
				ds_conteudo_w := lpad(r_c03_w.cd_unimed,4,'0') || lpad(r_c03_w.ds_reservado,3,' ') || lpad(r_c03_w.id_benef,13,0) ||
						...
						lpad(r_c03_w.cd_cgc_cpf,15,0) || lpad(coalesce(r_c03_w.nr_rg,0),15,0) || rpad(coalesce(r_c03_w.sg_uf_rg,' '),2,' ') ||
						...
						rpad(r_c03_w.nm_beneficiario,120,' ') || lpad(coalesce(r_c03_w.ie_filho,0),2,0) || lpad(coalesce(r_c03_w.cd_empresa_origem,0),10,0) ||
						...
						rpad(coalesce(r_c03_w.nr_pis_pasep,0),11,0);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_moviment_benef_a300_pck.processar_arquivo_ptu_80 ( nr_seq_lote_p bigint, arq_texto_p utl_file.file_type) FROM PUBLIC;
