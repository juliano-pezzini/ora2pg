...
CREATE OR REPLACE PROCEDURE ptu_moviment_benef_a300_pck.processar_arquivo_ptu_90 ( nr_seq_lote_p bigint, arq_texto_p utl_file.file_type) AS $body$
DECLARE
	...
	for r_c01_w in current_setting('ptu_moviment_benef_a300_pck.c01')::CURSOR(nr_seq_lote_pc(nr_seq_lote_p) loop
		...
		for r_c02_w in current_setting('ptu_moviment_benef_a300_pck.c02')::CURSOR(nr_seq_mov_produto_pc(r_c01_w.nr_seq_mov_produto) loop
			ds_conteudo_w := lpad(r_c02_w.ds_reservado,4,' ') || lpad(coalesce(r_c02_w.cd_filial,0),3,0) || rpad(coalesce(r_c02_w.ds_razao_social,' '),40,' ') ||
					...
					rpad(r_c02_w.ds_reservado,2,' ') || lpad(coalesce(r_c02_w.nr_ddd,0),4,0) || '                ' ||
					...
					lpad(coalesce(r_c02_w.nr_fax,0),9,0) || lpad(coalesce(r_c02_w.nr_contrato,0),15,0) || lpad(coalesce(somente_numero(r_c02_w.nr_endereco),0),6,0) || rpad(coalesce(r_c02_w.nm_empr_abrev,' '),30,' ');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_moviment_benef_a300_pck.processar_arquivo_ptu_90 ( nr_seq_lote_p bigint, arq_texto_p utl_file.file_type) FROM PUBLIC;
