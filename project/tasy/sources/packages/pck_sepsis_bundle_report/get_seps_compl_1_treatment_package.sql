...
SET client_encoding TO 'UTF8';
   ...
   -----------------------
CREATE OR REPLACE FUNCTION pck_sepsis_bundle_report.get_seps_compl_1_treatment (nr_atendimento_p gqa_protocolo_pac.nr_atendimento%type) RETURNS bigint AS $body$
DECLARE
...
union
      ...
      r_1_treatment c_1_treatment%rowtype;


BEGIN

      -- 3350 ICCA sepsis - Compliance measure 1 treatment
      dt_inicio_protocolo_w := pck_sepsis_bundle_report.get_dt_ini_protocol(nr_atendimento_p, current_setting('pck_sepsis_bundle_report.cd_etp_1_treatment_w')::bigint, dt_inicio_protocolo_w);
      ...
      if (dt_inicio_protocolo_w IS NOT NULL AND dt_inicio_protocolo_w::text <> '') then
         ...
         loop
            fetch c_1_treatment
               into r_1_treatment;
            EXIT WHEN NOT FOUND; /* apply on c_1_treatment */
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION pck_sepsis_bundle_report.get_seps_compl_1_treatment (nr_atendimento_p gqa_protocolo_pac.nr_atendimento%type) FROM PUBLIC;
