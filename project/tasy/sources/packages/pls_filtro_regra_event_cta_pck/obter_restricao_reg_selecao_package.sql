...
CREATE OR REPLACE FUNCTION pls_filtro_regra_event_cta_pck.obter_restricao_reg_selecao ( ie_considera_selecao_p boolean, ie_tipo_regra_p pls_pp_cta_combinada.ie_tipo_regra%type, nr_id_transacao_p pls_pp_cta_selecao.nr_id_transacao%type, ie_excecao_p pls_pp_cta_filtro.ie_excecao%type, nr_seq_filtro_p pls_pp_cta_filtro.nr_sequencia%type, ds_campo_p out text, ds_tabela_p out text, valor_bind_p INOUT sql_pck.t_dado_bind) AS $body$
DECLARE
...
ds_alias_sel_w := pls_filtro_regra_event_cta_pck.obter_alias_tabela('selecao');
...
if (ie_considera_selecao_p = true) then
	ds_tabela_p := ds_tabela_p || ', ' || pls_util_pck.enter_w ||
			' pls_pp_cta_selecao ' || ds_alias_sel_w;
	...
	ds_campo_p := ds_campo_p || 	', ' || pls_util_pck.enter_w ||
					...
					'   from 	pls_pp_cta_selecao ' || ds_alias_sel_w || pls_util_pck.enter_w ||
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION pls_filtro_regra_event_cta_pck.obter_restricao_reg_selecao ( ie_considera_selecao_p boolean, ie_tipo_regra_p pls_pp_cta_combinada.ie_tipo_regra%type, nr_id_transacao_p pls_pp_cta_selecao.nr_id_transacao%type, ie_excecao_p pls_pp_cta_filtro.ie_excecao%type, nr_seq_filtro_p pls_pp_cta_filtro.nr_sequencia%type, ds_campo_p out text, ds_tabela_p out text, valor_bind_p INOUT sql_pck.t_dado_bind) FROM PUBLIC;
