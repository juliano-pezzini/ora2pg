...
c_filtro CURSOR(nr_seq_filtro_pc	pls_pp_cta_filtro.nr_sequencia%type) FOR
	SELECT	a.nr_sequencia,
		a.ie_tipo_guia,
		a.ie_tipo_guia_princ,
		a.nr_seq_tipo_atendimento,
		a.nr_seq_tipo_atend_princ,
		a.ie_origem_conta,
		coalesce(a.ie_apresentacao,'T') ie_apresentacao,
		ie_regime_atend_princ,
		ie_regime_atendimento,
		ie_saude_ocupacional
	from	pls_pp_cta_filtro_conta a
...
if (nr_seq_filtro_p IS NOT NULL AND nr_seq_filtro_p::text <> '') then
	...
	for r_c_filtro_w in c_filtro(nr_seq_filtro_p) loop
		...
		begin
			...
			SELECT * FROM pls_filtro_regra_event_cta_pck.obter_restricao_conta(	r_c_filtro_w.ie_tipo_guia, r_c_filtro_w.ie_tipo_guia_princ, r_c_filtro_w.nr_seq_tipo_atendimento, r_c_filtro_w.nr_seq_tipo_atend_princ, r_c_filtro_w.ie_origem_conta, r_c_filtro_w.ie_apresentacao, r_c_filtro_w.ie_regime_atend_princ, r_c_filtro_w.ie_regime_atendimento, r_c_filtro_w.ie_saude_ocupacional, valor_bind_w) INTO STRICT _ora2pg_r;
 ds_campos_w := _ora2pg_r.ds_campos_p; valor_bind_w := _ora2pg_r.valor_bind_p;
			...
			loop
				...
				CALL CALL CALL CALL CALL pls_filtro_regra_event_cta_pck.gerencia_selecao_filtro(nr_id_transacao_p, nr_seq_filtro_p, ie_tipo_regra_p,
							ie_excecao_p, tb_seq_conta_w, tb_seq_resumo_w,
							tb_seq_conta_proc_w, tb_seq_conta_mat_w, tb_seq_selecao_w,
							'S');
			end loop;
			close cursor_w;
		exception
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_filtro_regra_event_cta_pck.aplica_filtro_conta ( ie_considera_selecao_p INOUT boolean, qt_filtro_processado_p INOUT integer, ie_tipo_regra_p pls_pp_cta_combinada.ie_tipo_regra%type, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, ie_excecao_p pls_pp_cta_filtro.ie_excecao%type, nr_seq_filtro_p pls_pp_cta_filtro.nr_sequencia%type, dt_inicio_vigencia_p pls_pp_cta_combinada.dt_inicio_vigencia%type, dt_fim_vigencia_p pls_pp_cta_combinada.dt_fim_vigencia%type, nr_seq_conta_p pls_conta.nr_sequencia%type, nr_seq_analise_p pls_analise_conta.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
