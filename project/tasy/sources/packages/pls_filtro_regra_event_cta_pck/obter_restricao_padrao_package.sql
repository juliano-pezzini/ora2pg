...
CREATE OR REPLACE FUNCTION pls_filtro_regra_event_cta_pck.obter_restricao_padrao ( ie_tipo_regra_p pls_pp_cta_combinada.ie_tipo_regra%type, dt_inicio_vigencia_p pls_pp_cta_combinada.dt_inicio_vigencia%type, dt_fim_vigencia_p pls_pp_cta_combinada.dt_fim_vigencia%type, nr_seq_conta_p pls_conta.nr_sequencia%type, nr_seq_analise_p pls_analise_conta.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, ds_restricao_mat_p out text, ds_restricao_proc_p out text, valor_bind_p INOUT sql_pck.t_dado_bind) AS $body$
DECLARE
...
if (dt_inicio_vigencia_p IS NOT NULL AND dt_inicio_vigencia_p::text <> '') then
	...
	if (ie_tipo_regra_p = 'M') then	-- regra material
		ds_restricao_mat_w := ds_restricao_mat_w || ' and ' || ds_alias_mat_w || '.dt_atendimento_referencia between :dt_inicio_vigencia_pc and :dt_fim_vigencia_pc ' || pls_util_pck.enter_w;		
	elsif (ie_tipo_regra_p = 'P') then	-- regra procedimento
		ds_restricao_proc_w := ds_restricao_proc_w || ' and ' || ds_alias_pro_w || '.dt_procedimento_referencia between :dt_inicio_vigencia_pc and :dt_fim_vigencia_pc ' || pls_util_pck.enter_w;
	else	-- regra conta
		ds_restricao_w := ds_restricao_w || ' and ' || ds_alias_con_w || '.dt_atendimento_referencia between :dt_inicio_vigencia_pc and :dt_fim_vigencia_pc ' || pls_util_pck.enter_w;
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION pls_filtro_regra_event_cta_pck.obter_restricao_padrao ( ie_tipo_regra_p pls_pp_cta_combinada.ie_tipo_regra%type, dt_inicio_vigencia_p pls_pp_cta_combinada.dt_inicio_vigencia%type, dt_fim_vigencia_p pls_pp_cta_combinada.dt_fim_vigencia%type, nr_seq_conta_p pls_conta.nr_sequencia%type, nr_seq_analise_p pls_analise_conta.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, ds_restricao_mat_p out text, ds_restricao_proc_p out text, valor_bind_p INOUT sql_pck.t_dado_bind) FROM PUBLIC;
