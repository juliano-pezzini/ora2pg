...
if (nr_seq_filtro_p IS NOT NULL AND nr_seq_filtro_p::text <> '') then
	...
	for r_c_filtro_w in c_filtro(nr_seq_filtro_p) loop
		...
		begin
			...
			SELECT * FROM pls_filtro_regra_event_cta_pck.obter_restricao_procedimento(	r_c_filtro_w.cd_area_procedimento, r_c_filtro_w.cd_especialidade, r_c_filtro_w.cd_grupo_proc, r_c_filtro_w.ie_origem_proced, r_c_filtro_w.cd_procedimento, r_c_filtro_w.nr_seq_grupo_servico, r_c_filtro_w.ie_tipo_despesa_proc, nr_seq_filtro_p, valor_bind_w) INTO STRICT _ora2pg_r;
 ds_campos_w := _ora2pg_r.ds_campos_p; ds_tabela_w := _ora2pg_r.ds_tabela_p; valor_bind_w := _ora2pg_r.valor_bind_p;
			...
			loop
				...
				for i in tb_seq_conta_w.first..tb_seq_conta_w.last loop
					...
					ie_registro_valido_w := pls_filtro_regra_event_cta_pck.ie_reg_valido_fil_proce(r_c_filtro_w.ie_taxa_coleta,
											tb_ie_exame_coleta_w(i));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_filtro_regra_event_cta_pck.aplica_filtro_procedimento ( ie_considera_selecao_p INOUT boolean, qt_filtro_processado_p INOUT integer, ie_tipo_regra_p pls_pp_cta_combinada.ie_tipo_regra%type, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, ie_excecao_p pls_pp_cta_filtro.ie_excecao%type, nr_seq_filtro_p pls_pp_cta_filtro.nr_sequencia%type, dt_inicio_vigencia_p pls_pp_cta_combinada.dt_inicio_vigencia%type, dt_fim_vigencia_p pls_pp_cta_combinada.dt_fim_vigencia%type, nr_seq_conta_p pls_conta.nr_sequencia%type, nr_seq_analise_p pls_analise_conta.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
