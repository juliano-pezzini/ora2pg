...
CREATE OR REPLACE PROCEDURE pls_filtro_regra_event_cta_pck.aplica_filtro_prestador ( ie_considera_selecao_p INOUT boolean, qt_filtro_processado_p INOUT integer, ie_tipo_regra_p pls_pp_cta_combinada.ie_tipo_regra%type, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, ie_excecao_p pls_pp_cta_filtro.ie_excecao%type, nr_seq_filtro_p pls_pp_cta_filtro.nr_sequencia%type, dt_inicio_vigencia_p pls_pp_cta_combinada.dt_inicio_vigencia%type, dt_fim_vigencia_p pls_pp_cta_combinada.dt_fim_vigencia%type, nr_seq_conta_p pls_conta.nr_sequencia%type, nr_seq_analise_p pls_analise_conta.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
tb_seq_prest_pag_w	pls_util_cta_pck.t_number_table;
...
c_filtro CURSOR(nr_seq_filtro_pc	pls_pp_cta_filtro.nr_sequencia%type) FOR
	SELECT	a.nr_sequencia,
		...
		a.nr_seq_prestador,
		a.cd_prestador_cod
	from	pls_pp_cta_filtro_prest a
...
if (nr_seq_filtro_p IS NOT NULL AND nr_seq_filtro_p::text <> '') then
	...
	for r_c_filtro_w in c_filtro(nr_seq_filtro_p) loop
		...
		if (qt_filtro_processado_p = 0) then
			...
			CALL CALL CALL CALL CALL pls_filtro_regra_event_cta_pck.atualiza_campo_auxiliar(nr_id_transacao_p, nr_seq_filtro_p, ie_excecao_p);
		end if;
		
		begin
			-- restricao para todas as outras situacoes

			SELECT * FROM pls_filtro_regra_event_cta_pck.obter_restricao_prestador(	r_c_filtro_w.nr_seq_prestador, r_c_filtro_w.cd_prestador_cod, valor_bind_w) INTO STRICT _ora2pg_r;
 ds_campos_w := _ora2pg_r.ds_campos_p; valor_bind_w := _ora2pg_r.valor_bind_p;
			
			-- montar o select padrao juntamente as restricoes.

			valor_bind_w := pls_filtro_regra_event_cta_pck.obter_select_filtro(	ie_considera_selecao_p, ie_tipo_regra_p, nr_id_transacao_p, ie_excecao_p, nr_seq_filtro_p, dt_inicio_vigencia_p, dt_fim_vigencia_p, nr_seq_conta_p, nr_seq_analise_p, cd_estabelecimento_p, ds_campos_w, ds_tabela_w, dados_restricao_w, valor_bind_w);
			...
			loop
				fetch cursor_w bulk collect into tb_seq_conta_w, tb_seq_resumo_w, tb_seq_conta_proc_w,
								 ...
								 tb_seq_protocolo_w, tb_seq_prest_pag_w
				...
				for i in tb_seq_conta_w.first..tb_seq_conta_w.last loop
					...
					ie_registro_valido_w := pls_filtro_regra_event_cta_pck.ie_reg_valido_fil_prest(	r_c_filtro_w.nr_seq_tipo_prestador,
												r_c_filtro_w.nr_seq_tipo_prest_prot,
												tb_dt_ref_w(i),
												tb_seq_prest_pag_w(i),
												tb_seq_protocolo_w(i));
					...
					if (ie_registro_valido_w) then
						...
						tb_seq_resumo_ger_w(qt_reg_w) := tb_seq_resumo_w(i);
				...
				CALL CALL CALL CALL CALL pls_filtro_regra_event_cta_pck.gerencia_selecao_filtro(nr_id_transacao_p, nr_seq_filtro_p, ie_tipo_regra_p,
							ie_excecao_p, tb_seq_conta_ger_w, tb_seq_resumo_ger_w,
							tb_seq_conta_proc_ger_w, tb_seq_conta_mat_ger_w, tb_seq_selecao_ger_w,
							'S');
			end loop;
			close cursor_w;
		exception
			...
			CALL CALL CALL CALL CALL CALL pls_filtro_regra_event_cta_pck.trata_erro_sql_dinamico(null, ds_select_w, null,
						nm_usuario_p, 'S');
		end;
	...
	if (qt_filtro_processado_p > 0) then
		...
		CALL CALL CALL CALL CALL pls_filtro_regra_event_cta_pck.atualiza_campo_valido(nr_id_transacao_p, nr_seq_filtro_p, ie_excecao_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_filtro_regra_event_cta_pck.aplica_filtro_prestador ( ie_considera_selecao_p INOUT boolean, qt_filtro_processado_p INOUT integer, ie_tipo_regra_p pls_pp_cta_combinada.ie_tipo_regra%type, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, ie_excecao_p pls_pp_cta_filtro.ie_excecao%type, nr_seq_filtro_p pls_pp_cta_filtro.nr_sequencia%type, dt_inicio_vigencia_p pls_pp_cta_combinada.dt_inicio_vigencia%type, dt_fim_vigencia_p pls_pp_cta_combinada.dt_fim_vigencia%type, nr_seq_conta_p pls_conta.nr_sequencia%type, nr_seq_analise_p pls_analise_conta.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
