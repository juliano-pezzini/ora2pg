...
CREATE OR REPLACE PROCEDURE pls_filtro_regra_event_cta_pck.gerencia_selecao_filtro ( nr_id_transacao_p pls_pp_cta_selecao.nr_id_transacao%type, nr_seq_filtro_p pls_pp_cta_filtro.nr_sequencia%type, ie_tipo_regra_p pls_pp_cta_combinada.ie_tipo_regra%type, ie_excecao_p pls_pp_cta_filtro.ie_excecao%type, tb_seq_conta_p pls_util_cta_pck.t_number_table, tb_seq_resumo_p pls_util_cta_pck.t_number_table, tb_seq_conta_proc_p pls_util_cta_pck.t_number_table, tb_seq_conta_mat_p pls_util_cta_pck.t_number_table, tb_seq_selecao_p pls_util_cta_pck.t_number_table, ie_valido_p pls_pp_cta_selecao.ie_valido%type) AS $body$
DECLARE
...
if (nr_id_transacao_p IS NOT NULL AND nr_id_transacao_p::text <> '') and (tb_seq_conta_p.count > 0) then
	...
	if (ie_excecao_p = 'N') then
		...
		CALL CALL CALL CALL CALL pls_filtro_regra_event_cta_pck.atualizar_lista_filtro(tb_atualizar_seqs_w, ie_valido_p);
		
		-- se tiver algum registro para gravar entao manda ele pro banco.

		CALL CALL CALL CALL CALL pls_filtro_regra_event_cta_pck.gravar_lista_filtro(	nr_id_transacao_p, nr_seq_filtro_p, ie_tipo_regra_p, tb_gravar_conta_w,
					tb_gravar_resumo_w, tb_gravar_proc_w, tb_gravar_mat_w, ie_valido_p);
	...
	-- caso for uma regra de excecao entao atualiza o registro da selecao apenas, EXCEcaO NaO INSERE NINGUeM.
		...
		CALL CALL CALL CALL CALL pls_filtro_regra_event_cta_pck.atualizar_lista_filtro(tb_seq_selecao_p, ie_valido_p);
	end if;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_filtro_regra_event_cta_pck.gerencia_selecao_filtro ( nr_id_transacao_p pls_pp_cta_selecao.nr_id_transacao%type, nr_seq_filtro_p pls_pp_cta_filtro.nr_sequencia%type, ie_tipo_regra_p pls_pp_cta_combinada.ie_tipo_regra%type, ie_excecao_p pls_pp_cta_filtro.ie_excecao%type, tb_seq_conta_p pls_util_cta_pck.t_number_table, tb_seq_resumo_p pls_util_cta_pck.t_number_table, tb_seq_conta_proc_p pls_util_cta_pck.t_number_table, tb_seq_conta_mat_p pls_util_cta_pck.t_number_table, tb_seq_selecao_p pls_util_cta_pck.t_number_table, ie_valido_p pls_pp_cta_selecao.ie_valido%type) FROM PUBLIC;
