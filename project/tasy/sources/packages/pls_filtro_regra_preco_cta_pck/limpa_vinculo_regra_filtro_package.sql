...
if (coalesce(nr_seq_conta_mat_p::text, '') = '') then
	...
	-- so vai atras de participante caso NAO tenha sido informado uma conta de material

	tb_tipo_item_limpa_w(nr_indice_w) := 'PP';
	nr_indice_w := nr_indice_w + 1;
end if;
...
if (tb_tipo_item_limpa_w.count > 0) then
	...
	for i in tb_tipo_item_limpa_w.first .. tb_tipo_item_limpa_w.last loop
		...
		begin
			valor_bind_w := pls_filtro_regra_preco_cta_pck.obter_select_filtro(	false, 'S', ie_destino_regra_p, tb_tipo_item_limpa_w(i), null, null, null, null, null, nr_seq_lote_conta_p, nr_seq_protocolo_p, nr_seq_lote_processo_p, nr_seq_conta_p, nr_seq_conta_proc_p, nr_seq_conta_mat_p, nr_seq_analise_p, cd_estabelecimento_p, null, null, null, valor_bind_w);
			...
			loop
				...
				SELECT * FROM pls_filtro_regra_preco_cta_pck.atualiza_regra_filtro(	ie_destino_regra_p, tb_tipo_item_limpa_w(i), tb_seq_conta_w, tb_seq_conta_proc_w, tb_seq_conta_mat_w, tb_nr_seq_partic_w, tb_seq_selecao_w) INTO STRICT _ora2pg_r;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_filtro_regra_preco_cta_pck.limpa_vinculo_regra_filtro ( ie_destino_regra_p text, nr_seq_lote_conta_p pls_protocolo_conta.nr_seq_lote%type, nr_seq_protocolo_p pls_protocolo_conta.nr_sequencia%type, nr_seq_lote_processo_p pls_cta_lote_processo.nr_sequencia%type, nr_seq_conta_p pls_conta.nr_sequencia%type, nr_seq_conta_proc_p pls_conta_proc.nr_sequencia%type, nr_seq_conta_mat_p pls_conta_mat.nr_sequencia%type, nr_seq_analise_p pls_analise_conta.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
