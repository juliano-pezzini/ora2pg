...
tb_cd_guia_ref_w	pls_util_cta_pck.t_varchar2_table_20;
tb_nr_seq_material_w	pls_util_cta_pck.t_number_table;
tb_ie_restrige_hosp_w	pls_util_cta_pck.t_varchar2_table_10;
...
if (nr_seq_filtro_p IS NOT NULL AND nr_seq_filtro_p::text <> '') then
	...
	for r_c_filtro_w in c_filtro(nr_seq_filtro_p) loop
		...
		begin
			...
			SELECT * FROM pls_filtro_regra_preco_cta_pck.obter_restricao_material(	r_c_filtro_w.nr_seq_material, r_c_filtro_w.nr_seq_estrut_mat, r_c_filtro_w.nr_seq_grupo_material, r_c_filtro_w.ie_tipo_despesa, r_c_filtro_w.nr_seq_tipo_uso, r_c_filtro_w.ie_generico_unimed, r_c_filtro_w.ie_mat_autorizacao_esp, r_c_filtro_w.ie_restringe_hosp, valor_bind_w) INTO STRICT _ora2pg_r;
 ds_campos_w := _ora2pg_r.ds_campos_p; valor_bind_w := _ora2pg_r.valor_bind_p;
			...
			loop
				fetch cursor_w bulk collect into	tb_seq_conta_w, tb_seq_conta_proc_w,
									...
									tb_seq_selecao_w, tb_nr_seq_segurado_w,
									tb_nr_seq_guia_w, tb_cd_guia_ref_w,
									tb_nr_seq_material_w, tb_ie_restrige_hosp_w
				...
				for i in tb_seq_conta_w.first..tb_seq_conta_w.last loop
					...
					-- faz a verificacao dos campos do select, aqui e tratado tudo que NAO vai na clausula where

					-- do sql dinamico por questaes de performance

					ie_valido_w := pls_filtro_regra_preco_cta_pck.ie_reg_valido_fil_mat(	r_c_filtro_w.ie_mat_autorizacao_esp,
										tb_nr_seq_segurado_w(i),
										tb_nr_seq_guia_w(i),
										tb_cd_guia_ref_w(i),
										tb_nr_seq_material_w(i),
										tb_ie_restrige_hosp_w(i));
					
					-- se retornou como valido e alimentado em outras variaveis para ser enviadas ao banco
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_filtro_regra_preco_cta_pck.aplica_filtro_material ( ie_considera_selecao_p INOUT boolean, ie_tipo_regra_p pls_cp_cta_combinada.ie_tipo_regra%type, ie_destino_regra_p text, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, ie_excecao_p pls_cp_cta_filtro.ie_excecao%type, nr_seq_filtro_p pls_cp_cta_filtro.nr_sequencia%type, dt_inicio_vigencia_p pls_cp_cta_combinada.dt_inicio_vigencia%type, dt_fim_vigencia_p pls_cp_cta_combinada.dt_fim_vigencia%type, nr_seq_lote_conta_p pls_protocolo_conta.nr_seq_lote%type, nr_seq_protocolo_p pls_protocolo_conta.nr_sequencia%type, nr_seq_lote_processo_p pls_cta_lote_processo.nr_sequencia%type, nr_seq_conta_p pls_conta.nr_sequencia%type, nr_seq_conta_proc_p pls_conta_proc.nr_sequencia%type, nr_seq_conta_mat_p pls_conta_mat.nr_sequencia%type, nr_seq_analise_p pls_analise_conta.nr_sequencia%type, cd_acao_analise_p pls_acao_analise.cd_acao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
