...
qt_reg_w 		integer;
ie_valido_w		boolean;
tb_ie_cart_valida_w	pls_util_cta_pck.t_varchar2_table_10;
tb_ie_grupo_valido_w	pls_util_cta_pck.t_varchar2_table_10;
...
c_filtro CURSOR(	nr_seq_filtro_pc	pls_cp_cta_filtro.nr_sequencia%type) FOR
	SELECT	a.nr_sequencia,
		...
		a.qt_idade_final,
		a.ie_pcmso
	from	pls_cp_cta_filtro_benef a
...
if (nr_seq_filtro_p IS NOT NULL AND nr_seq_filtro_p::text <> '') then
	...
	for r_c_filtro_w in c_filtro(nr_seq_filtro_p) loop
		...
		begin
			...
			SELECT * FROM pls_filtro_regra_preco_cta_pck.obter_restricao_beneficiario(	r_c_filtro_w.ie_tipo_segurado, r_c_filtro_w.nr_seq_regra_atend_cart, r_c_filtro_w.cd_convenio, r_c_filtro_w.cd_categoria, r_c_filtro_w.nr_seq_grupo_operadora, r_c_filtro_w.nr_seq_congenere, r_c_filtro_w.qt_idade_inicial, r_c_filtro_w.qt_idade_final, r_c_filtro_w.ie_pcmso, valor_bind_w) INTO STRICT _ora2pg_r;
 ds_campos_w := _ora2pg_r.ds_campos_p; ds_tabela_w := _ora2pg_r.ds_tabela_p; valor_bind_w := _ora2pg_r.valor_bind_p;
			...
			valor_bind_w := pls_filtro_regra_preco_cta_pck.obter_select_filtro(	ie_considera_selecao_p, 'N', ie_destino_regra_p, ie_tipo_regra_p, nr_id_transacao_p, ie_excecao_p, nr_seq_filtro_p, dt_inicio_vigencia_p, dt_fim_vigencia_p, nr_seq_lote_conta_p, nr_seq_protocolo_p, nr_seq_lote_processo_p, nr_seq_conta_p, nr_seq_conta_proc_p, nr_seq_conta_mat_p, nr_seq_analise_p, cd_estabelecimento_p, ds_campos_w, ds_tabela_w, dados_restricao_w, valor_bind_w);
			...
			loop
				fetch cursor_w bulk collect into	tb_seq_conta_w, tb_seq_conta_proc_w,
									...
									tb_seq_selecao_w, tb_ie_cart_valida_w,
									tb_cd_convenio_w, tb_cd_categoria_w, 
									tb_ie_grupo_valido_w, tb_qt_idade_w
				...
				for i in tb_seq_conta_w.first..tb_seq_conta_w.last loop
					...
					ie_valido_w := pls_filtro_regra_preco_cta_pck.ie_reg_valido_fil_benef(	tb_ie_cart_valida_w(i),
										tb_cd_convenio_w(i),
										tb_cd_categoria_w(i), 
										tb_ie_grupo_valido_w(i), 
										tb_qt_idade_w(i),
										r_c_filtro_w.qt_idade_inicial,
										r_c_filtro_w.qt_idade_final,
										r_c_filtro_w.cd_convenio,
										r_c_filtro_w.cd_categoria);
					
					-- caso NAO tenha os campos na regra retorna S e se tem o campo e for valido retorna S

					-- todo o resto NAO e valido (null ou N)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_filtro_regra_preco_cta_pck.aplica_filtro_beneficiario ( ie_considera_selecao_p INOUT boolean, ie_tipo_regra_p pls_cp_cta_combinada.ie_tipo_regra%type, ie_destino_regra_p text, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, ie_excecao_p pls_cp_cta_filtro.ie_excecao%type, nr_seq_filtro_p pls_cp_cta_filtro.nr_sequencia%type, dt_inicio_vigencia_p pls_cp_cta_combinada.dt_inicio_vigencia%type, dt_fim_vigencia_p pls_cp_cta_combinada.dt_fim_vigencia%type, nr_seq_lote_conta_p pls_protocolo_conta.nr_seq_lote%type, nr_seq_protocolo_p pls_protocolo_conta.nr_sequencia%type, nr_seq_lote_processo_p pls_cta_lote_processo.nr_sequencia%type, nr_seq_conta_p pls_conta.nr_sequencia%type, nr_seq_conta_proc_p pls_conta_proc.nr_sequencia%type, nr_seq_conta_mat_p pls_conta_mat.nr_sequencia%type, nr_seq_analise_p pls_analise_conta.nr_sequencia%type, cd_acao_analise_p pls_acao_analise.cd_acao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
