...
tb_ie_carater_intern_w	pls_util_cta_pck.t_varchar2_table_10;
tb_cd_versao_tiss_w	pls_util_cta_pck.t_varchar2_table_20;
tb_ie_internado_w	pls_util_cta_pck.t_varchar2_table_10;
...
tb_qt_dias_inter_w	pls_util_cta_pck.t_number_table;
tb_ie_ref_guia_inter_w	pls_util_cta_pck.t_varchar2_table_10;
qt_reg_w		integer;
...
c_filtro CURSOR(	nr_seq_filtro_pc	pls_cp_cta_filtro.nr_sequencia%type) FOR
	...
	from	pls_cp_cta_filtro_conta a
...
if (nr_seq_filtro_p IS NOT NULL AND nr_seq_filtro_p::text <> '') then
	...
	for r_c_filtro_w in c_filtro(nr_seq_filtro_p) loop
		...
		begin
			...
			SELECT * FROM pls_filtro_regra_preco_cta_pck.obter_restricao_conta(	r_c_filtro_w.ie_atend_pcmso, r_c_filtro_w.ie_carater_internacao, r_c_filtro_w.ie_internado, r_c_filtro_w.ie_origem_conta, r_c_filtro_w.ie_ref_guia_internacao, r_c_filtro_w.ie_regime_internacao, r_c_filtro_w.ie_tipo_consulta, r_c_filtro_w.ie_tipo_guia, r_c_filtro_w.nr_seq_categoria, r_c_filtro_w.nr_seq_cbo_saude, r_c_filtro_w.nr_seq_clinica, r_c_filtro_w.nr_seq_grupo_doenca, r_c_filtro_w.nr_seq_tipo_acomodacao, r_c_filtro_w.nr_seq_tipo_atendimento, r_c_filtro_w.qt_dias_inter_final, r_c_filtro_w.qt_dias_inter_inicio, r_c_filtro_w.nr_seq_tipo_atend_princ, r_c_filtro_w.ie_acomodacao_autorizada, r_c_filtro_w.ie_regime_atendimento, r_c_filtro_w.ie_regime_atendimento_princ, r_c_filtro_w.ie_saude_ocupacional, valor_bind_w) INTO STRICT _ora2pg_r;
 valor_bind_w := _ora2pg_r.valor_bind_p; ds_campos_w := _ora2pg_r.ds_campos_p;
			...
			loop
				fetch cursor_w bulk collect into	tb_seq_conta_w, tb_seq_conta_proc_w,
									...
									tb_seq_selecao_w, tb_ie_carater_intern_w,
									tb_cd_versao_tiss_w, tb_ie_internado_w,
									tb_ie_ref_guia_inter_w, tb_qt_dias_inter_w
				...
				for i in tb_seq_conta_w.first..tb_seq_conta_w.last loop
					...
					-- verifica se o registro e valido, feito isso para NAO utilizar function em clausula where

					-- do cursor

					ie_registro_valido_w := pls_filtro_regra_preco_cta_pck.ie_reg_valido_fil_conta(	r_c_filtro_w.ie_internado,
												r_c_filtro_w.ie_carater_internacao,
												r_c_filtro_w.qt_dias_inter_final,
												r_c_filtro_w.qt_dias_inter_inicio,
												r_c_filtro_w.ie_ref_guia_internacao,
												tb_ie_internado_w(i),
												tb_ie_carater_intern_w(i),
												tb_cd_versao_tiss_w(i),
												tb_qt_dias_inter_w(i),
												tb_ie_ref_guia_inter_w(i));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_filtro_regra_preco_cta_pck.aplica_filtro_conta ( ie_considera_selecao_p INOUT boolean, ie_tipo_regra_p pls_cp_cta_combinada.ie_tipo_regra%type, ie_destino_regra_p text, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, ie_excecao_p pls_cp_cta_filtro.ie_excecao%type, nr_seq_filtro_p pls_cp_cta_filtro.nr_sequencia%type, dt_inicio_vigencia_p pls_cp_cta_combinada.dt_inicio_vigencia%type, dt_fim_vigencia_p pls_cp_cta_combinada.dt_fim_vigencia%type, nr_seq_lote_conta_p pls_protocolo_conta.nr_seq_lote%type, nr_seq_protocolo_p pls_protocolo_conta.nr_sequencia%type, nr_seq_lote_processo_p pls_cta_lote_processo.nr_sequencia%type, nr_seq_conta_p pls_conta.nr_sequencia%type, nr_seq_conta_proc_p pls_conta_proc.nr_sequencia%type, nr_seq_conta_mat_p pls_conta_mat.nr_sequencia%type, nr_seq_analise_p pls_analise_conta.nr_sequencia%type, cd_acao_analise_p pls_acao_analise.cd_acao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
