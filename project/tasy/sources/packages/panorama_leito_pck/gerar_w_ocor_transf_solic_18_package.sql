...
SET client_encoding TO 'UTF8';
	...
	----------------------------- 18 ---------------------------------
	XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*/
CREATE OR REPLACE PROCEDURE panorama_leito_pck.gerar_w_ocor_transf_solic_18 ( nr_atendimento_p bigint, cd_pessoa_fisica_p text, pIeConcluido text) AS $body$
DECLARE
...
BEGIN
	...
	if (dt_solicitacao_w IS NOT NULL AND dt_solicitacao_w::text <> '') then
		...
		ds_alerta_w  := substr(dt_solicit_w||' '|| 'D_1408'||'S_' || ds_status_w ||'<br>'||dt_alerta_w|| ' ',1,4000);
	...
	if (ds_alerta_w IS NOT NULL AND ds_alerta_w::text <> '') then
		CALL panorama_leito_pck.insert_ocorrencia(nr_atendimento_p,
						18,
						'assets/icons/status-and-feedback/arrow-right.svg',
						'assets/icons/status-and-feedback/arrow-right.svg',
						654196, /* Transferencia solicitada */
						967418,
						null,
						null,
						ds_alerta_w,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE panorama_leito_pck.gerar_w_ocor_transf_solic_18 ( nr_atendimento_p bigint, cd_pessoa_fisica_p text, pIeConcluido text) FROM PUBLIC;
