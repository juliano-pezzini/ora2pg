...
CREATE OR REPLACE PROCEDURE tosho_pck.update_tasy_error_log (file_name_p text, file_like_p text, nr_seq_event_p text) AS $body$
DECLARE

       nr_seq_int_call_log_w integration_call_log.nr_sequencia%type;
       current_setting('tosho_pck.nm_interface_w')::varchar(255)	integration_call_log.nm_interface%type;
...
BEGIN
    if (file_name_p like file_like_p||'%')then
        ...
        into STRICT   nr_seq_int_call_log_w,
               nm_message_w,
               current_setting('tosho_pck.nm_interface_w')::varchar(255),
               ie_message_type_w,
        ...
        message_data_w := tosho_pck.read_file(file_name_p, file_like_p, nr_seq_event_p, message_data_w);

        ds_error_w := 'Error occured in file' || file_name_p ||' , Time of Error' || clock_timestamp();

        record_integration_call_log('tosho_tasy', 'tosho_tasy', clock_timestamp()	, current_setting('tosho_pck.nm_interface_w')::varchar(255), nm_message_w,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE tosho_pck.update_tasy_error_log (file_name_p text, file_like_p text, nr_seq_event_p text) FROM PUBLIC;
