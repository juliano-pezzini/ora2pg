...
CREATE OR REPLACE PROCEDURE tosho_pck.read_file (ds_file_p text, ds_file_search_p text,cd_data_event_p text ,ds_messsage INOUT text) AS $body$
DECLARE
...
BEGIN
        ds_file_line_w := null;
        CALL pls_utl_file_pck.nova_leitura(cd_data_event_p, ds_file_p); --read the file
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE tosho_pck.read_file (ds_file_p text, ds_file_search_p text,cd_data_event_p text ,ds_messsage INOUT text) FROM PUBLIC;
