...
CREATE OR REPLACE PROCEDURE tws_user_health_prof_pck.generate_user_tasy_tws ( nm_usuario_tasy_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, cd_pessoa_fisica_p pessoa_fisica.cd_pessoa_fisica%type, nr_seq_perfil_wsuite_p wsuite_usuario.nr_seq_perfil_wsuite%type) AS $body$
DECLARE
...
C01 CURSOR FOR
 	SELECT	a.id id_application,
		...
		(select c.id from client c where  c.nm_client = 'healthprofessional' and c.id_application = a.id) id_client,
		(select r.id from role r where  r.nm_role = 'healthprofessional' and r.id_application = a.id) id_role
	from	application a
...
if (id_subject_w IS NOT NULL AND id_subject_w::text <> '' AND cd_pessoa_fisica_p IS NOT NULL AND cd_pessoa_fisica_p::text <> '') then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE tws_user_health_prof_pck.generate_user_tasy_tws ( nm_usuario_tasy_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, cd_pessoa_fisica_p pessoa_fisica.cd_pessoa_fisica%type, nr_seq_perfil_wsuite_p wsuite_usuario.nr_seq_perfil_wsuite%type) FROM PUBLIC;
