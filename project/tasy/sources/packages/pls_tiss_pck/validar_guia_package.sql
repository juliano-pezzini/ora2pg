...
CREATE OR REPLACE PROCEDURE pls_tiss_pck.validar_guia (nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nr_seq_guia_plano_imp_p pls_guia_plano_imp.nr_sequencia%type, nr_seq_guia_plano_p INOUT pls_guia_plano.nr_sequencia%type) AS $body$
DECLARE

    /* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    Finalidade: Consistir dados referente a guia, estas consistencias sao informacoes que
    sao obrigatorias na Guia e se nao existir ira bloquear o recebimento da mesmo
    -------------------------------------------------------------------------------------------------------------------
    ...
    C01 CURSOR(nr_seq_guia_plano_imp_pc pls_guia_plano_imp.nr_sequencia%type) FOR
      ...
      SELECT nr_sequencia, cd_tabela, cd_procedimento, ds_procedimento
...
BEGIN
    ...
    select cd_usuario_plano,
           pls_obter_dados_medico_imp(nr_conselho_prof_solic,
                                      pls_obter_sg_uf_tiss(cd_uf_conselho_prof_solic),
                                      cd_conselho_prof_solic,
                                      'CP',
                                      'G') cd_medico_solic,
           pls_obter_prestador_imp(cd_cgc_prest_solic,
                                   cd_cpf_prest_solic,
                                   cd_prestador_solic,
                                   '',
                                   '',
                                   '',
                                   'G') nr_seq_prest_solic,
           pls_obter_prestador_imp('',
                                   '',
                                   cd_prestador_exec,
                                   '',
                                   '',
    ...
    --OS 1680444 - Retirado tratativa devido a criacao de nova ocorrencia combinada.
    --Valida se a versao do TISS enviada no arquivo e a mesma utilizada pela operadora
    /*if  ( cd_versao_w <> pls_obter_versao_tiss ) then
      pls_gravar_glosa_guia_imp('5028', ' Versao TISS enviada o arquivo ' || cd_versao_w|| ' Guia nao foi gerada', nr_seq_guia_plano_imp_p, null, null, nm_usuario_p, cd_estabelecimento_p);
    end if;*/
  
    --Valida se existe beneficiario na base, se nao existir ja gera a glosa e bloqueia a importacao
    if (coalesce(nr_seq_segurado_w::text, '') = '') then
      ...
      if (ie_consultar_benef_w = 'S') then
        ...
        cd_unimed_benef_w := substr(lpad(cd_usuario_plano_imp_w, 17, '0'),
                                    1,
                                    4);

        if ((lpad(cd_unimed_exec_w, 4, '0'))::numeric  <>
           (lpad(cd_unimed_benef_w, 4, '0'))::numeric ) then
      ...
      if (coalesce(nr_seq_segurado_w::text, '') = '') then
        CALL pls_gravar_glosa_guia_imp('1001',
                                  'Carteira importada: ' ||
    ...
    select count(1)
      ...
      from pls_segurado b, pls_segurado_carteira a
     where a.nr_seq_segurado = b.nr_sequencia
       and a.nr_seq_segurado = nr_seq_segurado_w
       and b.ie_tipo_segurado <> 'P'
       and (coalesce(a.dt_validade_carteira::text, '') = '' or
           a.dt_validade_carteira >= clock_timestamp())
       and a.dt_inicio_vigencia <= clock_timestamp();

    --Verificar a data de validade da carteira do beneficiario
    if (qt_benef_valido_w = 0) then
      CALL pls_gravar_glosa_guia_imp('1017',
                                'Carteira importada: ' ||
                                cd_usuario_plano_imp_w ||
                                ' Guia nao foi gerada',
                                nr_seq_guia_plano_imp_p,
    ...
    --Verificar se o prestador existe na base se nao existir ja gera a glosa e bloqueia a importacao
    if (coalesce(nr_seq_prestador_solic_w::text, '') = '' and
       coalesce(nr_seq_prestador_exec_w::text, '') = '') then
    
      CALL pls_gravar_glosa_guia_imp('1203',
                                'Cod prestador: ' || cd_cgc_prest_solic_w ||
                                cd_cpf_prest_solic_w ||
                                cd_prestador_solic_w ||
                                ' Guia nao foi gerada',
                                nr_seq_guia_plano_imp_p,
    ...
    --Valida o CBO do medico se o mesmo foi informado e se existe na Base
    if (coalesce(nr_seq_cbo_saude_w::text, '') = '') then
      CALL pls_gravar_glosa_guia_imp('1213',
                                'CBO nao informado ou invalido. Medico: ' ||
                                nm_profissional_solic_w || ' CBO: ' ||
                                cd_cbo_saude_w || ' Guia nao foi gerada',
                                nr_seq_guia_plano_imp_p,
                                null,
                                null,
                                nm_usuario_p,
    ...
    if (ie_tipo_prest_w = 'PJ') then
      select count(1)
       ...
       where nr_seq_prestador = nr_seq_prestador_solic_w
         ...
         and ie_situacao = 'A'
         and trunc(coalesce(dt_solicitacao_w, clock_timestamp()), 'dd') between
             trunc(coalesce(dt_inclusao, coalesce(dt_solicitacao_w, clock_timestamp())), 'dd') and
             fim_dia(coalesce(dt_exclusao, coalesce(dt_solicitacao_w, clock_timestamp())));
    elsif (ie_tipo_prest_w = 'PF') then
      qt_medico_w := 1;
    end if;

    --Verifica se o medico esta vinculado ao prestador
    if (qt_medico_w = 0) then
      ie_regra_integracao_aut_w := pls_obter_se_regra_integ_aut(nr_seq_prestador_solic_w,
                                                                '1210',
                                                                nm_usuario_p);
      if (ie_regra_integracao_aut_w = 'N') then
        CALL pls_gravar_glosa_guia_imp('1210',
                                  'Cod prestador: ' ||
                                  nr_seq_prestador_solic_w || ' / Solic.: ' ||
                                  nm_profissional_solic_w ||
                                  ' Guia nao foi gerada',
                                  nr_seq_guia_plano_imp_p,
                                  null,
    ...
    for C01_w in C01(nr_seq_guia_plano_imp_p) loop
      begin
        --Retorna o codigo do procedimento e a origem ja convertidos
        pls_gerar_dados_guia_proc_imp(C01_w.cd_procedimento,
                                      C01_w.cd_tabela,
                                      null,
                                      cd_estabelecimento_p,
                                      cd_procedimento_w,
        ...
        if (qt_procedimento_w = 0) then
          CALL pls_gravar_glosa_guia_imp('1801',
                                    ' Cod procedimento: ' ||
    ...
    for C02_w in C02(nr_seq_guia_plano_imp_p) loop
      begin
        if (coalesce(C02_w.nr_seq_material::text, '') = '') then
          CALL pls_gravar_glosa_guia_imp('2001',
                                    ' Cod material: ' || C02_w.cd_material ||
                                    ' - ' || C02_w.ds_material ||
                                    ' Guia nao foi gerada',
                                    nr_seq_guia_plano_imp_p,
                                    null,
    ...
    --Se os dados da importacao nao geraram glosa o sistema gera a guia e aplica as consistencias padrao
    if (qt_glosa_w = 0) then
    
      --Obter qual o processo utilizado pela Operadora  1 - Autorizacao ou 2 - Requisicao
      --Por padrao o valor e 1 - Autorizacao
      ie_funcao_w := pls_obter_param_atend_geral('FA');

      if (ie_funcao_w = '1') then
        pls_exec_processo_guia_imp(nm_usuario_p,
                                   cd_estabelecimento_p,
                                   nr_seq_guia_plano_imp_p,
                                   nr_seq_guia_plano_w);
      elsif (ie_funcao_w = '2') then
        pls_exec_processo_req_imp(nm_usuario_p,
                                  cd_estabelecimento_p,
                                  nr_seq_guia_plano_imp_p,
                                  nr_seq_guia_plano_w);
      end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_tiss_pck.validar_guia (nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nr_seq_guia_plano_imp_p pls_guia_plano_imp.nr_sequencia%type, nr_seq_guia_plano_p INOUT pls_guia_plano.nr_sequencia%type) FROM PUBLIC;
