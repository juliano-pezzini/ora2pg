...
SET client_encoding TO 'UTF8';
    ...
    /**
	+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	PROCEDURE JPN_INJECTION_ORDER_INFO
	INJECTION SUPPORT SYSTEM
	+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    **/
CREATE OR REPLACE PROCEDURE tosho_presc_injection_pck.jpn_injection_order_info (NR_PRESCRICAO_P bigint) AS $body$
DECLARE
			...
			C01 CURSOR FOR  SELECT '12' AS MESSAGE_CLASSIFICATION,
								  ...
								  TO_CHAR(DT_ATUALIZACAO_W,'YYYYMMDD') AS SYORI_DATE1,
								  '      ' AS SEQ_NO1,
								  '    ' AS WS_NO,
								  ' ' AS INDEX_KBN,
								  '  ' AS XX_KBN,
								  ...
								  '                    ' AS SH_KANA_NAME, --NOT USED
								  SUBSTR(OBTER_NOME_MEDICO(AP1.CD_MEDICO_RESP, 'N'),1,20) AS SH_NAME,
								  TO_CHAR(DT_ATUALIZACAO_W, 'YYYYMMDD') AS SYORI_DATE2,
								  TO_CHAR(DT_ATUALIZACAO_W, 'HH24MISS') AS SYORI_TIME,
								  TO_CHAR(CR1.DT_FIM, 'YYYYMMDD') AS STOP_DATE1,
								  '1' AS STOP_RSN_FLG, --FIXED
								  ...
								  ' ' AS EOF_KBN,  --(CASO SEJA A ULTIMA LINHA, INSERIR E)
								  '00000' AS RECORD_LEN,
								  '   ' AS RECORD_NUM,
								  TO_CHAR(clock_timestamp(), 'YYYYMMDD') || TO_CHAR(clock_timestamp(), 'HH24MISS') AS TIMESTAMP1,
								  ' ' AS PICKING, --(FIXED)
								  SEND_KBN_W AS SEND_KBN,	
								  ' ' AS OUTPUT_MODE, --VERIFICAR THIAGO   
								  '0000000000' AS RESERVE,--(FIXED)
								  ...
								  --NAO EXISTEM TODOS OS SI_TIPE_OF_PRESCRIPTION NA TABELA VALOR_DOMINIO VERIFICAR NA BASE DO JAPAO
							      ORDER_NO_W  AS ORDER_NO,
			  			   	      --NAO EXISTEM TODOS OS SI_TIPE_OF_PRESCRIPTION NA TABELA VALOR_DOMINIO VERIFICAR NA BASE DO JAPAO
								  ...
								  OBTER_INICIAIS_SETOR(OBTER_DESC_SETOR_ATEND(tosho_presc_injection_pck.obter_set_atend_unidade(AP1.NR_ATENDIMENTO, 'A'))) AS KA_NAME_R,
								  ...
								  TO_CHAR(CR1.DT_INICIO, 'YYYYMMDD') AS IN_DATE1,
								  TO_CHAR(CR1.DT_INICIO, 'HH24MISS') AS IN_TIME1,
								  TO_CHAR(CM1.DT_INICIO, 'YYYYMMDD') AS START_DATE1,
								  TO_CHAR(CM1.DT_FIM, 'YYYYMMDD') AS END_DATE1,
								  '        ' AS STOP_DATE2, --NOT USED
								  ' ' AS STOP_COMMENT1, --NOT USED
								  '1' AS IVH_MUKIN, --VALIDAR COM O THIAGO
								  ...
								  ' ' AS CHG_KBN, --NOT USED
								  LPAD(CR1.NR_SEQUENCIA_RP, 3, '0') AS RP_NUM1,
								  LPAD(CR1.NR_SEQUENCIA, 5, '0') AS RP_SEQ1,
								  '   ' AS SET_CD1, --NOT USED
								  '   ' AS SET_CD_2, --NOT USED
								  '   ' AS SET_KBN, --FIXED SPACE
								  '                                        ' AS SET_NAME, --FIXED SPACE
								  TO_CHAR(CR1.DT_INICIO, 'YYYYMMDD') AS DAY_XX,
								  '          ' AS BASE_DAY, --PENDENTE DO CAMPO
								  ' ' AS STATUS1, --NOT USED SPACE
								  ' ' AS BUMON_FLG1, --FIXO
								  ' ' AS MIX_SIJI, --FIXO
								  ' ' AS MIX, --FIXO
								  ' ' AS IVH_SIJI, --FIXO
								  ...
								  CM1.IE_DRIP_SHOT AS SYUGI_KBN, 
								  CM1.IE_VIA_APLICACAO AS SYUGI_CD,
								  (SELECT DS_VIA_APLICACAO
								     FROM VIA_APLICACAO
								    WHERE IE_VIA_APLICACAO = CM1.IE_VIA_APLICACAO) AS SYUGI_NAME, 
								  (SELECT DS_VIA_APLICACAO
								     FROM VIA_APLICACAO
								    WHERE IE_VIA_APLICACAO = CM1.IE_VIA_APLICACAO) AS ROOT_NAME, 
								  ' ' AS YOHO_DSP_NAME, --PENDENTE CAMPO 
								  ...
								  MA1.NR_CODIGO_BARRAS AS BCD_NO,
								  TO_CHAR(CM1.DT_FIM, 'YYYYMMDDHH24MISS') AS TON_KIGEN,
								  ...
								  LPAD(CR1.NR_SEQUENCIA_RP, 3, '0') AS RP_NUM2,
								  ...
								  ' ' AS HARAIDASI_FLG,
								  '    ' AS YAKKOU_CD,
								  IP1.CD_INTERVALO AS ZAIKEI_CD,


								  IE_ALTA_VIGILANCIA_X AS DRUG_FLAG,
...
BEGIN
				...
				IF (SI_TYPE_OF_PRESCRIPTION_W = 'INTI' OR SI_TYPE_OF_PRESCRIPTION_W = 'IPSI')
				THEN NYUGAI_KBN_W := '2';
				...
				IF (SI_TYPE_OF_PRESCRIPTION_W = 'INTI' AND IE_ALTO_RISCO_X = '0')
				THEN XX_SYBT_W := '420';
				ELSIF (SI_TYPE_OF_PRESCRIPTION_W = 'INTI' AND IE_ALTO_RISCO_X = '1')
				THEN XX_SYBT_W := '421';
				...
				THEN NYUGAI_KBN_W := '1';
				ELSIF (SI_TYPE_OF_PRESCRIPTION_W = 'IPSI' OR SI_TYPE_OF_PRESCRIPTION_W = 'INTI')
				THEN NYUGAI_KBN_W := '2';
				END IF;
			    ...
			    IF (SI_TYPE_OF_PRESCRIPTION_W = 'INTI')
				THEN COUNTER_LOOP_C01 := COUNTER_LOOP_C01 + 1;
				     CONTADOR_H2_W := CONTADOR_H2_W + 1;
				     ORDER_NO_W := 'H2' || '1' || TO_CHAR(clock_timestamp(), 'YYYY') || TO_CHAR(clock_timestamp(), 'MM') || LPAD(CONTADOR_H2_W,4,0) || SEQUENCIAL_W ||  '00';
				...
				FOR R_C01 IN C01 LOOP BEGIN
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.SYORI_DATE1, 8, 'R');
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.SEQ_NO1, 6, 'R');
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.WS_NO, 4, 'R');
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.INDEX_KBN, 1, 'R');
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.XX_KBN, 2, 'R');
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.XX_TYPE, 3, 'R');
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.XX_SEQ, 5, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.SYORI_DATE2, 8, 'R');					
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.SEQ_NO2, 5, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.ORDER_NO, 15, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.KA_NAME_R, 6, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.IN_TIME1, 6, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.STOP_COMMENT1, 50, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.RP_NUM1, 3, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.BCD_NO, 480, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.RP_NUM2, 5, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.YHK_NAME_NL, 48, 'R');
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.YHK_NAME_J, 66, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.HD_YKH1_NAME, 48, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.HD_YKH2_NAME, 48, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.HD_YKH3_NAME, 48, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.HD_YKH4_NAME, 48, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.HD_YKH5_NAME, 48, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.HD_TANI5_NAME, 4, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.DRUG_FLAG, 1, 'R');
					...
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.NONLABEL, 1, 'R');
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.HIGHRISK_FLG, 1, 'R');
					CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL tosho_presc_injection_pck.append_text(R_C01.YOBI_FLG2, 8, 'R');
				...
				ELSIF (SI_TYPE_OF_PRESCRIPTION_W  = 'OPTI')
				THEN COUNTER_LOOP_C01 := COUNTER_LOOP_C01 + 1;
				     CONTADOR_H4_W := CONTADOR_H4_W + 1;
				     ORDER_NO_W := 'H4' || '1' || TO_CHAR(clock_timestamp(), 'YYYY') || TO_CHAR(clock_timestamp(), 'MM') || LPAD(CONTADOR_H4_W,4,0) || SEQUENCIAL_W ||  '00';
				...
				ELSIF (SI_TYPE_OF_PRESCRIPTION_W  = 'IPSI')
				THEN COUNTER_LOOP_C01 := COUNTER_LOOP_C01 + 1;
				...
				ELSIF (SI_TYPE_OF_PRESCRIPTION_W  = 'OPSI')
				THEN COUNTER_LOOP_C01 := COUNTER_LOOP_C01 + 1;
				     CONTADOR_H0_W := CONTADOR_H0_W + 1;
				     ORDER_NO_W := 'H0' || '1' || TO_CHAR(clock_timestamp(), 'YYYY') || TO_CHAR(clock_timestamp(), 'MM') || LPAD(CONTADOR_H0_W,4,0) || SEQUENCIAL_W ||  '00';
				...
				ELSIF (IE_MEDICACAO_PACIENTE_W = 'S')
				THEN COUNTER_LOOP_C01 := COUNTER_LOOP_C01 + 1;
			    ...
			    DS_LOG_MESSAGE := 'File successfully uploaded for Injection Order Info '
				                  || CHR(13)
				...
				RECORD_INTEGRATION_CALL_LOG(NM_USUARIO_MEDIC_W, NM_USUARIO_MEDIC_W, clock_timestamp(), 'JAPAN_TOSHO_PDATA',
											'INJECTION ORDER INFO', 'T', 'E', NULL, DS_LOG_MESSAGE, 
			...
			WHEN OTHERS THEN
				...
				DS_SCRIPT_RESEND_ERROR := 'TOSHO_PRESC_INJECTION_PCK.JPN_INJECTION_ORDER_INFO ('
										  || NR_PRESCRICAO_P
										  || ');';

				DS_LOG_MESSAGE := 'Error occured while sending the integration message for Injection Order Info'
								  || CHR(13)
								  || 'Prescription Number: ' || NR_PRESCRICAO_P
								  || CHR(13)
				...
				DATA_NOME_ARQUIVO_LOG := 'H'||'S'||'BI'||'AB'||TO_CHAR(clock_timestamp(), 'YYYYMMDD')||'.log';
				DS_ARQ_LOG_MESSAGE := TO_CHAR(clock_timestamp(), 'YYYYMMDD HH24MISS') || ' Error occured in file creation for Injection Order Info ' || ', In CPOE ' ||  ' Tasy';
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE tosho_presc_injection_pck.jpn_injection_order_info (NR_PRESCRICAO_P bigint) FROM PUBLIC;
