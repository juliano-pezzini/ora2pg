...
SET client_encoding TO 'UTF8';
	...
	--VT>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Obter valor total de tributos vinculado ao projeto da NF <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<--	
...
CREATE OR REPLACE FUNCTION projeto_recurso_pck.obter_vl_total_trib_nf (nr_seq_nf_p bigint, nr_seq_item_nf_p bigint) RETURNS bigint AS $body$
DECLARE
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION projeto_recurso_pck.obter_vl_total_trib_nf (nr_seq_nf_p bigint, nr_seq_item_nf_p bigint) FROM PUBLIC;
