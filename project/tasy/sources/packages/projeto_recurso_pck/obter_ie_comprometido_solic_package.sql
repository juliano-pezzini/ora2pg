...
SET client_encoding TO 'UTF8';
	...
	--VC>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Solicitacao de Compra  (Conceito Booleano)<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<--

	/*
	Objetivo: Retornar 1 se a solcilitacao de compra passada por parametro possui valor comprometido no projeto recurso passado por parametro.
	Parametros: 
...
CREATE OR REPLACE FUNCTION projeto_recurso_pck.obter_ie_comprometido_solic ( nr_seq_proj_rec_p bigint, nr_solic_compra_p bigint) RETURNS bigint AS $body$
DECLARE
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION projeto_recurso_pck.obter_ie_comprometido_solic ( nr_seq_proj_rec_p bigint, nr_solic_compra_p bigint) FROM PUBLIC;
