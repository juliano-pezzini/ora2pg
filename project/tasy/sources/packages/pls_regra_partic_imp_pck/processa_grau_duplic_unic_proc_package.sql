...
CREATE OR REPLACE PROCEDURE pls_regra_partic_imp_pck.processa_grau_duplic_unic_proc ( dados_val_partic_p pls_ocor_imp_pck.dados_val_partic, nr_id_transacao_p pls_oc_cta_selecao_imp.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
c02 CURSOR(	nr_seq_conta_proc_pc	pls_conta_proc_imp.nr_sequencia%type) FOR
...
for r_c01_w in c01(nr_id_transacao_p) loop
	...
	for r_c02_w in c02(r_c01_w.nr_seq_conta_proc) loop
		...
		-- se o grau de participação anterior for diferente ao atual 
	...
	-- se os últimos partics são diferentes grava na observação
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_regra_partic_imp_pck.processa_grau_duplic_unic_proc ( dados_val_partic_p pls_ocor_imp_pck.dados_val_partic, nr_id_transacao_p pls_oc_cta_selecao_imp.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
