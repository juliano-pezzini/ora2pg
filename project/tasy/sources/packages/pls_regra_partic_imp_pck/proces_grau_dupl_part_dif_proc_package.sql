...
CREATE OR REPLACE PROCEDURE pls_regra_partic_imp_pck.proces_grau_dupl_part_dif_proc (dados_val_partic_p pls_ocor_imp_pck.dados_val_partic, nr_id_transacao_p pls_oc_cta_selecao_imp.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
for r_c01_w in c01(nr_id_transacao_p) loop
	...
	nr_seq_prestador_conv_ultimo_w:= null;
	...
	for r_c02_w in c02(r_c01_w.nr_seq_conta_proc) loop
		...
		ds_observacao_temp_w := substr(ds_observacao_temp_w ||	'Conta: ' || r_c02_w.nr_seq_conta || ' Proc: ' || r_c02_w.nr_seq_proc ||
									...
									' Participante: ' || r_c02_w.nr_seq_prestador_conv || pls_util_pck.enter_w, 1, 10000);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_regra_partic_imp_pck.proces_grau_dupl_part_dif_proc (dados_val_partic_p pls_ocor_imp_pck.dados_val_partic, nr_id_transacao_p pls_oc_cta_selecao_imp.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
