...
CREATE OR REPLACE PROCEDURE pls_sip_pck.sip_nv_popula_dado_cta_aberta ( nr_seq_lote_p pls_lote_sip.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
c01 CURSOR(	nr_seq_lote_pc		pls_lote_sip.nr_sequencia%type) is
	with dados_guia_alt as (SELECT	distinct a.nr_seq_conta
				from	sip_nv_dados a
				where	a.nr_seq_lote_sip = nr_seq_lote_pc
				and	a.ie_tipo_guia = '6')
	SELECT	b.nr_seq_conta,
		a.nr_seq_conta_referencia nr_seq_conta_princ
	from	dados_guia_alt b,
		pls_conta a
	where	a.nr_sequencia = b.nr_seq_conta
	and	(a.nr_seq_conta_referencia IS NOT NULL AND a.nr_seq_conta_referencia::text <> '');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_sip_pck.sip_nv_popula_dado_cta_aberta ( nr_seq_lote_p pls_lote_sip.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
