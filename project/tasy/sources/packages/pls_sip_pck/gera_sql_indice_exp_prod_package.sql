...
ds_retorno_w	:=	'select	a.nr_seq_segurado, ' || pls_util_pck.enter_w ||
			...
			'	''CAP'' ie_origem_exposicao, ' || pls_util_pck.enter_w ||
			...
			'where	b.nr_seq_plano		= a.nr_seq_plano ' || pls_util_pck.enter_w ||
			...
			-- apenas benef que seja entendido que o nivel de carencia seja produto

			'and	a.ie_nivel_carencia	= ''P'' ' || pls_util_pck.enter_w ||
			'and	c.ie_sexo		in (a.ie_sexo, ''A'') ' || pls_util_pck.enter_w ||
			'and	b.qt_dias		> 0 ' || pls_util_pck.enter_w ||
			-- Considera ainda, apenas as carencias de produtos que possuirem vigencia dentro do periodo do SIP

			'and	a.dt_contratacao between b.dt_inicio_vig_plano_ref and b.dt_fim_vig_plano_ref ' || pls_util_pck.enter_w ||
			'and	a.dt_contratacao + b.qt_dias >= to_date(:dt_inicio_periodo_p) ' || pls_util_pck.enter_w ||
			'union ' || pls_util_pck.enter_w ||
			-- Busca as carencias dentro do grupo vinculado diretamente ao produto
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION pls_sip_pck.gera_sql_indice_exp_prod ( ie_tipo_contratacao_in_sip_p text, cd_classificacao_p sip_item_assistencial.cd_classificacao%type, nr_seq_lote_p pls_lote_sip.nr_sequencia%type, nr_seq_item_assist_p sip_item_assistencial.nr_sequencia%type, dt_periodo_inicial_p pls_lote_sip.dt_periodo_inicial%type, dt_periodo_final_p pls_lote_sip.dt_periodo_final%type, dados_bind_p INOUT sql_pck.t_dado_bind) FROM PUBLIC;
