...
CREATE OR REPLACE PROCEDURE pls_sip_pck.gera_indice_exp_car_cont ( nr_seq_lote_sip_p pls_lote_sip.nr_sequencia%type, dt_periodo_inicial_p pls_lote_sip.dt_periodo_inicial%type, dt_periodo_final_p pls_lote_sip.dt_periodo_final%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
Finalidade:	Rotina responsavel por gerar o indice de exposicao do beneficiario com base nas carencias a nivel
	...
	Apos o levantamento dos indices necessarios, e chamada uma rotina que vai calcular e gravar os indices
...
tb_indice_exp_geral_w		t_exposicao_benef;
tb_indice_exp_contrato_w	t_exposicao_benef;
...
for r_c01_w in current_setting('pls_sip_pck.c_item_assist_exp')::CURSOR(nr_seq_lote_p(nr_seq_lote_sip_p) loop
	...
	dados_bind_w := pls_sip_pck.gera_sql_indice_exp_cont(	r_c01_w.ie_tipo_contratacao_in_sip, r_c01_w.cd_classificacao, nr_seq_lote_sip_p, r_c01_w.nr_seq_item_assist, dt_periodo_inicial_p, dt_periodo_final_p, dados_bind_w);
	...
	exit when tb_indice_exp_w.nr_seq_segurado.count = 0;
		...
		-- aplica os IF Extra GERAL

		tb_indice_exp_geral_w := pls_sip_pck.if_extra_idx_exp_geral(tb_indice_exp_w, tb_indice_exp_geral_w);
		
		-- aplica os IF EXTRA do CONTRATO, usando como base os indices filtrados pelo GERAL

		tb_indice_exp_contrato_w := pls_sip_pck.if_extra_idx_exp_contrato(tb_indice_exp_geral_w, tb_indice_exp_contrato_w);
		
		-- Chama a rotina que gerencia o indice, calculando ele e gravando.

		tb_indice_exp_contrato_w := pls_sip_pck.gerencia_indice_exposto(tb_indice_exp_contrato_w, nm_usuario_p);
		-- Limpa os vetores auxiliares

		tb_indice_exp_geral_w := pls_sip_pck.limpar_tb_exposicao(tb_indice_exp_geral_w);
		tb_indice_exp_contrato_w := pls_sip_pck.limpar_tb_exposicao(tb_indice_exp_contrato_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_sip_pck.gera_indice_exp_car_cont ( nr_seq_lote_sip_p pls_lote_sip.nr_sequencia%type, dt_periodo_inicial_p pls_lote_sip.dt_periodo_inicial%type, dt_periodo_final_p pls_lote_sip.dt_periodo_final%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
