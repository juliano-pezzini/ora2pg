...
ds_retorno_w	:=	'select	a.nr_seq_segurado, ' || pls_util_pck.enter_w ||
			...
			'	trunc(a.dt_contratacao, ''dd'') dt_inicio_vig_carencia, ' || pls_util_pck.enter_w ||
			...
			'	''CAC'' ie_origem_exposicao, ' || pls_util_pck.enter_w ||
			'	(	select	count(1) ' || pls_util_pck.enter_w ||
			'		from	sip_nv_benef_exp_item	x ' || pls_util_pck.enter_w ||
			'		where	x.nr_seq_lote		= :nr_seq_lote_p ' || pls_util_pck.enter_w ||
			'		and	x.nr_seq_item_assist	= :nr_seq_item_assist_p ' || pls_util_pck.enter_w ||
			...
			'	decode(nvl(b.ie_mes_posterior, ''N''), ''S'', trunc(add_months(a.dt_contratacao, 1), ''MONTH''), a.dt_contratacao) + b.qt_dias dt_ini_contratacao ' || pls_util_pck.enter_w ||
			...
			'where	b.nr_seq_contrato	= a.nr_seq_contrato ' || pls_util_pck.enter_w ||
			...
			'and	d.nr_seq_item_assist	= :nr_seq_item_assist_p ' || pls_util_pck.enter_w ||
			'and	e.nr_seq_tipo_carencia	= c.nr_sequencia ' || pls_util_pck.enter_w ||
			'and	e.nr_seq_lote		= :nr_seq_lote_p ' || pls_util_pck.enter_w ||
			-- apenas benef que seja entendido que o nivel de carencia seja contrato

			'and	a.ie_nivel_carencia	= ''C'' ' || pls_util_pck.enter_w ||
			'and	c.ie_sexo		in (a.ie_sexo, ''A'') ' || pls_util_pck.enter_w ||
			'and	b.qt_dias		> 0 ' || pls_util_pck.enter_w ||
			'union ' || pls_util_pck.enter_w ||
			-- Busca as carencias dentro do grupo vinculado diretamente ao contrato
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION pls_sip_pck.gera_sql_indice_exp_cont ( ie_tipo_contratacao_in_sip_p text, cd_classificacao_p sip_item_assistencial.cd_classificacao%type, nr_seq_lote_p pls_lote_sip.nr_sequencia%type, nr_seq_item_assist_p sip_item_assistencial.nr_sequencia%type, dt_periodo_inicial_p pls_lote_sip.dt_periodo_inicial%type, dt_periodo_final_p pls_lote_sip.dt_periodo_final%type, dados_bind_p INOUT sql_pck.t_dado_bind) FROM PUBLIC;
