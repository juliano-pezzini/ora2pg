...
CREATE OR REPLACE PROCEDURE pls_sip_pck.gera_indice_exp_car_prod ( nr_seq_lote_sip_p pls_lote_sip.nr_sequencia%type, dt_periodo_inicial_p pls_lote_sip.dt_periodo_inicial%type, dt_periodo_final_p pls_lote_sip.dt_periodo_final%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
-- Variaveis para buscar o indice
...
for r_c01_w in current_setting('pls_sip_pck.c_item_assist_exp')::CURSOR(nr_seq_lote_p(nr_seq_lote_sip_p) loop
	...
	dados_bind_w := pls_sip_pck.gera_sql_indice_exp_prod(	r_c01_w.ie_tipo_contratacao_in_sip, r_c01_w.cd_classificacao, nr_seq_lote_sip_p, r_c01_w.nr_seq_item_assist, dt_periodo_inicial_p, dt_periodo_final_p, dados_bind_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_sip_pck.gera_indice_exp_car_prod ( nr_seq_lote_sip_p pls_lote_sip.nr_sequencia%type, dt_periodo_inicial_p pls_lote_sip.dt_periodo_inicial%type, dt_periodo_final_p pls_lote_sip.dt_periodo_final%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
