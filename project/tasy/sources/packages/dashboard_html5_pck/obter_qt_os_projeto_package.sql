...
CREATE OR REPLACE PROCEDURE dashboard_html5_pck.obter_qt_os_projeto (nr_seq_proj_analise_p bigint, nr_seq_proj_prog_p bigint, nr_seq_proj_teste_p bigint, ie_papel_executor1_p text, ie_papel_executor2_p text, ie_atividade_p text, qt_os_projeto_p INOUT bigint) AS $body$
BEGIN
        if (coalesce(ie_papel_executor1_p::text, '') = '') and (coalesce(ie_papel_executor2_p::text, '') = '') and (coalesce(ie_atividade_p::text, '') = '') then
            select count(1)
              into STRICT qt_os_projeto_p
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE dashboard_html5_pck.obter_qt_os_projeto (nr_seq_proj_analise_p bigint, nr_seq_proj_prog_p bigint, nr_seq_proj_teste_p bigint, ie_papel_executor1_p text, ie_papel_executor2_p text, ie_atividade_p text, qt_os_projeto_p INOUT bigint) FROM PUBLIC;
