...
CREATE OR REPLACE FUNCTION bifrost_icca.sendintegrationhttp ( direct boolean, eventName text, javaClass text, content text, synchronous char, userName text, usesTasyBackend char, headers text ) RETURNS text AS $body$
DECLARE
...
BEGIN
    ...
    if activated = 'S' then
        ...
        tasyHost := bifrost_icca.gettasyhost(userName, usesTasyBackend);

        if (tasyHost IS NOT NULL AND tasyHost::text <> '') then							

			begin	

				contentConverted := bifrost_icca.convertcontent(content);
				...
				tenantToken := bifrost_icca.gettenanttoken();
				...
				bifrostMessageId := bifrost_icca.getbifrostmessageid(response);
			...
			CALL CALL bifrost_icca.logmessage(bifrostMessageId, eventName, javaClass, content, resultValue, userName, errorMessage);
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON FUNCTION bifrost_icca.sendintegrationhttp ( direct boolean, eventName text, javaClass text, content text, synchronous char, userName text, usesTasyBackend char, headers text ) FROM PUBLIC;
