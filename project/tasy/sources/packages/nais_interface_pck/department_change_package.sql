...
CREATE OR REPLACE PROCEDURE nais_interface_pck.department_change ( parameter_id_p bigint, ds_file_output_p INOUT text) AS $body$
DECLARE
		...
		SELECT 	01 update_class,
				02 processing_class,
				coalesce(d.medical_record_id,0) patient_id,
				04 transfer_instruction,
				to_char(c.department_admit_date,'YYYYMMDD') move_start_date,
				to_char(c.department_admit_date,'hh24mi') move_start_time,
				...
				coalesce(d.encounter_doctor_id, ' ') attending_doctor,
				0 outcome_classification,
                ' ' ds_finish, 
				to_char(d.encouter_admit_date,'YYYYMMDD') date_of_admission,
				to_char(d.encouter_admit_date,'hh24mi') time_of_admission,
				'01' admission_type,
                ' ' ds_spare,
				chr(13) eot_two	

		from    bft_encounter_movement_v c,
                bft_encounter_v d
		where   c.encounter_id = d.encounter_id
		and     c.movement_id = parameter_id_p;	
...
BEGIN
            ...
            begin
                ...
                where a.nr_seq_interno = parameter_id_p;
            end;

            select  coalesce(max(b.establishment_id), 1)
            into STRICT    cd_establishment
            from    bft_encounter_movement_v a,
                    bft_encounter_v b
            where   a.patient_id = b.patient_id
            and     a.movement_id = parameter_id_p;	
            ...
            select  coalesce(CD_DEPARTAMENTO,0)
            into STRICT    cd_department_w
            from    ATEND_PACIENTE_UNIDADE
            where   NR_SEQ_INTERNO=parameter_id_p;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE nais_interface_pck.department_change ( parameter_id_p bigint, ds_file_output_p INOUT text) FROM PUBLIC;
