...
CREATE OR REPLACE PROCEDURE nais_interface_pck.unexecutedorder_nutguidance (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) AS $body$
DECLARE
	...
	c02 CURSOR FOR
		SELECT
			...
			to_char(a.dt_inicio, 'YYYYMMDD') dt_of_consultation, 
			...
			to_char(a.dt_inicio, 'YYYYMMDD')  scheduled_date_of_execution,
			to_char(a.dt_inicio, 'HH24MI') schedule_time_of_execution,
		...
		from    cpoe_recomendacao a,
				...
				bft_encounter_v v
		where    
				b.nr_seq_rec_cpoe	     	   = a.nr_sequencia 
				...
				and b.nr_seq_interno           = (SELECT max(y.nr_seq_interno) from prescr_recomendacao y where y.nr_prescricao = nr_seq_registro_p)
				and ie_tipo_file_p        	   like '%unexecutedOrderMessageRequestNutGuidance%'		
...
union all
		...
		where    
				...
				and (select count(k.nr_seq_interno) from prescr_recomendacao k where k.nr_prescricao = d.nr_prescricao and coalesce(k.dt_suspensao::text, '') = '') = 0
				and ie_tipo_file_p             like '%unexecutedOrderMessageCancelNutGuidance%';
...
BEGIN
		...
		if ( ie_tipo_file_p = 'unexecutedOrderMessageRequestNutGuidance') then
            begin     
				...
				where    
					...
					and d.nr_atendimento 		   = y.nr_atendimento
					and h.nr_seq_sub_grp           = 'N' 
					and b.nr_seq_interno           = (SELECT max(y.nr_seq_interno) from prescr_procedimento y where y.nr_prescricao = nr_seq_registro_p);
		...
		if (ie_tipo_file_p = 'unexecutedOrderMessageCancelNutGuidance') then      
            begin  
				...
				select	coalesce(g.dt_lib_suspensao, d.dt_atualizacao),
						d.nr_prescricao,
						coalesce(a.nm_usuario_susp,d.nm_usuario),
						d.cd_estabelecimento,
						coalesce(g.cd_departamento_med, ''),
						coalesce(g.cd_especialidade_med, '')
				into STRICT 	current_setting('nais_interface_pck.dt_suspencao_w')::timestamp,
        ...
        if (current_setting('nais_interface_pck.ds_error_w')::coalesce(varchar(2000)::text, '') = '') then
            record_integration_call_log(current_setting('nais_interface_pck.nm_usuario_w')::varchar(20), 'NAIS', clock_timestamp(), 'nais.unexecutedOrder.message', 'unexecutedOrderMessageRequestNutGuidance',
            ...
            record_integration_call_log(wheb_usuario_pck.get_nm_usuario, 'NAIS', clock_timestamp(), 'nais.unexecutedOrder.message', 'unexecutedOrderMessageRequestNutGuidance',
            'E', 'E', null, 'MJ', null,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE nais_interface_pck.unexecutedorder_nutguidance (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) FROM PUBLIC;
