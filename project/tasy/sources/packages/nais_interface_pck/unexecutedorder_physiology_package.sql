...
CREATE OR REPLACE PROCEDURE nais_interface_pck.unexecutedorder_physiology (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) AS $body$
DECLARE
	...
	c02 CURSOR FOR
		...
		where
			...
			and   ie_tipo_file_p       like '%unexecutedOrderMessageRequestPhysiology%'		
...
union all
		...
		where
			...
			and ie_tipo_file_p        like '%unexecutedOrderMessageCancelPhysiology%';
...
BEGIN
		...
		if (ie_tipo_file_p = 'unexecutedOrderMessageRequestPhysiology') then
            begin             
                PERFORM set_config('nais_interface_pck.ie_processing_classification_w', '01', false);

				select	f.dt_prescricao,
						f.nr_prescricao,
						coalesce(f.nm_usuario,c.nm_usuario_susp),
		...
		if (ie_tipo_file_p = 'unexecutedOrderMessageCancelPhysiology') then
            begin   
        ...
        if (current_setting('nais_interface_pck.ds_error_w')::coalesce(varchar(2000)::text, '') = '') then
            record_integration_call_log(current_setting('nais_interface_pck.nm_usuario_w')::varchar(20), 'NAIS', clock_timestamp(), 'nais.unexecutedOrder.message', 'unexecutedOrderMessageRequestPhysiology',
            ...
            record_integration_call_log(wheb_usuario_pck.get_nm_usuario, 'NAIS', clock_timestamp(), 'nais.unexecutedOrder.message', 'unexecutedOrderMessageRequestPhysiology',
            'E', 'E', null, 'MJ', null,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE nais_interface_pck.unexecutedorder_physiology (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) FROM PUBLIC;
