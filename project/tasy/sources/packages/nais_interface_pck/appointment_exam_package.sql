...
CREATE OR REPLACE PROCEDURE nais_interface_pck.appointment_exam ( parameter_id_p bigint, ds_file_output_p INOUT text) AS $body$
DECLARE
		...
		SELECT 	substr(obter_prontuario_pf(obter_estab_agenda(a.cd_agenda),a.cd_pessoa_fisica),1,255) patient_id,
				...
				' ' urgent_exam_flag,
				substr(obter_conversao_externa_int(null, 'PROC_INTERNO', 'NR_SEQ_CLASSIF', 17, 'NAIS'),1,255) appointment_classification
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE nais_interface_pck.appointment_exam ( parameter_id_p bigint, ds_file_output_p INOUT text) FROM PUBLIC;
