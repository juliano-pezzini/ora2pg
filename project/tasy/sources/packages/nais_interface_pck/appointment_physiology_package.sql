...
CREATE OR REPLACE PROCEDURE nais_interface_pck.appointment_physiology ( nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) AS $body$
DECLARE
	...
	c02 CURSOR FOR
		SELECT substr(obter_prontuario_pf(obter_estab_atend(C.nr_atendimento),C.cd_pessoa_fisica),1,10) patient_id,
				...
				'Physiology' yo_comment1_physiology,
			...
			WHERE   A.nr_seq_cpoe_tipo_pedido   = b.nr_sequencia
			  ...
			  AND   ie_tipo_file_p              like '%appointmentMessageRequestPhysiology%' 
...
UNION ALL
		...
		WHERE   A.nr_seq_cpoe_tipo_pedido   = b.nr_sequencia
		  ...
		  AND   ie_tipo_file_p              like '%appointmentMessageCancelPhysiology%';
...
BEGIN
		...
		PERFORM set_config('nais_interface_pck.json_output_list_w', philips_json_list(), false);
		 ...
		 if ( ie_tipo_file_p = 'appointmentMessageRequestPhysiology') then
		...
		if ( ie_tipo_file_p = 'appointmentMessageCancelPhysiology') then  
		...
		FOR r_c02 IN c02 LOOP
			BEGIN
				...
				CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL nais_interface_pck.append_text(r_c02.yo_comment1_physiology,50, 'R');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE nais_interface_pck.appointment_physiology ( nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) FROM PUBLIC;
