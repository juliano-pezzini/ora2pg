...
CREATE OR REPLACE PROCEDURE nais_interface_pck.appointment_specimen (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) AS $body$
DECLARE
	...
	c02 CURSOR FOR
		...
		where
			...
			AND   ie_tipo_file_p      like '%appointmentMessageRequestSpecimen%'
...
UNION ALL
		...
		where
			...
			AND   ie_tipo_file_p      like '%appointmentMessageCancelSpecimen%';
...
BEGIN
	...
	if ( ie_tipo_file_p = 'appointmentMessageRequestSpecimen') then
	...
	if ( ie_tipo_file_p = 'appointmentMessageCancelSpecimen') then  
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE nais_interface_pck.appointment_specimen (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) FROM PUBLIC;
