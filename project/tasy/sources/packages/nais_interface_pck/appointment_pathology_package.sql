...
CREATE OR REPLACE PROCEDURE nais_interface_pck.appointment_pathology (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) AS $body$
DECLARE
	...
	c02 CURSOR FOR
		SELECT substr(obter_prontuario_pf(obter_estab_atend(C.nr_atendimento),C.cd_pessoa_fisica),1,10) patient_id,
				...
				'Pathology' yo_comment1_pathology,
			...
			WHERE   A.nr_seq_cpoe_tipo_pedido   = b.nr_sequencia
			  ...
			  AND   ie_tipo_file_p              like '%appointmentMessageRequestPathology%' 
...
UNION ALL
		...
		SELECT substr(obter_prontuario_pf(obter_estab_atend(C.nr_atendimento),C.cd_pessoa_fisica),1,10) patient_id,
			...
			WHERE   A.nr_seq_cpoe_tipo_pedido   = b.nr_sequencia
			  ...
			  AND   ie_tipo_file_p              like '%appointmentMessageCancelPathology%';
...
BEGIN
		...
		if ( ie_tipo_file_p = 'appointmentMessageRequestPathology') then
	...
	if ( ie_tipo_file_p = 'appointmentMessageCancelPathology') then  
		...
		FOR r_c02 IN c02 LOOP
			BEGIN
				...
				CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL nais_interface_pck.append_text(r_c02.yo_comment1_pathology,50, 'R');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE nais_interface_pck.appointment_pathology (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) FROM PUBLIC;
