...
CREATE OR REPLACE PROCEDURE nais_interface_pck.discharge_confirmed ( parameter_id_p bigint, ds_file_output_p INOUT text) AS $body$
DECLARE
		...
		SELECT	current_setting('nais_interface_pck.nr_serial_w')::bigint serial_number,
				...
				current_setting('nais_interface_pck.ie_process_class_w')::varchar(20) PROCESSING_CLASSIFICATION,
				' ' RESPONSE_CLASS,
				162  TELEGRAM_LENGTH,
				' ' EOT,
		...
		C02 CURSOR FOR
		SELECT 	case
				   when coalesce(b.dt_alta::text, '') = '' then '03'
				   else '01'
				end as update_class,
				02 processing_class,
				coalesce(c.medical_record_id,0) patient_id,
				02 transfer_instruction,
				coalesce(to_char(b.dt_alta,'YYYYMMDD'), ' ') move_start_date,
				coalesce(to_char(b.dt_alta,'hh24mi'), ' ') move_start_time,
				...
				coalesce(c.encounter_doctor_id, ' ') attending_doctor,
		...
		from atendimento_paciente b,
			bft_encounter_v c
		where b.nr_atendimento = c.encounter_id 
		and	  b.nr_atendimento = parameter_id_p;	
...
BEGIN
            ...
            select coalesce(max(a.nm_usuario), 'NAIS')
                ...
                where a.nr_atendimento = parameter_id_p;
            end;
            ...
            and     a.nr_atendimento 		= parameter_id_p;
			...
			select		dt_alta
			into STRICT		discharge_date
			from		atendimento_paciente
			where		nr_atendimento = parameter_id_p;

            if (coalesce(discharge_date::text, '') = '') then
                PERFORM set_config('nais_interface_pck.ie_process_class_w', '03', false);
            else
                PERFORM set_config('nais_interface_pck.ie_process_class_w', '01', false);
            end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE nais_interface_pck.discharge_confirmed ( parameter_id_p bigint, ds_file_output_p INOUT text) FROM PUBLIC;
