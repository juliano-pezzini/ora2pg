...
CREATE OR REPLACE PROCEDURE nais_interface_pck.appointment_nutguidance (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) AS $body$
DECLARE
	...
	c02 CURSOR FOR
		SELECT  substr(obter_prontuario_pf(obter_estab_atend(A.nr_atendimento),A.cd_pessoa_fisica),1,10) patient_id,
				...
				'Nutritional guidance'comment_1, 
				' 'comment_2,
		...
		and      ie_tipo_file_p             like '%appointmentMessageRequestNutritionalGuidance%' 
...
UNION ALL
		...
		and (select count(k.nr_seq_interno) from prescr_recomendacao k where k.nr_prescricao = b.nr_prescricao and coalesce(k.dt_suspensao::text, '') = '') = 0
		and      ie_tipo_file_p             like '%appointmentMessageCancelNutritionalGuidance%';
...
BEGIN
	...
	if ( ie_tipo_file_p = 'appointmentMessageRequestNutritionalGuidance') then
	...
	if ( ie_tipo_file_p = 'appointmentMessageCancelNutritionalGuidance') then  
	...
	FOR r_c02 IN c02 LOOP
			BEGIN
				...
				CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL nais_interface_pck.append_text(r_c02.date_of_appointment,8,'L');
				CALL CALL CALL CALL CALL CALL CALL CALL CALL CALL nais_interface_pck.append_text(r_c02.time_appointment,4,'L');
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE nais_interface_pck.appointment_nutguidance (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) FROM PUBLIC;
