...
CREATE OR REPLACE PROCEDURE nais_interface_pck.unexecutedorder_injection (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) AS $body$
DECLARE
	...
	c02 CURSOR FOR
		...
		where
			...
			and d.nr_seq_mat_cpoe     = c.nr_sequencia
			...
			and d.nr_seq_interno      = (SELECT max(y.nr_seq_interno) from prescr_material y where y.nr_prescricao = nr_seq_registro_p)
			and ie_tipo_file_p        like '%unexecutedOrderMessageRequestInjection%'		
...
union all
		...
		where
			...
			and (select count(k.nr_seq_interno) from prescr_material k where k.nr_prescricao = d.nr_prescricao and coalesce(k.dt_suspensao::text, '') = '') = 0
			and ie_tipo_file_p        like '%unexecutedOrderMessageCancelInjection%';
...
BEGIN
		...
		if ( ie_tipo_file_p = 'unexecutedOrderMessageRequestInjection') then
            begin 
		...
		if (ie_tipo_file_p = 'unexecutedOrderMessageCancelInjection') then   
            begin  
        ...
        if (current_setting('nais_interface_pck.ds_error_w')::coalesce(varchar(2000)::text, '') = '') then
            record_integration_call_log(current_setting('nais_interface_pck.nm_usuario_w')::varchar(20), 'NAIS', clock_timestamp(), 'nais.unexecutedOrder.message', 'unexecutedOrderMessageRequestInjection',
            ...
            record_integration_call_log(wheb_usuario_pck.get_nm_usuario, 'NAIS', clock_timestamp(), 'nais.unexecutedOrder.message', 'unexecutedOrderMessageRequestInjection',
            'E', 'E', null, 'MJ', null,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE nais_interface_pck.unexecutedorder_injection (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) FROM PUBLIC;
