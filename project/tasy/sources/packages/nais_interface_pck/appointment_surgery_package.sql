...
CREATE OR REPLACE PROCEDURE nais_interface_pck.appointment_surgery (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) AS $body$
DECLARE
	...
	c02 CURSOR FOR
		...
		WHERE C.nr_seq_cpoe_tipo_pedido   = b.nr_sequencia
		   AND   A.nr_seq_cpoe_order_unit    = C.nr_sequencia
		   AND   E.nr_seq_proc_cpoe          = A.nr_sequencia
		   AND   E.nr_prescricao             = F.nr_prescricao
		   and   e.nr_seq_interno            = (SELECT max(y.nr_seq_interno) from prescr_procedimento y where y.nr_prescricao = nr_seq_registro_p)
		   AND   b.nr_seq_sub_grp            = 'S' 
		   AND   ie_tipo_file_p              like '%appointmentMessageRequestSurgery%' 
...
UNION ALL
		...
		WHERE C.nr_seq_cpoe_tipo_pedido   = b.nr_sequencia
		   ...
		   AND   E.nr_seq_proc_cpoe          = A.nr_sequencia
		   AND   E.nr_prescricao             = F.nr_prescricao
		   AND   E.nr_seq_interno            = nr_seq_registro_p
		   AND   b.nr_seq_sub_grp            = 'S' 
		   and (select count(k.nr_seq_interno) from prescr_procedimento k where k.nr_prescricao = e.nr_prescricao and coalesce(k.dt_suspensao::text, '') = '') = 0
		   AND   ie_tipo_file_p              like '%appointmentMessageCancelSurgery%';
...
BEGIN
	...
	if ( ie_tipo_file_p = 'appointmentMessageRequestSurgery') then
	...
	if ( ie_tipo_file_p = 'appointmentMessageCancelSurgery') then  
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE nais_interface_pck.appointment_surgery (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) FROM PUBLIC;
