...
CREATE OR REPLACE PROCEDURE nais_interface_pck.unexecutedorder_bacteria (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) AS $body$
DECLARE
	...
	c02 CURSOR FOR
		...
		where
			...
			and   ie_tipo_file_p       like '%unexecutedOrderMessageRequestBacteria%'		
...
union all
		...
		where
			...
			and ie_tipo_file_p        like '%unexecutedOrderMessageCancelBacteria%';
...
BEGIN
		...
		if (ie_tipo_file_p = 'unexecutedOrderMessageRequestBacteria') then
            begin
		...
		if (ie_tipo_file_p = 'unexecutedOrderMessageCancelBacteria') then
            begin   
        ...
        if (current_setting('nais_interface_pck.ds_error_w')::coalesce(varchar(2000)::text, '') = '') then
            record_integration_call_log(current_setting('nais_interface_pck.nm_usuario_w')::varchar(20), 'NAIS', clock_timestamp(), 'nais.unexecutedOrder.message', 'unexecutedOrderMessageRequestBacteria',
            ...
            record_integration_call_log(wheb_usuario_pck.get_nm_usuario, 'NAIS', clock_timestamp(), 'nais.unexecutedOrder.message', 'unexecutedOrderMessageRequestBacteria',
            'E', 'E', null, 'MJ', null,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE nais_interface_pck.unexecutedorder_bacteria (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) FROM PUBLIC;
