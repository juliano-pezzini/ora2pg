...
CREATE OR REPLACE PROCEDURE nais_interface_pck.unexecutedorder_radiology (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) AS $body$
DECLARE
	...
	c02 CURSOR FOR
		...
		where
			...
			and   ie_tipo_file_p       like '%unexecutedOrderMessageRequestRadiology%'		
...
union all
		...
		where
			...
			and ie_tipo_file_p        like '%unexecutedOrderMessageCancelRadiology%';
...
BEGIN
		...
		if (ie_tipo_file_p = 'unexecutedOrderMessageRequestRadiology') then
            begin             
		...
		if (ie_tipo_file_p = 'unexecutedOrderMessageCancelRadiology') then
            begin   
        ...
        if (current_setting('nais_interface_pck.ds_error_w')::coalesce(varchar(2000)::text, '') = '') then
            record_integration_call_log(current_setting('nais_interface_pck.nm_usuario_w')::varchar(20), 'NAIS', clock_timestamp(), 'nais.unexecutedOrder.message', 'unexecutedOrderMessageRequestRadiology',
            ...
            record_integration_call_log(wheb_usuario_pck.get_nm_usuario, 'NAIS', clock_timestamp(), 'nais.unexecutedOrder.message', 'unexecutedOrderMessageRequestRadiology',
            'E', 'E', null, 'MJ', null,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE nais_interface_pck.unexecutedorder_radiology (nr_seq_registro_p bigint, ds_file_output_p INOUT text, ie_tipo_file_p text) FROM PUBLIC;
