...
CREATE OR REPLACE PROCEDURE fis_sef_edoc_arq_geral_pck.fis_gerar_arq_0150_edoc () AS $body$
DECLARE

-- c_reg_0150
c_reg_0150 CURSOR FOR
SELECT 	*
from 	fis_sef_edoc_0150
...
type reg_c_reg_0150 is table of fis_sef_edoc_0150%RowType;
...
fetch c_reg_0150 bulk collect	into vetReg0150 limit 1000;
	...
	PERFORM set_config('fis_sef_edoc_arq_geral_pck.ds_linha_w', substr(current_setting('fis_sef_edoc_arq_geral_pck.ds_sep_w')::varchar(1) || vetReg0150[i].cd_reg 	|| --01
		       current_setting('fis_sef_edoc_arq_geral_pck.ds_sep_w')::varchar(1) || vetReg0150[i].cd_part 	|| --02
		       current_setting('fis_sef_edoc_arq_geral_pck.ds_sep_w')::varchar(1) || vetReg0150[i].ds_nome 	|| --03
		       current_setting('fis_sef_edoc_arq_geral_pck.ds_sep_w')::varchar(1) || lpad(vetReg0150[i].cd_pais,5,'0') 	|| --04
		       current_setting('fis_sef_edoc_arq_geral_pck.ds_sep_w')::varchar(1) || vetReg0150[i].cd_cnpj 	|| --05
		       current_setting('fis_sef_edoc_arq_geral_pck.ds_sep_w')::varchar(1) || vetReg0150[i].cd_cpf 	|| --06
		       current_setting('fis_sef_edoc_arq_geral_pck.ds_sep_w')::varchar(1) || --07 Vazio1
		       current_setting('fis_sef_edoc_arq_geral_pck.ds_sep_w')::varchar(1) || vetReg0150[i].sg_uf 		|| --08
		       current_setting('fis_sef_edoc_arq_geral_pck.ds_sep_w')::varchar(1) || vetReg0150[i].nr_ie 		|| --09
		       current_setting('fis_sef_edoc_arq_geral_pck.ds_sep_w')::varchar(1) || vetReg0150[i].nr_ie_st 	|| --10
		       current_setting('fis_sef_edoc_arq_geral_pck.ds_sep_w')::varchar(1) || vetReg0150[i].cd_mun 	|| --11
		       current_setting('fis_sef_edoc_arq_geral_pck.ds_sep_w')::varchar(1) || vetReg0150[i].cd_im 		|| --12
		       current_setting('fis_sef_edoc_arq_geral_pck.ds_sep_w')::varchar(1) || vetReg0150[i].ds_suframa 	|| --13
	...
	current_setting('fis_sef_edoc_arq_geral_pck.fis_registros_w')::registro(current_setting('fis_sef_edoc_arq_geral_pck.qt_cursor_w')::bigint).cd_registro 		:= vetReg0150[i].cd_reg;

	-- Tipo de arquivo 4 e 6
	if (current_setting('fis_sef_edoc_arq_geral_pck.vetregcontrole')::fis_sef_edoc_controle%RowType.ie_tipo_arquivo in ('4','6')) then
		CALL fis_sef_edoc_arq_geral_pck.fis_gerar_arq_0175_edoc(vetReg0150[i].nr_sequencia);
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_sef_edoc_arq_geral_pck.fis_gerar_arq_0150_edoc () FROM PUBLIC;
