...
CREATE OR REPLACE PROCEDURE fis_sef_edoc_arq_geral_pck.fis_gerar_arq_0450_edoc () AS $body$
DECLARE

-- c_reg_0450
c_reg_0450 CURSOR FOR
SELECT 	*
from 	fis_sef_edoc_0450
...
type reg_c_reg_0450 is table of fis_sef_edoc_0450%RowType;
vetReg0450 reg_c_reg_0450;

--
ds_txt_w varchar(255);
...
fetch c_reg_0450 bulk collect	into vetReg0450 limit 1000;
  ...
  ds_txt_w := vetReg0450[i].ds_txt;

  if length(ds_txt_w) < 10 then
     ds_txt_w := rpad(ds_txt_w,11,' ');
  end if;
	...
	PERFORM set_config('fis_sef_edoc_arq_geral_pck.ds_linha_w', substr(current_setting('fis_sef_edoc_arq_geral_pck.ds_sep_w')::varchar(1) || vetReg0450[i].cd_reg 	|| --01
		       current_setting('fis_sef_edoc_arq_geral_pck.ds_sep_w')::varchar(1) || vetReg0450[i].cd_inf 	|| --02
		       current_setting('fis_sef_edoc_arq_geral_pck.ds_sep_w')::varchar(1) || ds_txt_w 	|| --03
	...
	current_setting('fis_sef_edoc_arq_geral_pck.fis_registros_w')::registro(current_setting('fis_sef_edoc_arq_geral_pck.qt_cursor_w')::bigint).cd_registro 		:= vetReg0450[i].cd_reg;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE fis_sef_edoc_arq_geral_pck.fis_gerar_arq_0450_edoc () FROM PUBLIC;
