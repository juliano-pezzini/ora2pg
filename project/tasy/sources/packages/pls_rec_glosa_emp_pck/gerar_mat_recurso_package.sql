...
CREATE OR REPLACE PROCEDURE pls_rec_glosa_emp_pck.gerar_mat_recurso ( nr_seq_demost_analise_p pls_dma_demons_analise.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE

_ora2pg_r RECORD;
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade:	Gera os materiais para as guias
...
tb_cd_material_imp_w		dbms_sql.varchar2_table;
...
c01 CURSOR(	nr_seq_demost_analise_pc	pls_dma_demons_analise.nr_sequencia%type) FOR
	SELECT	nextval('pls_grg_guia_mat_seq') nr_sequencia,
		a.nr_sequencia nr_seq_grg_guia,
		g.nr_sequencia nr_seq_guia_mat,
		c.nr_sequencia nr_seq_dma_procedimento,
		coalesce(g.nr_seq_material_conversao, g.nr_seq_material) nr_seq_material,
		c.cd_procedimento cd_material_imp,
		c.cd_tabela cd_tabela_imp,
		g.vl_material vl_cobrado_fat,
	...
	and	g.cd_material_ops	= c.cd_procedimento  -- o demonstrativo nao tem um campo especifico para material
...
begin
	...
	exit when tb_nr_sequencia_w.count = 0;
		...
		SELECT * FROM pls_rec_glosa_emp_pck.grava_mat_recurso(	tb_nr_sequencia_w, tb_nr_seq_grg_guia_w, tb_nr_seq_guia_mat_w, tb_nr_seq_dma_procedimento_w, tb_nr_seq_material_w, tb_cd_material_imp_w, tb_cd_tabela_imp_w, tb_vl_cobrado_fat_w, tb_vl_glosado_w, tb_vl_recursado_w, tb_vl_acatado_w, tb_dt_realizacao_imp_w, nm_usuario_p, 'N', 'S') INTO STRICT _ora2pg_r;
 	tb_nr_sequencia_w := _ora2pg_r.tb_nr_sequencia_p; tb_nr_seq_grg_guia_w := _ora2pg_r.tb_nr_seq_grg_guia_p; tb_nr_seq_guia_mat_w := _ora2pg_r.tb_nr_seq_guia_mat_p; tb_nr_seq_dma_procedimento_w := _ora2pg_r.tb_nr_seq_dma_procedimento_p; tb_nr_seq_material_w := _ora2pg_r.tb_nr_seq_material_p; tb_cd_material_imp_w := _ora2pg_r.tb_cd_material_imp_p; tb_cd_tabela_imp_w := _ora2pg_r.tb_cd_tabela_imp_p; tb_vl_cobrado_fat_w := _ora2pg_r.tb_vl_cobrado_fat_p; tb_vl_glosado_w := _ora2pg_r.tb_vl_glosado_p; tb_vl_recursado_w := _ora2pg_r.tb_vl_recursado_p; tb_vl_acatado_w := _ora2pg_r.tb_vl_acatado_p; tb_dt_realizacao_imp_w := _ora2pg_r.tb_dt_realizacao_imp_p;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_rec_glosa_emp_pck.gerar_mat_recurso ( nr_seq_demost_analise_p pls_dma_demons_analise.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
