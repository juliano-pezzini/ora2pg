...
CREATE OR REPLACE PROCEDURE pls_tipos_ocor_pck.aplica_filtro_prestador ( ie_considera_selecao_p boolean, dados_regra_p pls_tipos_ocor_pck.dados_regra, dados_filtro_p pls_tipos_ocor_pck.dados_filtro, dados_consistencia_p pls_tipos_ocor_pck.dados_consistencia, dados_forma_geracao_ocor_p pls_tipos_ocor_pck.dados_forma_geracao_ocor, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE

/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: gerencia a aplicacao do filtro de prestador
-------------------------------------------------------------------------------------------------------------------
...
c_filtro CURSOR(	nr_seq_filtro_pc	pls_oc_cta_filtro.nr_sequencia%type,
	...
	SELECT	a.cd_especialidade_medica cd_especialidade_medica,
		...
		a.nr_seq_prest_inter	 nr_seq_prest_inter,
		a.nr_seq_grupo_prest_int nr_seq_grupo_prest_int,
		coalesce(a.ie_tipo_pessoa_prest_int,'A') ie_tipo_pessoa_prest_int
...
if (dados_regra_p.nr_sequencia IS NOT NULL AND dados_regra_p.nr_sequencia::text <> '') then
	...
	for	rw_tipo_filtro_w in cs_tipo_filtro(dados_filtro_p.nr_sequencia) loop
		...
		for	r_c_filtro_w in c_filtro(dados_filtro_p.nr_sequencia, rw_tipo_filtro_w.ie_tipo_filtro) loop
			...
			dados_filtro_prest_w.nr_seq_prest_inter		:= r_c_filtro_w.nr_seq_prest_inter;
			dados_filtro_prest_w.nr_seq_grupo_prest_int	:= r_c_filtro_w.nr_seq_grupo_prest_int;
			dados_filtro_prest_w.ie_tipo_pessoa_prest_int	:= r_c_filtro_w.ie_tipo_pessoa_prest_int;
			...
			begin
				...
				if	(dados_regra_p.ie_incidencia_selecao = 'PM' AND dados_filtro_p.ie_processo_excecao = 'N') then
					...
					valor_bind_w := pls_tipos_ocor_pck.obter_restricao_prestador(	dados_regra_p, dados_filtro_p, dados_filtro_prest_w, 'PROC', valor_bind_w);
					-- restricao para o filtro de material

					valor_bind_w := pls_tipos_ocor_pck.obter_restricao_prestador(	dados_regra_p, dados_filtro_p, dados_filtro_prest_w, 'MAT', valor_bind_w);
				else
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_tipos_ocor_pck.aplica_filtro_prestador ( ie_considera_selecao_p boolean, dados_regra_p pls_tipos_ocor_pck.dados_regra, dados_filtro_p pls_tipos_ocor_pck.dados_filtro, dados_consistencia_p pls_tipos_ocor_pck.dados_consistencia, dados_forma_geracao_ocor_p pls_tipos_ocor_pck.dados_forma_geracao_ocor, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
