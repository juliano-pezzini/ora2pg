...
if (dados_filtro_p.nr_sequencia IS NOT NULL AND dados_filtro_p.nr_sequencia::text <> '') then
	...
	CALL pls_gerencia_upd_obj_pck.atualizar_objetos(nm_usuario_p, 'PLS_TIPOS_OCOR_PCK.APLICA_FILTRO_PROCEDIMENTO()', 'PLS_GRUPO_SERVICO_TM');

	-- gerencia a atualizacao da tabela TM

	CALL pls_gerencia_upd_obj_pck.atualizar_objetos(nm_usuario_p, 'PLS_TIPOS_OCOR_PCK.APLICA_FILTRO_PROCEDIMENTO()', 'PLS_ESTRUTURA_OCOR_TM');
	...
	for r_c_filtro in c_filtro(dados_filtro_p.nr_sequencia) loop
		...
		begin
			...
			if	(dados_regra_p.ie_incidencia_selecao = 'PM' AND dados_filtro_p.ie_processo_excecao = 'N') then
				...
				valor_bind_w := pls_tipos_ocor_pck.obter_restricao_procedimento(	dados_regra_p, dados_filtro_p, dados_filtro_proc_w, 'PROC', valor_bind_w);
			else
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_tipos_ocor_pck.aplica_filtro_procedimento ( ie_considera_selecao_p boolean, dados_regra_p pls_tipos_ocor_pck.dados_regra, dados_filtro_p pls_tipos_ocor_pck.dados_filtro, dados_consistencia_p pls_tipos_ocor_pck.dados_consistencia, dados_forma_geracao_ocor_p pls_tipos_ocor_pck.dados_forma_geracao_ocor, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
