...
if (dados_filtro_p.nr_sequencia IS NOT NULL AND dados_filtro_p.nr_sequencia::text <> '') then
	...
	for	r_c_filtro_w in c_filtro(dados_filtro_p.nr_sequencia) loop
		...
		dados_filtro_interc_w.nr_sequencia            		:= r_c_filtro_w.nr_sequencia;
		...
		begin
			...
			if	(dados_regra_p.ie_incidencia_selecao = 'PM' AND dados_filtro_p.ie_processo_excecao = 'N') then
				...
				valor_bind_w := pls_tipos_ocor_pck.obter_restricao_intercambio(	dados_regra_p, dados_filtro_p, dados_filtro_interc_w, 'PROC', valor_bind_w);
				-- restricao para o filtro de material

				valor_bind_w := pls_tipos_ocor_pck.obter_restricao_intercambio(	dados_regra_p, dados_filtro_p, dados_filtro_interc_w, 'MAT', valor_bind_w);
			else
				-- restricao para todas as outras situacoes

				valor_bind_w := pls_tipos_ocor_pck.obter_restricao_intercambio(	dados_regra_p, dados_filtro_p, dados_filtro_interc_w, null, valor_bind_w);
			end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_tipos_ocor_pck.aplica_filtro_intercambio ( ie_considera_selecao_p boolean, dados_regra_p pls_tipos_ocor_pck.dados_regra, dados_filtro_p pls_tipos_ocor_pck.dados_filtro, dados_consistencia_p pls_tipos_ocor_pck.dados_consistencia, dados_forma_geracao_ocor_p pls_tipos_ocor_pck.dados_forma_geracao_ocor, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
