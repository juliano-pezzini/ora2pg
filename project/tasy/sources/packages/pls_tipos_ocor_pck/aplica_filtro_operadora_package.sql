...
--aplica_filtro_operadora



CREATE OR REPLACE PROCEDURE pls_tipos_ocor_pck.aplica_filtro_operadora ( ie_considera_selecao_p boolean, dados_regra_p pls_tipos_ocor_pck.dados_regra, dados_filtro_p pls_tipos_ocor_pck.dados_filtro, dados_consistencia_p pls_tipos_ocor_pck.dados_consistencia, dados_forma_geracao_ocor_p pls_tipos_ocor_pck.dados_forma_geracao_ocor, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE

/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: gerencia a aplicacao do filtro da operadora
-------------------------------------------------------------------------------------------------------------------
...
dados_filtro_oper_w	pls_tipos_ocor_pck.dados_filtro_oper;
...
if (dados_regra_p.nr_sequencia IS NOT NULL AND dados_regra_p.nr_sequencia::text <> '') then
	...
	for	r_c_filtro_w in c_filtro(dados_filtro_p.nr_sequencia) loop
		...
		dados_filtro_oper_w.ie_tipo_intercambio		:= r_c_filtro_w.ie_tipo_intercambio;
		dados_filtro_oper_w.nr_seq_congenere		:= r_c_filtro_w.nr_seq_congenere;
		dados_filtro_oper_w.nr_seq_congenere_sup	:= r_c_filtro_w.nr_seq_congenere_sup;
		dados_filtro_oper_w.nr_seq_grupo_cooperativa	:= r_c_filtro_w.nr_seq_grupo_cooperativa;
		dados_filtro_oper_w.nr_sequencia		:= r_c_filtro_w.nr_sequencia;
		dados_filtro_oper_w.sg_estado_operadora		:= r_c_filtro_w.sg_estado_operadora;
		...
		begin
			...
			if	(dados_regra_p.ie_incidencia_selecao = 'PM' AND dados_filtro_p.ie_processo_excecao = 'N') then
				...
				valor_bind_w := pls_tipos_ocor_pck.obter_restricao_operadora(	dados_regra_p, dados_filtro_p, dados_filtro_oper_w, 'PROC', valor_bind_w);
				-- restricao para o filtro de material

				valor_bind_w := pls_tipos_ocor_pck.obter_restricao_operadora(	dados_regra_p, dados_filtro_p, dados_filtro_oper_w, 'MAT', valor_bind_w);
			else
				-- restricao para todas as outras situacoes

				valor_bind_w := pls_tipos_ocor_pck.obter_restricao_operadora(		dados_regra_p, dados_filtro_p, dados_filtro_oper_w, null, valor_bind_w);
			end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_tipos_ocor_pck.aplica_filtro_operadora ( ie_considera_selecao_p boolean, dados_regra_p pls_tipos_ocor_pck.dados_regra, dados_filtro_p pls_tipos_ocor_pck.dados_filtro, dados_consistencia_p pls_tipos_ocor_pck.dados_consistencia, dados_forma_geracao_ocor_p pls_tipos_ocor_pck.dados_forma_geracao_ocor, nr_id_transacao_p pls_selecao_ocor_cta.nr_id_transacao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
