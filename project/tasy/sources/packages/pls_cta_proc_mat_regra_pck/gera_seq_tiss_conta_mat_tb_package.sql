...
CREATE OR REPLACE PROCEDURE pls_cta_proc_mat_regra_pck.gera_seq_tiss_conta_mat_tb ( tb_nr_seq_conta_p INOUT dbms_sql.number_table, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
nr_seq_max_item_tiss_w	pls_conta_mat_regra.nr_seq_item_tiss_vinculo%type;
...
c01 CURSOR(	tb_nr_seq_conta_pc	pls_conta.nr_sequencia%type,
		nr_seq_max_item_tiss_pc	pls_conta_mat_regra.nr_seq_item_tiss_vinculo%type) FOR
...
for i in tb_nr_seq_conta_w.first..tb_nr_seq_conta_w.last loop
	...
	loop
	fetch c01 bulk collect into	tb_nr_seq_conta_mat_regra_w,
					tb_nr_seq_item_tiss_w,
					tb_nr_seq_item_tiss_vinc_w limit pls_util_pck.qt_registro_transacao_w;
					
	exit when tb_nr_seq_conta_mat_regra_w.count = 0;
 	...
 	tb_nr_seq_conta_mat_regra_w := _ora2pg_r.tb_nr_seq_conta_mat_regra_p; tb_nr_seq_item_tiss_w := _ora2pg_r.tb_nr_seq_item_tiss_p; tb_nr_seq_item_tiss_vinc_w := _ora2pg_r.tb_nr_seq_item_tiss_vinc_p;
		...
		tb_nr_seq_conta_mat_regra_w.delete;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_cta_proc_mat_regra_pck.gera_seq_tiss_conta_mat_tb ( tb_nr_seq_conta_p INOUT dbms_sql.number_table, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
