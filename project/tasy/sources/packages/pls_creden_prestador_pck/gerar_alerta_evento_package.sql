...
CREATE OR REPLACE PROCEDURE pls_creden_prestador_pck.gerar_alerta_evento ( nr_seq_credenciamento_p pls_creden_prestador.nr_sequencia%type, ie_tipo_processo_p pls_regra_geracao_evento.ie_tipo_processo_cred%type, nm_usuario_p usuario.nm_usuario%type, ie_commit_p text) AS $body$
DECLARE
...
C01 CURSOR FOR
	...
	and	a.ie_situacao = 'A'
	and	a.ie_evento_disparo = 16
	and	a.ie_tipo_processo_cred = ie_tipo_processo_p;
...
if (nr_seq_wsuite_usuario_w IS NOT NULL AND nr_seq_wsuite_usuario_w::text <> '') then
	for c01_w in C01 loop
		begin
		ds_titulo_w	:= replace(c01_w.ds_titulo,'@NR_CREDENCIAMENTO',nr_seq_credenciamento_p);
		ds_mensagem_w	:= replace(c01_w.ds_mensagem,'@NR_CREDENCIAMENTO',nr_seq_credenciamento_p);
		
		nr_seq_evento_controle_w := pls_creden_prestador_pck.gerar_evento_controle(c01_w.nr_seq_tipo_evento, ds_titulo_w, ds_mensagem_w, nm_usuario_p, nr_seq_evento_controle_w);
		...
		for c02_w in C02(c01_w.nr_seq_evento_mens ) loop
			...
			if (c02_w.ie_forma_envio = 'NTF') then
				...
				CALL CALL pls_creden_prestador_pck.gerar_evento_controle_dest(nr_seq_evento_controle_w, c02_w.ie_forma_envio, c02_w.ie_tipo_pessoa_dest, null, ds_mensagem_w, nm_usuario_p);
				...
				if (ds_email_w IS NOT NULL AND ds_email_w::text <> '') then
					...
					insert	into	pls_email(	nr_sequencia, cd_estabelecimento, nm_usuario_nrec,
							...
							nr_seq_credenciamento, ds_assunto, ds_mensagem )
						values (nextval('pls_email_seq'), cd_estabelecimento_w, nm_usuario_p,
							...
							nr_seq_credenciamento_p, ds_titulo_w, ds_mensagem_w );
				...
				for c03_w in C03(c02_w.nr_sequencia) loop
					...
					for c04_w in C04(c03_w.nm_usuario_destino) loop
						begin
						CALL CALL pls_creden_prestador_pck.gerar_evento_controle_dest(nr_seq_evento_controle_w, c02_w.ie_forma_envio, c02_w.ie_tipo_pessoa_dest, c04_w.nm_destino, ds_mensagem_w, nm_usuario_p);
						end;
					end loop; --C04
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_creden_prestador_pck.gerar_alerta_evento ( nr_seq_credenciamento_p pls_creden_prestador.nr_sequencia%type, ie_tipo_processo_p pls_regra_geracao_evento.ie_tipo_processo_cred%type, nm_usuario_p usuario.nm_usuario%type, ie_commit_p text) FROM PUBLIC;
