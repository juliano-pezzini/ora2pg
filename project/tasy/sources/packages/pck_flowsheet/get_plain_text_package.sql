...
CREATE OR REPLACE FUNCTION pck_flowsheet.get_plain_text (ds_tabela_p text, ds_campo_long_p text, ds_campo_chave_p text, ds_valor_chave_p bigint) RETURNS varchar AS $body$
DECLARE
...
BEGIN
    ...
    if (clob_w IS NOT NULL AND clob_w::text <> '') then
      ...
      if substr(clob_w, 1, 1) = '<' then
        ...
        clob_w := replace(clob_tmp_w, chr(38)||'#x200B;', '');
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION pck_flowsheet.get_plain_text (ds_tabela_p text, ds_campo_long_p text, ds_campo_chave_p text, ds_valor_chave_p bigint) FROM PUBLIC;
