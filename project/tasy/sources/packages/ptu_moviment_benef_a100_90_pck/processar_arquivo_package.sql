...
CREATE OR REPLACE PROCEDURE ptu_moviment_benef_a100_90_pck.processar_arquivo ( nr_seq_lote_p bigint, cd_unimed_destino_p ptu_mov_benef_destino.cd_unimed_destino%type, arq_texto_p utl_file.file_type) AS $body$
DECLARE
...
BEGIN
	PERFORM set_config('ptu_moviment_benef_a100_90_pck.nr_sequencial_arq_w', 0, false);
	open C01;
	loop
	fetch C01 into
			nr_seq_intercabio_w,
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		if (tp_registro_w = 1) then
			...
			ds_conteudo_w	:= lpad(cd_unimed_destino_w,4,0) || lpad(cd_unimed_origem_w,4,0) || to_char(dt_geracao_w,'YYYYMMDD') ||
					...
					'16';
			CALL CALL CALL CALL CALL CALL ptu_moviment_benef_a100_90_pck.incluir_linha_arq('101',ds_conteudo_w,arq_texto_p);
			...
			ds_conteudo_w	:= lpad(ds_reservado_w,4,' ') || lpad(cd_filial_w,3,0) || rpad(nm_empr_comp_w,40,' ') ||
					...
					lpad(nr_fax_w,9,'0') || lpad(nr_endereco_empresa_w,6,'0') || rpad(nm_empr_abrev_w,30,' ');
			CALL CALL CALL CALL CALL CALL ptu_moviment_benef_a100_90_pck.incluir_linha_arq('102',ds_conteudo_w,arq_texto_p);
			...
			CALL CALL CALL CALL CALL CALL ptu_moviment_benef_a100_90_pck.incluir_linha_arq('103',ds_conteudo_w,arq_texto_p);
			...
			CALL CALL CALL CALL CALL CALL ptu_moviment_benef_a100_90_pck.incluir_linha_arq('104',ds_conteudo_w,arq_texto_p);
			...
			CALL CALL CALL CALL CALL CALL ptu_moviment_benef_a100_90_pck.incluir_linha_arq('105',ds_conteudo_w,arq_texto_p);
			...
			CALL CALL CALL CALL CALL CALL ptu_moviment_benef_a100_90_pck.incluir_linha_arq('106',ds_conteudo_w,arq_texto_p);
			...
			CALL CALL CALL CALL CALL CALL ptu_moviment_benef_a100_90_pck.incluir_linha_arq('107',ds_conteudo_w,arq_texto_p);
			...
			CALL CALL CALL CALL CALL CALL ptu_moviment_benef_a100_90_pck.incluir_linha_arq('108',ds_conteudo_w,arq_texto_p);
			...
			CALL CALL CALL CALL CALL CALL ptu_moviment_benef_a100_90_pck.incluir_linha_arq('109',ds_conteudo_w,arq_texto_p);
	...
	current_setting('ptu_moviment_benef_a100_90_pck.ds_arquivo_w')::text := pls_hash_ptu_pck.obter_hash_txt(current_setting('ptu_moviment_benef_a100_90_pck.ds_arquivo_w')::text);
	ds_conteudo_w		:=	lpad(ds_hash_w,32,' ');
	CALL CALL CALL CALL CALL CALL ptu_moviment_benef_a100_90_pck.incluir_linha_arq('998',ds_conteudo_w,arq_texto_p);

	update 	ptu_intercambio
	set 	ds_hash_a100	= ds_hash_w,
		nm_arquivo 	= get_nome_arquivo
	where 	nr_sequencia 	= ptu_moviment_benef_a100_90_pck.get_cd_intercambio();
	commit;
	end;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_moviment_benef_a100_90_pck.processar_arquivo ( nr_seq_lote_p bigint, cd_unimed_destino_p ptu_mov_benef_destino.cd_unimed_destino%type, arq_texto_p utl_file.file_type) FROM PUBLIC;
