...
CREATE OR REPLACE PROCEDURE gerar_resumo_oftalmo_2_pkg.gerar_resumo_oftalmo_2 ( cd_pessoa_fisica_p text, nm_usuario_p text, ds_fundoscopia_p text) AS $body$
DECLARE
...
union all
  ...
  where  nr_atendimento  = current_setting('gerar_resumo_oftalmo_2_pkg.nr_atendimento_w')::bigint 
  and    ie_situacao    <> 'I';
  ...
  and  b.nr_atendimento  = current_setting('gerar_resumo_oftalmo_2_pkg.nr_atendimento_w')::bigint 
  ...
  where  a.nr_atendimento  = current_setting('gerar_resumo_oftalmo_2_pkg.nr_atendimento_w')::bigint 
  and  coalesce(a.cd_motivo_exc_conta::text, '') = '' 
  ...
  C08 CURSOR FOR 
    ...
    and  a.nr_atendimento  = current_setting('gerar_resumo_oftalmo_2_pkg.nr_atendimento_w')::bigint 
    order by 1;
...
WHERE a.nr_seq_lente = b.nr_sequencia and a.nr_seq_cons_med = h.nr_sequencia and h.nr_atendimento = current_setting('gerar_resumo_oftalmo_2_pkg.nr_atendimento_w')::bigint      order by a.dt_registro desc;
...
BEGIN 
  PERFORM set_config('gerar_resumo_oftalmo_2_pkg.cd_pessoa_fisica_w', cd_pessoa_fisica_p, false);
  PERFORM set_config('gerar_resumo_oftalmo_2_pkg.nm_usuario_w', nm_usuario_p, false);
  ...
  from  pessoa_fisica 
  where  cd_pessoa_fisica  = current_setting('gerar_resumo_oftalmo_2_pkg.cd_pessoa_fisica_w')::varchar(10);
  ...
  /*if  (ds_alergia_w is not null) then 
    gerar_resumo_oftalmo_2_pkg.addresumo('ALERGIAS' ||ds_quebra_w); 
    gerar_resumo_oftalmo_2_pkg.addresumo(ds_alergia_w ||ds_quebra_w); 
    ds_alergia_w := null; 
  end if;*/
  ...
  loop 
    fetch C01 into 
      ds_tipo_w, 
      current_setting('gerar_resumo_oftalmo_2_pkg.nr_atendimento_w')::bigint, 
      current_setting('gerar_resumo_oftalmo_2_pkg.dt_entrada_w')::timestamp, 
      ds_classif_atend_w, 
      ds_idade_w, 
      ds_convenio_w, 
      nm_medico_atend_w, 
    ...
    loop 
      ...
      CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo(ds_consulta_w);
      ds_consulta_w := '';
 
    end loop;
    close C09;
 
        CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo('*************************************************************'||ds_quebra_w);
    ...
    if (ds_tipo_w = wheb_mensagem_pck.get_texto(307144)) then -- Atendimento 
      ...
      if (to_char(current_setting('gerar_resumo_oftalmo_2_pkg.dt_entrada_w')::timestamp,'dd/mm/yyyy') = '01/01/1998') then 
        CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo(' || ' || wheb_mensagem_pck.get_texto(307152) || ': '||ds_idade_w ||' || ' ||RPAD(nm_medico_atend_w,40,' ') || ds_quebra_w);
      else 
        CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo(wheb_mensagem_pck.get_texto(307155) || ': '|| to_char(current_setting('gerar_resumo_oftalmo_2_pkg.dt_entrada_w')::timestamp,'dd/mm/yyyy') || ' || ' || wheb_mensagem_pck.get_texto(307152) || ': '||ds_idade_w ||' || ' ||RPAD(nm_medico_atend_w,40,' ') || 
...
ds_quebra_w);
      ...
      /*if  (to_char(dt_entrada_w,'dd/mm/yyyy') = '01/01/1998') then 
        gerar_resumo_oftalmo_2_pkg.addresumo('IDADE: '||ds_idade_w || ds_quebra_w); 
      else 
        gerar_resumo_oftalmo_2_pkg.addresumo('DATA: '|| to_char(dt_entrada_w,'dd/mm/yyyy') || ' IDADE: '||ds_idade_w || ds_quebra_w); 
      end if;*/
 
 
      CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo('- '||RPAD(coalesce(ds_classif_atend_w,' '),8,' ')||'.'||ds_quebra_w||ds_quebra_w);
      open C10;
      loop 
      fetch C10 into 
      ...
      EXIT WHEN NOT FOUND; /* apply on C10 */
        begin 
        CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo(wheb_mensagem_pck.get_texto(307525) || ': '||substr(obter_nome_pf(cd_medico_executor_w),1,80) || ds_quebra_w);
        end;
      end loop;
      close C10;
...
||ds_diagnostico_w||ds_quebra_w;
      ...
      loop 
        ...
        if (ie_tem_laudo_w = 'N') then 
          ie_tem_laudo_w  := 'S';
          CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo(wheb_mensagem_pck.get_texto(307537) ||ds_quebra_w);
        end if;
        CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo(' ' || wheb_mensagem_pck.get_texto(307539) || ': '||nm_medico_resp_w||ds_quebra_w);
        CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo(ds_titulo_laudo_w ||ds_quebra_w);
        ...
        CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.gravar_resumo_banco(nr_seq_laudo_w);
...
ds_procedimento_w || ds_quebra_w;
      ...
      if (ds_resumo_aval_w IS NOT NULL AND ds_resumo_aval_w::text <> '') then 
        CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo(ds_resumo_aval_w || ds_quebra_w);
      end if;
...
--'||ds_proced_agenda_w||ds_quebra_w;                                                                ---O.S. 487586 _ABMACIEL 
      ...
      if (ds_consulta_w IS NOT NULL AND ds_consulta_w::text <> '') then 
        begin 
        /*gerar_resumo_oftalmo_2_pkg.addresumo('CONSULTA'||ds_quebra_w);*/
 
        CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo(ds_consulta_w || ds_quebra_w);
        ds_consulta_w  := null;
        end;
      end if;
      ...
      if (ds_diagnostico_long_w IS NOT NULL AND ds_diagnostico_long_w::text <> '') then 
        begin 
        CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo(wheb_mensagem_pck.get_texto(307206)||ds_quebra_w);
        CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo(ds_diagnostico_long_w || ds_quebra_w);
        end;
      end if;
 
      if (ds_cirurgia_long_w IS NOT NULL AND ds_cirurgia_long_w::text <> '') then 
        begin 
        CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo(wheb_mensagem_pck.get_texto(307207)||ds_quebra_w);
        CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo(ds_cirurgia_long_w || ds_quebra_w);
        end;
      end if;
 
      if (ds_exames_long_w IS NOT NULL AND ds_exames_long_w::text <> '') then 
        --gerar_resumo_oftalmo_2_pkg.addresumo('EXAMES'||ds_quebra_w);                       --O.S. 487586 _ABMACIEL 
        CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo(ds_exames_long_w || ds_quebra_w);
      end if;
			 
			if (ds_exame_long_w IS NOT NULL AND ds_exame_long_w::text <> '') then	 
				begin 
				CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo(wheb_mensagem_pck.get_texto(325391)||ds_enter_w);
				CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.addresumo(ds_exame_long_w || ds_quebra_w);
				end;
			end if;
      ...
      ds_consulta_w  := LPAD(wheb_mensagem_pck.get_texto(307542) || ': ',20,' ') || to_char(current_setting('gerar_resumo_oftalmo_2_pkg.dt_entrada_w')::timestamp,'dd/mm/yyyy') ||ds_quebra_w;
  ...
  CALL CALL CALL CALL gerar_resumo_oftalmo_2_pkg.gravar_resumo_banco(null);
  PERFORM set_config('gerar_resumo_oftalmo_2_pkg.ds_resumo_w', null, false);
  END;

$body$
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE gerar_resumo_oftalmo_2_pkg.gerar_resumo_oftalmo_2 ( cd_pessoa_fisica_p text, nm_usuario_p text, ds_fundoscopia_p text) FROM PUBLIC;
