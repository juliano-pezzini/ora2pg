...
CREATE OR REPLACE PROCEDURE pls_pp_filtro_prod_medica_pck.vincula_item_lote_producao ( nr_id_transacao_p pls_pp_rp_cta_selecao.nr_id_transacao%type, nr_seq_lote_p pls_pp_lote.nr_sequencia%type, nr_seq_regra_periodo_p pls_pp_lote.nr_seq_regra_periodo%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE

					
tb_seq_conta_w		pls_util_cta_pck.t_number_table;
tb_seq_resumo_w		pls_util_cta_pck.t_number_table;
ie_tipo_registro_w	pls_pp_rp_cta_selecao.ie_tipo_registro%type;
...
c_proc_mat CURSOR(	nr_id_transacao_pc	pls_pp_rp_cta_selecao.nr_id_transacao%type,
			nr_seq_filtro_pc	pls_pp_rp_cta_selecao.nr_seq_filtro%type,
			ie_tipo_registro_pc	pls_pp_rp_cta_selecao.ie_tipo_registro%type) FOR
	SELECT	distinct a.nr_seq_conta,
		a.nr_seq_resumo
	from	pls_pp_rp_cta_selecao a
...
for r_c_filtro_w in c_filtro(nr_seq_lote_p, nr_seq_regra_periodo_p) loop
	...
	loop
		fetch c_proc_mat bulk collect into tb_seq_conta_w, tb_seq_resumo_w
		...
		forall i in tb_seq_conta_w.first..tb_seq_conta_w.last
			...
			where	a.nr_seq_conta = tb_seq_conta_w(i)
			and	a.nr_sequencia = tb_seq_resumo_w(i);
		commit;
	end loop;
	close c_proc_mat;
end loop;

-- limpa as variaveis utilizadas acima
...
-- essa tabela contera todos os itens que fazem parte do lote, sendo producao ou qualquer ocorrencia financeira
open c_pp_item(nr_seq_lote_p);
loop
	fetch c_pp_item bulk collect into 	tb_seq_prestador_w, tb_seq_evento_w, tb_seq_conta_w,
						tb_seq_resumo_w, tb_vl_liberado_w, tb_vl_glosa_w,
						tb_vl_liquido_w, tb_dt_item_w, tb_ie_tipo_contrat_w
	limit pls_util_pck.qt_registro_transacao_w;
	exit when tb_seq_conta_w.count = 0;
	...
	forall i in tb_seq_conta_w.first..tb_seq_conta_w.last
		insert into pls_pp_item_lote(
			...
			nr_seq_evento, nr_seq_conta, nr_seq_resumo,
			...
			tb_seq_evento_w(i), tb_seq_conta_w(i), tb_seq_resumo_w(i),
			'1', tb_vl_liberado_w(i), tb_vl_glosa_w(i),
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_pp_filtro_prod_medica_pck.vincula_item_lote_producao ( nr_id_transacao_p pls_pp_rp_cta_selecao.nr_id_transacao%type, nr_seq_lote_p pls_pp_lote.nr_sequencia%type, nr_seq_regra_periodo_p pls_pp_lote.nr_seq_regra_periodo%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
