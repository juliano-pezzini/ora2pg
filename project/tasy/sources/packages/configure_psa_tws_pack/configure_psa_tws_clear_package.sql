...
CREATE OR REPLACE PROCEDURE configure_psa_tws_pack.configure_psa_tws_clear ( u_nm_client client.nm_client%type, u_nm_application application.nm_application%type ) AS $body$
BEGIN
  ...
  where id_role in (SELECT a.id from role a
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE configure_psa_tws_pack.configure_psa_tws_clear ( u_nm_client client.nm_client%type, u_nm_application application.nm_application%type ) FROM PUBLIC;
