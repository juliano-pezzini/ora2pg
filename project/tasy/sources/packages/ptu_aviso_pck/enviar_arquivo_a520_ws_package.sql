...
CREATE OR REPLACE PROCEDURE ptu_aviso_pck.enviar_arquivo_a520_ws ( nr_seq_arquivo_p ptu_aviso_arquivo.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
ds_resposta_completa_w	varchar(32647);
ds_parametros_w		varchar(100);
ds_webservice_envio_w	pls_end_webservice_a520.ds_webservice_envio%type;
ds_servlet_w		pls_end_webservice_a520.ds_servlet%type;
ds_erro_w		ptu_fat_baixa_interc.ds_falha%type;

ie_status_w		ptu_aviso_arquivo.ie_status%type;
...
BEGIN
	...
	begin
		...
		if (ie_status_w in ('1', '4')) then
			...
			CALL ptu_aviso_pck.alterar_status_arq_a520(nr_seq_arquivo_p, '2', nm_usuario_p);
			...
			ds_servlet_w		:= ptu_aviso_pck.pls_obter_dados_ws_a520('SV?');
			ds_webservice_envio_w	:= ptu_aviso_pck.pls_obter_dados_ws_a520('CP');
			...
			if (ds_resposta_completa_w = 'OK') then
			
				CALL ptu_aviso_pck.alterar_status_arq_a520(nr_seq_arquivo_p, '3', nm_usuario_p);
				
			else	
			
				CALL ptu_aviso_pck.alterar_status_arq_a520(nr_seq_arquivo_p, '4', nm_usuario_p);
				CALL ptu_inserir_log_arq_cobr_ws(nr_seq_arquivo_p, null, 'Erro:'||substr(ds_resposta_completa_w, 1, 4000), nm_usuario_p);
			end if;
	...
	exception
		...
		when others then
			...
			CALL ptu_inserir_log_arq_cobr_ws(nr_seq_arquivo_p, null, 'Erro:'||ds_erro_w, nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_aviso_pck.enviar_arquivo_a520_ws ( nr_seq_arquivo_p ptu_aviso_arquivo.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
