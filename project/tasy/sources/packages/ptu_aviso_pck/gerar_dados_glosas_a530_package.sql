...
-- Gerar dados de glosas do A530



CREATE OR REPLACE PROCEDURE ptu_aviso_pck.gerar_dados_glosas_a530 ( nr_seq_arquivo_p ptu_aviso_arquivo.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
c01 CURSOR( nr_seq_arquivo_pc	ptu_aviso_arquivo.nr_sequencia%type ) FOR
	SELECT	aa.nr_sequencia nr_seq_arquivo,
		...
		CASE WHEN coalesce(c.ie_glosa::text, '') = '' THEN pls_obter_se_conta_glosada(c.nr_sequencia)  ELSE c.ie_glosa END  ie_glosa
...
for r_c01_w in c01( nr_seq_arquivo_p ) loop
	if (r_c01_w.ie_glosa = 'S') then -- Somente se for glosa total
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_aviso_pck.gerar_dados_glosas_a530 ( nr_seq_arquivo_p ptu_aviso_arquivo.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
