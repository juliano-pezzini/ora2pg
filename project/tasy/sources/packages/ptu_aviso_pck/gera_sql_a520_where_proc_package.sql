...
CREATE OR REPLACE FUNCTION ptu_aviso_pck.gera_sql_a520_where_proc ( dados_a520_p dados_lote_a520_t, alias_p alias_a520_t, ie_join_proc_p text, dados_gerais_a520_p dados_gerais_a520_t, dados_bind_p INOUT sql_pck.t_dado_bind) AS $body$
DECLARE

/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade:	Gera o SQL, parte do WHERE relacionada ao procedimento.

		Existe o parametro ie_join_proc_p, que se estiver marcado como N, sera considerado o where
		com base na conta, senao sera considerado que a query principal ja esta a nivel de procedimento
...
proc_w 		alias_p.procedimento%type;
...
select	max((	select	count(1)
		from	ptu_regra_aviso_cob_proc	x
...
if (ie_join_proc_p = 'N') then
	...
	if (qt_ie_status_w > 0) then
		...
		ds_sql_w := ds_sql_w||ds_and_w||proc_w||'.ie_status	in ('||in_ie_status_w||') ' || pls_util_pck.enter_w;
		ds_and_w := 'and	';
	end if;
	
	if (qt_ie_tipo_despesa_w > 0) then
	
		ds_sql_w := ds_sql_w||ds_and_w||proc_w||'.ie_tipo_despesa	in ('||in_ie_tipo_despesa_w||') ' || pls_util_pck.enter_w;
		ds_and_w := 'and	';
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION ptu_aviso_pck.gera_sql_a520_where_proc ( dados_a520_p dados_lote_a520_t, alias_p alias_a520_t, ie_join_proc_p text, dados_gerais_a520_p dados_gerais_a520_t, dados_bind_p INOUT sql_pck.t_dado_bind) FROM PUBLIC;
