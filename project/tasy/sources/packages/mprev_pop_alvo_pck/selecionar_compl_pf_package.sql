...
CREATE OR REPLACE PROCEDURE mprev_pop_alvo_pck.selecionar_compl_pf ( nr_seq_regra_cubo_p bigint, nr_seq_populacao_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
BEGIN
	...
	PERFORM set_config('mprev_pop_alvo_pck.ds_select_w', ' select	benef.nr_sequencia ' || current_setting('mprev_pop_alvo_pck.enter_w')::pls_util_pck.enter_w%type ||
				...
				'		mprev_compl_pessoa_cubo compl ' || current_setting('mprev_pop_alvo_pck.enter_w')::pls_util_pck.enter_w%type ||
				' where 	benef.nr_seq_pessoa_cubo = pessoa.nr_sequencia ' || current_setting('mprev_pop_alvo_pck.enter_w')::pls_util_pck.enter_w%type ||
				' and		pessoa.nr_sequencia = compl.nr_seq_pessoa_cubo ', false);
	...
	if (nr_seq_regra_cubo_p IS NOT NULL AND nr_seq_regra_cubo_p::text <> '')then
      ...
      tabela_regra_compl_pf_w.sg_estado	:= r_c_regra_compl.sg_estado;
			...
			PERFORM set_config('mprev_pop_alvo_pck.ds_restricao_w', mprev_pop_alvo_pck.obter_restricao_compl_pf(tabela_regra_compl_pf_w), false);
			...
			loop
				...
				forall i in tb_pop_alvo_sel_compl_pf_w.first .. tb_pop_alvo_sel_compl_pf_w.last
					...
					values (nextval('mprev_pop_alvo_selecao_seq'), nm_usuario_p, clock_timestamp(),
						...
						tabela_regra_compl_pf_w.ie_incluir, 'HDM_REGRA_CUBO_COMPL', 6);
				commit;
			end loop;
			close cursor_w;
		end loop;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE mprev_pop_alvo_pck.selecionar_compl_pf ( nr_seq_regra_cubo_p bigint, nr_seq_populacao_p bigint, nm_usuario_p text) FROM PUBLIC;
