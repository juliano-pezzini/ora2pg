...
CREATE OR REPLACE PROCEDURE mprev_pop_alvo_pck.selecionar_diag ( nr_seq_regra_cubo_p bigint, nr_seq_populacao_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
BEGIN
	...
	if (nr_seq_regra_cubo_p IS NOT NULL AND nr_seq_regra_cubo_p::text <> '')then
		for r_c_regra_diag in c_regra_diag(nr_seq_regra_cubo_p) loop
			...
			PERFORM set_config('mprev_pop_alvo_pck.ds_restricao_w', mprev_pop_alvo_pck.obter_restricao_diag(regra_diag_w), false);
			...
			loop
				...
				forall i in tb_pop_alvo_sel_diag_w.first .. tb_pop_alvo_sel_diag_w.last
					...
					values (nextval('mprev_pop_alvo_selecao_seq'), nm_usuario_p, clock_timestamp(),
						...
						regra_diag_w.ie_incluir, 'MPREV_REGRA_CUBO_DIAG', 2);
				commit;
			end loop;
			close cursor_w;
		end loop;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE mprev_pop_alvo_pck.selecionar_diag ( nr_seq_regra_cubo_p bigint, nr_seq_populacao_p bigint, nm_usuario_p text) FROM PUBLIC;
