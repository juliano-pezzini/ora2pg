...
CREATE OR REPLACE PROCEDURE mprev_pop_alvo_pck.gravar_pessoas_populacao ( nr_seq_populacao_alvo_p bigint, nm_usuario_p text, ds_select_comp_p text) AS $body$
DECLARE



	tb_pessoas_pop_alvo_w	mprev_pop_alvo_pck.table_pessoas_pop_alvo;
...
BEGIN
	...
	current_setting('mprev_pop_alvo_pck.bind_sql_w')::sql_pck.t_dado_bind := sql_pck.bind_variable(':nr_seq_controle_pc', nr_seq_controle_w, current_setting('mprev_pop_alvo_pck.bind_sql_w')::sql_pck.t_dado_bind);
	current_setting('mprev_pop_alvo_pck.bind_sql_w')::sql_pck.t_dado_bind := sql_pck.bind_variable(':nr_seq_populacao_alvo_pc', nr_seq_populacao_alvo_p, current_setting('mprev_pop_alvo_pck.bind_sql_w')::sql_pck.t_dado_bind);
	...
	loop
		CALL CALL mprev_pop_alvo_pck.limpar_tipo_tabelas('tb_pessoas_pop_alvo_w');
		...
		forall i in tb_pessoas_pop_alvo_w.cd_pessoa_fisica.first .. tb_pessoas_pop_alvo_w.cd_pessoa_fisica.last
			...
			values (nextval('mprev_pop_alvo_pessoa_seq'), clock_timestamp(), nm_usuario_p,
				...
				'N', upper(elimina_acentuacao(tb_pessoas_pop_alvo_w.nm_pessoa(i))), obter_cod_ibge_pf(tb_pessoas_pop_alvo_w.cd_pessoa_fisica(i)),
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE mprev_pop_alvo_pck.gravar_pessoas_populacao ( nr_seq_populacao_alvo_p bigint, nm_usuario_p text, ds_select_comp_p text) FROM PUBLIC;
