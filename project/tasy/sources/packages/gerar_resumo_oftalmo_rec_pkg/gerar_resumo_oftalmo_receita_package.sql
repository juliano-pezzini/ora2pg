...
CREATE OR REPLACE PROCEDURE gerar_resumo_oftalmo_rec_pkg.gerar_resumo_oftalmo_receita ( cd_pessoa_fisica_p text, nm_usuario_p text, ds_fundoscopia_p text) AS $body$
DECLARE
...
union all
  ...
  C02 CURSOR FOR 
  SELECT  to_char(dt_consulta,'dd/mm/yyyy hh:mi:ss'), 
  ...
  where  nr_atendimento  = current_setting('gerar_resumo_oftalmo_rec_pkg.nr_atendimento_w')::bigint 
  and    ie_situacao    <> 'I';
  ...
  and  b.nr_atendimento  = current_setting('gerar_resumo_oftalmo_rec_pkg.nr_atendimento_w')::bigint 
  ...
  where  a.nr_atendimento  = current_setting('gerar_resumo_oftalmo_rec_pkg.nr_atendimento_w')::bigint 
  and  coalesce(a.cd_motivo_exc_conta::text, '') = '' 
  ...
  C08 CURSOR FOR 
    ...
    and  a.nr_atendimento  = current_setting('gerar_resumo_oftalmo_rec_pkg.nr_atendimento_w')::bigint 
    order by 1;
...
WHERE a.nr_seq_lente = b.nr_sequencia and a.nr_seq_cons_med = h.nr_sequencia and h.nr_atendimento = current_setting('gerar_resumo_oftalmo_rec_pkg.nr_atendimento_w')::bigint      order by a.dt_registro desc;
  ...
  C14 CURSOR FOR 
	SELECT		substr(obter_nome_medico(a.cd_medico_cirurgiao,'N'),1,200) nm_medico_cirurgiao, 
			...
			a.nr_cirurgia 
	from		cirurgia a 
	where		a.nr_atendimento = nr_atend_cirurgia_w 
...
BEGIN 
  PERFORM set_config('gerar_resumo_oftalmo_rec_pkg.cd_pessoa_fisica_w', cd_pessoa_fisica_p, false);
  PERFORM set_config('gerar_resumo_oftalmo_rec_pkg.nm_usuario_w', nm_usuario_p, false);
  ...
  from  pessoa_fisica 
  where  cd_pessoa_fisica  = current_setting('gerar_resumo_oftalmo_rec_pkg.cd_pessoa_fisica_w')::varchar(10);
  ...
  /*if  (ds_alergia_w is not null) then 
    gerar_resumo_oftalmo_rec_pkg.addresumo('ALERGIAS' ||ds_quebra_w); 
    gerar_resumo_oftalmo_rec_pkg.addresumo(ds_alergia_w ||ds_quebra_w); 
    ds_alergia_w := null; 
  end if;*/
  ...
  loop 
    fetch C01 into   
      ds_tipo_w, 
      current_setting('gerar_resumo_oftalmo_rec_pkg.nr_atendimento_w')::bigint, 
      current_setting('gerar_resumo_oftalmo_rec_pkg.dt_entrada_w')::timestamp, 
      ds_classif_atend_w, 
      ds_idade_w, 
      ds_convenio_w, 
      nm_medico_atend_w, 
    ...
    loop 
      ...
      if (insere_cabecalho_w) then 
        ...
        ds_consulta_w    := ds_consulta_w ||LPAD(upper(wheb_mensagem_pck.get_texto(307499)) ,20,' ')||ds_quebra_w; -- LENTES DE CONTATO 
        insere_cabecalho_w  := False;
      end if;
       
      if (dt_registro_ant_w <> dt_consulta_w) then 
        ds_consulta_w    := ds_consulta_w ||LPAD(upper(wheb_mensagem_pck.get_texto(307501)) || ': ',20,' ')||dt_consulta_w||ds_quebra_w; -- DATA REGISTRO 
        dt_registro_ant_w  := dt_consulta_w;
      end if;
       
      if (ds_observacao_w IS NOT NULL AND ds_observacao_w::text <> '') then 
        ds_consulta_w  := ds_consulta_w ||LPAD(upper(wheb_mensagem_pck.get_texto(307502)) || ': ',20,' ')||ds_observacao_w||ds_quebra_w; -- OBSERVAÇÃO 
      end if;
      ...
      if (ie_lado_w IS NOT NULL AND ie_lado_w::text <> '') then 
        if (ie_lado_w = 'A') then 
          ...
          ds_lentes_w  :=  ds_lentes_w || ' ' || wheb_mensagem_pck.get_texto(308972) || ie_lado_w; -- O 
        end if;
      end if;
      ...
      if (qt_grau_esferico_w IS NOT NULL AND qt_grau_esferico_w::text <> '') then 
        if (length(ds_lentes_w || ' ' || wheb_mensagem_pck.get_texto(307507) || '=' || qt_grau_esferico_w) > 67) then -- Grau esférico 
          ds_consulta_w  :=  ds_consulta_w || trim(both ds_lentes_w) || ds_quebra_w;
          ds_lentes_w    :=  null;
        end if;
        if (position(',' in qt_grau_esferico_w) = 0) then 
          ds_lentes_w  :=  ds_lentes_w || ' ' || wheb_mensagem_pck.get_texto(307507) || '=' || qt_grau_esferico_w||',00'; -- Grau esférico 
        else
          ds_lentes_w  :=  ds_lentes_w || ' ' || wheb_mensagem_pck.get_texto(307507) || '=' || qt_grau_esferico_w; -- Grau esférico 
        end if;
      end if;
      if (qt_grau_cilindrico_w IS NOT NULL AND qt_grau_cilindrico_w::text <> '') then 
        if (length(ds_lentes_w || ' ' || wheb_mensagem_pck.get_texto(307512) || '=' || qt_grau_cilindrico_w) > 67) then -- Grau cilíndrico 
          ds_consulta_w  :=  ds_consulta_w || trim(both ds_lentes_w) || ds_quebra_w;
          ds_lentes_w    :=  null;
        end if;
        ds_lentes_w  :=  ds_lentes_w || ' ' || wheb_mensagem_pck.get_texto(307512) || '=' || qt_grau_cilindrico_w; -- Grau cilíndrico 
      end if;
      ...
      if (qt_diametro_w IS NOT NULL AND qt_diametro_w::text <> '') then 
        if (length(ds_lentes_w || ' ' || wheb_mensagem_pck.get_texto(307522) || '=' || qt_diametro_w) > 67) then -- Diâmetro 
          ds_consulta_w  :=  ds_consulta_w || trim(both ds_lentes_w) || ds_quebra_w;
          ds_lentes_w    :=  null;
        end if;
        ds_lentes_w  :=  ds_lentes_w || ' ' || wheb_mensagem_pck.get_texto(307522) || '=' || qt_diametro_w; -- Diâmetro 
      end if;
      ...
      if (qt_adicao_w IS NOT NULL AND qt_adicao_w::text <> '') then 
        if (length(ds_lentes_w || ' ' || wheb_mensagem_pck.get_texto(307185) || '=' || qt_adicao_w) > 67) then -- Adição 
          ds_consulta_w  :=  ds_consulta_w || trim(both ds_lentes_w) || ds_quebra_w;
          ds_lentes_w    :=  null;
        end if;
        ds_lentes_w  :=  ds_lentes_w || ' ' || wheb_mensagem_pck.get_texto(307185) || '=' || qt_adicao_w; -- Adição 
      end if;
      ...
      CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(ds_consulta_w);
      ds_consulta_w := '';
       
    end loop;
    close C09;
   
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo('*************************************************************'||ds_quebra_w);
    ...
    if (ds_tipo_w = wheb_mensagem_pck.get_texto(307144)) then -- Atendimento 
      ...
      if (to_char(current_setting('gerar_resumo_oftalmo_rec_pkg.dt_entrada_w')::timestamp,'dd/mm/yyyy') = '01/01/1998') then 
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(' || ' || wheb_mensagem_pck.get_texto(307152) || ': '||ds_idade_w ||' || ' ||RPAD(nm_medico_atend_w,40,' ') || ds_quebra_w); -- Idade 
      else
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(wheb_mensagem_pck.get_texto(307155) || ': '|| to_char(current_setting('gerar_resumo_oftalmo_rec_pkg.dt_entrada_w')::timestamp,'dd/mm/yyyy') || ' || ' || wheb_mensagem_pck.get_texto(307152) || ': '||ds_idade_w ||' || ' ||RPAD(nm_medico_atend_w,40,' ') || -- Data	--Idade 
...
ds_quebra_w);
      ...
      /*if  (to_char(dt_entrada_w,'dd/mm/yyyy') = '01/01/1998') then 
        gerar_resumo_oftalmo_rec_pkg.addresumo('IDADE: '||ds_idade_w || ds_quebra_w); 
      else 
        gerar_resumo_oftalmo_rec_pkg.addresumo('DATA: '|| to_char(dt_entrada_w,'dd/mm/yyyy') || ' IDADE: '||ds_idade_w || ds_quebra_w); 
      end if;*/
 
       
      CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo('- '||RPAD(coalesce(ds_classif_atend_w,' '),8,' ')||'.'||ds_quebra_w||ds_quebra_w);
      open C10;
      loop 
      fetch C10 into   
      ...
      EXIT WHEN NOT FOUND; /* apply on C10 */
        begin 
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(wheb_mensagem_pck.get_texto(307525) || ': '||substr(obter_nome_pf(cd_medico_executor_w),1,80) || ds_quebra_w); -- Cirurgião 
        end;
      end loop;
      close C10;
      ...
      loop 
        ...
        ds_consulta_w  := ds_consulta_w ||LPAD(wheb_mensagem_pck.get_texto(307164) || ': ',18,' ')  ||ds_queixa_paciente_w  ||ds_quebra_w; -- História Clínica 
        if (ds_av_sc_od_w IS NOT NULL AND ds_av_sc_od_w::text <> '') or (ds_av_sc_oe_w IS NOT NULL AND ds_av_sc_oe_w::text <> '') then 
          ds_consulta_w  := ds_consulta_w ||LPAD(wheb_mensagem_pck.get_texto(307166) || ' ' || wheb_mensagem_pck.get_texto(307169) || ': ',20,' ')||ds_av_sc_od_w ||ds_quebra_w; -- AC VISUAL SC OD 
          ds_consulta_w  := ds_consulta_w ||LPAD(wheb_mensagem_pck.get_texto(307171) || ': ',20,' ')||ds_av_sc_oe_w ||ds_quebra_w; -- OE 
        end if;
     
        if (ds_av_cc_od_w IS NOT NULL AND ds_av_cc_od_w::text <> '') or (ds_av_cc_oe_w IS NOT NULL AND ds_av_cc_oe_w::text <> '') then 
          ds_consulta_w  := ds_consulta_w ||LPAD(wheb_mensagem_pck.get_texto(307172) || ' ' || wheb_mensagem_pck.get_texto(307169) || ': ',20,' ')||ds_av_cc_od_w ||ds_quebra_w; -- AC VISUAL CC OD 
          ds_consulta_w  := ds_consulta_w ||LPAD(wheb_mensagem_pck.get_texto(307171) || ': ',20,' ')||ds_av_cc_oe_w ||ds_quebra_w; -- OE 
        end if;
   
        if (ds_meo_w IS NOT NULL AND ds_meo_w::text <> '') then 
          ds_consulta_w  := ds_consulta_w ||LPAD(wheb_mensagem_pck.get_texto(307173) || ': ',20,' ') ||ds_meo_w ||ds_quebra_w; -- MEO 
        end if;
...
null) or (ds_visao_receita_oe_w IS NOT NULL AND ds_visao_receita_oe_w::text <> '') or (vl_adicao_od_w IS NOT NULL AND vl_adicao_od_w::text <> '') or (vl_adicao_oe_w IS NOT NULL AND vl_adicao_oe_w::text <> '') then 
          ds_consulta_w  := ds_consulta_w ||LPAD(wheb_mensagem_pck.get_texto(307528) || ': ',20,' ')||ds_quebra_w; -- REFRAÇÃO 
        end if;
...
(vl_eixo_rd_od_w,3,' ') ||'   20/'||vl_visao_rd_od_w ||' '||ds_visao_rd_od_w ||ds_quebra_w;
          ds_consulta_w  := ds_consulta_w ||LPAD(wheb_mensagem_pck.get_texto(307171) || ':',20,' ')|| ds_impressao2_w ||' / '||LPAD('-'||vl_dc_rd_oe_w,7,' ') ||' / '||LPAD -- OE 
 
(vl_eixo_rd_oe_w,3,' ') ||'   20/'||vl_visao_rd_oe_w||' '||ds_visao_rd_oe_w ||ds_quebra_w;
...
(vl_eixo_re_od_w,3,' ') ||'   20/'||vl_visao_re_od_w||' '||ds_visao_re_od_w ||ds_quebra_w;
          ds_consulta_w  := ds_consulta_w ||LPAD(wheb_mensagem_pck.get_texto(307171) || ':',20,' ')|| ds_impressao2_w ||' / '||LPAD('-'||vl_dc_re_oe_w,7,' ') ||' / '||LPAD --OE 
 
(vl_eixo_re_oe_w,3,' ') ||'   20/'||vl_visao_re_oe_w||' '||ds_visao_re_oe_w ||ds_quebra_w;
...
;
          ...
          ds_consulta_w  := ds_consulta_w ||LPAD(upper(wheb_mensagem_pck.get_texto(307182)) || ' ' || wheb_mensagem_pck.get_texto(307169) || ':',20,' ')|| ds_impressao1_w ||' / '||LPAD('-'||vl_dc_receita_od_w,7,' ') ||' / '||LPAD(vl_eixo_receita_od_w,3,' ') ||'   20/'||vl_visao_receita_od_w||' '||ds_visao_receita_od_w ||ds_quebra_w; -- RECEITA OD 
          ds_consulta_w  := ds_consulta_w ||LPAD(wheb_mensagem_pck.get_texto(307171) || ':',20,' ')|| ds_impressao2_w ||' / '||LPAD('-'||vl_dc_receita_oe_w,7,' ') ||' / '||LPAD -- OE 
(vl_eixo_receita_oe_w,3,' ') ||'   20/'||vl_visao_receita_oe_w||' '||ds_visao_receita_oe_w ||ds_quebra_w;
        end if;
   
        if (vl_adicao_od_w IS NOT NULL AND vl_adicao_od_w::text <> '') or (vl_adicao_oe_w IS NOT NULL AND vl_adicao_oe_w::text <> '') then 
          ds_consulta_w  := ds_consulta_w ||LPAD(upper(wheb_mensagem_pck.get_texto(307185)) || ' ' || wheb_mensagem_pck.get_texto(307169) || ':',20,' ')||LPAD('+'||vl_adicao_od_w,7,' ') ||ds_quebra_w; -- ADIÇÃO OD 
          ds_consulta_w  := ds_consulta_w ||LPAD(wheb_mensagem_pck.get_texto(307171) || ':',20,' ')||LPAD('+'||vl_adicao_oe_w,7,' ') ||ds_quebra_w; -- OE 
        end if;
         
        if (ds_obs_receita_w IS NOT NULL AND ds_obs_receita_w::text <> '') then 
          ds_consulta_w  := ds_consulta_w ||LPAD(wheb_mensagem_pck.get_texto(307532) || ': ',20,' ')|| ds_obs_receita_w || ds_quebra_w; -- OBS RECEITA 
        end if;
         
        if (vl_pressao_od_w IS NOT NULL AND vl_pressao_od_w::text <> '') or (vl_pressao_oe_w IS NOT NULL AND vl_pressao_oe_w::text <> '') then 
          ds_consulta_w  := ds_consulta_w ||LPAD(wheb_mensagem_pck.get_texto(307187) || ': ',20,' ')||LPAD(wheb_mensagem_pck.get_texto(307169) || ': ',4,' ')||vl_pressao_od_w ||'  ' || wheb_mensagem_pck.get_texto(307171) || ': '||vl_pressao_oe_w||' ' || wheb_mensagem_pck.get_texto(307189) || ': '||hr_pressao_w ||ds_quebra_w;
															--PIO															--OD																				--OE 
				end if;
 
     
        if (ds_biomicroscopia_w IS NOT NULL AND ds_biomicroscopia_w::text <> '') then 
          ds_consulta_w  := ds_consulta_w ||ds_quebra_w||LPAD(wheb_mensagem_pck.get_texto(307202) || ': ',20,' ')||ds_biomicroscopia_w||ds_quebra_w; -- BIOMICROSCOPIA 
        end if;
       
     
        if (ds_fundoscopia_w IS NOT NULL AND ds_fundoscopia_w::text <> '') then 
          begin 
          ds_consulta_w  := ds_consulta_w ||ds_quebra_w||LPAD(coalesce(ds_fundoscopia_p,wheb_mensagem_pck.get_texto(307201))||': ',20,' ')||ds_fundoscopia_w  ||ds_quebra_w; -- FO 
          end;
        end if;
       
         
        if (ds_outros_exames_w IS NOT NULL AND ds_outros_exames_w::text <> '') then 
          ds_consulta_w  := ds_consulta_w ||ds_quebra_w||LPAD(upper(wheb_mensagem_pck.get_texto(307203)) || ': ',20,' ')||ds_outros_exames_w||ds_quebra_w; -- OUTROS EXAMES 
        end if;
   
        if (ds_hip_diag_w IS NOT NULL AND ds_hip_diag_w::text <> '') then 
          ds_consulta_w  := ds_consulta_w ||ds_quebra_w||LPAD(upper(wheb_mensagem_pck.get_texto(307534)) || ': ',20,' ')||ds_hip_diag_w||ds_quebra_w; -- HIPÓTESE DIAGNÓSTICA 
        end if;
   
        if (ds_conduta_w IS NOT NULL AND ds_conduta_w::text <> '') then 
          ds_consulta_w  := ds_consulta_w ||ds_quebra_w||LPAD(upper(wheb_mensagem_pck.get_texto(307204)) || ': ',20,' ')||ds_conduta_w||ds_quebra_w; -- CONDUTA 
        end if;
...
||ds_diagnostico_w||ds_quebra_w;
      ...
      loop 
        ...
        if (ie_tem_laudo_w = 'N') then 
          ie_tem_laudo_w  := 'S';
          CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(wheb_mensagem_pck.get_texto(307537) ||ds_quebra_w); -- LAUDO DE EXAMES 
        end if;
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(' ' || wheb_mensagem_pck.get_texto(307539) || ': '||nm_medico_resp_w||ds_quebra_w); -- Médico 
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(ds_titulo_laudo_w ||ds_quebra_w);
        /* Grava o conteúdo atual do resumo */
 
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.gravar_resumo_banco(null);
        /* Grava um novo registro referênciando o Laudo (LAUDO_PACIENTE) */
 
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.gravar_resumo_banco(nr_seq_laudo_w);
      ...
      loop 
        ...
        if (ds_olho_cirurgia_w = 'A') then 
          ds_olho_cirurgia_w  := wheb_mensagem_pck.get_texto(308949) || ': '; -- AO 
        elsif (ds_olho_cirurgia_w IS NOT NULL AND ds_olho_cirurgia_w::text <> '') then
          ds_olho_cirurgia_w  := wheb_mensagem_pck.get_texto(308972) ||ds_olho_cirurgia_w ||' - '; -- O 
        end if;
...
ds_procedimento_w || ds_quebra_w;
      ...
      EXIT WHEN NOT FOUND; /* apply on C11 */
        ...
        ds_resumo_aval_w  := ds_resumo_aval_w ||ds_resumo_aval_aux_w ||ds_quebra_w;        end;
      end loop;
      close C11;
       
       
      if (ds_resumo_aval_w IS NOT NULL AND ds_resumo_aval_w::text <> '') then 
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(ds_resumo_aval_w || ds_quebra_w);
      end if;
...
--'||ds_proced_agenda_w||ds_quebra_w;                                                                ---O.S. 487586 _ABMACIEL 
      ...
      if (ds_consulta_w IS NOT NULL AND ds_consulta_w::text <> '') then   
        begin 
        /*gerar_resumo_oftalmo_rec_pkg.addresumo('CONSULTA'||ds_quebra_w);*/
 
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(ds_consulta_w || ds_quebra_w);
        ds_consulta_w  := null;
        end;
      end if;
      ...
      if (ds_diagnostico_long_w IS NOT NULL AND ds_diagnostico_long_w::text <> '') then   
        begin 
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(wheb_mensagem_pck.get_texto(307206)||ds_quebra_w); -- Diagnósticos 
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(ds_diagnostico_long_w || ds_quebra_w);
        end;
      end if;
   
      if (ds_cirurgia_long_w IS NOT NULL AND ds_cirurgia_long_w::text <> '') then   
        begin 
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(wheb_mensagem_pck.get_texto(307207)||ds_quebra_w); -- Cirurgias 
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(ds_cirurgia_long_w || ds_quebra_w);
        end;
      end if;
       
      if (ds_exames_long_w IS NOT NULL AND ds_exames_long_w::text <> '') then 
        --gerar_resumo_oftalmo_rec_pkg.addresumo('EXAMES'||ds_quebra_w);                       --O.S. 487586 _ABMACIEL 
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(ds_exames_long_w || ds_quebra_w);
      end if;
       
      end;
    elsif (ds_tipo_w = wheb_mensagem_pck.get_texto(307145)) then -- Complemento 
 
      ds_consulta_w  := LPAD(wheb_mensagem_pck.get_texto(307542) || ': ',20,' ') || to_char(current_setting('gerar_resumo_oftalmo_rec_pkg.dt_entrada_w')::timestamp,'dd/mm/yyyy') ||ds_quebra_w; -- DATA COMPLEMENTO 
      ds_consulta_w  := ds_consulta_w ||LPAD(upper(wheb_mensagem_pck.get_texto(307145)) || ': ',20,' ')  ||ds_classif_atend_w ||ds_quebra_w; -- COMPLEMENTO 
      ds_consulta_w  := ds_consulta_w ||LPAD(upper(wheb_mensagem_pck.get_texto(307209)) || ': ',20,' ') || nm_usuario2_w || ds_quebra_w; -- USUÁRIO CADASTRO 
      CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(ds_consulta_w);
      ds_consulta_w  := '';
    end if;
    end;
  ...
  CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(ds_quebra_w||'*************************************************************'||ds_quebra_w);
  CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo('- ' || upper(wheb_mensagem_pck.get_texto(307545)) || ds_quebra_w); -- RECEITAS 
  open C12;
      loop 
      fetch C12 into   
      ...
      EXIT WHEN NOT FOUND; /* apply on C12 */
        begin 
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(ds_quebra_w || wheb_mensagem_pck.get_texto(307144) || ': ' || nr_atendimento_hosp_w || ds_quebra_w); -- Atendimento 
		CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(wheb_mensagem_pck.get_texto(307155) || ': ' || to_char(dt_receita_w,'dd/mm/yyyy hh24:mi:ss') || ds_quebra_w || ds_quebra_w); -- Data 
		...
		CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(ds_receita_w);
		ds_receita_w := '';
  ...
  CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(ds_quebra_w||'*************************************************************'||ds_quebra_w);
  CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo('- ' || upper(wheb_mensagem_pck.get_texto(307207)) || ds_quebra_w); -- CIRURGIAS 
  open C13;
      loop 
      fetch C13 into   
        nr_atend_cirurgia_w;
      EXIT WHEN NOT FOUND; /* apply on C13 */
        begin 
        CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(ds_quebra_w || wheb_mensagem_pck.get_texto(307144) || ': ' || nr_atend_cirurgia_w || ds_quebra_w); -- Atendimento 
		...
		open C14;
			...
			EXIT WHEN NOT FOUND; /* apply on c14 */
				begin 
				CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(ds_quebra_w||'- ' || wheb_mensagem_pck.get_texto(307882) || ': ' || nm_medico_cirurgiao_w || ' || ' || wheb_mensagem_pck.get_texto(307883) || ': ' || nm_medico_anestesista_w || ds_quebra_w); -- Médico cirurgião	-- Médico anestesista 
				CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo('- ' || wheb_mensagem_pck.get_texto(307884) || ': ' || ds_proc_principal_w || ds_quebra_w); -- Procedimento principal 
				...
				open C15;
					...
					EXIT WHEN NOT FOUND; /* apply on C15 */
						begin 
						CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo('- ' || wheb_mensagem_pck.get_texto(307155) || ': ' || to_char(dt_desc_cirurgica_w,'dd/mm/yyyy hh24:mi:ss') || ' || ' || nm_resp_desc_cirurgica_w || ' || ' || cd_doenca_cid_cirurgia_w || ds_quebra_w); -- Data 
						...
						select	ds_texto 
						into STRICT	ds_desc_cirurgica_w 
						from	tasy_conversao_rtf 
						where	nr_sequencia = nr_cirurgia_w;
 
						CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.addresumo(ds_quebra_w||ds_desc_cirurgica_w);
						ds_desc_cirurgica_w := '';
						end;
					end loop;
				close C15;
  ...
  CALL CALL CALL CALL gerar_resumo_oftalmo_rec_pkg.gravar_resumo_banco(null);
  PERFORM set_config('gerar_resumo_oftalmo_rec_pkg.ds_resumo_w', null, false);
  END;

$body$
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE gerar_resumo_oftalmo_rec_pkg.gerar_resumo_oftalmo_receita ( cd_pessoa_fisica_p text, nm_usuario_p text, ds_fundoscopia_p text) FROM PUBLIC;
