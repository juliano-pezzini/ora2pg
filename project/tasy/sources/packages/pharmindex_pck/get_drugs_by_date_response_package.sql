...
CREATE OR REPLACE PROCEDURE pharmindex_pck.get_drugs_by_date_response ( nr_sequencia_p bigint) AS $body$
DECLARE
...
c01 CURSOR FOR
	SELECT	*
	from	xmltable('/getDrugsByDateResult/item' passing getDrugsByDateResult_w columns
...
if (ie_status_w = 'E') then
	...
	select	a.getDrugsByDateResult
	into STRICT	getDrugsByDateResult_w
	from	xmltable(
			xmlnamespaces(
			'http://ws.pharmindex.pl/demo/' as "ns1",
			'http://schemas.xmlsoap.org/soap/envelope/' as "SOAP-ENV"),
			'SOAP-ENV:Envelope/SOAP-ENV:Body/ns1:getDrugsByDateResponse' passing xml_w columns
		getDrugsByDateResult xmltype path 'getDrugsByDateResult') a;
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		reg_integracao_w.nm_elemento		:= 'getDrugsByDateResult';
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pharmindex_pck.get_drugs_by_date_response ( nr_sequencia_p bigint) FROM PUBLIC;
