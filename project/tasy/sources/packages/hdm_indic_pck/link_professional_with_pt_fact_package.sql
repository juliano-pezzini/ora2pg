...
CREATE OR REPLACE PROCEDURE hdm_indic_pck.link_professional_with_pt_fact () AS $body$
BEGIN
		merge into hdm_indic_ft_partic_pf y
		using(	SELECT	a.nr_sequencia nr_seq_dimension,
			...
			and	y.dt_complete_date between pkg_date_utils.start_of(d.dt_inicio,'MONTH', 0) and pkg_date_utils.start_of(coalesce(d.dt_termino,d.dt_inicio),'MONTH', 0)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE hdm_indic_pck.link_professional_with_pt_fact () FROM PUBLIC;
