...
CREATE OR REPLACE PROCEDURE hdm_indic_pck.link_program_with_di_fact () AS $body$
BEGIN
		merge into hdm_indic_ft_clinic_ant_pr y
		using(	/* Program whit module */

			SELECT	a.nr_sequencia nr_seq_dimension,
				x.nr_sequencia nr_seq_fact
			FROM hdm_indic_dm_day y, mprev_programa_modulo pm, mprev_participante g, mprev_programa_partic e, mprev_prog_partic_modulo d, hdm_indic_dm_program a, hdm_indic_ft_clinic_ant x
...
union
			...
			SELECT	distinct
				...
				x.nr_sequencia nr_seq_fact
			FROM hdm_indic_dm_day y, mprev_participante g, mprev_programa_partic e, hdm_indic_dm_program a, hdm_indic_ft_clinic_ant x
LEFT OUTER JOIN hdm_indic_dm_day z ON (x.nr_seq_day_end = z.nr_sequencia)
WHERE y.dt_complete_date <= coalesce(e.dt_exclusao,y.dt_complete_date) and (z.dt_complete_date > e.dt_exclusao or coalesce(z.dt_complete_date::text, '') = '') and a.nr_seq_program = e.nr_seq_programa and a.nr_seq_module = 0 and y.nr_sequencia = x.nr_seq_day_start  and g.nr_sequencia	= e.nr_seq_participante and x.nr_dif_person	= g.cd_pessoa_fisica group by
		...
		using(	SELECT	0 nr_seq_dimension,
			...
			where	not exists (select	1
						from	hdm_indic_ft_clinic_ant_pr a
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE hdm_indic_pck.link_program_with_di_fact () FROM PUBLIC;
