...
CREATE OR REPLACE PROCEDURE hdm_indic_pck.link_program_with_hv_fact () AS $body$
BEGIN
		merge into hdm_indic_ft_habit_pr y
		using(	/* Program whit module */

			SELECT	a.nr_sequencia nr_seq_dimension,
				x.nr_sequencia nr_seq_fact
			FROM hdm_indic_dm_day y, mprev_programa_modulo pm, mprev_participante g, mprev_programa_partic e, mprev_prog_partic_modulo d, hdm_indic_dm_program a, hdm_indic_ft_habit x
...
union
			...
			SELECT	distinct
				...
				x.nr_sequencia nr_seq_fact
			FROM hdm_indic_dm_day y, mprev_participante g, mprev_programa_partic e, hdm_indic_dm_program a, hdm_indic_ft_habit x
LEFT OUTER JOIN hdm_indic_dm_day z ON (x.nr_seq_day_end = z.nr_sequencia)
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE hdm_indic_pck.link_program_with_hv_fact () FROM PUBLIC;
