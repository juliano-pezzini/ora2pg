...
CREATE OR REPLACE FUNCTION sla_dashboard_pck.reg_sla_gerencia_cliente_ref (dt_referencia_p timestamp, ie_tipo_sla_p text) RETURNS SETOF T_GERENCIA_SLA_CLIENTE AS $body$
DECLARE
...
--                                        ( 100 - dividir(((SUM(DECODE(ie_indicador, 3, 1, 0)) + SUM(DECODE(ie_indicador, 4, 1, 0))) * 100),(SUM(DECODE(ie_indicador, 1, 1, 0)) * 2)) ) pr_atingido,
                                        ...
                                        --( 100 - dividir(((SUM(sla_dashboard_pck.obter_se_estouro_sla(ie_indicador,3,sla_detail)) + SUM(sla_dashboard_pck.obter_se_estouro_sla(ie_indicador,4,sla_detail))) * 100),(SUM(DECODE(ie_indicador, 1, 1, 0)) * 2)) ) pr_atingido,

					(100 - (dividir((sum(sla_dashboard_pck.Obter_se_estouro_SLA(ie_indicador,3,sla_detail)) + sum(sla_dashboard_pck.Obter_se_estouro_SLA(ie_indicador,4,sla_detail))), (sum(CASE WHEN ie_indicador=1 THEN 1  ELSE 0 END ) * sla_dashboard_pck.obter_multiplicador_sla(sla_detail))) * 100)) pr_atingido,
					qt_target
                                FROM    sla_information_v3
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION sla_dashboard_pck.reg_sla_gerencia_cliente_ref (dt_referencia_p timestamp, ie_tipo_sla_p text) FROM PUBLIC;
