...
CREATE OR REPLACE FUNCTION sla_dashboard_pck.obter_sla_estouro_gerencia (ie_tipo_sla_p bigint,mes_ref_p bigint, ano_ref_p bigint, nr_seq_localizacao_p bigint) RETURNS SETOF T_SLA_ESTOURO_GERENCIA_TB AS $body$
DECLARE
...
BEGIN
    sql_w := 	'select a.nr_seq_localizacao,
		...
		where   a.nr_seq_localizacao = :nr_seq_localizacao
		and     (a.dono_estouro_resp is not null or a.dono_estouro_sol is not null)';
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION sla_dashboard_pck.obter_sla_estouro_gerencia (ie_tipo_sla_p bigint,mes_ref_p bigint, ano_ref_p bigint, nr_seq_localizacao_p bigint) FROM PUBLIC;
