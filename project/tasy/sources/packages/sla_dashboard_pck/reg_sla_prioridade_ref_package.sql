...
CREATE OR REPLACE FUNCTION sla_dashboard_pck.reg_sla_prioridade_ref (dt_referencia_p timestamp, ie_tipo_sla_p text) RETURNS SETOF T_PRIORIDADE_SLA AS $body$
DECLARE
...
and 	((CASE WHEN ie_indicador=4 THEN 1  ELSE 0 END  = 0) or (CASE WHEN ie_indicador=4 THEN 1  ELSE 0 END = 1 AND sla_detail_finish = CASE WHEN ie_tipo_sla_p='SWC' THEN '2'  ELSE '1' END ))
and 	((CASE WHEN ie_indicador=3 THEN 1  ELSE 0 END  = 0) or (CASE WHEN ie_indicador=4 THEN 1  ELSE 0 END = 1 AND sla_detail_start = CASE WHEN ie_tipo_sla_p='SWC' THEN '2'  ELSE '1' END ))
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION sla_dashboard_pck.reg_sla_prioridade_ref (dt_referencia_p timestamp, ie_tipo_sla_p text) FROM PUBLIC;
