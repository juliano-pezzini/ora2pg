...
CREATE OR REPLACE FUNCTION sql_pck.executa_sql_big_cursor ( ds_sql_p text, bind_sql_valor_p INOUT sql_pck.t_dado_bind) AS $body$
DECLARE
...
ds_sql_clob_w		varchar(32000); --Foi usado o varchar2 com 32 mil caracteres, pois na versão 10G do oracle não suporte o CLOB no execute immediate
...
begin
	EXECUTE 'begin open :cursor_w for ''' || replace(ds_sql_clob_w, '''', '''''') || ''' ' || ds_using_w || '; end; ' using in out cursor_w;
	-- limpeza de memória
	tb_bind_w.delete;
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION sql_pck.executa_sql_big_cursor ( ds_sql_p text, bind_sql_valor_p INOUT sql_pck.t_dado_bind) FROM PUBLIC;
