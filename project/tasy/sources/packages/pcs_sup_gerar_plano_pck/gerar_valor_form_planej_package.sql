...
CREATE OR REPLACE PROCEDURE pcs_sup_gerar_plano_pck.gerar_valor_form_planej ( nr_sequencia_p bigint, cd_material_p bigint, --ie_simular_p			varchar2,
 vl_retorno_p INOUT bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
BEGIN
	...
	/*if	(qt_dias_fim_mes_w < 0) then
		begin

		select	round(last_day(sysdate) - sysdate)
		into	qt_dias_fim_mes_w
		from	dual;
...
/*select	a.nr_sequencia,
		...
		a.ds_composicao
from	pcs_formulas a
where	ds_composicao is not null
and 	instr(ds_formula_orig_w,'#F') > 0; --nome da macro */
	if (ds_formula_orig_w IS NOT NULL AND ds_formula_orig_w::text <> '') then
		...
		ds_formula_orig_w	:= replace(ds_formula_orig_w,'DIAS',to_char(qt_dias_fim_mes_w));
		...
		if (substr(ds_formula_orig_w,1,2) = 'SE') then
			...
			while(ie_cond_verdadeira_w <> 'S') loop
				...
				--ie_simular_p,
							ie_cond_verdadeira_w := pcs_sup_gerar_plano_pck.obter_se_condicao_verdadeira(ds_informacao_w, cd_material_p, 
							ie_cond_verdadeira_w, cd_estabelecimento_p, nm_usuario_p);
	...
	--		if	(ie_simular_p = 'N') then
				--begin
				CALL CALL pcs_sup_gerar_plano_pck.gravar_valor_formula_planej(
					nr_sequencia_p,
					nm_formula_w,
					vl_retorno_atrib_w,
					cd_material_p,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pcs_sup_gerar_plano_pck.gerar_valor_form_planej ( nr_sequencia_p bigint, cd_material_p bigint,  vl_retorno_p INOUT bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
