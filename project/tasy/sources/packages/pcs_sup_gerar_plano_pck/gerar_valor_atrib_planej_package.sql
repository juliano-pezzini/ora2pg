...
CREATE OR REPLACE PROCEDURE pcs_sup_gerar_plano_pck.gerar_valor_atrib_planej ( nr_sequencia_p bigint, cd_material_p bigint, vl_retorno_p INOUT bigint, cd_estabelecimento_p bigint, cd_empresa_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
BEGIN
	...
	if (ds_sql_original_w IS NOT NULL AND ds_sql_original_w::text <> '') then
		...
		EXIT WHEN NOT FOUND; /* apply on c01 */
			...
			CALL CALL pcs_sup_gerar_plano_pck.gravar_valor_atrib_planej(	nm_atributo_w,
						vl_retorno_atrib_w,
						cd_material_p,
						cd_estabelecimento_w,
						nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pcs_sup_gerar_plano_pck.gerar_valor_atrib_planej ( nr_sequencia_p bigint, cd_material_p bigint, vl_retorno_p INOUT bigint, cd_estabelecimento_p bigint, cd_empresa_p bigint, nm_usuario_p text) FROM PUBLIC;
