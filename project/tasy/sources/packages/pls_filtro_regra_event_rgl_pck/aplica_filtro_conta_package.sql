...
if (nr_seq_filtro_p IS NOT NULL AND nr_seq_filtro_p::text <> '') then
	...
	for r_c_filtro_w in c_filtro(nr_seq_filtro_p) loop
		...
		begin
			...
			SELECT * FROM pls_filtro_regra_event_rgl_pck.obter_restricao_conta(	r_c_filtro_w.ie_tipo_guia, r_c_filtro_w.ie_tipo_guia_princ, r_c_filtro_w.nr_seq_tipo_atendimento, r_c_filtro_w.nr_seq_tipo_atend_princ, r_c_filtro_w.ie_origem_conta, r_c_filtro_w.ie_apresentacao, ds_campos_w, r_c_filtro_w.ie_regime_atend_princ, r_c_filtro_w.ie_regime_atendimento, valor_bind_w) INTO STRICT _ora2pg_r;
 r_c_filtro_w.ie_saude_ocupacional := _ora2pg_r.ds_campos_p; valor_bind_w := _ora2pg_r.valor_bind_p;
			...
			loop
				...
				CALL CALL CALL CALL CALL pls_filtro_regra_event_rgl_pck.gerencia_selecao_filtro(nr_id_transacao_p, nr_seq_filtro_p, ie_tipo_regra_p,
							ie_excecao_p, tb_seq_conta_rec_w, tb_seq_resumo_rec_w,
							tb_seq_conta_proc_rec_w, tb_seq_conta_mat_rec_w, tb_seq_selecao_w,
							'S');
			end loop;
			close cursor_w;
		exception
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_filtro_regra_event_rgl_pck.aplica_filtro_conta ( ie_considera_selecao_p INOUT boolean, qt_filtro_processado_p INOUT integer, ie_tipo_regra_p pls_pp_cta_combinada.ie_tipo_regra%type, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, ie_excecao_p pls_pp_cta_filtro.ie_excecao%type, nr_seq_filtro_p pls_pp_cta_filtro.nr_sequencia%type, dt_inicio_vigencia_p pls_pp_cta_combinada.dt_inicio_vigencia%type, dt_fim_vigencia_p pls_pp_cta_combinada.dt_fim_vigencia%type, nr_seq_analise_rec_p pls_analise_conta.nr_sequencia%type, nr_seq_prot_rec_p pls_rec_glosa_protocolo.nr_sequencia%type, nr_seq_cta_rec_p pls_rec_glosa_conta.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
