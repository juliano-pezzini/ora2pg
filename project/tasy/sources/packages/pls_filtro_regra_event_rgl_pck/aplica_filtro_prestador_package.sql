...
tb_seq_protocolo_rec_w	pls_util_cta_pck.t_number_table;
tb_seq_prest_pag_rec_w	pls_util_cta_pck.t_number_table;
...
if (nr_seq_filtro_p IS NOT NULL AND nr_seq_filtro_p::text <> '') then
	...
	for r_c_filtro_w in c_filtro(nr_seq_filtro_p) loop
		...
		begin
			...
			SELECT * FROM pls_filtro_regra_event_rgl_pck.obter_restricao_prestador(	r_c_filtro_w.nr_seq_prestador, r_c_filtro_w.cd_prestador_cod, valor_bind_w) INTO STRICT _ora2pg_r;
 ds_campos_w := _ora2pg_r.ds_campos_p; valor_bind_w := _ora2pg_r.valor_bind_p;
			...
			loop
				fetch cursor_w bulk collect into tb_seq_conta_rec_w, tb_seq_resumo_rec_w, tb_seq_conta_proc_rec_w,
								 ...
								 tb_seq_protocolo_rec_w, tb_seq_prest_pag_rec_w
				...
				tb_seq_resumo_rec_ger_w.delete;
				...
				for i in tb_seq_conta_rec_w.first..tb_seq_conta_rec_w.last loop
					...
					ie_registro_valido_w := pls_filtro_regra_event_rgl_pck.ie_reg_valido_fil_prest(	r_c_filtro_w.nr_seq_tipo_prestador,
												r_c_filtro_w.nr_seq_tipo_prest_prot,
												tb_dt_ref_rec_w(i),
												tb_seq_prest_pag_rec_w(i),
												tb_seq_protocolo_rec_w(i));
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_filtro_regra_event_rgl_pck.aplica_filtro_prestador ( ie_considera_selecao_p INOUT boolean, qt_filtro_processado_p INOUT integer, ie_tipo_regra_p pls_pp_cta_combinada.ie_tipo_regra%type, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, ie_excecao_p pls_pp_cta_filtro.ie_excecao%type, nr_seq_filtro_p pls_pp_cta_filtro.nr_sequencia%type, dt_inicio_vigencia_p pls_pp_cta_combinada.dt_inicio_vigencia%type, dt_fim_vigencia_p pls_pp_cta_combinada.dt_fim_vigencia%type, nr_seq_analise_rec_p pls_analise_conta.nr_sequencia%type, nr_seq_prot_rec_p pls_rec_glosa_protocolo.nr_sequencia%type, nr_seq_cta_rec_p pls_rec_glosa_conta.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
