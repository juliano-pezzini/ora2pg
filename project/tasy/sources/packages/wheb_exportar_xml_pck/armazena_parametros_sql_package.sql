...
CREATE OR REPLACE PROCEDURE wheb_exportar_xml_pck.armazena_parametros_sql (ds_comando_p text,ar_nm_parametros_sql_p INOUT myArray,ie_upper_param_p boolean default true) AS $body$
DECLARE
	...
	contador_w		bigint 	:= 1;
	caracteres_validos_w	varchar(80)  :='abcdefghijklmnopqrstuvxywzABCDEFGHIJKLMNOPRQSTUVXYWZ_0123456789';
	ie_valido_w		boolean;
	
BEGIN
		ds_comando_w := ds_comando_p;
		pos_w := position(':' in ds_comando_w);
		while( pos_w > 0 ) and ( contador_w < 3000 ) loop /*Variavel de controle para evitar LOOP*/
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE wheb_exportar_xml_pck.armazena_parametros_sql (ds_comando_p text,ar_nm_parametros_sql_p INOUT myArray,ie_upper_param_p boolean default true) FROM PUBLIC;
