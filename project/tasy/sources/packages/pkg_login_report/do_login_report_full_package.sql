...
CREATE OR REPLACE PROCEDURE pkg_login_report.do_login_report_full (dt_fromDate_p timestamp, dt_toDate_p timestamp) AS $body$
DECLARE
...
BEGIN
    ...
    /* Selects the login records to be processed by the first pass. */
     ...
     WITH acessos AS (SELECT pkg_login_report.normalize_date(a.dt_acesso, 15) + (14/1440) AS dt_acesso, pkg_login_report.normalize_date(a.dt_saida, 15) AS dt_saida FROM TASY_CONTROLE_LOGIN a
                       WHERE a.ds_aplicacao = 'TasySwing' AND TRUNC(a.dt_saida) >= startDate AND TRUNC(a.dt_acesso) <= endDate AND (a.dt_saida IS NOT NULL AND a.dt_saida::text <> ''))
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pkg_login_report.do_login_report_full (dt_fromDate_p timestamp, dt_toDate_p timestamp) FROM PUBLIC;
