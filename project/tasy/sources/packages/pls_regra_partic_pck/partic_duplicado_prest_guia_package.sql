...
CREATE OR REPLACE PROCEDURE pls_regra_partic_pck.partic_duplicado_prest_guia ( dados_val_partic_p pls_tipos_ocor_pck.dados_val_partic, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
c02 CURSOR(	nr_seq_segurado_pc	pls_conta.nr_seq_segurado%type,
	...
	SELECT	proc.nr_sequencia nr_seq_proc,
		...
		obter_nome_pf(partic.cd_medico) nm_medico,
...
union all
	...
	SELECT	proc.nr_sequencia nr_seq_proc,
		proc.nr_seq_conta,
		cta.cd_medico_executor cd_medico,
		cta.nr_seq_prestador_exec nr_seq_prestador,
		obter_nome_pf(cta.cd_medico_executor) nm_medico,
		pls_obter_dados_prestador(cta.nr_seq_prestador_exec, 'N') nm_prestador,
		gp.ds_grau_participacao,
		coalesce(proc.ie_glosa,'N') ie_glosa
	from	pls_conta cta,
		pls_conta_proc proc,
...
for r_c01_w in c01(nr_id_transacao_p) loop
	...
	case(dados_val_partic_p.ie_incidencia)
		...
		when 'GPDH' then
			...
			-- faz as verificacoes para o nr_seq_prestador antes, se for igual, continua a validacao dos participantes medicos
			...
			for r_c04_w in c04(	r_c01_w.nr_seq_segurado, r_c01_w.cd_guia_referencia,
				...
				ds_registro_atual_w := coalesce(to_char(r_c04_w.nr_seq_prestador),0); -- Caso tenha mais de um nr_seq_prestador nulo, valida o participante

				if	((coalesce(dados_val_partic_p.ie_glosa, 'N') = 'S') and (r_c04_w.ie_glosa = 'N')) or (coalesce(dados_val_partic_p.ie_glosa, 'N') = 'N') then
			...
			-- se os ultimos nr_seq_prestador sao iguais entao sera validado o participante medico

			if (qt_partic_igual_w > 1) then
			
			qt_partic_igual_w := 0;	
			
					-- faz as verificacoes pelo medico
		...
		when 'GPD' then	
			...
			-- Verifica se os prestadores sao iguais e caso sim, vai verifcar se o participante e duplicado		
			...
			for r_c05_w in c05(	r_c01_w.nr_seq_segurado, r_c01_w.cd_guia_referencia,
					...
					if	((coalesce(dados_val_partic_p.ie_glosa, 'N') = 'S') and (r_c05_w.ie_glosa = 'N')) or (coalesce(dados_val_partic_p.ie_glosa, 'N') = 'N') then
						-- para validar o participante usa prestador e medico juntos
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_regra_partic_pck.partic_duplicado_prest_guia ( dados_val_partic_p pls_tipos_ocor_pck.dados_val_partic, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
