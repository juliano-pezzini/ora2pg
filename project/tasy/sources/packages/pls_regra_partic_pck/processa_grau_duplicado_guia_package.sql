...
CREATE OR REPLACE PROCEDURE pls_regra_partic_pck.processa_grau_duplicado_guia ( dados_val_partic_p pls_tipos_ocor_pck.dados_val_partic, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
				...
				if	((coalesce(dados_val_partic_p.ie_glosa, 'N') = 'S') and (r_c02_w.ie_glosa = 'N')) or (coalesce(dados_val_partic_p.ie_glosa, 'N') = 'N') then
					...
					if (coalesce(nr_seq_grau_partic_ultimo_w, r_c02_w.nr_seq_grau_partic) = r_c02_w.nr_seq_grau_partic) then
						...
						if (r_c02_w.origem_grau_partic = 'P' ) then
							...
							into STRICT	qt_partic_grau_w
							from	pls_conta a
							where 	nr_seq_grau_partic = r_c02_w.nr_seq_grau_partic
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_regra_partic_pck.processa_grau_duplicado_guia ( dados_val_partic_p pls_tipos_ocor_pck.dados_val_partic, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
