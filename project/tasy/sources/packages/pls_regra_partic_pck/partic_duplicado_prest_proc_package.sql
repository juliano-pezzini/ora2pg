...
CREATE OR REPLACE PROCEDURE pls_regra_partic_pck.partic_duplicado_prest_proc ( nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type, dados_val_partic_p pls_tipos_ocor_pck.dados_val_partic) AS $body$
DECLARE
...
for r_c01_w in c01(nr_id_transacao_p) loop
	...
	-- aqui verifica o campo do prestador e caso duplicado, verifica se o participante e duplicado
	...
	for r_c02_w in c02(r_c01_w.nr_seq_conta_proc) loop
		...
		ds_registro_atual_w := r_c02_w.cd_medico;
			...
			if	((coalesce(dados_val_partic_p.ie_glosa, 'N') = 'S') and (coalesce(r_c02_w.ie_glosa,'N') = 'N')) or (coalesce(dados_val_partic_p.ie_glosa, 'N') = 'N') then
				-- para validar o participante usa prestador e medico juntos
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_regra_partic_pck.partic_duplicado_prest_proc ( nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type, dados_val_partic_p pls_tipos_ocor_pck.dados_val_partic) FROM PUBLIC;
