...
CREATE OR REPLACE PROCEDURE pls_regra_partic_pck.regra_partic_proc ( dados_val_partic_p pls_tipos_ocor_pck.dados_val_partic, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
union all
				...
				from	pls_grupo_partic_tm x
				where	x.nr_seq_regra = nr_seq_regra_pc
				and	x.nr_seq_prestador = cta.nr_seq_prestador_exec
				and	x.nr_seq_grau_partic = cta.nr_seq_grau_partic
				and    	x.ie_gera_ocorrencia = 'S');

c_partic_proc CURSOR(	nr_seq_conta_proc_pc	pls_conta_proc.nr_sequencia%type,
			nr_seq_regra_pc		pls_grupo_partic_tm.nr_seq_regra%type) FOR
...
union all
	...
	if (qt_grupo_casou_w = qt_grupo_regra_w) then
		...
		for r_c_partic_proc_w in c_partic_proc(	r_c01_w.nr_seq_conta_proc,
							dados_val_partic_p.nr_seq_regra_partic) loop

			if	((coalesce(dados_val_partic_p.ie_glosa, 'N') = 'S') and (r_c_partic_proc_w .ie_glosa = 'N')) or (coalesce(dados_val_partic_p.ie_glosa, 'N') = 'N') then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_regra_partic_pck.regra_partic_proc ( dados_val_partic_p pls_tipos_ocor_pck.dados_val_partic, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
