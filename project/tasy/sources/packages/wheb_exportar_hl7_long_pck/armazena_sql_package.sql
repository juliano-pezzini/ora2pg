...
CREATE OR REPLACE PROCEDURE wheb_exportar_hl7_long_pck.armazena_sql (cursor_p integer, ar_resultado_p INOUT myArray) AS $body$
DECLARE
...
BEGIN
    dbms_sql.describe_columns(cursor_p,col_cnt,dtab);
		...
		ELSE
          ...
          END IF;
				END IF;
				CALL CALL wheb_exportar_hl7_long_pck.armazena_result_todos_sql(ar_resultado_p[contador_w].nm,ar_resultado_p[contador_w].vl,ar_resultado_p[contador_w].v2);
			END LOOP;
		END IF;
	END;
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE wheb_exportar_hl7_long_pck.armazena_sql (cursor_p integer, ar_resultado_p INOUT myArray) FROM PUBLIC;
