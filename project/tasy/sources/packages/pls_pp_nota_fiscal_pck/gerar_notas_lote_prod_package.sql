...
CREATE OR REPLACE PROCEDURE pls_pp_nota_fiscal_pck.gerar_notas_lote_prod ( nr_seq_lote_p bigint, cd_serie_nf_p text, cd_operacao_nf_p bigint, dt_emissao_p timestamp, cd_natureza_operacao_p bigint, ds_observacao_p text, ds_complemento_p text, dt_base_venc_p timestamp, nr_nota_fiscal_p text, nm_usuario_p usuario.nm_usuario%type, ie_somente_nota_p text, nr_seq_prestador_p bigint, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, ie_base_venc_p text, nr_seq_nota_fiscal_p INOUT bigint) AS $body$
DECLARE
...
if (coalesce(ie_base_venc_p, 'N') = 'N') then
	...
	-- Obter dia após adicionar o mês, para que não ocorra de obter dia 31 em mês que não há dia 31.
...
insert into nota_fiscal(nr_sequencia,				cd_estabelecimento,		cd_cgc_emitente,
	...
	values (nextval('nota_fiscal_seq'),		cd_estabelecimento_p,		cd_cgc_prestador_w,
	...
	if (ie_tipo_despesa_w IS NOT NULL AND ie_tipo_despesa_w::text <> '') then
		...
		insert into nota_fiscal_item(nr_sequencia,						cd_estabelecimento,					cd_cgc_emitente,
			...
			values (nr_seq_nota_fiscal_w,					cd_estabelecimento_p,					cd_cgc_prestador_w,
				...
				0,							0,							0,
				0,							0,							nm_usuario_p,
				clock_timestamp(),						ds_complemento_p, 					CASE WHEN ie_tipo_item_w='M' THEN null  ELSE cd_procedimento_w END ,
				CASE WHEN ie_tipo_item_w='M' THEN null  ELSE ie_origem_proced_w END ,	CASE WHEN cd_conta_financ_w=0 THEN null  ELSE cd_conta_financ_w END ,	CASE WHEN ie_tipo_item_w='P' THEN null  ELSE cd_material_w END ,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_pp_nota_fiscal_pck.gerar_notas_lote_prod ( nr_seq_lote_p bigint, cd_serie_nf_p text, cd_operacao_nf_p bigint, dt_emissao_p timestamp, cd_natureza_operacao_p bigint, ds_observacao_p text, ds_complemento_p text, dt_base_venc_p timestamp, nr_nota_fiscal_p text, nm_usuario_p usuario.nm_usuario%type, ie_somente_nota_p text, nr_seq_prestador_p bigint, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, ie_base_venc_p text, nr_seq_nota_fiscal_p INOUT bigint) FROM PUBLIC;
