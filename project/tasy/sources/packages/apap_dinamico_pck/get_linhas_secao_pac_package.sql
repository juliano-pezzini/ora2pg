...
CREATE OR REPLACE FUNCTION apap_dinamico_pck.get_linhas_secao_pac (nr_seq_grupo_p bigint) RETURNS SETOF LINHAS_PAC_T AS $body$
DECLARE
...
c_linhas CURSOR FOR
	SELECT  distinct d.nr_seq_linked_data,
		  ...
		  case
			when y.ie_read_only = 'S' then 'S'
			when apap_dinamico_pck.get_doc_read_only(b.nr_atendimento, a.nr_sequencia) = 'S' then 'S'
			when g.ie_readonly = 'S' then 'S'
			else 'N'
		  end ie_readonly,
		  g.ds_mascara,
		  --decode(Obter_Regra_Atributo2(d.nm_tabela,g.nm_atributo,g.nr_sequencia,'VLN',cd_estabelecimento_w,cd_perfil_w,cd_funcao_w,nm_usuario_w),'S',null,
		  --nvl(Obter_Regra_Atributo2(d.nm_tabela,g.nm_atributo,g.nr_sequencia,'VLD',cd_estabelecimento_w,cd_perfil_w,cd_funcao_w,nm_usuario_w),g.vl_padrao)) vl_padrao,
		  ...
		  b.cd_pessoa_fisica,
		  d.nr_seq_exame,
		  (SELECT	coalesce(max('S'),'N')	from documento_setor x where x.nr_seq_documento = a.nr_sequencia) ie_regra_setor,
		  (select	coalesce(max('S'),'N')	from documento_acesso x where x.nr_seq_documento = a.nr_sequencia) ie_regra_perfil,
		  obter_regra_acesso_apap(a.nr_sequencia,'P',b.nr_atendimento) ie_acesso_perfil,
		  obter_regra_acesso_apap(a.nr_sequencia,'S',b.nr_atendimento) ie_acesso_setor,
		  y.ie_grupo,
		  d.ds_tooltip,
		  d.ie_line,
		  d.nr_size,
		  d.ie_reference_line,
		  d.nr_reference_line,
		  d.nr_escala_intervalo,
		  coalesce(y.ie_tooltip_complex, 'N') ie_tooltip_complex,
		  d.ie_exibe_grafico ie_exibe_grafico,
...
union
	...
	select  distinct d.nr_seq_linked_data,
			...
			d.nr_seq_exame,
			null ie_regra_setor,
			null ie_regra_perfil,
			null ie_acesso_perfil,
			null ie_acesso_setor,
			y.ie_grupo,
			d.ds_tooltip,
			d.ie_line,
			d.nr_size,
...
union
	...
	select  distinct d.nr_seq_linked_data,
			...
			b.cd_pessoa_fisica,
			null nr_seq_exame,
			(select	coalesce(max('S'),'N')	from documento_setor x where x.nr_seq_documento = b.nr_seq_documento) ie_regra_setor,
			(select	coalesce(max('S'),'N')	from documento_acesso x where x.nr_seq_documento = b.nr_seq_documento) ie_regra_perfil,
			obter_regra_acesso_apap(b.nr_seq_documento,'P',b.nr_atendimento) ie_acesso_perfil,
			obter_regra_acesso_apap(b.nr_seq_documento,'S',b.nr_atendimento) ie_acesso_setor,
			null ie_grupo,
			d.ds_tooltip,
			d.ie_line,
			d.nr_size,
...
for r_linhas in c_linhas
   ...
   qt_casas_w := 0;
   /*
	if	(r_linhas.nr_seq_linked_data in (399,383,384,385,386,294)) then
		qt_casas_w := 0;
	elsif	(r_linhas.nr_seq_linked_data in (533)) then	
		qt_casas_w := 1;
	end if;
*/
   if (r_linhas.ie_grupo <> 'ES') and (r_linhas.ie_grupo <> 'BH') and (r_linhas.nr_seq_linked_data not in (399,426)) then
	...
	if (r_linhas.ie_regra_setor = 'S') then
		if (r_linhas.ie_acesso_setor = 'L') then
			ie_doc_read_only_w	:= 'S';
		end if;	
	elsif (r_linhas.ie_regra_perfil = 'S') then
		if (r_linhas.ie_acesso_perfil = 'L') then	
			ie_doc_read_only_w	:= 'S';
		end if;	
	end if;	
	exception
   ...
   if ((r_linhas.ie_componente = 'lcb') and (position(':CD_ESCALA_DOR' in upper(ds_valores_w)) > 0)) then
	  --ds_valores_w := replace(upper(ds_valores_w),':CD_ESCALA_DOR',cd_escala_dor_w);
   ...
   linhas_pac_r_w.ie_line              := r_linhas.ie_line;
   linhas_pac_r_w.nr_size              := r_linhas.nr_size;
   linhas_pac_r_w.ie_reference_line    := r_linhas.ie_reference_line;
   linhas_pac_r_w.nr_reference_line    := r_linhas.nr_reference_line;
   linhas_pac_r_w.nr_escala_intervalo  := r_linhas.nr_escala_intervalo;
   linhas_pac_r_w.ie_tooltip_complex   := r_linhas.ie_tooltip_complex;
   linhas_pac_r_w.ie_exibe_grafico		:= r_linhas.ie_exibe_grafico;
   ...
   linhas_pac_r_w.ie_plot_axis			:= r_linhas.ie_plot_axis;
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION apap_dinamico_pck.get_linhas_secao_pac (nr_seq_grupo_p bigint) FROM PUBLIC;
