...
CREATE OR REPLACE PROCEDURE pls_cartao_integracao_pck.retorno_cancelamento (nr_sequencia_p intpd_fila_transmissao.nr_sequencia%type) AS $body$
DECLARE
...
BEGIN
        ...
        case
            when ie_status_http_w in (200, 201) then
                ...
                if retorno_w.exist('ReturnCode') then
                    ret_pagto_cartao_cr_w.cd_retorno := retorno_w.get['ReturnCode'].get_string();
                end if;
                if retorno_w.exist('ReturnMessage') then
                    ret_pagto_cartao_cr_w.ds_mensagem_retorno := retorno_w.get['ReturnMessage'].get_string();
                end if;
        ...
        if ret_pagto_cartao_cr_w.cd_retorno = '0' then
            if not pls_cartao_integracao_pck.atualiza_situacao(nr_seq_solict_pagto_p => ret_pagto_cartao_cr_w.nr_seq_solic_pagto_cartao,
                                     ...
                                     ie_situacao_novo_p    => '5', --Cancelada
    ...
    /*
     Procedimento para processar o retorno da captura da solicitacao de pagamento
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_cartao_integracao_pck.retorno_cancelamento (nr_sequencia_p intpd_fila_transmissao.nr_sequencia%type) FROM PUBLIC;
