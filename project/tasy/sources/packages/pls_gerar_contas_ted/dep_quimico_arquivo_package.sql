...
CREATE OR REPLACE PROCEDURE pls_gerar_contas_ted.dep_quimico_arquivo ( nr_seq_lote_p pls_ted_conta_lote.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) AS $body$
DECLARE
...
begin
ds_local_w := pls_utl_file_pck.novo_arquivo(33, 'dep_quimico_'||to_char(clock_timestamp(), 'ddmmyyyyhh24miss')||'.txt', 'S', null, ds_local_w, 'U');
exception
when others then
	ds_local_w := null;
end;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_contas_ted.dep_quimico_arquivo ( nr_seq_lote_p pls_ted_conta_lote.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) FROM PUBLIC;
