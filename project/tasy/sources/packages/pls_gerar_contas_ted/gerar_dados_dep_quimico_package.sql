...
and exists (	select	1
			...
			and	y.nr_seq_tipo_conta = nr_seq_tipo_conta_p  -- Dependencia química na Copel
...
SELECT	ds_prestador,
	cd_despesa,
	dt_item,
	(vl_lib - coalesce(vl_coparticipacao,0)) vl_liberado,
	nr_seq_conta,
	nr_seq_prest_exec,
	ie_tipo_guia,
...
from	(	select		substr(pls_obter_dados_prestador(a.nr_seq_prestador_exec,'NF'),1,255) ds_prestador,
		...
		and	a.nr_seq_tipo_conta = nr_seq_tipo_conta_p
...
union all
		...
		and	a.nr_seq_segurado = nr_seq_segurado_pc
		and	a.nr_seq_tipo_conta = nr_seq_tipo_conta_p
		and 	trunc(a.dt_fechamento_conta,'MONTH') = TRUNC(dt_competencia_p,'MONTH')) alias13;
c02_w 					c02%rowtype;
...
BEGIN
	...
	While c01%Found loop
		...
		While c02%found loop
			...
			c02_w.dt_item,
			c02_w.cd_despesa, c02_w.vl_liberado,
...
if (nr_seq_lote_p IS NOT NULL AND nr_seq_lote_p::text <> '') then
	...
	set	vl_total_conta = vl_total_lote_w,
		vl_total_reemb = 0,
		vl_total_geral = vl_total_lote_w,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_contas_ted.gerar_dados_dep_quimico ( nr_seq_lote_p pls_ted_conta_lote.nr_sequencia%type, dt_competencia_p pls_ted_conta_lote.dt_competencia %type, nr_seq_segurado_p pls_ted_conta_lote.nr_seq_segurado %type, nr_seq_titular_p pls_ted_conta_lote.nr_seq_titular %type, nr_seq_contrato_p pls_ted_conta_lote.nr_seq_contrato %type, nr_seq_contrato_grupo_p pls_ted_conta_lote.nr_seq_contrato_grupo %type, cd_cgc_p pls_ted_conta_lote.cd_cgc %type, nr_seq_tipo_conta_p pls_ted_conta_lote.nr_seq_tipo_conta%type default null, nm_usuario_p usuario.nm_usuario%type DEFAULT NULL, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type DEFAULT NULL) FROM PUBLIC;
