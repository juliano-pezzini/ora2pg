...
SET client_encoding TO 'UTF8';
	...
	-->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>INSERIR ATENDIMENTO<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<--
CREATE OR REPLACE PROCEDURE mprev_pop_alvo_prest_pck.selecionar_atend ( nr_seq_regra_cubo_p bigint, nr_seq_populacao_p bigint, nm_usuario_p text) AS $body$
DECLARE


	c_regra_atend CURSOR(nr_seq_regra_cubo_pc	bigint) FOR
...
BEGIN
	...
	if (nr_seq_regra_cubo_p IS NOT NULL AND nr_seq_regra_cubo_p::text <> '')then
		...
		PERFORM set_config('mprev_pop_alvo_prest_pck.ds_select_w', ' select	pessoa.nr_sequencia 				' || current_setting('mprev_pop_alvo_prest_pck.enter_w')::pls_util_pck.enter_w%type ||
					...
					' where		pessoa.cd_pessoa_fisica = atend.cd_pessoa_fisica	', false);
		...
		for r_c_regra_atend in c_regra_atend(nr_seq_regra_cubo_p) loop
			...
			regra_atend_w.qt_ocorrencia_maxima	:= coalesce(r_c_regra_atend.qt_ocorrencia_maxima,99999);
			...
			PERFORM set_config('mprev_pop_alvo_prest_pck.ds_restricao_w', mprev_pop_alvo_prest_pck.obter_restricao_atend(regra_atend_w), false);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE mprev_pop_alvo_prest_pck.selecionar_atend ( nr_seq_regra_cubo_p bigint, nr_seq_populacao_p bigint, nm_usuario_p text) FROM PUBLIC;
