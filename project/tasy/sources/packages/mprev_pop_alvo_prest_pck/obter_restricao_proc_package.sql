...
CREATE OR REPLACE FUNCTION mprev_pop_alvo_prest_pck.obter_restricao_proc (dados_regra_p mprev_pop_alvo_prest_pck.regra_proc) RETURNS varchar AS $body$
DECLARE
...
BEGIN
	...
	if (dados_regra_p.cd_procedimento IS NOT NULL AND dados_regra_p.cd_procedimento::text <> '') then
		...
		current_setting('mprev_pop_alvo_prest_pck.bind_sql_w')::sql_pck.t_dado_bind := sql_pck.bind_variable(':cd_procedimento_'||dados_regra_p.cd_procedimento||'_pc', dados_regra_p.cd_procedimento, current_setting('mprev_pop_alvo_prest_pck.bind_sql_w')::sql_pck.t_dado_bind);
	end if;
	...
	if (dados_regra_p.ie_origem_proced IS NOT NULL AND dados_regra_p.ie_origem_proced::text <> '') then
		...
		current_setting('mprev_pop_alvo_prest_pck.bind_sql_w')::sql_pck.t_dado_bind := sql_pck.bind_variable(':ie_origem_proced_'||dados_regra_p.ie_origem_proced||'_pc', dados_regra_p.ie_origem_proced, current_setting('mprev_pop_alvo_prest_pck.bind_sql_w')::sql_pck.t_dado_bind);
	end if;
	...
	current_setting('mprev_pop_alvo_prest_pck.bind_sql_w')::sql_pck.t_dado_bind := sql_pck.bind_variable(':qt_ocorrencia_'||dados_regra_p.qt_ocorrencia||'_pc', dados_regra_p.qt_ocorrencia, current_setting('mprev_pop_alvo_prest_pck.bind_sql_w')::sql_pck.t_dado_bind);
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION mprev_pop_alvo_prest_pck.obter_restricao_proc (dados_regra_p mprev_pop_alvo_prest_pck.regra_proc) FROM PUBLIC;
