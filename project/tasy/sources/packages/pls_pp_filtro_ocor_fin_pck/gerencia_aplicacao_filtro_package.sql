...
CREATE OR REPLACE PROCEDURE pls_pp_filtro_ocor_fin_pck.gerencia_aplicacao_filtro ( nr_seq_lote_p pls_pp_lote.nr_sequencia%type, nr_seq_regra_periodo_p pls_pp_lote.nr_seq_regra_periodo%type, nr_id_transacao_p pls_pp_rp_ofin_selecao.nr_id_transacao%type, cd_condicao_pagamento_p pls_pp_lote.cd_condicao_pagamento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
-- Informando o período no cadastro ou informando os filtros diretamente no lote
-- para ocorrência financeira só é permitido filtros para prestador e evento
...
for r_c_conf_filtro_w in c_conf_filtro(nr_seq_lote_p, nr_seq_regra_periodo_p) loop
	...
	CALL CALL CALL CALL CALL pls_pp_filtro_ocor_fin_pck.prepara_reg_proces_filtro(nr_id_transacao_p, r_c_conf_filtro_w.ie_excecao);
	...
	if (r_c_conf_filtro_w.ie_filtro_pagamento = 'S') then
		SELECT * FROM pls_pp_filtro_ocor_fin_pck.aplica_filtro_pagamento(	ie_considera_tab_selecao_w, qt_filtro_processado_w, nr_id_transacao_p, r_c_conf_filtro_w.ie_excecao, r_c_conf_filtro_w.nr_seq_filtro, cd_condicao_pagamento_p, nm_usuario_p) INTO STRICT 	ie_considera_tab_selecao_w, qt_filtro_processado_w;
	...
	if (r_c_conf_filtro_w.ie_filtro_prestador = 'S') then
		SELECT * FROM pls_pp_filtro_ocor_fin_pck.aplica_filtro_prestador(	ie_considera_tab_selecao_w, qt_filtro_processado_w, nr_id_transacao_p, r_c_conf_filtro_w.ie_excecao, r_c_conf_filtro_w.nr_seq_filtro, cd_condicao_pagamento_p, nm_usuario_p) INTO STRICT 	ie_considera_tab_selecao_w, qt_filtro_processado_w;
	...
	CALL CALL CALL CALL CALL pls_pp_filtro_ocor_fin_pck.atualiza_sel_excecao_end(nr_id_transacao_p, ie_processou_filtro_w, r_c_conf_filtro_w.ie_excecao);
end loop;
...
CALL CALL CALL CALL CALL CALL pls_pp_filtro_ocor_fin_pck.limpar_invalidos(nr_id_transacao_p);
