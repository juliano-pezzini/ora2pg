...
if (nr_seq_tipo_prestador_p IS NOT NULL AND nr_seq_tipo_prestador_p::text <> '') then
	...
	where	nr_seq_tipo	= nr_seq_tipo_prestador_p;

	if (qt_tipo_prest_w = 0) then
		ds_restricao_prest_w	:= ds_restricao_prest_w ||
					' and	'|| ds_alias_prest_w ||'.nr_seq_tipo_prestador = :nr_seq_tipo_prestador_pc ' || pls_util_pck.enter_w;	
	else
		ds_restricao_prest_w	:= ds_restricao_prest_w ||
					' and	exists	(select	1 ' ||
