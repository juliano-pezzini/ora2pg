...
BEGIN
-- gerencia a aplicaaao do filtro de material
...
if (nr_seq_filtro_p IS NOT NULL AND nr_seq_filtro_p::text <> '') then
	...
	for r_c_filtro in c_filtro(nr_seq_filtro_p) loop
		...
		begin
			-- obtam a restriaao do material

			SELECT * FROM pls_ocor_imp_pck.obter_restricao_material(	r_c_filtro.nr_seq_material, r_c_filtro.nr_seq_grupo_material, r_c_filtro.nr_seq_estrut_mat, r_c_filtro.ie_tipo_despesa_mat, r_c_filtro.dt_dia_semana, r_c_filtro.ie_feriado, r_c_filtro.ie_tipo_feriado, r_c_filtro.hr_inicial, r_c_filtro.hr_final, r_c_filtro.vl_minimo_item, r_c_filtro.vl_max_item, r_c_filtro.ie_material_tuss, ds_campos_w, ds_campos_proc_w, valor_bind_w) INTO STRICT _ora2pg_r;
 ds_campos_w := _ora2pg_r.ds_campos_p; ds_campos_proc_w := _ora2pg_r.ds_campos_proc_p; valor_bind_w := _ora2pg_r.valor_bind_p;
			
			-- Montar o select padrao juntamente as restriaaes.						

			valor_bind_w := pls_ocor_imp_pck.obter_select_filtro(	ie_considera_selecao_p, ie_incidencia_selecao_regra_p, ie_incidencia_selecao_filtro_p, ie_processo_excecao_p, nr_seq_ocorrencia_p, nr_id_transacao_p, nr_seq_filtro_p, nr_seq_lote_protocolo_p, nr_seq_protocolo_p, nr_seq_conta_p, nr_seq_conta_proc_p, nr_seq_conta_mat_p, dt_inicio_vigencia_p, dt_fim_vigencia_p, ds_campos_w, null, dados_restricao_w, ds_campos_proc_w, null, null, ds_campos_w, null, dados_restricao_w, cd_estabelecimento_p, valor_bind_w);
			...
			loop
				...
				for i in tb_seq_conta_w.first .. tb_seq_conta_w.last loop
					...
					ie_registro_valido_w := pls_ocor_imp_pck.ie_reg_valido_fil_mat(	r_c_filtro.hr_inicial, tb_hr_inicial_w(i),
