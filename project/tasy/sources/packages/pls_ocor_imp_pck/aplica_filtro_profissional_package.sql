...
c_filtro CURSOR(	nr_seq_filtro_pc	pls_oc_cta_filtro.nr_sequencia%type,
	...
	SELECT	a.nr_sequencia,
		...
		coalesce(a.ie_medico,'A') ie_medico,
		a.nr_seq_cbo_saude
...
BEGIN
-- gerencia a aplicaaao do filtro de Profissional
...
if (nr_seq_filtro_p IS NOT NULL AND nr_seq_filtro_p::text <> '') then
	...
	for r_c_filtro in c_filtro(nr_seq_filtro_p, ie_medico_p) loop
		...
		begin
			...
			-- obtam a restriaao do Profissional

			SELECT * FROM pls_ocor_imp_pck.obter_restricao_profissional(	r_c_filtro.cd_medico, r_c_filtro.nr_seq_conselho, r_c_filtro.nr_seq_grau_partic, r_c_filtro.ie_medico_cooperado, r_c_filtro.ie_medico, r_c_filtro.nr_seq_cbo_saude, ie_filtro_mat_p, ie_incidencia_selecao_regra_p, ie_incidencia_selecao_filtro_p, ie_processo_excecao_p, ds_tabelas_w, valor_bind_w) INTO STRICT _ora2pg_r;
 ds_tabelas_w := _ora2pg_r.ds_tabelas_p; valor_bind_w := _ora2pg_r.valor_bind_p;
