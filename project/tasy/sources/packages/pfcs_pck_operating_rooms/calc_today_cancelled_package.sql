...
CREATE OR REPLACE PROCEDURE pfcs_pck_operating_rooms.calc_today_cancelled (cd_establishment_p bigint, nm_user_p text) AS $body$
DECLARE
        ...
        SELECT pat.patient_id id_patient,
            ...
            appt.ds_cancellation_reason ds_cancellation_reason,
            appt.id_appointment cd_appointment
        from pfcs_patient pat,
            pfcs_appointment appt
        where  upper(appt.si_status) = PFCS_PCK_CONSTANTS.SI_STATUS_CANCELLED
            and trunc(appt.dt_atualizacao) = trunc(clock_timestamp())
...
BEGIN
        ...
        for c01_w in cur_patients(nr_seq_organization_w) loop
             qt_cancelled_w := qt_cancelled_w + 1;
...
;
            ...
            values (
                ...
                PFCS_PCK_INDICATORS.NR_OR_CANCELLED_SURGERY,
                cd_establishment_p);
         ...
         := pfcs_pck.pfcs_generate_results(
            vl_indicator_p => qt_cancelled_w, nr_seq_indicator_p => PFCS_PCK_INDICATORS.NR_OR_CANCELLED_SURGERY, nr_seq_operational_level_p => cd_establishment_p, nm_usuario_p => nm_user_p, nr_seq_panel_p => pfcs_panel_seq_w);

        CALL pfcs_pck.pfcs_update_detail(
            nr_seq_indicator_p => PFCS_PCK_INDICATORS.NR_OR_CANCELLED_SURGERY,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pfcs_pck_operating_rooms.calc_today_cancelled (cd_establishment_p bigint, nm_user_p text) FROM PUBLIC;
