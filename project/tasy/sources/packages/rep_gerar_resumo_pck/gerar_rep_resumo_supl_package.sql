...
CREATE OR REPLACE PROCEDURE rep_gerar_resumo_pck.gerar_rep_resumo_supl () AS $body$
DECLARE
	...
	C01 CURSOR FOR
	SELECT	substr(c.ds_material,1,100),
		a.qt_dose,
...
WHERE a.cd_material	= c.cd_material and a.nr_prescricao	= nr_prescricao_w and a.ie_agrupador	= 12;
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		if (current_setting('rep_gerar_resumo_pck.ie_rtf_w')::varchar(1) = 'N') then
			PERFORM set_config('rep_gerar_resumo_pck.ds_resumo_w', substr(current_setting('rep_gerar_resumo_pck.ds_resumo_w')::text ||
					rep_gerar_resumo_pck.get_tr_html(ds_material_w, to_char(qt_dose_w) , ds_unidade_medida_w, ds_intervalo_w, null, 
							ds_unid_med_w, ds_horarios_w, ie_suspenso_w, 7),1,30000), false);
			...
			PERFORM set_config('rep_gerar_resumo_pck.ds_resumo_w', substr(current_setting('rep_gerar_resumo_pck.ds_resumo_w')::text ||
					rep_gerar_resumo_pck.get_tr_rtf(ds_material_w, to_char(qt_dose_w) , ds_unidade_medida_w, ds_intervalo_w, null,
							ds_unid_med_w, ds_horarios_w, 7),1,30000), false);
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE rep_gerar_resumo_pck.gerar_rep_resumo_supl () FROM PUBLIC;
