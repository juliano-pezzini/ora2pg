...
---------------------------------------------------------------
CREATE OR REPLACE PROCEDURE val_mensal_consignado.val_consig_dif_saldo ( cd_material_p bigint, cd_fornecedor_p text, cd_estabelecimento_p bigint , dt_mesano_referencia_p timestamp, nr_movimento_estoque_p bigint) AS $body$
DECLARE
...
ie_aumenta_diminui_valor_w	varchar(1);
...
if (vl_diferenca_w <> 0) then
	begin
	if (qt_estoque_w <> 0) and (vl_estoque_w > vl_diferenca_w) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE val_mensal_consignado.val_consig_dif_saldo ( cd_material_p bigint, cd_fornecedor_p text, cd_estabelecimento_p bigint , dt_mesano_referencia_p timestamp, nr_movimento_estoque_p bigint) FROM PUBLIC;
