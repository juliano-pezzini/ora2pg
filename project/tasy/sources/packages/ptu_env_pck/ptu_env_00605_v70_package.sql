...
CREATE OR REPLACE PROCEDURE ptu_env_pck.ptu_env_00605_v70 ( nr_seq_guia_p pls_guia_plano_proc.nr_seq_guia%type, nr_seq_requisicao_p pls_requisicao_proc.nr_seq_requisicao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, nr_seq_pedido_novo_p INOUT ptu_pedido_compl_aut.nr_sequencia%type) AS $body$
DECLARE


/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: Gerar a transacao de pedido de complemento de autorizacao do PTU, via SCS
-------------------------------------------------------------------------------------------------------------------
...
qt_dosagem_total_w			ptu_pedido_compl_aut_serv.qt_dosagem_total%type;
qt_unidade_medida_w			ptu_pedido_compl_aut_serv.qt_unidade_medida%type;
qt_registros_ww				integer;
nr_protocolo_atendimento_w		ptu_pedido_compl_aut.nr_protocolo_atendimento%type;
...
qt_regras_w				integer;
ds_token_w				ptu_pedido_compl_aut.ds_token%type;
...
nm_prestador_w				ptu_pedido_compl_aut.nm_prestador%type;
sg_conselho_profissional_w		ptu_pedido_compl_aut.sg_conselho_profissional%type;
nr_conselho_profissional_w		ptu_pedido_compl_aut.nr_conselho_profissional%type;
uf_conselho_profissional_w		ptu_pedido_compl_aut.uf_conselho_profissional%type;
cd_cbo_prest_solic_w			ptu_pedido_compl_aut.cd_cbo_prest_solic%type;
cd_cbo_prest_exec_w			ptu_pedido_compl_aut.cd_cbo_prest_exec%type;
ie_tipo_acomodacao_w			ptu_pedido_compl_aut.ie_tipo_acomodacao%type;
ie_complementar_1_w			ptu_pedido_compl_aut.ie_complementar_1%type;
ie_complementar_2_w			ptu_pedido_compl_aut.ie_complementar_2%type;
ie_complementar_3_w			ptu_pedido_compl_aut.ie_complementar_3%type;
...
if (nr_seq_requisicao_p IS NOT NULL AND nr_seq_requisicao_p::text <> '') then
	...
	select	pls_obter_dados_segurado(nr_seq_segurado, 'C'),
		...
		eliminar_caractere(substr(pls_obter_dados_prestador(nr_seq_prestador,'N'),1,70)),
		nr_seq_cbo_saude,
	...
	if (ie_tipo_guia_w = '2') and (ie_tipo_atendimento_w = '7' or coalesce(ie_regime_atendimento_w,9) = 3) and (coalesce(nr_seq_prestador_exec_w::text, '') = '') then
		...
		if (ie_tipo_segurado_w = 'T') then
			...
			if (ie_tipo_contrato_w = 'F') and (qt_registros_ww > 0) then
				...
				if (qt_registros_ww > 0) then
					...
					cd_unimed_w	:= adiciona_zeros_esquerda(substr(cd_carteirinha_w,1,4),4);
				end if;
			end if;
	...
	if ((pls_obter_unimed_benef(nr_seq_segurado_w) IS NOT NULL AND (pls_obter_unimed_benef(nr_seq_segurado_w))::text <> '')) then
		...
		CALL pls_requisicao_gravar_hist(nr_seq_requisicao_p,'L',expressao_pck.obter_desc_expressao(962781)||cd_cooperativa_ww,null,nm_usuario_p);
	else
	...
	if ((pls_obter_unimed_benef(nr_seq_segurado_w) IS NOT NULL AND (pls_obter_unimed_benef(nr_seq_segurado_w))::text <> '')) then
		...
		CALL pls_guia_gravar_historico(nr_seq_guia_p,2,expressao_pck.obter_desc_expressao(962781)||cd_cooperativa_ww,'',nm_usuario_p);
	else
		CALL wheb_mensagem_pck.exibir_mensagem_abort(246588);
	end if;
end if;
...
insert	into ptu_pedido_compl_aut(nr_sequencia, ie_tipo_cliente, cd_unimed_executora,
	...
	ds_indicacao_clinica, ie_sexo, nr_idade, ie_liminar,
	nr_protocolo_atendimento, ds_token,
...
if	(ie_anexo_guia_w	= 'S') then
	ptu_inserir_dados_anexo_pedido(nr_seq_guia_p, nr_seq_requisicao_p, null, nr_seq_pedido_novo_p, nm_usuario_p);
end if;
*/
...
for c01_w in C01( nr_seq_guia_p, nr_seq_requisicao_p ) loop
	...
	if (c01_w.nr_seq_pacote IS NOT NULL AND c01_w.nr_seq_pacote::text <> '') and (coalesce(ie_permite_pacote_w,'N')	= 'S') then
		...
		if (ie_tipo_pacote_w	= 'A') 	and (coalesce(ie_baixo_risco_w,'N') = 'N')  then
			...
			for c03_w in C03( nr_seq_composicao_w ) loop
				...
				from	procedimento
				where	cd_procedimento		= c03_w.cd_procedimento
				and	ie_origem_proced	= c03_w.ie_origem_proced;

				if (cd_tipo_procedimento_w = 37) then
					--ds_proc_diaria_w	:= ds_proc_diaria_w||' '||cd_proc_pacote_w||' - '||substr(obter_descricao_procedimento(cd_proc_pacote_w,ie_origem_proced_pcte_w),1,100)||',';

					ie_existe_diaria_w	:= 'S';
				elsif (cd_tipo_procedimento_w = 38) then
					--ds_proc_taxa_w	:= ds_proc_taxa_w||' '||cd_proc_pacote_w||' - '||substr(obter_descricao_procedimento(cd_proc_pacote_w,ie_origem_proced_pcte_w),1,100)||',';

					ie_existe_taxa_w	:= 'S';

				elsif (ie_classificacao_proc_pacote_w = '1') then
					ds_procedimento_w	:= substr(Obter_Descricao_Procedimento(c03_w.cd_procedimento, c03_w.ie_origem_proced),1,255);
					...
					begin
						...
						and	a.cd_procedimento	= c03_w.cd_procedimento
						and	a.ie_origem_proced	= c03_w.ie_origem_proced;
					exception
					when others then
					...
					if (ie_existe_procedimento_w = 'N') then
						ds_procedimento_observ_w	:= substr(' Procedimento: '||c03_w.cd_procedimento||' - '||ds_procedimento_w||' Valor: '||vl_procedimento_pacote_w||', ',1,999);
					else
						ds_procedimento_observ_w	:= substr(ds_procedimento_observ_w||' '||c03_w.cd_procedimento||' - '||ds_procedimento_w||' Valor: '||vl_procedimento_pacote_w||', ',1,999);
					end if;
			...
			if	((sg_estado_congenere_w <> 'X') and (sg_estado_exec_w <> 'X') and (sg_estado_congenere_w <> sg_estado_exec_w))  or (ie_pacote_estado_w = 'S')  then
				...
				for c04_w in C04( nr_seq_composicao_w ) loop
					...
					begin
						cd_material_w	:= substr(pls_obter_seq_codigo_material(c04_w.nr_seq_material,''),1,10);
					exception
					when others then
					...
					if (ie_tipo_despesa_w = '7') then
						begin
							...
							and	a.nr_sequencia		= c04_w.nr_seq_material;
						exception
						when others then
							vl_opme_w	:= '';
						end;
						...
						--ds_material_w	:= ds_material_w||' '||cd_material_w||' - '||ds_opme_w||',';

						ie_existe_material_w	:= 'S';
					elsif (ie_tipo_despesa_w = '2') then
						--ds_material_medicamento_w	:= ds_material_medicamento_w||' '||cd_material_w||' - '||ds_opme_w||',';
			...
			/*if	(vl_total_pacote_w is not null) then
	...
	if (nr_seq_requisicao_p IS NOT NULL AND nr_seq_requisicao_p::text <> '') then
		...
		insert	into ptu_pedido_compl_aut_serv(nr_sequencia, nr_seq_pedido, ie_tipo_tabela,
			 ...
			 ie_tipo_ordem, vl_uni_servico, nr_seq_item,
			 ie_servico_proprio)
		values (nr_seq_pedido_compl_serv_w, nr_seq_pedido_novo_p, coalesce(c01_w.ie_tipo_tabela, ie_tipo_tabela_w),
			 ...
			 null, c01_w.vl_procedimento, sq_item_w,
			 c01_w.ie_servico_proprio);
...
for c02_w in C02( nr_seq_guia_p, nr_seq_requisicao_p ) loop
	...
	SELECT * FROM ptu_env_pck.ptu_obter_dados_mat(c02_w.nr_seq_material, c02_w.cd_material_ptu, ie_codificacao_w, ie_tipo_despesa_w) INTO STRICT ie_codificacao_w, ie_tipo_despesa_w;
	...
	begin
		cd_material_w	:= substr(pls_obter_seq_codigo_material(c02_w.nr_seq_material,''),1,10);
	exception
	when others then
	...
	if (cd_material_w IS NOT NULL AND cd_material_w::text <> '') then
		...
		if (coalesce(c02_w.ie_pacote_ptu,'N')	= 'S') then
			ie_pacote_w	:= 'S';
		else
			ie_pacote_w	:= 'N';
		end if;
		...
		if (nr_seq_requisicao_p IS NOT NULL AND nr_seq_requisicao_p::text <> '') then
			...
			if (ie_tipo_despesa_w in ('3','7')) or ((trim(both c02_w.cd_material_ptu) IS NOT NULL AND (trim(both c02_w.cd_material_ptu))::text <> '')) then
				ds_material_ptu_w	:= substr(coalesce(ds_material_ptu_w,pls_obter_desc_material(c02_w.nr_seq_material)),1,80);
			end if;
			...
			if (ie_tipo_tabela_w = '2') then
				cd_ref_fabricante_imp_gen_w := coalesce(coalesce(cd_ref_fabricante_imp_w, cd_ref_fabricante_w), c02_w.nr_seq_material);
			end if;
			...
			insert	into ptu_pedido_compl_aut_serv(nr_sequencia, nr_seq_pedido, ie_tipo_tabela,
				 ...
				 vl_uni_servico, nr_seq_item, qt_dosagem_total,
				 qt_unidade_medida)
			values (nr_seq_pedido_compl_serv_w, nr_seq_pedido_novo_p, ie_tipo_tabela_w,
				 ...
				 clock_timestamp(), trim(both somente_numero(coalesce(coalesce(nr_registro_anvisa_w, c02_w.nr_registro_anvisa), nr_registro_anvisa_gen_w))), cd_ref_fabricante_imp_gen_w,
				 ...
				 ie_frequencia_dose_w, ie_pacote_w, CASE WHEN ie_tipo_anexo_w='OP' THEN ie_opcao_fabricante_w  ELSE null END ,
				 c02_w.vl_material, sq_item_w, qt_dosagem_total_w,
				qt_unidade_medida_w);
		elsif (nr_seq_guia_p IS NOT NULL AND nr_seq_guia_p::text <> '') then
		...
		if (coalesce(c02_w.cd_material_ptu,0)	<> 0) then
			...
			if (qt_reg_generico_w	> 0) then
				if (coalesce(c02_w.vl_material,0) = 0) 	and (coalesce(ds_material_ptu_w,'X')	<> 'X') then
					CALL wheb_mensagem_pck.exibir_mensagem_abort(196741); -- E obrigatorio informar valor para procedimentos e materiais genericos ou pacotes!
				end if;

				if (coalesce(ds_material_ptu_w,'X')	= 'X') then
					CALL wheb_mensagem_pck.exibir_mensagem_abort(216706); -- E obrigatorio informar descricao para procedimentos e materiais genericos.
				end if;
			end if;
		end if;
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_env_pck.ptu_env_00605_v70 ( nr_seq_guia_p pls_guia_plano_proc.nr_seq_guia%type, nr_seq_requisicao_p pls_requisicao_proc.nr_seq_requisicao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, nr_seq_pedido_novo_p INOUT ptu_pedido_compl_aut.nr_sequencia%type) FROM PUBLIC;
