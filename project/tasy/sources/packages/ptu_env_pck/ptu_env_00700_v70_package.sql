...
CREATE OR REPLACE PROCEDURE ptu_env_pck.ptu_env_00700_v70 ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, nm_usuario_p text, nr_seq_decurso_p INOUT ptu_decurso_prazo.nr_sequencia%type) AS $body$
DECLARE

/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: Atualizar os itens da guia conforme a geracao de ocorr~encia combinada.
-------------------------------------------------------------------------------------------------------------------
...
if (nr_seq_guia_p IS NOT NULL AND nr_seq_guia_p::text <> '') then
	...
	if (ie_estagio_guia_w	<> 11) then
		--Esta transacao deve ser enviada somente quando o pedido estiver em auditoria na operadora de origem!
	...
	when others then
		...
		exception
		when others then
			--Nao e possivel enviar um decurso de prazo sem pedido de autorizacao ou pedido de complemento de autorizacao!
	...
	if (coalesce(dt_atualizacao_w::text, '') = '') then
		--Nao e possivel enviar um decurso de prazo sem a resposta de pedido de autorizacao ou pedido de complemento de autorizacao!
	...
	if (coalesce(dt_atualizacao_w::text, '') = '') then
	--Nao e possivel enviar um decurso de prazo sem a resposta de pedido de autorizacao ou complemento de autorizacao!
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_env_pck.ptu_env_00700_v70 ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, nm_usuario_p text, nr_seq_decurso_p INOUT ptu_decurso_prazo.nr_sequencia%type) FROM PUBLIC;
