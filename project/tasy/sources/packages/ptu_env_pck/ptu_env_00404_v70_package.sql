...
CREATE OR REPLACE PROCEDURE ptu_env_pck.ptu_env_00404_v70 ( nr_seq_guia_p bigint, nr_seq_requisicao_p bigint, nr_seq_complemento_p bigint, ds_mensagem_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_auditoria_p INOUT bigint) AS $body$
DECLARE

/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: Gerar a transacao PTU de resposta de auditoria, via SCS
-------------------------------------------------------------------------------------------------------------------
...
if (nr_seq_guia_p IS NOT NULL AND nr_seq_guia_p::text <> '') then
	...
	if (qt_reg_auditoria_w	<> qt_reg_confirm_w) then
		...
		into STRICT	nr_seq_auditoria_p
		from	ptu_resposta_auditoria
...
values (nr_seq_auditoria_p, '00404', ie_tipo_cliente_w,
	 ...
	 nm_usuario_p, clock_timestamp(), pls_obter_versao_scs);
...
if	((nr_seq_guia_p IS NOT NULL AND nr_seq_guia_p::text <> '') or (nr_seq_requisicao_p IS NOT NULL AND nr_seq_requisicao_p::text <> '')) then
	...
	fetch c01 into
		...
		nr_seq_req_proc_w,
		sq_item_w,
		ie_origem_recebida_w;
	EXIT WHEN NOT FOUND; /* apply on c01 */
		begin

		if (coalesce(sq_item_w::text, '') = '') then
		...
		if (ie_tipo_tabela_w	in ('0','1','4')) then
			...
			if (cd_servico_w IS NOT NULL AND cd_servico_w::text <> '') then
				...
				if (pls_obter_versao_scs = '090') then
					...
					where 	cd_proc_mat_generico = coalesce(cd_servico_conversao_w,cd_servico_w)
					and	ie_situacao		= 'A';

					if (qt_registro_w > 0) and (coalesce(ds_proced_w,'X')	= 'X') then
				...
				values (nextval('ptu_resp_auditoria_servico_seq'), ie_tipo_tabela_w, cd_servico_w,
					 ie_origem_recebida_w, ds_proced_w, ie_proc_autor_w,
					 ...
					 coalesce(sq_item_w,sq_item_aux_w));
			end if;
			...
			pls_obter_mat_tiss_vigente( nr_seq_material_w, clock_timestamp(), coalesce(cd_servico_conversao_w,cd_servico_w), 'O', 'N', pls_obter_versao_tiss);
			exception
			when others then
				nr_seq_material_w	:= 0;
			end;
			...
			if (cd_servico_w IS NOT NULL AND cd_servico_w::text <> '') then
				if (coalesce(ds_opme_w,'X')	= 'X') then
					ds_material_w	:= substr(coalesce(pls_obter_desc_material(nr_seq_material_w),expressao_pck.obter_desc_expressao(347311)),1,80);
				else
				...
				if (pls_obter_versao_scs = '090') then
					...
					if (qt_registro_w > 0) and (coalesce(ds_material_w,'X')	= 'X') then
						CALL wheb_mensagem_pck.exibir_mensagem_abort(216706);
					else
						ds_material_w	:= '';
					end if;
...
if (qt_reg_aprov_w	= 0) and (qt_reg_neg_w	> 0) then
	update	ptu_resposta_auditoria
	set	ds_observacao	= ds_observacao || ' - Pedido negado',
		dt_validade	 = NULL
	where	nr_sequencia	= nr_seq_auditoria_p;
elsif (qt_reg_aprov_w	> 0) and (qt_reg_neg_w	= 0) then
	update	ptu_resposta_auditoria
	set	ds_observacao	= ds_observacao || ' - Pedido autorizado'
	where	nr_sequencia	= nr_seq_auditoria_p;
elsif (qt_reg_aprov_w	> 0) and (qt_reg_neg_w	> 0) then
	update	ptu_resposta_auditoria
	set	ds_observacao	= ds_observacao || ' - Pedido parcialmente autorizado'
	where	nr_sequencia	= nr_seq_auditoria_p;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_env_pck.ptu_env_00404_v70 ( nr_seq_guia_p bigint, nr_seq_requisicao_p bigint, nr_seq_complemento_p bigint, ds_mensagem_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_auditoria_p INOUT bigint) FROM PUBLIC;
