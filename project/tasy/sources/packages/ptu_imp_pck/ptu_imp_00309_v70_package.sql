...
qt_reg_resp_ci_w		integer;
qt_reg_confir_ci_w		integer;
...
nr_seq_motivo_cancel_w		pls_guia_plano.nr_seq_motivo_cancel%type;
...
select	count(1)
into STRICT	qt_reg_resp_ci_w
from	ptu_comunicacao_internacao	a
where	a.nr_seq_execucao	= nr_seq_execucao_p;

select	count(1)
into STRICT	qt_reg_confir_ci_w
from	ptu_confirmacao	x
where	x.nr_seq_execucao	= nr_seq_execucao_p
and	x.ie_tipo_resposta	= 'IA';

if (qt_reg_resp_ci_w	<> qt_reg_confir_ci_w ) then
	ie_tipo_resposta_w	:= 'IA';
end if;
...
if (ie_tipo_resposta_w	= 'C') and (ie_identificador_p  = 1) then
	...
	if (coalesce(nr_seq_cancel_wsd_w::text, '') = '') then
		if (nr_seq_motivo_cancel_w IS NOT NULL AND nr_seq_motivo_cancel_w::text <> '') then
			nr_seq_cancel_wsd_w := nr_seq_motivo_cancel_w;
		else
			select	max(nr_sequencia)
			into STRICT 	nr_seq_cancel_wsd_w
			from 	pls_guia_motivo_cancel
	...
	if (nr_seq_requisicao_w IS NOT NULL AND nr_seq_requisicao_w::text <> '') then
		...
		CALL pls_requisicao_gravar_hist(nr_seq_requisicao_w,'L',expressao_pck.obter_desc_expressao(962706),null,nm_usuario_p);
...
if (ie_tipo_resposta_w	in ('PI','C','DP')) then
	if (nr_seq_requisicao_w IS NOT NULL AND nr_seq_requisicao_w::text <> '') then
		CALL pls_requisicao_gravar_hist(nr_seq_requisicao_w,'L',expressao_pck.obter_desc_expressao(962708)||cd_unimed_beneficiario_p,null,nm_usuario_p);
		-- Se for uma requisicao recebida por webService(TISS) entao a guia gerada deve ser atualizada conforme a requisicao

		CALL ptu_atualizar_guia_proc_ws(nr_seq_requisicao_w, nm_usuario_p);
	elsif (nr_seq_guia_w IS NOT NULL AND nr_seq_guia_w::text <> '') then
		CALL pls_guia_gravar_historico(nr_seq_guia_w,2,expressao_pck.obter_desc_expressao(962708)||cd_unimed_beneficiario_p,'',nm_usuario_p);
	end if;
elsif (ie_tipo_resposta_w	= 'RA') then
	if (nr_seq_requisicao_w IS NOT NULL AND nr_seq_requisicao_w::text <> '') then
		CALL pls_requisicao_gravar_hist(nr_seq_requisicao_w,'L',expressao_pck.obter_desc_expressao(962708)||cd_unimed_executora_p,null,nm_usuario_p);
		-- Se for uma requisicao recebida por webService(TISS) entao a guia gerada deve ser atualizada conforme a requisicao

		CALL ptu_atualizar_guia_proc_ws(nr_seq_requisicao_w, nm_usuario_p);
	elsif (nr_seq_guia_w IS NOT NULL AND nr_seq_guia_w::text <> '') then
		CALL pls_guia_gravar_historico(nr_seq_guia_w,2,expressao_pck.obter_desc_expressao(962708)||cd_unimed_executora_p,'',nm_usuario_p);
	end if;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_imp_pck.ptu_imp_00309_v70 ( ie_tipo_cliente_p ptu_confirmacao.ie_tipo_cliente%type, cd_unimed_executora_p ptu_confirmacao.cd_unimed_executora%type, cd_unimed_beneficiario_p ptu_confirmacao.cd_unimed_beneficiario%type, nr_seq_execucao_p ptu_confirmacao.nr_seq_execucao%type, nr_seq_origem_p ptu_confirmacao.nr_seq_origem%type, ie_identificador_p ptu_confirmacao.ie_tipo_identificador%type, cd_transacao_p ptu_confirmacao.cd_transacao%type, nr_versao_p ptu_confirmacao.nr_versao%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nr_seq_confirmacao_p INOUT ptu_confirmacao.nr_sequencia%type, ie_possui_registro_p INOUT text) FROM PUBLIC;
