...
CREATE OR REPLACE PROCEDURE ptu_imp_pck.ptu_processa_00418_v70 ( nr_seq_consulta_p ptu_consulta_prestador.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nr_seq_resp_consulta_prest_p INOUT ptu_resp_consulta_prest.nr_sequencia%type) AS $body$
DECLARE


/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: Realizar a validacao do arquivo 00418 - Consulta Prestado do PTU

Rotina utilizada nas transacoes ptu via WebService.
Ira ler a consulta de prestador que foi realizada e ira gerar a resposta desta consulta.,
gravando as informacoes nas tabelas que serao consultadas na tela.
-------------------------------------------------------------------------------------------------------------------
...
begin
	for c01_w in C01(nm_prestador_w, cd_cgc_cpf_w, nr_cons_prof_w, uf_cons_prof_w, sg_cons_prof_w) loop
		begin
			...
			if (obter_se_somente_numero(c01_w.cd_prestador) = 'S') and (length(c01_w.cd_prestador) < 9) then
				ie_cod_valido_w	:= 'S';
			else
				ie_cod_valido_w	:= 'N';
			end if;

			if (ie_cod_valido_w = 'S') then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_imp_pck.ptu_processa_00418_v70 ( nr_seq_consulta_p ptu_consulta_prestador.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nr_seq_resp_consulta_prest_p INOUT ptu_resp_consulta_prest.nr_sequencia%type) FROM PUBLIC;
