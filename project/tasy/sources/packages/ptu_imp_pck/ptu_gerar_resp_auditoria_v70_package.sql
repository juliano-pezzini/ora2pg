...
CREATE OR REPLACE PROCEDURE ptu_imp_pck.ptu_gerar_resp_auditoria_v70 ( nr_seq_resp_aud_p ptu_resposta_auditoria.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) AS $body$
DECLARE
...
for c01_w in C01( nr_seq_resp_aud_p ) loop
	--Verificar a origem da resposta de auditoria, se a mesma for gerada para um Pedido de Autorizacao ou Pedido de Complemento
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_imp_pck.ptu_gerar_resp_auditoria_v70 ( nr_seq_resp_aud_p ptu_resposta_auditoria.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) FROM PUBLIC;
