...
CREATE OR REPLACE PROCEDURE ptu_imp_pck.ptu_processa_00412_v70 ( nr_seq_cons_benef_p ptu_consulta_beneficiario.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p ptu_consulta_beneficiario.nm_usuario%type, nr_seq_resp_cons_p INOUT ptu_resp_consulta_benef.nr_sequencia%type) AS $body$
DECLARE


/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade:  Validar a importacao da consulta de dados do beneficiario.

Rotina utilizada nas transacoes ptu via WebService.
...
cd_local_atend_w		ptu_resp_nomes_benef.cd_local_cobranca%type;
nr_seq_segurado_repasse_w	pls_segurado_repasse.nr_sequencia%type;

-- Buscar informacoes dos beneficiarios quando informada a carteirinha
...
C01 CURSOR(	cd_carteirinha_wc		pls_segurado_carteira.cd_usuario_plano%type) FOR
	SELECT	distinct(a.cd_pessoa_fisica) cd_pessoa_fisica,
		substr(a.nm_pessoa_fisica,1,120) nm_pessoa_fisica,
		a.ie_sexo,
		substr(a.nm_pessoa_fisica,1,25) nm_beneficiario,
		...
		pls_obter_dados_segurado(c.nr_sequencia, 'NST') nm_social
	from	pls_segurado			c,
	...
	--and	c.ie_tipo_segurado	in('A','B','P'); Retirado restricao, pois nao estavam sendo verificados todos os tipos de beneficiarios.


-- Buscar informacoes dos beneficiarios quando nao informada a carteirinha
	...
	for c01_w in C01( cd_carteirinha_w ) loop
		for c03_w in C03( c01_w.nr_seq_plano ) loop
			...
			begin
				...
				and	((coalesce(dt_fim_repasse::text, '') = '') or (trunc(dt_fim_repasse) >= trunc(clock_timestamp())))
				and	(dt_liberacao IS NOT NULL AND dt_liberacao::text <> '')
				and (trunc(dt_repasse) <= trunc(clock_timestamp()))
				and	ie_tipo_compartilhamento = '1';

			exception
			when others then
				nr_seq_segurado_repasse_w := null;
			end;

			if (nr_seq_segurado_repasse_w IS NOT NULL AND nr_seq_segurado_repasse_w::text <> '') then
				select	adiciona_zeros_esquerda(substr(pls_obter_cd_seq_congenere(coalesce(nr_seq_congenere_atend, nr_seq_congenere),'CD'),1,255), 4)
			...
			insert	into ptu_resp_nomes_benef(nr_sequencia, nr_seq_resp_benef, nm_beneficiario,
				 ...
				 cd_rede_atendimento, ie_tipo_plano, nm_social)
			values (nextval('ptu_resp_nomes_benef_seq'), nr_seq_resp_cons_p, c01_w.nm_beneficiario,
				...
				ie_abrangencia_w, coalesce(cd_local_atend_w, cd_unimed_beneficiario_w), c01_w.dt_validade_carteira,
				c01_w.dt_inclusao_operadora, c01_w.dt_rescisao, c01_w.ie_sexo,
				c01_w.nr_via_solicitacao, nm_usuario_p, clock_timestamp(),
				c03_w.cd_rede_atendimento, ie_regulamentacao_w, c01_w.nm_social);
	...
	for c04_w in C04( nr_cpf_w ) loop
		for c03_w in C03( c04_w.nr_seq_plano ) loop
			...
			begin
				...
				into STRICT	nr_seq_segurado_repasse_w
				from	pls_segurado_repasse
				where	nr_seq_segurado = c04_w.nr_seq_segurado
				and	((coalesce(dt_fim_repasse::text, '') = '') or (trunc(dt_fim_repasse) >= trunc(clock_timestamp())))
				and	(dt_liberacao IS NOT NULL AND dt_liberacao::text <> '')
				and (trunc(dt_repasse) <= trunc(clock_timestamp()))
				and	ie_tipo_compartilhamento = '1';
			...
			values (nextval('ptu_resp_nomes_benef_seq'), nr_seq_resp_cons_p, c04_w.nm_beneficiario,
				...
				clock_timestamp(), nm_usuario_p, coalesce(ie_tipo_acomodacao_ptu_w,'C'),
				ie_abrangencia_w, coalesce(cd_local_atend_w, cd_unimed_beneficiario_w), c04_w.dt_validade_carteira,
				c04_w.dt_inclusao_operadora, c04_w.dt_rescisao, c04_w.ie_sexo,
				c04_w.nr_via_solicitacao, nm_usuario_p, clock_timestamp(),
				c03_w.cd_rede_atendimento, ie_regulamentacao_w, c04_w.nm_social);
	...
	for c05_w in C05( nr_cns_w ) loop
		for c03_w in C03( c05_w.nr_seq_plano ) loop
			...
			begin
				...
				into STRICT	nr_seq_segurado_repasse_w
				from	pls_segurado_repasse
				where	nr_seq_segurado = c05_w.nr_seq_segurado
				and	((coalesce(dt_fim_repasse::text, '') = '') or (trunc(dt_fim_repasse) >= trunc(clock_timestamp())))
				and	(dt_liberacao IS NOT NULL AND dt_liberacao::text <> '')
				and (trunc(dt_repasse) <= trunc(clock_timestamp()))
				and	ie_tipo_compartilhamento = '1';
			...
			values (nextval('ptu_resp_nomes_benef_seq'), nr_seq_resp_cons_p, c05_w.nm_beneficiario,
				...
				clock_timestamp(), nm_usuario_p, coalesce(ie_tipo_acomodacao_ptu_w,'C'),
				ie_abrangencia_w, coalesce(cd_local_atend_w, cd_unimed_beneficiario_w), c05_w.dt_validade_carteira,
				c05_w.dt_inclusao_operadora, c05_w.dt_rescisao, c05_w.ie_sexo,
				c05_w.nr_via_solicitacao, nm_usuario_p, clock_timestamp(),
				c03_w.cd_rede_atendimento, ie_regulamentacao_w, c05_w.nm_social);
	...
	for c02_w in C02( nm_benef_consulta_w, dt_nasc_benef_w  ) loop
		for c03_w in C03( c02_w.nr_seq_plano ) loop
			...
			begin
				...
				into STRICT	nr_seq_segurado_repasse_w
				from	pls_segurado_repasse
				where	nr_seq_segurado = c02_w.nr_seq_segurado
				and	((coalesce(dt_fim_repasse::text, '') = '') or (trunc(dt_fim_repasse) >= trunc(clock_timestamp())))
				and	(dt_liberacao IS NOT NULL AND dt_liberacao::text <> '')
				and (trunc(dt_repasse) <= trunc(clock_timestamp()))
				and	ie_tipo_compartilhamento = '1';
			...
			values (nextval('ptu_resp_nomes_benef_seq'), nr_seq_resp_cons_p, c02_w.nm_beneficiario,
				...
				clock_timestamp(), nm_usuario_p, coalesce(ie_tipo_acomodacao_ptu_w,'C'),
				ie_abrangencia_w, coalesce(cd_local_atend_w, cd_unimed_beneficiario_w), c02_w.dt_validade_carteira,
				c02_w.dt_inclusao_operadora, c02_w.dt_rescisao, c02_w.ie_sexo,
				c02_w.nr_via_solicitacao, nm_usuario_p, clock_timestamp(),
				c03_w.cd_rede_atendimento, ie_regulamentacao_w, c02_w.nm_social);
...
when others then
	ie_inseriu_w	:= 'N';
	-- Rollback pois caso houver erro o sistema devera abortar os registros que foram inseridos e inserir a inconsitencia.
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_imp_pck.ptu_processa_00412_v70 ( nr_seq_cons_benef_p ptu_consulta_beneficiario.nr_sequencia%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p ptu_consulta_beneficiario.nm_usuario%type, nr_seq_resp_cons_p INOUT ptu_resp_consulta_benef.nr_sequencia%type) FROM PUBLIC;
