...
CREATE OR REPLACE PROCEDURE ptu_imp_pck.ptu_gerar_resp_status_v70 ( nr_seq_controle_p bigint, nr_seq_pedido_status_p bigint, nr_versao_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_origem_p INOUT bigint) AS $body$
DECLARE

/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: Gerar a transacao PTU de resposta a um pedido de status de transacao, via SCS
-------------------------------------------------------------------------------------------------------------------
...
qt_proc_mat_aprov_w		pls_requisicao_proc.qt_procedimento%type;
...
if (coalesce(nr_seq_guia_w,0)	<> 0) then
	...
	CALL pls_guia_gravar_historico(nr_seq_guia_w,2,expressao_pck.obter_desc_expressao(962718)||cd_unimed_exec_w,'',nm_usuario_p);
	...
	CALL pls_requisicao_gravar_hist(nr_seq_requisicao_w,'L',expressao_pck.obter_desc_expressao(962718)||cd_unimed_exec_w,null,nm_usuario_p);
else
...
values (nr_seq_resp_ped_status_w, '00361', cd_unimed_benef_w,
	 ...
	 nr_seq_guia_w, nr_seq_requisicao_w, pls_obter_versao_scs,
...
EXIT WHEN NOT FOUND; /* apply on C01 */
	...
	if (pls_obter_versao_scs = '090') then
		...
		if (qt_registro_w > 0) then
			if (coalesce(ds_servico_w,'X')	= 'X') then
...
if (nr_seq_requisicao_w IS NOT NULL AND nr_seq_requisicao_w::text <> '') then
	...
	if (ie_estagio_w in (2,6)) and (ie_tipo_intercambio_w = 'E') then
		select	count(1)
		into STRICT	qt_exec_req_w
		from	pls_execucao_requisicao
		where	nr_seq_requisicao	= nr_seq_requisicao_w;

		if (qt_exec_req_w = 0) then
			CALL pls_executar_req_interc_aprov(nr_seq_requisicao_w, cd_estabelecimento_p, nm_usuario_p);
		end if;
	end if;
end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_imp_pck.ptu_gerar_resp_status_v70 ( nr_seq_controle_p bigint, nr_seq_pedido_status_p bigint, nr_versao_p text, cd_estabelecimento_p bigint, nm_usuario_p text, nr_seq_origem_p INOUT bigint) FROM PUBLIC;
