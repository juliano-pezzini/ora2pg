...
CREATE OR REPLACE PROCEDURE ptu_imp_pck.ptu_gerar_resp_pedido_aut_v70 ( nr_seq_controle_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE

/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: Gerar a transacao PTU de resposta de pedido de autorizacao, via SCS
-------------------------------------------------------------------------------------------------------------------
...
ie_origem_proced_receb_w	ptu_pedido_aut_servico.ie_origem_servico%type;
nm_social_w			ptu_resposta_autorizacao.nm_social%type;
qt_idade_benef_w		ptu_resposta_autorizacao.qt_idade_benef%type;
ie_complementar_1_w		ptu_resposta_autorizacao.ie_complementar_1%type;
ie_complementar_2_w		ptu_resposta_autorizacao.ie_complementar_2%type;
ie_complementar_3_w		ptu_resposta_autorizacao.ie_complementar_3%type;
...
if (nr_seq_guia_w IS NOT NULL AND nr_seq_guia_w::text <> '') then
	...
	CALL pls_guia_gravar_historico(nr_seq_guia_w,2,expressao_pck.obter_desc_expressao(620577)||' '||cd_unimed_exec_w,'',nm_usuario_p);
	...
	CALL pls_requisicao_gravar_hist(nr_seq_requisicao_w,'L',expressao_pck.obter_desc_expressao(620577)||' '||cd_unimed_exec_w,null,nm_usuario_p);
end if;
...
if (nr_seq_segurado_w IS NOT NULL AND nr_seq_segurado_w::text <> '') then
	begin
		select	substr(b.nm_pessoa_fisica,1,25),
			b.ie_sexo,
			b.dt_nascimento,
			pls_obter_dados_segurado(a.nr_sequencia, 'NST'),
	...
	exception
	when others then
		nm_segurado_w		:= expressao_pck.obter_desc_expressao(620578);
		ie_sexo_ww		:= null;
	...
	nm_segurado_w		:= expressao_pck.obter_desc_expressao(620578);
	nm_social_w		:= expressao_pck.obter_desc_expressao(620578);
end if;
...
values (nr_seq_resp_novo_w, '00501', cd_unimed_exec_w,
	 ...
	 'Recebido e processado o pedido de '||CASE WHEN nr_seq_pedido_aut_w=0 THEN expressao_pck.obter_desc_expressao(620579)  ELSE expressao_pck.obter_desc_expressao(827501) END , ie_tipo_cliente_w, nr_seq_requisicao_w,
	 nr_seq_controle_p, ie_tipo_resposta_w, '1',
	 coalesce(ie_tipo_acomodacao_w,'C'), nm_usuario_p, clock_timestamp(),
	 pls_obter_versao_scs, ie_sexo_w, to_date(dt_nascimento_w, 'dd/mm/yyyy'),
	 nm_social_w, qt_idade_benef_w, ie_complementar_1_w,
	 ie_complementar_2_w, ie_complementar_3_w);
	...
	if (ie_tipo_tabela_w	in ('0','1','4')) then
		...
		if (cd_servico_w IS NOT NULL AND cd_servico_w::text <> '') then
			if (coalesce(ds_opme_w,'X')	= 'X') then
				select	substr(coalesce(obter_descricao_procedimento(coalesce(cd_servico_consersao_w,cd_servico_w),coalesce(ie_origem_proced_w,5)), expressao_pck.obter_desc_expressao(612231)),1,80),
			...
			if (pls_obter_versao_scs = '090') then
				...
				where 	cd_proc_mat_generico	= coalesce(cd_servico_consersao_w,cd_servico_w)
				and	ie_situacao		= 'A';
			...
			values (nr_seq_resp_proc_w, nr_seq_resp_novo_w, ie_tipo_tabela_w,
				 ...
				 ie_origem_proced_receb_w, nr_seq_guia_proc_w, nr_seq_req_proc_w,
		...
		pls_obter_mat_tiss_vigente( nr_seq_material_w, clock_timestamp(), coalesce(cd_servico_consersao_w,cd_servico_w), 'O', 'N', pls_obter_versao_tiss);
		exception
		when others then
			nr_seq_material_w	:= 0;
		end;
		...
		if (cd_servico_w IS NOT NULL AND cd_servico_w::text <> '') then
			if (coalesce(ds_opme_w,'X')	= 'X') then
				select	substr(coalesce(pls_obter_desc_material(nr_seq_material_w), expressao_pck.obter_desc_expressao(612231)),1,80),
					nextval('ptu_resposta_aut_servico_seq')
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_imp_pck.ptu_gerar_resp_pedido_aut_v70 ( nr_seq_controle_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
