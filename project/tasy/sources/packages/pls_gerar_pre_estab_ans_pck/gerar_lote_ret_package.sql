...
nm_arquivo_w := replace(nm_arquivo_p,'ZTE','XTE');
...
-- se não encontrou o lote de origem levando em consideração o arquivo, então lança uma mensagem de erro
if (coalesce(nr_seq_lote_monitor_w::text, '') = '') then
	CALL wheb_mensagem_pck.exibir_mensagem_abort(333805);
end if;

-- Verifica se já existe um lote de retorno desse lote de envio
...
if (coalesce(nr_seq_lote_com_w::text, '') = '') then
	...
	values (	nextval('pls_monitor_tiss_lote_com_seq'),nm_usuario_p,clock_timestamp(),nm_usuario_p,clock_timestamp(),
			dt_mes_competencia_p,nr_seq_lote_monitor_w,cd_estabelecimento_p,'PE')
	returning nr_sequencia into nr_seq_lote_com_w;
end if;
...
-- Quando vier o código de glosa negando o Lote será salvo nesta rotina
if (cd_glosa_p IS NOT NULL AND cd_glosa_p::text <> '') then
	CALL pls_gerar_pre_estab_ans_pck.gerar_glosa_pre_ret( nr_seq_lote_monitor_ret_p, null, cd_glosa_p, null, nm_usuario_p);
end if;

-- Atualiza os dados do lote de retorno
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerar_pre_estab_ans_pck.gerar_lote_ret ( cd_ans_p pls_monitor_tiss_lote_ret.cd_ans%type, cd_versao_tiss_p pls_monitor_tiss_lote_ret.cd_versao_tiss%type, dt_geracao_lote_p pls_monitor_tiss_lote_ret.dt_geracao_lote%type, dt_mes_competencia_p pls_monitor_tiss_lote_ret.dt_mes_competencia%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nr_lote_p pls_monitor_tiss_lote_ret.nr_lote%type, cd_glosa_p pls_monitor_tiss_glosa.cd_glosa%type, nm_arquivo_p pls_monitor_tiss_lote_ret.nm_arquivo%type, qt_incluido_p pls_monitor_tiss_lote_ret.qt_total_incluido%type, qt_alterado_p pls_monitor_tiss_lote_ret.qt_total_alterado%type, qt_erros_p pls_monitor_tiss_lote_ret.qt_total_erro%type, qt_excluido_p pls_monitor_tiss_lote_ret.qt_total_excluido%type, nr_seq_lote_monitor_ret_p INOUT pls_monitor_tiss_lote_ret.nr_sequencia%type) FROM PUBLIC;
