...
SET client_encoding TO 'UTF8';
	...
	-->>>>>>>>>>>>>>>>>>>>>>>>>> Quantidade/Valor total de titulos a receber vinculados ao contrato <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<--
	/*
	Objetivo: Retornar a quantidade/valor total de titulos a receber vinculados ao contrato. 
	...
	*/
CREATE OR REPLACE FUNCTION gerenciamento_contrato_pck.obter_qt_vl_titulos_receber (nr_seq_contrato_p bigint, ie_qt_vl_p text, ie_atual_p text) RETURNS bigint AS $body$
DECLARE
...
BEGIN
			...
			if (ie_atual_p = 'N') then
				...
				select coalesce(sum(gerenciamento_contrato_pck.obter_valor_moeda_convertido(nr_seq_contrato_p, cd_moeda_contrato_w, a.cd_moeda, gerenciamento_contrato_pck.obter_vl_titulo_receber(a.nr_titulo))),0),
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION gerenciamento_contrato_pck.obter_qt_vl_titulos_receber (nr_seq_contrato_p bigint, ie_qt_vl_p text, ie_atual_p text) FROM PUBLIC;
