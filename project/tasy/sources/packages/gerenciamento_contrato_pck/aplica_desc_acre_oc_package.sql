...
SET client_encoding TO 'UTF8';
	...
	-->>>>>>>>>>>>>>>>>>>> Obter valor total liquido de um item aplicando os descontos e acrescimos da Ordem de Compra <<<<<<<<<<<<<<<<<<<<<<<<<<<<<--
	/*
	...
	vl_liq_item_p = Valor liquido do item da ordem de compra(pode ser obtido atravez da function obter_vl_liq_oci)
	*/
CREATE OR REPLACE FUNCTION gerenciamento_contrato_pck.aplica_desc_acre_oc ( nr_ordem_compra_p bigint, vl_liq_item_p bigint) RETURNS bigint AS $body$
DECLARE
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION gerenciamento_contrato_pck.aplica_desc_acre_oc ( nr_ordem_compra_p bigint, vl_liq_item_p bigint) FROM PUBLIC;
