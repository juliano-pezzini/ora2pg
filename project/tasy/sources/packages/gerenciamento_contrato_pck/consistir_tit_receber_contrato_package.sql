...
SET client_encoding TO 'UTF8';
	...
	-->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Consistir titulo a receber vinculado ao contrato <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<--		
	/*
	Objetivo: Consistir a criacao de um titulo a receber caso o contrato nao tenha saldo suficiente para atender esse titulo.
	Parametros: 
	nr_seq_contrato_p = Numero de sequencia do contrato.
	vl_saldo_titulo_p = Valor de saldo do titulo a receber.
	nm_usuario_p = Nome do usuario.
	*/
CREATE OR REPLACE PROCEDURE gerenciamento_contrato_pck.consistir_tit_receber_contrato (nr_seq_contrato_p bigint, nr_titulo_p bigint default 0, vl_saldo_titulo_p bigint DEFAULT NULL, nm_usuario_p text DEFAULT NULL, ie_atualizacao_saldo_p text default null) AS $body$
DECLARE
		...
		if (ie_consiste_saldo_contrato_w = 'C') then
			...
			if (nr_seq_contrato_p > 0) then
				...
				if (ie_atualizacao_saldo_p = 'S') then
					vl_saldo_contrato_w := coalesce(gerenciamento_contrato_pck.obter_valor_moeda_convertido(nr_seq_contrato_p, cd_moeda_contrato_w, coalesce(cd_moeda_titulo_w, cd_moeda_contrato_w), gerenciamento_contrato_pck.obter_vl_titulo_receber(nr_titulo_p)),0) + gerenciamento_contrato_pck.obter_valor_saldo(nr_seq_contrato_p, 'S');
				else
				...
				if (vl_saldo_contrato_w < vl_titulo_w) then
					...
					CALL wheb_mensagem_pck.exibir_mensagem_abort(wheb_mensagem_pck.get_texto(1082502) || CHR(10) ||
																...
																'VL_SALDO_W='|| campo_mascara_virgula_casas(vl_saldo_contrato_w + coalesce(gerenciamento_contrato_pck.obter_vl_titulo_receber(nr_titulo_p),0),4) ||						
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerenciamento_contrato_pck.consistir_tit_receber_contrato (nr_seq_contrato_p bigint, nr_titulo_p bigint default 0, vl_saldo_titulo_p bigint DEFAULT NULL, nm_usuario_p text DEFAULT NULL, ie_atualizacao_saldo_p text default null) FROM PUBLIC;
