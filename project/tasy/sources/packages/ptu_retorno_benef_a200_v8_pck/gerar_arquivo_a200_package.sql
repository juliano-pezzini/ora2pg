...
CREATE OR REPLACE PROCEDURE ptu_retorno_benef_a200_v8_pck.gerar_arquivo_a200 ( nr_seq_retorno_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ds_erro_p INOUT text, ds_nome_arq_p INOUT text, ds_caminho_rede_p INOUT text) AS $body$
DECLARE
...
current_setting('ptu_retorno_benef_a200_v8_pck.nm_arquivo_w')::varchar(255)			varchar(255);
ds_caminho_rede_w		varchar(255) := null;


BEGIN
PERFORM set_config('ptu_retorno_benef_a200_v8_pck.ds_arquivo_w', null, false);
...
into STRICT	current_setting('ptu_retorno_benef_a200_v8_pck.nm_arquivo_w')::varchar(255)
...
exception
when others then
	PERFORM set_config('ptu_retorno_benef_a200_v8_pck.nm_arquivo_w', null, false);
end;

begin
arq_texto_w := utl_file.fopen(ds_local_w,current_setting('ptu_retorno_benef_a200_v8_pck.nm_arquivo_w')::varchar(255),'w');
exception
when others then
...
CALL CALL CALL CALL CALL ptu_retorno_benef_a200_v8_pck.processar_arquivo(nr_seq_retorno_p,arq_texto_w);

ds_nome_arq_p := current_setting('ptu_retorno_benef_a200_v8_pck.nm_arquivo_w')::varchar(255);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_retorno_benef_a200_v8_pck.gerar_arquivo_a200 ( nr_seq_retorno_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ds_erro_p INOUT text, ds_nome_arq_p INOUT text, ds_caminho_rede_p INOUT text) FROM PUBLIC;
