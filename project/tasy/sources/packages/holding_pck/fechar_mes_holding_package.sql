...
CREATE OR REPLACE PROCEDURE holding_pck.fechar_mes_holding (nm_usuario_p usuario.nm_usuario%type, nr_sequencia_p ctb_mes_ref.nr_sequencia%type) AS $body$
DECLARE
...
nr_seq_mes_hold_w       ctb_mes_ref.nr_sequencia%type;
    ...
    if (nr_sequencia_hold_w IS NOT NULL AND nr_sequencia_hold_w::text <> '') then
        ...
        EXIT WHEN NOT FOUND; /* apply on c010 */
            ...
            when too_many_rows then
                ...
                CALL wheb_mensagem_pck.exibir_mensagem_abort(1148144, 'dt_referencia_w=' || to_char(dt_referencia_w,'mm/yyyy') || ';ds_empresa_w=' || ds_empresa_w);
            end;

            if (nr_seq_mes_emp_w IS NOT NULL AND nr_seq_mes_emp_w::text <> '') and (coalesce(dt_fec_mes_emp_w::text, '') = '') then
                begin
        ...
        if (coalesce(ds_empresas_w::text, '') = '') then
            CALL ctb_abrir_fechar_mes(nr_sequencia_p, 'N', nm_usuario_p);
        else
            /* Nao foi possivel fechar o mes #@mes_aberto_w#@ da(s) empresa(s) #@ds_empresas_w#@. Verifique as liberacoes e validacoes para fechamento de meses nestas empresas. */


            CALL wheb_mensagem_pck.exibir_mensagem_abort(1148147,'mes_aberto_w=' || to_char(dt_referencia_w,'mm/yyyy') || ';ds_empresas_w=' || ds_empresas_w);
        end if;
    else
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE holding_pck.fechar_mes_holding (nm_usuario_p usuario.nm_usuario%type, nr_sequencia_p ctb_mes_ref.nr_sequencia%type) FROM PUBLIC;
