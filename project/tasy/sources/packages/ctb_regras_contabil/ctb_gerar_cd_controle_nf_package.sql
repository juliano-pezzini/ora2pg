...
CREATE OR REPLACE PROCEDURE ctb_regras_contabil.ctb_gerar_cd_controle_nf ( nr_lote_contabil_p NOTA_FISCAL.NR_LOTE_CONTABIL%type, nm_usuario_p text ) AS $body$
DECLARE
		...
		IF (coalesce(movimento.nr_codigo_controle::text, '') = '') THEN
			BEGIN
				...
				INTO STRICT    ie_tipo_nota_w,
					...
					nr_comprovante_w
				FROM    NOTA_FISCAL
				WHERE   NR_SEQUENCIA = movimento.nr_seq_tab_orig;

				arr_data_w	 	:= ctb_regras_contabil.obter_prefix_regra_rest(cd_tipo_lote_contabil_w, ie_tipo_nota_w || ',' || cd_operacao_nf_w);
				ds_prefixo_w		:= arr_data_w(2);
				
				IF (coalesce(nr_comprovante_w::text, '') = '') THEN
				    nr_comprovante_w	:= ctb_regras_contabil.obter_estorno_nota_fiscal(nr_sequencia_ref_w);
				END IF;
				...
				SET	NR_CODIGO_CONTROLE 	= nr_comprovante_w
				WHERE   NR_LOTE_CONTABIL 	= nr_lote_contabil_p
				AND     NR_SEQUENCIA 		= movimento.nr_seq_tab_orig;
			EXCEPTION
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ctb_regras_contabil.ctb_gerar_cd_controle_nf ( nr_lote_contabil_p NOTA_FISCAL.NR_LOTE_CONTABIL%type, nm_usuario_p text ) FROM PUBLIC;
