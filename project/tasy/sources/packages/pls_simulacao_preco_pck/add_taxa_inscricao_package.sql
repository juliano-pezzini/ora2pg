...
CREATE OR REPLACE PROCEDURE pls_simulacao_preco_pck.add_taxa_inscricao ( nr_seq_simul_benef_p pls_simulpreco_individual.nr_sequencia%type, nr_seq_plano_p pls_plano.nr_sequencia%type, ie_titularidade_p text, vl_produto_p bigint) AS $body$
DECLARE
...
BEGIN
	...
	if (vl_inscricao_w > 0) then
		CALL CALL pls_simulacao_preco_pck.add_item(wheb_mensagem_pck.get_texto(1128350), vl_inscricao_w, nr_seq_simul_benef_p, null, '2');
	end if;
	END;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_simulacao_preco_pck.add_taxa_inscricao ( nr_seq_simul_benef_p pls_simulpreco_individual.nr_sequencia%type, nr_seq_plano_p pls_plano.nr_sequencia%type, ie_titularidade_p text, vl_produto_p bigint) FROM PUBLIC;
