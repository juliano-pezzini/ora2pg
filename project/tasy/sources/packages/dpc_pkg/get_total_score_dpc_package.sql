...
CREATE OR REPLACE FUNCTION dpc_pkg.get_total_score_dpc (nr_seq_patient_dpc_p bigint ) RETURNS bigint AS $body$
DECLARE
...
;
  ...
  if (hosp_days_w > qt_days_hosp_3_w) then
	...
	qt_fee_for_service_w	:= (hosp_days_w - qt_days_hosp_3_w) * qt_points_hosp_3_w;
  ...
  ds_return_w := qt_period_1_w + qt_period_2_w + qt_period_3_w + qt_fee_for_service_w;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION dpc_pkg.get_total_score_dpc (nr_seq_patient_dpc_p bigint ) FROM PUBLIC;
