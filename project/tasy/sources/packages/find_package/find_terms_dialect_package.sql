...
CREATE OR REPLACE FUNCTION find_package.find_terms_dialect ( pWords strArray ,pStartRow integer ,pEndRow integer ,pTable text ,pCode text ,pDescription text ,pIndexSeach text ,pRestriction text ,pRestrictionFrom text ,pExtraFields strArray ) RETURNS PIPELINE_CUR_PLUS AS $body$
DECLARE
      ...
      SELECT ' || pCode || ', ' || pDescription || ', matchRange ' || lExtraFields || ' , ROWNUM as rnum FROM (
        SELECT ' || pCode || ', ' || pDescription || lExtraFields || ', TREAT(scr AS SCORING).matchRange AS matchRange FROM (
          SELECT ' || pCode || ', ' || pDescription || lExtraFields || ', TasyAutoComplete.getScore(descrIndex, lIndexWords) AS scr FROM (
            ...
            'WHERE 1 = 1 ' || lRestriction || '
            UNION ALL
            SELECT T.' || pCode || ', DF.DS_DIALECT_FIELD_VALUE AS ' || pDescription || lTableExtraFields || ', TasyAutoComplete.ToIndexWithoutAccents(DF.DS_DIALECT_FIELD_VALUE) AS descrIndex
            FROM ' || pTable || ' T '|| lRestrictionFrom || '
            INNER JOIN DF_' || pTable || ' DF 
                ON DF.CD_ENTITY = T.' || pDescription || '
                AND UPPER(DF.DS_DIALECT_FIELD_NAME) LIKE ''' || pDescription || '%''
            WHERE 1 = 1 ' || lRestriction || '
          )
...
SECURITY DEFINER
 IMMUTABLE;
-- REVOKE ALL ON FUNCTION find_package.find_terms_dialect ( pWords strArray ,pStartRow integer ,pEndRow integer ,pTable text ,pCode text ,pDescription text ,pIndexSeach text ,pRestriction text ,pRestrictionFrom text ,pExtraFields strArray ) FROM PUBLIC;
