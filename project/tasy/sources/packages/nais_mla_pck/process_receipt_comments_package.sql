...
CREATE OR REPLACE PROCEDURE nais_mla_pck.process_receipt_comments ( record_p medtreatmentinforedtyp, cd_external_p cpoe_std_comment.cd_external%type, nr_linkage_sequencia_p cpoe_comment_linkage.nr_sequencia%type ) AS $body$
DECLARE
...
BEGIN
    ...
    if ( ds_additional_text_w <> ' ' and coalesce(si_format_value_w::text, '') = '') then
        ...
        ds_additional_text_w := nais_mla_pck.get_additional_text(nr_linkage_sequencia_p, si_format_value_w, nr_additional_text_w, dt_additional_text_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE nais_mla_pck.process_receipt_comments ( record_p medtreatmentinforedtyp, cd_external_p cpoe_std_comment.cd_external%type, nr_linkage_sequencia_p cpoe_comment_linkage.nr_sequencia%type ) FROM PUBLIC;
