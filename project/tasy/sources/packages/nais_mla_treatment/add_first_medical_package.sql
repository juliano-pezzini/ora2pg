...
CREATE OR REPLACE PROCEDURE nais_mla_treatment.add_first_medical ( record_p medtreatmentinforedtyp, message_data INOUT text, nr_seq_gasoterapia_p bigint ) AS $body$
DECLARE

        record_w medtreatmentinforedtyp;
        medical_affair_code NAIS_CONVERSION_MASTER%rowtype;
        ie_respiracao_ww varchar(10);
...
BEGIN        
		...
		when        no_data_found then
                    ie_respiracao_ww:= null;
		end;

        medical_affair_code :=get_medicalaffair_code('PC','valor_dominio_v','VL_DOMINIO',ie_respiracao_ww);
        record_w.internal_code := medical_affair_code.CD_MEDICAL_AFFAIR;
        record_w.internal_code := record_w.internal_code;
        record_w.dosage := 0;
        record_w.unit := ' ';
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE nais_mla_treatment.add_first_medical ( record_p medtreatmentinforedtyp, message_data INOUT text, nr_seq_gasoterapia_p bigint ) FROM PUBLIC;
