...
CREATE OR REPLACE PROCEDURE pls_cta_consist_carac_item_pck.processar_procedimentos ( dados_regra_p dados_regra) AS $body$
DECLARE
...
dados_restricao_w := pls_cta_consist_carac_item_pck.obter_restricao_regra('RESTRICAO', var_cur_w, 'P', dados_regra_p);
...
if	(((dados_restricao_w.ds_restricao_lote IS NOT NULL AND dados_restricao_w.ds_restricao_lote::text <> '') or
	...
	begin
		...
		dados_restricao_w := pls_cta_consist_carac_item_pck.obter_restricao_regra('BINDS', var_cur_w, 'P', dados_regra_p);
		...
		dbms_sql.define_array(var_cur_w, 4, current_setting('pls_cta_consist_carac_item_pck.tb_liberado_w')::dbms_sql.varchar2_table, current_setting('pls_cta_consist_carac_item_pck.qt_reg_transacao_w')::integer, 1);
		dbms_sql.define_array(var_cur_w, 5, current_setting('pls_cta_consist_carac_item_pck.tb_regra_carac_w')::dbms_sql.number_table, current_setting('pls_cta_consist_carac_item_pck.qt_reg_transacao_w')::integer, 1);
		...
		var_retorno_w := dbms_sql.fetch_rows(var_cur_w);
			...
			dbms_sql.column_value(var_cur_w, 4, current_setting('pls_cta_consist_carac_item_pck.tb_liberado_w')::dbms_sql.varchar2_table);
			dbms_sql.column_value(var_cur_w, 5, current_setting('pls_cta_consist_carac_item_pck.tb_regra_carac_w')::dbms_sql.number_table);
	...
	exception
		...
		CALL CALL pls_cta_consist_carac_item_pck.tratar_erro(ds_sql_w, dados_regra_p);
	end;
end if;
END;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_cta_consist_carac_item_pck.processar_procedimentos ( dados_regra_p dados_regra) FROM PUBLIC;
