...
CREATE OR REPLACE FUNCTION pfcs_pck_los.get_avg_los ( cd_establishment_p bigint, cd_unit_p bigint default null, cd_unit_classification_p text  DEFAULT NULL) RETURNS bigint AS $body$
DECLARE

        vl_avg_los_w double precision;

BEGIN
        select coalesce(avg(svc.dt_authored_on - enc.period_start),0)
        into STRICT vl_avg_los_w
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION pfcs_pck_los.get_avg_los ( cd_establishment_p bigint, cd_unit_p bigint default null, cd_unit_classification_p text  DEFAULT NULL) FROM PUBLIC;
