...
CREATE OR REPLACE PROCEDURE ptu_retorno_benef_a200_90_pck.processar_arquivo ( nr_seq_retorno_p bigint, arq_texto_p utl_file.file_type) AS $body$
DECLARE
...
BEGIN
PERFORM set_config('ptu_retorno_benef_a200_90_pck.nr_sequencial_arq_w', 0, false);
...
for r_C01_w in C01(nr_seq_retorno_p) loop
	...
	if (r_C01_w.tp_registro = 1) then
		...
		CALL CALL CALL CALL CALL CALL ptu_retorno_benef_a200_90_pck.incluir_linha_arq('201', ds_conteudo_w, arq_texto_p);
		...
		CALL CALL CALL CALL CALL CALL ptu_retorno_benef_a200_90_pck.incluir_linha_arq('202', ds_conteudo_w, arq_texto_p);
		...
		CALL CALL CALL CALL CALL CALL ptu_retorno_benef_a200_90_pck.incluir_linha_arq('209', ds_conteudo_w, arq_texto_p);
	end if;
end loop;

current_setting('ptu_retorno_benef_a200_90_pck.ds_arquivo_w')::text := pls_hash_ptu_pck.obter_hash_txt(current_setting('ptu_retorno_benef_a200_90_pck.ds_arquivo_w')::text);
ds_conteudo_w	:= lpad(ds_hash_w,32,' ');
CALL CALL CALL CALL CALL CALL ptu_retorno_benef_a200_90_pck.incluir_linha_arq('998',ds_conteudo_w,arq_texto_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_retorno_benef_a200_90_pck.processar_arquivo ( nr_seq_retorno_p bigint, arq_texto_p utl_file.file_type) FROM PUBLIC;
