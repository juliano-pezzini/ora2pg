...
CREATE OR REPLACE FUNCTION obter_saldo_disp_estoque_pck.get_qt_comp_kit_estoque ( cd_estabelecimento_p bigint, cd_material_p bigint, cd_local_estoque_p bigint default null, nr_seq_lote_p bigint default null) RETURNS bigint AS $body$
DECLARE
...
BEGIN
	...
	if (current_setting('obter_saldo_disp_estoque_pck.ie_disp_comp_kit_estoque_w')::parametro_estoque.ie_disp_comp_kit_estoque%type = 'S') then
	...
	elsif (current_setting('obter_saldo_disp_estoque_pck.ie_disp_comp_kit_estoque_w')::parametro_estoque.ie_disp_comp_kit_estoque%type = 'C') then
		if (cd_local_estoque_p > 0) then
			if	((nr_seq_lote_p > 0) and (current_setting('obter_saldo_disp_estoque_pck.ie_estoque_lote_w')::material_estab.ie_estoque_lote%type = 'S')) then
				select	coalesce(sum(a.qt_material), 0)
				into STRICT	qt_kit_w
	...
	qt_kit_w	:=	dividir(qt_kit_w, current_setting('obter_saldo_disp_estoque_pck.qt_conv_estoque_w')::material.qt_conv_estoque_consumo%type);
	return qt_kit_w;
	end;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION obter_saldo_disp_estoque_pck.get_qt_comp_kit_estoque ( cd_estabelecimento_p bigint, cd_material_p bigint, cd_local_estoque_p bigint default null, nr_seq_lote_p bigint default null) FROM PUBLIC;
