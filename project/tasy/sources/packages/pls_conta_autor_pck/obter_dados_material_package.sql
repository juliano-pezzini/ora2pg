...
CREATE OR REPLACE FUNCTION pls_conta_autor_pck.obter_dados_material ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nr_seq_material_p pls_guia_plano_mat.nr_seq_material%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) RETURNS SETOF T_CONTA_AUTOR_DATA AS $body$
DECLARE
...
for r_C01 in C01(nr_seq_guia_p, nr_seq_material_p) loop
	...
	for r_c02_w in C02(nr_seq_guia_p, r_C01.nr_seq_material) loop
		begin
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION pls_conta_autor_pck.obter_dados_material ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nr_seq_material_p pls_guia_plano_mat.nr_seq_material%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) FROM PUBLIC;
