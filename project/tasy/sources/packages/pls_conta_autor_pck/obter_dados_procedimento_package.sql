...
CREATE OR REPLACE FUNCTION pls_conta_autor_pck.obter_dados_procedimento ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, ie_origem_proced_p pls_guia_plano_proc.ie_origem_proced%type, cd_procedimento_p pls_guia_plano_proc.cd_procedimento%type) RETURNS SETOF T_CONTA_AUTOR_DATA AS $body$
DECLARE
...
if (cd_procedimento_p IS NOT NULL AND cd_procedimento_p::text <> '') and (ie_origem_proced_p IS NOT NULL AND ie_origem_proced_p::text <> '') then
	...
	for r_C01 in C01(nr_seq_guia_p, ie_origem_proced_p, cd_procedimento_p) loop
		...
		for r_c02_w in C02(nr_seq_guia_p,r_C01.cd_procedimento,r_C01.ie_origem_proced ) loop
			begin
	...
	for r_C03 in C03(nr_seq_guia_p) loop
		...
		for r_c02_w in C02(nr_seq_guia_p,r_C03.cd_procedimento,r_C03.ie_origem_proced ) loop
			begin
			/* tratamento para atualizar a quantidade utilizada e saldo das guias de complemento*/
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION pls_conta_autor_pck.obter_dados_procedimento ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, ie_origem_proced_p pls_guia_plano_proc.ie_origem_proced%type, cd_procedimento_p pls_guia_plano_proc.cd_procedimento%type) FROM PUBLIC;
