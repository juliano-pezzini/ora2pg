...
if (tb_nr_titulo_receber_p.count > 0) then

	forall i in tb_nr_titulo_receber_p.first..tb_nr_titulo_receber_p.last
		insert into pls_pp_item_lote(	
			nr_sequencia, dt_atualizacao, dt_atualizacao_nrec, 
			ie_tipo_item, nm_usuario, nm_usuario_nrec, 
			nr_titulo_receber, vl_item, vl_glosa, 
			vl_liquido, nr_seq_evento, nr_seq_regra_tit_rec, 
			...
			'4', tb_nm_usuario_p(i), tb_nm_usuario_p(i), 
			tb_nr_titulo_receber_p(i), tb_vl_saldo_titulo_p(i), 0, 
	...
	for i in tb_nr_titulo_receber_p.first..tb_nr_titulo_receber_p.last loop

		CALL baixa_titulo_receber(	tb_cd_estabelecimento_p(i), pls_pp_lote_pagamento_pck.cd_tipo_receb_pgto_w,
					tb_nr_titulo_receber_p(i), pls_pp_lote_pagamento_pck.nr_seq_trans_fin_rec_pgto_w,
					tb_vl_saldo_titulo_p(i), clock_timestamp(),
					tb_nm_usuario_p(i), 0,
					null, null,
					0, 0);
		commit;
		...
		where	nr_titulo = tb_nr_titulo_receber_p(i);
	end loop;

	-- coloca uma observação no título para identificar o motivo do estorno da baixa
	forall i in tb_nr_titulo_receber_p.first..tb_nr_titulo_receber_p.last
		update	titulo_receber_liq
		set	ds_observacao = 'Baixa gerada através da regra (' || tb_nr_seq_regra_p(i) || ') de evento para títulos a receber no lote ' || tb_nr_seq_lote_p(i) || '.' || pls_util_pck.enter_w ||
					' Função OPS - Pagamentos de Produção Médica (nova).'
		where	nr_sequencia = tb_nr_seq_baixa_w(i)
		and	nr_titulo = tb_nr_titulo_receber_p(i);
	commit;

	-- por último atualiza a baixa do título (rotina padrão após realizar a baixa de um título)
	for i in tb_nr_titulo_receber_p.first..tb_nr_titulo_receber_p.last loop

		CALL atualizar_saldo_tit_rec(tb_nr_titulo_receber_p(i), tb_nm_usuario_p(i));
