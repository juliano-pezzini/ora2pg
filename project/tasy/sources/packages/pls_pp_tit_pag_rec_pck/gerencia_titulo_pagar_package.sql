...
CREATE OR REPLACE PROCEDURE pls_pp_tit_pag_rec_pck.gerencia_titulo_pagar ( nr_seq_lote_p pls_pp_lote.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) AS $body$
DECLARE
...
ds_alias_tit_pag_filtr_w	varchar(60);
...
ds_alias_tit_pag_filtr_w := pls_pp_tit_pag_rec_pck.obter_alias_tabela('titulo_pagar_filtr');
...
-- abre o cursor com as regras de título a pagar
for r_c_regra_w in c_regra( nr_seq_lote_p ) loop
	...
	if (ie_regra_valida_w = 'S') then
		...
		valor_bind_w := pls_pp_tit_pag_rec_pck.obter_restr_titulo_pagar(
				pls_pp_lote_pagamento_pck.dt_referencia_lote_ini_w, pls_pp_lote_pagamento_pck.dt_referencia_lote_fim_w, r_c_regra_w.ie_tipo_pessoa, r_c_regra_w.cd_pessoa_fisica, r_c_regra_w.cd_cgc, r_c_regra_w.nr_seq_trans_fin_contab, r_c_regra_w.ie_tipo_titulo, r_c_regra_w.ie_origem_titulo, r_c_regra_w.nr_seq_classe_tit_pagar, cd_estabelecimento_p, valor_bind_w);
				
		ds_select_w := 	'select	' || ds_alias_tit_pag_filtr_w || '.nr_titulo,' || pls_util_pck.enter_w ||
				'	' || ds_alias_tit_pag_filtr_w || '.vl_saldo_titulo,' || pls_util_pck.enter_w ||
				'	' || ds_alias_tit_pag_filtr_w || '.cd_pessoa_fisica,' || pls_util_pck.enter_w ||
				'	' || ds_alias_tit_pag_filtr_w || '.cd_cgc,' || pls_util_pck.enter_w ||
				'	' || ds_alias_tit_pag_filtr_w || '.qt_tit_pag,' || pls_util_pck.enter_w ||
				'	z.nr_seq_prestador_pgto' || pls_util_pck.enter_w ||
				'from	(select	' || ds_alias_tit_pag_w || '.nr_titulo, ' || pls_util_pck.enter_w ||
				'		' || ds_alias_tit_pag_w || '.vl_saldo_titulo, ' || pls_util_pck.enter_w ||
				'		' || ds_alias_tit_pag_w || '.cd_pessoa_fisica, ' || pls_util_pck.enter_w ||
				'		' || ds_alias_tit_pag_w || '.cd_cgc, ' || pls_util_pck.enter_w ||
				'		(select	count(1) ' || pls_util_pck.enter_w ||
				'		from	pls_pp_item_lote ' || ds_alias_pp_item_lote_w || pls_util_pck.enter_w ||
				'		where	' || ds_alias_pp_item_lote_w || '.nr_titulo_pagar = ' || ds_alias_tit_pag_w || '.nr_titulo) qt_tit_pag,' || pls_util_pck.enter_w ||
				'		pls_pp_tit_pag_rec_pck.obter_prestador_tmp_pf_pj(:nr_seq_prestador_dest_pc, ' || ds_alias_tit_pag_w || '.cd_pessoa_fisica, ' || ds_alias_tit_pag_w || '.cd_cgc) nr_seq_prestador' || pls_util_pck.enter_w ||
				'	from	titulo_pagar ' || ds_alias_tit_pag_w || pls_util_pck.enter_w ||
				'	where	1 = 1 ' || pls_util_pck.enter_w ||
					ds_restricao_regra_w || ')' || ds_alias_tit_pag_filtr_w || ',' || pls_util_pck.enter_w ||
				'	pls_pp_prestador_tmp z' || pls_util_pck.enter_w ||
				'where z.nr_seq_prestador = ' || ds_alias_tit_pag_filtr_w || '.nr_seq_prestador' || pls_util_pck.enter_w ||
		...
		SELECT * FROM pls_pp_tit_pag_rec_pck.insere_titulo_pagar_item(	nr_contador_w, tb_nr_titulo_pagar_w, tb_vl_saldo_titulo_w, tb_nr_seq_evento_w, tb_nr_seq_regra_w, tb_nr_seq_prestador_w, tb_cd_centro_custo_w, tb_nr_seq_lote_w, tb_nm_usuario, tb_cd_estabelecimento_w) INTO STRICT _ora2pg_r;
 	nr_contador_w := _ora2pg_r.nr_contador_p; tb_nr_titulo_pagar_w := _ora2pg_r.tb_nr_titulo_pagar_p; tb_vl_saldo_titulo_w := _ora2pg_r.tb_vl_saldo_titulo_p; tb_nr_seq_evento_w := _ora2pg_r.tb_nr_seq_evento_p; tb_nr_seq_regra_w := _ora2pg_r.tb_nr_seq_regra_p; tb_nr_seq_prestador_w := _ora2pg_r.tb_nr_seq_prestador_p; tb_cd_centro_custo_w := _ora2pg_r.tb_cd_centro_custo_p; tb_nr_seq_lote_w := _ora2pg_r.tb_nr_seq_lote_p; tb_nm_usuario := _ora2pg_r.tb_nm_usuario_p; tb_cd_estabelecimento_w := _ora2pg_r.tb_cd_estabelecimento_p;
		loop
			fetch cursor_w into nr_titulo_w, vl_saldo_titulo_w, cd_pessoa_fisica_w, cd_cgc_w, qt_tit_pag_w, nr_seq_prestador_w;
			EXIT WHEN NOT FOUND; /* apply on cursor_w */
			
			-- Se o título a pagar não está na tabela pls_pp_item_lote então dá continuidade no processo, senão, ignora o título
			if (qt_tit_pag_w = 0) then
				-- armazena em tables para enviar ao banco a cada 'X' registros
				tb_nr_titulo_pagar_w(nr_contador_w) := nr_titulo_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_pp_tit_pag_rec_pck.gerencia_titulo_pagar ( nr_seq_lote_p pls_pp_lote.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) FROM PUBLIC;
