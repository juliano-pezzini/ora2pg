...
if (ie_considera_selecao_p = false) then
	...
	ds_alias_pres_w := pls_pp_filtro_rec_glosa_pck.obter_alias_tabela('prestador');
	...
	if (ie_forma_pagamento_w = 'P') then
		-- data de competencia do protocolo ou data de liberacao de pagamento do protocolo

		if (ie_tipo_data_ref_p in ('1','2')) then
			ds_restricao_w := ds_restricao_w || ' and ' || ds_alias_pcl_orig_w || '.ie_status in (''3'',''4'',''6'') ' || pls_util_pck.enter_w;
		-- data de competencia do resumo de conta medica

		elsif (ie_tipo_data_ref_p = '3') then
			ds_restricao_w := ds_restricao_w || ' and ' || ds_alias_res_w || '.ie_status_protocolo in (''3'',''4'',''6'') ' || pls_util_pck.enter_w;
		end if;
	elsif (ie_forma_pagamento_w = 'C') then
		-- data de competencia do protocolo ou data de liberacao de pagamento do protocolo

		if (ie_tipo_data_ref_p in ('1','2')) then
			ds_restricao_w := ds_restricao_w || 	' and ' || ds_alias_con_w || '.ie_status = ''2'' ' || pls_util_pck.enter_w;
		-- data de competencia do resumo de conta medica

		elsif (ie_tipo_data_ref_p = '3') then
			ds_restricao_w := ds_restricao_w || 	' and ' || ds_alias_res_w || '.ie_status = ''F'' ' || pls_util_pck.enter_w ||
								' and ' || ds_alias_con_orig_w || '.ie_status = ''F'' ' || pls_util_pck.enter_w;
		end if;
	end if;
	...
	if (ie_tipo_data_ref_p = '1') then
		ds_restricao_w := ds_restricao_w || ' and ' || ds_alias_prot_w || '.dt_competencia_lote between :dt_inicio_pc and :dt_fim_pc' || pls_util_pck.enter_w;

	-- data de liberacao de pagamento do protocolo

	elsif (ie_tipo_data_ref_p = '2') then
		ds_restricao_w := ds_restricao_w || ' and ' || ds_alias_prot_w || '.dt_liberacao_pag between :dt_inicio_pc and :dt_fim_pc' || pls_util_pck.enter_w;
		
	-- data de competencia do resumo de conta medica

	elsif (ie_tipo_data_ref_p = '3') then
		ds_restricao_w := ds_restricao_w || ' and ' || ds_alias_res_w || '.dt_competencia_pgto between :dt_inicio_pc and :dt_fim_pc' || pls_util_pck.enter_w;	
	end if;
	...
	if (cd_condicao_pagamento_p IS NOT NULL AND cd_condicao_pagamento_p::text <> '') then
		...
		if (ie_tipo_data_ref_p = '1') then
			ds_restr_cpag_w := ds_restr_cpag_w || ' and ' || ds_alias_prot_w || '.dt_competencia_lote between prepag.dt_inicio_vigencia_ref and prepag.dt_fim_vigencia_ref';

		-- data de liberacao de pagamento do protocolo

		elsif (ie_tipo_data_ref_p = '2') then
			ds_restr_cpag_w := ds_restr_cpag_w || ' and ' || ds_alias_prot_w || '.dt_liberacao_pag between prepag.dt_inicio_vigencia_ref and prepag.dt_fim_vigencia_ref';
			
		-- data de competencia do resumo de conta medica

		elsif (ie_tipo_data_ref_p = '3') then
			ds_restr_cpag_w := ds_restr_cpag_w || ' and ' || ds_alias_res_w || '.dt_competencia_pgto between prepag.dt_inicio_vigencia_ref and prepag.dt_fim_vigencia_ref';	
		end if;
		
		ds_restr_cpag_w := ds_restr_cpag_w || pls_util_pck.enter_w;
		...
		ds_restricao_w	:= ds_restricao_w ||
					...
					'		where	prepag.nr_seq_prestador = ' || ds_alias_pres_w || '.nr_sequencia' || pls_util_pck.enter_w ||
							ds_restr_cpag_w ||
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION pls_pp_filtro_rec_glosa_pck.obter_restricao_padrao ( ie_considera_selecao_p boolean, ie_tipo_data_ref_p pls_pp_lote.ie_tipo_dt_referencia%type, dt_inicio_referencia_p pls_pp_lote.dt_referencia_inicio%type, dt_fim_referencia_p pls_pp_lote.dt_referencia_fim%type, cd_condicao_pagamento_p pls_pp_lote.cd_condicao_pagamento%type, ie_recurso_proprio_p pls_pp_lote.ie_recurso_proprio%type, cd_estabelecimento_p pls_pp_lote.cd_estabelecimento%type, ds_restricao_mat_p out text, ds_restricao_proc_p out text, valor_bind_p INOUT sql_pck.t_dado_bind) FROM PUBLIC;
