...
if (nr_seq_filtro_p IS NOT NULL AND nr_seq_filtro_p::text <> '') then
	...
	for r_c_filtro_w in c_filtro(nr_seq_filtro_p) loop
		...
		begin
			...
			SELECT * FROM pls_pp_filtro_rec_glosa_pck.obter_restricao_produto(	r_c_filtro_w.nr_seq_plano, ie_tipo_data_ref_p, valor_bind_w) INTO STRICT _ora2pg_r;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_pp_filtro_rec_glosa_pck.aplica_filtro_produto ( ie_considera_selecao_p INOUT boolean, qt_filtro_processado_p INOUT integer, nr_id_transacao_p pls_pp_cta_rec_selecao.nr_id_transacao%type, ie_excecao_p pls_pp_rp_cta_filtro.ie_excecao%type, nr_seq_filtro_p pls_pp_rp_cta_filtro.nr_sequencia%type, ie_tipo_data_ref_p pls_pp_lote.ie_tipo_dt_referencia%type, dt_inicio_referencia_p pls_pp_lote.dt_referencia_inicio%type, dt_fim_referencia_p pls_pp_lote.dt_referencia_fim%type, cd_condicao_pagamento_p pls_pp_lote.cd_condicao_pagamento%type, ie_recurso_proprio_p pls_pp_lote.ie_recurso_proprio%type, ie_incidencia_filtro_p text, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
