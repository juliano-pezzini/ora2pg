...
CREATE OR REPLACE PROCEDURE pls_odonto_pck.insert_itens_lote_exec ( nr_seq_prestador_p pls_requisicao.nr_seq_prestador_exec%type, nr_seq_segurado_p pls_requisicao.nr_seq_segurado%type, nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, ie_origem_execucao_p pls_lote_execucao_req.ie_origem_consulta%type, cd_estabelecimento_p bigint, nr_seq_usuario_web_p pls_usuario_web.nr_sequencia%type, nm_usuario_p pls_lote_execucao_req.nm_usuario%type, ds_observacao_p pls_lote_execucao_req.ds_observacao%type, nr_seq_lote_execucao_p INOUT pls_itens_lote_execucao.nr_seq_lote_exec%type) AS $body$
DECLARE
...
--pls_gerencia_upd_obj_pck.atualizar_objetos(nm_usuario_p, 'PLS_GERAR_ITENS_LOTE_EXECUCAO', 'PLS_ESTRUTURA_MATERIAL_TM');
--Atualizando a tabela de grupo de procedimentos
--PLS_GERENCIA_UPD_OBJ_PCK.ATUALIZAR_OBJETOS(nm_usuario_p, 'PLS_GERAR_ITENS_LOTE_EXECUCAO', 'PLS_GRUPO_SERVICO_TM');
-- gerencia a atualizacao da tabela TM para para grupos de MATERIAIS
--PLS_GERENCIA_UPD_OBJ_PCK.ATUALIZAR_OBJETOS(nm_usuario_p, 'PLS_GERAR_ITENS_LOTE_EXECUCAO', 'PLS_GRUPO_MATERIAL_TM');
if (ie_origem_execucao_p = 'P' or ie_origem_execucao_p = 'T') and (length(nr_seq_prestador_p)	<= 10) then
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		if (ie_tipo_item_w	= 'P') then
			...
			pls_validar_lib_proc_prestador( nr_seq_prestador_p, null, cd_proc_mat_w,
							ie_origem_proced_w, ie_tipo_guia_w, clock_timestamp(),
							cd_estabelecimento_p, ie_carater_atendimento_w, nr_seq_tipo_atendimento_w,
							ie_internado_w, nr_seq_plano_w, ie_proc_mat_liberado_w,
							nr_seq_regra_lib_w, nr_seq_requisicao_p);
			...
			pls_validar_lib_mat_prestador(	nr_seq_prestador_p, null, cd_proc_mat_w,
							clock_timestamp(), cd_estabelecimento_p, null,
							ie_internado_w,ie_proc_mat_liberado_w,nr_seq_regra_lib_w);
...
if (nr_seq_prestador_p = 1) then
ie_permite_execucao_w := 'S';
end if;
			...
			if (ie_fecha_pacote_exec_w = 'S') then
				...
				pls_excecao_preco_pacote( nr_seq_preco_pacote_w, nr_seq_congenere_w, nm_usuario_p,
							  nr_seq_intercambio_w, nr_seq_prestador_p, ie_excessao_pacote_w,
							  null);
	...
	begin
			...
			exception
			when no_data_found then
				RAISE EXCEPTION '%', 'nr_seq_lote_execucao_w' || nr_seq_lote_execucao_w ||
				'nr_seq_proc_mat_w' || nr_seq_proc_mat_w ||
				'nr_seq_requisicao_w' || nr_seq_requisicao_w ||
				'qt_item_solicitado_w' || qt_item_solicitado_w ||
				'qt_item_aprovado_w' || qt_item_aprovado_w ||
				'qt_item_executado_w' || qt_item_executado_w ||
				'qt_item_pendente_w' || qt_item_pendente_w ||
				'dt_requisicao_w' || dt_requisicao_w ||
				'dt_validade_senha_w' || dt_validade_senha_w ||
				'cd_proc_mat_w' || cd_proc_mat_w ||
				'cd_medico_solicitante_w' || cd_medico_solicitante_w ||
				'ie_tipo_item_w' || ie_tipo_item_w ||
				'ie_tipo_guia_w' || ie_tipo_guia_w ||
				'nr_seq_regra_lib_w' || nr_seq_regra_lib_w ||
				'teste'
				 USING ERRCODE = '45011';
			end;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_odonto_pck.insert_itens_lote_exec ( nr_seq_prestador_p pls_requisicao.nr_seq_prestador_exec%type, nr_seq_segurado_p pls_requisicao.nr_seq_segurado%type, nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, ie_origem_execucao_p pls_lote_execucao_req.ie_origem_consulta%type, cd_estabelecimento_p bigint, nr_seq_usuario_web_p pls_usuario_web.nr_sequencia%type, nm_usuario_p pls_lote_execucao_req.nm_usuario%type, ds_observacao_p pls_lote_execucao_req.ds_observacao%type, nr_seq_lote_execucao_p INOUT pls_itens_lote_execucao.nr_seq_lote_exec%type) FROM PUBLIC;
