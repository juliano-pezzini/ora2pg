...
CREATE OR REPLACE PROCEDURE pls_odonto_pck.desfazer_validacao_req ( nr_seq_requisicao_p bigint, nm_usuario_p text) AS $body$
DECLARE


/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
Finalidade: Desfazer a consistencia da requisicao, voltando seu estagio para "Usuario (Aguardando acao)".
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
...
if (coalesce(nr_seq_requisicao_p,0) <> 0) then
	...
	if (ie_estagio_w <> '1') then
		CALL pls_requisicao_gravar_hist(nr_seq_requisicao_p,'L','Acao executada: Desfazer consistir.',null,nm_usuario_p);
	end if;		
	...
	if (ie_requisicao_executada_w = 'S') then
		...
		--Nao foi possivel desfazer a consistencia, essa requisicao ja foi executada !
	end if;
	...
	if (qt_auditoria_iniciada_w	> 0 ) then
		...
		--Nao foi possivel desfazer a consistencia, o processo de auditoria desta requisicao ja foi iniciado !
	end if;	
	...
	if (qt_auditoria_assumida_w	> 0) then
		...
		--Nao foi possivel desfazer a consistencia, a analise desta requisicao ja foi assumida por um grupo de auditores !
	end if;	
	...
	if (qt_auditoria_w	<> 0) then
		...
		if (ie_status_auditoria_w = 'AJ') then
			...
			--Nao foi possivel desfazer a consistencia, a analise desta requisicao esta aguardando justificativa do prestador !
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_odonto_pck.desfazer_validacao_req ( nr_seq_requisicao_p bigint, nm_usuario_p text) FROM PUBLIC;
