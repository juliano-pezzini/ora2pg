-- Generated by Ora2Pg, the Oracle database Schema converter, version 23.1
-- Copyright 2000-2022 Gilles DAROLD. All rights reserved.
-- DATASOURCE: dbi:Oracle:host=srv-dbora-03.whebdc.com.br;service_name=DEV_1815;port=1521

SET client_encoding TO 'UTF8';




CREATE OR REPLACE PROCEDURE pfcs_pck_admission_prioritizer.pfcs_pa_inpatient_orders (cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE

        nr_seq_indicator_w          pfcs_panel.nr_seq_indicator%type := PFCS_PCK_INDICATORS.NR_ED_INPATIENT_ORDERS;
        pfcs_panel_detail_seq_w     pfcs_panel_detail.nr_sequencia%type;
        pfcs_panel_seq_w            pfcs_panel.nr_sequencia%type;

        qt_register_w               integer := 0;
        qt_inp_orders_delayed_w     integer := 0;
        qt_time_rule_w              integer;

        c01 CURSOR FOR
        SELECT pfcs_get_human_name(pat.nr_sequencia, PFCS_PCK_CONSTANTS.DS_PATIENT) nm_patient,
            pfcs_get_patient_gender(pat.gender) gender,
            to_char(round(months_between(coalesce(pat.deceased_date, clock_timestamp()), pat.birthdate))) qt_patient_age,
            pat.birthdate,
            pat.patient_id id_patient,
            pfcs_get_patient_diagnosis(enc.nr_sequencia) diagnosis,
            pfcs_get_human_name(pfcs_get_practitioner_seq(enc.nr_sequencia, PFCS_PCK_CONSTANTS.CD_ATTENDING_PHYSICIAN), PFCS_PCK_CONSTANTS.DS_PRACTITIONER) nm_practioner,
            ((clock_timestamp() - coalesce(psr.dt_authored_on, psr.dt_atualizacao)) * 24 * 60 ) total_time,
            pfcs_get_code_status(pat.nr_sequencia, enc.nr_sequencia, PFCS_PCK_CONSTANTS.IE_YES_BR) code_status,
            (uni.cd_unidade_basica || PFCS_PCK_CONSTANTS.DS_HYPHEN || uni.cd_unidade_compl) ds_request_unit,
            pfcs_pck_utils.get_full_location(psr.nr_seq_location) ds_requested_location,
            pfcs_pck_utils.get_full_location(pfcs_get_pat_location(pat.nr_sequencia, enc.nr_sequencia)) ds_current_location,
            PFCS_GET_SPECIAL_REQUESTS(enc.nr_sequencia, uni.nr_seq_location,PFCS_PCK_CONSTANTS.IE_SPECIAL_REQ_ENCOUNTER) special_request,
            pfcs_decode_bed_status(uni.ie_status_unidade) ds_status,
            uni.ie_status_unidade cd_status,
            pfcs_get_edi_score(enc.nr_sequencia) nr_edi_score,
            coalesce(pfcs_get_checklist(enc.nr_sequencia, pat.nr_sequencia, PFCS_PCK_CONSTANTS.CD_MPL),null) ds_care_status,
            coalesce(pfcs_get_checklist(enc.nr_sequencia, pat.nr_sequencia, PFCS_PCK_CONSTANTS.CD_TOOLTIP),null) ds_checklist,
            pfcs_get_frequent_flyer(enc.nr_sequencia) flag_recur_pat,
            pfcs_get_lace_plus(enc.nr_sequencia, enc.nr_seq_organization) readmission_risk,
            pfcs_get_recurring_patient(pat.nr_sequencia) ds_recur_pat,
            enc.id_encounter nr_encounter_varchar,
            enc.si_status,
            enc.nr_sequencia encounter_sequence,
            enc.period_start,
            enc.si_classif,
            sec.ds_setor_atendimento ds_department,
            sec.cd_setor_atendimento cd_unit,
            uni.ie_temporario ie_temporary,
            pfcs_obs_contributor_pck.get_laceplus(enc.nr_sequencia) ds_readm_risk_contrb,
            pfcs_obs_contributor_pck.get_freqflyer_visits(enc.nr_sequencia) ds_recur_pat_adm_data,
            pfcs_obs_contributor_pck.get_freqflyer_comorbidities(enc.nr_sequencia) ds_recur_pat_comorbd,
            pfcs_obs_contributor_pck.get_freqflyer_visit_reason(enc.nr_sequencia) ds_recur_pat_reasons,
            pfcs_obs_contributor_pck.get_edi_vital_warnings(enc.nr_sequencia) ds_edi_vitals_warn,
            pfcs_obs_contributor_pck.get_edi_contributors(enc.nr_sequencia) ds_edi_contrb
        from pfcs_service_request psr,
            pfcs_encounter enc,
            pfcs_patient pat,
            unidade_atendimento uni,
            setor_atendimento sec
        where upper(psr.cd_service) = PFCS_PCK_CONSTANTS.CD_TRANSFER
            and upper(psr.si_status) = PFCS_PCK_CONSTANTS.SI_STATUS_ACTIVE
            and upper(psr.si_intent) = PFCS_PCK_CONSTANTS.SI_INTENT_PLAN
            and enc.nr_seq_patient = pat.nr_sequencia
            and enc.nr_sequencia = psr.nr_seq_encounter
            and psr.nr_seq_location = uni.nr_seq_location
            and uni.cd_setor_atendimento = sec.cd_setor_atendimento
            and sec.cd_classif_setor = PFCS_PCK_CONSTANTS.CD_ED 
            and pat.ie_active = TO_CHAR(PFCS_PCK_CONSTANTS.IE_TRUE)
            and (enc.period_start IS NOT NULL AND enc.period_start::text <> '')
            and coalesce(enc.period_end::text, '') = ''
            and sec.ie_situacao = PFCS_PCK_CONSTANTS.IE_ACTIVE
            and (pfcs_get_pat_location(pat.nr_sequencia, enc.nr_sequencia) IS NOT NULL AND (pfcs_get_pat_location(pat.nr_sequencia, enc.nr_sequencia))::text <> '')
            and sec.cd_estabelecimento_base = cd_estabelecimento_p;

        
BEGIN
        
        begin
            select coalesce(qt_time, PFCS_PCK_CONSTANTS.QT_TIME_RULE)
                into STRICT qt_time_rule_w
            from pfcs_rule_inp_orders LIMIT 1;

            exception
                when no_data_found then
                    qt_time_rule_w := PFCS_PCK_CONSTANTS.QT_TIME_RULE;
        end;

        for c01_w in c01 loop
            qt_register_w := qt_register_w + 1;

            if (c01_w.total_time > qt_time_rule_w) then
                qt_inp_orders_delayed_w := qt_inp_orders_delayed_w + 1;

                select  nextval('pfcs_panel_detail_seq')
                into STRICT  pfcs_panel_detail_seq_w
;

                insert into pfcs_panel_detail(
                    nr_sequencia,
                    nm_usuario,
                    dt_atualizacao,
                    nm_usuario_nrec,
                    dt_atualizacao_nrec,
                    ie_situation,
                    nr_seq_indicator,
                    nr_seq_operational_level)
                values (
                    pfcs_panel_detail_seq_w,
                    nm_usuario_p,
                    clock_timestamp(),
                    nm_usuario_p,
                    clock_timestamp(),
                    PFCS_PCK_CONSTANTS.IE_TEMPORARY,
                    PFCS_PCK_INDICATORS.NR_ED_INPATIENT_ORDERS_DELAYS,
                    cd_estabelecimento_p);

                insert into pfcs_detail_bed(
                    nr_sequencia,
                    nm_usuario,
                    dt_atualizacao,
                    nm_usuario_nrec,
                    dt_atualizacao_nrec,
                    nr_seq_detail,
                    ds_location,
                    ds_request_unit,
                    cd_department,
                    ds_department,
                    ds_status,
                    cd_status,
                    ie_temporary)
                values (
                    nextval('pfcs_detail_bed_seq'),
                    nm_usuario_p,
                    clock_timestamp(),
                    nm_usuario_p,
                    clock_timestamp(),
                    pfcs_panel_detail_seq_w,
                    c01_w.ds_requested_location,
                    c01_w.ds_request_unit,
                    c01_w.cd_unit,
                    c01_w.ds_department,
                    c01_w.ds_status,
                    c01_w.cd_status,
                    c01_w.ie_temporary);

                insert into pfcs_detail_patient(
                    nr_sequencia,
                    nm_usuario,
                    dt_atualizacao,
                    nm_usuario_nrec,
                    dt_atualizacao_nrec,
                    nr_seq_detail,
                    nr_encounter_varchar,
                    dt_entrance,
                    qt_time_total_pa,
                    id_patient,
                    nm_patient,
                    ds_gender,
                    dt_birthdate,
                    ds_age_range,
                    ds_dnr_status,
                    ds_special_request,
                    ds_physician,
                    ds_symptoms,
                    qt_edi_score,
                    ds_care_status,
                    ds_checklist,
                    ds_readmission_risk,
                    ie_freq_flyer,
                    ds_recurring_patient,
                    ds_readm_risk_contrb,
                    ds_rec_pat_adm_data,
                    ds_rec_pat_comorbd,
                    ds_rec_pat_reasons,
                    ds_edi_vitals_warn,
                    ds_edi_contrb,
                    ds_current_location)
                values (
                    nextval('pfcs_detail_patient_seq'),
                    nm_usuario_p,
                    clock_timestamp(),
                    nm_usuario_p,
                    clock_timestamp(),
                    pfcs_panel_detail_seq_w,
                    c01_w.nr_encounter_varchar,
                    c01_w.period_start,
                    c01_w.total_time,
                    c01_w.id_patient,
                    c01_w.nm_patient,
                    c01_w.gender,
                    c01_w.birthdate,
                    c01_w.qt_patient_age,
                    c01_w.code_status,
                    c01_w.special_request,
                    c01_w.nm_practioner,
                    c01_w.diagnosis,
                    c01_w.nr_edi_score,
                    c01_w.ds_care_status,
                    c01_w.ds_checklist,
                    c01_w.readmission_risk,
                    c01_w.flag_recur_pat,
                    c01_w.ds_recur_pat,
                    c01_w.ds_readm_risk_contrb,
                    c01_w.ds_recur_pat_adm_data,
                    c01_w.ds_recur_pat_comorbd,
                    c01_w.ds_recur_pat_reasons,
                    c01_w.ds_edi_vitals_warn,
                    c01_w.ds_edi_contrb,
                    c01_w.ds_current_location);

            end if;

            select  nextval('pfcs_panel_detail_seq')
            into STRICT  pfcs_panel_detail_seq_w
;

            insert into pfcs_panel_detail(
                nr_sequencia,
                nm_usuario,
                dt_atualizacao,
                nm_usuario_nrec,
                dt_atualizacao_nrec,
                ie_situation,
                nr_seq_indicator,
                nr_seq_operational_level)
            values (
                pfcs_panel_detail_seq_w,
                nm_usuario_p,
                clock_timestamp(),
                nm_usuario_p,
                clock_timestamp(),
                PFCS_PCK_CONSTANTS.IE_TEMPORARY,
                nr_seq_indicator_w,
                cd_estabelecimento_p);

            insert into pfcs_detail_bed(
                nr_sequencia,
                nm_usuario,
                dt_atualizacao,
                nm_usuario_nrec,
                dt_atualizacao_nrec,
                nr_seq_detail,
                ds_location,
                ds_request_unit,
                ds_department,
                ds_status,
                cd_status,
                ie_temporary)
            values (
                nextval('pfcs_detail_bed_seq'),
                nm_usuario_p,
                clock_timestamp(),
                nm_usuario_p,
                clock_timestamp(),
                pfcs_panel_detail_seq_w,
                c01_w.ds_requested_location,
                c01_w.ds_request_unit,
                c01_w.ds_department,
                c01_w.ds_status,
                c01_w.cd_status,
                c01_w.ie_temporary);

            insert into pfcs_detail_patient(
                nr_sequencia,
                nm_usuario,
                dt_atualizacao,
                nm_usuario_nrec,
                dt_atualizacao_nrec,
                nr_seq_detail,
                nr_encounter_varchar,
                dt_entrance,
                qt_time_total_pa,
                id_patient,
                nm_patient,
                ds_gender,
                dt_birthdate,
                ds_age_range,
                ds_dnr_status,
                ds_special_request,
                ds_physician,
                ds_symptoms,
                qt_edi_score,
                ds_care_status,
                ds_checklist,
                ds_readmission_risk,
                ie_freq_flyer,
                ds_recurring_patient,
                ds_readm_risk_contrb,
                ds_rec_pat_adm_data,
                ds_rec_pat_comorbd,
                ds_rec_pat_reasons,
                ds_edi_vitals_warn,
                ds_edi_contrb,
                ds_current_location)
            values (
                nextval('pfcs_detail_patient_seq'),
                nm_usuario_p,
                clock_timestamp(),
                nm_usuario_p,
                clock_timestamp(),
                pfcs_panel_detail_seq_w,
                c01_w.nr_encounter_varchar,
                c01_w.period_start,
                c01_w.total_time,
                c01_w.id_patient,
                c01_w.nm_patient,
                c01_w.gender,
                c01_w.birthdate,
                c01_w.qt_patient_age,
                c01_w.code_status,
                c01_w.special_request,
                c01_w.nm_practioner,
                c01_w.diagnosis,
                c01_w.nr_edi_score,
                c01_w.ds_care_status,
                c01_w.ds_checklist,
                c01_w.readmission_risk,
                c01_w.flag_recur_pat,
                c01_w.ds_recur_pat,
                c01_w.ds_readm_risk_contrb,
                c01_w.ds_recur_pat_adm_data,
                c01_w.ds_recur_pat_comorbd,
                c01_w.ds_recur_pat_reasons,
                c01_w.ds_edi_vitals_warn,
                c01_w.ds_edi_contrb,
                c01_w.ds_current_location);
        end loop;

         := pfcs_pck.pfcs_generate_results(
            vl_indicator_p              => qt_register_w, nr_seq_indicator_p          => nr_seq_indicator_w, vl_indicator_aux_p          => qt_inp_orders_delayed_w, nr_seq_operational_level_p  => cd_estabelecimento_p, nm_usuario_p                => nm_usuario_p, nr_seq_panel_p              => pfcs_panel_seq_w);

        CALL pfcs_pck.pfcs_update_detail(
            nr_seq_indicator_p          => nr_seq_indicator_w,
            nr_seq_panel_p              => pfcs_panel_seq_w,
            nr_seq_operational_level_p  => cd_estabelecimento_p,
            nm_usuario_p                => nm_usuario_p);

        CALL pfcs_pck.pfcs_update_detail(
            nr_seq_indicator_p          => PFCS_PCK_INDICATORS.NR_ED_INPATIENT_ORDERS_DELAYS,
            nr_seq_panel_p              => pfcs_panel_seq_w,
            nr_seq_operational_level_p  => cd_estabelecimento_p,
            nm_usuario_p                => nm_usuario_p);

        CALL pfcs_pck.pfcs_activate_records(
            nr_seq_indicator_p          => PFCS_PCK_INDICATORS.NR_ED_INPATIENT_ORDERS_DELAYS,
            nr_seq_operational_level_p  => cd_estabelecimento_p,
            nm_usuario_p                => nm_usuario_p);

        CALL pfcs_pck.pfcs_activate_records(
            nr_seq_indicator_p          => nr_seq_indicator_w,
            nr_seq_operational_level_p  => cd_estabelecimento_p,
            nm_usuario_p                => nm_usuario_p);

    END;

$body$
LANGUAGE PLPGSQL
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pfcs_pck_admission_prioritizer.pfcs_pa_inpatient_orders (cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
