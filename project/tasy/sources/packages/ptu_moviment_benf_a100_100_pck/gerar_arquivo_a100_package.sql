...
CREATE OR REPLACE PROCEDURE ptu_moviment_benf_a100_100_pck.gerar_arquivo_a100 ( nr_seq_lote_p bigint, cd_unimed_destino_p bigint, cd_intercambio_envio_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ds_erro_p INOUT text, ds_nome_arq_p INOUT text, ds_caminho_rede_p INOUT text) AS $body$
DECLARE
	...
	current_setting('ptu_moviment_benf_a100_100_pck.nm_arquivo_w')::varchar(255)			varchar(255);
	ds_caminho_rede_w		varchar(255) := null;
	
	
BEGIN
	PERFORM set_config('ptu_moviment_benf_a100_100_pck.ds_arquivo_w', null, false);
	CALL CALL CALL CALL CALL CALL CALL CALL ptu_moviment_benf_a100_100_pck.set_nm_usuario(nm_usuario_p);
	CALL CALL CALL CALL CALL CALL CALL CALL CALL ptu_moviment_benf_a100_100_pck.set_cd_estabelecimento(cd_estabelecimento_p);
	CALL CALL CALL ptu_moviment_benf_a100_100_pck.set_cd_intercambio(cd_intercambio_envio_p);
	begin
	select	max(ds_local),
		max(ds_local_rede)
	...
	PERFORM set_config('ptu_moviment_benf_a100_100_pck.nm_arquivo_w', ptu_obter_nome_exportacao(cd_intercambio_envio_p,'IN'), false);
	CALL CALL CALL ptu_moviment_benf_a100_100_pck.set_nome_arquivo(current_setting('ptu_moviment_benf_a100_100_pck.nm_arquivo_w')::varchar(255));
	
	begin
	arq_texto_w := utl_file.fopen(ds_local_w,current_setting('ptu_moviment_benf_a100_100_pck.nm_arquivo_w')::varchar(255),'w');
	exception
	when others then
	...
	CALL CALL CALL CALL CALL ptu_moviment_benf_a100_100_pck.processar_arquivo(nr_seq_lote_p,cd_unimed_destino_p,arq_texto_w);
	
	utl_file.fclose(arq_texto_w);
	
	ds_nome_arq_p := current_setting('ptu_moviment_benf_a100_100_pck.nm_arquivo_w')::varchar(255);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_moviment_benf_a100_100_pck.gerar_arquivo_a100 ( nr_seq_lote_p bigint, cd_unimed_destino_p bigint, cd_intercambio_envio_p bigint, nm_usuario_p text, cd_estabelecimento_p bigint, ds_erro_p INOUT text, ds_nome_arq_p INOUT text, ds_caminho_rede_p INOUT text) FROM PUBLIC;
