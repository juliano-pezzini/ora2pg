...
SET client_encoding TO 'UTF8';
  ...
  -- Grafico B (Encounter Count/Clinical Unit. <> 4)
CREATE OR REPLACE FUNCTION pck_cdsdvt_report.generate_graph_b (dt_ini_p timestamp, dt_fim_p timestamp, cd_unidade_p text, ds_age_p text) RETURNS boolean AS $body$
DECLARE
...
BEGIN
  --Limpa tabelas
  delete FROM dvt_critical_care_graph_b;
  ...
  loop
      ...
      insert into dvt_critical_care_graph_b(ds_label,nr_valor)
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION pck_cdsdvt_report.generate_graph_b (dt_ini_p timestamp, dt_fim_p timestamp, cd_unidade_p text, ds_age_p text) FROM PUBLIC;
