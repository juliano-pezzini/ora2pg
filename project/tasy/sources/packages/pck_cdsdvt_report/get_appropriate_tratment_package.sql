...
SET client_encoding TO 'UTF8';
   ...
   -----------------
CREATE OR REPLACE FUNCTION pck_cdsdvt_report.get_appropriate_tratment (start_date_p timestamp, end_date_p timestamp, cd_setor_p text, ds_age_p text, nr_week_p bigint) RETURNS bigint AS $body$
DECLARE


      nr_appropriate_tratment_w bigint;

      c1 CURSOR FOR
        SELECT a.nr_atendimento,
              count(b.qt_resultado) nr_appropriate_tratment
...
BEGIN
      ...
      loop
         ...
         nr_appropriate_tratment_w := nr_appropriate_tratment_w + r1.nr_appropriate_tratment;

      end loop;
      close c1;

      --
      return nr_appropriate_tratment_w;
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION pck_cdsdvt_report.get_appropriate_tratment (start_date_p timestamp, end_date_p timestamp, cd_setor_p text, ds_age_p text, nr_week_p bigint) FROM PUBLIC;
