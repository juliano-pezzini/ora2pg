...
CREATE OR REPLACE PROCEDURE pep_gerar_resumo_pck.gerar_pep_resumo_solucao () AS $body$
DECLARE
	...
	EXIT WHEN NOT FOUND; /* apply on C01 */
		...
		EXIT WHEN NOT FOUND; /* apply on C02 */
			...
			if (current_setting('pep_gerar_resumo_pck.ie_rtf_w')::varchar(1) = 'N') then
				ds_componentes_w	:= substr(ds_componentes_w ||
							pep_gerar_resumo_pck.get_tr_html(qt_dose_w||' '|| ds_unidade_medida_W || ' - ' ||ds_material_w,
								null,null,null,null,null,3),1,30000);	
								
				if (ds_observacao_w IS NOT NULL AND ds_observacao_w::text <> '') then
					ds_componentes_w := substr(ds_componentes_w || pep_gerar_resumo_pck.get_tr_html_obs(ds_observacao_w),1,30000);
				end if;
			
				if (ds_justificativa_w IS NOT NULL AND ds_justificativa_w::text <> '') then
					ds_componentes_w := substr(ds_componentes_w || pep_gerar_resumo_pck.get_tr_html_justif(ds_justificativa_w),1,30000);
				end if;
				
			else
				ds_componentes_w	:= substr(ds_componentes_w ||
							pep_gerar_resumo_pck.get_tr_rtf(qt_dose_w||' '|| ds_unidade_medida_W || ' - ' ||ds_material_w,
								    null,null,null,null,null,3),1,30000);	
			end if;
		...
		if (current_setting('pep_gerar_resumo_pck.ie_rtf_w')::varchar(1) = 'N') then		
			...
			PERFORM set_config('pep_gerar_resumo_pck.ds_resumo_w', substr(current_setting('pep_gerar_resumo_pck.ds_resumo_w')::text ||
					pep_gerar_resumo_pck.get_tr_html(ds_solucao_w,ds_horarios_w,null,null,null,null, 2)||
					pep_gerar_resumo_pck.get_tr_html(ds_componentes_w,null,null,null,null,null,1),1,30000), false);
		else
			PERFORM set_config('pep_gerar_resumo_pck.ds_resumo_w', substr(current_setting('pep_gerar_resumo_pck.ds_resumo_w')::text ||
					pep_gerar_resumo_pck.get_tr_rtf(ds_solucao_w,ds_horarios_w,null,null,null,null, 2)||
					pep_gerar_resumo_pck.get_tr_rtf(ds_componentes_w,null,null,null,null,null,1),1,30000), false);
		end if;
		end;
	end loop;
	close C01;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pep_gerar_resumo_pck.gerar_pep_resumo_solucao () FROM PUBLIC;
