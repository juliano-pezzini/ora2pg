...
-- Enviar Pedido de Autorização 00600 - Pedido de Autorização
CREATE OR REPLACE PROCEDURE ptu_envio_scs_ws_pck.ptu_envio_ped_autorizacao ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, nr_versao_ptu_p ptu_pedido_autorizacao.nr_versao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, nr_seq_pedido_novo_p INOUT ptu_pedido_autorizacao.nr_sequencia%type) AS $body$
DECLARE
...
if (ie_alto_custo_w	in ('1','2')) or (ie_tipo_guia_w	= 1)then
	...
	nm_prestador_alto_custo_w		:= substr(pls_obter_dados_prestador(coalesce(nr_seq_prestador_exec_w, nr_seq_prestador_w),'N'),1,25);
else
	nr_seq_prest_alto_custo_w	:= 0;
...
values (nextval('ptu_pedido_autorizacao_seq'), dt_execucao_w, ie_tipo_cliente_w,
	 ...
	 CASE WHEN nr_transacao_solicitante_w=0 THEN 'N'  ELSE 'S' END , nr_transacao_solicitante_w, cd_versao_tiss_w) returning  nr_sequencia, nr_seq_execucao into nr_seq_pedido_novo_p, nr_seq_controle_exec_w;
	...
	if (nr_seq_pacote_w IS NOT NULL AND nr_seq_pacote_w::text <> '') and (coalesce(ie_permite_pacote_w,'N')	= 'S') then
		...
		if (ie_tipo_pacote_w	= 'A') then
			...
			EXIT WHEN NOT FOUND; /* apply on C03 */
				...
				ds_proc_pacote_w	:= substr(obter_descricao_procedimento(cd_proc_pacote_w,null),1,100);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_envio_scs_ws_pck.ptu_envio_ped_autorizacao ( nr_seq_guia_p pls_guia_plano.nr_sequencia%type, nr_seq_requisicao_p pls_requisicao.nr_sequencia%type, nr_versao_ptu_p ptu_pedido_autorizacao.nr_versao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, nr_seq_pedido_novo_p INOUT ptu_pedido_autorizacao.nr_sequencia%type) FROM PUBLIC;
