...
--Enviar 00605 - Pedido de Complemento de Autorização
CREATE OR REPLACE PROCEDURE ptu_envio_scs_ws_pck.ptu_envio_ped_compl_aut_web ( nr_seq_guia_p pls_guia_plano_proc.nr_seq_guia%type, nr_seq_requisicao_p pls_requisicao_proc.nr_seq_requisicao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, nr_seq_pedido_novo_p INOUT ptu_pedido_compl_aut.nr_sequencia%type) AS $body$
DECLARE
...
ie_origem_proced_w		ptu_pedido_compl_aut_serv.ie_origem_servico%type;
...
cd_material_w			ptu_pedido_compl_aut_serv.cd_servico%type;
...
vl_procedimento_w		ptu_pedido_compl_aut_serv.vl_uni_servico%type;
vl_material_w			ptu_pedido_compl_aut_serv.vl_uni_servico%type;
...
ds_opme_w			pls_material.ds_material%type;
...
if (nr_seq_requisicao_p IS NOT NULL AND nr_seq_requisicao_p::text <> '') then
	...
	select	max(ie_req_interc_lib_manual)
...
values (nextval('ptu_pedido_compl_aut_seq'), ie_tipo_cliente_w, cd_cooperativa_w,
	 cd_cooperativa_ww, nextval('ptu_controle_execucao_seq'), nr_seq_origem_w,
	 ...
	 ds_indicacao_clinica_w, ie_sexo_w, nr_idade_w) returning nr_sequencia, nr_seq_execucao into nr_seq_pedido_novo_p, nr_seq_execucao_w;
...
insert	into ptu_controle_execucao(nr_sequencia, dt_atualizacao, nm_usuario,
	 ...
	 dt_atualizacao_nrec)
values (nr_seq_execucao_w, clock_timestamp(), nm_usuario_p,
...
EXIT WHEN NOT FOUND; /* apply on c01 */
	...
	if (nr_seq_requisicao_p IS NOT NULL AND nr_seq_requisicao_p::text <> '') then
		...
		values (nextval('ptu_pedido_compl_aut_serv_seq'), nr_seq_pedido_novo_p, ie_tipo_tabela_w,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_envio_scs_ws_pck.ptu_envio_ped_compl_aut_web ( nr_seq_guia_p pls_guia_plano_proc.nr_seq_guia%type, nr_seq_requisicao_p pls_requisicao_proc.nr_seq_requisicao%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type, nr_seq_pedido_novo_p INOUT ptu_pedido_compl_aut.nr_sequencia%type) FROM PUBLIC;
