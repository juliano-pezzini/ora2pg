...
CREATE OR REPLACE PROCEDURE pls_gerencia_envio_ans_pck.selecionar_proc_lote ( nr_seq_lote_p pls_monitor_tiss_lote.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE


tb_ins_conta_seq_w		pls_util_cta_pck.t_number_table;
tb_ins_conta_proc_seq_w		pls_util_cta_pck.t_number_table;
tb_ins_cta_val_seq_w		pls_util_cta_pck.t_number_table;
tb_ins_proc_val_seq_w		pls_util_cta_pck.t_number_table;
tb_ins_tiss_alt_seq_w		pls_util_cta_pck.t_number_table;
tb_ins_cta_rec_seq_w		pls_util_cta_pck.t_number_table;
tb_ins_proc_rec_seq_w		pls_util_cta_pck.t_number_table;
tb_ins_cta_disc_seq_w		pls_util_cta_pck.t_number_table;
tb_ins_proc_disc_seq_w		pls_util_cta_pck.t_number_table;

tb_upd_conta_seq_w		pls_util_cta_pck.t_number_table;
tb_upd_conta_proc_seq_w		pls_util_cta_pck.t_number_table;
tb_upd_cta_val_seq_w		pls_util_cta_pck.t_number_table;
tb_upd_proc_val_seq_w		pls_util_cta_pck.t_number_table;
tb_upd_tiss_alt_seq_w		pls_util_cta_pck.t_number_table;
tb_upd_cta_rec_seq_w		pls_util_cta_pck.t_number_table;
tb_upd_proc_rec_seq_w		pls_util_cta_pck.t_number_table;
tb_upd_cta_disc_seq_w		pls_util_cta_pck.t_number_table;
tb_upd_proc_disc_seq_w		pls_util_cta_pck.t_number_table;
...
c01 CURSOR(	nr_seq_lote_pc			pls_monitor_tiss_lote.nr_sequencia%type,
	...
	SELECT	e.nr_seq_conta,
		...
		(SELECT	max(p.nr_sequencia)
		 from	pls_monitor_tiss_proc_val p
		 where	p.nr_seq_conta = e.nr_seq_conta
		 and	p.nr_seq_conta_proc = e.nr_seq_conta_proc
		 and	p.nr_seq_cta_val = x.nr_sequencia) nr_seq_cta_proc_val,
	...
	and	(e.nr_seq_conta_proc IS NOT NULL AND e.nr_seq_conta_proc::text <> '')
...
union all
	...
	and  	e.ie_tipo_evento not in ('AV', 'AD', 'FR', 'PR', 'PD')
	group by e.nr_seq_conta,
		 e.nr_seq_conta_proc,
		 x.nr_sequencia,
		 e.nr_seq_conta_rec,
...
union all
	...
	and  	(e.nr_seq_conta_proc IS NOT NULL AND e.nr_seq_conta_proc::text <> '')
	and  	ie_permite_rec_glosa_pc = 'S'
	and 	e.nr_seq_conta_disc = x.nr_seq_conta_disc
	and  	e.ie_tipo_evento not in ('AV', 'AD', 'FR', 'FC', 'PC') --remove aqui os registros de reconhecimento e pagto eles retornam no unin acima. alem do fechamento rec que sempre excluiu
	group by e.nr_seq_conta,
	        e.nr_seq_conta_proc,
	        x.nr_sequencia,
...
for c01_w in c01(nr_seq_lote_p, current_setting('pls_gerencia_envio_ans_pck.dt_mes_competencia_ini_w')::pls_monitor_tiss_lote.dt_mes_competencia%type, current_setting('pls_gerencia_envio_ans_pck.dt_mes_competencia_fim_w')::pls_monitor_tiss_lote.dt_mes_competencia%type, current_setting('pls_gerencia_envio_ans_pck.ie_permite_rec_glosa_w')::pls_monitor_tiss_param.ie_permite_rec_glosa%type) loop
	...
	if (coalesce(c01_w.nr_seq_cta_proc_val::text, '') = '') then
		tb_ins_conta_seq_w(qt_registro_ins_w) := c01_w.nr_seq_conta;
		tb_ins_conta_proc_seq_w(qt_registro_ins_w) := c01_w.nr_seq_conta_proc;
		tb_ins_cta_val_seq_w(qt_registro_ins_w) := c01_w.nr_seq_cta_val;
		tb_ins_proc_val_seq_w(qt_registro_ins_w) := c01_w.nr_seq_cta_proc_val;
		...
		tb_ins_proc_rec_seq_w(qt_registro_ins_w) := c01_w.nr_seq_proc_rec;
		tb_ins_cta_disc_seq_w(qt_registro_ins_w) := c01_w.nr_seq_conta_disc;
		tb_ins_proc_disc_seq_w(qt_registro_ins_w) := c01_w.nr_seq_disc_proc;

		qt_registro_ins_w := qt_registro_ins_w + 1;
	else
		tb_upd_conta_seq_w(qt_registro_upd_w) := c01_w.nr_seq_conta;
		tb_upd_conta_proc_seq_w(qt_registro_upd_w) := c01_w.nr_seq_conta_proc;
		tb_upd_cta_val_seq_w(qt_registro_upd_w) := c01_w.nr_seq_cta_val;
		tb_upd_proc_val_seq_w(qt_registro_upd_w) := c01_w.nr_seq_cta_proc_val;
		...
		tb_upd_proc_rec_seq_w(qt_registro_upd_w) := c01_w.nr_seq_proc_rec;
		tb_upd_cta_disc_seq_w(qt_registro_upd_w) := c01_w.nr_seq_conta_disc;
		tb_upd_proc_disc_seq_w(qt_registro_upd_w) := c01_w.nr_seq_disc_proc;
	...
	if ( qt_registro_ins_w >= current_setting('pls_gerencia_envio_ans_pck.qt_registro_transacao_w')::integer ) then
		...
		if (tb_ins_conta_proc_seq_w.count > 0) then
			forall i in tb_ins_conta_proc_seq_w.first..tb_ins_conta_proc_seq_w.last
				insert into pls_monitor_tiss_proc_val(
					nr_sequencia, nr_seq_conta, nr_seq_conta_proc,
					nr_seq_cta_val,	dt_evento, ie_tipo_evento,
					ie_item_atualizado, nr_seq_cta_alt, ie_glosa,
					nm_usuario_nrec, dt_atualizacao_nrec, nm_usuario,
					dt_atualizacao,	nr_seq_conta_rec, nr_seq_proc_rec,
					nr_seq_conta_disc, nr_seq_disc_proc
				) values (
					nextval('pls_monitor_tiss_proc_val_seq'), tb_ins_conta_seq_w(i),	tb_ins_conta_proc_seq_w(i),
					tb_ins_cta_val_seq_w(i),(SELECT max(z.dt_evento)
					...
					tb_ins_tiss_alt_seq_w(i), (select	ie_glosa
								   ...
								   where 	z.nr_sequencia = tb_ins_conta_proc_seq_w(i)),
					nm_usuario_p, clock_timestamp(),
					nm_usuario_p, clock_timestamp(),
					tb_ins_cta_rec_seq_w(i), tb_ins_proc_rec_seq_w(i), tb_ins_cta_disc_seq_w(i),
					tb_ins_proc_disc_seq_w(i));
			commit;

			tb_ins_conta_seq_w.delete;
			tb_ins_conta_proc_seq_w.delete;
			tb_ins_cta_val_seq_w.delete;
			tb_ins_proc_val_seq_w.delete;
			tb_ins_tiss_alt_seq_w.delete;
			tb_ins_cta_rec_seq_w.delete;
			tb_ins_proc_rec_seq_w.delete;
			tb_ins_cta_disc_seq_w.delete;
			tb_ins_proc_disc_seq_w.delete;
			qt_registro_ins_w := 0;
		end if;
	end if;
	...
	if ( qt_registro_upd_w >= current_setting('pls_gerencia_envio_ans_pck.qt_registro_transacao_w')::integer ) then
		...
		if (tb_upd_conta_proc_seq_w.count > 0) then
			forall i in tb_upd_conta_proc_seq_w.first..tb_upd_conta_proc_seq_w.last
				...
				set	dt_evento			= (	SELECT 	max(z.dt_evento)
					...
					ie_glosa 			= (select	ie_glosa
									   from 	pls_conta_proc z
									   where 	z.nr_sequencia = tb_upd_conta_proc_seq_w(i)),
					nm_usuario			= nm_usuario_p,
					dt_atualizacao			= clock_timestamp(),
					nr_seq_conta_rec		= tb_upd_cta_rec_seq_w(i),
					nr_seq_proc_rec			= tb_upd_proc_rec_seq_w(i),
					nr_seq_conta_disc		= tb_upd_cta_disc_seq_w(i),
					nr_seq_disc_proc		= tb_upd_proc_disc_seq_w(i)
				where	nr_sequencia 			= tb_upd_proc_val_seq_w(i);

			commit;

			tb_upd_conta_seq_w.delete;
			tb_upd_conta_proc_seq_w.delete;
			tb_upd_cta_val_seq_w.delete;
			tb_upd_proc_val_seq_w.delete;
			tb_upd_tiss_alt_seq_w.delete;
			tb_upd_cta_rec_seq_w.delete;
			tb_upd_proc_rec_seq_w.delete;
			tb_upd_cta_disc_seq_w.delete;
			tb_upd_proc_disc_seq_w.delete;
			qt_registro_upd_w := 0;
		end if;
	end if;
end loop;
...
if (tb_ins_conta_proc_seq_w.count > 0) then
	forall i in tb_ins_conta_proc_seq_w.first..tb_ins_conta_proc_seq_w.last
		insert into pls_monitor_tiss_proc_val(
			...
			nr_seq_conta_proc, nr_seq_cta_val,
			dt_evento,
			ie_tipo_evento,
			ie_item_atualizado,
			...
			nextval('pls_monitor_tiss_proc_val_seq'), tb_ins_conta_seq_w(i),
			tb_ins_conta_proc_seq_w(i), tb_ins_cta_val_seq_w(i),
			...
			nm_usuario_p, clock_timestamp(),
			nm_usuario_p, clock_timestamp(),
			tb_ins_cta_rec_seq_w(i), tb_ins_proc_rec_seq_w(i),
			tb_ins_cta_disc_seq_w(i), tb_ins_proc_disc_seq_w(i));
	commit;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerencia_envio_ans_pck.selecionar_proc_lote ( nr_seq_lote_p pls_monitor_tiss_lote.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
