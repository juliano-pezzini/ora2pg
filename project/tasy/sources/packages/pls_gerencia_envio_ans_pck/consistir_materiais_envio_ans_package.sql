...
--Procedure que realiza a consistncia dos materiais lanados na contas mdicas



CREATE OR REPLACE PROCEDURE pls_gerencia_envio_ans_pck.consistir_materiais_envio_ans ( nr_seq_lote_p pls_monitor_tiss_lote.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
C01 CURSOR(	nr_seq_lote_pc		pls_monitor_tiss_lote.nr_sequencia%type )  FOR
	SELECT	a.nr_sequencia,
		...
		a.cd_tabela_ref,
		a.cd_material,
		coalesce(a.vl_material, 0) vl_apresentado,
...
nr_seq_processo_w := pls_gerencia_envio_ans_pck.grava_log_tempo_processo(	nr_seq_lote_p, 'Consistindo materiais do lote', 'I', nm_usuario_p, nr_seq_processo_w);
...
for	r_C01_w in C01( nr_seq_lote_p ) loop
	...
	if ( coalesce(r_C01_w.cd_grupo_proc::text, '') = '' ) then
		...
		tb_nr_seq_cta_mat_val_w(qt_registro_w)	:= r_C01_w.nr_sequencia;
	...
	--INC009 - Cdigo do material no informado
	...
	if ( coalesce(r_C01_w.cd_material::text, '') = '' ) then
		...
		tb_nr_seq_cta_mat_val_w(qt_registro_w)	:= r_C01_w.nr_sequencia;
		tb_nr_seq_lote_w(qt_registro_w)		:= null;
		tb_cd_inconsistencia_w(qt_registro_w)	:= 'INC009';
		qt_registro_w := qt_registro_w + 1;
	end if;

	--INC032 - Quantidade apresentada do procedimento deve ser maior que zero

	if ( r_C01_w.qt_material <= 0 ) then
		tb_nr_seq_cta_val_w(qt_registro_w)	:= r_C01_w.nr_seq_cta_val;
		tb_nr_seq_cta_proc_val_w(qt_registro_w)	:= null;
		tb_nr_seq_cta_mat_val_w(qt_registro_w)	:= r_C01_w.nr_sequencia;
		tb_nr_seq_lote_w(qt_registro_w)		:= null;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerencia_envio_ans_pck.consistir_materiais_envio_ans ( nr_seq_lote_p pls_monitor_tiss_lote.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
