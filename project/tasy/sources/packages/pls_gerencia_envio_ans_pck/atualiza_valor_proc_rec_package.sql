...
--Procedure utilizada para atualizar os valores de procedimentos do recurso de glosa



CREATE OR REPLACE PROCEDURE pls_gerencia_envio_ans_pck.atualiza_valor_proc_rec ( nr_seq_lote_p pls_monitor_tiss_lote.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE


qt_pago_w		pls_monitor_tiss_proc_val.qt_liberado%type;
vl_recursado_w		pls_rec_glosa_proc.vl_recursado%type;
vl_pago_w		pls_rec_glosa_proc.vl_acatado%type;
vl_glosa_w		pls_rec_glosa_proc.vl_recursado%type;
nr_index_w		integer;
...
for r_C01_w in C01(nr_seq_lote_p) loop
	...
	if (r_C01_w.dt_pagamento_recurso IS NOT NULL AND r_C01_w.dt_pagamento_recurso::text <> '') then
		...
		where	a.nr_seq_proc_rec = r_c01_w.nr_seq_proc_rec
		and	a.nr_seq_conta_rec = r_c01_w.nr_seq_conta_rec;
	else
	...
	if (nr_index_w >= pls_util_pck.qt_registro_transacao_w) then
		CALL pls_gerencia_envio_ans_pck.atualizar_monit_tiss_proc_rec(dados_proc_update_w, nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_gerencia_envio_ans_pck.atualiza_valor_proc_rec ( nr_seq_lote_p pls_monitor_tiss_lote.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
