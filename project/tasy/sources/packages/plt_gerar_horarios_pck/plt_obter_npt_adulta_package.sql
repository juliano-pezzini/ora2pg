...
CREATE OR REPLACE PROCEDURE plt_gerar_horarios_pck.plt_obter_npt_adulta ( nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_prescr_usuario_p text, nr_seq_regra_p bigint, ie_edicao_p text, ie_estendidos_p bigint, dt_referencia_p timestamp, ie_exibir_hor_suspensos_p text, dt_plano_p text) AS $body$
DECLARE
...
BEGIN
	...
	if (nr_atendimento_p IS NOT NULL AND nr_atendimento_p::text <> '') then
		...
		from	(	SELECT	a.nr_prescricao,
							...
							'Protocolo ' || y.ds_npt ds_npt,
							CASE WHEN x.ie_suspenso='S' THEN substr(obter_status_solucao_prescr(6,a.nr_prescricao,x.nr_sequencia),1,3)  ELSE CASE WHEN x.ie_status='P' THEN  'W'  ELSE substr(obter_status_solucao_prescr(6,a.nr_prescricao,x.nr_sequencia),1,3) END  END  ie_status_solucao,
							...
							substr(x.qt_fase_npt ||' fase(s) '|| coalesce(obter_desc_intervalo(x.cd_intervalo),obter_desc_intervalo_prescr(x.cd_intervalo)),1,240) qt_fase_npt,
							...
							substr(plt_obter_desc_prod_npt_adulta(a.nr_prescricao,x.nr_sequencia),1,1000) ds_hint,
...
WHERE x.nr_prescricao = a.nr_prescricao and a.nr_atendimento = nr_atendimento_p and plt_gerar_horarios_pck.obter_se_mostra_rep_inter(a.ie_motivo_prescricao) = 'S' and a.nr_prescricao	between current_setting('plt_gerar_horarios_pck.nr_prescr_inicial_w')::bigint and current_setting('plt_gerar_horarios_pck.nr_prescr_final_w')::bigint and a.dt_validade_prescr	between dt_validade_limite_p and dt_fim_w and x.ie_npt_adulta = 'S' and ((current_setting('plt_gerar_horarios_pck.ie_regra_incl_npt_adulta_w')::char(1) = 'S') or
							((current_setting('plt_gerar_horarios_pck.ie_regra_incl_npt_adulta_w')::char(1) = 'R') and (adep_obter_regra_inclusao(	'NAN',
							...
							substr(x.qt_fase_npt ||' fase(s) '|| coalesce(obter_desc_intervalo(x.cd_intervalo),obter_desc_intervalo_prescr(x.cd_intervalo)),1,240),
							...
							CASE WHEN x.ie_suspenso='S' THEN substr(obter_status_solucao_prescr(6,a.nr_prescricao,x.nr_sequencia),1,3)  ELSE CASE WHEN x.ie_status='P' THEN  'W'  ELSE substr(obter_status_solucao_prescr(6,a.nr_prescricao,x.nr_sequencia),1,3) END  END ,
							substr(obter_pendencia_assinatura(nm_usuario_p,a.nr_prescricao,'PR'),1,1),
							substr(plt_obter_desc_prod_npt_adulta(a.nr_prescricao,x.nr_sequencia),1,1000),
		...
		where	plt_obter_se_item_periodo(	dt_validade_limite_p,	dt_fim_w,				get_ie_plano_por_setor,		dt_plano_p,			t.dt_status,
											...
											dt_referencia_p,		ie_edicao_p,			ie_exibir_hor_suspensos_p,	t.dt_suspensao,		'NAN',
											t.nr_seq_npt,			t.nr_prescricao,		'O') = 'S'
		group by
				nr_prescricao,
				nr_seq_npt,
				ds_npt,
		...
		from	(	SELECT	a.nr_prescricao,
					...
					FROM prescr_medica a, nut_pac x
LEFT OUTER JOIN protocolo_npt y ON (x.nr_seq_protocolo = y.nr_sequencia)
WHERE x.nr_prescricao = a.nr_prescricao and a.cd_pessoa_fisica = current_setting('plt_gerar_horarios_pck.cd_pessoa_fisica_w')::varchar(10) and plt_gerar_horarios_pck.obter_se_mostra_rep_inter(a.ie_motivo_prescricao) = 'S' and a.nr_prescricao	between current_setting('plt_gerar_horarios_pck.nr_prescr_inicial_w')::bigint and current_setting('plt_gerar_horarios_pck.nr_prescr_final_w')::bigint and a.dt_validade_prescr	between dt_validade_limite_p and dt_fim_w and x.ie_npt_adulta = 'S' and ((current_setting('plt_gerar_horarios_pck.ie_regra_incl_npt_adulta_w')::char(1) = 'S') or
							((current_setting('plt_gerar_horarios_pck.ie_regra_incl_npt_adulta_w')::char(1) = 'R') and (adep_obter_regra_inclusao(	'NAN',
																								wheb_usuario_pck.get_cd_estabelecimento, 
																								wheb_usuario_pck.get_cd_setor_atendimento, 
																								wheb_usuario_pck.get_cd_perfil, 
