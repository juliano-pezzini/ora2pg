...
CREATE OR REPLACE PROCEDURE plt_gerar_horarios_pck.plt_sincronizar_supl_oral ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_validade_limite_p timestamp, ie_agrupar_acm_sn_p text, ie_prescr_usuario_p text, ie_edicao_p text, ie_estendidos_p bigint, dt_referencia_p timestamp, ie_exibir_hor_suspensos_p text, dt_plano_p text) AS $body$
DECLARE
	...
	dt_fim_w			timestamp;
	ds_informacoes_ww	varchar(4000);
...
BEGIN
	...
	if (nr_atendimento_p IS NOT NULL AND nr_atendimento_p::text <> '') then
		...
		from	(	SELECT	a.nr_prescricao,
							...
							c.dt_horario,
							CASE WHEN coalesce(obter_se_mostra_adic(15),'S')='N' THEN null  ELSE substr(x.ds_observacao,1,255) END  ds_observacao,
		...
		where	plt_obter_se_item_periodo(	dt_validade_limite_p,		dt_fim_w,				get_ie_plano_por_setor,		dt_plano_p,			dt_horario,
											...
											dt_referencia_p,			ie_edicao_p,			ie_exibir_hor_suspensos_p,	dt_susp_item,		'S',
	...
	CALL Atualizar_plt_controle(nm_usuario_p, nr_atendimento_p, current_setting('plt_gerar_horarios_pck.cd_pessoa_fisica_w')::varchar(10), 'S', 'N',null);	
