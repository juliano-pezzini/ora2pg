...
BEGIN
	...
	if (nr_atendimento_p IS NOT NULL AND nr_atendimento_p::text <> '') then
		...
		SELECT	nr_prescricao,
				...
				ie_prescr_farm,
				ie_recem_nato
		from	(	SELECT	null nr_prescricao,
...
WHERE z.nr_seq_exame = x.nr_seq_exame and y.cd_procedimento = x.cd_procedimento and y.ie_origem_proced = x.ie_origem_proced and y.cd_procedimento = c.cd_procedimento and y.ie_origem_proced = c.ie_origem_proced and w.nr_sequencia = x.nr_seq_proc_interno and w.nr_sequencia = c.nr_seq_proc_interno and x.nr_prescricao = c.nr_prescricao and x.nr_sequencia = c.nr_seq_procedimento and x.nr_prescricao = a.nr_prescricao and c.nr_prescricao = a.nr_prescricao and a.nr_atendimento = nr_atendimento_p and plt_gerar_horarios_pck.obter_se_mostra_rep_inter(a.ie_motivo_prescricao) = 'S' and a.nr_prescricao	between current_setting('plt_gerar_horarios_pck.nr_prescr_inicial_w')::bigint and current_setting('plt_gerar_horarios_pck.nr_prescr_final_w')::bigint and ((a.dt_validade_prescr	between dt_validade_limite_p	and dt_fim_w)or (a.ie_hemodialise IS NOT NULL AND a.ie_hemodialise::text <> '')) and coalesce(w.ie_tipo,'N') <> 'G' and coalesce(w.ie_tipo,'N') <> 'BS' and coalesce(w.ie_ivc,'N') <> 'S' and coalesce(w.ie_ctrl_glic,'XPTO') <> 'CIG' and (x.nr_seq_proc_interno IS NOT NULL AND x.nr_seq_proc_interno::text <> '') and coalesce(x.nr_seq_prot_glic::text, '') = '' and (x.nr_seq_exame IS NOT NULL AND x.nr_seq_exame::text <> '') and coalesce(x.nr_seq_solic_sangue::text, '') = '' and coalesce(x.nr_seq_derivado::text, '') = '' and coalesce(x.nr_seq_exame_sangue::text, '') = '' and coalesce(c.ie_situacao,'A') = 'A' and (ie_dep_int_w = 'S' or coalesce(x.nr_seq_origem::text, '') = '') and ((current_setting('plt_gerar_horarios_pck.ie_regra_incl_coleta_w')::char(1) = 'S') or
		...
		from	(	SELECT	null nr_prescricao,
					...
					from	(	select	a.nr_prescricao,
								...
								FROM exame_laboratorio z, procedimento y, prescr_proc_hor c, prescr_medica a, prescr_procedimento x
LEFT OUTER JOIN material_exame_lab v ON (x.cd_material_exame = v.cd_material_exame)
WHERE z.nr_seq_exame = x.nr_seq_exame and y.cd_procedimento = x.cd_procedimento and y.ie_origem_proced = x.ie_origem_proced and y.cd_procedimento = c.cd_procedimento and y.ie_origem_proced = c.ie_origem_proced and x.nr_prescricao = c.nr_prescricao and x.nr_sequencia = c.nr_seq_procedimento and x.nr_prescricao = a.nr_prescricao and c.nr_prescricao = a.nr_prescricao and a.cd_pessoa_fisica = current_setting('plt_gerar_horarios_pck.cd_pessoa_fisica_w')::varchar(10) and PLT_Gerar_Horarios_PCK.OBTER_SE_MOSTRA_REP_INTER(a.ie_motivo_prescricao) = 'S' and a.nr_prescricao	between current_setting('plt_gerar_horarios_pck.nr_prescr_inicial_w')::bigint and current_setting('plt_gerar_horarios_pck.nr_prescr_final_w')::bigint and ((a.dt_validade_prescr	between dt_validade_limite_p	and dt_fim_w)or (a.ie_hemodialise IS NOT NULL AND a.ie_hemodialise::text <> '')) and coalesce(x.nr_seq_proc_interno::text, '') = '' and (x.nr_seq_exame IS NOT NULL AND x.nr_seq_exame::text <> '') and coalesce(x.nr_seq_solic_sangue::text, '') = '' and coalesce(x.nr_seq_derivado::text, '') = '' and coalesce(x.nr_seq_exame_sangue::text, '') = '' and coalesce(c.ie_situacao,'A') = 'A' and (ie_dep_int_w = 'S' or coalesce(x.nr_seq_origem::text, '') = '') and ((current_setting('plt_gerar_horarios_pck.ie_regra_incl_coleta_w')::char(1) = 'S') or
										 ((current_setting('plt_gerar_horarios_pck.ie_regra_incl_coleta_w')::char(1) = 'R') and (adep_obter_regra_inclusao(	'LAB', 
																											cd_estabelecimento_p, 
																											cd_setor_usuario_p, 
																											cd_perfil_p, 
...
WHERE z.nr_seq_exame = x.nr_seq_exame and y.cd_procedimento = x.cd_procedimento and y.ie_origem_proced = x.ie_origem_proced and y.cd_procedimento = c.cd_procedimento and y.ie_origem_proced = c.ie_origem_proced and w.nr_sequencia = x.nr_seq_proc_interno and w.nr_sequencia = c.nr_seq_proc_interno and x.nr_prescricao = c.nr_prescricao and x.nr_sequencia = c.nr_seq_procedimento and x.nr_prescricao = a.nr_prescricao and c.nr_prescricao = a.nr_prescricao and a.cd_pessoa_fisica = current_setting('plt_gerar_horarios_pck.cd_pessoa_fisica_w')::varchar(10) and PLT_Gerar_Horarios_PCK.OBTER_SE_MOSTRA_REP_INTER(a.ie_motivo_prescricao) = 'S' and a.nr_prescricao	between current_setting('plt_gerar_horarios_pck.nr_prescr_inicial_w')::bigint and current_setting('plt_gerar_horarios_pck.nr_prescr_final_w')::bigint and ((a.dt_validade_prescr	between dt_validade_limite_p	and dt_fim_w)or (a.ie_hemodialise IS NOT NULL AND a.ie_hemodialise::text <> '')) and coalesce(w.ie_tipo,'N') <> 'G' and coalesce(w.ie_tipo,'N') <> 'BS' and coalesce(w.ie_ivc,'N') <> 'S' and coalesce(w.ie_ctrl_glic,'XPTO') <> 'CIG' and (x.nr_seq_proc_interno IS NOT NULL AND x.nr_seq_proc_interno::text <> '') and coalesce(x.nr_seq_prot_glic::text, '') = '' and (x.nr_seq_exame IS NOT NULL AND x.nr_seq_exame::text <> '') and coalesce(x.nr_seq_solic_sangue::text, '') = '' and coalesce(x.nr_seq_derivado::text, '') = '' and coalesce(x.nr_seq_exame_sangue::text, '') = '' and coalesce(c.ie_situacao,'A') = 'A' and (ie_dep_int_w = 'S' or coalesce(x.nr_seq_origem::text, '') = '') and ((current_setting('plt_gerar_horarios_pck.ie_regra_incl_coleta_w')::char(1) = 'S') or
					...
					where	plt_obter_se_item_periodo(	dt_validade_limite_p,	dt_fim_w,				get_ie_plano_por_setor,		dt_plano_p,			t.dt_horario,
														dt_inicial_horarios_p,	dt_final_horarios_p,	ie_prescr_usuario_p,		nm_usuario_p,		ie_estendidos_p,
														dt_referencia_p,		ie_edicao_p,			ie_exibir_hor_suspensos_p,	t.dt_suspensao,		'L',
														t.nr_seq_procedimento,	t.nr_prescricao,		'O') = 'S') alias152
		group by
			nr_prescricao,
			nr_seq_procedimento,	
			cd_procedimento,
