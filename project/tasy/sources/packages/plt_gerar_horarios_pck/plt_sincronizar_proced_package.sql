...
CREATE OR REPLACE PROCEDURE plt_gerar_horarios_pck.plt_sincronizar_proced ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_validade_limite_p timestamp, ie_prescr_usuario_p text, ie_edicao_p text, ie_estendidos_p bigint, dt_referencia_p timestamp, ie_exibir_hor_suspensos_p text, dt_plano_p text, nr_seq_modelo_p bigint) AS $body$
DECLARE
...
BEGIN
	...
	if (nr_atendimento_p IS NOT NULL AND nr_atendimento_p::text <> '') then
		...
		SELECT	nr_prescricao,
				CASE WHEN substr(obter_se_associado_dif(nr_prescricao,nr_seq_procedimento),1,1)='N' THEN  nr_seq_procedimento||nr_prescricao  ELSE nr_seq_procedimento END ,
				nr_sequencia,
				ie_status_hor,
				cd_procedimento,
				ds_procedimento,
		...
		from	(	SELECT	a.nr_prescricao,
							...
							c.nr_sequencia,
							substr(plt_obter_status_hor_proc_lib(c.dt_fim_horario,c.dt_suspensao,c.ie_dose_especial,c.dt_inicio_proced,c.dt_lib_horario),1,15) ie_status_hor,
							c.cd_procedimento,
							...
							CASE WHEN coalesce(obter_se_mostra_adic(12),'S')='N' THEN null  ELSE substr(obter_mat_med_assoc_proc(a.nr_prescricao,c.nr_seq_procedimento,'S') ||' '|| substr(x.ds_observacao,1,255),1,2000) END  ds_mat_med_assoc,
					...
					and	plt_obter_se_item_periodo(	dt_validade_limite_p,		dt_fim_w,		get_ie_plano_por_setor,		dt_plano_p,		c.dt_horario,
										...
										x.nr_sequencia,			a.nr_prescricao,	'S') = 'S'
					group by
							a.nr_prescricao,
							c.nr_seq_procedimento,
							c.nr_sequencia,
							...
							c.dt_horario,
							x.ie_lado,
							substr(obter_mat_med_assoc_proc(a.nr_prescricao,c.nr_seq_procedimento,'S') ||' '|| substr(x.ds_observacao,1,255),1,2000),
							a.ie_prescr_farm,
...
union all
					...
					and		((current_setting('plt_gerar_horarios_pck.ie_regra_incl_procedimento_w')::char(1) = 'S') or
							((current_setting('plt_gerar_horarios_pck.ie_regra_incl_procedimento_w')::char(1) = 'R') and (adep_obter_regra_inclusao(	'PROC',
																									...
																									a.cd_setor_Atendimento,
																									x.cd_setor_atendimento,
																									a.nr_prescricao, -- nr_prescricao_p. Passei nulo porque criaram o param na adep_obter_regra_inclusao como default null, e nao haviam passado nada
					...
					group by
							...
							a.ie_prescr_farm,
							coalesce(a.ie_recem_nato, 'N'),
							x.dt_suspensao) alias129
		where	plt_obter_se_item_periodo(	dt_validade_limite_p,								dt_fim_w,								get_ie_plano_por_setor,		dt_plano_p,		dt_horario,
											coalesce(current_setting('plt_gerar_horarios_pck.dt_horario_inicio_sinc_w')::timestamp,dt_validade_limite_p),	coalesce(current_setting('plt_gerar_horarios_pck.dt_horario_fim_sinc_w')::timestamp,dt_fim_w),	ie_prescr_usuario_p,		nm_usuario_p,	ie_estendidos_p,
											dt_referencia_p,									ie_edicao_p,							ie_exibir_hor_suspensos_p,	dt_susp_item,	'P',
...
union all
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		if (nr_horario_w IS NOT NULL AND nr_horario_w::text <> '') then		
			ds_comando_update_w	:=	' update w_rep_t ' ||
							...
							' and ((ds_diluicao = :ds_mat_med_assoc) or (ds_diluicao is null)) ' ||
	...
	CALL Atualizar_plt_controle(nm_usuario_p, nr_atendimento_p, current_setting('plt_gerar_horarios_pck.cd_pessoa_fisica_w')::varchar(10), 'P', 'N',null);	
