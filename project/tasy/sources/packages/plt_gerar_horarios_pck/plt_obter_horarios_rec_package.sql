...
CREATE OR REPLACE PROCEDURE plt_gerar_horarios_pck.plt_obter_horarios_rec ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_prescr_usuario_p text, ie_edicao_p text, ie_estendidos_p bigint, dt_referencia_p timestamp, ie_exibir_hor_suspensos_p text, dt_plano_p text) AS $body$
DECLARE
...
BEGIN
	...
	if (nr_atendimento_p IS NOT NULL AND nr_atendimento_p::text <> '') then
		...
		from	(SELECT	c.dt_horario,
				...
				and		((current_setting('plt_gerar_horarios_pck.ie_regra_incl_recomendacao_w')::char(1) = 'S') or
				...
				and		((coalesce(c.ie_horario_especial,'N') = 'N') or 
						 ((coalesce(c.ie_horario_especial,'N') = 'S') and (obter_se_acm_sn(x.ie_acm, x.ie_se_necessario) = 'S')) or (c.dt_fim_horario IS NOT NULL AND c.dt_fim_horario::text <> ''))) t
		where	plt_obter_se_item_periodo(	dt_validade_limite_p,	dt_fim_w,				get_ie_plano_por_setor,		dt_plano_p,		t.dt_horario,
											...
											dt_referencia_p,		ie_edicao_p,			ie_exibir_hor_suspensos_p,	t.dt_suspensao,	'R',
