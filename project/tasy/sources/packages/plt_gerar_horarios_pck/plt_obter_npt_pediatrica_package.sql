...
CREATE OR REPLACE PROCEDURE plt_gerar_horarios_pck.plt_obter_npt_pediatrica ( nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_prescr_usuario_p text, nr_seq_regra_p bigint, ie_edicao_p text, ie_estendidos_p bigint, dt_referencia_p timestamp, ie_exibir_hor_suspensos_p text, dt_plano_p text) AS $body$
DECLARE
...
BEGIN
	...
	if (nr_atendimento_p IS NOT NULL AND nr_atendimento_p::text <> '') then
		...
		from	(	SELECT	a.nr_prescricao,
							...
							wheb_mensagem_pck.get_texto(1111250) || ' ' || x.hr_prim_horario ds_npt,
					...
					and		((current_setting('plt_gerar_horarios_pck.ie_regra_incl_npt_pediatrica_w')::char(1) = 'S') or
							((current_setting('plt_gerar_horarios_pck.ie_regra_incl_npt_pediatrica_w')::char(1) = 'R') and (adep_obter_regra_inclusao(	'NPP',
		...
		where	plt_obter_se_item_periodo(	dt_validade_limite_p,	dt_fim_w,				get_ie_plano_por_setor,		dt_plano_p,			t.dt_status,
											...
											dt_referencia_p,		ie_edicao_p,			ie_exibir_hor_suspensos_p,	t.dt_suspensao,		'NPP',
	...
	CALL Atualizar_plt_controle(nm_usuario_p, nr_atendimento_p, current_setting('plt_gerar_horarios_pck.cd_pessoa_fisica_w')::varchar(10), 'NPP', 'N',null);	
