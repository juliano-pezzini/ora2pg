...
/* ################ Start get_version_info2           ####################### */

	


CREATE OR REPLACE FUNCTION phi_ordem_servico_pck.get_version_info2 (ds_version_p text) RETURNS SETOF T_VERSION_ROW_DATA AS $body$
DECLARE
...
BEGIN
		...
		begin
			...
			t_version_row_w.ds_hotfix	:= substr(ds_version_p, instr(ds_version_p, '.', 1, 3) +1, (instr(ds_version_p, '-', -1) -1 - instr(ds_version_p, '.', 1, 3) -1));
			t_version_row_w.ds_build	:= substr(ds_version_p, instr(ds_version_p, '-', -1), length(ds_version_p));
		end;
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION phi_ordem_servico_pck.get_version_info2 (ds_version_p text) FROM PUBLIC;
