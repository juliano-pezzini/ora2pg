...
CREATE OR REPLACE PROCEDURE hpms_proposal_request_pck.enviar_email_proposta (ds_email_p text, nr_seq_proposta_onl_p pls_proposta_online.nr_sequencia%type, ie_acao_envio_p pls_solic_prop_regra_email.ie_acao_envio%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) AS $body$
DECLARE
...
ds_segurado_macro_w	varchar(4000);

C01 CURSOR(	ie_acao_envio_pc	text,
...
C02 CURSOR(nr_seq_regra_pc pls_solic_prop_regra_email.nr_sequencia%type)FOR
...
C04 CURSOR(nr_seq_contrato_pc pls_contrato.nr_sequencia%type)FOR
...
ds_remetente_w	:= Obter_Valor_Param_Usuario(1395, 2, Obter_Perfil_Ativo, nm_usuario_p, cd_estabelecimento_p);
...
ds_segurado_macro_w := null;
for r_c04_w in C04(nr_seq_contrato_w) loop
	ds_segurado_macro_w := ds_segurado_macro_w || r_c04_w.nm_pessoa_fisica || ' (' || r_c04_w.cd_usuario_plano || ')' || chr(10);
end loop;

if	(ds_email_p IS NOT NULL AND ds_email_p::text <> '' AND ds_remetente_w IS NOT NULL AND ds_remetente_w::text <> '') then
	for r_c01_w in c01(ie_acao_envio_p, cd_estabelecimento_p) loop
		begin
		select	ds_email,
			ds_assunto
		...
		ds_mensagem_w	:= substr(replace(ds_mensagem_w, '@LISTA_BENEFICIARIOS', ds_segurado_macro_w), 1, 4000);
		...
		insert	into pls_email(nr_sequencia, cd_estabelecimento, nm_usuario,
			...
			ds_assunto, nr_seq_proposta_online)
		values (nextval('pls_email_seq'), cd_estabelecimento_p, nm_usuario_p,
			clock_timestamp(), nm_usuario_p, clock_timestamp(),
			6, 'P', 6,
			ds_remetente_w, ds_mensagem_w, ds_email_p,
			ds_assunto_w, nr_seq_proposta_onl_p) returning nr_sequencia into nr_seq_email_w;
		end;
		...
		for r_c02_w in C02(r_c01_w.nr_sequencia) loop
				...
				if (r_C02_w.cd_relatorio IS NOT NULL AND r_C02_w.cd_relatorio::text <> '') then
					for r_C03_w in C03(	r_C02_w.cd_classif_relat,
						...
						if (upper(r_C03_w.cd_parametro) = 'NR_SEQ_PROPOSTA') then
							ds_valor_parametro_w	:= nr_seq_proposta_onl_p;
						elsif (upper(r_c03_w.cd_parametro) = 'NR_SEQ_CONTRATO') then
							ds_valor_parametro_w	:= nr_seq_contrato_w;
						else
