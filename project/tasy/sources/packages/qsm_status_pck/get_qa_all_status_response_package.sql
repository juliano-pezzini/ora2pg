...
CREATE OR REPLACE PROCEDURE qsm_status_pck.get_qa_all_status_response ( nr_sequencia_p bigint) AS $body$
DECLARE
...
ie_regra_conversao_w	varchar(1);
qa_status_w			xml;
...
C05 CURSOR(xml_cp xml) FOR
	SELECT	column_value
	from	xmltable(xmlnamespaces(
				'http://www.mmm.com/his/qsmed/status' as "stat",
				'http://www.mmm.com/his/qsmed' as "qai"),
				'qai:module' passing xml_cp);
...
select	qaStatus
into STRICT	qa_status_w
from	xmltable(
		xmlnamespaces(
		'http://www.mmm.com/his/qsmed/status' as "stat",
		'http://schemas.xmlsoap.org/soap/envelope/' as "soapenv"),
		'soapenv:Envelope/soapenv:Body/stat:getQAStatusResponse' passing xml_w columns
		qaStatus	xmltype		path	'stat:qaStatus');
		
for r_C02_w in C02(qa_status_w) loop
		begin	
		begin
		select	ds_case,
				ds_client
		...
		from	xmltable(xmlnamespaces(
					'http://www.mmm.com/his/qsmed/status' as "stat"),
					'stat:qaStatus' passing r_C02_w.column_value columns
	...
	for r_C03_w in C03(r_C02_w.column_value) loop
		begin
			for r_C05_w in C05(r_C03_w.ds_module) loop
			begin
				...
				from	xmltable(xmlnamespaces(
							'http://www.mmm.com/his/qsmed/status' as "stat",
							'http://www.mmm.com/his/qsmed' as "qai"),
							'qai:module' passing r_C05_w.column_value columns
				...
				and		a.nr_seq_episodio	= 	coalesce((SELECT max(nr_sequencia)
												 from 	episodio_paciente
												 where 	nr_episodio = ds_case_w),ds_case_w)
				and		ds_module_id	= 		ds_id_w;
				
				delete FROM episodio_pac_doc_status
				where	nr_seq_document	= nr_seq_doc_w;

				for r_C04_w in C04(r_C05_w.column_value) loop
				begin
					select	coalesce(max('S'),'N')
					into STRICT	ie_regra_conversao_w
					from	conversao_meio_externo
					where	nm_tabela	= 'PERFIL'
					and		nm_atributo	= 'CD_PERFIL'
					and		cd_externo	= r_C04_w.ds_role
					and		nr_seq_regra = nr_seq_regra_w;
					
					if (ie_regra_conversao_w = 'S')then
						for r_conversao_w in conversao_c(nr_seq_regra_w, r_C04_w.ds_role) loop
							begin
							...
							values (
								...
								r_C04_w.ds_status, r_C04_w.ds_role, r_conversao_w.cd_interno);
							end;
						end loop;
					else
						insert into episodio_pac_doc_status(
							...
							values (
								nextval('episodio_pac_doc_status_seq'), nm_usuario_w, clock_timestamp(),
								nm_usuario_w, clock_timestamp(), nr_seq_doc_w,
								r_C04_w.ds_status, r_C04_w.ds_role, null);
					end if;
					...
					from	episodio_paciente
					where	nr_episodio	= ds_case_w
                    or  nr_sequencia = ds_case_w;

					select	max(nr_atendimento)
					into STRICT	nr_atendimento_w
					from	atendimento_paciente
					where	nr_seq_episodio	= coalesce((SELECT max(nr_sequencia)
												 from 	episodio_paciente
												 where 	nr_episodio = ds_case_w),ds_case_w);
					
					if ((r_C04_w.ds_role = 'ADM') and ((r_C04_w.ds_status = 'RELEASED') or (r_C04_w.ds_status = 'CANCELLED'))) then
...
exception
when others then
	ds_erro_w	:= dbms_utility.format_error_backtrace||substr(sqlerrm,1,4000);
	rollback;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE qsm_status_pck.get_qa_all_status_response ( nr_sequencia_p bigint) FROM PUBLIC;
