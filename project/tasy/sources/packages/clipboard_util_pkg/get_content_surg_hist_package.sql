...
CREATE OR REPLACE FUNCTION clipboard_util_pkg.get_content_surg_hist ( nr_atendimento_p bigint, nr_sequencia_p text, ie_record_type_p text, config_option_p bigint ) RETURNS varchar AS $body$
DECLARE
...
BEGIN
        FOR i IN (WITH RECURSIVE cte AS (
         ...
         JOIN cte c ON ()
            ...
            FOR r_c01 IN c01 LOOP IF ( ie_record_type_p = 'SUR_HIST' ) THEN
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION clipboard_util_pkg.get_content_surg_hist ( nr_atendimento_p bigint, nr_sequencia_p text, ie_record_type_p text, config_option_p bigint ) FROM PUBLIC;
