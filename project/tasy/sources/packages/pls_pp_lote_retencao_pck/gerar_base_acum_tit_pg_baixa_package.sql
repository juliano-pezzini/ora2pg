...
CREATE OR REPLACE PROCEDURE pls_pp_lote_retencao_pck.gerar_base_acum_tit_pg_baixa ( nr_seq_lote_p pls_pp_lr_lote.nr_sequencia%type, cd_tributo_p tributo.cd_tributo%type, ie_rest_estab_p tributo.ie_restringe_estab%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
-- aqui não precisa restringir pelo tipo de vencimento (competência ou vencimento) pois em ambos os casos os campos
-- que precisam ser verificados são os mesmos
c01 CURSOR(	cd_tributo_pc		tributo.cd_tributo%type,
	...
	and	d.dt_baixa between dt_comp_inicio_pc and dt_comp_fim_pc
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_pp_lote_retencao_pck.gerar_base_acum_tit_pg_baixa ( nr_seq_lote_p pls_pp_lr_lote.nr_sequencia%type, cd_tributo_p tributo.cd_tributo%type, ie_rest_estab_p tributo.ie_restringe_estab%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
