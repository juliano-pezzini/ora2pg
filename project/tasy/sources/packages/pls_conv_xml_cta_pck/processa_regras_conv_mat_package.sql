...
CREATE OR REPLACE PROCEDURE pls_conv_xml_cta_pck.processa_regras_conv_mat () AS $body$
DECLARE
...
ds_campo_w := 	' select a.nr_sequencia nr_seq_item, '|| pls_util_pck.enter_w ||
			' :nr_seq_regra nr_seq_regra, '|| pls_util_pck.enter_w ||
			' :nr_seq_material nr_seq_material '|| pls_util_pck.enter_w ||
...
for r_c01_w in C01 loop
	...
	bind_sql_valor_w := sql_pck.bind_variable(':nr_seq_material', r_c01_w.nr_seq_material, bind_sql_valor_w);

	-- monta a restricao de acordo com os dados da regra que esta sendo processada

	bind_sql_valor_w := pls_conv_xml_cta_pck.obter_restricao_regra_mat(	'a', r_c01_w.dt_inicio_vigencia, r_c01_w.dt_fim_vigencia, r_c01_w.ie_tipo_tabela, r_c01_w.nr_seq_prestador, r_c01_w.nr_seq_tipo_prestador, r_c01_w.nr_seq_grupo_prestador, r_c01_w.cd_material_orig_inicial, r_c01_w.cd_material_orig_final, r_c01_w.nr_seq_material_orig, r_c01_w.ie_tipo_despesa_mat, bind_sql_valor_w);
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_conv_xml_cta_pck.processa_regras_conv_mat () FROM PUBLIC;
