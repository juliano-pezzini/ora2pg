...
CREATE OR REPLACE FUNCTION tiss_dashboard_data_pck.obter_max_cd_glosa_conv (cd_convenio_p convenio.cd_convenio%type, dt_ref_p timestamp) RETURNS MOTIVO_GLOSA.CD_MOTIVO_GLOSA%TYPE AS $body$
DECLARE
...
BEGIN
	...
	from (
		...
		and 	(a.cd_motivo_glosa IS NOT NULL AND a.cd_motivo_glosa::text <> '')	
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION tiss_dashboard_data_pck.obter_max_cd_glosa_conv (cd_convenio_p convenio.cd_convenio%type, dt_ref_p timestamp) FROM PUBLIC;
