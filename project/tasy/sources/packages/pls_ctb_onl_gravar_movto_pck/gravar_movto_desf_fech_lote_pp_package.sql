...
CREATE OR REPLACE PROCEDURE pls_ctb_onl_gravar_movto_pck.gravar_movto_desf_fech_lote_pp ( nr_seq_lote_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
WHERE l.nr_sequencia		= i.nr_seq_lote and p.nr_sequencia		= i.nr_seq_prestador and e.nr_sequencia		= i.nr_seq_evento  and e.ie_tipo_evento	= 'F' and f.nr_documento		= l.nr_sequencia and f.nr_seq_doc_compl	= e.nr_sequencia and f.nr_doc_analitico	= i.nr_sequencia and f.cd_tipo_lote_contabil = 41 and f.nm_tabela		= 'PLS_PP_ITEM_LOTE' and f.nm_atributo		= 'VL_ITEM' and coalesce(f.ds_origem, 'X')	<> 'ESTORNO' and l.nr_sequencia		= nr_seq_lote_p group by i.vl_item,
		f.nm_tabela,
		f.nm_atributo,
		f.cd_tipo_lote_contabil,
		f.nr_seq_info,
...
union
	...
	FROM pls_prestador p, pls_pp_lote l, ctb_documento f, pls_evento e, pls_pp_item_lote i
LEFT OUTER JOIN pls_pp_lanc_programado z ON (i.nr_seq_lanc_prog = z.nr_sequencia)
WHERE l.nr_sequencia		= i.nr_seq_lote and p.nr_sequencia		= i.nr_seq_prestador and e.nr_sequencia		= i.nr_seq_evento  and e.ie_tipo_evento	= 'F' and coalesce(i.vl_acao_negativo,0) <> 0 and f.nr_documento		= l.nr_sequencia and f.nr_seq_doc_compl	= e.nr_sequencia and f.nr_doc_analitico	= i.nr_sequencia and f.cd_tipo_lote_contabil = 41 and f.nm_tabela		= 'PLS_PP_ITEM_LOTE' and f.nm_atributo		= 'VL_ITEM' and coalesce(f.ds_origem, 'X')	<> 'ESTORNO' and l.nr_sequencia		= nr_seq_lote_p group by i.vl_acao_negativo,
		f.nm_tabela,
		f.nm_atributo,
		f.cd_tipo_lote_contabil,
		f.nr_seq_info,
...
WHERE c.nr_sequencia		= d.nr_seq_conta_proc and b.nr_sequencia		= c.nr_seq_conta and b.nr_sequencia		= d.nr_seq_conta and d.nr_sequencia		= i.nr_seq_resumo and b.nr_sequencia		= i.nr_seq_conta and a.nr_sequencia		= i.nr_seq_lote  and ((d.ie_situacao = 'A') or (coalesce(d.ie_situacao::text, '') = '')) and f.nr_documento		= b.nr_seq_protocolo and f.nr_seq_doc_compl	= b.nr_sequencia and f.nr_doc_analitico	= d.nr_sequencia and f.cd_tipo_lote_contabil = 41 and f.nm_tabela		= 'PLS_CONTA_MEDICA_RESUMO' and f.nm_atributo		in ('VL_LIBERADO', 'VL_GLOSA', 'VL_AJUSTE') and coalesce(f.ds_origem, 'X')	<> 'ESTORNO' and a.nr_sequencia		= nr_seq_lote_p group by d.vl_liberado,
		d.vl_glosa,
		d.vl_apresentado,
		d.vl_calculado,
		b.nr_seq_protocolo,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_ctb_onl_gravar_movto_pck.gravar_movto_desf_fech_lote_pp ( nr_seq_lote_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
