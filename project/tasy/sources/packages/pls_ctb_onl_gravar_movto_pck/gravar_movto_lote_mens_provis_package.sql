...
CREATE OR REPLACE PROCEDURE pls_ctb_onl_gravar_movto_pck.gravar_movto_lote_mens_provis ( nr_seq_lote_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
	...
	and	h.nm_atributo in ('VL_COPARTICIPACAO', 'VL_POS_ESTABELECIDO', 'VL_ITEM', 'VL_CONTA_CO', 'VL_ANTECIPACAO')
	and	h.cd_tipo_lote_contabil in (49, 50)
	and	coalesce(h.ds_origem, 'X') <> 'ESTORNO'
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_ctb_onl_gravar_movto_pck.gravar_movto_lote_mens_provis ( nr_seq_lote_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
