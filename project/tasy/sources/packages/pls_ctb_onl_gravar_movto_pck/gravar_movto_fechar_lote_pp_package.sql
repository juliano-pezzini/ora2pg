...
CREATE OR REPLACE PROCEDURE pls_ctb_onl_gravar_movto_pck.gravar_movto_fechar_lote_pp ( nr_seq_lote_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
	...
	SELECT	coalesce(i.vl_item,0) vl_item,  /*Contabilizar os valores dos itens, conforme o lancamento no pagamento de producao medica*/
		fim_mes(l.dt_mes_competencia) dt_movimento,
...
WHERE l.nr_sequencia		= i.nr_seq_lote and p.nr_sequencia		= i.nr_seq_prestador and e.nr_sequencia		= i.nr_seq_evento  and e.ie_tipo_evento	= 'F' and coalesce(i.vl_acao_negativo,0) <> 0 and l.ie_status		= 'D' and l.nr_sequencia		= nr_seq_lote_p;
	...
	SELECT	'P' ie_proc_mat, /*Procedimento*/
		fim_mes(a.dt_mes_competencia) dt_movimento,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_ctb_onl_gravar_movto_pck.gravar_movto_fechar_lote_pp ( nr_seq_lote_p bigint, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
