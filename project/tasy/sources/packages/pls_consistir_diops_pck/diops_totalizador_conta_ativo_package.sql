...
CREATE OR REPLACE PROCEDURE pls_consistir_diops_pck.diops_totalizador_conta_ativo ( nr_seq_periodo_p bigint, cd_estabelecimento_p bigint) AS $body$
DECLARE


	cd_conta_contabil_w		conta_contabil.cd_conta_contabil%type;
	ds_conta_w			diops_fin_mov_ativo.ds_conta%type;
	vl_saldo_final_w		diops_fin_mov_ativo.vl_saldo_final%type;	
	vl_saldo_final_inferior_w	diops_fin_mov_ativo.vl_saldo_final%type;
...
BEGIN
	...
	EXIT WHEN NOT FOUND; /* apply on c_contas */
		begin
			...
			Ex: 1.5.3.4.2 (SUPERIOR) and 1.5.3.4.2.1 (CHILD) */
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_consistir_diops_pck.diops_totalizador_conta_ativo ( nr_seq_periodo_p bigint, cd_estabelecimento_p bigint) FROM PUBLIC;
