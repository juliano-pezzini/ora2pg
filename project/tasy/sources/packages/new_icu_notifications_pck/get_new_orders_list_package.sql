...
CREATE OR REPLACE FUNCTION new_icu_notifications_pck.get_new_orders_list (nm_usuario_p text) RETURNS SETOF T_ALERTS_TABLE AS $body$
DECLARE
...
BEGIN
	...
	if (coalesce(new_icu_notifications_pck.get_visualiza_alertas(nm_usuario_p, 'ANO'), 'X') <> 'S') then
		return;
	end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION new_icu_notifications_pck.get_new_orders_list (nm_usuario_p text) FROM PUBLIC;
