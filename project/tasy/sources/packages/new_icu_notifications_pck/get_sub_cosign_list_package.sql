...
CREATE OR REPLACE FUNCTION new_icu_notifications_pck.get_sub_cosign_list (nm_usuario_p text) RETURNS SETOF T_ALERTS_TABLE AS $body$
DECLARE
...
BEGIN
	...
	if (coalesce(new_icu_notifications_pck.get_visualiza_alertas(nm_usuario_p, 'AAC'), 'X') <> 'S') then
		return;
	end if;
	...
	loop
		...
		qt_items_w := new_icu_notifications_pck.get_sub_cosign_items(c_encounter_w.nr_atendimento, nm_usuario_p);
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION new_icu_notifications_pck.get_sub_cosign_list (nm_usuario_p text) FROM PUBLIC;
