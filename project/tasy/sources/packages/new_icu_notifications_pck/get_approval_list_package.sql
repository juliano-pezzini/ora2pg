...
CREATE OR REPLACE FUNCTION new_icu_notifications_pck.get_approval_list (nm_usuario_p text) RETURNS SETOF T_ALERTS_TABLE AS $body$
DECLARE
...
BEGIN
	...
	if (coalesce(new_icu_notifications_pck.get_visualiza_alertas(nm_usuario_p, 'ANA'), 'X') <> 'S') then
		return;
	end if;
	...
	loop
		...
		qt_items_w := new_icu_notifications_pck.get_approval_items(nm_usuario_p, cd_pessoa_fisica_w, cd_perfil_w, c_encounter_w.nr_atendimento);
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION new_icu_notifications_pck.get_approval_list (nm_usuario_p text) FROM PUBLIC;
