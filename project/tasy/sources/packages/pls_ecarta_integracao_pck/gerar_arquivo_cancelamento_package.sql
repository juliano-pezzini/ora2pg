...
CREATE OR REPLACE PROCEDURE pls_ecarta_integracao_pck.gerar_arquivo_cancelamento ( pls_solicitacao_p pls_solicitacao_row, ds_caminho_p pls_ecarta_arquivo_solic.ds_caminho%type, nm_arq_zip_p pls_ecarta_arquivo_solic.nm_arquivo%type) AS $body$
DECLARE
...
BEGIN
	...
	PERFORM set_config('pls_ecarta_integracao_pck.nm_procedimento_w', 'gerar arquivo de cancelamento', false);
	...
	doc_elmt := xmlDOM.createElement(doc, 'CancelamentoLote');
	ar_node := xmlDOM.appendChild(corp_node, xmlDOM.makeNode(doc_elmt));

	doc_elmt := xmlDOM.createElement(doc, 'cdMatrizObjetoCliente');
	item_node := xmlDOM.appendChild(ar_node, xmlDOM.makeNode(doc_elmt));
	cdata_sct := xmlDOM.createCDATASection(doc, current_setting('pls_ecarta_integracao_pck.pls_matriz_t')::pls_matriz_r.cd_identificador);
	item_node := xmlDOM.appendChild(item_node, xmlDOM.makeNode(cdata_sct));

	doc_elmt := xmlDOM.createElement(doc, 'numeroLote');
	item_node := xmlDOM.appendChild(ar_node, xmlDOM.makeNode(doc_elmt));
	...
	set	dt_envio_cancelamento	= clock_timestamp(),
	...
	nm_arquivo_w := 'e-Carta_' || current_setting('pls_ecarta_integracao_pck.pls_matriz_t')::pls_matriz_r.cd_identificador || '_' || current_setting('pls_ecarta_integracao_pck.nr_seq_ecarta_lote_w')::pls_ecarta_lote.nr_sequencia%type || '_cancelamento.xml';
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_ecarta_integracao_pck.gerar_arquivo_cancelamento ( pls_solicitacao_p pls_solicitacao_row, ds_caminho_p pls_ecarta_arquivo_solic.ds_caminho%type, nm_arq_zip_p pls_ecarta_arquivo_solic.nm_arquivo%type) FROM PUBLIC;
