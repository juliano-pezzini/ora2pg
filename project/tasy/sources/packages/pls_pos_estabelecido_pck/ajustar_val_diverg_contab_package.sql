...
CREATE OR REPLACE PROCEDURE pls_pos_estabelecido_pck.ajustar_val_diverg_contab ( nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
nr_seq_contab_w		pls_conta_pos_estab_contab.nr_sequencia%type;	
...
C05 CURSOR(	nr_seq_conta_pos_pc pls_conta_pos_proc_contab.nr_seq_conta_pos_proc%type) FOR	
	SELECT	nr_sequencia,	
	...
	from	pls_conta_pos_proc_contab ctb
	where	ctb.nr_seq_conta_pos_proc	= nr_seq_conta_pos_pc
	and	ctb.vl_custo_operacional	> 0;
...
BEGIN
	...
	for r_c04_w in c04 loop
		...
		for r_c05_w in c05(r_c04_w.nr_seq_conta_pos) loop
			...
			if ( i > pls_Util_cta_pck.qt_registro_transacao_w) then
				SELECT * FROM pls_pos_estabelecido_pck.update_divergencias_val_contab(	tb_vl_tx_mat_upd, tb_vl_materiais_upd, tb_vl_tx_co_upd, tb_vl_co_upd, tb_vl_tx_servico_upd, tb_vl_servico_upd, tb_seq_item_w, nm_usuario_p) INTO STRICT _ora2pg_r;
		...
		if (coalesce( vl_lib_taxa_co_w,0) != coalesce(r_c04_w.vl_lib_taxa_co,0)) then
			...
			if ( vl_diferenca_w <> 0) then
				...
				if (vl_diferenca_w > 0) then
					...
					from	pls_conta_pos_proc_contab
				...
				tb_seq_tx_co_w(i_tx_co_w) := nr_seq_contab_w;				
				if ( i_tx_co_w > pls_util_cta_pck.qt_registro_transacao_w) then
				
					SELECT * FROM pls_pos_estabelecido_pck.upd_campo_reg_contab(tb_seq_tx_co_w, tb_vl_tx_co_upd, 'TX_CO') INTO STRICT _ora2pg_r;
 tb_seq_tx_co_w := _ora2pg_r.tb_seq_p; tb_vl_tx_co_upd := _ora2pg_r.tb_valor_p;
		...
		if (coalesce(vl_lib_taxa_material_w,0)	!= coalesce(r_c04_w.vl_lib_taxa_material,0)) then
			...
			if (vl_diferenca_w <> 0) then
				...
				tb_seq_tx_mat_w(i_tx_mat_w) := nr_seq_contab_w;				
				if ( i_tx_mat_w > pls_util_cta_pck.qt_registro_transacao_w) then
				
					SELECT * FROM pls_pos_estabelecido_pck.upd_campo_reg_contab(tb_seq_tx_mat_w, tb_vl_tx_mat_upd, 'TX_MAT') INTO STRICT _ora2pg_r;
 tb_seq_tx_mat_w := _ora2pg_r.tb_seq_p; tb_vl_tx_mat_upd := _ora2pg_r.tb_valor_p;
		...
		if (coalesce(vl_lib_taxa_servico_w,0)	!= coalesce(r_c04_w.vl_lib_taxa_servico,0)) then
			...
			if (vl_diferenca_w <> 0) then
				...
				tb_seq_tx_serv_w(i_tx_serv_w) := nr_seq_contab_w;				
				if ( i_tx_serv_w > pls_util_cta_pck.qt_registro_transacao_w) then
				
					SELECT * FROM pls_pos_estabelecido_pck.upd_campo_reg_contab(tb_seq_tx_serv_w, tb_vl_tx_servico_upd, 'TX_SERV') INTO STRICT _ora2pg_r;
 tb_seq_tx_serv_w := _ora2pg_r.tb_seq_p; tb_vl_tx_servico_upd := _ora2pg_r.tb_valor_p;
		...
		if (coalesce(vl_medico_w,0) != coalesce(r_c04_w.vl_medico,0)) then
			...
			if (vl_diferenca_w <> 0) then
				...
				tb_seq_vl_serv_w(i_vl_serv_w) := nr_seq_contab_w;				
				if ( i_vl_serv_w > pls_util_cta_pck.qt_registro_transacao_w) then
				
					SELECT * FROM pls_pos_estabelecido_pck.upd_campo_reg_contab(tb_seq_vl_serv_w, tb_vl_servico_upd, 'VL_SERV') INTO STRICT _ora2pg_r;
 tb_seq_vl_serv_w := _ora2pg_r.tb_seq_p; tb_vl_servico_upd := _ora2pg_r.tb_valor_p;
		...
		if (coalesce(vl_custo_operacional_w,0)	!= coalesce(r_c04_w.vl_custo_operacional,0)) then
			...
			if (vl_diferenca_w <> 0) then
				...
				tb_seq_vl_co_w(i_vl_co_w) := nr_seq_contab_w;				
				if ( i_vl_co_w > pls_util_cta_pck.qt_registro_transacao_w) then
				
					SELECT * FROM pls_pos_estabelecido_pck.upd_campo_reg_contab(tb_seq_vl_co_w, tb_vl_co_upd, 'VL_CO') INTO STRICT _ora2pg_r;
 tb_seq_vl_co_w := _ora2pg_r.tb_seq_p; tb_vl_co_upd := _ora2pg_r.tb_valor_p;
		...
		if (coalesce(vl_materiais_w,0)	!= coalesce(r_c04_w.vl_materiais,0)) then
			...
			if (vl_diferenca_w <> 0) then
				...
				tb_seq_vl_mat_w(i_vl_mat_w) := nr_seq_contab_w;				
				if ( i_vl_mat_w > pls_util_cta_pck.qt_registro_transacao_w) then
				
					SELECT * FROM pls_pos_estabelecido_pck.upd_campo_reg_contab(tb_seq_vl_mat_w, tb_vl_materiais_upd, 'VL_MAT') INTO STRICT _ora2pg_r;
 ...
 tb_seq_vl_mat_w := _ora2pg_r.tb_seq_p; tb_vl_materiais_upd := _ora2pg_r.tb_valor_p;
	...
	for r_c04_w in c04 loop
		...
		if ( i > pls_util_cta_pck.qt_registro_transacao_w) then
			...
			SELECT * FROM pls_pos_estabelecido_pck.upd_campo_reg_contab(tb_seq_prov_w, tb_vl_prov_upd, 'VL_PROV') INTO STRICT _ora2pg_r;
 tb_seq_prov_w := _ora2pg_r.tb_seq_p; tb_vl_prov_upd := _ora2pg_r.tb_valor_p;
		...
		if (qt_registro_w > 1) then
			select	sum(coalesce(vl_pos_estabelecido,0) + coalesce(vl_administracao,0))
			...
			from	pls_conta_pos_proc_contab
			where	nr_seq_conta_pos_proc	= r_c04_w.nr_seq_conta_pos
			and	coalesce(vl_pos_estabelecido,0) + coalesce(vl_administracao,0)	= 0;
			
			
			if ( qt_registro_w		> 0) and ( vl_custo_operacional_aux_w = vl_beneficiario_w ) and (vl_custo_operacional_aux_w	> 0) then
				
				delete	FROM pls_conta_pos_proc_contab	a
				where	a.nr_seq_conta_pos_proc	= r_c04_w.nr_seq_conta_pos
				and	coalesce(vl_pos_estabelecido,0) + coalesce(vl_administracao,0)	= 0
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_pos_estabelecido_pck.ajustar_val_diverg_contab ( nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
