...
--Geracao para faturamento, da taxa de administracao de p_s-estabelecido.



CREATE OR REPLACE PROCEDURE pls_pos_estabelecido_pck.gerar_taxa_faturamento_adm_pos ( tb_nr_seq_conta_pos_estab_p pls_util_cta_pck.t_number_table, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
tb_cta_fat_taxa_upd_proc_w	pls_util_cta_pck.t_number_table;
tb_vl_fat_taxa_upd_proc_w	pls_util_cta_pck.t_number_table;
...
tb_cta_fat_taxa_upd_mat_w	pls_util_cta_pck.t_number_table;
tb_vl_fat_taxa_upd_mat_w	pls_util_cta_pck.t_number_table;
tb_seq_resumo_upd_mat_w		pls_util_cta_pck.t_number_table;
nr_seq_conta_fat_taxa_w		pls_conta_pos_taxa_contab.nr_sequencia%type;
...
tb_proc_vl_ajuste_w		pls_util_cta_pck.t_number_table;
tb_proc_nr_seq_w		pls_util_cta_pck.t_number_table;
tb_mat_vl_ajuste_w		pls_util_cta_pck.t_number_table;
tb_mat_nr_seq_w			pls_util_cta_pck.t_number_table;
i_mat_ajuste_w			integer := 0;
i_proc_ajuste_w			integer := 0;
...
vl_fat_taxa_w			double precision;
...
BEGIN
	...
	--Apenas executar_ caso tiveram contas selecionadas

	if ( tb_nr_seq_conta_pos_estab_p.count > 0) then
		
		--Percorre todas as contas selecionadas para geracao dos valores para faturamento da taxa administrativa de p_s.
		...
		for j in tb_nr_seq_conta_pos_estab_p.first..tb_nr_seq_conta_pos_estab_p.last loop
			...
			for r_c01_w in C01(tb_nr_seq_conta_pos_estab_p(j)) loop
				...
				for r_c02_w in C02(r_c01_w.nr_seq_conta_proc,r_c01_w.nr_seq_conta_mat) loop
					...
					vl_fat_taxa_w	:= round((r_c02_w.vl_taxa_manutencao * pr_pagamento_liberado_w)::numeric,2);
					
					if (r_c02_w.ie_tipo_item = 'P') then
						nr_seq_conta_fat_taxa_w	:= null;
						
						select	max(nr_sequencia)
						into STRICT	nr_seq_conta_fat_taxa_w
						from	pls_conta_pos_proc_tx_fat
						...
						if ( coalesce(nr_seq_conta_fat_taxa_w::text, '') = '') and ( coalesce(r_c01_w.nr_seq_conta_resumo::text, '') = '') then
						...
						if ( coalesce(nr_seq_conta_fat_taxa_w::text, '') = '') then
							...
							tb_vl_taxa_ins_proc_w(i_ins_proc_w)		:= vl_fat_taxa_w;
							...
							if (i_ins_proc_w > pls_util_cta_pck.qt_registro_transacao_w) then
								...
								SELECT * FROM pls_pos_estabelecido_pck.ins_fat_tx_pos( tb_seq_resumo_ins_proc_w, tb_seq_pos_estab_tx_ins_proc_w, tb_seq_prest_pgto_ins_proc_w, tb_vl_taxa_ins_proc_w, tb_seq_conta_ins_proc_w, nm_usuario_p, 'P') INTO STRICT _ora2pg_r;
  tb_seq_resumo_ins_proc_w := _ora2pg_r.tb_seq_resumo_p; tb_seq_pos_estab_tx_ins_proc_w := _ora2pg_r.tb_seq_pos_estab_tx_p; tb_seq_prest_pgto_ins_proc_w := _ora2pg_r.tb_seq_prest_pgto_p; tb_vl_taxa_ins_proc_w := _ora2pg_r.tb_vl_taxa_p; tb_seq_conta_ins_proc_w := _ora2pg_r.tb_seq_conta_p;
							...
							tb_cta_fat_taxa_upd_proc_w(i_upd_proc_w)  	:= r_c01_w.nr_seq_conta_resumo;	
							tb_vl_fat_taxa_upd_proc_w(i_upd_proc_w)  	:= vl_fat_taxa_w;	
							tb_seq_resumo_upd_proc_w(i_upd_proc_w)		:= nr_seq_conta_fat_taxa_w;
							
							if (i_upd_proc_w > pls_util_cta_pck.qt_registro_transacao_w) then
											
								SELECT * FROM pls_pos_estabelecido_pck.upd_fat_tx_pos( tb_cta_fat_taxa_upd_proc_w, tb_vl_fat_taxa_upd_proc_w, tb_seq_resumo_upd_proc_w, nm_usuario_p, 'P') INTO STRICT _ora2pg_r;
  tb_cta_fat_taxa_upd_proc_w := _ora2pg_r.tb_cta_fat_taxa_p; tb_vl_fat_taxa_upd_proc_w := _ora2pg_r.tb_vl_fat_taxa_p; tb_seq_resumo_upd_proc_w := _ora2pg_r.tb_seq_resumo_p;
								i_upd_proc_w := 0;
						...
						if ( coalesce(nr_seq_conta_fat_taxa_w::text, '') = '') then
							...
							tb_vl_taxa_ins_mat_w(i_ins_mat_w)		:= vl_fat_taxa_w;
							...
							if (i_ins_mat_w > pls_util_cta_pck.qt_registro_transacao_w) then
								...
								SELECT * FROM pls_pos_estabelecido_pck.ins_fat_tx_pos( tb_seq_resumo_ins_mat_w, tb_seq_pos_estab_tx_ins_mat_w, tb_seq_prest_pgto_ins_mat_w, tb_vl_taxa_ins_mat_w, tb_seq_conta_ins_mat_w, nm_usuario_p, 'M') INTO STRICT _ora2pg_r;
  tb_seq_resumo_ins_mat_w := _ora2pg_r.tb_seq_resumo_p; tb_seq_pos_estab_tx_ins_mat_w := _ora2pg_r.tb_seq_pos_estab_tx_p; tb_seq_prest_pgto_ins_mat_w := _ora2pg_r.tb_seq_prest_pgto_p; tb_vl_taxa_ins_mat_w := _ora2pg_r.tb_vl_taxa_p; tb_seq_conta_ins_mat_w := _ora2pg_r.tb_seq_conta_p;
							...
							tb_cta_fat_taxa_upd_mat_w(i_upd_mat_w)  	:= r_c01_w.nr_seq_conta_resumo;	
							tb_vl_fat_taxa_upd_mat_w(i_upd_mat_w)  	:= vl_fat_taxa_w;	
							tb_seq_resumo_upd_mat_w(i_upd_mat_w)		:= nr_seq_conta_fat_taxa_w;
							
							if (i_upd_mat_w > pls_util_cta_pck.qt_registro_transacao_w) then
					
								SELECT * FROM pls_pos_estabelecido_pck.upd_fat_tx_pos( tb_cta_fat_taxa_upd_mat_w, tb_vl_fat_taxa_upd_mat_w, tb_seq_resumo_upd_mat_w, nm_usuario_p, 'M') INTO STRICT _ora2pg_r;
  tb_cta_fat_taxa_upd_mat_w := _ora2pg_r.tb_cta_fat_taxa_p; tb_vl_fat_taxa_upd_mat_w := _ora2pg_r.tb_vl_fat_taxa_p; tb_seq_resumo_upd_mat_w := _ora2pg_r.tb_seq_resumo_p;
								i_upd_mat_w := 0;
  ...
  tb_cta_fat_taxa_upd_mat_w := _ora2pg_r.tb_cta_fat_taxa_p; tb_vl_fat_taxa_upd_mat_w := _ora2pg_r.tb_vl_fat_taxa_p; tb_seq_resumo_upd_mat_w := _ora2pg_r.tb_seq_resumo_p;
		...
		--Percorre todas as contas selecionadas para geracao dos valores de faturamento. Nessa parte da rotina, ocorrer_o eventuais ajustes na distribuicao dos valores das taxas.
					...
					if (r_c02_w.ie_tipo_item = 'P') then
						...
						if ( coalesce(nr_seq_conta_fat_taxa_w::text, '') = '') then
							...
							if (i_ins_proc_w > pls_util_cta_pck.qt_registro_transacao_w) then
								...
								SELECT * FROM pls_pos_estabelecido_pck.ins_atualiza_vl_tx_fat( tb_seq_pos_estab_tx_ins_proc_w, tb_vl_taxa_ins_proc_w, tb_seq_conta_ins_proc_w, nm_usuario_p, 'P') INTO STRICT _ora2pg_r;
  tb_seq_pos_estab_tx_ins_proc_w := _ora2pg_r.tb_seq_pos_estab_tx_p; tb_vl_taxa_ins_proc_w := _ora2pg_r.tb_vl_taxa_p; tb_seq_conta_ins_proc_w := _ora2pg_r.tb_seq_conta_p;
							...
							tb_vl_fat_taxa_upd_proc_w(i_upd_proc_w)	:= vl_fat_taxa_w;
							tb_cta_fat_taxa_upd_proc_w(i_upd_proc_w)	:= nr_seq_conta_fat_taxa_w;
							
							if (i_upd_proc_w > pls_util_cta_pck.qt_registro_transacao_w) then
						
								SELECT * FROM pls_pos_estabelecido_pck.upd_atualiza_vl_tx_fat(	tb_vl_fat_taxa_upd_proc_w, tb_cta_fat_taxa_upd_proc_w, nm_usuario_p, 'P') INTO STRICT _ora2pg_r;
 	tb_vl_fat_taxa_upd_proc_w := _ora2pg_r.tb_vl_fat_taxa_p; tb_cta_fat_taxa_upd_proc_w := _ora2pg_r.tb_cta_fat_taxa_p;
								i_upd_proc_w := 0;
							else
								i_upd_proc_w := i_upd_proc_w + 1;
							end if;
						...
						if ( coalesce(nr_seq_conta_fat_taxa_w::text, '') = '') then
							...
							if (i_ins_mat_w > pls_util_cta_pck.qt_registro_transacao_w) then
								...
								SELECT * FROM pls_pos_estabelecido_pck.ins_atualiza_vl_tx_fat( tb_seq_pos_estab_tx_ins_mat_w, tb_vl_taxa_ins_mat_w, tb_seq_conta_ins_mat_w, nm_usuario_p, 'P') INTO STRICT _ora2pg_r;
  tb_seq_pos_estab_tx_ins_mat_w := _ora2pg_r.tb_seq_pos_estab_tx_p; tb_vl_taxa_ins_mat_w := _ora2pg_r.tb_vl_taxa_p; tb_seq_conta_ins_mat_w := _ora2pg_r.tb_seq_conta_p;
							...
							tb_vl_fat_taxa_upd_mat_w(i_upd_mat_w)	:= vl_fat_taxa_w;
							tb_cta_fat_taxa_upd_mat_w(i_upd_mat_w)	:= nr_seq_conta_fat_taxa_w;
							
							if (i_upd_mat_w > pls_util_cta_pck.qt_registro_transacao_w) then
					
								SELECT * FROM pls_pos_estabelecido_pck.upd_atualiza_vl_tx_fat(	tb_vl_fat_taxa_upd_mat_w, tb_cta_fat_taxa_upd_mat_w, nm_usuario_p, 'P') INTO STRICT _ora2pg_r;
 	tb_vl_fat_taxa_upd_mat_w := _ora2pg_r.tb_vl_fat_taxa_p; tb_cta_fat_taxa_upd_mat_w := _ora2pg_r.tb_cta_fat_taxa_p;
								i_upd_mat_w := 0;
							else
								i_upd_mat_w := i_upd_mat_w + 1;
							end if;
  ...
  tb_vl_fat_taxa_upd_proc_w := _ora2pg_r.tb_vl_fat_taxa_p; tb_cta_fat_taxa_upd_proc_w := _ora2pg_r.tb_cta_fat_taxa_p;
		...
		SELECT * FROM pls_pos_estabelecido_pck.ins_atualiza_vl_tx_fat( tb_seq_pos_estab_tx_ins_mat_w, tb_vl_taxa_ins_mat_w, tb_seq_conta_ins_mat_w, nm_usuario_p, 'M') INTO STRICT _ora2pg_r;
  tb_seq_pos_estab_tx_ins_mat_w := _ora2pg_r.tb_seq_pos_estab_tx_p; tb_vl_taxa_ins_mat_w := _ora2pg_r.tb_vl_taxa_p; tb_seq_conta_ins_mat_w := _ora2pg_r.tb_seq_conta_p;
						
		SELECT * FROM pls_pos_estabelecido_pck.upd_atualiza_vl_tx_fat( tb_vl_fat_taxa_upd_mat_w, tb_cta_fat_taxa_upd_mat_w, nm_usuario_p, 'M') INTO STRICT _ora2pg_r;
  tb_vl_fat_taxa_upd_mat_w := _ora2pg_r.tb_vl_fat_taxa_p; tb_cta_fat_taxa_upd_mat_w := _ora2pg_r.tb_cta_fat_taxa_p;

		for j in tb_nr_seq_conta_pos_estab_p.first..tb_nr_seq_conta_pos_estab_p.last loop
		
			--Ajuste de diverg_ncia de valores em materiais	
			...
			for r_c03_w in C03_mat(tb_nr_seq_conta_pos_estab_p(j)) loop
				...
				if ( coalesce(vl_taxa_manutencao_w,0)	!= coalesce(r_c03_w.vl_fat,0)) then
					
					vl_diferenca_w	:= coalesce(vl_taxa_manutencao_w,0) - coalesce(r_c03_w.vl_fat,0);
					...
					tb_mat_vl_ajuste_w(i_mat_ajuste_w) := vl_diferenca_w;
					tb_mat_nr_seq_w(i_mat_ajuste_w) := nr_seq_conta_fat_taxa_w;				
					if (i_mat_ajuste_w > pls_util_cta_pck.qt_registro_transacao_w) then
						SELECT * FROM pls_pos_estabelecido_pck.ajusta_diverg_fat_adm_tx_pos(tb_mat_nr_seq_w, tb_mat_vl_ajuste_w, 'M') INTO STRICT _ora2pg_r;
 tb_mat_nr_seq_w := _ora2pg_r.tb_seq_item_p; tb_mat_vl_ajuste_w := _ora2pg_r.tb_valor_ajuste_p;
						i_mat_ajuste_w := 0;
					else
						i_mat_ajuste_w := i_mat_ajuste_w + 1;
					end if;
			...
			for r_c03_proc_w in C03_proc(tb_nr_seq_conta_pos_estab_p(j)) loop
				...
				if ( coalesce(vl_taxa_manutencao_w,0)	!= coalesce(r_c03_proc_w.vl_fat,0)) then
					
					vl_diferenca_w	:= coalesce(vl_taxa_manutencao_w,0) - coalesce(r_c03_proc_w.vl_fat,0);
					...
					tb_proc_vl_ajuste_w(i_proc_ajuste_w) := vl_diferenca_w;
					tb_proc_nr_seq_w(i_proc_ajuste_w) := nr_seq_conta_fat_taxa_w;					
					if (i_proc_ajuste_w > pls_util_cta_pck.qt_registro_transacao_w) then
						SELECT * FROM pls_pos_estabelecido_pck.ajusta_diverg_fat_adm_tx_pos(tb_mat_nr_seq_w, tb_mat_vl_ajuste_w, 'M') INTO STRICT _ora2pg_r;
 tb_mat_nr_seq_w := _ora2pg_r.tb_seq_item_p; tb_mat_vl_ajuste_w := _ora2pg_r.tb_valor_ajuste_p;
						i_proc_ajuste_w := 0;
					else
						i_proc_ajuste_w := i_proc_ajuste_w + 1;
					end if;
				end if;
		...
		SELECT * FROM pls_pos_estabelecido_pck.ajusta_diverg_fat_adm_tx_pos(tb_proc_nr_seq_w, tb_proc_vl_ajuste_w, 'P') INTO STRICT _ora2pg_r;
 tb_proc_nr_seq_w := _ora2pg_r.tb_seq_item_p; tb_proc_vl_ajuste_w := _ora2pg_r.tb_valor_ajuste_p;
		SELECT * FROM pls_pos_estabelecido_pck.ajusta_diverg_fat_adm_tx_pos(tb_mat_nr_seq_w, tb_mat_vl_ajuste_w, 'M') INTO STRICT _ora2pg_r;
 tb_mat_nr_seq_w := _ora2pg_r.tb_seq_item_p; tb_mat_vl_ajuste_w := _ora2pg_r.tb_valor_ajuste_p;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE pls_pos_estabelecido_pck.gerar_taxa_faturamento_adm_pos ( tb_nr_seq_conta_pos_estab_p pls_util_cta_pck.t_number_table, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
