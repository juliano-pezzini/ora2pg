...
CREATE OR REPLACE PROCEDURE gerar_int_dankia_pck.dankia_gerar_movimentacao ( nr_atendimento_p bigint, cd_setor_atendimento_p bigint, cd_setor_atend_old_p bigint, cd_unidade_compl_p text, cd_unidade_compl_old_p text, cd_estabelecimento_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
BEGIN
	...
	if (get_ie_int_dankia = 'S') then
		...
		where 	cd_setor_atendimento = cd_setor_atend_old_p;
		...
		if (current_setting('gerar_int_dankia_pck.qt_existe_w')::bigint = 0) and
			((ie_dispensario_new_w = 'S') or (ie_dispensario_old_w = 'S')) then
		...
		if (ie_dispensario_old_w = 'S') then
			...
			values (	nextval('dankia_disp_atend_paciente_seq'),
					...
					current_setting('gerar_int_dankia_pck.cd_pessoa_fisica_w')::atendimento_paciente.cd_pessoa_fisica%type,
					nr_atendimento_p,
					cd_setor_atend_old_p,
					ie_tipo_atendimento_w,
					cd_unidade_compl_old_p,
					'S',
					current_setting('gerar_int_dankia_pck.dt_alta_w')::atendimento_paciente.dt_alta%type,
					'A',
					null,
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_int_dankia_pck.dankia_gerar_movimentacao ( nr_atendimento_p bigint, cd_setor_atendimento_p bigint, cd_setor_atend_old_p bigint, cd_unidade_compl_p text, cd_unidade_compl_old_p text, cd_estabelecimento_p bigint, nm_usuario_p text) FROM PUBLIC;
