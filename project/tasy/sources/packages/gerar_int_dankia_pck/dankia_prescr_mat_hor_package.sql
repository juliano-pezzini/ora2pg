...
CREATE OR REPLACE PROCEDURE gerar_int_dankia_pck.dankia_prescr_mat_hor ( nr_seq_mat_hor_p bigint, cd_material_p bigint, nr_seq_lote_p bigint, qt_dispensar_p bigint, ie_alteracao_p text, cd_local_estoque_p bigint, nm_usuario_p text) AS $body$
DECLARE
...
WHERE a.nr_prescricao = b.nr_prescricao and a.nr_seq_material = b.nr_sequencia and b.nr_prescricao = c.nr_prescricao   and a.nr_sequencia = nr_seq_mat_hor_p and exists (SELECT 	1
						from 	local_estoque l
						where 	l.cd_local_estoque = coalesce(coalesce(a.cd_local_estoque, b.cd_local_estoque),c.cd_local_estoque)
						and 	l.ie_tipo_local = '11');
		
		if (gerar_int_dankia_pck.get_ie_local_dankia(coalesce(cd_local_estoque_p, current_setting('gerar_int_dankia_pck.cd_local_estoque_w')::local_estoque.cd_local_estoque%type)) = 'S') and (nr_seq_prescricao_w > 0) then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE gerar_int_dankia_pck.dankia_prescr_mat_hor ( nr_seq_mat_hor_p bigint, cd_material_p bigint, nr_seq_lote_p bigint, qt_dispensar_p bigint, ie_alteracao_p text, cd_local_estoque_p bigint, nm_usuario_p text) FROM PUBLIC;
