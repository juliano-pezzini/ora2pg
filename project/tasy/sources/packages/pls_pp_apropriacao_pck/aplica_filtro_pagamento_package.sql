...
CREATE OR REPLACE PROCEDURE pls_pp_apropriacao_pck.aplica_filtro_pagamento ( ie_considera_selecao_p INOUT boolean, qt_filtro_processado_p INOUT integer, nr_id_transacao_p pls_pp_rp_aprop_selecao.nr_id_transacao%type, ie_excecao_p pls_pp_rp_cta_filtro.ie_excecao%type, nr_seq_filtro_p pls_pp_rp_cta_filtro.nr_sequencia%type, cd_condicao_pagamento_p pls_pp_lote.cd_condicao_pagamento%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
if (nr_seq_filtro_p IS NOT NULL AND nr_seq_filtro_p::text <> '') then
	...
	for r_c_filtro_w in c_filtro(nr_seq_filtro_p) loop
		...
		begin
			...
			SELECT * FROM pls_pp_apropriacao_pck.obter_restricao_pagamento(	r_c_filtro_w.nr_seq_evento, valor_bind_w) INTO STRICT _ora2pg_r;
 ds_campos_w := _ora2pg_r.ds_campos_p; ds_tabela_w := _ora2pg_r.ds_tabela_p; valor_bind_w := _ora2pg_r.valor_bind_p;
		...
		exception
			...
			CALL CALL CALL CALL CALL CALL pls_pp_apropriacao_pck.trata_erro_sql_dinamico(null, ds_select_w, null,
						nm_usuario_p, 'S');
		end;
