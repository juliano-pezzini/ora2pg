...
-- SELECIONAR_RECURSOS_GLOSAS



CREATE OR REPLACE PROCEDURE pls_ar_gerar_resultado_pck.selecionar_recursos_glosas ( nr_seq_lote_p pls_ar_lote.nr_sequencia%type, dt_inicio_p pls_ar_lote.dt_inicio%type, dt_fim_p pls_ar_lote.dt_fim%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) AS $body$
BEGIN

	begin
		insert into pls_ar_rec_glosa(nr_sequencia,
			nm_usuario,
			nm_usuario_nrec,
			dt_atualizacao,
			dt_atualizacao_nrec,
			...
			nr_seq_conta)
		SELECT	nextval('pls_ar_rec_glosa_seq'),
			nm_usuario_p,
			nm_usuario_p,
			clock_timestamp(),
			clock_timestamp(),
			...
			t.nr_seq_conta
		FROM (	select	sum(b.vl_total_recursado) vl_total_recursado,
				c.nr_seq_segurado,
				b.nr_sequencia nr_seq_rec_glosa,
				c.nr_seq_plano,
				a.dt_competencia_lote,
				b.nr_seq_conta,
				c.ie_tipo_segurado
			from	pls_rec_glosa_protocolo	a,
				pls_rec_glosa_conta	b,
				pls_conta		c
			where	a.dt_liberacao_pag between dt_inicio_p and dt_fim_p
