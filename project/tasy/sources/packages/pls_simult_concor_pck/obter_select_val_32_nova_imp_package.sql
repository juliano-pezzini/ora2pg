...
if (dados_regra_cab_p.qt_regra_proc > 0) then
	...
	bind_sql_p := pls_simult_concor_pck.obter_filtro_item(	'P', 'proc', dados_regra_ocor_p, dados_regra_cab_p, cd_guia_referencia_p, dt_item_p, nr_seq_conta_p, nr_seq_prestador_exec_princ_p, bind_sql_p, 'N');
						
	-- obtem campos adicionais que sao especificos para procedimento
	bind_sql_p := pls_simult_concor_pck.obter_campo_adic_proc_nova_imp(	dados_regra_ocor_p, dados_regra_cab_p, cd_guia_referencia_p, dt_item_p, nr_seq_conta_p, nr_seq_prestador_exec_princ_p, bind_sql_p);
	...
	PERFORM set_config('pls_simult_concor_pck.ds_sql_w', current_setting('pls_simult_concor_pck.ds_sql_w')::varchar(4000) || pls_util_pck.enter_w ||
			...
			'	(select count(distinct proc.ie_origem_proced||proc.cd_procedimento)' || pls_util_pck.enter_w ||
			'	 from   pls_conta_proc_ocor_imp_v proc' || pls_util_pck.enter_w ||
			...
			'	 		and    	x.cd_procedimento 	= proc.cd_procedimento)) qt_casou_proc_and, ' || pls_util_pck.enter_w ||
			'	(select count(distinct proc.ie_origem_proced||proc.cd_procedimento)' || pls_util_pck.enter_w ||
			'	 from   pls_conta_proc_ocor_imp_v proc' || pls_util_pck.enter_w ||
			'	 where  proc.nr_seq_segurado = :nr_seq_segurado_pc' || pls_util_pck.enter_w ||
			ds_filtro_proc_w ||
			...
			'	 		and    	x.ie_origem_proced 	= proc.ie_origem_proced' || pls_util_pck.enter_w ||
			'	 		and    	x.cd_procedimento 	= proc.cd_procedimento)) qt_casou_proc_ou, ' || pls_util_pck.enter_w ||
			'	(select count(distinct proc.ie_origem_proced||proc.cd_procedimento)' || pls_util_pck.enter_w ||
			'	 from   pls_conta_proc_ocor_imp_v proc' || pls_util_pck.enter_w ||
			'	 where  proc.nr_seq_segurado = :nr_seq_segurado_pc' || pls_util_pck.enter_w ||
			...
			'	 and	x.ie_and_or		= ''AND'' ' || pls_util_pck.enter_w ||
			'	 and    x.ie_origem_proced 	= proc.ie_origem_proced' || pls_util_pck.enter_w ||
			'	 and    x.cd_procedimento 	= proc.cd_procedimento) ie_agrup_item ' || pls_util_pck.enter_w ||
...
if (dados_regra_cab_p.qt_regra_mat > 0) then
	...
	bind_sql_p := pls_simult_concor_pck.obter_filtro_item(	'M', 'mat', dados_regra_ocor_p, dados_regra_cab_p, cd_guia_referencia_p, dt_item_p, nr_seq_conta_p, nr_seq_prestador_exec_princ_p, bind_sql_p, 'N');
						
	-- obtem campos adicionais que sao especificos para material
	bind_sql_p := pls_simult_concor_pck.obter_campo_adic_mat_nova_imp(	dados_regra_ocor_p, dados_regra_cab_p, cd_guia_referencia_p, dt_item_p, nr_seq_conta_p, nr_seq_prestador_exec_princ_p, bind_sql_p);
	
	PERFORM set_config('pls_simult_concor_pck.ds_sql_w', current_setting('pls_simult_concor_pck.ds_sql_w')::varchar(4000) || pls_util_pck.enter_w ||
			'select	''M'' ie_tipo_registro,' || pls_util_pck.enter_w ||
			'	(select count(distinct mat.nr_seq_material)' || pls_util_pck.enter_w ||
			'	 from   pls_conta_mat_ocor_imp_v mat' || pls_util_pck.enter_w ||
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION pls_simult_concor_pck.obter_select_val_32_nova_imp ( dados_regra_cab_p pls_simult_concor_pck.t_dado_cab_regra, dados_regra_ocor_p pls_tipos_ocor_pck.dados_regra, nr_seq_segurado_p pls_conta.nr_seq_segurado%type, cd_guia_referencia_p pls_conta.cd_guia_referencia%type, dt_item_p pls_oc_cta_selecao_ocor_v.dt_item%type, nr_seq_conta_p pls_oc_cta_selecao_ocor_v.nr_seq_conta%type, nr_seq_prestador_exec_princ_p pls_conta.nr_seq_prestador_exec_princ%type, nr_id_transacao_p pls_oc_cta_selecao_ocor_v.nr_id_transacao%type, bind_sql_p INOUT sql_pck.t_dado_bind) FROM PUBLIC;
