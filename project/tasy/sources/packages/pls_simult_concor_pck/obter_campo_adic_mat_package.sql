...
CREATE OR REPLACE FUNCTION pls_simult_concor_pck.obter_campo_adic_mat ( dados_regra_ocor_p pls_tipos_ocor_pck.dados_regra, dados_regra_cab_p pls_simult_concor_pck.t_dado_cab_regra, cd_guia_referencia_p pls_conta.cd_guia_referencia%type, dt_item_p pls_oc_cta_selecao_ocor_v.dt_item%type, nr_seq_conta_p pls_oc_cta_selecao_ocor_v.nr_seq_conta%type, nr_seq_prestador_exec_princ_p pls_conta.nr_seq_prestador_exec_princ%type, bind_sql_p INOUT sql_pck.t_dado_bind) AS $body$
DECLARE
...
bind_sql_p := pls_simult_concor_pck.obter_filtro_item(	'M', 'matadic', dados_regra_ocor_p, dados_regra_cab_p, cd_guia_referencia_p, dt_item_p, nr_seq_conta_p, nr_seq_prestador_exec_princ_p, bind_sql_p, 'A');
...
if (dados_regra_cab_p.ie_tipo_verificacao = '1' and dados_regra_cab_p.qt_dias_considerar > 0) then
	...
	ds_campos_adic_w := ds_campos_adic_w || ',' || pls_util_pck.enter_w ||
		...
		'	 from   pls_conta_mat_ocor_v matadic' || pls_util_pck.enter_w ||
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION pls_simult_concor_pck.obter_campo_adic_mat ( dados_regra_ocor_p pls_tipos_ocor_pck.dados_regra, dados_regra_cab_p pls_simult_concor_pck.t_dado_cab_regra, cd_guia_referencia_p pls_conta.cd_guia_referencia%type, dt_item_p pls_oc_cta_selecao_ocor_v.dt_item%type, nr_seq_conta_p pls_oc_cta_selecao_ocor_v.nr_seq_conta%type, nr_seq_prestador_exec_princ_p pls_conta.nr_seq_prestador_exec_princ%type, bind_sql_p INOUT sql_pck.t_dado_bind) FROM PUBLIC;
