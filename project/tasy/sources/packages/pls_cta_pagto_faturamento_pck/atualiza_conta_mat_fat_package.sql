...
--Faturamento, atualizar dados conta_mat
CREATE OR REPLACE PROCEDURE pls_cta_pagto_faturamento_pck.atualiza_conta_mat_fat ( nr_seq_lote_p pls_lote_protocolo_conta.nr_sequencia%type, nr_seq_protocolo_p pls_protocolo_conta.nr_sequencia%type, nr_seq_lote_processo_p pls_cta_lote_processo.nr_sequencia%type, nr_seq_conta_p pls_conta.nr_sequencia%type, nr_seq_conta_mat_p pls_conta_proc.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, ie_excluir_fat_p text) AS $body$
DECLARE
...
for	r_C01_w in C01(	nr_seq_lote_p, nr_seq_protocolo_p, nr_seq_lote_processo_p,
			nr_seq_conta_p, nr_seq_conta_mat_p) loop

	
	-- Verificar se existe valor de pós para o item 
	qt_pos_estabelecido_w := pls_cta_pagto_faturamento_pck.obter_qt_pos_estab(r_C01_w.nr_sequencia, 'M');
	...
	if (qt_pos_estabelecido_w	> 0) then
		...
		if	((ie_calcula_preco_benef_w <> 'S') and ( dados_parametros_w.ie_calculo_pos_estab in ('C','P','O'))) then
			...
			dados_pos_estab_w.qt_item(index_dados_pos_estab_w) 	:= r_C01_w.qt_material;
			...
			--Se o valor de pós-estabelecido do beneficiário for maior que zero.
			if (vl_tot_beneficiario_w	> 0) then
				
				--Não é necessário fazer update para todos os itens, basta que para cada conta seja feito,
				--com esse exists, podemos controlar para que a estrutura de registros que posteriormente será
				--enviada para o forall, não ocntenha duplicatas do nr_seq_conta.
			...
			--Se quantidade de registros nas variáveis tabela chegar ao limite determinado, então manda atualizar no banco
...
CALL pls_cta_pagto_faturamento_pck.zera_valores_coparticipacao(tb_seq_mat_w, tb_seq_contas_w, 'M');
CALL pls_cta_pagto_faturamento_pck.atualiza_dados_pos_estab(dados_pos_estab_w, tb_seq_contas_pos_w, tb_total_benef_w, 'M');

--Abre e percorre duas vezes o cursor, para que  na primeira execução, seja executado tudo o que pode ser processado em lote
--e o que não pode ser executado em lote ou venha a depender de valores gerados e/ou obtidos na primeira iteração pelos itens do cursor,
--são executados nessa segunda iteração
for	r_C01_w in C01(	nr_seq_lote_p, nr_seq_protocolo_p, nr_seq_lote_processo_p,
			nr_seq_conta_p, nr_seq_conta_mat_p) loop
...
if (r_C01_w.ie_glosa <> 'S') then
	...
	--Se existe registro de pós-estabelecido para o item
	if (qt_pos_estabelecido_w	> 0) then
		
		if (dados_parametros_w.ie_geracao_pos_estabelecido = 'C' ) then
			CALL pls_gerar_valor_pos_estab(r_C01_w.nr_seq_conta, nm_usuario_p, 'C', null, r_C01_w.nr_sequencia, 'A');
		end if;
	else	
		if (coalesce(ie_excluir_fat_p,'S') = 'S') then
		
			delete	FROM pls_conta_mat_aprop
			where	nr_seq_conta_mat = r_C01_w.nr_sequencia;
...
LANGUAGE PLPGSQL
;
-- REVOKE ALL ON PROCEDURE pls_cta_pagto_faturamento_pck.atualiza_conta_mat_fat ( nr_seq_lote_p pls_lote_protocolo_conta.nr_sequencia%type, nr_seq_protocolo_p pls_protocolo_conta.nr_sequencia%type, nr_seq_lote_processo_p pls_cta_lote_processo.nr_sequencia%type, nr_seq_conta_p pls_conta.nr_sequencia%type, nr_seq_conta_mat_p pls_conta_proc.nr_sequencia%type, nm_usuario_p usuario.nm_usuario%type, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type, ie_excluir_fat_p text) FROM PUBLIC;
