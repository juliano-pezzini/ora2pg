...
CREATE OR REPLACE FUNCTION wheb_ad_pck.obter_informacoes_ad_tasy (campo_retorno_p text, restricao_p text, filtro_customizado_p text, cd_estabelecimento_p bigint) RETURNS varchar AS $body$
DECLARE
...
BEGIN
	...
	return obter_informacoes_ad(ds_base_name_w, ds_server_ip_w, ds_dominio_w, ds_adm_user_w, ds_adm_pass_w, campo_retorno_p, restricao_p, filtro_customizado_p);
	end;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION wheb_ad_pck.obter_informacoes_ad_tasy (campo_retorno_p text, restricao_p text, filtro_customizado_p text, cd_estabelecimento_p bigint) FROM PUBLIC;
