...
CREATE OR REPLACE PROCEDURE ptu_a950_pck.processa_guias_hm ( nr_seq_arq_xml_p pls_imp_arq_A950.nr_sequencia%type, ds_guias_hm_p pls_imp_arq_A950.ds_xml%type, nm_usuario_p usuario.nm_usuario%type) AS $body$
DECLARE
...
if (nr_total_guias_hm_w > 0) then
	...
	while(nr_guia_atual_w <= nr_total_guias_hm_w) loop
		...
		if (i >= pls_util_pck.qt_registro_transacao_w) then
			...
			SELECT * FROM ptu_a950_pck.insere_guia_hm(	nr_seq_arq_xml_p, nm_usuario_p, tb_qt_filme_amb_w, tb_nr_auxiliares_amb_w, tb_nr_porte_anest_amb_w, tb_ie_inc_rol_w, tb_cd_amb_w, tb_qt_ut_amb_w, tb_ds_porte_rol_w, tb_ie_grupos_planilha_w, tb_ie_classificacao_w, tb_qt_uco_w, tb_cd_procedimento_uni_w, tb_nr_porte_anest_w, tb_nr_auxiliar_w, tb_qt_filme_w, tb_ie_quantidade_w, tb_ds_doc_racio_w, tb_nr_prazo_exec_w, tb_nr_prazo_origem_w, tb_nr_prazo_total_w, tb_ds_item_rol_w, tb_ds_amb_rol_w) INTO STRICT _ora2pg_r;
 tb_qt_filme_amb_w := _ora2pg_r.qt_filme_amb_p; tb_nr_auxiliares_amb_w := _ora2pg_r.nr_auxiliares_amb_p; tb_nr_porte_anest_amb_w := _ora2pg_r.nr_porte_anest_amb_p; tb_ie_inc_rol_w := _ora2pg_r.ie_inc_rol_p; tb_cd_amb_w := _ora2pg_r.cd_amb_p; tb_qt_ut_amb_w := _ora2pg_r.qt_ut_amb_p; tb_ds_porte_rol_w := _ora2pg_r.ds_porte_rol_p; tb_ie_grupos_planilha_w := _ora2pg_r.ie_grupos_planilha_p; tb_ie_classificacao_w := _ora2pg_r.ie_classificacao_p; tb_qt_uco_w := _ora2pg_r.qt_uco_p; tb_cd_procedimento_uni_w := _ora2pg_r.cd_procedimento_uni_p; tb_nr_porte_anest_w := _ora2pg_r.nr_porte_anest_p; tb_nr_auxiliar_w := _ora2pg_r.nr_auxiliar_p; tb_qt_filme_w := _ora2pg_r.qt_filme_p; tb_ie_quantidade_w := _ora2pg_r.ie_quantidade_p; tb_ds_doc_racio_w := _ora2pg_r.ds_doc_racio_p; tb_nr_prazo_exec_w := _ora2pg_r.nr_prazo_exec_p; tb_nr_prazo_origem_w := _ora2pg_r.nr_prazo_origem_p; tb_nr_prazo_total_w := _ora2pg_r.nr_prazo_total_p; tb_ds_item_rol_w := _ora2pg_r.ds_item_rol_p; tb_ds_amb_rol_w := _ora2pg_r.ds_amb_rol_p;
			i := 0;
		else
			i := i + 1;
		end if;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE ptu_a950_pck.processa_guias_hm ( nr_seq_arq_xml_p pls_imp_arq_A950.nr_sequencia%type, ds_guias_hm_p pls_imp_arq_A950.ds_xml%type, nm_usuario_p usuario.nm_usuario%type) FROM PUBLIC;
