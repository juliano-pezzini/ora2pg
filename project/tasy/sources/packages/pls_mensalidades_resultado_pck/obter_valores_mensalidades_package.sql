...
CREATE OR REPLACE FUNCTION pls_mensalidades_resultado_pck.obter_valores_mensalidades ( ie_dimensao_um_p text, ie_dimensao_dois_p text, ds_vl_dimensao_um_p text, ds_vl_dimensao_dois_p text, dt_inicial_p text, dt_final_p text, ds_filtros_p text, ie_tipo_item_p text, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) RETURNS SETOF T_VALORES_MENSALIDADES AS $body$
DECLARE
...
t_valores_mensalidades_w	t_valores_mensalidades;
valor_bind_w			sql_pck.t_dado_bind;
cursor_w			sql_pck.t_cursor;


retorno_valores_mensalidades_w REFCURSOR;
...
if (upper(ie_dimensao_um_p) = 'PAG') or (upper(ie_dimensao_dois_p) = 'PAG') then
	if (upper(ie_dimensao_um_p) = 'PAG') then
...
if (ie_tipo_item_p IS NOT NULL AND ie_tipo_item_p::text <> '') then
	sql_restricao_w := sql_restricao_w || ' and	b.ie_tipo_item = ' || ie_tipo_item_p || pls_util_pck.enter_w;
end if;
...
sql_base_w := 	'	select	b.ie_tipo_item,						' 	|| pls_util_pck.enter_w ||
		'		a.vl_item,						'	|| pls_util_pck.enter_w ||
		'		d.nr_seq_lote,						'	|| pls_util_pck.enter_w ||
		'		d.nr_sequencia nr_seq_mensalidade,			'	|| pls_util_pck.enter_w ||
		'		c.nr_sequencia nr_seq_mensalidade_seg			'	|| pls_util_pck.enter_w ||
		'	from	pls_ar_mensalidade 		a,			'	|| pls_util_pck.enter_w ||
		'		pls_mensalidade_seg_item 	b,			'	|| pls_util_pck.enter_w ||
		'		pls_mensalidade_segurado	c,			'	|| pls_util_pck.enter_w ||
		'		pls_mensalidade			d,			'	|| pls_util_pck.enter_w ||
		'		pls_ar_lote			lote,			'	|| pls_util_pck.enter_w ||
		'		pls_segurado			s			'	|| pls_util_pck.enter_w ||
		'	where	lote.nr_sequencia	= a.nr_seq_lote			'	|| pls_util_pck.enter_w ||
		'	and	c.nr_sequencia		= b.nr_seq_mensalidade_seg 	'	|| pls_util_pck.enter_w ||
		'	and	d.nr_sequencia		= c.nr_seq_mensalidade		'	|| pls_util_pck.enter_w ||
		'	and	s.nr_sequencia		= a.nr_seq_segurado		'	|| pls_util_pck.enter_w ||
		'	and	b.nr_sequencia		= a.nr_seq_item_mens	 	'	|| pls_util_pck.enter_w || sql_restricao_w;

valor_bind_w := sql_pck.executa_sql_cursor(sql_base_w, valor_bind_w);

loop
	fetch cursor_w bulk collect into t_valores_mensalidades_w
	limit current_setting('pls_mensalidades_resultado_pck.qt_registro_transacao_w')::integer;
	exit when t_valores_mensalidades_w.count = 0;

	for i in t_valores_mensalidades_w.first..t_valores_mensalidades_w.last loop
		RETURN NEXT t_valores_mensalidades_w(i);
	end loop;
end loop;
close cursor_w;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION pls_mensalidades_resultado_pck.obter_valores_mensalidades ( ie_dimensao_um_p text, ie_dimensao_dois_p text, ds_vl_dimensao_um_p text, ds_vl_dimensao_dois_p text, dt_inicial_p text, dt_final_p text, ds_filtros_p text, ie_tipo_item_p text, cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) FROM PUBLIC;
