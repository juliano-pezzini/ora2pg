...
SET client_encoding TO 'UTF8';
    ...
    /*
        Get All registers to fill the WCPanel
    */
CREATE OR REPLACE FUNCTION clinical_pathway_preview_pck.get_patient_preview_plan (nr_seq_protocolo_int_pac_p protocolo_int_pac_etapa.nr_seq_protocolo_int_pac%TYPE, ie_nome_amigavel_p text DEFAULT 'N') RETURNS SETOF PLAN_TABLE_T AS $body$
DECLARE
        ...
        plan_c CURSOR FOR
            SELECT c.nr_sequencia NR_SEQ_EVENTO,
                ...
                (SELECT CASE WHEN Count(1)=0 THEN 'N'  ELSE 'S' END
                 ...
                 REGEXP_REPLACE(c.ds_evento_plano, '[^0-9A-Za-z]', '') || '_' || c.nr_dia_protocolo || '_' || c.nr_seq_ordem || '_' || coalesce(c.ie_ocultar_visao_paciente, 'N') COLUMN_ID,
                 CASE WHEN coalesce(c.dt_real::text, '') = '' THEN  'N'  ELSE 'S' END  IE_FINISHED
              FROM protocolo_int_pac_etapa    b
              INNER JOIN  protocolo_int_pac_evento   c ON
                 b.nr_sequencia = c.nr_seq_prt_int_pac_etapa
            ...
            ORDER BY IE_TIPO_EVENTO, NR_SEQ_ACAO;
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION clinical_pathway_preview_pck.get_patient_preview_plan (nr_seq_protocolo_int_pac_p protocolo_int_pac_etapa.nr_seq_protocolo_int_pac%TYPE, ie_nome_amigavel_p text DEFAULT 'N') FROM PUBLIC;
