...
SET client_encoding TO 'UTF8';
    ...
    /*
        Get all WCPanel's columns
    */
CREATE OR REPLACE FUNCTION clinical_pathway_preview_pck.get_patient_preview_header (nr_seq_protocolo_int_pac_p protocolo_int_pac_etapa.nr_seq_protocolo_int_pac%TYPE, ie_nome_amigavel_p text DEFAULT 'N') RETURNS SETOF HEADER_TABLE_T AS $body$
DECLARE
        ...
        header_data_c CURSOR FOR
            SELECT
                ...
                IE_EVENTO_SUMARIO,
                DT_PREVISTA,
                clinical_pathway_preview_pck.get_preview_patient_outcome(SEQ_ETAPA, 'N', ie_nome_amigavel_p) DS_OUTCOMES,
                CD_DPC,
                ENCOUNTER_TYPE,
                NM_PESSOA_FISICA
            FROM (
              SELECT
                ...
                REGEXP_REPLACE(a.ds_evento_plano, '[^0-9A-Za-z]', '') || '_' || a.nr_dia_protocolo || '_' || a.nr_seq_ordem || '_' || coalesce(a.ie_ocultar_visao_paciente, 'N') COLUMN_ID,
                (SELECT MIN(x.IE_TIPO_ATENDIMENTO) FROM protocolo_int_pac_evento x WHERE a.NR_SEQ_PRT_INT_PAC_ETAPA = x.NR_SEQ_PRT_INT_PAC_ETAPA AND a.NR_DIA_PROTOCOLO = x.NR_DIA_PROTOCOLO) ENCOUNTER_TYPE,
                clinical_pathway_preview_pck.get_resp_user_order_daily(nr_seq_protocolo_int_pac_p, a.dt_prevista) NM_PESSOA_FISICA
            ...
            GROUP BY
                ...
                IE_EVENTO_SUMARIO,
                DT_PREVISTA,
                clinical_pathway_preview_pck.get_preview_patient_outcome(SEQ_ETAPA, 'N', ie_nome_amigavel_p),
                CD_DPC,
                clinical_pathway_preview_pck.get_header_events_id(SEQ_ETAPA, NR_DIA_EVENTO, NR_ORDEM),
                NR_ORDEM,
                ENCOUNTER_TYPE,
                NM_PESSOA_FISICA
...
BEGIN
        ...
        FOR header_data_r IN header_data_c LOOP
            ...
            header_row_w.NM_PESSOA_FISICA := header_data_r.NM_PESSOA_FISICA;
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION clinical_pathway_preview_pck.get_patient_preview_header (nr_seq_protocolo_int_pac_p protocolo_int_pac_etapa.nr_seq_protocolo_int_pac%TYPE, ie_nome_amigavel_p text DEFAULT 'N') FROM PUBLIC;
