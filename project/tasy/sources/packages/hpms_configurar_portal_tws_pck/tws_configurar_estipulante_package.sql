...
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
CREATE OR REPLACE PROCEDURE hpms_configurar_portal_tws_pck.tws_configurar_estipulante ( cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) AS $body$
DECLARE
...
if ( (cd_estabelecimento_p IS NOT NULL AND cd_estabelecimento_p::text <> '') and cd_estabelecimento_p > 0 ) then
	...
	begin
		ds_mensagem_w := 'IMPORT_USERS_TO_PSA_TWS_PACK';
		CALL import_users_to_psa_tws_pack.import_stipulators();
	exception
	when others then
		CALL wheb_mensagem_pck.exibir_mensagem_abort(1112449,'DS_ROTINA='||ds_mensagem_w);
	end;
	...
	begin
		...
		CALL hpms_configurar_portal_tws_pck.atualizar_mensagens_estip();
	exception
	when others then
		CALL wheb_mensagem_pck.exibir_mensagem_abort( 1112449,'DS_ROTINA='||ds_mensagem_w);
	end;	
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE hpms_configurar_portal_tws_pck.tws_configurar_estipulante ( cd_estabelecimento_p estabelecimento.cd_estabelecimento%type) FROM PUBLIC;
