...
CREATE OR REPLACE FUNCTION pfcs_pck_discharge_manager.get_care_status ( cd_establishment_p bigint, cd_unit_p bigint, cd_unit_classification_p text, cd_category_p text, ie_over_threshold_p text) RETURNS bigint AS $body$
DECLARE
...
BEGIN
        ...
        where uni.nr_seq_location = coalesce(enc.nr_seq_location, pfcs_get_pat_location(pat.nr_sequencia, enc.nr_sequencia))
            ...
            and (   (sec.cd_classif_setor = cd_unit_classification_w) or (coalesce(cd_unit_classification_w::text, '') = '' and sec.ie_ocup_hospitalar <> PFCS_PCK_CONSTANTS.IE_NO)   )
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION pfcs_pck_discharge_manager.get_care_status ( cd_establishment_p bigint, cd_unit_p bigint, cd_unit_classification_p text, cd_category_p text, ie_over_threshold_p text) FROM PUBLIC;
