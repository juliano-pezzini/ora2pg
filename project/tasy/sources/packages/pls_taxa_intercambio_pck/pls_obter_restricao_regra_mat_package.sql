...
--retorna as restricoes para materiais



CREATE OR REPLACE FUNCTION pls_taxa_intercambio_pck.pls_obter_restricao_regra_mat ( ie_opcao_p text, cursor_p integer, nr_seq_item_p pls_conta_proc.nr_sequencia%type, dados_taxa_intercambio_p dados_taxa_intercambio) RETURNS DADOS_FILTRO_INTERCAMBIO AS $body$
DECLARE
...
BEGIN
	...
	if (ie_opcao_p = 'RESTRICAO') then
		--Se for informado o material, entao restringe por ele.

		if (nr_seq_item_p IS NOT NULL AND nr_seq_item_p::text <> '') then
			ds_restr_w.ds_filtro	:= ds_restr_w.ds_filtro||' and	mat.nr_sequencia = :nr_seq_mat'|| pls_util_pck.enter_w;
		--
			...
			ds_restr_w.ds_filtro	:= ds_restr_w.ds_filtro||	'and	exists(select	1 '||pls_util_pck.enter_w||
								...
								'	where	aux.nr_seq_conta = mat.nr_seq_conta'||pls_util_pck.enter_w||
								'	and	aux.ie_tipo_regra = :ie_tipo_regra_pc)'||pls_util_pck.enter_w;
		end if;			
	else	
		if (nr_seq_item_p IS NOT NULL AND nr_seq_item_p::text <> '') then
			dbms_sql.bind_variable(cursor_p, ':nr_seq_mat', nr_seq_item_p);
		else
			dbms_sql.bind_variable(cursor_p, ':ie_tipo_regra_pc', dados_taxa_intercambio_p.ie_tipo_regra);
		end if;
	end if;
	
	--Se for informado um congenere superior, entao faz a restricao pelas contas
	...
	--Se for informado um grupo decongenere , entao faz a restricao
	...
	--Se tiver informado um plano, entao faz a restricao
	...
	--Considerar produtos de intercambio de beneficio por obito
	...
	--Restringir quando congenere for informado
	...
	--Restringir pelo grupo de cooperativa vinculado ao beneficiario
	...
	--Restringir por grupo de receitas
...
SECURITY DEFINER
;
-- REVOKE ALL ON FUNCTION pls_taxa_intercambio_pck.pls_obter_restricao_regra_mat ( ie_opcao_p text, cursor_p integer, nr_seq_item_p pls_conta_proc.nr_sequencia%type, dados_taxa_intercambio_p dados_taxa_intercambio) FROM PUBLIC;
