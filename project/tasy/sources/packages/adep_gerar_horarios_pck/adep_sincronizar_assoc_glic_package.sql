...
CREATE OR REPLACE PROCEDURE adep_gerar_horarios_pck.adep_sincronizar_assoc_glic ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_exibir_hor_realizados_p text, ie_exibir_hor_suspensos_p text, ie_regra_inclusao_p text, ie_data_lib_prescr_p text, ie_exibir_suspensos_p text, ie_agrupar_acm_sn_p text, ie_agrupar_dose_esp_p text, ie_obs_diferenciar_p text, dt_horario_p timestamp, nr_horario_p integer, ie_prescr_setor_p text, cd_setor_paciente_p bigint) AS $body$
DECLARE
	...
	and	((c.dt_horario between dt_horario_inicio_sinc_w and dt_horario_fim_sinc_w) or (c.dt_fim_horario IS NOT NULL AND c.dt_fim_horario::text <> '') or (coalesce(c.ie_horario_especial,'N') = 'S'))
...
BEGIN
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		ds_comando_update_w	:=	' update w_adep_t ' ||
						...
						' and nvl(nr_seq_prot_glic, 0) = nvl(:nr_seq_prot_glic, 0) ' ||
		...
		CALL exec_sql_dinamico_bv('ADEP', ds_comando_update_w,	'nr_prescricao=' || to_char(current_setting('adep_gerar_horarios_pck.nr_prescricao_w')::prescr_medica.nr_prescricao%type) || ds_sep_bv_w ||
									...
									'nr_seq_prot_glic=' || to_char(nr_seq_prot_glic_w) || ds_sep_bv_w ||
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE adep_gerar_horarios_pck.adep_sincronizar_assoc_glic ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_exibir_hor_realizados_p text, ie_exibir_hor_suspensos_p text, ie_regra_inclusao_p text, ie_data_lib_prescr_p text, ie_exibir_suspensos_p text, ie_agrupar_acm_sn_p text, ie_agrupar_dose_esp_p text, ie_obs_diferenciar_p text, dt_horario_p timestamp, nr_horario_p integer, ie_prescr_setor_p text, cd_setor_paciente_p bigint) FROM PUBLIC;
