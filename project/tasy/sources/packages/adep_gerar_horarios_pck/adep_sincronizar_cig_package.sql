...
CREATE OR REPLACE PROCEDURE adep_gerar_horarios_pck.adep_sincronizar_cig ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_exibir_hor_realizados_p text, ie_exibir_hor_suspensos_p text, ie_regra_inclusao_p text, ie_data_lib_prescr_p text, ie_exibir_suspensos_p text, ie_agrupar_acm_sn_p text, ie_data_lib_proced_p text, ie_so_proc_setor_usuario_p text, dt_horario_p timestamp, nr_horario_p integer, ie_prescr_setor_p text, cd_setor_paciente_p bigint) AS $body$
DECLARE
	...
	SELECT	a.nr_prescricao,
			...
			substr(obter_mat_med_assoc_proc(a.nr_prescricao,c.nr_seq_procedimento,'S') || adep_gerar_horarios_pck.obter_ds_justificativa(obter_justificativa_item(x.nr_prescricao, x.nr_sequencia, 'P')),1,2000) ds_mat_med_assoc,
	...
	group by
		...
		x.cd_intervalo,
		x.qt_procedimento,
		substr(obter_mat_med_assoc_proc(a.nr_prescricao,c.nr_seq_procedimento,'S') || adep_gerar_horarios_pck.obter_ds_justificativa(obter_justificativa_item(x.nr_prescricao, x.nr_sequencia, 'P')),1,2000),
		x.ie_suspenso,
		x.nr_seq_proc_cpoe
	order by
		dt_horario,
...
BEGIN
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		if (nr_horario_w IS NOT NULL AND nr_horario_w::text <> '') then
			...
			CALL exec_sql_dinamico_bv('ADEP', ds_comando_update_w,	'vl_hora=S' || to_char(nr_seq_horario_w) || 'H' || ie_status_horario_w || ds_sep_bv_w ||
										...
										'ie_status_w=' || ie_status_horario_w|| ds_sep_bv_w ||
	...
	close c01;
	CALL Atualizar_adep_controle(nm_usuario_p, nr_atendimento_p, 'CIG', 'N');
	END;
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE adep_gerar_horarios_pck.adep_sincronizar_cig ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_exibir_hor_realizados_p text, ie_exibir_hor_suspensos_p text, ie_regra_inclusao_p text, ie_data_lib_prescr_p text, ie_exibir_suspensos_p text, ie_agrupar_acm_sn_p text, ie_data_lib_proced_p text, ie_so_proc_setor_usuario_p text, dt_horario_p timestamp, nr_horario_p integer, ie_prescr_setor_p text, cd_setor_paciente_p bigint) FROM PUBLIC;
