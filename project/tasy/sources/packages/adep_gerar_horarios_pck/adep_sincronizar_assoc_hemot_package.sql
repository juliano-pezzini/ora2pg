...
CREATE OR REPLACE PROCEDURE adep_gerar_horarios_pck.adep_sincronizar_assoc_hemot ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_exibir_hor_realizados_p text, ie_exibir_hor_suspensos_p text, ie_regra_inclusao_p text, ie_data_lib_prescr_p text, ie_exibir_suspensos_p text, ie_agrupar_acm_sn_p text, ie_agrupar_dose_esp_p text, ie_obs_diferenciar_p text, dt_horario_p timestamp, nr_horario_p integer, ie_mostrar_substituido_p text, ie_prescr_setor_p text, cd_setor_paciente_p bigint, ie_agrupar_suspensos_p text, cd_item_p bigint, ie_quimio_p text, ie_palm_p text) AS $body$
DECLARE
	...
	SELECT	a.nr_prescricao,
			c.nr_seq_material,
			c.nr_sequencia,
			CASE WHEN 	coalesce(c.dt_primeira_checagem::text, '') = '' THEN 				CASE WHEN 	coalesce(c.dt_recusa::text, '') = '' THEN 					substr(obter_status_hor_medic(c.dt_fim_horario,c.dt_suspensao,c.ie_dose_especial,c.nr_seq_processo,c.nr_seq_area_prep),1,15)  ELSE substr(obter_status_recusa_pac(c.dt_fim_horario,c.dt_suspensao),1,15) END   ELSE 'Q' END ,
			c.cd_material,
			y.ds_material,
			...
			CASE WHEN get_ie_gravar_diluicao_sol='S' THEN  substr(adep_obter_inf_dil_obs(a.nr_prescricao,c.nr_seq_material),1,2000)  ELSE CASE WHEN substr(adep_obter_inf_dil_obs(a.nr_prescricao,c.nr_seq_material),1,2000) IS NULL THEN  null  ELSE ds_texto_dilu_w END  END  ds_dil_obs,
			x.nr_dia_util,
...
BEGIN
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		for i in 1..vt_ds_inf_w.count loop
			...
			if (vt_ds_inf_w[i].cd_material	= cd_material_w) and (vt_ds_inf_w[i].cd_intervalo	= cd_intervalo_w) and (vt_ds_inf_w[i].qt_dose		= qt_dose_w) and				
				(((ie_medicacao_paciente_w <> 'S') and (coalesce(ds_medic_nao_padr_w::text, '') = ''))  or (vt_ds_inf_w[i].ds_medic_nao_padr		= ds_medic_nao_padr_w) or (vt_ds_inf_w[i].ds_medic_nao_padrao		= ds_medic_nao_padrao_w)) and (coalesce(vt_ds_inf_w[i].ie_via_aplicacao,'XPTO') = coalesce(ie_via_aplicacao_w,'XPTO')) and (vt_ds_inf_w[i].cd_unidade_medida = cd_unidade_medida_dose_w) and (vt_ds_inf_w[i].ie_suspenso = ie_status_w) and (coalesce(vt_ds_inf_w[i].ie_obs_medica, 'XPTO') = coalesce(ie_obs_medica_w, 'XPTO')) and
	...
	close c01;	
	/* Atualizar_adep_controle(nm_usuario_p, nr_atendimento_p, 'M', 'N'); -*/
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE adep_gerar_horarios_pck.adep_sincronizar_assoc_hemot ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_exibir_hor_realizados_p text, ie_exibir_hor_suspensos_p text, ie_regra_inclusao_p text, ie_data_lib_prescr_p text, ie_exibir_suspensos_p text, ie_agrupar_acm_sn_p text, ie_agrupar_dose_esp_p text, ie_obs_diferenciar_p text, dt_horario_p timestamp, nr_horario_p integer, ie_mostrar_substituido_p text, ie_prescr_setor_p text, cd_setor_paciente_p bigint, ie_agrupar_suspensos_p text, cd_item_p bigint, ie_quimio_p text, ie_palm_p text) FROM PUBLIC;
