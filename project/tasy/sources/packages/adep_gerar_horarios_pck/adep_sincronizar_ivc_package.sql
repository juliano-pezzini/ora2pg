...
DECLARE
	...
	SELECT	a.nr_prescricao,
			...
			c.nr_sequencia,
			substr(CASE WHEN coalesce(c.dt_suspensao::text, '') = '' THEN coalesce(ie_status_ivc,'N')  ELSE 'S' END ,1,15),
			c.cd_procedimento,
			...
			substr(obter_mat_med_assoc_proc(a.nr_prescricao,c.nr_seq_procedimento,'S') || substr(adep_gerar_horarios_pck.obter_ds_justificativa(obter_justificativa_item(x.nr_prescricao, x.nr_sequencia, 'P')), 1, 1000),1,2000) ds_mat_med_assoc,
	...
	and	((ie_regra_inclusao_p = 'S') or
		 ((ie_regra_inclusao_p = 'R') and (adep_obter_regra_inclusao(	'IVC',
																		...
																		x.nr_seq_exame) = 'S')))
	and	(((Obter_se_acm_sn_agora_especial(x.ie_acm, x.ie_se_necessario, x.ie_urgencia, c.ie_dose_especial) = 'N') and (c.dt_horario between coalesce(dt_horario_inicio_sinc_w,dt_validade_limite_p) and coalesce(dt_horario_fim_sinc_w,dt_fim_w))) or
	...
	group by
		...
		x.cd_intervalo,
		x.qt_procedimento,
		substr(obter_mat_med_assoc_proc(a.nr_prescricao,c.nr_seq_procedimento,'S') || substr(adep_gerar_horarios_pck.obter_ds_justificativa(obter_justificativa_item(x.nr_prescricao, x.nr_sequencia, 'P')), 1, 1000),1,2000),
		x.ie_suspenso,
		x.nr_seq_proc_cpoe
	order by
		c.dt_horario,
...
BEGIN
	...
	close c01;
	CALL Atualizar_adep_controle(nm_usuario_p, nr_atendimento_p, 'IVC', 'N');
	END;
