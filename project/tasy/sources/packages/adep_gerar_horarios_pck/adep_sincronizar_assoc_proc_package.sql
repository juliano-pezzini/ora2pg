...
CREATE OR REPLACE PROCEDURE adep_gerar_horarios_pck.adep_sincronizar_assoc_proc ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_exibir_hor_realizados_p text, ie_exibir_hor_suspensos_p text, ie_regra_inclusao_p text, ie_data_lib_prescr_p text, ie_exibir_suspensos_p text, ie_agrupar_acm_sn_p text, ie_agrupar_dose_esp_p text, ie_obs_diferenciar_p text, dt_horario_p timestamp, nr_horario_p integer, ie_prescr_setor_p text, cd_setor_paciente_p bigint, ie_quimio_p text) AS $body$
DECLARE
    ...
    ie_horario_especial_w prescr_mat_hor.ie_horario_especial%type;
	c01 CURSOR FOR	
	SELECT	a.nr_prescricao,
			c.nr_seq_material,
			c.nr_sequencia,
			CASE WHEN 	coalesce(c.dt_recusa::text, '') = '' THEN 					CASE WHEN coalesce(c.dt_primeira_checagem::text, '') = '' THEN 					CASE WHEN get_ie_usa_status_gedipa='S' THEN  					substr(obter_status_hor_medic(c.dt_fim_horario,c.dt_suspensao,c.ie_dose_especial,c.nr_seq_processo,c.nr_seq_area_prep),1,15)  ELSE substr(obter_status_hor_medic(c.dt_fim_horario,c.dt_suspensao,c.ie_dose_especial,null,null),1,15) END   ELSE 'Q' END   ELSE substr(obter_status_recusa_pac(c.dt_fim_horario,c.dt_suspensao),1,15) END ,
			c.cd_material,
			z.ds_material,
...
union all
	...
	select	a.nr_prescricao,
		...
		c.dt_suspensao dt_suspensao,
		substr(coalesce(obter_se_item_dif_adep(a.nr_prescricao,c.nr_seq_material,x.nr_agrupamento,'M',ie_obs_diferenciar_p,x.ds_observacao),'N'),1,1) ie_diferenciado,
		substr(' Assoc. gasoterapia ('||substr(obter_desc_gas(y.nr_seq_gas),1,100)||')'||CASE WHEN coalesce(adep_obter_inf_dil_obs(a.nr_prescricao,c.nr_seq_material)::text, '') = '' THEN null  ELSE ' - ' END ||adep_obter_inf_dil_obs(a.nr_prescricao,c.nr_seq_material),1,2000) ds_dil_obs,
		x.nr_dia_util,
		c.dt_horario,
		x.nr_seq_mat_cpoe
	from	material z,
...
BEGIN
	...
	EXIT WHEN NOT FOUND; /* apply on c01 */
		...
		select  a.ie_horario_especial
        into STRICT    ie_horario_especial_w
        from    prescr_mat_hor a
        where   a.nr_sequencia = nr_seq_horario_w;
		if ((nr_horario_w IS NOT NULL AND nr_horario_w::text <> '') and coalesce(ie_horario_especial_w, 'N') = 'N') then
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE adep_gerar_horarios_pck.adep_sincronizar_assoc_proc ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_exibir_hor_realizados_p text, ie_exibir_hor_suspensos_p text, ie_regra_inclusao_p text, ie_data_lib_prescr_p text, ie_exibir_suspensos_p text, ie_agrupar_acm_sn_p text, ie_agrupar_dose_esp_p text, ie_obs_diferenciar_p text, dt_horario_p timestamp, nr_horario_p integer, ie_prescr_setor_p text, cd_setor_paciente_p bigint, ie_quimio_p text) FROM PUBLIC;
