...
CREATE OR REPLACE PROCEDURE adep_gerar_horarios_pck.adep_obter_horarios_coleta ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, ie_aba_p text, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_exibir_hor_realizados_p text, ie_exibir_hor_suspensos_p text, ie_regra_inclusao_p text, ie_data_lib_prescr_p text, ie_exibir_suspensos_p text, ie_data_lib_proced_p text, ie_so_proc_setor_usuario_p text, ie_prescr_setor_p text, cd_setor_paciente_p bigint ) AS $body$
DECLARE

	dt_horario_w		timestamp;
	dt_validade_inicio_w	timestamp;
	...
	and	coalesce(a.dt_validade_prescr,a.dt_inicio_prescr + 24/24) between dt_validade_inicio_w and dt_validade_fim_w
	...
	and	((ie_regra_inclusao_p = 'S') or
		 ((ie_regra_inclusao_p = 'R') and (adep_obter_inclusao_regra(	'LAB',
		 																a.nr_prescricao,
...
BEGIN
	dt_validade_inicio_w	:= dt_validade_limite_p;
	dt_validade_fim_w		:= current_setting('adep_gerar_horarios_pck.dt_fim_w')::timestamp;
	open c01;
	loop
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE adep_gerar_horarios_pck.adep_obter_horarios_coleta ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, ie_aba_p text, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_exibir_hor_realizados_p text, ie_exibir_hor_suspensos_p text, ie_regra_inclusao_p text, ie_data_lib_prescr_p text, ie_exibir_suspensos_p text, ie_data_lib_proced_p text, ie_so_proc_setor_usuario_p text, ie_prescr_setor_p text, cd_setor_paciente_p bigint ) FROM PUBLIC;
