...
CREATE OR REPLACE PROCEDURE adep_gerar_horarios_pck.adep_obter_supl_oral ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_exibir_hor_realizados_p text, ie_exibir_hor_suspensos_p text, ie_regra_inclusao_p text, ie_data_lib_prescr_p text, ie_lib_pend_rep_p text, ie_exibir_suspensos_p text, ie_agrupar_acm_sn_p text, ie_exibe_sem_lib_farm_p text, ie_prescr_setor_p text, cd_setor_paciente_p bigint, ie_palm_p text) AS $body$
DECLARE
	...
	SELECT	CASE WHEN coalesce(nr_seq_dieta_cpoe::text, '') = '' THEN  CASE WHEN ie_acm_sn='S' THEN  CASE WHEN obter_se_agrupar_acm_sn_adep(nr_prescricao,nr_seq_material,ie_agrupar_acm_sn_p)='N' THEN  nr_prescricao  ELSE null END   ELSE null END   ELSE null END ,
			...
			qt_dose,
			ds_prescricao,
			CASE WHEN ie_acm_sn='S' THEN  CASE WHEN obter_se_agrupar_acm_sn_adep(nr_prescricao,nr_seq_material,ie_agrupar_acm_sn_p)='N' THEN  CASE WHEN ie_suspenso='S' THEN  ie_suspenso  ELSE null END   ELSE null END   ELSE CASE WHEN ie_suspenso='S' THEN  ie_suspenso  ELSE null END  END  ie_status,
			ds_observacao,
			ie_oncologia,
			ie_regra_disp,
			nr_seq_dieta_cpoe,
...
WHERE y.cd_material = x.cd_material and x.nr_prescricao = c.nr_prescricao and x.nr_sequencia = c.nr_seq_material and x.nr_prescricao = a.nr_prescricao  and obter_se_exibir_rep_adep_setor(cd_setor_paciente_p,a.cd_setor_atendimento,coalesce(a.ie_adep,'S')) = 'S' and a.nr_atendimento = nr_atendimento_p and a.dt_validade_prescr between dt_validade_limite_p and dt_fim_w --and	((obter_se_prescr_lib_adep(a.dt_liberacao_medico, a.dt_liberacao, a.dt_liberacao_farmacia, ie_data_lib_prescr_p, a.ie_lib_farm) = 'S') or
  ...
  and ((ie_prescr_setor_p = 'N') or (ie_prescr_setor_p = 'S' AND a.cd_setor_atendimento = cd_setor_paciente_p)) and Obter_se_horario_liberado(c.dt_lib_horario, c.dt_horario) = 'S' and ((get_ie_vigente = 'N') or (clock_timestamp() between a.dt_inicio_prescr and a.dt_validade_prescr + get_minutos_vigente/1440)) group by
	...
	group by
		...
		qt_dose,
		ds_prescricao,
		CASE WHEN ie_acm_sn='S' THEN  CASE WHEN obter_se_agrupar_acm_sn_adep(nr_prescricao,nr_seq_material,ie_agrupar_acm_sn_p)='N' THEN  CASE WHEN ie_suspenso='S' THEN  ie_suspenso  ELSE null END   ELSE null END   ELSE CASE WHEN ie_suspenso='S' THEN  ie_suspenso  ELSE null END  END ,
		ds_observacao,
		ie_oncologia,
		ie_regra_disp,
		nr_seq_dieta_cpoe,
		cd_unidade_medida_dose;
	c02 CURSOR FOR
	SELECT	CASE WHEN coalesce(nr_seq_dieta_cpoe::text, '') = '' THEN  nr_prescricao  ELSE null END ,
			CASE WHEN coalesce(nr_seq_dieta_cpoe::text, '') = '' THEN  nr_seq_material  ELSE null END ,
			cd_material,
			ds_material,
			ie_acm_sn,	
			cd_intervalo,
	...
	from	(
		SELECT	a.nr_prescricao,
			...
			substr(x.ds_observacao || substr(adep_gerar_horarios_pck.obter_ds_justificativa(obter_justificativa_item(a.nr_prescricao, x.nr_sequencia, 'S')), 1, 1000),1,2000) ds_observacao,
			CASE WHEN ie_palm_p='S' THEN obter_se_setor_oncologia(a.cd_setor_atendimento)  ELSE null END  ie_oncologia,
			CASE WHEN x.ie_regra_disp='E' THEN 'E'  ELSE null END  ie_regra_disp,
			x.nr_seq_dieta_cpoe
		from	material y,
	...
	group by
		CASE WHEN coalesce(nr_seq_dieta_cpoe::text, '') = '' THEN  nr_prescricao  ELSE null END ,
		CASE WHEN coalesce(nr_seq_dieta_cpoe::text, '') = '' THEN  nr_seq_material  ELSE null END ,
		cd_material,
		ds_material,
		ie_acm_sn,	
...
SECURITY DEFINER
;
-- REVOKE ALL ON PROCEDURE adep_gerar_horarios_pck.adep_obter_supl_oral ( cd_estabelecimento_p bigint, cd_setor_usuario_p bigint, cd_perfil_p bigint, nm_usuario_p text, nr_atendimento_p bigint, dt_inicial_horarios_p timestamp, dt_final_horarios_p timestamp, dt_validade_limite_p timestamp, ie_exibir_hor_realizados_p text, ie_exibir_hor_suspensos_p text, ie_regra_inclusao_p text, ie_data_lib_prescr_p text, ie_lib_pend_rep_p text, ie_exibir_suspensos_p text, ie_agrupar_acm_sn_p text, ie_exibe_sem_lib_farm_p text, ie_prescr_setor_p text, cd_setor_paciente_p bigint, ie_palm_p text) FROM PUBLIC;
