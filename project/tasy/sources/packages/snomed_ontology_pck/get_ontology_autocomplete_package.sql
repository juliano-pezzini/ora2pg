...
CREATE OR REPLACE FUNCTION snomed_ontology_pck.get_ontology_autocomplete (ds_term_p text, rownum_p bigint) RETURNS SETOF ONTOLOGY_AUTOCOMPLETE_TABLE AS $body$
DECLARE

        ds_term_w	varchar(2000);

        row_w ontology_autocomplete;
...
SECURITY DEFINER
 STABLE;
-- REVOKE ALL ON FUNCTION snomed_ontology_pck.get_ontology_autocomplete (ds_term_p text, rownum_p bigint) FROM PUBLIC;
