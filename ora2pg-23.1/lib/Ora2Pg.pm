...
sub _get_sql_statements
{
    ...
    %{$self->{colinfo}} = $self->_column_attributes('', $self->{schema}, 'TABLE');
    $self->logit("Extracting column information...\n", 1);

		foreach my $table (@ordered_tables)
		{
    	$self->logit("Starting the export of table $table...\n", 1);
			# Do not process nested table
			if (!$self->{is_mysql} && exists $self->{tables}{$table}{table_info}{nested} && $self->{tables}{$table}{table_info}{nested} ne 'NO')
			{
				$self->logit("WARNING: nested table $table will not be exported.\n", 1);
    	...
    	$self->logit("Setting global count $global_count...\n", 1);
    	...
    	$self->logit("Finishing data exports...\n", 1);
  	...
  	$self->logit("Wainting all child die...\n", 1);
		...
		if ($self->{pg_dsn} && !$self->{oracle_speed}) {
    	$self->logit("Starting a new transaction...\n", 1);
		...
		foreach my $table (@ordered_tables)
		{
      	...
      	$self->logit("DATADIFF reduction...\n", 1);
			...
			if ($self->{disable_triggers} && !$self->{oracle_speed})
			{
      	$self->logit("Disable triggers of current table if requested...\n", 1);
				my $trig_type = 'USER';
				$trig_type = 'ALL' if (uc($self->{disable_triggers}) eq 'ALL');
